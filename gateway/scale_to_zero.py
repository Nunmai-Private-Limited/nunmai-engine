...
SCALE_TO_ZERO_ENV = "NUNMAI_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(nunmai_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under NUNMAI_HOME."""
    if nunmai_home is None:
        from nunmai_constants import get_nunmai_home
        nunmai_home = get_nunmai_home()
    return Path(nunmai_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
