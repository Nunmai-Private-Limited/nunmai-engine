...
ONLY the NUNMAI_HOME contextvar installed. Every gate fails OPEN: an unavailable store, a failing
...
def _profile_session_db_probe(profile_home: Path) -> Optional[Any]:
    ...
    from nunmai_cli.goals import _get_session_db
...
def profile_has_active_heartbeat(profile_home: Path) -> bool:
    from nunmai_cli.heartbeat import store_has_active_heartbeat

    return _gate(profile_home, store_has_active_heartbeat)


def profile_has_active_loop(profile_home: Path) -> bool:
    from nunmai_cli.loops import store_has_active_loop
