...
(`nunmai_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `nunmai_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, nunmai#85796 / gateway-gateway#210); violating any
...
(or `NUNMAI_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`/login` is registered in `nunmai_cli/commands.py` with `busy_policy="dispatch"` and
...
`nunmai serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
gateway (`gateway run`) SURVIVES the app: the serve backend's `/api/gateway/*` endpoints spawn it
detached (`_spawn_nunmai_action` — `start_new_session` / `DETACHED_PROCESS`), so `before-quit`'s
...
- **One launch-home identity.** "Does this task serve a routed profile?" compares the override
  with `nunmai_constants.get_routing_process_nunmai_home()` (`agent/secret_scope.py::
  serves_routed_profile` and `_is_process_home`, `tools/environments/local.py::_is_routed_home`,
  `nunmai_cli/env_loader.py::_process_nunmai_home`), never with `os.environ["NUNMAI_HOME"]` read
  live: an embedding host that mirrors the served profile into the env var per turn (Nunmai
  WebUI) pins its own home with `pin_process_nunmai_home()`, and without a pin the resolver is
  `get_process_nunmai_home()` unchanged. Do not add another routing decision that compares
  against `get_process_nunmai_home()` directly; that resolver is for process-level assets.
