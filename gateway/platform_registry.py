...
class PlatformEntry:
    ...
    # status displays (``nunmai setup``, ``nunmai status``, the dashboard
    ...
    # Env vars this platform needs (for ``nunmai setup`` display).
    ...
    # built-ins).  Used by ``nunmai gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved NUNMAI_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `nunmai`
        # invocation -- including plain `nunmai chat`, which never touches any
        ...
        # `nunmai setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``nunmai setup``/``gateway status``, channel
