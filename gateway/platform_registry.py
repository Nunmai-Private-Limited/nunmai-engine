...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``nunmai setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``nunmai gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # `nunmai` invocation, so the import happens only when a lookup asks for it.
