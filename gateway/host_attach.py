...
Multiplex-only (Teknium ruling): exactly one MULTIPLEXING ``nunmai gateway run`` per host, serving
...
def _record_home(record) -> Path:
    ...
    return Path(record.home) if getattr(record, "home", "") else Path(get_default_nunmai_root())
...
def _identity_matches(identity, record, home: Path) -> bool:
    ...
    if not isinstance(identity, dict) or identity.get("pid") != record.pid:
        return False
    reported = identity.get("nunmai_home")
    if not reported:
    ...
    try:
        ...
        return bool(_same_nunmai_home(Path(str(reported)), home))
    except Exception:
...
def attach_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"`nunmai -p {gateway.profile_label} gateway restart`.")
...
def _unknown_served_message(gateway: HostGateway, profile: str) -> str:
    return (
        ...
        f"   Take the host over (only from the home that launched it):  nunmai gateway run --replace\n"
        f"   Start anyway:  nunmai gateway run --force")
...
def _refuse_message(gateway: HostGateway, profile: str) -> str:
    ...
    return (
        ...
        f"   Or start one anyway:         nunmai gateway run --force\n"
