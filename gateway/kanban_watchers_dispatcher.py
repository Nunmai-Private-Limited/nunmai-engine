...
def _kbd():
    from nunmai_cli import kanban_db_dispatch
...
def _default_profile_secret_scope():
    ...
    LLM reads ``auxiliary.*`` from ``get_nunmai_home()``, so its credentials come
    ...
    token = set_secret_scope(
        build_profile_secret_scope(Path(get_nunmai_home())), profile_home=str(get_nunmai_home()))
    try:
        yield
    finally:
