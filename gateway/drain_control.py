...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{NUNMAI_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``NUNMAI_HOME`` is a **durable** store — on Nunmai
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``nunmai gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    ...
    base = home if home is not None else get_nunmai_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable NUNMAI_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (NUNMAI_HOME is a
    durable Fly volume on Nunmai Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    NUNMAI_HOME volume, or an expired same-epoch orphan (#85433), is
