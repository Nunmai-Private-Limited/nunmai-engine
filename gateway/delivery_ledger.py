...
def _db_path():
    # Launch home, not get_nunmai_home(): a multiplexed gateway records a served profile's replies
    # under that profile's home override, but the boot sweep reads from the launch context, so both
    # must open the one shared store (adapter_profile tells the bots apart). No get_nunmai_home()
    # fallback for an unset NUNMAI_HOME: a default gateway run in the foreground has none.
    return get_process_nunmai_home() / "state.db"
