...
verb (``nunmai_cli/profiles.py`` create/delete fire it through the control socket) and by the supervised
    ...
    async def _unserve_profile(self, name: str, home: "Path") -> None:
        ...
        with _profile_runtime_scope(Path(home), hydrate_secrets=False):
            ...
            with _log_suppressed(logging.DEBUG, "profile handle release failed", exc_info=True):
                from nunmai_state_registry import close_all_under
...
def _profile_lifecycle_verb(runner, *, serve: bool):
    ...
    async def apply(name):
        from nunmai_cli.profiles import profiles_to_serve, profile_is_parked
...
def _for_each_served_profile(runner, body) -> None:
    ...
    ``get_nunmai_home()`` and ``get_secret()`` see the LAUNCH profile's values, and under
    ``gateway.multiplex_profiles`` a fail-closed credential read logs ``no profile secret scope on a
    multiplexed call`` on every tick (the skills-sync pulls resolved Nunmai credentials this way, four
...
def migrate_profile_identity_verb(runner):
    """Build the ``migrate-profile-identity`` control-verb handler for ``nunmai profile rename``
...
def purge_profile_identity_verb(runner):
    """Build the ``purge-profile-identity`` control-verb handler for ``nunmai profile delete``
