...
Multiplex-only (Teknium ruling): exactly ONE ``nunmai serve`` and ONE ``nunmai gateway run``
per host, each multiplexing every profile. The per-``NUNMAI_HOME`` gateway lock/PID files
...
in the tree (``$NUNMAI_GATEWAY_LOCK_DIR`` else ``$XDG_STATE_HOME/nunmai/gateway-locks``),
which scopes to the **OS user**. That is the correct granularity: separate OS users have
separate ``$HOME``s, separate ``~/.nunmai`` profile roots, separate ports-by-convention and
...
**Relationship to ``spawn-ledger.json``** (``nunmai_cli/process_identity.py``): the ledger stays
the append-only machine roster of every long-lived Nunmai process (Desktop's attach ladder reads
...
#: never stands down behind it (#119824) — but ``nunmai plugins install`` from a terminal still
...
# The PATH is part of the key because the lock dir is env-derived (NUNMAI_GATEWAY_LOCK_DIR):
...
class HostRecord:
    ...
    profiles: tuple[str, ...]
    updated_at: str
    #: NUNMAI_HOME the owner was launched from. The attach channel (``gateway.control_socket``) is
...
def _pid_incarnation_matches(pid: int, create_time: Optional[float]) -> Optional[bool]:
    ...
    from nunmai_cli.process_identity import _pid_alive_matches
...
def probe_owner(record: HostRecord, *, timeout: float = PROBE_TIMEOUT_S) -> Optional[dict]:
    ...
    headers = {"X-Nunmai-Token": token, "Authorization": f"Bearer {token}"} if token else {}
...
def _write_private_text(path: Path, text: str) -> None:
    ...
    """
    if sys.platform == "win32":
        from nunmai_cli.windows_ssh_runtime import write_private_file
...
def publish_record(
    ...
    *,
    ...
    ``home`` is the launch NUNMAI_HOME — the key an attaching client needs to reach this owner's
    control socket.
    """
    role = _validated_role(role)
    record = HostRecord(
...
def _multiplex_profiles_enabled() -> bool:
    ...
    explicit ``false`` is RETIRED (``nunmai_cli.gateway_multiplex_mode``) — it is warned about and
