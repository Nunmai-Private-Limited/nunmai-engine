...
def restore_identity(
    ...
    *transport_profile* is what the routing index persisted at ingress (``SessionEntry.transport_profile``);
    ...
    authorization_home = (
        Path(get_process_nunmai_home()) if transport_name == primary_profile
...
def resolve_identity(
    ...
    *adapter* is the receiving adapter when the caller holds it; otherwise the source's own
    ...
    if transport_name == primary_profile:
        authorization_home = Path(primary_home) if primary_home is not None else Path(get_process_nunmai_home())
    else:
        from nunmai_cli.profiles import get_profile_dir
