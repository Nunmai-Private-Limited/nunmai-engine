...
from nunmai_cli.status_report import build_status_fields
...
HISTORY_UNREADABLE = ("⚠️ I can't read this conversation's history right now (your earlier messages "
                      "exist but cannot be loaded). Run `nunmai doctor --fix` on the host, or use /new "
                      "to start fresh.")
...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Nunmai balance and hand off to the portal. Does NOT charge, confirm,
        ...
        lines = ["💳 **Nunmai balance**"] + [ln for ln in view.balance_lines if not ln.lstrip().startswith("📈")]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # Nunmai credits + monthly-grant gauge (shared with CLI/TUI). Gates on "a Nunmai account is
        # logged in" — NOT the inference provider — so a Nunmai user inferring elsewhere still sees
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_nunmai_home()`` at call time, a
