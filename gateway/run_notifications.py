...
class GatewayNotificationsMixin:
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Nunmai update finished." if exit_code == 0
                        else "❌ Nunmai update failed (exit code {}).".format(exit_code)
                    )
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Nunmai update finished." if exit_code == 0 else "❌ Nunmai update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Nunmai update finished successfully." if exit_code == 0 else
                        "❌ Nunmai update failed. Check the gateway logs or run `nunmai update` manually for details."
                    )
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        from nunmai_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        ...
        if cause == "corrupt":
            ...
            message = (
                ...
                "recovery tools or restore a backup unless `nunmai doctor` confirms damage."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `nunmai doctor` for diagnostics."
            )
