...
# (`nunmai update` re-runs it in the terminal) and only a short tail is quoted in chat.
_UPDATE_FAILED_NOTICE = (
    "❌ Nunmai update failed; the previous version is still running. Run `nunmai update` on the "
...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``nunmai update --gateway`` and its watcher exchange under NUNMAI_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_nunmai_home / ".update_pending.json",
            claimed=_nunmai_home / ".update_pending.claimed.json", output=_nunmai_home / ".update_output.txt",
            exit_code=_nunmai_home / ".update_exit_code",
            prompt=_nunmai_home / ".update_prompt.json", response=_nunmai_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``nunmai update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Nunmai update finished." if exit_code == 0 else _UPDATE_FAILED_NOTICE
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Nunmai update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            if chat_id:
                ...
                if exit_code == 0:
                    msg = "✅ Nunmai update finished successfully."
                    if output:
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.delivery import resolve_delivery_transport
        from gateway.run import _nunmai_home, _non_conversational_metadata
        notify_path = _nunmai_home / ".restart_notify.json"
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        """Extra startup line when the gateway's inference is carried by the Nunmai free tier; None otherwise.
        ...
        try:
            ...
            from nunmai_cli.anon_auth import guest_carries_inference
            ...
            return None
        return "Inference: Nunmai free tier (nous/welcome). Sign in for more: /login"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Nunmai is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from nunmai_constants import get_default_nunmai_root, profile_cli_selector
        from nunmai_state import _default_db_path, classify_persistence_error
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `nunmai` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_nunmai_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `nunmai {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   nunmai {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) nunmai {profile_arg}sessions recover "
                ...
                f"Run `nunmai {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `nunmai {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                f"recovery tools or restore a backup unless `nunmai {profile_arg}doctor` confirms damage."
            )
        else:
            ...
            if failure.cause not in _SELF_CLEARING_STORAGE_CAUSES:
                action = f"{action} Then `nunmai {profile_arg}gateway restart`."
            message = (
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Nunmai-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_nunmai_home()/state.db``) resolve from the ambient scope.
        ...
        if get_nunmai_home_override() == str(profile_home):
