...
from nunmai_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while Nunmai is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while Nunmai is busy."),
}
...
def _spawn_detached_update(nunmai_cmd, output_path, exit_code_path) -> None:
    """Spawn ``nunmai update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\nunmai.exe — that shim holds its own file open, and the
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "nunmai_cli.main", "update", "--gateway"],
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach Nunmai commands (adapter ``typed_command_prefix``,
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `nunmai gateway restart` to reset."
        if not queued:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _nunmai_home / name, build(), indent=None)
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `nunmai skills diff <name>`, which diffs a bundled skill against its stock version).
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Nunmai is working."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_nunmai_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``nunmai debug share`` from the CLI does full uploads."""
        from nunmai_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_nunmai_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``nunmai update`` detached (``setsid``) so it survives the gateway
        ...
        try:
            _spawn_detached_update(nunmai_cmd, output_path, exit_code_path)
        except Exception as e:
