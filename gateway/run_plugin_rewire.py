...
``nunmai plugins install/enable`` / the Desktop / ``plugins.manage`` nudging the gateway over the control
...
def reload_plugins_verb(runner: Any, loop: asyncio.AbstractEventLoop) -> Callable[..., dict]:
    ...
    def _handler(params: Optional[dict] = None) -> dict:
        ...
        from gateway.run import _profile_runtime_scope
        params = params or {}
        gateway_home = Path(get_nunmai_home())
        requested = Path(str(params.get("home") or gateway_home)).expanduser()
        req_key = nunmai_home_key(requested)
        profile_name: Optional[str] = None
        if req_key != nunmai_home_key(gateway_home):
            served = (getattr(runner, "_served_profile_homes", None) or {}).items()
            profile_name = next((str(n) for n, h in served if nunmai_home_key(h) == req_key), None)
            if profile_name is None:
