...
from nunmai_cli.config import atomic_config_write, clear_model_endpoint_credentials
...
async def _persist_model_switch_to_config(result, config_path) -> None:
    ...
    """
    from nunmai_cli.config import read_user_config_raw, save_config
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from nunmai_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from nunmai_cli.model_switch import format_model_for_display, resolve_display_context_length_async
        ...
        # Provider-aware chain: Codex OAuth, Copilot and Nunmai caps win over the raw models.dev entry.
    ...
    async def _send_model_picker(self, event: MessageEvent, source, adapter, session_key: str, listing_kwargs: dict, on_model_selected) -> bool:
        ...
        from nunmai_cli.model_switch_providers import list_picker_providers
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (nunmai_state.py:1994-2009)
            ...
            source=source,
            config_path=(profile_home or _nunmai_home) / "config.yaml",
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (nunmai_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
