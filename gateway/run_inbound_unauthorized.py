...
def pairing_code_reply(platform_name: str, code: str, profile_arg: str = "") -> str:
    ...
    approve_cmd = f"nunmai {profile_arg}pairing approve {platform_name} {code}"
    return (
...
def unauthorized_owner_hint(
    platform_name: str, user_id: str, user_name: str = "", *, nunmai_home: str,
) -> str:
    ...
    allowlist = (
        f"add the ID to {env_var} in {nunmai_home}/.env and restart the gateway"
    ...
    return (
        ...
        f"{nunmai_home}/config.yaml so unknown senders receive a pairing code you can approve with "
        f"`nunmai pairing approve {platform_name} <code>`."
    )
