...
``<NUNMAI_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _mark_exited_quietly(exit_code: int, reason: str) -> None:
    ...
    ``gateway_state.json`` (so ``nunmai gateway status`` and every other reader of that file stop
...
def _process_nunmai_home() -> Path:
    """NUNMAI_HOME for process-level identity files (ignore profile overrides)."""
    return get_process_nunmai_home() if os.environ.get("NUNMAI_HOME", "").strip() else get_nunmai_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_nunmai_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<NUNMAI_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
