...
2. An event-loop heartbeat file at ``<NUNMAI_HOME>/state/gateway.heartbeat`` so
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<NUNMAI_HOME>/state/gateway.heartbeat``."""
...
def get_loop_tick_socket_path(
    ...
    ``<NUNMAI_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``nunmai_cli.gateway.probe_gateway_loop_liveness`` for the
