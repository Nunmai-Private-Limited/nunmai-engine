...
``gateway/assets/status_phrases.yaml``; users add profile-relative catalogs under ``NUNMAI_HOME``
via ``status_phrases.yaml`` / ``status_phrases/*.yaml`` or ``display.status_phrases: {path:
<NUNMAI_HOME-relative>, mode: append|replace}``. Absolute paths and ``..`` escapes are ignored on
...
# Nunmai UI surfaces, not app/vendor buckets.  Long-running-only: regular tool/thinking/interim
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None,
    ...
    for section in sections:
        for key in _CONFIG_KEYS:
            _merge_phrase_config(catalog, section.get(key), base_dir=nunmai_home)
    return catalog
