...
* Stateless request/response adapters (the API server,
  ...
  ``X-Nunmai-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Nunmai-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Nunmai-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Nunmai-Session-Id`` is 403-gated on
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "nunmai-engine"),
