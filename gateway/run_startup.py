...
class GatewayStartupMixin:
    ...
    def _log_agent_budget() -> None:
        ...
        ``NUNMAI_MAX_ITERATIONS`` before this runs, so the env slot already carries the config value;
        ...
        from nunmai_cli.config import TURN_LIMIT_UNLIMITED, resolve_turn_limit
        limit = resolve_turn_limit(os.getenv("NUNMAI_MAX_ITERATIONS"))
        logger.info("Agent budget: max_iterations=%s (agent.max_turns from config.yaml, else the NUNMAI_MAX_ITERATIONS bridge)",
    ...
    async def _drain_startup_restore_queue(self) -> int:
        ...
        while queue:
            ...
            try:
                ...
                with suppress(Exception):
                    setattr(event, "_nunmai_startup_restore_replay", True)
                await adapter.handle_message(event)
            except Exception:
    ...
    async def _run_boot_probe_in_launch_scope(self, fn):
        ...
        if getattr(self.config, "multiplex_profiles", False):
            ...
            try:
                async with _async_profile_runtime_scope(get_nunmai_home()):
    ...
    async def _recover_unclean_sessions(self) -> tuple[int, int]:
        ...
        resumed = ledgered = 0
        max_age = max(60 * 60, int(max(1.0, _float_env("NUNMAI_AGENT_TIMEOUT", 1800)) * 2))
    ...
    def _crash_left_reply(self, history: list, started: float, origin) -> Optional[str]:
        ...
        from nunmai_cli.timefmt import coerce_epoch
    ...
    def _start_loop_heartbeat_task(self) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "Failed to start gateway loop heartbeat", exc_info=True):
            ...
            task._nunmai_supervised_watcher = True  # type: ignore[attr-defined]
    ...
    def _open_faulthandler_log(self):
        ...
        from gateway.run import get_nunmai_home
        log_dir = getattr(self.config, "log_dir", None) or os.path.join(str(get_nunmai_home()), "logs")
    ...
    async def _start_log_startup_environment(self) -> None:
        ...
        with suppress(Exception):
            ...
            if _redact_raw.lower() in {"1", "true", "yes", "on"}:
                ...
                logger.warning(
                    "Secret redaction: DISABLED (NUNMAI_REDACT_SECRETS=%s). API keys and tokens may appear "
        ...
        with _log_suppressed(logging.DEBUG, "security advisory check failed at gateway startup", exc_info=True):
            from nunmai_cli.security_advisories import detect_compromised, gateway_log_message
            _adv_msg = gateway_log_message(detect_compromised())
            if _adv_msg:
                logger.warning("%s", _adv_msg)
                logger.warning("Run `nunmai doctor` on the gateway host for full remediation steps.")
        ...
        with _log_suppressed(logging.DEBUG, "check_systemd_timing_alignment failed: %s"):
            ...
            if _alignment is not None and _alignment.get("mismatch"):
                logger.warning(
                    ...
                    "mid-drain. Run `nunmai gateway install --force` to regenerate the unit, or shorten "
    ...
    def _register_launch_profile_config_hooks() -> None:
        ...
        with _profile_runtime_scope(get_process_nunmai_home()):
            GatewayStartupMixin._register_config_hooks(
                "shell-hook/webhook registration failed at gateway startup", level=logging.WARNING)
    ...
    def _register_config_hooks(fail_fmt: str, *fail_args, level: int = logging.DEBUG) -> None:
        ...
        Gateway has no TTY, so consent must come from --accept-hooks, NUNMAI_ACCEPT_HOOKS, or
    ...
    def _recover_secondary_process_checkpoints(self, process_registry) -> int:
        ...
        from nunmai_constants import get_nunmai_home
        launch_home = get_nunmai_home().resolve()
        recovered = 0
    ...
    async def _start_recover_previous_run(self) -> None:
        ...
        _clean_marker = _nunmai_home / ".clean_shutdown"
        ...
        if connected_count != 0:
            if startup_nonretryable_errors:
                ...
                logger.error(
                    ...
                    "then `nunmai gateway restart`): %s. The gateway is DEGRADED — it serves the "
    ...
    async def _start_impl(self) -> bool:
        logger.info("Starting Nunmai Gateway...")
        ...
        # The gateway is a boot owner of the Nunmai free tier, beside `cmd_chat` and `nunmai serve`: every
    ...
    async def _handoff_resolve_destination(
        ...
        try:
            new_thread_id = await transport.adapter.create_handoff_thread(
                home_chat_id, f"Nunmai — {cli_title}",
            )
        except Exception as exc:
