...
from nunmai_constants import get_nunmai_home, get_nunmai_home_override
...
_AGENT_ENV_BRIDGE = {
    ...
    "reconnect_attention_after": "NUNMAI_RECONNECT_ATTENTION_AFTER_SECONDS",
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from nunmai_cli.auth import AuthError, is_rate_limited_auth_error
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    Defaults to the active gateway home (``_nunmai_home`` monkeypatches apply); multiplexers pass a path.
    """
    ...
    try:
        from nunmai_cli.config import get_config_path, read_raw_config
...
def _resolve_nunmai_bin() -> Optional[list[str]]:
    """Nunmai update command argv: ``nunmai`` on PATH, else ``python -m nunmai_cli.main``, else None."""
    import shutil
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("NUNMAI_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            if _ckpt_cfg.get("auto_prune", False):
                ...
                # unmounted share); orphan cleanup is only via explicit `nunmai checkpoints prune`.
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="nunmai-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's NUNMAI_HOME serves this source: ``source.profile``, then
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    ...
    if not replace:
        ...
        logger.error(
            "Another gateway instance is already running (PID %d, NUNMAI_HOME=%s). "
            "Use 'nunmai gateway restart' to replace it, or 'nunmai gateway stop' first.",
            existing_pid, nunmai_home)
        print(
            f"\n❌ Gateway already running (PID {existing_pid}).\n"
            f"   Use 'nunmai gateway restart' to replace it,\n"
            f"   or 'nunmai gateway stop' to kill it first.\n"
            f"   Or use 'nunmai gateway run --replace' to auto-replace.\n")
        return False
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to NUNMAI_HOME; distinct-home multi-profile setups coexist.
