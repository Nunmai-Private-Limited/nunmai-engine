...
# nunmai_bootstrap must be the very first import (UTF-8 stdio on Windows; no-op on POSIX).
try:
    import nunmai_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # a partial ``nunmai update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
from nunmai_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
from nunmai_cli.fallback_config import pre_agent_fallback_notice
...
def _hygiene_compression_timeout_message(
    *, total_exhausted: bool, elapsed: float, idle_timeout: float, progress_observed: bool) -> str:
    ...
    model config, so the copy names /compress, /new and `nunmai doctor`, never a config key or the
    ...
    if total_exhausted:
        return lead
    return lead + " If this keeps happening, run `nunmai doctor` on the host."
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while Nunmai still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "nunmai", "off"}:
        mode = "native"
    if mode != "nunmai":
    ...
    # copy_context carries profile secret scope / NUNMAI_HOME override (executors don't propagate ContextVars).
...
def _ensure_windows_gateway_venv_imports() -> None:
    """Make detached Windows gateway runs see the Nunmai venv packages.
...
_PROVIDER_ERROR_REPLIES = (
    ...
    (_GATEWAY_AUTH_ERROR_RE, "⚠️ Sign-in to the AI model service failed. Use /login to sign in again, "
                             "or ask whoever runs this bot to run `nunmai doctor` on the host."),
    ...
    (_GATEWAY_ENDPOINT_UNREACHABLE_RE, "⚠️ The AI model service isn't reachable right now — the configured model "
                                       ...
                                       "if it persists, run `nunmai doctor` on the host."),
    (_GATEWAY_CONNECTION_ERROR_RE, "⚠️ Nunmai could not reach the AI model service (no further detail from the "
                                   "SDK). Use /retry to try again; if it persists, run `nunmai doctor` on the host."))
...
def _gateway_provider_error_reply(text: str) -> str:
    ...
    return (
        ...
        "models. Details are in the gateway log (`nunmai logs`).")
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from nunmai_cli.commands_platforms import _sanitize_telegram_name
...
def _coerce_gateway_timestamp(value: Any) -> Optional[float]:
    ...
    if isinstance(value, (int, float)):
        # Some platform events use milliseconds; Nunmai state rows use seconds.
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("NUNMAI_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("NUNMAI_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from nunmai_time import get_timezone as _get_msg_tz
...
def _restart_notification_pending() -> bool:
    ...
    return (_nunmai_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _nunmai_home / ".restart_pending.json"
...
os.environ["_NUNMAI_GATEWAY"] = "1"

_ensure_ssl_certs()

sys.path.insert(0, str(Path(__file__).parent.parent))

from nunmai_constants import get_nunmai_home, get_nunmai_home_override, get_process_nunmai_home
# The PROCESS's own home, never an import-time ContextVar: a multiplexed backend (``nunmai serve``)
...
_nunmai_home = get_process_nunmai_home()

# Load ~/.nunmai/.env first: user-managed env files must override stale shell exports on restart.
from nunmai_cli.env_loader import load_nunmai_dotenv
_env_path = _nunmai_home / '.env'
load_nunmai_dotenv(nunmai_home=_nunmai_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env per turn for rotated keys while config.yaml stays authoritative for budgets (else a
    stale NUNMAI_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_nunmai_dotenv(
            nunmai_home=_nunmai_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_nunmai_home)
...
def _bridge_max_turns_from_config(home: "Path") -> None:
    ...
    nunmai_state reads the routed profile's ``sessions.*`` from its own config under scope."""
...
def _current_max_iterations() -> int:
    ...
    ``int()`` crash. A routed profile (NUNMAI_HOME override, multiplexed turns) reads ITS
    ``agent.max_turns`` straight from config: the ``NUNMAI_MAX_ITERATIONS`` bridge is one process-wide
    ...
    return _resolve_turn_limit(os.getenv("NUNMAI_MAX_ITERATIONS"))
...
def _cron_tick_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    <name>`` gateway's own profile may sit outside ``profiles/`` (custom NUNMAI_HOME). One host
    process ticks all of them regardless of ``gateway.multiplex_profiles``. Adapter startup
    already skips ``active``."""
    from nunmai_cli.profiles import get_active_profile_name, get_profile_dir
...
def _cron_profile_gate(name: str, home: "Path") -> bool:
    ...
    Same stand-down the serve/Desktop ticker applies (``nunmai_cli/web_server.py``): a host that
    has not finished converging onto the one host gateway (``nunmai gateway migrate --multiplex``)
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from nunmai_cli.env_loader import hydrate_profile_secret_sources

    home_token = set_nunmai_home_override(str(profile_home))
    try:
...
def _profile_runtime_scope(
    ...
    ``set_nunmai_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
def load_gateway_config_for_runner() -> "GatewayConfig":
    ...
    See #64674.
    """
    from nunmai_cli.gateway_multiplex_mode import log_multiplex_decision, resolve_multiplex_mode
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_nunmai_home()``'s config, so an unscoped call only connects the launch
    ...
    actionable ``nunmai mcp login`` warning instead of opening an authorize tab.
    """
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.nunmai/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from nunmai_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["NUNMAI_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "NUNMAI_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "NUNMAI_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "NUNMAI_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "NUNMAI_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "NUNMAI_SESSION_STALL_TIMEOUT",
    "restart_drain_timeout": "NUNMAI_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "NUNMAI_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "NUNMAI_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "NUNMAI_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "NUNMAI_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "NUNMAI_CJK_FTS", "search_slow_ms": "NUNMAI_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "NUNMAI_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "NUNMAI_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "NUNMAI_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["NUNMAI_MAX_ITERATIONS"] = str(raw)
    elif "NUNMAI_MAX_ITERATIONS" in os.environ:
        del os.environ["NUNMAI_MAX_ITERATIONS"]
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. NUNMAI_MAX_ITERATIONS=60 written by an old `nunmai setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "NUNMAI_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["NUNMAI_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["NUNMAI_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["NUNMAI_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`nunmai cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["NUNMAI_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
def _load_bridge_config(config_path: Path) -> dict:
    ...
    from nunmai_cli.config_effective import load_user_config_effective
    return load_user_config_effective(config_path)


_config_path = _nunmai_home / 'config.yaml'
_cfg: dict = {}
if _config_path.exists():
    try:
        ...
        print(
            ...
            "your current config.yaml. Run `nunmai doctor` to investigate.",
            file=sys.stderr)

# IPv4 preference must apply before any HTTP clients are created.
try:
    from nunmai_constants import apply_ipv4_preference
...
try:
    from nunmai_cli.config import print_config_warnings
    print_config_warnings()
except Exception as _bootstrap_exc:
    print(f"  Warning: config validation failed: {_bootstrap_exc}", file=sys.stderr)

try:
    from nunmai_cli.config import warn_deprecated_cwd_env_vars
    warn_deprecated_cwd_env_vars()
except Exception as _bootstrap_exc:
    print(f"  Warning: deprecation check failed: {_bootstrap_exc}", file=sys.stderr)

os.environ["NUNMAI_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# NUNMAI_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_gateway_model_context(
    ...
    def _pin_still_applies() -> bool:
        ...
        from nunmai_cli.route_identity import should_clear_context_pin
    ...
    def _custom_ctx() -> Optional[int]:
        from nunmai_cli.config import get_custom_provider_context_length
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from nunmai_cli.config import _deep_merge
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `nunmai skills config`")

        # Check optional skills (shipped with repo but not installed)
        from nunmai_constants import get_optional_skills_dir
        ...
        if optional_dir.exists():
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                return (
                    ...
                    f"Install it with: `nunmai skills install {install_path}`")
    except Exception:
        pass
    return None
...
def _gateway_config_home() -> Path:
    """Return the Nunmai home that gateway config reads should use."""
    override = get_nunmai_home_override()
    return Path(override) if override else _nunmai_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    (``_nunmai_home`` monkeypatches apply); multiplexers pass a path.
    """
...
def _resolve_nunmai_bin() -> Optional[list[str]]:
    """Nunmai update/restart argv: the running interpreter's ``python -m nunmai_cli.main``
    (exactly this install), else ``nunmai`` on PATH, else None. The module argv must win: a
    PATH-first lookup lets an attacker-planted ``nunmai`` shadow the running install when
    /update or /restart re-execs it (#111569)."""
    try:
        import importlib.util
        if importlib.util.find_spec("nunmai_cli") is not None:
            return [sys.executable, "-m", "nunmai_cli.main"]
    except Exception:
        pass
    import shutil
    nunmai_bin = shutil.which("nunmai")
    if nunmai_bin:
        return [nunmai_bin]
    return None
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall nunmai-engine"),
...
class GatewayRunner(
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        if config is not None:
            from nunmai_cli.gateway_multiplex_mode import standalone_launcher_decision, log_multiplex_decision
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS NUNMAI_HOME silently loses /resume etc.
        ...
        # restart of the day (the ``nunmai update`` one) looked hung and failed fleet verification.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local NUNMAI_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local NUNMAI_HOME override
        ...
        #88235 broadcast.
        """
        from nunmai_state import AsyncSessionDB, _default_db_path
        ...
        def _open():
            ...
            if borrowed is not None:
                ...
                wrapper.__dict__["_nunmai_borrowed_handle"] = True
                return wrapper
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        """
        def _close(db) -> None:
            if getattr(db, "__dict__", {}).get("_nunmai_borrowed_handle"):
                return
            ...
            # instead (#90837).
            from nunmai_state_registry import release_or_close
            try:
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.nunmai/cache/documents:/output') is configured. "
            "This is fine if the model already emits host-visible paths, but MEDIA file delivery can fail "
            "for container-local paths like '/workspace/...' or '/output/...'.")

    _VOICE_MODE_PATH = _nunmai_home / "gateway_voice_mode.json"
    ...
    def _update_runtime_status(self, gateway_state: Optional[str] = None, exit_reason: Optional[str] = None) -> None:
        # ``active_work`` names each unit only while draining — that is when an observer (``nunmai
    ...
    def _thread_metadata_for_source(
        ...
        if profile and metadata is not None:
            ...
            metadata["nunmai_profile"] = profile
        return metadata
    ...
    def _get_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        return GatewayRunner._get_or_create_pool(self, "_executor", _TURN_MAX_WORKERS, "nunmai-gateway")
    ...
    def _get_housekeeping_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        return GatewayRunner._get_or_create_pool(self, "_housekeeping_executor", _HOUSEKEEPING_MAX_WORKERS, "nunmai-gateway-hk")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's NUNMAI_HOME serves this source: the pinned identity's runtime
        home, else ``source.profile``, then ``_profile_name_for_source`` (sources bypassing
        ``build_source``), then the active profile."""
        from gateway.profile_routing import ProfileRouteRejected
        from gateway.session_identity import identity_of
        from nunmai_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            logger.warning(
                ...
                "falling back to global NUNMAI_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``nunmai gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``nunmai gateway stop`` signals the gateway. The
...
def _housekeeping_paste_sweep() -> None:
    from nunmai_cli.debug import _sweep_expired_pastes
...
def _launch_sessions_dir(config) -> Optional[Tuple[Path, Path]]:
    ...
    MUST be called outside any profile scope — ``get_nunmai_home()`` is what identifies the launch
    home. Consumed by :func:`_profile_sessions_dir`.
    """
    sessions_dir = getattr(config, "sessions_dir", None)
    if sessions_dir is None:
        return None
    return get_nunmai_home(), Path(sessions_dir)
...
def _housekeeping_state_db_maintenance(launch: Optional[Tuple[Path, Path]] = None) -> None:
    ...
    Profile-scoped by its caller: ``acquire()``, ``get_nunmai_home()`` and ``load_config()`` all
    ...
    from nunmai_cli.config import load_config as _load_full_config
    from nunmai_state_registry import acquire, release_or_close
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from nunmai_state_registry import borrow_live_shared_session_dbs
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_nunmai_home()):
...
def _start_gateway_housekeeping(
    ...
    chores: list[tuple[int, str, Any]] = [
        ...
        # ``nunmai gateway status`` / ``/api/status`` warn when it ages past 2x ``interval`` with the
    ...
    chores += [
        # Per served profile: each profile has its own skills tree, curator state, Nunmai login
        # and state.db.
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no NUNMAI_HOME), it is only a
    ...
    # home launched it — `nunmai -p X gateway run --replace` means "replace the process serving X".
    # Argv and NUNMAI_HOME can never prove that (the host singleton runs one home's argv while
    ...
    try:
        ...
        if owner is not None and owner.pid == existing_pid and owner.serves(
                profile_name_for_home(get_nunmai_home())):
            logger.warning(
    ...
    # (no service supervisor — e.g. `nunmai gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_nunmai_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_nunmai_home)
        our_home = _get_process_nunmai_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("nunmai_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates nunmai_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_nunmai_home(recorded_home, our_home):
            return refuse("pid record belongs to a different NUNMAI_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / NUNMAI_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "NUNMAI_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """NUNMAI_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "NUNMAI_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--nunmai-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this NUNMAI_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        ...
        logger.error(
            "Another gateway instance is already running (PID %d, NUNMAI_HOME=%s) and did not "
            "publish a host record this process could attach to.",
            existing_pid, nunmai_home)
        print(
            ...
            f"   Take the host over:  nunmai gateway run --replace\n"
            f"   Or stop it first:    nunmai gateway stop\n")
        return False
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_nunmai_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (NUNMAI_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_nunmai_home())
        return False
    ...
    with suppress(Exception):
        (get_nunmai_home() / "gateway.pid").unlink(missing_ok=True)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from nunmai_logging import setup_logging, _safe_stderr
    setup_logging(nunmai_home=_nunmai_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from nunmai_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from nunmai_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(nunmai_home=_nunmai_home, config=_best_effort(_raw_cfg))
...
def _start_gateway_make_restart_signal_handler(runner):
    ...
    def restart_signal_handler():
        ...
        logger.info(
            "SIGUSR1 received (systemctl reload / nunmai gateway restart): performing a graceful "
...
def _start_gateway_make_shutdown_signal_handler(runner, _signal_initiated_shutdown: list):
    ...
    def shutdown_signal_handler(received_signal=None):
        ...
        # `nunmai gateway stop` writes the marker, THEN signals: the planned-stop watcher can consume
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _nunmai_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
def _claim_host_gateway_role(force: bool = False) -> None:
    ...
    if not force:
        ...
        decision = standalone_attach_decision(get_nunmai_home(), None)
    ...
    try:
        ...
        if outcome is hr.HostLockOutcome.ACQUIRED:
            ...
            hr.publish_record(hr.ROLE_GATEWAY, profiles=(), home=str(get_nunmai_home()))
    ...
    if profile_is_standalone(get_nunmai_home()):
        ...
        if live_owner is not None:
            decision = standalone_attach_decision(get_nunmai_home(), live_owner)
            if decision is not None:
...
def _migrate_command() -> str:
    from nunmai_cli.gateway_migrate import MIGRATE_COMMAND
...
def _owner_is_standalone() -> bool:
    ...
    try:
        ...
        if owner is None or owner.pid == os.getpid():
            return False
        answered = request_serve_profile(profile_name_for_home(get_nunmai_home()), owner=owner)
...
def _refuse_second_host_gateway(owner) -> None:
    ...
    message = (
        ...
        f"   Or start one anyway (skips the host-lock check):  nunmai gateway run --force\n"
...
def _log_standalone_profiles_at_boot(runner) -> None:
    ...
    try:
        if not getattr(runner.config, "multiplex_profiles", False):
            return
        from nunmai_cli.profiles import profiles_to_serve, profile_is_standalone
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its NUNMAI_HOME, so from here on "does a socket answer?" is
        ...
        def _rescan_profiles_handler() -> dict:
            """``nunmai profile create/delete`` asks the multiplexer to reconcile ``profiles/`` now
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`nunmai gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from nunmai_cli.nous_auth_keepalive import stop_nous_auth_keepalive
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False,
    ...
    # Duplicate-instance guard scoped to NUNMAI_HOME (the host record is absent or unusable here).
    ...
    # Windows has no add_signal_handler, so `nunmai gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `nunmai gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `nunmai
    ...
    _best_effort(_start_keepalive, "Nunmai auth keepalive did not start: %s")
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``NUNMAI_IGNORE_USER_CONFIG=1``) as ``nunmai_cli/main.py``."""
    from nunmai_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in nunmai_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``nunmai-engine`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from nunmai_startup_watchdog import arm_startup_watchdog
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Nunmai Gateway - Multi-platform messaging")
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from nunmai_logging import drain_log_queue
