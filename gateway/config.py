...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``nunmai config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    # ``None``: a request the gateway settles at boot, not a verdict. ``nunmai_cli.gateway_multiplex_mode
    ...
    # route); NUNMAI_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_nunmai_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.nunmai/config.yaml > legacy gateway.json > defaults."""
