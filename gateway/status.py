"""Gateway runtime status helpers: PID/lock/marker files under ``{NUNMAI_HOME}`` (one set per
...
from nunmai_constants import _get_platform_default_nunmai_home, get_nunmai_home, get_process_nunmai_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_nunmai_home() / "gateway-starts.log"
...
def _get_process_nunmai_home() -> Path:
    """Launch-home NUNMAI_HOME for identity files (PID, lock, status, markers):
    ``get_nunmai_home()`` honors the per-session ``_NUNMAI_HOME_OVERRIDE`` and would misroute
    them."""
    return get_process_nunmai_home()


def _canonical_nunmai_home(path: Path | str) -> Path:
    """Stable absolute NUNMAI_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_nunmai_home(left: Path | str, right: Path | str) -> bool:
    """Compare NUNMAI_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_nunmai_home(left)))
    return left_c == os.path.normcase(str(_canonical_nunmai_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT NUNMAI_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``nunmai_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("nunmai_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_nunmai_home()
        return not _same_nunmai_home(recorded_home, base)
    except Exception:
        return True


# Mirrors nunmai_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (nunmai_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_nunmai_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import nunmai_constants
    default_homes = (nunmai_constants.get_default_nunmai_root, _get_platform_default_nunmai_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_nunmai_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_nunmai_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Cross-profile rendezvous dir for machine-local locks; ``NUNMAI_GATEWAY_LOCK_DIR`` overrides.

    Scope is the **OS user**, not the kernel host: separate users have separate ``$HOME``s,
    separate ``~/.nunmai`` profile roots and separate credentials, so "one gateway per host"
    ...
    ``gateway.pid``/``gateway.lock`` above deliberately stay under each profile's NUNMAI_HOME.
    """
    override = os.getenv("NUNMAI_GATEWAY_LOCK_DIR")
    if override:
    ...
    return state_home / "nunmai" / _LOCKS_DIRNAME
...
def retained_gateway_state(runtime: Any) -> str:
    ...
    ``nunmai gateway stop`` keeps the last ``startup_failed`` + ``exit_reason`` on disk for
    ...
    under the same rule, so the dashboard agrees with ``nunmai gateway status``. Any other retained
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Nunmai gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    Nunmai entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("nunmai-gateway", "nunmai-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "nunmai_cli.main" not in joined and "nunmai_cli/main.py" not in joined and not any(
        b in ("nunmai", "nunmai.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `nunmai gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    Nunmai-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the Nunmai gateway."""
...
_NUNMAI_HOME_ASSIGNMENT_RE = re.compile(r"(?:^|\s)nunmai_home=(?:\"([^\"]*)\"|'([^']*)'|(\S+))")


def nunmai_home_assignments(command: str) -> list[str]:
    """Values of every ``NUNMAI_HOME=<value>`` assignment in ``command`` (the caller lowercases
    and normalizes separators). Values are token-bounded, quotes stripped: the substring test
    this replaces let ``NUNMAI_HOME=/root/profiles/ops`` claim a ``/root/profiles/ops2`` gateway.
    The name is token-bounded too (``FOO=nunmai_home=/x`` is not an assignment), and a trailing
    separator on the value is stripped -- ``NUNMAI_HOME=/root/.nunmai/`` (systemd ``Environment=``
    or a shell wrapper spelling) is the same home as ``/root/.nunmai``; callers strip the profile
    home the same way."""
    return [
        next(g for g in m.groups() if g is not None).rstrip("/")
        for m in _NUNMAI_HOME_ASSIGNMENT_RE.finditer(command)
    ]


def command_line_names_nunmai_home(command_lc: str, home_lc: str) -> bool:
    """True when ``command_lc`` carries ``NUNMAI_HOME=<home_lc>`` (both lowercased, ``/``-separated,
    no trailing separator). Argv reaches us space-joined, so an unquoted value with a space in it
    (``NUNMAI_HOME=C:/Users/John Doe/.nunmai``) is cut at the space by the token parser; a
    token-bounded literal match of the whole home recovers that spelling."""
    if home_lc in nunmai_home_assignments(command_lc):
        return True
    return re.search(rf"(?:^|\s)nunmai_home={re.escape(home_lc)}/?(?=\s|$)", command_lc) is not None
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``nunmai_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``NUNMAI_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    ...
    if profile_name is not None and profile_name != "default":
        if profile_flag_value(command_lc) == profile_name.lower():
            return True
        return command_line_names_nunmai_home(command_lc, home_lc)
    ...
    # or a conflicting explicit NUNMAI_HOME= (its absence is not disqualifying -- NUNMAI_HOME usually
    # arrives via the env).
    if profile_flag_value(command_lc) is not None:
        return False
    return not nunmai_home_assignments(command_lc) or command_line_names_nunmai_home(command_lc, home_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "nunmai_home": str(_canonical_nunmai_home(_get_process_nunmai_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``nunmai_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `nunmai update` (and the dashboard) prove whether a running gateway actually
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``nunmai_home`` matches the current process (legacy records: True);
    another NUNMAI_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("nunmai_home")
    return not record_home or _same_nunmai_home(record_home, _get_process_nunmai_home())
...
def profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Profile id of any Nunmai home: ``<root>/profiles/<name>`` → ``<name>``, the default root →
    ...
    try:
        ...
        if home.resolve() == Path(get_default_nunmai_root()).resolve():
            return "default"
    except Exception:
        return None
    return None
...
def multiplexer_liveness_for_profile(profile_dir: Path) -> Optional[tuple[int, dict[str, Any]]]:
    ...
    reports it stopped while ``nunmai -p X status`` says running — the two must agree.
    """
    ...
    from nunmai_cli.gateway_multiplex_served import live_default_gateway_pid
    from nunmai_constants import get_default_nunmai_root
    ...
    if name != "default" and not _same_nunmai_home(profile_dir, get_default_nunmai_root() / "profiles" / name):
        return None
    ...
    if pid is None:
        return None
    return pid, read_runtime_status(get_default_nunmai_root() / "gateway_state.json") or {}
...
def resolve_gateway_liveness(
    *, profile_dir: Optional[Path] = None, runtime: Any = _UNSET,
    ...
    # the multiplexer IS its gateway (mirrors `nunmai -p X status` / `gateway list`). Unscoped, the
    # question is about the process's OWN home — which is a named profile inside a pooled
    # `nunmai --profile X serve` (the Desktop's per-profile backend answers its REST without
    # `?profile=`), so it takes the same rung instead of reporting the served profile stopped.
    own_home = profile_dir if scoped else _get_process_nunmai_home()
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # The record's nunmai_home must match the home asked about (this process unscoped) so a stale
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to nunmai_home.
    profile = _profile_label_for_home(_get_process_nunmai_home())
    if profile:
...
# token. ``nunmai.service`` + ``nunmai- gateway.service``). See #5646.
...
def _get_takeover_marker_path(nunmai_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``nunmai_home`` is given only for a verified cross-home handoff."""
    home = _canonical_nunmai_home(nunmai_home or _get_process_nunmai_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_nunmai_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``nunmai gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-NUNMAI_HOME --replace while ignoring a marker accidentally written into another
    ...
    target_home = record.get("target_nunmai_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_nunmai_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_nunmai_home")
        if replacer_home is not None and not _same_nunmai_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_nunmai_home(target_home or _get_process_nunmai_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_nunmai_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_nunmai_home": str(_canonical_nunmai_home(_get_process_nunmai_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("nunmai_home")
    if (
        ...
        return None
    target_home = _canonical_nunmai_home(raw_home)
    ...
    pid_record_home = pid_record.get("nunmai_home")
    if (
        ...
        or not _same_nunmai_home(pid_record_home, target_home)
    ):
        return None
