...
The PID file lives at ``{NUNMAI_HOME}/gateway.pid``.  NUNMAI_HOME defaults to
``~/.nunmai`` but can be overridden via the environment variable.  This means
separate NUNMAI_HOME directories naturally get separate PID files — a property
...
from nunmai_constants import get_nunmai_home, _get_platform_default_nunmai_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_nunmai_home() / "gateway-starts.log"
...
def _get_process_nunmai_home() -> Path:
    """Return the process-level NUNMAI_HOME, skipping context-local overrides.
    ...
    ``get_nunmai_home()`` honors ``_NUNMAI_HOME_OVERRIDE`` contextvar used for
...
def _canonical_nunmai_home(path: Path | str) -> Path:
    """Return a stable absolute NUNMAI_HOME path for persisted identity data."""
...
def _same_nunmai_home(left: Path | str, right: Path | str) -> bool:
    ...
    return os.path.normcase(str(_canonical_nunmai_home(left))) == os.path.normcase(
        str(_canonical_nunmai_home(right))
    )


# Mirrors nunmai_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (nunmai_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a NUNMAI_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.nunmai/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ...
    try:
        ...
        if _same_nunmai_home(canonical, get_default_nunmai_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_nunmai_home(canonical, _get_platform_default_nunmai_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    NUNMAI_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``nunmai_home`` for locks written before the
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting NUNMAI_HOME."""
    home = _get_process_nunmai_home()
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Nunmai' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Nunmai gateway lifecycle subcommand from a command line.
    ...
    test also matched ``nunmai_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\nunmai-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Nunmai's
    ...
    for token in tokens:
        ...
        if basename in ("nunmai-gateway", "nunmai-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "nunmai_cli.main" in joined
        or "nunmai_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("nunmai", "nunmai.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `nunmai gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Nunmai-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Nunmai gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a NUNMAI_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.nunmai`` or ``$NUNMAI_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``nunmai_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``NUNMAI_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a NUNMAI_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"nunmai_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit NUNMAI_HOME= on the argv. NUNMAI_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "nunmai_home=" in command_lc and f"nunmai_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # NUNMAI_HOME-local.  Persist the owning gateway's process home so an
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``nunmai_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `nunmai update` (and the
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `nunmai status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``NUNMAI_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``nunmai_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    NUNMAI_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different NUNMAI_HOME dirs).
    """
    ...
    # is inferable; readers fall back to deriving it from nunmai_home.
...
# enabled in the user's systemd (e.g. ``nunmai.service`` + ``nunmai-
# gateway.service``).
#
...
def _get_takeover_marker_path(nunmai_home: Optional[Path] = None) -> Path:
    ...
    ``nunmai_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = nunmai_home or _get_process_nunmai_home()
    return _canonical_nunmai_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-NUNMAI_HOME --replace while
    ...
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_nunmai_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_nunmai_home")
        if replacer_home is not None and not _same_nunmai_home(
    ...
    # ``nunmai gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's NUNMAI_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_nunmai_home(
            target_home or _get_process_nunmai_home()
        )
        ...
        record = {
            ...
            "target_nunmai_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_nunmai_home": str(
                _canonical_nunmai_home(_get_process_nunmai_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target NUNMAI_HOME's gateway PID record, and the live
    ...
    if not isinstance(pid_record_home, str) or not _same_nunmai_home(
