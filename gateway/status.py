...
from nunmai_constants import _get_platform_default_nunmai_home, get_nunmai_home
...
def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``NUNMAI_GATEWAY_LOCK_DIR`` overrides."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return any(needle in command_lc for needle in (
            f"--profile {profile_lc}", f"-p {profile_lc}", f"nunmai_home={home_lc}"
        ))
    # Default profile: accept unless argv names another profile or a conflicting explicit
    # NUNMAI_HOME= (its absence is not disqualifying -- NUNMAI_HOME usually arrives via the env).
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    return not ("nunmai_home=" in command_lc and f"nunmai_home={home_lc}" not in command_lc)
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's nunmai_home must match this process so a stale record
