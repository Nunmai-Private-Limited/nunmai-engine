...
Defenses 1 and 2 (the ``_NUNMAI_GATEWAY`` guard on ``nunmai gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.nunmai.gateway")``,
...
State lives in ``<NUNMAI_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_nunmai_home() / "gateway" / "restart_loop.json"
