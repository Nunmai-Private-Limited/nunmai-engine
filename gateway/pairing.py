...
# path eagerly freezes it to whatever NUNMAI_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see nunmai_constants.set_nunmai_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``nunmai pairing`` CLI) re-imports this module later with the final
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_nunmai_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``nunmai_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{NUNMAI_HOME}/pairing`` while newer code/docs may
    write ``{NUNMAI_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own NUNMAI_HOME using the same legacy/consolidated layout rules
    as ``nunmai -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current NUNMAI_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp NUNMAI_HOME
        ...
        if profile:
            ...
            # ``nunmai -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `nunmai` after
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``nunmai
