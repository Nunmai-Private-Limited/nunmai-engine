...
def _nunmai_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
...
class _ResponsesStream:
    ...
    async def emit_status(self, payload: Dict[str, Any]) -> None:
        ...
        ``nunmai.status`` custom event; not a Responses output item."""
        await self.response.write(self._api._sse_frame(payload, event="nunmai.status"))
...
class OpenAICompatRoutesMixin:
    ...
    def _spawn_stream_agent(self, stream_q, **run_kwargs) -> tuple:
        ...
        def _on_status(kind, message=None):
            # Lifecycle/warning status (provider wait, auto-recovery countdown, fallback switch) as a
            # ``nunmai.status`` event, so a client sees why the stream is silent instead of a dead socket.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-Nunmai-Session-Key scopes long-term memory per channel; independent of
        # X-Nunmai-Session-Id (the key persists across transcripts, the id rotates on /new).
        gateway_session_key, key_err = self._parse_session_key_header(request)
        if key_err is not None:
            return key_err
        # X-Nunmai-Session-Id continues an existing session (history from state.db, not the body);
        ...
        run_kwargs = dict(
            ...
            # #98619: only an explicitly provided X-Nunmai-Session-Id is wake-capable (the
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``nunmai.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        response_headers = {"X-Nunmai-Session-Id": (provided_session_id or result.get("session_id", session_id))}
        ...
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Nunmai extras.
        response_data = {
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    await response.write(_sse_frame(delta[1], event="nunmai.tool.progress"))
                    ...
                    await response.write(_sse_frame(delta[1], event="nunmai.status"))
                else:
            ...
            if finish_reason != "stop":
                ...
                finish_chunk["nunmai"] = {
                    ...
                    **_nunmai_extras(completed, is_partial, is_failed, "" if presentation_muted else err_msg, finish_reason)}
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Session precedence: previous_response_id chain > declared X-Nunmai-Session-Key > fresh
