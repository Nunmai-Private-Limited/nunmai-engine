...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    ...
    if raw == "~/.nunmai":
        return nunmai_home
    if raw.startswith("~/.nunmai/"):
        return nunmai_home / raw.removeprefix("~/.nunmai/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return nunmai_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under NUNMAI_HOME/scripts."""
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__nunmai_ignore__") is True
        ):
            return False, None
