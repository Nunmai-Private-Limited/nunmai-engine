...
There are two ways to add a platform to the Nunmai gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nunmai/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nunmai code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nunmai gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nunmai_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nunmai core.
...
And add it to the `nunmai-gateway` composite:

```python
"nunmai-gateway": {
    "includes": [..., "nunmai-your-platform"]
}
```
...
## 12. Status Display (`nunmai_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nunmai_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nunmai_cli/ toolsets.py \
  --include="*.py" -l | sort -u
