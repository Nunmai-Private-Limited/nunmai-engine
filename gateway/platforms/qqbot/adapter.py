...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.nunmai/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Nunmai upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "nunmai-engine",
                    "$device": "nunmai-engine",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.nunmai/.update_response`` for the
          detached ``nunmai update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``nunmai update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `nunmai setup gateway` / `nunmai gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``nunmai update --gateway`` watcher. Button
        ...
        ``~/.nunmai/.update_response`` so the detached update process
        can read it.
        """
