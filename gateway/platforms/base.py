...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``NUNMAI_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "nunmai_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_nunmai_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_nunmai_dir(new_subpath, old_name)
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; NunmaiAgent/1.0)", "Accept": accept}
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_NUNMAI_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the NUNMAI_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active NUNMAI_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (NUNMAI_HOME symlinked).

    ``NUNMAI_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when NUNMAI_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
...
def _profile_dirs() -> List[Path]:
    ...
    try:
        return [p for p in (_NUNMAI_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []


def _credential_home_roots() -> List[Path]:
    """Every Nunmai home whose credential stores the denylist must cover: the ACTIVE home
    (the per-turn NUNMAI_HOME override under ``gateway.multiplex_profiles``), the shared root
    ...
    return list(dict.fromkeys((get_nunmai_home(), _NUNMAI_ROOT, *_profile_dirs())))


def _kanban_root() -> Path:
    """Kanban is root-shared across profiles by design (``kanban_db.kanban_home``)."""
    return Path(os.environ.get("NUNMAI_KANBAN_HOME", "").strip() or _NUNMAI_ROOT).expanduser()
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.nunmai/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Nunmai cache dirs (``/root/.nunmai/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.nunmai/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `nunmai serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.nunmai/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.nunmai"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``NUNMAI_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a Nunmai cache, an operator root (``NUNMAI_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, Nunmai-root secrets).
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``NUNMAI_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for Nunmai commands ("!" where the client eats a leading "/").
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("NUNMAI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower() or "interrupt")
        self._busy_text_debounce_seconds: float = _float_env("NUNMAI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35)
        self._busy_text_hard_cap_seconds: float = _float_env("NUNMAI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0)
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-NUNMAI_HOME
        ...
        remedy = (f" Stop that gateway first (nunmai --profile {owner_profile} gateway stop)."
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from nunmai_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    def _get_human_delay() -> float:
        """Random human-like pacing delay (s) from NUNMAI_HUMAN_DELAY_MODE: "off" (default) |
        "natural" 800-2500ms | "custom" via NUNMAI_HUMAN_DELAY_MIN_MS /
        NUNMAI_HUMAN_DELAY_MAX_MS."""
        mode = os.getenv("NUNMAI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        lo, hi = 800, 2500
        if mode != "natural":  # custom mode tolerates malformed env vars
            lo = _or_default(lambda: int(os.getenv("NUNMAI_HUMAN_DELAY_MIN_MS", str(lo))), lo)
            hi = _or_default(lambda: int(os.getenv("NUNMAI_HUMAN_DELAY_MAX_MS", str(hi))), hi)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: NUNMAI_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def _wants_auto_tts(self, event: MessageEvent, session_key: str, interrupt_event: asyncio.Event,
        ...
        generation = getattr(interrupt_event, "_nunmai_run_generation", None)
        return bool(
    ...
    async def _fire_post_delivery_callback(self, session_key: str, interrupt_event: asyncio.Event) -> None:
        ...
        _post_cb = self.pop_post_delivery_callback(
            session_key, generation=getattr(interrupt_event, "_nunmai_run_generation", None))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            self._streaming_tts_completed_turns.discard(self._streaming_tts_turn_key(
                session_key, getattr(interrupt_event, "_nunmai_run_generation", None),
                event=event) or "")
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile NUNMAI_HOME isolation."""
