...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    process-env read — those are OS/system-level network settings, not a per-profile Nunmai concept."""
...
# ``nunmai pairing approve`` command (nunmai_cli/subcommands/pairing.py) that lets the owner fix it.
...
UNAUTHORIZED_ACTION_NOTICE = (
    ...
    "`nunmai pairing approve {platform} <request-id>` on the host (`nunmai pairing list` shows the id).")
...
class BasePlatformAdapter(ABC):
    ...
    # ``nunmai gateway migrate`` can tell "URL changes" from "this profile would be skipped" as new
