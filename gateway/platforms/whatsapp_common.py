...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/nunmai) is mirrored to NUNMAI_HOME so npm install works."""
    import shutil
    from nunmai_constants import get_nunmai_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    nunmai_home_bridge = get_nunmai_home() / "scripts" / "whatsapp-bridge"
    try:
    ...
    try:
        ...
        shutil.copytree(install_bridge, nunmai_home_bridge, dirs_exist_ok=False)
        return nunmai_home_bridge
    except Exception:
        return install_bridge
