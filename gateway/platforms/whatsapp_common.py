...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "◆ *Nunmai Engine*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to NUNMAI_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/nunmai), this function
    mirrors the bridge source to a writable NUNMAI_HOME location and returns that
    ...
    # Try NUNMAI_HOME location first
    nunmai_home = get_nunmai_home()
    nunmai_home_bridge = nunmai_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to NUNMAI_HOME if needed
    if nunmai_home_bridge.exists():
        return nunmai_home_bridge

    # Mirror the bridge source to NUNMAI_HOME
    try:
        nunmai_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            nunmai_home_bridge,
            dirs_exist_ok=False,
