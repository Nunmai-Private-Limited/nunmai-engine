...
def decode_json_list_literal(raw):
    """Decode a JSON-encoded allowlist written by ``nunmai config set``.
...
def env_is_connected(*names: str) -> Callable[[Any], bool]:
    ...
    non-blank. Resolves ``nunmai_cli.gateway.get_env_value`` at call time (scope-aware, .env-backed) so
