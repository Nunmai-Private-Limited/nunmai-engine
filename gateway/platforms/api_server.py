...
def _nunmai_version() -> str:
    """Canonical Nunmai version: ``nunmai_cli.__version__`` (dist-info can be stale on
...
def _request_agent_overrides(
    ...
    The virtual model (``nunmai-engine``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Nunmai-native
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        from nunmai_state_wal import apply_wal_with_fallback
...
_CORS_HEADERS = {
    ...
    "Access-Control-Allow-Headers": "Authorization, Content-Type, Idempotency-Key, X-Nunmai-Session-Id"}
_SECURITY_HEADERS = {
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Nunmai session/sandbox is reused."""
...
class _ProviderAuthResolutionError(RuntimeError):
    ...
    def user_text(self) -> str:
        ...
        from nunmai_cli.auth import is_rate_limited_auth_error
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through nunmai-engine's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Nunmai-native session-chat
        ...
        self._response_store_home = str(get_nunmai_home())
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "nunmai-engine"
        (precedence owned by ``nunmai_cli.model_switch.resolve_effective_model``)."""
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Nunmai-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Nunmai-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_nunmai_home()``
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "nunmai-engine" upstream as a provider model id 400s)."""
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `nunmai auth add` without `nunmai model`).
    ...
    def _select_agent_runtime(
        ...
        # nunmai_cli.model_switch.resolve_effective_model.
        from nunmai_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "nunmai-engine", "version": _nunmai_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "nunmai-engine",
            "version": _nunmai_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — nunmai-engine plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "nunmai", "permission": [],
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "nunmai.api_server.capabilities", "platform": "nunmai-engine",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<NUNMAI_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Nunmai home.
            try:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        # Exact-title lookup (`nunmai peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `nunmai peer dm`
            ...
            try:
                ...
                # which would fail `nunmai peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Nunmai session row. Existence check, insert and
        ...
        # would persist "nunmai-engine" and later send it to the provider literally.
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Nunmai-Session-Id`` (+ ``X-Nunmai-Session-Key`` when declared) response headers."""
        headers = {"X-Nunmai-Session-Id": session_id}
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the NUNMAI_HOME
    ...
    async def _answer_through_live_bot_chat(self, ctx: Dict[str, Any]) -> Optional["web.Response"]:
        ...
        This is the ``nunmai peer dm`` transport. Running the turn here would make this process a
        ...
        if record["status"] == "settled":
            return web.json_response(
                {"object": "nunmai.session.chat.completion", "session_id": session_id,
        ...
        if record["status"] in ("queued", "claimed"):
            return web.json_response(
                {"object": "nunmai.session.chat.queued", "session_id": session_id,
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        one bounded re-run of a transient failure; ``nunmai peer dm`` is the client)."""
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "nunmai.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `nunmai cron run` /
    ...
    async def _handle_cron_fire(self, request: "web.Request") -> "web.Response":
        ...
        with _reserve_pending_api_work(self) as reservation:
            ...
            # `nunmai pause` ESTOP: refuse the fire and ask NAS to retry later.
            ...
            with suppress(ImportError):
                ...
                if _estop_check_paused("cron-webhook", logger):
                    return web.json_response(
                        {"error": "nunmai is paused (ESTOP)", "job_id": job_id},
                        status=503,
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        reach ``NUNMAI_SESSION_PROFILE``: the persistent-Docker container key is derived from it, so an
