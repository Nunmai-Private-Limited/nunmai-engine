...
Connects Nunmai Engine to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_file(nunmai_home: str, account_id: str) -> Path:
    return _account_dir(nunmai_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(nunmai_home, account_id)
...
def _sync_buf_path(nunmai_home: str, account_id: str) -> Path:
    return _account_dir(nunmai_home) / f"{account_id}.sync.json"


def _load_sync_buf(nunmai_home: str, account_id: str) -> str:
    path = _sync_buf_path(nunmai_home, account_id)
    if not path.exists():
        return ""
    try:
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Nunmai adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(nunmai_home, self._account_id)
            if persisted:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Nunmai regardless of this "
