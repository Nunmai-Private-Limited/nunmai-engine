...
``<NUNMAI_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_nunmai_home() -> Path:
    """NUNMAI_HOME for process-level identity files (ignore task overrides)."""
    from nunmai_constants import get_nunmai_home, get_process_nunmai_home

    # get_process_nunmai_home expands ``~``/``$VAR`` (python -m gateway.run skips the CLI normalizer).
    return get_process_nunmai_home() if os.environ.get("NUNMAI_HOME", "").strip() else get_nunmai_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_nunmai_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<NUNMAI_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `nunmai doctor`.",
            verdict,
        )
