...
All hooks write atomic JSON payloads under ``<nunmai_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active NUNMAI_HOME."""
    from nunmai_constants import get_nunmai_home
    flush_dir = get_nunmai_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<nunmai_home>/pending_messages/``), so a runtime cap rotation no longer silently
