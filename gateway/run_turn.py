...
def _tool_call_logger() -> logging.Logger:
    """Process-wide ``nunmai.tool_calls`` Logger + one RotatingFileHandler on logs/tool_calls.log.
    Named Loggers live in ``logging.Logger.manager.loggerDict`` forever, so the former per-turn name
    (``nunmai.tool_calls.<id(log_queue)>``) leaked one Logger per logged turn (#62950); a single
    shared handler also keeps concurrent turns from double-writing lines."""
    tool_logger = logging.getLogger("nunmai.tool_calls")
...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        if override and override.get("provider"):
            try:
                ...
                # endpoint (openai-codex on the Nunmai URL). Run this turn on the whole default route and say
        ...
        # Provider resolved but no model.default (`nunmai auth add` without `nunmai model`): use the
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens):
        ...
        try:
            _lease_token = await _lease_registry.acquire(
                ...
                timeout=_float_env("NUNMAI_TURN_LEASE_TIMEOUT", DEFAULT_LEASE_WAIT),
            )
    ...
    async def _hmwa_hygiene_detached_attempt(
        ...
        try:
            ...
            attempt.future = loop.run_in_executor(
                ...
                # secret scope / NUNMAI_HOME override live in ContextVars, and a bare run_in_executor worker
    ...
    async def _hmwa_first_contact_notes(self, source, history, turn_sidecar_notes):
        ...
        from gateway.run import _nunmai_home, _home_target_env_var, _load_gateway_config
        if history:
            return
        if not await self.async_session_store.has_any_sessions():
            ...
            try:
                ...
                if profile_build_mode(_onb_cfg) == "ask" and not is_seen(_onb_cfg, PROFILE_BUILD_FLAG):
                    ...
                    mark_seen(_nunmai_home / "config.yaml", PROFILE_BUILD_FLAG)
                else:
        ...
        if not home_env:
            with suppress(Exception):
                ...
                if prof and prof != "default" and _lgc().get_home_channel(source.platform):
                    home_env = "set"
        if not home_env:
            # Slack routes every command through the parent `/nunmai`; bare `/sethome` would fail.
            sethome_cmd = "/nunmai sethome" if source.platform == Platform.SLACK else "/sethome"
            await self._deliver_platform_notice(
                source, f"📬 No home channel is set for {platform_name.title()}. "
                f"A home channel is where Nunmai delivers cron job results and cross-platform "
    ...
    def _hmwa_apply_message_timestamp(self, event, message_text):
        ...
        try:
            from nunmai_time import get_timezone as _get_evt_tz
    ...
    # Chat-side next steps keyed by HTTP status; Nunmai commands only (/login is the gateway's own
    ...
    async def _hmwa_agent_error_reply(self, e, event, source, session_entry, session_key, prepared):
        ...
        return self._hmwa_add_failed_turn_notice(
            ...
            "Technical details are in the gateway log (`nunmai logs`).",
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    ...
                    "configured. Use /login, or run `nunmai setup` on the host.",
    ...
    async def _execute_mcp_reload(self, event: MessageEvent) -> str:
        ...
        if multiplex and not get_nunmai_home_override():
        ...
        try:
            ...
            # a browser OAuth flow either: an expired token parks with a `nunmai mcp login` hint.
    ...
    async def _run_agent_via_proxy(
        ...
        """Forward the message to a remote Nunmai API server instead of running a local AIAgent.
        ...
        # OpenAI chat format. The remote keeps continuity via X-Nunmai-Session-Id; send the current
        ...
        body = {"model": "nunmai-engine", "messages": api_messages, "stream": True}
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        progress_mode, _tool_progress_explicit = resolve_tool_progress(
            user_config, platform_key, get_secret("NUNMAI_TOOL_PROGRESS_MODE"),
        )
        ...
        # "log" mode: tool calls go to ~/.nunmai/logs/tool_calls.log instead of the chat. Gateway-only.
    ...
    def _run_agent_start_turn_worker(self, turn_ctx: TurnContext, run_sync: Callable[[], Any]) -> "GatewayRunner._RunAgentWorker":
        ...
        *Inactivity* timeout (agent.gateway_timeout / NUNMAI_AGENT_TIMEOUT, env wins; 0 = unlimited),
        ...
        _agent_timeout, _agent_warning = (
            v if v > 0 else None
            for v in (_float_env("NUNMAI_AGENT_TIMEOUT", 1800), _float_env("NUNMAI_AGENT_TIMEOUT_WARNING", 900))
        )
    ...
    def _run_agent_evict_on_fallback(self, turn_ctx: TurnContext) -> None:
        ...
        with suppress(Exception):
            from nunmai_cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    async def _run_agent_drain_pending(
        ...
        if pending and pending.strip().startswith("/"):
            ...
            if _pending_cmd_word:
                with suppress(Exception):
                    from nunmai_cli.commands import resolve_command as _rc_pending
    ...
    async def _run_agent_notify_long_running(
        ...
        Interval: agent.gateway_notify_interval / NUNMAI_AGENT_NOTIFY_INTERVAL (default 180s; 0 or
        ...
        _NOTIFY_INTERVAL = _float_env("NUNMAI_AGENT_NOTIFY_INTERVAL", 180)
