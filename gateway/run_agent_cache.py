...
class GatewayAgentCacheMixin:
    ...
    def _is_intentional_model_switch(self, session_key: str, agent: Any, config_model: str) -> bool:
        ...
        override names that model, or the Nunmai gateway moved the session off the ``nous/welcome``
    ...
    def _bind_adapter_run_generation(self, adapter: Any, session_key: str, generation: int | None) -> None:
        ...
        with suppress(Exception):
            ...
            if interrupt_event is not None:
                interrupt_event._nunmai_run_generation = int(generation)
    ...
    async def _interrupt_and_clear_session(
        ...
        if running_agent and running_agent is not _AGENT_PENDING_SENTINEL:
            ...
            try:
                from nunmai_cli.plugins import invoke_hook as _invoke_hook
    ...
    def _ephemeral_change_key(context, redact_pii: bool) -> str:
        ...
        try:
            ...
            home_display = str(display_nunmai_home())
        except Exception:
            home_display = ""
        key_tuple = (
    ...
    def _run_release_in_profile_scope(self, target, args: tuple, session_key: Optional[str]) -> None:
        ...
        root Nunmai dir even when the gateway was launched under a named profile. Its scope is
            ...
            if session_key and store is not None:
                try:
                    ...
                    logger.warning("Could not resolve the owning profile for %s; releasing under the default profile",
                                   session_key, exc_info=True)
            owner_home = Path(owner) if owner else get_default_nunmai_root()
            if current_secret_scope() is None or nunmai_home_key(get_nunmai_home()) != nunmai_home_key(owner_home):
