...
boundary. POSIX: ``$NUNMAI_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
...
def _detect_supervisor() -> str:
    ...
    if env.get("INVOCATION_ID"):
        return "systemd"
    if sys.platform == "darwin" and (env.get("XPC_SERVICE_NAME", "").startswith("ai.nunmai")
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "nunmai_home")},
