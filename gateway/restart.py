...
# Set by ``nunmai gateway run --external-supervisor``. Unlike systemd's INVOCATION_ID
# and launchd's XPC_SERVICE_NAME, this survives wrappers that replace the child
# environment (e.g. ``sudo env -i``).
EXTERNAL_GATEWAY_SUPERVISOR_ENV = "NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR"

# Forwarded by the stderr-timestamp launchd wrapper (nunmai_cli/stderr_timestamp.py) to the gateway
# grandchild, which sees ``XPC_SERVICE_NAME=0``. Read only via :func:`launchd_job_label`.
LAUNCHD_LABEL_ENV = "NUNMAI_LAUNCHD_LABEL"
...
def launchd_job_label(environ: Mapping[str, str] | None = None) -> str | None:
    """The ``ai.nunmai.*`` launchd job label in *environ*, or ``None`` (no darwin gate).
    ...
    and finds its label only in the wrapper's re-export, ``NUNMAI_LAUNCHD_LABEL``. Both go through
    the same ``ai.nunmai`` predicate: app-coalition labels (``application.<bundle>…``, exported
    ...
    for var in ("XPC_SERVICE_NAME", LAUNCHD_LABEL_ENV):
        ...
        if label.startswith("ai.nunmai"):
            return label
    return None


def launchd_service_label(environ: Mapping[str, str] | None = None, *, platform: str = sys.platform) -> str | None:
    """Return this process's ``ai.nunmai.*`` launchd job label, or ``None`` off darwin.
...
def read_launchd_exit_timeout_s(
    ...
    *,
    ...
    is not launchd-owned (non-darwin, or no ``ai.nunmai`` job label — see :func:`launchd_job_label`), ``launchctl`` is missing
...
def is_gateway_supervisor_process(environ: Mapping[str, str] | None = None) -> bool:
    ...
    ``NUNMAI_LAUNCHD_LABEL`` the grandchild sees), the s6 sentinel, or the explicit
    ``--external-supervisor`` opt-in. The generalized ``NUNMAI_SUPERVISED_CHILD`` launcher marker is
    ...
    return bool(env.get("INVOCATION_ID") or env.get("NUNMAI_S6_SUPERVISED_CHILD") or (xpc_service and xpc_service != "0")
...
def is_supervised_gateway_launch(environ: Mapping[str, str] | None = None) -> bool:
    ...
    Superset of :func:`is_gateway_supervisor_process` that also honours ``NUNMAI_SUPERVISED_CHILD``,
    the marker every generated launcher exports (systemd unit, launchd plist, s6 run script, Windows
    Scheduled Task — see ``nunmai_cli.main._apply_profile_override``). This is the identity the
    ...
    """
    env = os.environ if environ is None else environ
    if env.get("NUNMAI_SUPERVISED_CHILD"):
        return True
