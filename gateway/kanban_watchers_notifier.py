...
def _kbc():
    from nunmai_cli import kanban_db_connect
    return kanban_db_connect


def _kbn():
    from nunmai_cli import kanban_db_notify
...
def _warn_anchorless_thread_sub_once(sub: dict, platform: str) -> None:
    ...
    logger.warning(
        ...
        "profile route; it will not be delivered. Re-subscribe with `nunmai kanban notify-subscribe ... "
        ...
        if route.matches(platform.value, guild_id=guild, chat_id=chat,
            ...
            if route.profile != profile:
                _warn_unroutable_sub_once(
                    ...
                    "Re-subscribe with `nunmai kanban notify-subscribe ... --notifier-profile %s`.",
...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        # NUNMAI_KANBAN_DB pins the board path.
        kb = self.kb
...
def _fmt_gave_up(ev, n) -> tuple:
    ...
    return (
        f"⛔ {n.head} is now blocked: {count}{last}. Fix the cause, then `nunmai kanban unblock "
        f"{n.task_id}` (or `nunmai kanban reassign {n.task_id}`). Logs: `nunmai kanban log {n.task_id}`.",
        None, None,
    )
...
class _KanbanNotification:
    ...
    def _served_wake_profile(self) -> Optional[str]:
        ...
        ``None`` keeps the historical path: a standalone ``nunmai -p <name>`` gateway owns its own
