...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_nunmai_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # prefix every reply "**Nunmai Engine:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``nunmai gateway enroll``.
    Modes, in precedence order:
      ...
      2.  Nunmai Portal (default): ``resolve_nous_access_token()``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.nunmai/.env``), so a hosted container re-provisions every boot; the
