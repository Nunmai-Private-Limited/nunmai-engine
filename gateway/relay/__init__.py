"""Relay/connector support package for the Nunmai gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``nunmai gateway enroll`` writes them to
    ``~/.nunmai/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``NUNMAI_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``nunmai gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from nunmai_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Nunmai Engine:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``nunmai gateway enroll`` CLI. Three modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Nunmai Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Nunmai Portal** (default): ``resolve_nous_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Nunmai Portal (default, unchanged behaviour).
        from nunmai_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Nunmai access token. In
    that case the runtime resolves the agent's own Nunmai access token (the same
    ...
    up. The creds live ONLY in process memory — never written to ``~/.nunmai/.env``.
    ...
    sets neither ``NUNMAI_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``nunmai gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'nunmai'}"
