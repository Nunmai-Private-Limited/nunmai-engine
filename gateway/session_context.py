...
Replaces the old ``os.environ``-based ``NUNMAI_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "NUNMAI_SESSION_PLATFORM", "NUNMAI_SESSION_SOURCE", "NUNMAI_SESSION_CHAT_ID",
    "NUNMAI_SESSION_CHAT_TYPE", "NUNMAI_SESSION_CHAT_NAME", "NUNMAI_SESSION_THREAD_ID",
    "NUNMAI_SESSION_USER_ID", "NUNMAI_SESSION_USER_ID_ALT", "NUNMAI_SESSION_USER_NAME",
    "NUNMAI_SESSION_SCOPE_ID", "NUNMAI_SESSION_KEY", "NUNMAI_SESSION_ID",
    "NUNMAI_UI_SESSION_ID", "NUNMAI_SESSION_MESSAGE_ID", "NUNMAI_SESSION_PROFILE",
    "NUNMAI_BROWSER_CONTROL_PRINCIPAL", "NUNMAI_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "NUNMAI_CRON_SESSION", "NUNMAI_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("NUNMAI_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("NUNMAI_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("NUNMAI_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("NUNMAI_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("NUNMAI_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``NUNMAI_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``NUNMAI_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds NUNMAI_SESSION_PLATFORM, CLI/TUI/
# desktop bind NUNMAI_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``NUNMAI_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("NUNMAI_PLATFORM") or get_session_env("NUNMAI_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("NUNMAI_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``NUNMAI_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
