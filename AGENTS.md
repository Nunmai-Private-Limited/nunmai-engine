# Nunmai Engine - Development Guide

Instructions for AI coding assistants and developers working on the nunmai-engine codebase.

**Never give up on the right solution.**

## What Nunmai Is

Nunmai is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Nunmai ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`nunmai tools`, `nunmai setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `NUNMAI_HOME`. Mocks hide integration bugs.
...
- **New `NUNMAI_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `nunmai tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.nunmai/plugins/` (or via a
  pip entry point), and promote them in the Nunmai Research Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `nunmai <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `nunmai webhook`, `nunmai cron`, `nunmai tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.nunmai/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Nunmai Cloud. Only the first two are spawned
by us and carry `NUNMAI_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_NUNMAI_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `NUNMAI_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`nunmai --tui` against that same backend) is
...
`$HOME/.nunmai/nunmai-engine/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _NUNMAI_CORE_TOOLS list
├── cli.py                # NunmaiCLI class — interactive CLI orchestrator (~11k LOC)
├── nunmai_state.py       # SessionDB — SQLite session store (FTS5 search)
├── nunmai_constants.py   # get_nunmai_home(), display_nunmai_home() — profile-aware paths
├── nunmai_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── nunmai_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── nunmai-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `nunmai --tui`
...
**User config:** `~/.nunmai/config.yaml` (settings), `~/.nunmai/.env` (API keys only).
**Logs:** `~/.nunmai/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_nunmai_home()`.
Browse with `nunmai logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Nunmai: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`nunmai_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `NunmaiCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.nunmai/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`nunmai_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/nunmai` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `nunmai_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `NunmaiCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `nunmai --tui` or `NUNMAI_TUI=1`.
...
npm run dev       # watch mode (rebuilds nunmai-ink + tsx --watch)
npm start         # production
npm run build     # full build (nunmai-ink + tsc)
...
### TUI in the Dashboard (`nunmai dashboard` → `/chat`)

The dashboard embeds the real `nunmai --tui` — **not** a rewrite.  See `nunmai_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `nunmai_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `nunmai --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `nunmai --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@nunmai/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `nunmai serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `NUNMAI_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `nunmai` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `nunmai --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Nunmai built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Nunmai core.
Use the plugin route instead: create `~/.nunmai/plugins/<name>/plugin.yaml`
and `~/.nunmai/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Nunmai tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_NUNMAI_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_NUNMAI_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_nunmai_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `NUNMAI_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_nunmai_home()` for the base directory — never `Path.home() / ".nunmai"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `nunmai_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `nunmai_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `nunmai tools`, `nunmai setup`, most CLI subcommands | `nunmai_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`nunmai_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
nunmai_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.nunmai/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Nunmai gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `nunmai_cli/skin_engine.py`:
...
Users create `~/.nunmai/skins/<name>.yaml`:
...
Nunmai has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.nunmai/plugins/` and pip-installed entry points.

### General plugins (`nunmai_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.nunmai/plugins/`, `./.nunmai/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `nunmai` at startup so
  `nunmai <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$NUNMAI_HOME/plugins/`, `./.nunmai/plugins/` (opt-in via
`NUNMAI_ENABLE_PROJECT_PLUGINS`), and `nunmai_agent.memory_providers` entry
...
`post_setup(nunmai_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `nunmai <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `nunmai --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `nunmai_cli/main.py`, etc.).
...
into `~/.nunmai/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `nunmai memory setup` / `post_setup()` without
...
`~/.nunmai/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Nunmai Research Discord
...
2. User: `$NUNMAI_HOME/plugins/model-providers/<name>/`
...
[`nunmai-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
companion repo, not in this tree.

### Bot Mode (`apps/desktop/src/plugins/nunmai-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Nunmai
...
`ui_meta['nunmai-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `nunmai skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.nunmai.tags`, `metadata.nunmai.category`,
`metadata.nunmai.related_skills`, `metadata.nunmai.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.nunmai.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Nunmai tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Nunmai Engine" is the secondary collaborator. If the
   contributor's commit shows "Nunmai Engine" as author (because they
   used Nunmai to draft the skill), replace it with their actual name
...
lives in the `nunmai-engine-dev` skill at
...
`"messaging"`); `_NUNMAI_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `nunmai tools` (the curses UI) or the
...
Nunmai returns a delegation id immediately and the result re-enters the
...
go to `~/.nunmai/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `nunmai_cli/curator.py` wires `nunmai curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.nunmai/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `nunmai cron <verb>`
...
- File lock at `~/.nunmai/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `nunmai kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `nunmai_cli/kanban.py` wires `nunmai kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`nunmai-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `NUNMAI_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
- **Plan** (`nunmai_cli/update_inventory.py`, `nunmai update --plan`):
...
- **Snapshot** (`nunmai_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `nunmai-gateway*` unit / `ai.nunmai.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.nunmai/logs/update_receipts/` (`latest.json` pointer; steps,
...
`nunmai serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_nunmai_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
Nunmai-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `NUNMAI_BACKGROUND_NOTIFICATIONS` env var):
...
Nunmai supports **profiles** — multiple fully isolated instances, each with its own
`NUNMAI_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `nunmai_cli/main.py` sets
`NUNMAI_HOME` before any module imports. All `get_nunmai_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_nunmai_home()` for all NUNMAI_HOME paths.** Import from `nunmai_constants`.
   NEVER hardcode `~/.nunmai` or `Path.home() / ".nunmai"` in code that reads/writes state.
   ```python
   # GOOD
   from nunmai_constants import get_nunmai_home
   config_path = get_nunmai_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".nunmai" / "config.yaml"
   ```

2. **Use `display_nunmai_home()` for user-facing messages.** Import from `nunmai_constants`.
   This returns `~/.nunmai` for default or `~/.nunmai/profiles/<name>` for profiles.
   ```python
   # GOOD
   from nunmai_constants import display_nunmai_home
   print(f"Config saved to {display_nunmai_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.nunmai/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_nunmai_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_nunmai_home()`,
   not `Path.home() / ".nunmai"`.

4. **Tests that mock `Path.home()` must also set `NUNMAI_HOME`** — since code now uses
   `get_nunmai_home()` (reads env var), not `Path.home() / ".nunmai"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path / ".nunmai")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not NUNMAI_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".nunmai" / "profiles"`, NOT `get_nunmai_home() / "profiles"`.
   This is intentional — it lets `nunmai -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `nunmai_cli.update_cmd._nunmai_holder_subcommand`
  (top-level subcommand of any Nunmai argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.nunmai` paths
Use `get_nunmai_home()` from `nunmai_constants` for code paths. Use `display_nunmai_home()`
for user-facing print/log messages. Hardcoding `~/.nunmai` breaks profiles — each profile
has its own `NUNMAI_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `nunmai_cli/curses_ui.py`. See `nunmai_cli/tools_config.py` for an example.
...
canary ledger, nunmai#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `NUNMAI_HOME`.

### Tests must not write to `~/.nunmai/`
The `_isolate_nunmai_home` autouse fixture in `tests/conftest.py` redirects `NUNMAI_HOME` to a temp dir. Never hardcode `~/.nunmai/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_nunmai_home()` resolve within the temp dir.
Use the pattern from `tests/nunmai_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `NUNMAI_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.nunmai/` | Your real config+auth.json                  | Temp dir per test                         |
...
Nunmai supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/nunmai_cli/test_venv_holder_windows_live.py` on a real
