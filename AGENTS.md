...
Nunmai is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Nunmai ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`nunmai tools`, `nunmai setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `NUNMAI_HOME` — two of them (A→B→A) when the
...
- **New `NUNMAI_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `nunmai tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.nunmai/plugins/` or pip
  entry point), promoted in the Nunmai Research Discord `#plugins-skills-and-skins`. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `nunmai <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`nunmai webhook`, `nunmai cron`, `nunmai tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.nunmai/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Nunmai Cloud, and
only the first two carry `NUNMAI_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Nunmai desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_NUNMAI_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `NUNMAI_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`nunmai --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.nunmai/nunmai-engine/venv`
...
├── toolsets.py           # TOOLSETS dict, _NUNMAI_CORE_TOOLS
├── cli.py                # NunmaiCLI (REPL, slash dispatch) + nunmai_cli/cli_*_mixin.py
├── nunmai_state.py       # SessionDB facade; nunmai_state_*.py siblings
├── nunmai_constants.py   # get_nunmai_home(), display_nunmai_home() — profile-aware paths
├── nunmai_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── nunmai_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
**User state:** `~/.nunmai/config.yaml` (settings), `~/.nunmai/.env` (secrets only),
`~/.nunmai/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_nunmai_home()`. Browse logs with `nunmai logs [--follow] [--level] [--session]`.
...
families: `nunmai_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`nunmai_cli/kanban.py` (14), `nunmai_cli/web_server.py` (13 + 24 routers), `nunmai_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `nunmai_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::nunmai_cli.plugin_compat.NunmaiPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `nunmai_cli.update_cmd._nunmai_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `nunmai_cli/AGENTS.md`.
- **Never hardcode `~/.nunmai`.** `get_nunmai_home()` for code paths, `display_nunmai_home()`
  for user-facing text (both from `nunmai_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.nunmai/profiles`) so `nunmai -p x profile list`
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `nunmai_home_key()` or resolve at call time.
  ...
  `NUNMAI_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Machine facts and resource lookup go through `nunmai_platform`.** `nunmai_platform.host` is the
  ...
  hand-written known-path table outside `nunmai_platform/` fails
  `tests/test_managed_runtime_resolution.py` unless allowlisted with a reason; resolvers land in
  `nunmai_platform/resolver/`. Lookup never installs, downloads, or starts anything.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `nunmai webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `NUNMAI_HOME` first.
...
The `[tool.uv] exclude-newer = "14 days"` quarantine covers **Nunmai's own dependencies only**
(`uv lock`/`sync`, `nunmai update`, `tools.lazy_deps.ensure` extras — `install policy "core"`).
...
abide by our 14 day rule … Only nunmai' dependencies themselves have to." We recommend (not require)
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `NUNMAI_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `NUNMAI_TEST_FILE_RETRIES=0` disables); a worker killed by signal or the file timeout is never
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/nunmai_cli/`,
  `tests/agent/`, `tests/nunmai_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `nunmai_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.nunmai/`.** The autouse `_isolate_nunmai_home` fixture in
  `tests/conftest.py` redirects `NUNMAI_HOME`; never hardcode `~/.nunmai/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_nunmai_home()` stay
  in the temp dir (pattern: `tests/nunmai_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      ...
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `NUNMAI_HOME` — code reads the
  env var, not `Path.home()/.nunmai`.
...
`tests/nunmai_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `nunmai_cli/`, `main.py` | `nunmai_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `nunmai update` pipeline, profiles / multiplex |
...
| `plugins/`, `nunmai_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `nunmai_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`nunmai_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`nunmai-engine-dev` skill, not here.
