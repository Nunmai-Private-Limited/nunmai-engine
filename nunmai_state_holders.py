...
an unlinked SQLite DB/WAL/SHM generation.  ``nunmai_state`` supplies only the
...
from nunmai_state_errors import is_sqlite_lock_error
...
_NUNMAI_EXECUTABLES = frozenset({"nunmai", "nunmai-engine", "nunmai-acp"})
_NUNMAI_PYTHON_MODULES = frozenset({"acp_adapter", "nunmai_cli.main"})
_NUNMAI_PYTHON_SCRIPTS = frozenset({"nunmai_cli/main.py", "run_agent.py"})
...
def describe_holder_pid(pid: int) -> str:
    """``PID 123 (nunmai gateway run)`` for operator-facing holder lists; /proc argv first, psutil elsewhere."""
...
def _looks_like_nunmai(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Nunmai execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _NUNMAI_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _NUNMAI_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _NUNMAI_PYTHON_SCRIPTS
    )
...
def _argv_env_home(argv: Sequence[str]) -> Optional[str]:
    """``NUNMAI_HOME=<path>`` env-style assignment on the argv (``env NUNMAI_HOME=… nunmai …``)."""
    for token in reversed(list(argv)):
        if isinstance(token, str) and token.startswith("NUNMAI_HOME="):
            return token[len("NUNMAI_HOME="):]
    return None
...
def _store_install_layout(this_home: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    string test: an arbitrary ``<X>/profiles/<n>/`` tree is not a Nunmai install, and promoting
    ``<X>`` to "ours" swallows an unrelated instance living under it — the literal two-instance
    shape of #92401. The root store (``~/.nunmai/state.db``) is its own root with no profile name,
    ...
    try:
        ...
        if os.path.basename(this_home) == ".nunmai":
...
def _argv_home_selection(
    ...
    serves; its ``--nunmai-home``/``NUNMAI_HOME=``/``--profile``/``-p`` selection does. Same
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the NUNMAI_HOME root, so an absolute-path token
    containing a ``/.nunmai`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Nunmai home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.nunmai deferred
    ...
    2. The process's own ``--nunmai-home``/``NUNMAI_HOME=``/``--profile``/``-p``
    ...
    3. Path tokens. A token under ``<install root>/profiles/<other>`` is another
       ...
       part of an install layout (a custom ``NUNMAI_HOME`` is served BY the binary
       under ``~/.nunmai`` — dismissing on it admits maintenance under a live writer).
    """
    ...
    for index, normalized in path_tokens:
        ...
        if index == 0 and not argv0_locates_home:
            continue
        if "/.nunmai" in normalized or normalized.endswith("/.nunmai"):
            other_home_seen = True
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # NUNMAI_HOME would otherwise make every holder invisible and let maintenance proceed.
...
def in_process_state_db_holders(
    ...
    """
    from nunmai_state_registry import other_generations_for_path
...
def held_store_refusal(db_path: Path, *, command: str, force_hint: Optional[str] = "--force") -> Optional[str]:
    ...
    Running ``nunmai sessions optimize-storage`` underneath a fleet of live gateways put every agent into
    ...
    lines = [f"Refusing `nunmai sessions {command}`: another process is using {db_path}."]
    ...
    lines += [
        ...
        f"Stop them first (`nunmai {profile_arg}gateway stop`, quit the Desktop app, pause cron), then re-run.",
    ]
    if force_hint:
