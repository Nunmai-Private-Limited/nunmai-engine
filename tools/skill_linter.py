...
Design contract (matches the Nunmai "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    nunmai_meta = meta.get("nunmai") if isinstance(meta, dict) else None
    if not isinstance(nunmai_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.nunmai.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in nunmai_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.nunmai.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("nunmai", "agent", "nunmai agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Nunmai Engine' (proper caps) "
