...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    if not isinstance(nunmai_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.nunmai.{tags, related_skills}.")
    elif "tags" not in nunmai_meta:
        yield _warn("missing-metadata", "metadata.nunmai.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("nunmai", "agent", "nunmai agent") and (
        author != "Nunmai Engine"):
        yield _warn("author-caps", f"author '{author}' should be 'Nunmai Engine' (proper caps) "
