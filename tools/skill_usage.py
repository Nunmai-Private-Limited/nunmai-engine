...
Tracks per-skill usage metadata in a sidecar JSON file (~/.nunmai/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.nunmai/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.nunmai/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.nunmai/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``nunmai update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Nunmai metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.nunmai/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``nunmai curator
    list-archived`` to help users pass a name to ``nunmai curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``nunmai curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``nunmai curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``nunmai curator
...
def adopt_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    if is_bundled(skill_name):
        ...
        # claim Nunmai' own shipped skill was agent-authored and change nothing
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.nunmai/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.nunmai/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.nunmai/skills/<skill>/SKILL.md) and category-nested
    (~/.nunmai/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
