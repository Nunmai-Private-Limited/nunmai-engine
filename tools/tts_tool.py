...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.nunmai/config.yaml``. Nunmai
...
Configuration is loaded from ~/.nunmai/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``nunmai_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from nunmai_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``nunmai auth add <provider_id>``).
...
from tools.xai_http import nunmai_xai_user_agent
...
# Base URL now resolved via nunmai_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_nunmai_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.nunmai/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.nunmai/config.yaml.
    ...
    try:
        ...
        logger.debug("nunmai_cli.config not available, using default TTS config")
        return {}
...
# custom voice-cloning scripts, etc.) into Nunmai without any Python code
...
#           output_format: wav
#
# Nunmai writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__NUNMAI_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Nunmai secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Nunmai secrets (salvage of #56332) while still
    ...
    scrubbed = nunmai_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from nunmai the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``nunmai_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `nunmai setup` to configure, "
    ...
    from nunmai_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `nunmai model` or set XAI_API_KEY.")
    ...
    # output_format when Nunmai actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": nunmai_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import nunmai_cli as _nunmai_cli

            _nunmai_version = str(_nunmai_cli.__version__)
        except Exception:
            _nunmai_version = "0.0.0"
        # Include Nunmai client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Nunmai caches Piper voice models.

    Resolves to ``~/.nunmai/cache/piper-voices/`` under the active
    NUNMAI_HOME so voice downloads follow profile boundaries.
    """
    from nunmai_constants import get_nunmai_dir
    root = Path(get_nunmai_dir("cache/piper-voices", "piper_voices_cache"))
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("NUNMAI_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `nunmai setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run nunmai setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'nunmai setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'nunmai tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_nunmai_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
