...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``nunmai gateway restart|stop|uninstall``
    targeting nunmai-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_NUNMAI_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (nunmai
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``nunmai gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Nunmai. Perform authorized "
