...
# nunmai-engine#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from nunmai-engine#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    client stub reconnects on its side (NUNMAI_RPC_PERSISTENT).
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"nunmai_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "nunmai_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "nunmai_kernel_runner.py")
