...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    return {
        ...
        "chat_name": get_session_env("NUNMAI_SESSION_CHAT_NAME") or None, "thread_id": thread_id,
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    TUI/CLI sessions cannot be captured as a cron ``origin`` (no ``NUNMAI_SESSION_PLATFORM``/``CHAT_ID`` is
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    from NUNMAI_CRON_AUTO_DELIVER_*, or ``local`` when it has no concrete target; other
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    """Scripts must be relative paths within NUNMAI_HOME/scripts/ (absolute / ~ / drive-letter
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``nunmai_cli.cron._builtin_gateway_liveness`` (#87033) so the
