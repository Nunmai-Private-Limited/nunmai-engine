...
# dependency could read every Nunmai secret from process.env.
...
# than a concrete path (also compared in nunmai_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``nunmai_home_key()``): the multiplexed
...
def _socket_safe_tmpdir() -> str:
    ...
    (``nunmai_constants.socket_safe_tmpdir``)."""
...
# Owner Nunmai home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Nunmai' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nunmai_home_override': ('nunmai_constants', 'get_nunmai_home_override'),
    'nunmai_home_key': ('nunmai_constants', 'nunmai_home_key'),
    ...
    'node_tool_runnable': ('nunmai_constants', 'node_tool_runnable'),
