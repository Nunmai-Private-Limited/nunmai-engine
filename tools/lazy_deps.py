"""Lazy dependency installer for opt-in Nunmai backends.
...
(never system Python); durable-target mode (``NUNMAI_LAZY_INSTALL_TARGET``, sealed images)
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey Nunmai") engines (sync with the `wake` extra) ──────
    ...
    # made _is_satisfied() reject every newer compatible release, so `nunmai update` kept downgrading a
    ...
    # HF Agent Trace Viewer upload (nunmai trace upload / /upload-trace). huggingface-hub is a SHARED
    ...
    # from mere package presence, the `nunmai update` lazy-refresh pass re-asserts THIS pin on every install
...
_LAZY_TARGET_ENV = "NUNMAI_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    modes; (2) the sealed venv (``NUNMAI_DISABLE_LAZY_INSTALLS=1``) blocks only without a durable
    ...
    if cfg is not None and not bool((cfg.get("security") or {}).get("allow_lazy_installs", True)):
        return False
    if os.environ.get("NUNMAI_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return ("unsupported on Windows: Matrix E2EE depends on python-olm, which has no Windows wheel and "
                "requires make + libolm to build from sdist. Run Nunmai under WSL to use Matrix on Windows.")
    return None
...
def _is_satisfied(spec: str) -> bool:
    """Present AND inside the spec's version range, so ``nunmai update`` propagates pin bumps to
...
def _core_constraints_file() -> Optional[Path]:
    ...
    try:
        ...
        if not pins:
            return None
        fd, path = tempfile.mkstemp(prefix="nunmai-core-constraints-", suffix=".txt")
...
# Whose dependency-security policy an install runs under. ``core``: Nunmai's own packages (LAZY_DEPS
# extras, refreshed by ``nunmai update``) resolve under the checkout's ``[tool.uv]`` policy — the 14-day
...
# by our 14 day rule; they can have their own security policy on that. Only Nunmai' dependencies themselves
# have to"), so Nunmai's project config is not applied — a plugin floored on a release younger than 14 days
# would otherwise be uninstallable through Nunmai while installing fine everywhere else.
...
def _uv_binary() -> Optional[str]:
    """Managed uv first ($NUNMAI_HOME/bin is never on PATH), then PATH. A lookup, not ensure_uv():
...
def _write_constraints_file(lines: tuple[str, ...]) -> Path:
    import tempfile
    fd, path = tempfile.mkstemp(prefix="nunmai-plugin-constraints-", suffix=".txt")
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    return Path(path)
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300, constraint_lines: tuple[str, ...] = (),
    ...
    ``nunmai_cli.tools_config._pip_install`` (no CLI dependency).

    *constraint_lines* pins the resolver (plugin installs pass Nunmai' own declared ranges so a plugin
    ...
    try:
        ...
        uv_env = nunmai_subprocess_env(inherit_credentials=False)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    if _lazy_install_target() is None:
        ...
        with contextlib.suppress(Exception):
            from nunmai_cli.config import get_managed_system
            managed_by = get_managed_system()
        if managed_by:
            raise FeatureUnavailable(
                feature, missing,
                f"unsupported on {managed_by}-managed installs: this build's packages come from {managed_by}, "
                f"so Nunmai cannot install them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Nunmai instead).")
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300,
    ...
    resolves; *policy* defaults to ``"plugin"`` (the plugin's own dependency policy, not Nunmai's
    ...
    target = _lazy_install_target()
    if not _allow_lazy_installs():
        sealed = os.environ.get("NUNMAI_DISABLE_LAZY_INSTALLS") == "1" and target is None
        reason = ("runtime installs are disabled on this deployment: the agent environment is immutable "
                  "and no writable install target is configured (NUNMAI_LAZY_INSTALL_TARGET)"
...
def active_features() -> list[str]:
    ...
    asyncpg are deliberately not proof a backend was enabled. Drives ``nunmai update``."""
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    """Re-run ``ensure`` for every active feature (``nunmai update``); returns
