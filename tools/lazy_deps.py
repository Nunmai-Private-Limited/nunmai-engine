"""
Lazy dependency installer for opt-in Nunmai Engine backends.

Many Nunmai features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``nunmai-engine[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``nunmai tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``NUNMAI_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/nunmai`` read-only), setting
  ``NUNMAI_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Nunmai, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (NUNMAI_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (nunmai trace upload / /upload-trace).
    #
    ...
    # presence, the `nunmai update` lazy-refresh pass re-asserts THIS pin on
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``NUNMAI_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``nunmai update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Nunmai under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``nunmai update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``nunmai_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        # which() misses the uv Nunmai installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Nunmai
    ...
    # deployment sets NUNMAI_MANAGED=true *and* NUNMAI_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Nunmai cannot install "
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``NUNMAI_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``NUNMAI_DISABLE_LAZY_INSTALLS=1``) and sets
      ``NUNMAI_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``nunmai update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``nunmai update``. Never raises; lazy-install failures
