...
Registered only under the dispatcher (``NUNMAI_KANBAN_TASK`` set) or when the profile
enables the ``kanban`` toolset. Tools rather than ``nunmai kanban`` shell-outs: they run
...
from nunmai_cli.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever NUNMAI_KANBAN_* is present but not ours."""
...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    (NUNMAI_KANBAN_TASK) per flag; else the profile toolset decides."""
    if _is_delegated_child_context():
        return False
    if os.environ.get("NUNMAI_KANBAN_TASK") and _is_dispatcher_owned_worker():
...
# (NUNMAI_KANBAN_RUN_ID unresolvable) must not run these: expected_run_id=None
...
def _persisted_identity() -> str:
    ...
    ``nunmai_cli.profiles.current_profile_name`` resolves the profile this call runs FOR — the bound
    home override under a multiplexed tick or turn, else the dispatcher's ``NUNMAI_PROFILE`` pin,
    ...
    """
    from nunmai_cli.profiles import current_profile_name
...
def _reject_delegated_child_mutation(tool_name: str) -> None:
    """A delegate_task child shares the parent's process, so inherited NUNMAI_KANBAN_*
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if _is_delegated_child_context() or not _is_dispatcher_owned_worker():
        return None
    return os.environ.get("NUNMAI_KANBAN_TASK") or None


def _require_task_id(args: dict) -> str:
    tid = _default_task_id(args.get("task_id"))
    _check(tid, "task_id is required (or set NUNMAI_KANBAN_TASK in the env)")
    return tid


def _own_task_env(task_id: str, var: str) -> Optional[str]:
    """``$var`` only when this worker is scoped to ``task_id``; else None."""
    return os.environ.get(var) if os.environ.get("NUNMAI_KANBAN_TASK") == task_id else None


def _worker_run_id(task_id: str) -> Optional[int]:
    """This worker's dispatcher run id when it is scoped to task_id."""
    raw = _own_task_env(task_id, "NUNMAI_KANBAN_RUN_ID")
    try:
...
def _stamp_worker_session_metadata(task_id: str, metadata: Optional[dict]) -> Optional[dict]:
    ...
    session_id = _own_task_env(task_id, "NUNMAI_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> None:
    """A dispatcher-spawned worker may only mutate its own NUNMAI_KANBAN_TASK; a
    ...
    """
    env_tid = os.environ.get("NUNMAI_KANBAN_TASK")
...
def _worker_guard(tool_name: str, args: dict) -> str:
    ...
    A dispatcher-spawned worker (``NUNMAI_KANBAN_TASK`` set) that cannot name
    ...
    and fails closed. CLI / human / orchestrator paths (no ``NUNMAI_KANBAN_TASK``)
    ...
    if (
        ...
        and os.environ.get("NUNMAI_KANBAN_TASK")
        and _worker_run_id(tid) is None
    ):
        raise _Reject(
            f"{tool_name} refused: this worker cannot resolve its "
            "NUNMAI_KANBAN_RUN_ID, so it cannot prove ownership of the card's "
...
def _board(board: Optional[str], *, quiet_close: bool = False):
    ...
    from nunmai_cli import kanban_db as kb
    from nunmai_cli import kanban_db_connect as kbc
...
# worker context (no ``NUNMAI_KANBAN_TASK``). - No durable note on these auto-heartbeats; that's reserved
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    succeed. ``NUNMAI_KANBAN_RUN_ID`` pins the run row so a reclaimed stale run is not
    heartbeated; ``NUNMAI_KANBAN_CLAIM_LOCK`` absent -> default claimer (local workers)."""
    global _auto_heartbeat_last_attempt, _auto_heartbeat_fence_warned
    tid = os.environ.get("NUNMAI_KANBAN_TASK")
    now = time.monotonic()
    ...
    try:
        from nunmai_cli import kanban_db_dispatch as kbd
        with _board(None, quiet_close=True) as (kb, conn):
            ops = ((kb.heartbeat_claim, {"claimer": os.environ.get("NUNMAI_KANBAN_CLAIM_LOCK")}),
            ...
            for fn, kwargs in ops:
                ...
                try:
                    ...
                    # inherited NUNMAI_DELEGATED_CHILD_CONTEXT next to NUNMAI_KANBAN_TASK, so it is
                    ...
                    if not _auto_heartbeat_fence_warned:
                        ...
                        logger.warning(
                            ...
                            "NUNMAI_DELEGATED_CHILD_CONTEXT together with NUNMAI_KANBAN_TASK, so the board "
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    tid = os.environ.get("NUNMAI_KANBAN_TASK") if _is_dispatcher_owned_worker() else None
    now = time.monotonic()
    ...
    # when the dispatcher did not pin NUNMAI_PROFILE (echoed notes would otherwise
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        try:
            ...
            return tool_error(
                ...
                f"to finish, or an operator can run `nunmai kanban complete --force {tid}`.")
...
def _handle_request_review(args: dict, **kw) -> str:
    ...
    if reviewer:
        from nunmai_cli.profiles import list_profile_names, profile_exists
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    from nunmai_cli import kanban_db_dispatch as kbd
    with _board(args.get("board")) as (kb, conn):
        # The dispatcher pins NUNMAI_KANBAN_CLAIM_LOCK at spawn; the default
        # claimer covers locally-driven workers that bypassed the dispatcher.
        kb.heartbeat_claim(conn, tid, claimer=os.environ.get("NUNMAI_KANBAN_CLAIM_LOCK"))
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # args["author"] override could forge a directive from ``nunmai-system``.
    ...
    # comment from an authoritative-looking name like ``nunmai-system`` and poison the future-worker context
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        chunks: list[bytes] = []
        total = 0
        with httpx.stream("GET", current_url, headers={"User-Agent": "nunmai-kanban/attach"},
...
def _persisted_session_id(session_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        state = SessionDB(db_path=get_nunmai_home() / "state.db", read_only=True)
...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        self_tid = (os.environ.get("NUNMAI_KANBAN_TASK")
        ...
        session_id = (_persisted_session_id(args.get("session_id"))
                      ...
                      or _persisted_session_id(get_session_env("NUNMAI_SESSION_ID", "")))
        ...
        new_tid = kb.create_task(
            ...
            parents=tuple(parents), tenant=args.get("tenant") or os.environ.get("NUNMAI_TENANT"),
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    Gateway sessions: ``NUNMAI_SESSION_PLATFORM``/``CHAT_ID`` ContextVars. TUI/desktop:
    those are cleared but the subprocess inherits ``NUNMAI_SESSION_KEY`` -> ``platform="tui"``
    for the TUI poller. ``NUNMAI_SESSION_ID`` is deliberately NOT a fallback: it is set for
    ...
    platform, chat_id = env("NUNMAI_SESSION_PLATFORM", ""), env("NUNMAI_SESSION_CHAT_ID", "")
    if not platform or not chat_id:
        session_key = env("NUNMAI_SESSION_KEY", "") or os.environ.get("NUNMAI_SESSION_KEY", "")
        if not session_key:
            return None
        platform, chat_id = "tui", session_key
    chat_type = env("NUNMAI_SESSION_CHAT_TYPE", "") or None
    thread_id = env("NUNMAI_SESSION_THREAD_ID", "") or None
    message_id = env("NUNMAI_SESSION_MESSAGE_ID", "") or ""
    notifier_profile = env("NUNMAI_SESSION_PROFILE", "")
    ...
    delivery_metadata: dict[str, Any] = {
        k: v for k, v in (
            ...
            ("scope_id", env("NUNMAI_SESSION_SCOPE_ID", "")),
            ("parent_chat_id", env("NUNMAI_SESSION_PARENT_CHAT_ID", "")),
        ) if v}
    ...
    return dict(
        ...
        user_id=env("NUNMAI_SESSION_USER_ID", "") or None,
        user_id_alt=env("NUNMAI_SESSION_USER_ID_ALT", "") or None,
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        from nunmai_cli import kanban_db_notify as _kbn
