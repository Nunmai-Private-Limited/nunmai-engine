...
def heartbeat_current_worker_from_env() -> bool:
    ...
    attempted. ``NUNMAI_KANBAN_RUN_ID`` pins the run row so a reclaimed stale run is not
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    steer was injected; never raises. Own comments (``NUNMAI_PROFILE``) are skipped."""
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # forge a directive from ``nunmai-system``. Cross-task commenting stays unrestricted —
...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        session_id = (args.get("session_id") or (self_task.session_id if self_task else None)
                      or _current_origin_session_id() or os.environ.get("NUNMAI_SESSION_ID"))
        ...
        new_tid = kb.create_task(
            ...
            created_by=os.environ.get("NUNMAI_PROFILE") or "worker", session_id=session_id)
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    notifier_profile = env("NUNMAI_SESSION_PROFILE", "") or os.environ.get("NUNMAI_PROFILE")
