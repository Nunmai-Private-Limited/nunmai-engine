...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not restore:
        action, message = "manifest_cleared", (f"Cleared manifest entry for '{name}'. Future `nunmai update` runs "
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Bundled skills ``nunmai update`` keeps because the user edited them (same test the sync
...
_OPT_OUT_MESSAGES = {  # (enabled, changed) -> message
    ...
    (False, True): "Opted back in. The next `nunmai update` (or `nunmai skills opt-in --sync`) will re-seed bundled skills.",
    (False, False): "Not opted out — no marker to remove."}


def set_bundled_skills_opt_out(enabled: bool) -> dict:
    """Toggle the .no-bundled-skills marker (on-disk half of ``nunmai skills opt-out`` / ``opt-in``;
    removing present skills is ``remove_pristine_bundled_skills``)."""
    ss = _ss()
    marker = ss._nunmai_home() / ss.NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            ss._nunmai_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding (`nunmai skills opt-out`).\n"
