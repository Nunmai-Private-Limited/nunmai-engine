...
The LLM writes a Python script that calls Nunmai tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Nunmai does.
...
def _missing_nunmai_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from nunmai_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "nunmai_tools may be first on sys.path. Check nunmai_tools.__file__ "
...
def generate_nunmai_tools_module(enabled_tools: List[str],
    ...
    """Source of the nunmai_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage nunmai_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/nunmai_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/nunmai_tools.py",
                             generate_nunmai_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"NUNMAI_RPC_DIR={quoted_rpc_dir} NUNMAI_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (nunmai-engine#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Nunmai tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.nunmai.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from nunmai_tools import json_parse, shell_quote, retry`. "
