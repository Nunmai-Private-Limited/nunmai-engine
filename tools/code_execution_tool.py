...
Lets the LLM write a Python script that calls Nunmai tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `nunmai_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `nunmai_tools.py` with file-based RPC stubs
...
# available on every platform Nunmai itself runs on.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nunmai_dir("cache/exec", "exec_spill")
...
# match a safe prefix, the operational NUNMAI_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Nunmai
# code can lose the DB-layer Kanban mutation guard while still inheriting
# NUNMAI_HOME.
#
# NB: the broad "NUNMAI_" prefix was deliberately removed (#27303) — it leaked
# NUNMAI_*-named config that lacks a secret substring (e.g. NUNMAI_BASE_URL,
# NUNMAI_KANBAN_DB, NUNMAI_*_WEBHOOK).  The child only needs the few
# location/profile vars in _NUNMAI_CHILD_ALLOWED below; NUNMAI_RPC_SOCKET /
# NUNMAI_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational NUNMAI_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set nunmai_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_NUNMAI_CHILD_ALLOWED = frozenset({
    "NUNMAI_HOME",
    "NUNMAI_PROFILE",
    "NUNMAI_CONFIG",
    "NUNMAI_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational NUNMAI_* vars (_NUNMAI_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret NUNMAI_* vars dropped by the tightened allowlist (#27303). The
    # broad "NUNMAI_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like NUNMAI_KANBAN_DB / NUNMAI_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _NUNMAI_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("NUNMAI_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped NUNMAI_* var.
            _dropped_nunmai.append(k)
    if _dropped_nunmai:
        logger.debug(
            "execute_code: dropped %d non-allowlisted NUNMAI_* var(s) from the "
...
# ---------------------------------------------------------------------------
# nunmai_tools.py code generator
# ---------------------------------------------------------------------------
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    nunmai_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'nunmai_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_nunmai_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the nunmai_tools.py stub module.
...
"""Auto-generated Nunmai tools RPC stubs."""
...
def _connect():
    ...
    NUNMAI_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["NUNMAI_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("NUNMAI_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (NUNMAI_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("NUNMAI_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated Nunmai tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("NUNMAI_RPC_DIR") or os.path.join(tempfile.gettempdir(), "nunmai_rpc")
_seq = 0
...
def _execute_remote(
    ...
    sandbox_dir = f"{temp_dir}/nunmai_exec_{sandbox_id}"
    ...
    try:
        ...
        # --- Session-kernel path (nunmai-engine#96873) -------------------
        ...
        tools_src = generate_nunmai_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/nunmai_tools.py", tools_src)
        ...
        env_prefix = (
            f"NUNMAI_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"NUNMAI_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("NUNMAI_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["NUNMAI_RPC_SOCKET"] = rpc_endpoint
    child_env["NUNMAI_RPC_TOKEN"] = rpc_token
    ...
    # NUNMAI_TIMEZONE is an internal Nunmai setting and must not leak
    # into child processes.
    _tz_name = os.getenv("NUNMAI_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("NUNMAI_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``nunmai_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Nunmai's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Nunmai's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Nunmai-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Nunmai-
    ...
    _strip_nunmai_owned_pythonpath(child_env)
    _nunmai_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_nunmai_python_environment(child_python):
        _pp_parts.append(_nunmai_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import nunmai_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Nunmai "
            "environment; nunmai root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.nunmai.gateway`, but the
    ...
    # --- Set up temp directory with nunmai_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="nunmai_sandbox_")
    ...
    # selector from NUNMAI_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"nunmai_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated nunmai_tools module.
        ...
        tools_src = generate_nunmai_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "nunmai_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  NUNMAI_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.nunmai/.env')).
...
def _load_config() -> dict:
    ...
    ``nunmai --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (nunmai-engine's python). Reproducible and the
...
# Interpreter paths already reported as outside the Nunmai environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the nunmai root from every
...
def _uses_nunmai_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Nunmai's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``nunmai tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and nunmai-engine's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.nunmai/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Nunmai's own python (the common case — stdlib plus Nunmai's "
    ...
    description = (
        "Run Python that calls Nunmai tools programmatically. Use when you "
        ...
        "loses that state.\n\n"
        f"Available via `from nunmai_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from nunmai_tools import {import_str}` "
