"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Nunmai.
...
delegates every real request to Nunmai as its *client-delegation* backend. Nunmai stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Nunmai' answer is what the voice speaks.
...
# backend (Nunmai) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Nunmai, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Nunmai agent: a full AI agent with tools — it can run commands, read and edit files, "
...
def _resolve_credentials(live: Dict[str, Any]) -> tuple[str, str]:
    ...
    The Nunmai-managed audio proxy does not carry ``/live/sessions``; this mode is direct-key only.
    """
