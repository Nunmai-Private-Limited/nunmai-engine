...
def _resolve_stt_language(
    ...
    (plus *extra_keys* aliases, e.g. ``language_code``) > ``stt.language`` > ``NUNMAI_LOCAL_STT_LANGUAGE``
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Whether *provider* is exempt from Nunmai's remote upload cap."""
...
def _get_provider(stt_config: dict) -> str:
    ...
    # The managed "Nunmai Subscription" selection is the OpenAI backend routed via the managed gateway.
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(target=_watch, name="nunmai-stt-idle-unload", daemon=True)
        ...
        if not _is_local_stt_provider(provider, stt_config):
            ...
            if Path(file_path).suffix.lower() == ".caf":
                work_dir = cleanup.enter_context(
                    TemporaryDirectory(prefix="nunmai-caf-", ignore_cleanup_errors=True)
                )
...
def _no_provider_error(provider: str, stt_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Surface it — with its `nunmai tools` remediation — instead of the all-provider setup hint (#93045).
