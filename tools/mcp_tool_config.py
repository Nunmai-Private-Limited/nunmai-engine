...
def _get_mcp_stderr_log() -> Any:
    ...
    from nunmai_constants import get_nunmai_home, nunmai_home_key, mkdir_under_nunmai_home
    ...
    with _mcp_stderr_log_lock:
        ...
        if fh is None or fh.closed:
            try:
                ...
                mkdir_under_nunmai_home(log_dir)
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from nunmai_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_BOARD"):
...
def _node_fallback(command: str, *, windows: Optional[bool] = None) -> str:
    ...
    The managed tree comes from ``iter_nunmai_node_dirs`` (Windows unpacks into ``<home>\\node``, POSIX into
    ``<home>/node/bin``) under the active profile's ``get_nunmai_home()``; on Windows the real files are
    ...
    # /usr/local/bin: canonical Node location (from-source Linux, Nunmai Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    directories = [*map(str, iter_nunmai_node_dirs(get_nunmai_home())), os.path.join(home, ".local", "bin"),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``nunmai_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Nunmai already supervises the
