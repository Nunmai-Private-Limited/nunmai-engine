...
Ships **no in-tree provider**: enable a plugin and select it in ``nunmai tools`` → Video
    ...
    if configured:
        return json.dumps(error_response(
            error=(f"video_gen.provider='{configured}' is set but no plugin registered that name. "
                   f"Run `nunmai plugins list` to see installed video gen backends, or "
                   f"`nunmai tools` → Video Generation to pick one."),
            error_type="provider_not_registered", provider=configured))
    return json.dumps(error_response(
        error=("No video generation backend is configured. Run `nunmai tools` → "
