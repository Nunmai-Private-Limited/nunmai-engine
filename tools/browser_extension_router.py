...
- **No server-bound identity ⇒ legacy.** Generic Nunmai callers keep the
...
def extension_controller_available(action: str) -> bool:
    ...
    try:
        ...
        if not browser_control_enabled():
            return False
        session_id = get_session_env("NUNMAI_SESSION_ID", "") or None
        principal_id = get_session_env("NUNMAI_BROWSER_CONTROL_PRINCIPAL", "") or None
...
def routed_browser_handler(
    ...
    *,
    ...
    try:
        ...
        session_id = session_id or get_session_env("NUNMAI_SESSION_ID", "") or None
