...
it is automatically downloaded from GitHub releases to $NUNMAI_HOME/bin/tirith.
...
def _get_nunmai_home() -> str:
    """Return the Nunmai home directory, respecting NUNMAI_HOME env var."""
    return str(get_nunmai_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nunmai_home(), ".tirith-install-failed")
...
def _nunmai_bin_dir() -> str:
    """Return $NUNMAI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nunmai_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NUNMAI_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NUNMAI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NUNMAI_HOME/bin/tirith
    ...
    if os.path.isfile(nunmai_bin) and os.access(nunmai_bin, os.X_OK):
        _resolved_path = nunmai_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nunmai_bin
