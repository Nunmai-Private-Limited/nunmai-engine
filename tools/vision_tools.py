...
Nunmai, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
# ``agent.auxiliary_client`` pulls credential_pool → nunmai_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("NUNMAI_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: NUNMAI_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("NUNMAI_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_nunmai_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_nunmai_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $NUNMAI_HOME/cache/vision/ and cleaned up
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Nunmai, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def _materialize_video_from_terminal_backend(video_source: str, task_id: Optional[str]) -> Path:
    ...
    temp_dir = get_nunmai_dir("cache/video", "temp_video_files")
    temp_dir.mkdir(parents=True, exist_ok=True)
