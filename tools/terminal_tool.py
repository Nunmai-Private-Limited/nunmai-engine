...
processes survive cleanup, idle reaping, or Nunmai exit.
...
def _current_session_profile() -> str:
    """Active session's Nunmai profile name, or "" (same lookup discipline as
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``nunmai-engine=1`` for the current
    profile — leftovers of Nunmai processes that died without firing
    ...
    Nunmai processes sharing a profile) and the once-per-interpreter flag so
...
def _routed_home_task_key(profile_scoped: bool) -> Optional[str]:
    ...
    """
    from nunmai_constants import get_nunmai_home_override, profile_name_for_home
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``nunmai serve``, Desktop
    ...
    env values (possibly stale from ``nunmai setup``); env values for omitted keys
    ...
    context-local ``NUNMAI_HOME`` override (multiplexed dashboard / gateway
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Nunmai-owned control-plane
