...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Nunmai logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Nunmai' logging
        (agent.log via nunmai_logging), tagged with the server name. Without this, the SDK's default
