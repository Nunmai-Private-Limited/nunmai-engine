...
# ladders, parked-server self-probes and repeated `nunmai mcp test` runs re-run the preflight
...
# The cache is also persisted under the Nunmai home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``nunmai_constants.get_nunmai_home()`` so the cache follows the
    active profile and is isolated across Nunmai homes. The cache directory
    is created on demand. Returns ``None`` when Nunmai home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "nunmai-engine-osv-check/1.0"},
        method="POST")
