...
# Config-based allowlist, keyed by Nunmai home: under gateway.multiplex_profiles one process serves
...
def _is_nunmai_provider_credential(name: str) -> bool:
    """True if ``name`` is a Nunmai-managed provider credential per
    ``_NUNMAI_PROVIDER_ENV_BLOCKLIST`` or a dynamic Nunmai-internal secret
    ...
    in the ``execute_code`` child); non-Nunmai keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _is_nunmai_internal_secret, _is_provider_env_blocklisted)
    ...
    return _is_nunmai_internal_secret(name) or _is_provider_env_blocklisted(name)


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). Nunmai-managed provider credentials
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Nunmai provider credentials; refused
...
def scoped_passthrough_additions(present: Iterable[str]) -> dict[str, str]:
    ...
    ``os.environ`` (``load_nunmai_dotenv`` skips the process-global load for a routed home), so a
