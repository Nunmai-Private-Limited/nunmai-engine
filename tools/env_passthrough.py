...
def _is_nunmai_provider_credential(name: str) -> bool:
    """True if ``name`` is a Nunmai-managed provider credential (API key,
    token, or similar) per ``_NUNMAI_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Nunmai API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Nunmai credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Nunmai-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_nunmai_internal_secret(name):
        return True
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Nunmai-managed provider credentials (from
    ``_NUNMAI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Nunmai-managed
    ...
    Non-Nunmai third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_nunmai_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Nunmai provider "
                "credential %r (blocked by _NUNMAI_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Nunmai-managed provider credentials must not be passed
                ...
                if _is_nunmai_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Nunmai "
                        "provider credential %r from config.yaml (blocked "
                        "by _NUNMAI_PROVIDER_ENV_BLOCKLIST). Operator "
