...
broad, declarative). New skills land in ~/.nunmai/skills/ (or ``skills.create_dir``);
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live NUNMAI_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch NUNMAI_HOME and later bind a different profile per session (#40677).
    """
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    if len(others) == 1:
        ...
        base += (
            ...
            f"it, switch profiles (`nunmai -p {other_profile}`) or edit the file directly "
            f"(file tools / terminal).")
    elif others:
        names = ", ".join(f"'{p}'" for p, _ in others)
        base += (
            f" Skills by that name exist in other profiles: {names}. Switch profiles (`nunmai -p "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path, before: Optional[str] = None) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match Nunmai skill standards.")
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    from nunmai_constants import mkdir_under_nunmai_home
    mkdir_under_nunmai_home(skill_dir)
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    if unsafe := _validate_delete_target(skill_dir):  # defense-in-depth before rmtree
        return _err(unsafe)
    # Curator consolidations must be RECOVERABLE (`nunmai curator restore`): archive instead
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
    ...
    # record as STATE_ARCHIVED (`nunmai curator status`/`restore`); only a hard delete forgets.
    with suppress(Exception):
        ...
        # ~/.nunmai/skills/.archive/ is documented as the maximum destructive action the curator may take,
        # and `nunmai curator restore` promises the skill can be brought back. Route through the recoverable
