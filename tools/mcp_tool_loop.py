...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if get_nunmai_home_override() is not None:
            lock_path = str(get_nunmai_home() / ".mcp-discovery.lock")
        else:
            if _origin._MCP_DISCOVERY_LOCK_PATH is None:
                _origin._MCP_DISCOVERY_LOCK_PATH = str(get_nunmai_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local NUNMAI_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from nunmai_constants import get_nunmai_home_override, reset_nunmai_home_override, set_nunmai_home_override
