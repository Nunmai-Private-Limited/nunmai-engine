...
def build_profile_terminal_scope(nunmai_home: "Any") -> Dict[str, str]:
    ...
    if config_exists:
        from nunmai_cli.config import fast_safe_load
...
def install_profile_terminal_scope(nunmai_home: "Any") -> Token:
    """Build AND install a profile's policy; on failure install the refusal scope. Never raises."""
    try:
        return set_terminal_scope(build_profile_terminal_scope(nunmai_home))
