...
def _default_home() -> str:
    return os.getenv("NUNMAI_HOME") or os.path.expanduser("~/.nunmai")
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('nunmai' → 'default')."""
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    try:
        from tools.bot_mode_probe import (
            BOT_CHAT_TITLE, _handle, _nunmai_root, _peers, _profile_name as _self_profile_name, _roster,
            is_bot_mode_managed,
        )
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        return _start_delivery(["nunmai", "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery(["nunmai", "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
