...
teammate agent (another Nunmai profile on this install, or an agent on a
...
DM each other via a prompt-injected ``nunmai -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``nunmai -p`` / ``nunmai peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``nunmai -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``nunmai peer dm <peer>[/<name>] < <tmp>``
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "nunmai-dm-*.txt"),
        (temp_root, "nunmai-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``nunmai -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative nunmai next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("nunmai", "nunmai.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("NUNMAI_HOME") or os.path.expanduser("~/.nunmai"))
    return acquire_turn_lock(_nunmai_root(home), argv[2])
