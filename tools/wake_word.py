...
# Minimum gap between two wake fires, so one "hey nunmai" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey nunmai", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey nunmai" model (tools/wakewords/) — the default; alias names resolve
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Nunmai backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `nunmai tools` "
