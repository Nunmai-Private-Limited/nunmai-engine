...
class MCPServerRunMixin:
    ...
    def _log_park(self, msg: str, *args) -> None:
        ...
        transition — ``nunmai mcp list`` already surfaces the parked state, so one identical
    ...
    async def _park(self, revival_reason: str) -> bool:
        ...
        while True:
            ...
            # refresh or `nunmai mcp login` — still revives immediately regardless of the
    ...
    async def _prepare_run(self, config: dict) -> bool:
        ...
        # routes it through Nunmai' approval system.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # exit the reconnect loop and the MCP server would stay dead until Nunmai is fully
                # restarted. See #9930.
                self.session = None
                raise
            except Exception as exc:
    ...
    async def _on_initial_connect_error(self, exc: Exception, root: BaseException,
        ...
        if failure_class == "permanent":
            ...
            detail = (f"authentication, parking until credentials change; re-authenticate with "
                      f"`nunmai mcp login {self.name}`" if _errors._is_auth_error(root)
