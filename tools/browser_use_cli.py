...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Nunmai's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Nunmai's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_nunmai_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `nunmai tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Nunmai' own bin dir ($NUNMAI_HOME/bin) — where install.sh puts uv/uvx
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Nunmai' managed uv (bootstrapped on demand via
    ``nunmai_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$NUNMAI_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Nunmai-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through nunmai tools).
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `nunmai tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Nunmai launches on a
    SNAPSHOT of their real profile (see nunmai_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Nunmai-managed copy of "
