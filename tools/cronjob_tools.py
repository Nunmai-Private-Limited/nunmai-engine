...
# `cronjob(action="run")` executes in-process (comfortably below NUNMAI_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default NUNMAI_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with NUNMAI_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (NUNMAI_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with NUNMAI_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `nunmai cron run` that died mid-run). The ticker does this at startup; one-shot
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `nunmai cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    # Reap BEFORE the async/sync branch: the one-shot `nunmai cron run` path returns early
    ...
    # completion, so don't claim-and-dispatch (direct callers like `nunmai cron run` exit right after).
...
def _cronjob_schema_overrides() -> dict:
    ...
    params["properties"]["script"]["description"] = _script_description(display_nunmai_home())
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Jobs run on the main agent model (whatever `nunmai model` is set to when they fire) unless the user pins one. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "pinned": {
                "type": "boolean",
                "description": "For create/update. ONLY set when the user explicitly asks to pin (or unpin) a job's model. pinned=true locks the CURRENT main agent model (and its provider) onto the job so later `nunmai model` / `/model` changes never touch it; pinned=false releases the lock so the job follows the main agent model again. Never set it on your own initiative: by default jobs follow the main model."
            },
            "prompt": {
                "type": "string",
            ...
            "script": {
                "type": "string",
                "description": _script_description("the profile NUNMAI_HOME")
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        ...
        or is_truthy_value(get_session_env("NUNMAI_CRON_SESSION", ""))
    )


# Agent-facing arguments forwarded verbatim to cronjob(). model / provider / base_url are
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `nunmai cron
