...
# ---------------------------------------------------------------------------
# When Nunmai runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=nunmai-worker-<pid>``
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Nunmai gateway runtime.

    Both supervisor markers and ``_NUNMAI_GATEWAY`` are inherited by every
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"nunmai-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from nunmai_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"nunmai-worker-{session.id}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/nunmai_bg_{session.id}.log"
        pid_path = f"{temp_dir}/nunmai_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/nunmai_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``nunmai_cli.goals``) to support
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``nunmai -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``nunmai -p <bot> chat -Q
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `nunmai update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Nunmai desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.nunmai/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Nunmai'}"
    elif _reason == "lost":
