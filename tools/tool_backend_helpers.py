...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``nunmai auth add
    ...
    2. The environment / ``~/.nunmai/.env``. Under a multiplexed gateway turn
       ...
       ``nunmai_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``nunmai auth
    ...
    when omitted, ``nunmai_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `nunmai auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``nunmai auth add openai-api``) via
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via nunmai "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.nunmai/.env`` (via
    ``nunmai_cli.config.get_env_value`` when available) so tool-side
