...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Coarse gate: entitled to the Nunmai Tool Gateway (paid Portal access OR a live free
...
def nous_tool_gateway_unavailable_message(capability: str = "the Nunmai Tool Gateway", *,
    ...
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `nunmai model` to refresh your "
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``nunmai_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def _env_source_suppressed(provider_id: str, env_var: str) -> bool:
    """``nunmai auth remove <provider>`` records ``env:<VAR>`` in ``suppressed_sources`` and promises
    the variable is ignored until ``nunmai auth add``; a value still living in a long-running
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``nunmai_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``nunmai auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``nunmai
...
# Provider value the managed "Nunmai Subscription" picker rows write for every category;
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `nunmai tools` selection: ``"nous"`` (managed
...
# (nunmai_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via nunmai "
            f"tools), but {failure}. Run 'nunmai tools' to change it.")
