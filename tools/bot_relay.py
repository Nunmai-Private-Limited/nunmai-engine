...
Nunmai Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own NUNMAI root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in nunmai_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "nunmai" if profile == "default" else profile
    if (
...
def _nunmai_cli() -> str:
    """Resolve the nunmai CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``nunmai``
    entrypoint. A bare ``"nunmai"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("nunmai.exe" if sys.platform == "win32" else "nunmai")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("nunmai")
    if found:
        return found
    return "nunmai"
...
# turns concurrently: deliveries spawn separate ``nunmai`` subprocesses, so
