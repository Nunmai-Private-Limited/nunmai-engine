...
from tools.bot_mode_probe import _default_home, _nunmai_root, alias_forms
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/nunmai-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``nunmai -p <profile> *BOT_CHAT_TURN_ARGS``.
...
# (``nunmai_cli.quiet_single_query.adopt_unanswered_turn``, which consumes the variable before the turn).
RESUME_UNANSWERED_TURN_ENV = "NUNMAI_RESUME_UNANSWERED_TURN"
...
def _ensure_dirs(root: Path | str) -> Path:
    ...
    for sub in (OUTBOX_DIR, CLAIMED_DIR, REPLIES_DIR):
        ...
        mkdir_under_nunmai_home(base / sub)
    return base
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import nunmai_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("nunmai" if profile == "default" else profile)
...
def _target_aliases(row: dict) -> set[str]:
    ...
    ``default`` is ``@nunmai`` on every gateway, so its title is the only bare form that can single it out."""
...
def _title_slug(row: dict) -> str:
    ...
    title is empty, reserved (a bot titled "Nunmai") or not a valid handle."""
...
def remote_target_forms(roster: list[dict], local_taken: "set[str] | frozenset[str]" = frozenset()) -> list[str]:
    ...
    ``@cos-bot``, since bare ``@nunmai`` is always this gateway's own default); else
...
def qualify_sender_stamp(message: str, from_handle: Any, from_connection: Any, roster: list[dict],
    ...
    ``remote_target_forms`` renders it, else ``handle@connection``. A relayed ``@nunmai`` is another
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_nunmai_root(Path(_default_home())))
...
def _nunmai_cli() -> str:
    """nunmai CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "nunmai" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``nunmai`` entrypoint. A bare ``"nunmai"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("nunmai.exe" if sys.platform == "win32" else "nunmai")
    return str(sibling) if sibling.is_file() else shutil.which("nunmai") or "nunmai"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_nunmai_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``NUNMAI_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``NUNMAI_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict], profile_home: "str | Path | None" = None) -> dict[str, str]:
    """Environment for one delivery turn's ``nunmai -p <profile>`` child. The dispatcher's own
    NUNMAI_TURN_AUTHOR is dropped first so a delivery without an author never inherits the author of the turn
    ...
    ``os.environ``; ``-p`` alone only pinned NUNMAI_HOME. ``profile_home`` is the target's home when the
...
# Deliveries are separate ``nunmai`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``nunmai``
