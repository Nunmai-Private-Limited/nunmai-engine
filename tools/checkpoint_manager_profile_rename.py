...
The checkpoint store lives under ``NUNMAI_HOME/checkpoints`` and therefore moves with a renamed
...
def _rekey_project(store: Path, meta: Dict, old_workdir: Path, new_workdir: Path) -> None:
    ...
    A retry (``nunmai profile migrate-identity``) after a mid-way failure finds the new identity
