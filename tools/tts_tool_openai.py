...
Also owns the managed-gateway (Nunmai portal ``openai-audio`` proxy) route selection that
...
# DeepInfra base URL is resolved via nunmai_cli.models.deepinfra_base_url (shared).
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    Nunmai proxy, so callers coerce the request). Strict on the stored ``tts`` selection: ``"nous"``
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Resolve DeepInfra credentials/model (live ``nunmai_cli.models`` catalog, no hardcoded ids), then
    delegate to the OpenAI-compatible handler."""
    api_key = _origin()._resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError("DEEPINFRA_API_KEY not set. Run `nunmai setup` to configure, or set the env var directly.")
