...
# startup notice and `nunmai doctor` read it through terminal_backend_unavailable_reason() so the user
...
def _modal_unavailable_reason(modal_state: Dict[str, Any]) -> tuple[str, str]:
    ...
    if modal_state["managed_mode_blocked"] or modal_state["mode"] == "managed":
        tail = (("Nunmai Tool Gateway access is not currently available and no direct Modal credentials/config "
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior Nunmai processes that died before
...
def _ssh_pre(config: Dict[str, Any]) -> bool:
    ...
    return _reject("the SSH host and user are not configured (TERMINAL_SSH_HOST / TERMINAL_SSH_USER); "
                   "run `nunmai setup terminal` to enter them or pick the 'local' backend")
