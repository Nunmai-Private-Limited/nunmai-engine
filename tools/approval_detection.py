...
# Sensitive write targets, matched via ~ / $HOME / $NUNMAI_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once NUNMAI_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NUNMAI_ENV_PATH = (
    r'(?:~\/\.nunmai/|(?:\$home|\$\{home\})/\.nunmai/|(?:\$nunmai_home|\$\{nunmai_home\})/)' r'\.env\b'
)
# ~/.nunmai/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_NUNMAI_CONFIG_PATH = (
    r'(?:~\/\.nunmai/|(?:\$home|\$\{home\})/\.nunmai/|(?:\$nunmai_home|\$\{nunmai_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_NUNMAI_ENV_PATH}|{_NUNMAI_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    # between `nunmai` and `gateway` (`nunmai -p ade gateway restart`) are allowed so a profile flag can't slip past.
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.nunmai.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.nunmai...'; do
    # launchctl bootout "$label"`) never has "nunmai" after the verb, and that slipped past and restarted 4 gateways
    ...
    (r'\A(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:nunmai|ai\.nunmai)\b)', "stop/restart nunmai launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Nunmai file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.nunmai/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Nunmai config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Nunmai-managed security file (~/.nunmai/config.yaml or .env). sed -i bypasses the
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Nunmai home prefixes to ~/ and ~/.nunmai/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/NUNMAI_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Nunmai home
...
def _rewrite_resolved_nunmai_home(command: str) -> str:
    """Resolved NUNMAI_HOME (and its realpath) -> ``~/.nunmai/`` so the _NUNMAI_CONFIG_PATH /
    _NUNMAI_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.nunmai")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart nunmai gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    return (
        ...
        and re.fullmatch(r"nunmai-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.nunmai.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a nunmai-gateway
