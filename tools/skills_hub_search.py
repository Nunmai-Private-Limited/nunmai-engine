"""Skills Hub discovery: the centralized Nunmai index fetch (cached, stale-
...
from tools.skills_hub_official import NunmaiIndexSource, OptionalSkillSource
...
def _load_nunmai_index() -> Optional[dict]:
    ...
    locally for NUNMAI_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _nunmai_index_cache_file()
    cached = _read_json_if_fresh(cache_file, NUNMAI_INDEX_TTL)
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = hub()._skills_hub_http_get(
                NUNMAI_INDEX_URL, timeout=15, follow_redirects=True,
            ...
            logger.debug("Nunmai index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_nunmai_index_cache_file(), float("inf"))
...
def _search_one_source(
    ...
    try:
        ...
        if provider_filter and isinstance(src, (NunmaiIndexSource, GitHubSource)):
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "nunmai-index" and getattr(src, "is_available", False) for src in sources
    )
...
def _index_miss_fallback_sources(
    ...
    """
    if not query.strip() or provider_filter or not any(src.source_id() == "nunmai-index" for src in active):
        return []
    if source_counts.get("nunmai-index") != 0:
        return []
