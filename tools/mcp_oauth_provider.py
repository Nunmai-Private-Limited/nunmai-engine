"""Shared ``OAuthClientProvider`` customizations for Nunmai MCP OAuth.
...
if TYPE_CHECKING:
    from tools.mcp_oauth import NunmaiTokenStorage
...
def _default_auth_request_user_agent() -> str:
    """``Nunmai-Agent/<version>`` for SDK-built OAuth requests that would otherwise carry no User-Agent at
    ...
    return f"Nunmai-Agent/{__version__}"
...
class NunmaiProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_nunmai_logger`` to keep their own logger name).
    ...
    async def _perform_authorization(self):
        ...
        if (getattr(self, "_nunmai_oauth_flow", "browser") == "device"
                or ("urn:ietf:params:oauth:grant-type:device_code" in grants and "authorization_code" not in grants)):
            from tools.mcp_oauth import OAuthNonInteractiveError
            raise OAuthNonInteractiveError(
                "MCP device authorization requires `nunmai mcp login <server> --flow device`; "
    ...
    def _request_google_offline_access(self) -> None:
        ...
        if inner is None or getattr(inner, "_nunmai_offline_access", False):
            return
        ...
        _with_offline_access._nunmai_offline_access = True  # type: ignore[attr-defined]
        self.context.redirect_handler = _with_offline_access

    async def _nunmai_accept_origin_issued_metadata(self, response):
        ...
        runs), while the RFC 9207 ``iss`` check and Nunmai' refresh-token binding use the document's issuer.
    ...
    def _prepare_token_request(self, request):
        ...
        else the shared ``Nunmai-Agent/<version>`` default. These requests are built by hand — the
        ...
        ua = getattr(self, "_nunmai_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return stamp_default_user_agent(request)

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``NunmaiTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if NunmaiTokenStorage._coerce_secret_auth_method(data):
    ...
    # _nunmai_release_refresh_fence. Never shared across instances.
    _nunmai_fence: int | None = None
                ...
                while True:
                    ...
                    try:
                        ...
                        if failure:
                            ...
                            sent = await self._nunmai_accept_origin_issued_metadata(sent)
            finally:
                await self._nunmai_release_refresh_fence()
    ...
    async def _refresh_token(self):
        ...
        """
        self._coerce_client_secret_post()
        await self._nunmai_acquire_refresh_fence()
        try:
            ...
            candidate = await self._nunmai_rotated_candidate()
            if candidate is not None:
                if not self._nunmai_install_disk_pair(candidate):
                ...
                if self._nunmai_live_ttl() and self.context.is_token_valid():
    ...
    async def _nunmai_acquire_refresh_fence(self) -> None:
        ...
        if tokens_path is None:  # pragma: no cover - non-Nunmai storage
            return
        self._nunmai_fence = await acquire_refresh_fence(tokens_path())

    async def _nunmai_release_refresh_fence(self) -> None:
        """Release the fence if held. Idempotent and never raises."""
        from tools.mcp_oauth import release_refresh_fence

        fd, self._nunmai_fence = self._nunmai_fence, None
        if fd is not None:
            release_refresh_fence(fd)

    async def _nunmai_rotated_candidate(self):
    ...
    def _nunmai_install_disk_pair(self, tokens) -> bool:
    ...
    async def _initialize(self) -> None:
        ...
        storage = self.context.storage
        from tools.mcp_oauth import NunmaiTokenStorage
        if isinstance(storage, NunmaiTokenStorage) and self.context.oauth_metadata is None:
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        """
        try:
            return await self._nunmai_handle_refresh_response(response)
        finally:
            await self._nunmai_release_refresh_fence()

    async def _nunmai_handle_refresh_response(self, response) -> bool:
        if not (200 <= response.status_code < 300):
            self._nunmai_logger.warning("Token refresh failed: %s", response.status_code)
            # A writer outside the fence (interactive `nunmai mcp login`, or a
            # pre-fence Nunmai sharing this NUNMAI_HOME) may have rotated the
            ...
            if await self._nunmai_reload_tokens_after_refresh_failure():
                self._nunmai_logger.info(
        ...
        try:
            ...
            self._nunmai_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
    ...
    async def _nunmai_reload_tokens_after_refresh_failure(self) -> bool:
        ...
        a writer outside the fence (an interactive ``nunmai mcp login`` or a
        pre-fence Nunmai) having rotated the grant between our read and our
        ...
        if (
            self._nunmai_install_disk_pair(candidate)
            and self._nunmai_live_ttl()
...
def bind_issuer_from_context(context: Any) -> None:
    ...
    it. No-op when metadata is not discovered yet or storage is not Nunmai'."""
    from tools.mcp_oauth import NunmaiTokenStorage
    storage = getattr(context, "storage", None)
    issuer = _metadata_issuer(context)
    if isinstance(storage, NunmaiTokenStorage) and issuer:
...
def enforce_refresh_token_issuer(context: Any) -> None:
    ...
    if not isinstance(storage, NunmaiTokenStorage) or tokens is None or not getattr(tokens, "refresh_token", None):
        return
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "NunmaiTokenStorage"]:
    ...
    return cfg, mo.NunmaiTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "NunmaiTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
