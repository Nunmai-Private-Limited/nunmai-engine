...
tick ``maybe_pull_skills`` and ``nunmai sync``. Lives under tools/ so it never imports the CLI at
module load; ``skills_sync_client_wire`` / ``skills_sync_client_org`` are re-exported here.
ACCESS GATE (pre-launch): INERT unless the user is a Nunmai admin per the ``tool_gateway_admin``
...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.nunmai/skills/ skills qualify."""
...
# Gate claim (NAS's wire name; means "Nunmai admin" / Permissions.ADMIN_ACCESS). The bearer comes
...
class SyncInertError(RuntimeError):
    """Sync must no-op: not logged in, no bearer, or not a Nunmai admin. Caught by the gate-and-swallow hooks."""
...
# Configuration -- env-first so Nunmai Cloud can enable sync via environment alone. Every knob:
# NUNMAI_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
...
def resolve_sync_base_url() -> Optional[str]:
    """NUNMAI_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
...
def sync_feature_enabled() -> bool:
    """Master switch; the gate-and-swallow entrypoints ALSO require the Nunmai-admin gate and a base URL."""
    return _sync_config_bool("NUNMAI_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """False (default): edits to an org skill stay LOCAL until ``nunmai sync propose``. True: every
    edit is proposed right away (an admin still approves unless the editor is one)."""
    return _sync_config_bool("NUNMAI_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)


def sync_default_opt_in() -> bool:
    """False (default): opt-IN -- a skill syncs only after ``nunmai sync enable`` or a plane manifest
    opting it in. True: opt-OUT -- every eligible skill syncs unless disabled (Nunmai Cloud default)."""
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.nunmai/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.nunmai/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.nunmai/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.nunmai/skills/.sync_device_id. An existing file always wins; else seeded
    from NUNMAI_SYNC_DEVICE_NAME (first use only, for Nunmai Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("NUNMAI_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.nunmai/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "nunmai skill sync") -> Dict[str, Any]:
...
def _gate_and_swallow(op: str, run: Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]):
    """Run *run(identity)* only if all gates hold (Nunmai admin, feature on, base URL); None if inert/error."""
    try:
...
def sync_status() -> Dict[str, Any]:
    """Snapshot for ``nunmai sync status``; never raises. ``org_available`` False = not in a shared org."""
