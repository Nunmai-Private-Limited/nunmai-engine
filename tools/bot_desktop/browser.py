...
def profile_dir() -> Path:
    ...
    ``~`` expands, and a relative path is anchored at this profile's NUNMAI_HOME (where the rest of the screen's
    state lives), so ``pin`` means ``<NUNMAI_HOME>/pin`` and two profiles never share one jar by accident."""
    override = os.path.expanduser(os.environ.get("AGENT_BROWSER_PROFILE", "").strip())
    if override:
        return Path(override) if os.path.isabs(override) else runtime.get_nunmai_home() / override
...
def dock_exec_line(exe: str, user_data_dir: str) -> str:
    ...
    Desktop Entry spec (a browser under ``/opt/Google Chrome/`` or a profile under a spaced NUNMAI_HOME
