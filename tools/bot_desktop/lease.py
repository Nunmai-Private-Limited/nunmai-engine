...
Authority lives ON DISK, ``<NUNMAI_HOME>/bot-desktop/lease.json`` under an fcntl lock, because the
processes that must agree do not share memory: ``nunmai serve`` (viewer bridge), the messaging
...
from nunmai_constants import get_nunmai_home, nunmai_home_key, secure_parent_dir
...
def _path(profile_key: Optional[str]) -> Path:
    """``profile_key`` is the NUNMAI_HOME path of the profile whose lease is meant (the RFB bridge
    serves several profiles from one process); ``None`` means the current profile."""
    home = Path(profile_key) if profile_key else get_nunmai_home()
...
def _transition(profile_key: Optional[str], mutate: Callable[[Lease], bool]) -> Lease:
    key, path = nunmai_home_key(profile_key) if profile_key else nunmai_home_key(), _path(profile_key)
    with _locked(path):
        lease = _read(path)
        if not mutate(lease):
            return lease
...
def _reset_for_tests() -> None:
    with _lock:
        _listeners.clear()
    p = get_nunmai_home() / "bot-desktop" / "lease.json"
