"""Bot Desktop runtime: one headless Xfce desktop per Nunmai profile, served over RFB on a private
Unix socket, viewed and driven from Nunmai Desktop.

Layout under ``<NUNMAI_HOME>/bot-desktop/``: ``display`` (allocated X display number), ``rfb.sock``
...
``get_nunmai_home()`` so N profiles in one gateway get N desktops: one screen per bot on the shared
machine.
...
def state_dir() -> Path:
    return get_nunmai_home() / "bot-desktop"
...
def install_command() -> Optional[str]:
    ...
    prefixed with ``sudo`` unless Nunmai already runs as root, so it is both what the pane shows and what
    ...
    every service under ``s6-setuidgid nunmai`` (UID 10000 by default) and ships no ``sudo`` binary, so an
...
def installable() -> bool:
    ...
    image: services drop to the ``nunmai`` user and no ``sudo`` binary is installed. The packages can only
...
_ALLOC_LOCK = Path(os.environ.get("XDG_RUNTIME_DIR") or Path.home() / ".cache") / "nunmai-bot-desktop-alloc.lock"
...
def _spawn_and_wait(sd: Path, wait_seconds: float) -> DesktopStatus:
    ...
    with contextlib.ExitStack() as alloc:
        ...
        child_env.update({
            "NUNMAI_BD_PROFILE": _profile_name(),
            "NUNMAI_BD_DISPLAY_NUM": str(num),
            "NUNMAI_BD_SOCKET": str(sd / "rfb.sock"),
            "NUNMAI_BD_XAUTH": str(sd / "Xauthority"),
            "NUNMAI_BD_ENV_FILE": str(env_file),
            "NUNMAI_BD_CONFIG_HOME": str(sd / "xdg"),
            "NUNMAI_BD_GEOMETRY": geometry(),
        })
        ...
        if (browser := dock_launch()) is not None:
            ...
            child_env["NUNMAI_BD_BROWSER_EXEC"] = browser[0]
            child_env["NUNMAI_BD_BROWSER_EXEC_LINE"] = dock_exec_line(*browser)
