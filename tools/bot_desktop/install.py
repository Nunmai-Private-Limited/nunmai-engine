...
def assert_not_running() -> None:
    with _install_lock:
        if nunmai_home_key() in _running:
...
def install_packages(*, ask_password: Callable[[], str], on_line: Callable[[str], None],
    ...
    key = nunmai_home_key() if claimed else None
    try:
...
def _drain_until(proc: subprocess.Popen, on_line: Callable[[str], None], deadline: float) -> bool:
    ...
    Readiness-polled rather than a blocking ``for line in proc.stdout``: from an unprivileged Nunmai no
