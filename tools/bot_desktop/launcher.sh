#!/usr/bin/env bash
# Nunmai Bot Desktop — one headless Xfce desktop per Nunmai profile, served over RFB.
#
# Spawned by tools/bot_desktop/runtime.py with NUNMAI_BD_* variables set. Runs TigerVNC's Xvnc
...
: "${NUNMAI_BD_PROFILE:?}"      # profile name (display name in the VNC title)
: "${NUNMAI_BD_DISPLAY_NUM:?}"  # allocated by runtime.py
: "${NUNMAI_BD_SOCKET:?}"       # RFB unix socket path
: "${NUNMAI_BD_XAUTH:?}"        # Xauthority path
: "${NUNMAI_BD_ENV_FILE:?}"     # where to publish DISPLAY/XAUTHORITY/DBUS_SESSION_BUS_ADDRESS
: "${NUNMAI_BD_CONFIG_HOME:?}"  # per-profile XDG_CONFIG_HOME (xfconf lives here)
GEOM="${NUNMAI_BD_GEOMETRY:-1440x900}"
DEPTH=24

export XDG_CONFIG_HOME="$NUNMAI_BD_CONFIG_HOME"
export XDG_CACHE_HOME="${NUNMAI_BD_CACHE_HOME:-$NUNMAI_BD_CONFIG_HOME/.cache}"
export XDG_DATA_HOME="${NUNMAI_BD_DATA_HOME:-$NUNMAI_BD_CONFIG_HOME/.local-share}"
...
mkdir -p "$XDG_CONFIG_HOME/xfce4/xfconf/xfce-perchannel-xml" "$XDG_CONFIG_HOME/autostart" \
         "$XDG_CACHE_HOME" "$XDG_DATA_HOME" "$(dirname "$NUNMAI_BD_SOCKET")"

export DISPLAY=":$NUNMAI_BD_DISPLAY_NUM"
export XAUTHORITY="$NUNMAI_BD_XAUTH"
...
rm -f "$NUNMAI_BD_SOCKET"
# no-tmp: ok — the X11 protocol fixes its lock and socket under /tmp; this is not our scratch dir
xlock="/tmp/.X${NUNMAI_BD_DISPLAY_NUM}-lock"
if [[ -e "$xlock" ]] && ! kill -0 "$(tr -d ' ' < "$xlock" 2>/dev/null)" 2>/dev/null; then
  rm -f "$xlock" "/tmp/.X11-unix/X${NUNMAI_BD_DISPLAY_NUM}"  # no-tmp: ok — X11 display socket, fixed by the protocol
fi
...
# ---- look: dark theme from whatever the host ships (first match wins), Nunmai wallpaper ----
...
: "${NUNMAI_BD_WALLPAPER:="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/wallpaper.png"}"
...
<channel name="xfwm4" version="1.0">
  <property name="general" type="empty">
    ...
    <property name="theme" type="string" value="NUNMAI_BD_WM_THEME"/>
    <property name="title_font" type="string" value="DejaVu Sans Bold 9"/>
  </property>
</channel>
EOF
sed -i "s|NUNMAI_BD_WM_THEME|$WM_THEME_NAME|" "$X/xfwm4.xml"
...
<channel name="xsettings" version="1.0">
  <property name="Net" type="empty">
    ...
    <property name="ThemeName" type="string" value="NUNMAI_BD_GTK_THEME"/>
    <property name="IconThemeName" type="string" value="NUNMAI_BD_ICON_THEME"/>
  </property>
...
EOF
sed -i "s|NUNMAI_BD_GTK_THEME|$GTK_THEME_NAME|; s|NUNMAI_BD_ICON_THEME|$ICON_THEME_NAME|" "$X/xsettings.xml"
    ...
    <property name="screen0" type="empty">
      <property name="monitorVNC-0" type="empty">
        <property name="workspace0" type="empty">
          ...
          <property name="last-image" type="string" value="NUNMAI_BD_WALLPAPER_PLACEHOLDER"/>
...
EOF
sed -i "s|NUNMAI_BD_WALLPAPER_PLACEHOLDER|$NUNMAI_BD_WALLPAPER|" "$X/xfce4-desktop.xml"
...
if [[ ! -e "$X/xfce4-panel.xml" ]]; then
  ...
  add_launcher() {  # name icon executable [Exec= line] — skipped when the executable is missing.
    ...
    printf '[Desktop Entry]\nVersion=1.0\nType=Application\nName=%s\nIcon=%s\n%s\nTerminal=false\nStartupNotify=false\n' \
      "$1" "$2" "${4:-Exec=$3}" > "$L/launcher-$n/nunmai.desktop"
    dock_ids+=("$n")
  }
  ...
  [[ -n "${NUNMAI_BD_BROWSER_EXEC:-}" ]] && \
    add_launcher "Browser" internet-web-browser "$NUNMAI_BD_BROWSER_EXEC" "${NUNMAI_BD_BROWSER_EXEC_LINE:-}" && \
    echo "$n" > "$L/.nunmai-browser-launcher"
  ...
  for id in "${dock_ids[@]}"; do
    ...
    dock_items+="<property name=\"plugin-$id\" type=\"string\" value=\"launcher\"><property name=\"items\" type=\"array\"><value type=\"string\" value=\"nunmai.desktop\"/></property></property>"
  done
...
# after `nunmai profile rename` the old path would open a browser with an empty, unshared cookie jar.
L="$XDG_CONFIG_HOME/xfce4/panel"
# Profiles seeded before the marker existed have the launcher but no `.nunmai-browser-launcher`; recover
# it from our own Browser entry (nunmai.desktop is ours by name) so their Exec= follows a rename too.
if [[ -n "${NUNMAI_BD_BROWSER_EXEC:-}" && -n "${NUNMAI_BD_BROWSER_EXEC_LINE:-}" && ! -r "$L/.nunmai-browser-launcher" ]]; then
  for d in "$L"/launcher-*/nunmai.desktop; do
    [[ -f "$d" ]] || continue
    while IFS= read -r line; do
      if [[ "$line" == "Name=Browser" ]]; then
        bn="${d%/nunmai.desktop}"; bn="${bn##*/launcher-}"
        printf '%s\n' "$bn" > "$L/.nunmai-browser-launcher"; break 2
      fi
    done < "$d"
  done
fi
if [[ -n "${NUNMAI_BD_BROWSER_EXEC:-}" && -n "${NUNMAI_BD_BROWSER_EXEC_LINE:-}" && -r "$L/.nunmai-browser-launcher" ]]; then
  bn="$(cat "$L/.nunmai-browser-launcher")"
  d="$L/launcher-$bn/nunmai.desktop"
  [[ -f "$d" ]] && sed -i "s|^Exec=.*|$(printf '%s' "$NUNMAI_BD_BROWSER_EXEC_LINE" | sed 's/[&|\\]/\\&/g')|" "$d"
fi

# Tests seed the config tree on a fake PATH and stop here (no X server needed).
[[ -n "${NUNMAI_BD_SEED_ONLY:-}" ]] && exit 0
...
Xvnc "$DISPLAY" -geometry "$GEOM" -depth "$DEPTH" -dpi 96 \
  -rfbport -1 -rfbunixpath "$NUNMAI_BD_SOCKET" -rfbunixmode 0600 \
  -SecurityTypes None -AlwaysShared -AcceptSetDesktopSize -FrameRate 30 -SendCutText=0 -MaxCutText 262144 \
  -desktop "nunmai:$NUNMAI_BD_PROFILE" -auth "$XAUTHORITY" -nolisten tcp \
...
dbus-run-session -- bash -c '
  ...
  printf "DISPLAY=%s\nXAUTHORITY=%s\nDBUS_SESSION_BUS_ADDRESS=%s\nXDG_CONFIG_HOME=%s\nXDG_CACHE_HOME=%s\nXDG_DATA_HOME=%s\n" \
    ...
    > "$NUNMAI_BD_ENV_FILE.tmp" && mv -f "$NUNMAI_BD_ENV_FILE.tmp" "$NUNMAI_BD_ENV_FILE"
