"""Secret-scrub policy for Nunmai child processes: pure data + predicates for which env
names are Nunmai-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``nunmai_subprocess_env``) live in ``tools.environments.local``."""
...
# Nunmai-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "NUNMAI_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Nunmai: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Nunmai-managed
    # inference credential — Claude subscription auth is not a working Nunmai provider path. It arrives via
...
def _is_provider_env_blocklisted(name: str) -> bool:
    ...
    ``OPENAI_API_KEY``; consistent with ``_is_nunmai_internal_secret``, which
    already folds (``key.upper()``)."""
    return (name in _NUNMAI_PROVIDER_ENV_BLOCKLIST
            or name.upper() in _NUNMAI_PROVIDER_ENV_BLOCKLIST)
...
# ``_sanitize_subprocess_env``); execute_code, nunmai_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``NUNMAI_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`nunmai_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Nunmai credential; these ARE Nunmai-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("NUNMAI_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Nunmai venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Nunmai venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Nunmai-owned entries only).
...
# dependencies into the Nunmai venv path instead of the project's own ``.venv`` — silently clobbering the
# Nunmai environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Nunmai venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Nunmai itself
...
# _strip_nunmai_owned_pythonpath() which removes only Nunmai-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_nunmai_internal_secret(key: str) -> bool:
    """True for Nunmai-internal secrets injected under *dynamic* names the static
...
# is script data, not a Nunmai gate, and deleting it by name shape broke routed ``no_agent``
# cron scripts (#119539). ``NUNMAI_*`` never counts (``NUNMAI_MEDIA_ALLOW_DIRS``,
# ``NUNMAI_ALLOW_PRIVATE_URLS`` are process settings, not adapter gates).
...
def is_profile_gate_env(name: str, _prefixes: Optional[frozenset] = None) -> bool:
    ...
    upper = name.upper()
    if upper.startswith("NUNMAI_") or upper.startswith("_"):
        return False
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    "HASS_TOKEN", "EMAIL_PASSWORD", "NUNMAI_DASHBOARD_SESSION_TOKEN",
