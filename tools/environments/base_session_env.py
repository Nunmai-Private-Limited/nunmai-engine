...
# backend serves many sessions, so a snapshot carrying the FIRST session's NUNMAI_SESSION_ID
# would make every LATER session source a foreign identity. Every bridged name starts with
# one of these prefixes (or is NUNMAI_UI_SESSION_ID); unit tests use this regex as the
...
# "default" environment), so ``export -p`` dumping the FIRST session's NUNMAI_SESSION_ID into the snapshot
# makes every LATER session ``source`` that stale value and see a FOREIGN session's identity — overriding
# the correct per-command Popen env (issue: cross-session NUNMAI_SESSION_ID leak via the shared snapshot).
# Stripping them from the snapshot is safe because they are re-injected on every command; a snapshot should
# only carry the user's own shell state (PATH, functions, exports they set), not Nunmai' per-turn session
...
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    ...
    "NUNMAI_CRON_SESSION|NUNMAI_BROWSER_CONTROL_|NUNMAI_DELEGATED_CHILD_CONTEXT)")
...
def _export_dump_excluding_session_vars(tmp_path: str, excluded_names: Iterable[str] = ()) -> str:
    ...
    ``curl … | bash #`` smuggled into a Matrix room/display name via ``NUNMAI_SESSION_CHAT_NAME``) land in
    ...
    return (
        ...
        # AI_AGENT / NUNMAI_AGENT are per-command attribution markers re-exported
        ...
        "NUNMAI_DELEGATED_CHILD_CONTEXT NUNMAI_CRON_SESSION "
...
def _snapshot_bootstrap_script(
    *, quoted_cwd: str, quoted_snap: str, snap_tmp_template: str, excluded_names: Iterable[str], cwd_marker: str,
    ...
    return (
        "umask 077\n"
        f"__nunmai_snap_tmp=$(mktemp {snap_tmp_template}) || exit 1\n"
        f"{_export_dump_excluding_session_vars(_SNAP_TMP, excluded_names)}\n"
        "__nunmai_fns=$(declare -F | awk '{print $3}' | grep -vE '^_[^_]') || true\n"
        f"[ -n \"$__nunmai_fns\" ] && declare -f $__nunmai_fns >> {_SNAP_TMP} 2>/dev/null || true\n"
        f"alias -p >> {_SNAP_TMP}\n"
...
def _wrap_command_script(
    ...
    ``declare -x`` lines when sourcing. AI_AGENT/NUNMAI_AGENT advertise the harness to remote
    ...
    parts += restore
    parts += [
        'export AI_AGENT="${AI_AGENT:-nunmai-engine}" NUNMAI_AGENT="${NUNMAI_AGENT:-true}"',
