...
from tools.environments.remote_common import (
    bash_argv, client_env_with, load_nunmai_env_vars, prepend_unset, resolve_passthrough_env, run_capture)
...
# Module-level binding: tests patch ``docker._load_nunmai_env_vars`` to fake the .env file.
_load_nunmai_env_vars = load_nunmai_env_vars
...
def _get_active_profile_name() -> str:
    """Active Nunmai profile name, or ``"default"`` on any error. Resolved at container-create
...
def reap_orphan_containers(
    *, max_age_seconds: int = 600, profile_filter: str | None = None, docker_exe: str | None = None,
    ...
    """Remove stale nunmai-tagged containers left behind by prior processes (SIGKILL/OOM
...
def find_docker() -> Optional[str]:
    """Locate the docker/podman CLI (cached): ``NUNMAI_DOCKER_BINARY`` override, ``docker``
...
def _ensure_docker_available() -> None:
    ...
    if not docker_exe:
        raise _docker_unavailable(
            ...
            hint="Install Docker (or fix PATH) and retry, or run `nunmai setup terminal` to switch to Local.")
    try:
        ...
        raise _docker_unavailable(
            ...
            hint="Start Docker (e.g. `systemctl start docker` or launch Docker Desktop), then retry — "
                 "or run `nunmai setup terminal` to switch to Local.")
    except Exception:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # s6-overlay images (e.g. nunmai-engine:latest) already use /init as PID 1 and exec
        ...
        # Labels identify nunmai containers to the orphan reaper (nunmai-engine=1),
    ...
    def _mount_args(self, volumes, host_cwd, auto_mount_cwd, task_id) -> tuple[list[str], list[str]]:
        ...
        Persistent mode bind-mounts from TERMINAL_SANDBOX_DIR (default ~/.nunmai/sandboxes/)."""
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        return resolve_passthrough_env(self._forward_env, nunmai_env_loader=_load_nunmai_env_vars)
    ...
    def _recreate_container(self) -> bool:
        ...
        existing = self._find_reusable_container(
            self._labels.get("nunmai-task-id", ""),
            self._labels.get("nunmai-profile", ""),
        ...
        if not self._container_id:
            if not self._image:
                ...
                return False
            try:
                new_name = f"nunmai-{uuid.uuid4().hex[:8]}"
                result = run_capture(
    ...
    def _find_reusable_container(
        ...
        reused after ``nunmai egress disable`` (baked-in proxy env and CA mounts), and every
