...
# (NUNMAI_DEBUG_INTERRUPT=1). Off by default to avoid flooding gateway logs.
_DEBUG_INTERRUPT = env_var_enabled("NUNMAI_DEBUG_INTERRUPT")
...
def get_sandbox_dir() -> Path:
    ...
    overlays/SIF cache). ``TERMINAL_SANDBOX_DIR`` overrides ``{NUNMAI_HOME}/sandboxes``."""
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    p = Path(custom) if custom else get_nunmai_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Nunmai backends. Subclasses
    ...
    def fetch_file(self, remote_path: str, local_dest: Path, *, max_bytes: int) -> None:
        ...
        import base64
        import binascii
        marker = f"__NUNMAI_FETCH_{uuid.uuid4().hex[:12]}__"
    ...
    def _wait_for_process(
        ...
        if stdin_errors := getattr(proc, "_nunmai_stdin_errors", None):
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the ``__NUNMAI_CWD_{session}__`` marker from ``result["output"]``, update
