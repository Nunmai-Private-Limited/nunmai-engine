...
from tools.environments.local_env_policy import (  # noqa: F401 — _NUNMAI_PROVIDER_ENV_BLOCKLIST stays importable from here
    _ALWAYS_STRIP_KEYS, _NUNMAI_PROVIDER_ENV_BLOCKLIST, _NUNMAI_PROVIDER_ENV_FORCE_PREFIX,
    _is_nunmai_internal_secret, _is_provider_env_blocklisted, _is_terminal_first_party_env,
...
from tools.environments.local_pythonpath import (
    _build_nunmai_repo_root_aliases, _strip_nunmai_owned_pythonpath_and_runtime_markers)
...
# get_temp_dir() defaults to NUNMAI_HOME/cache/terminal (real storage, not tmpfs), so
...
# Background artifacts come in triplets (nunmai_bg_<id>.log/.pid/.exit). A live
...
def cleanup_terminal_temp_cache(max_age_hours: float = TERMINAL_TEMP_MAX_IDLE_HOURS) -> int:
    ...
    from nunmai_constants_scratch import subtree_touched_since
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local NUNMAI_HOME override, then the subprocess HOME contract."""
    from nunmai_constants import apply_subprocess_home_env, get_nunmai_home_override
    try:
        if value := get_nunmai_home_override():
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (NUNMAI_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping Nunmai-managed secrets. ``_NUNMAI_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_NUNMAI_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_NUNMAI_PROVIDER_ENV_FORCE_PREFIX):]
            if not _is_nunmai_internal_secret(key):
                out[key] = value
            continue
        if _is_nunmai_internal_secret(key) or key.upper() in plugin_strip_folded:
            continue
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, Nunmai-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the nunmai install dir so bare ``nunmai`` resolves
    ...
    # Keep bare ``nunmai`` invocations available to child jobs even when the gateway was launched by a
    ...
    if path_key is not None:
        out[path_key] = _prepend_nunmai_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Nunmai-managed secrets from a subprocess environment (background/PTY
...
def _scrub_credentials(env: dict, *, inherit_credentials: bool) -> dict:
    ...
    for key in list(env):
        if (key.upper() in strip_folded
                ...
                or key.startswith(_NUNMAI_PROVIDER_ENV_FORCE_PREFIX)
                or _is_nunmai_internal_secret(key)):
            del env[key]
    return env
...
def build_subprocess_env(
    ...
    bridges NUNMAI_HOME + HOME and ``extra`` is applied last so caller overrides win.
...
def served_profile_child_env(
    ...
    """Child env for a process that acts FOR the active (possibly served) profile: ``nunmai -p X``
    ...
    (its ``.env`` + hydrated sources, what a standalone ``nunmai -p X`` loads itself) are overlaid — never
    ...
    ``nunmai_subprocess_env`` snapshot."""
    from agent.secret_scope import (
        UnscopedSecretError, build_profile_secret_scope, current_secret_scope, is_multiplex_active)
    from nunmai_constants import apply_scratch_tmp_env, get_nunmai_home_override
    env = dict(base) if base is not None else nunmai_subprocess_env(inherit_credentials=inherit_credentials)
    target = str(target_home or get_nunmai_home_override() or "")
    if target:
        env["NUNMAI_HOME"] = target
...
def _is_routed_home(target_home: "str | Path") -> bool:
    ...
    mirrors the served profile into ``NUNMAI_HOME``, the live env var names the served home and the
    launch residue would never be stripped from that profile's child env."""
    from nunmai_constants import get_routing_process_nunmai_home
    try:
        return Path(target_home).resolve() != get_routing_process_nunmai_home().resolve()
    except OSError:
        return True
...
def strip_launch_profile_env(env: dict, target_home: "str | Path | None" = None) -> dict:
    ...
    the secret scrub removes credentials but not settings (``NUNMAI_MODEL``, ``TERMINAL_ENV``,
    ``NUNMAI_LANGUAGE``...), so a standalone ``nunmai -p X`` worker and a served one saw different
    ...
    installing a NUNMAI_HOME override without that flag."""
    from agent.secret_scope import _is_global_env, load_env_file
    from nunmai_constants import get_nunmai_home_override, get_process_nunmai_home
    target = target_home or get_nunmai_home_override()
    if not target or not _is_routed_home(target):
        return env
    launch_home = get_process_nunmai_home()
    from nunmai_cli.config import TERMINAL_CONFIG_ENV_MAP
...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    """Ordered bash.exe candidates on Windows: NUNMAI_GIT_BASH_PATH, our portable Git
    under %LOCALAPPDATA%\\nunmai\\git (PortableGit ``bin`` and MinGit ``usr\\bin``),
    ...
    roots = [
        lad and os.path.join(lad, "nunmai", "git", "bin"),
        lad and os.path.join(lad, "nunmai", "git", "usr", "bin"),
...
def _find_bash() -> str:
    ...
    # First candidate that can actually start wins: a stale NUNMAI_GIT_BASH_PATH
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "NUNMAI_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
...
def _resolve_nunmai_bin_dir() -> str | None:
    """Directory holding the ``nunmai`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``nunmai`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    nunmai executable; ``sys.executable``'s dir if it holds the shim."""
    ...
    elif (os.path.isabs(argv0) and (base == "nunmai" or base.startswith("nunmai."))
    ...
    _NUNMAI_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _NUNMAI_BIN_DIR


def _prepend_nunmai_bin_dir(existing_path: str) -> str:
    """Prepend the nunmai install dir to ``existing_path`` if missing."""
...
def _managed_runtime_path_entries() -> list[str]:
    """Existing Nunmai-managed runtime dirs: ``$NUNMAI_HOME/node`` (+``/bin``) and
    ``$NUNMAI_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    ...
    try:
        ...
        return [str(d) for d in (*iter_nunmai_node_dirs(), get_nunmai_home() / "bin") if d.is_dir()]
    except Exception:
        return []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Nunmai runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# spellings — the Windows gateway launcher renders Nunmai-owned paths under the
# configured NUNMAI_HOME spelling (possibly a junction to another drive).
...
_nunmai_repo_root_aliases: tuple[Path, ...] = _build_nunmai_repo_root_aliases(
    _nunmai_repo_root, Path(__file__).absolute().parents[2], get_process_nunmai_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_nunmai_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
def _kill_process_group_posix(proc) -> None:
    ...
    try:
        ...
        if (pgid := getattr(proc, "_nunmai_pgid", None)) is None:
            raise
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Nunmai host itself — controller-side platform behavior
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no system temp dir), ``NUNMAI_HOME/cache/terminal`` (real storage: a
        tmpfs system temp dir fills under Nunmai load; pruned by ``cleanup_terminal_temp_cache``),
        ...
        so always the NUNMAI_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "nunmai_terminal")
    ...
    def _force_kill_process(self, proc):
        ...
        with contextlib.suppress(OSError):
            pgid = getattr(proc, "_nunmai_pgid", None) or os.getpgid(proc.pid)
