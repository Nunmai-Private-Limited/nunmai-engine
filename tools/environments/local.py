...
from tools.environments.local_env_policy import (
    ...
    _is_nunmai_internal_secret, _is_terminal_first_party_env,
    ...
    for key, value in items.items():
        if key.startswith(_NUNMAI_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if not _is_nunmai_internal_secret(key):
                out[key] = value
            continue
        if _is_nunmai_internal_secret(key) or key in plugin_strip:
            continue
        first_party = _is_terminal_first_party_env(key)
        passthrough = is_env_passthrough(key)
        if key in _NUNMAI_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
            continue
...
def nunmai_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    if not inherit_credentials:
        strip |= _NUNMAI_PROVIDER_ENV_BLOCKLIST
    for key in list(env):
        if (key in strip or key.startswith(_NUNMAI_PROVIDER_ENV_FORCE_PREFIX)
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no /tmp), ``NUNMAI_HOME/cache/terminal`` (real storage: tmpfs /tmp
        fills under Nunmai load; pruned by ``cleanup_terminal_temp_cache``), /tmp,
