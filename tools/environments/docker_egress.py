...
_CONTAINER_CA = "/etc/ssl/certs/nunmai-egress-ca.crt"
_NODE_OPTIONS_SENTINEL = "_NUNMAI_EGRESS_NODE_OPTIONS_APPEND"
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    env_overrides: dict[str, str] = {
        ...
        "NUNMAI_EGRESS_PROXY": "1",  # lets the in-sandbox agent know it is proxy-aware
        _NODE_OPTIONS_SENTINEL: "--use-openssl-ca"}

    # Proxy tokens under the standard provider env names (and their aliases) so
    # SDKs work unchanged; NUNMAI_PROXY_TOKEN_* copies are for diagnostics.
    for m in mappings:
