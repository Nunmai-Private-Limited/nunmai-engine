...
from tools.environments.local_env_policy import _NUNMAI_PROVIDER_ENV_BLOCKLIST, _is_nunmai_internal_secret


def load_nunmai_env_vars() -> dict[str, str]:
    """``~/.nunmai/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            nunmai_env_loader: Callable[[], dict[str, str]] = load_nunmai_env_vars,
    ...
    is filtered through the Nunmai provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_nunmai_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _NUNMAI_PROVIDER_ENV_BLOCKLIST)
    nunmai_env = nunmai_env_loader() if forward_keys else {}
