...
def _snapshot_store() -> Path:
    ...
    return get_nunmai_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .nunmai/ as a tar archive (sandboxes run as root, so /root/.nunmai)."""
        # --exclude: live sockets cannot be archived ("socket ignored") and must not fail the download.
        data = self._exec("tar cf - --exclude='*.sock' -C / root/.nunmai", timeout=120, fail_label="bulk download", capture=True)
