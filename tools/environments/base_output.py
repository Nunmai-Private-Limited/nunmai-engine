...
def _new_output_collector(proc, bounded_capture: bool) -> _BoundedOutputCollector:
    ...
    spill file under ``$NUNMAI_HOME/cache/terminal-output`` (created only on actual overflow;
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    ``proc._nunmai_stdin_errors`` (surfaced by ``_wait_for_process`` as ``stdin_error``) and
