...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Nunmai'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``NUNMAI_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Nunmai
...
def _snapshot_store_path() -> Path:
    return get_nunmai_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.nunmai"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_nunmai = (
            "/.nunmai"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.nunmai"
        )
        archive_member = remote_nunmai.lstrip("/")
        remote_tar = f"/tmp/.nunmai_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
