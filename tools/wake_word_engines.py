...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_nunmai") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under NUNMAI_HOME. Keywords are tokenized at RUNTIME.
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey nunmai").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="nunmai-kws-", delete=False,
                                         encoding="utf-8")
