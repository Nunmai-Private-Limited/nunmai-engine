...
Trust levels:
  - builtin:   Ships with Nunmai. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.nunmai/\.env|\~/\.nunmai/\.env',
     "nunmai_env_access", "critical", "exfiltration",
     "directly references Nunmai secrets file"),
    ...
    #     For Nunmai/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_NUNMAI_CONFIG_FILES),
     "nunmai_config_mod", "high", "persistence",
     "modification language aimed at Nunmai configuration files (verify intent)"),
    (_shell_write_re(_NUNMAI_CONFIG_FILES),
     "nunmai_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Nunmai configuration files"),
    (r'\.nunmai/config\.yaml|\.nunmai/SOUL\.md',
     "nunmai_config_ref", "low", "persistence",
     "references Nunmai configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Nunmai-native name; `.clawhubignore` is honored
