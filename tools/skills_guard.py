...
_NUNMAI_CONFIG_FILES = r'\.nunmai/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.nunmai/\.env|\~/\.nunmai/\.env',
     "nunmai_env_access", "critical", "exfiltration", "directly references Nunmai secrets file"),
    ...
    # invocation: any client plugin that relays Nunmai' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Nunmai/other config (setup docs routinely say
    ...
    (_prose_modify_re(_NUNMAI_CONFIG_FILES),
     "nunmai_config_mod", "high", "persistence", "modification language aimed at Nunmai configuration files (verify intent)"),
    (_shell_write_re(_NUNMAI_CONFIG_FILES),
     "nunmai_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Nunmai configuration files"),
    (r'\.nunmai/config\.yaml|\.nunmai/SOUL\.md',
     "nunmai_config_ref", "low", "persistence", "references Nunmai configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Nunmai-native; `.clawhubignore` is honored for skills published through ClawHub.
