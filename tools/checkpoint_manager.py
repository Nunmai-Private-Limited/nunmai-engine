...
-----------------------------------------------------------------------------
    ...
    ~/.nunmai/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/nunmai/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Nunmai convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Nunmai wrote
...
def _git_env(
    ...
    The shared store is internal Nunmai infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``nunmai checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `nunmai checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "nunmai@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Nunmai Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "NUNMAI_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``NUNMAI_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at NUNMAI_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "NUNMAI_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Nunmai just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Nunmai last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Nunmai last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Nunmai'
        last write or that Nunmai never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Nunmai has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Nunmai never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Nunmai wrote it: restoring it back is
                # safe — its last content was Nunmai-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Nunmai changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Nunmai changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Nunmai' last write — per the agent-write ledger —
        are left untouched, and only Nunmai-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Nunmai-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Nunmai created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `nunmai checkpoints` CLI
# ---------------------------------------------------------------------------
