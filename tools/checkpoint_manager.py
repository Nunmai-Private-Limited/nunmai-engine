...
config / ``--checkpoints`` flag.  One store under ``~/.nunmai/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/nunmai/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
from nunmai_cli.gitlock import clear_stale_tmp_packs
from utils import env_int, rmtree_readonly

logger = logging.getLogger(__name__)

CHECKPOINT_BASE = get_nunmai_home() / "checkpoints"
...
def _resolve_checkpoint_base() -> Path:
    ...
    changed it, else live profile-scoped NUNMAI_HOME — under the multiplexed gateway one process
    ...
    return CHECKPOINT_BASE if CHECKPOINT_BASE != _CHECKPOINT_BASE_AT_IMPORT else get_nunmai_home() / "checkpoints"

_STORE_DIRNAME, _INDEXES_DIRNAME, _PROJECTS_DIRNAME, _LEDGERS_DIRNAME = "store", "indexes", "projects", "ledgers"
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/nunmai", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (Nunmai convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("NUNMAI_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "nunmai@local"), ("user.name", "Nunmai Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell Nunmai' writes from later user edits."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    users may want to recover; the archive falls under retention and ``nunmai checkpoints
    ...
    logger.info("Migrated pre-v2 checkpoint repos to %s. "
                "Clear with `nunmai checkpoints clear-legacy` when safe.", legacy_root)
    return legacy_root
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "NUNMAI_WORKDIR").exists():
                workdir = (child / "NUNMAI_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Nunmai just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what Nunmai
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        Nunmai.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, names_out.split("\x00")):
            ...
            nunmai_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if nunmai_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after Nunmai' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is Nunmai-created (delete to restore) — unless ``max_file_size_mb`` kept
...
def auto_prune_from_config() -> Dict[str, object]:
    ...
    cleanup is only via explicit ``nunmai checkpoints prune``. Never raises."""
    try:
...
def checkpoint_footprint_notice() -> Optional[str]:
    ...
    try:
        ...
        return (f"Filesystem checkpoints (/rollback) are on: {format_bytes(size)} across "
                ...
                f"always kept). Not using /rollback? `nunmai config set checkpoints.enabled false` then "
                f"`nunmai checkpoints clear`; or lower `checkpoints.retention_days`.")
    except Exception as exc:
