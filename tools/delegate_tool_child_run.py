...
def _dump_subagent_timeout_diagnostic(
    *, child: Any, task_index: int, timeout_seconds: float, duration_seconds: float,
    ...
    response", 0 API calls, nothing to inspect): ``~/.nunmai/logs/subagent-timeout-<sid>-<ts>.log`` with the
...
def _validate_child_output_schema(
    ...
    try:
        ...
        # unmarked turn is misread as the dispatcher-owned worker by every NUNMAI_KANBAN_* gate.
