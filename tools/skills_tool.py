...
# Cache validation (mirrors nunmai_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.nunmai/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set NUNMAI_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped NUNMAI_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from NUNMAI_HOME/.env."""
    env_path = get_nunmai_home() / ".env"
    ...
    # the canonical readers in nunmai_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set NUNMAI_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (NUNMAI_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("NUNMAI_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("NUNMAI_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_nunmai_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.nunmai/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``NUNMAI_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``NUNMAI_PLATFORM`` environment variable
    3. ``NUNMAI_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from nunmai_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("NUNMAI_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.nunmai/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``nunmai skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile NUNMAI_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from nunmai_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: nunmai plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `nunmai skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.nunmai/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `nunmai skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.nunmai.* first (agentskills.io convention), fall back to top-level
        nunmai_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            nunmai_meta = metadata.get("nunmai", {}) or {}

        tags = _parse_tags(nunmai_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            nunmai_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `nunmai sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
