...
_DOCKER_PULL = "docker pull ghcr.io/nunmai-private-limited/nunmai-engine:latest"
...
def _prepare_session_socket_dir(session_name: str) -> str:
    ...
    with our PID BEFORE first use — another nunmai process's orphan reaper rmtree's any
...
def _daemon_idle_timeout_seconds() -> int:
    ...
    browser's lifetime, and a crashed nunmai leaves it to the orphan reaper (#110064)."""
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # Browser Use mode + ``browser.engine: lightpanda`` drives a Nunmai-spawned
...
def _shares_bot_desktop_browser(session_info: Dict[str, Any]) -> bool:
    ...
    attach, Lightpanda) is a browser Nunmai launched with this profile's Bot Desktop DISPLAY, so it is the
