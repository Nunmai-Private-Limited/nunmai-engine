...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(nunmai_home).expanduser() if nunmai_home else Path.home() / ".nunmai") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Nunmai's live source checkout "
        ...
        "is pushed. To change this checkout, stop Nunmai, run the command externally, then restart "
        "Nunmai.")
