...
# prefix, the exact-name NUNMAI_ allowlist, or (Windows) an OS-essential name.
# The broad "NUNMAI_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (NUNMAI_BASE_URL, NUNMAI_KANBAN_DB, *_WEBHOOK).
# NUNMAI_RPC_SOCKET / NUNMAI_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. NUNMAI_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Nunmai code loses the Kanban mutation guard
# while still inheriting NUNMAI_HOME.
_NUNMAI_CHILD_ALLOWED = frozenset({
    "NUNMAI_HOME", "NUNMAI_PROFILE", "NUNMAI_CONFIG", "NUNMAI_ENV", "NUNMAI_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational NUNMAI_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret NUNMAI_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    if _dropped_nunmai:
        logger.debug(
            ...
            len(_dropped_nunmai), ", ".join(sorted(_dropped_nunmai)),
        )
    ...
    # non-allowlisted NUNMAI_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from nunmai_constants import apply_scratch_tmp_env, apply_subprocess_home_env, get_nunmai_home_override
    ...
    # Only TZ reaches the child; NUNMAI_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — nunmai_time resolves the routed profile's).
    from nunmai_time import get_timezone_name
    ...
    # NUNMAI_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    _home_override = get_nunmai_home_override()
    if _home_override:
        child_env["NUNMAI_HOME"] = _home_override
        apply_scratch_tmp_env(child_env)  # TMPDIR follows the routed home, like HOME does
    # PYTHONPATH: the staging dir (nunmai_tools.py) must always be importable even when project
    # mode changes CWD. Nunmai's root is added ONLY when the child runs in Nunmai's Python env —
    # exposing Nunmai's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Nunmai-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Nunmai-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Nunmai-
    ...
    if _uses_nunmai_python_environment(child_python):
        ...
        # Surface once per interpreter so "import nunmai_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Nunmai "
                    "environment; nunmai root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
def _uses_nunmai_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Nunmai's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the nunmai root."""
