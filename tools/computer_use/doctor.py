"""
`nunmai computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Nunmai side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Nunmai provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `NUNMAI_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Nunmai identity under nunmai_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["nunmai_identity"] = identity
