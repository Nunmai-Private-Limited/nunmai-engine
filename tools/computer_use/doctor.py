"""`nunmai computer-use doctor` — thin client for cua-driver's `health_report` MCP tool. cua-driver owns the health
...
def _build_identity(binary: str, report: Report) -> Report:
    """Nunmai-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    try:  # prefer real health_report; on denial/non-schema, synthesize via probes
        ...
        print(f"cua-driver could not be started from {binary!r}: {e}\n"
              "  The Nunmai runtime interpreter cannot execute this binary; the tool may still work because the\n"
              "  shell resolves a different copy on PATH. Fix: install cua-driver outside the protected directory\n"
              "  (e.g. the upstream installer's default under your user profile) or point NUNMAI_CUA_DRIVER_CMD at\n"
              "  a copy the runtime can execute, then re-run `nunmai computer-use doctor`.", file=sys.stderr)
        return 2
    except RuntimeError as e:
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under nunmai_identity (and environment under
        # nunmai_environment when present) so overall/checks parsers keep working.
        payload = {**report, "nunmai_identity": identity}
        if environment:
            payload["nunmai_environment"] = environment
