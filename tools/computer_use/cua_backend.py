...
etc.) surface as specific blocked checks via `nunmai computer-use doctor`
...
# fetches the latest release, so a `NUNMAI_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `NUNMAI_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "NUNMAI_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Nunmai should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Nunmai should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Nunmai YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. Nunmai passes
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"nunmai-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    return (
        "window discovery returned no windows; run `nunmai computer-use "
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Nunmai runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Nunmai process uses POSIX subprocess spawning inside WSL.
    ...
    *,
    ...
    if not resolved_app:
        raise RuntimeError(
            ...
            "Run `nunmai computer-use install` to restore it."
        )
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Nunmai session's YOLO choice
    ...
    * ``unrestricted`` — explicit Nunmai YOLO; launch-time risk
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Nunmai an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``NUNMAI_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Nunmai Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `nunmai computer-use doctor` succeeds from a login shell.
    """
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Nunmai process is restarted.
            os.path.join(
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``NUNMAI_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Nunmai' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "Nunmai computer use requires cua-driver 0.20.0 or newer",
        }
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `nunmai computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``NUNMAI_CUA_DRIVER_CMD`` override is authoritative even when broken, and
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `nunmai tools` and enable the Computer Use toolset to install it automatically."
    )
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Nunmai-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `nunmai computer-use doctor` and check "
                f"{display_nunmai_home()}/logs/agent.log for the phase timings."
            )
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Nunmai talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Nunmai did not replay it. Take fresh state before "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Nunmai run, with
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Nunmai
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by NUNMAI_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `nunmai computer-use install` to repair it."
            raise RuntimeError(f"cua-driver is not ready: {reason}. {repair}")
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Nunmai' minimal core.
    ...
    def capture(
        ...
        Maps nunmai `capture(mode, app)` → cua-driver `list_windows` +
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Nunmai adapter, and uses cua-driver's
