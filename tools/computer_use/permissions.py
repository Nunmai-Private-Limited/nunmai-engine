...
identity (``com.trycua.driver``), not Nunmai, so ``grant`` launches CuaDriver via LaunchServices for correct dialog
...
def stale_tcc_grant_hint(*missing: str) -> str:
    ...
    return ("If System Settings already shows CuaDriver ON, the stored grant is stale (it no longer matches the installed "
            f"driver's signature): run `{resets}`, then `nunmai computer-use permissions grant`.")
