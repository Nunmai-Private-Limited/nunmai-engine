...
`health_report` (surfaced by `nunmai computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Nunmai runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Nunmai-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Nunmai's explicit approval bypass onto Cua's immutable mode.

    Nunmai has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "NUNMAI_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                ...
                elif backend_name == "nunmai-desktop":
                    # a desktop owned by another service (the Nunmai platform's agent desktops), driven over HTTP
                    from tools.computer_use.nunmai_desktop_backend import NunmaiDesktopBackend

                    backend = NunmaiDesktopBackend(permission_mode=permission_mode)
                else:
                    raise RuntimeError(
                        f"Unknown NUNMAI_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Nunmai YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Nunmai sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Nunmai process that spawned it
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$NUNMAI_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_nunmai_dir("cache/vision", "temp_vision_images")
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Nunmai' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_nunmai_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nunmai_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `nunmai computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
