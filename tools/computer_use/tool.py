...
# Hard-blocked regardless of approval level (e.g. logout kills the session Nunmai runs in). Alt is
...
# Per-Nunmai-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Nunmai's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("NUNMAI_COMPUTER_USE_BACKEND", "cua").lower()
...
def _scoped_sid(session_id: str) -> str:
    """Cache key for one Nunmai session's backend. Outside a served-profile scope it is the bare id
    ...
    return sid if get_nunmai_home_override() is None else f"{sid}@{nunmai_home_key()}"
...
def _get_backend(session_id: str = "") -> ComputerUseBackend:
    ...
    while True:
        with _backend_lock:
            ...
            permission_mode = _cua_permission_mode(bare_sid)  # approval state is keyed by the Nunmai session id
...
def _shutdown_backend_atexit() -> None:
    ...
    the Nunmai process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (NUNMAI_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `nunmai computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$NUNMAI_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from nunmai_constants import get_nunmai_dir  # lazy so tests can patch get_nunmai_dir
    cache_dir = get_nunmai_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Nunmai' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        if (cached := _AUX_VISION_ROUTE_CACHE.get(key := (nunmai_home_key(), str(provider), str(model)))) is not None:
            return cached
        stage = "decision"
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$NUNMAI_HOME/cache/vision/``) and merge
