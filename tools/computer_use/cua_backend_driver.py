...
# var would only LOOK like it pinned. Point NUNMAI_CUA_DRIVER_CMD at a specific binary instead.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command to its DrvFS ``/mnt/<drive>/...`` form when Nunmai runs in WSL
    (a Windows cua-driver manifest can report ``C:\\...`` while Nunmai spawns via POSIX). Non-Windows paths and
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    """Candidate commands in resolution order. ``override`` / a non-empty ``NUNMAI_CUA_DRIVER_CMD`` is authoritative
...
def _resolve_mcp_invocation(driver_cmd: str, *, timeout: float = 6.0) -> Tuple[str, List[str]]:
    ...
    keeps working without a Nunmai patch.
...
def _manifest_contract_reason(manifest: Optional[Dict[str, Any]]) -> str:
    ...
    if tuple(int(part) for part in match.groups()) < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return "Nunmai computer use requires cua-driver 0.20.0 or newer"
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (f"cua-driver {state.get('latest_version') or '?'} is available "
            f"(you have {state.get('current_version') or '?'}); update with `nunmai computer-use install --upgrade`.")
