...
local server > env > managed Nunmai gateway). Facade-owned state and helpers
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `nunmai model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": nunmai_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``nunmai_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
