...
_PROBE_INITIALIZE_BODY = (  # JSON-RPC ``initialize`` body for the content-type preflight POST
    ...
    '"capabilities":{},"clientInfo":{"name":"nunmai-probe","version":"0.1"}}}')
...
class MCPServerTransportMixin:
    ...
    async def _complete_handshake_at_offered_version(self, session):
        ...
        client_info = getattr(session, "_client_info", None) or types.Implementation(name="nunmai-engine", version="0")
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _core._ensure_mcp_sdk():
            raise ImportError(f"MCP server '{self.name}' requires the 'mcp' Python SDK, but "
                              "it is not installed. Run `nunmai setup` to install MCP support, then retry.")
        ...
        new_pids: set = set()
        # Subprocess stderr goes to ~/.nunmai/logs/mcp-stderr.log so banners can't corrupt the TUI.
