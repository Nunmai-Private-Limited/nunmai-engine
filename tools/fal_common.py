...
def _managed_fal_billing_error(exc: BaseException, what: str) -> Optional[str]:
    """Human-readable tail for a Nunmai managed-gateway ``BILLING_ERROR`` response, else None.
    ...
    detail = upstream.get("error") or "Nunmai Portal rejected the charge authorization"
    return (
        f"{error.get('message') or 'Charge authorization failed'} (BILLING_ERROR; {code}: {detail}). "
        "This is a Nunmai Portal billing configuration issue, not a missing local API key. "
        f"The managed route cannot run this {what} until Nunmai enables its billing meter; "
...
def _managed_fal_rate_limit_message(what: str, name: str, retry_after: Optional[float]) -> str:
    ...
    return (
        f"Nunmai Subscription gateway rate-limited {what} '{name}' (HTTP 429; {hint}). "
...
class _ManagedFalSyncClient:
    """Drives a Nunmai-managed fal-queue gateway via ``fal_client.SyncClient`` primitives; carries
    ...
    def submit(
        ...
        # The Nunmai gateway currently records a keyed submission before billing
