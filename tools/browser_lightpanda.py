...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_nunmai_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("nunmai home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$NUNMAI_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _state_dir() -> Path:
    from nunmai_constants import get_nunmai_home
    path = Path(get_nunmai_home()) / "cache" / "browser-use" / "lightpanda"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Nunmai spawns.
    ...
    bounds it without Nunmai managing eviction.
    """
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Nunmai under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Nunmai is gone; return the count. A live owner is
