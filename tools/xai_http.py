...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`nunmai_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
...
def _xai_base_url_override() -> str:
    """``NUNMAI_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    return str(get_env_value("NUNMAI_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Nunmai-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.nunmai/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``NUNMAI_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Nunmai-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``nunmai_cli.config.get_env_value`` so keys stored in ``~/.nunmai/.env`` (the standard Nunmai
    ...
    scoping is identical to the fallback branch, and the base URL honors ``NUNMAI_XAI_BASE_URL`` /
