"""Skills Hub official sources: repo-shipped optional skills and the centralized Nunmai index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _nunmai_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    """Skills from the repo's ``optional-skills/`` directory: official (Nunmai-maintained) but not
    activated by default — absent from the system prompt and not copied to ~/.nunmai/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "Nunmai-Private-Limited/nunmai-engine"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.nunmai.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``nunmai update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.nunmai.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        nunmai_meta = meta_block.get("nunmai") if isinstance(meta_block, dict) else None
        upstream = nunmai_meta.get("upstream") if isinstance(nunmai_meta, dict) else None
...
class NunmaiIndexSource(SkillSource):
    """Skill source backed by the centralized Nunmai Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_nunmai_index
            self._index, self._loaded = _load_nunmai_index(), True
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "nunmai-index"), identifier=entry.get("identifier", ""),
