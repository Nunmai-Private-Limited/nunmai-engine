...
refresh; this module supplies ``NunmaiTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Nunmai' Client ID Metadata
...
async def acquire_refresh_fence(path: "Path", *, timeout: float = _REFRESH_FENCE_TIMEOUT_SECONDS) -> int:
    ...
    try:
        ...
        mkdir_under_nunmai_home(lock_path.parent)
...
def _get_token_dir(nunmai_home: str | Path | None = None) -> Path:
    """``NUNMAI_HOME/mcp-tokens/`` — per-profile token directory."""
    from nunmai_constants import get_nunmai_home

    return Path(nunmai_home if nunmai_home is not None else get_nunmai_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "NunmaiTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "NunmaiTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``nunmai
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `nunmai mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class NunmaiTokenStorage:
    """Persist OAuth state as ``NUNMAI_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._nunmai_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``nunmai_issuer`` is Nunmai bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("nunmai_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["nunmai_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``nunmai_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("nunmai_issuer"):
            return
        data["nunmai_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        mkdir_under_nunmai_home(token_dir)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            if not parsed["code"] and not parsed["error"]:
                ...
                body = "<h2>Authorization already received</h2><p>You can close this tab and return to Nunmai.</p>"
            else:
                result.update(auth_code=parsed["code"], state=parsed["state"], error=parsed["error"], iss=parsed["iss"])
                body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Nunmai.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nunmai mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://nunmai.in/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Nunmai' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
def remove_oauth_tokens(server_name: str, *, nunmai_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    NunmaiTokenStorage(server_name, nunmai_home=nunmai_home).remove()
...
def _server_declined_cimd(storage: "NunmaiTokenStorage | None") -> bool:
    ...
    in its 401 branch — after Nunmai must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "NunmaiTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def login_connect_timeout(config: dict) -> float:
    """Connect bound for an interactive OAuth login probe (CLI ``nunmai mcp login``, dashboard and
...
def _configure_callback_port(cfg: dict, storage: "NunmaiTokenStorage | None" = None) -> int:
...
def _invalidate_tokens_on_client_change(
    storage: "NunmaiTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: nunmai mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "NunmaiTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Nunmai is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `nunmai mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if NunmaiOAuthClientProvider is None:
        from tools.mcp_oauth_provider import NunmaiProviderMixin

        NunmaiOAuthClientProvider = type("NunmaiOAuthClientProvider", (NunmaiProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Nunmai' token-endpoint fixes (see ``NunmaiProviderMixin``).",
            "__module__": __name__, "_nunmai_logger": logger})
    return NunmaiOAuthClientProvider(server_url=server_url, **kwargs)
