...
URL of Nunmai' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``NunmaiTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Nunmai Engine"
...
from nunmai_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__nunmai_user_skipped__"
...
def _get_token_dir(nunmai_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses NUNMAI_HOME so each profile gets its own OAuth tokens.
    Layout: ``NUNMAI_HOME/mcp-tokens/``
    """
    from nunmai_constants import get_nunmai_home

    base = Path(nunmai_home) if nunmai_home is not None else Path(get_nunmai_home())
...
def _cached_redirect_port(storage: "NunmaiTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Nunmai restarts and chooses a
...
def _cached_redirect_uri(storage: "NunmaiTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``nunmai mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `nunmai mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# NunmaiTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class NunmaiTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NUNMAI_HOME/mcp-tokens/<server_name>.json         -- tokens
        NUNMAI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NUNMAI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        NUNMAI_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, nunmai_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._nunmai_home = Path(nunmai_home) if nunmai_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._nunmai_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._nunmai_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._nunmai_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._nunmai_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Nunmai records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``nunmai mcp login`` /
        ``nunmai mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._nunmai_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Nunmai.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://nunmai.in/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Nunmai' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nunmai mcp login <server>` later to "
...
NunmaiOAuthClientProvider: Any = None


def _get_nunmai_oauth_provider_class() -> type | None:
    global NunmaiOAuthClientProvider
    if NunmaiOAuthClientProvider is not None:
        return NunmaiOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _NunmaiOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._nunmai_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_nunmai_token_user_agent", None)
            if ua:
    ...
    _NunmaiOAuthClientProvider.__name__ = "NunmaiOAuthClientProvider"
    _NunmaiOAuthClientProvider.__qualname__ = "NunmaiOAuthClientProvider"
    NunmaiOAuthClientProvider = _NunmaiOAuthClientProvider
    return NunmaiOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = NunmaiTokenStorage(server_name, nunmai_home=nunmai_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Nunmai only decides whether a
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Nunmai picks
...
def _has_cached_client_info(storage: "NunmaiTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "NunmaiTokenStorage | None") -> bool:
    ...
    Nunmai has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "NunmaiTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Nunmai would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Nunmai Engine" / "Nunmai" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Nunmai Engine")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Nunmai is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: nunmai mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Nunmai auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Nunmai is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = NunmaiTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`nunmai mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_nunmai_oauth_provider_class()
