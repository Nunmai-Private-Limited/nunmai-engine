...
def _deepinfra_model(section: Dict[str, Any], kind: str) -> Optional[str]:
    ...
    from nunmai_cli.models import deepinfra_model_ids
...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the Nunmai-managed selection too: the resolver returns the user's
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``nunmai_constants.set_nunmai_home_override``
