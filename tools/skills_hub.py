#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Nunmai Skills Hub.
...
Used by nunmai_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _nunmai_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            nunmai_meta = metadata.get("nunmai", {})
            if isinstance(nunmai_meta, dict):
                tags = nunmai_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Nunmai Research) but not activated
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``nunmai update``. Rather than telling them to update first, resolve
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                nunmai_meta = meta_block.get("nunmai", {})
                if isinstance(nunmai_meta, dict):
                    tags = nunmai_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``nunmai_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Nunmai centralized index source
# ---------------------------------------------------------------------------

NUNMAI_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
NUNMAI_INDEX_TTL = 6 * 3600  # 6 hours


def _nunmai_index_cache_file() -> Path:
    return _index_cache_dir() / "nunmai-index.json"


def _load_nunmai_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for NUNMAI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    nunmai_index_cache_file = _nunmai_index_cache_file()
    if nunmai_index_cache_file.exists():
        try:
            age = time.time() - nunmai_index_cache_file.stat().st_mtime
            if age < NUNMAI_INDEX_TTL:
                return json.loads(nunmai_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Nunmai index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Nunmai index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Nunmai index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        nunmai_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        nunmai_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class NunmaiIndexSource(SkillSource):
    """Skill source backed by the centralized Nunmai Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_nunmai_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nunmai-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nunmai-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        NunmaiIndexSource(auth=auth), # Centralized index (search + resolved install paths)
