...
When any profile carries ``ui_meta['nunmai-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient process NUNMAI_HOME (env, else the platform default) as a string."""
...
def _nunmai_root(home: Path) -> Path:
    """Root ~/.nunmai for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @nunmai.
    return "nunmai" if name == "default" else name
...
def _roster(root: Path) -> list[tuple[str, Path]]:
    ...
    from nunmai_constants import PROFILE_ID_RE, named_profile_is_live
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['nunmai-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("nunmai-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "nunmai-bots")) is not None
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    return _swallow(lambda: _any_managed(_nunmai_root(_resolve_home(home))), False)
...
def _profile_role(profile_dir: Path) -> str:
    ...
    WHO to message for a job. A friendly ``display_name`` (``nunmai profile rename``) that
...
def _display_name(name: str, profile_dir: Path) -> str:
    ...
    then profile.yaml ``display_name`` (``nunmai profile rename``), else the @handle — the
    renamed primary signs as ``Maia (@nunmai)``, not ``nunmai (@nunmai)`` (#89720)."""
    return next((n for n in _friendly_names(profile_dir) if n), None) or _handle(name)


# Tokens the Desktop mention parser reserves; a bot titled "Nunmai" never hijacks @nunmai.
_RESERVED_ALIASES = frozenset({"all", "everyone", "user", "default", "nunmai"})
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``nunmai peer``) from config.yaml, read
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `nunmai peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _nunmai_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Nunmai profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient NUNMAI_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_nunmai_home_override(str(resolved))
        try:
