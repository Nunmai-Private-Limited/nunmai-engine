"""Central registry for all nunmai-engine tools: each tool file calls ``registry.register()``
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_nunmai_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales, so results are cached ~30 s: env-var flips via ``nunmai tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "NUNMAI_SESSION_ID",
    "NUNMAI_BROWSER_CONTROL_PRINCIPAL",
    "NUNMAI_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Nunmai-home override
...
def _core_tools_gated_by(fn: Callable) -> Set[str]:
    """Names of ``_NUNMAI_CORE_TOOLS`` members whose registered ``check_fn`` is *fn*."""
    try:
        from toolsets import _NUNMAI_CORE_TOOLS
        core = frozenset(_NUNMAI_CORE_TOOLS)
    except Exception:
        return set()
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``nunmai tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved NUNMAI_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("nunmai_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``nunmai_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``nunmai_plugins.pkg.handlers`` is still owned by the
                # ``nunmai_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``nunmai tools enable`` lands quickly."""
        result = []
