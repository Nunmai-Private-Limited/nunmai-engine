"""Central registry for all nunmai-engine tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (nunmai_constants itself is stdlib-only, so no cycle).
...
# timescales. Cache results for ~30 s so env-var flips via ``nunmai tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Nunmai-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("NUNMAI_SESSION_ID", ""),
            get_session_env("NUNMAI_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("NUNMAI_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``nunmai tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved NUNMAI_HOME. A
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``nunmai_plugins.{name}``), not the exact module string.
                # A handler defined in ``nunmai_plugins.pkg.handlers`` is
                # still owned by the ``nunmai_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``nunmai tools enable foo``)
