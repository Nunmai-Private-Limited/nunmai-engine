...
from nunmai_cli.fallback_config import scoped_fallback_chain

logger = logging.getLogger("tools.delegate_tool")  # log-record parity with the origin module

# Runtime-provider sentinel for providers that are not natively known; must
# match nunmai_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _load_config() -> dict:
    ...
    (follows NUNMAI_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``NUNMAI_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("NUNMAI_IGNORE_USER_CONFIG") != "1":
        try:
...
def _resolve_child_runtime(
    ...
    or is Nunmai Portal (dual-wire); a pinned ``delegation.command`` must exist on PATH or the spawn fails loudly;
    ...
    # otherwise). Nunmai Portal is dual-wire within one provider (anthropic/* → Messages, else chat_completions), so
    ...
    # would pin a child Nunmai/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
