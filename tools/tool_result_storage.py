...
is ALWAYS host-side ``$NUNMAI_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = os.path.join(tempfile.gettempdir(), "nunmai-results")
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $NUNMAI_HOME/cache/spillover; returns path str or None.
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/nunmai-results" if temp_dir else STORAGE_DIR
