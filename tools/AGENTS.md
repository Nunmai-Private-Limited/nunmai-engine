...
- **Paths in schema descriptions use `display_nunmai_home()`** (schema is built at import, after
  `_apply_profile_override()` set `NUNMAI_HOME`). **State files use `get_nunmai_home()`**, never
  `Path.home()/.nunmai`, so each profile gets its own state.
