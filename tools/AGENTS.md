...
For custom/local-only tools do NOT edit core: create `~/.nunmai/plugins/<name>/plugin.yaml` +
...
1. `tools/your_tool.py`:
   ...
   ```
2. `toolsets.py`: add the name to `_NUNMAI_CORE_TOOLS` (all platforms) or a new toolset. **Required**
   — discovery registers the schema, but a tool is only exposed if a toolset names it.
   `_NUNMAI_CORE_TOOLS` is the default bundle every platform's base toolset inherits, not dead code.
...
- **Paths in schema descriptions use `display_nunmai_home()`** (schema is built at import; under
  multiplex it shows the launch home, which is display-only). **State files use `get_nunmai_home()`**
  at call time, never `Path.home()/.nunmai` and never a module constant, so each served profile gets
  its own state.
...
- **`check_fn` answers reachability/opt-in for the profile it runs under, never surface.** Results
  are TTL-cached in `registry.py::_check_fn_cache` keyed by `nunmai_home_key()`, and one process
...
- New tools integrate with existing setup UX (`nunmai tools`, `nunmai setup`, auto-install) rather
  than a raw env var; secrets go in `OPTIONAL_ENV_VARS` (`nunmai_cli/AGENTS.md`).
...
`nunmai tools` (curses) or `tools.<platform>.enabled/disabled` in config.yaml. `browser_exec`
...
`nunmai_constants.apply_subprocess_home_env`, `env_passthrough.py::resolve_passthrough_value`) is
how a terminal, `execute_code`, background process, delegation child, ACP or MCP stdio child gets
its environment; a child that acts FOR the served profile (`nunmai -p X` workers, `key_cmd`
...
imports against a temp `NUNMAI_HOME` (see `tests/tools/test_approval_config_readonly.py`).
