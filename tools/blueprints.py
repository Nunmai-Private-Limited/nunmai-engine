...
``metadata.nunmai.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nunmai.blueprint`` is a mapping with a
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.nunmai.blueprint`` block) ready for ``nunmai skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"nunmai": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
