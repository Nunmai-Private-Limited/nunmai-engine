...
`nunmai skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nunmai.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nunmai.blueprint`` is a mapping containing
    ...
    nunmai = meta.get("nunmai") if isinstance(meta, dict) else None
    blueprint = nunmai.get("blueprint") if isinstance(nunmai, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nunmai.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.nunmai.blueprint`` block) they can hand
    to ``nunmai skills publish`` to share. ``body`` is the plain-language
