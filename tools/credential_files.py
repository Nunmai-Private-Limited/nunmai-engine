...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.nunmai",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``NUNMAI_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside NUNMAI_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because NUNMAI_HOME is exactly where
    ...
    """
    nunmai_home = _resolve_nunmai_home()

    # Reject absolute paths — they bypass the NUNMAI_HOME sandbox entirely.
    ...
    host_path = nunmai_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape NUNMAI_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, nunmai_home)
    ...
    # inside NUNMAI_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.nunmai/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="nunmai-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching nunmai_constants.get_nunmai_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_nunmai_dir()`` for backward compatibility with old directory layouts.
    """
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_nunmai_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.nunmai``,
    SSH ``<remote_home>/.nunmai``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    backend's Nunmai cache lands):

    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.nunmai`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.nunmai`` is shell-expanded by the remote shell, so tool
