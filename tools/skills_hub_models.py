...
def _nunmai_tags(fm: dict) -> Any:
    """``metadata.nunmai.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    nunmai_meta = metadata.get("nunmai", {}) if isinstance(metadata, dict) else None
    return nunmai_meta.get("tags", []) if isinstance(nunmai_meta, dict) else []
