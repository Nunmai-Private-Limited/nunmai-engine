...
def return_to_args(*, op: Optional[str] = None) -> dict[str, Any]:
    ...
    field and keeps the vendor's own done page. The dev build registers ``nunmai-dev://`` instead, and
    announces itself to its backend with ``NUNMAI_DESKTOP_DEV_SERVER``."""
    if session_platform() != "desktop":
        return {}
    target = "nunmai-desktop-dev" if os.environ.get("NUNMAI_DESKTOP_DEV_SERVER") else "nunmai-desktop"
