...
# Where the vendor's done page sends the browser after consent; the dev desktop registers nunmai-dev://.
ConnectorReturnTarget = Literal["nunmai-desktop", "nunmai-desktop-dev", "portal"]

# Nunmai dispatch caps batches lower, so client-side chunking is deliberately absent.
WIRE_BATCH_MAX = 25
...
class ConnectorExecuteCall(_Wire):
    ...
    # On the wire for the multi-account switch; never sent by nunmai today, because the vendor answers
...
class ConnectorConnectionsRequest(_Wire):
    ...
    return_to: Optional[ConnectorReturnTarget] = Field(default=None, alias="returnTo")
    # The caller's operation id, echoed on the nunmai://connections/done link.
    op: Optional[str] = Field(default=None, min_length=1, max_length=128)
