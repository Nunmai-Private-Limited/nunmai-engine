...
def operation_session_key(session_id: Optional[str]) -> str:
    ...
    (``NUNMAI_SESSION_KEY``), falling back to the agent's session id where no gateway bound one.
    ...
    return get_session_env("NUNMAI_SESSION_KEY", "") or str(session_id or "")


def session_platform() -> str:
    from gateway.session_context import get_session_env

    platform = get_session_env("NUNMAI_SESSION_PLATFORM", "") or get_session_env("NUNMAI_SESSION_SOURCE", "")
