...
def probe_with_rollback(
        server_name: str, cfg: dict, nunmai_home: str, flow, reconnect_live: bool, *,
    ...
    from nunmai_cli.mcp_config import _oauth_tokens_present, _probe_single_server
    from tools.mcp_dashboard_oauth import exception_message
    from tools.mcp_oauth import NunmaiTokenStorage, login_connect_timeout
    ...
    def undo() -> None:
        ...
        if flow is not None and _ACTIVE.get((nunmai_home, server_name)) not in (None, flow):
            return
...
# (nunmai home, server) -> the newest card attempt. A retry or a new operation replaces an attempt
...
def run_worker(
        nunmai_home: str, server_name: str, cfg: dict, reconnect_live: bool, *,
    ...
    try:
        ...
        try:
            ...
            secret_token = set_secret_scope(
                {**build_profile_secret_scope(Path(nunmai_home)), **(env or {})}, profile_home=nunmai_home)
            if not (reuse_saved and flow is not None
                ...
                with force_interactive_oauth(), dashboard_oauth_flow(flow):
                    probe_with_rollback(
                        server_name, cfg, nunmai_home, flow, reconnect_live, on_commit=on_commit)
        finally:
        ...
        if flow is not None:
            ...
            with _COMMIT_GUARD:
                if _ACTIVE.get((nunmai_home, server_name)) is flow:
                    _ACTIVE.pop((nunmai_home, server_name), None)
...
def start(
    ...
    flow = DashboardOAuthFlow(
        ...
        nunmai_home=nunmai_home, redirect_uri="", reconnect_live=False)
    with _COMMIT_GUARD:
        older = _ACTIVE.get((nunmai_home, server_name))
        _ACTIVE[(nunmai_home, server_name)] = flow
    ...
    threading.Thread(
        target=run_worker, args=(nunmai_home, server_name, cfg, False),
