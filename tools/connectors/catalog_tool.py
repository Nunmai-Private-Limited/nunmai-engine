...
NOT_HERE = (
    "manage_catalog is available only in the Nunmai desktop app, where the approval card can be drawn. "
    "Tell the user to install from a terminal instead: `nunmai plugins install <id>` for a plugin, "
    "`nunmai skills install <id>` for a skill."
)
...
MANAGE_CATALOG_SCHEMA = {
    "name": "manage_catalog",
    "description": (
        "Find and install Nunmai catalog plugins and hub skills for the user. 'search' lists matches "
...
def _plugin_rows(query: str) -> List[Dict[str, Any]]:
    from nunmai_cli.plugin_catalog import filter_entries, load_catalog_live
    from nunmai_cli.plugins_cmd import PluginOperationError, _read_install_metadata
