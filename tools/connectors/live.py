...
session's profile scope) resolves it through ``nunmai_home_key``."""
...
def _profile_key(profile_home: Optional[str]) -> str:
    ...
    return nunmai_home_key(profile_home or get_process_nunmai_home())
...
def open(operation: ConnectionOperation) -> None:  # noqa: A001 - the verb is the API
    operation.profile_key = nunmai_home_key()
