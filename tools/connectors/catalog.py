...
def target_scope(profile: str):
    ...
    scope = (launch_profile_runtime_scope(server._nunmai_home) if home is None
...
class HostInstaller:
    ...
    def refuse(self, entry: Any) -> None:
        ...
        from nunmai_cli.plugins_cmd_catalog import _refuse_unsupported_catalog_platform, raise_if_removed
    ...
    def skill_meta(self, identifier: str) -> Optional[Dict[str, Any]]:
        ...
        from nunmai_cli.skills_hub import _sources
...
class _Runner:
    ...
    def _resolve(self, target: Target) -> None:
        if target.kind == "plugin":
            ...
            if entry is None:
                raise LookupError(f"'{target.name}' is not in the Nunmai plugin catalog")
...
def _plugin_row(entry: Any) -> Dict[str, Any]:
    requirements = [f"Nunmai {entry.requires_nunmai}"] if entry.requires_nunmai else []
    requirements += [f"{name} environment variable" for name in entry.capabilities.requires_env]
    from nunmai_cli.plugin_catalog_presence import presence
...
def _save_credentials(env: Dict[str, str]) -> None:
    from nunmai_cli.config import save_env_value, validate_env_var_name_for_write
