...
def _configured_names() -> List[str]:
    from nunmai_cli.mcp_catalog import installed_servers
...
class _CatalogBackend:
    ...
    def installs_with_oauth(self, name: str) -> bool:
        ...
        its token comes from ``nunmai auth <provider>``, so the plain probe covers it."""
    ...
    def start_install_oauth(self, name: str, env: Dict[str, str]) -> Any:
        ...
        from nunmai_cli.mcp_catalog import card_install_config
        from tools.connectors import mcp_oauth
    ...
    def install(self, name: str, env: Dict[str, str]) -> List[str]:
        ...
        from nunmai_cli.mcp_catalog import _inline_non_secret_value, card_install_config
        from nunmai_cli.mcp_config import _probe_single_server, _save_mcp_server
    ...
    def enable(self, name: str) -> None:
        ...
        from nunmai_cli.web_routers._common import config_write_scope
...
def _late_key(operation: ConnectionOperation) -> Tuple[str, str]:
    ...
    return (operation.profile_key or nunmai_home_key(), operation.session_key)
...
def adopt_late_connections(agent: Any) -> List[str]:
    ...
    key = (nunmai_home_key(), session_key)
...
def _install_now(runner: _Runner, operation: ConnectionOperation, target: Target, env: Dict[str, str]) -> None:
    ...
    if missing:
        ...
        _fail(operation, target, f"set {', '.join(missing)} in the environment or "
                                 f"{display_nunmai_home()}/.env, then install again")
        return
    actor = _actor(target)
    if _installs_with_oauth(runner, target):
