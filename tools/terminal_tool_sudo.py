...
def _in_delegated_child_context() -> bool:
    ...
    threads and inherit process-wide ``NUNMAI_INTERACTIVE=1``, which does NOT mean they can reach
...
def _prompt_for_sudo_password(timeout_seconds: int = 45, *, command: str = "") -> str:
    ...
    try:
        ...
        os.environ.pop("NUNMAI_SPINNER_PAUSE", None)
...
def _transform_sudo_command(
    ...
    # delegate_task children inherit NUNMAI_INTERACTIVE=1 (and possibly a stale thread-local
    ...
    should_prompt_for_sudo = (
        env_var_enabled("NUNMAI_INTERACTIVE") or _get_sudo_password_callback() is not None
