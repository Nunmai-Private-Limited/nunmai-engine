...
`mcp_tool.py`, and `nunmai_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# NunmaiMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_nunmai_provider_class() -> Optional[type]:
    ...
    class NunmaiMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`NunmaiTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, NunmaiTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Nunmai' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._nunmai_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import NunmaiTokenStorage
                        if isinstance(storage, NunmaiTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import NunmaiTokenStorage
            if not isinstance(storage, NunmaiTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``nunmai mcp reauth``.
            ...
            back to ``nunmai mcp reauth``.
            """
            try:
                if self._nunmai_preregistered:
                    return
                ...
                # walking back into the same refusal. `nunmai mcp login`
    ...
    return NunmaiMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_NUNMAI_PROVIDER_CLS: Optional[type] = _make_nunmai_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`NunmaiMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `nunmai mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``nunmai mcp remove <name>`` and (indirectly) by
        ``nunmai mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, nunmai_home), None)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, nunmai_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        async with entry.lock:
            tokens_path = _get_token_dir(nunmai_home) / f"{_safe_filename(server_name)}.json"
            try:
