...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    Ported in spirit from nearai/ironclaw#5029 (dual line/byte cap on ``read_file``). Where nunmai
...
def read_file_tool(path: str, offset: int = 1, limit: int = DEFAULT_READ_LIMIT, task_id: str = "default") -> str:
    ...
    → Nunmai internal denylist → document extraction → binary-extension guard
    ...
    try:
        ...
        # Nunmai internal denylist (prompt injection via catalog metadata,
