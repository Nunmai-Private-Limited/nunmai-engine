...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``nunmai_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from nunmai_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where nunmai previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_nunmai_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Nunmai.
...
def _get_nunmai_config_resolved() -> str | None:
    """Return the resolved absolute path of the Nunmai config file (cached)."""
    global _nunmai_config_resolved, _nunmai_config_resolved_loaded
    if _nunmai_config_resolved_loaded:
        return _nunmai_config_resolved
    _nunmai_config_resolved_loaded = True
    try:
        from nunmai_cli.config import get_config_path
        _nunmai_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _nunmai_config_resolved = str(Path(_expand_tilde("~/.nunmai/config.yaml")).resolve())
        except Exception:
            _nunmai_config_resolved = None
    return _nunmai_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Nunmai config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    nunmai_config = _get_nunmai_config_resolved()
    if nunmai_config and (resolved == nunmai_config or normalized == nunmai_config):
        return (
...
# .cursorrules (or a project-local .nunmai config tree) outlives the current
...
def _get_real_nunmai_home() -> str | None:
    """Return the realpath of the authoritative Nunmai home (cached)."""
    global _real_nunmai_home_cached, _real_nunmai_home_loaded
    if _real_nunmai_home_loaded:
        return _real_nunmai_home_cached
    _real_nunmai_home_loaded = True
    try:
        from nunmai_constants import get_nunmai_home
        _real_nunmai_home_cached = os.path.realpath(str(get_nunmai_home()))
    except Exception:
        try:
            _real_nunmai_home_cached = os.path.realpath(_expand_tilde("~/.nunmai"))
        except Exception:
            _real_nunmai_home_cached = None
    return _real_nunmai_home_cached
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.nunmai home is governed by its own guards
    ...
    # ``.nunmai`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_nunmai_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .nunmai config dirs (e.g. <repo>/.nunmai/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.nunmai`` — matching
        # any ancestor named .nunmai would gate every write inside a
        # checkout that happens to live under ~/.nunmai (e.g. the
        # nunmai-engine repo itself at ~/.nunmai/nunmai-engine).
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Nunmai mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Nunmai state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Nunmai process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Nunmai scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Nunmai internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under NUNMAI_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == NUNMAI_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with nunmai_tools.write_file() for very "
            "large files."
        )
