...
One ``CDPSupervisor`` runs per Nunmai ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Nunmai logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "nunmai-dialog-bridge.invalid"
