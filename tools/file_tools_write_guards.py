...
# resolve per call in production (below) because ``get_nunmai_home()`` /
...
_nunmai_config_resolved: str | None = None
_nunmai_config_resolved_loaded = False
_real_nunmai_home_cached: str | None = None
_real_nunmai_home_loaded = False
...
def _nunmai_home_real() -> str:
    from nunmai_constants import get_nunmai_home
    return os.path.realpath(str(get_nunmai_home()))


def _get_nunmai_config_resolved() -> str | None:
    """Resolved absolute path of the Nunmai config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``NUNMAI_HOME`` scope (#107327);
    a test may pin it via ``_nunmai_config_resolved`` + ``_nunmai_config_resolved_loaded``."""
    if _nunmai_config_resolved_loaded:
        return _nunmai_config_resolved
    try:
        ...
        # home rather than the active multiplex ``NUNMAI_HOME`` — comparing
        ...
        # ``get_nunmai_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from nunmai_constants import get_nunmai_home
            return str((Path(str(get_nunmai_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_nunmai_home() -> str | None:
    """Realpath of the authoritative Nunmai home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``NUNMAI_HOME`` scope (#107327);
    a test may pin it via ``_real_nunmai_home_cached`` + ``_real_nunmai_home_loaded``.
    Consumers exempting a whole TREE want ``_nunmai_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_nunmai_home_loaded:
        return _real_nunmai_home_cached
    try:
        return _nunmai_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_nunmai_config_resolved``): re-derive from ``get_nunmai_home()``
        # rather than ``_expand_tilde("~/.nunmai")`` so the protected-instruction
        ...
        # the consumer: the ``~/.nunmai`` exemption is skipped, so the gate runs.
        try:
            from nunmai_constants import get_nunmai_home
            return os.path.realpath(str(get_nunmai_home()))
        except Exception:
...
def _nunmai_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the Nunmai home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the Nunmai ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.nunmai`` component rule, which
    gated them like a project-local ``<repo>/.nunmai/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.nunmai`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``NUNMAI_HOME`` alone."""
    home = _get_real_nunmai_home()
    if not home:
        return ()
    try:
        from nunmai_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    nunmai_config = _get_nunmai_config_resolved()
    if nunmai_config and nunmai_config in candidates:
        return (
            f"Refusing to write to Nunmai config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.nunmai/config.yaml directly or use 'nunmai config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .nunmai tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from nunmai_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.nunmai itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.nunmai`` component rule, which would match the home.
    # ``_nunmai_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.nunmai/<file> cannot read as project-local ``.nunmai`` config.
    for real_home in _nunmai_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .nunmai config dirs (<repo>/.nunmai/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.nunmai.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".nunmai":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Nunmai mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.nunmai" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Nunmai state (a write the host never reads). Not profile isolation — that guard was
