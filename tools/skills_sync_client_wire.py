...
Independent of local skill state (no ``~/.nunmai`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Nunmai speaks "
...
class SyncClient:
    """Sync client bound to a base URL + Nunmai bearer. Org refs/objects live behind SEPARATE ``org/``
