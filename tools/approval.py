...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Nunmai YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (NUNMAI_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``nunmai chat -q``
    exports NUNMAI_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    Single-query ``-q`` and cron clear the presence trio: ``nunmai chat -q`` exports
    NUNMAI_INTERACTIVE=1 for sudo prompts, and a gateway sets NUNMAI_EXEC_ASK=1 at startup and
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               ...
                               "NUNMAI_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set NUNMAI_INTERACTIVE or "
                       "NUNMAI_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_nunmai_interactive_context': ('tools.approval_context', 'reset_nunmai_interactive_context'),
    ...
    'set_nunmai_interactive_context': ('tools.approval_context', 'set_nunmai_interactive_context'),
}
