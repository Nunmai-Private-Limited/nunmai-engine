...
# Nunmai session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["NUNMAI_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_nunmai_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("nunmai_interactive", None)
...
def _is_interactive_cli() -> bool:
    ...
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("NUNMAI_INTERACTIVE")
...
def _get_session_platform() -> str:
    ...
    return _session_env("NUNMAI_SESSION_PLATFORM")


def _is_cron_approval_context() -> bool:
    """True when the current approval decision is running inside cron."""
    return is_truthy_value(_session_env("NUNMAI_CRON_SESSION"))
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``NUNMAI_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``nunmai chat -q`` exports
    ``NUNMAI_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("NUNMAI_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set NUNMAI_GATEWAY_SESSION; concurrent paths bind
    NUNMAI_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("NUNMAI_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``NUNMAI_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
...
def approval_timeout_notice_kwargs() -> dict:
    ...
    ``90 seconds``) and a tripled ``approvals.timeout`` value the user can paste into ``nunmai config set``."""
