...
~/.nunmai/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
# retarget NUNMAI_HOME after import, and frozen constants would resolve (and for
...
# once under the launch NUNMAI_HOME and later scope requests to a different profile via
# set_nunmai_home_override(). See #65828.
...
def _nunmai_home() -> Path:
    return _live(NUNMAI_HOME, _NUNMAI_HOME_AT_IMPORT, get_nunmai_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _nunmai_home() / "skills")
...
# Written by `nunmai profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors nunmai_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # NUNMAI_BUNDLED_SKILLS env first, then repo-relative
...
def _write_manifest(entries: Dict[str, str]):
    ...
    mkdir_under_nunmai_home(_manifest_file().parent)
    try:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `nunmai skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `nunmai skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.nunmai/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``nunmai-engine``)."""
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing NUNMAI_HOME / malicious manifest entry).
