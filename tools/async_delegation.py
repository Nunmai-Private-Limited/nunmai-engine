...
def _connect() -> sqlite3.Connection:
    from nunmai_cli.sqlite_util import open_db
    # Same state.db as nunmai_state.SessionDB -- reuse its owner-only (0600)
    # hardening so this writer doesn't create/leave the file (and its WAL
    # sidecars) at the process umask. See nunmai_state._secure_state_db_files.
    from nunmai_state import _secure_state_db_files
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from nunmai_state_repair import apply_durability_barriers
    from nunmai_state_schema import reconcile_state_schema
...
def _transaction():
    from nunmai_cli.sqlite_util import transaction
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"NUNMAI_SESSION_{k.upper()}", ""))}
...
def _replay_pending(conn, rows, target_queue, now: float) -> int:
    ...
    home, restored = nunmai_home_key(get_nunmai_home()), 0
...
def sweep_orphaned_completions(target_queue, *, now: Optional[float] = None) -> int:
    ...
    home = nunmai_home_key(get_nunmai_home())
    with _orphan_lock:
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``NUNMAI_SESSION_ID``
    ...
    request-scoped ``NUNMAI_SESSION_CHAT_ID`` (raw X-Nunmai-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("NUNMAI_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("NUNMAI_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
