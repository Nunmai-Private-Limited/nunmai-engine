...
selectable via ``nunmai tools`` → Image Generation; the active model is
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Nunmai cache roots can be translated without side effects. SSH can still
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `nunmai tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  3. Configure a different image_gen provider via `nunmai tools` "
        "→ Image Generation (run `nunmai plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `nunmai plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `nunmai tools` → Image Generation."
                ),
...
# model/provider via `nunmai tools` or `/skills`.
