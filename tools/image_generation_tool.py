#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``nunmai tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `nunmai tools` selection, or ``None`` for direct FAL.
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error(
                ...
                "the Nunmai Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            if billing is not None:
                raise ValueError(
                    f"Nunmai Subscription gateway rejected model '{model}' (HTTP {status}): {billing}") from exc
            gateway_message = ""
            if status in {401, 402, 403}:
                gateway_message = "\n\n" + nous_tool_gateway_unavailable_message(
                    "managed FAL image generation", force_fresh=True)
            raise ValueError(
                f"Nunmai Subscription gateway rejected model '{model}' (HTTP {status}). This model "
                f"may not yet be enabled on the Nunmai Portal's FAL proxy. Either:\n"
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.nunmai", "singularity": "/root/.nunmai",
                          "modal": "/root/.nunmai", "ssh": "~/.nunmai"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.nunmai"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `nunmai tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nunmai account that has the managed FAL gateway enabled "
                     "(`nunmai setup`)")
    lines.append("  3. Configure a different image_gen provider via `nunmai tools` → Image Generation "
                 "(run `nunmai plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `nunmai plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `nunmai tools` → "
...
def _maybe_route_managed_model(
    ...
    if provider is None:
        if plugin_name == "krea":
            return None
        return _provider_error(
            f"image_gen.model='{model_id}' is a Nunmai Portal model but the Portal image backend is not "
            f"available. Pick another model via `nunmai tools` → Image Generation.", "provider_not_registered")
