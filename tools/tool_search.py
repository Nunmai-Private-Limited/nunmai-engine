...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._NUNMAI_CORE_TOOLS``)
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``nunmai_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
# Session-gated GUI toolsets: off ``_NUNMAI_CORE_TOOLS`` so non-GUI clients never pay
