...
# ``nunmai send`` CLI, the kanban notifier and the opt-in MCP server import the helpers.
...
def _handle_send(args):
    ...
    # (`nunmai send` MESSAGE) and crash the UTF-8 marshal inside platform SDK
    # request bodies (feishu/lark, #113799). Every send_message caller (model tool
    # call, `nunmai send`, dashboard console) enters here, so scrub once before the
...
def _not_configured_error(platform_name, platform, entry):
    ...
    process actually read (a hardcoded ``~/.nunmai`` does not exist on a Windows or profile home)."""
    ...
    # gateway started from the default root (the reporter's shell had NUNMAI_HOME=<root>/profiles/<p>)
    # never reads this profile's .env at all.
    try:
        from gateway.status import read_runtime_status, runtime_status_pid_is_live
        from nunmai_constants import get_default_nunmai_root, nunmai_home_key
        root = get_default_nunmai_root()
        gateways = [(home, read_runtime_status())]
        if nunmai_home_key(root) != nunmai_home_key(home):
...
def _home_chat_id(config, platform, platform_name):
    ...
    return None, (f"No home channel set for {platform_name} to determine where to send the message. "
                  ...
                  f"or set a home channel via: nunmai config set {home_env} <channel_id>")
...
def _mirror_sent_message(platform_name, chat_id, mirror_text, thread_id):
    ...
    try:
        ...
        return bool(mirror_to_session(
            ...
            source_label=get_session_env("NUNMAI_SESSION_PLATFORM", "cli"),
...
def _maybe_skip_cron_duplicate_send(platform_name: str, chat_id: str, thread_id: str | None):
    ...
    from gateway.session_context import get_session_env
    auto_platform = get_session_env("NUNMAI_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    auto_chat_id = get_session_env("NUNMAI_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not (auto_platform and auto_chat_id and auto_platform == platform_name and auto_chat_id == str(chat_id)
            and (get_session_env("NUNMAI_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None) == thread_id):
        return None
...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "message": {
                "type": "string",
                "description": "The message text to send. To send an image or file, include MEDIA:<local_path> (e.g. 'MEDIA:~/.nunmai/cache/scratch/report.pdf') in the message — the platform will deliver it as a native media attachment."
            },
            "emoji": {
                "type": "string",
