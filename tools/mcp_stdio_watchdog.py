...
<url>``) is spawned as a direct child of the Nunmai process. Nunmai's own
...
Nunmai process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Nunmai startup's opt-in
