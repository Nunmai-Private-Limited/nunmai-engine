...
def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Nunmai root. Avoids ``Path.home()`` /
    ``nunmai_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
    reads HOME/passwd, which the conftest never rewrites."""
    try:
        home = Path(os.path.expanduser("~"))
        if sys.platform == "win32":
            base = os.environ.get("LOCALAPPDATA", "").strip()
            root = Path(base) / "nunmai" if base else home / "AppData" / "Local" / "nunmai"
        else:
            root = home / ".nunmai"
        return root.resolve()
    except Exception:
...
#: Exported by the hermetic conftest alongside the NUNMAI_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``NUNMAI_HOME`` to
...
# under Nunmai test : isolation", and it inherits into subprocess children by default — so a : child that
# received the patched ``NUNMAI_HOME`` also received the marker, : and a child that resolves a production DB
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the NUNMAI_HOME redirect together, ancestry
    ...
    loses at the same moment it loses the ``NUNMAI_HOME`` redirect: that child aims at the production DB
...
# single-process run over tests/nunmai_cli/ accumulated 16-25 GB RSS (OOM
