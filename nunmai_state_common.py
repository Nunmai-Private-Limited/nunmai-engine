...
Extracted verbatim from nunmai_state.py so the SessionDB mixin modules
(nunmai_state_search / nunmai_state_schema / nunmai_state_portability) can
reference them without importing nunmai_state (which would be a cycle).
nunmai_state re-imports every name here for backward compatibility.
"""
...
# `nunmai sessions optimize-storage` rebuilds it on a capable host.
...
# service, the Desktop app's `nunmai serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `nunmai_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not nunmai_state)
# because the search/schema mixins cannot import nunmai_state (cycle).
#
