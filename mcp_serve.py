"""
Nunmai MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Nunmai-specific extra)

Usage:
    nunmai mcp serve
    nunmai mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NUNMAI_HOME."""
    try:
        from nunmai_constants import get_nunmai_home
        return get_nunmai_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NUNMAI_HOME", Path.home() / ".nunmai")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nunmai_constants import get_nunmai_home
        directory_file = get_nunmai_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NUNMAI_HOME", Path.home() / ".nunmai")
...
class EventBridge:
    ...
    This is the Nunmai equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from nunmai_constants import get_nunmai_home
            db_file = get_nunmai_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NUNMAI_HOME", Path.home() / ".nunmai")) / "state.db"
        try:
