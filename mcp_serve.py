"""
Nunmai MCP Server — expose messaging conversations as MCP tools (`nunmai mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Nunmai-specific
channels_list. Client config: {"mcpServers": {"nunmai": {"command": "nunmai", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("nunmai.mcp_serve")
...
def _nunmai_home() -> Path:
    try:
        ...
        return Path(os.environ.get("NUNMAI_HOME", Path.home() / ".nunmai"))


def _get_sessions_dir() -> Path:
    return _nunmai_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_nunmai_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_nunmai_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Nunmai analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Nunmai MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("nunmai", instructions=(
        "Nunmai Engine messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nunmai MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
