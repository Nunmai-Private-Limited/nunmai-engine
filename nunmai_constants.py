...
_UNSET = object()
_NUNMAI_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_NUNMAI_HOME_OVERRIDE", default=_UNSET
)
...
def _nunmai_home_from_env() -> Path:
    """Resolve NUNMAI_HOME from the process environment only.

    Reads the ``NUNMAI_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_nunmai_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_nunmai_home`
    and :func:`get_process_nunmai_home` so the two never drift.
    """
    val = os.environ.get("NUNMAI_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_nunmai_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``NUNMAI_HOME`` is
...
def get_nunmai_home() -> Path:
    """Return the Nunmai home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_nunmai_home_override`) → ``NUNMAI_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``NUNMAI_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``NUNMAI_HOME`` explicitly (see the systemd
    template in ``nunmai_cli/gateway.py`` and the kanban dispatcher in
    ``nunmai_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    ...
    return _nunmai_home_from_env()


def nunmai_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Nunmai home/profile directory.
...
def get_process_nunmai_home() -> Path:
    """Return the Nunmai home for the running process, ignoring task overrides.

    Unlike :func:`get_nunmai_home`, this never follows the context-local
    override set by :func:`set_nunmai_home_override`.  It resolves only the
    process ``NUNMAI_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_nunmai_root(). The function resolves
# NUNMAI_HOME against the native home on every call (~80us of path
...
# Its result depends only on (NUNMAI_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates NUNMAI_HOME mid-process.
_default_nunmai_root_memo: "tuple[str, str, Path] | None" = None


def get_default_nunmai_root() -> Path:
    """Return the root Nunmai directory for profile-level operations.

    In standard deployments this is the platform-native Nunmai home
    (``~/.nunmai`` on POSIX, ``%LOCALAPPDATA%\\nunmai`` on native Windows).

    In Docker or custom deployments where ``NUNMAI_HOME`` points outside
    ``~/.nunmai`` (e.g. ``/opt/data``), returns ``NUNMAI_HOME`` directly
    — that IS the root.

    In profile mode where ``NUNMAI_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.nunmai/profiles/coder``) and Docker
    ...
    env_home = os.environ.get("NUNMAI_HOME", "")
    if _default_nunmai_root_memo is not None:
        memo_native, memo_env, memo_result = _default_nunmai_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # NUNMAI_HOME is under ~/.nunmai (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — NUNMAI_HOME itself is the root
                result = env_path
    _default_nunmai_root_memo = (str(native_home), env_home, result)
    return result
...
# Files whose presence marks a directory as a real Nunmai home. A fresh home
...
def _is_nunmai_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<nunmai-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a Nunmai home: the classic ``~/.nunmai`` layout, a
    root carrying Nunmai-home marker files (Docker/custom ``NUNMAI_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``nunmai profile delete``), or the process's resolved
    default Nunmai root.
    """
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir():
            return True
        if any((root / marker).exists() for marker in _NUNMAI_HOME_MARKERS):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_nunmai_root().resolve(
            strict=False
        )
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Nunmai home (see :func:`_is_nunmai_profiles_root`). A default Nunmai home
    whose path merely contains a ``profiles`` segment
    (e.g. ``/tmp/foo/profiles/notahome/.nunmai``) is not a named profile,
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            return candidate
        # Stop at a default Nunmai home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".nunmai":
            return None
    return None
...
def assert_named_profile_home_live(path: str | Path) -> None:
    ...
    Default ``NUNMAI_HOME`` (not under ``profiles/``) is unchanged.
    """
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``NUNMAI_OPTIONAL_SKILLS``.
    """
    override = os.getenv("NUNMAI_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_nunmai_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Nunmai-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``NUNMAI_OPTIONAL_MCPS``.
    """
    override = os.getenv("NUNMAI_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_nunmai_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``NUNMAI_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<NUNMAI_HOME>/skills`` last-resort
    """
    override = os.getenv("NUNMAI_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_nunmai_dir(
    ...
    *,
    ...
    """Resolve a Nunmai subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to NUNMAI_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to NUNMAI_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Nunmai home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local NUNMAI_HOME.
...
def iter_nunmai_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Nunmai-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\nunmai\\node``. POSIX installs use
    ``$NUNMAI_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_nunmai_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with nunmaiManagedNodePathEntries() in
...
# Install tree root (this file lives at <install_root>/nunmai_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks nunmai-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Nunmai-managed Node trees live under ``$NUNMAI_HOME/node`` (or a profile's
    ``NUNMAI_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_nunmai_node_executable``
    used to trust file presence alone, so ``nunmai update`` would pick that
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        ...
        "upgrade until the app is closed (re-run `nunmai update` afterwards).",
        flush=True,
    )
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the nunmai.exe quarantine);
    ...
    match = re.search(
        rf"node-v{_NUNMAI_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$NUNMAI_HOME/node`` on POSIX.
    ...
    ``NUNMAI_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_nunmai_managed_node() -> str | None:
    """Install a Nunmai-managed Node tree and return its npm path.
    ...
    Nunmai never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$NUNMAI_HOME/node`` (the same tree a fresh install
    ...
    for directory in iter_nunmai_node_dirs():
...
def heal_nunmai_managed_node() -> bool:
    """Redownload Nunmai-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not nunmai_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NUNMAI_HOME": str(get_nunmai_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_nunmai_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_NUNMAI_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
...
def find_nunmai_node_executable(command: str) -> str | None:
    """Return a Nunmai-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_NUNMAI_NODE_TARGET_MAJOR``) heal the
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Nunmai-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Nunmai-managed installs.

    This is for Nunmai-owned subprocesses that should not be broken by a bad,
...
def with_nunmai_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_nunmai_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``nunmai update`` — leaving a **dangling symlink** that ``which`` still
...
def display_nunmai_home() -> str:
    """Return a user-friendly display string for the current NUNMAI_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.nunmai``
        profile:  ``~/.nunmai/profiles/coder``
        custom:   ``/opt/nunmai-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.nunmai``.  For code that needs a real ``Path``, use
    :func:`get_nunmai_home` instead.
    """
    ...
    try:
        ...
        # ``~/AppData\Local\nunmai/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``NUNMAI_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the nunmai-engine install tree (the directory this
    ...
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks nunmai-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means NUNMAI_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "normally stored under the nunmai home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    if not nunmai_home:
        return None
    profile_home = os.path.join(nunmai_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("NUNMAI_REAL_HOME") or os.getenv("NUNMAI_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Nunmai profile HOME.

    ``NUNMAI_HOME`` scopes Nunmai state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={NUNMAI_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{NUNMAI_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{NUNMAI_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Nunmai' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Nunmai itself runs inside WSL.
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_nunmai_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, nunmai_logging.py, nunmai_time.py, etc.).
    """
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``nunmai_cli`` modules using three different Windows
    ...
    A few sites outside ``nunmai_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``nunmai_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Nunmai itself. An ImportError
...
# `nunmai_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Nunmai.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("nunmai_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `nunmai update` is exactly
