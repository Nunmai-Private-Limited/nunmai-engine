"""Shared constants for Nunmai Engine.
...
from nunmai_platform.host.runtime import _detect_container, is_container, is_termux, is_wsl  # noqa: F401

_profile_fallback_warned: bool = False
_UNSET = object()
_NUNMAI_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_NUNMAI_HOME_OVERRIDE", default=_UNSET)
...
def set_nunmai_home_override(path: str | Path | None) -> Token:
    """Set a context-local Nunmai home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _NUNMAI_HOME_OVERRIDE.set(value)


def reset_nunmai_home_override(token: Token) -> None:
    """Restore the previous context-local Nunmai home override."""
    _NUNMAI_HOME_OVERRIDE.reset(token)


def get_nunmai_home_override() -> str | None:
    """Return the active context-local Nunmai home override, if any."""
    override = _NUNMAI_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _expand_nunmai_home(path: str) -> Path:
    """Expand environment and user-home syntax in a Nunmai home path."""
    return Path(os.path.expanduser(os.path.expandvars(path)))


def _get_platform_default_nunmai_home() -> Path:
    """Return the platform-native default Nunmai home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "nunmai"
    return Path.home() / ".nunmai"


def sudo_invoker_default_home() -> Path | None:
    """The invoking user's native ``~/.nunmai`` when this process is root under ``sudo``, else None.

    sudo strips NUNMAI_HOME and sets HOME=/root, so the process's own default is root's; the profile
    ...
    try:
        return Path(pwd.getpwnam(sudo_user).pw_dir) / ".nunmai"
    except KeyError:  # SUDO_USER not in passwd (chroot/container)
        return None


def _warn_profile_fallback_once() -> None:
    """Warn once when NUNMAI_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    ...
    # read re-ran on every get_nunmai_home() call (#90065).
    _profile_fallback_warned = True
    try:
        fallback_home = _get_platform_default_nunmai_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[NUNMAI_HOME fallback] NUNMAI_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass NUNMAI_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_nunmai_home() -> Path:
    """Nunmai home: context-local override → ``NUNMAI_HOME`` env var → platform default."""
    override = get_nunmai_home_override()
    if override:
        return _expand_nunmai_home(override)
    if not os.environ.get("NUNMAI_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_nunmai_home()
...
def nunmai_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Nunmai home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_nunmai_home()
    raw = str(candidate)
...
def reset_nunmai_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_nunmai_home() -> Path:
    """Nunmai home of the running process, ignoring task overrides.
    ...
    ``NUNMAI_HOME`` live on purpose: routed-profile DECISIONS compare against
    :func:`get_routing_process_nunmai_home` instead (#119242).
    """
    val = os.environ.get("NUNMAI_HOME", "").strip()
    return _expand_nunmai_home(val) if val else _get_platform_default_nunmai_home()


# Host-pinned identity of the profile this process serves as its own (None: follow NUNMAI_HOME).
_PINNED_PROCESS_NUNMAI_HOME: str | None = None


def pin_process_nunmai_home(path: str | Path | None) -> None:
    ...
    ``os.environ["NUNMAI_HOME"]`` for legacy readers (Nunmai WebUI) otherwise makes every turn's own
    ...
    ``nunmai_cli.env_loader._process_nunmai_home``) misjudge the same way. ``None`` clears the pin.

    Process-global on purpose: it names the process's own identity, not a per-task value. It is NOT
    folded into :func:`get_process_nunmai_home`: :func:`get_nunmai_home` falls back to that for
    tasks carrying no override, and the host's mirror exists precisely so those readers see the
    served profile. Hosts that never mutate ``NUNMAI_HOME`` need not call this (no-op).
    """
    global _PINNED_PROCESS_NUNMAI_HOME
    _PINNED_PROCESS_NUNMAI_HOME = None if path is None else str(path)


def process_nunmai_home_is_pinned() -> bool:
    return _PINNED_PROCESS_NUNMAI_HOME is not None


def get_routing_process_nunmai_home() -> Path:
    """Launch home for routed-profile decisions: the pinned home, else :func:`get_process_nunmai_home`."""
    pinned = _PINNED_PROCESS_NUNMAI_HOME
    return _expand_nunmai_home(pinned) if pinned else get_process_nunmai_home()


# Nunmai-managed runtime downloads at the root of a home (GGUF models, llama.cpp runtimes,
# managed Node): re-downloadable on demand and routinely tens to hundreds of GB. Shared by
# ``nunmai backup`` (excludes them) and ``profile create --clone-all`` (skips them from the
# default profile) so the two lists cannot drift apart.
LOCAL_RUNTIME_ROOT_DIRS: frozenset[str] = frozenset({"models", "runtimes", "node"})

# get_default_nunmai_root() memo keyed on (native home, expanded NUNMAI_HOME) so it stays
# fresh when a test or plugin mutates either input; saves ~80us/call at 31+ sites.
_default_nunmai_root_memo: "tuple[str, str, Path] | None" = None


def get_default_nunmai_root() -> Path:
    """Root Nunmai dir for profile-level ops: ``<root>`` when ``NUNMAI_HOME=<root>/profiles/<name>``."""
    global _default_nunmai_root_memo
    native_home = _get_platform_default_nunmai_home()
    env_home = os.environ.get("NUNMAI_HOME", "").strip()
    env_path = _expand_nunmai_home(env_home) if env_home else None
    memo_key = (str(native_home), str(env_path) if env_path is not None else "")
    memo = _default_nunmai_root_memo
    if memo is not None and memo[:2] == memo_key:
        return memo[2]
    result = native_home
    if env_path is not None:
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.nunmai (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else NUNMAI_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_nunmai_root_memo = (*memo_key, result)
    return result
...
# Files marking a real Nunmai home; arbitrary dirs with a ``profiles`` segment lack them.
_NUNMAI_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_nunmai_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<nunmai-home>/profiles``.

    Accepts the classic ``~/.nunmai`` layout, a root carrying Nunmai-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _NUNMAI_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_nunmai_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Nunmai home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_nunmai_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".nunmai":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the Nunmai root itself, so its basename is an installation detail (``.nunmai``
    on POSIX and commonly ``nunmai`` on Windows), not the profile id ``default``.
    """
...
def named_profile_has_servable_identity(profile_home: str | Path) -> bool:
    ...
    An EMPTY ``.env`` is all a crashed ``nunmai profile create`` leaves behind, and it is enough for
...
def named_profile_is_live(profile_home: str | Path) -> bool:
    ...
    never be started as a backend (whose ``ensure_nunmai_home`` would rebuild the full tree)."""
    home = Path(profile_home)
...
def mkdir_under_nunmai_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_nunmai_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("NUNMAI_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``NUNMAI_OPTIONAL_MCPS``)."""
    return _packaged_dir("NUNMAI_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``NUNMAI_BUNDLED_SKILLS``)."""
    return _packaged_dir("NUNMAI_BUNDLED_SKILLS", default, "skills")


def get_nunmai_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Nunmai subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_nunmai_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_nunmai_node_dirs(home: Path | None = None) -> list[Path]:
    """Nunmai-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with nunmaiManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_nunmai_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_nunmai_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Nunmai-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_nunmai_node_path())
    return result is not None and result.returncode == 0


_NUNMAI_NODE_TARGET_MAJOR = int(os.environ.get("NUNMAI_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/nunmai_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks nunmai-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def nunmai_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Nunmai-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Nunmai-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `nunmai update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_NUNMAI_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_NUNMAI_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%NUNMAI_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the nunmai.exe quarantine); when
    ...
    from nunmai_platform.host import facts
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NUNMAI_HOME": str(get_nunmai_home()), **extra_env},
...
def bootstrap_nunmai_managed_node() -> str | None:
    """Install a Nunmai-managed Node tree under ``$NUNMAI_HOME/node`` and return its npm path.

    Nunmai never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_nunmai_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # NUNMAI_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, NUNMAI_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_nunmai_managed_node() -> bool:
    """Redownload Nunmai-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not nunmai_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _NUNMAI_NODE_TARGET_MAJOR
    return False


def find_nunmai_node_executable(command: str) -> str | None:
    """Nunmai-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_nunmai_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_nunmai_node_executable(command)
    if managed:
        return managed
    if nunmai_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_nunmai_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Nunmai-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_nunmai_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``nunmai update``
...
def display_nunmai_home(home: Path | None = None) -> str:
    """User-facing ``~/`` display string for NUNMAI_HOME (``~/.nunmai/profiles/coder``).

    ``home`` overrides the lookup for callers that run before the CLI has applied the sticky
    ``active_profile`` (``get_nunmai_home()`` would emit the wrong-profile fallback warning there).
    """
    if home is None:
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``nunmai ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``nunmai`` follows the sticky ``active_profile``
    file, which can name a different database than the one that failed (#105887). A custom
    home outside the profile tree has no selector (only NUNMAI_HOME names it)."""
    name = profile_name_for_home(get_nunmai_home())
    return f"-p {name} " if name and name != "default" else ""


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved NUNMAI_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks nunmai-user traversal in Docker (UID 10000).
    # A credential file here means NUNMAI_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "nunmai-engine install directory (%s). Credential files are "
            "normally stored under the nunmai home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{NUNMAI_HOME}/home`` when the profile-home directory exists."""
    nunmai_home = get_nunmai_home_override() or (env or {}).get("NUNMAI_HOME") or os.getenv("NUNMAI_HOME")
    if not nunmai_home:
        return None
    profile_home = str(_expand_nunmai_home(nunmai_home) / "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "NUNMAI_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Nunmai profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={NUNMAI_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{NUNMAI_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: MutableMapping[str, str]) -> None:
    """Apply Nunmai' subprocess HOME contract to *env* in-place: ``HOME``/``NUNMAI_REAL_HOME``
    per the home mode, and the temp vars re-pointed at ``env["NUNMAI_HOME"]``'s scratch dir."""
    real_home = get_real_home(env)
    if real_home:
        env["NUNMAI_REAL_HOME"] = real_home
...
# --- Scratch dir: Nunmai' own temp space, never the system /tmp ---
# System temp is tmpfs on most Linux distros and containers, so browser profiles, PTY probes,
# download spools and every ``tempfile.mkdtemp()`` a Nunmai-launched script performs eat RAM
# and vanish on reboot. ``NUNMAI_HOME/cache/scratch`` is real storage with an IDLE retention:
...
SCRATCH_DIR_MARKER_ENV = "NUNMAI_SCRATCH_DIR"
...
# ``nunmai_rpc_<32 hex>.sock`` (~49) to the temp root, so a root longer than this budget
...
# Canonical home of "is this install package-manager managed": ``nunmai_cli.config`` re-exports
...
# Explicit opt-out (``NUNMAI_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `nunmai update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system(home: str | Path | None = None) -> str | None:
    """Return the package manager owning this install, if any.
    Signals: NUNMAI_MANAGED env var (systemd service) or a ``.managed`` marker file in
    NUNMAI_HOME (NixOS activation script — interactive shells don't see the service env).
    ...
    marker = os.getenv("NUNMAI_MANAGED", "").strip().lower() or None
    managed_marker = (Path(home) if home is not None else get_nunmai_home()) / ".managed"
...
def _container_or_chmod_skipped() -> bool:
    """Container/chmod-skip detection: the ``NUNMAI_CONTAINER``/``NUNMAI_SKIP_CHMOD`` operator
...
def _resolve_nunmai_uid_gid() -> tuple[int | None, int | None]:
    """Read NUNMAI_UID / NUNMAI_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns NUNMAI_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Nunmai commonly set these to map the in-container user to a host user so
    ...
    return _env_int("NUNMAI_UID"), _env_int("NUNMAI_GID")


def _chown_to_nunmai_uid(path) -> None:
    """Chown ``path`` to ``NUNMAI_UID:NUNMAI_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    created by ``ensure_nunmai_home`` on Docker deployments. See #34107.
    """
...
def apply_secure_dir_policy(path, *, home: str | Path | None = None) -> None:
    """Apply the canonical Nunmai home-directory permission policy to *path*.
    ...
    script set them (#77579); in a container only an explicit ``NUNMAI_HOME_MODE`` is applied
    (a bind-mounted data dir is often shared with sibling containers, #10757); elsewhere
    ``NUNMAI_HOME_MODE`` (e.g. ``0701``, ``2770``) overrides the mode. ``NUNMAI_UID`` /
    ``NUNMAI_GID`` ownership is applied when those env vars are set (#34107).
    ...
    Import-safe twin of ``nunmai_cli.config._secure_dir`` (which delegates here), so callers
    outside the CLI package — like :func:`get_scratch_dir` — share one policy implementation.
    """
    if get_managed_system(home) is not None:
        return
    explicit_mode = os.environ.get("NUNMAI_HOME_MODE", "").strip()
...
def get_scratch_dir(home: str | Path | None = None, *, prune: bool = True) -> Path:
    """``<home>/cache/scratch`` (created, owner-only); *home* defaults to the active Nunmai home.

    Every Nunmai process and child gets ``TMPDIR``/``TMP``/``TEMP`` pointed here at boot (see
    ...
    Permissions follow :func:`apply_secure_dir_policy`, so an explicit ``NUNMAI_HOME_MODE`` or
    a managed/shared home is honored instead of a blanket ``0700`` (#117347).
    """
    base = Path(home) if home is not None else get_nunmai_home()
...
def prune_scratch_dir(scratch: Path | None = None, max_idle_hours: float = SCRATCH_MAX_IDLE_HOURS) -> int:
    ...
    *max_idle_hours*, reaping processes and git worktree registrations rooted in them
    first (``nunmai_constants_scratch``); return the count removed."""
    from nunmai_constants_scratch import prune_idle_entries
...
def scratch_dir_usage_bytes(scratch: Path | None = None) -> int:
    """Total bytes under the scratch dir (for ``nunmai doctor``); 0 when unreadable."""
    root = scratch if scratch is not None else get_scratch_dir(prune=False)
    total = 0
...
def apply_scratch_tmp_env(env: MutableMapping[str, str]) -> bool:
    """Point ``TMPDIR``/``TMP``/``TEMP`` in *env* at the scratch dir of ``env["NUNMAI_HOME"]``.

    A temp var the user (or the OS: macOS ``/var/folders``, Windows ``%TEMP%``) set is
    respected and nothing changes. A value Nunmai itself exported earlier — recognisable
    because it equals ``NUNMAI_SCRATCH_DIR`` — is re-derived, so a child running under another
    ...
    for key in SCRATCH_TMP_ENV_VARS:
        ...
        if value and value != ours:
            return False
    home = env.get("NUNMAI_HOME", "").strip()
    try:
        scratch = str(get_scratch_dir(_expand_nunmai_home(home) if home else get_process_nunmai_home()))
    except (RuntimeError, OSError):
        # No NUNMAI_HOME and no resolvable user home (a child env built from nothing on
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Nunmai runs in WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under NUNMAI_HOME."""
    return get_nunmai_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under NUNMAI_HOME."""
    return get_nunmai_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under NUNMAI_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_nunmai_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._nunmai_ipv4_patched = True  # type: ignore[attr-defined]
...
def openrouter_variant_base(model_id: str) -> str | None:
    ...
    Lives here rather than in ``nunmai_cli.models`` so the metadata layer
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``nunmai_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``nunmai_cli``
...
def project_venv_dir(project_root) -> Path | None:
    ...
    Installers that keep the interpreter out of the checkout (``$NUNMAI_HOME/venvs/<name>``, the layout the
    ...
    path skipped the import probe, reclassified every ``nunmai tools`` dependency as missing and failed the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Nunmai (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("nunmai_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    nunmai update",
        "If that also fails, reinstall: https://nunmai.in",
    ]
