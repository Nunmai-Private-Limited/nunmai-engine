...
from nunmai_state_common import (FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS,
    routed_sessions_setting)
from nunmai_state_errors import is_fts_scoped_corruption_error, is_sqlite_lock_error
...
def fts5_cjk_so_path() -> Path:
    ...
    return Path(env).expanduser() if env else get_nunmai_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on) for the profile being served."""
    value = routed_sessions_setting("cjk_fts", "NUNMAI_CJK_FTS")
...
class SessionFtsSetupMixin:
    ...
    def _is_fts_write_corruption_error(exc: sqlite3.DatabaseError) -> bool:
        ...
        gateway transcript retry: see :func:`nunmai_state_errors.is_fts_scoped_corruption_error`."""
