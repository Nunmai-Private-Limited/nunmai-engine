...
YAML files in ``<NUNMAI_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``NUNMAI_BUNDLES_DIR`` override (tests) or ``<NUNMAI_HOME>/skill-bundles``."""
    override = os.environ.get("NUNMAI_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_nunmai_home() / "skill-bundles"
...
# File-level CRUD — used by `nunmai bundles`.
