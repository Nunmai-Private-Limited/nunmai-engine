...
Storage
-------
Bundles live in ``~/.nunmai/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``NUNMAI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``nunmai bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NUNMAI_HOME.

    Honors ``NUNMAI_BUNDLES_DIR`` for tests; falls back to
    ``<NUNMAI_HOME>/skill-bundles``.
    """
    override = os.environ.get("NUNMAI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_nunmai_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `nunmai bundles` CLI subcommand.
# ---------------------------------------------------------------------------
