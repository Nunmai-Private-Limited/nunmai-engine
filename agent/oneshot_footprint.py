"""What a finite one-shot session (``nunmai chat -q`` / ``--oneshot``, ``nunmai -z``) does NOT do.

A one-shot run has no later session in its NUNMAI_HOME to learn for: the process answers one query and
...
delegation dispatcher already read (``NUNMAI_SINGLE_QUERY_SESSION``), so interactive sessions are untouched:
...
def is_single_query_session() -> bool:
    ...
    return str(get_session_env("NUNMAI_SINGLE_QUERY_SESSION", "") or "") == "1"
