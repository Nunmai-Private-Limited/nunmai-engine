...
def _probe_managed_runtime(provider: str, model: str, cfg: Optional[Dict[str, Any]]) -> Optional[bool]:
    ...
    from nunmai_cli.local_runtime.capabilities import is_managed_provider, managed_model_supports_vision
...
def _probe_models_dev(provider: str, model: str, cfg: Optional[Dict[str, Any]]) -> Optional[bool]:
    ...
    if (provider or "").strip().lower() == "openai-codex":
        # A VALID Codex ``-900k`` picker variant is a Nunmai-side alias of its base slug; the catalog
...
def _accepted_mimes() -> frozenset:
    ...
    try:
        from agent.auxiliary_client import _runtime_main_value
        from nunmai_cli.local_runtime.capabilities import ACCEPTED_IMAGE_MIMES, is_managed_provider
