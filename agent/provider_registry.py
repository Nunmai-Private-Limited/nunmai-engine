...
re-registration logging, and the snapshot/restore pair :mod:`nunmai_cli.plugins`
...
class ProviderRegistry(Generic[P]):
    ...
    def merged(self, scope: Optional[str] = None) -> Dict[str, P]:
        ...
        with self._lock:
            ...
            merged.update(self._scoped_providers.get(scope or nunmai_home_key(), {}))
        return merged
    ...
    def get_provider(self, name: str, *, scope: Optional[str] = None) -> Optional[P]:
        ...
        with self._lock:
            return (
                self._scoped_providers.get(scope or nunmai_home_key(), {}).get(key)
...
def configured_provider_name(section: str, logger: logging.Logger) -> Optional[str]:
    """Read ``<section>.provider`` from config.yaml, mapping the managed Nunmai
