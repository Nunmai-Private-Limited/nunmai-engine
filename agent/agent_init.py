...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.nunmai/.env``. See NousResearch/hermes-agent#2765.
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$NUNMAI_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$NUNMAI_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Nunmai never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .nunmai.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / NUNMAI_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.nunmai/SOUL.md as the primary
    ...
    # report cumulative micros spent.  Surfaced behind NUNMAI_DEV_CREDITS.
    ...
    # both live under ~/.nunmai/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that nunmai_logging.setup_logging() does not
        ...
        # handler level inside nunmai_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from nunmai_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = nunmai_xai_default_headers()
    ...
    # A multiplexed gateway may enter a different NUNMAI_HOME after
    ...
    # worker (kanban_show tool is present iff NUNMAI_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["NUNMAI_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.nunmai/sessions/ alongside gateway sessions
    nunmai_home = get_nunmai_home()
    agent.logs_dir = nunmai_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.nunmai/sessions/session_{sid}.json)
    ...
    try:
        from nunmai_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `nunmai --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Nunmai behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "nunmai"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Nunmai' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "nunmai", "off"}:
        _ra().logger.warning(
