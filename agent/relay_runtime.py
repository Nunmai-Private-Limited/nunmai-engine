...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when Nunmai finalization forwards runtime metadata. Filter to
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "Nunmai Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "Nunmai could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside Nunmai native ownership; leaving it unchanged and "
                    "disabling Nunmai-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "Nunmai Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # Nunmai only enters Relay's initialization path after an
                ...
                logger.warning(
                    "Nunmai Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Nunmai Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "Nunmai Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
...
class RelayRuntime:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    ...
                    old_handle,
                    output={"nunmai.session.segment_reason": reason},
                    metadata={
                        RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                        RUNTIME_INSTANCE_KEY: self.runtime_id,
                    },
                ...
                logger.warning(
                    "Nunmai Relay segment close failed (session=%s segment=%d); "
            ...
            scope_metadata = {
                RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                RUNTIME_INSTANCE_KEY: self.runtime_id,
                "nunmai.session.segment": session.segment,
                "nunmai.session.segment_reason": reason,
            }
            parent_handle = None
            if session.parent_session_id:
                with self._sessions_lock:
            ...
            try:
                ...
                logger.warning(
                    "Nunmai Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Nunmai session, if active."""
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Nunmai session identified by ``event``."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "Nunmai Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "Nunmai Relay drained %d orphaned scope(s) before closing %s",
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Nunmai Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"nunmai-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "Nunmai Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Nunmai Relay runtime operation failed", exc_info=True)
            return None
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Nunmai Relay runtime initialization failed", exc_info=True
                )
...
# Set >0 while the native Relay pipeline is mid-dispatch of a Nunmai callback
...
class RelaySessionCoordinator:
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Nunmai Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"nunmai.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Nunmai Relay conversation initialization failed",
                    exc_info=True,
                )
        ...
        *,
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Nunmai turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Nunmai turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Nunmai Relay segment rotation failed", exc_info=True
                )
            try:
                ...
                logger.warning("Nunmai Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "Nunmai Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "Nunmai Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "Nunmai Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Nunmai Relay compaction notification failed", exc_info=True
            )
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a Nunmai tool). Nested managed execution here is structurally
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "Nunmai [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "Nunmai Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nunmai-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
