...
# The Nunmai-authored assistant row that closes a durable turn which ended without one. A
...
# so renderers show a Nunmai notice and room pollers never read it as the model's reply.
...
def provider_label_for(provider: Any) -> str:
    """Human-friendly provider name for chat copy (``"OpenRouter"``, ``"Nunmai Portal"``…)."""
...
_NEXT_STEPS_LOOP = (
    ...
    "If it happens again, run `nunmai doctor` and share the error details."
)
...
_NONRETRYABLE_COPY: Dict[str, str] = {
    FailoverReason.model_not_found.value: (
        ...
        "(or `nunmai model` in a terminal).{prefix_hint}"
    ),
    FailoverReason.format_error.value: (
        "{label} rejected this request as malformed, so the model didn't answer. Start a clean "
        "session with /new or switch models with /model; if it keeps happening, run `nunmai doctor`."
    ),
    ...
    FailoverReason.ssl_cert_verification.value: (
        "Nunmai couldn't verify {label}'s security certificate, so the connection was refused. "
...
_AUTH_COPY: Dict[str, str] = {
    ...
    "api_key": (
        ...
        "Settings → Providers, or run `nunmai setup` in a terminal."
    ),
}
...
_FAILURE_CODE_COPY: Dict[str, str] = {
    "context_overflow": (
        "This conversation has grown too long for {model} to read, and Nunmai couldn't shrink "
    ...
    "truncated": (
        ...
        "Nunmai didn't run the incomplete action. Nothing was changed. Send `continue`, ask for "
    ...
    "loop_error": (
        "Nunmai hit repeated errors and stopped this turn so it wouldn't keep retrying. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "interpreter_shutdown": (
        "Nunmai was shutting down and stopped this turn. Your conversation is saved — reopen "
...
_ONE_OFF_COPY: Dict[str, str] = {
    "payload_too_large": (
        ...
        "Nunmai couldn't shrink it enough automatically. Start a new session with /new (your "
    ...
    "server_context_rejection": (
        ...
        "about {tokens:,} tokens — well under the {window:,}-token window Nunmai knows for "
        ...
        "capacity, or the server runs {model} with a smaller window than Nunmai assumes. Wait a "
    ...
    "local_processing_error": (
        "Nunmai hit an internal error while handling the model's reply and stopped this turn. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "reasoning_only": (
    ...
    "nous_rate_limit": (
        ...
        "a backup provider with `nunmai fallback add`."
    ),
}
...
def site_copy(code: str, **fields: Any) -> str:
    ...
    fields.setdefault("home", display_nunmai_home())
...
def exhausted_copy(reason: str, *, label: str, attempts: int, summary: str, reset_seconds: Optional[float] = None) -> str:
    ...
    return (
        ...
        f"add a backup provider with `nunmai fallback add`.\n\nProvider said: {summary}"
    )
...
def oauth_relogin_command(provider: Any) -> str:
    ...
    named profile: a profile's credentials are its own (93889b770da), so a bare ``nunmai auth`` from
    ...
    if slug == "nous":
        return f"nunmai {profile_cli_selector()}portal"
    return f"nunmai {profile_cli_selector()}auth add {slug} --type oauth"
...
def relogin_command_hint(provider: Any) -> str:
    ...
    the exact OAuth command for a known OAuth slug, ``nunmai auth add <slug>`` for a known API-key
    ...
    slug = str(provider or "").strip().lower()
    if not slug:
        return f"nunmai {profile_cli_selector()}auth add <provider>"
    from agent.error_surface import auth_kind

    if auth_kind(slug) == "oauth":
        return oauth_relogin_command(slug)
    return f"nunmai {profile_cli_selector()}auth add {slug}"
...
def nonretryable_copy(
    ...
    body = template.format(label=label, model=model, home=display_nunmai_home(), prefix_hint=prefix_hint,
