...
# Mirror nunmai_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``nunmai_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["NUNMAI_SESSION_ID"] = child_session_id
    try:
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer Nunmai
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `nunmai setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Nunmai's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Nunmai' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|nunmai|off).
    # The memory-provider context handoff below is intentionally Nunmai-only:
    ...
    try:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the NUNMAI_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("NUNMAI_SESSION_SOURCE", "cli"),
                        model=agent.model,
        ...
        # engines (e.g. nunmai-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See nunmai-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Nunmai' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Nunmai' transcript
    unchanged.
    """
    auto_mode = str(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="nunmai_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
