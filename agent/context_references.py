...
from nunmai_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(nunmai_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(nunmai_home / rel for rel in _SENSITIVE_NUNMAI_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.nunmai/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Nunmai path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
