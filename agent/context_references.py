...
from nunmai_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
_SENSITIVE_NUNMAI_DIRS = (Path("skills") / ".hub",)
...
def preprocess_context_references(
    ...
    # NUNMAI_HOME override is lost and the credential-path guard checks the launch profile's .env.
...
# Desktop persists a large plain-text paste as a `.txt` under this Nunmai-managed
...
def _composer_paste_roots() -> list[Path]:
    ...
    return [nunmai_dir / COMPOSER_PASTES_DIRNAME for nunmai_dir in _nunmai_dirs()]
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, nunmai_home = Path(os.path.expanduser("~")).resolve(), get_nunmai_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {nunmai_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [nunmai_home / rel for rel in _SENSITIVE_NUNMAI_DIRS]
    if path in blocked_exact:
        raise ValueError("path is a sensitive credential file and cannot be attached")
    if any(_is_under(path, blocked_dir) for blocked_dir in blocked_dirs):
        raise ValueError("path is a sensitive credential or internal Nunmai path and cannot be attached")
