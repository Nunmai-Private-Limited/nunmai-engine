...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    # nunmai_home_key() in the key: the resolved api_key/base_url are per-profile, and under a
    ...
    cache_key = (nunmai_home_key(), provider, model)
    now = time.monotonic()
...
def _run_references_parallel(
    ...
    *agent*, the wait polls every ``_REFERENCE_POLL_INTERVAL_S`` so a user interrupt
    ...
    # Propagate the turn's contextvars (approval callbacks, Nunmai conversation tag).
