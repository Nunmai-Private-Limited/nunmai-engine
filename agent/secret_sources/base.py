...
env-var-shaped values at process startup, AFTER ``~/.nunmai/.env`` has loaded
and BEFORE the rest of Nunmai reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
NUNMAI_HOME, under a registry-enforced wall-clock timeout, no background
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `nunmai secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `nunmai secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `nunmai secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `nunmai secrets {name} setup` to re-authenticate.",
