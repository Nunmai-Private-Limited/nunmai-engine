...
cache problem must never block Nunmai startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Nunmai home used for cache paths.

    ``home_path`` is whatever ``load_nunmai_dotenv()`` already resolved;
    falling back to ``$NUNMAI_HOME`` / ``~/.nunmai`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<nunmai_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if NUNMAI_HOME is ever made traversable.
            try:
