...
a cache problem must never block Nunmai startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_nunmai_dotenv()``, else ``$NUNMAI_HOME``/``~/.nunmai``."""
    if home_path is None:
        from nunmai_constants import get_nunmai_home

        home_path = get_nunmai_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<nunmai_home>/cache/<basename>``::
