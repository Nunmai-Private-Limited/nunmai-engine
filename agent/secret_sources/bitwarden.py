...
Pulls API keys from BSM at startup so they need not live in ``~/.nunmai/.env``.
``bws`` is auto-installed into ``<nunmai_home>/bin/bws`` (one pinned version,
...
# <nunmai_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<nunmai_home>/bin/bws`` first, then PATH, then optional auto-install."""
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `nunmai secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `nunmai secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `nunmai secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
