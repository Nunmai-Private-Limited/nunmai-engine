...
# long"). Codex message ids can run 400+ chars; Nunmai ``msg_...`` ids stay under the cap.
...
def _input_image_part(part: Dict[str, Any], role: str = "user", *, keep_empty_url: bool) -> Optional[Dict[str, Any]]:
    ...
    if not _nonempty_str(url) and not keep_empty_url:
        return None
    url = str(url or "")
    # Lazy import: the prep module only depends on nunmai_constants at import time (no cycle).
...
def _replay_reasoning_items(
    ...
    matching issuer. ``id`` (store=False lookups 404) and the Nunmai provenance fields are stripped."""
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on Nunmai threading encrypted reasoning back across turns for
    ...
    Nunmai' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
