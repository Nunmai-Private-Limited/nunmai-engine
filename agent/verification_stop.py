...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: ``NUNMAI_VERIFY_ON_STOP`` env var, then ``agent.verify_on_stop``
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether ``nunmai verify`` has a runtime recipe here: a saved
    ``.nunmai/environment.json`` or a statically detected recipe with a start
