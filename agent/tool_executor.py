...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    ``NUNMAI_CONCURRENT_TOOL_TIMEOUT_S`` is the legacy bridge, ``0``/negative disables."""
...
class _ConcurrentToolAuthorizationGate:
    ...
    def run(self, callback):
        if not self._serialization_lock.acquire(timeout=self._lock_timeout):
            ...
            # ``nunmai mcp login``. Parking keeps the task alive so the 300s self-probe (and an explicit
...
def _dispatch_authorized_once(
    ...
    *,
    ...
    """Nunmai policy (scope → plugin pre-hooks → guardrails) then the one real dispatch.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state.dispatched:
                raise RuntimeError("Nunmai tool execution callback invoked more than once")
