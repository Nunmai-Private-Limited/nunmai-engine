...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_NUNMAI_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Nunmai expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile NUNMAI_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped NUNMAI_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``nunmai_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_nunmai_home()`` themselves.
        """
        if "nunmai_home" not in kwargs:
            from nunmai_constants import get_nunmai_home
            kwargs["nunmai_home"] = str(get_nunmai_home())
        for provider in self._providers:
            try:
