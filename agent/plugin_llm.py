...
Plugins built on Nunmai Engine often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Nunmai
...
:class:`~nunmai_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Nunmai' main config
...
handles every provider, fallback chain, and per-task override Nunmai
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nunmai_cli.plugins.PluginContext`
