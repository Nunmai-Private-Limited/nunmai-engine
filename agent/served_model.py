...
capture rides an ``httpx`` response hook on the client Nunmai builds for the agent; it stores
...
_HOOK_MARK = "_nunmai_served_model_hook"
...
def result_model_fields(agent: Any) -> dict[str, Optional[str]]:
    ...
    last response carried one, else Nunmai' own fallback route (primary → active model)."""
