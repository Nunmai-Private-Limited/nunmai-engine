...
def scoped_aws_session_kwargs() -> Dict[str, str]:
    ...
    Under a NUNMAI_HOME override the process env holds the LAUNCH profile's ``AWS_*`` (or nothing), so
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Nunmai with Bedrock support: pip install -e '.[bedrock]'"
        )
    try:
...
def _cached_client(cache: Dict[str, Any], service: str, region: str):
    ...
    if get_nunmai_home_override() is None:
        ...
        return cache[region]
    key = (nunmai_home_key(), service, region)
...
def invalidate_runtime_client(region: str) -> bool:
    ...
    if get_nunmai_home_override() is not None:
        return _bedrock_clients_by_home.pop((nunmai_home_key(), "bedrock-runtime", region), None) is not None
...
def resolve_bedrock_bearer_token(env: Optional[Dict[str, str]] = None) -> str:
    ...
    Under a NUNMAI_HOME override the read goes through the profile secret scope so a
...
def discover_bedrock_models(region: str, provider_filter: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    if get_nunmai_home_override() is not None:
        cache_key = f"{nunmai_home_key()}|{cache_key}"
