...
upstream prefix cache warm.  See ``nunmai-engine-dev``'s
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- nunmai-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``NUNMAI_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("NUNMAI_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the NUNMAI_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound NUNMAI_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_nunmai_root` (NOT ``get_nunmai_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_nunmai_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nunmai never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NUNMAI_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the NUNMAI_HOME ContextVar
    ...
    # Pointer to the nunmai-engine skill + docs for user questions about Nunmai
    ...
    stable_parts.append(
        NUNMAI_AGENT_HELP_GUIDANCE if _has_skill_view
        else NUNMAI_AGENT_HELP_GUIDANCE_NO_SKILLS
    )
    ...
    # NUNMAI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Nunmai, any interactive coding surface in a code
    ...
    # `nunmai -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['nunmai-bots']).
    ...
    # Active-profile hint — names the Nunmai profile the agent is running
    # under so it doesn't conflate ~/.nunmai/skills/ (default profile) with
    # ~/.nunmai/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient NUNMAI_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_nunmai_root(): get_nunmai_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_nunmai_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_nunmai_root())
    else:
        _home_str = _root_str = str(get_nunmai_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_nunmai_root()), which in ambient
        # profile mode is NOT get_nunmai_home().
    ...
    if not agent.skip_context_files:
        ...
        # (developing Nunmai). Every other surface (desktop chat panel,
    ...
    from nunmai_time import get_timezone as _nunmai_tz, now as _nunmai_now
    now = _nunmai_now()
