"""Anthropic Messages API adapter for Nunmai Engine.

Translates between Nunmai's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in nunmai_cli/runtime_provider.py.
    """
...
# Nunmai effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to nunmai's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to nunmai's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + NunmaiAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://nunmai.in")
        headers.setdefault("X-Title", "Nunmai Engine")
        headers.setdefault("User-Agent", f"NunmaiAgent/{_NUNMAI_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Nunmai loads that into the process env from
    # ``~/.nunmai/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Nunmai notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Nunmai historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``nunmai models``) never mutates ``~/.nunmai/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Nunmai)
      ...
      5. Anthropic credential_pool OAuth entry (~/.nunmai/auth.json)
    ...
    # 1. Nunmai-managed OAuth/setup token env var
    ...
    # 5. Nunmai credential_pool OAuth entry.
...
# ── Nunmai-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.nunmai/.anthropic_oauth.json (our own file).
...
def run_nunmai_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Nunmai with your Claude Pro/Max subscription.")
    print()
...
def read_nunmai_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Nunmai OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See nunmai-engine#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See nunmai-engine: extended-thinking + parallel
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Nunmai Engine", "Claude Code")
                text = text.replace("Nunmai agent", "Claude Code")
                text = text.replace("nunmai-engine", "claude-code")
                text = text.replace("Nunmai Research", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Nunmai-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Nunmai surfaces in its CLI. We
