"""Anthropic Messages API adapter: client construction + the Messages call for Nunmai's
...
# Nunmai effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Nunmai tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://nunmai.in", "X-Title": "Nunmai Engine",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    nunmai's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str], route: str = None):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.nunmai/.env) and then sends dual
...
def _custom_provider_extra_headers(base_url) -> Dict[str, str]:
    ...
    if not base_url:
        return {}
    try:
        from nunmai_cli.config import get_custom_provider_extra_headers
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to nunmai's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Nunmai tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Nunmai Engine", "Claude Code"), ("Nunmai agent", "Claude Code"), ("Nunmai Research", "Anthropic"),
)
# The slug is rewritten only as a standalone prose word. Joined to a host, path, repo, mailbox
# or quoted as an identifier (``nunmai-engine.nousresearch.com``, ``~/.nunmai/nunmai-engine/venv``,
# ``Nunmai-Private-Limited/nunmai-engine``, ``skill_view(name='nunmai-engine')``) it is an address the model
...
_OAUTH_SLUG_PATTERN = re.compile(r"""(?<![\w./:@'"`-])nunmai-engine(?![\w/@-]|\.\w)""")
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Nunmai shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # Nunmai Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``); normalizing would
    ...
    # which silently hides reasoning text that Nunmai surfaces in its CLI. We request "summarized" so the
...
def create_anthropic_message(
    ...
    ``on_response(httpx_response)`` exposes headers the parsed Message drops (Nunmai Portal's
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nunmai_home': ('nunmai_constants', 'get_nunmai_home'),
    ...
    'read_nunmai_oauth_credentials': ('agent.anthropic_credentials', 'read_nunmai_oauth_credentials'),
    ...
    'run_nunmai_oauth_login_pure': ('agent.anthropic_credentials', 'run_nunmai_oauth_login_pure'),
