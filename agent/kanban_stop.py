...
Nunmai treats that as a clean exit → ``rc=0`` → dispatcher ``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    """On when ``NUNMAI_KANBAN_TASK`` is set, unless ``NUNMAI_KANBAN_STOP_NUDGE`` disables it."""
    if (os.environ.get("NUNMAI_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
    return bool((os.environ.get("NUNMAI_KANBAN_TASK") or "").strip())
