...
def set_multiplex_active(active: bool) -> None:
    ...
    (``nunmai_constants.pin_process_nunmai_home``) unless an embedding host already pinned one:
    from here on "is this task routed" compares the override against the home the process was
    launched with, not against whatever a host later mirrors into ``os.environ["NUNMAI_HOME"]``.
    ...
    from nunmai_constants import (
        ...
        pin_process_nunmai_home,
        process_nunmai_home_is_pinned,
    )
    _MULTIPLEX_ACTIVE = bool(active)
    if _MULTIPLEX_ACTIVE:
        if not process_nunmai_home_is_pinned():
            _AUTO_PINNED_HOME = get_routing_process_nunmai_home()
            pin_process_nunmai_home(_AUTO_PINNED_HOME)
    elif _AUTO_PINNED_HOME is not None:
        if get_routing_process_nunmai_home() == _AUTO_PINNED_HOME:
            pin_process_nunmai_home(None)
        _AUTO_PINNED_HOME = None
...
class _BoundScope(NamedTuple):
    ...
    deliberately skips the NUNMAI_HOME override (kanban spawn-env builds, MCP
    owner scopes)."""
...
def serves_routed_profile() -> bool:
    ...
    multiplexing, else when a NUNMAI_HOME override names another home (dashboard/desktop backend,
    ...
    ``NUNMAI_HOME`` pins its own home with ``nunmai_constants.pin_process_nunmai_home`` so the
    mirror cannot flip this predicate."""
    if is_multiplex_active():
        return True
    from nunmai_constants import get_nunmai_home_override, get_routing_process_nunmai_home, nunmai_home_key
    own = nunmai_home_key(get_routing_process_nunmai_home())
    bound = _SECRET_SCOPE.get()
    if bound is not None and bound.profile_home and nunmai_home_key(bound.profile_home) != own:
        return True
    override = get_nunmai_home_override()
    return override is not None and nunmai_home_key(override) != own
...
class UnscopedSecretError(RuntimeError):
    ...
    def __init__(self, secret_name: str = "", developer_detail: str = ""):
        ...
        super().__init__(
            f"Nunmai could not read {what} (an internal profile-scoping bug on the multiplexed "
            "gateway, not your configuration). Run `nunmai gateway restart`; if it keeps happening, "
            "report it with `nunmai debug share`."
        )
...
def set_secret_scope(secrets: Optional[Mapping[str, str]], *, profile_home: Optional[str] = None) -> Token:
    ...
    deliberately skips the NUNMAI_HOME override."""
    if secrets is None:
...
_GLOBAL_ENV_EXACT = frozenset({
    # Nunmai runtime / deployment
    "NUNMAI_HOME", "NUNMAI_PROFILE", "NUNMAI_GATEWAY_LOCK_DIR",
    "NUNMAI_MAX_ITERATIONS", "NUNMAI_API_TIMEOUT",
    "NUNMAI_REDACT_SECRETS", "NUNMAI_NOUS_TIMEOUT_SECONDS",
    "_NUNMAI_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_WORKSPACES_ROOT", "NUNMAI_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "NUNMAI_KANBAN_",
    "NUNMAI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
# ``invalidate_env_file_cache()`` is the explicit knob; ``nunmai_cli.config.invalidate_env_cache()``
# calls it for Nunmai's own .env writers.
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``nunmai_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(nunmai_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(nunmai_home) / ".env")
    try:
        from nunmai_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(nunmai_home))
    except Exception:
        external_secrets = {}
    ...
    if bridged is not None and _is_process_home(nunmai_home):
        secrets.setdefault("GATEWAY_ALLOW_ALL_USERS", bridged)
    return secrets


def _is_process_home(nunmai_home: Path) -> bool:
    """Is *nunmai_home* the profile this process serves as its own? Same launch-home identity as
    ``serves_routed_profile()``: a host that mirrors a served profile into ``NUNMAI_HOME`` would
    ...
    try:
        return Path(nunmai_home).resolve() == get_routing_process_nunmai_home().resolve()
    except OSError:
        return False
