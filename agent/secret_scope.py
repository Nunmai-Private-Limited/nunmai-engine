...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the NUNMAI_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "NUNMAI_MAX_ITERATIONS", "NUNMAI_MAX_TOKENS", "NUNMAI_API_TIMEOUT",
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Nunmai writes
    ...
    ``nunmai_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Nunmai parser: save_env_value
    ...
    from nunmai_cli.config import _parse_env_value
