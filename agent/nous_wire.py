"""Nunmai Portal ``anthropic/*`` wire selection when ``nous.anthropic_wire`` is ``auto``.

Portal serves Claude two ways and Nunmai cannot tell which from the request: an OpenRouter
...
def maybe_switch_wire_after_first_response(agent: Any, response: Any, api_call_count: int) -> bool:
    ...
    if not model.lower().startswith("anthropic/"):
        return False
    try:
        from nunmai_cli.providers import _nous_anthropic_wire
