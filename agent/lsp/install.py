...
appropriate.  All installs go to a Nunmai-owned bin staging dir,
``<NUNMAI_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``nunmai lsp
  status``.
...
# ``<NUNMAI_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `nunmai lsp status` reports the host's presence.
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(nunmai_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $NUNMAI_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Nunmai installed and
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = nunmai_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a nunmai-owned target dir.
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = nunmai_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``nunmai lsp status`` CLI to give users a quick
