...
Installs go to a Nunmai-owned staging dir, ``<NUNMAI_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``nunmai lsp status`` reports it).
...
# ``bin`` in ``<NUNMAI_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `nunmai lsp status` reports its presence.
...
def nunmai_lsp_bin_dir() -> Path:
    """Return the Nunmai-owned bin staging dir for LSP servers."""
    from nunmai_constants import get_nunmai_home

    p = get_nunmai_home() / "lsp" / "bin"
...
def _npm_bin_dir() -> Path:
    ...
    return nunmai_lsp_bin_dir().parent / "node_modules" / ".bin"
...
def _existing_binary(name: str, *, is_windows: Optional[bool] = None) -> Optional[str]:
    ...
    bases = [nunmai_lsp_bin_dir() / name] + ([_npm_bin_dir() / name] if win else [])
...
def _link_into_bin(target: Path) -> str:
    ...
    link = nunmai_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    if pm is None:
        return None
    # Managed Node first: $NUNMAI_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Nunmai installed.
    ...
    if pm_bin is None:
        ...
        return None
    staging = nunmai_lsp_bin_dir().parent  # <NUNMAI_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = nunmai_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``nunmai lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "nunmai_lsp_bin_dir"]
