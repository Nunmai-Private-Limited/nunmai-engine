"""Language Server Protocol (LSP) integration for Nunmai Engine.
...
# Routed multiplex profiles (NUNMAI_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``nunmai chat`` exit leaks pyright processes for a few seconds
