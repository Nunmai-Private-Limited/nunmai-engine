...
_VUE_REINSTALL = (
    "delete <NUNMAI_HOME>/lsp/node_modules/@vue and <NUNMAI_HOME>/lsp/bin/vue-language-server*, "
    "then run: nunmai lsp install vue-language-server"
)
_VUE_TUNNEL_MSG = (
    "vue-language-server: the installed @vue/language-server is 3.x, which only works behind a client-hosted "
    f"tsserver tunnel Nunmai does not run — no diagnostics will arrive. Reinstall the self-hosting 2.x line: {_VUE_REINSTALL}"
)
_VUE_TSDK_MSG = (
...
def _node_modules_trees(bin_path: str, root: str) -> List[str]:
    ...
    the launcher's own tree (symlinks resolved), Nunmai staging, then the project's."""
    from agent.lsp.install import nunmai_lsp_bin_dir
    trees = [str(nunmai_lsp_bin_dir().parent / "node_modules"), os.path.join(root, "node_modules")]
...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<NUNMAI_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_nunmai_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <NUNMAI_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = nunmai_lsp_session_dir()
    inner = (
        ...
        f"-HostName Nunmai -HostProfileId nunmai -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def nunmai_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from nunmai_constants import get_nunmai_home
    d = os.path.join(str(get_nunmai_home()), "lsp", "pses")
