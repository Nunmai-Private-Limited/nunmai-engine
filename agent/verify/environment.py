...
The manifest lives at ``<project>/.nunmai/environment.json`` and is the
...
MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".nunmai") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``nunmai verify``.
    """
