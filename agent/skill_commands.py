...
# applied by nunmai_cli/commands_platforms.py, not here.
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("NUNMAI_PLATFORM") or get_session_env("NUNMAI_SESSION_PLATFORM")
    except Exception:
...
def _resolve_skill_commands_home() -> str:
    """Effective Nunmai home the scan is scoped to (profiles carry their own
    ...
    ``set_nunmai_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    """Append a ``[Skill config: ...]`` block with resolved ``metadata.nunmai.config``
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_nunmai_home()}/config.yaml):")
...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]]) -> None:
    ...
    if skill_command_collision_note(name) is not None:
        logger.warning("Skill %r generates slash command '/%s' which collides with a core Nunmai command; "
...
def build_preloaded_skills_prompt(
    ...
    this path bypasses the scan-time filter, and ``nunmai -s <skill>`` must not
...
def build_auto_load_prompt(
    ...
    *home_override* makes home resolution EXPLICIT (same seam as ``build_skills_system_prompt``): the config,
    ...
    that lost the NUNMAI_HOME ContextVar cannot pin the launch profile's skills into another profile's prompt.
    """
    from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override
    home_token = set_nunmai_home_override(str(home_override)) if home_override is not None else None
    try:
