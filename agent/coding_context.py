...
In a code workspace on an interactive surface Nunmai adopts a **coding posture**: a
...
# Operating brief. Tool names referenced here are in the coding toolset and _NUNMAI_CORE_TOOLS.
...
def _agent_config_value(config: Optional[dict[str, Any]], key: str, default: Any, *, readonly: bool) -> Any:
    ...
    if config is None:
        try:
            from nunmai_cli.config import load_config, load_config_readonly
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        apply it only when the user hasn't pinned a selection (``--toolsets``, ``NUNMAI_TUI_TOOLSETS``)."""
