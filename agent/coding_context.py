"""Coding-context awareness — base Nunmai, every interactive surface.

When the user runs Nunmai inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Nunmai shifts into a **coding posture**. This module is the
...
from nunmai_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("nunmai.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "nunmai", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _NUNMAI_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``NUNMAI_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Nunmai should operate in its coding posture right now."""
