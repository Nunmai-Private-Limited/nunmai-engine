...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `nunmai backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - nunmai_home (str): The active NUNMAI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.nunmai``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "nunmai").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'nunmai memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], nunmai_home: str) -> None:
        ...
        Called by 'nunmai memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``nunmai_home`` is the active NUNMAI_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE NUNMAI_HOME.

        ``nunmai backup`` only walks NUNMAI_HOME, so any provider state kept
        ...
        of the archive, and ``nunmai import`` restores them to their original
