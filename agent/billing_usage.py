...
def nous_logged_in() -> bool:
    """Cheap local auth-state check: a Nunmai access token is present. Fail-closed."""
    try:
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``NUNMAI_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``NUNMAI_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
    name = (os.getenv("NUNMAI_DEV_CREDITS_FIXTURE") or "").strip().lower()
