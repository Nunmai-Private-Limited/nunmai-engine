"""Cross-session rate limit guard for Nunmai Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Nunmai Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Nunmai triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Nunmai retries), and every one of those calls counts
...
def _state_path() -> str:
    """Return the path to the Nunmai rate limit state file."""
    try:
        from nunmai_constants import get_nunmai_home
        base = get_nunmai_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".nunmai")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that Nunmai Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Nunmai rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Nunmai rate limit state: %s", exc)


def nous_rate_limit_remaining() -> Optional[float]:
    """Check if Nunmai Portal is currently rate-limited.
...
def clear_nous_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Nunmai request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Nunmai rate limit state: %s", exc)
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from Nunmai Portal is a real account rate limit.

    Nunmai Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Nunmai, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Nunmai is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Nunmai.

    Tripping the cross-session breaker on (b) blocks ALL Nunmai requests
    (and all models, since Nunmai is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Nunmai emits
