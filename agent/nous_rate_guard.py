...
auxiliary) can check whether Nunmai Portal is currently rate-limited before making
requests. Without it each 429 fans out into up to 9 calls per turn (3 SDK
retries x 3 Nunmai retries), all counted against RPH.
"""
...
def _state_path(*, anonymous: bool = False) -> str:
    """Path to the Nunmai rate limit state file."""
    try:
...
def record_nous_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, error_context: Optional[dict[str, Any]] = None,
    ...
    """Record that Nunmai Portal is rate-limited in the shared state file.
    ...
    try:
        ...
        logger.info("Nunmai rate limit recorded: resets in %.0fs (at %.0f)", reset_at - now, reset_at)
...
def is_genuine_nous_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, last_known_state: Optional[Any] = None,
    ...
    Nunmai multiplexes upstream providers behind one key, so a 429 may be one
    upstream model out of capacity (clears in seconds) rather than our quota;
    tripping the breaker on that would block every Nunmai model for minutes.
...
def is_long_welcome_rate_limit(error_context: Any) -> bool:
    """True for a Nunmai welcome-tier ``rate_limited`` refusal whose reset is long enough to be an
    exhausted allowance (``WELCOME_LONG_WAIT_SECONDS``), as parsed into ``error_context``
    (``welcome_refusal`` from ``nunmai_cli.anon_auth.parse_welcome_refusal``). Capacity refusals
