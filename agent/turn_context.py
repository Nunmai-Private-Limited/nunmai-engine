...
def export_current_turn_boundary(agent: Any, result: Any, user_message: Any) -> Any:
    ...
    Hosts that settle their own transcript by index (nunmai-webui) must never guess which
...
def build_turn_context(
    ...
    # Tag log records on this thread with the session ID for ``nunmai logs``; bind the
...
def build_api_messages(
    ...
    for idx, msg in enumerate(canonical_messages):
        ...
        # wire for every route that does not replay it (OpenRouter/Nunmai do).
