...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``nunmai logs``.
    ...
    if (
        ...
        # Nunmai only initiates compaction in "nunmai" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Nunmai preflight compression for codex app-server "
                "(mode=%s); Nunmai will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Nunmai (via _emit_status), so it always
