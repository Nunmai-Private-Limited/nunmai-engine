...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Nunmai starts and finds ``~/.openclaw/``.

    Points users at ``nunmai claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nunmai claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Nunmai, run "
        "`nunmai claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nunmai claw cleanup` (renames it to ~/.openclaw.pre-migration — "
