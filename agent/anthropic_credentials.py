...
``ANTHROPIC_API_KEY``, Nunmai-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.nunmai/.anthropic_oauth.json`` (Nunmai PKCE) and
...
# Claude Code's macOS Keychain entry (generic password). Nunmai reads it
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Nunmai so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".nunmai-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "nunmai_pkce": lambda: _get_nunmai_oauth_file()
}
...
def claude_code_credentials_path() -> Path:
    ...
    like the Claude CLI itself (blank = unset, as in ``nunmai_cli.foreign_sessions``). The supported opt-out of
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from nunmai_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("NUNMAI_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout_seconds, target_path=cred_path):
            ...
            if is_rotation_consumed_uncommitted(refresh_token, source_path=cred_path):
                logger.debug("Refresh token was already consumed by an uncommitted rotation "
                             "- refusing to replay it; run 'nunmai auth add anthropic'")
                return None
            ...
            try:
                ...
                if is_terminal_anthropic_refresh_error(e):
                    ...
                    logger.warning(
                        "Claude Code OAuth refresh token is terminally invalid (%s); Nunmai cannot use this "
                        "login. Run 'nunmai auth add anthropic' to give Nunmai its own login.", e)
                else:
            ...
            try:
                ...
                logger.error(
                    ...
                    "run 'nunmai auth add anthropic' to give Nunmai its own login",
                    cred_path, e,
                )
...
def _mirror_claude_code_credentials_to_keychain(
    ...
    """After a Nunmai refresh, write the rotated pair into the Claude Code Keychain item too (#98334).
...
def _resolve_claude_code_token_from_credentials(creds: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if not refreshed:
        logger.debug("Token refresh failed — run 'nunmai auth add anthropic' to give Nunmai its own login")
    return refreshed or None


def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Nunmai historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``nunmai models``) never mutate
...
# ── Nunmai-native PKCE OAuth flow (~/.nunmai/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_nunmai_oauth_file() -> Path:
    return get_nunmai_home() / ".anthropic_oauth.json"


def _root_nunmai_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        return None if root.resolve(strict=False) == get_nunmai_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_nunmai_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Nunmai-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize Nunmai with your Claude Pro/Max subscription.", "",
    ...
    try:
        from nunmai_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_nunmai_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Nunmai-managed OAuth credentials from ~/.nunmai/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_nunmai_oauth_file(), "Nunmai OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_nunmai_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed nunmai_pkce tokens to ~/.nunmai/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own NUNMAI_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_nunmai_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "Nunmai OAuth credentials",
    )
