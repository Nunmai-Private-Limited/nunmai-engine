...
from nunmai_constants import OPENROUTER_MODELS_URL, openrouter_variant_base
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    for env_var in ("NUNMAI_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_nunmai_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``nunmai -q``, cron) start cold and Nunmai bypasses the persistent context cache, so
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # 1M verified on OpenRouter & Nunmai portal 2026-08; qwen3-max = 256K Coding Plan snapshot)
    ...
    # GLM — Nunmai + OpenRouter /v1/models (2026-09-09): 5.3 / 5.3-flash 1,310,720 (:batch/:US 1,048,576);
...
def _strip_openrouter_routing_variant(
    ...
    :func:`nunmai_cli.models.validate_requested_model` applies. Sharing the
...
def _is_codex_route(provider: str, base_url: str, custom_providers: list | None) -> bool:
    ...
    ``openai-codex`` provider (also behind a ``NUNMAI_CODEX_BASE_URL`` / ``model.base_url`` proxy)
    ...
    with contextlib.suppress(Exception):  # config unreadable → not a known Codex route
        from nunmai_cli.config import get_custom_provider_api_mode
...
def _get_context_cache_path() -> Path:
    ...
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "context_length_cache.yaml"
...
def _invalidate_cached_context_length(model: str, base_url: str) -> None:
    ...
    for memo_key in list(_BEDROCK_PROBE_FAILURE_CACHE):  # snapshot: another thread may be memoising
        if memo_key[:2] == (nunmai_home_key(), stripped) and memo_key[2] in (model, bare):
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    # OpenRouter/Nunmai: "maximum context length is N … (A of text input, B of tool input, C in the output)" -> ctx - A - B.
...
# DashScope, Anthropic (available_tokens / "maximum allowed number of output tokens"), OpenRouter/Nunmai,
...
def _normalize_model_version(model: str) -> str:
    """Dots -> dashes so Nunmai ids (claude-opus-4-6) compare with OpenRouter's (claude-opus-4.6)."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Nunmai picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_nous_context_length(model: str, base_url: str = "", api_key: str = "") -> Tuple[Optional[int], str]:
    """``(context_length, source)`` for a Nunmai Portal model: portal /v1/models is authoritative
    ("portal"). Fallback matches OR's prefixed ids against the bare Nunmai id with dot/dash
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx is not None and ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info("Rejecting OpenRouter metadata context=%s for %r (known 32K underreport, Nunmai path); falling through to hardcoded defaults", ctx, or_id)
            return None
        return ctx
...
def _validate_cached_context_length(model: str, base_url: str, cached: int, *, api_key: str = "") -> Optional[int]:
    ...
    for hit, log, msg, shown in drop_rules:
        if hit:
            ...
            _invalidate_cached_context_length(model, base_url)
            return None
    # Nunmai Portal: /v1/models is authoritative. Bypass (don't drop) the cache so step
    # 5b reconciles OR-seeded entries without touching disk when the portal is down.
    if _infer_provider_from_url(base_url) == "nous":
        logger.debug("Bypassing persistent cache for %s@%s (Nunmai portal authoritative)", model, base_url)
        return None
...
def _resolve_bedrock_context_length(model: str, base_url: str) -> Optional[int]:
    ...
    from nunmai_constants import nunmai_home_key
    memo_key = (nunmai_home_key(), cache_key_url.rstrip('/'), model, region)
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from nunmai_cli.models import get_copilot_model_context
            ctx = get_copilot_model_context(model, api_key=api_key)
            if ctx:
                return ctx
    # 5b/5c. Nunmai portal and Codex OAuth (lower limits than the direct API for the same slug; its
...
def get_model_context_length(
    ...
    model_overrides / custom_providers / endpoint-scoped; 1 persistent cache (Nunmai, LM
    ...
    Nunmai, Codex OAuth, GMI, Ollama, OpenRouter live, models.dev); 6 OpenRouter for
    ...
    # in nunmai_cli.models.validate_requested_model.
    ...
    # (NUNMAI_CODEX_BASE_URL, model.base_url, custom api_mode: codex_responses) the URL looks
