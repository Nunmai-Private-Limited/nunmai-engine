...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``NUNMAI_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_nunmai_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Nunmai Engine.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Nunmai from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Nunmai' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Nunmai minimum.
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Nunmai grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a Nunmai-side alias: it is
...
# Nunmai-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Nunmai picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Nunmai picker aliases — the Codex catalog only
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Nunmai sends to providers:
