"""OpenAI-shape bridge shared by Nunmai' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Nunmai as
...
the agent's *own* tool notifications. Nunmai' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Nunmai' perspective).
...
of its own (Copilot) forwards everything Nunmai offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Nunmai'
agent-level tools, because re-offering the overlapping ones makes Nunmai re-run
...
class StreamChunks(list):
    ...
    Nunmai reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``nunmai_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Nunmai'
