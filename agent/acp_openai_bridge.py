...
ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Nunmai' tool schemas travel INTO the
...
read/edit/execute tools forwards only Nunmai' agent-level tools, since re-offering overlapping ones
makes Nunmai redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``nunmai_projected_messages``)
    Nunmai reads off the ``create`` result; a plain list would drop them on the stream path."""
