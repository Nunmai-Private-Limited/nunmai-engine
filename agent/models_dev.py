...
thread refreshes) → disk cache (~/.nunmai/models_dev_cache.json, any age) →
...
from nunmai_constants import openrouter_variant_base
...
# Nunmai provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # Nunmai provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Nunmai ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_nunmai_ids(mdev_id: str) -> List[str]:
    """Return the Nunmai provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for nunmai_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(nunmai_id)
...
def _models_dev_id(
    ...
    """models.dev provider id for a Nunmai provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a Nunmai provider id nor a "
...
def _nunmai_path(name: str) -> Path:
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / name


def _get_cache_path() -> Path:
    return _nunmai_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _nunmai_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_nunmai_home(etag_path.parent)
...
def _get_provider_models(
    ...
    """Resolve a Nunmai provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Nunmai
...
def _provider_override_section(provider: str, *, config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Nunmai OR models.dev id), or None."""
    overrides = (
    ...
    if not overrides or not provider_key:
        return None
    # Forward (Nunmai → models.dev id) and reverse (caller passed a models.dev id, config keyed by Nunmai id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_nunmai_ids(provider_key)]
...
def _builtin_model_metadata(
    ...
    """Built-in metadata for a provider/model pair, if Nunmai has a vendor-specific entry."""
...
def get_provider_info(
    ...
    """Provider metadata by Nunmai or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(
    ...
    """Full model metadata by Nunmai or models.dev provider ID (exact match, then case-insensitive), or
