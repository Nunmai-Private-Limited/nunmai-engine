...
Data resolution order:
  ...
  2. Disk cache (~/.nunmai/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Nunmai ↔ models.dev
# ---------------------------------------------------------------------------
...
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Nunmai' provider is "meta-ai" (and the
...
def _models_dev_to_nunmai_ids(mdev_id: str) -> List[str]:
    ...
    if _MODELS_DEV_TO_PROVIDER is None:
        ...
        for nunmai_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(nunmai_id)
...
def _get_cache_path() -> Path:
    ...
    return get_nunmai_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``nunmai config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Nunmai provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Nunmai id.
    for nunmai_id in _models_dev_to_nunmai_ids(provider_key):
        if nunmai_id != provider_key:
            candidates.append(nunmai_id)
...
def _get_provider_models(
    ...
    """Resolve a Nunmai provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``nunmai model``), which is an interactive user-facing flow
...
# Gemma models whose TPM quotas are too small for normal Nunmai agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Nunmai provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Nunmai ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Nunmai or models.dev provider ID.  Tries exact match then
