...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    """Nunmai 401 that survived a credential refresh: likely Portal OAuth expired/revoked,
    ...
    try:
        ...
        pass
    _plines(agent, "🔐 Nunmai 401 — Portal authentication failed.")
    if _body_text:
        _plines(agent, f"   Response: {_body_text}")
    try:
        from nunmai_cli.anon_auth import is_anonymous_agent
        if is_anonymous_agent(agent):
            ...
            _plines(agent, "   Your session ended and Nunmai couldn't start a new one.",
                    "   Sign in with a Nunmai account (it's free), or switch providers with /model.")
            return
    except Exception:
        pass
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: nunmai auth add nous",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_nunmai_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `nunmai doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_nunmai_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Nunmai-managed OAuth/setup tokens",
        ...
        "     • Nunmai login (OAuth): run 'nunmai auth add anthropic' to sign in again, then retry",
        "     • Inspect what Nunmai holds: nunmai auth list anthropic",
        "     • Legacy cleanup: nunmai config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: nunmai config set ANTHROPIC_API_KEY \"\"",
    )
...
def _refresh_credentials_after_401(
    ...
    if (
        ...
        if agent._try_refresh_nous_client_credentials(force=True):
            agent._buffer_vprint("🔐 Nunmai agent key refreshed after 401. Retrying request...")
            return True
...
_WELCOME_ROUTE_HEAL_COPY = {
    ...
    "named_on_welcome_host": "Reconnected to your Nunmai account's own route.",
}


def _recover_welcome_tier(agent: Any, classified: Any, _retry: TurnRetryState) -> bool:
    """Two one-shot repairs for the Nunmai free tier, both silent on the wire and named once in chat.
...
def recover_after_classification(
    ...
    Nunmai paid-entitlement refresh → Codex stale-reasoning strip on 401 ``token_expired`` →
    ...
    Returns ``(retry_now, recovered_with_pool)``; the latter feeds the Nunmai rate-limit guard."""
    ...
    if (
        ...
        if _try_refresh_nous_paid_entitlement_credentials(agent):
            _vlines(agent, "🔐 Nunmai paid access verified — refreshed runtime credentials and retrying request...")
            return True, False
    ...
    # Route rejecting a reasoning disable: a reasoning-mandatory route (Nunmai Portal / OpenRouter,
    ...
    if (
        ...
        try:
            from nunmai_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
        ...
        return
    if provider == "nous" and _print_nous_entitlement_guidance(agent, "Nunmai model access"):
        return
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            ...
            _vlines(
                ...
                "      refreshed by another client (Codex CLI, VS Code) or another Nunmai profile.",
            ...
            _vlines(
                ...
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `nunmai model`.",
            )
        else:  # nous
            _vlines(
                agent,
                "   💡 Nunmai Portal OAuth token was rejected (HTTP 401). Your token may be",
                "      expired, revoked, or your account may be out of credits. To fix:",
                "      1. Re-authenticate: nunmai portal",
                "      2. Check your portal account: https://portal.nousresearch.com",
            )
            # ``:free`` is OpenRouter slug syntax; Nunmai Portal will reject the model
            ...
            if isinstance(model, str) and model.endswith(":free"):
                _vlines(
                    ...
                    "         Nunmai Portal won't recognize that model name. Either switch to a",
                    f"         Nunmai catalog model, or run `/model openrouter:{model}` to use OpenRouter.",
                )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: nunmai setup",
...
def _welcome_tier_guidance(classified: Any, *, model: Any, in_chat: bool, door: bool = True) -> str:
    """Copy for a Nunmai free-tier refusal the classifier parsed (``welcome_refusal`` /
    ...
    if not refusal and not route:
        return ""
    from nunmai_cli.anon_auth import welcome_refusal_copy, welcome_route_refusal_copy
    if refusal:
...
def _welcome_outage_copy(base_url: Any, classified: Any, *, anonymous: bool = False) -> str:
    """On the Nunmai free tier, a transport / server failure that outlived every retry reads as one
    ...
    try:
        from nunmai_cli.anon_auth import FREE_TIER_OUTAGE_COPY, route_is_welcome_host
...
def nonretryable_client_error_result(
    ...
    if _welcome_hint:
        ...
        _vlines(agent, f"   💡 Fix: pick another model (/model), or check `{display_nunmai_home()}/logs/agent.log`.")
    ...
    if classified.reason == FailoverReason.upstream_blocked:
        _vlines(
            ...
            "      set `extra_headers: {User-Agent: NunmaiAgent/1.0}` on the custom_providers entry,",
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "      To route future blocks to another provider automatically: nunmai fallback add",
        )
    ...
    if classified.reason == FailoverReason.ssl_cert_verification:
        _vlines(
            agent,
            "   💡 Nunmai couldn't verify the provider's security certificate. This fails the same",
            ...
            "      • Missing/stale system CA store? Refresh it (in Nunmai's venv: `uv pip install",
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``), ``"continue"`` (re-enter the retry loop; Nunmai
...
def _is_genuine_nous_rate_limit(agent: Any, api_error: Exception, error_context: Any, classified: Any = None) -> bool:
    """Record a genuine account-level Nunmai 429 to the cross-session breaker; upstream
...
def route_classified_error(
    ...
    chain once; genuine Nunmai 429 → cross-session breaker + re-enter the loop exactly once."""
    ...
    # Nunmai Portal: a genuine account-level 429 is recorded to a shared file so ALL
    ...
    if (
        ...
        # Re-enter the loop exactly once so the top-of-loop Nunmai guard runs
