...
def recover_after_classification(
    ...
    Nunmai paid-entitlement refresh → credential-pool rotation → image shrink →
    ...
    # Reasoning-mandatory route (Nunmai Portal / OpenRouter, e.g. GLM-5.3) 400s on
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `nunmai auth` to re-authenticate.",
            )
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        nunmai fallback add   (interactive picker — same as `nunmai model`)",
        )
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.nunmai/config.yaml` to extend the per-call timeout. (Nunmai's built-in floor is 600s for "
