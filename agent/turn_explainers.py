...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "replaced": (
        ...
        "`nunmai doctor --fix` or in-place FTS repair — stop "
    ...
    "deleted_wal": (
        "the turn was stopped because a live Nunmai process held a retired "
        ...
        "Check the logs for whether Nunmai captured the retired generation, "
        "then read the adjacent state.db.retired-wal-*/manifest.json. If "
        "manifest.main.mode is `copied`, inspect that artifact with `nunmai "
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                # the process default, and a bare `nunmai` follows active_profile (#105887).
