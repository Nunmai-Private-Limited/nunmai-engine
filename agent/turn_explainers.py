...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    # logger.error at nunmai_state.py::_raise_if_db_replaced — never in the chat reply.
    "replaced": (
        "the session database file was replaced while Nunmai was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop Nunmai "
        "(`nunmai {profile_arg}gateway stop`), run `nunmai {profile_arg}doctor` — not "
        "`nunmai {profile_arg}doctor --fix`, which would repair the wrong file in place — "
        "then start it again and send your message once more. Advanced recovery steps are "
        "in the log."
    ),
    "deleted_wal": (
        "another Nunmai process still holds an old copy of the session database's write-ahead "
        "log, so Nunmai stopped writing to keep the file safe and this message was not saved (a "
        "copy is kept in {home}/sessions/). Nothing is lost. Quit every Nunmai process on this "
        "profile (Desktop app, `nunmai {profile_arg}gateway stop`, dashboard, cron), run "
        "`nunmai {profile_arg}doctor` — it names any process still holding the log — then start "
        "Nunmai again and send your message once more. Do not run `doctor --fix` or delete "
    ...
    "corrupt": (
        ...
        "1. Run `nunmai {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   nunmai {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) nunmai {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `nunmai {profile_arg}doctor --fix` "
        "(or restart Nunmai, which repairs the index on open), then "
        "send your message again."
    ),
    "disk": (
        "Nunmai couldn't save this conversation to disk, so it stopped rather than lose "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    "Nunmai couldn't save this conversation, so it stopped rather than lose your messages. "
    "Possible causes: the drive is out of room, or another Nunmai process is holding the "
    "database. Close other Nunmai windows, run `nunmai {profile_arg}doctor` to check "
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``nunmai_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        *, task_id: Optional[str] = None,
        ...
        if landed:
            ...
            # Nunmai-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``NUNMAI_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="NUNMAI_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``NUNMAI_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="NUNMAI_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            from nunmai_constants import display_nunmai_home, profile_cli_selector
            from nunmai_state_errors import STORAGE_RECOVERY_DOCS_URL

            # Copy-pasteable, so pin every `nunmai` command to the profile whose store failed:
            # a multi-profile backend (Desktop serve) hosts sessions whose state.db is NOT the
            # process default, and a bare `nunmai` follows active_profile (#105887).
            body = (
                _PERSISTENCE_CAUSE_EXPLANATIONS.get(
                    persistence_cause or "unknown", _PERSISTENCE_DEFAULT_EXPLANATION
                )
                .replace("{home}", display_nunmai_home())
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_nunmai_root() / "backups")
                )
