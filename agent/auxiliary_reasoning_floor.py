...
mandatory for this endpoint and cannot be disabled" — the Nunmai Portal on gpt-6-astra); before this
...
def _catalog_marks_mandatory(provider: Optional[str], base_url: Optional[str], model: Optional[str]) -> bool:
    """True when the route's ``/v1/models`` catalog (OpenRouter, Nunmai Portal) flags *model*
    ...
    from nunmai_cli import models_reasoning_caps as caps_mod
