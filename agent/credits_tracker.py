"""Nunmai credits: parse ``x-nous-credits-*`` / ``x-nous-tool-pool-*`` response
...
def _is_nous_welcome_route(base_url: str) -> bool:
    """True when *base_url* is the Nunmai welcome host, which serves only the free tier. Local data only;
    ...
    try:
        from nunmai_cli.anon_auth import route_is_welcome_host
    except ImportError:
        return False
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """True when *model* is a Nunmai free-tier model, using ONLY local data: (1) ``:free`` suffix — canonical
    Nunmai free SKU marker; (2) ``stealth/`` prefix — stealth-preview SKUs are free without the suffix
    (naming-convention trust: a PAID ``stealth/`` model would wrongly suppress the banner); (3) a PEEK into
    ``nunmai_cli.models``' pricing cache (filled by the model picker; a miss never fetches). Fail-open to
    ...
    if not base_url:
        return False
    # (4) the Nunmai free tier: the welcome host serves only the free tier. A free-tier identity carries $0
    ...
    if _is_nous_welcome_route(base_url):
        return True
    try:
        from nunmai_cli.models import _is_model_free
        from nunmai_cli.models_pricing import peek_cached_pricing
...
def evaluate_credits_notices(state: CreditsState, latch: dict, *, model_is_free: bool = False) -> tuple[list[AgentNotice], list[str]]:
    ...
    if target_band != latch.get("usage_band"):
        ...
        if target_band is not None:
            # Absolute dollars used (a bare "N%" is only meaningful against a Nunmai cap): cap − remaining,
...
def parse_credits_headers(headers: Mapping[str, str], provider: str = "") -> Optional[CreditsState]:
    ...
    try:
        ...
        # version header is absent — the hot path for non-Nunmai providers.
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning("credits header version %d unsupported, ignoring — update Nunmai", version_val)
            return None
        fields: dict[str, Any] = {
...
# ── Dev fixtures (NUNMAI_DEV_CREDITS_FIXTURE): throwaway scaffolding to trigger any notice state
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Fixture CreditsState for NUNMAI_DEV_CREDITS_FIXTURE, or None (unknown name / unset). Prod-leak guard:
    applies ONLY when NUNMAI_DEV_CREDITS is also on, so a stray fixture env var never surfaces fabricated balances."""
    name = os.environ.get("NUNMAI_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("NUNMAI_DEV_CREDITS")):
        return None
...
def _warm_nous_pricing_cache() -> None:
    """Fill the in-process Nunmai pricing catalog so :func:`is_free_tier_model`'s peek can answer.
...
def rewarm_pricing_before_depleted_notice(agent) -> bool:
    """Depleted account, model the peek cannot vouch for, Nunmai catalog cold: start a background warm
    ...
    subscription-billed model. False = decide now: peek warm, not on Nunmai, a warm in flight (the
    ...
    if getattr(agent, "provider", "") != "nous" or not base_url:
        return False
    from nunmai_cli.models_pricing import peek_cached_pricing, pricing_fetch_suppressed
