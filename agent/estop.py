...
``nunmai pause`` writes a sentinel file at ``$NUNMAI_HOME/ESTOP``;
``nunmai resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Nunmai is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.nunmai/ESTOP``.
...
def _nunmai_home() -> Path:
    """Resolve the active NUNMAI_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active NUNMAI_HOME."""
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on NUNMAI_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Nunmai is paused ({reason}). New work is on hold; "
            "run `nunmai resume` to pick things back up."
        )
    return (
        "⏸️ Nunmai is paused. New work is on hold; "
        "run `nunmai resume` to pick things back up."
    )
