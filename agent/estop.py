...
``nunmai pause`` writes a sentinel at ``$NUNMAI_HOME/ESTOP``; ``nunmai resume``
...
(fail safe, e.g. ``touch ~/.nunmai/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.nunmai).
from agent.file_safety import _nunmai_home_path as _nunmai_home, _nunmai_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `nunmai pause`."""
    return _nunmai_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (NUNMAI_HOME=~/.nunmai/profiles/<n>) must still honor an operator's ~/.nunmai/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Nunmai is paused{tag}. New work is on hold; run `nunmai resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `nunmai resume` (%s)",
