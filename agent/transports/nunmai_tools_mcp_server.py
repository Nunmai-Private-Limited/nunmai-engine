...
Codex owns the loop and tool list there, so a curated subset of Nunmai tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.nunmai-tools]``. Run: ``python -m agent.transports.nunmai_tools_mcp_server``.
"""

from __future__ import annotations

# First, like every entry point: stdio, import-path and environ-lifetime fixes (nunmai_bootstrap).
...
if __name__ == "__main__":
    try:
        ...
        pass  # a partial ``nunmai update`` can leave the bootstrap unregistered
...
NUNMAI_TOOLS_MCP_SERVER_NAME = "nunmai-tools"
...
# Each name MUST match a registered Nunmai tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read NUNMAI_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on NUNMAI_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Nunmai tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"nunmai-tools MCP server requires the 'mcp' package: {exc}") from exc

    from model_tools import get_tool_definitions, handle_function_call

    mcp = MCPServer(
        NUNMAI_TOOLS_MCP_SERVER_NAME,
        instructions=(
    ...
    # Authoritative Nunmai schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Nunmai process", name)
            continue
    ...
    logger.info("nunmai-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Keep Nunmai' own banners off stdout (the MCP wire).
