...
_XAI_CLIENT_WEB_SEARCH_ALIAS = "nunmai_web_search"
...
# nunmai_<name> and map them back before local dispatch.
...
# (nunmai_<name>), map back in normalize_response so Nunmai dispatch is unaffected. See #85589.
...
_RESERVED_TOOL_ALIAS_PREFIX = "nunmai_"
...
def _openai_prefers_native_web_search() -> bool:
    ...
    Nunmai tool in place rather than swap in a built-in the endpoint might reject.
...
def _alias_wire_tools(
    ...
    swaps it 1:1 for the built-in, client mode keeps Nunmai dispatch under an alias.
    ...
    # Keep Nunmai's client-side functions available under wire aliases.
    ...
    # xAI server-side web search vs Nunmai web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Nunmai dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``nunmai_web_search`` so Grok cannot hijack the name. The alias is mapped back to
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Nunmai-side aliases; the backend knows only the base slug.
            "model": wire_model,
