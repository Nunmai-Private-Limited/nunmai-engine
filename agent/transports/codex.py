...
# Wire-name used when Nunmai keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Nunmai's configured provider
...
# rename on the wire (nunmai_<name>), map back in normalize_response so
# Nunmai dispatch is unaffected.
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Nunmai web providers.
        #
        ...
        #    keep Nunmai dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``nunmai_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Nunmai dispatches
                ...
                # (nunmai_web_search / nunmai_search_files, #85589).
