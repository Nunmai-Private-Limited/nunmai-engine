...
_XAI_TOOL_SEARCH_ALIAS = "nunmai_tool_search"
...
def _rename_tool_search_bridge_for_xai(tools: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    If a real tool already holds ``nunmai_tool_search``, the bridge takes a ``_2``/``_3`` suffix.
    """
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    """Clamp Nunmai' extended effort set (``ultra``) to the OpenAI-compat wire vocabulary.

    Nunmai' internal effort set extends the wire vocabulary with ``ultra`` (the /reasoning command documents
...
def _route_replays_reasoning_details(base_url: Any) -> bool:
    ...
    reasoning array, also consumed by the Nunmai Portal).
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def _normalize_tool_call(self, tc: Any) -> ToolCall | None:
        ...
        if tc_function is None or name is None:
            return None
        # Reverse only aliases THIS request emitted; a real ``nunmai_tool_search`` tool stays itself.
