...
Owns one Codex thread per Nunmai session: drives ``turn/start``, consumes
...
# Nunmai' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_NUNMAI_TO_CODEX_PERMISSION_PROFILE = {
...
def _image_part_to_turn_input(item: dict) -> Optional[dict]:
    """Map one Nunmai image part onto the app-server ``UserInput`` shape.
...
class CodexAppServerSession:
    """One Codex thread per Nunmai session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        # A codex thread id persisted by an earlier process for this Nunmai session: the first
        ...
        # Nunmai' composed system prompt (SOUL.md, memory, channel overrides). Sent ONCE per thread as
        ...
        # Nunmai' prior transcript, appended to developerInstructions ONLY when a thread is started from
        ...
        self._permission_profile = permission_profile or _NUNMAI_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("NUNMAI_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        if self._client is None:
            ...
            self._client.initialize(client_name="nunmai", client_title="Nunmai Engine", client_version=_get_nunmai_version())
        ...
        # Nunmai supplies the agent identity through its own system prompt; ``personality: "none"`` strips
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Nunmai' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own nunmai-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == NUNMAI_TOOLS_MCP_SERVER_NAME else "decline"
...
# Nunmai approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Nunmai approval choice onto codex's approval decision wire value."""
...
def _get_nunmai_version() -> str:
    """Best-effort Nunmai version string for codex's userAgent line."""
    try:
