...
# Nunmai' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Nunmai/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Nunmai session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NUNMAI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NUNMAI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="nunmai",
            client_title="Nunmai Engine",
            client_version=_get_nunmai_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Nunmai' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Nunmai' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own nunmai-tools callback we
            # auto-accept — the user already approved Nunmai' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Nunmai
        approval-mode/timeout logic. The Nunmai-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Nunmai approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Nunmai approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Nunmai returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Nunmai approval mode/timeout resolution
    ...
    # "prompt expired"; the Nunmai-side messaging already distinguishes them.
    return "decline"
