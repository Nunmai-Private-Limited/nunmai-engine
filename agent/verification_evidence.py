...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("nunmai-verify-", "nunmai-ad-hoc-")
...
def _db_path() -> Path:
    return get_nunmai_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "nunmai verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``nunmai verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="nunmai verify", kind="verify",
