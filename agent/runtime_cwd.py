...
_SESSION_CWD: ContextVar = ContextVar("NUNMAI_SESSION_CWD", default=_UNSET)
...
def scoped_session_cwd() -> str:
    ...
    Remote and container paths may not exist on the Nunmai host. Callers that only need
...
def resolve_context_cwd() -> Path | None:
    ...
    launch dir). An existing configured path is honored verbatim — including the Nunmai source tree, a
    legitimate workspace when developing Nunmai; fallback-directory policy lives in the caller."""
