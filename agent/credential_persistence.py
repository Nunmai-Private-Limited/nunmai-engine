...
``auth.json``.  They intentionally have no dependency on ``nunmai_cli.auth`` so
...
# Sources Nunmai owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Nunmai-owned OAuth/device-code state)
