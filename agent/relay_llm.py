...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Nunmai turn when present."""
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Nunmai's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Nunmai-side callbacks run while the native pipeline drives
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Nunmai Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
