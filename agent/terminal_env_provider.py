...
Providers live in ``~/.nunmai/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.nunmai/cache`` files land
  inside the backend (e.g. ``"~/.nunmai"`` for home-synced backends,
  ``"/root/.nunmai"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_nunmai_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Nunmai cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``nunmai setup`` after this backend is selected.
