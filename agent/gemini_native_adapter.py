...
_API_CLIENT = f"nunmai-engine/{_NUNMAI_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Nunmai typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.nunmai/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.nunmai/.env (get one at https://aistudio.google.com/app/apikey), or run `nunmai setup` to "
...
class GeminiNativeClient:
    ...
    # (No NUNMAI_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
