...
from nunmai_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, split_command_line, windows_hide_flags
...
def _home_key() -> str:
    return str(get_nunmai_home().expanduser().resolve())
...
def register_from_config(cfg: Optional[Dict[str, Any]], *, accept_hooks: bool = False) -> List[ShellHookSpec]:
    ...
    if env_var_enabled("NUNMAI_SAFE_MODE"):  # hooks are user customizations too — fire zero user-configured code
    ...
    if not specs:
        return []
    from nunmai_cli.plugins import get_plugin_manager  # lazy: avoids import cycle
    ...
    for spec in specs:
        ...
        if not already_allowlisted and not _prompt_and_record(spec.event, spec.command, accept_hooks=effective_accept):
            logger.warning("shell hook for %s (%s) not allowlisted — skipped. Use --accept-hooks / "
                           "NUNMAI_ACCEPT_HOOKS=1 / hooks_auto_accept: true, or approve at the TTY prompt next run.",
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    """Parse config hooks without registering (``nunmai hooks list`` / doctor)."""
...
def re_register_config_hooks() -> None:
    ...
    Only the idempotence keys for the *current* Nunmai home are cleared — ``discover_and_load(force=True)``
...
# application") for a text file, so every ``command: "~/.nunmai/agent-hooks/x.sh"`` example in
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    # NUNMAI_HOME follows the routed profile (the import-time environ holds the launch profile's), and
...
# pre_tool_call dialects in check order — Nunmai ``action`` then Claude-Code ``decision`` — as (verb key,
# block-message primary, secondary, modify payload key); both translate to the canonical Nunmai shape.
...
def _parse_pre_tool_call(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Nunmai-only escalation to the human-approval gate (#92553). Claude-Code's ``decision:
...
def _parse_pre_verify(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    # "continue" (Nunmai) / "block" (Claude-Code Stop) both mean keep going; no message is a no-op.
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Nunmai wire-shape dict, or ``None``."""
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning("Failed to persist shell hook allowlist to %s: %s. The approval is in-memory for this run, "
                       ...
                       "--accept-hooks / NUNMAI_ACCEPT_HOOKS).", p, exc)
...
def _resolve_effective_accept(cfg: Dict[str, Any], accept_hooks_arg: bool) -> bool:
    """Any truthy opt-in channel wins: explicit arg, NUNMAI_ACCEPT_HOOKS, hooks_auto_accept."""
    if accept_hooks_arg or os.environ.get("NUNMAI_ACCEPT_HOOKS", "").strip().lower() in _TRUTHY:
        return True
...
def run_once(spec: ShellHookSpec, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    """Fire one hook with a synthetic payload (``nunmai hooks test`` / doctor) through the production path."""
