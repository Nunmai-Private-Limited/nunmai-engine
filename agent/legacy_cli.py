"""Argument layer for the legacy ``nunmai-engine`` runner (``run_agent.main``).

A console script calls its target with no arguments, so pointing ``nunmai-engine``
...
# nunmai_bootstrap first (UTF-8 stdio on Windows; no-op on POSIX), like every other entry point.
try:
    import nunmai_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # partial `nunmai update` — only skips the Windows UTF-8 stdio setup

# The `nunmai-engine` console script lands here without nunmai_cli.main: repair a `nunmai update` killed
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="nunmai-engine",
        description="Legacy single-query Nunmai Engine runner. For the full CLI use `nunmai`.",
    )
    parser.add_argument("--version", action="version", version=f"Nunmai Engine v{__version__} ({__release_date__})")
...
def main(argv: Optional[List[str]] = None, *, run: Optional[Callable[..., object]] = None) -> int:
    ...
    if query is None and not args.list_tools:
        parser.print_help()
        print("\nNo query given: pass one with --query (or run `nunmai` for the interactive CLI).")
        return 0
