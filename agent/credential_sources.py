...
``nunmai auth remove <provider> <N>`` must make the entry stay gone across
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    ...
    "Nunmai uses only its own logins; run `nunmai auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex and Claude OAuth refresh tokens are single-use and rotate, so once Nunmai borrows a CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, Nunmai never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.nunmai/.env; hint when the shell exports it."""
    ...
    # .env as utf-8-sig like nunmai_cli/config.py: a BOM-sensitive read would
