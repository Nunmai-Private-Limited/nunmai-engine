...
INSPECTION_STAMP_ATTR = "data-nunmai-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-nunmai-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-nunmai-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-nunmai-vault-slot]").forEach((n) => n.removeAttribute("data-nunmai-vault-slot"));
