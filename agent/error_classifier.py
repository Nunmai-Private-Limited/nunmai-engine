...
# The Nunmai gateway's own words for "the free tier will not serve this" — a billing wall for a
...
_BILLING_ERROR_CODES = frozenset({
    ...
    # Nunmai paid model behind an empty credit balance arrives as a 404 (#115702).
...
# Parameters Nunmai sends on SOME routes only → hosts where that is deliberate.
...
# A reasoning-mandatory route answering ``reasoning: {enabled: false}`` (Nunmai Portal + OpenRouter wording).
...
def is_reasoning_required_rejection(error_msg: str) -> bool:
    ...
    this endpoint and cannot be disabled", the Nunmai Portal on gpt-6-astra). The opposite of
...
# 404: Nunmai API surfaces credit depletion as a paid model vanishing from the
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from nunmai_cli.plugins import get_plugin_error_classification
...
def _nous_welcome_tier(c: _Ctx) -> Optional[Verdict]:
    """The Nunmai inference gateway's welcome-tier (free tier) refusals, read from the structured body.
    ...
    """
    from nunmai_cli.anon_auth import (
...
def classify_api_error(
    ...
    ``base_url`` (optional) is the route the call went to; the Nunmai welcome tier keys its
...
def _off_route_host(c: _Ctx) -> str:
    ...
    from nunmai_cli.route_identity import provider_owns_route
...
def _classify_400(c: _Ctx) -> Verdict:
    ...
    if code == "invalid_encrypted_content" or "invalid_encrypted_content" in msg or (
        ...
        return _V_INVALID_ENCRYPTED
    # Route rejecting a reasoning disable: a reasoning-mandatory route (GLM-5.3 on Nunmai Portal /
