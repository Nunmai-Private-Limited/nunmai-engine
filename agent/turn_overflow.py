...
_GITHUB_MODELS_HINT = (
    ...
    "      request at ~8K tokens. Nunmai' system prompt + tool schemas baseline",
    "      exceeds that floor, so this endpoint cannot run an agentic loop.",
    "      Use the `copilot` provider with a Copilot subscription token (`nunmai",
...
def _recover_context_length(st: _Recovery, _retry: TurnRetryState, error_msg: str) -> OverflowVerdict:
    ...
    if is_output_cap_error(error_msg):
        return st.fail_turn(
            ...
            notices=(
                ...
                "   💡 Nunmai has no user setting for the output cap — check the endpoint's default max output "
    ...
    # Nunmai knows for this model (after adopting any limit the server reported), so compressing
    ...
    # Nunmai assumes, and "wait and /retry" would fail identically forever; compress instead.
