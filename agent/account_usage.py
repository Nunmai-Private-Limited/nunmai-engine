...
from nunmai_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
class AccountUsageSnapshot:
    ...
    # fields Nunmai does not normalize yet. Only populated by providers that fetch a JSON body.
...
def _nous_snapshot(windows: list, details: list, tail: list, *, source: str, plan: Optional[str] = None) -> Optional[AccountUsageSnapshot]:
    """Nunmai snapshot with *tail* lines appended, or None when there is nothing to show."""
    if not windows and not details:
        return None
    return _snapshot("nous", source, windows, details + tail, title="Nunmai credits", plan=plan)
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        from nunmai_cli.nous_account import nous_portal_topup_url
...
def _nous_logged_in() -> bool:
    """Cheap local auth-state check: a Nunmai access token is present. Fail-open False."""
    try:
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Rendered Nunmai-credits /usage lines, or [] when there's nothing to show. Independent of any live agent
    (logged-in gate, then a bounded portal fetch); shared by CLI ``_show_usage`` and the TUI ``session.usage`` RPC.
    Fail-open: any hiccup or timeout → []. NUNMAI_DEV_CREDITS_FIXTURE renders from the fixture instead of the portal."""
    try:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        if getattr(state, "paid_access", True) is False:
            details.append(_DEPLETED_LINE)
        return _nous_snapshot(windows, details, ["(dev fixture — NUNMAI_DEV_CREDITS_FIXTURE)"], source="dev-fixture")
    except (AttributeError, TypeError):
        return None
...
def _codex_reset_outcome(body: dict, available: int) -> CodexResetRedeemResult:
    ...
    if code == "reset":
        ...
        try:
            from nunmai_cli.auth import clear_codex_pool_quota_cooldowns
...
def redeem_codex_reset_credit(
    *, base_url: Optional[str] = None, api_key: Optional[str] = None, force: bool = False,
    ...
    try:
        ...
        return _unavailable("No Codex credentials available. Run `nunmai auth` to sign in with your ChatGPT account.")
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `nunmai auth` and sign in with your ChatGPT account.")
