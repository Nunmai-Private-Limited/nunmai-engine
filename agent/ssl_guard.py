...
_REPAIR_HINT = (
    "Repair: run `nunmai doctor --fix` (auto-reinstalls certifi), or "
...
def verify_ca_bundle() -> None:
    ...
    is missing/corrupt."""
    if is_truthy_value(os.getenv("NUNMAI_SKIP_SSL_GUARD", "")):
        logger.debug("SSL CA bundle guard skipped via NUNMAI_SKIP_SSL_GUARD")
        return
