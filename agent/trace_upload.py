"""Upload a Nunmai session transcript to Hugging Face as an agent trace, re-emitted in the **Claude Code
...
def build_trace_jsonl(messages: List[Dict[str, Any]], *, session_id: str, model: str = "", cwd: str = "", redact: bool = True) -> str:
    ...
    for msg in messages:
        ...
        entry = {  # key order is the wire order
            ...
            "sessionId": session_id, "version": _NUNMAI_VERSION, "gitBranch": git_branch, "uuid": turn_uuid,
...
def upload_session_trace(
    ...
    """CLI/gateway entry point: load, convert, upload to ``{user}/nunmai-traces``. Status string, never raises."""
    if not session_id:
