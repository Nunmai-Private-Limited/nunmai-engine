"""Upload a Nunmai session transcript to Hugging Face as an agent trace.

Nunmai stores sessions in its own SQLite store (``nunmai_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``nunmai trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Nunmai' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "nunmai-traces"
_NUNMAI_VERSION = "nunmai-engine"
...
# ---------------------------------------------------------------------------
# Conversion: Nunmai OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Nunmai' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Nunmai conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.nunmai/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `nunmai trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/nunmai-traces`` dataset. Returns a
