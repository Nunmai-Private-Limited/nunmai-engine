...
the other affinity hints Nunmai already sends (OpenRouter's sticky ``session_id``, xAI's
...
def opencode_transport(provider: Optional[str], model: Optional[str], base_url: Optional[str]) -> tuple[Optional[str], str]:
    ...
    The main runtime (``nunmai_cli/runtime_provider.py``) always re-derives from the effective model;
    ...
    hosts all count.
    """
    from nunmai_cli.models import normalize_opencode_base_url, normalize_opencode_model_id, opencode_model_api_mode
    from nunmai_cli.runtime_provider_custom import _get_named_custom_provider, _opencode_family_for_custom
...
def custom_provider_session_affinity_headers(
    ...
    try:
        from nunmai_cli.config import get_custom_provider_session_affinity_header
