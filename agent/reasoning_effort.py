...
Nunmai' internal effort ladder (``VALID_REASONING_EFFORTS`` plus ``none``) is wider than any
...
# disable can be clamped when a provider publishes it as a level. ``ultra`` is Nunmai-internal
...
#: Widest OpenAI-compatible wire vocabulary (OpenRouter, Nunmai Portal).
...
def is_astra_model(model: Optional[str]) -> bool:
    """``gpt-6-astra`` or its Nunmai-side ``-900k`` picker alias, with or without a ``vendor/`` prefix.
...
def effort_display_label(effort: Optional[str], provider: Optional[str] = None, model: Optional[str] = None) -> str:
    ...
    it verbatim, else ``"<level> (sends <clamped> on this route)"`` so a Nunmai-internal step such as
...
def clamp_reasoning_config(reasoning_config: Optional[dict], supported: Sequence[str] = OPENAI_COMPAT_WIRE_EFFORTS) -> Optional[dict]:
    ...
    The entry clamp for an OpenAI-compatible chat-completions request builder: Nunmai-internal
