...
Nunmai' internal effort ladder (``nunmai_constants.VALID_REASONING_EFFORTS``
...
# nunmai_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
# disable can be clamped too when a provider publishes it as a level).
EFFORT_LADDER: tuple[str, ...] = (
    "none", "minimal", "low", "medium", "high", "xhigh", "max", "ultra",
)

# ``ultra`` is Nunmai-internal ladder vocabulary (the Codex product tier); no
