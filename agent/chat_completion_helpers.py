...
from nunmai_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from nunmai_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _provider_preferences_for_agent(agent) -> Dict[str, Any]:
    ...
    with contextlib.suppress(Exception):
        ...
        from nunmai_constants import resolve_per_model_provider_routing
...
def _merge_nous_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    ...
    The Nunmai profile is only consulted by the OpenAI-wire transport; ``session_id``
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("NUNMAI_STREAM_STALE_GIVEUP", 5)
...
def _configured_stale_base(agent) -> float:
    """Per-provider ``stale_timeout_seconds`` config, else NUNMAI_STREAM_STALE_TIMEOUT (180s)."""
    cfg = get_provider_stale_timeout(agent.provider, agent.model)
    return cfg if cfg is not None else env_float("NUNMAI_STREAM_STALE_TIMEOUT", 180.0)


def _local_stream_stale_timeout_default() -> float:
    """Local-provider stale ceiling: ``agent.local_stream_stale_timeout`` (900s) or
    NUNMAI_LOCAL_STREAM_STALE_TIMEOUT. Shared by the stream stale detector and the
    ...
    return env_float("NUNMAI_LOCAL_STREAM_STALE_TIMEOUT", local_default)
...
def _cloud_stale_timeout_for(agent, api_kwargs: dict) -> float:
    ...
    if explicit is not None:
        return explicit
    return _cloud_stale_timeout(env_float("NUNMAI_STREAM_STALE_TIMEOUT", 180.0), api_kwargs)
...
def _bedrock_converse_call(api_kwargs: dict, *, stream: bool, on_stream_denied=None):
    """Pop the Nunmai routing keys and call ``converse`` / ``converse_stream`` (boto3
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from nunmai_cli.local_runtime.load_progress import get_loading_progress, get_prefill_progress
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS,
    NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each),
    NUNMAI_CODEX_TTFB_DISABLE_ABOVE_TOKENS / NUNMAI_CODEX_TTFB_STRICT,
    NUNMAI_CODEX_TTFB_MAX_SECONDS (opt-in ceiling, default 0 = none), NUNMAI_CODEX_HARD_TIMEOUT_SECONDS.
    """
    ...
    if codex and openai_codex_backend:
        ...
        hard_timeout = env_float("NUNMAI_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
        if hard_timeout > 0:
    ...
    ttfb_enabled = codex
    ttfb_explicit = env_float("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", -1.0) != -1.0
    ttfb_timeout = env_float("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if ttfb_timeout <= 0:
        ttfb_enabled = False
        ...
        disable_above = env_float("NUNMAI_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        strict = os.environ.get("NUNMAI_CODEX_TTFB_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
        if not strict and disable_above > 0 and est_tokens >= disable_above and ttfb_timeout < idle_default:
            logger.info("Scaling openai-codex no-event TTFB watchdog from %.0fs to %.0fs "
                ...
                "Set NUNMAI_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.", ttfb_timeout, idle_default,
        ...
        ttfb_cap = env_float("NUNMAI_CODEX_TTFB_MAX_SECONDS", 0.0)
        if ttfb_cap > 0 and ttfb_timeout > ttfb_cap:
            logger.info("Capping openai-codex no-event TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens) per NUNMAI_CODEX_TTFB_MAX_SECONDS.", ttfb_timeout, ttfb_cap,
                f"{est_tokens:,}")
        ...
        if local_ceiling > ttfb_timeout:
            logger.info("Local provider detected (%s) — no-event TTFB watchdog raised from %.0fs to %.0fs "
                "(agent.local_stream_stale_timeout); set NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS for an explicit cutoff.",
    ...
    idle_explicit = env_float("NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", -1.0) != -1.0
    idle_timeout = env_float("NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_default)
...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    _qwen_meta = {"sessionId": agent.session_id or "nunmai", "promptId": str(uuid.uuid4())} if _is_qwen else None
    _profile = None
    with contextlib.suppress(Exception):
        from providers import get_provider_profile
...
def _assistant_content_for_storage(agent, assistant_message):
    ...
    # history / state.db / gateway delivery (no-op with NUNMAI_REDACT_SECRETS off).
...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    from nunmai_cli.providers import host_mandated_api_mode
...
def _fallback_api_mode_hint(fb: dict, fb_provider: str, fb_base_url_hint: Optional[str]) -> tuple[bool, str]:
    ...
    from nunmai_cli.runtime_provider import _get_named_custom_provider, _parse_api_mode
...
def _fallback_api_mode_resolved(agent, fb_provider: str, fb_model: str, fb_base_url: str) -> str:
    ...
    from nunmai_cli.runtime_provider_custom import _opencode_family_for_custom
...
def _log_fallback_activated(agent, reason, old_model, old_provider, fb_model, fb_provider) -> None:
    ...
    if reason != FailoverReason.billing:
        ...
        return
    from nunmai_constants import get_nunmai_home, profile_name_for_home
    profile = profile_name_for_home(get_nunmai_home()) or "default"
    remedy = "nunmai model" if profile == "default" else f"nunmai -p {profile} model"
    logger.warning(
...
def _iteration_summary_api_messages(agent, messages: list) -> list:
    ...
    for msg in messages:
        ...
        # every Nunmai-internal underscore-prefixed scaffolding key.
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        ...
        # Strict machine-readable mode (nunmai chat -Q, oneshot, background review): keep diagnostics out of
...
class _BedrockStream:
    ...
    def _on_stale(self, stale_elapsed: float) -> None:
        ...
        self.last_event = time.time()
        # Raises RuntimeError past NUNMAI_STREAM_STALE_GIVEUP; otherwise end
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _stream_timeouts(self) -> tuple[float, float, float]:
        ...
        ``request_timeout_seconds`` wins over NUNMAI_API_TIMEOUT (1800s) and
        NUNMAI_STREAM_READ_TIMEOUT (120s); connect/pool cover the handshake, not
        inference: 30s, or capped at 60s when configured."""
        cfg = get_provider_request_timeout(self.agent.provider, self.agent.model)
        base = cfg if cfg is not None else env_float("NUNMAI_API_TIMEOUT", 1800.0)
        if cfg is not None:
            return base, cfg, min(base, 60.0)
        read = env_float("NUNMAI_STREAM_READ_TIMEOUT", 120.0)
    ...
    def _choiceless_chunk(chunk, finish_reason):
        ...
        # Nunmai Portal usage frames (choices=[] + lastOne=true, no [DONE]) are a
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        if self.agent.provider == "moa":
            # Nunmai interrupts the managed stream; Relay alone closes the provider stream.
    ...
    def _call(self):
        _max_stream_retries = env_int("NUNMAI_STREAM_RETRIES", 2)
        # The one stream_options compatibility retry (#9705) is not a network retry and must not
        # consume the transient budget: on the last attempt (or NUNMAI_STREAM_RETRIES=0) the
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        NUNMAI_LOCAL_STREAM_STALE_TIMEOUT — an infinite one stalled sessions on a
