"""Why an auxiliary client could not be built — and the Nunmai credential failure behind it.
...
def _quarantined_nous_error(exc: BaseException) -> BaseException:
    ...
    """
    from nunmai_cli.auth import AuthError, get_provider_auth_state
    from nunmai_cli.auth_nous import _terminal_quarantine_marker
...
def pool_cooldown_message(provider_id: str) -> Optional[str]:
    ...
    were found. Run nunmai auth add …" — wrong on both counts for a valid OAuth grant that is
    ...
    return (f"Provider '{provider_id}' is set in config.yaml but {which} cooling down after a "
            ...
            f"add another credential with `nunmai auth add {provider_id}`, or switch to a "
            "different provider with `nunmai model`.")
...
def missing_provider_credentials_message(provider_id: str) -> str:
    ...
    if env_vars:
        ...
        problem, remedy = "no credentials were found", f"Run `nunmai auth add {provider_id}` to sign in"
    else:
        problem = "no credentials were found"
    switch = "switch to a different provider with `nunmai model`."
...
def _nous_credential_present(exc: BaseException) -> bool:
    """True when a Nunmai credential exists that failed — a coded error (``invalid_grant``, a quarantine
    marker, ``refresh_failed``) or persisted Nunmai auth state. The uncoded "not logged into Nunmai Portal"
    with no stored state is the normal condition for users who never chose Nunmai; the auto-route walk
...
def record_nous_credential_failure(exc: BaseException) -> str:
    """Remember *exc* as the latest Nunmai credential failure.

    Logged once per distinct message: WARNING when a real credential failed, DEBUG when Nunmai was
    simply never logged into Nunmai.
    """
    ...
    detail = f"Nunmai Portal runtime credentials unavailable: {message}"
    ...
    if first_time:
        ...
        logger.log(level, "Auxiliary Nunmai client unavailable: %s", detail)
    return detail
...
def nous_credential_failure_detail() -> Optional[str]:
    """The latest recorded Nunmai credential failure, or None when the last resolution succeeded."""
    with _lock:
