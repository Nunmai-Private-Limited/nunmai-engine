...
``kind: backend``) or ``~/.nunmai/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        ...
        Used by :mod:`nunmai_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Nunmai gating, and the
