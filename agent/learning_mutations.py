...
Shared by CLI ``nunmai journey``, the TUI ``/journey`` overlay and the desktop.
Deleting a skill *archives* it (``nunmai curator restore`` recovers it);
...
def _locate_memory(node_id: str) -> tuple[Path, list[str], int]:
    ...
    path = get_nunmai_home() / "memories" / _MEMORY_FILES[source]
    if not path.exists():
