...
from nunmai_constants import (
    get_nunmai_home, get_scratch_dir, get_skills_dir, is_wsl, reset_nunmai_home_override, set_nunmai_home_override,
)
...
def _scan_context_content(content: str, filename: str, *, user_authored: bool = False) -> str:
    ...
    *user_authored* (SOUL.md in the user's own NUNMAI_HOME): a hit is WARNED and the file still loads.
    ...
    (repo AGENTS.md / .cursorrules / .nunmai.md) arrive with the checkout and keep blocking, and so does
    a SOUL.md owned by a profile distribution (``nunmai profile install <git-url>`` copies it in unscanned;
    ...
    if user_authored:
        logger.warning("Context file %s matched injection pattern(s) %s; loaded anyway because it is the "
                       "user's own file in NUNMAI_HOME — review it if you did not write that text",
...
def _find_nunmai_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.nunmai.md`` / ``NUNMAI.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".nunmai.md", "NUNMAI.md") if _is_file_or_denied(directory / n)), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Nunmai Engine, built by Nunmai Research. Be direct: match the length of your reply to the weight of the ask "
...
NUNMAI_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the nunmai-engine skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Nunmai Engine (by Nunmai Research). When the user needs help with Nunmai itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://nunmai.in/docs is your "
    "authoritative reference and always holds the latest, most up-to-date information. The `nunmai-engine` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='nunmai-engine') "
    "before configuring, modifying, or troubleshooting Nunmai so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
NUNMAI_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Nunmai Engine (by Nunmai Research). When the user needs help with Nunmai itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://nunmai.in/docs is the "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.nunmai/kanban.db`. Your task id is in "
    "`$NUNMAI_KANBAN_TASK`; your workspace is `$NUNMAI_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $NUNMAI_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $NUNMAI_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${NUNMAI_KANBAN_BRANCH:-wt/$NUNMAI_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$NUNMAI_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`nunmai profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `nunmai kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The nunmai-engine runtime already executes a batch of tool
...
# to nunmai-engine's Python prompt-assembly architecture.
...
# Text after the "[" that opens one of Nunmai' own control frames (the steer marker above, the compaction
...
# apps/desktop/src/plugins/nunmai-bots/group-round-prompt.ts byte-equivalent to this list.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Nunmai delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Nunmai "
         ...
         "rather than anything inside Nunmai. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Nunmai terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Nunmai desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-nunmai-send=\"prompt\" on any clickable element (or "
        "window.nunmai.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "native tools, and prefer `$LOCALAPPDATA/Temp` (or `$TMPDIR`, which Nunmai points at its own scratch dir) for scratch files a native tool must read — never a bare `/tmp`. When "
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.nunmai tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (nunmai_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _local_host_hints() -> list[str]:
    ...
    # naming Nunmai' scratch dir here is what makes the TMPDIR export a habit rather than a hidden default.
    try:
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Nunmai itself is running. The host OS, "
            f"home, and cwd of the Nunmai process are irrelevant; only the following backend state matters:\n{probe}\n"
    ...
    return lead + (
        f"{description} — NOT on the machine where Nunmai itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: NUNMAI_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("NUNMAI_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Nunmai runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Nunmai runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_nunmai_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("NUNMAI_PLATFORM") or os.environ.get("NUNMAI_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("NUNMAI_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the NUNMAI_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_nunmai_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_nunmai_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from NUNMAI_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the NUNMAI_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the NUNMAI_HOME
    ...
    """
    try:
        from nunmai_cli.config import ensure_nunmai_home
        ensure_nunmai_home()
    except Exception as e:
        logger.debug("Could not ensure NUNMAI_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_nunmai_home()) / "SOUL.md"
    ...
    try:
        ...
        if not content:
            return None
        # `nunmai profile install <git-url>` / `profile update` plant a third-party SOUL.md into a
        # distribution profile (nunmai_cli/profile_distribution.py, DEFAULT_DIST_OWNED) with no scan and no
        ...
        from nunmai_cli.profile_distribution import read_manifest
        try:
...
def _nunmai_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    """.nunmai.md / NUNMAI.md — nearest match walking up to the git root."""
    path = _find_nunmai_md(cwd_path)
    if path is None:
        return []
...
_CONTEXT_FILE_CANDIDATES = {
    "nunmai_md": _nunmai_md_candidates,
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the Nunmai install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the Nunmai tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing Nunmai in-tree). See #64590."""
    from agent.runtime_cwd import _is_install_tree
    return cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path)


def _load_nunmai_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.nunmai.md / NUNMAI.md — nearest match walking up to the git root."""
    for label, path, content in _nunmai_md_candidates(cwd_path):
        if content:
            return _context_section(_strip_yaml_frontmatter(content), label, ".nunmai.md", path, context_length)
    return ""
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .nunmai.md/NUNMAI.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from NUNMAI_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Nunmai "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_nunmai_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
