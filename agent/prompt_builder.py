...
from nunmai_constants import (
    get_nunmai_home, get_skills_dir, is_wsl, reset_nunmai_home_override, set_nunmai_home_override,
)
...
def _find_nunmai_md(cwd: Path) -> Optional[Path]:
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".nunmai.md", "NUNMAI.md") if (directory / n).is_file()), None)
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Nunmai appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def _load_nunmai_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    nunmai_md_path = _find_nunmai_md(cwd_path)
    content = _read_context_file(nunmai_md_path) if nunmai_md_path else ""
    if not content:
        return ""
    label = str(nunmai_md_path.relative_to(cwd_path)) if nunmai_md_path.is_relative_to(cwd_path) else nunmai_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".nunmai.md", nunmai_md_path, context_length)
...
def build_context_files_prompt(
    ...
    # An explicitly configured cwd is honored verbatim — the Nunmai tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Nunmai
    # in-tree). See #64590.
