...
_NUNMAI_MD_NAMES = (".nunmai.md", "NUNMAI.md")


def _find_nunmai_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.nunmai.md`` or ``NUNMAI.md``.
    ...
    # could pick up a .nunmai.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _NUNMAI_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Nunmai Engine, an intelligent AI assistant created by Nunmai Research. "
...
NUNMAI_AGENT_HELP_GUIDANCE = (
    "You run on Nunmai Engine (by Nunmai Research). When the user needs help with "
    "Nunmai itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nunmai.in/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `nunmai-engine` skill with skill_view(name='nunmai-engine') "
...
NUNMAI_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    ...
    "Nunmai itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nunmai.in/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.nunmai/kanban.db`. Your task id is in "
    "`$NUNMAI_KANBAN_TASK`; your workspace is `$NUNMAI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NUNMAI_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $NUNMAI_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${NUNMAI_KANBAN_BRANCH:-wt/$NUNMAI_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$NUNMAI_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`nunmai profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `nunmai kanban <verb>` for board operations. Use "
...
#
# The nunmai-engine runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to nunmai-engine's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Nunmai "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Nunmai floating over another application, so the
    user is rarely asking about Nunmai — they are asking about the thing behind
    ...
    sentences = [
        ...
        "rather than anything inside Nunmai. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Nunmai terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Nunmai desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.nunmai.send(\"prompt\") — or a "
        "data-nunmai-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Nunmai WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Nunmai itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Nunmai runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Nunmai itself is running. The host OS, home, and cwd "
                f"of the Nunmai process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Nunmai "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Nunmai
    ...
    extra = (os.getenv("NUNMAI_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.nunmai/skills/`` directory.  External dirs
    ...
    # NUNMAI_HOME would otherwise fall back to the launch (default) home and
    ...
    try:
        ...
        # Trusted project-local dirs (./.nunmai/skills, ./.agents/skills at
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Nunmai Engine itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `nunmai-engine` skill "
            "first. It has the actual commands (e.g. `nunmai config set …`, `nunmai tools`, "
            "`nunmai setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(
    ...
    """Load SOUL.md from NUNMAI_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the NUNMAI_HOME ContextVar falls
    ...
    _home = Path(home_override) if home_override is not None else get_nunmai_home()
...
def _load_nunmai_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.nunmai.md / NUNMAI.md — walk to git root."""
    nunmai_md_path = _find_nunmai_md(cwd_path)
    if not nunmai_md_path:
        return ""
    try:
        content = nunmai_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = nunmai_md_path.name
        try:
            rel = str(nunmai_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".nunmai.md", context_length=context_length,
            read_path=str(nunmai_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", nunmai_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .nunmai.md / NUNMAI.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from NUNMAI_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Nunmai install/source
    ...
    # explicitly configured cwd is honored verbatim — the Nunmai tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Nunmai in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Nunmai install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_nunmai_md(cwd_path, context_length)
    ...
    # SOUL.md from NUNMAI_HOME only — skip when already loaded as identity
    if not skip_soul:
