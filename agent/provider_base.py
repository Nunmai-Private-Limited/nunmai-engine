...
Every tool-provider ABC shares the same identity + ``nunmai tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``nunmai tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    def is_available(self) -> bool:
        ...
        ``nunmai setup`` call it on every paint.
        """
        return True
