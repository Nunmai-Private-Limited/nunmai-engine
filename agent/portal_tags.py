...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=nunmai-engine",
        "client=nunmai-client-v<__version__>",
    ]

The version is sourced live from ``nunmai_cli.__version__`` so it auto-aligns
...
``nunmai_cli.__version__`` is the canonical source of truth.
"""
...
def _nunmai_version() -> str:
    """Return the current Nunmai release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``nunmai_cli`` cannot be imported (should
...
def nunmai_client_tag() -> str:
    ...
    """
    return f"client=nunmai-client-v{_nunmai_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Nunmai session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Nunmai conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.nunmai/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Nunmai
    ...
    """
    tags = ["product=nunmai-engine", nunmai_client_tag()]
