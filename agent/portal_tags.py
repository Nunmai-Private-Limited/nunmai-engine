...
Every Nunmai request to the Nunmai Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=nunmai-engine", "client=nunmai-client-v<__version__>"]``.
The version is read live from ``nunmai_cli.__version__`` — do NOT pre-compute it
...
# Ambient affinity scope (ROUTING value): OpenRouter's sticky ``session_id``, Nunmai
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("nunmai_affinity_scope", default=None)
...
def nunmai_client_tag() -> str:
    """``client=nunmai-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if nunmai_cli is unimportable)."""
    try:
        from nunmai_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=nunmai-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    """Fresh list of the canonical Nunmai Portal tags.
