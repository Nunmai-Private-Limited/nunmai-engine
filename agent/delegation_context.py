...
The parent Nunmai process may itself be a Kanban dispatcher worker with
NUNMAI_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's NUNMAI_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "nunmai_non_dispatcher_owned_context",
    default=False,
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``NUNMAI_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$NUNMAI_KANBAN_TASK`` — letting
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``NUNMAI_KANBAN_*`` vars while losing the ContextVar in
