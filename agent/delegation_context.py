...
A Nunmai process may itself be a Kanban dispatcher worker with NUNMAI_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("nunmai_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("nunmai_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "NUNMAI_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "NUNMAI_KANBAN_TASK", "NUNMAI_KANBAN_RUN_ID", "NUNMAI_KANBAN_CLAIM_LOCK",
    "NUNMAI_KANBAN_GOAL_MODE", "NUNMAI_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``NUNMAI_KANBAN_*`` identity gate should use."""
    return not (is_delegated_child_process_context() or _NON_DISPATCHER_OWNED_CONTEXT.get())


def owned_kanban_task() -> str:
    """The board task this execution OWNS: ``NUNMAI_KANBAN_TASK`` for the dispatcher-owned
    ...
    if not is_dispatcher_owned_worker_context():
        return ""
    return (os.environ.get("NUNMAI_KANBAN_TASK") or "").strip()
...
def _fenced_kanban_root() -> str:
    ...
    try:
        from nunmai_cli.kanban_db import kanban_home
...
def scrub_kanban_env(env: Mapping[str, str] | MutableMapping[str, str]) -> dict[str, str]:
    ...
    against a temp ``NUNMAI_HOME`` got a silently read-only board there. An inherited
    path-valued marker is kept (a grandchild that moved NUNMAI_HOME must not re-fence
...
def kanban_path_is_fenced(path: "os.PathLike[str] | str") -> bool:
    ...
    descendant only when *path* is the dispatcher-pinned ``NUNMAI_KANBAN_DB`` or lies under the
    ...
    pinned = os.environ.get("NUNMAI_KANBAN_DB", "").strip()
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("NUNMAI_KANBAN_TASK")
            or (env and (env.get("NUNMAI_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
