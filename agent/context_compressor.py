...
# (nunmai_state._rows_to_conversation), so any code that mutates a loaded or flushed dict's content in place
...
_COMPRESSION_MARKER_PREFIX = "⟪NUNMAI-CONTEXT-COMPRESSION:"
_COMPRESSION_MARKER_TEMPLATE = (
    _COMPRESSION_MARKER_PREFIX
    + " {omitted:,} of {total:,} chars omitted here by Nunmai's context compressor. "
...
def _today_for_prompt() -> str:
    ...
    try:
        ...
        # via nunmai_time.now(). The compaction summary is a mid-conversation message that is NOT part of
        ...
        return _nunmai_now().strftime("%Y-%m-%d")
