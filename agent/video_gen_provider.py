...
as ``kind: backend``) or ``~/.nunmai/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``nunmai tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NUNMAI_HOME/cache/videos/``, creating parents as needed."""
    from nunmai_constants import get_nunmai_home

    path = get_nunmai_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NUNMAI_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$NUNMAI_HOME/cache/videos/``.
