...
def _constants_path(getter_name: str) -> Path:
    """Call ``nunmai_constants.<getter_name>()`` (local import avoids cycles); ``~/.nunmai`` on any failure."""
    try:
        import nunmai_constants

        return getattr(nunmai_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.nunmai"))


def _nunmai_home_path() -> Path:
    """Active NUNMAI_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_nunmai_home")


def _nunmai_root_path() -> Path:
    """Nunmai root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_nunmai_root")


def _nunmai_dirs() -> list[Path]:
    """Resolved active NUNMAI_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (NUNMAI_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_nunmai_home_path(), _nunmai_root_path()))))
...
def _guard_homes(path: str = "") -> set[str]:
    ...
    containers, and spawned workers pin ``HOME`` to ``{NUNMAI_HOME}/home``, which leaves
    ...
    with suppress(Exception):
        from nunmai_constants import get_real_home, get_subprocess_home, _profile_home_path
...
# they are a routine local form (see nunmai_cli/windows_ssh_runtime.py) and
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    # Secret material under NUNMAI_HOME, on both the active profile and the global
    ...
    paths = [
        ...
        *(str(base / f) for f in nunmai_files for base in (_nunmai_home_path(), _nunmai_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved NUNMAI_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("NUNMAI_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# NUNMAI_HOME / root subpaths that the agent's generic file tools must not
...
_NUNMAI_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing", "vault", "browser-profile")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _nunmai_dirs():
        for sub in _NUNMAI_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside NUNMAI_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under NUNMAI_HOME / <root>. The agent never
...
# Directory-prefix read denies under NUNMAI_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Nunmai MCP token directory and cannot be read directly.",
     "is a Nunmai MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Nunmai real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Nunmai real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Nunmai credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Nunmai credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Nunmai path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under NUNMAI_HOME and the global root (exact files, plus anything
    ...
    nunmai_dirs = _nunmai_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in nunmai_dirs):
        reason = (
            "is an internal Nunmai cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in nunmai_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Nunmai credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in nunmai_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Nunmai read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from NUNMAI_HOME: ``~/.nunmai`` -> ``"default"``,
    ``~/.nunmai/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _nunmai_home_path().resolve().relative_to(_nunmai_root_path().resolve() / "profiles").parts
...
#   <NUNMAI_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.nunmai/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nunmai state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.nunmai`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .nunmai / <thing>; inner_idx = the .nunmai part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".nunmai"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Nunmai process never reads — the "
        "authoritative file is likely {inner_path!r} under the real NUNMAI_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.nunmai/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Nunmai "
        "process never reads. The authoritative file is {inner_path!r} under "
        "the real NUNMAI_HOME.",
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Nunmai scope, or is inside
    ...
    try:
        ...
        root_real = _nunmai_root_path().resolve()
