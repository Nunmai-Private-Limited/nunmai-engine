...
def _nunmai_home_path() -> Path:
    """Resolve the active NUNMAI_HOME (profile-aware) without circular imports."""
    try:
        from nunmai_constants import get_nunmai_home  # local import to avoid cycles
...
def _nunmai_root_path() -> Path:
    """Resolve the Nunmai root dir (always the parent of any profile, never per-profile)."""
    try:
        from nunmai_constants import get_default_nunmai_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    nunmai_home = _nunmai_home_path()
    nunmai_root = _nunmai_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(nunmai_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(nunmai_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(nunmai_home / "cache" / "bws_cache.enc.json"),
            str(nunmai_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved NUNMAI_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("NUNMAI_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    nunmai_dirs = []
    for base in (_nunmai_home_path(), _nunmai_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in nunmai_dirs:
                nunmai_dirs.append(real)
        except Exception:
            continue

    for base_real in nunmai_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Nunmai cache files under ``NUNMAI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under NUNMAI_HOME and the global Nunmai
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.nunmai/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active NUNMAI_HOME (profile-aware) AND the global
    # Nunmai root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (NUNMAI_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    nunmai_dirs: list[Path] = []
    for base in (_nunmai_home_path(), _nunmai_root_path()):
        try:
            real = base.resolve()
            if real not in nunmai_dirs:
    ...
    for hd in nunmai_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Nunmai cache file "
    ...
    for hd in nunmai_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Nunmai credential store "
    ...
    for hd in nunmai_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Nunmai MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Nunmai MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in nunmai_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Nunmai real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Nunmai real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Nunmai read (see
...
#
# Nunmai profiles are separate NUNMAI_HOME dirs under
...
#
# Reference: May 2026 incident where a nunmai-security profile session
# edited skills under both ``~/.nunmai/profiles/nunmai-security/skills/``
# AND ``~/.nunmai/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under NUNMAI_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from NUNMAI_HOME.
    ...
    """
    try:
        home_real = _nunmai_home_path().resolve()
...
# ``…/sandboxes/<backend>/<task>/home/.nunmai/…`` segment and warns
# regardless of which Nunmai profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.nunmai/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.nunmai`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.nunmai/…`` and returns the
    index where the inner Nunmai-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .nunmai / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".nunmai":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nunmai state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.nunmai``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.nunmai`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Nunmai resolver to
    succeed, so it works correctly even when called from contexts where
    NUNMAI_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Nunmai process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real NUNMAI_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.nunmai/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.nunmai/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host NUNMAI_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Nunmai state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Nunmai "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real NUNMAI_HOME. Use the "
