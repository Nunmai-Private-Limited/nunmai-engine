...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Nunmai accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and nunmai-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Nunmai deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Nunmai UI bridge -------------------------------------
# The app-server bypasses the Nunmai tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Nunmai tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Nunmai' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.nunmai-tools.* prefix is stripped (users see Nunmai tools).
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Nunmai tool name for a codex item (mirrors CodexEventProjector)."""
    item_type = item.get("type") or ""
    if item_type == "mcpToolCall":
        server, tool = item.get("server") or "mcp", item.get("tool") or "unknown"
        return tool if server == NUNMAI_TOOLS_MCP_SERVER_NAME else f"mcp.{server}.{tool}"
...
# Durable codex thread binding: ``sessions.model_config.codex_thread_id`` (nunmai_state), written after the
# turn's projected rows were committed, read by the next AIAgent built for the same Nunmai session so an
# API-server restart (or the per-request agents of /api/sessions/{id}/chat) resumes the model-side thread
# instead of starting an empty one while Nunmai' own transcript continues (#100531).
...
def _ensure_codex_session(agent, messages: List[Dict[str, Any]] | None = None) -> None:
    ...
    # Approval callback: Nunmai' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, NUNMAI_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Nunmai' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
    ...
    # Nunmai owns the prompt: the same composition the standard loop sends as its system message
    ...
    # Nunmai' credential never enters the JSON-RPC payload (#75186). openai/openai-codex keep codex's defaults.
    model_provider = None
    if str(getattr(agent, "provider", "") or "").strip().lower() == "custom":
        from nunmai_cli.runtime_provider_custom import codex_model_provider_id
