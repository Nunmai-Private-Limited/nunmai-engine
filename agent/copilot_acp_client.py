...
This adapter lets Nunmai treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape Nunmai expects from an OpenAI client.
"""
...
def _resolve_args() -> list[str]:
    raw = os.getenv("NUNMAI_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    NUNMAI_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set NUNMAI_HOME explicitly if they
...
def _format_messages_as_prompt(
    ...
    if model:
        sections.append(f"Nunmai requested model hint: {model}")

    # Copilot has no tools of its own that would collide with Nunmai', so it
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float) -> tuple[str, str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"NUNMAI_COPILOT_ACP_COMMAND / NUNMAI_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "Nunmai ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "directly with a Copilot subscription token) via `nunmai setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by Nunmai yet.",
            )
