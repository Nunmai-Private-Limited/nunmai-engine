"""Shared SKILL.md preprocessing helpers: ``${NUNMAI_*}`` template tokens and
...
# ${NUNMAI_SKILL_DIR} / ${NUNMAI_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(NUNMAI_SKILL_DIR|NUNMAI_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${NUNMAI_SKILL_DIR} / ${NUNMAI_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "NUNMAI_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "NUNMAI_SESSION_ID": str(session_id) if session_id else None,
    }
