...
# Matches ${NUNMAI_SKILL_DIR} / ${NUNMAI_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${NUNMAI_SESSION_ID} with no session) are
...
def substitute_template_vars(
    ...
    """Replace ${NUNMAI_SKILL_DIR} / ${NUNMAI_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "NUNMAI_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "NUNMAI_SESSION_ID" and session_id:
