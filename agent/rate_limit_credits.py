...
class RateLimitCreditsMixin:
    ...
    def _capture_nous_model_switch(self, http_response: Any) -> None:
        """Record the Nunmai gateway's ``x-nous-model-switch`` header (a named account asked for the
        free tier's model; the gateway served its backing model and named it). Applied between
        calls by ``nunmai_cli.anon_auth.apply_model_switch``. Fail-open."""
        headers = _response_headers(http_response)
        if not headers:
            return
        try:
            from nunmai_cli.anon_auth import note_model_switch
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        depletion-notice bug cannot vanish silently. NUNMAI_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("NUNMAI_DEV_CREDITS"))
        ...
        if dev:
            # NUNMAI_DEV_CREDITS streams each capture to agent.log (`nunmai logs -f`, grep 'credits ▸').
