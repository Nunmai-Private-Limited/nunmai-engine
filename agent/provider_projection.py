"""Fold an agent-as-provider's own activity back into Nunmai' turn state.

Most providers are models: they ask Nunmai to run a tool and Nunmai runs it, so
...
time Nunmai sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Nunmai would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Nunmai tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``nunmai_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``nunmai_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "nunmai_projected_messages", None)
    ...
    raw_iters = getattr(response, "nunmai_provider_tool_iterations", 0)
    try:
