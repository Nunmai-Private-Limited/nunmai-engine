...
work must never come back as pending ``tool_calls`` (Nunmai would re-run it) — but
...
``reasoning``. The client hands back ``nunmai_projected_messages`` (completed
assistant/tool rows) and ``nunmai_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    try:
        iterations = int(getattr(response, "nunmai_provider_tool_iterations", 0) or 0)
