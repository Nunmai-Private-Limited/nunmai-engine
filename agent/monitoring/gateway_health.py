...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "nunmai.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("nunmai.gateway.up", int(bool(gateway_running))),
        metric("nunmai.gateway.active_agents", active_agents),
        metric("nunmai.gateway.busy", int(bool(busy))),
        metric("nunmai.gateway.drainable", int(bool(drainable))),
        metric("nunmai.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("nunmai.gateway.state", 1, **{"nunmai.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"nunmai.platform": str(platform), "nunmai.platform.state": state}
        metrics.append(metric("nunmai.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("nunmai.platform.degraded", int(is_degraded), **pattrs, **{"nunmai.error_code": error_code}))
        if is_degraded:
