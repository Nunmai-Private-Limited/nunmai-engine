...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("nunmai.gateway.up", 1 if gateway_running else 0, base),
        _metric("nunmai.gateway.active_agents", active_agents, base),
        _metric("nunmai.gateway.busy", 1 if busy else 0, base),
        _metric("nunmai.gateway.drainable", 1 if drainable else 0, base),
        _metric("nunmai.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("nunmai.gateway.state", 1, base, **{"nunmai.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"nunmai.platform": str(platform), "nunmai.platform.state": state},
        ))
        metrics.append(_metric(
            "nunmai.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"nunmai.platform": str(platform), "nunmai.platform.state": state, "nunmai.error_code": error_code},
        ))
        if is_degraded:
