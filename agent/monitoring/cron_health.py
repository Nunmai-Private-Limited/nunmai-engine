...
def build_cron_health_snapshot() -> CronHealthSnapshot:
    metrics: list[GatewayMetric] = []
    for name, reader in (
        ("nunmai.cron.scheduler.heartbeat_age_seconds", get_ticker_heartbeat_age),
        ("nunmai.cron.scheduler.last_success_age_seconds", get_ticker_success_age),
    ):
        try:
            value = reader()
    ...
    try:
        metrics.append(
            GatewayMetric("nunmai.cron.jobs.running", len(get_running_job_ids()), {})
        )
    except Exception:
