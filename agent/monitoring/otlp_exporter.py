...
configured under ``monitoring.export.otlp``. Lets an operator stream Nunmai
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install nunmai-engine[otlp]``), imported lazily so the
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    for ev in batch:
        try:
            name = f"nunmai.{ev.get('event', 'event')}"
