...
_OBSERVABLE_METRIC_NAMES = (
    "nunmai.gateway.up", "nunmai.gateway.state", "nunmai.gateway.active_agents", "nunmai.gateway.busy",
    "nunmai.gateway.drainable", "nunmai.gateway.restart_requested", "nunmai.gateway.background_work",
    "nunmai.gateway.background_delegations", "nunmai.platform.up", "nunmai.platform.degraded",
    "nunmai.cron.scheduler.heartbeat_age_seconds", "nunmai.cron.scheduler.last_success_age_seconds",
    "nunmai.cron.scheduler.catch_up_occurrences", "nunmai.cron.jobs.enabled", "nunmai.cron.jobs.running",
    "nunmai.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="nunmai-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("nunmai.gateway.background_work", _read_background_work_count),
            ("nunmai.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("nunmai.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("nunmai.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'nunmai-engine[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="nunmai-gateway-health-export", daemon=True)
            thread.start()
