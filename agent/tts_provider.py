...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Nunmai with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.nunmai/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Nunmai
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nunmai tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``nunmai setup`` for
