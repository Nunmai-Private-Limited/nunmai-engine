...
# (Nunmai Portal → OpenAI → OpenRouter → …). OpenRouter/Nunmai run a quality-first
...
def _forced_provider_from_env() -> str | None:
    ...
    `NUNMAI_PET_IMAGE_PROVIDER=<name>` (e.g. `openrouter`) bypasses the normal
...
def generate(
    ...
    *aspect_ratio* picks the canvas: ``"square"`` for single-character base
    ...
    def _run(extra: dict) -> tuple[Path | None, str]:
        ...
        if refs:
            # Providers disagree on the ref kwarg name: our OpenRouter/Nunmai
