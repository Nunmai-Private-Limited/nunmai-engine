...
# OpenRouter/Nunmai run a quality-first model chain and may fall back depending on
...
_PROVIDER_LABELS = {"nous": "Nunmai Portal", "openrouter": "OpenRouter", "openai": "OpenAI", "openai-codex": "OpenAI (Codex)", "krea": "Krea"}
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    Preference: ``NUNMAI_PET_IMAGE_PROVIDER`` (QA override, unknown values ignored),
    ...
    forced = os.environ.get("NUNMAI_PET_IMAGE_PROVIDER", "").strip().lower()
    ...
    raise GenerationError(
        ...
        "Open `nunmai tools` → Image Generation and configure Nunmai Portal, "
...
def generate(
    ...
    *aspect_ratio* ``"landscape"`` (row strips) gives every frame horizontal room so
    ...
    # Providers disagree on the ref kwarg name: our OpenRouter/Nunmai backends read
