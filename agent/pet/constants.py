...
row taxonomy is inferred from the atlas shape so Nunmai can render both legacy
...
# rows (1536x1872), while older Nunmai/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Nunmai' activity state names. They are not always identical to the
...
# Legacy Nunmai/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Nunmai activity names -> accepted row-name aliases in descending
