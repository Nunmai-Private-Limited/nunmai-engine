...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in nunmai_state.py — kept local
# to avoid importing nunmai_state at module load time (its module-level
# DEFAULT_DB_PATH = get_nunmai_home() / "state.db" breaks tests that
# monkeypatch get_nunmai_home to return a str).
...
from nunmai_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.nunmai/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Nunmai tool use: "
    ...
    return (
        ...
        f"context, but Nunmai needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Nunmai config, set `model.ollama_num_ctx: 65536` "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `nunmai auth reset anthropic`. Until "
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    """Refresh Nunmai runtime credentials after a fresh paid-entitlement check."""
    try:
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Nunmai' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `nunmai fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from nunmai_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.nunmai/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see nunmai_state.classify_persistence_error).
    ...
    # all run inside Codex). Default Nunmai path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Nunmai internals.
        #
        # Nunmai invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Nunmai tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Nunmai Portal rate limit guard ──────────────────────
            # If another session already recorded that Nunmai is rate-
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Nunmai Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from nunmai_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from nunmai_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("NUNMAI_DUMP_REQUESTS"):
                ...
                from nunmai_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Nunmai/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Nunmai rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Nunmai.
                if agent.provider == "nous":
                    try:
                ...
                if (
                    ...
                    if _try_refresh_nous_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Nunmai paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Nunmai agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Nunmai 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "Nunmai model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: nunmai auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `nunmai doctor` for credential-chain diagnostics, or")
                    ...
                    from nunmai_constants import display_nunmai_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Nunmai-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: nunmai config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: nunmai config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and Nunmai' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from nunmai_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `nunmai model`."
                        )
                ...
                # ── Nunmai Portal: record rate limit & skip retries ─────
                # When Nunmai returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Nunmai Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Nunmai).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Nunmai model for minutes.  We use
                ...
                if (
                    ...
                    try:
                        ...
                        if _genuine_nous_rate_limit:
                            ...
                            logger.info(
                                "Nunmai 429 looks like upstream capacity "
                    ...
                    if _genuine_nous_rate_limit:
                        ...
                        # top-of-loop Nunmai guard handles fallback or
                ...
                # which Nunmai' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Nunmai' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`nunmai",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Nunmai may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See nunmai-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `nunmai auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `nunmai model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nunmai Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: nunmai portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nousresearch.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Nunmai Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Nunmai Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Nunmai catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: nunmai setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        nunmai fallback add   (interactive picker — same as `nunmai model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.nunmai/config.yaml` to extend the per-call "
                            f"timeout. (Nunmai's built-in floor is 600s for "
        ...
        try:
            ...
            # with the iterations Nunmai never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Nunmai mid-turn, resume logic still sees the
                    ...
                    from nunmai_state import classify_persistence_error
                ...
                # timeout (NUNMAI_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from nunmai_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("NUNMAI_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: nunmai --resume <session-id>"
                )
