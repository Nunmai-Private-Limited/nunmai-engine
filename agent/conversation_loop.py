...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in nunmai_state.py; kept local so importing
# nunmai_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Nunmai tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Nunmai "
        ...
        "again. A known-good starting point is 65,536 tokens. In Nunmai config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `nunmai auth reset anthropic`. Until that "
...
def _run_conversation_turn(
    ...
    # Adopt ~/.nunmai/.env credential/base-url edits made since the last turn — a
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    """Append a Nunmai-authored assistant boundary when a failed turn left ``user`` as the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('nunmai_constants', 'PARTIAL_STREAM_STUB_ID'),
