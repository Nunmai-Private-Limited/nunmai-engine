...
A pre-run snapshot of ``~/.nunmai/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.nunmai/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.nunmai/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_nunmai_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.nunmai/cron/jobs.json``)."""
    return get_nunmai_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.nunmai/skills/`` and prune old ones.
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.nunmai/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.nunmai/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `nunmai curator rollback --list` to see available snapshots)",
            None,
        )
