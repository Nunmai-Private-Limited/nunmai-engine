...
Text auto chain: main provider+model → OpenRouter → Nunmai Portal → custom endpoint →
native Anthropic → direct API-key providers → None. Vision auto chain: main
provider (if a supported vision backend) → OpenRouter → Nunmai → Anthropic → custom.
...
from nunmai_cli.config_providers import _canonical_api_mode
...
from nunmai_constants import OPENROUTER_BASE_URL, nunmai_home_key
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``NUNMAI_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from nunmai_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `nunmai update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Nunmai owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Nunmai sees one failure.
    # Nunmai owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Nunmai sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Nunmai control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="nunmai-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_NUNMAI_TOOL_CALLS)."""
    try:
...
# provider — mirrors nunmai_cli.auth._PROVIDER_ALIASES. Without this group an explicit
...
def _provider_alias_table() -> Dict[str, str]:
    """The same alias table the main provider path resolves against (nunmai_cli.auth).
    ...
    if _ALIAS_TABLE is None:
        ...
        with contextlib.suppress(Exception):
            from nunmai_cli.auth import _PROVIDER_ALIASES as _auth_table
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        from nunmai_cli.auth import resolve_api_key_provider_credentials
        from nunmai_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            # Nunmai is OAuth (resolver raises); anonymous reads return the full catalog.
            try:
                from nunmai_cli.models_pricing import _resolve_nous_pricing_credentials
                api_key, base_url = _resolve_nous_pricing_credentials()
            except Exception:
                logger.debug("No Nunmai credentials for catalog", exc_info=True)
        if not base_url:
        ...
        if base_url.endswith("/v1"):  # fetch_models_with_pricing appends /v1/models
            base_url = base_url[:-3]
        # Nunmai-only args must match the pickers' or the seeded cache loses sale chrome and
        # policy-catalog expiry.
        _nous_kwargs = {}
        if is_nous:
            from nunmai_cli.models_pricing import _NOUS_CATALOG_TTL_SECONDS
    ...
    if is_nous:
        ...
        try:
            from nunmai_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
            ids = restrict_to_nous_policy(ids, nous_policy_allowed_ids())
        except Exception:
            logger.debug("Nunmai policy filter unavailable", exc_info=True)
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "nous":
        try:
            ...
            logger.debug("Nunmai policy check unavailable", exc_info=True)
    return picked
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from nunmai_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``NUNMAI_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``NUNMAI_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("NUNMAI_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("NUNMAI_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "NunmaiAgent"}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://nunmai.in",
    "X-Title": "Nunmai Engine",
    "User-Agent": f"NunmaiAgent/{_NUNMAI_VERSION}",
}

# Nunmai Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks nunmai_cli.__version__ — never inline a literal here.
from agent.portal_tags import nous_portal_tags as _nous_portal_tags


def _nous_extra_body() -> dict:
    """Fresh Nunmai Portal ``extra_body`` (per call, so a hot-reloaded version is reflected)."""
    return {"tags": _nous_portal_tags()}


# Set at resolve time — True if the auxiliary client points to Nunmai Portal
...
_AUTH_JSON_PATH = get_nunmai_home() / "auth.json"
...
def _auth_json_path():
    ...
    would otherwise authenticate to Nunmai with the default profile's token."""
    from nunmai_cli.auth import _auth_file_path
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from nunmai_cli.auth import normalize_actual_base_url
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from nunmai_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("NUNMAI_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        wire_tools, wire_aliases = _alias_wire_tools(
            ...
            {"provider": getattr(self._client, "_nunmai_aux_effective_provider", "") or None, "base_url": host},
            is_xai,
        )
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Nunmai ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from nunmai_cli.providers import is_actual_route

        if is_actual_route(
            getattr(self._client, "_nunmai_aux_effective_provider", ""),
        ...
        try:
            ...
            # Undo only the aliases THIS request emitted, before the call reaches Nunmai dispatch.
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, is_oauth: bool = False, base_url: str | None = None):
        ...
        # Caller URL first; fall back to the SDK client's host only for Nunmai Portal — a blanket
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Nunmai plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nunmai_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring NUNMAI_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "NUNMAI_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_nous_auth() -> Optional[dict]:
    """Nunmai provider state dict from the credential pool or ~/.nunmai/auth.json; None when not active with tokens."""
    ...
    try:
        ...
        logger.debug("Could not read Nunmai auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract a usable Nunmai inference JWT from stored auth state."""
    from nunmai_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Nunmai auxiliary credentials from the selected pool entry."""
    try:
        from nunmai_cli.auth import _agent_key_is_usable
        pool = load_pool("nous")
    except Exception as exc:
        logger.debug("Auxiliary Nunmai pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Nunmai pool selection failed: %s", exc)
        return None
    if entry is None:
        return None
    ...
    if force_refresh or not _agent_key_is_usable(_entry_state(entry), _nous_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary Nunmai pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    """Fresh Nunmai runtime credentials (pool first, then auth store + JWT refresh) — mirrors the main
    ...
    if pooled is not None:
        return pooled
    try:
        from nunmai_cli.auth import resolve_nous_runtime_credentials
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("NUNMAI_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from nunmai_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("NUNMAI_XAI_BASE_URL"))
    ...
    try:
        from nunmai_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from nunmai_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from nunmai_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from nunmai_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from nunmai_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from nunmai_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import nunmai_xai_default_headers
        headers = dict(nunmai_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from nunmai_cli.config import cfg_get, load_config_readonly
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nous:
        logger.warning("Auxiliary Nunmai client unavailable: no Nunmai authentication found (run: nunmai auth).")
        _mark_provider_unhealthy("nous", ttl=60, reason="no Nunmai authentication found", level=logging.DEBUG)
        return None, None
    if runtime is None and nous:
        logger.debug("Auxiliary Nunmai: runtime JWT refresh failed; checking stored auth.json token.")
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary Nunmai client unavailable: no usable inference JWT found "
                "(run: nunmai auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60, reason="no usable Nunmai inference JWT", level=logging.DEBUG)
            return None, None
        base_url = str(
    ...
    with contextlib.suppress(Exception):
        ...
        from nunmai_cli.anon_auth import is_anonymous_request
        anonymous = is_anonymous_request("nous", api_key)
        remaining = nous_rate_limit_remaining(anonymous=anonymous)
        if remaining is not None and remaining > 0:
            logger.debug("Auxiliary: skipping Nunmai Portal (rate-limited, resets in %.0fs)", remaining)
            ...
            _mark_provider_unhealthy(
                ...
                reason="Nunmai Portal rate-limited", level=logging.INFO)
            return None, None
    ...
    from nunmai_cli.anon_auth import GUEST_MODEL, route_is_welcome_host
    global auxiliary_is_nous
    if route_is_welcome_host(base_url):
        auxiliary_is_nous = True
        logger.debug("Auxiliary/%s: Nunmai free tier; using %s", lane, GUEST_MODEL)
        return _create_openai_client(api_key=api_key, base_url=base_url), GUEST_MODEL
    auxiliary_is_nous = True
    logger.debug("Auxiliary client: Nunmai Portal")
    ...
    if not _aux_probe_active():
        try:
            from nunmai_cli.models import get_nous_recommended_aux_model
...
def _refresh_nous_recommended_model(*, vision: bool, stale_model: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("Nunmai recommended-model refresh failed (%s); using default %s", exc, _NOUS_MODEL)
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from nunmai_cli import config as _cfg_mod
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `nunmai setup` or `nunmai model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=nunmai_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model


def _codex_base_url_override() -> str:
    """Profile-scoped ``NUNMAI_CODEX_BASE_URL`` (same read as the API-key env vars: under a
    multiplexer the routed profile's .env decides the endpoint, never a sibling's process env)."""
    return _scoped_key_env("NUNMAI_CODEX_BASE_URL").rstrip("/")
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from nunmai_cli.runtime_provider import _resolve_azure_foundry_runtime
        from nunmai_cli.auth import AuthError
        from nunmai_cli.config import load_config_readonly
    except ImportError:
        return None, None
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Nunmai account API says paid access is allowed."""
    try:
        from nunmai_cli.nous_account import get_nous_portal_account_info
        return get_nous_portal_account_info(force_fresh=True).paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Nunmai paid-entitlement refresh check failed: %s", exc)
        return False
...
def _evict_cached_clients(provider: str) -> None:
    ...
    Scoped to the calling profile (``nunmai_home_key()`` is the first key slot): a rotation in
...
def _refresh_copilot_credentials() -> bool:
    from nunmai_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from nunmai_cli.auth import resolve_codex_runtime_credentials
...
def _refresh_nous_credentials() -> bool:
    ...
    return _creds_have_api_key(resolve_nous_runtime_credentials(
        timeout_seconds=env_float("NUNMAI_NOUS_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_nunmai_fallback_destination", None)
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._nunmai_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.nunmai/.env value after `nunmai model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: nunmai model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.nunmai/.env",
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    compression silently charged to a Nunmai Portal balance)."""
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`nunmai model`) "
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → Nunmai → custom → Codex → API-key providers, only with no policy
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_nunmai_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "NUNMAI_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Nunmai owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._nunmai_aux_effective_provider = "actual"
        return client
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_nunmai_aux_effective_provider", effective_provider)
...
def _resolve_nous_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if client is None:
        logger.warning("resolve_provider_client: nous requested but Nunmai Portal not configured (run: nunmai auth)")
        return None, None
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: nunmai model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: nunmai model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    if not custom_base:
        ...
        return None, None
    from nunmai_cli.config import normalize_extra_headers
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: nunmai doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from nunmai_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("nunmai_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider) and Nunmai + vision (the
    ...
    if not model and provider != "auto" and not (provider == "nous" and is_vision):
        ...
        # not explicitly request one. (# compression-current-model) Nunmai + vision is the one carve-out: the
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → OpenRouter → Nunmai → DeepInfra).
...
def _vision_main_provider_client(
    ...
    if main_provider == "nous":
        # Nunmai picks its vision model from Portal tier-aware slots inside _try_nous(vision=True);
...
def _vision_auto_route(
    ...
    """Auto-detect order: 1. main provider + model, 2. OpenRouter, 3. Nunmai Portal, 4. DeepInfra, 5. stop."""
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs (Nunmai Portal product tags when Nunmai-backed, else {})."""
...
def _client_cache_key(
    ...
    # Profile home leads the key: callers that omit api_key (pool / Nunmai auth.json paths) would
    # otherwise share one client across multiplex profiles holding different credentials.
    return (nunmai_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _refresh_nous_auxiliary_client(
    *, cache_provider: str, model: Optional[str], async_mode: bool, base_url: Optional[str] = None,
    ...
    """Refresh Nunmai runtime creds, rebuild the client, and replace the cache entry.
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    try:
        from nunmai_cli.providers import get_provider
...
def _resolve_task_provider_model(
    ...
    from nunmai_cli.runtime_provider_custom import expand_direct_api_alias
    if provider:
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    if limit is None:
        return None
    from nunmai_constants import nunmai_home_key
    return _cached_semaphore(_aux_sync_semaphores, (nunmai_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from nunmai_constants import nunmai_home_key
    return _cached_semaphore(_aux_async_semaphores, (nunmai_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
def _nous_on_messages_wire(provider_norm: str, model: str) -> bool:
    """True when a Nunmai Portal route serves ``model`` over /v1/messages (dual-wire catalog)."""
...
def _merge_aux_extra_body(
    ...
    """Caller extra_body + profile body/reasoning + generic reasoning fallback + Nunmai tags."""
    ...
    # _get_task_extra_body) takes the same wire clamp: Nunmai-only ``ultra`` never reaches the
...
def _build_call_kwargs(
    ...
    # ``extra_body.reasoning`` fallback. Clamp Nunmai-internal levels (``ultra``) to the
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from nunmai_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Nunmai manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
    ...
    if client is None:
        raise AuxiliaryClientUnavailable(f"No LLM provider configured for task={task} "
                                         f"provider={resolved_provider}. Run: nunmai setup")
...
def _parameter_rungs(client: Any, max_tokens: Optional[int]) -> tuple:
    ...
    return (
        ...
        # The endpoint refuses the *disable* rather than the field (Nunmai Portal gpt-6-astra: "Reasoning is
...
def _refreshed_nous_step(route: _LadderRoute, kwargs: Dict[str, Any], message: str) -> Optional[_LadderStep]:
    """Rebuild the Nunmai client after a credential event; None when nothing refreshed."""
...
def _ladder_nous_rungs(
    ...
    """Nunmai-only rungs: stale-model self-heal, paid-account refresh, 401 refresh.
    ...
    if _is_model_not_found_error(first_err) and client_is_nous:
        ...
        if healed_model and healed_model != kwargs.get("model"):
            logger.warning("Auxiliary %s%s: model %r no longer in Nunmai catalog; "
    ...
    if _is_payment_error(first_err) and client_is_nous and _nous_portal_account_has_fresh_paid_access():
        step = _refreshed_nous_step(
            route, kwargs,
            "Auxiliary %s%s: refreshed Nunmai runtime credentials after paid account check, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
    ...
    if _is_auth_error(first_err) and client_is_nous:
        step = _refreshed_nous_step(
            route, kwargs, "Auxiliary %s%s: refreshed Nunmai runtime credentials after 401, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
                step, lambda exc: _credential_rung_accepts(exc) or _is_connection_error(exc))
            if first_err is None:
...
def _aux_recovery_ladder(
    ...
    strips → Nunmai heal/refresh → credential refresh/pool rotation → provider fallback.
