...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from nunmai_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    try:
        ...
        logger.debug("Auxiliary Nunmai runtime credential resolution failed: %s", exc)
        return None
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(os.getenv("NUNMAI_XAI_BASE_URL", ""))
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Cross-session rate guard: another session's 429 means skip Nunmai rather than pile onto the tapped RPH bucket.
    with contextlib.suppress(Exception):
        from agent.nous_rate_guard import nous_rate_limit_remaining
        _remaining = nous_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug("Auxiliary: skipping Nunmai Portal (rate-limited, resets in %.0fs)", _remaining)
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from nunmai_cli.models import opencode_zen_free_runtime as _oc_free_rt
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `nunmai model`.")
