...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.nunmai/auth.json active provider)
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.nunmai/auth.json for an active Nous provider.
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: nunmai auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
