...
Resolution order for text tasks (auto mode):
  ...
  3. Nunmai Portal (~/.nunmai/auth.json active provider)
...
from nunmai_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``NUNMAI_CA_BUNDLE`` /
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Nunmai owns auxiliary retry + provider/model fallback policy (the
    ...
    # Nunmai sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Nunmai control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="nunmai-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Nunmai triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once Nunmai dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Nunmai
...
# Nunmai catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from nunmai_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Nunmai returns its compaction pick), so it
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``NUNMAI_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``NUNMAI_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "NunmaiAgent",
}
...
# Nunmai Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Nunmai Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with nunmai_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh Nunmai Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``nunmai_cli.__version__`` is
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send Nunmai' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Nunmai Engine)",
    ...
    if _is_official_codex_base_url(base_url):
        ...
        headers.update({
            "User-Agent": f"NunmaiAgent/{__version__}",
            "originator": "nunmai-engine",
        })
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Nunmai-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Nunmai Portal — a blanket fallback would flip
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Nunmai plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nunmai_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.nunmai/auth.json for an active Nunmai provider.

    Returns the provider state dict if Nunmai is active with tokens,
    otherwise None.
    """
...
def _nous_base_url() -> str:
    """Resolve the Nunmai inference base URL from env or default."""
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Nunmai runtime credentials when available.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``nunmai auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``nunmai_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Nunmai auth store.
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Nunmai —
    # if another session already recorded a 429, skip Nunmai entirely
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Nunmai Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not nous:
        logger.warning(
            "Auxiliary Nunmai client unavailable: no Nunmai authentication found "
            "(run: nunmai auth)."
        )
    ...
    if runtime is None and nous:
        logger.debug(
            "Auxiliary Nunmai: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    ...
    if not _aux_probe_active():
        ...
        # model is irrelevant to "is Nunmai resolvable?", and the Portal
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Nunmai Portal's recommended model after a stale-model 404.
    ...
    the Nunmai → OpenRouter catalog, every auxiliary call 404s with
    ...
    try:
        ...
        logger.debug(
            "Nunmai recommended-model refresh failed (%s); using default %s",
            exc, _NOUS_MODEL,
        )
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=nunmai_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`nunmai_cli.runtime_provider._resolve_azure_foundry_runtime` —
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second nunmai
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``nunmai config aux reset``)."""
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Nunmai → OpenRouter catalog. The Nunmai
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Nunmai routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``nunmai_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._nunmai_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Nunmai' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Nunmai get their chosen chat model; users
      ...
      2. OpenRouter → Nunmai → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `nunmai model` but the
    #    old OPENAI_BASE_URL lingers in ~/.nunmai/.env. ──
    ...
    # on aggregators (OpenRouter, Nunmai) who previously got routed to a
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_nunmai_aux_effective_provider", provider)
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = nunmai_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Nunmai owns
...
def resolve_provider_client(
    ...
    #
    # Nunmai + vision is the one carve-out: the branch below resolves its model
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nunmai Portal not configured (run: nunmai auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: nunmai model)")
                return None, None
    ...
    # OpenRouter / Nunmai bills for side tasks they thought were running on
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: nunmai doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(nunmai_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Nunmai → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Nunmai — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Nunmai is the
        ...
        #   3. Nunmai Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "nous":
                # Nunmai resolves its vision model from the Portal's tier-aware
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Nunmai Portal product tags when the auxiliary client is backed
    by Nunmai Portal. Returns empty dict otherwise.
    """
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`nunmai_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Nunmai will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Nunmai use
    ...
    # Anthropic Messages adapters translate Nunmai reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Nunmai reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `nunmai model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Nunmai Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Nunmai → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Nunmai-routed calls.
        _heal_is_nous = (
        ...
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Nunmai catalog; "
        ...
        # ── Nunmai auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "inference-api.nousresearch.com")
        )
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Nunmai runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Nunmai runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Nunmai
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the Nunmai → OpenRouter catalog, 404'ing every auxiliary call. Force a
        ...
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Nunmai catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Nunmai runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Nunmai runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
