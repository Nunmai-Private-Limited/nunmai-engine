...
# `export NUNMAI_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in nunmai_cli/main.py, gateway/run.py, and
# cli.py) or `NUNMAI_REDACT_SECRETS=false` in ~/.nunmai/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Nunmai — used by
    ``nunmai config``, ``nunmai status``, ``nunmai dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical nunmai log line
...
# (``security.redact_secrets: false`` / NUNMAI_REDACT_SECRETS) applies to
