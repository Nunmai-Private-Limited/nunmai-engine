...
# `export NUNMAI_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in nunmai_cli/main.py, gateway/run.py, and cli.py) or `NUNMAI_REDACT_SECRETS=false` in
# ~/.nunmai/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("NUNMAI_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a NUNMAI_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in nunmai_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("NUNMAI_REDACT_SECRETS") if scope else None
        if raw is None and scope is None:
            ...
            raw = load_env().get("NUNMAI_REDACT_SECRETS")
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``nunmai config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Nunmai' own ``config.yaml`` where ``nunmai mcp add --env`` writes
...
_NUNMAI_HOME_PREFIXES = ("$NUNMAI_HOME/", "${NUNMAI_HOME}/")
# ``$HOME/.nunmai/config.yaml`` keeps the ``.nunmai`` segment, so stripping the prefix is
...
def _is_under_nunmai_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active Nunmai home or root.

    The default home's basename is an installation detail — ``.nunmai`` on POSIX, ``nunmai``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$NUNMAI_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.nunmai`` directory, ``$NUNMAI_HOME``, or the resolved Nunmai home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\nunmai`` and carries no ``.nunmai`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    nunmai_home = False
    for prefix in _NUNMAI_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies Nunmai
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return nunmai_home or ".nunmai" in parts[:-1] or _is_under_nunmai_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Nunmai ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
