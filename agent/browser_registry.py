...
2. Legacy preference order — ``browser-use`` → ``browserbase`` — filtered by
   ...
   because it covers both the managed Nunmai gateway and direct API key path;
...
def list_providers(*, scope: Optional[str] = None) -> List[BrowserProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or nunmai_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[BrowserProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or nunmai_home_key(), {}).get(key) or _providers.get(key)
...
def registry_generation(*, scope: Optional[str] = None) -> tuple[int, int]:
    ...
    active_scope = scope or nunmai_home_key()
    with _lock:
...
def _resolve(configured: Optional[str]) -> Optional[BrowserProvider]:
    ...
    plugins added under ``~/.nunmai/plugins/browser/<vendor>/`` are subject
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(nunmai_home_key(), {}))
