...
See the ``nunmai-engine-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Nunmai through a client
    shim, and a shim that cannot carry Nunmai tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_NUNMAI_TOOL_CALLS = False``
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • PINNED skills (marked via 'nunmai curator pin'). You are an "
    ...
    "'nunmai curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'nunmai skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'nunmai curator pin'). Pin blocks "
    ...
    "reply and recommend 'nunmai curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Nunmai tool calls back would
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # scratch (fresh _nunmai_now() timestamp, fresh
