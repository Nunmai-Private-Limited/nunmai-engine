...
from nunmai_cli.auth_plugin_providers import plugin_refresh_hook
...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``nunmai_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
        from nunmai_cli.auth import is_source_suppressed
...
# singleton to re-seed from and the user can re-add via ``nunmai auth add``.
...
# selection. On Windows several Nunmai processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Nunmai backend after 15000ms"). Logging the condition at
...
_EXTRA_KEYS = frozenset({
    ...
    # Nunmai guest identity (``auth_method: anonymous``): the anon_ credential is the refresh material.
...
# Nunmai singleton metadata mirrored between auth.json state and ``entry.extra``.
...
class PooledCredential:
    ...
    # Epoch of the last deliberate ``nunmai auth reset`` of this entry. Sticky: a later exhaustion
    ...
    def runtime_base_url(self) -> Optional[str]:
        ...
        if self.provider == "openai-codex":
            ...
            return get_secret_str("NUNMAI_CODEX_BASE_URL", "").strip().rstrip("/") or self.base_url
...
def _codex_principal_identity(access_token: Any) -> Optional[Tuple[str, str]]:
    ...
    Decoded without signature verification: this only decides whether two credentials Nunmai
...
def _codex_entry_tracks_singleton(entry: PooledCredential, singleton_tokens: Dict[str, Any]) -> bool:
    ...
    ``nunmai auth add openai-codex`` (must never be overwritten — adopting turned two logins into
...
def custom_provider_pool_key_candidates(
    ...
    ``nunmai auth add <key>`` stores ``providers.<key>`` credentials under the
...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.nunmai/auth.json`` even when NUNMAI_HOME
...
def _write_through_provider_state_to_global_root(
    ...
    ``nunmai_cli.auth._write_through_xai_oauth_to_global_root``.
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    """Root ``.anthropic_oauth.json`` when *entry* is a borrowed nunmai_pkce row, else None."""
    if entry.source != "nunmai_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
        from agent.anthropic_credentials import _root_nunmai_oauth_file
        return _root_nunmai_oauth_file()
    except Exception:
        return None
...
# ``nunmai_cli.auth`` refresh function and terminal-error predicate (looked
...
# ``nunmai_cli.auth_plugin_providers.is_refreshable_oauth_provider``); any other provider is returned
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "NUNMAI_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "NUNMAI_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool(CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``nunmai model`` / ``nunmai auth`` login writes new tokens
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nunmai device_code entry from auth.json ``providers.nous`` if state differs.
        ...
        "refresh token reuse" revocation on the Nunmai Portal.
        """
        ...
        try:
            ...
            if not any(v not in (None, "") and getattr(entry, k, None) != v for k, v in comparable.items()):
                return entry
            logger.debug("Pool entry %s: syncing Nunmai state from auth.json", entry.id)
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        token. Applies to Nunmai, OpenAI Codex and xAI OAuth singletons.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Nunmai processes, or two processes adopt the same on-disk
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        For ``claude_code`` / ``nunmai_pkce`` the singleton file — not
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "NUNMAI_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). nunmai_pkce -> ~/.nunmai/.anthropic_oauth.json
        ...
        ``endswith``: manual:nunmai_pkce is pool-owned and a singleton for it
        would be a second authority for the same refresh-token family.
        """
        if entry.source == "claude_code":
            store = "~/.claude/.credentials.json"
        elif entry.source == "nunmai_pkce":
            store = "~/.nunmai/.anthropic_oauth.json"
        else:
            return
        try:
            ...
            if entry.source == "claude_code":
                ...
                ac._write_nunmai_oauth_credentials(*args, target=_singleton_target_for_entry(self, entry))
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("nunmai_pkce"))
        updated = replace(
            entry,
            ...
            if self.provider == "anthropic":
                ...
                if synced is not entry:
                    ...
                    if force and entry.runtime_api_key and entry.runtime_api_key != stale_key:
                        logger.debug("Nunmai entry %s: adopting peer-rotated token, skipping refresh", entry.id)
                        return entry
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            if entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("nunmai_pkce"),
                        )
                ...
                # Backstop for pool-owned sources (nunmai_pkce, manual:dashboard_pkce):
            ...
            if is_terminal_anthropic_refresh_error(exc):
                ...
                # CLI's credentials file here — only Nunmai' own row goes DEAD.
                logger.warning(
                    "Anthropic OAuth refresh token for %s is terminally invalid (%s); the credential "
                    "leaves rotation. Re-run 'nunmai auth add anthropic' to sign in again.",
            ...
            # entries from the pool (mirrors the Nunmai quarantine path).
            if getattr(auth_mod, terminal_fn_name)(exc):
                # WARNING, not debug: this is the moment a login is lost. At the default log level a
                # silent quarantine looked like "I logged in once and Nunmai keeps failing" (#113023).
                logger.warning(
                    "%s OAuth refresh token is terminally invalid (%s); clearing local token state. "
                    "Re-run 'nunmai auth add %s' to sign in again.", display, exc, self.provider)
            ...
            if isinstance(exc, TimeoutError):
                ...
                logger.debug("Nunmai refresh skipped: auth store lock busy; not benching entry")
                return entry
            if auth_mod._is_terminal_nous_refresh_error(exc):
                logger.warning(
                    "Nunmai refresh token is terminally invalid (%s); clearing local token state. "
                    "Re-run 'nunmai auth add nous' to sign in again.", exc)
    ...
    def _mark_dead_refresh_grant(self, entry: PooledCredential, exc: Exception) -> None:
        ...
        (``nunmai auth add``) survives, and an unmarked survivor re-enters rotation and re-fires the
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        # Nunmai refresh can require network access and happens when runtime
    ...
    def _reset_cleared_after(self, entry: PooledCredential) -> Optional[float]:
        """Epoch of a ``nunmai auth reset`` persisted by another process AFTER *entry*'s status, else None."""
        try:
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        The user may have re-authed (``nunmai model`` / ``nunmai auth``, the
        Claude Code CLI, another profile) leaving fresh tokens on disk while
        the pool entry is frozen behind ``last_error_reset_at``. A ``nunmai auth
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `nunmai auth add %s`",
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``nunmai auth remove`` must stay stable across loads)."""
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    # Only auto-discover external credentials (Claude Code, Nunmai PKCE) when
    ...
    # API-key vs OAuth is a user-visible choice at `nunmai setup`. The API-key
    ...
    # ~/.nunmai/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"nunmai_pkce", "claude_code"})
        return
    ...
    sources = [("nunmai_pkce", read_nunmai_oauth_credentials())]
    if adopt_external_logins_enabled():
        ...
        # earlier (adopting) process persisted, or it keeps rotating a login Nunmai no longer reads.
...
def _seed_nous_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    if not (state and has_runtime_material):
        return
    # Prefer a user-supplied label embedded in the singleton state (``nunmai
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from nunmai_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Nunmai owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `nunmai auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        # `nunmai auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.nunmai/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "nunmai auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _env_payload(*, env_var: str, token: str, base_url: str) -> Dict[str, Any]:
    ...
    try:
        from nunmai_cli.env_loader import get_secret_source
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `nunmai auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons and Nunmai PKCE disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "nunmai_pkce"
