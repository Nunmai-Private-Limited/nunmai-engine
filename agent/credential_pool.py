...
# un-throttled log fires on *every* selection. On Windows several Nunmai
...
# readiness handshake ("Timed out connecting to Nunmai backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``nunmai_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``nunmai model`` / ``nunmai auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Nunmai process
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Nunmai
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``NUNMAI_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "NUNMAI_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "NUNMAI_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("nunmai_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Nunmai
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("nunmai_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Nunmai process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Nunmai profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `nunmai model` / `nunmai auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `nunmai model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``nunmai auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `nunmai auth remove <provider> <N>` is stable across all source types.
    try:
        from nunmai_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Nunmai PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `nunmai setup` ("Claude
        ...
        # Claude Code CLI, nunmai_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.nunmai/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `nunmai setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"nunmai_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_nunmai_oauth_credentials
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `nunmai auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (nunmai auth remove copilot gh_cli) must not pay either on
        ...
        # Nunmai auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.nunmai/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `nunmai auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `nunmai auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Nunmai auth store.  Without this gate the removal is instantly
        ...
        # Nunmai owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``nunmai model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``nunmai auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from nunmai_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.nunmai/.env over os.environ — the user's config file is the
# authoritative source for Nunmai credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_nunmai_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `nunmai auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.nunmai/.env.
    ...
    try:
        from nunmai_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.nunmai/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `nunmai auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Nunmai
