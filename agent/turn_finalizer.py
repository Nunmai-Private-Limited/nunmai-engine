...
def _resolve_budget_fallback(
    ...
    # inherits ``NUNMAI_KANBAN_TASK`` via os.environ but exhausting ITS budget must not
...
def finalize_turn(
    ...
    result = {
        ...
        "model": agent.model,
        # requested_model / served_model: proxy-reported deployment or Nunmai' own fallback route.
        ...
        **{key: getattr(agent, f"session_{key}") for key in _SESSION_COST_KEYS},
        # Requested service tier, for billing audits (`nunmai -z --usage-file`).
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        ...
        result["error"] = final_response or (
            ...
            f"health (`nunmai {profile_cli_selector()}doctor`), then send your message again"
        )
