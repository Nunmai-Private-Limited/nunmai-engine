...
(``gateway/platforms/webhook.py``): inbound wakes Nunmai when the world
changes; outbound tells the world when Nunmai does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Nunmai-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``NUNMAI_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.nunmai/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/hermes-events
          ...
          secret_env: NUNMAI_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `nunmai hooks list`
...
Headers::
    ...
    User-Agent:              Nunmai-Agent-Outbound-Webhook
    X-Nunmai-Event:          <hook event name>
    X-Nunmai-Delivery:       <delivery_id>
    X-Nunmai-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("NUNMAI_SAFE_MODE"):
        logger.info("NUNMAI_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Nunmai-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Nunmai-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
