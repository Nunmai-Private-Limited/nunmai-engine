...
(``X-Nunmai-Signature-256: sha256=<hex>`` over the raw body) when a secret is configured;
``NUNMAI_SAFE_MODE=1`` skips registration; registration is idempotent.
"""
...
def iter_configured_targets(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    """Parse ``hooks.outbound`` without registering anything (``nunmai hooks list``)."""
    if not isinstance(cfg, dict):
        return []
...
def _serialize_payload(event: str, kwargs: Dict[str, Any], delivery_id: str) -> bytes:
    ...
    (also the ``X-Nunmai-Delivery`` header) and ``timestamp`` live inside the HMAC-signed
...
def _build_delivery(event: str, target: WebhookTarget, body: bytes, delivery_id: str) -> Dict[str, Any]:
    headers = {
        "Content-Type": "application/json", "User-Agent": "Nunmai-Agent-Outbound-Webhook",
        "X-Nunmai-Event": event, "X-Nunmai-Delivery": delivery_id,
    }
    if target.secret:
