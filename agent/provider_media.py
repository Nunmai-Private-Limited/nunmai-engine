"""``$NUNMAI_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$NUNMAI_HOME/cache/<kind>/``, creating parents as needed."""
    from nunmai_constants import get_nunmai_home
    path = get_nunmai_home() / "cache" / kind
