#!/usr/bin/env python3
"""Google Workspace API CLI for Nunmai Engine.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Nunmai-facing JSON contract and falls back to the Python client
...
from _nunmai_home import get_nunmai_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NUNMAI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Nunmai Engine")
