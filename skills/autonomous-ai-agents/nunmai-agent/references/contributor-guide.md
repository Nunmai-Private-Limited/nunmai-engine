...
For occasional contributors and PR authors. Full developer docs: https://nunmai.in/docs/developer-guide/
...
Config: `~/.nunmai/config.yaml` (settings), `~/.nunmai/.env` (API keys) — both under `$NUNMAI_HOME` when it is set.
...
`_NUNMAI_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_nunmai_home()` for paths,
never hardcode `~/.nunmai`. For custom/local-only tools, write a plugin in
`~/.nunmai/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `nunmai_cli/commands.py`
...
- Tests auto-redirect `NUNMAI_HOME` to temp dirs — never touch real `~/.nunmai/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/nunmai_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_nunmai_home()` from `nunmai_constants` for all paths (profile-safe)
