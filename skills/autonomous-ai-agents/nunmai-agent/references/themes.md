# Themes / Skins — Author a Nunmai Color Theme

Author a Nunmai **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`nunmai_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.nunmai/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Nunmai itself.
...
- Write access to the Nunmai home dir — `~/.nunmai` by default, or `$NUNMAI_HOME`
  / the active profile's dir. Skins live in `<nunmai-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `nunmai config set`).
...
3. `write_file` it to `<nunmai-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `nunmai config set display.skin default` (or they can `/skin default`).
...
```
nunmai skin set <key> <hex>      # e.g. nunmai skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `nunmai skin set background "#08201f"` changes
only the background; `nunmai skin use <name>` / `nunmai skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.nunmai`** when a profile is active — resolve the real home
  from `$NUNMAI_HOME` first, falling back to `~/.nunmai`.
...
- **Never hand-edit `config.yaml` to activate.** Use `nunmai config set
...
- **You apply it, not the user.** `nunmai config set display.skin <name>` is
...
- `read_file` the written `<nunmai-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `nunmai config get display.skin` and confirm it reports `<name>`.
