...
Edit with `nunmai config edit` or `nunmai config set section.key value`.
Full reference: https://nunmai.in/docs/user-guide/configuration
...
`nunmai config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `nunmai tools` (interactive) or `nunmai tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_NUNMAI_CORE_TOOLS` is the default bundle most platforms inherit).
