# Nunmai Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Nunmai Portal subscription without me
...
## Layer 1 — Is this thing a Nunmai plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Nunmai process**. Its LLM calls go through Nunmai's already-configured provider. | Already uses Portal if user's Nunmai is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Nunmai. That LLM call is already authenticated
via Nunmai's provider config. The plugin's own server auth (e.g.
...
"Sign in with Nunmai Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Nunmai Engine has that *feels* like OAuth — `nunmai login --provider nous`
opening a browser, user signs in, token lands in `~/.nunmai/auth.json` — is a
**Nunmai-specific browser flow**. Under the hood it produces a credential
Nunmai uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Nunmai's existing Portal credential out of `~/.nunmai/auth.json`.
...
rides on the credential Nunmai already holds.

Where this could live in Nunmai:
...
Only when the consumer is *another first-party Nunmai thing* (a future SDK, a
Nunmai-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Nunmai itself.
...
1. First decide: Nunmai plugin (runs inside Nunmai) or separate app? If plugin,
   it already uses Portal via Nunmai's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Nunmai agent itself") is fully supported. Nunmai treats agent
...
pitch in the Tool Gateway announcement is specifically about Nunmai Engine's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Nunmai is configured to use Portal as its provider. It is
