...
Write plugins for the Nunmai desktop app: statusbar items, layout panes,
...
system (`~/.nunmai/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$NUNMAI_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$NUNMAI_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.nunmai-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Nunmai desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$NUNMAI_HOME/desktop-plugins/` (usually
  `~/.nunmai/desktop-plugins/`).

## How to Run

1. Create `$NUNMAI_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.nunmai/...` by default, or `~/.nunmai/profiles/<profile>/...` under a
...
The ONLY import surface is `@nunmai/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Nunmai
  ...
  (`nunmai://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.nunmai/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@nunmai/plugin-sdk`, `react`, and
