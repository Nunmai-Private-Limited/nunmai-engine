...
Nunmai runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`nunmai config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Nunmai's env scrubber dropping
...
Nunmai-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Nunmai tool and most Windows APIs. Prefer forward slashes in code
