...
Set via `nunmai model` (picker) or `nunmai setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://nunmai.in/docs/integrations/providers
...
| nous | OAuth device code | `nunmai auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `nunmai auth add openai-codex` |
| qwen-oauth | OAuth | `nunmai auth add qwen-oauth` |
| minimax-oauth | OAuth | `nunmai auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`nunmai auth`).
Fallback chain when the primary fails: `nunmai fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`nunmai_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   nunmai config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
