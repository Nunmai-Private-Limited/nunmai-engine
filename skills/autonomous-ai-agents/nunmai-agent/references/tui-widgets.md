...
Author widget apps for the Nunmai TUI (`nunmai --tui`): glanceable ambient
...
- The TUI must be in use (`nunmai --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.nunmai/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.nunmai/logs/tui_gateway_crash.log` if a widget never appears.
