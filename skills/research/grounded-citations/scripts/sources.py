...
Ledger path resolution (first wins):
  --ledger PATH
  $NUNMAI_CITATION_LEDGER
  $NUNMAI_HOME/cache/citations/ledger.json
"""
...
from _nunmai_home import get_nunmai_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("NUNMAI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_nunmai_home() / "cache" / "citations" / "ledger.json"
