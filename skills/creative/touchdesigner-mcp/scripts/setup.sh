...
NUNMAI_HOME_DIR="${NUNMAI_HOME:-$HOME/.nunmai}"
NUNMAI_CFG="${NUNMAI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nunmai config has twozero_td MCP entry ──
if [[ ! -f "$NUNMAI_CFG" ]]; then
    echo -e " ${FAIL} Nunmai config not found at ${NUNMAI_CFG}"
    manual_steps+=("Create ${NUNMAI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NUNMAI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nunmai config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nunmai config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NUNMAI_CFG} manually"); }
    manual_steps+=("Restart Nunmai session to pick up config change")
fi
