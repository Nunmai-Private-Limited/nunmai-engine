...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `nunmai skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.nunmai.tags`, `metadata.nunmai.category`,
`metadata.nunmai.related_skills`, `metadata.nunmai.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.nunmai.*` by the loader.
...
2. **Prose references native Nunmai tools or the MCP servers the skill expects, in backticks**
...
4. **`author` credits the human first.** External contributor's real name + GitHub handle first,
   "Nunmai Engine" second. A commit authored as "Nunmai Engine" (they drafted with Nunmai) is replaced
...
in the `nunmai-engine-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.nunmai/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `nunmai_cli/curator.py` → `nunmai curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.nunmai/skills/.usage.json`
