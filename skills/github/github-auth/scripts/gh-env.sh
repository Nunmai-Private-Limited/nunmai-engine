#!/usr/bin/env bash
# GitHub environment detection helper for Nunmai Engine skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _nunmai_env="${NUNMAI_HOME:-$HOME/.nunmai}/.env"; [ -f "$_nunmai_env" ] && grep -q "^GITHUB_TOKEN=" "$_nunmai_env" 2>/dev/null; then
