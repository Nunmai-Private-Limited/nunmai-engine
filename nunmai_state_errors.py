...
Shared by nunmai_state and its mixins; string predicates match wrapped RPC
...
STORAGE_RECOVERY_DOCS_URL = "https://nunmai.in/docs/user-guide/session-storage-recovery"
...
_DELETED_WAL_GENERATION_MSG = (
    "FATAL: session storage stopped writing because another Nunmai process still holds a deleted "
    "state.db-wal or state.db-shm inode (an old copy of the write-ahead log). Nothing is lost: quit "
    "every Nunmai process on this profile (Desktop app, gateway, dashboard, cron), run `nunmai doctor` "
    "(it names the processes still holding the log), then start Nunmai again. Do not delete the WAL "
    "yourself and do not run `nunmai doctor --fix` while they are running. "
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `nunmai sessions recover --source <state.db> "
