...
unprivileged `nunmai` user, giving the layout most people have —
$NUNMAI_HOME/nunmai-engine plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  NUNMAI_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .nunmai-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `nunmai setup` & the browser tools for speed.
...
GIT_ROOT="${NUNMAI_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t nunmai-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    set +e
    curl -fsSL https://nunmai-engine.nunmai.in/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/nunmai-engine.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `nunmai` launcher to ~/.local/bin and the
# checkout to $NUNMAI_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/nunmai-engine.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q nunmai-engine.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t nunmai-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Nunmai sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/nunmai-sandbox/next"
