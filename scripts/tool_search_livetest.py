#!/usr/bin/env python3
"""Live test harness for Nunmai Engine's Tool Search feature.
...
# Force-isolate the test environment BEFORE any nunmai imports.
ORIGINAL_HOME = os.environ.get("NUNMAI_HOME")
ORIGINAL_AUTH = Path.home() / ".nunmai" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.nunmai/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.nunmai/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nunmai_ts_live_"))
    nunmai_home = home_dir / ".nunmai"
    nunmai_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nunmai_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nunmai" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nunmai_home / ".env")
        ...
        load_nunmai_dotenv(nunmai_home=str(Path.home() / ".nunmai"))
    ...
    (nunmai_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nunmai_home
...
def reset_module_state():
    """Drop cached modules so the new NUNMAI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "nunmai_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original NUNMAI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NUNMAI_HOME"] = ORIGINAL_HOME
    else:
