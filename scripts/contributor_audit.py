...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Nunmai\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (nunmai-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "nunmai@nousresearch.com",
    "nunmai-audit@example.com",
    "nousbot@nousresearch.com",
    "nunmai@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "Nunmai-Private-Limited/nunmai-engine",
                "--state", "merged",
