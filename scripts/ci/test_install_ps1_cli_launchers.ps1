# Behavioral test for install.ps1's nunmai launcher staging (PR #92092,
...
#
# The test lifts the real Install-NunmaiCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $NunmaiHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-NunmaiCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-NunmaiCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'nunmai-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'nunmai-engine'
    ...
    $missingThrew = $false
    try {
        Install-NunmaiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing nunmai.exe fails the launcher stage'
    ...
    $nunmaiV1 = [byte[]](77, 90, 1)
    $nunmaiV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'nunmai.exe'), $nunmaiV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-NunmaiCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $nunmaiV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'nunmai.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'nunmai-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'nunmai.exe'), $nunmaiV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'nunmai-acp.exe'), $acp)
    Install-NunmaiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $nunmaiV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'nunmai.exe'))) `
        'installer refreshes an existing Nunmai launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'nunmai-acp.exe'))) `
    ...
    Install-NunmaiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'nunmai.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'nunmai.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'nunmai.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'nunmai.exe')) -and $cmdBody.Contains('%*')) `
