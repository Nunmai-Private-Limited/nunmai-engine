...
# tree generates llms.txt — the index every LLM (Nunmai included, via the
# nunmai-engine skill) reads to learn what Nunmai can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/nunmai_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"nunmai_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Nunmai-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``nunmai serve`` backend, the Docker image)
