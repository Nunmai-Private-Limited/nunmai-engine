...
# changing PATH, installing software, or touching the user's Nunmai home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("nunmai-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
