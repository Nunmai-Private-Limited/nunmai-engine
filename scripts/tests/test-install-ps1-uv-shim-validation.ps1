...
# trust any file at $NunmaiHome\bin\uv.exe -- including the Chocolatey ShimGen
...
$testRoot = Join-Path $env:TEMP ("nunmai-uv-shim-test-" + [Guid]::NewGuid().ToString('N'))
$NunmaiHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
New-Item -ItemType Directory -Force -Path $testRoot | Out-Null
. $installScript -NunmaiHome $NunmaiHome -InstallDir $InstallDir
...
function Invoke-InstallUvScenario {
    ...
    if (Test-Path $NunmaiHome) { Remove-Item -LiteralPath $NunmaiHome -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $NunmaiHome 'bin') | Out-Null
