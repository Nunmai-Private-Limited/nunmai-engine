...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[nunmai] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[nunmai`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[nunmai`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        NunmaiHome = $(if ($paths) { $paths.nunmai_home } else { $null })
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-nunmai'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-nunmai') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
