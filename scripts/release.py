...
VERSION_FILE = REPO_ROOT / "nunmai_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level NUNMAI_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (nunmai journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "nunmai.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.nunmai/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (Nunmai Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Nunmai fallback when no local token, sync gateway session model after fallback)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows nunmai-shim resolution + prefer --update on recovery; #52378)
    ...
    "nunmai@marian.local": "Schrotti77",
    ...
    "274096618+nunmai-engine-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "nunmai@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on nunmai mcp add)
    ...
    "nunmai@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (NUNMAI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (NUNMAI_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: nunmai-eval community link)
    ...
    "nunmaiagent26@gmail.com": "nunmaiagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+nunmaiagent26@users.noreply.github.com": "nunmaiagent26",
    ...
    "hex.nunmai@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Nunmai runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level NUNMAI_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Nunmai
    ...
    # Keep the bootstrap installer (Nunmai-Setup.dmg CFBundleShortVersionString)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "nunmai@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Nunmai-Private-Limited/nunmai-engine",
    ...
    lines.append(f"# Nunmai Engine v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Nunmai Engine. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Nunmai Engine Release Tool")
    ...
    print(f"{'='*60}")
    print("  Nunmai Engine Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Nunmai Engine v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Nunmai Engine v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Nunmai Engine v{new_version} ({calver_date})' "
