...
Environment:
    NUNMAI_TEST_WORKERS  Override worker count (default: os.cpu_count())
    NUNMAI_TEST_PATHS    Override discovery roots (colon-sep; on Windows
...
def _runner_scratch_root() -> str:
    ...
    the root's shape: under the Nunmai home conftest relocates the basetemp; under a dot-dir
    ...
    if os.name == "nt" or not os.path.isdir("/var/tmp"):  # no-tmp: ok — probing the disk-backed FHS root
        root = os.path.join(tempfile.gettempdir(), "nunmai-pytest")
    else:
        root = "/var/tmp/nunmai-pytest"  # no-tmp: ok — /var/tmp is disk-backed by FHS, never tmpfs
...
#                        ghcr.io/nunmai-private-limited/nunmai-engine:test image, via
#                        ``NUNMAI_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or NUNMAI_TEST_FILE_TIMEOUT.
#
...
# Set to 0 to disable (env: NUNMAI_TEST_FILE_RETRIES).
...
def _split_pathspec(value: str) -> List[str]:
    ...
    ``NUNMAI_TEST_PATHS``) into individual paths.
...
def _effective_file_timeout(
    ...
    but a handful of large-collection files (e.g. ``tests/test_nunmai_state.py``,
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/nunmai-engine/nunmai-engine/tests/acp_adapter/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("NUNMAI_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $NUNMAI_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("NUNMAI_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("NUNMAI_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: NUNMAI_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(
            os.environ.get("NUNMAI_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)
        ),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: NUNMAI_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: NUNMAI_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or NUNMAI_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("NUNMAI_TEST_SLICE")
