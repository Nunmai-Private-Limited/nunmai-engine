#!/usr/bin/env bash
# Canonical test runner for nunmai-engine. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.nunmai/...)
#
# Usage:
...
# (NUNMAI_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.nunmai/nunmai-engine/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.nunmai/nunmai-engine/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${NUNMAI_PYTHON:-}" ] && [ -x "$NUNMAI_PYTHON" ] \
    && "$NUNMAI_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: NUNMAI_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `nunmai` binary rather
  # than the devShell hook.
  PYTHON="$NUNMAI_PYTHON"
  echo "▶ no local venv — using Nix dev venv via NUNMAI_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and NUNMAI_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.nunmai/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.nunmai"
...
#
#   * NUNMAI_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * NUNMAI_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     NUNMAI_GIT_SHA build-arg the workflow bakes in).
#   * NUNMAI_E2E_REQUIRE_TUI turns a missing Ink TUI build into a failure in
...
# NUNMAI_RUN_SLOW_PET_TESTS / NUNMAI_E2E_BROWSER / NUNMAI_RUN_E2E opt-ins
# forwarded below.
# Keep this an explicit allowlist (no NUNMAI_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in NUNMAI_TEST_IMAGE NUNMAI_TEST_WORKERS NUNMAI_TEST_PATHS \
  NUNMAI_TEST_FILE_TIMEOUT NUNMAI_TEST_FILE_RETRIES NUNMAI_TEST_SLICE \
  NUNMAI_GATEWAY_LOCK_DIR NUNMAI_E2E_REQUIRE_TUI CI GITHUB_ACTIONS; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${NUNMAI_RUN_SLOW_PET_TESTS:+NUNMAI_RUN_SLOW_PET_TESTS="$NUNMAI_RUN_SLOW_PET_TESTS"} \
  ${NUNMAI_E2E_BROWSER:+NUNMAI_E2E_BROWSER="$NUNMAI_E2E_BROWSER"} \
  ${NUNMAI_RUN_E2E:+NUNMAI_RUN_E2E="$NUNMAI_RUN_E2E"} \
