...
``stage2-hook.sh`` seeds ``auth.json`` from ``NUNMAI_AUTH_JSON_BOOTSTRAP`` only
...
Design constraints
------------------
- Pure stdlib, no nunmai_cli imports: runs early in the boot hook, before the
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a NUNMAI_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    NUNMAI_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             NUNMAI_AUTH_JSON_BOOTSTRAP path handle it)
