...
distributions it is a RAM-backed tmpfs that fills under Nunmai load. Nunmai resolves scratch
space through one helper (``nunmai_constants.get_scratch_dir()`` → ``NUNMAI_HOME/cache/scratch``,
which every Nunmai process also exports as ``TMPDIR``/``TMP``/``TEMP``), and prompts + skills
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"nunmai_constants.get_scratch_dir() (or $TMPDIR / tempfile, which Nunmai points there), tell the "
