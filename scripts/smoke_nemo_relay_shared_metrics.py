"""Run a real Nunmai CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_nunmai_executable(nunmai_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "nunmai",
        Path(".venv") / "Scripts" / "nunmai.exe",
    ):
        candidate = nunmai_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("nunmai")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Nunmai executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Nunmai source checkout containing .venv/bin/nunmai",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated NUNMAI_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["nunmai.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['nunmai.client.active']}"
        )
    [model] = by_name["nunmai.model_route.count"]
    expected_model = {
        "name": "nunmai.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['nunmai.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["nunmai.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['nunmai.task_run.started']}"
        )
    [terminal] = by_name["nunmai.task_run.finished"]
    ...
    [tool] = by_name["nunmai.tool_call.count"]
    ...
    lifecycle = by_name["nunmai.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["nunmai.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Nunmai development environment requires jsonschema"
        ) from exc
    ...
    if metrics["nunmai.client.active"] != [
        {
            "name": "nunmai.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['nunmai.client.active']}"
        )
    [model] = metrics["nunmai.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['nunmai.model_route.count']}"
        )
    [terminal] = metrics["nunmai.task_run.finished"]
    ...
    [tool] = metrics["nunmai.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["nunmai.skill.lifecycle.count"]
    ...
    loads = metrics["nunmai.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    nunmai_repo = args.nunmai_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    nunmai = _resolve_nunmai_executable(nunmai_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="nunmai-relay-shared-metrics-"))
    home = root / "nunmai-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["NUNMAI_HOME"] = str(home)
        python_paths = [str(nunmai_repo)]
    ...
    (root / "nunmai.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "nunmai.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Nunmai exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Nunmai model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Nunmai did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Nunmai did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "nunmai.shared_metrics.v2.schema.json",
    )

    print("Nunmai -> NeMo Relay shared-metrics smoke test passed")
