...
When Nunmai runs in TUI mode, the gateway child process communicates with
...
# ``get_nunmai_home() / "plugins"`` (user) and ``./.nunmai/plugins/`` (project,
# gated behind ``NUNMAI_ENABLE_PROJECT_PLUGINS``) — see
# ``nunmai_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``nunmai_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import nunmai_constants (this script
    ...
    # plugins in get_nunmai_home()/plugins/ and project plugins in
    # ./.nunmai/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_nunmai_home() / "plugins"]
    if os.environ.get("NUNMAI_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".nunmai" / "plugins")
