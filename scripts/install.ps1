# ============================================================================
# Nunmai Engine Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://nunmai-engine.nunmai.in/install.ps1)
#
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$NunmaiHome = $(if ($env:NUNMAI_HOME) { $env:NUNMAI_HOME } else { "$env:LOCALAPPDATA\nunmai" }),
    [string]$InstallDir = $(if ($env:NUNMAI_HOME) { "$env:NUNMAI_HOME\nunmai-engine" } else { "$env:LOCALAPPDATA\nunmai\nunmai-engine" }),
    ...
    # builds apps/desktop into a launchable Nunmai.exe.
    #
    # Why opt-in:
    #   * Nunmai-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Nunmai.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Nunmai.exe
    ...
    #     `nunmai desktop` already builds on demand.
...
# NUNMAI_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[nunmai] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'NunmaiInstall.LongPath').Type) {
            Add-Type -Namespace 'NunmaiInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [NunmaiInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -NunmaiHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('NunmaiHome')) {
    $NunmaiHome = ConvertTo-LongPath $NunmaiHome
} else {
    $NunmaiHome = ConvertTo-LongPath $(
        if ($env:NUNMAI_HOME) { $env:NUNMAI_HOME } else { "$env:LOCALAPPDATA\nunmai" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:NUNMAI_HOME) { "$env:NUNMAI_HOME\nunmai-engine" } else { "$env:LOCALAPPDATA\nunmai\nunmai-engine" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: NunmaiHome=$NunmaiHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:Nunmai-Private-Limited/nunmai-engine.git"
$RepoUrlHttps = "https://github.com/Nunmai-Private-Limited/nunmai-engine.git"
$PythonVersion = "3.11"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NunmaiHome)) {
        New-Item -ItemType Directory -Force -Path $NunmaiHome | Out-Null
    }
    $envFile = Join-Path $NunmaiHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `nunmai update`'s npx
    ...
    $prefixDir = Join-Path $NunmaiHome "node"
...
function Install-Uv {
    # Nunmai owns its own uv at $NunmaiHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nunmai_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `nunmai update` stay in sync.
    $managedUv = Join-Path $NunmaiHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $NunmaiHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NunmaiHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NunmaiHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NunmaiHome "bin"
        ...
        # (nunmai_cli/managed_uv.py looks only at $NunmaiHome\bin\uv.exe).
...
# Put the Nunmai-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone nunmai-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Nunmai-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Nunmai-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("nunmai-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and NUNMAI_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Nunmai belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # nunmai_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".nunmai-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Nunmai-managed interpreter
    ...
    #
    # Under Nunmai-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Nunmai-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Nunmai setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Nunmai uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nunmai\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Nunmai needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Nunmai copy of Git ourselves is predictable and
    ...
    ``NUNMAI_GIT_BASH_PATH`` (User scope) so Nunmai can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Nunmai-managed PortableGit install instead..."
    }

    # Download PortableGit into $NunmaiHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $NunmaiHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Nunmai features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Nunmai needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NUNMAI_GIT_BASH_PATH`` (User env scope) so Nunmai can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NunmaiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NunmaiHome\git\usr\bin\bash.exe
    $candidates += "$NunmaiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NunmaiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NUNMAI_GIT_BASH_PATH", $candidate, "User")
            $env:NUNMAI_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set NUNMAI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Nunmai may not find Git Bash."
    Write-Info "If needed, set NUNMAI_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Nunmai-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Nunmai requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Nunmai requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Nunmai-managed Node.js installation instead..."

    # Prefer a Nunmai-managed Node from a previous run over a too-old system one.
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        Update-ManagedNpm "$NunmaiHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Nunmai-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $NunmaiHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NunmaiHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$NunmaiHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$NunmaiHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$NunmaiHome\node") {
                    try {
                        Rename-Item "$NunmaiHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Nunmai-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$NunmaiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $NunmaiHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nunmai-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nunmai
                    # update` path (#4735).
                    ...
                    $stashName = "nunmai-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. nunmai-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``nunmai update`` and
                    ...
                    if ($LASTEXITCODE -ne 0) {
                        ...
                        # as ``nunmai update`` (which also prunes these refs).
                        $dropped = 0
                        ...
                        if ($dropped -gt 0) {
                            ...
                            $rescueRef = "refs/nunmai-update-backups/$rescueKind-$Branch-$rescueStamp-$($preResetSha.Substring(0, [Math]::Min(12, $preResetSha.Length)))"
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Nunmai behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running nunmai processes) and try again."
                throw
            }
        }
    }
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($extractedDir) {
                    ...
                    # `nunmai update` (see the notes at the shared clone-path
    ...
    # next `nunmai update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Nunmai-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Nunmai-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running nunmai process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running nunmai processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Nunmai_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (nunmai.exe) plus its child tree.
            & taskkill /F /T /IM nunmai.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM nunmai.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m nunmai_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not nunmai.exe.
            ...
            # Nunmai children re-exec through .nunmai-runtime, so killing only
            ...
            # user (see nunmai_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Nunmai " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `nunmai update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Nunmai (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nunmai.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'nunmai-engine\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nunmai-engine package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nunmai.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nunmai`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Nunmai processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Nunmai processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Nunmai and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register nunmai.exe in dist-info/RECORD but fail to
        ...
        # `nunmai` command while nunmai-engine.exe / nunmai-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m nunmai_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `nunmai dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\nunmai_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `nunmai dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nunmai dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: nunmai_cli/web_server.py"
        }
    }
...
function Install-NunmaiCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the nunmai launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring nunmai.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "nunmai.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the nunmai command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # nunmai_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("nunmai", "nunmai-acp")) {
    ...
    $requiredExe = Join-Path $Destination "nunmai.exe"
    $requiredCmd = Join-Path $Destination "nunmai.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the nunmai command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up nunmai command..."
    
    if ($NoVenv) {
        $nunmaiBin = "$InstallDir"
    } else {
        # $NunmaiHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `nunmai update`'s autostash
        ...
        # installer staged under nunmai-engine\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-NunmaiCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $nunmaiBin = "$NunmaiHome\bin"
        Install-NunmaiCommandLaunchers -Root $InstallDir -Destination $nunmaiBin | Out-Null
    }
    ...
    #   nunmai-engine\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The nunmai-engine\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept nunmai via $nunmaiBin)"
        }
    }
    ...
    # Set NUNMAI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nunmai instead
    # of the Unix default ~/.nunmai
    $currentNunmaiHome = [Environment]::GetEnvironmentVariable("NUNMAI_HOME", "User")
    if (-not $currentNunmaiHome -or $currentNunmaiHome -ne $NunmaiHome) {
        [Environment]::SetEnvironmentVariable("NUNMAI_HOME", $NunmaiHome, "User")
        Write-Success "Set NUNMAI_HOME=$NunmaiHome"
    }
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.nunmai-bootstrap-complete which tells the Nunmai
    ...
    # Nunmai-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `nunmai update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".nunmai-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the NUNMAI_HOME directory structure ($NunmaiHome, default %LOCALAPPDATA%\nunmai)
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NunmaiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NunmaiHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NunmaiHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Nunmai's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$NunmaiHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in nunmai_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $NunmaiHome"
    
    # Seed bundled skills into $NunmaiHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NunmaiHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $NunmaiHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NunmaiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NunmaiHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Nunmai-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nunmai setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nunmai-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nunmai-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# failure is non-fatal (browser_exec can still run via uvx, and `nunmai tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $NunmaiHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Nunmai' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Nunmai' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'nunmai tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `nunmai
...
# paths still lazy-install via install_cua_driver() (nunmai_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `nunmai computer-use install` runs. Bounded
        ...
        # Nunmai' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'nunmai tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: nunmai computer-use install"
            }
        } else {
...
# the final `electron` -> `Nunmai` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Nunmai.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_nunmai_desktop command
    # resolves apps/desktop/release/win-unpacked/Nunmai.exe directly,
    ...
    # enforces the supported Node lines and prepends the Nunmai-managed Node to
    ...
    # The --dir mode produces an unpacked Nunmai.exe in
    ...
    # are NOT trying to work around). The Nunmai icon + product name are
    # stamped onto Nunmai.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\nunmai-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Nunmai.exe",
        "$desktopDir\release\win-arm64-unpacked\Nunmai.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Nunmai.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Nunmai icon + identity are stamped onto Nunmai.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, nunmai-engine#38216).
    ...
    #    Nunmai.exe. We deliberately do NOT point them at `nunmai desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Nunmai.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nunmai.lnk')
        )
        ...
        # with the Nunmai icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.nunmai/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nunmai setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nunmai_cli.main setup
    } else {
        python -m nunmai_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $nunmaiCmd = "$InstallDir\venv\Scripts\nunmai.exe"
    if (-not (Test-Path $nunmaiCmd)) {
        $nunmaiCmd = "nunmai"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NunmaiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nunmai whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nunmai gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NunmaiHome\logs\gateway.log"
            Start-Process -FilePath $nunmaiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NunmaiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nunmai gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nunmai gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NunmaiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NunmaiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NunmaiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NunmaiHome\nunmai-engine\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nunmai              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nunmai setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nunmai config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nunmai config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nunmai gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nunmai update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Nunmai repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.nunmai-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Nunmai-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Nunmai to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://nunmai-engine.nunmai.in/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
