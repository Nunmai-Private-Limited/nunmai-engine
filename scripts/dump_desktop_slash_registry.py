...
Run after changing any ``desktop=`` value or alias in ``nunmai_cli/commands.py``;
``tests/nunmai_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from nunmai_cli.commands import desktop_surface_registry
