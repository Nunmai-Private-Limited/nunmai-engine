...
Every writer of ``~/.nunmai/config.yaml`` (and ``profiles/*/config.yaml``) must go through
...
* any ``yaml.dump`` / ``yaml.safe_dump`` / ``atomic_yaml_write`` call inside ``nunmai_cli/config.py``
  or ``nunmai_cli/config_*.py`` (the config system has exactly one writer).
...
DEFAULT_TREES = ("nunmai_cli", "agent", "gateway", "tui_gateway", "cron", "plugins", "tools", "cli.py", "utils.py")
...
CONFIG_MODULE_RE = re.compile(r"^nunmai_cli/config(_[a-z_]+)?\.py$")
...
def scan_file(path: Path) -> list[str]:
    ...
    def flag(node: ast.Call, why: str) -> None:
        ...
        if SUPPRESS in line:
            return
        problems.append(f"{rel}:{node.lineno}: {why} — route it through nunmai_cli.config.atomic_config_write")
