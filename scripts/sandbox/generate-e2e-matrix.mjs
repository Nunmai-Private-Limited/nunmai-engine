 ...
 *   The artifact published on the website right now -- Nunmai-Setup.exe has
 ...
 * @typedef {InstallMethod | 'nunmai-update' | 'open-app-update' | 'nunmai-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; nunmai-desktop-app-update starts the app
 *   via `nunmai desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      ...
      // The same one-liner with -IncludeDesktop: builds Nunmai.exe AND
      ...
      // Website Nunmai-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `nunmai desktop`.
      { method: 'nunmai-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Nunmai-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `nunmai desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'nunmai-desktop-app-update';
}
