...
# Python-driven tool session (e.g. Nunmai terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Nunmai-Private-Limited/nunmai-engine.git"
REPO_URL_HTTPS="https://github.com/Nunmai-Private-Limited/nunmai-engine.git"
NUNMAI_HOME="${NUNMAI_HOME:-$HOME/.nunmai}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NUNMAI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NUNMAI_INSTALL_DIR"
...
#   code at /usr/local/lib/nunmai-engine, command at /usr/local/bin/nunmai,
#   data still at /root/.nunmai (NUNMAI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$NUNMAI_HOME/.no-bundled-skills so future"
            echo "                   'nunmai update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Nunmai.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.nunmai/nunmai-engine"
            echo "                   default (root, Linux): /usr/local/lib/nunmai-engine"
            echo "  --nunmai-home PATH  Data directory (default: ~/.nunmai, or \$NUNMAI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Nunmai installs the code under"
            echo "  /usr/local/lib/nunmai-engine and links the command into"
            echo "  /usr/local/bin/nunmai (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NUNMAI_HOME"
            echo "  (default /root/.nunmai).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$NUNMAI_HOME/nunmai-engine are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `nunmai update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Nunmai-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Nunmai Engine","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install nunmai command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `nunmai` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NUNMAI_HOME/nunmai-engine
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NUNMAI_HOME/nunmai-engine
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nunmai-engine
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $NUNMAI_HOME/nunmai-engine — then preserve it)
#
# Always no-op when the user set --dir or $NUNMAI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NUNMAI_HOME.
    if is_termux; then
        INSTALL_DIR="$NUNMAI_HOME/nunmai-engine"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NUNMAI_HOME/nunmai-engine/.git" ]; then
            INSTALL_DIR="$NUNMAI_HOME/nunmai-engine"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nunmai-engine)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nunmai-engine"
        ...
        # /usr/local/bin/nunmai wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/nunmai"
        log_info "  Data:    $NUNMAI_HOME (unchanged)"
...
# Point a Nunmai-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $NUNMAI_HOME/node/bin — which is NOT on
...
# Nunmai's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Nunmai-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$NUNMAI_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$NUNMAI_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$NUNMAI_HOME/node/etc/npmrc"
}

get_nunmai_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nunmai" ]; then
        echo "$link_dir/nunmai"
    else
        echo "nunmai"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://nunmai-engine.nunmai.in/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Nunmai owns its own uv at $NUNMAI_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nunmai_cli/managed_uv.py) looks in the same
    # place, so install.sh and `nunmai update` stay in sync.
    local _managed_uv="$NUNMAI_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $NUNMAI_HOME/bin ..."
    mkdir -p "$NUNMAI_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-$NUNMAI_HOME}/nunmai-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-$NUNMAI_HOME}/nunmai-uv-installer.$$.sh")"
    ...
    # directly into $NUNMAI_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$NUNMAI_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Nunmai currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Nunmai requires Python >=3.11,<3.14"
...
# replaced with the Nunmai-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Nunmai-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Nunmai-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Nunmai-managed Node from a previous run over a too-old system one.
    if [ -x "$NUNMAI_HOME/node/bin/node" ] && [ -x "$NUNMAI_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$NUNMAI_HOME/node/bin/node" --version)"; then
        export PATH="$NUNMAI_HOME/node/bin:$PATH"
        log_success "Node.js $("$NUNMAI_HOME/node/bin/node" --version) found (Nunmai-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Nunmai-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Nunmai requires Node 22.22+, 24.11+, or 26+) — installing Nunmai-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.nunmai/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.nunmai/node/..."
    ...
    # Place into ~/.nunmai/node/ and symlink binaries into the same bin dir
    # the nunmai command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$NUNMAI_HOME/node"
    mkdir -p "$NUNMAI_HOME"
    mv "$extracted_dir" "$NUNMAI_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$NUNMAI_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$NUNMAI_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$NUNMAI_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$NUNMAI_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.nunmai/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Nunmai install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Nunmai Engine itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `nunmai update` path
                # (#4735).
                ...
                local stash_name
                stash_name="nunmai-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``nunmai update`` and reset to the
            ...
            if ! git pull --ff-only origin "$BRANCH"; then
                ...
                if [ "${dropped:-0}" -gt 0 ]; then
                    ...
                    rescue_ref="refs/nunmai-update-backups/$rescue_kind-$BRANCH-$(date -u +%Y%m%d-%H%M%S)-$(git rev-parse --short=12 HEAD)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Nunmai behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up nunmai command..."

    if [ "$USE_VENV" = true ]; then
        NUNMAI_BIN="$INSTALL_DIR/venv/bin/python"
        NUNMAI_ENTRYPOINT="$INSTALL_DIR/nunmai"
    else
        NUNMAI_BIN="$(which nunmai 2>/dev/null || echo "")"
        if [ -z "$NUNMAI_BIN" ]; then
            log_warn "nunmai not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$NUNMAI_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$NUNMAI_ENTRYPOINT" ]; }; then
        log_warn "Nunmai launcher prerequisites not found"
    ...
    # Create a user-facing shim for the nunmai command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $NUNMAI_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$NUNMAI_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/nunmai"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/nunmai" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NUNMAI_BIN" "$NUNMAI_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/nunmai" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    # Also expose `nunmai-engine`. The `nunmai-engine` console script declared in
    ...
    rm -f "$command_link_dir/nunmai-engine"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nunmai-engine" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NUNMAI_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/nunmai-engine" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NUNMAI_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/nunmai-engine"
    log_success "Installed nunmai-engine launcher → $command_link_display_dir/nunmai-engine"

    # Also expose `nunmai-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `nunmai-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Nunmai as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/nunmai-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nunmai-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NUNMAI_BIN" "$NUNMAI_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/nunmai-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/nunmai-acp"
    log_success "Installed nunmai-acp launcher → $command_link_display_dir/nunmai-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nunmai command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nunmai' \
                >/dev/null 2>&1; then
        ...
        log_info "nunmai not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Nunmai Engine — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Nunmai Engine — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Nunmai Engine — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so nunmai works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.nunmai directory structure (config at top level, code in subdir)
    mkdir -p "$NUNMAI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.nunmai/.env (top level, easy to find)
    if [ ! -f "$NUNMAI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NUNMAI_HOME/.env"
            log_success "Created ~/.nunmai/.env from template"
        else
            touch "$NUNMAI_HOME/.env"
            log_success "Created ~/.nunmai/.env"
        fi
    else
        log_info "~/.nunmai/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$NUNMAI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.nunmai/config.yaml (top level, easy to find)
    if [ ! -f "$NUNMAI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NUNMAI_HOME/config.yaml"
            log_success "Created ~/.nunmai/config.yaml from template"
        fi
    else
        log_info "~/.nunmai/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in nunmai_cli/default_soul.py — the
    ...
    if [ ! -f "$NUNMAI_HOME/SOUL.md" ]; then
        cat > "$NUNMAI_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.nunmai/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.nunmai/"

    # Seed bundled skills into ~/.nunmai/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `nunmai update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'nunmai update'." \
            > "$NUNMAI_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $NUNMAI_HOME/.no-bundled-skills"
        log_info "  Future 'nunmai update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.nunmai/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.nunmai/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NUNMAI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$NUNMAI_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.nunmai/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$NUNMAI_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Nunmai Engine browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Nunmai will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Nunmai Engine browser tools — explicit browser override."
...
# install — the same closure `nunmai update` installs
# (nunmai_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `nunmai update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `nunmai tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$NUNMAI_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Nunmai' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$NUNMAI_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'nunmai tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `nunmai tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `nunmai computer-use install` on their own).
    ...
    # Same upstream installer `nunmai computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Nunmai install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Nunmai'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        log_info "Install later with: nunmai computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nunmai setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Nunmai to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$NUNMAI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'nunmai whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NUNMAI_CMD="$(get_nunmai_command_path)"
                $NUNMAI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nunmai whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nunmai gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NUNMAI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NUNMAI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nunmai gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nunmai gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NUNMAI_CMD gateway > "$NUNMAI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.nunmai/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nunmai gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nunmai gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.nunmai-bootstrap-complete, which tells the Nunmai
    ...
    local marker_path="$INSTALL_DIR/.nunmai-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NUNMAI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NUNMAI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NUNMAI_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}nunmai${NC}              Start chatting"
    echo -e "   ${GREEN}nunmai setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}nunmai config${NC}       View/edit configuration"
    echo -e "   ${GREEN}nunmai config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}nunmai gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nunmai update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nunmai' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nunmai' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nunmai' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$NUNMAI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$NUNMAI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$NUNMAI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$NUNMAI_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Nunmai` rename
...
# _purge_electron_build_cache() used by `nunmai desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Nunmai-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `nunmai desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Nunmai" ]; then
            app="$desktop_dir/release/linux-unpacked/Nunmai"
        elif [ -x "$desktop_dir/release/linux-unpacked/nunmai" ]; then
            app="$desktop_dir/release/linux-unpacked/nunmai"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Nunmai.app" \
            "$desktop_dir/release/mac/Nunmai.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `nunmai desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading NUNMAI_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if NUNMAI_HOME="$NUNMAI_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from nunmai_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `nunmai update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Nunmai-managed Node
            # provisioned during prerequisites/node-deps (at $NUNMAI_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $NUNMAI_HOME.
    # $NUNMAI_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'nunmai update' on this host install.
