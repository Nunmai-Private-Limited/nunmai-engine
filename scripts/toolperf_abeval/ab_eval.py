...
measured in the production session DB) through `nunmai chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    NUNMAI_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Nunmai home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "NUNMAI_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "nunmai_cli.main", "chat", "--query", q,
