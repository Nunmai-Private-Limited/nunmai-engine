...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Nunmai home, same model, same tasks, same reps.
...
1. Create a dedicated Nunmai home with credentials for the models under test:
   ...
   integration at it; no Nunmai observability plugin needs to be enabled.
