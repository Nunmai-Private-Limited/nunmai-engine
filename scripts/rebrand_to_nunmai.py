...
POST_REPLACEMENTS = [
    ...
    # --- added 2026-09-26 while rebranding upstream 0.21.5 ---
    # Support and source links a Nunmai user is shown (README footers, `nunmai debug`, the desktop
    # "send diagnostics" dialog, issue/PR templates, CONTRIBUTING). They pointed a Nunmai customer at
    # the upstream tracker. Numbered issue/PR references in code comments are history and stay.
    (re.compile(r"https://github\.com/NousResearch/hermes-agent/issues(?![/\d])"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/issues"),
    (re.compile(r"https://github\.com/NousResearch/hermes-agent/(?:blob|tree)/main/"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/"),
    (re.compile(r"https://github\.com/NousResearch/hermes-agent(?=[\s\"'<>)\]]|$)"), "https://github.com/Nunmai-Private-Limited/nunmai-engine"),
    (re.compile(r"ssh://git@github\.com/NousResearch/hermes-agent\.git"), "ssh://git@github.com/Nunmai-Private-Limited/nunmai-engine.git"),
    (re.compile(r"git@github\.com:NousResearch/hermes-agent(?!\.git)"), "git@github.com:Nunmai-Private-Limited/nunmai-engine"),
    (re.compile(r"api\.github\.com/repos/NousResearch/hermes-agent"), "api.github.com/repos/Nunmai-Private-Limited/nunmai-engine"),
    # Docker images: `nunmai setup` and the browser tool tell the user which image to pull.
    (re.compile(r"ghcr\.io/nousresearch/hermes-agent(?=[:\s\"'`]|$)"), "ghcr.io/nunmai-private-limited/nunmai-engine"),
    (re.compile(r"(?<![\w/.])nousresearch/hermes-agent(?=[:\s\"'`]|$)"), "ghcr.io/nunmai-private-limited/nunmai-engine"),
    # README docs badge.
    (re.compile(r"https://img\.shields\.io/badge/Docs-hermes--agent\.nousresearch\.com-FFD700"), "https://img.shields.io/badge/Docs-nunmai.in-6BE7C8"),
    # The non-agentic warning names a THIRD PARTY's chat models. "Nous Research Hermes 3" became
    # "Nunmai Research Hermes 3", which claims them as ours; the plain model family name is accurate.
    (re.compile(r"Nunmai Research Hermes 3"), "Hermes 3"),
    (re.compile(r"Nunmai Hermes 3"), "Hermes 3"),
    # Code and scripts a Nunmai install FETCHES: the zip updater, install.ps1 tag/branch archives, the
    # desktop bootstrap runner, the plugin-validate action. Left on upstream they would install the
    # unbranded product over a Nunmai install. Our public repo carries the same paths.
    (re.compile(r"https://github\.com/NousResearch/hermes-agent/archive/"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/archive/"),
    (re.compile(r"https://raw\.githubusercontent\.com/NousResearch/hermes-agent/"), "https://raw.githubusercontent.com/Nunmai-Private-Limited/nunmai-engine/"),
    (re.compile(r"https://github\.com/NousResearch/hermes-agent/(?=pulls|discussions|security/advisories/new|#|;)"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/"),
    (re.compile(r"https://github\.com/NousResearch/hermes-agent(?=[#;])"), "https://github.com/Nunmai-Private-Limited/nunmai-engine"),
    (re.compile(r"git\+https://github\.com/NousResearch/hermes-agent@"), "git+https://github.com/Nunmai-Private-Limited/nunmai-engine@"),
    (re.compile(r"NousResearch/hermes-agent/\.github/actions/"), "Nunmai-Private-Limited/nunmai-engine/.github/actions/"),
    # The desktop updater's notion of the OFFICIAL repo: a Nunmai install must count as official, or
    # passive update checks treat it as a fork. CI guards likewise run on OUR repository.
    (re.compile(r"github\.com/nousresearch/hermes-agent(?=['\"])"), "github.com/nunmai-private-limited/nunmai-engine"),
    (re.compile(r"github\.repository == 'NousResearch/hermes-agent'"), "github.repository == 'Nunmai-Private-Limited/nunmai-engine'"),
    # Bare owner/repo slug in prose, gh examples and the skills hub (not #NNN refs, not HF datasets).
    (re.compile(r"(?<![\w/.@:-])NousResearch/hermes-agent(?![\w/#.-])"), "Nunmai-Private-Limited/nunmai-engine"),
    # Docker Hub tag page + image name in prose, e.g. "(nousresearch/hermes-agent)".
    (re.compile(r"https://hub\.docker\.com/r/nousresearch/hermes-agent/tags"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/pkgs/container/nunmai-engine"),
    (re.compile(r"(?<![\w/.])nousresearch/hermes-agent(?=[:\s\"'`)]|$)"), "ghcr.io/nunmai-private-limited/nunmai-engine"),
    # package.json metadata (mixed-case slug), a URL string split across lines, a slug ending a sentence,
    # and the desktop updater's own tests (their inputs must be the new official remote).
    (re.compile(r"https://github\.com/NousResearch/Hermes-Agent(?=\.git|/issues|#readme)"), "https://github.com/Nunmai-Private-Limited/nunmai-engine"),
    (re.compile(r"https://raw\.githubusercontent\.com/NousResearch/hermes-agent(?=\")"), "https://raw.githubusercontent.com/Nunmai-Private-Limited/nunmai-engine"),
    (re.compile(r"NousResearch/hermes-agent\.(?=\\n|\s|$)"), "Nunmai-Private-Limited/nunmai-engine."),
    (re.compile(r"git@github\.com:nousresearch/hermes-agent\.git"), "git@github.com:nunmai-private-limited/nunmai-engine.git"),
    (re.compile(r"https://github\.com/NousResearch/hermes-agent/'"), "https://github.com/Nunmai-Private-Limited/nunmai-engine/'"),
]
