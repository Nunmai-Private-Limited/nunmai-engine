...
start_ui() {
  ...
  "$py" -c 'import os, signal, sys; os.setsid(); signal.signal(signal.SIGTERM, signal.SIG_DFL); os.execv(sys.argv[1], sys.argv[1:])' \
    "$browser" --app="http://127.0.0.1:$port/" --user-data-dir="${TMPDIR:-/tmp}/nunmai-update-ui-$$" \
...
KEEP_STASH=""
if "$NUNMAI_BIN" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed nunmai predates --keep-stash; running without it"
fi
log "running: nunmai update --yes --gateway $KEEP_STASH --branch $BRANCH"
publish_stage "Updating code and dependencies"
OUT="$("$NUNMAI_BIN" update --yes --gateway $KEEP_STASH --branch "$BRANCH" 2>&1)"; CODE=$?
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  ...
  "$NUNMAI_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
...
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run nunmai debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
