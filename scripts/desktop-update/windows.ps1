...
# (%NUNMAI_HOME%\nunmai-setup.exe). That binary has no self-update path --
# copy_self_to_nunmai_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `nunmai update` refreshes
...
#     -InstallRoot <path>   repo checkout (NUNMAI_HOME\nunmai-engine)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Nunmai.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .nunmai-update-in-progress in place (tests)
#
...
# .nunmai-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim NUNMAI_HOME\.nunmai-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining NUNMAI_UPDATE_STARTED_AT from the Desktop hand-off.
# nunmai_cli/update_lock.py's ancestry rule lets our
# `nunmai update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Nunmai.exe pid.
try {
    Add-Type -Namespace NunmaiHandoff -Name Win32 -MemberDefinition @'
...
$NunmaiHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $NunmaiHome ".nunmai-update-in-progress"
$LogDir = Join-Path $NunmaiHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $NunmaiHome ".nunmai-update-result.json"
$script:Ui = $null
$script:UiStage = "Nunmai will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale nunmai-update-ui-* leftovers from interrupted
    ...
    # update marker (.nunmai-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "nunmai-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-NunmaiStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("nunmai-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [NunmaiHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `nunmai debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"nunmai debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [NunmaiHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Nunmai.exe before writing the replacement,
    ...
    # DO NOT spawn Nunmai.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [NunmaiHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [NunmaiHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [NunmaiHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Nunmai process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [NunmaiHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `nunmai update` deliberately runs its build steps with stdout
# inherited (nunmai_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:NUNMAI_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:NUNMAI_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:NUNMAI_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:NUNMAI_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `nunmai update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (nunmai_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:NUNMAI_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:NUNMAI_UPDATE_PROGRESS_LOG
}
...
if (-not ("NunmaiUpdateJob" -as [type])) {
...
public static class NunmaiUpdateJob {
...
function Invoke-NunmaiStep([string]$Exe, [string[]]$NunmaiArgs, [string]$Tag) {
    ...
    # afterwards, where `nunmai debug share` picks it up.
    #
    ...
    # .nunmai-update-result.json, clearing .nunmai-update-in-progress,
    ...
    # Nunmai" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($NunmaiArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [NunmaiUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `nunmai update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [NunmaiUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-NunmaiVenvDir([string]$Root) {
    # Match nunmai_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; NUNMAI_SELFTEST_FAIL=1 exercises the
# error state, NUNMAI_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:NUNMAI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:NUNMAI_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:NUNMAI_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-NunmaiStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Nunmai install to
...
#            update log, the shape of every real `nunmai update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:NUNMAI_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:NUNMAI_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:NUNMAI_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:NUNMAI_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "nunmai-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "nunmai-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "nunmai-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "nunmai-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "nunmai-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "nunmai-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "nunmai-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "nunmai-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `nunmai update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-NunmaiStep $powershell @(
    ...
    $flood = Invoke-NunmaiStep $powershell @(
    ...
    $stall = Invoke-NunmaiStep $powershell @(
    ...
    try {
        $logstall = Invoke-NunmaiStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-NunmaiStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-NunmaiStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:NUNMAI_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from NUNMAI_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-NunmaiVenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-NunmaiStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Nunmai to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Nunmai window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Nunmai fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Nunmai files"
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Nunmai install open ($shim locked after 20s). Nothing was changed. Close other Nunmai windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the nunmai.exe shim guard, which step 2 just PROVED
    ...
    # _quarantine_running_nunmai_exe must first rename the running nunmai.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that nunmai.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Nunmai window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Nunmai.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m nunmai_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Nunmai installer or `nunmai doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    $updateArgs = @("-m", "nunmai_cli.main", "update", "--yes") + $gatewayArg + @("--force", "--branch", $Branch)
    ...
    # collides with the "close all Nunmai windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m nunmai_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed nunmai predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-NunmaiStep $pythonExe $updateArgs "update"
    Write-HandoffLog "nunmai update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-NunmaiUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `nunmai update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "nunmai update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-NunmaiStep $pythonExe @("-m", "nunmai_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import nunmai_cli.main; from nunmai_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-NunmaiStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Nunmai runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    # before `nunmai update` captures its Windows pause inventory, leaving the
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed -and -not $NoGateway) {
        $gatewayRestart = Invoke-NunmaiStep $pythonExe @("-m", "nunmai_cli.main", "gateway", "start", "--all") "gateway restart"
        if ($gatewayRestart.Code -ne 0) {
            ...
            $manualAction = $true
            $manualMsg = "Update complete, but Nunmai could not restart every messaging gateway. Run `nunmai gateway start --all` in a terminal."
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `nunmai desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `nunmai debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Nunmai is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Nunmai was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Nunmai."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Nunmai"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Nunmai to finish (it could not restart itself)."
