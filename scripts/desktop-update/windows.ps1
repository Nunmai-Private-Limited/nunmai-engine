...
try {
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Nunmai install open (venv\Scripts\nunmai.exe locked after 20s). Nothing was changed. Close other Nunmai windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    $updateArgs = @("-m", "nunmai_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
