 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.nunmai/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.nunmai', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (NUNMAI_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.NUNMAI_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.nunmai', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.NUNMAI_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.nunmai', 'document_cache');
const AUDIO_CACHE_DIR = process.env.NUNMAI_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.nunmai', 'audio_cache');
...
// `nunmai update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Nunmai Engine*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Nunmai itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Nunmai Engine', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Nunmai and trigger
      ...
      // Ignore Nunmai' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `nunmai_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `nunmai_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
