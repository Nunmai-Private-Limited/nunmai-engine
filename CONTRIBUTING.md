...
├── cli.py                    # NunmaiCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (nunmai-cli, nunmai-telegram, etc.)
├── nunmai_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│   ├── auth.py                   # Provider resolution, OAuth, Nunmai Portal
...
| `~/.nunmai/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
...
- **Session persistence**: All conversations are stored in SQLite (`nunmai_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.nunmai/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
