...
Runs tool-calling agent tasks in Nunmai-Agent's execution environments (local,
docker, modal) and writes trajectories in Nunmai format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Nunmai execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Nunmai execution environment, emitting Nunmai trajectories."""
    ...
    def _convert_to_nunmai_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Nunmai trajectory format used by batch_runner.py."""
        system_msg = NUNMAI_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + NUNMAI_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_nunmai_format(messages, task), "completed": completed, "api_calls": api_call_count,
