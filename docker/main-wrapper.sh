#!/bin/sh
# shellcheck shell=sh
# /opt/nunmai/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching NUNMAI_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `nunmai` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `nunmai <args>` (subcommand passthrough)
#
# Drop to nunmai via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${NUNMAI_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${NUNMAI_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export NUNMAI_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset NUNMAI_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid nunmai "$@"; exec "$@"; }
...
# an arbitrary non-root, non-nunmai UID, the bootstrap was skipped and the
# baked image dirs (owned by the nunmai build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u nunmai)" ]; then
    cat >&2 <<EOF
[nunmai] ERROR: container started with --user $cur_uid (an arbitrary, non-nunmai UID) — not supported.
...
# to the nunmai user's home before dropping privileges so libraries that
...
_nunmai_orig_cwd="${NUNMAI_ORIG_CWD:-$PWD}"
...
# the user's command so `nunmai chat` starts in the Docker -w
...
# A leading flag is a nunmai global option (`-p <profile> gateway run`), never an executable:
...
# Nunmai subcommand pass-through.
drop nunmai "$@"
