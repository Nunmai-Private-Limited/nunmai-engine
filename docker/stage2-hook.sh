...
# symlink or a directory someone else owns (chowning that one would hand nunmai a
# directory whose creator keeps an fd into it), and chown rather than assume —
# `usermod -u` above does not chown outside the home dir, so a NUNMAI_UID remap
...
if [ -n "${XDG_RUNTIME_DIR:-}" ]; then
    ...
    if refuse_symlinked_path "create" "$XDG_RUNTIME_DIR"; then
        :
    elif [ -n "$xdg_owner" ] && [ "$xdg_owner" != "0" ] && [ "$xdg_owner" != "$actual_nunmai_uid" ]; then
        echo "[stage2] Warning: $XDG_RUNTIME_DIR is owned by uid $xdg_owner (not root or nunmai) — refusing to adopt it"
    else
        mkdir -p "$XDG_RUNTIME_DIR" 2>/dev/null || \
            echo "[stage2] Warning: could not create XDG_RUNTIME_DIR $XDG_RUNTIME_DIR (continuing)"
        if [ -d "$XDG_RUNTIME_DIR" ]; then
            chown nunmai:nunmai "$XDG_RUNTIME_DIR" 2>/dev/null || \
...
# --- Sync deploy-injected Nunmai routing overrides into every profile .env ---
# Under multiplex, nunmai_cli.auth_nous reads NUNMAI_PORTAL_BASE_URL (or its
...
# carries the value into $NUNMAI_HOME/.env and every profiles/*/.env: the
...
# (/etc/nunmai/.env) composition reverted by #111600 is restored.
...
sync_routing_overrides() {
    ...
    for _name in NUNMAI_PORTAL_BASE_URL NOUS_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL; do
        eval "_value=\${$_name:-}"
        ...
        if [ ! -f "$_file" ] && ! (umask 077 && as_nunmai touch "$_file") 2>/dev/null; then
            echo "[stage2] Warning: could not create $_file — the Nunmai routing overrides will not reach this profile's secret scope"
            return 0
        fi
...
sync_routing_overrides "$NUNMAI_HOME/.env"
for _profile_dir in "$NUNMAI_HOME"/profiles/*/; do
...
# The image populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/nunmai/.playwright)
# with ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``, plus
# ``chromium-<build>/chrome-linux64/`` on a NUNMAI_BOT_DESKTOP build.
# agent-browser (the runtime CLI Nunmai spawns for the
