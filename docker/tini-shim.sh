...
# Nunmai WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: NUNMAI_TINI_SHIM_TARGET overrides the `/init` path (and
# NUNMAI_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${NUNMAI_TINI_SHIM_TARGET:-/init}"
WRAPPER="${NUNMAI_TINI_SHIM_WRAPPER:-/opt/nunmai/docker/main-wrapper.sh}"
