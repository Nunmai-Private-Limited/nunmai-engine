...
timeout. Config is env-only (``NUNMAI_STARTUP_WATCHDOG=0``,
``NUNMAI_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "NUNMAI_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "NUNMAI_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (nunmai_cli.main / nunmai_cli.gateway /
...
def _process_nunmai_home() -> Path:
    """NUNMAI_HOME for diagnostic files — stdlib-only replica of the nunmai_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<NUNMAI_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_nunmai_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``NUNMAI_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``NUNMAI_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
