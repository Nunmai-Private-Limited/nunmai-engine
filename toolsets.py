...
_NUNMAI_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `nunmai-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _NUNMAI_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_NUNMAI_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _NUNMAI_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `nunmai tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``nunmai tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _NUNMAI_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    # (model_tools._select_tool_names). Never configurable, never in `nunmai tools`.
    "setup": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (NUNMAI_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _NUNMAI_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in nunmai_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Nunmai toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. nunmai-acp is the
    ...
    "nunmai-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors nunmai-cli; `nunmai tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "nunmai-cron": _bundle("Default cron toolset - same core tools as nunmai-cli; gated by `nunmai tools`"),
    "nunmai-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "nunmai-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "nunmai-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "nunmai-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "nunmai-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "nunmai-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "nunmai-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "nunmai-email": _bundle("Email bot toolset - interact with Nunmai via email (IMAP/SMTP)"),
    "nunmai-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "nunmai-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "nunmai-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "nunmai-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "nunmai-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "nunmai-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "nunmai-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "nunmai-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "nunmai-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _NUNMAI_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "nunmai-sms": _bundle("SMS bot toolset - interact with Nunmai via SMS (Twilio)"),
    "nunmai-webhook": _ts("Webhook toolset - receive and process external webhook events", _NUNMAI_WEBHOOK_SAFE_TOOLS),
    "nunmai-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "nunmai-telegram", "nunmai-discord", "nunmai-whatsapp", "nunmai-slack",
            "nunmai-signal", "nunmai-bluebubbles", "nunmai-homeassistant", "nunmai-email",
            "nunmai-sms", "nunmai-mattermost", "nunmai-matrix", "nunmai-dingtalk",
            "nunmai-feishu", "nunmai-wecom", "nunmai-wecom-callback", "nunmai-weixin",
            "nunmai-qqbot", "nunmai-webhook", "nunmai-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _NUNMAI_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only nunmai-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_NUNMAI_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `nunmai-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("nunmai-"):
        return []
    platform_name = name[len("nunmai-"):]
    try:
...
def profile_role_toolsets(profile_home: Optional[Path] = None) -> Tuple[Set[str], Set[str]]:
    ...
    from nunmai_cli.profiles import read_profile_meta
    from nunmai_constants import get_nunmai_home
    role = read_profile_meta(Path(profile_home or get_nunmai_home())).get("role")
