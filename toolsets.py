...
_NUNMAI_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (NUNMAI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `nunmai tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NUNMAI_KANBAN_TASK env "
    ...
    # Coding posture (base Nunmai — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in nunmai_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Nunmai toolsets (CLI + messaging platforms)
    #
    ...
    # the `nunmai send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "nunmai-cli": {
        ...
        "tools": _NUNMAI_CORE_TOOLS,
        "includes": []
    },

    "nunmai-cron": {
        # Mirrors nunmai-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nunmai tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nunmai-cli; gated by `nunmai tools`",
        "tools": _NUNMAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nunmai-email": {
        "description": "Email bot toolset - interact with Nunmai via email (IMAP/SMTP)",
        "tools": _NUNMAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nunmai-sms": {
        "description": "SMS bot toolset - interact with Nunmai via SMS (Twilio)",
        "tools": _NUNMAI_CORE_TOOLS,
        "includes": []
    },
    ...
    "nunmai-webhook": {
        ...
        "tools": _NUNMAI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "nunmai-gateway": {
        ...
        "tools": [],
        "includes": ["nunmai-telegram", "nunmai-discord", "nunmai-whatsapp", "nunmai-slack", "nunmai-signal", "nunmai-bluebubbles", "nunmai-homeassistant", "nunmai-email", "nunmai-sms", "nunmai-mattermost", "nunmai-matrix", "nunmai-dingtalk", "nunmai-feishu", "nunmai-wecom", "nunmai-wecom-callback", "nunmai-weixin", "nunmai-qqbot", "nunmai-webhook", "nunmai-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``nunmai-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_NUNMAI_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``nunmai-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nunmai-<name>).
        # Gives them _NUNMAI_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("nunmai-"):
            platform_name = name[len("nunmai-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NUNMAI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
