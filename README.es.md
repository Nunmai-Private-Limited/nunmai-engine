...
# Nunmai Engine ☤
<p align="center">
  <a href="https://nunmai.in/">Nunmai Engine</a> | <a href="https://nunmai.in/">Nunmai Desktop</a>
</p>
<p align="center">
  <a href="https://nunmai.in/docs/"><img src="https://img.shields.io/badge/Docs-nunmai.in-6BE7C8?style=for-the-badge" alt="Documentación"></a>
  <a href="https://discord.gg/NousResearch"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord"></a>
  <a href="https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/LICENSE"><img src="https://img.shields.io/badge/Licencia-MIT-green?style=for-the-badge" alt="Licencia: MIT"></a>
  <a href="https://nunmai.in"><img src="https://img.shields.io/badge/Creado%20por-Nunmai%20Research-blueviolet?style=for-the-badge" alt="Creado por Nunmai Research"></a>
...
> **Nota:** En Windows nativo, Nunmai funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\nunmai\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Nunmai usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://nunmai.in/docs/getting-started/termux). En Termux, Nunmai instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\nunmai`; WSL2 instala en `~/.nunmai` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
nunmai              # ¡empieza a chatear!
```
...
```bash
nunmai              # CLI interactiva — inicia una conversación
nunmai model        # Elige tu proveedor y modelo LLM
nunmai tools        # Configura qué herramientas están habilitadas
nunmai config set   # Establece valores de configuración individuales
nunmai gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
nunmai setup        # Ejecuta el asistente de configuración completo
nunmai claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
nunmai update       # Actualiza a la última versión
nunmai doctor       # Diagnostica cualquier problema
```

📖 **[Documentación completa →](https://nunmai.in/docs/)**
...
Nunmai tiene dos puntos de entrada: inicia la interfaz de terminal con `nunmai`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `nunmai`                                      | Ejecuta `nunmai gateway setup` + `nunmai gateway start`, luego envía un mensaje al bot |
...
Para las listas de comandos completas, consulta la [guía de CLI](https://nunmai.in/docs/user-guide/cli) y la [guía del Gateway de Mensajería](https://nunmai.in/docs/user-guide/messaging).
...
| Sección                                                                                             | Contenido                                                    |
| --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Inicio rápido](https://nunmai.in/docs/getting-started/quickstart)              | Instalar → configurar → primera conversación en 2 minutos   |
| [Uso de CLI](https://nunmai.in/docs/user-guide/cli)                             | Comandos, atajos de teclado, personalidades, sesiones        |
| [Configuración](https://nunmai.in/docs/user-guide/configuration)               | Archivo de configuración, proveedores, modelos, todas las opciones |
| [Gateway de Mensajería](https://nunmai.in/docs/user-guide/messaging)           | Telegram, Discord, Slack, WhatsApp, Signal, Home Assistant   |
| [Seguridad](https://nunmai.in/docs/user-guide/security)                        | Aprobación de comandos, emparejamiento por DM, aislamiento en contenedor |
| [Herramientas y Toolsets](https://nunmai.in/docs/user-guide/features/tools)   | Más de 40 herramientas, sistema de toolsets, backends de terminal |
| [Sistema de Habilidades](https://nunmai.in/docs/user-guide/features/skills)   | Memoria procedimental, Skills Hub, creación de habilidades   |
| [Memoria](https://nunmai.in/docs/user-guide/features/memory)                   | Memoria persistente, perfiles de usuario, mejores prácticas  |
| [Integración MCP](https://nunmai.in/docs/user-guide/features/mcp)              | Conecta cualquier servidor MCP para capacidades extendidas   |
| [Programación Cron](https://nunmai.in/docs/user-guide/features/cron)           | Tareas programadas con entrega a plataforma                  |
| [Archivos de Contexto](https://nunmai.in/docs/user-guide/features/context-files) | Contexto de proyecto que da forma a cada conversación      |
| [Arquitectura](https://nunmai.in/docs/developer-guide/architecture)            | Estructura del proyecto, bucle del agente, clases principales |
| [Contribuir](https://nunmai.in/docs/developer-guide/contributing)              | Configuración de desarrollo, proceso de PR, estilo de código |
| [Referencia de CLI](https://nunmai.in/docs/reference/cli-commands)             | Todos los comandos y flags                                   |
| [Variables de Entorno](https://nunmai.in/docs/reference/environment-variables) | Referencia completa de variables de entorno                  |

---

## Migración desde OpenClaw

Si vienes de OpenClaw, Nunmai puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`nunmai setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
nunmai claw migrate              # Migración interactiva (preset completo)
nunmai claw migrate --dry-run    # Vista previa de qué se migraría
nunmai claw migrate --preset user-data   # Migrar sin secretos
nunmai claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.nunmai/skills/openclaw-imports/`
...
Consulta `nunmai claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-nunmai.sh`:

```bash
git clone https://github.com/Nunmai-Private-Limited/nunmai-engine.git
cd nunmai-engine
./setup-nunmai.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/nunmai
./nunmai              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Nunmai y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [NunmaiClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta Nunmai Engine y OpenClaw en la misma cuenta de WeChat.
...
Creado por [Nunmai Research](https://nunmai.in).
