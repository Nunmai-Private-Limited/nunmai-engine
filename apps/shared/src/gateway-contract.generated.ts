...
/** One persisted reaction row (``nunmai_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
/** One ``nunmai_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` / ``free_tier_pending`` mark the cached-only path). */
...
/** ``nunmai_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); the sale fields are Nunmai Portal-only. */
...
/** ``nunmai_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
/** One ``config_schema`` key of a plugin manifest, rendered by the Plugins hub (``nunmai_cli.plugins_settings.plugin_settings_fields``). ``secret`` fields carry no value: ``env`` names the ``.env`` variable and ``has_value`` whether it is set. */
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``NunmaiSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
export interface RpcMethods {
  ...
  /** Read-only billing view (no scope); the Nunmai free tier is answered locally without a portal call. */
  ...
  /** Run ``nunmai <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** The browser leg came back (nunmai://connections/done): read the accounts now, not at the next tick. */
  ...
  /** Upload a force-redacted debug bundle to Nunmai-internal diagnostics storage. */
  ...
  /** Re-read ~/.nunmai/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.nunmai/sessions/saved (classic /save). */
  ...
  /** Token / context / cost counters for the session (+ Nunmai credit lines when available). */
...
export interface BackendGatewayEventMap {
  ...
  /** This profile's screen started or stopped (also for transitions made outside nunmai serve). */
