...
export const $nunmaiHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.nunmai/logs/bootstrap-installer.log')
    $nunmaiHome.set('~/.nunmai')
  ...
  try {
    const [logPath, nunmaiHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_nunmai_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $nunmaiHome.set(nunmaiHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Nunmai" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Nunmai-Setup.exe --update);
...
export async function launchNunmaiDesktop(): Promise<void> {
  ...
  await invoke('launch_nunmai_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Nunmai repository' },
