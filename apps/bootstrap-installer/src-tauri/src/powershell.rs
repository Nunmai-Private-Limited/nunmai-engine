...
/// read side does not see EOF until the last of them closes it. `nunmai update`
...
/// stranded with it.
///
/// Same bound `Invoke-NunmaiStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `nunmai_home_override` propagates to the child as $NUNMAI_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, nunmai_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = nunmai_home_override {
        cmd.env("NUNMAI_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, nunmai_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = nunmai_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_nunmai_home() {
    ...
    /// child. `nunmai update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
