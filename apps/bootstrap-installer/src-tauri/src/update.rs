...
//!
//! Driven when the installer is launched as `Nunmai-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Nunmai desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `nunmai update`
//!      or repair bootstrap can race locked files),
//!   2. run `nunmai update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in nunmai_cli/main.py),
//!   3. run `nunmai desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `nunmai update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_nunmai) and
...
/// `nunmai update` exit code meaning "another nunmai process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// nunmai_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `nunmai update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `nunmai update` claims
/// the same file (see `nunmai_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `nunmai update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in nunmai_cli/update_lock.py — all three read
...
/// a dashboard-spawned `nunmai update`) still blocks.
...
/// the marker name our PID — because a `nunmai update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let nunmai_home = crate::paths::nunmai_home();
    let install_root = nunmai_home.join("nunmai-engine");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_nunmai`
    ...
    //
    // The same marker is the cross-process update lock (nunmai_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `nunmai update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Nunmai update is already running (PID {}, started {} ago). \
    ...
    let nunmai = resolve_nunmai(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the nunmai CLI under {}. Is Nunmai installed? \
    ...
    // async on Windows. If it still holds the venv shim, `nunmai update`
    ...
    // ---- stage 2: nunmai update -----------------------------------------
    // Pass --branch so `nunmai update` targets the branch this installer was
    ...
    // without --branch, `nunmai update` switches the checkout to `main` (a
    ...
    // --force skips `nunmai update`'s Windows running-exe guard (which would
    ...
    // time `nunmai update` runs there is no legitimate nunmai.exe to protect,
    // and the guard would only produce a false "Nunmai is still running" stop.
    //
    ...
    // probe only checks the nunmai.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Nunmai windows" message.
    ...
    // Retry-once for the update-boundary crash. `nunmai update` lazily imports
    ...
    // second `nunmai update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Nunmai" state
    // a retry can't fix.
    ...
    // checkout predating the NUNMAI_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        other => {
            let msg = format!(
                "nunmai update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::nunmai_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: nunmai desktop --build-only ----------------------------
    // `nunmai update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `nunmai update` already does above, and `nunmai update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `nunmai desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Nunmai manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_nunmai_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Nunmai to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Nunmai.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Nunmai still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_nunmai(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_nunmai(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Nunmai.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Nunmai.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// User-facing message for an exit-2 refusal from `nunmai update`.
///
/// Exit 2 has several causes (another update holds the marker, a live
/// nunmai.exe or venv holder, a self-mapped `.pyd` after the code swap), and
...
fn concurrent_update_message(stdout_tail: &[String]) -> String {
    match stdout_tail.iter().rposition(|l| l.trim_start().starts_with('✗')) {
        ...
        None => "Nunmai is still running. Close all Nunmai windows and try \
                 the update again."
            .to_string(),
    }
}

/// Spawn `nunmai <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `nunmai update` is precisely the shape that
...
/// Path to the venv nunmai shim under an install root, regardless of existence.
fn venv_nunmai(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("nunmai.exe")
    } else {
        install_root.join("venv").join("bin").join("nunmai")
    }
}

/// Resolve the nunmai CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `nunmai` on PATH.
fn resolve_nunmai(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_nunmai(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "nunmai.exe" } else { "nunmai" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let nunmai_home = crate::paths::nunmai_home();
    let mut envs = vec![(
        "NUNMAI_HOME".to_string(),
        nunmai_home.as_os_str().to_os_string(),
    )];
    // `nunmai update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `nunmai update` child claims that SAME lock (nunmai_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Nunmai is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in nunmai_cli/update_lock.py.
    envs.push((
        "NUNMAI_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        nunmai_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_nunmai_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Nunmai.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.nunmai-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.nunmai-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::nunmai_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_nunmai_is_under_install_root() {
        let root = Path::new("/x/nunmai-engine");
        let shim = venv_nunmai(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/nunmai-engine"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/nunmai-engine"));
        assert!(
            envs.iter().any(|(k, v)| k == "NUNMAI_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the nunmai update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_nunmai(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/nunmai-engine");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.nunmai\nunmai-engine\venv\scripts\NUNMAI.EXE"),
            Path::new(r"c:\users\tester\.nunmai\nunmai-engine\venv\Scripts\nunmai.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.nunmai\nunmai-engine\apps\desktop\Nunmai.exe"),
            Path::new(r"C:\Users\tester\.nunmai\nunmai-engine\venv\Scripts\nunmai.exe"),
        ));
    }
    ...
    fn concurrent_update_message_shows_the_childs_refusal_block() {
        ...
        let tail = lines(
            "→ Fetching updates...\n\
             ...
             ✗ Another Nunmai update is already running (started 3m 42s ago, process 65285).\n\
             \n  Wait for it to finish, then run `nunmai update` again.\n",
        );
        assert_eq!(
    ...
    fn concurrent_update_message_falls_back_without_a_refusal_block() {
        let generic = "Nunmai is still running. Close all Nunmai windows and try the update again.";
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".nunmai-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `nunmai update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(nunmaiHome, child.pid) races ahead
    ...
    // checkout's `nunmai update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".nunmai-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Nunmai, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Nunmai.app"]),
            Some(PathBuf::from("/Applications/Nunmai.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Nunmai.app");
        let tmp = base.join("Nunmai.app.nunmai-update-new");
        let old = base.join("Nunmai.app.nunmai-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Nunmai.app");
        let tmp = base.join("Nunmai.app.nunmai-update-new"); // intentionally absent
        let old = base.join("Nunmai.app.nunmai-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Nunmai.app");
        let tmp = base.join("Nunmai.app.nunmai-update-new"); // absent
        let old = base.join("Nunmai.app.nunmai-update-old");
        write_marker(&target, "OLD");
