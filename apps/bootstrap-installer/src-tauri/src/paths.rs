...
//!
//! Mirrors `nunmai_constants.get_nunmai_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\nunmai
//!   macOS:   ~/.nunmai
//!   Linux:   ~/.nunmai  (override via $NUNMAI_HOME)
//!
//! NOTE (macOS): Python's get_nunmai_home(), scripts/install.sh, and the
//! Electron desktop's resolveNunmaiHome() ALL use ~/.nunmai on macOS — there
...
/// Returns the canonical Nunmai home directory, respecting $NUNMAI_HOME if set.
pub fn nunmai_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("NUNMAI_HOME") {
    ...
    {
        // %LOCALAPPDATA%\nunmai — matches scripts/install.ps1's $NunmaiHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("nunmai");
        }
    }

    // macOS + Linux + fallback: ~/.nunmai (matches Python get_nunmai_home(),
    // install.sh, and the Electron desktop's resolveNunmaiHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".nunmai");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    nunmai_home().join("bootstrap-cache")
}
...
/// NUNMAI_HOME so it survives repo checkout deletion (unlike anything under
/// nunmai-engine/).
///
/// On Windows this is `%LOCALAPPDATA%\nunmai\nunmai-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_nunmai`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under NUNMAI_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves NUNMAI_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    nunmai_home().join(".nunmai-update-in-progress")
}
...
pub fn copy_self_to_nunmai_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to NUNMAI_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_NUNMAI_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".nunmai-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under NUNMAI_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[nunmai-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("NUNMAI_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_nunmai_home() -> String {
    nunmai_home().to_string_lossy().into_owned()
}
