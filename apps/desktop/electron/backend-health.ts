...
export interface NunmaiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForNunmaiReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Nunmai gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForNunmaiReady().
 *
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `Nunmai Cloud agent ${hostname} is down ` +
      ...
      'Check https://nunmai.in for status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also contact Nunmai at ai@nunmai.in for immediate assistance. ' +
 ...
 * `isReauthRequired` so startNunmai latches; a bare `needsOauthLogin` (the
...
export async function waitForNunmaiReady(baseUrl: string, options: NunmaiReadyOptions): Promise<void> {
  ...
  throw new Error(`Nunmai backend did not become ready: ${detail}`)
}
