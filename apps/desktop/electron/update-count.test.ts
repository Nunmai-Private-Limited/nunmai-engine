...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Nunmai Test')
    git('config', 'user.email', 'nunmai@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/Nunmai-Private-Limited/nunmai-engine.git',
      targetSha: SHA_B
    }),
