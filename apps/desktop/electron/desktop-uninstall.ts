 ...
 * the three user-facing uninstall modes to the `nunmai uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m nunmai_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `nunmai_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'nunmai_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Nunmai.app/Contents/MacOS/Nunmai  → …/Nunmai.app
 *   Windows: …\Nunmai\Nunmai.exe                 → …\Nunmai  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Nunmai.app/Contents/MacOS/Nunmai → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Nunmai.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Nunmai.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Nunmai$/i.test(dir) || /[\\/]nunmai-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/nunmai
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import nunmai_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, nunmaiHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export NUNMAI_HOME=${q(nunmaiHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import nunmai_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Nunmai install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "NUNMAI_HOME=${String(nunmaiHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
