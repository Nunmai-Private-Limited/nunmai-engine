...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'NUNMAI_DESKTOP_REMOTE_URL is set but NUNMAI_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nunmai backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `nunmai serve` the desktop never
