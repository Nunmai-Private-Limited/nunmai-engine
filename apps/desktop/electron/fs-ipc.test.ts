/**
 * `nunmai:fs:reveal` answers what it did (#115167). `shell.showItemInFolder`
...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-fs-ipc-'))
...
const reveal = (target: string) => electron.handlers.get('nunmai:fs:reveal')!({}, target)

afterEach(() => {
  electron.showItemInFolder.mockClear()
})

describe('nunmai:fs:reveal', () => {
