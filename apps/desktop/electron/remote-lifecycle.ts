...
const READY_RE = /^NUNMAI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
async function probeNunmaiVersion(ssh, nunmaiPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(nunmaiPath)} --version 2>&1`)).trim()
...
function buildSpawnCommand(nunmaiPath, profile, opts: any = {}) {
  ...
  const dashCmd =
    ...
    `exec env NUNMAI_DESKTOP=1 ${nunmai} ${profileArgs}${subCmd}`
...
async function spawnRemoteDashboard(
  ssh,
  { nunmaiPath, profile, token, ownershipId, nunmaiHome = '~/.nunmai', assertInstallClear = async () => {} }
) {
