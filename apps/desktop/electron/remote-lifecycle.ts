 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Nunmai dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Nunmai install on the remote (login-shell probe),
 ...
 *     `NUNMAI_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForNunmai in.
 *
 * The minted NUNMAI_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.nunmai/desktop-ssh'
...
// Resolve the remote nunmai executable. An EXPLICIT path is honored strictly
...
async function locateNunmai(ssh, remoteNunmaiPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The nunmai binary or wrapper script
  ...
  if (remoteNunmaiPath) {
    if (await isExecutable(remoteNunmaiPath)) {
      return resolveLauncher(remoteNunmaiPath)
    }

    const err: any = new Error(
      `The Nunmai path you set is not an executable on the remote host: "${remoteNunmaiPath}". ` +
        'Check the path (it must be the full path to the `nunmai` binary on the remote, e.g. ' +
        '~/nunmai-engine/.venv/bin/nunmai), or clear it to auto-detect.'
    )

    err.kind = 'nunmai-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v nunmai')}`)).trim()
  ...
  candidates.push('~/.local/bin/nunmai')
  candidates.push('/usr/local/bin/nunmai')
  candidates.push('~/.nunmai/nunmai-engine/venv/bin/nunmai')
  ...
  const err: any = new Error(
    'Nunmai is not installed on the remote host (could not find a `nunmai` executable). ' +
      'Install it on the remote with:  curl -fsSL https://nunmai-engine.nunmai.in/install.sh | sh  ' +
      '— or set the Nunmai path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<nunmai> --version`,
// e.g. "Nunmai Engine v0.18.2 ..."), or '' on failure. Surfaces WHICH nunmai a
// connection uses, so a stale/unexpected install is visible.
async function probeNunmaiVersion(ssh, nunmaiPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(nunmaiPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Nunmai Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The NUNMAI_HOME the remote dashboard will use (explicit env wins, else
// ~/.nunmai). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteNunmaiHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${NUNMAI_HOME:-$HOME/.nunmai}"')).trim().split('\n').pop()

    return out || '~/.nunmai'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Nunmai home.')
...
marker=home/'.nunmai-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Nunmai checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, nunmaiHome) {
  const home = assertSafeRemoteHome(nunmaiHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Nunmai install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Nunmai update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Nunmai update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteNunmaiProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteNunmaiHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Nunmai profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
async function readRemoteInstallId(ssh) {
  // The stable backend identity the roster collapses on (`nunmai_cli/install_identity.py`:
  ...
  const root = remoteInstallRoot(assertSafeRemoteHome(await probeRemoteNunmaiHome(ssh)))
  ...
  try {
    ...
    const error: any = new Error('Could not read the remote Nunmai install id.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Nunmai home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'nunmaiPath', 'nunmaiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !nunmaiPath) {
    return false
  }
  ...
  const script =
    ...
    `pid=${Number(pid)}\n` +
    `expected=os.path.expanduser(${shq(nunmaiPath)})\n` +
    // The installer-facing launcher is intentionally preserved for invocation
    // (#74411), but it may `exec python <install-dir>/nunmai`, leaving neither
    // launcher nor NUNMAI_HOME-derived entrypoint in argv. The ownership-scoped
    // token path + random nonce + exact profile below are the alternative proof.
    `nunmai_home=os.path.expanduser(${shq(nunmaiHome)}) if ${shq(nunmaiHome)} else ""\n` +
    'expected_entries={expected}\n' +
    'if nunmai_home:\n' +
    ' expected_entries.add(os.path.join(nunmai_home,"nunmai-engine","venv","bin","nunmai"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.nunmaiPath)
  const expectedHome = lock.nunmaiHome ? expandRemotePath(lock.nunmaiHome) : "''"
  ...
  const executableMatch = lock.nunmaiHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.nunmaiPath === expected.nunmaiPath &&
    lock.nunmaiHome === expected.nunmaiHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.nunmaiPath)})
nunmai_home=os.path.expanduser(${py(lock.nunmaiHome)})
expected_entries={expected_path,os.path.join(nunmai_home,"nunmai-engine","venv","bin","nunmai")}
...
// execing Nunmai. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"nunmai-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(nunmaiPath, profile, opts: any = {}) {
  const nunmai = expandRemotePath(nunmaiPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.nunmaiHome || '~/.nunmai')}/.nunmai-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.nunmaiHome || '~/.nunmai')}/.nunmai-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env NUNMAI_DESKTOP=1${opts.guestOnboarding === true ? ' NUNMAI_GUEST_ONBOARDING=1' : ''} ${nunmai} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} nunmai-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, nunmaiPath) {
  const nunmai = expandRemotePath(nunmaiPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${nunmai} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, nunmaiPath))) {
    const err: any = new Error(
      'The remote Nunmai install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Nunmai on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(nunmaiPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        nunmaiHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForNunmai, a token-gated
...
async function connect(deps) {
  ...
  const nunmaiHome = await probeRemoteNunmaiHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, nunmaiHome)
  const nunmaiPath = await locateNunmai(ssh, remoteNunmaiPath)
  log(`located nunmai at ${nunmaiPath}`)
  const nunmaiVersion = await probeNunmaiVersion(ssh, nunmaiPath)

  if (nunmaiVersion) {
    log(`remote nunmai version: ${nunmaiVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Nunmai Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.nunmaiPath === nunmaiPath &&
      lock.nunmaiHome === nunmaiHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    nunmaiHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, nunmaiHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForNunmai(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
