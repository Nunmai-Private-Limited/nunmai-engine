 ...
 *     `X-Nunmai-Session-Token` header; WS uses `?token=`.
...
// nunmai_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - nunmai_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - nunmai_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / nunmai #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-nunmai_session_at', '__Secure-nunmai_session_at', 'nunmai_session_at']
const RT_COOKIE_VARIANTS = ['__Host-nunmai_session_rt', '__Secure-nunmai_session_rt', 'nunmai_session_rt']

// Keep this aligned with nunmai_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['nunmai', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startNunmai latches
...
/** Which Nunmai profile the remote SSH dashboard should actually run as.
...
// Nunmai Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteNunmaiPath = String(entry.remoteNunmaiPath || '').trim()

  if (remoteNunmaiPath) {
    out.remoteNunmaiPath = remoteNunmaiPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Nunmai installation. Preserve an explicit mapping
  // when it is a valid Nunmai profile identifier; otherwise fall back to the
...
export interface ProfileRouteOptions {
  ...
  ownEntry?: boolean
  /** `NUNMAI_DESKTOP_ISOLATED_BACKEND=1`: opt out of the host singleton. */
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_nunmai_action` registers the (often dynamic, e.g.
 ...
 * `NUNMAI_HOME` — so it keeps a pooled, profile-scoped backend even though every
 ...
 *     (multiplex-only: one `nunmai serve` per HOST). The descriptor carries
 ...
 *     `NUNMAI_DESKTOP_ISOLATED_BACKEND=1`, which gives this app a private
 *     backend, and a MUTATING request the server cannot scope at all — that
 *     one keeps a pooled backend whose NUNMAI_HOME does the scoping, so a
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process NUNMAI_HOME can belong to a different
    ...
    // The same holds for the LOCAL host backend: with one `nunmai serve` per
  ...
  //    a `nunmai serve` child of its own — UNLESS this request mutates state
  //    the server cannot scope, in which case the pooled backend's own
  //    NUNMAI_HOME is the only scope left and it keeps one.
 ...
 * `nunmai:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://nunmai.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `nunmai:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a nunmai session ACCESS-token cookie
