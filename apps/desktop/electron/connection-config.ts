...
// The Nunmai portal (NAS) does NOT use Nunmai gateway session cookies — it is a
 ...
 *     `--profile`, so its `NUNMAI_HOME` scopes it.
 *
...
/**
 * True if the cookie jar holds a live Nunmai PORTAL (Privy) session — a non-empty
 ...
 * the Nunmai gateway session cookies, so cloud sign-in / discovery liveness
