...
// registry, and the nunmai:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // NUNMAI_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.NUNMAI_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Nunmai' TUI into
    ...
    env.TERM_PROGRAM = 'Nunmai'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a nunmai/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from NUNMAI_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.NUNMAI_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `nunmai:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('nunmai:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('nunmai:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('nunmai:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('nunmai:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('nunmai:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('nunmai:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
