// Attach to the host's running Nunmai backend (multiplex-only, Desktop half).
//
...
export function spawnLedgerPath(nunmaiHomeRoot: string, join: (...parts: string[]) => string): string {
  return join(nunmaiHomeRoot, SPAWN_LEDGER_FILENAME)
}
...
export async function attachToHostBackend(
  ...
  if (decision.action === 'spawn') {
    if (decision.reason === 'isolated') {
      deps.log('[attach] NUNMAI_DESKTOP_ISOLATED_BACKEND is set; spawning a dedicated backend')
    }
  ...
  for (const record of ordered) {
    ...
    if (attached) {
      deps.log(
        `[attach] attached to the running Nunmai backend on ${attached.baseUrl} ` +
