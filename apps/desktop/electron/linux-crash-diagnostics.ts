...
// launcher (.desktop entry, Omarchy's nunmai-desktop wrapper) discards stderr.
...
export interface CrashDiagnosticsHost {
  /** Create the logs directory. May throw (read-only or invalid NUNMAI_HOME). */
...
// a read-only or invalid NUNMAI_HOME/logs must degrade to "no crash log", never
