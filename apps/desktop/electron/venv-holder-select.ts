 ...
 *
 * The pre-update handoff kills Nunmai-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `nunmai`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Nunmai-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Nunmai and holds venv shims mapped).
 */
export function isNunmaiOwnedVenvDaemon(
