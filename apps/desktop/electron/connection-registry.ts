 ...
 * runtime, remote gateways, Nunmai Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@nunmai/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteNunmaiPath || '').trim() !== String(activeSsh.remoteNunmaiPath || '').trim() ||
...
/** Turn `ls ~/.nunmai/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Nunmai Cloud
 * instances are platform-managed — we never run `nunmai update` against them.
 ...
 */
/**
 * Auth mode a stored remote-shaped entry actually uses. A Nunmai Cloud gateway
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteNunmaiPath: input.remoteNunmaiPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Nunmai-managed names; an empty result stores nothing.
 ...
 * `remoteNunmaiPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Nunmai Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Nunmai Cloud' : 'Remote gateway')
