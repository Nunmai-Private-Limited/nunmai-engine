...
let home: string
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-tray-'))
  native.windows = []
  native.trays = []
  native.fail = false
  vi.clearAllMocks()
...
test('opt-in minimize and primary Close preserve windows while explicit Quit still exits', async () => {
  ...
  main.restore()
  await native.ipc.get('nunmai:minimize-to-tray:set')!(null, true)
  expect(native.ipc.get('nunmai:minimize-to-tray:get')!()).toEqual({ enabled: true, available: true })
  main.minimize()
