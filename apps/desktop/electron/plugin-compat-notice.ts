 ...
 *
 * The Python side (nunmai_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes NUNMAI_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(nunmaiHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(nunmaiHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(nunmaiHome: string, userData: string): PendingNotice | null {
  const report = readReport(nunmaiHome)
  ...
  const copy = report.in_effect
    ? {
        ...
        message: `These plugins were built for an older Nunmai and were turned off: ${nameList}. Nunmai works normally without them.`,
        detail: `Look for an updated version of each plugin or ask its author.\n\n${list}`
      }
    : {
        title: 'Some plugins need an update',
        message: `These plugins were built for an older Nunmai and will stop working on ${report.removal_date}: ${nameList}.`,
