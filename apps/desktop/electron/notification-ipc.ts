...
export function registerNativeNotifications({
  ...
  ipcMain.handle('nunmai:notify', async (event, payload: NunmaiNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('nunmai:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('nunmai:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('nunmai:notification-action', { sessionId: payload.sessionId, actionId: action.id })
