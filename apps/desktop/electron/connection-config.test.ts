...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  ...
  assert.equal(remoteRequestMatchesBaseUrl('ws://nunmai.example.com/api/ws', 'http://hermes.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://nunmai.example.com/other/api/ws', 'https://hermes.example.com/gateway'),
    false
  )
  assert.equal(
