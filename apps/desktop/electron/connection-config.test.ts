...
const ROUTES = [
  ...
  },
  {
    ...
    // profile-scope has only the backend PROCESS's NUNMAI_HOME left as a
  ...
  {
    name: 'NUNMAI_DESKTOP_ISOLATED_BACKEND keeps a local profile on its own pooled backend',
    profile: 'coder',
    opts: {
      primaryProfile: 'default',
      globalRemote: false,
...
test('resolveProfileApiRequest keeps an unscopable mutating route on a process-scoped backend', () => {
  ...
  // nothing left but the backend process's own NUNMAI_HOME, so it must NOT fall
