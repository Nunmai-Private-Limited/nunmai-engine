...
describe('resolveSubdirWithin', () => {
  it('rejects path traversal', () => {
    const root = mkdtemp('nunmai-plugin-root-')
...
describe('findDesktopEntry', () => {
  it('finds root plugin.js', () => {
    const root = mkdtemp('nunmai-plugin-detect-')
...
describe('detectPluginComponents', () => {
  ...
  it('detects agent-only layout', async () => {
    const root = mkdtemp('nunmai-plugin-agent-')
    roots.push(root)
  ...
  it('detects dual layout', async () => {
    const root = mkdtemp('nunmai-plugin-dual-')
    roots.push(root)
    fs.mkdirSync(path.join(root, 'desktop'), { recursive: true })
...
describe('probePluginRepo', () => {
  ...
  it('probes a monorepo subdirectory through the sparse partial clone', async () => {
    const repo = mkdtemp('nunmai-plugin-monorepo-')
    roots.push(repo)
    const git = (...args: string[]) => execFileSync('git', args, { cwd: repo, stdio: 'pipe' })
    const plugin = path.join(repo, 'integrations', 'nunmai')
    ...
    const result = await probePluginRepo('git', `${pathToFileURL(repo).href}#integrations/nunmai`)
