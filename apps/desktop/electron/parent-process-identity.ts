export type ParentWatchdogEnv = {
  NUNMAI_PARENT_PID: string
  NUNMAI_PARENT_START_MARKER?: string
  NUNMAI_PARENT_NONCE?: string
  /** Spawn tag consumed by nunmai_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { NUNMAI_PARENT_PID: String(pid) }
  env.NUNMAI_SPAWN = spawnTag(pid, startMarker)
  ...
  env.NUNMAI_PARENT_START_MARKER = startMarker
  env.NUNMAI_PARENT_NONCE = nonce

  return env
}

/** Build the `NUNMAI_SPAWN` tag mirrored by nunmai_cli/process_identity.py.
