...
export function markerPath(nunmaiHome) {
  return path.join(nunmaiHome, '.nunmai-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(nunmaiHome)
  let raw
 ...
 *
 * The Tauri-based nunmai-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(nunmaiHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(nunmaiHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Nunmai to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(nunmaiHome, opts)
