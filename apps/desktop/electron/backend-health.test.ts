...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForNunmaiReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForNunmaiReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Nunmai backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Nunmai backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Nunmai backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForNunmaiReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForNunmaiReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; a bare needsOauthLogin hint is not', () => {
  // The unsigned-in throw must set isReauthRequired so startNunmai latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Nunmai gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForNunmaiReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isNousCloudAgentUrl detects cloud agent hosts', () => {
  ...
  assert.equal(isNousCloudAgentUrl('https://nunmai.in'), false)
  assert.equal(isNousCloudAgentUrl('not-a-url'), false)
})

test('waitForNunmaiReady classifies a persistent cloud agent 503 as cloud-backend-down', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForNunmaiReady('https://ares-3009.agents.nousresearch.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
...
test('waitForNunmaiReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
