...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/nunmai-runner-test',
    sourceRepoRoot: null,
    nunmaiHome: '/tmp/nunmai-runner-test',
    logRoot: '/tmp/nunmai-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'nunmai-engine', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'nunmai-engine')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/nunmai-engine',
      nunmaiHome: '/tmp/nunmai'
    }),
    ['--dir', '/tmp/nunmai-engine', '--nunmai-home', '/tmp/nunmai', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/nunmai-engine', '--nunmai-home', '/tmp/nunmai', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/nunmai',
      nunmaiHome: '/tmp/home'
    }),
    ['--dir', '/tmp/nunmai', '--nunmai-home', '/tmp/home', '--branch', 'main']
  )
})
...
test('installer log lines reach the emitter without escape sequences; \\r redraws keep the last frame', () => {
  ...
  assert.equal(cleanInstallerLogLine('\u001b]0;nunmai\u0007Installing Nunmai'), 'Installing Nunmai')
