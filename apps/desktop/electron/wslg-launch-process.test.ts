...
beforeAll(async () => {
  fixtureDir = await mkdtemp(path.join(tmpdir(), 'nunmai-wslg-process-'))
  await build({
    entryPoints: [
  ...
  await writeFile(
    ...
    const fd = Number(process.env.NUNMAI_DESKTOP_READY_FD)
    ...
    console.log(JSON.stringify({ kind: 'launcher', pid: process.pid,
      ...
      readyEnv: process.env.NUNMAI_DESKTOP_READY_FD
    }))
  ...
  const launcher = spawn(
    ...
    [...(inspect ? ['--inspect=127.0.0.1:0'] : []), path.join(fixtureDir, 'launcher.mjs')],
    {
      ...
      env: {
        ...process.env,
        NODE_OPTIONS: '',
        NUNMAI_DESKTOP_READY_FD: readyFd,
