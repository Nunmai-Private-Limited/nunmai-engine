...
Nunmai Python environment (including the serve dependencies):

```sh
NUNMAI_TEST_REAL_SERVE=1 \
NUNMAI_TEST_PYTHON=/absolute/path/to/venv/bin/python \
NUNMAI_TEST_ELECTRON=/absolute/path/to/native/electron \
node ../../node_modules/vitest/vitest.mjs run \
  --config electron/pool-retirement-live-fixture/vitest.config.ts
```

`NUNMAI_TEST_ELECTRON` is optional when Electron resolves from the desktop/root
...
- A hidden, sandboxed native renderer imports the real `src/store/gateway.ts` and
  `NunmaiGateway`. It opens four actual WebSockets (legacy and registry-local scopes
...
- Cleanup waits for every owned backend exit and verifies the coordinator has zero
  live leases. The temporary HOME, per-child NUNMAI_HOME, Electron userData,
