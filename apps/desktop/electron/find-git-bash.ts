 ...
 *   1. NUNMAI_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\nunmai\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect NUNMAI_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.NUNMAI_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'nunmai', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'nunmai', 'git', 'usr', 'bin', 'bash.exe'))
  }
