...
test('the native preload exposes routed peer opening and default preference events with unsubscribe', async () => {
  ...
  assert.equal(name, 'nunmaiDesktop')
  const route = { connectionId: 'remote-work', profile: 'work' }

  await bridge.openWindow(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['nunmai:window:openInstance', route])
  await bridge.openWindow()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['nunmai:window:openInstance', undefined])
  await bridge.profile.setDefault(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['nunmai:profile:default:set', route])
  await bridge.profile.getDefault()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['nunmai:profile:default:get'])
  bridge.setF12ShortcutActive(true)
  assert.equal(electron.ipcRenderer.send.mock.lastCall?.[0], 'nunmai:f12ShortcutActive')
  ...
  assert.equal(shortcutChannel, 'nunmai:f12-shortcut')
  ...
  assert.equal(channel, 'nunmai:profile:default:changed')
  listener({}, route)
  listener({}, null)
