 ...
 *
 * The staged Tauri `nunmai-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `nunmai update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `nunmai update`, swaps/relaunches
 * the app, and writes .nunmai-update-result.json. With the app gone before
 * the update starts, the NUNMAI_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `nunmai update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into NUNMAI_HOME on *every* platform
 * (`nunmai-setup.exe` on Windows, `nunmai-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_nunmai_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `nunmai update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(nunmaiHome, 'nunmai-setup.exe')
 ...
 *
 * `copy_self_to_nunmai_home` deliberately no-ops during `--update`
 ...
 * "Another Nunmai update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `nunmai update`, which
 ...
 * the installer is written to NUNMAI_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and Nunmai keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and Nunmai keeps running as before. " +
