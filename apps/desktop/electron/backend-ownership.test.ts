...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'nunmai serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/nunmai serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m nunmai_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/nunmai --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Nunmai Runtime\\nunmai.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('nunmai chat --query serve'), false)
