...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', NUNMAI_HOME: '/home/b/.nunmai' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'nunmai_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.nunmai/nunmai-engine/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.nunmai/nunmai-engine' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.nunmai/nunmai-engine'`,
      `exec '/home/b/.nunmai/nunmai-engine/venv/bin/python' '-m' 'nunmai_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
