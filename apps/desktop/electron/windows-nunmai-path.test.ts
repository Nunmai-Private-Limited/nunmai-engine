// Unit tests for the pure Windows `nunmai` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvNunmaiCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `nunmai` shim shadows
//      the real nunmai.cmd/nunmai.exe.
...
//   3. resolveVenvNunmaiCommand() — must probe the venv python via
//      canImportNunmaiCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvNunmai: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvNunmai: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvNunmai: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvNunmai: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvNunmai: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvNunmai: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvNunmaiCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportNunmaiCli: async () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    nunmaiHome: '/fake/nunmai-home',
...
test('resolveVenvNunmaiCommand: returns null off Windows', async () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(await resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null for a .cmd/.bat script command', async () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(await resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.cmd', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the basename is not nunmai/nunmai.exe', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvNunmaiCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the parent dir is not Scripts', async () => {
  const deps = makeDeps()

  assert.equal(await resolveVenvNunmaiCommand('/root/venv/bin/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the venv python does not exist on disk', async () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(await resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: probes the venv python before trusting it (returns null on failed probe)', async () => {
  let probed = false

  const deps = makeDeps({
    canImportNunmaiCli: async (python: string) => {
      probed = true
  ...
  const result = await resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', ['serve'], deps)
...
test('resolveVenvNunmaiCommand: returns the resolved python backend descriptor when the probe passes', async () => {
  const deps = makeDeps()
  const result = await resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'nunmai_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvNunmaiCommand: is case-insensitive on nunmai.exe and the Scripts dir name', async () => {
  const deps = makeDeps()

  assert.ok(await resolveVenvNunmaiCommand('/root/venv/Scripts/NUNMAI.EXE', [], deps))
  assert.ok(await resolveVenvNunmaiCommand('/root/venv/SCRIPTS/nunmai.exe', [], deps))
})
