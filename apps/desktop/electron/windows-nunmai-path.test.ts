...
test('resolveVenvNunmaiCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.cmd', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the basename is not nunmai/nunmai.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvNunmaiCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvNunmaiCommand('/root/venv/bin/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', [], deps), null)
})

test('resolveVenvNunmaiCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportNunmaiCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', ['serve'], deps)
...
test('resolveVenvNunmaiCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvNunmaiCommand('/root/venv/Scripts/nunmai.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvNunmaiCommand: is case-insensitive on nunmai.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvNunmaiCommand('/root/venv/Scripts/NUNMAI.EXE', [], deps))
  assert.ok(resolveVenvNunmaiCommand('/root/venv/SCRIPTS/nunmai.exe', [], deps))
})
