...
export function createIntroRevealWindowController({
  ...
  function spawnIntroRevealWindow() {
    ...
    win.on('closed', () => {
      ...
      if (main && !main.isDestroyed()) {
        main.webContents.send('nunmai:intro-reveal:closed')
      }
    })
  ...
  ipcMain.handle('nunmai:intro-reveal:open', (_event, payload?: IntroRevealOpenPayload) => openIntroReveal(payload))
  ipcMain.handle('nunmai:intro-reveal:close', (_event, payload?: IntroRevealClosePayload) => closeIntroReveal(payload))
  ipcMain.on('nunmai:intro-reveal:ready', event => {
    if (event.sender === introRevealWindow?.webContents) {
      introRevealShow?.reveal()
    }
  })
  ipcMain.on('nunmai:intro-reveal:skip', event => {
    const main = mainWindow()

    if (event.sender === introRevealWindow?.webContents && main && !main.isDestroyed()) {
      main.webContents.send('nunmai:intro-reveal:skip')
    }
  })
