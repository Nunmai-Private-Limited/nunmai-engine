...
/**
 * Nunmai-managed Node.js directories, in preferred lookup order.
 *
 ...
 * `$NUNMAI_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_nunmai_node_dirs()` in nunmai_constants.py, which the Electron main
 * process cannot import.
 */
function nunmaiManagedNodePathEntries(
  nunmaiHome,
  ...
  const root = pathModule.join(nunmaiHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const nunmaiNodeDirs = nunmaiManagedNodePathEntries(nunmaiHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([nunmaiNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function resolveNunmaiHomePath(nunmaiHome, { pathModule, homedir = os.homedir() }: any) {
  // fish (and any shell when the value is quoted) hands a literal `~` through; path.resolve()
  // would pin it under cwd and the Python backend inherits that absolute path via NUNMAI_HOME.
  let raw = String(nunmaiHome)
...
function normalizeNunmaiHomeRoot(
  nunmaiHome,
  ...
  const resolved = resolveNunmaiHomePath(nunmaiHome, { pathModule, homedir })
...
/**
 * Parent env for a local `nunmai serve` child of `profile` (#68367).
 *
 * `nunmai desktop` loads its launch profile's `.env`/`.op.env` into os.environ
 ...
 * `active_profile` like a bare `nunmai serve` (`_apply_profile_override`).
 */
function profileBackendParentEnv({
  ...
  const inheritedHome = currentEnv?.NUNMAI_HOME ? resolveNunmaiHomePath(currentEnv.NUNMAI_HOME, { pathModule }) : null
  const launchHome = inheritedHome && isProfileHome(inheritedHome, pathModule) ? inheritedHome : nunmaiHome
  const name = profile || readTextOrEmpty(fsModule, pathModule.join(nunmaiHome, 'active_profile')).trim()
  const targetHome = !name || name === 'default' ? nunmaiHome : pathModule.join(nunmaiHome, 'profiles', name)
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). nunmai_bootstrap sets this inside the child too, but only
