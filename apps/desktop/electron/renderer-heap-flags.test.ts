...
// that never go through the `nunmai desktop` launcher.
...
describe('renderer heap flags', () => {
  ...
  it('merges with a js-flags switch already on argv instead of overwriting it', () => {
    ...
    const planned = planLaunchSwitches(cfg, ['/app/nunmai', '--js-flags=--expose-gc', '--disable-gpu'])
    ...
    expect(planLaunchSwitches(cfg, ['/app/nunmai', '--js-flags', '--max-old-space-size=4096'])).toEqual([
      { name: 'disable-gpu' },
