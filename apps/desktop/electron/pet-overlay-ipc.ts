...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('nunmai:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('nunmai:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('nunmai:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('nunmai:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('nunmai:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('nunmai:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('nunmai:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('nunmai:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('nunmai:pet-overlay:control', payload)
  })
}
