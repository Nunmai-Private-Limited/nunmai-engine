...
function withTempDir(run: (dir: string) => void) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-secret-file-'))
...
test('attachment data URL helper reads bytes above the preview default without changing that limit', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-large-attachment-'))
...
test('resolveRequestedPathForIpc resolves relative paths from the trimmed base directory', () => {
  const baseDir = path.join(os.tmpdir(), 'nunmai-desktop-base')
...
test('resolveReadableFileForIpc validates existence type size and sensitivity', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-hardening-'))
...
test('resolveReadableFileForIpc blocks common sensitive files', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-sensitive-'))
...
test('resolveReadableFileForIpc blocks symlinks whose realpath is sensitive', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-realpath-'))
...
test('resolveDirectoryForIpc accepts directories and rejects invalid directory targets', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-dir-'))
...
test('resolveDirectoryForIpc accepts directory symlinks or junctions', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-dir-link-'))
...
test('homeRelativeAttachmentCandidates tries the home dir and the NUNMAI_HOME attachments dir', () => {
  const candidates = homeRelativeAttachmentCandidates(
    'AppData/Local/nunmai/attachments/foo.xlsx',
    '/Users/alice',
    '/Users/alice/AppData/Local/nunmai'
  )

  assert.deepEqual(candidates, [
    path.join('/Users/alice', 'AppData/Local/nunmai/attachments/foo.xlsx'),
    path.join('/Users/alice/AppData/Local/nunmai', 'attachments', 'foo.xlsx')
  ])
})
...
test('homeRelativeAttachmentCandidates normalizes Windows backslashes before joining', () => {
  const candidates = homeRelativeAttachmentCandidates(
    'AppData\\Local\\nunmai\\attachments\\foo.xlsx',
    '/Users/alice',
    '/Users/alice/.nunmai'
  )

  assert.equal(candidates[0], path.join('/Users/alice', 'AppData/Local/nunmai/attachments/foo.xlsx'))
})

test('homeRelativeAttachmentCandidates returns nothing for an absolute path', () => {
  assert.deepEqual(
    homeRelativeAttachmentCandidates('/already/absolute/foo.xlsx', '/Users/alice', '/Users/alice/.nunmai'),
    []
  )
})
...
test('homeRelativeAttachmentCandidates returns nothing for a file: URL', () => {
  assert.deepEqual(
    homeRelativeAttachmentCandidates('file:///already/resolved/foo.xlsx', '/Users/alice', '/Users/alice/.nunmai'),
    []
  )
})

test('homeRelativeAttachmentCandidates returns nothing for empty input', () => {
  assert.deepEqual(homeRelativeAttachmentCandidates('', '/Users/alice', '/Users/alice/.nunmai'), [])
  assert.deepEqual(homeRelativeAttachmentCandidates('   ', '/Users/alice', '/Users/alice/.nunmai'), [])
})
...
test('homeRelativeAttachmentCandidates second candidate falls back to basename only', () => {
  ...
  const candidates = homeRelativeAttachmentCandidates('foo.xlsx', '/Users/alice', '/Users/alice/.nunmai')

  assert.deepEqual(candidates, [
    path.join('/Users/alice', 'foo.xlsx'),
    path.join('/Users/alice/.nunmai', 'attachments', 'foo.xlsx')
  ])
})
