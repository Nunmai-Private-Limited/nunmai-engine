...
import type { NunmaiNotification } from './notification-types'
...
function setup(alreadyRunning = false) {
  ...
  const notify = (payload: NunmaiNotification) =>
    Promise.resolve(
...
it('bounds failed delivery without dropping older callbacks, and retries only on a later request', async () => {
  ...
  expect(h.source.webContents.send).toHaveBeenCalledWith('nunmai:focus-session', 'old-session')
...
it('releases naturally closed notifications while retaining other click targets', async () => {
  ...
  expect(h.source.webContents.send).toHaveBeenCalledWith('nunmai:focus-session', 'active-session')
...
it('preserves activation, dedupe and source ownership while fencing daemon ID reuse', async () => {
  ...
  expect(h.source.webContents.send).toHaveBeenCalledWith('nunmai:notification-action', {
    sessionId: 'runtime',
    actionId: 'reject'
  })
  ...
  expect(h.primary.webContents.send).toHaveBeenCalledWith('nunmai:notification-activate', {
    activate: '/plugin',
    notifyId: undefined,
    tag: 'plugin'
  })
  ...
  expect(reused.source.webContents.send).toHaveBeenCalledWith('nunmai:focus-session', 'new-session')
