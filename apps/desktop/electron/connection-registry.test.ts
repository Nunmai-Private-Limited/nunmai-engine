...
test('primary SSH reuse rejects a descriptor with a different remote Nunmai path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteNunmaiPath: '/srv/nunmai', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'nunmai-vps', kind: 'ssh', label: 'Nunmai VPS', host: 'nunmai-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'nunmai-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'nunmai-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'nunmai-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteNunmaiPath: '/opt/nunmai' },
  ...
  assert.equal(resolve({ ...base, remoteNunmaiPath: '/opt/nunmai' }), 'ssh-path')
...
// The electron and @nunmai/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@nunmai/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'nunmai-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('a cloud entry is saved as oauth even when the payload says token (#89529)', () => {
  ...
  for (const authMode of [undefined, 'token'] as const) {
    const cloud = normalizeConnectionInput(
      { kind: 'cloud', label: 'C', url: 'https://c.nunmai.cloud', authMode, token: { enc: 'x' } },
      emptyRegistry()
    )
...
test('a stored cloud entry left on token auth with no token reads back as oauth (#89529)', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'cloud-bare', kind: 'cloud', label: 'Bare cloud', url: 'https://a.nunmai.cloud', authMode: 'token' },
      {
        id: 'cloud-keyed',
        kind: 'cloud',
        label: 'Keyed cloud',
        url: 'https://b.nunmai.cloud',
        authMode: 'token',
        token: { v: 1 }
      },
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteNunmaiPath, '/opt/nunmai')
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'nunmai-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'nunmai-alex')
  assert.equal(applied.lastUsed, 'nunmai-alex')
})
