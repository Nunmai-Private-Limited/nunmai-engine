...
export function installHudModifierTap({
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('nunmai:hud-modifier:status', status())
      }
    }
  }
  ...
  for (const [name, handler] of Object.entries(handlers)) {
    ipcMain.handle(`nunmai:hud-modifier:${name}`, async (event, value) => {
      if (!trusted(event)) {
  ...
  const dispose = () => {
    disposed = true
    stop()
    Object.keys(handlers).forEach(name => ipcMain.removeHandler(`nunmai:hud-modifier:${name}`))
