 ...
 * Opt in with NUNMAI_TEST_REAL_SERVE=1; see pool-retirement-live-fixture/README.md.
 */
...
function isolatedEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    NUNMAI_DESKTOP_USER_DATA_DIR: join(root, 'user-data')
  }
}
...
test.skipIf(process.env.NUNMAI_TEST_REAL_SERVE !== '1' || process.platform === 'win32')(
  'native retirement preserves backend-only cron and hands both waiters capacity only after real child exit',
  async () => {
    const python = process.env.NUNMAI_TEST_PYTHON
    assert.ok(python && existsSync(python), 'Set NUNMAI_TEST_PYTHON to an installed Nunmai Python environment')
    const electron = process.env.NUNMAI_TEST_ELECTRON || (require('electron') as string)
    assert.ok(existsSync(electron), 'NUNMAI_TEST_ELECTRON must name a real native Electron executable')
    const root = mkdtempSync(join(tmpdir(), 'nunmai-pool-retirement-live-'))
    const resultPath = join(root, 'result.json')
    let child: ChildProcess | undefined
    let output = ''

    try {
      mkdirSync(join(root, '.nunmai'))
      ...
      await build({
        ...
        platform: 'browser',
        format: 'iife',
        alias: { '@': join(desktop, 'src'), '@nunmai/shared': join(repo, 'apps/shared/src') },
