 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `nunmai update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `nunmai desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = escapeHtml(details.title || 'Nunmai couldn\u2019t start the desktop UI')
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>nunmai desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>nunmai desktop --force-build</code>, then restart the app.</p>
