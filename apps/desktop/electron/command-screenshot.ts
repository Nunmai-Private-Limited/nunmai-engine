...
export function installCommandScreenshot({ rendererUrl }: { rendererUrl: string }): () => void {
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('nunmai:screenshot:status', status())
      }
    }
  }
  ...
  const start = (requestPermission = false) => {
    ...
    monitor.start(
      window => {
        ...
        // Retain the last Nunmai chat window when another application takes focus.
        ...
        if (requestId) {
          recipient.webContents.send('nunmai:screenshot:request', requestId)
        }
      },
      result => {
  ...
  const handle = (name: string, callback: (event: IpcMainInvokeEvent, value: unknown) => unknown) => {
    const channel = `nunmai:screenshot:${name}`
  ...
  ipcMain.on('nunmai:screenshot:subscribe', onSubscribe)
  ...
  const dispose = () => {
    ...
    ipcMain.removeListener('nunmai:screenshot:subscribe', onSubscribe)
