// Multiplex-only, Desktop half: ONE `nunmai serve` per HOST serves every
...
// other producer of `nunmai serve` children: the per-profile backend pool.
...
//   * `NUNMAI_DESKTOP_ISOLATED_BACKEND=1` — the escape hatch that gives this
...
export interface HostBackendCollapseOptions {
  /** `NUNMAI_DESKTOP_ISOLATED_BACKEND=1`: this app wants its own backend. */
  isolated?: boolean
   ...
   * own `NUNMAI_HOME` is then the only scope there is, so the collapse must
...
export class SecondLocalBackendError extends Error {
  ...
  constructor(poolKey: string) {
    super(
      `Refusing to start a second local Nunmai backend for "${poolKey}": one backend serves every profile on this host. ` +
        'Set NUNMAI_DESKTOP_ISOLATED_BACKEND=1 for a private backend.'
    )
 ...
 * the one place a local `nunmai serve` child is actually started, so a future
