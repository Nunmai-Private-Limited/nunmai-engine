...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    nunmaiHome: 'C:\\Users\\andre\\.nunmai',
    python: 'C:\\Users\\andre\\.nunmai\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.nunmai-update-in-progress/)
...
test('every emitted PowerShell script keeps try blocks attached to their catch/finally handlers', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ...
    'C:\\Users\\alice\\.nunmai'
  )
  scripts.push(
    decode(atomicWindowsSpawnCommand({ nunmaiHome: 'C:\\Users\\alice\\.nunmai', python: 'C:\\py\\python.exe' })),
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command nunmai.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          nunmaiHome: 'C:\\Users\\alice\\.nunmai',
          nunmaiPath: 'C:\\Nunmai\\nunmai.exe',
          python: 'C:\\Nunmai\\python.exe'
        })
      }

      if (script.includes('.nunmai-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('nunmai_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.nunmai\\profiles\\research')
  assert.match(script, /\.nunmai-update-in-progress/)
...
test('Windows probe validates Nunmai and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $nunmaiHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Nunmai is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Nunmai is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Nunmai's\\python.exe" }, 'inspect', [
    'C:\\x y\\nunmai.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'nunmai_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Nunmai''s/)
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { nunmaiPath: lock.nunmaiPath, nunmaiHome: lock.nunmaiHome }
