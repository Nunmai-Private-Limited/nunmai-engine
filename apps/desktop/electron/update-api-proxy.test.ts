...
test('update API uses an HTTPS proxy only when the environment selects one', () => {
  clearProxyEnv()
  const url = 'https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/commits/main'
...
test('update API request reaches the configured HTTP CONNECT proxy', async () => {
  ...
  try {
    ...
    const status = await new Promise<number | undefined>((resolve, reject) => {
      https
        .get(
          'https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/commits/main',
          {
            agent: updateCheckAgent('https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/commits/main')
          },
