...
test('normalizeNunmaiHomeRoot expands a literal leading ~ against the home directory, not cwd', () => {
  assert.equal(
    normalizeNunmaiHomeRoot('~/.nunmai', { pathModule: path.posix, homedir: '/Users/test' }),
    '/Users/test/.nunmai'
  )
  assert.equal(
    normalizeNunmaiHomeRoot('~/.nunmai/profiles/oracle', { pathModule: path.posix, homedir: '/Users/test' }),
    '/Users/test/.nunmai'
  )
  assert.equal(
    normalizeNunmaiHomeRoot('~\\.nunmai', { pathModule: path.win32, homedir: 'C:\\Users\\test' }),
    'C:\\Users\\test\\.nunmai'
  )
  assert.equal(normalizeNunmaiHomeRoot('~', { pathModule: path.posix, homedir: '/Users/test' }), '/Users/test')
})
...
// `nunmai desktop` loads its launch profile's .env/.op.env into os.environ and
// hands that env to Electron; these cover what a profile backend inherits (#68367).
function withNunmaiRoot(files: Record<string, string>, run: (root: string) => void) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-profile-env-'))
...
test('a named profile backend does not inherit secrets the root .env/.op.env loaded into Desktop', () => {
  withNunmaiRoot(ROOT_SCOPE_FILES, root => {
...
test('the launch profile backend inherits the Desktop env unchanged', () => {
  withNunmaiRoot(ROOT_SCOPE_FILES, root => {
    for (const profile of ['default', null, undefined]) {
      assert.deepEqual(
        profileBackendParentEnv({ nunmaiHome: root, profile, currentEnv: ROOT_LAUNCHED_ENV, platform: 'linux' }),
        ROOT_LAUNCHED_ENV
      )
    }
  })
...
test('a primary backend without an explicit profile follows the sticky active_profile', () => {
  withNunmaiRoot({ ...ROOT_SCOPE_FILES, active_profile: 'urbot\n' }, root => {
    const env = profileBackendParentEnv({ nunmaiHome: root, profile: null, currentEnv: ROOT_LAUNCHED_ENV })
...
test('Desktop launched from a named profile keeps that profile out of the default backend', () => {
  withNunmaiRoot(
    ...
    root => {
      const currentEnv = {
        NUNMAI_HOME: path.join(root, 'profiles', 'work'),
        TLON_SHIP_CODE: 'work-code',
        OP_SERVICE_ACCOUNT_TOKEN: 'work-op',
        OPENAI_API_KEY: 'shell-key'
      }

      assert.deepEqual(profileBackendParentEnv({ nunmaiHome: root, profile: 'default', currentEnv }), {
        NUNMAI_HOME: currentEnv.NUNMAI_HOME,
        OPENAI_API_KEY: 'shell-key'
      })
      assert.deepEqual(profileBackendParentEnv({ nunmaiHome: root, profile: 'work', currentEnv }), currentEnv)
    }
  )
})

test('Windows matches profile homes and dotenv names case-insensitively', () => {
  const root = 'C:\\Users\\test\\AppData\\Local\\nunmai'
  ...
  const currentEnv = {
    NUNMAI_HOME: 'c:\\users\\test\\appdata\\local\\NUNMAI',
    Path: 'C:\\Windows',
    Telegram_Bot_Token: 'root-token'
  }

  const scoped = (profile: string) =>
    profileBackendParentEnv({ nunmaiHome: root, profile, currentEnv, platform: 'win32', fsModule })

  assert.deepEqual(scoped('default'), currentEnv)
  assert.deepEqual(scoped('urbot'), { NUNMAI_HOME: currentEnv.NUNMAI_HOME, Path: 'C:\\Windows' })
})
