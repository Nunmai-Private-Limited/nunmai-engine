...
// Relative, not `@nunmai/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Nunmai Desktop cannot save remote gateway tokens. ' +
        ...
        'or set NUNMAI_DESKTOP_REMOTE_URL and NUNMAI_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set NUNMAI_DESKTOP_REMOTE_URL and NUNMAI_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// NUNMAI_DESKTOP_REMOTE_* env vars.
 ...
 * "AppData/Local/nunmai/attachments/foo.xlsx" on Windows, or
 * ".nunmai/attachments/foo.xlsx" elsewhere) rather than relative to cwd
 * (#115609). Pure and DI-testable: takes `home`/`nunmaiHome` as arguments
 ...
 */
function homeRelativeAttachmentCandidates(raw, home, nunmaiHome) {
  ...
  return [path.join(home, normalized), path.join(nunmaiHome, 'attachments', path.basename(normalized))]
}
