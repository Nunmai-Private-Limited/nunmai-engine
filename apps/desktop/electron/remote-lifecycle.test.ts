...
test('readRemoteInstallId reports the INSTALL root id for a profile-pinned home', async () => {
  ...
  const pinned = fakeSsh([
    [/NUNMAI_HOME/, '/Users/zillajr/.nunmai/profiles/dixie\n'],
    [/cat .*install_id/, '0f8a1c2b3d4e5f60718293a4b5c6d7e8\n']
  ])

  assert.equal(await readRemoteInstallId(pinned), '0f8a1c2b3d4e5f60718293a4b5c6d7e8')
  assert.equal(
    pinned.calls.some(cmd => cmd.includes('/Users/zillajr/.nunmai/install_id')),
    true
  )
  assert.equal(
...
test.skipIf(process.platform === 'win32')(
  ...
  async t => {
    ...
    const dir = await mkdtemp(path.join(os.tmpdir(), 'nunmai-zsh-probe-'))

    try {
      const nunmai = path.join(dir, 'nunmai')
      await writeFile(nunmai, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

      const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

      assert.equal(await remoteSupportsSshOwnership(ssh, nunmai), true)
    } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Nunmai Engine v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeNunmaiVersion(versionSsh, '/x/nunmai'), 'Nunmai Engine v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/nunmai'), true)
