// The standalone desktop-plugin root (`<NUNMAI_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.nunmai-package.json` marker
...
export const PACKAGE_MARKER = '.nunmai-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.nunmai-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every nunmai home the app knows about locally: the default plus each profile. */
export async function localHomes(nunmaiHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(nunmaiHome, 'profiles'))

  return [nunmaiHome, ...profiles.map(name => path.join(nunmaiHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(nunmaiHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(nunmaiHome, 'profiles'))) {
    const scopedRoot = path.join(nunmaiHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(nunmaiHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(nunmaiHome)) {
