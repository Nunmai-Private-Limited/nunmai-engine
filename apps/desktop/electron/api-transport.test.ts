/**
 * Unit + live-transport tests for the Electron main process's Nunmai REST
...
describe('htmlResponseError', () => {
  ...
  it('does not blame credentials when the redirect only fixes the scheme or a trailing slash', () => {
    for (const [url, location] of [
      ...
      ['https://gateway.example.com/nunmai/api/health', 'https://gateway.example.com/nunmai/api/health/']
    ]) {
