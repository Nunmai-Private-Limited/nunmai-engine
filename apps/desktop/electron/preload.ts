...
const launchFlags = ipcRenderer.sendSync('nunmai:launch-flags')
...
contextBridge.exposeInMainWorld('nunmaiDesktop', {
  ...
  // Launch-flag fact: the Nunmai free tier is on for this launch
  // (NUNMAI_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only; the same
  // decision is stamped onto every backend the app spawns.
  guestOnboardingEnabled: launchFlags?.guestOnboarding === true,
  // Launch-flag fact: skip the first-run film (NUNMAI_SKIP_INTRO=1 or
  // --skip-intro). Rehearsal aid for the guided chat behind it.
  skipIntro: launchFlags?.skipIntro === true,
  getConnection: (profile, opts) => ipcRenderer.invoke('nunmai:connection', profile, opts),
  ...
  touchBackend: (profile, options) => ipcRenderer.invoke('nunmai:backend:touch', profile, options),
  getPoolLimits: () => ipcRenderer.invoke('nunmai:pool-limits:get'),
  setPoolLimits: limits => ipcRenderer.invoke('nunmai:pool-limits:set', limits),
  ...
  openWindow: (options?: DesktopProfileRoute) => ipcRenderer.invoke('nunmai:window:openInstance', options),
  ...
  windowControls: {
    ...
    minimize: () => ipcRenderer.send('nunmai:window-control', 'minimize'),
    toggleMaximize: () => ipcRenderer.send('nunmai:window-control', 'toggle-maximize'),
    close: () => ipcRenderer.send('nunmai:window-control', 'close')
  },
  wakeIndicator: {
  ...
  chatOnboarding: {
    grow: request => ipcRenderer.send('nunmai:chat-onboarding:grow', request),
    soloBoot: () => ipcRenderer.send('nunmai:chat-onboarding:solo-boot')
  },
  introReveal: {
    open: (payload?: { hideMain?: boolean }) => ipcRenderer.invoke('nunmai:intro-reveal:open', payload),
    close: (payload?: { showMain?: boolean }) => ipcRenderer.invoke('nunmai:intro-reveal:close', payload),
    skip: () => ipcRenderer.send('nunmai:intro-reveal:skip'),
    ready: () => ipcRenderer.send('nunmai:intro-reveal:ready'),
    onSkip: callback => {
      const listener = () => callback()

      ipcRenderer.on('nunmai:intro-reveal:skip', listener)

      return () => ipcRenderer.removeListener('nunmai:intro-reveal:skip', listener)
    },
    onClosed: callback => {
      const listener = () => callback()

      ipcRenderer.on('nunmai:intro-reveal:closed', listener)

      return () => ipcRenderer.removeListener('nunmai:intro-reveal:closed', listener)
    }
  },
  petOverlay: {
  ...
  hudModifier: {
    getSettings: () => ipcRenderer.invoke('nunmai:hud-modifier:settings:get'),
    setEnabled: enabled => ipcRenderer.invoke('nunmai:hud-modifier:settings:set', enabled),
    openPermissionSettings: () => ipcRenderer.invoke('nunmai:hud-modifier:permission'),
    onStatus: callback => {
      const listener = (_event: Electron.IpcRendererEvent, status: HudModifierStatus) => callback(status)
      ipcRenderer.on('nunmai:hud-modifier:status', listener)

      return () => ipcRenderer.removeListener('nunmai:hud-modifier:status', listener)
    }
  ...
  screenshot:
    process.platform === 'darwin'
      ? {
          getSettings: () => ipcRenderer.invoke('nunmai:screenshot:settings:get'),
          setEnabled: enabled => ipcRenderer.invoke('nunmai:screenshot:settings:set', enabled),
          openPermissionSettings: kind => ipcRenderer.invoke('nunmai:screenshot:permission', kind),
          capture: requestId => ipcRenderer.invoke('nunmai:screenshot:capture', requestId),
          onStatus: callback => {
            const listener = (_event, status) => callback(status)
            ipcRenderer.on('nunmai:screenshot:status', listener)

            return () => ipcRenderer.removeListener('nunmai:screenshot:status', listener)
          },
          onRequest: callback => {
            const channel = 'nunmai:screenshot:request'
            const listener = (_event, requestId) => callback(requestId)

            if (ipcRenderer.listenerCount(channel) === 0) {
              ipcRenderer.send('nunmai:screenshot:subscribe', true)
            }
            ...
            return () => {
              ...
              if (ipcRenderer.listenerCount(channel) === 0) {
                ipcRenderer.send('nunmai:screenshot:subscribe', false)
              }
            }
          }
        }
  ...
  profile: {
    getDefault: () => ipcRenderer.invoke('nunmai:profile:default:get'),
    setDefault: (route: DesktopProfileRoute) => ipcRenderer.invoke('nunmai:profile:default:set', route),
    onDefaultChanged: (callback: (route: DesktopProfileRoute | null) => void) => {
      ...
      ipcRenderer.on('nunmai:profile:default:changed', listener)

      return () => ipcRenderer.removeListener('nunmai:profile:default:changed', listener)
    },
  ...
  saveImageBuffer: (data, ext, name) => ipcRenderer.invoke('nunmai:saveImageBuffer', { data, ext, name }),
  capturePreview: payload => ipcRenderer.invoke('nunmai:capturePreview', payload),
  savePastedText: text => ipcRenderer.invoke('nunmai:savePastedText', { text }),
  ...
  minimizeToTray: {
    get: () => ipcRenderer.invoke('nunmai:minimize-to-tray:get'),
    set: on => ipcRenderer.invoke('nunmai:minimize-to-tray:set', on),
    onChanged: callback => {
      const listener = (_event, status) => callback(status)
      ipcRenderer.on('nunmai:minimize-to-tray:changed', listener)

      return () => ipcRenderer.removeListener('nunmai:minimize-to-tray:changed', listener)
    }
  },
  setDisableF12: blocked => ipcRenderer.send('nunmai:devtools:disable-f12', blocked),
  setF12ShortcutActive: active => ipcRenderer.send('nunmai:f12ShortcutActive', Boolean(active)),
  onF12Shortcut: callback => {
    const listener = (_event, input) => callback(input)
    ipcRenderer.on('nunmai:f12-shortcut', listener)

    return () => ipcRenderer.removeListener('nunmai:f12-shortcut', listener)
  },
  ...
  mcpOauth: {
    ...
    listen: () => ipcRenderer.invoke('nunmai:mcp-oauth:listen'),
    wait: (id, timeoutMs) => ipcRenderer.invoke('nunmai:mcp-oauth:wait', id, timeoutMs),
    cancel: id => ipcRenderer.invoke('nunmai:mcp-oauth:cancel', id)
  },
  ...
  reconcileDesktopPlugins: () => ipcRenderer.invoke('nunmai:fs:reconcileDesktopPlugins'),
  ...
  terminal: {
    attach: id => ipcRenderer.invoke('nunmai:terminal:attach', id),
  ...
  removeDesktopPlugin: payload => ipcRenderer.invoke('nunmai:plugin:removeDesktop', payload),
  ...
  onPoolBackendRetiring: callback => {
    const listener = (_event, payload) => callback(payload)
    ipcRenderer.on('nunmai:pool:retiring', listener)

    return () => ipcRenderer.removeListener('nunmai:pool:retiring', listener)
  },
  ...
  recycleBackend: profile => ipcRenderer.invoke('nunmai:backend:recycle', profile),
  ...
  relaunchApp: () => ipcRenderer.invoke('nunmai:app:relaunch'),
  getMachineProfile: () => ipcRenderer.invoke('nunmai:machine:profile'),
  ...
  updates: {
    check: opts => ipcRenderer.invoke('nunmai:updates:check', opts),
