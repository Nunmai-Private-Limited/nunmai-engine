...
test('decodingFileProbe rejects a missing file', () => {
  const missing = path.join(os.tmpdir(), `nunmai-icon-missing-${process.pid}.png`)
  assert.equal(decodingFileProbe(missing), false)
})

test('decodingFileProbe rejects a directory', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-icon-dir-'))
...
test('resolving a packaged icon never probes a path inside app.asar', () => {
  const resources = path.join('/opt', 'Nunmai', 'resources')
...
test('appIconCandidates keeps the documented precedence ladder', () => {
  const mac = appIconCandidates({
    isWindows: false,
    appRoot: '/Applications/Nunmai.app/Contents/Resources',
    unpackedPathFor: p => `${p}.unpacked`
  })

  assert.deepEqual(mac, [
    path.join('/Applications/Nunmai.app/Contents/Resources.unpacked', 'dist', 'apple-touch-icon.png'),
    path.join('/Applications/Nunmai.app/Contents/Resources', 'public', 'apple-touch-icon.png'),
    path.join('/Applications/Nunmai.app/Contents/Resources', 'dist', 'apple-touch-icon.png')
  ])
...
// styles to it. A packaged .app already carries the Nunmai icon; only dev runs
