...
import { buildDesktopBackendEnv, nunmaiManagedNodePathEntries, normalizeNunmaiHomeRoot } from './backend-env'
...
//
// Mirror the Python logs (nunmai_logging.py RotatingFileHandler, maxBytes x
...
async function waitForUpdateToFinish() {
  ...
  try {
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Nunmai update did not finish',
        `${result.message}\n\nDetails: ${path.join(NUNMAI_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function unwrapWindowsVenvNunmaiCommand(command, backendArgs) {
...
// (covers a bare `nunmai` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'nunmai_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `nunmai --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, NUNMAI_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveNunmaiBackend step 4); execFileSync of a .cmd without
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the nunmai-import
...
// #105855 was a run of GitHub outages that read as a Nunmai bug because the
// UI hid the cause.
...
function venvNunmaiShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nunmai.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nunmai')
}
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    preflightStateDb(NUNMAI_HOME, rememberLog)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Nunmai will keep running — try again, or run \`nunmai update\` from a terminal.`
...
function preflightStateDb(nunmaiHome, rememberLog) {
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      NUNMAI_HOME,
      NUNMAI_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithNunmaiManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
...
function resolveNunmaiBackend(backendArgs) {
  ...
  if (overrideRoot && isNunmaiSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nunmai source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isNunmaiSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nunmai source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (process.env.NUNMAI_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (nunmaiCommand) {
      const unwrapped = unwrapWindowsVenvNunmaiCommand(nunmaiCommand, backendArgs)
      ...
      if (shouldTrustNunmaiOverride(nunmaiOverride) || verifyNunmaiCli(nunmaiCommand, { shell: shellForProbe })) {
...
async function runEnsureRuntime(backend: any): Promise<any> {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nunmai bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NUNMAI_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any
    ...
    return ensureRuntime(resolveNunmaiBackend(backend.args))
  }
  ...
  if (!isNunmaiSourceRoot(ACTIVE_NUNMAI_ROOT)) {
    throw new Error(
      `Nunmai install at ${ACTIVE_NUNMAI_ROOT} is missing or incomplete. ` +
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nunmai on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Nunmai.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Nunmai venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Nunmai backend.'
                  )
                )
...
// hasLiveOauthSession, which looks for nunmai_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Nunmai Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Nunmai Cloud sign-in.'))
...
// a valid stored value (pins the root NUNMAI_HOME explicitly); null means "no
...
function resetNunmaiConnection({ soft = false } = {}) {
  ...
  const nunmaiProcess = backendConnectionState.invalidate()
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  const backend = await ensureRuntime(resolveNunmaiBackend(backendArgs))
  ...
  child.once('exit', (code, signal) => {
    rememberLog(`Nunmai backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
...
function startNunmai() {
  return localBackendLifecycle.start(runNunmaiStart)
}

async function runNunmaiStart() {
  ...
  const connectionPromise = (async () => {
    ...
    nunmaiProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nunmai backend exit (${signal || code})`)
      ...
      rememberLog(`Nunmai backend exited (${signal || code})`)
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => nunmaiProcess.exitCode === null && !nunmaiProcess.killed,
      rememberLog
    })
...
function createWindow() {
  ...
  startNunmai().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('nunmai:connection', async (_event, profile, extra) => {
...
ipcMain.handle('nunmai:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'nunmai:connection', keyed by the composite
...
ipcMain.on('nunmai:connection:active-route', (event, route) => {
...
ipcMain.handle('nunmai:backend:touch', async (_event, profile) => {
...
ipcMain.handle('nunmai:window:openInstance', async () => {
...
ipcMain.handle('nunmai:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveNunmaiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))
