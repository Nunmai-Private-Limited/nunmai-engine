...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NUNMAI_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NUNMAI_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (nunmai #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Nunmai gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Nunmai backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Nunmai backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function startNunmai() {
  ...
  const connectionPromise = (async () => {
    ...
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(nunmaiProcess, {
...
ipcMain.handle('nunmai:connection', async (_event, profile) => {
...
ipcMain.handle('nunmai:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('nunmai:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nunmai:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as nunmai://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('nunmai:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('nunmai:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('nunmai:updates:check', async () =>
