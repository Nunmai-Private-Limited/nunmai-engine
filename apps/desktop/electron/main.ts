...
import { isReauthRequiredError, waitForNunmaiReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import { canImportNunmaiCli, PROBE_TIMEOUT_MS, shouldTrustNunmaiOverride, verifyNunmaiCli } from './backend-probes'
...
import { isNunmaiOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvNunmaiCommand
} from './windows-nunmai-path'
import {
...
const USER_DATA_OVERRIDE = process.env.NUNMAI_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NUNMAI_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.NUNMAI_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with NUNMAI_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nunmai] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[nunmai] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. NUNMAI_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[nunmai] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[nunmai] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via NUNMAI_DESKTOP_PASSWORD_STORE, bridged by the
// `nunmai desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[nunmai] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[nunmai] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `nunmai desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Nunmai sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[nunmai] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[nunmai] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[nunmai] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[nunmai] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[nunmai] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('nunmai:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Nunmai burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nunmai] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[nunmai] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[nunmai] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nunmai] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NUNMAI_HOME — the user-facing root for everything Nunmai-related. Mirrors
// scripts/install.ps1's $NunmaiHome and scripts/install.sh's $NUNMAI_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nunmai (matches install.ps1)
//   macOS / Linux: ~/.nunmai (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nunmai directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nunmai yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NUNMAI_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NUNMAI_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nunmai / %LOCALAPPDATA%\nunmai.
function resolveNunmaiHome() {
  if (process.env.NUNMAI_HOME) {
    return normalizeNunmaiHomeRoot(process.env.NUNMAI_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'nunmai-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NUNMAI_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nunmai and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NUNMAI_HOME')

    if (fromRegistry) {
      return normalizeNunmaiHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nunmai')
    const legacy = path.join(app.getPath('home'), '.nunmai')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nunmai setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.nunmai')
}

const NUNMAI_HOME = resolveNunmaiHome()
...
// the `nunmai desktop` launcher's argv, so a packaged app opened from its
...
// processes only from the browser's pre-launch command line.
{
  ...
  try {
    desktopLaunchYaml = fs.readFileSync(path.join(NUNMAI_HOME, 'config.yaml'), 'utf8')
  } catch {
  ...
  for (const planned of planLaunchSwitches(readDesktopLaunchConfig(desktopLaunchYaml), process.argv.slice(1))) {
    ...
    console.log(
      `[nunmai] desktop launch switch from config.yaml: --${planned.name}${planned.value === undefined ? '' : `=${planned.value}`}`
    )
  }
}

function pathWithNunmaiManagedNode(...entries) {
  const managed = nunmaiManagedNodePathEntries(NUNMAI_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_NUNMAI_ROOT — the canonical mutable Nunmai install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NUNMAI_ROOT = path.join(NUNMAI_HOME, 'nunmai-engine')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NUNMAI_ROOT, 'venv')
...
// boots, going straight to `resolveNunmaiBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NUNMAI_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NUNMAI_ROOT, '.nunmai-bootstrap-complete')
...
// active-profile.json records which Nunmai profile the desktop launches its
// local backend as. When set, startNunmai() passes `nunmai --profile <name>
// dashboard …`, which deterministically pins NUNMAI_HOME (see
// _apply_profile_override in nunmai_cli/main.py) and bypasses the sticky
// ~/.nunmai/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nunmai_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = DESKTOP_PROFILE_NAME_RE
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// nunmaiDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NUNMAI_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nunmai_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NUNMAI_HOME, 'logs', 'desktop.log')
...
const BOOT_FAKE_MODE = process.env.NUNMAI_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.NUNMAI_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.NUNMAI_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nunmai free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (nunmai:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NUNMAI_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.NUNMAI_DESKTOP_APP_NAME || 'Nunmai'
...
// 'nunmai:native-theme' ('dark' | 'light' | 'system'); we pin
...
// nunmai:window-control IPC channel. See titleBarOverlayOptions.
...
// electron-builder derives from the build `appId` (com.nousresearch.nunmai) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.nunmai')
}

// Seed the native About panel with the live Nunmai version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `nunmai update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNunmaiVersion(),
  copyright: 'Copyright © 2026 Nunmai Research'
})
...
// counts startNunmai() calls that have not settled; `primaryRecoverySuppressed`
// is set by every intentional invalidate of the slot and cleared by the next
// startNunmai(), so the dying child's stale exit never respawns behind a
...
// startNunmai(); this pool only holds EXTRA profile
...
// The legacy NUNMAI_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.NUNMAI_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.NUNMAI_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Nunmai backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `nunmai:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.NUNMAI_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startNunmai() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning nunmai serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nunmai backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  nunmaiLog.push(...lines)

  if (nunmaiLog.length > 300) {
    nunmaiLog.splice(0, nunmaiLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nunmai-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('nunmai:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nunmai:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('nunmai:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_NUNMAI_ROOT
  })
}
...
// The Tauri updater writes NUNMAI_HOME/.nunmai-update-in-progress for the whole
...
// (`force_kill_other_nunmai`, taskkill /IM nunmai.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(NUNMAI_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--nunmai-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `nunmai update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Nunmai will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(NUNMAI_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      const handoffLogPath = path.join(NUNMAI_HOME, 'logs', 'desktop-update-handoff.log')
      ...
      // and the same reveal primitive as 'nunmai:logs:reveal'.
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Nunmai update',
          message: "Nunmai couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Nunmai to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `nunmai` — must not shadow `nunmai.cmd`/`nunmai.exe`.
...
async function unwrapWindowsVenvNunmaiCommand(command, backendArgs) {
  return resolveVenvNunmaiCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `nunmai serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(NUNMAI_HOME, rememberLog)
...
function isNunmaiSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nunmai_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.NUNMAI_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several Nunmai deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves NUNMAI_GIT_BASH_PATH
...
// normal NUNMAI_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nunmai', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nunmai', 'git', 'bin', 'git.exe'))
  }
...
function recentNunmaiLog() {
  return nunmaiLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nunmai root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NUNMAI_ROOT.
// NUNMAI_DESKTOP_NUNMAI_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NUNMAI_DESKTOP_NUNMAI_ROOT && path.resolve(process.env.NUNMAI_DESKTOP_NUNMAI_ROOT),
    !IS_PACKAGED && isNunmaiSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNunmaiSourceRoot(ACTIVE_NUNMAI_ROOT) ? ACTIVE_NUNMAI_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NUNMAI_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nunmai:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Nunmai can't update itself from inside the app. Download the latest version from the Nunmai website, " +
        `or reinstall Nunmai to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
      nunmaiRoot: updateRoot,
      branch
    }
  }
        ...
        headers: githubApiHeaders(
          {
            ...
            'User-Agent': 'nunmai-desktop-update-check'
          },
          token
        ),
        timeout: 10_000
...
// Windows that binary owns ALL repo mutation — running `nunmai update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(NUNMAI_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `nunmai update` can write
// fresh entry points. On Windows this is the file the running backend
// `nunmai.exe` holds open; on POSIX it's never mandatory-locked.
function venvNunmaiShimPath(updateRoot) {
  const venvDir = resolveVenvDir(updateRoot)

  return IS_WINDOWS ? path.join(venvDir, 'Scripts', 'nunmai.exe') : path.join(venvDir, 'bin', 'nunmai')
}
...
// Kill only Nunmai-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `nunmai`,
...
function killNunmaiOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isNunmaiOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Nunmai-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `nunmai.exe` that
// spawned its own grandchildren (a `nunmai` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Nunmai backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Nunmai backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.nunmaiBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Nunmai backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.nunmaiBackendIdentity
...
// `nunmaiProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nunmai.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nunmai
// REPL, a pty terminal, the gateway) keep `nunmai.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const nunmaiProcess = backendConnectionState.getProcess()
  ...
  // backend is `python.exe -m nunmai_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (nunmaiProcess && Number.isInteger(nunmaiProcess.pid)) {
    initialPids.push(nunmaiProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(nunmaiProcess, {
  ...
  // `nunmai gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvNunmaiShimPath(updateRoot), NUNMAI_HOME)

  // Reap Nunmai-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killNunmaiOwnedVenvDaemons(updateRoot)

  const shim = venvNunmaiShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentNunmaiProcess = backendConnectionState.getProcess()

        if (currentNunmaiProcess && Number.isInteger(currentNunmaiProcess.pid)) {
          stragglers.push(currentNunmaiProcess.pid)
        }
...
// `nunmai update`). Instead we spawn the staged Nunmai-Setup binary with
// --update and quit, so it can run `nunmai update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `nunmai update` → relaunch),
      ...
      // in: the NUNMAI_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `nunmai update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `nunmai desktop`, never the Tauri installer that self-copies
      // nunmai-setup.exe into NUNMAI_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `nunmai` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `nunmai update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `nunmai update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, nunmaiRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(NUNMAI_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Nunmai — this window will close and the updater will open. Don’t reopen Nunmai yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    await preflightStateDb(NUNMAI_HOME, rememberLog)
    ...
    // nunmai.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running nunmai, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Nunmai install open ' +
        '(a second Nunmai window or a terminal running nunmai?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvNunmaiShimPath(updateRoot))
      }
    ...
    // Nunmai processes running from this venv.  The updater normally refuses
    ...
    // `nunmai update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `nunmai update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          NUNMAI_HOME,
          NUNMAI_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithNunmaiManagedNode(venvBin)
        },
      ...
      // The `nunmai update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(NUNMAI_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Nunmai update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(NUNMAI_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(resolveVenvDir(updateRoot), IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNunmai = path.join(venvBin, IS_WINDOWS ? 'nunmai.exe' : 'nunmai')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Nunmai launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the nunmai
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.nunmai-bootstrap-complete')),
      hasVenvNunmai: fileExists(venvNunmai),
...
// Python-level pre-update snapshot inside `nunmai update`.
async function preflightStateDb(nunmaiHome, rememberLog) {
  const stateDbPath = path.join(nunmaiHome, 'state.db')
    ...
    if (stat.size > 100) {
      ...
      if (
        !(await readPreUpdateBackupEnabled(
          resolveNunmaiBackend(['config', 'get', 'updates.pre_update_backup', '--json']),
          nunmaiHome
        ))
      ) {
      ...
      const emergencyPath = path.join(nunmaiHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(nunmaiHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(nunmaiHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `nunmai update`, swaps/relaunches the app bundle, and writes
// .nunmai-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'nunmai update', percent: null })

    return { ok: true, manual: true, command: 'nunmai update', nunmaiRoot: updateRoot }
  }
  ...
  // A remote-served Desktop owns no local messaging gateway: `nunmai update
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      NUNMAI_HOME,
      NUNMAI_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithNunmaiManagedNode(path.join(resolveVenvDir(updateRoot), 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Nunmai — this window will close. Don’t reopen Nunmai yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_NUNMAI_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isNunmaiSourceRoot(ACTIVE_NUNMAI_ROOT) &&
    ...
    (await canImportNunmaiCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_NUNMAI_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    }))
  )
}
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `nunmai
...
function resolveWebDist() {
  const override = process.env.NUNMAI_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !fs.existsSync(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NUNMAI_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: nunmai desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nunmai desktop --force-build`
  )
...
function resolveNunmaiCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nunmai.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.NUNMAI_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNunmaiCwd(), sanitized: Boolean(trimmed) }
  }
...
async function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'nunmai_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_NUNMAI_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Nunmai at ${ACTIVE_NUNMAI_ROOT}`,
    command,
    args: ['-m', 'nunmai_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      nunmaiHome: NUNMAI_HOME,
      pythonPathEntries: [ACTIVE_NUNMAI_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NUNMAI_ROOT,
    bootstrap: true,
    shell: false
  }
}

async function resolveNunmaiBackend(backendArgs) {
  // 1. Explicit override -- NUNMAI_DESKTOP_NUNMAI_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NUNMAI_DESKTOP_NUNMAI_ROOT && path.resolve(process.env.NUNMAI_DESKTOP_NUNMAI_ROOT)

  if (overrideRoot && isNunmaiSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `Nunmai source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `nunmai` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNunmaiSourceRoot.)
  if (!IS_PACKAGED && isNunmaiSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Nunmai source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_NUNMAI_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\nunmai\\nunmai-engine (Windows) or ~/.nunmai/nunmai-engine.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Nunmai runtime at ${ACTIVE_NUNMAI_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `nunmai` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NUNMAI_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NUNMAI_DESKTOP_IGNORE_EXISTING !== '1') {
    let nunmaiCommand = null
    const nunmaiOverride = process.env.NUNMAI_DESKTOP_NUNMAI

    if (nunmaiOverride) {
      const resolvedOverride = findOnPath(nunmaiOverride)

      if (resolvedOverride) {
        nunmaiCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nunmaiOverride, { isWsl: IS_WSL })) {
        nunmaiCommand = nunmaiOverride
      } else {
        rememberLog(`Ignoring Windows Nunmai override under WSL: ${nunmaiOverride}`)
      }
    } else {
      nunmaiCommand = findOnPath('nunmai')
    }

    if (nunmaiCommand) {
      if (looksLikeDesktopAppBinary(nunmaiCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nunmai CLI: ${nunmaiCommand}`)
        nunmaiCommand = null
      }
    }

    if (nunmaiCommand) {
      const unwrapped = await unwrapWindowsVenvNunmaiCommand(nunmaiCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `nunmai` shim
      ...
      const shellForProbe = isCommandScript(nunmaiCommand)

      // NUNMAI_DESKTOP_NUNMAI is an explicit deployment override (used by
      ...
      if (
        shouldTrustNunmaiOverride(nunmaiOverride) ||
        (await verifyNunmaiCli(nunmaiCommand, { shell: shellForProbe }))
      ) {
        ...
        // unwrapWindowsVenvNunmaiCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Nunmai CLI at ${nunmaiCommand}`,
          command: nunmaiCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Nunmai CLI at ${nunmaiCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nunmai_cli module via system Python.
  ...
  if (python) {
    ...
    // having nunmai_cli installed -- common on dev boxes that have
    ...
    if (await canImportNunmaiCli(python)) {
      return {
        kind: 'python',
        label: `installed nunmai_cli module via ${python}`,
        command: python,
        args: ['-m', 'nunmai_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nunmai_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNunmaiBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nunmai Engine not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNunmaiBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nunmai install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Nunmai recovery was handed off to Nunmai Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nunmaiHome: NUNMAI_HOME,
      logRoot: path.join(NUNMAI_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nunmai install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startNunmai() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nunmai:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveNunmaiBackend(backend.args), assertStillOwned)
  }
  ...
  if (!isNunmaiSourceRoot(ACTIVE_NUNMAI_ROOT)) {
    throw new Error(missingInstallPartMessage(`Nunmai source files are missing or incomplete at ${ACTIVE_NUNMAI_ROOT}`))
  }

  // On Windows, preflight Git Bash. Nunmai' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\nunmai\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nunmai` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Nunmai needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Nunmai.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isNunmaiSourceRoot()
    // plus an importable nunmai_cli before it hands back the active runtime.
  ...
  backend.label = `Nunmai at ${ACTIVE_NUNMAI_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nunmai runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----nunmai-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Nunmai backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Nunmai backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Nunmai-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Nunmai-Session-Token`` header — used by the auth-mode probe before
...
const URL_EFFECTIVE_MARK = 'nunmai-url-effective:'
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nunmai:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNunmaiCwd()
  ...
  // (e.g. "AppData/Local/nunmai/attachments/foo.xlsx" on Windows, or
  // ".nunmai/attachments/foo.xlsx" elsewhere) rather than relative to the
  ...
  if (!fileExists(resolved) && !directoryExists(resolved)) {
    for (const candidate of homeRelativeAttachmentCandidates(raw, app.getPath('home'), NUNMAI_HOME)) {
      if (fileExists(candidate)) {
        resolved = candidate
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('nunmai:preview-file-changed', payload)
}
...
// waitForNunmaiReady how to read a 401 — rejected session vs gated route.
...
async function waitForNunmai(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForNunmaiReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('nunmai:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('nunmai:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('nunmai:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('nunmai:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nunmai...".
function sendPowerResume() {
  ...
  webContents.send('nunmai:power-resume')
}
...
// 'nunmai:power-battery' push below.
ipcMain.handle('nunmai:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nunmai:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(NUNMAI_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (nunmai://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${NUNMAI_PROTOCOL}://open/capabilities?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'nunmai:deep-link-ready' flushes it.
  ...
  webContents.send('nunmai:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('nunmai:window-state-changed', state)
}
  ...
  window.webContents.on('before-input-event', (event, input) => {
    ...
    if (input.key === 'F12') {
      ...
      if (decision === 'forward') {
        ...
        window.webContents.send('nunmai:f12-shortcut', toF12KeyboardEventPayload(input))
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('nunmai:context-menu-spellcheck', {
...
//
// Hosted Nunmai gateways gate the dashboard behind an OAuth provider (e.g.
// Nunmai Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``nunmai_session_at``),
...
// "not signed in", which on the initial boot path (startNunmai → the renderer's
// single-shot boot() with no retry) surfaced as the "Nunmai couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Nunmai
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Nunmai Cloud agent…' : 'Sign in to Nunmai gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nunmai backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in nunmai_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
function resolvePortalBaseUrl() {
  const raw = process.env.NUNMAI_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// Discover the hosted (Nunmai Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Nunmai Cloud. Open Settings → Gateway, choose Nunmai Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Nunmai Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Nunmai Cloud session has expired. Sign in to Nunmai Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Nunmai Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every nunmai:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteNunmaiPath`/`remoteProfile` —
  ...
  onDefaultChanged: route => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.webContents.isDestroyed()) {
        win.webContents.send('nunmai:profile:default:changed', route)
      }
    }
  }
})
...
// contains "nunmai", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isNunmaiProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('nunmai')
  } catch {
...
//   1. Legacy ~/.nunmai/active_profile (explicit CLI choice via nunmai profile use)
//   2. Running gateway (gateway.pid with verified liveness + nunmai identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(NUNMAI_HOME, 'active_profile'),
    nunmaiHome: NUNMAI_HOME,
    profilesRoot: path.join(NUNMAI_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NUNMAI_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.NUNMAI_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Nunmai Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteNunmaiPath: (ssh || savedSsh)?.remoteNunmaiPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by NUNMAI_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Nunmai Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Nunmai Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteNunmaiPath: input.sshRemoteNunmaiPath ?? existingBlock.remoteNunmaiPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Nunmai gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.NUNMAI_DESKTOP_REMOTE_TOKEN,
      url: process.env.NUNMAI_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      nunmaiPath: result.nunmaiPath,
      nunmaiHome: result.nunmaiHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { nunmaiPath: result.nunmaiPath, nunmaiHome: result.nunmaiHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteNunmaiPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteNunmaiPath: sshConfig.remoteNunmaiPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForNunmai: (baseUrl, token) => waitForNunmai(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds NUNMAI_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        nunmaiVersion: result.nunmaiVersion || '',
        ...
        // per-profile pools also use bare keys. Only startNunmai' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.nunmaiVersion || 'nunmai (version unknown)'} at ${result.nunmaiPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteNunmaiVersion: result.nunmaiVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (NUNMAI_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.NUNMAI_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startNunmai() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nunmai
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Nunmai Research" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteNunmaiPath: input.sshRemoteNunmaiPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteNunmaiPath || '')
          let nunmaiPath
          let nunmaiVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            nunmaiPath = runtime.nunmaiPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.nunmaiPath])
            nunmaiVersion = inspection.version
            supported = inspection.supported
          } else {
            nunmaiPath = await remoteLifecycle.locateNunmai(ssh, sshConfig.remoteNunmaiPath || '')
            nunmaiVersion = await remoteLifecycle.probeNunmaiVersion(ssh, nunmaiPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, nunmaiPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Nunmai on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteNunmaiPath: nunmaiPath,
            remoteNunmaiVersion: nunmaiVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNunmai())
  ...
  // connect to Nunmai gateway". Mirror the renderer's connect here so the test
...
function resetNunmaiConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startNunmai() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const nunmaiProcess = invalidatePrimaryConnection()
  stopBackendChild(nunmaiProcess)
...
// startNunmai() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetNunmaiConnection() invalidates it.
  const nunmaiProcess = backendConnectionState.getProcess()
  const dying = nunmaiProcess && !nunmaiProcess.killed ? nunmaiProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('nunmai:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nunmai:connections:changed', payload)
    }
  }
}
...
// startNunmai() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(
  ...
  if (route.backend === 'primary') {
    const connection = await startNunmai()
  ...
  // spawning, so two children never share one profile's NUNMAI_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteNunmaiPath: source.remoteNunmaiPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. nunmai:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: nunmaiLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForNunmai(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.nunmaiPath || !state?.nunmaiHome) {
  ...
  return {
    ...
    nunmaiPath: state.nunmaiPath,
    nunmaiHome: state.nunmaiHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteNunmaiPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.nunmaiPath ? platform : await probeWindowsRemote(ssh, config.remoteNunmaiPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          nunmaiPath: runtime.nunmaiPath,
          nunmaiHome: runtime.nunmaiHome,
          pythonPath: runtime.python
        }
      }
    }

    const nunmaiPath = await remoteLifecycle.locateNunmai(ssh, config.remoteNunmaiPath || '')
    const nunmaiHome = await remoteLifecycle.probeRemoteNunmaiHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, nunmaiPath, nunmaiHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { nunmaiPath: state.nunmaiPath, nunmaiHome: state.nunmaiHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForNunmai(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startNunmai() but without the boot-progress UI,
...
async function runPoolBackendStart(
  ...
  if (remote) {
    await waitForNunmai(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // Everything below starts a LOCAL `nunmai serve` child. Multiplex-only says
  ...
  // --profile wins over the inherited NUNMAI_HOME env (see _apply_profile_override
  // step 3 in nunmai_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveNunmaiBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertPoolEntryStillOwned(poolKey, entry)
  const nunmaiCwd = resolveNunmaiCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(NUNMAI_HOME, 'profiles', key))
  )
  rememberLog(`Starting Nunmai backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          ...profileBackendParentEnv({ nunmaiHome: NUNMAI_HOME, profile }),
          NUNMAI_HOME,
          ...backend.env,
          // Pin the gateway's tool/terminal cwd to the same directory we chose for
          // the child process. Inherited TERMINAL_CWD (or a stale config bridge)
          // can still point at the install dir even when spawn cwd is home.
          TERMINAL_CWD: nunmaiCwd,
          NUNMAI_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          NUNMAI_WEB_DIST: webDist,
          ...(readyFile ? { NUNMAI_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`Nunmai backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Nunmai backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Nunmai backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Nunmai backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForNunmai(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive,
    label: `Nunmai backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Nunmai backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function broadcastPoolBackendRetiring(poolKey: string) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nunmai:pool:retiring', { poolKey })
    }
  }
}
...
// backend whose ensure_nunmai_home() recreates the deleted profile directory.
//
...
const ISOLATED_BACKEND = process.env.NUNMAI_DESKTOP_ISOLATED_BACKEND === '1'
...
function startAttachedBackendMonitor(attached: AttachedBackend) {
  ...
  attachedBackendMonitor = setInterval(() => {
    void waitForNunmai(attached.baseUrl, attached.token, undefined, 'token', {}).catch(() => {
      stopAttachedBackendMonitor()
      rememberLog(`[attach] attached backend on ${attached.baseUrl} (pid ${attached.pid}) is gone; recovering`)
      invalidatePrimaryConnection()
      scheduleUnexpectedPrimaryRecovery({ error: 'The Nunmai backend this app attached to exited.', ready: true })
    })
...
function attachToRunningHostBackend(): Promise<AttachedBackend | null> {
  const options = { isolated: ISOLATED_BACKEND, ledgerPath: spawnLedgerPath(NUNMAI_HOME, path.join) }
...
function hostBackendAttachDeps() {
  return {
    ...
    waitForReady: (baseUrl: string, token: string) => waitForNunmai(baseUrl, token, undefined, 'token', {})
  }
}

function hostSpawnGatePath() {
  return path.join(NUNMAI_HOME, 'desktop-backend-spawn.json')
}
...
function startNunmai({ supervisorRecovery = false }: { supervisorRecovery?: boolean } = {}) {
  primaryRecoverySuppressed = false
  primaryStartsInFlight += 1

  const start = localBackendLifecycle.start(() => runNunmaiStart({ supervisorRecovery }))
...
function reportPrimaryRecoveryCrashLoop(code: number | null, signal: string | null): boolean {
  ...
  const message =
    'Nunmai backend keeps crashing right after it restarts; not restarting it again. Relaunch Nunmai Desktop.'
...
function runPrimaryRecoverySpawn(code: number | null, signal: string | null) {
  startNunmai({ supervisorRecovery: true }).catch(respawnError => {
    ...
    // releaseStart (startNunmai) already ran: same-promise reaction order, so
    // hasPendingStart is false here. See the ordering contract in startNunmai.
 ...
 * retry, so both the per-request short-circuit in runNunmaiStart and the
...
async function runNunmaiStart({ supervisorRecovery = false }: { supervisorRecovery?: boolean } = {}) {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Nunmai Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startNunmai() call re-throws the same error
  ...
  // latched (ensureBackend -> startNunmai), so a log line here would flood the
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Nunmai backend', 8)
  ...
  // later nunmai:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Nunmai backend at ${remote.baseUrl}`, 24)
      await waitForNunmai(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nunmai backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, nunmaiLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.nunmai/active_profile file) and
    // resolves NUNMAI_HOME the same way `nunmai -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Nunmai runtime', 28)

        return resolveNunmaiBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    if (setup.kind === 'attached') {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Attached to the running Nunmai backend',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.spawn', `Starting Nunmai backend via ${backend.label}`, 84)
    rememberLog(`Starting Nunmai backend via ${backend.label}`)
    ...
    const nunmaiProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: nunmaiCwd,
        env: desktopBackendSpawnEnv(
          {
            // Never another profile's dotenv credentials from the Desktop env (#68367).
            ...profileBackendParentEnv({ nunmaiHome: NUNMAI_HOME, profile: activeProfile }),
            // Explicitly pin NUNMAI_HOME for the child so Python's get_nunmai_home()
            // resolves to the SAME location our resolveNunmaiHome() picked. Without
            // this pin, Python falls back to ~/.nunmai on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\nunmai, which differs from C:\Users\<u>\.nunmai.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets NUNMAI_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(nunmaiProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(nunmaiProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, nunmaiProcess)

    if (!processOwner) {
      stopBackendChild(nunmaiProcess)
      await waitForBackendExit(nunmaiProcess)
      releaseBackendChild(nunmaiProcess)
      throw new Error('Nunmai backend start was superseded by a newer connection attempt.')
    }

    nunmaiProcess.stdout.on('data', rememberLog)
    nunmaiProcess.stderr.on('data', rememberLog)
    ...
    nunmaiProcess.once('error', error => {
      releaseBackendChild(nunmaiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nunmai backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Nunmai backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Nunmai backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nunmai backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nunmaiProcess.once('exit', (code, signal) => {
      releaseBackendChild(nunmaiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Nunmai backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Nunmai backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Nunmai backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nunmai backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNunmaiLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nunmai backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nunmai backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForNunmai(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    ...
    const childAlive = () => nunmaiProcess.exitCode === null && !nunmaiProcess.killed
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Nunmai backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nunmai backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed NUNMAI_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startNunmai() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `nunmai:openExternal`, not here. See window-open-policy.ts.
 ...
 * always carries the `persist:nunmai-preview` partition, so the partition is
 ...
 * audited `nunmai:openExternal` channel. Popup requests themselves stay
...
function installPreviewGuestPreload() {
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('will-attach-webview', (_attachEvent, webPreferences, params) => {
      if (params.partition !== 'persist:nunmai-preview') {
        return
      }
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('nunmai:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Nunmai is
...
// pushes pet state over IPC (nunmai:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via nunmai:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Nunmai app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Nunmai icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see nunmai:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  // leaves the sprite (nunmai:pet-overlay:ignore-mouse), but the window MUST
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Nunmai in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('nunmai:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function rehomePetOverlay() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('nunmai:pet-overlay:control', { type: 'bounds', bounds: resolved })
  }
}
...
// window showing only the composer and its scrollback, so Nunmai can be driven
...
// left the user with NO Nunmai window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('nunmai:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('nunmai:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[nunmai] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('nunmai:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `nunmai:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, nunmai:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('nunmai:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('nunmai:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('nunmai:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'nunmai desktop --force-build',
      ...
      // reclaim, an external kill). Nunmai never does this itself and never reloads it
      ...
      onRendererTerminated: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          title: 'Nunmai desktop UI was terminated',
          errorDescription:
  ...
  const startup = defaultRoute ? connectDesktopProfileRoute(defaultRoute) : startNunmai()
...
ipcMain.handle('nunmai:connection', async (event, profile, extra) => {
...
ipcMain.handle('nunmai:connection:for', async (_event, payload) => {
...
ipcMain.on('nunmai:connection:active-route', (event, route) => recordWindowConnectionRoute(event.sender, route))
...
// "Starting Nunmai…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nunmai:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetNunmaiConnection({ soft: true }),
...
ipcMain.handle('nunmai:backend:touch', async (_event, profile, options) => {
...
ipcMain.handle('nunmai:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('nunmai:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('nunmai:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('nunmai:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nunmai:window:openInstance', async (event, options) => {
...
ipcMain.handle('nunmai:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('nunmai:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveNunmaiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Nunmai is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `nunmai-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, NUNMAI_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('nunmai:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('nunmai:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('nunmai:zoom:get', event => {
...
ipcMain.on('nunmai:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nunmai:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('nunmai:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNunmai() call restarts the
...
ipcMain.handle('nunmai:bootstrap:repair', async () => {
  // Forceful repair: force the next startNunmai() through the full installer
  ...
  // "bypass active runtime" path inside startNunmai() and fall through
...
ipcMain.handle('nunmai:boot-progress:get', async () => bootProgressState)
ipcMain.handle('nunmai:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('nunmai:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('nunmai:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('nunmai:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('nunmai:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('nunmai:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('nunmai:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('nunmai:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('nunmai:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('nunmai:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('nunmai:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('nunmai:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('nunmai:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nunmai:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('nunmai:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteNunmaiPath: entry.remoteNunmaiPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteNunmaiPath: connection.remoteNunmaiPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteNunmaiProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root NUNMAI_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('nunmai:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// nunmai:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('nunmai:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('nunmai:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `nunmai update` to every eligible registered connection at once.
...
ipcMain.handle('nunmai:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/nunmai/update', {}, { timeoutMs: 15_000 })
...
// analogue of the nunmai:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('nunmai:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('nunmai:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startNunmai() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('nunmai:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Nunmai Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('nunmai:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('nunmai:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nunmai:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nunmai:profile:default:get', async () => desktopProfilePreferences.getDefault())
ipcMain.handle('nunmai:profile:default:set', async (_event, route) => desktopProfilePreferences.setDefault(route))
ipcMain.handle('nunmai:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of nunmai:profile:set: records the profile the
...
ipcMain.handle('nunmai:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('nunmai:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NUNMAI_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nunmai:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.on('nunmai:f12ShortcutActive', (event, active) => {
  if (active) {
...
ipcMain.handle('nunmai:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nunmai:window:readBelow', async event => {
...
async function handleNunmaiApiRequest(request) {
  ...
  // backend calls ensure_nunmai_home() which recreates the profile directory,
  ...
  // NUNMAI_HOME is then the scope, so a destructive call can never fall
...
ipcMain.handle('nunmai:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its NUNMAI_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleNunmaiApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleNunmaiApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('nunmai:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('nunmai:data-url-read-max:get', () => ({
...
ipcMain.handle('nunmai:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('nunmai:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nunmai:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('nunmai:readFileText', async (_event, filePath) => {
...
// `nunmai:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('nunmai:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('nunmai:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('nunmai:writeClipboard', (_event, text) => {
...
ipcMain.handle('nunmai:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('nunmai:readClipboard', () => clipboard.readText())

ipcMain.handle('nunmai:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('nunmai:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('nunmai:context-menu:edit', (event, command) => {
...
ipcMain.handle('nunmai:context-menu:copy-image', event => {
...
ipcMain.handle('nunmai:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('nunmai:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('nunmai:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('nunmai:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nunmai:savePastedText', async (_event, payload) => {
  ...
  return writeComposerPaste(NUNMAI_HOME, text)
})

ipcMain.handle('nunmai:saveClipboardImage', async () => {
...
ipcMain.handle('nunmai:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nunmai:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nunmai:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('nunmai:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('nunmai:active-work', (event, payload) => {
...
ipcMain.on('nunmai:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nunmai:native-theme', (_event, mode) => {
...
ipcMain.on('nunmai:translucency:support', event => {
...
// it arrives from `nunmai desktop --local` or directly on Nunmai.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('nunmai:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('nunmai:translucency', (_event, payload) => {
...
ipcMain.on('nunmai:keep-awake', (_event, on) => {
...
ipcMain.handle('nunmai:quick-entry:settings:get', async () => {
...
ipcMain.handle('nunmai:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('nunmai:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('nunmai:quick-entry:submit', {
...
ipcMain.on('nunmai:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('nunmai:quick-entry:state', payload)
  }
})

ipcMain.on('nunmai:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('nunmai:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('nunmai:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `nunmai:window:openSession`).
...
ipcMain.handle('nunmai:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nunmai:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nunmai:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('nunmai:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNunmaiCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nunmai:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNunmaiCwd()
}))

ipcMain.handle('nunmai:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nunmai:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nunmai:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nunmai:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nunmai:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('nunmai:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nunmai:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: nunmaiLog.slice(-200) }))
...
ipcMain.on('nunmai:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (nunmai:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('nunmai:updates:check', async (_event, opts) =>
...
ipcMain.handle('nunmai:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nunmai:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nunmai:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Nunmai version (the one `release.py` bumps in
// nunmai_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nunmai version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNunmaiVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nunmai_cli', '__init__.py')
...
// Renderer-bundle skew: `nunmai update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Nunmai version and push it into the native About panel
// just before showing it, so an in-place `nunmai update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveNunmaiVersion()} — app build out of date, update the desktop app`
        : resolveNunmaiVersion(),
      copyright: 'Copyright © 2026 Nunmai Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('nunmai:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveNunmaiVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    nunmaiRoot: resolveUpdateRoot(),
...
// The About page's "Restart Nunmai" button (shown when bundleSwapPending):
...
ipcMain.handle('nunmai:app:relaunch', async () => {
...
ipcMain.handle('nunmai:machine:profile', async () => {
...
// the actual removal via `nunmai uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nunmai_cli/uninstall.py + nunmai_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NUNMAI_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isNunmaiSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nunmai_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NUNMAI_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nunmai agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nunmai_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NUNMAI_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nunmai-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nunmai-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nunmai:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nunmai:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('nunmai:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('nunmai:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// nunmai:// deep links (e.g. nunmai://blueprint/morning-brief?time=08:00,
// nunmai://mcp/install?name=NAME&config=B64 — the vendor "Add to Nunmai"
// button, or nunmai://plugin/install?repo=owner/repo). Dev
// (`NUNMAI_DESKTOP_DEV_SERVER`) registers nunmai-dev:// instead — bare
// Electron or a stale OS handler often owns nunmai:// on dev machines.
...
// ---------------------------------------------------------------------------
const NUNMAI_PROTOCOL = DEV_SERVER ? 'nunmai-dev' : 'nunmai'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['nunmai-dev', 'nunmai'] : ['nunmai']
...
function handleDeepLink(url) {
  ...
  // nunmai://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nunmai:deep-link', payload)
...
ipcMain.handle('nunmai:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${NUNMAI_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NUNMAI_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(NUNMAI_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${NUNMAI_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `nunmai://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startNunmai), whose
...
app.whenReady().then(() => {
  ...
  // startNunmai() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching nunmai:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Nunmai never keeps another app's chord hostage.
