 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `nunmai update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.nunmaiHome, 'Nunmai home')
  const nunmaiPath = validateRemoteValue(target.nunmaiPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(nunmaiPath)

  const updateCommand =
    `env NUNMAI_HOME=${homeWord} ` +
    `NUNMAI_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'NUNMAI_UPDATE_ORIGIN_PROFILE=default ' +
    `NUNMAI_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `NUNMAI_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:NUNMAI_HOME=${psLiteral(home)}`,
    `$env:NUNMAI_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:NUNMAI_UPDATE_ORIGIN_PROFILE="default"',
    `$env:NUNMAI_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:NUNMAI_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:NUNMAI_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:NUNMAI_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:NUNMAI_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(nunmaiPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.nunmai-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Nunmai updated and every managed SSH profile is ready.'
        : restoreOk
