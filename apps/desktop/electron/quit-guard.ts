...
//
// That's only true for a backend the app owns. A remote URL or Nunmai Cloud
  ...
  return {
    ...
    detail: [
      ...
      backendOwned
        ...
        : 'The agent keeps running on the remote backend. Quitting only closes Nunmai on this computer; reconnect later to see the results.'
    ]
      .filter(line => line !== null)
      .join('\n')
      .trim(),
    message: work.count === 1 ? 'Nunmai is still working on 1 chat.' : `Nunmai is still working on ${work.count} chats.`
  }
}
