...
//   NUNMAI_WIN_SSH_HOST   ssh alias/host of the Windows box
//   NUNMAI_WIN_SSH_USER   remote user
//   NUNMAI_WIN_SSH_NUNMAI absolute path to the remote nunmai.exe under test
const liveHost = process.env.NUNMAI_WIN_SSH_HOST || ''
const liveUser = process.env.NUNMAI_WIN_SSH_USER || ''
const configuredNunmai = process.env.NUNMAI_WIN_SSH_NUNMAI || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Nunmai-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredNunmai)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredNunmai.replace('nunmai.exe', 'python.exe')}' -m nunmai_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredNunmai.replace('nunmai.exe', 'python.exe')
        const terminate = `& '${python}' -m nunmai_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.nunmaiPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nunmai_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nunmai_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
