...
// spawns `nunmai serve`; runtimes older than serve only have `dashboard`, so
...
export function createBackendServeSupportResolver(nunmaiHome: string, rememberLog: (message: string) => void) {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'nunmai_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, NUNMAI_HOME: nunmaiHome, ...(backend.env || {}) },
