...
const SCAN_MODULE = 'nunmai_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Nunmai processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'nunmai_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Nunmai process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Nunmai installation is free.\n' +
    '\n' +
    'Close other Nunmai windows and terminals, then retry.  If the problem\n' +
    'persists, run `nunmai update` in a terminal for detailed diagnostics.'
  )
}
