...
test('launches the unpacked helper without prompting and delivers only validated capture messages', () => {
  ...
  const monitor = new CommandScreenshotMonitor({
    platform: 'darwin',
    appPath: '/Applications/Nunmai.app/Contents/Resources/app.asar',
    spawn
  })
  ...
  assert.deepEqual(spawn.mock.calls[0], [
    '/Applications/Nunmai.app/Contents/Resources/app.asar.unpacked/dist/native/command-screenshot-monitor',
    [],
