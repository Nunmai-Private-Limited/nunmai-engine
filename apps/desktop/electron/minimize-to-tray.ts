...
export function createMinimizeToTray(options: Options) {
  ...
  const broadcast = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('nunmai:minimize-to-tray:changed', status())
      }
    }
  }
  ...
  const apply = async (on: boolean) => {
    ...
    if (!on) {
      ...
      try {
        ...
        tray.setToolTip('Nunmai')
        tray.setContextMenu(
          Menu.buildFromTemplate([
            { label: 'Show Nunmai', click: restore },
            { type: 'separator' },
            // Do not bypass the ordinary active-work confirmation or teardown.
            { label: 'Quit Nunmai', click: () => app.quit() }
          ])
        )
  ...
  ipcMain.handle('nunmai:minimize-to-tray:get', status)
  ipcMain.handle('nunmai:minimize-to-tray:set', (_event, on) => setEnabled(on === true))
