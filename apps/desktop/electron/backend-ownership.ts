  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (const entry of entries) {
        ...
        // if it reaches reapOrphans (see main.ts startNunmai + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:nunmai(?:\.exe)?|nunmai_cli\.main|nunmai_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
