...
// hardening, NUNMAI_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('nunmai:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('nunmai:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))
  ...
  ipcMain.handle('nunmai:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('nunmai:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<NUNMAI_HOME>/desktop-plugins`,
  // resolved from the main-process NUNMAI_HOME (see resolveNunmaiHome) — NOT from
  // the connected backend. A remote backend reports its own `nunmai_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(nunmaiHome, 'profiles', profile) : nunmaiHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(nunmaiHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(nunmaiHome, root)
    await reconcileUnifiedDesktopHalves(nunmaiHome, root)

    return root
  }

  ipcMain.handle('nunmai:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('nunmai:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(nunmaiHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(nunmaiHome, root)
  })

  // The LOCAL logs root (`<NUNMAI_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where NUNMAI_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('nunmai:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('nunmai:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('nunmai:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('nunmai:plugin:removeDesktop', async (_event, payload) =>
    removeDesktopPlugin(path.join(nunmaiHome, DESKTOP_PLUGINS_DIR), payload?.name)
  )

  // Rename a file/folder in place. The renderer passes the existing path + a new
  // base name; the destination is resolved in the SAME parent dir so a rename can
  // never move the item elsewhere or traverse out. Rejects on a name collision.
  ipcMain.handle('nunmai:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('nunmai:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('nunmai:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
