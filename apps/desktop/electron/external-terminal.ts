// Launching the Nunmai TUI in the user's OWN terminal emulator.
//
...
// `nunmai --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Nunmai runtime is often a venv Python invoked
//     as `python -m nunmai_cli.main`, not a `nunmai` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Nunmai
