 ...
 *   - the browser sees only a minimal "return to Nunmai" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Nunmai.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('nunmai:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('nunmai:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('nunmai:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
