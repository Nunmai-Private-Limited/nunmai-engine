...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-compat-'))
}
...
const REPORT = {
  ...
  plugins: {
    ...
    beta: [
      { file: 'a.py', line: 1, old: 'nunmai_cli.kanban_db.connect', new: 'nunmai_cli.kanban_db_connect.connect' },
      {
        file: 'a.py',
        line: 9,
        old: 'nunmai_cli.kanban_db.connect_closing',
        new: 'nunmai_cli.kanban_db_connect.connect_closing'
      }
    ]
  },
  lines: [
    '2 plugins use import paths that stop working on 2026-09-14 (10 days): alpha (1), beta (2)',
    'Details: nunmai plugins compat'
  ]
}
...
test('report → one notice naming plugins, date and the CLI command', () => {
  ...
  assert.match(n.detail, /nunmai plugins compat/)
})
