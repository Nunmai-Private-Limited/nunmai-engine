...
export function registerHudIpc({
  ...
  ipcMain.on('nunmai:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('nunmai:hud:windowing', event => {
  ...
  ipcMain.on('nunmai:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('nunmai:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('nunmai:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('nunmai:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nunmai:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nunmai:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nunmai:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nunmai:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('nunmai:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nunmai:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('nunmai:hud:close', async () => {
    closeHudWindow()
