...
// `nunmai desktop` launcher appends `desktop.electron_flags` to argv, but a
...
export function readDesktopLaunchConfig(yamlText: string): DesktopLaunchConfig {
  ...
  if (out.electronFlags.length === 0 && out.rendererMaxOldSpaceMb === 0) {
    ...
    if (meantToSet) {
      console.warn(
        '[nunmai] config.yaml: desktop.electron_flags / desktop.renderer_max_old_space_mb were ignored — ' +
 ...
 * re-applied (the `nunmai desktop` launcher put them there).
 */
