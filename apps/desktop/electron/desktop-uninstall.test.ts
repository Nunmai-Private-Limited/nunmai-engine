...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'nunmai_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'nunmai_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'nunmai_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Nunmai.app/Contents/MacOS/Nunmai', 'darwin'),
    '/Applications/Nunmai.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Nunmai.app/Contents/MacOS/Nunmai', 'darwin'),
    '/Users/x/Applications/Nunmai.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Nunmai\\Nunmai.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Nunmai'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\nunmai-desktop\\Nunmai.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\nunmai-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Nunmai.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NunmaiXXXX/nunmai', 'linux', { APPIMAGE: '/home/x/Apps/Nunmai.AppImage' }),
    '/home/x/Apps/Nunmai.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/nunmai/linux-unpacked/nunmai', 'linux', {}), '/opt/nunmai/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/nunmai', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Nunmai.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Nunmai.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.nunmai/nunmai-engine/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.nunmai/nunmai-engine',
    uninstallArgs: ['-m', 'nunmai_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/nunmai/linux-unpacked',
    nunmaiHome: '/home/x/.nunmai'
  })
  ...
  assert.match(script, /'-m' 'nunmai_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/nunmai\/linux-unpacked'/)
  assert.match(script, /export NUNMAI_HOME='\/home\/x\/\.nunmai'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.nunmai/nunmai-engine',
    agentRoot: '/home/x/.nunmai/nunmai-engine',
    uninstallArgs: ['-m', 'nunmai_cli.uninstall', '--mode', 'full'],
    appPath: null,
    nunmaiHome: '/home/x/.nunmai'
  })

  // System python + source on PYTHONPATH so import nunmai_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.nunmai\/nunmai-engine'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'nunmai_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'nunmai_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    nunmaiHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'nunmai_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\nunmai',
    uninstallArgs: ['-m', 'nunmai_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Nunmai',
    nunmaiHome: 'C:\\Users\\x\\AppData\\Local\\nunmai'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import nunmai_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\nunmai;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "nunmai_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Nunmai" >nul 2>&1/)
