...
describe('WSLg launch arguments', () => {
  it('preserves the invocation and restarts only once with native Wayland', () => {
    const args = ['.', '--inspect=9229', 'nunmai://session/example']
    ...
    expect(args).toEqual(['.', '--inspect=9229', 'nunmai://session/example'])
  })
  ...
  it('respects explicit backends and the existing config-bridged X11 hint', () => {
    ...
    expect(wslgLaunchArgs([], { ...env, NUNMAI_DESKTOP_DISABLE_GPU: 'true' }, 'linux')).toEqual([
