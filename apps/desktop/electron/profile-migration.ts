...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.nunmai`). Default's state.db and gateway.pid live here. */
  nunmaiHome: string
  /** Named-profile root (`~/.nunmai/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isNunmaiProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `nunmaiHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, nunmaiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${nunmaiHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, nunmaiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${nunmaiHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveNunmaiHome(profilesRoot: string, nunmaiHome?: string): string {
  if (nunmaiHome) {
    return nunmaiHome
  }

  // Tests that predate nunmaiHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live nunmai process.
 ...
 *
 * `nunmaiHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isNunmaiProcess'> & { nunmaiHome?: string }
): string[] {
  const nunmaiHome = resolveNunmaiHome(profilesRoot, deps.nunmaiHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, nunmaiHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.nunmaiHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$NUNMAI_HOME`, not `$NUNMAI_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `nunmai --profile default` and is worse than writing nothing
