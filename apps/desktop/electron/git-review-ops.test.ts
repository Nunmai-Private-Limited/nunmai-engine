...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'nunmai-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Nunmai Test'], { cwd: dir })
