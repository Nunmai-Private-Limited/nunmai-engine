...
function expectRequestHeaders(
  ...
  applyRemoteRequestHeaders({ url, requestHeaders: { Origin: 'app://nunmai' } }, callback, store.headersFor)

  expect(callback).toHaveBeenCalledOnce()
  expect(callback).toHaveBeenCalledWith(expected ? { requestHeaders: { Origin: 'app://nunmai', ...expected } } : {})
}
...
describe('OAuth login and registry extra headers', () => {
  ...
  it('injects extra headers on an OAuth partition session, not only defaultSession', () => {
    ...
    const callback = vi.fn()
    listeners[0]({ url: 'https://gateway.example/login', requestHeaders: { Origin: 'app://nunmai' } }, callback)

    expect(listeners).toHaveLength(1)
    expect(callback).toHaveBeenCalledWith({
      requestHeaders: { Origin: 'app://nunmai', ...accessHeaders }
    })
  })
})
