 ...
 * console. It writes NUNMAI_HOME/.nunmai-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Nunmai within 30 minutes.
...
export function handoffResultPath(nunmaiHome: string): string {
  return path.join(nunmaiHome, '.nunmai-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(nunmaiHome)
  let raw: string
