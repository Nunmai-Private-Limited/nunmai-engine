...
//
// Exactly ONE `nunmai serve` runs per HOST and multiplexes every profile, so
...
// (`nunmai_cli/process_identity.py::register_self`, called from
// `web_server.py` with `detail={host, port, profile}`), and an ungated
// loopback backend serves its session token at `GET /`
// (`window.__NUNMAI_SESSION_TOKEN__`, `web_server_dashboard.py`). The ledger is
...
/** Filename the CLI writes under the machine Nunmai root. */
 ...
 * `NUNMAI_DESKTOP_ISOLATED_BACKEND` in main.ts) and wins over every record.
 *
