// Backend subcommand routing for the desktop-managed Nunmai process.
//
// The desktop app launches its own headless backend via `nunmai serve` — it
...
// app hasn't updated yet, or an older `nunmai` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Nunmai profile to pin via `--profile`.
 */
 ...
 * `-m nunmai_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `nunmai_cli/subcommands/dashboard.py` source registers
