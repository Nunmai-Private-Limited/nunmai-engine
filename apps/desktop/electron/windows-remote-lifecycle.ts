...
const READY_RE = /^NUNMAI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitNunmaiPath = '') {
  const explicit = psLiteral(explicitNunmaiPath)
  ...
  const script = [
    ...
    '$nunmaiHome=$env:NUNMAI_HOME',
    'if(-not $nunmaiHome){$nunmaiHome=Join-Path $env:LOCALAPPDATA "nunmai"}',
    'Assert-NoReparse $nunmaiHome $true',
    '$candidate=[IO.Path]::Combine($nunmaiHome, "nunmai-engine\\venv\\Scripts\\nunmai.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "nunmai-engine\\.venv\\Scripts\\nunmai.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $nunmaiHome "nunmai-engine\\venv\\Scripts\\nunmai.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "nunmai-engine\\.venv\\Scripts\\nunmai.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command nunmai.exe -ErrorAction SilentlyContinue',
    ...
    '$nunmai=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$nunmai=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $nunmai){throw "Nunmai is not installed on the remote Windows host."}',
    'Assert-NoReparse $nunmai $false',
    'if($explicit -and $nunmai -ne $explicit){throw "The configured Nunmai path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($nunmai), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;nunmaiHome=$nunmaiHome;nunmaiPath=$nunmai;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(nunmaiHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class NunmaiMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$nunmaiHome=${psLiteral(nunmaiHome)}`,
    '$installRoot=$nunmaiHome',
    '$parent=Split-Path -Parent $nunmaiHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".nunmai-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[NunmaiMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, nunmaiHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(nunmaiHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitNunmaiPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitNunmaiPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'nunmai_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'nunmai_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'nunmai_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$nunmaiHome=${psLiteral(runtime.nunmaiHome)}`,
    '$installRoot=$nunmaiHome',
    '$parent=Split-Path -Parent $nunmaiHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".nunmai-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};nunmaiPath=${psLiteral(reservation.nunmaiPath)};nunmaiHome=${psLiteral(reservation.nunmaiHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.nunmaiPath === 'string' &&
    typeof lock.nunmaiHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.nunmaiPath === runtime.nunmaiPath &&
    lock.nunmaiHome === runtime.nunmaiHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteNunmaiPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.nunmaiHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.nunmaiPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Nunmai on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.nunmaiPath = inspection.path
  const nunmaiVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located nunmai at ${runtime.nunmaiPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, nunmaiPath: runtime.nunmaiPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForNunmai(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Nunmai SSH"', 'powershell.exe -NoLogo')
