...
export const ZOOM_STORAGE_KEY = 'nunmai:desktop:zoomLevel'
 ...
 * but forgot to emit 'nunmai:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('nunmai:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `nunmai:zoom:changed`, which never fired —
