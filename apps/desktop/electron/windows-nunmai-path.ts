/**
 * windows-nunmai-path.ts
 *
 * Pure, dependency-injected pieces of Windows `nunmai` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvNunmaiCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `nunmai` shim shadowed the real
 *      nunmai.cmd/nunmai.exe; the shim then failed the --version probe and
 ...
 *      and Nunmai launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvNunmaiCommand() — unwrapWindowsVenvNunmaiCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `nunmai`) must
 * not shadow `nunmai.cmd`/`nunmai.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Nunmai launcher that drives `nunmai update`.
...
export interface ResolveVenvNunmaiCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportNunmaiCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `nunmai`/`nunmai.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/nunmai(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m nunmai_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportNunmaiCli(). A
 * venv whose update died mid-`pip install` still has python.exe + nunmai.exe
 ...
 *
 * Returns null when `command` is not a venv nunmai shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvNunmaiCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvNunmaiCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^nunmai(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !(await canImportNunmaiCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv Nunmai at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Nunmai Python at ${python}`,
    command: python,
