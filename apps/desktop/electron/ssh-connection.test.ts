...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env NUNMAI_DASHBOARD_SESSION_TOKEN=abc123deadbeef NUNMAI_DESKTOP=1 nunmai dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /NUNMAI_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /NUNMAI_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Nunmai-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Nunmai-Session-Token: hdr_888'), /X-Nunmai-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Nunmai-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('nunmai', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v nunmai', 15000)
  assert.equal(args[args.length - 1], 'command -v nunmai')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `nunmai-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `nunmai-ssh-test-${process.pid}-${Date.now()}`)
...
test('withRemoteTimeout kills a hung probe remotely instead of orphaning it (#110478)', async () => {
  ...
  const wrapped = withRemoteTimeout('nunmai --version 2>&1', 15)
