...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startNunmai() boots the primary as the preference at that moment.
  ...
  // nunmai:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same NUNMAI_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
