...
test('on linux, fatal Chromium output lands in a file under the Nunmai logs dir', () => {
  const plan = linuxCrashDiagnostics('/home/u/.nunmai/logs', 'linux')
  ...
  assert.ok(logFile)
  assert.equal(path.dirname(logFile), '/home/u/.nunmai/logs')
...
test('other platforms get no Chromium logging switches and no crash reporter', () => {
  assert.equal(linuxCrashDiagnostics('/Users/u/.nunmai/logs', 'darwin'), null)
  assert.equal(linuxCrashDiagnostics('C:\\Users\\u\\.nunmai\\logs', 'win32'), null)
})
...
test('a logs dir that cannot be created degrades to no logging, never a dead shell', () => {
  ...
  // Read-only or invalid NUNMAI_HOME/logs: mkdir throws before app readiness.
...
test('a crash reporter that refuses to start is not fatal either', () => {
  ...
  enableLinuxCrashDiagnostics(linuxCrashDiagnostics('/home/u/.nunmai/logs', 'linux'), '/home/u/.nunmai/logs', {
    ensureLogsDir: () => {},
    reclaimChromiumLog: () => {},
    appendSwitch: name => switches.push(name),
    startCrashReporter: () => {
...
test('the Chromium log is bounded before Chromium appends to it', () => {
  ...
  assert.deepEqual(reclaimed, [path.join('/home/u/.nunmai/logs', CHROMIUM_LOG_FILENAME)])
})
