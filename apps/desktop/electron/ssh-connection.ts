...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `nunmai --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(NUNMAI_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Nunmai-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.nunmai/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'nunmai-desktop-ssh')
  }

  return path.join(os.homedir(), '.nunmai', 'desktop-ssh')
}
