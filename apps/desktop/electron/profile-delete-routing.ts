// Profile-delete routing logic for the `nunmai:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_nunmai_home() and
...
/** Parse a `nunmai:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its NUNMAI_HOME.
 *
...
/**
 * Route the next `nunmai:api` request away from the primary/window backend
 ...
 * ensure_nunmai_home() recreates the directory the delete just removed.
 */
