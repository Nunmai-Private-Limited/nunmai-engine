...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/nunmai-engine.git'), 'someone/nunmai-engine')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('ghcr.io/nunmai-private-limited/nunmai-engine', 'bb/gui'),
    'https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/commits/bb%2Fgui'
  )
})
