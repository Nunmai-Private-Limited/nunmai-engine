 ...
 *
 * So we delegate to `nunmai gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `nunmai update`'s
...
export function stopGatewayBeforeUpdate(
  nunmaiCliPath: string,
  nunmaiHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(nunmaiCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(nunmaiCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(nunmaiCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(nunmaiCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(nunmaiCliPath, args)
  }

  if (!existsSync(nunmaiCliPath)) {
    return false
  }

  try {
    exec(nunmaiCliPath, args, {
