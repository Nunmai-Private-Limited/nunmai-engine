 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Nunmai Engine by spawning
...
// Relative, not `@nunmai/shared/ansi`: the electron bundle is built by esbuild
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.nunmai-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(nunmaiHome) {
  return path.join(nunmaiHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.nunmai/nunmai-engine. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(nunmaiHome) {
  if (!nunmaiHome) {
    return null
  }

  const candidate = path.join(nunmaiHome, 'nunmai-engine', 'scripts', installScriptName())
...
function cachedScriptPath(nunmaiHome, commit) {
  return path.join(bootstrapCacheDir(nunmaiHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Nunmai-Private-Limited/nunmai-engine/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(nunmaiHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(nunmaiHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nunmaiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass NUNMAI_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          NUNMAI_HOME: nunmaiHome || process.env.NUNMAI_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nunmaiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, nunmaiHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--nunmai-home', nunmaiHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nunmaiHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nunmaiHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(nunmaiHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, nunmaiHome, emit })
    abortSignal?.throwIfAborted()
