...
test('output tail interleaves stdout and stderr attached from spawn time', () => {
  ...
  child.stderr.emit('data', Buffer.from("ModuleNotFoundError: No module named 'nunmai_cli'\n"))
...
test('exit line carries the buffered tail next to the exit code, preferring the signal', () => {
  ...
  assert.equal(
    formatBackendExitLine('Ignoring stale Nunmai backend exit', 1, null, tail),
    'Ignoring stale Nunmai backend exit (1)\nRecent backend output:\nTraceback (most recent call last):'
  )
  assert.equal(
    formatBackendExitLine('Nunmai backend exited', null, 'SIGTERM', tail),
    'Nunmai backend exited (SIGTERM)\nRecent backend output:\nTraceback (most recent call last):'
  )
})

test('exit line stays byte-identical to the legacy shape when the tail is empty or missing', () => {
  assert.equal(
    formatBackendExitLine('Nunmai backend exited', 0, null, createBackendOutputTail(64)),
    'Nunmai backend exited (0)'
  )
  assert.equal(formatBackendExitLine('Nunmai backend exited', 1, null, null), 'Nunmai backend exited (1)')
})
