 ...
 *
 * `nunmaiHome` must be the NUNMAI_HOME root, not Electron's userData dir: the
 * backend admits `@file:` attachments outside the chat's cwd only from
 * `<NUNMAI_HOME>/composer-pastes` (agent/context_references.py::_resolve_path),
 * and on Linux/macOS userData is a different tree (#117149).
 */
export async function writeComposerPaste(nunmaiHome: string, text: string): Promise<string> {
  const dir = path.join(nunmaiHome, COMPOSER_PASTES_DIRNAME)
