...
describe('readPreUpdateBackupEnabled', () => {
  it.each([
    ...
    const runtime = {
      ...
      args: ['-m', 'nunmai_cli.main', 'config', 'get', 'updates.pre_update_backup', '--json'],
      env: { PYTHONPATH: '/runtime/nunmai' }
    }
    ...
    expect(run).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          NUNMAI_HOME: '/profiles/active',
          PYTHONPATH: '/runtime/nunmai'
        })
      })
    )
  })
  ...
  it.each([
    ...
    await expect(
      readPreUpdateBackupEnabled({ command: '/runtime/nunmai', args: ['config', 'get'] }, '/profiles/active', run)
    ).resolves.toBe(true)
  })
})
