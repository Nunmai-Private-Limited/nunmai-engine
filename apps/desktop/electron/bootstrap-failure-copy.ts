...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Nunmai running. ' +
  'Close other Nunmai windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Nunmai stopped during the '${label}' step.`
    : 'Setting up Nunmai stopped before it could finish.'
...
/**
 * Error.message for an installed Nunmai with a piece missing (source tree,
 * Python environment). The renderer's install overlay offers the Repair install
 * button ('nunmai:bootstrap:repair'), so the copy points there. `whatIsMissing`
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Nunmai' installation is missing (it may have been deleted or quarantined by antivirus). " +
