...
// no longer bundles the Nunmai Engine Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Nunmai.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Nunmai'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Nunmai.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Nunmai'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NUNMAI_HOME for non-sandboxed runs -- matches main.ts's
// resolveNunmaiHome(). On Windows it's %LOCALAPPDATA%\nunmai; elsewhere
// it's ~/.nunmai. The fresh-install sandbox launchFresh() sets its own
// NUNMAI_HOME and never touches this.
const DEFAULT_NUNMAI_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'nunmai')
  }
  return path.join(os.homedir(), '.nunmai')
})()
const VENV_ROOT = path.join(DEFAULT_NUNMAI_HOME, 'nunmai-engine', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'nunmai-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NUNMAI_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Nunmai-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Nunmai-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Nunmai-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Nunmai-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NUNMAI_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NUNMAI_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const nunmaiHome = path.join(sandbox, 'nunmai-home')
  ...
  env.NUNMAI_DESKTOP_CWD = cwd
  env.NUNMAI_DESKTOP_IGNORE_EXISTING = '1'
  env.NUNMAI_DESKTOP_TEST_MODE = 'fresh-install'
  env.NUNMAI_DESKTOP_USER_DATA_DIR = userDataDir
  env.NUNMAI_HOME = nunmaiHome
  delete env.NUNMAI_DESKTOP_NUNMAI
  delete env.NUNMAI_DESKTOP_NUNMAI_ROOT
  ...
  console.log(`  NUNMAI_HOME: ${nunmaiHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(nunmaiHome, 'nunmai-engine', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Nunmai Engine Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of nunmai_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'nunmai-engine', 'nunmai_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Nunmai
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NUNMAI_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NUNMAI_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
