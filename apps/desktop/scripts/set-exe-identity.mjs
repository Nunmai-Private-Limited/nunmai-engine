#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Nunmai icon + version metadata onto the
// built Nunmai.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Nunmai.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `nunmai desktop`, the installer's
...
// path (which rebuilds via `nunmai desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Nunmai.exe>
//
...
// Stamp the Nunmai icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Nunmai',
      FileDescription: 'Nunmai',
      CompanyName: 'Nunmai Research',
      LegalCopyright: 'Copyright (c) 2026 Nunmai Research'
    }
  })

  console.log('[set-exe-identity] done — Nunmai icon + identity stamped')
}
