...
//   D=$(mktemp -d) && env -u NUNMAI_SHARED_AUTH_DIR NUNMAI_GUEST_ONBOARDING=1 NUNMAI_SKIP_INTRO=1 \
//     NUNMAI_HOME="$D/home" NUNMAI_DESKTOP_USER_DATA_DIR="$D/userdata" nunmai desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'nunmai-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.NUNMAI_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set NUNMAI_DESKTOP_PYTHON to your installed Nunmai Python executable.')
...
for (const dir of ['.nunmai/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, NUNMAI_HOME: path.join(sandbox, '.nunmai'), NUNMAI_SHARED_AUTH_DIR: path.join(sandbox, '.nunmai/shared'),
  NUNMAI_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), NUNMAI_DESKTOP_CWD: path.join(sandbox, 'work'),
  NUNMAI_DESKTOP_NUNMAI_ROOT: root, NUNMAI_DESKTOP_PYTHON: python,
  NUNMAI_DESKTOP_IGNORE_EXISTING: '1', NUNMAI_DESKTOP_APP_NAME: 'Nunmai Connector Rehearsal',
  NUNMAI_GUEST_ONBOARDING: '1', NUNMAI_SKIP_INTRO: '1', NUNMAI_DESKTOP_DEV_SERVER: url, NUNMAI_DESKTOP_CDP_PORT: '9344'
})
