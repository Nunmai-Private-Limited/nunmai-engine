...
const UPSTREAM_URL = 'https://github.com/Nunmai-Private-Limited/nunmai-engine.git'
...
const LEGACY_STORAGE_KEY = 'nunmai.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'nunmai.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.nunmaiDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `nunmai-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const nunmaiHome = join(sandbox, 'nunmai-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(nunmaiHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(nunmaiHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        NUNMAI_DESKTOP_APP_NAME: `NunmaiShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        NUNMAI_DESKTOP_NUNMAI_ROOT: target.targetRoot,
        NUNMAI_DESKTOP_IGNORE_EXISTING: '1',
        NUNMAI_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'nunmai-short-session-ab-'))
  const prepared = []
