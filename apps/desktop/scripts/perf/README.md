...
# Its own --user-data-dir + NUNMAI_HOME means it never collides with `hgui`.
npm run perf -- --spawn
...
```bash
node scripts/perf/run.mjs live-window --seconds 15 --json ~/.nunmai/cache/scratch/live-window.json
# Attribution is a separate pass, not an FPS comparison:
node scripts/perf/run.mjs live-window --seconds 10 --cpuprofile ~/.nunmai/cache/scratch
```
...
(separate lock scope), their own `NUNMAI_HOME` (separate backend + sessions),
