...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nunmai-dev-mock-${Date.now()}`))
  ...
  return { root, nunmaiHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(nunmaiHome, mockUrl) {
  fs.writeFileSync(
    path.join(nunmaiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(nunmaiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
