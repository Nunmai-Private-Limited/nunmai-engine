 ...
 * product name (`Nunmai`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nunmai'
 *
 * This is a hard failure with no obvious cause for the user — `nunmai desktop`
 ...
 * (the mac-unpacked Nunmai.app bundle) and Windows (win-unpacked), so we
 ...
 * Nunmai.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `nunmai desktop --build-only` (nunmai_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Nunmai.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Nunmai'}.exe`
