  ...
  if (halfInstalledDir) {
    lines.push(
      `[stage-native-deps] ${halfInstalledDir} exists but is not a loadable package — an ` +
        ...
        'To restore read_window_below: close every Nunmai window and gateway so the extract is not ' +
        'interrupted again, then run `nunmai desktop --force-build` — it removes the stale dir before npm.'
    )
  }
  return lines.join('\n')
}
