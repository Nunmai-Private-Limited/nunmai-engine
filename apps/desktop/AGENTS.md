...
How to build Nunmai Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Nunmai surface can also
...
- A **runtime home change** (switching the underlying `NUNMAI_HOME` profile) is
...
previews) and the preview pane's `<webview>` (`persist:nunmai-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:nunmai-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `nunmai:openExternal` IPC, which applies
...
its shape. "Plugin" means several unrelated things across Nunmai — do not assume
...
`env -u NODE_ENV NUNMAI_GUEST_ONBOARDING=1 NUNMAI_HOME=<tmp>/.nunmai NUNMAI_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`NUNMAI_PORTAL_BASE_URL=http://127.0.0.1:8765 NUNMAI_ANON_API_SECRET=test-secret NUNMAI_SHARED_AUTH_DIR=<tmp>/.nunmai/shared`
...
## Nunmai free tier: state is pulled, never latched in the renderer

The free tier (a Nunmai identity with no account, `nunmai_cli/anon_auth.py`) reaches the renderer
