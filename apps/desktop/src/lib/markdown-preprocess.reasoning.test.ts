...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Nunmai Desktop')).toBe('no Nunmai Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Nunmai')).toBe('no Nunmai')
  })
