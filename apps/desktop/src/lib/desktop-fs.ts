import { nunmaiApi } from '@/api/client'
import type {
  NunmaiConnection,
  NunmaiReadDirResult,
  NunmaiReadFileTextResult,
  NunmaiSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: NunmaiConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: NunmaiConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<NunmaiReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<NunmaiReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<NunmaiReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<NunmaiReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.nunmaiDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: NunmaiSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
