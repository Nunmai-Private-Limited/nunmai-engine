...
const UPDATE_BACKEND = 'Update the Nunmai backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.nunmaiDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Nunmai Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.nunmaiDesktop.openExternal(authUrl)
    let pollFailures = 0
