import { GatewayReauthRequiredError, isGatewayReauthRequired, resolveGatewayWsUrl } from '@nunmai/shared'
...
describe('desktop connection scope', () => {
  ...
  function registeredConnection(authMode: (typeof authModes)[number]) {
    return { ...aliasConnection(authMode), profile: 'remote-profile', registryScoped: true } as NunmaiConnection
  }
  ...
  it('a registry-scoped flag without a connectionId still takes the legacy mint', async () => {
    const desktop = fakeDesktop()
    const scopedWithoutId = { ...registeredConnection('token'), connectionId: undefined } as NunmaiConnection
