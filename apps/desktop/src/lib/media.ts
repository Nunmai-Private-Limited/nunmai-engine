...
import { capabilityScoped, nunmaiApi, type OwnerScope } from '@/api/client'
...
export async function resolveMediaDisplaySrc(path: string, owner?: OwnerScope): Promise<string> {
  ...
  if (owner?.connectionId === LOCAL_CONNECTION_ID && window.nunmaiDesktop?.readFileDataUrl) {
    return window.nunmaiDesktop.readFileDataUrl(filePathFromMediaPath(path))
  }
  ...
  if (window.nunmaiDesktop && (owner?.connectionId || owner?.profile)) {
    const result = await nunmaiApi<string | { dataUrl?: string }>({
  ...
  if (window.nunmaiDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.nunmaiDesktop?.readFileDataUrl) {
...
export function mediaImageKey(path: string, connection: NunmaiConnection | null, owner?: OwnerScope): string {
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.nunmaiDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `nunmai-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `nunmai-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.nunmai/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.nunmaiDesktop?.saveGatewayFile) {
  ...
  return window.nunmaiDesktop.saveGatewayFile({
    ...(owner.connectionId ? { connectionId: owner.connectionId } : {}),
    path,
    ...(owner.profile ? { profile: owner.profile } : {}),
