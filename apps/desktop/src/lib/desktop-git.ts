import type {
  ...
  NunmaiGitBranch,
  NunmaiGitWorktree,
  NunmaiRepoPullRequests,
  NunmaiRepoStatus,
  NunmaiReviewList,
  NunmaiReviewShipInfo
} from '@/global'
import { nunmaiApi } from '@/nunmai'
...
// (window.nunmaiDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['nunmaiDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.nunmaiDesktop

  if (!desktop) {
    throw new Error('Nunmai Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: NunmaiGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: NunmaiGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: NunmaiGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<NunmaiRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<NunmaiReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<NunmaiReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<NunmaiRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.nunmaiDesktop?.git
}
