...
// Nunmai is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.nunmaiMode
 ...
 * the only carriers of Nunmai' palette — are dropped so the paste target's
