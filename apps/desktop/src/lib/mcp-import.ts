...
const emptyNunmaiFlags = (): NunmaiAddFlags => ({
  args: [],
  auth: null,
  command: null,
  env: {},
...
function readNunmaiToken(flags: NunmaiAddFlags, tokens: string[], index: number): number {
...
function nunmaiEntry(flags: NunmaiAddFlags): McpImportEntry | null {
...
function fromNunmaiAdd(tokens: string[]): McpImportEntry | null {
  const flags = emptyNunmaiFlags()

  for (let i = 3; i < tokens.length; i++) {
    i = readNunmaiToken(flags, tokens, i)
...
function parseLine(line: string): McpImportEntry[] | null {
  ...
  if (tokens[0] === 'nunmai' && tokens[1] === 'mcp' && tokens[2] === 'add') {
    const entry = fromNunmaiAdd(tokens)
