...
import type { SessionMessage } from '@/types/nunmai'
...
describe('toChatMessages', () => {
  ...
  it('renders MEDIA tags as assistant attachment links', () => {
    const [message] = toChatMessages([
      {
        role: 'assistant',
        content: "MEDIA:/Users/brooklyn/.nunmai/cache/audio/tts_20260501_222725.mp3\n\nhow's that sound?",
        timestamp: 1
      }
    ])

    expect(chatMessageText(message)).toBe(
      "[Audio: tts_20260501_222725.mp3](#media:%2FUsers%2Fbrooklyn%2F.nunmai%2Fcache%2Faudio%2Ftts_20260501_222725.mp3)\n\nhow's that sound?"
    )
  })
  ...
  it('renders a native-vision turn as caption plus thumbnail, not raw placeholder text', () => {
    ...
    const ref = '@image:`/Users/me/Library/Application Support/Nunmai/composer-images/a.png`'
  ...
  // Nunmai closes a failed turn with an assistant-role row (agent/turn_failure_copy.py);
  // painted as the model's reply it read as the assistant refusing the request.
  it('renders the failed-turn boundary as a Nunmai notice, not a model reply', () => {
    const messages = toChatMessages([
