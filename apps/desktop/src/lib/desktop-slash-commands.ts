...
/** Why a known Nunmai command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Nunmai profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `nunmai_cli/commands.py::desktop_surface_registry` by
...
function isKnownNunmaiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Nunmai' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNunmaiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Nunmai and were never asked for. It is
