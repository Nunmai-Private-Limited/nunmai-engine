...
export const ERROR_CODE_KEYS = [
  ...
  'SESSION_NOT_OWNED',
  'disk_full',
  // The Nunmai free tier refused or could not serve the turn (agent/error_surface.py
...
export interface ErrorSurface {
  ...
  /** Auth layer only: display name of the failing provider ("Nunmai Portal"). */
...
/** True when the Nunmai free tier refused or could not serve the turn: the way
...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the Nunmai data folder so the user can free space (disk_full). */
  openNunmaiFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openNunmaiFolder: true, retry: true },
