...
//
// When someone is in a hands-free "Hey Nunmai" voice chat, the natural way to
...
// phrase (optionally addressed to Nunmai), so a real turn that merely contains
...
// Optional address prefixes so "nunmai stop" / "ok stop" / "hey nunmai, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey nunmai', 'hey nunmai,', 'nunmai', 'nunmai,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("nunmai") is not a stop command on its own.
      continue
    }
 ...
 * to Nunmai). Returns false for anything that merely contains "stop" as part of
