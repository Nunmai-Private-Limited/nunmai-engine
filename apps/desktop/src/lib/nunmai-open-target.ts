 ...
 * `nunmai://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `nunmai://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `nunmai://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type NunmaiOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const NUNMAI_PROTOCOL = 'nunmai:'
...
export function normalizeNunmaiOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('nunmai://') || trimmed.startsWith(`${NUNMAI_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // nunmai://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // nunmai://index-network/intent/1 → /index-network/intent/1
...
export function resolveNunmaiOpenPath(target: NunmaiOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeNunmaiOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeNunmaiOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeNunmaiOpenString(target.path)
 ...
 * (`nunmai://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromNunmaiDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveNunmaiOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveNunmaiOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `nunmai://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromNunmaiDeepLink('open', name, params)
}
