...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __nunmaiWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__nunmaiWatch || (win.__nunmaiWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('nunmai-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('nunmai-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __nunmaiWatchTag?: number }).__nunmaiWatchTag
