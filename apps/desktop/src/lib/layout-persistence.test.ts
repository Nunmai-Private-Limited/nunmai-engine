...
it('retains legacy data in memory when migration writes fail and retries on the next launch', () => {
  ...
  expect(window.localStorage.getItem('nunmai.desktop.layoutModeScopes.v1')).toBeNull()
  ...
  expect(window.localStorage.getItem('nunmai.desktop.layoutModeScopes.v1')).toBe('true')
  ...
  vi.stubGlobal('localStorage', {
    ...
    setItem: (key: string, value: string) => {
      if (key === 'nunmai.desktop.layoutModeScopes.v1') {
        throw new DOMException('Full', 'QuotaExceededError')
      }
