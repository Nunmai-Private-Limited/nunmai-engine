...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Nunmai.
 *
 ...
 * that into an ordinary Nunmai turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Nunmai keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Nunmai) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await nunmaiApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Nunmai; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await nunmaiApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Nunmai is running the tests…"). */
