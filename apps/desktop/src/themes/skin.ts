/**
 * Nunmai skin → DesktopTheme converter.
 *
 * A "skin" is the CLI/TUI theme unit: a YAML file in `$NUNMAI_HOME/skins/` (or a
 * built-in) resolved by `nunmai_cli/skin_engine.py` and pushed to every surface
 ...
 * skin Nunmai authors from a prompt lights up all three surfaces from one file.
 *
...
import { ensureContrast, mix } from '@nunmai/shared/color'
import type { NunmaiSkin, SkinColors } from '@nunmai/shared/skin'
 ...
 */
export function skinToDesktopTheme(skin: NunmaiSkin): DesktopTheme | null {
