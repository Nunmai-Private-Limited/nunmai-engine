/**
 * Live skin sync from the Nunmai backend.
 *
 * The backend resolves the active skin (built-in or `$NUNMAI_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Nunmai
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { NunmaiSkin } from '@nunmai/shared/skin'
...
const BACKEND_THEMES_KEY = 'nunmai-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: NunmaiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as NunmaiSkin)
