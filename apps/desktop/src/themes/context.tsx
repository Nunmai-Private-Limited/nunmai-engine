...
import { ensureContrast, mix, parseColor } from '@nunmai/shared/color'
...
const SKIN_KEY = 'nunmai-desktop-theme-v2'
const MODE_KEY = 'nunmai-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'nunmai-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'nunmai-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'nunmai-desktop-active-profile-v1'
...
// mix / ensureContrast live in @nunmai/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  root.dataset.nunmaiTheme = skinName
  root.dataset.nunmaiMode = rendered
  ...
  window.nunmaiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('nunmai-boot-background', chromeBg)
    window.localStorage.setItem('nunmai-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.nunmaiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.nunmaiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Nunmai authoring/activating a skin from a
