...
export interface Translations {
  ...
  boot: {
    ...
    steps: {
      ...
      startingNunmaiDesktop: string
    }
    errors: {
      backgroundExited: string
      backgroundExitedDuringStartup: string
      backendStopped: string
      restartNunmai: string
      openLogs: string
  ...
  settings: {
    ...
    gateway: {
      ...
      sshKeyTitle: string
      sshKeyDesc: string
      sshNunmaiPathTitle: string
      sshNunmaiPathDesc: string
      sshNunmaiPathPlaceholder: string
  ...
  commandCenter: {
    ...
    nunmaiActiveSessions: (version: string, count: number) => string
  ...
  freeTier: {
    /** Settings › Providers row title while the Nunmai identity is the free tier. */
    ...
    // One sentence per backend code (`nunmai_cli/anon_auth.py::ANON_*`); the copy never says
    // the free MODEL is off — what is unavailable is using Nunmai without signing in.
    setupFailed: {
      gateClosed: string
      paused: string
      ...
      locked: string
      generic: string
      /** The sign-in door, when the account service is reachable: the Nunmai row sits right below. */
      signInBelow: string
      tryAgain: string
      retrying: string
    }
  ...
  assistant: {
    thread: {
      ...
      errorOpenNunmaiFolder: string
      errorOpenNunmaiFolderFailed: string
