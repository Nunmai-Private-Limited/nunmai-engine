  ...
  settings: {
    ...
    plugins: {
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Nunmai 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 Nunmai 后端以便在桌面端管理此插件。',
      ...
      installModal: {
        ...
        agentLabel: '智能体插件',
        desktopLabel: '桌面 UI',
        agentTargetLocal: profile => `安装到 ${profile} 后端（~/.nunmai/plugins/）`,
    ...
    appearance: {
      ...
      tipsTitle: '应用内提示',
      tipsDesc: '指向应用某处的小气泡：空闲时偶尔出现，需要时 Nunmai 也会给你一条。关掉一条就不再出现。',
