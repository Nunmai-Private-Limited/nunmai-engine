...
export const ja = defineLocale({
  ...
  sessionImport: {
    title: '別のアプリから続ける',
    subtitle: '会話をNunmaiに取り込み、続きを始めましょう。',
    action: 'セッションを取り込む',
    readingFrom: '読み込み元',
  ...
  boot: {
    ready: 'Nunmai Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Nunmai の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート Nunmai バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingNunmaiDesktop: 'Nunmai Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'Nunmai バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Nunmai バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Nunmai バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateNunmai: 'Nunmai を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Nunmai Desktop を再起動してください。',
    ...
    native: {
      ...
      inputBody: 'Nunmai が応答を待っています。',
      turnDoneTitle: 'Nunmai が完了しました',
      turnDoneBody: '',
  ...
  sendDiagnostics: {
    ...
    failedHint:
      'ターミナルから `nunmai debug share --nous` を実行するか、`nunmai debug share --local` でアップロードせずにレポートを表示することもできます。',
  ...
  settings: {
    uninstallSection: {
      ...
      uninstallNunmai: 'Nunmai をアンインストール',
      chooseHowMuch:
      ...
      options: {
        gui: {
          ...
          description: 'このデスクトップアプリを削除します。Nunmai エージェント、設定、チャットはすべて残ります。',
          consequence: 'デスクトップ Chat GUI（このアプリとそのデータ）'
        },
        lite: {
          title: 'GUI とエージェントをアンインストール、データは保持',
          description:
            'アプリと Nunmai エージェントを削除しますが、将来の再インストールに備えて設定・チャット・シークレットは保持します。',
          consequence: 'Chat GUI と Nunmai エージェント（設定・チャット・シークレットは保持）'
        },
        full: {
          title: 'すべてアンインストール',
          description:
            'アプリ、エージェント、すべてのユーザーデータ（設定、チャット、定期ジョブ、シークレット、ログ）を削除します。',
          consequence: 'すべて——Chat GUI、Nunmai エージェント、およびすべての設定・チャット・シークレット・ログ'
        }
      }
    },
    subpages: {
    ...
    resetConfirm: 'すべての設定を Nunmai のデフォルトに戻しますか？',
    ...
    vault: {
      ...
      otpHint: '2FA を有効にするときにサイトが表示する「セットアップキー」。保存すると Nunmai がコードを生成します。',
      ...
      sources: {
        ...
        notInstalled: name =>
          `未検出です。${name} のコマンドラインツールをインストールしてサインインすると、Nunmai が自動的に検出します。`,
        disabledDesc: '検出済みですが、Nunmai では無効になっています。',
        lockedDesc:
          '検出済み。エージェントがログイン情報を必要とするときにロック解除を求めます。今すぐ解除することもできます。',
        unlockedDesc: 'このセッションでロック解除済み。30分間操作がないか Nunmai を閉じると自動的にロックされます。',
        statusLocked: 'ロック中',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Nunmai がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Nunmai が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Nunmai がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'Nunmai がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'Nunmai',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Nunmai をシステム設定に合わせます。',
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、Nunmaiもあなたのメッセージにリアクションします。',
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリや Nunmai からのヒントをときどき表示します。各ヒントは一度だけ表示されます。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      tipsReset: (count: number) => `${count}件のヒントをもう一度表示`,
      toursTitle: 'ガイドツアー',
      toursDesc:
        '各ステップを強調しながら、Nunmai がアプリを案内します。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Nunmai の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Nunmai を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Nunmai がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Nunmai が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Nunmai が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Nunmai がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Nunmai 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Nunmai ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      bundleOutOfSyncAction: 'インストーラーを入手',
      bundleSwapPending: '再起動して更新を完了',
      bundleSwapPendingDesc:
        '更新されたアプリはすでにインストール済みです。Nunmai を再起動するだけで新しいビルドが読み込まれます。チャットや設定はそのまま保持されます。',
      bundleSwapPendingAction: 'Nunmai を再起動',
      updates: '更新',
      checkNow: '今すぐ確認',
      checking: '確認中…',
      seeWhatsNew: '新機能を見る',
      ...
      automaticUpdatesDesc: 'Nunmai はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'ウィンドウの最小化やメインウィンドウを閉じる操作でシステムトレイ（macOS ではメニューバー）に隠し、Nunmai を実行し続けます。終了するにはトレイメニューの「Nunmai を終了」または Cmd+Q を使います。初期設定はオフで、このデバイスにのみ適用されます。',
    ...
    hudModifier: {
      ...
      permission:
        'システム設定 → プライバシーとセキュリティ → 入力監視で Nunmai を許可し、再試行してください。このジェスチャーはキー入力の記録や画面の撮影を行いません。',
      unavailable:
        'HUD ジェスチャーヘルパーを起動できなかったか、予期せず停止しました。再試行するか Nunmai を再起動してください。Nunmai 内の既存の HUD ショートカットは引き続き使用できます。',
      missingHelper:
        'この Nunmai には HUD ジェスチャーヘルパーが含まれていません。Nunmai を更新または再インストールしてから再試行してください。',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'どのアプリからでも左右の Command キーを同時に押すと、最前面のウインドウを撮影し、Nunmai の現在の下書きに添付します。自動送信はしません。初期設定はオフで、この Mac にのみ適用されます。機密情報が写る可能性があるため、送信前に添付画像を確認してください。',
      ...
      inputPermission:
        '入力監視の許可により、他のアプリがアクティブな間も両方の Command キーを検出できます。システム設定 → プライバシーとセキュリティ → 入力監視で Nunmai を許可し、ここに戻って再試行してください。',
      screenPermission:
        '画面収録の許可により、このショートカットを使ったときに最前面のアプリのウインドウを撮影できます。システム設定 → プライバシーとセキュリティ → 画面収録で Nunmai を許可し、ここに戻って再試行してください。macOS に求められた場合は Nunmai を再起動してください。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、Nunmai を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'Nunmai Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Nunmai バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには NUNMAI_DESKTOP_REMOTE_URL と NUNMAI_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc:
        'ローカルホストでプライベートな Nunmai バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Nunmai バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/nunmai などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'Nunmai Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Nunmai ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'Nunmai は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshNunmaiPathTitle: 'Nunmai パス（任意）',
      sshNunmaiPathDesc: 'リモートの nunmai バイナリへのフルパス。空欄 = 自動検出。',
      sshNunmaiPathPlaceholder: '自動検出',
      sshTestConnection: 'SSH をテスト',
      sshConnect: '接続',
      sshButtonsHint: '「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。',
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— Nunmai を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。Nunmai は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに Nunmai がインストールされていません。リモートでインストールする（curl -fsSL https://nunmai-engine.nunmai.in/install.sh | sh）か、Nunmai パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。Nunmai Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの Nunmai を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを Nunmai に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    model: {
      moaTitle: 'エージェント混合（Nunmai Agent）',
      moaPreset: 'プリセット',
      moaDescription:
        '「Nunmai Agent」プロバイダーのモデルとして表示される名前付きプリセットを設定します。集約モデルが実行を担当し、ツールループのすべての処理を行うため、実行費用のほぼ全額がそのプロバイダーに請求されます。参照モデルは既定でユーザーの各ターンに一度だけ助言します。',
      moaAggregator: '集約モデル',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは Nunmai が起動・管理します。`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Nunmai がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'Nunmai が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  skills: {
    ...
    skillArchivedMessage: 'nunmai curator restore で復元できます。',
  ...
  starmap: {
    ...
    emptyDesc: 'Nunmai がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Nunmai を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Nunmai を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Nunmai が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Nunmai デスクトップを設定' },
    ...
    nunmaiActiveSessions: (version, count) => `Nunmai ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: 'どちらの方法でも、あなたが管理するボットを接続し、資格情報はこの Nunmai にのみ保存されます。',
      quickSetup: 'クイックセットアップ',
      recommended: '推奨',
      quickHelp:
        'QR コードをスキャンして Telegram で確認します。Nunmai がボットを作成し、あなたの Telegram ユーザー ID を自動検出します。',
      createWithQr: 'QR で作成',
      starting: '開始中…',
      replaceWarning:
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@nunmai:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの Nunmai で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    defaultDescription: 'Nunmai の起動時と新しいチャットに使用します。既存のセッションのプロファイルは変わりません。',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Nunmai 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  modelAssignment: {
    saveFailed: 'Nunmai はモデルの変更を保存しませんでした。',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Nunmai が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Nunmai バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Nunmai から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには Nunmai バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'Nunmai を起動中...',
    placeholderReconnecting: 'Nunmai に再接続中…',
    ...
    commandDescs: {
      ...
      '/plan': '実行せずに .nunmai/plans/ に Markdown の実装計画を作成',
      '/moa': 'Nunmai Agent のプリセットで実行し、元のモデルに戻す',
      ...
      '/profile': 'アクティブな Nunmai プロファイルを切り替え',
      ...
      '/subscription': 'Nunmai のプランを確認し、ブラウザーで変更',
      '/topup': 'Nunmai の残高を表示し、請求を管理',
      '/platform': '問題のあるゲートウェイプラットフォームを一時停止、再開、一覧表示',
      '/version': 'Nunmai Engine のバージョンを表示',
      '/debug': 'デバッグレポートを作成',
    ...
    attachUrlDesc: 'Nunmai がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Nunmai にコミットと PR を任せる',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Nunmai はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Nunmai をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Nunmai バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Nunmai をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Nunmai は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Nunmai デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'Nunmai アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Nunmai を再度開きます。更新中はご自分で Nunmai を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Nunmai が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Nunmai が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'Nunmai を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて Nunmai を更新',
    foreignBlockerBody:
      'Nunmai はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'Nunmai は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  guidedGreeting: {
    line: 'やあ、どうぞ。Nunmai です。二分だけください、あなたに合わせて整えます。それから、本当にやりたいことに取りかかりましょう。\n\nまずは、何とお呼びすればいいですか。',
  ...
  install: {
    ...
    oneTimeTitle: 'Nunmai には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'Nunmai Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している Nunmai ゲートウェイに接続するか、このコンピューターに Nunmai をローカルインストールします。',
    connectExistingTitle: '既存の Nunmai に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'Nunmai をローカルにインストール',
    installLocalDesc: 'Nunmai をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。Nunmai Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Nunmai に接続',
    remoteSetupDesc:
    ...
    settingUpTitle: 'Nunmai Engine を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Nunmai CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Nunmai インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Nunmai インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Nunmai Engine のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Nunmai はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Nunmai を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Nunmai を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Nunmai を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Nunmai が自動接続します',
    ...
    authorizeThere: 'そこで Nunmai を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Nunmai をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Nunmai は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Nunmai を再起動中...',
      askRestart: 'Nunmai にサーバーの再起動を依頼',
      lookingRestart: taskId => `Nunmai は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Nunmai はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、Nunmai にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `Nunmai がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Nunmai がサーバーを再起動できませんでした。',
      stillWorking:
        'Nunmai はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  interfaceMode: {
    title: 'インターフェースモード',
    hint: '表示される内容が変わるだけで、Nunmai にできることは変わりません。',
    sessionNote:
      'シンプルモードで設定されています。ここでの変更はこのセッション中のみ有効です。自分の設定にするには詳細モードに切り替えてください。',
    simple: {
      label: 'シンプル',
      description:
        'Nunmai と話すための表示。サイドバーとチャットのみ。ターミナル、ファイル、差分のペインは表示しません。'
    },
    advanced: {
      label: '詳細',
  ...
  assistant: {
    ...
    thread: {
      ...
      loadingResponse: 'Nunmai が応答を読み込み中',
      ...
      errorLayerBodies: {
        generic:
          'Nunmai の返信中に問題が発生しました。再試行してください。問題が続く場合はエラー詳細をコピーしてください。',
        provider:
      ...
      errorCodes: {
        ...
        invalid_response: {
          ...
          body: provider => `${provider} は Nunmai が読み取れない内容を返しました。しばらくしてから再試行してください。`
        },
        empty_response: {
        ...
        ssl_cert_verification: {
          title: '安全な接続に失敗しました',
          body: provider =>
            `Nunmai は ${provider} との安全な接続を検証できませんでした。ネットワークやプロキシの設定を確認するか、プロバイダーを切り替えて再送してください。`
        }
      },
      errorLayers: {
        auth: '認証エラー',
    ...
    approval: {
      gatewayDisconnected: 'Nunmai ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.nunmai/config.yaml) に追加されます。Nunmai はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    secretDesc: 'Nunmai は続行するための認証情報が必要です。',
    ...
    vaultSaveDesc: origin =>
      `Nunmai は ${origin} のサインインページに到達しましたが、保存されたログイン情報がありません。ここで一度入力すると、このマシン上で暗号化して保存され、ページに直接入力されます。モデルはパスワードを一切見ません。`,
    ...
    vaultCodeDesc: site =>
      `${site} がワンタイムコード（SMS、メール、または認証アプリ）を求めています。ここに入力すると Nunmai がページに入力します。モデルはコードを一切見ません。`,
    vaultCodeLabel: 'コード',
    vaultCodeFootnote:
      'ヒント：「設定 → パスワードとログイン」でこのログインに認証キーを保存すると、Nunmai がコードを自動入力します。',
    vaultCodeSkip: 'スキップ',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Nunmai Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Nunmai Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`nunmai gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Nunmai が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Nunmai',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'Nunmai が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Nunmai です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
