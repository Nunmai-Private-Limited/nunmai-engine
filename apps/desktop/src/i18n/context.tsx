import { applyDocumentLocale, isRecord } from '@nunmai/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { getNunmaiConfigRecord, type NunmaiConfigRecord, retainConfigReadOrigin, saveNunmaiConfig } from '@/nunmai'
...
export interface I18nConfigClient {
  getConfig: () => Promise<NunmaiConfigRecord>
  saveConfig: (config: NunmaiConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.nunmaiDesktop?.api) {
    ...
    return getNunmaiConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.nunmaiDesktop?.api) {
    ...
    // No explicit scope: saveNunmaiConfig resolves the record's captured read
    // origin itself (resolveConfigWriteScope), and withConfigDisplayLanguage
    // retains that origin onto the derived record.
    return saveNunmaiConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: NunmaiConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: NunmaiConfigRecord, locale: Locale): NunmaiConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.nunmaiDesktop?.getMachineProfile?.().catch(() => null)
