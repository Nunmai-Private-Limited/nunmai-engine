...
import type { NunmaiConfigRecord } from '@/nunmai'
...
it('reads and persists the owning profile through A → B → A and restart', async () => {
  const configs: Record<string, NunmaiConfigRecord> = {
    default: { display: { language: 'en' } },
    coder: { display: { language: 'zh', skin: 'mono' } }
  }

  const api = vi.fn(async (request: NunmaiApiRequest) => {
    const key = request.profile || 'default'

    if (request.method === 'PUT') {
      configs[key] = structuredClone((request.body as { config: NunmaiConfigRecord }).config)
  ...
  window.nunmaiDesktop = { api } as never
...
it('isolates stale reads and failed saves when the same profile moves between connections', async () => {
  let finishBoot!: (config: NunmaiConfigRecord) => void
  let finishSaveRead!: (config: NunmaiConfigRecord) => void
  let rejectSave!: (reason: Error) => void
  let reads = 0

  const api = vi.fn((request: NunmaiApiRequest) => {
    if (request.method === 'PUT') {
