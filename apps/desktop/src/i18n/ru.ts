...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Nunmai и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Nunmai.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Nunmai. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Nunmai',
    continue: 'Продолжить в Nunmai',
  ...
  boot: {
    ready: 'Nunmai Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Nunmai',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Nunmai…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingNunmaiDesktop: 'Запуск Nunmai Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Nunmai завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Nunmai завершился при запуске.',
    ...
    failure: {
      title: 'Nunmai не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Nunmai старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateNunmai: 'Обновить Nunmai',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Nunmai Desktop.',
    ...
    native: {
      ...
      inputBody: 'Nunmai ожидает ваш ответ.',
      turnDoneTitle: 'Nunmai завершил',
      turnDoneBody: '',
  ...
  billingBlock: {
    titleNous: 'Кредиты Nunmai закончились',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Nunmai по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Nunmai в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Nunmai задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Nunmai был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Nunmai был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Nunmai следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Nunmai сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Nunmai — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Nunmai и откройте снова, затем вернитесь сюда.',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Nunmai останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Nunmai может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Nunmai обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Nunmai обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Nunmai обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Nunmai автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'Сворачивание окон или закрытие главного окна скрывает их в системном трее (строке меню macOS), оставляя Nunmai работать. Для выхода выберите «Выйти из Nunmai» в меню трея или нажмите Cmd+Q. По умолчанию выключено; действует только на этом устройстве.',
      ...
      loading: 'Загрузка конфигурации Nunmai…',
    ...
    hudModifier: {
      ...
      permission:
        'Разрешите Nunmai в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем повторите попытку. Жест не записывает нажатия клавиш и не снимает экран.',
      unavailable:
        'Вспомогательная программа жеста HUD не запустилась или неожиданно остановилась. Повторите попытку или перезапустите Nunmai. Обычное сочетание HUD по-прежнему работает внутри Nunmai.',
      missingHelper:
        'В этой установке Nunmai отсутствует вспомогательная программа жеста HUD. Обновите или переустановите Nunmai и повторите попытку.',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Nunmai. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Nunmai распознавать обе клавиши Command, когда активно другое приложение. Разрешите Nunmai доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Nunmai снимать переднее окно приложения при использовании этого сочетания. Разрешите Nunmai доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Nunmai, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Nunmai.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Nunmai, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Nunmai Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Nunmai Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Nunmai, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Nunmai, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Nunmai Cloud.',
      kindSshDesc: 'Установка Nunmai, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Nunmai (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Nunmai Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Nunmai в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте NUNMAI_DESKTOP_REMOTE_URL и NUNMAI_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Nunmai на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Nunmai.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в Nunmai Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Nunmai Cloud',
      cloudSignIn: 'Войти в Nunmai Cloud',
      cloudSignedIn: 'Вы вошли в Nunmai Cloud',
      cloudNeedsSignIn: 'Войдите в Nunmai Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Nunmai Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Nunmai Cloud',
      cloudSignedOutTitle: 'Выход из Nunmai Cloud',
      cloudSignedOutMessage: 'Сессия Nunmai Cloud сброшена.',
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /nunmai.',
      ...
      restartingMessage: 'Nunmai Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Nunmai ${version}` : ''}`,
      ...
      sshDesc:
        'Nunmai запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshNunmaiPathTitle: 'Путь к Nunmai (необязательно)',
      sshNunmaiPathDesc: 'Полный путь к бинарнику nunmai на удалённой машине. Пусто = автоопределение.',
      sshNunmaiPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Nunmai найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Nunmai запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Nunmai не установлен на удалённой машине. Установите его там (curl -fsSL https://nunmai-engine.nunmai.in/install.sh | sh) или задайте путь к Nunmai.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Nunmai Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Nunmai на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Nunmai. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Nunmai.',
    ...
    model: {
      moaPreset: 'Пресет',
      moaDescription:
        'Настройте именованные пресеты, которые появятся как модели провайдера Nunmai Agent. Модель-агрегатор выполняет каждый шаг цикла инструментов; почти вся стоимость запуска списывается у её провайдера. По умолчанию референсные модели дают совет один раз на сообщение пользователя.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Nunmai проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Nunmai на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
    ...
    toolsets: {
      ...
      nousIncluded: 'Входит в подписку Nunmai — войдите в Nunmai Portal, чтобы активировать.',
      nousAuthNeededTitle: 'Войдите в Nunmai Portal',
      nousAuthNeededMessage: provider => `${provider} сохранён, но не активируется, пока вы не войдёте в Nunmai Portal.`,
      nousAuthSignIn: 'Войти',
      nousAuthDoneTitle: 'Nunmai Portal подключён',
      nousAuthDoneMessage: 'Ваши бэкенды по подписке теперь активны.',
      nousAuthFailed: 'Вход в Nunmai Portal не завершён',
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через nunmai curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Nunmai создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Nunmai, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Nunmai, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Nunmai уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Nunmai desktop' },
    ...
    nunmaiActiveSessions: (version, count) => `Nunmai ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Оба способа подключают бота под вашим контролем и сохраняют его данные только в этой установке Nunmai.',
      quickSetup: 'Быстрая настройка',
      recommended: 'Рекомендуется',
      quickHelp:
        'Отсканируйте QR-код и подтвердите в Telegram. Nunmai создаст бота и определит ваш Telegram ID автоматически.',
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@nunmai:example.org' },
  ...
  profiles: {
    ...
    defaultDescription: 'Используется при запуске Nunmai и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Nunmai: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  modelAssignment: {
    saveFailed: 'Nunmai не сохранил это изменение модели.'
  },
  ...
  cron: {
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Nunmai выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Nunmai, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Nunmai. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Nunmai, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Nunmai...',
    placeholderReconnecting: 'Переподключение к Nunmai…',
    ...
    commandDescs: {
      ...
      '/goal': 'Задать постоянную цель, над которой Nunmai работает до завершения',
      ...
      '/plan': 'Записать план реализации в Markdown в .nunmai/plans/ без выполнения',
      '/moa': 'Выполнить запрос через Nunmai Agent, затем вернуть исходную модель',
      ...
      '/profile': 'Переключить активный профиль Nunmai',
      ...
      '/subscription': 'Показать план Nunmai и изменить его в браузере',
      '/topup': 'Показать баланс Nunmai и управлять оплатой',
      '/platform': 'Приостановить, возобновить или перечислить сбоящие платформы шлюза',
      '/version': 'Показать версию Nunmai Engine',
    ...
    attachUrlDesc: 'Nunmai загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Nunmai открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Nunmai…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Nunmai…',
    ...
    unsupportedMessage: 'Эта версия Nunmai не может обновлять себя из приложения.',
    connectionRetry: 'Проверьте соединение и попробуйте снова.',
    gitUnusable: 'Nunmai не удалось запустить Git на этом компьютере, поэтому проверить обновления не получилось.',
    ...
    availableBody: 'Новая версия Nunmai готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Nunmai готова к установке.',
    ...
    manualBody:
      'Nunmai установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Nunmai подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Nunmai (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Nunmai возьмёт процесс на себя в своём окне и автоматически перезапустит Nunmai, когда закончит. Пожалуйста, не открывайте Nunmai вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Nunmai переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Nunmai откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Nunmai?',
    blockerBody:
      'Nunmai нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Nunmai',
    foreignBlockerBody:
      'Nunmai не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Nunmai может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  guidedGreeting: {
    line: 'Заходите. Я Nunmai. Дайте мне пару минут — обустрою тут всё под вас, а потом займёмся тем, что вам правда нужно.\n\nДля начала: как к вам обращаться?',
  ...
  install: {
    ...
    oneTimeTitle: 'Nunmai требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Nunmai Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Nunmai или установите Nunmai локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Nunmai',
    ...
    installLocalTitle: 'Установить Nunmai локально',
    installLocalDesc: 'Скачайте Nunmai, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Nunmai Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Nunmai',
    ...
    settingUpTitle: 'Настройка Nunmai Engine',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Nunmai CLI или desktop. Остановите все работающие инстанции Nunmai и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Nunmai скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Nunmai Engine',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Nunmai завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Nunmai…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Nunmai',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Nunmai локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Nunmai подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Nunmai там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Nunmai там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Nunmai покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Nunmai перезапускается...',
      askRestart: 'Попросить Nunmai перезапустить сервер',
      lookingRestart: taskId => `Nunmai ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Nunmai работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Nunmai открыть страницу.',
      finishedRestarting: message => `Nunmai завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Nunmai не смог перезапустить сервер.',
      stillWorking:
        'Nunmai всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  interfaceMode: {
    ...
    hint: 'Меняет то, что показано, а не то, что умеет Nunmai.',
    sessionNote:
      'Задано простым режимом. Изменение здесь действует до конца сеанса; переключитесь в расширенный, чтобы сделать его своим.',
    simple: {
      label: 'Простой',
      description: 'Для общения с Nunmai. Боковая панель и чат; без терминала, файлов и панелей diff.'
    },
    advanced: {
      label: 'Расширенный',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Nunmai загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Nunmai не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.nunmai/config.yaml). Nunmai больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    sudoInstallDesc:
      'Nunmai нужен ваш пароль sudo, чтобы установить пакеты Bot Screen (TigerVNC + Xfce) на хосте шлюза. Он отправляется только на этот хост.',
    sudoPlaceholder: 'пароль sudo',
    secretTitle: 'Требуется секрет',
    secretDesc: 'Nunmai нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Nunmai Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Nunmai Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `nunmai gateway`?'
    }
  },
  errors: {
