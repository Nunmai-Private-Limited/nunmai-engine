import type { GatewayWsUrlResult } from '@nunmai/shared'
import type { TranslucencyState } from '@nunmai/shared/translucency'
...
import type { NunmaiNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    nunmaiDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`nunmai --tui
      ...
      // over nunmai:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      // Nunmai Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new NUNMAI_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: NunmaiApiRequest) => Promise<T>
      notify: (payload: NunmaiNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<NunmaiReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<NunmaiReadFileTextResult>
      selectPaths: (options?: NunmaiSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<NunmaiPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<NunmaiPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<NunmaiPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: NunmaiActiveWork) => void
      setTitleBarTheme?: (payload: NunmaiTitleBarTheme) => void
      ...
      /** Launch flag: the Nunmai free tier is on for this launch
       *  (NUNMAI_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (NUNMAI_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh NUNMAI_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<NunmaiReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<NUNMAI_HOME>/desktop-plugins),
      ...
      /** LOCAL `<NUNMAI_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<NunmaiGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<NunmaiGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<NunmaiGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<NunmaiRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: NunmaiReviewScope, baseRef?: null | string) => Promise<NunmaiReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: NunmaiReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<NunmaiReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<NunmaiRepoPullRequests>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: NunmaiTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<NunmaiTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: NunmaiWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: NunmaiPreviewFileChanged) => void) => () => void
...
export interface NunmaiTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface NunmaiTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `nunmai update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  nunmaiRoot?: string
...
export interface NunmaiConnection {
  baseUrl: string
...
export interface NunmaiTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface NunmaiActiveWork {
  count: number
  titles: string[]
}

export interface NunmaiWindowState {
  customWindowControls?: boolean
  darwinMajor?: number
  isFullscreen: boolean
  isMaximized?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Nunmai Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Nunmai Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Nunmai Cloud org (slug or id) to
...
// A discovered Nunmai Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface NunmaiApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface NunmaiPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface NunmaiReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface NunmaiPreviewWatch {
  id: string
  path: string
}
...
export interface NunmaiGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface NunmaiGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface NunmaiGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface NunmaiRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface NunmaiRepoStatus {
  branch: null | string
...
// turn began.
export type NunmaiReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface NunmaiReviewFile {
  path: string
  added: number
  removed: number
...
export interface NunmaiReviewList {
  files: NunmaiReviewFile[]
...
export interface NunmaiReviewPr {
  url: string
  state: string
  number: number
}
...
export interface NunmaiBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface NunmaiRepoPullRequests {
  ghReady: boolean
  prs: NunmaiBranchPullRequest[]
}
...
export interface NunmaiReviewShipInfo {
  ghReady: boolean
  pr: NunmaiReviewPr | null
}

export interface NunmaiReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface NunmaiReadDirResult {
  entries: NunmaiReadDirEntry[]
  error?: string
}

export interface NunmaiPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface NunmaiSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
