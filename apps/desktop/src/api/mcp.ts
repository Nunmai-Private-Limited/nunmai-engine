import type { McpCatalogResponse, McpServerSummary } from '@/types/nunmai'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.nunmaiDesktop.api<McpTestResult>({
 ...
 *  `saveNunmaiConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: ProfileScope): Promise<McpOAuthFlow> {
  return window.nunmaiDesktop.api<McpOAuthFlow>({
    ...capabilityScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return nunmaiApi<{ ok: boolean; status: string }>({
...
// `nunmai mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveNunmaiConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return nunmaiApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return nunmaiApi<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope): Promise<McpCatalogResponse> {
  return window.nunmaiDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.nunmaiDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
