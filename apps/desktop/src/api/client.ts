import { JsonRpcGatewayClient } from '@nunmai/shared'

import type { NunmaiApiRequest } from '@/global'
...
// "Timed out connecting to Nunmai backend" that hangs the UI (#48504).
//
...
export const GATEWAY_NOT_CONNECTED_MESSAGE = 'Nunmai gateway is not connected'

export class NunmaiGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Nunmai gateway connection closed',
      connectErrorMessage: 'Could not connect to Nunmai gateway',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Nunmai Desktop has no server-request registry for ${request.method} (${request.id})`),
 ...
 *  the ambient tag `nunmaiApi` spreads underneath (as capabilityScoped does). */
...
// process to route them to that host (nunmai:api's registry branch). Null
...
export function nunmaiApi<T>(request: NunmaiApiRequest): Promise<T> {
  return window.nunmaiDesktop.api<T>({ ...connectionScoped(), ...request })
}
