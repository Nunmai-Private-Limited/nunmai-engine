...
import { capabilityScoped, nunmaiApi, type ProfileScope, profileScoped } from './client'
...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return nunmaiApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return nunmaiApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `nunmai memory` / `nunmai curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return nunmaiApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return nunmaiApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return nunmaiApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return nunmaiApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function runCurator(): Promise<ActionResponse> {
  return nunmaiApi<ActionResponse>({
    ...profileScoped(),
...
export function updateNunmai(): Promise<ActionResponse> {
  return nunmaiApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/nunmai/update',
    method: 'POST'
  })
}
...
export function checkNunmaiUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return nunmaiApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/nunmai/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.nunmaiDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return nunmaiApi<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return nunmaiApi<AudioSpeakResponse>({
    ...profileScoped(),
...
export function setTtsLease(lease: string, active: boolean): Promise<AudioTtsLeaseResponse> {
  return nunmaiApi<AudioTtsLeaseResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return nunmaiApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return nunmaiApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `nunmai doctor` / `nunmai security
// audit` / `nunmai backup` / `nunmai debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return nunmaiApi<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return nunmaiApi<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return nunmaiApi<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return nunmaiApi<DebugShareResponse>({
