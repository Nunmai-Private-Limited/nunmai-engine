...
import { capabilityScoped, nunmaiApi, type ProfileScope, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getProfiles(): Promise<ProfilesResponse> {
  return nunmaiApi<ProfilesResponse>({
    path: '/api/profiles',
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return nunmaiApi<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string, scope?: ProfileScope): Promise<{ ok: boolean; path: string }> {
  ...
  return nunmaiApi<{ ok: boolean; path: string }>({
    ...profileOwnerScoped(scope),
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return nunmaiApi<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return nunmaiApi<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return nunmaiApi<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
