...
import { capabilityScoped, getApiRequestConnection, nunmaiApi, type ProfileScope, profileScoped } from './client'
...
export async function listSessions(
  ...
  const result = await nunmaiApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await nunmaiApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return nunmaiApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return nunmaiApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return nunmaiApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
