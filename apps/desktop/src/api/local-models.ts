import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/nunmai'

import { nunmaiApi, profileScoped } from './client'
...
export function getLocalModelsStatus(): Promise<LocalModelsStatus> {
  return nunmaiApi<LocalModelsStatus>({
    ...profileScoped(),
...
export function getLocalHardware(): Promise<LocalHardware> {
  return nunmaiApi<LocalHardware>({
    ...profileScoped(),
...
export function getLocalCatalog(): Promise<{ models: LocalCatalogModel[] }> {
  return nunmaiApi<{ models: LocalCatalogModel[] }>({
    ...profileScoped(),
...
export function installLocalRuntime(backend?: string): Promise<{ backend: string; job_id: string; tag: string }> {
  return nunmaiApi<{ backend: string; job_id: string; tag: string }>({
    ...profileScoped(),
...
export function quickstartLocalModels(modelId?: string): Promise<QuickstartResponse> {
  return nunmaiApi<QuickstartResponse>({
    ...profileScoped(),
...
export function downloadLocalModel(modelId: string): Promise<{ already_downloaded?: boolean; job_id: null | string }> {
  return nunmaiApi<{ already_downloaded?: boolean; job_id: null | string }>({
    ...profileScoped(),
...
export function getLocalRuntimeJob(jobId: string): Promise<LocalRuntimeJob> {
  return nunmaiApi<LocalRuntimeJob>({
    ...profileScoped(),
...
export function getLocalModelsJobs(): Promise<{ jobs: LocalRuntimeJob[] }> {
  return nunmaiApi<{ jobs: LocalRuntimeJob[] }>({
    ...profileScoped(),
...
export function activateLocalModel(modelId: string): Promise<{ job_id: string }> {
  return nunmaiApi<{ job_id: string }>({
    ...profileScoped(),
    body: { model_id: modelId },
    method: 'POST',
...
export function searchHFModels(q: string, limit = 20): Promise<{ hits: HFSearchHit[] }> {
  return nunmaiApi<{ hits: HFSearchHit[] }>({
    ...profileScoped(),
...
export function listHFRepoFiles(repo: string): Promise<{ files: HFFileGroup[] }> {
  return nunmaiApi<{ files: HFFileGroup[] }>({
    ...profileScoped(),
...
export function downloadBrowsedModel(
  ...
  return nunmaiApi<{ already_downloaded?: boolean; job_id: null | string; model_id: string }>({
    ...profileScoped(),
    body: { paths, repo },
    method: 'POST',
...
export function sideloadLocalModel(
  ...
  return nunmaiApi<{ already_present?: boolean; model_id: string; ok: boolean }>({
    ...profileScoped(),
    body: { path },
    method: 'POST',
