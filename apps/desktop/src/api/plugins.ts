import { reconnectBackoffDelayMs } from '@nunmai/shared'

import type { NunmaiConnection } from '@/global'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'

import { getApiRequestConnection, getApiRequestProfile, nunmaiApi, profileScoped } from './client'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<NunmaiConnection> {
  ...
  return withTimeout(
    window.nunmaiDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `nunmaiDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see NunmaiApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.nunmaiDesktop?.api) {
    throw new Error('Nunmai desktop bridge unavailable')
  }
