...
const nunmaiApi = vi.mocked(client.nunmaiApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    nunmaiApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'nunmai-pi' })

    await deleteSession('sess-1', { connectionId: 'nunmai-pi', profile: 'tommy' })

    expect(nunmaiApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'nunmai-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((nunmaiApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('getSession dial priority', () => {
  it('does not dial an explicitly scoped session read foreground', async () => {
    ...
    nunmaiApi.mockResolvedValue({ id: 'sess-5' } as never)
    ...
    expect(nunmaiApi.mock.calls[0][0]).toMatchObject({ profile: 'tommy', connectionId: 'local' })
    expect(nunmaiApi.mock.calls[0][0]).not.toHaveProperty('priority')
  })
})
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('falls back to the ACTIVE profile in the body when no owner is given', async () => {
    ...
    // `body.profile` and there is no per-profile backend whose NUNMAI_HOME
  ...
  it('omits the profile from the body only when there is no active profile at all', async () => {
    ...
    const req = nunmaiApi.mock.calls[0][0] as { body: Record<string, unknown> }
