...
): Promise<NunmaiConfigRecord> {
  const origin = capabilityScoped(profile ?? undefined)

  const record = await window.nunmaiDesktop.api<NunmaiConfigRecord>({ ...origin, ...request })
...
export function getNunmaiConfigRecord(
  profile?: ProfileScope,
...
export function saveNunmaiConfig(
...
/** Capability-scoped counterpart of saveNunmaiConfig — writes the config of
 *  the profile/connection the Capabilities scope selector points at (possibly
 *  on another registered gateway), mirroring getNunmaiConfigRecord. */
export function saveNunmaiConfigRecord(config: NunmaiConfigRecord, profile?: ProfileScope): Promise<{ ok: boolean }> {
  return window.nunmaiDesktop.api<{ ok: boolean }>({
    ...resolveConfigWriteScope(config, profile),
    path: '/api/config',
    method: 'PUT',
...
export function validateProviderCredential(
  ...
  return window.nunmaiDesktop.api<{
    ok: boolean
    reachable: boolean
    message: string
    models?: string[]
...
export function listOAuthProviders(profile?: ProfileScope): Promise<OAuthProvidersResponse> {
  return window.nunmaiDesktop.api<OAuthProvidersResponse>({
    ...capabilityScoped(profile),
...
export function submitOAuthCode(
  ...
  return window.nunmaiDesktop.api<OAuthSubmitResponse>({
    ...capabilityScoped(profile),
