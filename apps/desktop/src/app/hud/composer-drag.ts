...
function setWorkspaceTransfer(transferring: boolean): void {
  window.nunmaiDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.nunmaiDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.nunmaiDesktop?.hud?.beginMove?.()
}
 ...
 * (see nunmai:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.nunmaiDesktop?.hud?.endMove?.()
      }
