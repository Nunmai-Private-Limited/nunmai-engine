...
import { useNunmaiConfigRecord } from '../hooks/use-config-record'
...
import { useNunmaiConfig } from '../session/hooks/use-nunmai-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // "Restart Nunmai" from a toast: recycle the local backend the user is
  ...
  useEffect(() => {
    ...
    if (backendRestartRequest > 0) {
      if ($connection.get()?.mode === 'remote') {
        void reconnectGateway().catch(err => notifyError(err, translateNow('notifications.errors.restartNunmaiFailed')))
      ...
      void window.nunmaiDesktop
        ...
        .catch(err => notifyError(err, translateNow('notifications.errors.restartNunmaiFailed')))
    }
  ...
  const { refreshNunmaiConfig, sttEnabled, voiceMaxRecordingSeconds } = useNunmaiConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('nunmai:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('nunmai:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshNunmaiConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshNunmaiConfig])
  ...
  const configRecord = useNunmaiConfigRecord()
  ...
  const customWindowControls = connection?.customWindowControls ?? window.nunmaiDesktop?.windowControls?.custom ?? false
