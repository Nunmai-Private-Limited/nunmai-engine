...
import { getNunmaiConfigRecord, saveMcpServers } from '@/nunmai'
...
import { setNunmaiConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `nunmai://mcp/install` deep links. The payload is
 ...
 * confirming lets Nunmai spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getNunmaiConfigRecord())
      ...
      setNunmaiConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
