...
import { pathFromNunmaiDeepLink, resolveNunmaiOpenPath } from '@/lib/nunmai-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Nunmai" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.nunmaiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.nunmaiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // nunmai://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future nunmaiDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveNunmaiOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (nunmai://index-network/intent/1, nunmai://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromNunmaiDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.nunmaiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.nunmaiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
