...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { nunmaiDesktop: { readDir: typeof readDir } }).nunmaiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): NunmaiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { nunmaiDesktop: unknown }).nunmaiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: NunmaiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<NunmaiReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: NunmaiReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: NunmaiReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: NunmaiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<NunmaiReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.nunmaiDesktop is missing', async () => {
    delete (window as unknown as { nunmaiDesktop?: unknown }).nunmaiDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: NunmaiReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: NunmaiReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: NunmaiReadDirResult) => void) | undefined
  ...
  it('drops a child listing that was read before the preference flipped', async () => {
    const gitRoot = vi.fn(async () => '/p')
    const readFileDataUrl = vi.fn(async () => `data:text/plain;base64,${btoa('*.log\n')}`)
    let releaseChild: ((value: NunmaiReadDirResult) => void) | undefined
    ...
    readDir.mockImplementation(async path => {
      ...
      if (path === '/p/src') {
        return new Promise<NunmaiReadDirResult>(resolve => {
