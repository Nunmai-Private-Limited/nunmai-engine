...
import type { NunmaiReadDirEntry, NunmaiReadDirResult } from '@/global'
...
export type ProjectTreeEntry = NunmaiReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: NunmaiReadDirEntry) {
...
async function filterIgnored(entries: NunmaiReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<NunmaiReadDirResult> {
