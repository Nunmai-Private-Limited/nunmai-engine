...
const NUNMAI_PATHS_MIME = 'application/x-nunmai-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(NUNMAI_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(NUNMAI_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.nunmaiDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (nunmai --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.nunmaiDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[nunmai-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.nunmaiDesktop?.terminal?.write(sessionId, `${command}\r`)
