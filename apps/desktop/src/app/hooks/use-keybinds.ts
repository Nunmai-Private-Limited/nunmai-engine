...
export function useKeybinds(deps: KeybindRuntimeDeps): void {
  ...
  handlersRef.current = {
    ...
    'session.new': () => {
      ...
      window.dispatchEvent(new CustomEvent('nunmai:new-session-shortcut'))
    },
  ...
  useEffect(() => {
    const updateF12Ownership = () => {
      ...
      window.nunmaiDesktop?.setF12ShortcutActive?.(hasF12Binding || $capture.get() !== null)
    }
    ...
    return () => {
      stopBindings()
      stopCapture()
      window.nunmaiDesktop?.setF12ShortcutActive?.(false)
    }
  }, [])

  useEffect(() => {
    const stopF12Shortcut = window.nunmaiDesktop?.onF12Shortcut?.(input => {
