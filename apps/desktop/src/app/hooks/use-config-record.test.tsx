...
import type * as NunmaiApi from '@/nunmai'
import { bindConfigReadOrigin, getNunmaiConfigRecord } from '@/nunmai'
import { queryClient } from '@/lib/query-client'

import { NUNMAI_CONFIG_KEY, useNunmaiConfigRecord } from './use-config-record'

vi.mock('@/nunmai', async importOriginal => ({
  ...(await importOriginal<typeof NunmaiApi>()),
  getNunmaiConfigRecord: vi.fn()
}))
...
it('updates the write origin when a refetch replaces the displayed record', async () => {
  ...
  vi.mocked(getNunmaiConfigRecord).mockResolvedValueOnce(first).mockResolvedValueOnce(second)

  const { result } = renderHook(() => useNunmaiConfigRecord(), { wrapper })
  ...
  await queryClient.invalidateQueries({ queryKey: NUNMAI_CONFIG_KEY })
