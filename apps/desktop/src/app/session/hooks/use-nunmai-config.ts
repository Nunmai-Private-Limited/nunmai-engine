...
import { getNunmaiConfig, getNunmaiConfigDefaults } from '@/nunmai'
...
interface NunmaiConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useNunmaiConfig({ activeSessionIdRef }: NunmaiConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshNunmaiConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getNunmaiConfig(), getNunmaiConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Nunmai' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshNunmaiConfig, sttEnabled, voiceMaxRecordingSeconds }
}
