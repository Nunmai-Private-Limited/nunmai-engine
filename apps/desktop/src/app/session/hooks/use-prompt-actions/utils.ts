...
export async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.nunmaiDesktop?.readFileDataUrl(filePath)
...
export async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.nunmaiDesktop?.readFileDataUrlForAttach ?? window.nunmaiDesktop?.readFileDataUrl
...
export function renderRpcResult(response: unknown, name: string): string {
  ...
  if ('total' in r || 'input' in r || 'output' in r || 'calls' in r) {
    ...
    // Provider account limits (e.g. Codex quota windows) first, then Nunmai credits — same order as CLI /usage.
