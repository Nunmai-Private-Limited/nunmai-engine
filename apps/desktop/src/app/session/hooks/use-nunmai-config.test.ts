...
import { getNunmaiConfig } from '@/nunmai'
...
import { useNunmaiConfig } from './use-nunmai-config'

vi.mock('@/nunmai', () => ({
  getNunmaiConfig: vi.fn(),
  getNunmaiConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'nunmai.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getNunmaiConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getNunmaiConfig>>)

describe('useNunmaiConfig refreshNunmaiConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Nunmai' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useNunmaiConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshNunmaiConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useNunmaiConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getNunmaiConfig>>>()
    vi.mocked(getNunmaiConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useNunmaiConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshNunmaiConfig(true)
    })
    expect(getNunmaiConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getNunmaiConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getNunmaiConfig>>>()
    vi.mocked(getNunmaiConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useNunmaiConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshNunmaiConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getNunmaiConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getNunmaiConfig>>>()
    vi.mocked(getNunmaiConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshNunmaiConfig(true)
      refreshC = result.current.refreshNunmaiConfig(true)
    })
