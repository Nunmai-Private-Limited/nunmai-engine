import { registryBackendScopeKey } from '@nunmai/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshNunmaiConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshNunmaiConfig — two REST calls (config + defaults) per event, per
