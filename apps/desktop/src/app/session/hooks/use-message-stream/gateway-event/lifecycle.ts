...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'gateway.ready') {
    ...
    ingestBackendSkin((payload as { skin?: NunmaiSkin } | undefined)?.skin, { apply: false })
  ...
  if (event.type === 'setup.ready') {
    // The boot bootstrap (nunmai_cli/free_tier_bootstrap.py) resolved the
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (Nunmai activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as NunmaiSkin | undefined, { apply: true })
    }
