...
import type { NunmaiConfigRecord, SessionInfo } from '@/types/nunmai'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.nunmaiDesktop) {
      return
    }

    let alive = true

    void getNunmaiConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveNunmaiConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } }, writeScope)
      } catch (err) {
...
// / Program Files), which buried any files Nunmai wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.nunmaiDesktop` is
    ...
    const settings = window.nunmaiDesktop?.settings
