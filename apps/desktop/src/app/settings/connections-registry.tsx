...
/**
 * The auth mode the editor saves. A Nunmai Cloud gateway signs in through its
...
function editorFromConnection(conn: DesktopRegistryConnection): EditorState {
  return {
    ...
    remoteNunmaiPath: conn.remoteNunmaiPath || '',
 ...
 * agent sources (local runtime + any number of remote gateways / Nunmai Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.nunmaiDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.nunmaiDesktop.oauthLoginConnectionConfig(editorUrl)
    ...
    async (allowPlainTextToken = false) => {
      ...
      try {
        ...
        if (editor.kind === 'remote' || editor.kind === 'cloud') {
          ...
          payload.remoteNunmaiPath = editor.remoteNunmaiPath.trim()
        }
  ...
  // Fan out `nunmai update` to every eligible source; per-connection results
          ...
          {editor.kind === 'ssh' && (
            <>
              ...
              <ListRow
                action={
                  <Input
                    onChange={e => setEditor({ ...editor, remoteNunmaiPath: e.target.value })}
                    placeholder={t.settings.gateway.sshNunmaiPathPlaceholder}
                    value={editor.remoteNunmaiPath}
                  />
                }
                description={t.settings.gateway.sshNunmaiPathDesc}
                title={t.settings.gateway.sshNunmaiPathTitle}
              />
            </>
          )}
