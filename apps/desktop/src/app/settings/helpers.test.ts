...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: NunmaiConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: NunmaiConfigRecord = {}
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('NOUS_API_KEY')).toBe('Nunmai Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // NUNMAI_QWEN_ shares the NUNMAI_ stem with other integrations.
      expect(providerGroup('NUNMAI_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: NunmaiConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: NunmaiConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: NunmaiConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: NunmaiConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: NunmaiConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: NunmaiConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: NunmaiConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: NunmaiConfigRecord = { toolsets: ['memory'] }
      const next: NunmaiConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: NunmaiConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: NunmaiConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: NunmaiConfigRecord = { toolsets: [] }
      const next: NunmaiConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: NunmaiConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: NunmaiConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `nunmai config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: NunmaiConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: NunmaiConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: NunmaiConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: NunmaiConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: NunmaiConfigRecord = {}
      const draft: NunmaiConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: NunmaiConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: NunmaiConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: NunmaiConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: NunmaiConfigRecord = { toolsets: ['memory'] }
