...
import type { ToolsetConfig } from '@/types/nunmai'
...
const saveNunmaiConfigRecord = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/nunmai', () => ({
  // useNunmaiConfigRecord (via VoiceProviderFields) reads these from the barrel.
  ...
  saveNunmaiConfigRecord: (config: unknown, profile?: unknown) => saveNunmaiConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  getNunmaiConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveNunmaiConfig.mockResolvedValue({ ok: true })
  saveNunmaiConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveNunmaiConfigRecord).toHaveBeenCalled(), { timeout: 3000 })

    const saved = saveNunmaiConfigRecord.mock.calls.at(-1)?.[0] as Record<
      string,
    ...
    expect(saveNunmaiConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    const [saved, forwarded] = saveNunmaiConfigRecord.mock.calls.at(-1) as [
