...
import { type ProfileScope, saveNunmaiConfigRecord } from '@/nunmai'
 ...
 * the default browser's profile (cookies/logins) into a Nunmai-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config, writeScope } = useNunmaiConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveNunmaiConfigRecord({ browser: { use_real_profile: on } }, writeScope ?? profile)
