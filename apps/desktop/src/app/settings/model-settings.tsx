...
import { DEFAULT_REASONING_EFFORT, isReasoningEffort, REASONING_EFFORT_VALUES } from '@nunmai/shared'
...
import { nunmaiConfigCacheWriter, invalidateNunmaiConfig, useNunmaiConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `nunmai model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in nunmai_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile, subpage }: ModelSettingsProps) {
  ...
  const { data: config, writeScope } = useNunmaiConfigRecord(scopeProfile)
  const setConfig = useMemo(() => nunmaiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateNunmaiConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveNunmaiConfig(setNested({}, key, value), writeScope ?? scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `nunmai model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.nunmaiDesktop?.recycleBackend?.(scopeProfile)
          ...
          {needsSetup && !setupIsApiKey && selectedProviderRow && (
            <p className="mt-2 text-xs text-muted-foreground">
              {selectedProviderRow?.auth_type === 'api_key'
                ...
                : `${selectedProviderRow?.name} signs in through your browser — Nunmai runs the flow for you.`}
            </p>
          )}
