...
describe('ConnectionsRegistrySection', () => {
  it('refreshes a cached roster immediately after a successful connection test', async () => {
    ...
    Object.assign(window.nunmaiDesktop!, { getAgentRoster })
  ...
  it('signs a hand-registered Cloud connection in and saves it as oauth (#89529)', async () => {
    ...
    Object.assign(window.nunmaiDesktop!, { oauthLoginConnectionConfig })

    render(<ConnectionsRegistrySection />)

    await screen.findByText('Homelab')
    fireEvent.click(screen.getByText('Add connection'))
    fireEvent.click(screen.getByRole('button', { name: 'Nunmai Cloud' }))
    fireEvent.change(screen.getByPlaceholderText('Homelab'), { target: { value: 'Team cloud' } })
    fireEvent.change(screen.getByPlaceholderText('http://homelab.lan:9119'), {
      target: { value: 'https://team.nunmai.cloud' }
    })
    ...
    await waitFor(() => expect(oauthLoginConnectionConfig).toHaveBeenCalledWith('https://team.nunmai.cloud'))
    ...
    expect(save.mock.calls[0][0]).toMatchObject({
      ...
      kind: 'cloud',
      label: 'Team cloud',
      url: 'https://team.nunmai.cloud'
    })
    expect(save.mock.calls[0][0].token).toBeUndefined()
  })

  it('saves a custom remote Nunmai path for SSH connections', async () => {
    render(<ConnectionsRegistrySection />)
    ...
    fireEvent.change(screen.getByPlaceholderText('auto-detect'), {
      target: { value: '/opt/nunmai/bin/nunmai' }
    })
    fireEvent.click(screen.getByText('Save connection').closest('button')!)
    ...
    expect(save.mock.calls[0][0]).toMatchObject({
      ...
      kind: 'ssh',
      label: 'Build host',
      remoteNunmaiPath: '/opt/nunmai/bin/nunmai'
    })
  })

  it('clears a saved remote Nunmai path back to auto-detect', async () => {
    ...
    expect(pathInput.value).toBe('/opt/nunmai/bin/nunmai')
    fireEvent.change(pathInput, { target: { value: '   ' } })
    fireEvent.click(screen.getByText('Save connection').closest('button')!)

    await waitFor(() => expect(save).toHaveBeenCalledTimes(1))
    expect(save.mock.calls[0][0]).toMatchObject({ id: 'build-host', remoteNunmaiPath: '' })
  })
