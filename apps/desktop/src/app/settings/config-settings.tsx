...
import { getElevenLabsVoices, getNunmaiConfigSchema, saveNunmaiConfig } from '@/nunmai'
...
import type { ConfigFieldSchema, NunmaiConfigRecord } from '@/types/nunmai'

import { nunmaiConfigCacheWriter, useNunmaiConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<NunmaiConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useNunmaiConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => nunmaiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['nunmai-config-schema'] : ['nunmai-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getNunmaiConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `nunmai config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<NunmaiConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveNunmaiConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: NunmaiConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: NunmaiConfigRecord) => {
