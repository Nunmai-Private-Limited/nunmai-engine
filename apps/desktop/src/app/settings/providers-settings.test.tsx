...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `nunmai auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'nous' ? 'Nunmai Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Nunmai Portal' })
    await act(async () => {
  ...
  it('leaves the account connected when the removal prompt is dismissed', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByRole('button', { name: 'Remove Nunmai Portal' }))
    })
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Nunmai Portal'))
    })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `nunmai auth add qwen-oauth` or that provider's CLI to remove it.",
