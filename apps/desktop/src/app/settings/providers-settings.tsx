...
import { disconnectOAuthProvider, listOAuthProviders } from '@/nunmai'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/nunmai'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.nunmaiDesktop?.terminal)
...
//      in nunmai_cli/provider_catalog.py) — the SAME provider identity
//      `nunmai model` uses. This is authoritative: a provider tagged by the
...
function OAuthPicker({
  ...
  // The free tier holds a token but no account: it is never "connected"; the featured Nunmai row
  // names it (Nunmai · free tier) and offers the sign-in that keeps its connectors.
...
function ConnectedProviderRow({
  ...
  // Nunmai can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Nunmai can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Nunmai never deletes creds another tool owns behind a silent API call.
