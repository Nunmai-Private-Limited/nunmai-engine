...
import { getProfiles } from '@/nunmai'
...
// User-installed plugins first — mirrors `nunmai plugins list --user`.
...
// Agent plugins live under the BACKEND's nunmai home (profile-aware), so the
// path comes from the gateway — not from the renderer's local NUNMAI_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.nunmaiDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Nunmai (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
