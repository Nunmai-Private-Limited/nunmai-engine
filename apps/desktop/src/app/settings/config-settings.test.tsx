...
const getNunmaiConfigSchema = vi.fn()
...
vi.mock('@/nunmai', async () => ({
  ...
  getNunmaiConfigRecord: (profile?: string) => getNunmaiConfigRecord(profile),
  getNunmaiConfigSchema: () => getNunmaiConfigSchema(),
  saveNunmaiConfig: (config: unknown, profile?: string) => saveNunmaiConfig(config, profile),
...
beforeEach(() => {
  ...
  getNunmaiConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getNunmaiConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveNunmaiConfig).toHaveBeenCalledTimes(1))
      expect(saveNunmaiConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveNunmaiConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveNunmaiConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('threads the "Applies to" request scope into both the config read and the autosave write', async () => {
    ...
    getNunmaiConfigRecord.mockResolvedValue({ checkpoints: { enabled: false } })
    ...
    try {
      ...
      await vi.waitFor(() => expect(getNunmaiConfigRecord).toHaveBeenCalledWith('nash'))

      ;(await screen.findByRole('switch')).click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveNunmaiConfig).toHaveBeenCalledWith({ checkpoints: { enabled: true } }, 'nash'))
    } finally {
      vi.useRealTimers()
    }
  })
