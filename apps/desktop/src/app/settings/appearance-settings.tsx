...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useNunmaiConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveNunmaiConfig(setNested({}, 'display.resume_last_session', on), writeScope)
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.nunmaiDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
