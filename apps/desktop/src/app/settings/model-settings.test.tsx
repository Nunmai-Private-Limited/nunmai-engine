...
const getNunmaiConfigRecord = vi.fn()
const saveNunmaiConfig = vi.fn()
...
vi.mock('@/nunmai', async () => ({
  ...
  getNunmaiConfigRecord: () => getNunmaiConfigRecord(),
  saveNunmaiConfig: (config: unknown) => saveNunmaiConfig(config),
...
beforeEach(() => {
  ...
  getNunmaiConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveNunmaiConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) as a sparse patch, never the cached snapshot', async () => {
    ...
    getNunmaiConfigRecord.mockResolvedValue({
      agent: { reasoning_effort: 'medium', service_tier: 'normal' },
      auxiliary: { curator: { provider: 'auto', model: '', reasoning_effort: 'high' } }
    })
    await renderModelSettings()
    await waitFor(() => expect(getNunmaiConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() => expect(saveNunmaiConfig).toHaveBeenCalledWith({ agent: { service_tier: 'fast' } }))
  })
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'nunmai:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Nunmai Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/nunmai:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { nunmaiDesktop: { recycleBackend: typeof recycleBackend } }).nunmaiDesktop = {
      recycleBackend
    }
