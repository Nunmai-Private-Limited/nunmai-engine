...
import * as nunmai from '@/nunmai'

const mocked = vi.mocked(nunmai)
...
describe('BrowseSection', () => {
  ...
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(nunmai.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(nunmai.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(nunmai.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(nunmai.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(nunmai.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(nunmai.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(nunmai.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
    ...
    vi.mocked(nunmai.getLocalCatalog).mockResolvedValue({ models: [] })
