...
describe('GatewaySettings', () => {
  it('reconnects a moved agent under its new team without replacing its saved identity or changing another default', async () => {
    ...
    Object.assign(window.nunmaiDesktop, {
  ...
  // #114856: an env-pinned remote (NUNMAI_DESKTOP_REMOTE_URL) whose session
  ...
  describe('env-override remote', () => {
    ...
    const oauthProbe = {
      authMode: 'oauth',
      providers: [{ displayName: 'Nunmai Research', name: 'nous', supportsPassword: false }],
      reachable: true
    }
    ...
    it('reaches sign-in for a lapsed session instead of a dead env banner', async () => {
      ...
      Object.assign(window.nunmaiDesktop, { oauthLoginConnectionConfig, probeConnectionConfig })
      ...
      fireEvent.click(await screen.findByRole('button', { name: 'Sign in with Nunmai Research' }))
    ...
    it('leaves a saved (non-env) remote session editable and unchanged', async () => {
      ...
      expect(await screen.findByRole('button', { name: 'Sign in with Nunmai Research' })).toBeTruthy()
