...
import {
  getElevenLabsVoices,
  getNunmaiConfigSchema,
  type ProfileScope,
  profileScopeKey,
  saveNunmaiConfigRecord
} from '@/nunmai'
...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig, writeScope } = useNunmaiConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => nunmaiConfigCacheWriter(profile), [scopeKey])

  const { data: schemaResponse } = useQuery({
    queryKey: ['nunmai-config-schema'],
    queryFn: () => getNunmaiConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<NunmaiConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveNunmaiConfigRecord(diffConfig(baseline ?? {}, config), writeScope ?? profile)
        .then(() => {
          setBaseline(config)
