...
// Nunmai Cloud discovery lifecycle for the cloud-mode panel.
...
function GatewayConnectionSettings({ embedded, standalone }: { embedded: boolean; standalone: boolean }) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.nunmaiDesktop.setSecretStorageEncryption(on)
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.nunmaiDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteNunmaiPath: state.sshRemoteNunmaiPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.nunmaiDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.nunmaiDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'nunmai-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.nunmaiDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.nunmaiDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.nunmaiDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.nunmaiDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.nunmaiDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.nunmaiDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.nunmaiDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.nunmaiDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.nunmaiDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.nunmaiDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Nunmai Cloud panel: one portal sign-in, then a discovered-agent picker
      ...
      {/* An env-pinned remote (NUNMAI_DESKTOP_REMOTE_URL) still renders this
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/nunmai"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteNunmaiPath: event.target.value }))}
                placeholder={g.sshNunmaiPathPlaceholder}
                value={state.sshRemoteNunmaiPath}
              />
            }
            description={g.sshNunmaiPathDesc}
            title={g.sshNunmaiPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.nunmaiDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
