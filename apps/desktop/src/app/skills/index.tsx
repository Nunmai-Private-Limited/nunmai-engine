...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/nunmai'
...
export function SkillsView({
  ...
  const registryBridge = window.nunmaiDesktop?.connections
  const rosterBridge = window.nunmaiDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Nunmai (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Nunmai ships but
