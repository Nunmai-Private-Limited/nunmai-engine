...
export const NUNMAI_PATHS_MIME = 'application/x-nunmai-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Nunmai sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.nunmaiDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(NUNMAI_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob, isCurrent: () => boolean = () => true) => {
      ...
      try {
        ...
        const savedPath = await window.nunmaiDesktop?.saveImageBuffer(data, blobExtension(blob), name)
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.nunmaiDesktop?.saveClipboardImage()
   ...
   * Nunmai-managed composer-pastes file via the main process, then attached
  ...
  const attachPastedText = useCallback(
    async (text: string) => {
      const save = window.nunmaiDesktop?.savePastedText
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.nunmaiDesktop?.getPathForFile ? window.nunmaiDesktop.getPathForFile(file) : ''
