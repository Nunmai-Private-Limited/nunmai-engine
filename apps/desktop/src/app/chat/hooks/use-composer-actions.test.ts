...
describe('partitionDroppedFiles', () => {
  ...
  it('routes an OS folder drop (path-only, isDirectory) to inAppRefs, not the upload pipeline', () => {
    ...
    const folder = inAppRef('/Users/jeff/projects/nunmai', { isDirectory: true })
...
function stubTransfer(
  ...
  return {
    getData: (mime: string) => (mime === NUNMAI_PATHS_MIME ? internalRaw : mime === 'text/uri-list' ? uriList : ''),
    files: {
      length: files.length,
...
describe('extractDroppedFiles', () => {
  ...
  it('emits a dropped directory as a path-only entry with isDirectory (no File to upload)', () => {
    const transfer = stubTransfer([{ path: '/Users/jeff/projects/nunmai', isDirectory: true }]) as DataTransfer & {
    ...
    expect(result[0]?.path).toBe('/Users/jeff/projects/nunmai')
...
describe('attachmentPreviewDataUrl', () => {
  ...
  it('reads a local path via the local bridge even in remote mode (paperclip/paste/OS drop)', async () => {
    ...
    vi.stubGlobal('window', { nunmaiDesktop: { api, readFileDataUrl } })
...
describe('useComposerActions native image drops', () => {
  ...
  it('does not attach a screenshot when its draft changes during native image saving', async () => {
    ...
    const add = vi.fn()
    Object.defineProperty(window, 'nunmaiDesktop', { configurable: true, value: { saveImageBuffer } })
  ...
  it('copies dropped screenshot bytes before trusting a transient macOS path', async () => {
    ...
    const durablePath = '/Users/test/Library/Application Support/Nunmai/composer-images/composer_saved.png'
...
describe('useComposerActions generated paste title metadata', () => {
  ...
  it('marks only a Nunmai-generated large paste with a bounded title preview', async () => {
    const savePastedText = vi.fn(async () => '/tmp/composer-pastes/pasted-content.txt')
    const add = vi.fn<(attachment: ComposerAttachment) => void>()
    Object.defineProperty(window, 'nunmaiDesktop', { configurable: true, value: { savePastedText } })
...
describe('attachImagePath thumbnail separation', () => {
  ...
  it('does not resurrect an attachment removed while its queued resize is in flight', async () => {
    ...
    ;(
      window as unknown as {
        nunmaiDesktop: { readFileDataUrl: typeof readFileDataUrl }
      }
    ).nunmaiDesktop = { readFileDataUrl }
  ...
  it('merges a delayed thumbnail into the latest staged state of the same occurrence', async () => {
    ...
    updateComposerAttachment({
      ...
      label: 'photo.png',
      path: '/root/.nunmai/attachments/photo.png',
