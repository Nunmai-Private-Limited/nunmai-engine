...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/nunmai'
...
const SIDEBAR_NAV: SidebarNavItem[] = [
  ...
  // Artifacts and Scheduled jobs are outputs of running Nunmai the developer
...
// highlight markers (sqlite snippet() delimiters — see nunmai_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('nunmai:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('nunmai:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Nunmai to
