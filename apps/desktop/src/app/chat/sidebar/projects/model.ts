...
import type { SessionInfo } from '@/nunmai'
...
// zero Nunmai sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, NunmaiGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, NunmaiGitWorktree[]>>({})
