...
import { capabilityScoped, nunmaiApi, type ProfileScope } from '@/api/client'
import {
  cachedTimelineIndex,
  previousPromptRowId,
  timelineIndexKey
...
import type { SessionMessagesResponse } from '@/types/nunmai'
...
export async function fetchHistoryWindow(
  ...
  const response = await nunmaiApi<HistoryWindowResponse>({
    ...route,
