...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__nunmaiTourEngine) {
    ${driverIife}
    w.__nunmaiTourHolder = {};
    w.__nunmaiTourCollect = (${collectTourTargets.toString()});
    w.__nunmaiTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__nunmai-tour-style')) {
    var style = document.createElement('style');
    style.id = '__nunmai-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__nunmaiTourEngine(
    w.driver.js.driver,
    w.__nunmaiTourHolder,
    ${JSON.stringify(action)},
    w.__nunmaiTourCollect,
    document
  ));
})()`
}
