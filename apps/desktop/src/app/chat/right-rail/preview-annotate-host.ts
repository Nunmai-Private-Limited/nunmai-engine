...
export function overlayInstallScript(source: string): string {
  return '(function(){var api=' + source + ';window.__nunmaiAnnotate=api;api.install();})()'
}
...
export async function teardownAnnotateOverlay(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    (function () {
      if (window.__nunmaiAnnotate && window.__nunmaiAnnotate.teardown) {
        window.__nunmaiAnnotate.teardown();
      }
      window.__nunmaiAnnotate = null;
    })()
  `)
}
...
export async function waitAnnotateEvent(guest: PreviewAnnotateGuest): Promise<AnnotatePageEvent> {
  const event = await guest.executeJavaScript(`
    window.__nunmaiAnnotate ? window.__nunmaiAnnotate.wait() : Promise.resolve({ type: 'end' })
  `)
...
export async function syncAnnotatePins(guest: PreviewAnnotateGuest, pins: AnnotatePinChrome[]): Promise<void> {
  await guest.executeJavaScript(`
    window.__nunmaiAnnotate && window.__nunmaiAnnotate.showPins(${JSON.stringify(pins)})
  `)
}
...
export async function showAnnotateDraft(
  ...
  await guest.executeJavaScript(`
    window.__nunmaiAnnotate && window.__nunmaiAnnotate.showDraft(${JSON.stringify(rect)}, ${number})
  `)
}

export async function hideAnnotateDraft(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    window.__nunmaiAnnotate && window.__nunmaiAnnotate.hideDraft()
  `)
}
...
export async function captureAnnotateCrop(
  ...
  await tryGuest(guest, 'window.__nunmaiAnnotate ? window.__nunmaiAnnotate.beginCapture() : null')

  try {
    return await guest.capture(padRect(rect))
  } finally {
    await tryGuest(guest, 'window.__nunmaiAnnotate && window.__nunmaiAnnotate.endCapture()')
  }
}
