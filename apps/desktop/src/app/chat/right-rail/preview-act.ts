...
const preamble = () => `  var w = window;
  ...
  w.__nunmaiActHolder = w.__nunmaiActHolder || {};
  w.__nunmaiAct = ${actEngineSource()};
  w.__nunmaiWatch_fn = (${watchInPage.toString()});
  w.__nunmaiWatchTag = ${WATCH_TAG};
  var holder = w.__nunmaiActHolder;
  var act = function (a) { return w.__nunmaiAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__nunmaiWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__nunmaiHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'NUNMAI-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
