        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.nunmaiDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.nunmaiDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Nunmai to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.nunmaiDesktop?.watchPreviewFile ||
      !window.nunmaiDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.nunmaiDesktop.onPreviewFileChanged(payload => {
    ...
    void window.nunmaiDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.nunmaiDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.nunmaiDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:nunmai-preview')
    ...
    // `nunmai:openExternal` channel, never a popup side effect.
    const onGuestExternal = (event: Event) => {
      ...
      if (admitPreviewExternalUrl(url)) {
        void window.nunmaiDesktop?.openExternal?.(url)
      }
    }
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.nunmaiDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.nunmaiDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.nunmaiDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.nunmaiDesktop?.openExternal(currentUrl)
                : undefined
            }
