...
import { ContextMenu, ContextMenuTrigger, NUNMAI_CONTEXT_MENU_TRIGGER_ATTR } from '@/components/ui/context-menu'
...
function installBridge(partial: Partial<Window['nunmaiDesktop']> = {}) {
...
afterEach(() => {
  ...
  delete desktopWindow.nunmaiDesktop
})
...
describe('AppContextMenu', () => {
  ...
  it('offers the resolved copy only for loopback links on a remote gateway', async () => {
    ...
    installBridge({
      reachPreviewUrl: reachPreviewUrl as unknown as Window['nunmaiDesktop']['reachPreviewUrl'],
      writeClipboard: writeClipboard as unknown as Window['nunmaiDesktop']['writeClipboard']
    })
    mountMenu()
  ...
  it('runs edit verbs after the menu closed, with focus back on the editable', async () => {
    ...
    installBridge({ contextMenuEdit: contextMenuEdit as unknown as Window['nunmaiDesktop']['contextMenuEdit'] })
    mountMenu()
  ...
  it('keeps a modal textarea paste menu inside its dialog and restores focus', async () => {
    ...
    installBridge({
      contextMenuEdit: contextMenuEdit as unknown as Window['nunmaiDesktop']['contextMenuEdit'],
  ...
  it('keeps paste clickable even when the clipboard probe reports empty', async () => {
    ...
    installBridge({ readClipboard: readClipboard as unknown as Window['nunmaiDesktop']['readClipboard'] })
    mountMenu()
    const host = attach('<textarea>text</textarea>')
...
describe('ContextMenuTrigger asChild', () => {
  it('keeps the coordinator marker when the child overwrites data-slot', () => {
    ...
    expect(footer.hasAttribute(NUNMAI_CONTEXT_MENU_TRIGGER_ATTR)).toBe(true)
  })
})
