import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@nunmai/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as NunmaiGateway | null
  const gatewayRef = useRef<NunmaiGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.nunmaiDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<NunmaiGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as NunmaiGateway | null
      }),
    []
  )
