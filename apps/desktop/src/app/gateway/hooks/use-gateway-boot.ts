...
import type { DesktopBootProgress, NunmaiConnection, NunmaiWindowState } from '@/global'
import { NunmaiGateway } from '@/nunmai'
...
export function primaryRuntimeConnectionId(connection: Pick<NunmaiConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.nunmaiDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: NunmaiGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    let pendingWindowState: NunmaiWindowState | null = null

    const publish = (next: NunmaiConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Nunmai...". Once the
    ...
    // The other way a cold boot concludes. Main keeps startNunmai() available
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Nunmai…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Nunmai backend'
        )
        ...
        // as an opaque "Could not connect to Nunmai gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshNunmaiConfig().catch(() => undefined)
    ...
    async function getWindowBackend(startup = false): Promise<NunmaiConnection> {
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshNunmaiConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startNunmai re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new NunmaiGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Nunmai Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Nunmai gateway is not connected')
        }
    ...
    const offExit = desktop.onBackendExit(() => {
      ...
      notify({
        ...
        durationMs: 0,
        action: {
          label: translateNow('boot.errors.restartNunmai'),
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Nunmai…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Nunmai backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Nunmai…".
        const wsUrl = await withTimeout(
          resolveDesktopGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshNunmaiConfig(),
          ...
          // retry it), not brick boot behind the "Nunmai couldn't start"
