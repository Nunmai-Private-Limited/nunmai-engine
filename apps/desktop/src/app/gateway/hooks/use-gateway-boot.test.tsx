...
// bug that drives the REAL useGatewayBoot hook + REAL NunmaiGateway through a
...
function Harness({
  ...
  refreshNunmaiConfig = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshNunmaiConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { nunmaiDesktop?: unknown }).nunmaiDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { nunmaiDesktop?: unknown }).nunmaiDesktop
  window.localStorage.removeItem('nunmai.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
it('loads and tracks saved gateways without mounting the statusbar or Settings', async () => {
  ...
  type Listener = Parameters<NonNullable<Window['nunmaiDesktop']['connections']['onChanged']>>[0]
  ...
  ;(window as { nunmaiDesktop?: unknown }).nunmaiDesktop = desktop
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  ...
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForNunmai) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startNunmai()'s remote branch awaits waitForNunmai() for 45s before it
    ...
    // After ~45s waitForNunmai gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Nunmai backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshNunmaiConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshNunmaiConfig={refreshNunmaiConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForNunmai, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting Nunmai…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { nunmaiDesktop?: unknown }).nunmaiDesktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Nunmai gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Nunmai couldn't start" overlay — the socket is open
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // Nunmai" toast here would do nothing when clicked; the overlay's own
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Nunmai')
    expect($currentCwd.get()).toBe('C:\\Nunmai')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startNunmai() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshNunmaiConfig = vi.fn(async () => {
    ...
    expect(refreshNunmaiConfig).toHaveBeenCalledTimes(1)
    expect($desktopBoot.get().error).toBeTruthy()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
    await advanceBackoff()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
  ...
  it('a failed cold boot keeps its recovery surface while main replays cold-boot progress behind it (#112899)', async () => {
    // Main keeps startNunmai() available after the renderer's boot concluded
    ...
    const desktop = fakeDesktop()
    desktop.getConnection = vi.fn(async () => {
      throw new Error('Nunmai backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example')
    })
    desktop.getBootProgress = vi.fn(async () => ({
      error: 'Nunmai backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example',
      fakeMode: false,
      message: 'Desktop boot failed',
      phase: 'backend.error',
      progress: 24,
    ...
    for (let replay = 0; replay < 2; replay += 1) {
      act(() => {
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Resolving Nunmai backend',
        ...
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Connecting to remote Nunmai backend at https://gateway.tailnet.example:8443',
          phase: 'backend.remote',
          progress: 24,
          running: true,
          timestamp: Date.now()
