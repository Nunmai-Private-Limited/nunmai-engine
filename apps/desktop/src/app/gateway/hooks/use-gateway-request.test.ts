...
vi.mock('@/nunmai', async importOriginal => {
  const actual = await importOriginal<typeof NunmaiModule>()

  class FakeNunmaiGateway {
  ...
  return { ...actual, NunmaiGateway: FakeNunmaiGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as NunmaiGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as NunmaiGateway, 'default')
  $gateway.set(primary as unknown as NunmaiGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.nunmaiDesktop, 'getGatewayWsUrlFor')
  ...
  it.each(['token', 'oauth'] as const)(
    ...
    async authMode => {
      ...
      Object.defineProperty(window, 'nunmaiDesktop', { configurable: true, value: desktop })

      const primary = makePrimaryGateway()
      primary.request.mockRejectedValueOnce(new Error('connection closed')).mockResolvedValueOnce({ recovered: true })
      setPrimaryGateway(primary as unknown as NunmaiGateway, connection.profile)
      $gateway.set(primary as unknown as NunmaiGateway)
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as NunmaiGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { nunmaiDesktop: unknown }).nunmaiDesktop = { getConnection }
    $gateway.set(dropped)
