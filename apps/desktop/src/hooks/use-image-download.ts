...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'nunmai:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.nunmaiDesktop?.saveImageFromUrl) {
        if (await window.nunmaiDesktop.saveImageFromUrl(src)) {
