 ...
 *    desktop shell fires only while the user is AWAY from Nunmai. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@nunmai/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Nunmai (the toast above covers the foreground case). Isolated:
