import type { PluginRestOptions } from '@nunmai/plugin-sdk'
...
vi.mock('@/nunmai', () => ({ setApiRequestProfile: vi.fn() }))
...
describe('task attachment compatibility', () => {
  it('downloads the persisted attachment through its original remote owner', async () => {
    ...
    vi.stubGlobal('nunmaiDesktop', { saveGatewayFile: save })
  ...
  it('keeps a local-backend attachment on this computer after switching to a remote', async () => {
    ...
    detail = {
      ...legacyDetail,
      attachments: [{ id: 7, filename: 'notes.txt', stored_path: '/home/me/.nunmai/kanban/notes.txt' }]
    }
    openDrawer()
    ...
    await waitFor(() =>
      expect(save).toHaveBeenCalledWith({
        connectionId: 'local',
        path: '/home/me/.nunmai/kanban/notes.txt',
...
describe('task modal dialog', () => {
  ...
  it('shows the workspace path as its own value, not prefixed with the raw kind', async () => {
    const path = '/Users/example/.nunmai/kanban/workspaces/a_very_long_directory_name_that_must_wrap'
    detail = {
      ...legacyDetail,
      attachments: [],
