...
describe('routing', () => {
  ...
  it('resolves @nunmai to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@nunmai take a look', members)
  ...
  it('stamps a local member reply with its connection label and keeps (you) for that member only', async () => {
    ...
    room.rounds.sendToGroupChat('Core', [local], '@nunmai status?')
    await settle(room, 'Core')
  ...
  it('never re-drives a local member on its own sourced reply', async () => {
    ...
    expect(log(room, 'Core').map(entry => entry.text)).toEqual(['@nunmai status?', 'Reply 1 from this device.'])
  })
...
describe('turn prompt', () => {
  it('addresses the default profile as @nunmai', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @nunmai,/)
    ...
    expect(peer).toMatch(/group chat with @nunmai/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @nunmai made it treat
