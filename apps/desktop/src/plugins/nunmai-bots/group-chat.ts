...
import { atom, host } from '@nunmai/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'nunmai-bots-groups'
...
/** The v3 envelope stored under the default profile's `nunmai-bots-groups`
 ...
 *  clients (Nunmai Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Nunmai"
 *  (community report, Aug 21 2026: renamed default still read "Nunmai is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Nunmai (matching displayName and the
 *  @nunmai handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "Nunmai" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Nunmai' : profile)
  ...
  // title, then the local row's display_name, then default → Nunmai.
  ...
  return isDefault ? 'Nunmai' : trimmed
}
