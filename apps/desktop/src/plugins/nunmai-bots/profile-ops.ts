...
import { forgetSessionUnread, host, queryClient } from '@nunmai/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['nunmai-bots']
...
/** Permanently delete a bot's Nunmai profile, then remove plugin-local state
 ...
 * directory (nunmai-engine#52279). That is the "can't delete a bot" error. */
