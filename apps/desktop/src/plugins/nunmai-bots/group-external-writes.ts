 ...
 *
 * A member's hidden per-group session is an ordinary Nunmai session, so other
 * writers legitimately append to it: the user resuming it from the CLI
 * (`nunmai -p <bot> chat --resume "Group: <room> · <thread>"`), a cron job, the
...
/** The Nunmai-authored assistant row that closes a turn which failed before
