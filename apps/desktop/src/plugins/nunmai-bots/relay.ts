...
import { host, LruCache } from '@nunmai/plugin-sdk'
...
// to (local, remote URL, SSH, Nunmai Cloud, docker) must be able to find
...
// nunmai_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['nunmai-bots']?.title || profile?.display_name || ''),
