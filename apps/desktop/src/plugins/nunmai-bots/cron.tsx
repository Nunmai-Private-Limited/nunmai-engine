/**
 * Routines: the Nunmai cron jobs scoped to the bot you're chatting with — the
...
// nunmai -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `nunmai -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Nunmai-native schedule string; Advanced exposes it raw.
...
/** Compose the Nunmai schedule string from picker state. */
