...
describe('session-gone classification', () => {
  ...
  // The core loop closes a failed turn with a Nunmai-authored assistant row
  ...
  it('reports a failed turn whose transcript Nunmai closed with the failed-turn notice', async () => {
    let now = 1_000_000
    const clock = vi.spyOn(Date, 'now').mockImplementation(() => (now += 60_000))
