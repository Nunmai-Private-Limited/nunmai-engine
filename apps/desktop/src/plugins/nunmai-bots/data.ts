...
import { atom, host, queryClient, useQuery, useValue } from '@nunmai/plugin-sdk'
 ...
 *  protocol into the system prompt itself (nunmai-engine bot_mode_probe).
...
async function fetchRosterSnapshot(activeConnectionId: null | string | undefined): Promise<RosterSnapshot> {
  ...
  // Multi-source desktops (nunmai-engine #86875) also expose the union
...
function mergeMultiSourceRoster(
  ...
  for (const agent of agents) {
    ...
    profiles.push({
      ...
      // them a remote default titled "CoS Bot" only ever tags as @nunmai(-device)
 ...
 *  callable alias is 'nunmai' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'nunmai' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`nunmai profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Nunmai"
 *  can never hijack the primary profile's @nunmai alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'nunmai'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['nunmai-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Nunmai Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)(?:@([a-z0-9][a-z0-9_-]*))?/gi)) {
    ...
    if (token === 'nunmai') {
      token = byForm.has('nunmai') ? 'nunmai' : token
    }
