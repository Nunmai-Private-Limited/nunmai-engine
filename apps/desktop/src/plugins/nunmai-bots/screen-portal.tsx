...
import { Codicon, host, useValue } from '@nunmai/plugin-sdk'
import type { RpcEvent } from '@nunmai/plugin-sdk'
import type { ProfileGroupRoute } from '@nunmai/plugin-sdk'
...
export function useScreenPortalState(bot: RosterRow) {
  ...
  useEffect(() => {
    ...
    void displayRequest<DisplayStatus>(bot, 'display.status')
      ...
      .catch((error: unknown) => {
        // An older Nunmai without display.* is a settled answer (hide the surface);
