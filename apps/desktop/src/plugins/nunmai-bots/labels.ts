...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Nunmai". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `nunmai profile rename
  ...
  // never generic "Nunmai" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Nunmai (the agent it is)
...
/** The profile id the backend accepts (`nunmai_cli.profiles._PROFILE_ID_RE`
