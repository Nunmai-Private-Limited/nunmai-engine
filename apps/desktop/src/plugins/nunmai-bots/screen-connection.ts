...
import { host, resolveSiblingWsUrl } from '@nunmai/plugin-sdk'
import type { DisplayLease, PluginProfileRoute, RpcEvent } from '@nunmai/plugin-sdk'
...
/** JSON-RPC method-not-found: the bot's Nunmai predates the `display.*` surface. */
 ...
 * `~/.nunmai` path, so the profile key alone is ambiguous: the event must also have
