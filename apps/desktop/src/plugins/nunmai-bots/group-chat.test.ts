...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['nunmai-bots-groups']
}
...
describe('speaker labels', () => {
  it('relabels Nunmai control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as Nunmai in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Nunmai: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Nunmai', async () => {
    const { chat, rounds } = await loadRoom()
    const { formatGroupChatLine } = await import('./group-round-prompt')
    const data = await import('./data')

    // A renamed default (core display_name via `nunmai profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "Nunmai is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Nunmai, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Nunmai')
  ...
  it('qualifies twins per room and never renders a raw key when the roster row is missing', async () => {
    ...
    expect(chat.groupSpeakerLabel('local::reviewer')).toBe('reviewer')
    expect(chat.groupSpeakerLabel('spark::default')).toBe('Nunmai')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['nunmai-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['nunmai-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['nunmai-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'nunmai-bots-groups': 1 } } }
      }
  ...
  it('a remembered disband re-tombstones a mirror that still projects the room (#105275)', async () => {
    ...
    room.gateway.uiMeta['nunmai-bots-groups'] = {
      rooms: {
        'id:room-1': {
    ...
    room.gateway.uiMetaRevisions['nunmai-bots-groups'] = 9
  ...
  it('a same-name room recreated after the disband sync window survives a pull that lacks it', async () => {
    ...
    room.gateway.uiMeta['nunmai-bots-groups'] = { deleted: {}, rooms: {}, updatedAt: 5, version: 3 }
    room.gateway.uiMetaRevisions['nunmai-bots-groups'] = 9
