...
import * as sdk from '@nunmai/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['nunmai-bots'].chat) were
 ...
 *  click path's tile-staleness probe (nunmai-engine#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Nunmai Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (nunmai-engine#89617) — clicking Retry
  ...
  // until an app restart (nunmai-engine#93604). A resume is cheap and
...
export function notifyBotOpenFailure(error: unknown, bot: RosterRow, step: BotOpenStep, botName?: string) {
  ...
  if (botModeGatewayNeedsUpdate(error)) {
    const connectionLabel = bot.connectionLabel || bot.connectionId || 'Nunmai'
    host.notify?.({
      kind: 'error',
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Nunmai Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['nunmai-bots']
