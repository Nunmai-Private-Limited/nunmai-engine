...
import { Button, host, Input, useI18n } from '@nunmai/plugin-sdk'
...
// and the row falls back to the "run nunmai mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.nunmaiDesktop && window.nunmaiDesktop.openExternal) {
        window.nunmaiDesktop.openExternal(authUrl)
      } else {
