...
vi.mock('@nunmai/plugin-sdk', async () => {
...
async function contributions({
  ...
  // Bots pane does.
  if (profiles) {
    cache.set(JSON.stringify(['nunmai-bots', 'roster', cacheKeyConnection]), {
      key: ['nunmai-bots', 'roster', cacheKeyConnection],
      value: { profiles }
    })
  }
...
describe('@-mention completions', () => {
  ...
  it('surfaces default as @nunmai and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@nunmai', '@writer-homelab']))
   ...
   *  picker would surface as a retargeted local @nunmai. */
  const REMOTE_DEFAULTS: Array<Record<string, unknown>> = [
    {
      ...
      remoteSource: true,
      sourceScoped: true,
      ui_meta: { 'nunmai-bots': { title: 'CoS Bot' } }
    },
    {
      connectionId: 'wsl',
      connectionLabel: 'WSL',
  ...
  it.each([
    ...
    'lists titled remote defaults under their slug, qualified on collision, and keeps @nunmai local ($label)',
    async ({ profiles }) => {
      ...
      // the picker pins each to its connection; the local default stays @nunmai.
      expect(inserts).toEqual(expect.arrayContaining(['@nunmai', '@cos-bot@vps', '@cos-bot@wsl']))
      expect(inserts).not.toContain('@cos-bot')
      expect(inserts.filter(insert => insert === '@nunmai')).toHaveLength(1)
      ...
      // A bare @nunmai is this machine's default in either roster order.
      const local = await handler({ text: '@nunmai summarize' })
      expect(local.text).toMatch(/@nunmai = agent profile "default"/)
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/nunmai -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver only knows bare profile names / 'nunmai'.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/message_agent target: "nunmai"/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // the invariant that matters is that no nunmai command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`nunmai/)
  })
    ...
    hostMock.agents.mockResolvedValueOnce({
      agents: [
        ...
        },
        {
          ...
          handle: 'default-vps',
          profile: 'default',
          profileMetadata: { ui_meta: { 'nunmai-bots': { title: 'CoS Bot' } } }
        }
      ],
      sources: [
