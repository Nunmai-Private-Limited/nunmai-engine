...
// https://nunmai.in/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'nunmai-skill-pick', ...} to us (nunmai-engine#86243). We validate
...
export function HubSkillsSection({ bot, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'nunmai-skill-pick' || !data.name) {
        return
      }
