...
Bundled Nunmai Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@nunmai/plugin-sdk`, `react`, and
...
`$NUNMAI_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Nunmai theme
...
Stream availability, regional restrictions, and metadata vary. Nunmai is not
