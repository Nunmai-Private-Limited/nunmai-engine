import type { ConnectionRequestPayload, ToolLabel } from '@nunmai/shared'
...
export interface OAuthProviderStatus {
  /** Nunmai only: the tier name the token resolves to, when the backend knows
   *  one. Null for a free-tier identity and for older backends. */
  account_tier?: null | string
  error?: string
  expires_at?: null | string
  /** Nunmai only: true when the stored token belongs to a free-tier identity
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nunmai doesn't know how to remove it. */
...
export interface FreeTierStatus {
  ...
  enabled: boolean
  has_guest: boolean
  /** Display name for the route, e.g. "Nunmai · free tier". */
  label: string
  model: string
   ...
   *  (`nunmai_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in nunmai_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `nunmai model` uses — instead of
 ...
 * choices as `nunmai model`'s custom-provider setup (#93622). */
...
// The Nunmai pairing service mints a bot on the user's behalf: the desktop shows
...
export type NunmaiConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['nunmai-bots'].title) — the name the
...
// non-desktop) Nunmai simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (nunmai.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors nunmai_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Nunmai, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Nunmai but install on demand. */
 ...
 *  not Nunmai); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** nunmai-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** nunmai-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/nunmai/update/check` — the backend's own update state.
