...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Nunmai Engine Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Nunmai docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Nunmai Engine Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Nunmai docs', url: 'https://example.com/docs' }
    ])
  })
})
...
describe('buildToolView browser_navigate title', () => {
  it('shows failed title when navigate returns success=false', () => {
    ...
    expect(view.title).toBe('Failed to open nunmai-engine.nousresearch.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://nunmai.in/docs' },
        result: { success: true, url: 'https://nunmai.in/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened nunmai-engine.nousresearch.com/docs')
  })
})
