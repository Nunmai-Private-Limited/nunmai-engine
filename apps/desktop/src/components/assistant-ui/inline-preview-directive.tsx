 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.nunmai.send(prompt)` (or declarative
 * `data-nunmai-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'nunmai-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'nunmai-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `nunmai.send(prompt)`.
 ...
 *  wires `data-nunmai-send` so declarative HTML works with zero script:
 *  `<button data-nunmai-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.nunmai={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-nunmai-send]"):null;' +
    'if(el)send(el.getAttribute("data-nunmai-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `nunmai serve` — renders live.)
