...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's NUNMAI_HOME/logs. On a
  ...
  // NUNMAI_HOME/logs, and its parent is the Nunmai data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.nunmaiDesktop?.logsRoot?.()
      ...
      const result = await window.nunmaiDesktop?.openDir?.(resolve(root))
  ...
  const openNunmaiFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenNunmaiFolderFailed),
    [copy.errorOpenNunmaiFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.nunmaiDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openNunmaiFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openNunmaiFolder()} type="button">
          {copy.errorOpenNunmaiFolder}
        </button>
      )}
      {plan.retry && (
