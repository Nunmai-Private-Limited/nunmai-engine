...
const DELIVERY = 'Message from 🤖 Nunmai (@nunmai): please check the build'
...
describe('inter-agent collapse gate', () => {
  it('keeps the report after a relayed answer to this bot\u2019s own dispatch expanded', async () => {
    // Bot Mode round trip seen from the DISPATCHING bot: it messaged @nunmai,
    // the answer came back as an inbound "Message from" row, and the next
    // assistant message is its report to the human — not a reply to nunmai.
    render(
      <Harness
        messages={[
          user('u1', 'ask nunmai for the list'),
          dispatch('a0', '@Nunmai'),
          user('u2', DELIVERY),
          assistant('a1', 'here is the list nunmai sent', false)
        ]}
      />
    )

    await screen.findByText('here is the list nunmai sent')
    ...
    render(
      <Harness
        messages={[
          ...
          user('u4', 'Message from 🤖 Nunmai (@nunmai): unsolicited: build broke'),
