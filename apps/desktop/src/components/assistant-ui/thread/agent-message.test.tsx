...
describe('agent message detection', () => {
  it('matches the Bot Mode delivery prefix with sender and body', () => {
    const m = AGENT_MESSAGE_RE.exec('Message from 🤖 Nunmai: hello there')

    expect(m?.[1]?.trim()).toBe('Nunmai')
  ...
  it('recognizes a relayed sender re-stamped @handle@connection, keeping the bare handle for the avatar', () => {
    // The receiving gateway re-stamps a relayed "@nunmai" as "@nunmai@<connection>"
    // (#103731) — still an agent notice, and the avatar resolves by profile handle.
    const m = AGENT_MESSAGE_RE.exec('Message from 🤖 nunmai (@nunmai@Cloud-1): status?')

    expect(m?.[1]?.trim()).toBe('nunmai')
    expect(m?.[2]).toBe('nunmai')
  ...
  it('does not match prose that merely contains the phrase', () => {
    expect(AGENT_MESSAGE_RE.test('I got a Message from 🤖 Nunmai: earlier')).toBe(false)
