...
describe('frameSizeFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'nunmai-inline-preview-size',
    token: 'tok',
    height: 500,
    width: 300,
    ...over
...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'nunmai-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'nunmai-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects nunmai.send and the data-nunmai-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.nunmai={send:send}')
    expect(framed).toContain('data-nunmai-send')
    expect(framed).toContain('nunmai-inline-preview-intent')
  })
})
