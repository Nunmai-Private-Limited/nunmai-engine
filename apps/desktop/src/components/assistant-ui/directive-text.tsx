...
const NUNMAI_REF_TYPES = WIRE_REFERENCE_KINDS
type NunmaiRefType = (typeof NUNMAI_REF_TYPES)[number]
...
const NUNMAI_DIRECTIVE_RE = referenceRe()
...
export const nunmaiDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NUNMAI_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...nunmaiDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Nunmai directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.nunmaiDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.nunmaiDesktop?.readFileDataUrl(id)
