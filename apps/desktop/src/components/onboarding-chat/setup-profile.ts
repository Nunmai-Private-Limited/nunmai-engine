 ...
 *
 * It is not an anonymous session: it belongs to a persistent `nunmai-setup`
 ...
 * them the voice is just Nunmai, and the chat is titled `Welcome to Nunmai`.
 *
...
export const SETUP_PROFILE = 'nunmai-setup'
...
export const SETUP_CHAT_TITLE = 'Welcome to Nunmai'
...
export function buildFirstTaskRunbook(
  ...
  return [
    `You are Nunmai. The user's welcome chat just opened this session so one task can have room to run: ${task.trim()}.`,
...
const pluginRunbook = (root: string) => [
  'THIS IS A PLUGIN JOB: the thing you are building is a piece of the Nunmai app itself, and it will appear in the window the user is looking at right now. That is the whole point — do not let it become a script in a folder.',
  `A plugin is ONE file: \`${root}/<name>/plugin.js\`. Plain ESM, no build step, no package.json, no install. It imports from \`@nunmai/plugin-sdk\` and calls \`jsx()\` from \`react/jsx-runtime\` directly (there is no JSX compiler in this path — writing \`<div>\` will not work). It default-exports \`{ id, name, register(ctx) }\` and \`register\` calls \`ctx.register({ id, area, order, render })\`. The runtime loads it the moment you save, and reloads it on every later save, so there is no restart to ask them for.`,
  'LOOK BEFORE YOU WRITE. Read the `building-nunmai-desktop-plugins` skill first — it has the SDK surface, the areas you can render into, and the traps. If the machine has a checkout of NousResearch/plugins, read a plugin close to what you are making; those thirteen are reviewed and show the real shapes (a statusbar chip, a composer action, a full pane).',
  ...
  'Never ask them to restart the app, never edit anything outside their plugin folder, and never touch the Nunmai install itself. If the plugin errors on load, the app toasts it and keeps running — read the error, fix the file, save again.'
]
...
export async function buildFirstTaskSeedMessages(
  ...
  const root = plan === 'plugin' ? await window.nunmaiDesktop?.desktopPluginsRoot?.() : undefined
...
export async function ensureSetupProfile(request: GatewayRequest): Promise<void> {
  try {
    await request('profiles.create', {
      description: 'Where Nunmai met you — walks your first run, then checks in as you find your feet.',
      name: SETUP_PROFILE,
      clone_from: 'default',
      share_auth: true,
      no_alias: true,
