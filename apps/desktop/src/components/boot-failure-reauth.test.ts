...
describe('isRemoteReauthError', () => {
  ...
  it('ignores non-auth boot errors and nullish', () => {
    expect(isRemoteReauthError('Nunmai background process exited during startup.')).toBe(false)
...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'nous', displayName: 'Nunmai Research', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'Nunmai Research'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'nous', displayName: 'Nunmai Research', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / Nunmai Research')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Nunmai Research' })).toBe(
      'Sign in with Nunmai Research'
    )
  })
