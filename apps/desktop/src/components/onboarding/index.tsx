...
import { getGlobalModelOptions } from '@/nunmai'
...
// Nunmai Portal OAuth), ahead of OpenRouter and the rest of the key catalog.
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/Nunmai-Private-Limited/nunmai-engine#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `nunmai model`
...
const SHOW_ALL_KEY = 'nunmai-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure whenever Nunmai Portal
        ...
        {showRest ? (
          <>
            {/* Fireworks leads the expanded list, matching CANONICAL_PROVIDERS
                (Nunmai → Fireworks), but stays hidden until the user opens it. */}
