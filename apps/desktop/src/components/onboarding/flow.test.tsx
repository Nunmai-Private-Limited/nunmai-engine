...
// down to window.nunmaiDesktop.api so the test observes the wire body.
vi.mock('@/nunmai', async importOriginal => ({
  ...
  getGlobalModelOptions: async () => ({
    providers: [
      ...
      { models: ['deepseek/deepseek-v4-flash-0731'], name: 'Nunmai Portal', slug: 'nous' }
    ]
  })
}))
...
describe('ConfirmingModelPanel model pick', () => {
  it('persists a cross-provider pick against the picked model provider, not the sign-in provider', async () => {
    ...
    // model that only Nunmai Portal serves.
    ...
    if (flow.status === 'confirming_model') {
      ...
      expect(flow.label).toBe('Nunmai Portal')
    }
  })
})
