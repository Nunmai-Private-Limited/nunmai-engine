...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Nunmai gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.nunmaiDesktop
  Object.defineProperty(window, 'nunmaiDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'nunmaiDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  it('keeps keyboard focus inside the recovery surface', () => {
    ...
    const recoverySurface = screen.getByRole('dialog', { name: /Nunmai couldn't start/i })
