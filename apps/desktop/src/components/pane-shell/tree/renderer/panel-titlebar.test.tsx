...
beforeEach(() => {
  ...
  $connection.set({ isFullscreen: false, windowButtonPosition: { x: 20, y: 10 } } as unknown as NunmaiConnection)
...
describe('usePanelTitlebar', () => {
  ...
  it('ignores connection updates that leave the chrome where it is', () => {
    ...
    clusterLeft = 14
    act(() => {
      $connection.set({ ...$connection.get()!, connected: true } as NunmaiConnection)
    })
