...
function sessionsBotsTree() {
  ...
  registerPane('nunmai-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'nunmai-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('labels the zone menu rows from the pane string localizer, not the register-time title', () => {
    ...
    expect(hideOnlyZoneTabs('g-side').map(tab => tab.title)).toEqual(['Сеансы', 'nunmai-bots:pane'])
  })

  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    sessionsBotsTree()

    expect(setStripTabHidden('nunmai-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('nunmai-bots:pane')
    expect($hiddenStripTabs.get()).toContain('nunmai-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('nunmai-bots:pane')

    expect(setStripTabHidden('nunmai-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('nunmai-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('nunmai-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('nunmai-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('nunmai.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('nunmai-bots:pane')
    ...
    revealTreePane('nunmai-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('nunmai-bots:pane')
    expect(window.localStorage.getItem('nunmai.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'nunmai-bots:pane', title: 'nunmai-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'nunmai-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
