...
const TREE_KEY = 'nunmai.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'nunmai.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'nunmai.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['nunmai-bots:pane'], active: 'nunmai-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'nunmai-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'nunmai-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","nunmai-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['nunmai-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['nunmai-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'nunmai-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['nunmai-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'nunmai-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'nunmai-bots:pane'],
          active: 'nunmai-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'nunmai-bots:pane', 'nunmai-bots:routines'],
          active: 'nunmai-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'nunmai-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'nunmai-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'nunmai-bots:pane'])
    expect(botsGroup.active).toBe('nunmai-bots:pane')
    expect(routinesGroup.panes).toEqual(['nunmai-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['nunmai-bots:routines'],
          active: 'nunmai-bots:routines'
        }
      ]
    }
