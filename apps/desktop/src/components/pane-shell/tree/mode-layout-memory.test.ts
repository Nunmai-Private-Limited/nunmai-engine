...
it('restores independently customized modes through real pane bindings and reloads', async () => {
  ...
  for (const deck of decks) {
    for (const simpleId of ['sidebar-left', 'sidebar-right']) {
      ...
      const saved = window.localStorage.getItem('nunmai.desktop.layoutTree.v2')
      ...
      expect(window.localStorage.getItem('nunmai.desktop.layoutTree.v2')).toBe(saved)
...
it.each(['advanced', 'simple'] as const)(
  ...
  async initialMode => {
    ...
    window.localStorage.setItem('nunmai.desktop.layoutTree.v2', raw)
    window.localStorage.setItem('nunmai.desktop.paneStates.v1', JSON.stringify(legacyPanes))
    window.localStorage.setItem('nunmai.desktop.layoutPreset.active', 'custom')
    window.localStorage.setItem('nunmai.desktop.hiddenStripTabs.v1', '["bots"]')

    if (initialMode === 'simple') {
      window.localStorage.setItem('nunmai.desktop.interfaceMode.v1', 'simple')
    }
