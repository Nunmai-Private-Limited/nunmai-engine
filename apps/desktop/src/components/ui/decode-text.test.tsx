...
it('resolves once and stops ticking by default', () => {
  const { container } = render(<DecodeText prefix={1} text="NUNMAI" />)

  act(() => vi.advanceTimersByTime(settleTicks('NUNMAI') * TICK_MS))
  expect(container.textContent).toBe('NUNMAI')
...
it('keeps replaying only when the caller asks for loop', () => {
  const { container } = render(<DecodeText loop prefix={1} text="NUNMAI" />)
