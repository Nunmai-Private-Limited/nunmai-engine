 ...
 * `NunmaiPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/nunmai'
...
export type { PluginRestOptions } from '@/nunmai'
 ...
 *  under `nunmai.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.nunmaiDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Nunmai — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `nunmai://index-network/intent/1`),
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `nunmai.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.nunmaiDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.nunmaiDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.nunmaiDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.nunmaiDesktop
