 ...
 *
 *   source (plain ESM js) -> import allowlist (`@nunmai/plugin-sdk` / `react*`
 *   only) -> bare-specifier rewrite to live shim blobs (see sdk/runtime.ts)
 *   -> blob `import()` -> validate default NunmaiPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<nunmai home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<nunmai home>/plugins/<name>/desktop/
 ...
 * an exact pinned SHA + the static lint in nunmai_cli/plugin_validate_desktop.py),
...
import { createPluginContext, type NunmaiPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.nunmai/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@nunmai/plugin-sdk, react*) to their
 ...
 *  `@nunmai/plugin-sdk` / `react*` is refused up-front: a bare package would
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @nunmai/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: NunmaiPlugin }
    let deadline: ReturnType<typeof setTimeout> | undefined

    try {
      mod = await Promise.race([
        import(/* @vite-ignore */ url) as Promise<{ default?: NunmaiPlugin }>,
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default NunmaiPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of nunmai-bots predating its adoption in-tree) must
...
// ---------------------------------------------------------------------------
// The on-disk plugin door — ONE app-level root, `<nunmai home>/desktop-plugins/`:
//  - `<id>/plugin.js` — a standalone desktop plugin (agent- or user-written);
//  - `<package>/plugin.js` + `.nunmai-package.json` — the desktop HALF of a
 ...
 *  backend's nunmai_home — #66899). Resolving it also runs Electron's
 *  reconcile, so unified packages' desktop halves are current before we scan. */
async function diskRoots(): Promise<DiskRoot[]> {
  const root = await window.nunmaiDesktop?.desktopPluginsRoot?.()
 ...
 *  agent row. */
const PACKAGE_MARKER = '.nunmai-package.json'
...
async function readPackageMarker(desktop: Window['nunmaiDesktop'], folder: string): Promise<null | PackageMarker> {
  try {
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Nunmai Desktop to load larger plugins"
    )
  }
...
async function watchDiskPluginFile(desktop: NonNullable<Window['nunmaiDesktop']>, record: DiskPlugin): Promise<void> {
  if (record.watchId) {
...
function retireDiskPlugin(file: string, record: DiskPlugin): void {
  ...
  if (record.watchId) {
    void window.nunmaiDesktop?.stopPreviewFileWatch(record.watchId)
  }
...
export async function uninstallDiskPlugin(pluginId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  const remove = window.nunmaiDesktop?.removeDesktopPlugin

  if (!remove) {
    return { ok: false, error: 'this Nunmai Desktop build cannot remove desktop plugins — delete the folder by hand' }
  }
