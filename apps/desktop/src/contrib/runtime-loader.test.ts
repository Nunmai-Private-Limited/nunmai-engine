...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend nunmai_home', async () => {
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.nunmai/plugins')
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.nunmai/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.nunmai/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.nunmai/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.nunmai/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.nunmai/desktop-plugins/my-feature')
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.nunmai/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.nunmai-package.json', path: `${root}/uni/.nunmai-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.nunmai-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend nunmai_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.nunmai/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
...
describe('uninstallDiskPlugin (Plugins hub trash button)', () => {
  ...
  const seedStandalone = async () => {
    ...
    ;(window.nunmaiDesktop as unknown as { removeDesktopPlugin: unknown }).removeDesktopPlugin = removeDesktopPlugin
...
describe('specifier scanning is limited to code (strings/comments never load-block)', () => {
  ...
  it('never rewrites a mapped specifier quoted inside a string', async () => {
    ...
    try {
      ...
      const id = await loadRuntimePlugin(
        `const doc = "from '@nunmai/plugin-sdk'"
export default { id: 'quoted-spec', register: () => { globalThis.__captured = doc } }`,
        'quoted-spec'
      )

      expect(id).toBe('quoted-spec')
      expect((globalThis as unknown as { __captured?: string }).__captured).toBe("from '@nunmai/plugin-sdk'")
    } finally {
...
describe('loader hardening: hangs, leaks, duplicate ids, stale incarnations', () => {
  ...
  it('ctx.setInterval / ctx.addEventListener registrations die with the plugin on unload', async () => {
    ...
    try {
      await loadRuntimePlugin(
        `export default {
          ...
          register(ctx) {
            ...
            ctx.addEventListener(window, 'nunmai-probe', () => { globalThis.__scopedEvents++ })
          }
        }`,
        'scoped-lifetime'
      )
      ...
      window.dispatchEvent(new Event('nunmai-probe'))
