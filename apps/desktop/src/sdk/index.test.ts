...
describe('host.connections', () => {
  ...
  const originalDesktop = desktopWindow.nunmaiDesktop
  ...
  afterEach(() => {
    desktopWindow.nunmaiDesktop = originalDesktop
  })
  ...
  it('still rejects on a Desktop build without the connection registry', async () => {
    desktopWindow.nunmaiDesktop = undefined
...
describe('host.composer draft facade', () => {
  ...
  it('routes insertText and focus by address: tile for a session, resolved-active for null', async () => {
    ...
    window.addEventListener('nunmai:composer-insert', onInsert)
    window.addEventListener('nunmai:composer-focus', onFocus)
    ...
    window.removeEventListener('nunmai:composer-insert', onInsert)
    window.removeEventListener('nunmai:composer-focus', onFocus)
