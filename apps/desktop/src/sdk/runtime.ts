 ...
 * plugins resolve `@nunmai/plugin-sdk` through the vite alias; RUNTIME-loaded
...
// `sdk/runtime` — so a module-scope `{ __NUNMAI_PLUGIN_SDK__: sdk, … }` is
...
// `Object.keys(GLOBALS.__NUNMAI_PLUGIN_SDK__)` threw
...
function pluginNamespaces() {
  return {
    __NUNMAI_PLUGIN_SDK__: sdk,
    __NUNMAI_REACT__: React,
    __NUNMAI_REACT_JSX__: jsxRuntime,
    __NUNMAI_REACT_JSX_DEV__: jsxDevRuntime
  }
}
...
export function sdkImportMap(): Record<string, string> {
  cached ??= {
    '@nunmai/plugin-sdk': shimUrl('__NUNMAI_PLUGIN_SDK__'),
    'react/jsx-dev-runtime': shimUrl('__NUNMAI_REACT_JSX_DEV__'),
    'react/jsx-runtime': shimUrl('__NUNMAI_REACT_JSX__'),
    react: shimUrl('__NUNMAI_REACT__')
  }
