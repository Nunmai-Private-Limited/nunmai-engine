...
vi.mock('@/nunmai', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), nunmaiApi: vi.fn() }))
...
const { deleteProfile, nunmaiApi } = await import('@/nunmai')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its NUNMAI_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { nunmaiDesktop: unknown }).nunmaiDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(nunmaiApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(nunmaiApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(nunmaiApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
