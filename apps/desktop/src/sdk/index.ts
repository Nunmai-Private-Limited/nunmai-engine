/**
 * @nunmai/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__NUNMAI_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, nunmaiApi, type NunmaiGateway } from '@/nunmai'
...
import type { PaginatedSessions, UsageStats } from '@/types/nunmai'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Nunmai profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.nunmaiDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.nunmaiDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and NunmaiGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (nunmai-engine#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.nunmaiDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Nunmai Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.nunmaiDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Nunmai Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Nunmai Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (nunmai-engine#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.nunmaiDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Nunmai Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return nunmaiApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return nunmaiApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Nunmai gateway unavailable')
    }
   ...
   *  components that take a `NunmaiGateway` prop directly (e.g. `ConnectorsTab`),
   *  which need the instance, not just a JSON-RPC door. Re-read per use — the
   *  active instance changes on a profile swap. */
  getGateway: (): NunmaiGateway | null => $gateway.get(),
 ...
 *  "NUNMAI ENGINE" with it, a `chat.empty` contribution writes its own name. */
...
export type { NunmaiGateway } from '@/nunmai'
...
export type { NunmaiOpenTarget } from '@/lib/nunmai-open-target'
...
/** Compact labels for the reasoning levels exported from @nunmai/shared, so a
...
export type { StatusResponse } from '@/types/nunmai'
/** Public SDK name for the shared gateway wire event; kept stable for plugins. */
export type { GatewayEvent as RpcEvent } from '@nunmai/shared'
/** Bot Screen wire shapes, generated from `tui_gateway/contracts/display.py`. */
export type { DisplayLease, DisplayObserveResult, DisplayStatus, DisplayThumbnailResult } from '@nunmai/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@nunmai/shared'
/** Nunmai' reasoning levels, so a plugin surfacing a thinking depth uses the
...
export { contrastRatio } from '@nunmai/shared/color'
