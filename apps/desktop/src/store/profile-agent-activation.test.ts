...
const agentConn = (over: Partial<NunmaiConnection> = {}): NunmaiConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as NunmaiConnection
...
const getConnection = vi.fn<(profile?: string | null) => Promise<NunmaiConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<NunmaiConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { nunmaiDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<NunmaiConnection>(() => undefined))
