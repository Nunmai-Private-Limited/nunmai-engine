 ...
 * composer with the reply scrolling above it, so Nunmai can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.nunmaiDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.nunmaiDesktop?.hud
...
export function resetHudLayout(): void {
  void window.nunmaiDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.nunmaiDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.nunmaiDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
