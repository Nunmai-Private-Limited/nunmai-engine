import type { ConnectionState } from '@nunmai/shared'
...
import type { SessionInfo, UsageStats } from '@/types/nunmai'
...
const COMPOSER_MODEL_KEY = 'nunmai.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'nunmai.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'nunmai.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'nunmai.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'nunmai.desktop.composer.fast'
...
function composerScopeForConnection(connection: NunmaiConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'nunmai.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'nunmai.desktop.lastRoute'
...
function workspaceCwdKey(connection: NunmaiConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.nunmaiDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.nunmaiDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<NunmaiConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'nunmai.desktop.sessionOwnerHints.v1'
 ...
 * NunmaiConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<NunmaiConnection | null>) => {
 ...
 *  weaker when the route clamps a Nunmai-internal step such as `ultra`. Never
...
// until config loads, and re-seeded on every profile switch by useNunmaiConfig.
