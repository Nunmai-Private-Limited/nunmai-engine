...
import type { ProjectInfo } from '@/types/nunmai'
...
const nunmai = await import('@/nunmai')
const getNunmaiConfig = vi.mocked(nunmai.getNunmaiConfig)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('nunmai.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the Nunmai backend.'
        )
      })
    } as never)
...
describe('createProject', () => {
  ...
  it.each(['default', 'coder'])('creates in the active %s profile without leaving All profiles', async profile => {
    const created = { folders: [], id: 'p_new', name: 'Nunmai Engine', primary_path: '/srv/nunmai' }
    ...
    vi.mocked(nunmai.nunmaiApi).mockResolvedValue({ projects: [tree], active_id: created.id })
    $activeGatewayProfile.set(profile)
    setShowAllProfiles(true)

    await expect(createProject({ folders: ['/srv/nunmai'], name: created.name, use: true })).resolves.toEqual(created)
    ...
    expect(nunmai.nunmaiApi).toHaveBeenCalledWith(
  ...
  it('does not retarget a project create when the profile changes during reconnect', async () => {
    ...
    const pending = createProject({ folders: ['/srv/nunmai'], name: 'Nunmai Engine' })
    const rejection = expect(pending).rejects.toThrow('Active Nunmai profile changed while connecting')
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getNunmaiConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getNunmaiConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getNunmaiConfig).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero Nunmai sessions must still surface —
