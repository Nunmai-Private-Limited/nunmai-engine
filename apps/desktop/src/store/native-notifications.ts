...
import { type NunmaiOpenTarget, resolveNunmaiOpenPath } from '@/lib/nunmai-open-target'
...
const STORAGE_KEY = 'nunmai:native-notifications'
...
// "Backgrounded" = the user isn't on Nunmai. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `nunmai://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.nunmaiDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `nunmai://index-network/intent/1`). */
  activate?: NunmaiOpenTarget
 ...
 *  user is away from Nunmai — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveNunmaiOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveNunmaiOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.nunmaiDesktop
