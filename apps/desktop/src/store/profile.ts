import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@nunmai/shared'
import { atom, batch, computed } from 'nanostores'

import type { NunmaiConnection } from '@/global'
import { getProfiles, nunmaiApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/nunmai'
...
import type { ProfileInfo } from '@/types/nunmai'
...
// /api/profiles/active `current`). "default" is the root ~/.nunmai. This is the
...
function profileListSource(connection: NunmaiConnection | null): null | string {
  if (!connection) {
    return null
  }
...
const PROFILE_ORDER_STORAGE_KEY = 'nunmai.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'nunmai.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await nunmaiApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new NUNMAI_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.nunmaiDesktop.profile.set(name)
}
...
export function prewarmProfileBackend(name: string, connectionId: null | string = null): void {
  ...
  // tunnel and spawns `nunmai -p <profile> serve --isolated` on the remote
...
async function resolveConnectionForProfile(profile: string): Promise<NunmaiConnection | null> {
  const getConnection = window.nunmaiDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<NunmaiConnection | null> {
  const getConnectionFor = window.nunmaiDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'nunmai.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.nunmaiDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.nunmaiDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.nunmai) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.nunmaiDesktop?.touchBackend?.(target).catch(() => undefined)
}
