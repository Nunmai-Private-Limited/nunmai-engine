 ...
 * app's bounds and stays visible while Nunmai is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'nunmai.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'nunmai.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.nunmaiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.nunmaiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.nunmaiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.nunmaiDesktop?.petOverlay?.close()
}
