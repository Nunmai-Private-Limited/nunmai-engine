...
import type { NunmaiGitWorktree, NunmaiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: NunmaiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, NunmaiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, NunmaiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<NunmaiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<NunmaiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<NunmaiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<NunmaiGitWorktree[]>>()
const $noRepoStatus = atom<NunmaiRepoStatus | null>(null)
const $noWorktrees = atom<NunmaiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<NunmaiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<NunmaiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: NunmaiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: NunmaiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<NunmaiRepoStatus | null>
  seq: number
  target: string
}
