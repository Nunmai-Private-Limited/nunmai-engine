...
import type { FreeTierStatus } from '@/types/nunmai'
...
/** The provider slug the free-tier route and a signed-in Nunmai account share. */
...
export interface FreeTierSetupFailure {
  /** One of the backend's `anon_*` codes (`nunmai_cli/anon_auth.py`), or a newer one this build does not know. */
  code: string
  door: 'retry' | 'sign_in'
  message: string
  retryAfter: number
