...
beforeEach(() => {
  ...
  ensureGatewayAgent.mockImplementation(async (connectionId, profile, options) => {
    ...
    $connection.set({
      ...
      // The primary local descriptor from startNunmai() historically carried
  ...
  vi.stubGlobal('window', {
    nunmaiDesktop: { api, connections: { list, setLastUsed } },
    localStorage,
...
describe('selectConnection', () => {
  ...
  it('restores the last-used profile on switch-back even when the primary local descriptor is profile-less', async () => {
    ...
    expect(JSON.parse(localStorage.getItem('nunmai.desktop.lastProfileByConnection') || '{}')).toEqual({
      local: 'mac'
    })

    // A later resync republishes a profile-less primary descriptor (the
    // startNunmai shape). The remembered pair is the authority for "what was
