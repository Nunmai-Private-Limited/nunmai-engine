...
import type { NunmaiReviewFile, NunmaiReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Nunmai' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'nunmai.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'nunmai.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'nunmai.desktop.reviewTreeMode'
const SELECTED_KEY = 'nunmai.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<NunmaiReviewFile[]>([])
...
export const $reviewShipInfo = atom<NunmaiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['nunmaiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: NunmaiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly NunmaiReviewFile[], path: string): NunmaiReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.nunmaiDesktop?.openExternal?.(existing.url)
