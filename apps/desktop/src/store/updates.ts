...
import { checkNunmaiUpdate, getActionStatus, updateNunmai } from '@/nunmai'
...
import type { BackendUpdateCheckResponse } from '@/types/nunmai'
...
const UPDATE_TOAST_SNOOZE_KEY = 'nunmai:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'nunmai:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'nunmai:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateNunmai'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.nunmaiDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkNunmaiUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.nunmaiDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `nunmai update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'nunmai update',
        command: result.command ?? 'nunmai update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== nunmai-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateNunmai()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'nunmai update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkNunmaiUpdate(true)
...
const FOCUS_RECHECK_KEY = 'nunmai.updates.last-passive-check'
