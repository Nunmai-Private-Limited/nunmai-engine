...
export function setZoomPercent(percent: number): void {
  window.nunmaiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.nunmaiDesktop?.zoom) {
  void window.nunmaiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.nunmaiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
