 ...
 * `@nunmai/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-nunmai-glass]`
 * block in styles.css).
 */
...
} from '@nunmai/shared/translucency'
 ...
 * sets `nunmaiDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.nunmaiDesktop?.glassSupported === 'boolean'
    ? window.nunmaiDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.nunmaiDesktop?.translucencySupported === 'boolean'
    ? window.nunmaiDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-nunmai-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-nunmai-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-nunmai-glass-on', glassLive)
  root.toggleAttribute('data-nunmai-glass', glassOn)
  root.toggleAttribute('data-nunmai-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-nunmai-glass-scope', scope)
  } else {
    root.removeAttribute('data-nunmai-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.nunmaiDesktop?.setTranslucency?.(state)
  })
