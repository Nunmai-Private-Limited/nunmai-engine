...
import { getNunmaiConfigRecord, saveNunmaiConfig } from '@/nunmai'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getNunmaiConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveNunmaiConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
