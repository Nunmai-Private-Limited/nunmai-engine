 ...
 *
 * This store is the only writer of nunmai-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'nunmai-intro-reveal-seen-v1'
...
/** The film will never play for this identity (NUNMAI_SKIP_INTRO). Record it
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.nunmaiDesktop?.skipIntro !== true
}

/** The guided flow is on but its intro film was turned off at launch
 * (NUNMAI_SKIP_INTRO). Distinct from isIntroRevealEnabled(): false there also
...
export function startIntroReveal(): void {
  ...
  void window.nunmaiDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.nunmaiDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.nunmaiDesktop?.introReveal
