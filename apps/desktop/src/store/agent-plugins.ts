...
/**
 * Feature store for backend (agent) plugins — the native Nunmai plugins plus
 ...
 * primitives `nunmai plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Nunmai. */
  portable?: boolean
 ...
 *  NUNMAI_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's NUNMAI_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
 ...
 *  deletes `<NUNMAI_HOME>/plugins/<name>` and its install metadata). Drops the
