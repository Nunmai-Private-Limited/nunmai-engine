 ...
 *
 * The archive is the CLI's own `nunmai profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/nunmai'
...
import type { ProfileDesktopOverlay } from '@/types/nunmai'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Nunmai profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.nunmaiDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.nunmaiDesktop?.selectPaths?.({
