 ...
 *
 * Local models ship on main behind `--local` (either `nunmai desktop --local`
 * or the flag on Nunmai.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.nunmaiDesktop?.localModelsEnabled === true
)
