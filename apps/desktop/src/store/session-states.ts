...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@nunmai/shared'
...
const TILES_KEY = 'nunmai.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'nunmai.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'nunmai.desktop.botChatSessions.v1'
 ...
 *  (nunmai-engine#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_nunmai_home() re-creates
 * the profile directory the delete just removed (nunmai-engine#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (nunmai-engine#94235). Tiles persisted
...
// Dev hook for automation (mirrors __NUNMAI_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__NUNMAI_SESSION_TILES__ = {
