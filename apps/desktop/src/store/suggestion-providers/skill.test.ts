...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('nunmai-engine', '/Users/b/www/nunmai-engine')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('nunmai-engine', '/Users/b/www/nunmai-engine-suggest')).toBe(true)
  })
  ...
  it('does not suppress on substring-only overlap', () => {
    ...
    expect(collidesWithWorkspace('clean', '/Users/b/www/nunmai-engine')).toBe(false)
  })

  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('nunmai-engine', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('nunmai-engine-dev', [toolCall('skill_view', { name: 'github/nunmai-engine-dev' })])
    ).toBe(true)
    expect(
