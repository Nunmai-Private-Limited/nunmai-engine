 ...
 *  Working in `~/www/nunmai-engine`, the draft says "nunmai-engine" constantly
 *  — a "Use skill: nunmai-engine" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`nunmai-engine-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/nunmai-engine-dev`, `plugin:skill`). */
