...
describe('requestPluginCatalogInstallFromDeepLink', () => {
  ...
  it.each([
    ['unknown', 'not in the Nunmai plugin catalog'],
    ['unavailable', 'Could not load the Nunmai plugin catalog'],
  ...
  it('resolves against the live catalog feed by default and rejects unknown names', async () => {
    ...
    expect($pluginInstallRequest.get()).toBeNull()
    expect($notifications.get()[0]?.message).toContain('\u201Cweather-evil\u201D is not in the Nunmai plugin catalog')
