...
import type { NunmaiGitBaseBranch, NunmaiGitBranch } from '@/global'
import { getNunmaiConfig, nunmaiApi, type NunmaiGateway } from '@/nunmai'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/nunmai'
...
const PROJECT_SCOPE_KEY = 'nunmai.desktop.projectScope'
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Nunmai profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await nunmaiApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<NunmaiGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<NunmaiGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Nunmai
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getNunmaiConfig(context.profile))
...
// repo, returning where Nunmai should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<NunmaiGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<NunmaiGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.nunmaiDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.nunmaiDesktop?.writeClipboard?.(path)
  }
}
