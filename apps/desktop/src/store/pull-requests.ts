...
import type { NunmaiBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/nunmai'
...
export const $pullRequestsByBranch = atom<Record<string, NunmaiBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'nunmai.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('nunmai.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: NunmaiBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
