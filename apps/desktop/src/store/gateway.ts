...
import { NunmaiGateway, setApiRequestConnection } from '@/nunmai'
...
function dialProfile(
  desktop: NonNullable<typeof window.nunmaiDesktop>,
  profile: string,
...
const isOpen = (gateway: NunmaiGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: NunmaiConnection) => void
  /**
...
interface Secondary {
  ...
  connection: NunmaiConnection | null
...
interface GatewayRegistryState {
  ...
  primaryGateway: NunmaiGateway | null
   ...
   *  `nunmai serve --profile <primary>` child and can never stand in for a
  ...
  $gateway: ReturnType<typeof atom<NunmaiGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('nunmai.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<NunmaiGateway | null>(null),
...
export function setPrimaryGateway(gateway: NunmaiGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<NunmaiConnection, 'connectionId' | 'mode'> | null): void {
...
async function ridesPrimaryBackend(
  ...
  // `NUNMAI_DESKTOP_ISOLATED_BACKEND=1`), and for a pooled profile this is the
...
export function activeGateway(): NunmaiGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the nunmai module (null
...
function publishActiveConnection(connection: NunmaiConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new NunmaiGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: NunmaiGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Nunmai gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.nunmaiDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Nunmai Desktop.')
  }
...
function publishTurnLease(scope: string, activeTurn: boolean): void {
  void window.nunmaiDesktop?.touchBackend?.(scope, { activeTurn }).catch(() => undefined)
}
...
export async function ensureActiveGatewayOpen({
  explicit = false
}: { explicit?: boolean } = {}): Promise<NunmaiGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Nunmai gateway is not connected" on the Sessions `+` action (#88880).
