/**
 * The words Nunmai says during the guided first run.
 *
...
/** How Nunmai talks for the whole of the first run — the guided chat and the
 ...
 * anti-slop rules still hold, and a chirpy Nunmai would be worse than a flat
 * one.
 */
const PERSONA = [
...
/** Setting the machine up is always on offer: it is a first task Nunmai can do
...
export function buildChatOnboardingPrompt(suggestedName?: string | null, signedIn = false): string {
  ...
  return [
    ...
    '2. Then the tools they already use, so Nunmai can connect to them later: one short sentence, then ::onboarding{step="connectors"} on a line of its own.',
    ...
    ...(signedIn
      ? []
      : [
          'In that same turn, once, mention in ONE short clause that wiring those up later will want a model provider — a free Nunmai account is there if they want it, free tier, no card, and they can bring their own provider instead — then move straight on. Do not sell it, do not list providers, do not ask them to do it now, and never bring it up again: they will be asked properly at the point it actually matters.'
        ]),
