...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/nunmai'
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Nunmai still cannot reach ${url}.` }
    }
