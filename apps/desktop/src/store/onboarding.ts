import type { ModelOptionProvider } from '@nunmai/shared'
...
import type { OAuthProvider, OAuthStartResponse } from '@/types/nunmai'
...
export interface DesktopOnboardingState {
   ...
   *  overlay show its "Nunmai is ready" screen once even though the app is
...
const CONFIGURED_CACHE_KEY = 'nunmai-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'nunmai-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Nunmai is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nunmai Tool Gateway,
// mirroring nunmai_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Nunmai auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Nunmai subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `nunmai model` does (for Nunmai it honors the user's free/paid tier, so a
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Nunmai could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Nunmai still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Nunmai still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext, stillWanted?: () => boolean) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Nunmai Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.nunmaiDesktop?.openExternal) {
    try {
      await window.nunmaiDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Nunmai still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Nunmai still cannot reach ${resolvedUrl}.` }
    }
