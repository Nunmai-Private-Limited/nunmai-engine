...
<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Nunmai surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Nunmai and hear it back.</td></tr>
...
### Install with Nunmai (recommended)

Already have the Nunmai CLI? Just run:

```bash
nunmai desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Nunmai gateway or install Nunmai locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Nunmai Desktop website.](https://nunmai.in/).
...
left and right Command keys together in any app. Nunmai captures that app's
frontmost window and attaches the image to the last-active Nunmai composer,
...
```bash
# throwaway NUNMAI_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Nunmai Engine runtime into `NUNMAI_HOME`
(`~/.nunmai`, or `%LOCALAPPDATA%\nunmai` on Windows), using the same layout as a
CLI install.
...
- **Nunmai Engine** runs as a headless `nunmai serve` process and exposes the
...
1. `NUNMAI_DESKTOP_NUNMAI_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `NUNMAI_DESKTOP_NUNMAI`, or `nunmai` on `PATH`
5. a system Python that can import the Nunmai runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Nunmai
...
screen offers **Connect to existing Nunmai** before starting the local installer.
...
terminal commands, and file operations run against the remote Nunmai host, not
...
transport- or Nunmai-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Nunmai-Session-Token`.
...
Boot logs land in `NUNMAI_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\nunmai\nunmai-engine\.nunmai-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\nunmai\nunmai-engine\venv"
```

> The default Nunmai home on Windows is `%LOCALAPPDATA%\nunmai`. Set the `NUNMAI_HOME` env var if you've relocated it.
