...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.nunmaiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.nunmaiHome)
  const builder = await RealSessionBuilder.start(sandbox.nunmaiHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.nunmaiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.nunmaiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.nunmaiHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
