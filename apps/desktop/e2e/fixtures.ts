/**
 * Shared E2E fixtures for the Nunmai desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → nunmai serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `nunmai desktop --source`. The
...
// Runtime state of whatever Nunmai launched this run. A spec driven from inside
// an agent's terminal inherits NUNMAI_YOLO_MODE, NUNMAI_INTERACTIVE,
// NUNMAI_SESSION_ID…, and the sandboxed backend then skips approvals or binds
...
// NUNMAI_* the app needs themselves; only the harness's own knobs pass.
function isInheritedNunmaiRuntimeVar(name: string): boolean {
  return name.startsWith('NUNMAI_') && !name.startsWith('NUNMAI_DESKTOP_') && !name.startsWith('NUNMAI_E2E_')
}
...
function stripCredentials(env: Record<string, string | undefined>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(env)) {
    ...
    if (isCredentialEnvVar(key) || isInheritedNunmaiRuntimeVar(key)) {
      continue
    }
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nunmai-e2e-${prefix}-${Math.random()}`))
  const nunmaiHome = path.join(root, 'nunmai-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(nunmaiHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(nunmaiHome, 'config.yaml')
 ...
 */
export function writeEnvFile(nunmaiHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(nunmaiHome, '.env')
 ...
 */
function writeEmptyConfig(nunmaiHome: string): void {
  const configPath = path.join(nunmaiHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - NUNMAI_HOME → sandbox nunmai-home (isolated config/sessions)
 *  - NUNMAI_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - NUNMAI_DESKTOP_IGNORE_EXISTING=1 → don't pick up `nunmai` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - NUNMAI_DESKTOP_NUNMAI_ROOT → REPO_ROOT (dev checkout resolution)
 *  - NUNMAI_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    NUNMAI_HOME: sandbox.nunmaiHome,
    NUNMAI_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    NUNMAI_DESKTOP_IGNORE_EXISTING: '1',
    // One `nunmai serve` per host, and profile roots are HOME-anchored
    // (`~/.nunmai/profiles`, the default profile's own home): without both of
    ...
    NUNMAI_DESKTOP_ISOLATED_BACKEND: '1',
    HOME: sandbox.root,
    NUNMAI_DESKTOP_NUNMAI_ROOT: REPO_ROOT,
    NUNMAI_DESKTOP_APP_NAME: `NunmaiE2E-${Date.now()}`,
    ...
    NUNMAI_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated NUNMAI_HOME + userData
 * @param env      - the process environment (already has NUNMAI_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.nunmaiHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.nunmaiHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via NUNMAI_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`nunmai serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.nunmaiHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { NUNMAI_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Nunmai backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Nunmai.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Nunmai.app', 'Contents', 'MacOS', 'Nunmai')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'nunmai')
}
 ...
 * progress without spawning a real Nunmai backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * NUNMAI_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    NUNMAI_DESKTOP_BOOT_FAKE: '1',
    NUNMAI_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + nunmai-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).NUNMAI_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).NUNMAI_DESKTOP_NUNMAI
  delete (env as Record<string, string | undefined>).NUNMAI_DESKTOP_NUNMAI_ROOT
