 ...
 * packaged binary) with a real `nunmai serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → nunmai serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Nunmai title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Nunmai')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { nunmaiDesktop?: Record<string, unknown> }).nunmaiDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → nunmai serve is spawned → WS connects → config
