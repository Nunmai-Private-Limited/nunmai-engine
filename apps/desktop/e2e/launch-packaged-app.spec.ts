...
/**
 * E2E smoke tests for the packaged Nunmai desktop app.
 *
 ...
 * (credential stripping, isolated NUNMAI_HOME + userData, unique app name).
 *
...
test('window opens with the Nunmai title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      nunmaiDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).nunmaiDesktop?.hud?.open({ sessionId: null })
  )
