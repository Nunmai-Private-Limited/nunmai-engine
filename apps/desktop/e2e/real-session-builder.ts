 ...
 * AIAgent loop, using the E2E mock provider configured in `nunmaiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(nunmaiHome: string) {
  ...
  static async start(nunmaiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(nunmaiHome)
