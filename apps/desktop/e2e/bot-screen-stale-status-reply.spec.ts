...
// not reachable in this single-local-source rig: `window.nunmaiDesktop` is a
...
// `src/plugins/nunmai-bots/screen-connection.test.ts`; the second test here
...
async function seedBot(nunmaiHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(nunmaiHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
const PROFILE_KEY = '/e2e/alpha/.nunmai'
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.nunmaiHome, mock.url, 'alpha')
