 ...
 * second gateway, "Homelab", is a REAL second `nunmai serve` this spec spawns
 * with its own NUNMAI_HOME, profiles and session token, registered in the v2
...
function findNunmaiBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'nunmai')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['nunmai'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('nunmai binary not found: create the repo venv (uv sync) or put nunmai on PATH')
}
...
/**
 * Spawn a second, fully real `nunmai serve` as the remote gateway. Its
 * session token is pinned through NUNMAI_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        NUNMAI_HOME: home,
        NUNMAI_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote nunmai serve exited early (${child.exitCode}):\n${log}`)
    }
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote nunmai serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.nunmaiHome, mock.url)
    ...
    seedProfiles(sandbox.nunmaiHome, ['research'])
