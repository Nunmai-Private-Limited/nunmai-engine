...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Nunmai E2E'], { cwd: repo })
...
function configureRepoCwd(nunmaiHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(nunmaiHome, mockUrl)
  fs.appendFileSync(path.join(nunmaiHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.nunmaiHome, mock.url, repo)
