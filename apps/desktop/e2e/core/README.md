...
- **C2 transcript integrity** — `transcript-integrity.spec.ts`: one real app +
  one real `nunmai serve`, only the LLM faked (`provider.ts`, scripted per turn
...
- **C5 boot / process lifecycle** — `boot-lifecycle.spec.ts`: interactive
  ...
  child → zero sandbox processes (/proc census on the sandbox `NUNMAI_HOME`,
...
sandboxed `HOME`/`NUNMAI_HOME`/user-data per test, all `NUNMAI_*` and
...
`NUNMAI_E2E_CORE_ROOT` picks the sandbox parent dir (default: OS tmpdir);
`NUNMAI_E2E_CORE_KEEP=1` keeps sandboxes for post-mortem.
