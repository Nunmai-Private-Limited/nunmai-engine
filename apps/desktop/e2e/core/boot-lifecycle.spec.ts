 ...
 *
 * Real Electron + real `nunmai serve`; only the LLM is faked. Process facts
 * come from a /proc census of every process carrying this sandbox's
 * NUNMAI_HOME (so an orphan reparented to init is still counted), sampled
 ...
 *  4. relaunch the same NUNMAI_HOME repeatedly: each boot has exactly one
...
/** Every live process whose command line carries `tag` (tool children may scrub NUNMAI_HOME). */
...
test('boot handshake, supervised respawn, and zero orphans on quit', async () => {
  ...
  writeProviderHome(sandbox.nunmaiHome, provider.url)
