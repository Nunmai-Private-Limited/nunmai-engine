...
/**
 * HOME is faked too, not just NUNMAI_HOME: profile roots are anchored to
 * `Path.home()/.nunmai`, so a sandbox NUNMAI_HOME under the real ~/.nunmai
 * would read and write the real install's profiles/.
 */
export function createCoreSandbox(label: string): CoreSandbox {
  const parent = process.env.NUNMAI_E2E_CORE_ROOT || os.tmpdir()
  fs.mkdirSync(parent, { recursive: true })
  const root = fs.mkdtempSync(path.join(parent, `core-${label}-`))
  const home = path.join(root, 'home')
  const nunmaiHome = path.join(home, '.nunmai')
  ...
  // depend on the runner's keyring rather than on Nunmai.
  ...
  return {
    ...
    userDataDir,
    cleanup: () => {
      if (!process.env.NUNMAI_E2E_CORE_KEEP) {
 ...
 * approval prompts under test come from Nunmai's own detector.
 */
...
/**
 * The runner's own env minus credentials and every NUNMAI_* knob: an agent
 * shell exports NUNMAI_YOLO_MODE / _NUNMAI_GATEWAY, which the spawned backend
...
export function coreAppEnv(sandbox: CoreSandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (!value || CREDENTIAL_RE.test(key) || /^_?NUNMAI_/.test(key) || key === 'VIRTUAL_ENV') {
      continue
    }
  ...
  return {
    ...
    NUNMAI_DESKTOP_APP_NAME: `NunmaiCoreE2E-${path.basename(sandbox.root)}`,
    NUNMAI_DESKTOP_SKIP_QUIT_CONFIRM: '1',
    NUNMAI_DESKTOP_CDP_PORT: 'off',
...
/** Every live process whose environment carries this sandbox's NUNMAI_HOME (orphans included). */
export function sandboxProcesses(sandbox: CoreSandbox): ProcInfo[] {
  const needle = `NUNMAI_HOME=${sandbox.nunmaiHome}\0`
  const out: ProcInfo[] = []
...
/**
 * The `nunmai serve` backend(s) spawned for this sandbox.
 *
...
export async function routePrimaryWebSocket(app: ElectronApplication, backendPort: number, proxyPort: number) {
  await app.evaluate(
    ({ ipcMain }, { backendPort, proxyPort }) => {
      ...
      for (const channel of ['nunmai:connection', 'nunmai:gateway:ws-url']) {
...
export async function splitProfileRoute(app: ElectronApplication, profile: string): Promise<void> {
  await app.evaluate(({ ipcMain }, profile) => {
    const handlers = (ipcMain as any)._invokeHandlers as Map<string, (...args: any[]) => Promise<any>>
    const original = handlers.get('nunmai:connection:for')

    if (!original) {
      throw new Error('no ipc handler nunmai:connection:for')
    }

    ipcMain.removeHandler('nunmai:connection:for')
    ipcMain.handle('nunmai:connection:for', async (event: unknown, payload: any) => {
...
export function storedSessionForMarker(sandbox: CoreSandbox, profile: string, marker: string): null | string {
  const dbPath =
    profile === 'default'
      ? path.join(sandbox.nunmaiHome, 'state.db')
      : path.join(sandbox.nunmaiHome, 'profiles', profile, 'state.db')
...
export async function persistedTranscript(
  ...
  const result = await page.evaluate(
    async ({ sessionId, query }) =>
      (window as any).nunmaiDesktop.api({ path: `/api/sessions/${sessionId}/messages?order=oldest&limit=500${query}` }),
    { sessionId, query }
  )
