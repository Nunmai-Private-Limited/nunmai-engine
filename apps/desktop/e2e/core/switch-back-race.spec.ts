 ...
 * read is parked in main (ipcMain 'nunmai:api' wrapper) until the test opens
    ...
    try {
...

      {
        const wrapped = await app.evaluate(({ ipcMain }, b) => {
          ...
          const original = handlers?.get('nunmai:api')

          if (!original) {
            return false
          }

          ipcMain.removeHandler('nunmai:api')
          ipcMain.handle('nunmai:api', async (event: any, request: any) => {
        ...
        expect(wrapped, 'nunmai:api handler wrapped for gating').toBe(true)
      }
