...
// created profile's config.yaml under NUNMAI_HOME), not the toast.
...
test.afterEach(async ({}, info) => {
  ...
  const cfg = join(fixture.sandbox.nunmaiHome, 'profiles', 'fresh-scout', 'config.yaml')
...
test('a fresh (non-clone) bot gets a runnable model block on disk', async () => {
  ...
  const cfgPath = join(fixture!.sandbox.nunmaiHome, 'profiles', 'fresh-scout', 'config.yaml')
