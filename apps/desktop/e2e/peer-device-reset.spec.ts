 ...
 * Requires the built desktop and a repo venv (or NUNMAI_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.NUNMAI_DESKTOP_PYTHON) {
    return process.env.NUNMAI_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set NUNMAI_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['NUNMAI_DESKTOP_USER_DATA_DIR', 'NUNMAI_DESKTOP_IGNORE_EXISTING', 'NUNMAI_DESKTOP_NUNMAI_ROOT', 'NUNMAI_DESKTOP_APP_NAME', 'NUNMAI_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    NUNMAI_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      const remoteProfileHome = path.join(remote.nunmaiHome, 'profiles', REMOTE_PROFILE)
      fs.mkdirSync(remoteProfileHome, { recursive: true })

      for (const nunmaiHome of [local.nunmaiHome, remote.nunmaiHome, remoteProfileHome]) {
        writeMockProviderConfig(nunmaiHome, mock.url)
        writeEnvFile(nunmaiHome)
      }

      const port = await freePort()
      const remoteUrl = `http://127.0.0.1:${port}`
      child = spawn(pythonBinary(), ['-m', 'nunmai_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), NUNMAI_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },
