...
has to be thrown away or summarized. Nunmai has always done this in one batch:
...
Nunmai folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Nunmai's normal session flush is
...
With `micro_compact` unset or `false` Nunmai behaves exactly as it always has:
...
Defaults to `$NUNMAI_HOME/logs/agent.log`. It reports passes, outcome mix, net
