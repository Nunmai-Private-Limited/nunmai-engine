...
a provider run fails, or delivery starts failing. `nunmai cron list` shows some of
...
Add `nunmai cron doctor` as a read-only diagnostic command that summarizes cron
...
- `nunmai cron doctor` returns `0` and prints a healthy message when active jobs
...
- It returns `1` and prints grouped job-level issues when any active job has:
  ...
  - script path missing/outside `NUNMAI_HOME/scripts`, or
