# Nunmai Observer Hooks

Nunmai observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Nunmai' planner, model providers, memory, tool registry,
...
Nunmai also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "nunmai.observer.v1"
```

Hook callbacks are fail-open. Nunmai catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Nunmai only builds
...
The native NeMo Relay SDK integration maps Nunmai session, turn, LLM, and tool
