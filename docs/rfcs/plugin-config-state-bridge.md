...
or change arbitrary Nunmai configuration.
...
small host fact requested by the RFC. Settings writes use Nunmai'
...
`NUNMAI_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<NUNMAI_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Nunmai home on every operation.
...
The implementation is covered with real temporary-Nunmai-home tests for:
