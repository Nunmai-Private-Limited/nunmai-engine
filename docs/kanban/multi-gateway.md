...
Nunmai supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.nunmai/config.yaml`:
...
Or set the env var: `NUNMAI_KANBAN_DISPATCH_IN_GATEWAY=false`
