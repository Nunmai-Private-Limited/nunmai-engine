...
`nunmai config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.nunmai/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `NUNMAI_HOME`, secret scope, and
