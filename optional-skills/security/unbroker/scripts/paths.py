...
All per-subject data lives under PDD_DATA_DIR (default: $NUNMAI_HOME/unbroker),
which is the same trust boundary Nunmai uses for .env and OAuth tokens.
"""
...
def nunmai_home() -> Path:
    return Path(os.environ.get("NUNMAI_HOME") or (Path.home() / ".nunmai"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else nunmai_home() / "unbroker"
