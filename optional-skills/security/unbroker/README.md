...
The design is **Nunmai-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
nunmai skills install official/security/unbroker
```

Then start a new Nunmai session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Nunmai session:
...
- **PII handling.** Dossiers live under the Nunmai home directory (`0600`, optionally
