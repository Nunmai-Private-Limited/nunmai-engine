...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NUNMAI_HOME", os.path.expanduser("~/.nunmai")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NUNMAI_HOME", Path.home() / ".nunmai")) / "skills" / "red-teaming" / "godmode"
...
NUNMAI_HOME = Path(os.getenv("NUNMAI_HOME", Path.home() / ".nunmai"))
CONFIG_PATH = NUNMAI_HOME / "config.yaml"
PREFILL_PATH = NUNMAI_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nunmai" in model_lower or "nous" in model_lower:
        return "nunmai"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nunmai config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nunmai/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nunmai for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nunmai for changes to take effect.")
