#!/usr/bin/env python3
"""OpenClaw -> Nunmai migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Nunmai Engine, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Nunmai.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Nunmai memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Nunmai memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Nunmai-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Nunmai-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Nunmai command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.nunmai/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.nunmai/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Nunmai .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Nunmai .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Nunmai .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Nunmai .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Nunmai .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Nunmai config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Nunmai config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Nunmai.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Nunmai MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Nunmai config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'nunmai cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Nunmai config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Nunmai session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Nunmai custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Nunmai config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Nunmai config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Nunmai config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `nunmai config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`nunmai config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.nunmai/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Nunmai config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Nunmai in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Nunmai`` (prose), but lowercase matches
# like ``openclaw`` → ``nunmai`` (so filesystem paths like ``~/.openclaw``
# become ``~/.nunmai`` — the real Nunmai home — not the broken ``~/.Nunmai``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Nunmai'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Nunmai'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Nunmai'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Nunmai`` but maps ``openclaw`` → ``nunmai`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.nunmai/config.yaml`` (the real Nunmai home) instead of the broken
    ``~/.Nunmai/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Nunmai.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Nunmai memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Nunmai Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-nunmai/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Nunmai config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-nunmai/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Nunmai session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Nunmai config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Nunmai
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Nunmai-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Nunmai-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: nunmai config set",
                        )
                    continue
        ...
        for oc_key, nunmai_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and nunmai_key not in secret_additions:
                secret_additions[nunmai_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, nunmai_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and nunmai_key not in secret_additions:
                        secret_additions[nunmai_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nunmai_config = load_yaml_file(destination)
        current_model = nunmai_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = nunmai_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                nunmai_config["model"] = {"default": model_str}
            dump_yaml_file(destination, nunmai_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Nunmai still uses "edge"
        ...
        nunmai_config = load_yaml_file(destination)
        existing_tts = nunmai_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Nunmai destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Nunmai-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Nunmai-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nunmai_cfg_path = self.target_root / "config.yaml"
        nunmai_cfg = load_yaml_file(nunmai_cfg_path)
        existing_mcp = nunmai_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Nunmai config")
                continue

            nunmai_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                nunmai_srv["command"] = srv["command"]
                if srv.get("args"):
                    nunmai_srv["args"] = srv["args"]
                if srv.get("env"):
                    nunmai_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    nunmai_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                nunmai_srv["url"] = srv["url"]
                if srv.get("headers"):
                    nunmai_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    nunmai_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                nunmai_srv["enabled"] = False
            if srv.get("timeout"):
                nunmai_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                nunmai_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                nunmai_srv["tools"] = {}
                if tools_cfg.get("include"):
                    nunmai_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    nunmai_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(nunmai_cfg_path)
            nunmai_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(nunmai_cfg_path, nunmai_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'nunmai cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = nunmai_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Nunmai reasoning_effort
        ...
        if compaction:
            compression = nunmai_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'nunmai gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("NUNMAI_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nunmai_cfg_path = self.target_root / "config.yaml"
        nunmai_cfg = load_yaml_file(nunmai_cfg_path)
        sr = nunmai_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nunmai_cfg_path = self.target_root / "config.yaml"
        nunmai_cfg = load_yaml_file(nunmai_cfg_path)
        custom_providers = nunmai_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(nunmai_cfg_path)
            nunmai_cfg["custom_providers"] = custom_providers
            dump_yaml_file(nunmai_cfg_path, nunmai_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Nunmai config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            nunmai_cfg_path = self.target_root / "config.yaml"
            nunmai_cfg = load_yaml_file(nunmai_cfg_path)
            discord_nunmai = nunmai_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_nunmai["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_nunmai["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                nunmai_cfg["discord"] = discord_nunmai
                dump_yaml_file(nunmai_cfg_path, nunmai_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nunmai_cfg_path = self.target_root / "config.yaml"
        nunmai_cfg = load_yaml_file(nunmai_cfg_path)
        browser_nunmai = nunmai_cfg.get("browser") or {}
        changed = False

        # Map fields that have Nunmai equivalents
        if browser.get("cdpUrl"):
            browser_nunmai["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_nunmai["headless"] = browser["headless"]
            changed = True

        if changed:
            nunmai_cfg["browser"] = browser_nunmai
            if self.execute:
                self.maybe_backup(nunmai_cfg_path)
                dump_yaml_file(nunmai_cfg_path, nunmai_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            nunmai_mode = mode_map.get(mode, "manual")
            nunmai_cfg.setdefault("approvals", {})["mode"] = nunmai_mode
            if self.execute:
                self.maybe_backup(nunmai_cfg_path)
                dump_yaml_file(nunmai_cfg_path, nunmai_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{nunmai_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Nunmai Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Nunmai. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Nunmai Config Not Overwritten)",
                "",
                "These items already existed in your Nunmai config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Nunmai agent discovers these",
            "directories, it may read/write to them instead of the Nunmai state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `nunmai claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `nunmai claw cleanup` to fix it.",
            "",
            "## Nunmai-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `nunmai claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `nunmai setup` to configure any remaining settings",
            "- Run `nunmai mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `nunmai cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `nunmai cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `nunmai gateway install` if you need the gateway service",
            "- Review `~/.nunmai/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Nunmai Engine.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("NUNMAI_HOME") or str(Path.home() / ".nunmai"), help="Nunmai home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Nunmai targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Nunmai-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Nunmai Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.nunmai/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.nunmai/config.yaml")
        print("    2. Run: nunmai mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: nunmai cron")
