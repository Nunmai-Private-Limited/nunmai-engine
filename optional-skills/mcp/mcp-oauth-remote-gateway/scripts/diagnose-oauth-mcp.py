...
Usage:
  ...
  <server_name> matches the files in $NUNMAI_HOME/mcp-tokens/<server>.json etc.
...
def _nunmai_home():
    # Prefer Nunmai' own resolver (profile-safe); fall back to env then ~/.nunmai.
    try:
        from nunmai_constants import get_nunmai_home
        return str(get_nunmai_home())
    except Exception:
        return os.environ.get("NUNMAI_HOME") or os.path.expanduser("~/.nunmai")


def _tokens_dir():
    return os.path.join(_nunmai_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "nunmai-diag", "version": "1.0"}},
        },
        headers={
