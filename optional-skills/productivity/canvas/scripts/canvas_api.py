#!/usr/bin/env python3
"""Canvas LMS API CLI for Nunmai Engine.
...
def _check_config():
    ...
    if missing:
        nunmai_env = os.path.join(
            os.environ.get("NUNMAI_HOME", os.path.expanduser("~/.nunmai")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {nunmai_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Nunmai Engine"
    )
