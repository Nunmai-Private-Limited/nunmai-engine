...
Usage (via cron with --no-agent):
    ...
    nunmai cron create nunmai-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NUNMAI_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name nunmai-issues --repo Nunmai-Private-Limited/nunmai-engine --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Nunmai .env file
(``${NUNMAI_HOME:-~/.nunmai}/.env``) to avoid the 60 req/hr
