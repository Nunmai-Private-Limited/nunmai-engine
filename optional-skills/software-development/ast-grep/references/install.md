...
curl -fsSL "https://github.com/ast-grep/ast-grep/releases/download/${VERSION}/app-${TRIPLE}.zip" -o ~/.nunmai/cache/scratch/ast-grep.zip
unzip ~/.nunmai/cache/scratch/ast-grep.zip -d ~/.nunmai/cache/scratch/ast-grep
sudo mv ~/.nunmai/cache/scratch/ast-grep/ast-grep /usr/local/bin/sg
