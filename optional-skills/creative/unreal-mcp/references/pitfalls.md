...
### 1. Start order: editor first, Nunmai second

Nunmai probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Nunmai session.
...
restart the editor, restart the Nunmai session.
...
server fails to bind (Output Log) or Nunmai' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.nunmai/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Nunmai

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Nunmai' connection
lives in `~/.nunmai/config.yaml` via `nunmai mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Nunmai — just don't
mistake it for the Nunmai setup step.
...
explicitly warns against overlapping calls. Nunmai executes same-turn tool
...
### 9. Timeouts: Nunmai gives up before Unreal does

Nunmai' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Nunmai runs elsewhere (SSH backend,
