...
## 5. Nunmai Configuration

### Config location

`$NUNMAI_HOME/config.yaml` (defaults to `~/.nunmai/config.yaml` when `NUNMAI_HOME` is unset)
...
Restart the Nunmai session for changes to take effect. The MCP connection is
