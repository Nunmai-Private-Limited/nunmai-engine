...
Maps each role archetype to the Nunmai skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nunmai-engine
...
## Nunmai skills relevant to video production

### Visual / rendering skills (`nunmai-engine/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `nunmai plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`nunmai-engine/skills/creative/`)
...
### Audio / media skills (`nunmai-engine/skills/creative/` + `skills/media/`)
...
These are **not** Nunmai skills but external CLIs / APIs that profiles invoke.
...
## Built-in Nunmai tools for media review

These are native Nunmai tools — not invoked via terminal but through their own
...
key is present in `${NUNMAI_HOME:-~/.nunmai}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Nunmai profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NUNMAI_HOME:-~/.nunmai}/.env` → environment variable.
...
When a new Nunmai-public video skill ships:
