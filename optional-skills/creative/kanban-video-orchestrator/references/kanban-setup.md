...
the actual `setup.sh` that creates the project workspace, configures Nunmai
...
2. **Create profiles** — `nunmai profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nunmai/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nunmai kanban create` assigned to the director
...
```bash
nunmai profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nunmai/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nunmai/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nunmai
...
Each profile gets a `SOUL.md` at `~/.nunmai/profiles/<name>/SOUL.md` that
...
```bash
nunmai kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nunmai `.env` (`${NUNMAI_HOME:-$HOME/.nunmai}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nunmai_env="${NUNMAI_HOME:-$HOME/.nunmai}/.env"
    if grep -q "^${var}=" "$_nunmai_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nunmai_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nunmai_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nunmai ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nunmai OPENROUTER_API_KEY || exit 1
# ...
```
