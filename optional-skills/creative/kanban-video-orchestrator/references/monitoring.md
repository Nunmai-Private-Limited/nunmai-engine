...
nunmai kanban watch --tenant <project-slug>

# Snapshot of the board
nunmai kanban list --tenant <project-slug>
nunmai kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nunmai kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nunmai dashboard

# Inspect a specific task (includes comments + events)
nunmai kanban show <task-id>

# Follow a single task's event stream
nunmai kanban tail <task-id>
```

Verify available subcommands with `nunmai kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nunmai kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nunmai kanban block <id>` then `nunmai kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nunmai kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nunmai kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nunmai profile list` (profile exists?), `nunmai status` (gateway/dispatcher up?), and verify tenant. |
...
nunmai kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nunmai kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nunmai kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nunmai kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nunmai kanban block <task-id>
nunmai kanban archive <task-id>

# Diagnose what happened
nunmai kanban show <task-id>      # task body, comments, recent events
nunmai kanban tail <task-id>      # follow the live event stream
nunmai kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nunmai kanban create` for the director
...
while true; do
    clear
    nunmai kanban list --tenant <slug>
    echo "---"
    nunmai kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nunmai kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nunmai profile list` and start
