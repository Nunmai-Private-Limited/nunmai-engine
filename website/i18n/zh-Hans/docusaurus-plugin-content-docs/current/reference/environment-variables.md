...
内置 [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。在 `~/.nunmai/.env` 中设置。在这些变量生效之前，还必须启用该插件（`nunmai plugins enable observability/langfuse`，或在 `nunmai plugins` 中勾选）。
...
| `GATEWAY_PROXY_URL` | 将消息转发到的远程 Nunmai API 服务器 URL（[代理模式](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，gateway 仅处理平台 I/O——所有 agent 工作委托给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
...
| `NUNMAI_DEBUG_INTERRUPT` | 设为 `1` 可将详细的中断/取消追踪记录到 `agent.log`。 |
| `NUNMAI_DUMP_REQUESTS` | 将 API 请求载荷转储到日志文件（`true`/`false`） |
