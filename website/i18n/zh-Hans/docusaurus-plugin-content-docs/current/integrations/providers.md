...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 Nunmai 中启用[上下文压缩](/user-guide/configuration#context-compression)。
...
:::tip
故障转移仅通过 `config.yaml` 配置——或通过 `nunmai fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/user-guide/features/fallback-providers)。
:::
