...
同一订阅还解锁了 [Tool Gateway](../user-guide/features/tool-gateway.md)，将 Nunmai Engine 的工具调用路由至 Nunmai 托管的基础设施。五个后端，一次登录：
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.nunmai/auth.json` 中的 refresh token（刷新令牌），Nunmai 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
Nunmai Research 自家的 **Hermes 4** 系列（Hermes-4-70B、Hermes-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。

但**不建议在 Nunmai Engine 内部使用它们**。Hermes 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于研究工作流，或通过[订阅代理](../user-guide/features/subscription-proxy.md)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
Portal 自身的[模型信息页](https://portal.nousresearch.com/info)也有相同警告，因此这不是 Nunmai 侧的主观意见——这是 Nunmai Research 的官方指导。
...
你现有的提供商配置保持不变。可以在会话中途用 `/model` 切换，或在会话间用 `nunmai model` 切换——Portal 成为你的可用提供商之一，而非唯一选项。

### 无头环境 / SSH / 远程配置

OAuth 需要浏览器，但回调的 loopback 运行在 Nunmai 所在的机器上。对于远程主机，请参阅 [OAuth over SSH / 远程主机](../guides/oauth-over-ssh.md)——与其他基于 OAuth 的提供商相同的方式同样适用于 Portal（`ssh -L` 端口转发）。

### Profile 配置 {#profile-setup}

如果你使用 [Nunmai profiles（配置文件）](../user-guide/profiles.md)，Portal 的 refresh token 会通过共享令牌存储自动在所有 profile 间共享。在任意 profile 上登录一次，其余 profile 自动获取——无需为每个 profile 重复 OAuth 流程。
...
- **CLI 快捷方式：** `nunmai portal open`（在默认浏览器中打开同一页面）

## 配置参考

运行 `nunmai setup --portal` 后，`~/.nunmai/config.yaml` 将如下所示：
...
OAuth refresh token 单独存储在 `~/.nunmai/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。

## 令牌处理

Nunmai 在每次推理调用时从存储的 Portal refresh token 生成短期 JWT，而非复用长期 API 密钥。令牌生命周期完全自动管理——刷新、生成、在瞬时 401 时重试——你无需关心这些细节。

如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Nunmai 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `nunmai auth add nous` 重新登录；隔离状态在下次成功登录时自动清除。

## 故障排查

### `nunmai portal info` 显示"not logged in"
...
你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `nunmai auth add nous`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues)——我们将 Portal 目录同步至 Nunmai，缺口通常意味着可以更新的路由配置。
...
- **[订阅代理](../user-guide/features/subscription-proxy.md)** —— 在非 Nunmai 工具（其他 agent、脚本、第三方客户端）中使用你的 Portal 订阅
...
- **[Profiles](../user-guide/profiles.md)** —— 多个 Nunmai 配置共享一个 Portal 登录
