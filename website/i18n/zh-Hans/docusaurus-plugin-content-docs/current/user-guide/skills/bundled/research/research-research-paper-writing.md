...
本 skill 专为 Nunmai agent 设计。它使用 Nunmai 工具、委派、调度和记忆来支撑完整的研究生命周期。

### 相关 Skill

将本 skill 与其他 Nunmai skill 组合用于特定阶段：
...
| **plan** | 阶段 0（设置）：执行前创建结构化计划。写入 `.nunmai/plans/` | `skill_view("plan")` |
