...
[Nunmai Portal](/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `nunmai setup --portal` 即可登录并一键将 Nunmai 设为提供商。使用 `nunmai portal info` 查看当前配置。
:::
...
`nunmai model` 引导你选择提供商、完成认证（OAuth 流程会打开浏览器；API key 提供商会提示输入密钥），然后从该提供商的精选目录中选择具体模型。选择结果写入 `~/.nunmai/config.yaml` 的 `model.provider` 和 `model.model` 字段。
