...
一个例外：**启动 Windows 二进制文件的 MCP bridge。** 如果你通过 `cmd.exe` 使用 `chrome-devtools-mcp`（参见 [MCP 指南：WSL → Windows Chrome](../guides/use-mcp-with-nunmai.md#wsl2-bridge-nunmai-in-wsl-to-windows-chrome)），当 Nunmai 的当前工作目录是 `~` 时，Windows 可能会报 `UNC` 警告。此时请从 `/mnt/c/` 下的某个目录启动 Nunmai，以便 Windows 进程拥有一个带盘符的工作目录。
...
- 从 Windows 侧工具使用 **OpenAI 兼容 API 服务器**（当 `API_SERVER_ENABLED=true` 时由 `nunmai gateway` 暴露）。参见 [API Server 功能页](./features/api-server.md)。
...
Nunmai 的 [Tool Gateway](./features/tool-gateway.md) 和 API 服务器都是长期运行的进程。在 WSL2 中，有以下几种方式保持它们持续运行。
...
- **[MCP 指南 → WSL → Windows Chrome](../guides/use-mcp-with-nunmai.md#wsl2-bridge-nunmai-in-wsl-to-windows-chrome)** —— 从 WSL 中的 Nunmai 控制你已登录的 Windows Chrome。
