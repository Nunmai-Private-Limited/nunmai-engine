...
description: 通过 FAL.ai 文生图；支持 8 个模型，含 FLUX 2、GPT-Image、Nano Banana Pro、Ideogram、Recraft V4 Pro 等，可用 nunmai tools 切换。
sidebar_label: 文生图
sidebar_position: 6
---
...
Nunmai Engine 通过 FAL.ai 根据文字提示生成图像。默认内置 8 个模型，在速度、画质与成本上各有取舍。当前模型可通过 `nunmai tools` 配置，并持久化在 `config.yaml`。
...
:::tip Nunmai 订阅用户
若你持有付费 [Nunmai Portal](https://portal.nousresearch.com) 订阅，可通过 **[Tool Gateway](tool-gateway.md)** 使用文生图，**无需** `FAL_KEY`。模型选择在「直连 FAL」与「订阅网关」两条路径下保持一致。
...
进入 **🎨 Image Generation**，选择后端（Nunmai Subscription 或 FAL.ai），随后在表格中用方向键选择模型，回车确认：
...
```yaml
image_gen:
  provider: fal                 # 选 Nunmai Subscription 时为 `nous`
  model: fal-ai/flux-2/klein/9b
```

`image_gen.provider` 是唯一的选择键：`nous` 走托管网关，厂商名（如 `fal`）走直连。运行时始终按该选择路由——`provider: nous` 时 `.env` 里的 `FAL_KEY` 会被忽略；`provider: fal` 而缺少 `FAL_KEY` 会直接报错并提示运行 `nunmai tools`，不会静默回退。（旧的 `use_gateway` 键已废弃，读取时 `true` 等同于 `nous`。）

### GPT-Image 画质档位

`fal-ai/gpt-image-1.5` 的请求画质固定为 `medium`（约 1024×1024 下 $0.034/张）。面向用户**不开放** `low` / `high` 档位，以便 Nunmai Portal 侧计费在全体用户间更可预期（档位价差约 22×）。若需要更便宜的 GPT-Image 路线，请换其他模型；若追求更高画质，可考虑 Klein 9B 或同类 Imagen 系模型。
...
3. **提交** — `_submit_fal_request()` 根据凭据走直连 FAL 或 Nunmai 托管网关。  
...
- **需要 FAL 凭据**（直连 `FAL_KEY` 或 Nunmai 订阅网关）  
