...
如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Nunmai 的语音模式](../../guides/use-voice-mode-with-nunmai.md)。

## 前提条件

使用语音功能前，请确保已完成以下准备：

1. **已安装 Nunmai Engine** — 通过安装脚本（参见 [安装](../../getting-started/installation.md)）
...
付费的 [Nunmai Portal](./tool-gateway.md) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`nunmai setup --portal` 可一次性完成两项配置。
:::
