...
这会打开浏览器进行 Nous Portal OAuth 流程。Nunmai 将刷新令牌存储在 `~/.nunmai/auth.json` 中——与所有 Nunmai 提供商登录信息存放在同一位置。
...
当前已内置：`nous`（Nous Portal）。更多 OAuth 提供商可通过在 `nunmai_cli/proxy/adapters/` 中实现 `UpstreamAdapter` 接口来添加。
