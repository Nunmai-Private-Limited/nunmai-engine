...
[Adding Tools](../../developer-guide/adding-tools.md) 页面针对的是存放在 `tools/` 和 `toolsets.py` 中的 Nunmai 内置核心工具。

**→ [构建 Nunmai Plugin](../../developer-guide/plugins/index.md)** — 包含完整可运行示例的分步指南。
...
| 来源 | 路径 | 使用场景 |
|--------|------|----------|
| 内置 | `<repo>/plugins/` | 随 Nunmai 附带 — 参见 [Built-in Plugins](./built-in-plugins.md) |
...
| Nix | `services.nunmai-engine.extraPlugins` / `extraPythonPackages` | NixOS 声明式安装 — 参见 [Nix Setup](../../getting-started/nix-setup.md#plugins) |
...
插件可注册 `nunmai_cli.plugins.VALID_HOOKS` 当前接受的 24 个生命周期事件。**[Event Hooks 目录](./hooks.md#已发布的-plugin-hook-目录)**是精确触发时机、返回值处理、payload 字段和隐私说明的 canonical reference。
...
| 想要添加… | 方式 | 编写指南 |
|---|---|---|
| LLM 可调用的**工具** | Python 插件 — `ctx.register_tool()` | [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) · [Adding Tools](../../developer-guide/adding-tools.md) |
| **生命周期 hook**（LLM 前后、会话开始/结束、工具过滤） | Python 插件 — `ctx.register_hook()` | [Hooks reference](./hooks.md) · [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) |
| CLI / gateway 的**斜杠命令** | Python 插件 — `ctx.register_command()` | [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) · [Extending the CLI](../../developer-guide/extending-the-cli.md) |
| `nunmai <thing>` 的**子命令** | Python 插件 — `ctx.register_cli_command()` | [Extending the CLI](../../developer-guide/extending-the-cli.md) |
...
| **STT 后端**（自定义 whisper 二进制、本地 ASR CLI） | 配置驱动 — 将 `NUNMAI_LOCAL_STT_COMMAND` 环境变量设置为 shell 模板 | [Voice Message Transcription (STT)](./tts.md#voice-message-transcription-stt) |
| **通过 MCP 使用外部工具**（文件系统、GitHub、Linear、Notion、任意 MCP 服务器） | 配置驱动 — 在 `config.yaml` 中以 `command:` / `url:` 声明 `mcp_servers.<name>`。Nunmai 自动发现服务器的工具并与内置工具一同注册。 | [MCP](./mcp.md) |
| **额外 skill 来源**（自定义 GitHub 仓库、私有 skill 索引） | CLI — `nunmai skills tap add <repo>` | [Skills Hub](./skills.md#skills-hub) · [发布自定义 tap](./skills.md#publishing-a-custom-skill-tap) |
| **Gateway 事件 hook**（在 `gateway:startup`、`session:start`、`agent:end`、`command:*` 时触发） | 将 `HOOK.yaml` + `handler.py` 放入 `~/.nunmai/hooks/<name>/` | [Event Hooks](./hooks.md#gateway-event-hooks) |
...
在 NixOS 上，插件可通过模块选项声明式安装 — 无需 `nunmai plugins install`。完整详情请参见 **[Nix Setup 指南](../../getting-started/nix-setup.md#plugins)**。
