...
export function desktopInstallLink(name: string): string {
  return `nunmai://plugin/install?catalog=${encodeURIComponent(name)}`;
}
...
// nunmai_cli/plugin_catalog.py and website/scripts/extract-plugins.py.
export const CATEGORY_CONFIG: Record<string, { label: string; icon: string; blurb: string }> = {
  desktop: { label: "Desktop", icon: "\u{1F5A5}\u{FE0F}", blurb: "Panes, tabs and views for Nunmai Desktop" },
