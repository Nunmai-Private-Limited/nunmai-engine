#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Nunmai docs site.
...
LLM reads to learn what Nunmai does.

Both publish at:
  https://nunmai.in/docs/llms.txt
  https://nunmai.in/docs/llms-full.txt
...
SITE_BASE = "https://nunmai.in/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Nunmai as a Python Library", None),
        ("guides/use-mcp-with-nunmai", "Use MCP with Nunmai", None),
        ("guides/use-voice-mode-with-nunmai", "Use Voice Mode with Nunmai", None),
        ("guides/use-soul-with-nunmai", "Use SOUL.md with Nunmai", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Nunmai"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Nunmai Engine")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Nunmai Research. A terminal-native "
        ...
        "backends. Works with Nunmai Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/NousResearch/"
        "nunmai-engine/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Nunmai-Private-Limited/nunmai-engine")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Nunmai Engine — Full Documentation\n",
        (
            "This file is the entire Nunmai Engine documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Nunmai, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://nunmai.in/docs\n",
        "Short index: https://nunmai.in/docs/llms.txt\n",
        "\n---\n\n",
    ]
