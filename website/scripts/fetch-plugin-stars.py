...
LIVE_URL = "https://nunmai.in/docs/api/plugin-stars.json"
...
def _http_json(url: str, headers: dict[str, str], timeout: float = 15.0):
    req = urllib.request.Request(url, headers={"User-Agent": "nunmai-engine-docs", **headers})
...
def _graphql(query: str, token: str) -> dict:
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nunmai-engine-docs", "Authorization": f"Bearer {token}",
