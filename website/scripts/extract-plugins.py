...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Nunmai clients fetch this for live catalog refresh
  (``nunmai_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path, stars: dict[str, int] | None = None,
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "subdir": subdir,
            "requiresNunmai": str(raw.get("requires_nunmai") or "").strip(),
            ...
            "installCommand": f"nunmai plugins install {name}",
