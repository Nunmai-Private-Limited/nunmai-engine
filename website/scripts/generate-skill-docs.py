...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nunmai_meta = (fm.get("metadata") or {}).get("nunmai") or {}
    tags = nunmai_meta.get("tags") or []
    related = nunmai_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nunmai skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Nunmai Engine"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Nunmai ships with a large built-in skill library copied into `~/.nunmai/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Nunmai also syncs bundled skills on `nunmai update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nunmai/skills/` tree, it is still shipped with Nunmai; restore it with `nunmai skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with nunmai-engine — install via nunmai skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with nunmai-engine under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "nunmai skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "nunmai skills install official/blockchain/solana",
        "nunmai skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "nunmai skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
