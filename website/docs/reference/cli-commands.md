...
`nunmai -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
nunmai -z "summarize this repo" --usage-file /tmp/usage.json
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Nunmai Relay](/user-guide/messaging/relay). |
...
:::tip WSL users
Use `nunmai gateway run` instead of `nunmai gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s nunmai 'nunmai gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-nunmai-gateway-start-fails) for details.
:::
...
```bash
nunmai send --to telegram "MEDIA:/tmp/screenshot.png"
nunmai send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
nunmai send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
nunmai send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```
...
nunmai send --to discord:#ops --file /tmp/report.md
...
For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/nunmai-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/user-guide/features/kanban).
...
Subscriptions persist to `~/.nunmai/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
nunmai backup -o /tmp/nunmai.zip        # Full backup to specific path
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `nunmai config get model.default`). `--json` emits machine-readable output. |
...
| `--stop` | — | Stop running `nunmai dashboard` processes and exit. |
...
- **Gateway restart.** After a successful update, Nunmai attempts to restart all running gateway profiles automatically so they pick up the new code. Use `nunmai gateway restart` when you want to restart a gateway without applying an update.
