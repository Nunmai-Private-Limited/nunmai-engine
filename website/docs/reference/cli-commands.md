...
description: "Authoritative reference for Nunmai terminal commands and command families"
---
...
```bash
nunmai [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Nunmai profile to use for this invocation. Overrides the sticky default set by `nunmai profile use`. |
...
| `--ignore-user-config` | Ignore `~/.nunmai/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `NUNMAI_TUI=1`. Always wins over `display.interface`. |
...
### `nunmai-engine` (legacy single-query runner)

The install also ships `nunmai-engine`, a minimal runner that sends one query and exits: `nunmai-engine --query "summarize README.md"` (or `nunmai-engine "summarize README.md"`). `nunmai-engine --help` lists its options (`--model`, `--base-url`, `--max-turns`, `--enabled-toolsets`, `--disabled-toolsets`, `--list-tools`, `--save-trajectories`, …) and `nunmai-engine --version` prints the version; neither starts the agent. Run with no query, it prints the same help and exits. For anything else use `nunmai` (`nunmai -z <prompt>` is the scripted one-shot).
...
| `nunmai egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `nunmai lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `nunmai setup` | Interactive setup wizard for all or part of the configuration. |
...
| `nunmai secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.nunmai/.env`. |
| `nunmai migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `nunmai codex-runtime` | Noninteractive counterpart of `/codex-runtime`: `migrate [--dry-run] [--json]` regenerates the Nunmai-managed block in `~/.codex/config.toml` for the selected profile. See [Codex app-server runtime](../user-guide/features/codex-app-server-runtime.md#running-the-migration-from-a-script). |
| `nunmai status` | Show agent, auth, and platform status. |
| `nunmai usage` | Show the configured account's rate-limit windows (the `/usage` block) without a session; `--json` for scripts. |
| `nunmai cron` | Inspect and tick the cron scheduler. |
| `nunmai pause` / `nunmai resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `nunmai kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `nunmai project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `nunmai webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `nunmai hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `nunmai doctor` | Diagnose config and dependency issues. |
| `nunmai security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `nunmai approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `nunmai dump` | Copy-pasteable setup summary for support/debugging. |
| `nunmai prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `nunmai debug` | Debug tools — upload logs and system info for support. |
| `nunmai backup` | Back up Nunmai home directory to a zip file. |
| `nunmai checkpoints` | Inspect / prune / clear `~/.nunmai/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `nunmai import` | Restore a Nunmai backup from a zip file. |
| `nunmai logs` | View, tail, and filter agent/gateway/error log files. |
| `nunmai config` | Show, edit, migrate, and query configuration files. |
| `nunmai skin` | List, switch, and tweak display skins. |
| `nunmai console` | Open the safe Nunmai command console. |
| `nunmai pairing` | Approve or revoke messaging pairing codes. |
| `nunmai skills` | Browse, install, publish, audit, and configure skills. |
| `nunmai bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `nunmai curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `nunmai journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `nunmai memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `nunmai honcho`) register automatically when their provider is active. |
...
| `nunmai sessions` | Browse, export, prune, rename, and delete sessions. |
| `nunmai insights` | Show token/cost/activity analytics. |
| `nunmai claw` | OpenClaw migration helpers. |
| `nunmai import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `nunmai dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `nunmai serve` | Start the Nunmai backend server (headless; powers the desktop app and remote backends). |
| `nunmai desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `nunmai profile` | Manage profiles — multiple isolated Nunmai instances. |
| `nunmai completion` | Print shell completion scripts (bash/zsh/fish). |
| `nunmai --version` | Show version information. |
| `nunmai update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `NUNMAI_HOME` snapshot. |
| `nunmai uninstall` | Remove Nunmai from the system. |
...
| `--ignore-user-config` | Ignore `~/.nunmai/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Nunmai itself. |
...
```bash
nunmai
nunmai chat -q "Summarize the latest PRs"          # seeds an interactive session
nunmai chat --oneshot -q "Summarize the latest PRs"  # answer and exit
nunmai chat --provider openrouter --model anthropic/claude-sonnet-4.6
nunmai chat --toolsets web,terminal,skills
nunmai chat --quiet -q "Return only JSON"
nunmai chat -q "Inspect this repository" --format stream-json
nunmai chat --worktree -q "Review this repo and open a PR"
nunmai chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
nunmai chat --safe-mode -q "Is this bug mine or Nunmai'?"
```
...
```bash
nunmai chat -q "Summarize this repository" --format stream-json
```
...
(`NUNMAI_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `nunmai -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `nunmai -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
nunmai -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(nunmai -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.nunmai/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `NUNMAI_INFERENCE_MODEL` | Override the model for this run |
...
```bash
nunmai -z "…" --provider openrouter --model openai/gpt-5.5
# or:
NUNMAI_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 nunmai -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `nunmai chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".
...
`nunmai -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
nunmai -z "summarize this repo" --usage-file ~/.nunmai/cache/scratch/usage.json
jq .total_including_auxiliary.estimated_cost_usd ~/.nunmai/cache/scratch/usage.json
jq .auxiliary.by_task ~/.nunmai/cache/scratch/usage.json      # what did title generation / vision cost?
```

## `nunmai model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Nunmai chat session.
...
:::warning nunmai model vs /model — know the difference
**`nunmai model`** (run from your terminal, outside any Nunmai session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Nunmai chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Nunmai session first (`Ctrl+C` or `/quit`), then run `nunmai model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `nunmai model` from the terminal.
:::
...
| `migrate` | Fold per-profile standalone gateways onto the one host gateway (`--multiplex`, the only mode — `nunmai update` runs it automatically unless a real boundary blocks it). Re-running it converges a half-migrated host; a manifest on disk is the resume record, never a rollback (there is no `--standalone`). Runs a preflight (duplicate bot tokens, secondary port-binders without a `/p/<profile>/` ingress) and changes nothing when blocked. Flags: `--dry-run`, `-y`/`--yes`. See [Migrating from per-profile gateways](../user-guide/multi-profile-gateways.md#migrating-from-per-profile-gateways). |
| `migrate-legacy` | Remove legacy `nunmai.service` units left over from pre-rename installs. Profile units (`nunmai-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Nunmai Relay](../user-guide/messaging/relay.md). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `NUNMAI_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `nunmai update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `NUNMAI_GATEWAY_NO_SUPERVISE=1`. |
...
`nunmai gateway restart`, or service-restart update exits with status `75`
...
`nunmai gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `nunmai gateway run` instead of `nunmai gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s nunmai 'nunmai gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-nunmai-gateway-start-fails) for details.
:::

## `nunmai lsp`

```bash
nunmai lsp <subcommand>
```
...
```bash
nunmai slack manifest              # print manifest to stdout
nunmai slack manifest --write      # write to ~/.nunmai/slack-manifest.json
nunmai slack manifest --long-description-file AGENTS.md --write
nunmai slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$NUNMAI_HOME/slack-manifest.json`. |
| `--name NAME` | `Nunmai` | Bot display name in Slack. |
...
Run `nunmai slack manifest --write` again after `nunmai update` to pick
up any new commands.
...
```bash
nunmai send --to <target> "message text"
nunmai send --to <target> --file <path>
echo "message" | nunmai send --to <target>
nunmai send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.nunmai/.env` + `~/.nunmai/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `nunmai send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `nunmai send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
nunmai send --to telegram "MEDIA:~/.nunmai/cache/scratch/screenshot.png"
nunmai send --to telegram "Build chart for today MEDIA:~/.nunmai/cache/scratch/chart.png"   # with caption
nunmai send --to discord:#ops "MEDIA:~/.nunmai/cache/scratch/report.pdf"
```
...
```bash
nunmai send --to telegram "[[as_document]] MEDIA:~/.nunmai/cache/scratch/screenshot.png"
```

Examples:

```bash
nunmai send --to telegram "deploy finished"
echo "RAM 92%" | nunmai send --to telegram:-1001234567890
nunmai send --to discord:#ops --file ~/.nunmai/cache/scratch/report.md
nunmai send --to slack:#eng --subject "[CI]" --file build.log
nunmai send --list                  # all platforms
nunmai send --list telegram         # filter by platform
```
...
```bash
nunmai peer add <name> --url http://host:port --key <API_SERVER_KEY>
nunmai peer list
nunmai peer dm <peer>[/<agent>] "message"
nunmai peer run <peer>[/<agent>] --idempotency-key <key> "message"
nunmai peer status <peer>[/<agent>] <run_id>
nunmai peer stop <peer>[/<agent>] <run_id>
nunmai peer remove <name>
```

Bot-to-bot DMs across machines. Register another Nunmai gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`nunmai peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`nunmai -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `NUNMAI_PEER_<NAME>_KEY` in `~/.nunmai/.env`. |
...
includes the peer roster and the `nunmai peer dm` pattern, so agents discover
...
```bash
nunmai secrets bitwarden <subcommand>
nunmai secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.nunmai/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `nunmai claw migrate` (one-shot import of OpenClaw configuration into Nunmai) — `nunmai migrate` is the top-level config-rewrite command.


## `nunmai codex-runtime`

```bash
nunmai codex-runtime migrate [--dry-run] [--json]
```

Runs the `~/.codex/config.toml` migration that `/codex-runtime codex_app_server` triggers, without a chat session: Nunmai' `mcp_servers` (plus installed codex plugins and the `default_permissions` default) are projected into the managed block for the selected profile (`nunmai -p <name> codex-runtime migrate`). User text outside the block is kept verbatim; a user-owned `[mcp_servers.<name>]` with the same name as a Nunmai server is preserved and the Nunmai projection for that name skipped (reported as `preserved_user_servers`). The result is validated as TOML before an atomic write; exit code is 1 when the report contains errors.
...
```bash
nunmai security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Nunmai venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.nunmai/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Nunmai Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `nunmai login` / `nunmai logout` *(Deprecated)*

:::caution
`nunmai login` has been removed. Use `nunmai auth` to manage OAuth credentials, `nunmai model` to select a provider, or `nunmai setup` for full interactive setup.
:::
...
```bash
nunmai auth                                              # Interactive wizard
nunmai auth list                                         # Show all pools
nunmai auth list openrouter                              # Show specific provider
nunmai auth add openrouter --api-key sk-or-v1-xxx        # Add API key
nunmai auth add openrouter --type oauth                  # Browser login (OpenRouter PKCE) mints a key for you
nunmai auth add anthropic --type oauth                   # Add OAuth credential
nunmai auth add openai-codex --type oauth --priority 0   # Add an account and try it first
nunmai auth add openai-codex --browser                   # Codex: browser auth-code + PKCE on localhost:1455 instead of device code
nunmai auth remove openrouter 2                          # Remove by index
nunmai auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
nunmai auth reset openrouter                             # Clear cooldowns
nunmai auth reset openrouter 2                           # Clear the cooldown on one credential
nunmai auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
nunmai auth status anthropic                             # Show auth status for a provider
nunmai auth logout anthropic                             # Log out and clear stored auth state
nunmai auth spotify                                      # Authenticate Nunmai with Spotify via PKCE
```
...
```bash
nunmai usage                          # configured model provider, human-readable block
nunmai usage --provider openai-codex  # a specific provider
nunmai usage --json                   # one JSON document on stdout
```
...
```bash
nunmai cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
`$NUNMAI_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
```bash
nunmai kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.nunmai/kanban.db` for back-compat; additional boards live at `~/.nunmai/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `nunmai kanban boards switch`, the `NUNMAI_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](../user-guide/features/kanban.md#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `nunmai kanban`. Workers have `NUNMAI_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.nunmai/kanban/current`). |
...
nunmai kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
nunmai kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
nunmai kanban boards switch atm10-server
nunmai kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
nunmai kanban boards rm atm10-server
nunmai kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `NUNMAI_KANBAN_BOARD` env var → `~/.nunmai/kanban/current` file → `default`.
...
```bash
nunmai egress install                  # download the pinned iron-proxy binary
nunmai egress install --force          # re-download even if already installed

nunmai egress setup                    # interactive wizard: CA, mappings, config
nunmai egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
nunmai egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
nunmai egress setup --no-bitwarden     # explicitly switch back to env-based credentials
nunmai egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

nunmai egress start                    # spawn the managed proxy daemon
nunmai egress stop                     # SIGTERM (then SIGKILL after 5s grace)
nunmai egress restart                  # stop (if running) then start — needed for secret changes
nunmai egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

nunmai egress status                   # binary + config + pid + listening + mappings
nunmai egress status --show-tokens     # print proxy tokens in full (default: redacted)

nunmai egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
nunmai egress config                   # print the path to proxy.yaml for inspection
```
...
nunmai egress setup && nunmai egress start
nunmai config set terminal.backend docker   # if not already

# Switching credential source after the fact
nunmai egress setup --from-bitwarden       # env → bitwarden
nunmai egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
nunmai egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.nunmai/config.yaml proxy.extra_allowed_hosts: [api.example.com]
nunmai egress setup
nunmai egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
nunmai egress status                     # current state in one view
cat ~/.nunmai/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.nunmai/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.nunmai/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
nunmai project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
nunmai webhook <subscribe|list|remove|test>
```
...
### `nunmai webhook subscribe`

```bash
nunmai webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.nunmai/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
Subscriptions persist to `~/.nunmai/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
Exit status: `0` when the report lists no unresolved problems, `1` when at least one remains (including problems `--fix` could not repair), so a health gate or CI step can trust `nunmai doctor` as a check.
...
| Section | Details |
|---------|---------|
| **Header** | Nunmai version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, NUNMAI_HOME path |
...
:::tip
`nunmai dump` is specifically designed for sharing. For interactive diagnostics, use `nunmai doctor`. For a visual overview, use `nunmai status`.
:::

## `nunmai debug`

```bash
nunmai debug share [options]
```
...
```bash
nunmai backup [options]
```

Create a zip archive of your Nunmai configuration, skills, sessions, and data. The backup excludes the nunmai-engine codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/nunmai-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `nunmai-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Nunmai is running (WAL-mode safe).
...
- `models/`, `runtimes/`, `node/` at the root of `~/.nunmai` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
- Browser profiles: `browser-profile/` (the real-profile snapshot — copied Cookies / Login Data), `browser-profiles/` (live CDP profiles) at any depth, and `browser_profiles/` (the Browser Use CLI backend's Chromium user-data dir, with its own Login Data / Cookies) at the root of `~/.nunmai` and of each `profiles/<name>/`. Credential stores that must never enter an archive; all are regenerated on the next launch.
...
- The `nunmai-engine` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
nunmai backup                           # Full backup to ~/nunmai-backup-*.zip
nunmai backup -o ~/backups/nunmai.zip   # Full backup to specific path
nunmai backup --quick                   # Quick state-only snapshot
nunmai backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `nunmai checkpoints`

```bash
nunmai checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.nunmai/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `nunmai checkpoints` is equivalent. |
...
```bash
nunmai checkpoints                                  # status overview
nunmai checkpoints prune --retention-days 3         # aggressive cleanup
nunmai checkpoints prune --max-size-mb 200          # tighten size cap once
nunmai checkpoints clear-legacy -f                  # drop v1 archive dirs
nunmai checkpoints clear -f                         # wipe everything
```
...
```bash
nunmai import <zipfile> [options]
```

Restore a previously created Nunmai backup into your Nunmai home directory. All files in the archive overwrite existing files in your Nunmai home; `--force` only skips the confirmation prompt that fires when the target already has a Nunmai installation.
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: nunmai snapshot list
```

### Examples
```bash
nunmai import ~/nunmai-backup-20260423.zip           # Prompts before overwriting existing config
nunmai import ~/nunmai-backup-20260423.zip --force   # Overwrite without prompting
```

## `nunmai logs`

```bash
nunmai logs [log_name] [options]
```

View, tail, and filter Nunmai log files. All logs are stored in `~/.nunmai/logs/` (or `<profile>/logs/` for non-default profiles).
...
nunmai logs gateway -n 100

# Show only warnings and errors from the last hour
nunmai logs --level WARNING --since 1h

# Filter by a specific session
nunmai logs --session abc123

# Follow errors.log, starting from 30 minutes ago
nunmai logs errors --since 30m -f
...
nunmai logs --level WARNING --since 2h --session tg-12345
```
...
Nunmai uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `nunmai logs list` subcommand shows all log files including rotated ones.


## `nunmai prompt-size`

```bash
nunmai prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Nunmai layers
...
nunmai prompt-size --platform telegram

# Machine-readable output for scripts
nunmai prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`nunmai tools`) or
uninstall skills you don't need (`nunmai skills`). Context files (AGENTS.md,
...
`nunmai config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `nunmai config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `nunmai config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
nunmai pairing <list|approve|revoke|clear-pending>
```
...
```bash
nunmai skills browse
nunmai skills browse --source official
nunmai skills search react --source skills-sh
nunmai skills search https://mintlify.com/docs --source well-known
nunmai skills inspect official/security/1password
nunmai skills inspect skills-sh/vercel-labs/json-render/json-render-react
nunmai skills install official/migration/openclaw-migration
nunmai skills install skills-sh/anthropics/skills/pdf --force
nunmai skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
nunmai skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
nunmai skills check
nunmai skills update
nunmai skills config
nunmai skills reset google-workspace
nunmai skills reset google-workspace --restore --yes
nunmai skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
nunmai skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
nunmai skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Nunmai at a site exposing `/.well-known/skills/index.json`.
...
```bash
nunmai bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.nunmai/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.nunmai/skill-bundles/` and report added/removed bundles |

Examples:

```bash
nunmai bundles create backend-dev \
...
nunmai bundles list
nunmai bundles show backend-dev
nunmai bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.nunmai/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.nunmai/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `nunmai update`. Use `nunmai curator run --dry-run` to preview before that happens.
...
nunmai moa delete <name>
```

`nunmai moa configure` reuses Nunmai' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `nunmai fallback`

```bash
nunmai fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `nunmai model`) and append to the chain |
...
```bash
nunmai hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.nunmai/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.nunmai/shell-hooks-allowlist.json`.
...
```bash
nunmai memory <subcommand>
```

Set up and manage external memory provider plugins. Bundled providers: honcho, openviking, mem0, holographic, retaindb, byterover, supermemory; hindsight (plugin catalog) after `nunmai plugins install hindsight`. Only one external provider can be active at a time. Built-in memory (MEMORY.md/USER.md) is always active.
...
When an external memory provider is active, it may register its own top-level `nunmai <provider>` command for provider-specific management (e.g. `nunmai honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `nunmai --help` to see what's currently wired in.
:::
...
Starts Nunmai as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
nunmai mcp <subcommand>
```

Manage MCP (Model Context Protocol) server configurations and run Nunmai as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `nunmai mcp install deepwiki`). |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Nunmai](../guides/use-mcp-with-nunmai.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-nunmai-as-an-mcp-server).

## `nunmai plugins`

```bash
nunmai plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `nunmai plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Nunmai plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance on the installer's `.install-metadata.json` record (a `.nunmai-catalog.json` copy is written inside the plugin dir for convenience, but is never trusted). Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them, and on a catalog entry records the checked-out SHA rather than the reviewed pin. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist at install AND exempts that install from the update/enable/load-time kill-list checks. |
| `search [term] [--json]` | Search the Nunmai plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Nunmai Plugin](../developer-guide/plugins/index.md).

## `nunmai tools`

```bash
nunmai tools [--summary]
```
...
```bash
nunmai computer-use <subcommand>
```
...
`nunmai computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`nunmai tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Nunmai checks its version and runtime
...
installer. Nunmai never replaces a custom binary selected through
`NUNMAI_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`nunmai computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Nunmai integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Nunmai and links Cua's
skill pack into the Nunmai skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Nunmai passes Cua's canonical
...
`nunmai update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Nunmai update.

## `nunmai pets`

```bash
nunmai pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Nunmai shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
nunmai insights [--days N] [--source platform]
```
...
```bash
nunmai claw migrate [options]
```

Migrate your OpenClaw setup to Nunmai. Reads from `~/.openclaw` (or a custom path) and writes to `~/.nunmai`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Nunmai files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.nunmai/` (by default a single restore-point archive is written to `~/.nunmai/backups/pre-migration-*.zip` before apply; restorable with `nunmai import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Nunmai equivalents or **archived** for manual review.
...
nunmai claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
nunmai claw migrate --preset full

# Full migration including API keys
nunmai claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
nunmai claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
nunmai claw migrate --source /home/user/old-openclaw
```

## `nunmai import-agent`

```bash
nunmai import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Nunmai. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.nunmai/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Every successful import registers its source in `~/.nunmai/import-sync.json`; `nunmai import-agent --sync` then re-imports any registered source whose files changed (a cron-friendly way to keep an imported Claude Code / Codex setup current). See the **[import guide](../user-guide/import-from-other-agents.md)** for the full mapping tables.

## `nunmai serve`

```bash
nunmai serve [options]
```

Start the Nunmai **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](../user-guide/desktop.md) and remote clients connect to. It is the same server `nunmai dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `nunmai serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `nunmai dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `nunmai serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `NUNMAI_BACKEND_READY port=<port>`).

## `nunmai dashboard`

```bash
nunmai dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`nunmai serve`](#nunmai-serve) above.) Requires `cd ~/.nunmai/nunmai-engine && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.nunmai/nunmai-engine && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](../user-guide/features/web-dashboard.md) for full documentation.
...
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://nunmai.example.com/auth/callback`). Omit for localhost-only use. |
...
nunmai dashboard --port 8080 --no-open
...
```bash
nunmai profile <subcommand>
```

Manage profiles — multiple isolated Nunmai instances, each with its own config, sessions, skills, and home directory.
...
```bash
nunmai profile list
nunmai profile create work --clone
nunmai profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
nunmai profile use work
nunmai profile alias work --name h-work
nunmai profile export work -o work-backup.tar.gz
nunmai profile import work-backup.tar.gz --name restored
nunmai profile install github.com/user/my-distro --alias
nunmai profile update work
nunmai -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Nunmai commands, subcommands, and profile names.

Examples:

```bash
# Bash
nunmai completion bash >> ~/.bashrc

# Zsh
nunmai completion zsh >> ~/.zshrc

# Fish
nunmai completion fish > ~/.config/fish/completions/nunmai.fish
```

## `nunmai update`

```bash
nunmai update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `nunmai-engine` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`nunmai update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Nunmai may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `NUNMAI_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `nunmai config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Nunmai attempts to restart all running gateway profiles of the home being updated (its root and every `profiles/<name>` under it) automatically so they pick up the new code. Gateways and `nunmai-gateway*` services that belong to a different `NUNMAI_HOME` on the same machine — another install, or a scratch home running `nunmai update` — are named in the output and left alone. Use `nunmai gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.nunmai/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Nunmai makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `nunmai update`.
- **Pairing data snapshot.** Even when `--backup` is off, `nunmai update` takes a lightweight snapshot of `~/.nunmai/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `nunmai backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `nunmai.service` warning.** If Nunmai detects a pre-rename `nunmai.service` systemd unit (instead of the current `nunmai-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `nunmai --version` | Print version information. |
| `nunmai update` | Pull latest changes and reinstall dependencies. |

| `nunmai uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Nunmai, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
