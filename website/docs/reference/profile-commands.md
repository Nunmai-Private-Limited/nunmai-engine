...
This page covers all commands related to [Nunmai profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `nunmai profile` without a subcommand shows help.
...
```bash
nunmai profile use <name>
```

Sets `<name>` as the active profile. All subsequent `nunmai` commands (without `-p`) will use this profile.
...
nunmai profile use default
```

## `nunmai profile create`

```bash
nunmai profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `nunmai profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `nunmai update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.nunmai`), use `nunmai skills opt-out` / `nunmai skills opt-in`. |
...
nunmai profile create work2 --clone-from work

# Clone everything from a specific profile
nunmai profile create work2-backup --clone-from work --clone-all
```

## `nunmai profile describe`

```bash
nunmai profile describe [<name>] [options]
```
...
nunmai profile describe researcher

# Set it explicitly
nunmai profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
nunmai profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
nunmai profile describe --all --auto
```

## `nunmai profile delete`

```bash
nunmai profile delete <name> [options]
```
...
```bash
nunmai profile delete mybot
nunmai profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.nunmai`) cannot be deleted — use `nunmai uninstall` to remove everything.
:::

## `nunmai profile show`

```bash
nunmai profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

The skills count here (and in `nunmai profile list`) is counted on the spot. The Desktop and dashboard profile lists are polled every few seconds, so they show the last known count instead and refresh it in the background — a freshly started backend may briefly show `0` skills for a profile until the first background count lands, and a skill you just installed appears in those lists within about a minute.

This shows the profile's Nunmai home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ nunmai profile show work
Profile: work
Path:    ~/.nunmai/profiles/work
...
## `nunmai profile alias`

```bash
nunmai profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Nunmai installation.
...
```bash
nunmai profile alias work
# Creates/updates ~/.local/bin/work

nunmai profile alias work --name mywork
# Creates ~/.local/bin/mywork

nunmai profile alias work --remove
# Removes the wrapper script
```

## `nunmai profile rename`

```bash
nunmai profile rename <old-name> <new-name>
```
...
```bash
nunmai profile rename mybot assistant
# ~/.nunmai/profiles/mybot → ~/.nunmai/profiles/assistant
...
after the rename; `nunmai profile migrate-identity` retries that step too if it was reported as
failed.

## `nunmai profile migrate-identity`

```bash
nunmai profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `nunmai profile
...
#     nunmai profile migrate-identity mybot assistant

nunmai profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `nunmai profile purge-identity`

```bash
nunmai profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `nunmai profile delete`
...
is decided by `nunmai profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     nunmai profile purge-identity mybot

nunmai profile purge-identity mybot
# ✓ Session/routing identity purged: mybot
```

## `nunmai profile export`

```bash
nunmai profile export <name> [options]
```
...
```bash
nunmai profile export work
# Creates work.tar.gz in the current directory

nunmai profile export work -o ./work-2026-03-29.tar.gz
```
...
## `nunmai profile import`

```bash
nunmai profile import <archive> [options]
```
...
```bash
nunmai profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

nunmai profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nunmai -p telemetry`). |
...
nunmai profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
nunmai profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
nunmai profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
nunmai profile install ./telemetry/
```

### `nunmai profile update`

```bash
nunmai profile update <name> [--force-config] [--yes]
```
...
```bash
nunmai profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Nunmai version, author, env var requirements, the source URL/path, and
...
`nunmai profile list` also shows the distribution name and version in a
`Distribution` column, and `nunmai profile show <name>` / `delete <name>`
...
nunmai profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
nunmai profile install https://github.com/your-org/internal-assistant.git
```
...
`nunmai_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Nunmai version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Nunmai can clone from.
3. Tell recipients to run `nunmai profile install <your-repo-url>`.
...
## `nunmai -p` / `nunmai --profile`

```bash
nunmai -p <name> <command> [options]
nunmai --profile <name> <command> [options]
```

Global flag to run any Nunmai command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
nunmai -p work chat -q "Check the server status"
nunmai --profile dev gateway start
nunmai -p personal skills list
nunmai -p work config edit
```

## `nunmai completion`

```bash
nunmai completion <shell>
```
...
- `nunmai profile <TAB>` — subcommands (list, use, create, etc.)
- `nunmai profile use <TAB>` — profile names
- `nunmai -p <TAB>` — profile names
