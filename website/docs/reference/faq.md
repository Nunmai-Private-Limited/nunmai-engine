...
description: "Frequently asked questions and solutions to common issues with Nunmai Engine"
---
...
### What LLM providers work with Nunmai?

Nunmai Engine works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Nunmai Portal](../integrations/nous-portal.md)** — Nunmai Research's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `nunmai auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `nunmai model` or by editing `~/.nunmai/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Nunmai in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Nunmai inside WSL2
...
- let Nunmai use the resulting MCP browser tools

This is more reliable than trying to force Nunmai core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Nunmai](../guides/use-mcp-with-nunmai.md#wsl2-bridge-nunmai-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Nunmai Engine does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.nunmai/`.

### Can I use it offline / with local models?

Yes. Run `nunmai model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Nunmai minimum; set this to match your server's actual context window
```
...
Nunmai persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Nunmai — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Nunmai auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `NUNMAI_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Nunmai Engine itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Nunmai Engine instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Nunmai programmatically:
...
#### `nunmai: command not found` after installation
...
**Cause:** Nunmai requires Python 3.11 or newer.
...
**Cause:** Nunmai builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Nunmai runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Nunmai auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.nunmai/config.yaml`:
...
Independently of the init files, every terminal command's `PATH` is completed with the standard system directories (`/usr/local/bin`, `/opt/homebrew/bin`, …), the Nunmai-managed runtime dirs, and `~/.local/bin` when it exists (the `pip --user` / `pipx` / `uv tool` install target) — appended after your own entries, so precedence is unchanged. This covers backends started with a thin non-interactive PATH (systemd, GUI launchers, the Desktop SSH remote backend) without any configuration.
...
#### The agent says "Nunmai policy" or "Nunmai guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Nunmai runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Nunmai enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Nunmai does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Nunmai tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Nunmai execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](../user-guide/security.md) for Nunmai' documented execution controls and [Providers](../integrations/providers.md) for provider configuration.

#### "…refused this request because of a policy on your account"

**Meaning:** the provider rejected the request for an account-level reason that retrying cannot change — an aggregator's data/privacy settings excluded every endpoint for the model, or the model's upstream provider has blocked the account (for example `this user has been blocked for a previous policy violation`, which OpenRouter can relay inside an otherwise successful HTTP 200 stream). Nunmai sends the request once, does not retry it or rotate credentials, and moves to your fallback chain if one is configured.

**Solution:** check the account's status and data/privacy settings with the provider named in the reply, or switch to another model or provider with `/model`. `nunmai fallback add` routes future blocks to a backup automatically.
...
| "The AI model service isn't reachable right now — the configured model endpoint is **not running or is unreachable**." | Nothing accepted the connection (`Connection refused`, no route to host, DNS failure). | Start the model server / check `base_url`, then `/retry`; `nunmai doctor` on the host. |
| "Nunmai **could not reach** the AI model service (no further detail from the SDK)." | The SDK reported a generic `APIConnectionError` and kept no cause; neither of the above is certain. | `/retry`; `nunmai doctor` if it persists. The raw exception is in `nunmai logs`. |
...
**Solution:** Exit your session and use `nunmai model` from your terminal to add new providers:

```bash
# Exit the Nunmai chat session first (Ctrl+C or /quit)
...
After adding a new provider via `nunmai model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `nunmai model` (from terminal) |
| Enter/change API keys | `nunmai model` (from terminal) |
...
# Or set directly
nunmai config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.nunmai/.env` for conflicting entries.
:::
...
# Set a valid model
nunmai config set model.default anthropic/claude-opus-4.7

# Or specify per-session
nunmai chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `nunmai chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Nunmai detected the wrong context length for your model.
...
nunmai chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Nunmai may have the wrong context length for your model. Check what it detected:
...
**Cause:** Nunmai detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Nunmai never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `nunmai chat`
...
nunmai gateway status

# Start the gateway
nunmai gateway start

# Check logs for errors
cat ~/.nunmai/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `nunmai gateway setup`
- Check gateway logs: `cat ~/.nunmai/logs/gateway.log | tail -50`
...
Configure in `~/.nunmai/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.nunmai/nunmai-engine && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `nunmai gateway start` fails
...
tmux new -s nunmai 'nunmai gateway run'
# Reattach later: tmux attach -t nunmai

# Option 3: Background via nohup
nohup nunmai gateway run > ~/.nunmai/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'nunmai gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `nunmai gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
nunmai gateway install    # Re-snapshots your current PATH
nunmai gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.nunmai.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `nunmai chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `nunmai chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`nunmai prompt-size`](./cli-commands.md#nunmai-prompt-size) to measure it, then trim: disable toolsets you don't use (`nunmai tools`) and uninstall or disable skills you don't need (`nunmai skills`).
...
cd ~/.nunmai/nunmai-engine && uv pip install -e ".[mcp]"
...
Verify your `~/.nunmai/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
nunmai config show | grep -A 12 mcp_servers

# Restart Nunmai or reload MCP after config changes
nunmai chat
```

See also:
- [MCP (Model Context Protocol)](../user-guide/features/mcp.md)
- [Use MCP with Nunmai](../guides/use-mcp-with-nunmai.md)
...
:::warning
If an MCP server crashes mid-request, Nunmai will report a timeout. Check the server's own logs (not just Nunmai logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `nunmai profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Nunmai home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](../user-guide/features/memory-providers.md).

### What happens when I run `nunmai update`?

`nunmai update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `nunmai update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is a directory under `~/.nunmai/profiles/` that carries at least one identity file (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json` or `state.db`); a bare directory without one (a leftover from a log rotation or cron tick) is not a profile — it is not listed or served, `-p <name>` reports it as missing, and `nunmai profile create <name>` refuses to overwrite it until you move or remove it. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Nunmai can route subagents to a different model automatically. Set this in `~/.nunmai/config.yaml`:
...
Now when you tell Nunmai "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Nunmai do this?

**Current limitation:** Nunmai profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Nunmai reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `nunmai skills config` settings don't seem to take effect.

**Solution:** Use `nunmai skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`nunmai gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Nunmai creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Nunmai to another machine
...
1. Install Nunmai Engine on the new machine:
   ```bash
   curl -fsSL https://nunmai-engine.nunmai.in/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   nunmai backup
   ```
   This creates a zip of your entire `~/.nunmai/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/nunmai-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/nunmai-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   nunmai import ~/nunmai-backup-<timestamp>.zip
   ```

4. On the new machine, run `nunmai setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
nunmai profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
nunmai profile import ./work-backup.tar.gz work
```
...
### `nunmai backup` vs `nunmai profile export`

| Feature | `nunmai backup` | `nunmai profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.nunmai` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='nunmai-engine' ~/.nunmai/ newmachine:~/.nunmai/
```

:::tip
`nunmai backup` produces a consistent snapshot even while Nunmai is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Nunmai installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Nunmai-specific issue — it's a shell config permissions problem.
...
nunmai config show | head -20

# Re-run model selection
nunmai model

# Or test with a known-good model
nunmai chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues)
2. **Ask the community:** [Nunmai Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Nunmai version (`nunmai --version`), and the full error message
