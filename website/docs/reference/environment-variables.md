...
| `NUNMAI_HOME` | Override Nunmai config directory (default: `~/.nunmai`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |
...
Environment variables for the bundled [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) plugin. Set these in `~/.nunmai/.env`. The plugin must also be enabled (`nunmai plugins enable observability/langfuse`, or check the box in `nunmai plugins`) before any of these take effect.
...
| `GATEWAY_PROXY_URL` | URL of a remote Nunmai API server to forward messages to ([proxy mode](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
Connect Nunmai to an IRC server. No external dependencies. See [the IRC messaging guide](/user-guide/messaging/irc).
...
Connect Nunmai to a [SimpleX Chat](https://simplex.chat/) network via a local `simplex-chat` daemon. See [the SimpleX messaging guide](/user-guide/messaging/simplex).
...
Connect Nunmai to [Photon](https://photon.codes/) / Spectrum (iMessage and other Spectrum platforms) via the Node sidecar. See [the Photon messaging guide](/user-guide/messaging/photon).
...
| `NUNMAI_DISCORD_LIVENESS_FAILURE_THRESHOLD` | Compatibility/manual override for `discord.websocket_liveness_failure_threshold`. Consecutive unhealthy WebSocket samples before forcing a reconnect (default: `2`). Prefer the `config.yaml` key. |
...
| Variable | Description |
|----------|-------------|
| `NUNMAI_NEMO_RELAY_PLUGINS_TOML` | Explicit path to the standard NeMo Relay `plugins.toml` loaded process-wide by Nunmai core. When unset, Nunmai does not initialize Relay middleware, dynamic plugins, or exporters. The removed `NUNMAI_NEMO_RELAY_ATOF_*` and `NUNMAI_NEMO_RELAY_ATIF_*` variables are ignored; configure those outputs in the selected file instead. See [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about). |
...
| `NUNMAI_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `NUNMAI_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `NUNMAI_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
...
| `NUNMAI_DEBUG_INTERRUPT` | Set to `1` to log detailed interrupt/cancel tracing to `agent.log`. |
| `NUNMAI_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
...
:::tip
Use `nunmai config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
