...
| `/moa <prompt>` | Run a single prompt through the default [Nunmai Agent](/user-guide/features/mixture-of-agents) preset, then restore your current model. One-shot — does not change your session model. |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.nunmai/config.yaml`. See [Skill Bundles](/user-guide/features/skills#skill-bundles). |
...
| `/login` | Sign in with a Nunmai account. Runs off-turn: the consent link and code arrive in the session, and the sign-in settles when you approve it in the browser. See [Nunmai free tier](/user-guide/free-tier). |
...
| `/login` | Sign in with a Nunmai account. **Paired direct messages only** — in a group, channel, or broadcast-shaped platform Nunmai refuses. On Slack use `/nunmai login`. See [Nunmai free tier](/user-guide/free-tier). |
| `/whoami` | Show your slash command access level (admin / user). |
...
| `/moa <prompt>` | Run one prompt through the default [Nunmai Agent](/user-guide/features/mixture-of-agents) preset, then restore the session model. |
| `/branch [name]` (alias: `/fork`) | Branch the current session (explore a different path). |
