...
description: "Authoritative reference for Nunmai built-in tools, grouped by toolset"
---

# Built-in Tools Reference

This page documents Nunmai' built-in tools, grouped by toolset. Availability varies by platform, credentials, and enabled toolsets.
...
:::tip MCP Tools
In addition to built-in tools, Nunmai can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp__<server>__` (e.g., `mcp__github__create_issue` for the `github` MCP server). See [MCP Integration](../user-guide/features/mcp.md) for configuration.
:::
...
Nunmai Portal has enabled connectors for the signed-in account (the `managed_tools` claim on its
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `execute_code` | Run a Python script that can call Nunmai tools programmatically. Use this when you need 3+ tool calls with processing logic between them, need to filter/reduce large tool outputs before they enter your context, need conditional branching (… | — |
...
Scoped to the Feishu document-comment intelligent-reply handler (`gateway/platforms/feishu_comment.py`). Not exposed on `nunmai-cli` or the regular Feishu chat adapter.
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS, Windows, and Linux. | `cua-driver` on `$PATH` (install via `nunmai tools`). |
...
Registered when the agent is either (a) spawned by the kanban dispatcher (`NUNMAI_KANBAN_TASK` env set) or (b) running in a profile that explicitly enables the `kanban` toolset. Task-scoped workers use lifecycle tools for their assigned task; orchestrator profiles additionally get board-routing tools like `kanban_list` and `kanban_unblock`. See [Kanban Multi-Agent](../user-guide/features/kanban.md) for the full workflow.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `kanban_show` | Show the active kanban task assigned to this worker (title, description, comments, dependencies). | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_complete` | Mark the current task done with a structured handoff payload (results, artifacts, follow-ups). | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_block` | Block the current task on a question for the user — the dispatcher pauses, surfaces the question, and resumes once a human replies. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_review` | Hand the implementation to a reviewer with `summary`, optional structured `metadata`, and an optional reviewer profile. Moves the same task to `review`; it is not a block and does not affect block-loop accounting. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_request_changes` | Reviewer verdict for an actively claimed review run. Closes the review run, reapplies parent gating, and routes the task back to the original implementer without using a block. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_heartbeat` | Send a progress heartbeat during a long-running operation so the dispatcher knows the worker is still alive. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_comment` | Add a comment to the task thread without changing its state — useful for surfacing intermediate findings. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_create` | Fan out child tasks from the current task. Used by orchestrators and follow-up-spawning workers. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_link` | Link tasks with a parent → child dependency edge. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_url` | Attach a file to a task by URL — Nunmai downloads it server-side and stores it as a real attachment (capped at 25 MB). Only http/https URLs. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
| `kanban_attachments` | List the files attached to a task: id, filename, content_type, size, uploader, and the absolute on-disk path. | `NUNMAI_KANBAN_TASK` or `kanban` toolset |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `skill_manage` | Manage skills (create, update, delete). Skills are your procedural memory — reusable approaches for recurring task types. New skills go to ~/.nunmai/skills/; existing skills can be modified wherever they live. Actions: create (full SKILL.m… | — |
...
Enabled for sessions whose source is the Nunmai desktop app, on any backend it
is connected to (local, SSH, URL, or Nunmai Cloud). Absent from CLI, TUI,
messaging, and cron sessions.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `read_terminal` | Read what's currently shown in the in-app terminal pane of the Nunmai desktop GUI (the embedded shell beside this chat). | — |
| `close_terminal` | Close the read-only terminal tab for a background process in the Nunmai desktop GUI. Does NOT kill the process — only drops the tab/view; use process_manage(action='kill') to stop it. | — |
| `desktop_preview` | Drive the preview pane beside the chat in the Nunmai desktop app: `open` a web URL, localhost dev-server URL, or file path (HTML renders live); `close` the whole pane (omit `url`) or one tab inside it (pass the URL or file path); `read` what the pane currently shows — the in-app Browser's page text (URL + title + rendered text, pageable with `start`/`count`) or a file/artifact tab's identity. | — |
...
| `read_window_below` | Identify the OS window directly underneath the Nunmai desktop window — app name, title, bounds (metadata only, never pixels). On macOS, other apps' titles appear only when Screen Recording is already granted; the tool never prompts for it. | — |
| `focus_pane` | Reveal and focus a pane in the Nunmai desktop app (chat, files, terminal, review, sessions). | — |
...
| `gui_tour` | Give a live guided tour: dim the screen, highlight an element, and attach a narrated popover (driver.js). Works on the Nunmai app's own UI and on any page open in the preview pane; `targets` discovers what's on screen, `show` narrates step-by-step, `start` hands the user Next/Prev controls. | — |
...
| `apply_layout` | Apply a saved layout preset to the Nunmai desktop app when the user asks to rearrange the workspace. Built-ins: default (chat + sidebars), focus (chat only), terminal-deck, quad; plugin/user presets by id. To reveal ONE pane, use `focus_pane` instead. | — |
...
only at a genuinely idle moment. A tip from Nunmai shares that cooldown, so it
...
(`display.in_app_tips`, `display.in_app_tours`). Off covers Nunmai as well as
...
Opt-in toolset (not loaded in the default `nunmai-cli` set). Add via `--toolsets video` or include `video` in your `toolsets:` config.
...
Opt-in toolset (not loaded in the default `nunmai-cli` set). Add via `--toolsets video_gen` or enable it in `nunmai tools` → Video Generation, which also walks you through picking a backend.
...
- **OpenRouter** — every generative model on OpenRouter's video API (Veo 3.1, Sora 2 Pro, Kling 3, Seedance 2, Wan 3, Hailuo 3, Grok Imagine, FLUX 3 Video, …); text-to-video, image-to-video and reference-to-video; catalog and per-model limits fetched live (requires `OPENROUTER_API_KEY` or a credential added with `nunmai auth add openrouter`, billed to your OpenRouter credit).
...
Registered on the `nunmai-discord` platform toolset (gateway only). Uses the same bot token as the messaging adapter.
...
Registered on the `nunmai-discord` platform toolset. Moderation actions require the bot to hold the matching Discord permissions.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `nunmai auth spotify` once to authorize.
...
## `nunmai-yuanbao` toolset

Registered only on the `nunmai-yuanbao` platform toolset. Yuanbao is Tencent's chat app; these tools drive its DM/group/sticker APIs.
