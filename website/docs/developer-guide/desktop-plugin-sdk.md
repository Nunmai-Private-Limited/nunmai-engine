...
title: "Desktop Plugin SDK (@nunmai/plugin-sdk)"
description: "Extend the native Nunmai Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Nunmai Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `NunmaiPlugin`.
It imports one module — `@nunmai/plugin-sdk` — and gets everything: the app's
...
`$NUNMAI_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Nunmai. This page is the **native
desktop app** (`nunmai desktop`) SDK — the `@nunmai/plugin-sdk` module and
`$NUNMAI_HOME/desktop-plugins/`. The **web dashboard** (`nunmai dashboard`) has
its own, unrelated plugin system on `window.__NUNMAI_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a Nunmai Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$NUNMAI_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$NUNMAI_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `NunmaiPlugin` contract, appear in **Capabilities → Plugins**,
...
Create `$NUNMAI_HOME/desktop-plugins/hello/plugin.js` (that's `~/.nunmai/...`
...
```javascript
// ~/.nunmai/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@nunmai/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Nunmai' }),
      jsx('div', {
...
The only importable specifiers are `@nunmai/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `NunmaiPlugin`:

```ts
interface NunmaiPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `nunmai.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@nunmai/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@nunmai/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@nunmai/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@nunmai/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@nunmai/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@nunmai/plugin-sdk'
...
```javascript
import { host } from '@nunmai/plugin-sdk'
...
| Plugin | Was | Now |
|---|---|---|
| next-prompt (#120660) | `window.dispatchEvent(new CustomEvent('nunmai:composer-insert', …))` + a `setTimeout` `nunmai:composer-focus`; `[data-composer-target]`/`[data-pane-hidden]` scan for the visible target | `await host.composer.insertText(null, suggestion.text, { mode: 'block' })`, then `host.composer.focus(null)` if the pill lost the caret |
| prompt-snippets (#116030) | same `nunmai:composer-insert` event; `[data-slot="composer-input"]`/ProseMirror `textContent` + synthetic `InputEvent` fallback; `surfaceEditorEl().focus()` | `host.composer.insertText(sid, text, { mode: 'block' })`; `setDraft(sid, (await getDraft(sid) ?? '') + '\n' + text)` replaces the fallback; `host.composer.focus(sid)` — `sid = host.state.focusedSessionId.get()` |
...
```ts
import { SESSION_ROW_AREAS, type SessionRowSlotContribution } from '@nunmai/plugin-sdk'
...
- **better-session-appearance** — replace the `localStorage`
  `nunmai.desktop.sessionColors` write and the fiber-harvested `onChange` with
...
```ts
import { COMPOSER_AREAS, type ComposerModelPillContext, type ComposerModelPillProvider } from '@nunmai/plugin-sdk'
...
* **nunmai-appearance-hub** — mount its paper-texture / font / intro-copy
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@nunmai/plugin-sdk'
...
`window.nunmai.send('get-price eth')` (or a declarative
`<button data-nunmai-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@nunmai/plugin-sdk'
...
```ts
import { SIDEBAR_NAV_PREFS_AREA, type SidebarNavPrefsContribution } from '@nunmai/plugin-sdk'
...
for requests; `targetProfile` is the backend Nunmai profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Nunmai
...
ctx.os.notify({
  ...
  // Body click → focus Nunmai + navigate. Same vocabulary as OS deep links:
  activate: 'nunmai://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'nunmai://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `nunmai://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `nunmai://…` URL works as an OS deep link). Action buttons only render on
...
|--------|-------------|-------------------|
| keybind map (`nunmai.desktop.keybinds`) | `KEYBINDS_AREA` contribution | a raw map write rebinds every other plugin's shortcuts; the area merges per plugin and is torn down with it |
...
| `toolView.technical`, `embed-mode`, `titlebarAppActions`, `translucency.v2`, `user-bubble-transparency.v1`, `nunmaiDesktop.zoom.*` | follow-up keys after each store is audited | some drive the main process or window chrome; each needs its own guard and ownership review before it becomes plugin-writable |

Migration — `nunmai-appearance-hub`, which today does
`localStorage.setItem('nunmai.desktop.sessionListDensity', id)` followed by
...
```ts
// before
localStorage.setItem('nunmai.desktop.backdrop.v1', String(on))
window.dispatchEvent(new StorageEvent('storage', { key: 'nunmai.desktop.backdrop.v1', newValue: String(on) }))
// after — the store notifies its own subscribers; no synthetic StorageEvent
host.settings.set('backdrop.v1', on)
host.settings.set('sessionListDensity', id)          // was nunmai.desktop.sessionListDensity
host.settings.set('tabStripDefault', id)             // was nunmai.desktop.tabStripDefault
host.settings.set('reasoning.collapsedByDefault', on) // was nunmai.desktop.reasoning.collapsedByDefault
...
`localStorage.getItem('nunmai.desktop.keybinds')` to back up its shortcut — that
...
calling `window.nunmaiDesktop.api` raw.
...
JSON.parse(localStorage.getItem(                           host.pluginDecisions.get()
  'nunmai.desktop.pluginDecisions.v2'))                    ctx.onDispose(host.pluginDecisions.subscribe(fn))
localStorage.setItem('nunmai.desktop.pluginDecisions.v2')  // declined — host.navigate('/capabilities?tab=plugins')
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@nunmai/plugin-sdk'
...
```javascript
import { queryClient } from '@nunmai/plugin-sdk'
...
copies that half into `$NUNMAI_HOME/desktop-plugins/<id>/` beside a
`.nunmai-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.nunmai/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`nunmai plugins update`, or
...
a package into `~/.nunmai/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.nunmai/plugins` is not reachable as a filesystem —
...
the `nunmai://` scheme — a plain anchor on your website or README:
...
`nunmai-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Nunmai plugin and declare it in a
`manifest.json`:
...
nunmai-engine codebase directly (`nunmai_state`, `nunmai_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.nunmai/`)
...
```python
from nunmai_cli.plugin_events import broadcast_plugin_event
...
|---|---|
| `nunmai serve` (the Desktop backend): `plugin_api.py` routers, plugin slash commands, tools and hooks in the agent turn | every connected Desktop window |
| the `dashboard.turn_isolation` compute-host child (tools/hooks of an isolated turn) | relayed over the host pipe to `nunmai serve`, then every window |
| the stdio TUI (`nunmai` in a terminal) | that terminal's client |
| `nunmai gateway run` (messaging platforms), `nunmai chat`, cron, `nunmai plugins validate` | nobody — no Desktop client is attached to that process; the call is a logged no-op |
...
Migration (rss-reader): drop the `~/.nunmai/rss-reader/commands.jsonl` queue,
...
(`nunmai.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `NunmaiPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@nunmai/plugin-sdk` alias.
2. It's still lint-fenced to `@nunmai/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
`ctx.rest`, storage, `navigate`) and the `window.nunmaiDesktop` native bridge
...
the loader's import allowlist (`@nunmai/plugin-sdk` and `react*` only; a static
...
- **Only three specifiers resolve:** `@nunmai/plugin-sdk`, `react`,
...
| Plugin contract | `NunmaiPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `NunmaiOpenTarget`, `Contribution` |
...
### Agents: the `nunmai-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`nunmai-desktop-plugins`** skill — it carries the same contract as this page in
...
`$NUNMAI_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `nunmai logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@nunmai/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.nunmai/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.nunmai/logs/errors.log` for
