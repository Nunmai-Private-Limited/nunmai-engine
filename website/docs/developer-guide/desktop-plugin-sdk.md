...
All three take the same `NunmaiPlugin` contract, appear in **Settings → Plugins**,
...
by default, or `~/.nunmai/profiles/<name>/...` under a named profile). The folder
...
desktop app also scans `$NUNMAI_HOME/plugins/<id>/` — the regular agent-plugin
