...
description: "How Nunmai builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Nunmai deliberately separates:
...
2. **context** — caller-supplied `system_message`, project context files (`.nunmai.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# Nunmai runtime environment` boundary, with a
...
```
# Layer 1: Agent Identity (from ~/.nunmai/SOUL.md)
You are Nunmai, an AI assistant created by Nunmai Research.
...
The platform hint (Layer 10 above) is the per-surface guidance Nunmai
...
`SOUL.md` lives at `~/.nunmai/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_nunmai_home() / "SOUL.md"
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_nunmai_md(cwd_path)       # 1. .nunmai.md / NUNMAI.md (walks to git root)
    ...
    # SOUL.md from NUNMAI_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.nunmai.md`, `NUNMAI.md` | CWD up to git root | Nunmai-native project config |
| 2 | `AGENTS.md` | CWD only | Common agent instruction file |
| 3 | `CLAUDE.md` | CWD only | Claude Code compatibility |
| 4 | `.cursorrules`, `.cursor/rules/*.mdc` | CWD only | Cursor compatibility |

All context files are:
- **Security scanned** — checked for prompt injection patterns (invisible unicode, "ignore previous instructions", credential exfiltration attempts). A hit replaces a project file with a `[BLOCKED: …]` marker; the user's own `SOUL.md` in `NUNMAI_HOME` is warned about and loaded anyway (it is human-approved on write, so it is the same trust class as `config.yaml`)
...
- **YAML frontmatter stripped** — `.nunmai.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Nunmai concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.nunmai.md` / `NUNMAI.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Nunmai already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.nunmai/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.nunmai/MEMORY.md` and `~/.nunmai/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.nunmai.md`, `NUNMAI.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Nunmai.
- Ephemeral overlays such as `NUNMAI_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Nunmai assembles prompts for everyone, change Python and treat it as a code contribution
