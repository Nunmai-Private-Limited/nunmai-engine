...
description: "How Nunmai recovers state.db when the FTS index or the file itself is corrupt"
---
...
`nunmai doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `nunmai doctor --fix` while the gateway
is still up. Next steps:

```bash
nunmai gateway stop
NUNMAI_HOME="$HOME/.nunmai" nunmai sessions recover --source "$HOME/.nunmai/state.db" --inspect-only
# if recoverable:
NUNMAI_HOME="$HOME/.nunmai" nunmai sessions recover --source "$HOME/.nunmai/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
nunmai gateway stop
NUNMAI_HOME="$HOME/.nunmai" nunmai sessions repair --check-only
NUNMAI_HOME="$HOME/.nunmai" nunmai sessions repair
```
