...
description: "Wire contract between the Nunmai gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Nunmai gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `nunmai gateway enroll --wake-url <url>` (or
...
the inbound (`scope_id`, `user_id`) and, on a multiplexed gateway, the Nunmai
...
**per-gateway secret** (`nunmai gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `nunmai` CLI.
