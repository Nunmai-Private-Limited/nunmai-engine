...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Nunmai already has applies. The plugin never sees a
...
nunmai-engine itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Nunmai provider id (e.g. "openrouter")
...
using Nunmai built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Nunmai provider
        # (must be one Nunmai already knows about — same names as
        # `nunmai model` and config.yaml model.provider).
        ...
        # plugin sends — Nunmai does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.nunmai/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Nunmai' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Nunmai subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/tests/agent/test_plugin_llm.py)
