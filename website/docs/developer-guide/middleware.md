...
Nunmai middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Nunmai'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Nunmai calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Nunmai retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Nunmai guardrails,
...
- `telemetry_schema_version`: currently `nunmai.observer.v1`
- `middleware_schema_version`: currently `nunmai.middleware.v1`
...
Nunmai stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Nunmai runs
them as a nested chain in registration order. Middleware failures are fail-open:
Nunmai logs a warning and continues with the next middleware or the base
...
For each provider request, Nunmai applies middleware in this order:
...
For each tool call, Nunmai applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Nunmai pre-execution path against the effective arguments:
...
For isolated local testing, use one `NUNMAI_HOME` for plugin enablement and the
agent run:

```bash
export NUNMAI_HOME=$HOME/.nunmai/cache/scratch/nunmai-middleware-test
mkdir -p "$NUNMAI_HOME"
nunmai plugins enable <plugin-name>
nunmai chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run nunmai plugins enable <plugin-name>
uv run nunmai chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["nunmai_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", str(Path.home() / ".nunmai" / "cache" / "scratch" / "nunmai-middleware-demo"))
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Nunmai expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Nunmai treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Nunmai preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Nunmai does not convert downstream
