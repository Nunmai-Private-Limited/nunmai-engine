...
- **Profile-safe paths**: Never hardcode `~/.nunmai` — use `get_nunmai_home()` from `nunmai_constants` for code paths and `display_nunmai_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
