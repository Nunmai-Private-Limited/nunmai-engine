...
Chronos lets a hosted Nunmai gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Nunmai access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nunmai Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `nunmai-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> minted under the bootstrap-only client `nunmai-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Nunmai access token>`. NAS validates via
...
1. **Dashboard app** (`nunmai_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `NUNMAI_HOME` — exactly one replica runs each
  fire.
...
If `callback_url` / `portal_url` is blank or the agent has no Nunmai login,
...
config-only, so it cannot tell whether the stored Nunmai token is the identity NAS
maps to a provisioned instance (hop 1 above). When `provision` answers 403
`invalid_client` — the token in `auth.json` is a plain `nunmai-cli` user login
rather than the `nunmai-cli-vps` bootstrap session or an `agent:*` client — the
rejection is deterministic for the life of that credential: every arm, re-arm
and `list` would fail the same way, and a `nunmai auth` re-login makes it
