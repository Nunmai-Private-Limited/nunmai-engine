...
by `nunmai_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `nunmai gateway migrate` preflight and keeps the gateway standalone when a
...
- At startup, `GatewayRunner.__init__` calls
  ...
  `get_secret()`.
- The dashboard/Desktop backend (`nunmai serve`) has no such flag, so
  `nunmai_cli/web_server.py::start_server` calls
...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_nunmai_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`NUNMAI_HOME`, `NUNMAI_PROFILE`, proxy settings,
...
reload helper: `nunmai_cli.env_loader.load_nunmai_dotenv` skips the
...
## The NUNMAI_HOME override

`nunmai_constants.py` holds a context-local override consulted by
`get_nunmai_home()` before the `NUNMAI_HOME` env var. Everything that resolves
...
automatically. `get_process_nunmai_home()` exists for the few machine-level
assets that must not follow the override. `nunmai_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `nunmai_cli/profiles.py`
...
DB handles are resolved at call time through the active NUNMAI_HOME override —
...
NUNMAI_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `nunmai_home_key()`. |
...
- `agent/secret_scope.py`, `nunmai_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `nunmai_cli/profiles.py`
