...
Nunmai therefore calls `engine.clone_for_agent()` on the registered instance at each agent init.
...
Users select your engine via `nunmai plugins` → Provider Plugins → Context Engine, or by editing `config.yaml`:
...
When `compression.context_timeout_seconds > 0` (the default), Nunmai runs the
