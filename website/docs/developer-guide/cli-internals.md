...
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `nunmai_cli/main.py` sets
`NUNMAI_HOME` before any module imports, so every `get_nunmai_home()` reference scopes to the active
