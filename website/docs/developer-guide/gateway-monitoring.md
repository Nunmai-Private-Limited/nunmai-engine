...
Nunmai gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Nunmai's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `nunmai.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `nunmai.platform.up/degraded` with bounded `error_code` attributes |
...
`nunmai.gateway.active_agents`, `nunmai.gateway.background_work`, and
`nunmai.gateway.background_delegations` are complementary. `active_agents`
...
```bash
nunmai monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'nunmai-engine[otlp]'`),
...
a plain `nunmai gateway run`: the exporter lives in the gateway process, so
...
`nunmai.gateway.up`, `nunmai.platform.up`, and `nunmai.platform.degraded`.
...
absent_over_time(nunmai_gateway_up[5m])

# Locally owned bridge is explicitly down.
nunmai_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
nunmai_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
nunmai_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
nunmai_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(nunmai_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `nunmai.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
nunmai.status = failed|unknown
nunmai.delivery_outcome = failed|not_configured
nunmai.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `nunmai.job_key`;
...
Nunmai Engine-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log ~/.nunmai/cache/scratch/nunmai_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log ~/.nunmai/cache/scratch/nunmai_otel_capture.jsonl --wait 8
...
`nunmai.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
nunmai monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
  --log ~/.nunmai/cache/scratch/cap.jsonl --wait 8          # drive the real exporter
```
...
The `nunmai monitoring` CLI intentionally exposes `status` only. This first
release covers only Nunmai Engine-owned service-health and operational-diagnostic
signals, including Nunmai Engine-owned Relay transport health. Team Gateway's
