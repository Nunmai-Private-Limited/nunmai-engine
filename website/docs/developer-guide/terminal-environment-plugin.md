...
Nunmai runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.nunmai/plugins/`, registering a backend the user selects
...
| `nunmai setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `nunmai status` / `nunmai doctor` | `doctor_checks()` |
...
```python title="~/.nunmai/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.nunmai"    # where synced cache files land, or None
...
```yaml title="~/.nunmai/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
nunmai plugins enable acmebox
nunmai config set terminal.backend acmebox
```
