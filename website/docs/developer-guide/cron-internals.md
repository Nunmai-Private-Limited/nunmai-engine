...
description: "How Nunmai stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nunmai_cli/cron.py` | CLI `nunmai cron` subcommands |
...
Jobs are stored in `~/.nunmai/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`nunmai cron list`/`doctor`, the `cronjob_manage` tool, the web dashboard
...
The same store fields drive every topology: a standalone `nunmai -p X gateway
...
after `nunmai update` with a single gateway — never counts as a fresh gateway, so the ticker keeps
dispatching instead of yielding every tick to nobody.

In CLI mode, cron jobs only fire when `nunmai cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks Nunmai infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Nunmai
...
The flow (the managed scheduler is provided by Nunmai; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks Nunmai to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Nunmai token)
  → at fire time Nunmai calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Nunmai-minted JWT)
...
Config (all non-secret; on hosted agents Nunmai sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | Nunmai base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into Nunmai,
...
agent↔Nunmai wire contract lives in [Chronos managed-cron contract](chronos-managed-cron-contract.md).
...
```python
# ~/.nunmai/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.nunmai/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nunmai cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

### Stale-code yield

Before the tick lock, a gateway whose checkout was updated under it (boot revision ≠ disk revision) yields the tick when another process holds the gateway runtime lock — a fresher gateway's ticker dispatches instead, and the stale one must not race it with mixed `sys.modules`. The yield is raised (`CronTickYielded`) and persisted as the ticker's last error, so `nunmai cron status` reports **"Gateway is running STALE code — its cron ticker yields every tick and fires NOTHING"** with both revisions and the restart command, even though the liveness heartbeat keeps refreshing. `nunmai update` closes the loop: a gateway the post-update fleet version matrix proves stale is handed to the drain-first `request_restart` path (SIGUSR1) instead of being left running; a supervised gateway respawns on the new code, a bare `gateway run` is stopped and listed under "Restart manually".

## CLI Interface

The `nunmai cron` CLI provides direct job management:

```bash
nunmai cron list                    # Show all jobs
nunmai cron create                  # Interactive job creation (alias: add)
nunmai cron edit <job_id>           # Edit job configuration
nunmai cron pause <job_id>          # Pause a running job
nunmai cron resume <job_id>         # Resume a paused job
nunmai cron run <job_id>            # Trigger immediate execution
nunmai cron remove <job_id>         # Delete a job
```
