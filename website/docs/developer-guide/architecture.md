...
sidebar_position: 1
title: "Architecture"
description: "Nunmai Engine internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Nunmai Engine internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nunmai_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NunmaiCLI facade — mixins in nunmai_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nunmai_state.py           # SQLite session/state database facade (+ nunmai_state_*.py siblings)
├── nunmai_constants.py       # NUNMAI_HOME, profile-aware paths
...
│
├── nunmai_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `nunmai` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # nunmai skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nunmai tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nunmai gateway start/stop
│
...
```text
User input → NunmaiCLI.process_input()
...
Three discovery sources: `~/.nunmai/plugins/` (user), `.nunmai/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nunmai plugins` or `config.yaml`.
...
Exposes Nunmai as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nunmai -p <name>`) gets its own NUNMAI_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
