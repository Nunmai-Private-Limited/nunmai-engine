...
description: "How to build a memory provider plugin for Nunmai Engine"
---

# Building a Memory Provider Plugin

Memory provider plugins give Nunmai Engine persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](./context-engine-plugin.md), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `nunmai plugins`.
:::

## Installation Layouts

Nunmai discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Nunmai. Closed to new providers — see [CONTRIBUTING](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/CONTRIBUTING.md). |
| User | `$NUNMAI_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.nunmai/plugins/<name>/` | Opt-in via `NUNMAI_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `nunmai_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Nunmai, in `$NUNMAI_HOME/plugins/<name>/` when installed by a user, or in
`./.nunmai/plugins/<name>/` for a project-local one:
...
`nunmai_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."nunmai_agent.memory_providers"]
...
package layout. No copy under `$NUNMAI_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Nunmai reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `nunmai <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          nunmai_home (str): Active NUNMAI_HOME path. Use for storage.
        """
...
| Keyword | Meaning |
|---|---|
| `nunmai_home` | Active profile's storage directory. |
...
| `session_title_source` | Stored title provenance, when available: `derived`, `llm`, or `user`. Automatic sources must not be mistaken for explicit identity overrides; missing provenance retains a provider's legacy behavior. Shared constants live in `nunmai_state_common.py`. |
...
| `agent_workspace`, `agent_context` | Runtime agent scope. `agent_workspace` is `nunmai`; `agent_context` is `cron` for scheduler runs, `subagent` for `delegate_task` children, else `primary` — skip automatic writes for the non-primary values. |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `nunmai memory setup` | **Yes** |
| `save_config(values, nunmai_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
Older Nunmai versions can omit `previous_content`. Providers that require exact
...
None of the providers bundled with Nunmai advertise checkpoint API v2 — the
...
The gate binds to every compaction authority, not just the Nunmai
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Nunmai
...
Every setup surface Nunmai gives a bundled provider is driven by files in the provider's
...
| `nunmai memory setup` wizard | `get_config_schema()` declares the fields the wizard prompts for, `save_config(config, nunmai_home)` persists them, `post_setup(nunmai_home, config)` runs afterwards for anything interactive (OAuth, first sync); `get_status_config()` feeds `nunmai memory status` |
| `nunmai <provider> …` subcommands | `cli.py` with `register_cli(subparser)` ([Adding CLI Commands](#adding-cli-commands)) |
| Python dependencies | `pyproject.toml` `[project] dependencies` (or `python_dependencies` in `plugin.yaml`); installed under Nunmai' own pins at install time and re-applied across `nunmai update` |

Your provider's name, `memory.<name>` config section, data directory and tool names are the
contract with existing users. A provider that moves out of core keeps all four; Nunmai then
installs the catalog plugin automatically for anyone whose `memory.provider` still names it
(on `nunmai update`, and once at agent start when `security.allow_lazy_installs` is on).

## Config Schema

`get_config_schema()` returns a list of field descriptors used by `nunmai memory setup`:
...
Every field in `get_config_schema()` is prompted during `nunmai memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$NUNMAI_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, nunmai_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(nunmai_home) / "my-provider.json"
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `NUNMAI_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$NUNMAI_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `nunmai_cli.config.save_config(..., merge_existing=True)`.
...
the `messages` parameter; Nunmai will continue calling them with the legacy
signature.
...
All storage paths **must** use the `nunmai_home` kwarg from `initialize()`, not hardcoded `~/.nunmai`:

```python
# CORRECT — profile-scoped
from nunmai_constants import get_nunmai_home
data_dir = get_nunmai_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.nunmai/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `nunmai my-provider status`, `nunmai my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `nunmai <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `nunmai --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: nunmai my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the nunmai my-provider argparse tree.
