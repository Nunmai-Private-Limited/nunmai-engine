...
title: "Build a Nunmai Plugin"
description: "Step-by-step guide to building a complete Nunmai plugin with tools, hooks, data files, and skills"
---

# Build a Nunmai Plugin

This guide walks through building a complete Nunmai plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Nunmai has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](../../user-guide/features/tts.md#voice-message-transcription-stt) — set `NUNMAI_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](../../user-guide/features/hooks.md#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.nunmai/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](../../user-guide/features/skills.md) — `nunmai skills tap add <repo>` · [Publishing a tap](../../user-guide/features/skills.md#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Nunmai-Private-Limited/nunmai-engine`. Users install them into `~/.nunmai/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nunmai Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Nunmai can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Nunmai already owns. It does not replace native `plugin.yaml` plus
...
```bash
nunmai plugins install owner/repository --no-enable
nunmai plugins list
nunmai plugins enable <plugin-name>
```
...
Nunmai validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Nunmai.
...
Portable `streamable-http` entries are routed through Nunmai' existing native
...
same full-trust posture as other installed Nunmai plugins.
...
records it as Published. Nunmai keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Nunmai does not expose a
...
continue to work after a normal Nunmai upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Nunmai inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Nunmai releases can therefore load a plugin whose manifest contains
...
Nunmai enforces this contract with frozen external-plugin fixtures discovered
from an isolated `NUNMAI_HOME`. Those tests load and invoke the plugin through
...
Nunmai's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `nunmai plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `nunmai doctor` and after `nunmai update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `NunmaiPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `nunmai plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.nunmai/plugins/calculator
cd ~/.nunmai/plugins/calculator
```

### Validate with Plugin Doctor

`nunmai plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Nunmai itself. It reports invalid hook names, callbacks that do
...
```bash
nunmai plugins doctor . --ci
```

Doctor uses a temporary `NUNMAI_HOME`, restores plugin registration state after
...
This tells Nunmai: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `nunmai_cli/plugin_capabilities.py` for the
...
`nunmai_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`nunmai_agent.plugins` entry point:

```toml
[project.entry-points."nunmai_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."nunmai_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Nunmai reads these from installed metadata without importing your code, so
`nunmai plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Nunmai understands still loads with a warning.
...
| `python_dependencies` | list of str | PEP 508 requirements (e.g. `"requests>=2.0,<3"`). Installed into Nunmai' venv on `nunmai plugins install` / `enable` and **re-applied after every `nunmai update`** (see [Python dependencies](#python-dependencies)). A `pyproject.toml` beside `plugin.yaml` with `[project].dependencies` is the equivalent, preferred form. |
| `python_runtime` | str | `external` — the plugin manages its own interpreter/venv (sidecar pattern); Nunmai installs nothing and leaves any `pyproject.toml` alone. |
...
requires_plugins:
  - id: other-plugin
    version_range: ">=1.0,<2"
python_dependencies:
  - "somepkg>=1.0,<2"     # installed on install/enable, re-applied after nunmai update
config_schema:
...
When both exist the `pyproject.toml` wins. What Nunmai does with them:

- **Install / enable** — the declared packages are installed into Nunmai' venv with
  `uv pip install` (pip fallback) under a **constraints file built from Nunmai' own pinned
  dependencies**, so a plugin can never move a core package (httpx, pydantic, …) off the version
  Nunmai was tested with. Environment markers (`; sys_platform == "win32"`) are honoured.
...
- **`nunmai update` re-applies them** — the update's `uv sync` rebuilds the venv from Nunmai' lock
  and strips anything else. Afterwards Nunmai walks every profile's enabled plugins and reinstalls
  ...
  message** naming it, and memory providers are kept over everything else, because a Nunmai that
  boots without memory looks like data loss.
- **`nunmai plugins update`** re-runs the install for whatever the new revision declares.
- **`--no-deps`** on `nunmai plugins install` skips all of this for one plugin (no conflict gate,
...
- **Opt out with `python_runtime: external`** — plugins that keep a heavy runtime (torch, native
  ...
  `plugin.yaml`; Nunmai then installs nothing and the plugin never joins the shared resolution.
- **Nothing to load is an error** — `nunmai plugins validate` (and the catalog CI) fail a
...
`NUNMAI_HOME/plugins/` survives `nunmai update` and Desktop updates: the updater only rebuilds the
venv and the checkout, never the home directory.

### Dependency security policy

Nunmai quarantines **its own** dependencies: the checkout's `[tool.uv] exclude-newer = "14 days"`
keeps a freshly published release of any package Nunmai itself depends on out of `nunmai update`
...
run outside Nunmai's project policy (`uv pip install --no-config`, still under the core constraints
file above), so a plugin can floor on a release published yesterday and install today — and the
plugin's author, not Nunmai, is responsible for what that pulls in.
...
- **Adopt a new-release quarantine of your own** — wait ~14 days before floors move to a new
  ...
  can set `UV_EXCLUDE_NEWER` in Nunmai's environment; it applies to every install Nunmai runs.
...
4. **Accept `**kwargs`:** Nunmai injects context keywords (`task_id`, `session_id`, `user_task`,
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `nunmai my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Nunmai continues fine
...
Use plugin-relative config keys for user-visible behavior. Nunmai resolves them
...
`<NUNMAI_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `NUNMAI_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
NUNMAI_PLUGINS_DEBUG=1 nunmai plugins list
```
...
The same logs are always written to `~/.nunmai/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
nunmai logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `nunmai plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.nunmai/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `nunmai plugins update` / `remove` git-pull or delete it — your
...
# <nunmai home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.nunmai/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.nunmai/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `nunmai plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Nunmai into installing arbitrary packages — only specs Nunmai itself ships are eligible |
...
auditable in `~/.nunmai/logs/agent.log`. Plugins load after built-in
...
does not ship with Nunmai core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `nunmai plugins list`
...
loader, the plugin is disabled and Nunmai continues. The gate exists
...
You normally never edit this key by hand. `nunmai plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Nunmai injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: nunmai-engine"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$NUNMAI_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $NUNMAI_HOME/hook_outputs
```
...
- **The system prompt is Nunmai's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `nunmai_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Nunmai sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`nunmai.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Nunmai still loads on an older one.

### Register CLI commands

Plugins can add their own `nunmai <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for nunmai my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: nunmai my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for nunmai my-plugin."""
...
After registration, users can run `nunmai my-plugin status`, `nunmai my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `nunmai name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `nunmai chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `NUNMAI_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `nunmai <subcommand>` (e.g. `nunmai kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "nunmai kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Nunmai from a hook.
...
A plugin can load while the gateway (or the TUI/Desktop server) is already running: `nunmai plugins
...
(`nunmai_cli/plugins_activation.py`):
...
Install surfaces report exactly this split: `nunmai plugins install/enable` prints it after nudging the running
...
Nunmai has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.nunmai/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `nunmai gateway status` without SDK instantiation.
...
Nunmai also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](../../user-guide/features/plugins.md#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Nunmai without any Python plugin. Declare them in `~/.nunmai/config.yaml`:
...
Nunmai connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](../../user-guide/features/mcp.md).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.nunmai/hooks/<name>/`. Unlike plugins there is no `plugins.enabled` step: the gateway imports every valid hook directory at startup, so placing the files **is** the opt-in ([trust model](../../user-guide/features/hooks.md#gateway-hook-trust)):

```yaml
# ~/.nunmai/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.nunmai/hooks/long-task-alert/handler.py
...
nunmai skills search my-workflow --source myorg/skills-repo
nunmai skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install nunmai-plugin-calculator
# Plugin auto-discovered on next nunmai startup
```
