...
Default path: `~/.nunmai/state.db`

This is derived from `nunmai_constants.get_nunmai_home()` which resolves to
`~/.nunmai/` by default, or the value of `NUNMAI_HOME` environment variable.
