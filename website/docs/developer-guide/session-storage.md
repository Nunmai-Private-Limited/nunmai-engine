...
Abridged — see `SCHEMA_SQL` in `nunmai_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `nunmai_state.py` for the exact SQL.
