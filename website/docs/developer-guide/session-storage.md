...
Nunmai Engine uses a SQLite database (`~/.nunmai/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `nunmai_state.py` (facade) plus the `nunmai_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## Nunmai home and profile isolation

`get_nunmai_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `NUNMAI_HOME`
environment variable, and finally the platform default (`~/.nunmai` on macOS
and Linux; `%LOCALAPPDATA%/nunmai` on Windows). Consequently, the default
database is always `get_nunmai_home() / "state.db"`, not a path that callers
should hard-code as `~/.nunmai/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default Nunmai root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `NUNMAI_HOME`; falling back to the
...
of Nunmai. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `NUNMAI_HOME`. Without an explicit selector, a
profile-specific `NUNMAI_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `NUNMAI_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `NUNMAI_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`NUNMAI_HOME` explicitly.

Use `display_nunmai_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.nunmai/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `NUNMAI_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default Nunmai root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`NUNMAI_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal Nunmai commands, development shells, or application
...
Each named profile stores its transcript in its own `$NUNMAI_HOME/state.db`,
including when one `nunmai serve` process serves several profiles. In-session
...
splice boundary, Nunmai excludes only that leading item when it exactly matches
...
```
~/.nunmai/state.db (SQLite, WAL mode)
...
`nunmai sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `nunmai_state_common.py` (applied by `nunmai_state_schema.py`) for the full current column list
...
an authenticated `nunmai serve` (OAuth or the basic username/password provider)
...
- `reasoning_details` is always kept in history; on the chat-completions wire it is replayed only to OpenRouter and the Nunmai Portal (every other chat-completions route gets a copy without it, since strict schemas reject the field)
...
indexed columns — see `SCHEMA_SQL` in `nunmai_state_common.py` for the exact SQL.
...
Multiple nunmai processes (gateway + CLI sessions + worktree agents) share one
...
`session_persistence_failed:locked` and, on Linux, `nunmai_state_lockowners`
logs a WARNING naming the process that held the lock at that moment
(`PID 594094 (nunmai --worktree --yolo) holds WAL write lock on state.db-shm`),
...
because every Nunmai process has the DB open. Look for that line in
`~/.nunmai/logs/errors.log` next to the `database is locked` failure.

Lock contention is recognised by SQLite result code (`SQLITE_BUSY` /
`SQLITE_LOCKED`, `nunmai_state_errors.is_sqlite_lock_error`), not by message
...
db = SessionDB()                           # Default: ~/.nunmai/state.db
db = SessionDB(db_path=Path("~/.nunmai/cache/scratch/test.db").expanduser())  # Custom path
```
...
Default path: `get_nunmai_home() / "state.db"` — `~/.nunmai/state.db` for the
default profile, `~/.nunmai/profiles/<name>/state.db` for a named profile, or
wherever `NUNMAI_HOME` points (see [Nunmai home and profile isolation](#nunmai-home-and-profile-isolation)).
