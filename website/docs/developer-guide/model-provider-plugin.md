...
2. **User plugins** — `$NUNMAI_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
...
Four values are recognized. Nunmai picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Nunmai' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
```bash
export NUNMAI_HOME=/tmp/nunmai-plugin-test
...
See [Building a Nunmai Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a Nunmai Plugin](/developer-guide/plugins) — general plugin authoring
