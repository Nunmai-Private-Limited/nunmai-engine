...
The ACP adapter wraps Nunmai' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nunmai acp / nunmai-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.nunmai/.env
  -> configure stderr logging
  -> construct NunmaiACPAgent
...
- `allow_once` -> Nunmai `once`
- `allow_always` -> Nunmai `always`
- reject options -> Nunmai `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Nunmai tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  ...
  -> create AIAgent(platform="acp", enabled_toolsets=<_get_platform_tools(config, "acp"), as on the
                   gateway: platform_toolsets.acp (default nunmai-acp) plus the resolver's extras
...
retries, interrupt before any reply) is closed by the core loop with a Nunmai-authored
...
Instead it reuses Nunmai' runtime resolver:

- `acp_adapter/auth.py`
- `nunmai_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Nunmai provider/credentials. It also always advertises a terminal setup auth method (`nunmai-setup`, args `--setup`) so first-run ACP clients can open Nunmai' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.nunmai/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nunmai-acp` toolset definition
- `nunmai_cli/main.py` — `nunmai acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nunmai-acp` script
