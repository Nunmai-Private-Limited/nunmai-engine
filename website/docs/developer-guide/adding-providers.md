...
description: "How to add a new inference provider to Nunmai Engine — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Nunmai can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `nunmai model` menu entries
...
1. `nunmai_cli/auth.py` decides how credentials are found.
2. `nunmai_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `nunmai_cli/models.py` and `nunmai_cli/main.py` make the provider show up in the CLI. (`nunmai_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `nunmai_cli/auth.py`
2. `nunmai_cli/models.py`
3. `nunmai_cli/runtime_provider.py`
4. `nunmai_cli/main.py`
...
:::tip
`nunmai_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `nunmai setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$NUNMAI_HOME/plugins/model-providers/` both get picked up.
...
7. `nunmai model` menu includes the provider
8. `nunmai setup` wizard delegates to `main.py` automatically
...
User plugins at `$NUNMAI_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Nunmai Portal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `nunmai model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `nunmai_cli/auth.py`
- `_PROVIDER_LABELS` in `nunmai_cli/models_catalog_static.py` (re-exported by `nunmai_cli/models.py`)
- `_PROVIDER_ALIASES` in both `nunmai_cli/auth.py` and `nunmai_cli/models_catalog_static.py`
- CLI `--provider` choices in `nunmai_cli/main.py`
...
## Step 2: Add auth metadata in `nunmai_cli/auth.py`
...
- OAuth / auth-store path: Nunmai, OpenAI Codex

Questions to answer here:

- What env vars should Nunmai check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `nunmai_cli/models.py`
...
## Step 4: Resolve runtime data in `nunmai_cli/runtime_provider.py`
...
Be careful with API-key precedence. Nunmai already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `nunmai_cli/main.py`

A provider is not discoverable until it shows up in the interactive `nunmai model` flow.

Update these in `nunmai_cli/main.py`:
...
:::tip
`nunmai_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `nunmai model` and `nunmai setup` automatically.
:::
...
When you add a native provider, double-check that Nunmai is only sending fields that provider actually understands.
...
- `tests/nunmai_cli/test_runtime_provider_resolution.py`
- `tests/nunmai_cli/test_cli_provider_resolution.py`
- `tests/nunmai_cli/test_model_switch_custom_providers.py` (and adjacent `tests/nunmai_cli/test_model_switch_*.py`)
- `tests/nunmai_cli/test_setup_model_provider.py`
...
python -m pytest tests/nunmai_cli/test_runtime_provider_resolution.py tests/nunmai_cli/test_cli_provider_resolution.py tests/nunmai_cli/test_setup_model_provider.py tests/agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m nunmai_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m nunmai_cli.main model
...
- [ ] `ProviderConfig` added in `nunmai_cli/auth.py`
- [ ] aliases added in `nunmai_cli/auth.py` and `nunmai_cli/models.py`
- [ ] model catalog added in `nunmai_cli/models.py`
- [ ] runtime branch added in `nunmai_cli/runtime_provider.py`
- [ ] CLI wiring added in `nunmai_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `nunmai model` but not `nunmai setup`
