...
description: "Build wrapper CLIs that extend the Nunmai TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Nunmai exposes protected extension hooks on `NunmaiCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `nunmai_cli/cli_tui_mixin.py` (where these hooks are defined; `NunmaiCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Nunmai."""
...
Called after Nunmai registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
