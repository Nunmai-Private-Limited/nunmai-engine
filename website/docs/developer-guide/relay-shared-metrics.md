...
Nunmai includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Nunmai and does not require a Nunmai observability plugin. Nunmai remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Nunmai execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `nunmai-engine[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Nunmai `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `NUNMAI_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `NUNMAI_NEMO_RELAY_ATOF_*` and `NUNMAI_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Nunmai does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Nunmai requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Nunmai
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Nunmai installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Nunmai process so configured interceptors can
...
Collection remains off unless Nunmai policy enables it:
...
opt-in. Set `NUNMAI_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Nunmai does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Nunmai reports the error and does not invoke
...
retain the same `session_id` and add `nunmai.session.segment` plus
`nunmai.session.segment_reason` (`compaction` or `max_turns`).

## Working-Directory Scope Data

When Nunmai knows a session or task's logical working directory, its
`nunmai.session` and `nunmai.turn` start scopes include it as `data.cwd` in
...
Relay plugin configuration is a process-level deployment choice, not a Nunmai
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Nunmai process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Nunmai logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Nunmai
process and share the same policy boundary.

Run profiles in separate Nunmai processes when they require different trust
...
Nunmai core owns one Relay host and one isolated Relay session scope per Nunmai
...
marks do not require Nunmai plugin registration. Shared-metrics marks must
...
```text
Nunmai turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Nunmai shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Nunmai sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Nunmai used for the logical call, such as
...
New calls use `nunmai.model_route.count`. The previous
`nunmai.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `nunmai.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Nunmai
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Nunmai processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `nunmai.task_run`, parented to
the owning Nunmai session. The start counter contains only bounded execution
...
provider attempts for the same Nunmai API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Nunmai tool-call
...
cancellations. Active task ownership follows the task ID if Nunmai rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`nunmai.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Nunmai
...
tool or plugin names. Nunmai does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `nunmai.tool_approval` marks and
...
Successful skill mutations emit `nunmai.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `nunmai.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Nunmai derives reuse and patch-generation
...
```text
$NUNMAI_HOME/telemetry/shared_metrics/metrics.sqlite3
$NUNMAI_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Nunmai version,
...
schema field name, its current scope is one `NUNMAI_HOME`, so it is more
...
Deleting `$NUNMAI_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `NUNMAI_HOME`. To reset it, stop Nunmai
processes and remove `$NUNMAI_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Nunmai CLI turn against the deterministic local model server:
...
`$NUNMAI_HOME/telemetry/shared_metrics/outbox/` to the Nunmai telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the Nunmai telemetry service
```
...
Removing `$NUNMAI_HOME/telemetry/shared_metrics` still resets local identity,
