...
| `requires_env` | Interactive credential prompt during `nunmai plugins install` (see [Build a Nunmai Plugin](./plugins/index.md#gate-on-environment-variables) for the rich format) |
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Nunmai will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Nunmai Plugin → Lazy-install](./plugins/index.md#lazy-install-optional-python-dependencies) for the security model.
