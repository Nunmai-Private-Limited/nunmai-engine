...
Nunmai Engine uses a dual compression system and Anthropic prompt caching to
...
- **Failed probes use the current table without persisting it.** Failures have a
  five-minute in-memory cooldown scoped to Nunmai home, endpoint, model, and
  region. Expiry or explicit cache invalidation permits another attempt.

The cache remains at `context_length_cache.yaml` under the active Nunmai home.
...
Configure via `nunmai plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](./context-engine-plugin.md).

## Dual Compression System

Nunmai has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.nunmai/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|nunmai|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `nunmai`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
- Keys may be **provider-scoped** as `"<provider>:<substring>"` (e.g.
  ...
  elsewhere (OpenRouter, Nunmai, direct OpenAI) keeps the global `threshold`.
...
excluded because they already unlock the wider window), Nunmai raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$NUNMAI_HOME`
...
```bash
nunmai config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
nunmai config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Nunmai keeps the
...
`gpt-5.6-terra-900k`, `gpt-5.6-luna-900k`, `gpt-5.4-900k`). These are Nunmai-side aliases: the suffix is stripped before
...
thread context, so Nunmai' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Nunmai
  records the resulting compaction events (compression counters, session
  events). Set `nunmai` to let Nunmai' compression threshold initiate
  app-server compaction, or `off` to disable Nunmai-initiated automatic
  compaction entirely (codex may still compact natively).

Nunmai' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Nunmai' summary compressor.
...
encrypted `compaction` output item. Nunmai captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Nunmai
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Nunmai can avoid; user-facing docs for `/model`, fallback providers, and
