...
hgui() {
  ...
  root="$(_nunmai_root)" || { echo "hgui: not in a Nunmai checkout" >&2; return 1; }
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _nunmai_link_deps "$desktop" "$deps/apps/desktop"
    _nunmai_link_deps "$root" "$deps"
  else
  ...
  trap '_nunmai_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    NUNMAI_DESKTOP_NUNMAI_ROOT="$root" \
    NUNMAI_DESKTOP_PYTHON="$NUNMAI_MAIN_CHECKOUT/.venv/bin/python" \
    NUNMAI_DESKTOP_IGNORE_EXISTING=1 \
    NUNMAI_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `NUNMAI_DESKTOP_NUNMAI_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `nunmai`. |
| `NUNMAI_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `NUNMAI_DESKTOP_IGNORE_EXISTING` | Ignores any `nunmai` on `PATH` so it can't shadow the worktree. |
| `NUNMAI_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
_nunmai_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'nunmai_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
