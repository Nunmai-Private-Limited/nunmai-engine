...
| `rewind_session(session_id, n=1)` | Back up `n` user turns via soft-delete (keeps audit trail); thin wrapper over `SessionDB.rewind_user_turn` (`nunmai_state_rewind.py`), the one rewind shared with CLI `/undo`/`/retry` and the TUI. Returns `{rewound_count, turns_undone, target_text}`. |
...
The canonical transcript store is SQLite via `SessionDB` (from `nunmai_state`). The
...
Counts consecutive restarts via a JSON file (`{NUNMAI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `nunmai update`, `nunmai gateway restart`,
or `/restart`.
