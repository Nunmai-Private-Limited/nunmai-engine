...
description: "How Nunmai detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Nunmai' browser tooling:
...
(`nunmai-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Nunmai `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `nunmai-acp`, `nunmai-api-server`, and core toolsets (gated on CDP reachability)
- `nunmai_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
