...
Nunmai Engine saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Nunmai function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `nunmai` CLI
