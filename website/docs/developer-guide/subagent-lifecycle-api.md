...
Plugins can launch and supervise fresh Nunmai child sessions without importing
...
active agent turn fails closed with `No active Nunmai parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Nunmai can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Nunmai's existing unsafe-tool block remains enforced.
