...
- Using the **OpenAI-compatible API server** (exposed by `nunmai gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](./features/api-server.md).
...
The Nunmai [Tool Gateway](./features/tool-gateway.md) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.
...
- **[MCP guide → WSL → Windows Chrome](../guides/use-mcp-with-nunmai.md#wsl2-bridge-nunmai-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Nunmai in WSL.
