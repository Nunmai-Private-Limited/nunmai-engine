...
description: "What to do when Nunmai says another process holds an old copy of the session database's write-ahead log, and what the files beside state.db are"
---

# Session storage recovery

Nunmai keeps every conversation in one SQLite file per profile, `state.db`, with two
sidecar files SQLite manages itself: `state.db-wal` (the write-ahead log) and `state.db-shm`.
Several Nunmai processes can share that file safely — the gateway, the Desktop app, the
...
> another Nunmai process still holds an old copy of the session database's write-ahead log,
> so Nunmai stopped writing to keep the file safe …
...
1. **Quit every Nunmai process on that profile.** Desktop app, gateway, dashboard, cron:

   ```bash
   nunmai gateway stop          # add -p <profile> for a named profile
   ```

   then quit the Desktop app from its menu and stop any dashboard (`nunmai dashboard --stop`)
...
2. **Ask doctor who is still holding the log.**
   ...
   ```bash
   nunmai doctor                # add -p <profile> for a named profile
   ```
...
3. **Start Nunmai again** (one process first — the gateway or the Desktop app) and send your
   message once more. Your conversation resumes from where it stopped.

## Do not

- **Do not run `nunmai doctor --fix` while the processes are running.** Doctor refuses the
...
- **Do not copy `state.db` alone.** The three files are one image. Use a snapshot
  (`nunmai backup`) or `nunmai sessions recover`, never `cp state.db somewhere/`.
...
`nunmai sessions optimize`, `nunmai sessions optimize-storage` and `nunmai sessions prune`
...
```text
Refusing `nunmai sessions optimize-storage`: another process is using ~/.nunmai/state.db.
  PID 41230 (nunmai gateway run): state.db, state.db-shm, state.db-wal
  PID 41355 (nunmai serve --profile work): state.db-wal
...
Stop them first (`nunmai gateway stop`, quit the Desktop app, pause cron), then re-run.
...
| `state.db.retired-wal-<timestamp>-<pid>/` | A capture Nunmai made of the log copy a process was still holding when it refused to write, plus a `manifest.json` describing it. Forensic evidence, not a backup you restore blindly. | Keep it. If conversations from just before the incident are missing after recovery, attach the directory to a bug report; a maintainer can tell from `manifest.json` whether those frames belong on top of the current file. |
| `state.db.pre-update-emergency-<timestamp>.bak` | A snapshot the Desktop updater takes before it touches the store. | Keep it until you have used the updated app for a while. Restore only with every Nunmai process stopped: `nunmai sessions recover --source <file> --inspect-only` first. |
| `state.db.corrupt.<timestamp>.bak`, `*.malformed-backup` | Copies of a file Nunmai found damaged before it repaired or quarantined it. | Do not restore them over `state.db` — they are the same damage. Keep for a report; safe to delete once you are back to normal. |
| `state-snapshots/` | Quick snapshots `nunmai update` and `nunmai backup` take. | Restore with every Nunmai process stopped; see [`nunmai backup`](../reference/cli-commands.md#nunmai-backup). |

## When the three steps do not work

If every Nunmai process is stopped, `nunmai doctor` no longer lists a holder, and the
...
```bash
nunmai sessions recover --source ~/.nunmai/state.db --inspect-only
```
