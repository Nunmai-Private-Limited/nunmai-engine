...
Connect Nunmai to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Nunmai and the BlueBubbles server
...
Or set environment variables directly in `~/.nunmai/.env`:
...
By default, Nunmai responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Nunmai uses conservative defaults for `Nunmai` and `@Nunmai agent` variants.
...
When someone messages your iMessage, Nunmai automatically sends them a pairing code. Approve it with:
```bash
nunmai pairing approve bluebubbles <CODE>
```
Use `nunmai pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.nunmai/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.nunmai/.env`):
```bash
...
```
iMessage → Messages.app → BlueBubbles Server → Webhook → Nunmai
Nunmai → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Nunmai sends messages via the BlueBubbles REST API.
...
- `BLUEBUBBLES_SERVER_URL` (e.g. `http://192.168.1.10:1234`) — Nunmai **calls** your BlueBubbles server's API. This is the Server URL shown in BlueBubbles Server → Settings → API.
- The webhook (default `http://localhost:8645/bluebubbles-webhook`) — BlueBubbles **POSTs** new-message events to Nunmai. Its host/port/path come from `BLUEBUBBLES_WEBHOOK_HOST` / `BLUEBUBBLES_WEBHOOK_PORT` / `BLUEBUBBLES_WEBHOOK_PATH`.

### How the webhook is registered

You do **not** need to create a webhook in the BlueBubbles UI. When the gateway connects, Nunmai registers the webhook itself via the BlueBubbles REST API (`/api/v1/webhook`) for the `new-message` and `updated-message` events, and removes the registration again on clean shutdown.
...
- The webhook listener binds to `127.0.0.1` by default. That is fine when Nunmai and BlueBubbles run on the same machine; if they are on different machines, set `BLUEBUBBLES_WEBHOOK_HOST` to an address the Mac running BlueBubbles can reach.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Nunmai wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.nunmai/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Nunmai resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `nunmai logs gateway` for webhook errors (or `nunmai logs -f` to follow in real-time)
- Nunmai registers the webhook itself on connect — only inspect BlueBubbles Server → Settings → API → Webhooks if the log shows a registration failure
- A webhook row in the BlueBubbles UI is not proof of delivery; the end-to-end proof is Nunmai logging the message and replying
- If Nunmai and BlueBubbles run on different machines, the default webhook bind address `127.0.0.1` is unreachable from the Mac — set `BLUEBUBBLES_WEBHOOK_HOST` to a reachable address and restart the gateway

### Setup succeeded, but the adapter never starts
- `nunmai gateway setup` saves credentials to `~/.nunmai/.env`; it does not set `platforms.bluebubbles.enabled: true`
- An explicit `enabled: false` in `~/.nunmai/config.yaml` wins over credentials being present — check with `nunmai config get platforms.bluebubbles.enabled`
...
- Nunmai uses `BLUEBUBBLES_SERVER_URL` from `~/.nunmai/.env`, not the Server URL shown in the BlueBubbles UI (which can be stale after a DHCP change)
- Two macOS users on one Mac each run their own BlueBubbles server with its own API port and Apple ID — verify which one Nunmai reaches: `curl "http://<server-url>/api/v1/server/info?password=<password>"` and compare the `computer_id`
