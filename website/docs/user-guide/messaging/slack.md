...
sidebar_position: 4
title: "Slack"
description: "Set up Nunmai Engine as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Nunmai Engine to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Nunmai instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Nunmai uses the modern
...
The fastest path is to paste a manifest Nunmai generates for you. It
...
### Option A: From a Nunmai-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   nunmai slack manifest --agent-view --write
   ```
   This writes `~/.nunmai/slack-manifest.json` and prints paste-in
   ...
   ```bash
   nunmai slack manifest --agent-view \
...
4. Enter an app name (e.g., "Nunmai Engine") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Nunmai can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `nunmai-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Nunmai configuration issue.
:::
...
Nunmai uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
nunmai gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
nunmai gateway              # Foreground
nunmai gateway install      # Install as a user service
sudo nunmai gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Nunmai now
...
Every Nunmai command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Nunmai command with its description.

Under the hood: Nunmai ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/hermes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Nunmai
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Nunmai can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Nunmai only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Nunmai adds new commands (e.g. after `nunmai update`), regenerate
the manifest and update your Slack app:

```bash
nunmai slack manifest --write
```
...
3. Paste the new contents of `~/.nunmai/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/nunmai <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/nunmai bg run the tests` — Nunmai routes it the same way as `/bg
run the tests`. Free-form questions also work: `/nunmai what's the
...
Slack never delivers them to Nunmai.

As a workaround, Nunmai recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Nunmai treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@Nunmai !stop`) and with
...
Nunmai falls back to a text prompt, the prompt instructs you to reply
...
fails, Nunmai retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Nunmai /cmd` — reply as normal visible
messages instead.)
...
```bash
nunmai slack manifest --slashes-only > ~/.nunmai/cache/scratch/slashes.json
```
...
Understanding how Nunmai behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Nunmai Engine what time is it?`). In channels, Nunmai replies in a thread attached to that message. |
| **Threads** | If you @mention Nunmai inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.nunmai/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Nunmai, and "all" accepts every
      # other bot. Nunmai always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.native_task_cards` | `false` | When `true`, renders live tool calls as Slack-native plan/task cards. Cards work with Slack's built-in default `tool_progress: off`; an explicitly configured `display.tool_progress: off` (global or `display.platforms.slack`; `/verbose` writes the same key) disables cards too. Cards need a thread: when the card lane is active and the chat has no thread to anchor on (a top-level DM with `reply_in_thread: false`), Nunmai shows no tool progress instead of text bubbles, unless you explicitly set `tool_progress: new`/`all`, which falls back to editable text progress there. Recoverable native API failures fall back to one continuously edited text update. |
...
name in the thread. By default Nunmai sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Nunmai is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), Nunmai uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — Nunmai seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and Nunmai falls back
...
- Nunmai checks thread eligibility before attempting publication, so a
...
- On a supported threaded destination, if the native stream fails for a
  recoverable reason (API error, rate limit), Nunmai falls back to a single continuously edited text message so
...
- If Slack closes a stream during a long turn, Nunmai opens a fresh card in
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Nunmai in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Nunmai will respond.
...
By default Nunmai ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Nunmai instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Nunmai always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Nunmai agents are filtered consistently across workspaces.
...
`client_msg_id` — the same signature Nunmai uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to Nunmai *as*
the logged-in user.
...
manifest — regenerate with `nunmai slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Nunmai will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Nunmai Engine`).
...
Delivery works even when the cron process isn't co-located with the gateway — Nunmai falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Nunmai can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Nunmai also loads tokens from an **OAuth token file** at:

```
~/.nunmai/slack_tokens.json
```
...
- When a message arrives, Nunmai uses the correct workspace-specific client to respond.
...
Nunmai supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Nunmai Engine` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Nunmai Engine` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Nunmai now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Nunmai Engine`)
...
- Tokens should be stored in `~/.nunmai/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Nunmai config directory
