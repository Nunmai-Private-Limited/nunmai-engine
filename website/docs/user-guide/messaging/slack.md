...
```bash
nunmai slack manifest --slashes-only > /tmp/slashes.json
```
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, Nunmai keeps one editable text
...
- If the native stream can't start or update, Nunmai falls back to a single
