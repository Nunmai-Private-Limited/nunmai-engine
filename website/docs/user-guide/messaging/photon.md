...
Connect Nunmai to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Nunmai — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Nunmai runs it in a
...
nunmai photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Nunmai Engine` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.nunmai/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.nunmai/.env`
...
(device token, dashboard project id) lives in `~/.nunmai/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Nunmai
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Nunmai replies with a pairing code. Approve it with:

```bash
nunmai pairing approve photon <CODE>
```

Use `nunmai pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.nunmai/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nunmai/.env`):
...
By default Nunmai responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Nunmai` and
`@Nunmai agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Nunmai will reply.

## Status & troubleshooting

```bash
nunmai photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Nunmai uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run nunmai photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nunmai photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nunmai photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nunmai photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Nunmai sends images, voice
  ...
  media.
- **Native polls are supported.** Nunmai sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Nunmai, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Nunmai-sent messages
...
- **Message effects are supported.** Nunmai sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `nunmai send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<nunmai-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Nunmai is set
...
| `PHOTON_MENTION_PATTERNS` | Nunmai wake words  | JSON list / comma / newline regex patterns for group mentions |
