...
Connect Nunmai to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Nunmai supports two WeCom integration modes:
...
> Run `nunmai gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Nunmai decrypts the message, queues it for the agent
...
(Use `nunmai gateway start` only after `nunmai gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Nunmai and the token the
...
in `~/.nunmai/.env` values around `=` will also break signature checks. After
fixing, restart `nunmai gateway run`.
...
Check `nunmai gateway run` logs for the bound host/port. If the adapter bound to
