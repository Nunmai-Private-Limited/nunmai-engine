...
sidebar_position: 30
title: "Nunmai Relay"
description: "Connect Nunmai to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Nunmai Relay (Connector)
...
Nunmai Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nunmai Portal access token from your existing login
   (`~/.nunmai/auth.json`) — this proves which Nunmai org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no Nunmai Portal involved): with
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.nunmai/.env`:
...
`nunmai gateway enroll` refuses to run in managed/hosted installs — there the
...
`nunmai gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.nunmai/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Nunmai Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `nunmai auth add nous` (or `nunmai setup`) and retry.
...
startup. Restart it (`nunmai gateway restart`).
