...
If you have local or plugin commands that should stay visible in Telegram's `/` picker, prioritize them in `~/.nunmai/config.yaml`:
...
Send `/topic off` in the root DM. Nunmai flips the row off, clears the chat's `(thread_id → session_id)` bindings, and the root DM reverts to a normal Nunmai chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
