...
description: Control your smart home with Nunmai Engine via Home Assistant integration.
...
Nunmai Engine integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Nunmai Engine"
5. Copy the token
...
Nunmai Engine registers four tools for smart home control:
...
Configure which events the agent sees in `~/.nunmai/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Nunmai Engine".
...
The adapter reads credentials from `~/.nunmai/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Nunmai profile
...
running Nunmai — `curl -H "Authorization: Bearer <token>" <url>/api/` should
