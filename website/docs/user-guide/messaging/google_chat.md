...
sidebar_position: 12
title: "Google Chat"
description: "Set up Nunmai Engine as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Nunmai Engine to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Nunmai
...
> Run `nunmai gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Nunmai can read (e.g.,
`~/.nunmai/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a nunmai restart)
...
- Principal: `nunmai-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Nunmai calls
...
- **App name**: whatever you want users to see ("Nunmai" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/nunmai-chat-events`.
...
`ADDED_TO_SPACE` event that Nunmai uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Nunmai

Add the Google Chat section to `~/.nunmai/.env`:
...
On Docker / hosted images `/opt/nunmai/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `NUNMAI_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Nunmai is thinking…" marker, then
...
`~/.nunmai/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Nunmai is thinking…").
...
Thread support: when a user replies inside a thread, Nunmai detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Nunmai session.
...
3. Download the JSON. Move it onto the host that runs Nunmai.
4. Register the client with Nunmai (run under the profile you want it scoped to):
...
nunmai -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Nunmai home (e.g.
`~/.nunmai/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.nunmai/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.nunmai/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Nunmai isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `nunmai gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Nunmai will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.nunmai/google_chat_user_tokens/<sanitized_email>.json` (filesystem
