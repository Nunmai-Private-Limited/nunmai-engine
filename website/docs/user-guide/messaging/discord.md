...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Nunmai. `"all"` — accept all bot messages. |
...
- [Use Voice Mode with Nunmai](/guides/use-voice-mode-with-nunmai)
