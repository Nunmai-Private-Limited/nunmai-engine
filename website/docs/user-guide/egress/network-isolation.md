...
When running Nunmai inside Docker, the default `network_mode: host` gives the
...
The Nunmai [SECURITY.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ nunmai-engine │   │ nunmai-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   NUNMAI_UID=$(id -u) NUNMAI_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=nunmai,nunmai-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://nunmai-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/SECURITY.md) — Nunmai trust model and vulnerability reporting
- [Docker](../docker.md) — running Nunmai in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/docker-compose.yml) — default compose configuration
