...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/productivity/shopify` |
...
4. Save to `${NUNMAI_HOME:-~/.nunmai}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Nunmai","productType":"Apparel","tags":["test"]}}'
```
