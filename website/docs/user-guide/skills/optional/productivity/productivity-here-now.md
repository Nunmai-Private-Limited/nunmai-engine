...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NUNMAI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NUNMAI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NUNMAI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nunmai
```
...
```bash
PUBLISH="${NUNMAI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nunmai
```
...
```bash
DRIVE="${NUNMAI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nunmai`)    |
