...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Nunmai Engine"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${NUNMAI_HOME:-~/.nunmai}/.env`:
...
```bash
CANVAS="python $NUNMAI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
