...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/productivity/siyuan` |
...
3. Store it in `${NUNMAI_HOME:-~/.nunmai}/.env`:
   ```
...
```yaml
# In ~/.nunmai/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
