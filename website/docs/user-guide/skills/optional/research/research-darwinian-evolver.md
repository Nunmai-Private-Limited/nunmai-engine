...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Nunmai Engine |
| License | MIT |
...
import upstream classes into Nunmai itself.
...
```bash
mkdir -p ~/.nunmai/cache/darwinian-evolver && cd ~/.nunmai/cache/darwinian-evolver
...
```bash
cd ~/.nunmai/cache/darwinian-evolver/darwinian_evolver \
...
uv run darwinian_evolver parrot \
  ...
  --output_dir ~/.nunmai/cache/scratch/parrot_demo
```

Outputs:
- `~/.nunmai/cache/scratch/parrot_demo/snapshots/iteration_N.pkl` — pickled population per iteration
- `~/.nunmai/cache/scratch/parrot_demo/<jsonl>` — per-iteration JSON log (path printed at end)

Open `~/.nunmai/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.nunmai/skills/research/darwinian-evolver
DE_DIR=~/.nunmai/cache/darwinian-evolver/darwinian_evolver

cd "$DE_DIR" && \
  EVOLVER_MODEL='openai/gpt-4o-mini' \
  uv run --with openai python "$SKILL_DIR/scripts/parrot_openrouter.py" \
    --num_iterations 3 --num_parents_per_iteration 2 \
    --output_dir ~/.nunmai/cache/scratch/parrot_or
```
...
uv run --with openai python "$SKILL_DIR/scripts/show_snapshot.py" \
  ~/.nunmai/cache/scratch/parrot_or/snapshots/iteration_3.pkl
```
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Nunmai core.
   Custom driver scripts under `~/.nunmai/skills/...` are user-side and fine.
