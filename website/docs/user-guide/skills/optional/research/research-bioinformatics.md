...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/research/bioinformatics` |
...
2. Clone the relevant repo (shallow clone to save time):
   ...
   git clone --depth 1 https://github.com/GPTomics/bioSkills.git ~/.nunmai/cache/scratch/bioSkills

   # ClawBio (runnable pipelines)
   git clone --depth 1 https://github.com/ClawBio/ClawBio.git ~/.nunmai/cache/scratch/ClawBio
   ```
3. Read the specific skill:
   ```bash
   # bioSkills — each skill is at: <category>/<skill-name>/SKILL.md
   cat ~/.nunmai/cache/scratch/bioSkills/variant-calling/gatk-variant-calling/SKILL.md

   # ClawBio — each skill is at: skills/<skill-name>/
   cat ~/.nunmai/cache/scratch/ClawBio/skills/pharmgx-reporter/README.md
   ```
4. Follow the fetched skill as reference material. These are NOT Nunmai-format skills — treat them as expert domain guides. They contain correct parameters, proper tool flags, and validated pipelines.
...
- The fetched skills are NOT in Nunmai SKILL.md format. They use their own structure (bioSkills: code pattern cookbooks; ClawBio: README + Python scripts). Read them as expert reference material.
