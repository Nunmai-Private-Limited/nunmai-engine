...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Nunmai core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Nunmai native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Nunmai native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Nunmai native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
```bash
parallel-cli search "latest React 19 changes" --json -o ~/.nunmai/cache/scratch/react-19-search.json
```
...
## Recommended Nunmai usage patterns
...
- For large result sets, save JSON to `~/.nunmai/cache/scratch/*.json` (the Nunmai scratch dir) instead of stuffing everything into context.
- Do not silently choose Parallel when Nunmai native tools are already sufficient.
