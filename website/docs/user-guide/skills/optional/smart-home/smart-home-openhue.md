...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/smart-home/openhue` |
...
curl -sL "https://github.com/openhue/openhue-cli/releases/latest/download/openhue_Linux_x86_64.tar.gz" \
  | tar -xz -C ~/.nunmai/cache/scratch openhue \
  && install -m 0755 ~/.nunmai/cache/scratch/openhue ~/.local/bin/openhue
...
- Bridge must be on the same local network as the machine running Nunmai
