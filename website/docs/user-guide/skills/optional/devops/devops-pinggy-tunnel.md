...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/devops/pinggy-tunnel` |
...
```bash
LOG=~/.nunmai/cache/scratch/pinggy-8000.log
nohup ssh -p 443 \
    ...
    -R0:localhost:8000 free@a.pinggy.io \
    > "$LOG" 2>&1 &
echo $! > ~/.nunmai/cache/scratch/pinggy-8000.pid
```
...
```bash
sleep 4
grep -oE 'https://[a-z0-9-]+\.[a-z]+\.pinggy\.link' ~/.nunmai/cache/scratch/pinggy-8000.log | head -1
```
...
```bash
kill "$(cat ~/.nunmai/cache/scratch/pinggy-8000.pid)"
...
```bash
# 1. Tiny capturing server: every request gets appended to ~/.nunmai/cache/scratch/webhook-hits.log
cat >~/.nunmai/cache/scratch/webhook-server.py <<'PY'
import http.server, json, datetime, pathlib
LOG = pathlib.Path("~/.nunmai/cache/scratch/webhook-hits.log").expanduser()
...
PY
nohup python ~/.nunmai/cache/scratch/webhook-server.py >~/.nunmai/cache/scratch/webhook-server.log 2>&1 &
echo $! >~/.nunmai/cache/scratch/webhook-server.pid
...
nohup ssh -p 443 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    ...
    >~/.nunmai/cache/scratch/webhook-pinggy.log 2>&1 &
echo $! >~/.nunmai/cache/scratch/webhook-pinggy.pid
sleep 5
URL=$(grep -oE 'https://[a-z0-9-]+\.[a-z]+\.pinggy\.link' ~/.nunmai/cache/scratch/webhook-pinggy.log | head -1)
echo "Webhook URL: $URL"

# 3. While the agent works, watch hits land
tail -f ~/.nunmai/cache/scratch/webhook-hits.log
```

Hand `$URL` to the service that needs to call you. Teardown: `kill $(cat ~/.nunmai/cache/scratch/webhook-server.pid) $(cat ~/.nunmai/cache/scratch/webhook-pinggy.pid)`.
...
nohup python my_mcp_server.py --transport http --port 8765 \
    >~/.nunmai/cache/scratch/mcp-server.log 2>&1 &
echo $! >~/.nunmai/cache/scratch/mcp-server.pid
...
nohup ssh -p 443 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    ...
    >~/.nunmai/cache/scratch/mcp-pinggy.log 2>&1 &
echo $! >~/.nunmai/cache/scratch/mcp-pinggy.pid
sleep 5
URL=$(grep -oE 'https://[a-z0-9-]+\.[a-z]+\.pinggy\.link' ~/.nunmai/cache/scratch/mcp-pinggy.log | head -1)
echo "MCP URL: $URL"
echo "Bearer token: $TOKEN"
```

The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Nunmai' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
...
nohup ssh -p 443 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    ...
    >~/.nunmai/cache/scratch/llm-pinggy.log 2>&1 &
echo $! >~/.nunmai/cache/scratch/llm-pinggy.pid
sleep 5
URL=$(grep -oE 'https://[a-z0-9-]+\.[a-z]+\.pinggy\.link' ~/.nunmai/cache/scratch/llm-pinggy.log | head -1)
echo "Endpoint: $URL"
echo "Token:    $TOKEN"
...
python -m http.server 18000 --bind 127.0.0.1 >~/.nunmai/cache/scratch/origin.log 2>&1 &
ORIGIN_PID=$!

nohup ssh -p 443 \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
    -R0:localhost:18000 free@a.pinggy.io >~/.nunmai/cache/scratch/pinggy-verify.log 2>&1 &
SSH_PID=$!

sleep 5
URL=$(grep -oE 'https://[a-z0-9-]+\.[a-z]+\.pinggy\.link' ~/.nunmai/cache/scratch/pinggy-verify.log | head -1)
echo "URL: $URL"
curl -sI "$URL/" | head -1
