...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Nunmai Engine |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`nunmai-engine`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine.md), [`mcporter`](../../optional/mcp/mcp-mcporter.md), [`fastmcp`](../../optional/mcp/mcp-fastmcp.md) |
...
# MCP OAuth on a Remote Nunmai Gateway

## Overview

Nunmai' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Nunmai process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Nunmai runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Nunmai.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Nunmai' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Nunmai is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Nunmai** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Nunmai process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the nunmai user, an
empty or missing tokens directory (`$NUNMAI_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Nunmai detects a remote session it prints two
...
Both require an interactive terminal to the Nunmai host. The rest of this skill
is for when there is NO interactive TTY — Nunmai running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Nunmai Dashboard (try this BEFORE manual token surgery)

A remote Nunmai gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `nunmai dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'nunmai dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "NUNMAI_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`NUNMAI_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$NUNMAI_HOME/.env`. Those
...
Nunmai' `NunmaiTokenStorage` would have written, so on `/reload-mcp` Nunmai finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "NUNMAI|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Nunmai' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find NUNMAI_HOME and the config path

```bash
NUNMAI_HOME=$(python3 -c 'from nunmai_constants import get_nunmai_home; print(get_nunmai_home())')
echo "config: $NUNMAI_HOME/config.yaml"
echo "tokens: $NUNMAI_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Nunmai Engine (manual OAuth)",
...
**Stash `code_verifier` and `state` to disk** (e.g. `~/.nunmai/cache/scratch/.mcp-oauth-work/<server>.json`,
...
### 8. Write tokens in Nunmai' exact schema

`tools/mcp_oauth.py::NunmaiTokenStorage` expects two files under
`$NUNMAI_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "nunmai-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Nunmai (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Nunmai sees `auth: oauth`, calls `NunmaiTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Nunmai process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Nunmai needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
13. **Security: the stash file contains the `code_verifier`.** Delete `~/.nunmai/cache/scratch/.mcp-oauth-work/<server>.json` immediately after successful token exchange. There's no reason to keep a proof-of-identity secret around once it's consumed.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Nunmai does natively).
...
- **Don't claim Nunmai doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Nunmai. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Nunmai' config.
