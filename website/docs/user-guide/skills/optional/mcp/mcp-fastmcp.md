...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/mcp/fastmcp` |
...
| Related skills | [`nunmai-engine`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Nunmai or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Nunmai. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nunmai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nunmai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Nunmai integration, either:

- configure the server in `~/.nunmai/config.yaml` using the `native-mcp` skill, or
...
### Nunmai cannot see the deployed server

The server-building part may be correct while the Nunmai config is not. Load the `native-mcp` skill and configure the server in `~/.nunmai/config.yaml`, then restart Nunmai.
