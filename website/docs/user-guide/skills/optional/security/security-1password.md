...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Nunmai Engine |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Nunmai terminal calls (desktop app flow only)
...
### Service Account (recommended for Nunmai)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${NUNMAI_HOME:-~/.nunmai}/.env` (the skill will prompt for this on first load).
...
## Nunmai Execution Pattern (desktop app flow)

Nunmai terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-${NUNMAI_HOME:-$HOME/.nunmai}/cache/scratch}/nunmai-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nunmai-op.sock"
