...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Nunmai MCP catalog. The
...
Two halves, in this order: the editor side must be up before Nunmai connects.
...
### One-time, Nunmai side

    nunmai mcp install unreal-engine
...
`~/.nunmai/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Nunmai — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Nunmai connects
...
2. Start the Nunmai session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Nunmai
they appear as:
...
then happens at `nunmai mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Nunmai runs batched calls
...
- **Start order matters.** Editor + server up first, then the Nunmai
...
- **Timeouts on long operations.** Nunmai' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.nunmai/config.yaml` for
