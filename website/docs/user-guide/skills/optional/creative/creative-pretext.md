...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/pretext` |
...
This skill exists so Nunmai can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
2. **Start from a template**:
   ...
   - `write_file` to a new `.html` in `~/.nunmai/cache/scratch/` or the user's workspace.
