...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/ai-presenter-video` |
| Path | `optional-skills/creative/ai-presenter-video` |
| Version | `1.0.0` |
| Author | cclank (https://github.com/cclank/lanshu-create-ai-presenter-video), ported by Nunmai Engine |
| License | MIT |
...
> kept substantively verbatim in `references/`; Nunmai adaptations live in
> this hub file. Scripts are deterministic (no network, no credentials).

## Nunmai adaptations (read first)

- **Skill dir resolution** — upstream hardcoded its own agent's skills path.
  In Nunmai the loader expands `${NUNMAI_SKILL_DIR}` to this skill's installed
  directory, so every command below uses that token directly:

  ```bash
  SKILL_DIR="${NUNMAI_SKILL_DIR}"
  ```
