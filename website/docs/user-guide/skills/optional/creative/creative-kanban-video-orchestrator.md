...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Nunmai Kanban pipeline that
...
3. **Generates** a setup script that creates Nunmai profiles, project workspace, and the initial kanban task
...
For mapping role → which Nunmai skills + toolsets it loads, see
...
3. Creates each Nunmai profile via `nunmai profile create --clone`
...
7. Fires the initial `nunmai kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
nunmai kanban watch --tenant <project-tenant>     # live events
nunmai kanban list  --tenant <project-tenant>     # board snapshot
nunmai dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `nunmai kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${NUNMAI_HOME:-~/.nunmai}/.env` or the user's secret store.
