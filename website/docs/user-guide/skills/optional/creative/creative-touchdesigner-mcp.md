...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/touchdesigner-mcp` |
...
```
Nunmai Engine -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${NUNMAI_HOME:-$HOME/.nunmai}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Nunmai config (if missing)
...
3. **Restart Nunmai session** to pick up the new MCP server
