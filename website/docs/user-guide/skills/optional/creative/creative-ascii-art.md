...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/creative/ascii-art` |
| Path | `optional-skills/creative/ascii-art` |
| Version | `4.0.0` |
| Author | 0xbyt4, Nunmai Engine |
| License | MIT |
...
```bash
python -m pyfiglet "NUNMAI" -f slant | boxes -d stone
# Or without pyfiglet installed:
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=NUNMAI&font=Slant" | boxes -d stone
```
...
```bash
curl -s 'https://ascii.co.uk/art/cat' -o ~/.nunmai/cache/scratch/ascii_art.html
```
...
```python
import os, re, html
with open(os.path.expanduser('~/.nunmai/cache/scratch/ascii_art.html')) as f:
    text = f.read()
