...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/payments/stripe-link-cli` |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Nunmai can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Nunmai cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Nunmai attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Nunmai" --interval 5 --timeout 300
```
...
link-cli spend-request retrieve <lsrq_id> \
  --include card \
  --output-file ~/.nunmai/cache/scratch/link-card.json \
  --format json
```
...
```
rm -f ~/.nunmai/cache/scratch/link-card.json
```

## Optional: run as an MCP server instead

`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Nunmai' native MCP:

```
nunmai mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `nunmai mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
