...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so Nunmai can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
