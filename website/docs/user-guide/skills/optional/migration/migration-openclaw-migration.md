...
```bash
python ~/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.nunmai/skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
