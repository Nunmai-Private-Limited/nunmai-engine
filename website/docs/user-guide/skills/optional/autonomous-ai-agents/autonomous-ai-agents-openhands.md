...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/autonomous-ai-agents/openhands` |
| Path | `optional-skills/autonomous-ai-agents/openhands` |
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), Nunmai Engine |
| License | MIT |
| Platforms | linux, macos |
| Tags | `Coding-Agent`, `OpenHands`, `Model-Agnostic`, `LiteLLM` |
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`opencode`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-opencode), [`nunmai-engine`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine) |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Nunmai.

## When to Use

- User wants a coding task delegated to OpenHands specifically.
- User wants a coding agent that can run on a non-Anthropic / non-OpenAI provider (DeepSeek, Qwen, Ollama, vLLM, Nunmai, etc.) — sibling skills `claude-code` and `codex` are tied to one vendor.
- Multi-step file edits + shell commands inside a workspace.

For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Nunmai-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `nunmai-engine` (Nunmai subagents via `delegate_task`).
