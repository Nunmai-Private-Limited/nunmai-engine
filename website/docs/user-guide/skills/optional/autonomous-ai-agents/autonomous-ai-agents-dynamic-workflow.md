...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/autonomous-ai-agents/dynamic-workflow` |
| Path | `optional-skills/autonomous-ai-agents/dynamic-workflow` |
| Version | `2.0.0` |
| Author | Teknium + Nunmai Engine |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `orchestration`, `fan-out`, `subagents`, `delegation`, `verification`, `migration`, `audit`, `research`, `campaign` |
| Related skills | [`nunmai-engine`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine.md), [`simplify-code`](../../bundled/software-development/software-development-simplify-code.md) |
...
for serial chains. For a refactor or fix campaign on nunmai-engine itself, load
`nunmai-engine` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `NUNMAI_HOME`,
