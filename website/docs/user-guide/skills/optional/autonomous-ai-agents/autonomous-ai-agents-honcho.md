---
title: "Honcho — Configure and troubleshoot Honcho memory for Nunmai"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Nunmai"
---
...
Configure and troubleshoot Honcho memory for Nunmai.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Nunmai

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Nunmai profile its own peer identity while sharing a unified view of the user.
...
```bash
nunmai honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Nunmai profile's AI peer
...
Honcho models conversations as interactions between **peers**. Nunmai creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Nunmai instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Nunmai run |
| `global` | Single session across all directories |

Manual override: `nunmai honcho map my-project-name`
...
Each Nunmai profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nunmai.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nunmai.coder` host block in `honcho.json`
...
```bash
nunmai honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Nunmai when Honcho memory is active.
...
Config file: `$NUNMAI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nunmai honcho setup`. Ensure `memory.provider: honcho` is in `~/.nunmai/config.yaml`.

### Memory not persisting across sessions
Check `nunmai honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nunmai profile create <name> --clone`. For existing profiles: `nunmai honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nunmai honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nunmai honcho status` | Show resolved config, connection test, peer info for active profile |
| `nunmai honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nunmai honcho disable` | Disable Honcho for the active profile |
| `nunmai honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nunmai honcho peers` | Show peer identities across all profiles |
| `nunmai honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nunmai honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nunmai honcho sessions` | List known directory-to-session-name mappings |
| `nunmai honcho map <name>` | Map current working directory to a Honcho session name |
| `nunmai honcho identity` | Seed AI peer identity or show both peer representations |
| `nunmai honcho sync` | Create host blocks for all Nunmai profiles that don't have one yet |
| `nunmai honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Nunmai + Honcho |
| `nunmai memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nunmai memory status` | Show active memory provider and config |
| `nunmai memory off` | Disable external memory provider |
