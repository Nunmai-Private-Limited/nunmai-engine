...
| | |
|---|---|
| Source | Optional — install with `nunmai skills install official/autonomous-ai-agents/blackbox` |
...
| Related skills | [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`nunmai-engine`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine.md) |
...
Delegate coding tasks to [Blackbox AI](https://www.blackbox.ai/) via the Nunmai terminal. Blackbox is a multi-model coding agent CLI that dispatches tasks to multiple LLMs (Claude, Codex, Gemini, Blackbox Pro) and uses a judge to select the best implementation.
...
```
terminal(command="blackbox --prompt 'Fix the login bug'", workdir="~/.nunmai/cache/scratch/issue-1", background=true, pty=true)
terminal(command="blackbox --prompt 'Add unit tests for auth'", workdir="~/.nunmai/cache/scratch/issue-2", background=true, pty=true)
