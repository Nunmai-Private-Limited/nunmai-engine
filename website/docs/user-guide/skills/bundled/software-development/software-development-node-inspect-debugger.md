...
Driver script (save as `~/.nunmai/cache/scratch/cdp-debug.js`):
...
```bash
node ~/.nunmai/cache/scratch/cdp-debug.js
```

Nunmai-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:

```bash
mkdir -p ~/.nunmai/cache/scratch/cdp-tools && cd ~/.nunmai/cache/scratch/cdp-tools && npm i chrome-remote-interface
NODE_PATH=~/.nunmai/cache/scratch/cdp-tools/node_modules node ~/.nunmai/cache/scratch/cdp-debug.js
```

## Debugging Nunmai ui-tui
...
### Debugging a running `nunmai --tui`
...
require('fs').writeFileSync('~/.nunmai/cache/scratch/cpu.cpuprofile', JSON.stringify(profile));
// Open ~/.nunmai/cache/scratch/cpu.cpuprofile in Chrome DevTools → Performance tab
```
...
require('fs').writeFileSync('~/.nunmai/cache/scratch/heap.heapsnapshot', chunks.join(''));
```
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Nunmai, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
