...
This wrapper skill teaches the Nunmai `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Nunmai autodetection is a
planned cua-driver follow-up, so currently point Nunmai at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   Nunmai applies the same host-side floor in unrestricted mode. If it is not
...
Explicit Nunmai YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
Nunmai autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Nunmai at that
