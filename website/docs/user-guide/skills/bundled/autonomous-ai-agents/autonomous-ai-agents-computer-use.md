...
| Version | `2.1.0` |
| Author | Francesco Bonacci (f-trycua), Nunmai Engine |
| License | MIT |
...
Nunmai drives [cua-driver](https://github.com/trycua/cua) under the hood.
This skill teaches the Nunmai `computer_use` **action vocabulary**, which is
NOT the driver's raw MCP vocabulary. Call the actions documented below and
never the driver's tools by name: `capture` is a Nunmai action that maps to
the driver's `get_window_state`; `element=N` is a Nunmai argument that the
...
`mode` decides what Nunmai hands back to you, not what the driver does.
...
rejects image tool results), Nunmai routes the screenshot through the
...
|---|---|
| `cua-driver not installed` | Run `nunmai computer-use install`, or `nunmai tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `nunmai computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| "bare element_index is not accepted" / `snapshot_id_required` | The driver saw a raw index without its token. This is a wrapper defect, not something you fix by passing `snapshot_id` (Nunmai has no such argument). Re-capture once; if it repeats, tell the user to run `nunmai update` and fall back to `coordinate=[x, y]` from the capture's bounds meanwhile |
| "tool 'capture' has no reviewed risk classification" / `Unknown tool` | Something called the driver's MCP vocabulary directly (`capture`, `screenshot`, `get_window_state`, `click` with raw args). Only the `computer_use(action=…)` vocabulary in this file exists on the Nunmai side |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `nunmai computer-use doctor` |
...
| `nunmai computer-use doctor` says "could not be started … Access is denied" (Windows) | The Nunmai venv interpreter can't execute a binary under `C:\Program Files\WindowsApps`; the tool itself may still work because the shell resolves another copy on PATH. Fix once: reinstall cua-driver with the upstream installer (lands under the user profile) or set `NUNMAI_CUA_DRIVER_CMD` to a copy outside `WindowsApps`. The same denial spams `errors.log` for any other `WindowsApps` binary Nunmai spawns (e.g. `bws.exe`) |
| Anything else weird | **First action: ask the user to run `nunmai computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Nunmai intentionally keeps THIS skill focused on the Nunmai-side
...
links the pack into `~/.nunmai/skills/cua-driver` (Nunmai is a detected
...
calling the Nunmai actions from this file — the wrapper does the translation.
