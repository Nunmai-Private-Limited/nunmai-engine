...
| Related skills | [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`nunmai-engine`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine.md), [`opencode`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-opencode.md) |
...
# Claude Code — Nunmai Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Nunmai terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Nunmai interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
```
# Start a task
terminal(command="claude -p 'Start refactoring the database layer' --output-format json --max-turns 10 > ~/.nunmai/cache/scratch/session.json", workdir="/project", timeout=180)

# Resume with session ID
terminal(command="claude -p 'Continue and add connection pooling' --resume $(cat ~/.nunmai/cache/scratch/session.json | python -c 'import json,sys; print(json.load(sys.stdin)[\"session_id\"])') --max-turns 5", workdir="/project", timeout=120)
...
```json
{
  "hooks": {
    ...
    "Stop": [{
      "hooks": [{"type": "command", "command": "echo 'Claude finished a response' >> ~/.nunmai/cache/scratch/claude-activity.log"}]
    }]
  }
}
```
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Nunmai terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
