...
Ledger location: `$NUNMAI_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `NUNMAI_CITATION_LEDGER`.

## How to Run

```bash
S=~/.nunmai/skills/research/grounded-citations/scripts/sources.py
...
`nunmai skills install official/social-media/reddit-reading` or
`nunmai skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `NUNMAI_CITATION_LEDGER`) if their
