...
screencapture -w -o ~/.nunmai/cache/scratch/findmy.png
```

Then use `vision_analyze` to read the screenshot:
```
vision_analyze(image_url="~/.nunmai/cache/scratch/findmy.png", question="What devices/items are shown and what are their locations?")
```
...
peekaboo see --app "FindMy" --annotate --path ~/.nunmai/cache/scratch/findmy-ui.png

# Click on a specific device/item by element ID
peekaboo click --on B3 --app "FindMy"

# Capture the detail view
peekaboo image --app "FindMy" --path ~/.nunmai/cache/scratch/findmy-detail.png
```

Then analyze with vision:
```
vision_analyze(image_url="~/.nunmai/cache/scratch/findmy-detail.png", question="What is the location shown for this device/item? Include address and coordinates if visible.")
```
...
while true; do
    screencapture -w -o ~/.nunmai/cache/scratch/findmy-$(date +%H%M%S).png
