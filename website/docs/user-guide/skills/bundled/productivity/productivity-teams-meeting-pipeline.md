...
Everything operator-facing is a `nunmai teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${NUNMAI_HOME:-~/.nunmai}/.env`:
...
```bash
nunmai teams-pipeline validate              # config snapshot — run first after any change
nunmai teams-pipeline token-health          # Graph token status
nunmai teams-pipeline token-health --force-refresh   # force a fresh token acquisition
nunmai teams-pipeline list                  # recent meeting jobs
nunmai teams-pipeline list --status failed  # only failed jobs
nunmai teams-pipeline show <job-id>         # full detail of one job
nunmai teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
nunmai teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
nunmai teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
nunmai teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
nunmai teams-pipeline fetch --join-web-url "<url>" --organizer-user-id <id>   # organizer-scoped lookup (required for /meet/ short URLs)
```

### Subscription management

```bash
nunmai teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

nunmai teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
nunmai teams-pipeline delete-subscription <sub-id>
nunmai teams-pipeline maintain-subscriptions            # renew near-expiry ones
nunmai teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `nunmai teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `nunmai cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
