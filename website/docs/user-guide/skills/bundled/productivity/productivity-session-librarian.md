...
| List/filter by metadata (age, source, cost, tokens, workspace) | `nunmai sessions list` / `stats` via terminal |
| Rename | `nunmai sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `nunmai sessions archive <filters>` |
| Delete (destructive) | `nunmai sessions delete` / `nunmai sessions prune <filters>` |
| Export before deleting anything valuable | `nunmai sessions export --session-id <id> --format md` |
...
`nunmai sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `nunmai sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `nunmai sessions list`
