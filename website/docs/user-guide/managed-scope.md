...
`~/.nunmai/config.yaml`, `~/.nunmai/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/nunmai`:

```text
/etc/nunmai/
├── config.yaml     # managed config layer (wins over ~/.nunmai/config.yaml)
└── .env            # managed env layer (wins over ~/.nunmai/.env + shell)
```
...
The location can be relocated with the `NUNMAI_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `NUNMAI_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Nunmai.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export NUNMAI_MANAGED_DIR=/opt/org/nunmai-policy
```

:::warning
A user who can set `NUNMAI_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `nunmai doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/nunmai/config.yaml` (managed) | `/etc/nunmai/.env` (managed) |
| 2 | `~/.nunmai/config.yaml` (user) | `~/.nunmai/.env` (user) |
...
```bash
nunmai config        # shows a header naming the managed source + the pinned keys
nunmai doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Nunmai refuses and names the source:

```bash
$ nunmai config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/nunmai/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `nunmai config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/nunmai

# Pin some config values for every user on this machine
sudo tee /etc/nunmai/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/nunmai/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/nunmai
sudo chmod 0644 /etc/nunmai/config.yaml /etc/nunmai/.env
```

Changes take effect on the next Nunmai start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`nunmai doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Nunmai as `root`), managed scope is advisory.
