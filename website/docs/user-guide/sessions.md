...
Nunmai Engine automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.nunmai/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Nunmai stores session history so it can resume conversations, but it does not
...
Nunmai explicitly injects for that turn.
...
For example, if a user sends an image and asks Nunmai to make a meme from it,
Nunmai may inspect that image once with vision and run an image-processing
...
`nunmai sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `nunmai sessions optimize` merges FTS5 index segments and
VACUUMs the database without touching any session data. Both `optimize` and `prune` refuse
while another Nunmai process (gateway, Desktop, dashboard, cron) holds `state.db` — stop it
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`nunmai` or `nunmai chat`) |
| `oneshot` | Finite non-interactive runs: `nunmai chat --oneshot -q`, `-Q`, `nunmai -z`, and `-q` on non-TTY stdio. Hidden from the TUI, Desktop and dashboard session pickers (like `kanban` and `tool`), even when launched from inside a TUI or Desktop session — the run inherits that transport's environment but is not that conversation. Still counts as CLI history: `nunmai -c` / `--resume latest` continue the last one-shot, and `nunmai sessions list` shows it. An explicit `--source <tag>` always wins (`nunmai chat -q --source tui` is stored as `tui`). |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.nunmai/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `nunmai -c` again in the *same* terminal, Nunmai resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
nunmai -c "my project"   # → resumes "my project #3"
```
...
nunmai --resume 20250305_091523_a1b2c3d4
nunmai -r 20250305_091523_a1b2c3d4

# Resume by title
nunmai --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
nunmai --resume latest

# Or with the chat subcommand
nunmai chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `nunmai sessions list`.
...
nunmai --resume latest --in ./my-project

# Works with the TUI too
nunmai --tui --resume latest --in ./my-project
```
...
```bash
nunmai --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`nunmai sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
nunmai sessions list --workspace my-project
nunmai sessions list --workspace ~/code/nunmai-engine
```

### Conversation Recap on Resume

When you resume a session, Nunmai displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Nunmai session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.nunmai/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `nunmai -r "<title>"` from the shell) and pick up where the platform left off.
...
Nunmai automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `nunmai sessions list` or `nunmai sessions browse`.
...
```bash
nunmai sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Nunmai creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`nunmai -c "my project"`), it automatically picks the most recent session in the lineage.
...
Nunmai provides a full set of session management commands via `nunmai sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
nunmai sessions list

# Filter by platform
nunmai sessions list --source telegram

# Show more sessions
nunmai sessions list --limit 50
```
...
`nunmai sessions export` is one surface for every export format, selected with `--format`:
...
nunmai sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
nunmai sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
nunmai sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
nunmai sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `nunmai-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
nunmai sessions export --format trace

# One session to a local trace file
nunmai sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
nunmai sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.nunmai/session-exports`).

```bash
# Export one session to Markdown
nunmai sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
nunmai sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
nunmai sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
nunmai sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
nunmai sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
nunmai sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
nunmai sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
nunmai sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
nunmai sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
nunmai sessions pin 20250305_091523_a1b2c3d4
nunmai sessions pin 20250305 20250306

# Remove the pin
nunmai sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
nunmai sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
nunmai sessions pinned --json > pinned-sessions.json
```
...
nunmai sessions prune

# Custom age threshold — bare numbers are days
nunmai sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
nunmai sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
nunmai sessions prune --newer-than 5h

# Explicit window with absolute timestamps
nunmai sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
nunmai sessions prune --source telegram
nunmai sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
nunmai sessions prune --newer-than 5h --title "smoke test"   # title substring
nunmai sessions prune --older-than 30 --max-messages 3        # tiny sessions
nunmai sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
nunmai sessions prune --model gpt-5 --older-than 1w           # by model (substring)
nunmai sessions prune --provider openrouter --older-than 60   # by billing provider
nunmai sessions prune --branch feature/old-experiment         # by git branch
nunmai sessions prune --user 12345678 --chat-type group       # by messaging origin
nunmai sessions prune --max-tokens 500 --older-than 7         # by token usage
nunmai sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
nunmai sessions prune --newer-than 5h --dry-run

# Skip confirmation
nunmai sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `nunmai sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `nunmai sessions prune` keeps the 90-day cutoff. Every
...
`nunmai sessions archive` takes the same filters as `prune` but soft-hides
...
nunmai sessions archive --newer-than 5h

# Archive by title substring, preview first
nunmai sessions archive --title "dry run" --dry-run
nunmai sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `nunmai sessions archive` refuses to
...
`nunmai sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
nunmai sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`nunmai insights`](../reference/cli-commands.md#nunmai-insights).
...
`nunmai sessions repair-routing` finds message-bearing session rows with no
...
nunmai sessions repair-routing
...
nunmai sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
nunmai sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.nunmai/state.db ~/.nunmai/state.db.bak`).
...
`nunmai profile migrate-identity <old> <new>`), and `agent:main:…` rows inside
...
`database.journal_mode: delete` only applies to databases Nunmai creates. An
...
commits, and a downgrade underneath them destroys those commits — so Nunmai
...
nunmai sessions set-journal-mode delete     # WAL -> rollback journal
nunmai sessions set-journal-mode wal        # back to WAL
nunmai sessions set-journal-mode delete --db ~/.nunmai/kanban.db   # another Nunmai store
```
...
outright unless you pass `--force` after stopping every Nunmai process
...
Started a conversation in another agent CLI? You can pull it into Nunmai and
continue it here. Nunmai reads Claude Code's session logs
...
nunmai sessions import

# Limit to one tool, or point at a specific file
nunmai sessions import --from claude
nunmai sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
nunmai --resume @claude
nunmai --resume @codex
```

`nunmai sessions import` creates a new Nunmai session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `nunmai --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Nunmai Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Nunmai** copies the conversation into the
...
When Nunmai cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Nunmai uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Nunmai in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.nunmai/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.nunmai/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.nunmai/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.nunmai/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.nunmai/sessions/sessions.json` is a **legacy mirror** of it,
...
`nunmai sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.nunmai/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `nunmai sessions list`, the cause is
`state.db` not receiving them — run `nunmai sessions repair` and watch for a
...
`*.jsonl` files in `~/.nunmai/sessions/`. They are no longer written or
read by Nunmai. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Nunmai process in the same `NUNMAI_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.nunmai/config.yaml`:
...
`nunmai sessions recover` synthesizes for orphaned messages) whose last
...
`in its tip segment`). `nunmai sessions export` still works for such sessions.
...
nunmai sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `nunmai sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
