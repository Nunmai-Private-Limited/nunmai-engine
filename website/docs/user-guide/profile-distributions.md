...
A **profile distribution** packages a complete Nunmai agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Nunmai has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `nunmai profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `nunmai profile update` pulls new versions | Re-send the file |
...
| Made with | `nunmai profile install` / `update` | `/export` and `/import`, or `nunmai profile export` / `import` |
...
Before distributions, sharing a Nunmai agent meant sending someone:
...
```bash
nunmai profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `nunmai profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Nunmai in 2026, that's already true.
...
:::caution
**Nunmai does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `nunmai profile install` or `nunmai profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
nunmai profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.nunmai/profiles/research-bot/SOUL.md
...
Create `~/.nunmai/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
nunmai_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.nunmai/profiles/research-bot/.gitignore` with at minimum:
...
For cron, commit only `cron/jobs.json`. Nunmai treats every other entry under `cron/` as runtime state (locks, ledgers, output, suggestions, and future scheduler sidecars) and never installs or replaces it from a distribution. Root-level hidden entries under `skills/` are likewise local Nunmai bookkeeping; hidden files inside an authored skill directory remain part of that skill.

### Step 4 — Push to a git repo

```bash
cd ~/.nunmai/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `nunmai profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.nunmai/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
nunmai profile install github.com/you/research-bot

# Full HTTPS
nunmai profile install https://github.com/you/research-bot.git

# SSH
nunmai profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
nunmai profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
nunmai profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
nunmai profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
nunmai profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
nunmai profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Nunmai distribution.
...
```bash
cp ~/.nunmai/profiles/research-bot/.env.EXAMPLE ~/.nunmai/profiles/research-bot/.env
...
```bash
nunmai profile info research-bot
```
...
`nunmai profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
nunmai profile delete research-bot
```
...
Path:    ~/.nunmai/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `nunmai profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.nunmai/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
nunmai profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `nunmai profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.nunmai/profiles/polymarket-trader
...
# Anyone
nunmai profile install github.com/you/nunmai-polymarket-trader --alias
```
...
You built Nunmai-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
nunmai profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
nunmai profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `nunmai profile info <name>` and hold off on updates until you're ready.
:::
...
nunmai profile info research-bot | grep Version
...
```yaml
# ~/.nunmai/profiles/research-bot/local/my-overrides.yaml
...
nunmai profile delete research-bot --yes
nunmai profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
nunmai profile update research-bot --force-config --yes
```
...
nunmai profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.nunmai/profiles/forked-research-bot/
...
nunmai profile install ~/.nunmai/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
nunmai profile delete research-bot-test --yes
nunmai profile install ~/.nunmai/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Nunmai's managed
`profile-exports/` directory under the default Nunmai home, not in the current
...
source file. If the Nunmai home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.nunmai-profile-exports/`
...
```bash
nunmai profile export research-bot
nunmai profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
nunmai profile import ./research-bot.tar.gz
nunmai profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `nunmai -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.nunmai`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.nunmai`) is exported through an allow-list — only known Nunmai artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.nunmai/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — newly shipped jobs are installed paused. Review them with `nunmai -p <name> cron list` and resume only the jobs you trust. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `nunmai profile` — not a parallel command tree.
...
- Reserved profile names (`nunmai`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`nunmai profile export` / `import`](../reference/profile-commands.md#nunmai-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Nunmai](../guides/use-soul-with-nunmai.md) — authoring personalities
