...
2. At startup, after `.env` loads, Nunmai runs the helper ONCE via `/bin/sh -c` and parses its stdout as a dotenv blob.
...
secrets:
  command:
    ...
    # command: "pass show nunmai/env"
    # command: "secret-tool lookup service nunmai-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Nunmai' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Nunmai discards its stderr on purpose) |
