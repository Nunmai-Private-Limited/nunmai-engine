...
Nunmai Engine has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Nunmai can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`nunmai fallback` reuses the provider picker from `nunmai model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.nunmai/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Nunmai still honors it for back-compat, but `nunmai fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| Nunmai Portal | `nous` | `nunmai setup --portal` (fresh) or `nunmai auth add nous` (OAuth) |
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `nunmai model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `nunmai model` (Qwen Portal OAuth; optional: `NUNMAI_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `nunmai model` (MiniMax portal OAuth) |
...
| Nunmai Agent preset | `moa` (`model` = preset name) | A configured MoA preset whose aggregator has credentials — the fallback runs the whole preset (references + aggregator), not the aggregator alone |
...
The same re-resolution happens when the CLI falls back at **startup** because the primary provider's auth fails before the first request: the fallback model is sent its own configured effort, not the primary's. An explicit `nunmai chat --reasoning <level>` is kept across that startup switch — it is your intent for the run.
...
**Nunmai Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
|---------|-------------------|
| CLI sessions (interactive and `nunmai -z` one-shot) | ✔ (at startup when the primary's credentials/quota fail, mid-session, and a chain added or edited while a chat is open applies from its next turn) |
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `nunmai fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Nunmai uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `nunmai kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Nunmai first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, Nunmai follows your configured fallback policy and then stops:
...
```text
OpenRouter → Nunmai Portal → Custom endpoint → Codex OAuth →
...
| `"codex"` | Force Codex OAuth | `nunmai model` → ChatGPT or Codex Subscription |
...
`base_url` takes precedence over `provider`. Nunmai uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Nunmai treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Nunmai falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Nunmai logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Nunmai recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Nunmai doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
If no provider is available for compression, Nunmai drops middle conversation turns without generating a summary rather than failing the session.
