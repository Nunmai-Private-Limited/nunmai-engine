...
description: "Give Nunmai new powers with reviewed plugins you can install in one click"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Nunmai plugins you
can install by name with a single command:

```bash
nunmai plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/plugin-catalog)
directory of the nunmai-engine repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `nunmai plugins install` |
...
| `requires_nunmai` | Minimum Nunmai version, e.g. `>=0.19` (optional) |
...
- **Scanned at admission, trusted at install.** Admission CI runs the same
  security scanner the installer runs (`nunmai plugins validate` includes a
...
- **Desktop plugins run with the app's authority — review is the boundary.**
  ...
  do anything the app can (gateway RPC, the full `window.nunmaiDesktop`
  ...
  `eval`, importing anything other than `@nunmai/plugin-sdk`/`react`,
...
- **Removed list.** Plugins pulled from the catalog (for example after a
  ...
  refused at load time (`nunmai plugins remove <name>`, or reinstall with
...
The catalog name and the plugin's own manifest name can differ; `nunmai
...
```bash
nunmai plugins install touchdesigner
nunmai plugins enable td
```
...
skill, so one install gives Nunmai code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
nunmai plugins install snyk
nunmai plugins enable snyk
```

### Updating a catalog install

`nunmai plugins update <name>` never runs `git pull` for catalog installs —
...
saved under `~/.nunmai/plugins-backup/<name>-<sha>/` and the update warns you.
If the new pin renames the plugin's manifest, the old directory is removed and
your enabled flag follows the new name. `nunmai plugins list` shows catalog
installs as `catalog:<tier>@<sha>` so you can see provenance at a glance.

Provenance is recorded by the installer in `~/.nunmai/plugins/.install-metadata.json`,
outside the plugin's own tree — a repository cannot ship a file that makes it
look like a reviewed catalog install. (The `.nunmai-catalog.json` inside the
...
(`https://nunmai.in/docs/api/plugin-catalog.json`).
`search`/`install`/`update` fetch it at most every six hours and cache it under
`~/.nunmai/cache/`, so new entries and removals reach installed clients without
updating Nunmai. Offline, the cached copy is used for up to 24 hours, then the
...
`nunmai update`) installs its own pin, never the cached older one. Removals
from the in-tree list and the live list are always both enforced, whatever the
cache's age.

### Custom git URLs are different

`nunmai plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/plugin-catalog);
...
`nunmai plugins list --json` reports `update_available`, the Desktop Plugins
tab shows an **Update to 1.4.0** button, and `nunmai plugins update <name>`
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Nunmai
- [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) — write your own
