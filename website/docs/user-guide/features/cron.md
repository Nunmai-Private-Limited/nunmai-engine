...
Schedule tasks to run automatically with natural language or cron expressions. Nunmai exposes cron management through a single `cronjob_manage` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Nunmai itself through the `cronjob_manage` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the main agent model from `nunmai model`.

- **Per-job pin** — a job that carries its own model. Set it to a specific model via the dashboard, `nunmai cron create/edit --model … --provider …`, or by editing `~/.nunmai/cron/jobs.json`; or **lock in the current main model** with `nunmai cron create/edit --pin` (the agent's `cronjob_manage` tool can do this too with `pinned=true`, but only when you ask it to). `--unpin` (`pinned=false`) releases the lock. The agent cannot point a job at a *different* model — inference pins are user-owned.
...
- **Main agent model** — when neither of the above is set, a job runs on whatever `nunmai model` / `/model` is set to **at the moment it fires**. Change your main model and every unpinned job follows on its next run.
...
`nunmai setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nunmai Portal](../../integrations/nous-portal.md).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `nunmai cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob_manage` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Nunmai disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
nunmai cron create "every 2h" "Check server status"
nunmai cron create "every 1h" "Summarize new feed items" --skill blogwatcher
nunmai cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Nunmai will use the unified `cronjob_manage` tool internally.
...
A missing-credential verdict names the profile and `NUNMAI_HOME` the scheduler
read, e.g. `provider credential missing: No Codex credentials stored … [profile
'default', NUNMAI_HOME /opt/data]`. When an interactive session with "the same"
credential works, compare that path with the shell's `NUNMAI_HOME`: a gateway
started without the shell's environment (Docker `HOME` vs `NUNMAI_HOME`, a
...
Or: `nunmai config set cron.preflight false`

## Moving unpinned jobs to a new global default

An unpinned job follows the main agent model, so `nunmai model` moves your cron fleet with it.
When you want a job to *stay* on a model:

```bash
nunmai cron edit <job_id> --pin                                   # lock the current main model onto one job
nunmai cron edit <job_id> --provider <provider> --model <model>   # pin an explicit model
nunmai cron edit <job_id> --unpin                                 # follow the main model again
nunmai config set cron.model <model>                              # every unpinned job, without touching chat
```

`nunmai cron list` and the `cronjob_manage` tool report `pinned` per job.

## Skill-backed cron jobs

A cron job can load one or more skills before it runs the prompt. Each skill loads exactly as it does from `/skill-name` in a chat session, including the `[Skill config ...]` block with its resolved `metadata.nunmai.config` values from `config.yaml`.
...
nunmai cron create "every 1d at 09:00" \
...
```bash
nunmai cron edit <job_id> --schedule "every 4h"
nunmai cron edit <job_id> --prompt "Use the revised task"
nunmai cron edit <job_id> --skill blogwatcher --skill maps
nunmai cron edit <job_id> --add-skill maps
nunmai cron edit <job_id> --remove-skill blogwatcher
nunmai cron edit <job_id> --clear-skills
```
...
```bash
nunmai cron list
nunmai cron pause <job_id_or_name>
nunmai cron resume <job_id_or_name>
nunmai cron run <job_id_or_name>
nunmai cron remove <job_id_or_name>
nunmai cron edit <job_id_or_name> [...flags]
nunmai cron status
nunmai cron tick
```
...
### Pausing everything: `nunmai pause`

`nunmai pause [--reason ...]` is the global emergency stop (`nunmai resume` lifts it). While it is engaged no scheduled cron fire starts, whichever door it arrives through: the built-in ticker skips its dispatch, the managed-cron (hosted scheduler) fire webhook answers `503` with `Retry-After: 60` so the scheduler redelivers the fire after you resume, and the [misfire catch-up](#misfire-catch-up) sweep stays idle instead of force-firing everything that was held back. Runs already in flight are never killed, and nothing is lost: due work catches up on the first tick or sweep after `nunmai resume`. Explicit manual runs (`nunmai cron run`, the dashboard's Trigger button) are an operator override and still execute while paused.
...
```bash
nunmai cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
nunmai cron resume <job_id>
```
...
```bash
nunmai gateway install     # Install as a user service
sudo nunmai gateway install --system   # Linux: boot-time system service for servers
nunmai gateway             # Or run in foreground

nunmai cron list
nunmai cron status
```

`nunmai cron status` reports whether the scheduler is alive (gateway process, ticker heartbeat, last successful tick) and the soonest scheduled run across your active jobs, ordered by actual instant even when jobs store different UTC offsets. A `next_run_at` that is already more than 15 minutes in the past is never shown as an upcoming "Next run": `cron status` prints `⚠ Next run <time> is OVERDUE — passed 7h ago but the job has not fired`, `cron list` and the in-chat `/cron list` label the row `Overdue:`, the dashboard and the Desktop cron panel (including a Bot's Routines card) show `Overdue since`, and when the scheduler has stopped ticking `status` (with the gateway down) and the dashboard Cron page also say when it last ticked. That is the signature of a scheduler that stopped ticking — restart the gateway (`nunmai gateway restart`) so the next tick picks the overdue job up, or run it right away with `nunmai cron run <id>`.

For a named profile served by the default-profile multiplexer, `nunmai cron status` names that scheduler host and reports the named profile's own heartbeat health. Missing or stale heartbeats point to `nunmai --profile default gateway restart`. `cron list` and `cron create` also warn when that heartbeat is missing or stale; `cron status` additionally checks the last successful tick and reports tick errors.

### Gateway scheduler behavior

On each tick Nunmai:

1. loads jobs from `~/.nunmai/cron/jobs.json`
...
A file lock at `~/.nunmai/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
The worker is the gateway's own interpreter running `python -m cron.scheduler`, with the gateway's checkout pinned on its `PYTHONPATH` (plus any entries the gateway itself was started with), so it imports the same Nunmai tree the gateway runs — regardless of the venv's editable-install mapping, the unit's `WorkingDirectory`, or `PYTHONSAFEPATH` on the host. A worker that dies before acknowledging the handoff records its own stderr tail in the job's last error and in the execution ledger, so the failing import (or whatever killed it) is named instead of a bare exit code.

### Execution history

Nunmai records each claimed cron attempt in the profile-local
`~/.nunmai/cron/executions.db` before executor or provider dispatch. Attempts
...
`nunmai cron run` / `/cron run`, so a one-shot invocation with no scheduler
running heals the ledger too — Nunmai marks an abandoned attempt `unknown` only
...
Inspect recent attempts with `nunmai cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, Nunmai skips that replay and
...
in a row and suggesting you fix, pause (`nunmai cron pause <job>`), or remove
it. Any successful run resets the streak, and `nunmai cron list` shows the
...
(the run is still recorded — `nunmai cron runs` and the failure streak see it,
...
```bash
nunmai cron incidents                 # list incidents (newest activity first)
nunmai cron incidents --state alerted # filter: detected | alerted | resolved | closed
nunmai cron incidents ack <id>        # acknowledge — silence this signature for good
```
...
### Fleet health check: `nunmai cron doctor`

`nunmai cron doctor` is a read-only health check over every active job. It prints grouped, per-job issues and exits `1` while any finding stands, including historical late or catch-up dispatches (`0` when no findings remain).

A successful catch-up does not clear the lateness warning; the next on-time dispatch does. A watchdog such as `nunmai cron doctor || alert` can therefore keep alerting for a full schedule interval after the host wakes, even if the catch-up succeeds.
...
- script missing, not a file, or resolving outside `NUNMAI_HOME/scripts`,
...
`nunmai cron incidents` (durable failure records) and `nunmai cron runs`
...
| `"local"` | Save to local files only (`~/.nunmai/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `nunmai cron list` shows it in yellow as
`delivery_failed: <reason>`, `nunmai cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `nunmai profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
and in `nunmai cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.nunmai/cron/output/`), but no message is sent to the delivery target.
...
The run is then recorded as failed (`last_status`, failure streak, `nunmai cron runs` and `nunmai cron incidents`
all reflect it) and the failure notice is delivered like any other failed run. The full response is still saved
under `~/.nunmai/cron/output/` for triage. The marker is strict: mentioning or quoting `[CRON_FAILURE]` anywhere
...
Or set the `NUNMAI_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `NUNMAI_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$NUNMAI_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Nunmai-managed secrets are **not** inherited by cron scripts.
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.nunmai/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.nunmai/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.nunmai/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.nunmai/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Nunmai reads Job 1's most recent output from `~/.nunmai/cron/output/{job1_id}/*.md`
...
From the CLI: `nunmai cron create "every 6h" "Scan for news" --continuity`, and `nunmai cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
Before this rule, a pinned job whose provider failed could run on the first working `fallback_providers` entry instead, with a one-line notice in its output. If you relied on that, unpin the job (`nunmai cron edit <job_id> --unpin`) and set the model through `cron.model` instead.
...
Nunmai home. Its `## Error` section includes the chained traceback, with credential patterns
...
- `nunmai cron list`: a red missed-fire warning under the job
- `nunmai cron doctor`: a per-job missed-fire finding that makes the command exit `1`
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `nunmai gateway restart`).
...
`nunmai cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `nunmai config set cron.catch_up_missed false`. With this opt-out, a recurring
...
Runtimes that can't receive detached results (one-shot `nunmai -z`, `nunmai
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `nunmai tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `nunmai tools` cron-platform config wins; otherwise Nunmai falls back to the built-in defaults. If the cron-platform toolset config cannot be read at all (for example a malformed `platform_toolsets` block in `config.yaml`), the run fails with a recorded error instead of quietly running with every tool — check `nunmai cron list` / `nunmai cron doctor`. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
...
If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Nunmai should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.nunmai/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.nunmai/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.nunmai/scripts/flag-ready.sh
if test -f ~/.nunmai/cache/scratch/new-data-ready; then
  rm -f ~/.nunmai/cache/scratch/new-data-ready
  echo '{"wakeAgent": true}'
else
  echo '{"wakeAgent": false}'
fi
...
:::tip
Nunmai's own `~/.nunmai/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs are stored in `~/.nunmai/cron/jobs.json`. Output from job runs is saved to `~/.nunmai/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `nunmai update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob_manage` tool, `nunmai cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `NUNMAI_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Nunmai resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
