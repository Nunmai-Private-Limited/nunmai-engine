...
description: "How Nunmai Engine remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Nunmai Engine has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
Both are stored in `~/.nunmai/memories/` and are injected into the system prompt as a frozen snapshot at session start. The agent manages its own memory via the `memory` tool — it can add, replace, or remove entries.

:::caution One agent per Nunmai home
Don't point two agent processes at the same Nunmai home directory. Memory writes are automatic and load back into the system prompt at session start, so two writers sharing one home will compound each other's entries into state neither of them (nor you) authored. Memory is scoped per [profile](../profiles.md) by design — give a second agent its own profile, and if they need shared memory, use an [external memory provider](./memory-providers.md) instead.
:::
...
1. **Check whether the write actually happened.** Memory only persists when the model *calls the `memory` tool*; a sentence like "I've added that to my memory" is just text. Open the file and look for the entry:
   ...
   ```bash
   cat ~/.nunmai/memories/MEMORY.md
   cat ~/.nunmai/memories/USER.md
   ```
...
3. **Check you are reading the same memory you wrote.** Memory is per [profile](../profiles.md): `nunmai -p work` (or `work chat` / `work gateway start`) reads `~/.nunmai/profiles/work/memories/`, not `~/.nunmai/memories/`. A CLI session in the default profile and a Telegram bot on another profile do not share notes. `nunmai profile list` shows what exists.
...
- All CLI and messaging sessions are stored in SQLite (`~/.nunmai/state.db`) with FTS5 full-text search
...
```bash
nunmai sessions list    # Browse past sessions
```
...
The learning journey is a timeline view of everything Nunmai has learned — saved skills and memory entries plotted over time (oldest at top, newest at bottom), with a playable "constellation" scrubber that replays the build-up. The same graph data drives three surfaces:

- **Classic CLI / standalone** — `nunmai journey` (aliases: `nunmai learning`, `nunmai memory-graph`) renders the timeline in the terminal. Flags: `--play` animates the build-up (`--fps` to tune it), `--width`/`--height` override the render size, `--no-color` disables color, and `--json` dumps the raw graph payload.
...
Beyond viewing, the journey is also where you **prune and correct** what Nunmai has learned:

| Command | What it does |
|---------|--------------|
| `nunmai journey list` | List node ids — skill names and `memory:<source>:<index>` ids for memory chunks. |
| `nunmai journey delete <node> [-y]` | Delete a node. Skills are **archived** (restorable), memory chunks are removed. `-y` skips the confirmation. |
| `nunmai journey edit <node>` | Open the node's content (a skill's `SKILL.md` or the memory chunk) in `$EDITOR`. |
...
or update a skill. This is Nunmai' consent-aware learning loop: repeated
...
`~/.nunmai/pending/skills/<id>.json` when you want to read the whole change.
Full details in [Gating agent skill writes](./skills.md#gating-agent-skill-writes-skillswrite_approval).


## External Memory Providers

For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Nunmai ships with 7 external memory provider plugins — Honcho, OpenViking, Mem0, Holographic, RetainDB, ByteRover, and Supermemory — and more, such as Hindsight, are available from the [plugin catalog](plugins.md) via `nunmai plugins install <name>`.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
nunmai memory setup      # pick a provider and configure it
