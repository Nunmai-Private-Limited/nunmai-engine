...
description: "Durable SQLite-backed task board for coordinating multiple Nunmai profiles"
---
...
Nunmai Kanban is a durable task board, shared across all your Nunmai profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.nunmai/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.nunmai/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `nunmai kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `nunmai kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), reaps workers that outlived their finished run (a worker still alive after its own `kanban_complete`/`kanban_block` is terminated once its run has been closed for two minutes, leaving it time to finish its final turn — matched by PID *and* spawn-time fingerprint, so a recycled PID is never signalled; recorded as a `terminal_worker_reaped` event), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `NUNMAI_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
nunmai kanban boards list

# Create a new board.
nunmai kanban boards create atm10-server \
    --name "ATM10 Server" \
...
nunmai kanban --board atm10-server list
nunmai kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
nunmai kanban boards switch atm10-server
nunmai kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
nunmai kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `NUNMAI_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.nunmai/kanban/current` — the slug persisted by `nunmai kanban
   boards switch`.
4. `default`.
...
`nunmai dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Project** (in both modals) — binds the board to a Nunmai Project
...
- **Storage** — files land under
  `<nunmai-home>/kanban/attachments/<task_id>/` for the default board, or
  `<nunmai-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `NUNMAI_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
- **Download / remove** — the drawer lists each attachment with a download
  ...
  last reference is removed). From the CLI, `nunmai kanban attach-rm
...
nunmai gateway start

# 3. Create a task (you — or an orchestrator agent via kanban_create)
nunmai kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `nunmai kanban show t_abcd`.
...
Override the config flag at runtime via `NUNMAI_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `nunmai gateway
...
until one comes up — `nunmai kanban create` warns about this at creation
time.
...
its own transient scope (`systemd-run --user --scope --unit nunmai-worker-kanban-<task>-run-<run>`)
...
- **`nunmai kanban dispatch` from your own unit** (a `Type=oneshot` timer, a
...
Running `nunmai kanban daemon` as a separate process is **deprecated**;
...
Mounting one `kanban.db` in several Nunmai homes (containers, fleet hosts) shares
...
nunmai kanban create "nightly ops review" \
    --assignee ops \
...
```bash
nunmai kanban complete t_abc t_def t_hij --result "batch wrap"
nunmai kanban archive  t_abc t_def t_hij
nunmai kanban unblock  t_abc t_def
nunmai kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
nunmai -p planner tools enable kanban                      # CLI / TUI / Desktop chats
nunmai -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `nunmai tools` and the dashboard.
...
**Workers do not shell out to `nunmai kanban`.** When the dispatcher spawns a worker it sets `NUNMAI_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `nunmai kanban` CLI.
...
kanban_show()                                     # no args — uses NUNMAI_KANBAN_TASK
...
### Why tools instead of shelling to `nunmai kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `nunmai kanban complete` *inside* the container, where `nunmai` isn't installed and `~/.nunmai/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.nunmai/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `nunmai chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `NUNMAI_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.

The auto-injected kanban guidance teaches the model which tool to call when and in what order. It is injected only for the dispatcher-spawned worker that owns the task: an interactive session that merely has the `kanban` toolset enabled keeps the tools but is not told it "has been assigned ONE task", and a `delegate_task` child or a cron run fired from inside a worker — which inherit the worker's `NUNMAI_KANBAN_TASK` — neither receives the worker protocol nor records a terminal outcome against the worker's card when its own turn budget runs out.
...
```json
{
  ...
  "verification": ["pytest tests/nunmai_cli/test_kanban_db.py -q"],
...
2. `cd $NUNMAI_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
revoked key parks the card exactly like an implementer. `nunmai kanban show`
surfaces it as *Provider rejected this profile's credential or model — blocked
after one attempt*; fix the assignee profile's provider (`nunmai -p <profile>
auth` / `setup`), then `nunmai kanban unblock <id>`. The worker also writes its exit code as the
last line of its own log (`[kanban-worker-exit] rc=<code>`), so a per-tick
`nunmai kanban dispatch` process — which never reaped the worker and cannot
...
**Agent-side prevention:** Before the worker exits, Nunmai injects up to two
...
guard is active only for the dispatcher-spawned worker itself (`NUNMAI_KANBAN_TASK` is
...
and can be disabled with `NUNMAI_KANBAN_STOP_NUDGE=0`.
...
breaker block) until `nunmai kanban unblock <id>`, which also grants a fresh
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `nunmai sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
```bash
nunmai kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

nunmai kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `nunmai skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
nunmai kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
nunmai kanban set-model t_abcd claude-opus-4.6 --provider anthropic
nunmai kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.nunmai/profiles/<name>/`, and the dispatcher injects the profile-scoped `NUNMAI_HOME` when it spawns `nunmai -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.nunmai/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.nunmai/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.nunmai/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `nunmai kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
nunmai kanban create "Translate the docs site to French" \
...
A goal-mode worker's **Worker log** (dashboard drawer, `nunmai kanban log <id>`) carries the same live tool feed as any other worker's, plus one `kanban goal loop: turn N/M verdict=…` line per judged turn, so you can follow what the loop is doing while the card is running.
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `nunmai kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `nunmai kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `nunmai profile create --description "..."`, `nunmai profile describe <name> --text "..."`, `nunmai profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee`, else the root task's assignee (if it names an existing profile), else the active default profile.
...
Config knobs (all under `kanban:` in `~/.nunmai/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `nunmai profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.nunmai/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
```
nunmai kanban init                                     # create kanban.db + print daemon hint
nunmai kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
nunmai kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
nunmai kanban show <id> [--json]
nunmai kanban assign <id> <profile>                    # or 'none' to unassign
nunmai kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
nunmai kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
nunmai kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
nunmai kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
nunmai kanban diagnostics [--json]                     # board health snapshot (alias: diag)
nunmai kanban link <parent_id> <child_id>
nunmai kanban unlink <parent_id> <child_id>
nunmai kanban claim <id> [--ttl SECONDS]
nunmai kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
nunmai kanban complete <id>... [--result "..."] [--force]
nunmai kanban block <id> "<reason>" [--ids <id>...]
nunmai kanban unblock <id>...
nunmai kanban archive <id>...

nunmai kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
nunmai kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
nunmai kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

nunmai kanban tail <id>                                # follow a single task's event stream
nunmai kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
nunmai kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
nunmai kanban runs <id> [--json]                       # attempt history (one row per run)
nunmai kanban assignees [--json]                       # profiles on disk + per-assignee task counts
nunmai kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
nunmai kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `nunmai gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
nunmai kanban stats [--json]                           # per-status + per-assignee counts
nunmai kanban log <id> [--tail BYTES]                  # worker log from ~/.nunmai/kanban/logs/
nunmai kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
nunmai kanban notify-list [<id>] [--json]
nunmai kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
nunmai kanban context <id>                             # what a worker sees
nunmai kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
nunmai kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
nunmai kanban create "nightly backup audit" \
  --assignee ops --scheduled-at "2026-06-01T03:00:00Z"
```

### Respawn guard

The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR (`active_pr`). Two cooldowns hold a card without ever counting against it: `rate_limit_cooldown` after a quota-wall requeue and `infrastructure_cooldown` after the host refused to place the worker (no restart-safe systemd scope — see [Workers and systemd cgroups](#workers-and-systemd-cgroups)); both share the `NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS` window (default 300 s). This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).

To see why a ready card is not spawning, run `nunmai kanban dispatch --dry-run` — the output lists `Guarded (<reason>): <task id>` per held card (and `respawn_guarded`, `rate_limited`, `skipped_locked`, `memory_pressure` with `--json`). The gateway's and the standalone daemon's "dispatcher stuck" warning also names what the last tick held back, e.g. `Last tick held back: active_pr=1`.

`recent_success` and `active_pr` hold the **ready** lane only — they are the inputs to a review handoff, not signals against one. To have a reviewer, closer or other recovery profile pick up a card whose PR is already open, either move it to the review lane with `nunmai kanban request-review <id>` (accepted from `ready` as well as `running`; the review-lane spawn is not subject to either guard) or hand the ready card to that profile with `nunmai kanban assign <id> <profile>`: a handoff recorded *after* the PR comment — an operator reassign, a reviewer's changes-requested verdict, or a review reopen — lifts `active_pr` for the profile now named on the card, because that PR is exactly what it must work on. Only a change to a *different* profile counts: re-assigning the same profile, unassigning, or the dispatcher's own `kanban.default_assignee` fill-in does not lift the guard, so the assignee that opened the PR is still not re-spawned against it after a crash, reclaim or no-op reassign, and a newer PR comment posted after the handoff guards again. A deliberate re-queue after a success (drag `done→ready`, `unblock`, re-promotion) also lifts `recent_success`, so a manual re-run is never silently held for the whole window.
...
Every `nunmai kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `nunmai chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `nunmai_cli.kanban.run_slash()` entry point that reuses the `nunmai kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `nunmai kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.nunmai/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `nunmai kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`nunmai kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `nunmai kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
_metadata_: `{"changed_files": ["nunmai_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
nunmai kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
```
hotspot: nunmai_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
nunmai kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
nunmai kanban notify-list
nunmai kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
nunmai kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
nunmai kanban runs t_abcd
...
**Bulk close caveat.** `nunmai kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.

**Dependency refusal on complete names the parents.** `kanban_complete` / `nunmai kanban complete` / the dashboard's "mark done" and "request review" actions (single and bulk) on a card whose direct parent is not `done`/`archived` (a parent reopened mid-run, or an edge that predates the running-child refusal) reports `unsatisfied parent dependencies: t_… (todo)` instead of the generic "unknown id, stale run, or already terminal" text; the card stays in-flight. `kanban_show` lists the same parents under `unsatisfied_parents`, and `nunmai kanban show` / `nunmai kanban diagnostics` / the dashboard raise a `running_with_open_parents` warning on a running card in that state. Finish the parent or `nunmai kanban unlink <parent> <child>`; there is no force path through the dependency gate.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `nunmai kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `nunmai kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `nunmai kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`nunmai kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `nunmai kanban heartbeat $TASK` to signal liveness during long operations. |
...
`nunmai kanban tail <id>` shows these for a single task. `nunmai kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.nunmai/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
