...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Nunmai automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Nunmai Portal](../../integrations/nous-portal.md) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Nunmai auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
nunmai auth add openrouter --api-key sk-or-v1-your-second-key

# ...or let a browser login mint one (OpenRouter OAuth PKCE; stored as a plain API key)
nunmai auth add openrouter --type oauth

# Add a second Anthropic key
nunmai auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
nunmai auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  nunmai_pkce          oauth   id=ab12cd34 priority=0 nunmai_pkce ←
...
`nunmai auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `nunmai auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Nunmai, Codex), the add flow asks which type:
...
Each `nunmai auth add openai-codex` login becomes its own pool entry, but only **different** OpenAI accounts rotate independently: two logins of the same account share one token family upstream, so OpenAI revokes the older one and the second entry adds no quota. Nunmai warns at add time (`warning: this login is the same OpenAI account as openai-codex credential #N`) — log into a different account, or keep just one.

## CLI Commands

| Command | Description |
|---------|-------------|
| `nunmai auth` | Interactive pool management wizard |
| `nunmai auth list` | Show all pools and credentials |
| `nunmai auth list <provider>` | Show a specific provider's pool |
| `nunmai auth add <provider>` | Add a credential (prompts for type and key) |
| `nunmai auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `nunmai auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `nunmai auth add openai-codex --browser` | Codex only: sign in with the browser authorization-code + PKCE flow on `localhost:1455` instead of the default device code (for orgs that disable device-code grants); falls back to device code when the port is busy. Default for every Codex login via `auth.codex_login_flow: browser` |
| `nunmai auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `nunmai auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `nunmai auth remove <provider> <index>` | Remove credential by 1-based index |
| `nunmai auth reset <provider>` | Clear all cooldowns/exhaustion status (applies to running sessions too: a live gateway or chat picks the reset up on its next request instead of writing its stale cooldown back) |
| `nunmai auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `nunmai auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |

For Nunmai, `auth refresh` supports only the login's `device_code` singleton.
Independent Nunmai pool accounts are rejected before refresh; their tokens and
cooldowns are preserved. Reauthenticate with `nunmai auth add nous --type oauth`
...
Refresh-only lookups and peeks do not count. Status reads (`nunmai doctor`, the `/model`
...
Configure via `nunmai auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`nunmai auth priority` changes it) |
...
```bash
nunmai auth list openai-codex                 # find the index, id or label
nunmai auth priority openai-codex 1 99        # 1-based index, entry id, or exact label; large n = last
nunmai auth priority openai-codex work-seat 0 # ...and back to the front later
```

`nunmai auth priority <provider> <target> <priority>` reorders one credential and renumbers the
...
`nunmai auth reset` to clear; it is not refreshed on a timer, and it is still used once every
...
| **400 Codex model entitlement** (`The '<model>' model is not supported when using Codex with a ChatGPT account.`) | Bench this key for the rejected model only and rotate to the next key; other models keep using the key. Other 400s never rotate | Until `nunmai auth reset` (per model; an entitlement is a plan property, not a window) |
...
holding the same login rotated it first — or, for Nunmai, the profile holds no Portal login or token
pair to refresh with), the pool logs one WARNING naming the entry and the repair
command (`nunmai auth add <provider>`), and the credential leaves rotation — marked `dead`, or dropped
when it only mirrored a token file the pool has just cleared — until you sign in again. This applies to Anthropic, Codex, xAI
and Nunmai OAuth logins alike. A dead credential never re-enters rotation on a timer, so a lost login
shows up once in the log instead of failing quietly every hour.

**Every Codex login in an always-on home is dead: sign in again, do not wait for adoption.** Nunmai
...
without a Codex credential until you run `nunmai auth add openai-codex` in *that* home
(`nunmai -p <profile> auth add openai-codex` for a named profile), which offers the Codex CLI import
interactively. Profiles that should share one login can point at the same `NUNMAI_HOME` instead of
...
the remaining cooldown (or the `nunmai auth add <provider>` re-login for a dead one) — the first-run
...
When you set up a custom endpoint via `nunmai model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via nunmai model:
nunmai auth list
# Shows:
...
nunmai auth add Together.ai --api-key sk-together-second-key
```
...
Nunmai automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| OAuth tokens (auth.json) | Codex device code, Nunmai device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Nunmai PKCE OAuth | `~/.nunmai/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `nunmai auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `nunmai auth add`) are never auto-pruned.
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Nunmai can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Nunmai-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`nunmai_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`nunmai_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.nunmai/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Nunmai' credential store, so its token remains persistable.

An `env:` row is re-hydrated from the environment on every load, and the variable name does not have to be one Nunmai declares for the provider: numbered siblings (`OPENROUTER_API_KEY_2`, see [Auto-Discovery](#auto-discovery)) appear here automatically, and a hand-written row pointing at any other variable is filled the same way, without the secret ever being written to `auth.json`.
