...
Nunmai Engine ships with 7 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md, and more (such as Hindsight) are available from the [plugin catalog](./plugins.md). Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
nunmai memory setup      # interactive picker + configuration
nunmai memory status     # check what's active
nunmai memory off        # disable external provider
```

You can also select the active memory provider via `nunmai plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.nunmai/config.yaml`:

```yaml
memory:
  provider: openviking   # or honcho, mem0, holographic, retaindb, byterover, supermemory,
                         # or hindsight (plugin catalog — run `nunmai plugins install hindsight` first)
```

## How It Works

When a memory provider is active, Nunmai automatically:
...
**Setup Wizard:**
```bash
nunmai memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `nunmai honcho setup` command still works (it now redirects to `nunmai memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$NUNMAI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$NUNMAI_HOME/honcho.json` > `~/.nunmai/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/nunmai).
...
:::tip Migrating from `nunmai honcho`
If you previously used `nunmai honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Nunmai profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Nunmai profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Nunmai profile. Host key `nunmai` → default; `nunmai.<profile>` for others. |
...
```bash
nunmai profile create coder --clone
```

`--clone` creates a `nunmai.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Nunmai profile, creates host blocks for any profile without one, inherits settings from the default `nunmai` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `nunmai memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/nunmai).
...
nunmai memory setup    # select "openviking"
# Or manually:
nunmai config set memory.provider openviking
```

`nunmai memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.nunmai/.env`, and for named profiles use
`~/.nunmai/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Nunmai sends no peer ID and writes
...
Set `memory.openviking.agent: nunmai` to restore the old peer-scoped writes.
...
Nunmai sends `User-Agent: openviking-memory-nunmai/<version>` on OpenViking
...
**Setup (Platform):**
```bash
nunmai memory setup    # select "mem0" → "Platform"
# Or manually:
nunmai config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.nunmai/.env
```

**Setup (OSS):**
```bash
nunmai memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
nunmai memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
nunmai memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
nunmai memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
nunmai memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.nunmai/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.nunmai/.env
```
...
**Config:** `$NUNMAI_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.nunmai/.env`.
...
| `user_id` | `nunmai-user` | User identifier |
| `agent_id` | `nunmai` | Agent identifier |
...
**Switching modes:** Re-run `nunmai memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
:::info Plugin catalog
Hindsight is maintained by [vectorize-io](https://github.com/vectorize-io/hindsight) and installed from the [plugin catalog](./plugins.md) rather than bundled with Nunmai. Setup details live in the upstream docs: [hindsight.vectorize.io/sdks/integrations/nunmai](https://hindsight.vectorize.io/sdks/integrations/nunmai).
:::
...
| **Requires** | `nunmai plugins install hindsight`. Cloud: API key from [ui.hindsight.vectorize.io](https://ui.hindsight.vectorize.io). Local: LLM API key (OpenAI, Groq, OpenRouter, etc.) |
...
**Setup:**
```bash
nunmai plugins install hindsight   # from the plugin catalog
nunmai memory setup                # select "hindsight"
# Or manually:
nunmai config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.nunmai/.env
```

The plugin lands in `~/.nunmai/plugins/hindsight/` (per profile home) and is enabled under `plugins.enabled` in `config.yaml`. `nunmai memory setup`, `nunmai memory status`, `nunmai plugins list` and the dashboard Memory settings all work with the catalog-installed plugin. In local embedded mode the plugin installs `hindsight-all` on first use through Nunmai' lazy-install path, which honours `security.allow_lazy_installs`.

**Local mode UI:** `hindsight-embed -p nunmai ui start`

**Config:** `$NUNMAI_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `nunmai` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Nunmai Engine and the User` | Context label for retained memories |
...
See the [upstream Nunmai integration docs](https://hindsight.vectorize.io/sdks/integrations/nunmai) for the full configuration reference.

#### Migrating from bundled Hindsight

Hindsight used to ship inside the Nunmai tree (and as the `nunmai-engine[hindsight]` pip extra). If your `config.yaml` already has `memory.provider: hindsight`, there is nothing to do for most users:

- `nunmai update` installs the catalog plugin into every profile home that names the provider (this runs even when `security.allow_lazy_installs` is `false`).
- If the plugin is still missing on the first agent start (`nunmai chat`, the gateway, …), Nunmai installs it and prints `✓ Memory provider 'hindsight' moved out of core — installed its plugin from the catalog (your memory.hindsight settings and data are unchanged).`
- With `security.allow_lazy_installs: false`, the agent-start path instead logs one line — ``Memory provider 'hindsight' is not installed; security.allow_lazy_installs is off — run `nunmai plugins install hindsight`.`` — and you run `nunmai plugins install hindsight` yourself.

What changes on disk: the plugin appears in `~/.nunmai/plugins/hindsight/` and `config.yaml` gains `plugins.enabled: [hindsight]`. `memory.provider`, `memory.hindsight.*`, `$NUNMAI_HOME/hindsight/config.json`, `HINDSIGHT_API_KEY` in `.env` and your memory bank data are untouched. Verify with `nunmai memory status` (provider active) and `nunmai plugins list` (plugin installed and enabled).
...
**Setup:**
```bash
nunmai memory setup    # select "holographic"
# Or manually:
nunmai config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.nunmai-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$NUNMAI_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
nunmai memory setup    # select "retaindb"
# Or manually:
nunmai config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.nunmai/.env
```
...
# Then configure Nunmai
nunmai memory setup    # select "byterover"
# Or manually:
nunmai config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$NUNMAI_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=nunmai), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
nunmai memory setup    # select "supermemory"
# Or manually:
nunmai config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.nunmai/.env
```
...
Before running `nunmai memory setup`, set `base_url` in
`$NUNMAI_HOME/supermemory.json`:
...
Then run `nunmai memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$NUNMAI_HOME/supermemory.json`
...
| `container_tag` | `nunmai` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `nunmai-{identity}` → `nunmai-coder`) to isolate memories per Nunmai profile
...
| **Requires** | `pip install nunmai-memori` + `nunmai-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install nunmai-memori
nunmai-memori install
nunmai config set memory.provider memori
nunmai memory setup
```
...
| **Hindsight** (plugin catalog) | Cloud/Local | Free/Paid | 3 | `nunmai plugins install hindsight` | Knowledge graph + reflect synthesis |
...
| **Memori** | Cloud | Free/Paid | 5 | `nunmai-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](../profiles.md):

- **Local storage providers** (Holographic, ByteRover) use `$NUNMAI_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$NUNMAI_HOME/` so each profile has its own credentials
- **Cloud providers** (RetainDB) auto-derive profile-scoped project names
- **Env var providers** (OpenViking) are configured via each profile's `.env` file

## Providers Moving to the Plugin Catalog

Memory providers are moving out of the Nunmai tree into their maintainers' own repositories,
...
When a provider you have configured stops shipping with Nunmai, `nunmai update` installs its
...
`false`, in which case it logs the `nunmai plugins install <name>` one-liner instead).
