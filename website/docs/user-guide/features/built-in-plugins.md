...
description: "Plugins shipped with Nunmai Engine that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Nunmai ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.nunmai/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](./plugins.md) page for the general plugin system, and [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) to write your own.
...
2. **User** — `~/.nunmai/plugins/<name>/`
3. **Project** — `./.nunmai/plugins/<name>/` (requires `NUNMAI_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `nunmai_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `nunmai memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `nunmai plugins list` and the interactive `nunmai plugins` UI), but none load until you explicitly enable them:

```bash
nunmai plugins enable disk-cleanup
```

Or via `~/.nunmai/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Nunmai. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
nunmai plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `nunmai plugins enable <name>`.
...
| `nunmai-achievements` | dashboard tab | Steam-style collectible badges generated from your real Nunmai session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `nunmai memory` and `nunmai plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NUNMAI_HOME` or `/tmp/nunmai-*`, track it silently as `test` / `temp` / `cron-output`. | <!-- no-tmp: ok — documents the plugin's scope guard -->
...
| empty dirs under NUNMAI_HOME | always | Never |
...
**State** — everything lives at `$NUNMAI_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `NUNMAI_HOME` or `/tmp/nunmai-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end. User project trees (`workspace/`, `projects/`, `plans/`, `home/`) are never tracked or swept at all: a `test_*.py` or `tmp_*` file inside your project is source code, not scratch. `kanban/` (task attachments and workspaces) is never tracked either, and a tracked *directory* under a protected top level such as `cache/` is never removed — only the files inside it age out.
...
**Enabling:** `nunmai plugins enable security-guidance` (or check the box in `nunmai plugins`).

**Disabling again:** `nunmai plugins disable security-guidance`.
...
Traces Nunmai turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Nunmai' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `nunmai logs`.
...
```bash
nunmai tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Nunmai and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
nunmai plugins enable observability/langfuse
```

Then put the credentials in `~/.nunmai/.env`:

```bash
NUNMAI_LANGFUSE_PUBLIC_KEY=pk-lf-...
NUNMAI_LANGFUSE_SECRET_KEY=sk-lf-...
NUNMAI_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Nunmai turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `NUNMAI_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Nunmai session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `nunmai chat` session lives under one Langfuse session.

**Verify:**

```bash
nunmai plugins list                 # observability/langfuse should show "enabled"
nunmai chat -q "hello"              # check the Langfuse UI for a "Nunmai turn" trace
```
...
| `NUNMAI_LANGFUSE_RELEASE` | — | Release/version tag |
| `NUNMAI_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `NUNMAI_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `NUNMAI_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Nunmai-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Nunmai-prefixed wins when both are set.
...
**Disabling:** `nunmai plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Nunmai plugin. Do not run `nunmai plugins enable observability/nemo_relay`; Nunmai core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `NUNMAI_NEMO_RELAY_PLUGINS_TOML` to that file before starting Nunmai. The policy is process-wide for every profile hosted by that Nunmai process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `NUNMAI_NEMO_RELAY_ATOF_*` and `NUNMAI_NEMO_RELAY_ATIF_*` settings no longer activate exporters — a `.env` that still carries them (and no `NUNMAI_NEMO_RELAY_PLUGINS_TOML`) exports **nothing**, and the gateway logs one warning saying so. `nunmai doctor` reports these stale settings when no replacement `plugins.toml` is selected.

**Automatic migration.** `nunmai update` (and `nunmai migrate relay`, or `nunmai migrate relay --all-profiles` for every profile home) converts the legacy variables into `<nunmai home>/relay-plugins.toml`, sets `NUNMAI_NEMO_RELAY_PLUGINS_TOML` in that profile's `.env`, and comments the legacy lines out (nothing is deleted). Under a multiplexed gateway every profile home gets its own file. The generated file is validated through Relay before it is written; this is the shape it produces (note the `type = "file"` sink discriminator — a sink without it is rejected):
...
type = "file"
output_directory = "/home/you/.nunmai/telemetry/nemo-relay/atof"
filename = "nunmai-atof.jsonl"
mode = "append"
...
model_name = "unknown"
output_directory = "/home/you/.nunmai/telemetry/nemo-relay/atif"
...
Then add `NUNMAI_NEMO_RELAY_PLUGINS_TOML=/home/you/.nunmai/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `nunmai.session.segment` plus `nunmai.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.nunmai/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
nunmai plugins enable google_meet
nunmai meet setup   # preflight: playwright, chromium, auth file
nunmai meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `nunmai plugins disable google_meet`. Any saved transcripts stay in `~/.nunmai/workspace/meetings/` until you remove them.

### nunmai-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Nunmai session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Nunmai feature changes.

**How it works:**

- Scans your entire `~/.nunmai/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$NUNMAI_HOME/plugins/nunmai-achievements/state.json`
...
| Secret | Hidden until Nunmai detects the first related signal in your history |

**API** — routes mount under `/api/plugins/nunmai-achievements/`:
...
**State files** — live under `$NUNMAI_HOME/plugins/nunmai-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Nunmai updates. |
...
**Enabling:** Nothing to enable — `nunmai-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `nunmai dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/nunmai-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.nunmai/plugins/nunmai-achievements/` that ships no dashboard. The plugin's state files under `$NUNMAI_HOME/plugins/nunmai-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Nunmai plugin — see [Build a Nunmai Plugin](../../developer-guide/plugins/index.md). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.nunmai/plugins/<name>/`
- Manifest source is reported as `bundled` in `nunmai plugins list`
