...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Nunmai Plugin](/developer-guide/plugins) to write your own.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NUNMAI_HOME` or `/tmp/nunmai-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `NUNMAI_HOME` or `/tmp/nunmai-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
The old `NUNMAI_NEMO_RELAY_ATOF_*` and `NUNMAI_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `nunmai doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Bundled plugins are written exactly like any other Nunmai plugin — see [Build a Nunmai Plugin](/developer-guide/plugins). The only differences are:
