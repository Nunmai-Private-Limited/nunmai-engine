...
When Nunmai Engine runs inside a messaging gateway (Slack, Discord, Telegram,
...
2. **The gateway scans agent responses for file paths.** Any absolute path
   (`~/.nunmai/cache/scratch/...`) or home-relative path (`~/...`) ending in a supported
...
persona in `~/.nunmai/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nunmai/config.yaml` (switchable per session
via `/personality`).

The mechanic the agent has to use is simple: render the file to an
absolute path (e.g. `~/.nunmai/cache/scratch/q3-revenue.png`) and mention that path as
...
If you use Nunmai' kanban multi-agent workflow, workers can attach
...
kanban_complete(
    ...
    artifacts=[
        "~/.nunmai/cache/scratch/q3-revenue.png",
        "~/.nunmai/cache/scratch/q3-report.pdf",
    ],
)
```
...
Install MCP servers via `~/.nunmai/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Nunmai Engine's deliverable
