...
fills the password through Nunmai. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Nunmai generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `nunmai vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Nunmai
...
in, Nunmai picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Nunmai hands the master password to the manager's
...
Prefer not to use a detected manager? `nunmai vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `nunmai vault add`), bound to the checkout site,
...
- **CLI**: `nunmai vault list`, `nunmai vault add`, `nunmai vault rm <handle>`,
  `nunmai vault sources`.

Items live encrypted under `~/.nunmai/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `nunmai chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Nunmai: not in
