...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.nunmai/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Declare shell-script hooks in your `~/.nunmai/config.yaml` and Nunmai will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
