...
Nunmai has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.nunmai/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.nunmai/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
LOG_FILE = Path.home() / ".nunmai" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.nunmai/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.nunmai/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".nunmai" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.nunmai/hooks/session-webhook/HOOK.yaml
...
A popular pattern from the community: drop a Markdown checklist at `~/.nunmai/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Nunmai does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.nunmai/BOOT.md` with natural-language startup instructions.
...
Create `~/.nunmai/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `nunmai cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.nunmai/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.nunmai/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.nunmai/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.nunmai/hooks/boot-md/handler.py`**

```python
"""Run ~/.nunmai/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".nunmai" / "BOOT.md"
...
```bash
nunmai logs --follow --level INFO | grep boot-md
```
...
Delete `~/.nunmai/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Nunmai shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.nunmai/hooks/`
...
- Runtime event-name validity comes from `nunmai_cli.plugins.VALID_HOOKS`. `nunmai hooks list` lists configured shell/outbound hooks, not every available event; `nunmai hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current Nunmai session id |
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Nunmai's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, Nunmai may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "Nunmai, Teknium, Nunmai Research, kanban"
...
Fires **once per turn** after the tool-calling loop completes and the model has produced a final response, **before** that response is delivered to the user (CLI, gateway, or programmatic caller) and **before** the assistant row is persisted — the replacement is what the session stores, what `/resume` shows and what the next turn replays, so the transcript never diverges from what the user saw. Nunmai' own trailers (the file-mutation warning, the abnormal-exit note) are appended afterwards and are not part of `response_text`. Lets a plugin rewrite the assistant's final text using classical-programming methods — no extra inference tokens burned on SOUL flavor text or a skill-driven transform.
...
Declare shell-script hooks in your profile's `config.yaml` and Nunmai will run them as subprocesses whenever the corresponding plugin-hook event fires — in CLI, gateway, Desktop, TUI, and dashboard chat sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`nunmai_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.nunmai/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.nunmai/agent-hooks/` (by convention) | `~/.nunmai/plugins/<name>/` | `~/.nunmai/hooks/<name>/` |
...
On Windows, a `command` that starts with an existing script file — the `~/.nunmai/agent-hooks/x.sh` shape the examples below use — is spawned through that file's own interpreter (Git Bash for `.sh`/`.bash`, the running Nunmai Python for `.py`), because `CreateProcess` has no shebang support and rejects a bare script with `WinError 193`. Every other command, and every POSIX platform, passes `argv` straight to `Popen`, where the kernel already honours the shebang.

### JSON wire protocol

Each time the event fires, Nunmai spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
`profile` names the Nunmai profile that fired the hook (`"default"` outside profiles), so one
script can serve every profile behind a multiplexed gateway; the subprocess also runs with that
profile's `NUNMAI_HOME`. `tool_name` and `tool_input` are `null` for non-tool events (`pre_llm_call`, `subagent_stop`, session lifecycle). The `extra` dict carries all event-specific kwargs (`user_message`, `conversation_history`, `child_role`, `duration_ms`, …). Unserialisable values are stringified rather than omitted.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Nunmai-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // Nunmai-canonical
{"decision": "modify", "tool_input": {"new_string": "fixed content"}} // Claude-Code style

// Escalate a pre_tool_call to the human-approval gate (Nunmai-only; `message` and `rule_key`
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.nunmai/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `nunmai hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.nunmai/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nunmai/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.nunmai/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.nunmai/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.nunmai/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nunmai/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Nunmai event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.nunmai/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nunmai/agent-hooks/log-orchestration.sh
log=~/.nunmai/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Nunmai sees it, then persists the decision to `~/.nunmai/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `nunmai --accept-hooks chat`)
2. `NUNMAI_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.nunmai/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `nunmai hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.nunmai/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/nunmai/.nunmai/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `nunmai hooks list`.

### The `nunmai hooks` CLI

| Command | What it does |
|---------|--------------|
| `nunmai hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `nunmai hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `nunmai hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `nunmai hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.nunmai/agent-hooks/` so the path is easy to audit.
- Re-run `nunmai hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](../messaging/webhooks.md): inbound webhooks wake Nunmai when the world changes; outbound webhooks tell the world when Nunmai does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Nunmai POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* Nunmai instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.nunmai/config.yaml`:
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.nunmai/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `nunmai hooks list`).

### Wire format

Each firing POSTs a JSON body with the same top-level shape as shell hooks' stdin, plus delivery metadata. `profile` names the Nunmai profile that emitted the event (`"default"` outside profiles), so receivers behind a multiplexed gateway can tell profiles apart:
...
| `X-Nunmai-Event` | The hook event name |
| `X-Nunmai-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Nunmai-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Nunmai-Delivery` header) — remember recently seen ids and skip duplicates. Nunmai retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `NUNMAI_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`nunmai hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
