...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `nunmai tools`.
...
Nunmai Engine generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `nunmai tools` and persists in `config.yaml`.
...
If you have a paid [Nunmai Portal](https://portal.nousresearch.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `nunmai setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nunmai Subscription** as the image-gen backend via `nunmai tools`.

The **Nunmai Subscription** row is the only managed row. Its model picker spans every gateway the subscription runs — the FAL catalog above, native **Krea 2** (`krea-2-medium`, `krea-2-large`, `krea-2-medium-turbo`) and any Nunmai Portal image models — each model listed once, and the model you pick decides which gateway serves the request. Free tool-pool accounts see the FAL models only; Krea and Portal models are paid-subscription.

If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `nunmai tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
Navigate to **🎨 Image Generation**, pick your backend (Nunmai Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
```yaml
image_gen:
  provider: fal                 # `nous` if you picked Nunmai Subscription
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: `nous` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: nous`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via nunmai tools), but FAL_KEY is not set. Run 'nunmai tools' to change it.` rather than silently rerouting. Change providers via `nunmai tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nous` when `true`, but never written anymore.)

`max_parallel_requests` defaults to `4`. Nunmai clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Nunmai
...
Nunmai Portal proxies the chat-completions protocol only, so its picker offers
...
only for now; responses are saved to `$NUNMAI_HOME/cache/images/`.

## FAL: GPT Image 2.5

Select **GPT Image 2.5 Flare** or **GPT Image 2.5 Sunburst** under
`nunmai tools` → Image Generation → FAL.ai. The model IDs are:
...
```bash
nunmai config set image_gen.provider fal
nunmai config set image_gen.model openai/gpt-image-2.5/flare/text-to-image
```
...
Both accept up to 16 source images. Nunmai pins quality to `medium`, matching its
...
Select them through `nunmai tools` → Image Generation → OpenAI, or set:

```bash
nunmai config set image_gen.provider openai
nunmai config set image_gen.openai.model gpt-image-2.5-flare
```
...
`OPENAI_API_KEY` is required. Requests go through Nunmai' own HTTP client, which
...
Nunmai posts straight to the Codex backend's native
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Nunmai gateway, according to the stored `image_gen.provider` selection.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / Nunmai Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Nunmai materializes them to the local cache so delivery still works after expiry
