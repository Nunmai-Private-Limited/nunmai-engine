...
```bash
curl -fsSL https://nunmai.in/install.sh | bash -s -- --no-skills
```
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Nunmai to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.nunmai/plans/` relative to the active workspace/backend working directory.
...
- **Create locally, update in place**: New agent-created skills are written to `~/.nunmai/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
