...
If no backend has **ever** been selected (no `web.backend` / per-capability key written by you or `nunmai tools`), Nunmai picks the first available one based on which credentials are set:
