...
Nunmai runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Nunmai
...
1. Nunmai captures a baseline of current diagnostics for the file.
...
…). Nunmai auto-detects the binary on PATH or in
`<NUNMAI_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Nunmai at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<NUNMAI_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`nunmai lsp status` reports `installed` once `pwsh` is found; if the
...
Nunmai launches it as `laravel-lsp lsp` (stdio). There is no
auto-install recipe; `nunmai lsp status` shows `manual-only` until the
binary is found.
...
tree — Nunmai installs `typescript@6` (the last JavaScript-based line;
TypeScript 7 is the Go port and ships no `tsserver.js`) together with
the server when you run `nunmai lsp install typescript` /
`nunmai lsp install vue-language-server` or auto-install fires on first use.
...
Neovim setup) that Nunmai's generic client does not run, so it never
publishes diagnostics. If an earlier Nunmai installed 3.x, the log shows a
one-time `vue-language-server: ... 3.x` warning; delete
`<NUNMAI_HOME>/lsp/node_modules/@vue` and `<NUNMAI_HOME>/lsp/bin/vue-language-server*`,
then run `nunmai lsp install vue-language-server` (the recipe co-installs the
TypeScript SDK).

## CLI

```
nunmai lsp status          # service state + per-server install status
nunmai lsp list            # registry, optionally --installed-only
nunmai lsp install <id>    # eagerly install one server
nunmai lsp install-all     # try every server with a known recipe
nunmai lsp restart         # tear down running clients
nunmai lsp which <id>      # print resolved binary path
```

`nunmai lsp status` is the best starting point — it shows which
...
lsp:
  ...
  # rest of the process (until `nunmai lsp restart`); N = retried after
  ...
  #   auto    — install via npm/pip/go install into <NUNMAI_HOME>/lsp/bin
  #   manual  — only use binaries already on PATH
  install_strategy: auto

  # Node package manager for the npm-based servers: npm (default), pnpm
  # or yarn. Installs still land in <NUNMAI_HOME>/lsp/node_modules; a
  ...
  # in <NUNMAI_HOME>/lsp/.yarnrc.yml. pnpm 11 blocks git-hosted transitive
  ...
  # <NUNMAI_HOME>/lsp/.npmrc if your policy allows it.
  package_manager: npm
...
built-ins, so they can also take over an extension Nunmai already
handles.
...
give an absolute path) and `nunmai lsp status` lists it as
`installed`. A malformed entry is logged and skipped without
affecting the other servers.

## Installation locations

When `install_strategy: auto`, Nunmai installs binaries into
`<NUNMAI_HOME>/lsp/bin/`. NPM packages land in
`<NUNMAI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Nunmai-owned and is
...
default the pair stays broken until `nunmai lsp restart` or process
...
are not idle: removing a Nunmai-managed worktree (`nunmai -w` session
...
exists on disk (deleted outside Nunmai). The sweep is part of the idle
...
**single process** per Nunmai process: the first Python project spawns
...
**`nunmai lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NUNMAI_HOME>/lsp/bin/`. Run
`nunmai lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nunmai lsp status`**
...
When `nunmai lsp status` shows a `Backend warnings` section, install
...
`~/.nunmai/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.nunmai/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nunmai lsp restart` to clear the set;
