...
Desktop and other `nunmai serve` backends share the existing hourly maintenance timer (first poll after 90 seconds), independently of cron jobs. They measure inactivity from process startup and the most recent chat activity in the same profile, retaining that activity timestamp after a session closes or is reaped, and skip curator while a turn in that profile is running. A connected but inactive window does not block maintenance. This timer also polls personal and organization Skill Sync, subject to those features' own opt-in gates. A running messaging gateway for the same profile owns these chores instead.
...
On a brand-new install (or the first time a pre-curator install ticks after `nunmai update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `nunmai curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (14) become `stale`; skills unused for `archive_after_days` (30) are moved to `~/.nunmai/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `nunmai model`:**

```bash
nunmai model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`nunmai model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
nunmai curator status         # last run, counts, pinned list, LRU top 5
nunmai curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
nunmai curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
nunmai curator run --background  # fire-and-forget: start the run in a background thread
nunmai curator run --dry-run  # preview only — report without any mutations
nunmai curator backup         # take a manual snapshot of ~/.nunmai/skills/
nunmai curator rollback       # restore from the newest snapshot
nunmai curator rollback --list     # list available snapshots
nunmai curator rollback --id <ts>  # restore a specific snapshot
nunmai curator rollback -y         # skip the confirmation prompt
nunmai curator pause          # stop runs until resumed
nunmai curator resume
nunmai curator pin <skill>    # never auto-transition this skill
nunmai curator unpin <skill>
nunmai curator adopt <skill>    # hand an unmanaged skill to the curator
nunmai curator adopt --all-unmanaged   # hand over every unmanaged skill
nunmai curator list-unmanaged   # itemize skills with no provenance marker
nunmai curator restore <skill>  # move an archived skill back to active
nunmai curator list-archived    # list skills currently in ~/.nunmai/skills/.archive/
nunmai curator archive <skill>  # manually archive a single skill now
nunmai curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default: `archive_after_days`, 30)
nunmai curator ledger           # list the per-mutation audit ledger (all actors)
nunmai curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
nunmai curator rollback <entry-id>  # undo a single mutation from the ledger
nunmai curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before a consolidation pass (`consolidate: true`, the only pass that rewrites skill content in place), Nunmai takes a tar.gz snapshot of `~/.nunmai/skills/` at `~/.nunmai/skills/.curator_backups/<utc-iso>/skills.tar.gz`. The snapshot covers the live skill tree only: `.archive/`, the audit ledger, `.hub/`, and the backups themselves are never rolled in, and a rollback never rewinds them (an older copy would lose archived skills or ledger entries). If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
nunmai curator rollback        # restore newest snapshot (with confirmation)
nunmai curator rollback -y     # skip the prompt
nunmai curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Nunmai takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `nunmai curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.

The default prune-only pass takes no snapshot: it only moves whole directories into `.archive/`, which is its own undo (`nunmai curator restore`), and every mutation is in the ledger below. Snapshots are pruned to `curator.backup.keep` (default 2) on every pass to keep disk usage bounded:
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `nunmai curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`nunmai curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.nunmai/skills/.curator_ledger.jsonl`:
...
Archived skills are kept forever by default. If you want `~/.nunmai/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
nunmai curator purge --dry-run   # preview what would be deleted
nunmai curator purge             # delete archives older than the TTL (with confirmation)
nunmai curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.nunmai/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.nunmai/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.nunmai/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Nunmai at an external skill
...
**To see which skills the curator actually manages**, run `nunmai curator status`.
...
`nunmai curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `nunmai curator adopt <name>` hands one over
```
...
```bash
nunmai curator list-unmanaged                    # itemize them, with reasons
nunmai curator adopt <name> [<name> ...]         # hand specific skills over
nunmai curator adopt --all-unmanaged --dry-run   # preview the full list
nunmai curator adopt --all-unmanaged             # hand over everything (prompts)
nunmai curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `nunmai curator adopt` changes the policy, and
...
**wrote** it — Nunmai edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `nunmai curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `nunmai curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `nunmai curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
nunmai curator pin <skill>
nunmai curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.nunmai/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.nunmai/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.nunmai/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.nunmai/logs/curator/`:
...
```bash
nunmai curator restore <skill-name>
```

This moves the skill back from `~/.nunmai/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.nunmai/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `nunmai curator pause` — the pause persists across sessions; use `resume` to re-enable.
