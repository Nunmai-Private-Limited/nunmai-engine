...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.nunmai/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
nunmai curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
...
Before every real curator pass, Nunmai takes a tar.gz snapshot of `~/.nunmai/skills/` at `~/.nunmai/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
