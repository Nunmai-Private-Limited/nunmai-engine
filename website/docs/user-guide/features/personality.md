...
description: "Customize Nunmai Engine's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Nunmai Engine's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NUNMAI_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Nunmai is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Nunmai now seeds a default `SOUL.md` automatically in:

```text
~/.nunmai/SOUL.md
```

More precisely, it uses the current instance's `NUNMAI_HOME`, so if you run Nunmai with a custom home directory, it will use:
...
- Nunmai creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Nunmai loads `SOUL.md` only from `NUNMAI_HOME`
- Nunmai does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Nunmai falls back to a built-in default identity
...
If Nunmai loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NUNMAI_HOME`, the personality belongs to the Nunmai instance itself.

That also makes it easier to teach users:
- "Edit `~/.nunmai/SOUL.md` to change Nunmai' default personality."
...
- how Nunmai should handle uncertainty, disagreement, or ambiguity
...
## What Nunmai injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Nunmai falls back to a built-in default identity ("You are Nunmai Engine, an intelligent AI assistant created by Nunmai Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Nunmai ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Nunmai, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Nunmai its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.nunmai/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.nunmai/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-nunmai)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Nunmai speaks
- `display.skin` and `/skin` affect how Nunmai looks in the terminal
