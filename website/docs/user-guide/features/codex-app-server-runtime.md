...
Nunmai can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Nunmai becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).
...
:::tip
Not using OpenAI Codex? `nunmai setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nunmai Portal](/integrations/nous-portal).
:::
...
For `apply_patch` (file edit) approvals, Nunmai shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.
...
If you find a bug, [open an issue](https://github.com/NousResearch/hermes-agent/issues) with the output of `nunmai logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
