...
Nunmai can optionally hand `openai/*`, `openai-codex/*` and [named custom provider](#named-custom-providers) turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Nunmai becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Nunmai behavior is unchanged unless you flip the flag. Nunmai never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `nunmai setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nunmai Portal](../../integrations/nous-portal.md).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Nunmai session.
- **Nunmai' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Nunmai for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Nunmai' message shape so the self-improvement loop sees a normal-looking transcript.
- **Your Nunmai persona rides along** — the composed system prompt (SOUL.md, MEMORY.md/USER.md, per-channel `system_prompt` overrides) is sent to the codex thread once as developer instructions when the thread starts, and Codex's built-in personality is disabled so it cannot compete with yours.
...
These ship with `codex app-server` itself — no Nunmai involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Nunmai' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Nunmai also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Nunmai queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Nunmai tool callback (MCP server, registered in `~/.codex/config.toml`)

Nunmai registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Nunmai' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Nunmai' skill library.
- **`text_to_speech`** — TTS through Nunmai' configured provider.

When the model wants one of these, codex spawns the `nunmai_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Nunmai' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Nunmai tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Nunmai' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Nunmai' todo store (codex's `update_plan` is the in-runtime equivalent)
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which Nunmai does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `nunmai chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- The Nunmai tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_request_review` / `kanban_request_changes` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `NUNMAI_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `NUNMAI_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `NUNMAI_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `nunmai-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Nunmai also passes narrow app-server sandbox overrides when `NUNMAI_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`NUNMAI_KANBAN_WORKSPACES_ROOT`, `NUNMAI_KANBAN_WORKSPACE`, legacy `NUNMAI_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/NousResearch/hermes-agent/issues/27941)).
...
|  | Nunmai default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Nunmai' own `nunmai auth add openai-codex` writes to `~/.nunmai/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

   <a id="named-custom-providers"></a>**Or: a named custom provider.** A `providers.<name>` entry in Nunmai config can use this runtime when the **same name** is defined as a Codex provider. Nunmai config:
   ...
   Nunmai sends only `model` and `modelProvider = "my-gateway"` on `thread/start`; codex resolves `base_url` and reads the key from `env_key` in its own environment. **Nunmai never forwards the API key**, so `MY_GATEWAY_API_KEY` must be present in the process environment Nunmai runs in — `~/.nunmai/.env` is loaded at startup and provider credentials are inherited by the codex subprocess. Auxiliary calls (titles, compression, memory review) still use Nunmai' own `providers.my-gateway` entry.

   Caveats: the name after `custom:` is the `providers:` config key and must match the `[model_providers.<name>]` table name exactly — if it does not exist on the codex side, codex reports an unknown provider rather than silently using the Nunmai endpoint. Anonymous `provider: custom` (a bare `base_url`) is not eligible: it has no stable name to hand to codex, so it stays on Nunmai' standard runtime.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Nunmai auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Nunmai will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
- Migrates user MCP servers from `~/.nunmai/config.yaml` to `~/.codex/config.toml`.
...
- **Registers Nunmai' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
You can also set it manually in `~/.nunmai/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Nunmai runtime)
```

If the Nunmai process cannot resolve `codex` from `PATH` — typical for gateway services,
...
`model.codex_bin` is used everywhere Nunmai spawns codex: the `/codex-runtime` availability
...
Nunmai' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Nunmai runtime.
...
One detail: the review fork itself needs to call Nunmai' agent-loop tools (`memory`, `skill_manage`), which require Nunmai' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Nunmai owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Nunmai' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Nunmai shows a summary of what changed (`1 add, 1 update: src/new.py, src/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Nunmai' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Nunmai' managed block:

```toml
default_permissions = ":read-only"
```

(Nunmai will preserve your override on re-migration as long as it lives outside the `# managed by nunmai-engine` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Nunmai' auxiliary client uses the main provider/model when no per-task override is set.
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.nunmai/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Nunmai downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Nunmai' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Nunmai wraps everything it manages between two marker comments:

```toml
# managed by nunmai-engine — `nunmai codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end nunmai-engine managed section
```

Anything **outside** that block is yours. Re-running migration (via `/codex-runtime codex_app_server`, whenever you toggle the runtime on, or `nunmai codex-runtime migrate`) replaces the managed block in place but preserves user content above and below it verbatim. This means you can:

- Add your own MCP servers Nunmai doesn't know about
...
**Same-name servers.** If your own `[mcp_servers.<name>]` table (outside the block) uses the same name as a server in Nunmai' `mcp_servers`, your table wins: Nunmai skips its projection for that name instead of emitting a second `[mcp_servers.<name>]` header (which is invalid TOML and would stop codex from starting). The migration report lists such names under "Kept N user-owned MCP server(s)". To let Nunmai manage the server, delete your table and re-run the migration. The rendered file is parsed as TOML before it replaces `config.toml`; an unparsable result is reported and the existing file is left untouched.

### Running the migration from a script

```bash
nunmai codex-runtime migrate            # rewrite the managed block for the active profile
nunmai codex-runtime migrate --dry-run  # report only, no write
nunmai codex-runtime migrate --json     # machine-readable report (migrated, preserved_user_servers, errors, …)
nunmai -p work codex-runtime migrate    # a named profile's mcp_servers
```
...
By default, Nunmai points the codex subprocess at `~/.codex/` regardless of which Nunmai profile is active. This means `nunmai -p work` and `nunmai -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `NUNMAI_HOME`:

```bash
# Inside the work profile, you might wrap nunmai:
CODEX_HOME=~/.nunmai/profiles/work/codex nunmai chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `nunmai -p work` will operate on isolated Codex state.
...
Nunmai does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Nunmai' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Nunmai (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Nunmai-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Nunmai writes a `[plugins."<name>@openai-curated"]` block enabling it in your Nunmai session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Nunmai' codex runtime, Nunmai activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Nunmai doesn't touch credentials.

## Nunmai tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Nunmai registers itself as an MCP server in `~/.codex/config.toml`:

```toml
[mcp_servers.nunmai-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.nunmai_tools_mcp_server"]
env = { NUNMAI_HOME = "/your/.nunmai", PYTHONPATH = "...", NUNMAI_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed Nunmai tool), codex spawns the `nunmai_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Nunmai runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Nunmai Engine 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Nunmai UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `nunmai-tools` callback)
...
- Nunmai web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Nunmai auth and codex auth are separate sessions.** You need both `codex login` AND `nunmai auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Nunmai' `_import_codex_cli_tokens` — Nunmai won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
- **No inline patch preview in approval prompts when codex doesn't track the changeset.** Codex's `fileChange` approval params don't always carry the changeset. Nunmai caches the data from the corresponding `item/started` notification when possible, but if approval arrives before the item has streamed, the prompt falls back to whatever `reason` codex provides.
- **`fallback_providers` fail over only on quota and rate-limit failures.** When a codex app-server turn fails with a billing / usage-limit / rate-limit error, Nunmai switches to the configured [fallback provider](./fallback-providers.md) and retries the same turn on it; auth failures (`codex login` expired), turn timeouts and unknown-model errors do not fail over on this runtime and surface as the turn's error instead.
- **Prior Nunmai history is seeded only into a thread codex starts from scratch.** A codex thread that codex hands back via `thread/resume` already holds the conversation. When no resumable thread exists — the session ran on another provider before `/model` switched to openai-codex, codex could not resume the stored thread, or the running thread was retired — the new thread's `developerInstructions` carry Nunmai' system prompt followed by the session's prior turns (user and assistant text, tool names, tool-result previews; the most recent ~32K characters). When the composed prompt changes mid-session (for example `/personality` in the TUI or Desktop), the next turn retires the running thread and starts a new one carrying the updated prompt plus that same history seed.
- **The codex thread itself does survive a restart.** After each committed turn Nunmai stores the codex thread id on the session row (`codex_thread_id` in the session's `model_config`, `nunmai sessions` / `state.db`). The next agent built for that same Nunmai session — a later `/api/sessions/{id}/chat` request, or the first turn after the API server or gateway restarts — issues `thread/resume` for the stored id before `turn/start`, so the model keeps its own memory of the earlier turns (that is why no history seed is sent on resume). When codex cannot hand the thread back (its rollout was deleted, `CODEX_HOME` changed, the previous app-server was killed while still writing it), Nunmai fails closed: it drops the stored id, starts a fresh thread and shows one line — `Codex thread could not be resumed; starting a new one.` — on the status rail of the surface you are on (CLI, TUI/Desktop, messaging gateway). A `/new` session never resumes an older thread.
...
If you find a bug, [open an issue](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues) with the output of `nunmai logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Nunmai shell (CLI / TUI / gateway) ───┐
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  nunmai_tools_mcp_server.py (subprocess on demand)        │
