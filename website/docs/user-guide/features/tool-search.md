...
**Tool Search** is Nunmai' opt-in progressive-disclosure layer for that
...
Nunmai keeps its working-set core tools (`terminal`, `read_file`, `write_file`,
`patch`, `search_files`, `todo`, `memory`, `browser_*`, `web_search`,
`web_extract`, `clarify`, `execute_code`, `delegate_task`, and the rest of
`_NUNMAI_CORE_TOOLS`) loaded directly by default. Cold, event-triggered built-ins
...
When the model invokes `tool_call`, Nunmai **unwraps the bridge** and
...
in `nunmai_cli/config_defaults.py`. It is the single source of truth for the
...
When you are signed in to the Nunmai Portal, the bridge additionally reaches
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Nunmai therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Nunmai cannot safely
...
- **No JS sandbox.** Nunmai uses the simpler "structured tools" mode
