...
:::info Built-in Nunmai tools never defer
The tools that make up Nunmai' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_NUNMAI_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
