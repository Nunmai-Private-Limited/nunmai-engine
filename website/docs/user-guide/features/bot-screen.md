...
On a headless Linux gateway host (a server, a cloud VM, Nunmai Cloud) each bot
gets its **own desktop**: an Xfce screen the bot's `computer_use` and headed
browser act on, streamed live into Nunmai Desktop. Watch what the bot does,
...
Every Nunmai profile ("bot") has its own screen, its own browser profile and
...
- TigerVNC's `Xvnc` and the Xfce core components are installed on the host.
  Nothing installs them silently: `nunmai update` and fresh installs leave every
  machine as it is. When they are missing the Screen pane in Nunmai Desktop shows
  **Install on host** — one click runs the package manager on the gateway host
  ...
  that host only and is never stored) and streams the log. When Nunmai itself
  ...
  Docker image (`ghcr.io/nunmai-private-limited/nunmai-engine`, which also powers Nunmai Cloud) is
  ...
  `nunmai computer-use screen status` prints the exact line and
  `nunmai computer-use screen install` runs it:
...
- Memory. Measured in the official image: the gateway idles at ~300 MB, Xvnc +
  ...
  Before starting a screen, Nunmai checks that the host — or its container
  ...
  of **Start screen** and
  `nunmai computer-use screen start` refuses; a screen already running is never
...
```bash
docker build --build-arg NUNMAI_BOT_DESKTOP=1 -t nunmai-engine:screen .
```
...
Every bot's computer is one click away in three places of Nunmai Desktop:
...
1. Open the Screen with any of the entries above.
   ...
   **Start screen** in the pane, run `nunmai computer-use screen start` on the
...
user-data-dir per bot (`<NUNMAI_HOME>/bot-desktop/browser-profile`; set
`AGENT_BROWSER_PROFILE` to pin your own — `~` expands, and a relative path such
as `pin` resolves against that bot's `NUNMAI_HOME`, i.e. `<NUNMAI_HOME>/pin`).
...
`<NUNMAI_HOME>/bot-desktop/xdg/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml`:
...
`AGENT_BROWSER_EXECUTABLE_PATH` wins; otherwise Nunmai prefers a system
...
```bash
nunmai computer-use screen status          # installed? running? who holds control?
nunmai computer-use screen start           # start this profile's screen
nunmai computer-use screen stop            # stop it; refuses while a human holds control
nunmai computer-use screen stop --force    # ...unless you say so (also frees a stuck lease)
nunmai computer-use screen install [-y]    # apt/dnf/pacman the packages
nunmai -p research computer-use screen start   # another bot's screen
```
...
pane (**Start screen**), from `nunmai computer-use screen start`, or set the
...
State lives under `<NUNMAI_HOME>/bot-desktop/` per profile (RFB Unix socket,
...
- **Nunmai Desktop** bundles noVNC. It asks the gateway for a single-use ticket
  ...
  and Nunmai Cloud connections alike.
- **Control lease.** The gateway drops keyboard, pointer and clipboard messages
  ...
  the browser tools. It is a file under `<NUNMAI_HOME>/bot-desktop/`: no file
...
- **"Screen packages missing"** — click **Install on host** in the pane, or run
  ...
  Nunmai Desktop). The pane refuses a second install while one is running.
- **Screen starts then stops** — read `<NUNMAI_HOME>/bot-desktop/launcher.log`.
...
- **Bot says `human_has_control` after you left** — click **Hand back** in the
  ...
  `nunmai computer-use screen stop --force` releases the lease and stops the
  screen (without `--force` the command refuses while a human holds control, so a
  runbook can never yank a live takeover); `nunmai computer-use screen start`
