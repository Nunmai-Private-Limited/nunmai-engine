...
User plugins at `~/.nunmai/plugins/model-providers/<name>/` and `~/.nunmai/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
nunmai plugins search <term>                 # search the community plugin index
nunmai plugins install <name>                # install by index name (resolved to repo @ pinned ref)
...
Install from Git**. Legacy `nunmai://plugin-agent/…` and
...
`nunmai plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
nunmai plugins search telegram               # fuzzy search
nunmai plugins search                        # browse the whole index
nunmai plugins search --capability platform  # filter by declared capability
nunmai plugins search media --json           # machine-readable output
nunmai plugins search --refresh              # bypass the 24h local cache
```
...
```bash
nunmai plugins install nunmai-media-studio
```
...
overridable via `nunmai config set plugins.index_url <url>`). Fetches are
cached under `~/.nunmai/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Nunmai — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "nunmai-media-studio",
...
(`nunmai_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[nunmai-plugin-index](https://github.com/NousResearch/hermes-plugin-index)
...
version: 1.0.0
plugins:
  - name: nunmai-media-studio            # bare community-index name…
