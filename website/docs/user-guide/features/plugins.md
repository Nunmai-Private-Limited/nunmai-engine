...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Nunmai
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Nunmai Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Nunmai — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
| Nix | `services.nunmai-engine.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
Plugins can register the 26 lifecycle events currently accepted by `nunmai_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Nunmai Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Nunmai Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Nunmai Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `nunmai <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Nunmai auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `nunmai skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.nunmai/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `nunmai plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
nunmai plugins list                          # table: enabled / disabled / not enabled
...
nunmai plugins remove my-plugin              # uninstall
nunmai plugins enable my-plugin              # add to allow-list
nunmai plugins disable my-plugin             # remove from allow-list + add to disabled
...
```
nunmai://plugin/install?repo=owner/repo            # main install link
...
Clicking one opens Nunmai and shows a **confirmation dialog** — the repo id,
...
[Add to Nunmai link](/reference/mcp-config-reference#add-to-nunmai-link).
