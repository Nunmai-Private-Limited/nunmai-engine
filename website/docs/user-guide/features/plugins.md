...
title: "Plugins"
description: "Extend Nunmai with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Nunmai has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](../../developer-guide/adding-tools.md) page is for built-in Nunmai
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Nunmai Plugin](../../developer-guide/plugins/index.md)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.nunmai/plugins/` with a `plugin.yaml` and Python code:
...
Start Nunmai — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.nunmai/plugins/hello-world/plugin.yaml`**
...
**`~/.nunmai/plugins/hello-world/__init__.py`**

```python
"""Minimal Nunmai plugin — registers a tool and a hook."""
...
Drop both files into `~/.nunmai/plugins/hello-world/`, restart Nunmai, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Nunmai does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.nunmai/plugins/` are disabled by default. Enable them only for trusted repositories by setting `NUNMAI_ENABLE_PROJECT_PLUGINS=true` before starting Nunmai.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `nunmai <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `nunmai plugins install` |
| Distribute via pip | `[project.entry-points."nunmai_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Nunmai — see [Built-in Plugins](./built-in-plugins.md) |
| User | `~/.nunmai/plugins/` | Personal plugins |
| Project | `.nunmai/plugins/` | Project-specific plugins (requires `NUNMAI_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `nunmai_agent.plugins` entry_points | Distributed packages |
| Nix | `services.nunmai-engine.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](../../getting-started/nix-setup.md#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Nunmai also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.nunmai/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.nunmai/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `nunmai plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.nunmai/config.yaml`. This stops third-party code from running without your explicit consent.

:::note `plugins.enabled` governs plugins only
[Gateway event hooks](./hooks.md#gateway-event-hooks) under `~/.nunmai/hooks/<name>/` are not plugins and are **not** gated by `plugins.enabled` or `plugins.disabled`. That directory is trusted by placement: any subdirectory holding a valid `HOOK.yaml` + `handler.py` is imported by the gateway at startup, and placing the files there is the opt-in. See the [gateway hook trust model](./hooks.md#gateway-hook-trust).
:::
...
```bash
nunmai plugins                    # interactive toggle (space to check/uncheck)
nunmai plugins enable <name>      # add to allow-list
nunmai plugins disable <name>     # remove from allow-list + add to disabled
```

After `nunmai plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
nunmai plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Nunmai checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `nunmai plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`nunmai plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in Nunmai Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `nunmai plugins list` prints
...
`nunmai plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential Nunmai can find on
its own. Every clone, pinned `--ref` fetch and `nunmai plugins update` pull is
...
refuses anonymous access does Nunmai look for a credential. For an `https://`
...
before. The same resolution applies to `nunmai plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

`nunmai doctor` sends a configured `GITHUB_TOKEN`/`GH_TOKEN` to `api.github.com`
...
Several categories of plugin bypass `plugins.enabled` — they're part of Nunmai' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.nunmai/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.nunmai/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Nunmai tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Nunmai runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Nunmai never
materializes the prompt on another surface.

Nunmai still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Nunmai that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.nunmai/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 27 lifecycle events currently accepted by `nunmai_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](./hooks.md#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Nunmai has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.nunmai/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Nunmai also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) · [Adding Tools](../../developer-guide/adding-tools.md) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](./hooks.md) · [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Nunmai Plugin](../../developer-guide/plugins/index.md) · [Extending the CLI](../../developer-guide/extending-the-cli.md) |
| A **subcommand** for `nunmai <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](../../developer-guide/extending-the-cli.md) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `NUNMAI_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](./tts.md#stt-custom-command-providers) · [Python plugin guide](./tts.md#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Nunmai auto-discovers the server's tools and registers them alongside built-ins. | [MCP](./mcp.md) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `nunmai skills tap add <repo>` | [Skills Hub](./skills.md#skills-hub) · [Publishing a custom tap](./skills.md#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.nunmai/hooks/<name>/` | [Event Hooks](./hooks.md#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `nunmai plugins install` needed. See the **[Nix Setup guide](../../getting-started/nix-setup.md#plugins)** for full details.
...
```bash
nunmai plugins                               # unified interactive UI
nunmai plugins list                          # table: enabled / disabled / not enabled (bundled backends,
                                             # platforms and the live memory.provider count as enabled)
nunmai plugins search <term>                 # search the Nunmai plugin catalog
nunmai plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
nunmai plugins install user/repo             # install from Git, then prompt Enable? [y/N]
nunmai plugins install user/repo --enable    # install AND enable (no prompt)
nunmai plugins install user/repo --no-enable # install but leave disabled (no prompt)
nunmai plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
nunmai plugins remove my-plugin              # uninstall; also drops it from plugins.enabled/disabled/entries
                                             # and resets memory.provider when it was the live provider
nunmai plugins enable my-plugin              # add to allow-list
nunmai plugins disable my-plugin             # remove from allow-list + add to disabled (bundled platforms:
                                             # either spelling works, e.g. photon-platform or platforms/photon)
nunmai plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Nunmai Desktop registers the `nunmai://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
nunmai://plugin/install?catalog=NAME               # catalog entry, installs the reviewed pin
nunmai://plugin/install?repo=owner/repo            # any git repo
nunmai://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
nunmai://plugin/install?repo=owner/repo&force=1    # replace an existing install
nunmai://plugin/install?catalog=<name>             # reviewed catalog entry at its pinned commit
```

The `catalog=<name>` form is what the **Open in Nunmai Desktop** button on
...
For a `repo=` link, clicking one opens Nunmai and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `nunmai://plugin-agent/…` and
`nunmai://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `nunmai-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="nunmai://plugin/install?repo=owner/repo&enable=1">Install in Nunmai</a>
```

MCP servers have the equivalent link form — see
[Add to Nunmai link](../../reference/mcp-config-reference.md#add-to-nunmai-link).
...
When a plugin declares capabilities, `nunmai plugins install` (and
`nunmai plugins enable`) shows the list with one-line risk descriptions and
...
granted, `nunmai plugins update` surfaces the additions and asks again. New
...
`nunmai plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
nunmai plugins capabilities             # all plugins with declared/granted capabilities
nunmai plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Nunmai has not reviewed the plugin's code. Only install
...
### Discovering plugins — the Nunmai plugin catalog

`nunmai plugins search <term>` searches the **Nunmai plugin catalog** — the
curated, SHA-pinned catalog maintained in the nunmai-engine repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
nunmai plugins search telegram    # search the catalog
nunmai plugins browse             # browse every entry
nunmai plugins info <name>        # full details for one entry
```
...
recorded so `nunmai plugins update` can re-pin when the catalog moves:

```bash
nunmai plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`nunmai-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: nunmai-telegram-business       # bare plugin-catalog name…
...
```bash
nunmai plugins pack show ./nunmai-pack.yaml     # dry-run review
nunmai plugins pack install ./nunmai-pack.yaml  # review → confirm → install
nunmai plugins pack export > nunmai-pack.yaml   # snapshot the current install
nunmai plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `nunmai plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `nunmai plugins install`.
...
provenance (installed via `nunmai plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`nunmai skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `nunmai plugins install` and `nunmai plugins update` runs a static
...
On `nunmai plugins update`, a dangerous verdict on the updated tree
...
- **Documentation prose** (`README.md`, `AGENTS.md`, `docs/**/*.md`, `.txt`,
  ...
  **own** install directory (`rm -rf "$HOME/.nunmai/plugins/<name>"`) is a
...
Running `nunmai plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `nunmai plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Nunmai reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Nunmai rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Nunmai can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Nunmai drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Nunmai checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Nunmai' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
