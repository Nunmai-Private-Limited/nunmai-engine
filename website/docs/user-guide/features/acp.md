...
`platform_toolsets.acp` replaces the `nunmai-acp` default, and
...
only those servers are included, and `no_mcp` drops them all. `nunmai tools`
...
`approvals.mode: manual` does not help: Nunmai raises the permission request,
...
Conversations are persisted to Nunmai' session database and can be listed, loaded,
...
sessions, inspect unwanted rows with `nunmai sessions show <id>` and remove only
confirmed unwanted sessions with `nunmai sessions delete <id>`.
