...
description: "Connect Nunmai Engine to external tool servers via MCP — and control exactly which MCP tools Nunmai loads"
---

# MCP (Model Context Protocol)

MCP lets Nunmai Engine connect to external tool servers so the agent can use tools that live outside Nunmai itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Nunmai to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Nunmai' `config.yaml` — and `nunmai import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Nunmai tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Nunmai to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.nunmai/config.yaml`:
...
4. Ask Nunmai to use the MCP-backed capability.
...
(`nunmai --tui`, a callout above the composer) and the classic CLI (a panel):
...
3. **Authorization.** For an OAuth entry the card shows the authorization link.
   Nunmai never opens the browser by itself: click **Open in browser** on the
...
4. **Save.** Nunmai saves the server configuration, the tokens and your setup
   ...
   tokens; Nunmai asks you to authorize again only when they no longer work.
5. **Tools.** Nunmai then lists the server's tools and registers them. The
...
```bash
nunmai mcp                   # interactive picker (default)
nunmai mcp catalog           # plain-text list, scriptable
nunmai mcp install deepwiki  # install a catalog entry by name
```
...
- **API key** — Nunmai prompts at install time and writes the value to
  `~/.nunmai/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Nunmai points you at
  `nunmai auth <provider>` if you haven't authenticated already.
...
3. Run `nunmai mcp install n8n-official` and enter that URL when prompted.
4. Complete browser OAuth. If needed, run `nunmai mcp login n8n-official`
   or use **Authorize** on the configured server in Desktop or the dashboard.
5. Review tools with `nunmai mcp configure n8n-official`, then start a new
   session or use `/reload-mcp`.

The Nunmai backend must be able to reach the URL. n8n controls permissions
...
After credentials are configured, Nunmai probes the MCP server to list every
...
written (if not). Re-run `nunmai mcp configure <name>` once the server is
reachable to refine.
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Nunmai
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Nunmai`) for that entry instead of silently hiding it. Run `nunmai update`
to install the latest Nunmai when you see that.
...
from environment variables (which include everything in `~/.nunmai/.env`).
...
to register, then run `nunmai mcp login <name>` and restart (or
...
pre-registered with a pinned `redirect_port`, Nunmai keeps the registered
...
same machine as the Nunmai process. For a remote host, use `nunmai mcp login`
over SSH port-forwarding.

### Updating tool selection later

```bash
nunmai mcp configure linear
```
...
MCPs are never auto-updated. Re-run `nunmai mcp install <name>` to refresh
after a Nunmai update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/Nunmai-Private-Limited/nunmai-engine/blob/main/optional-mcps).
...
rejected), and Nunmai's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Nunmai connects to directly.
...
- you do not want Nunmai spawning a local subprocess for that integration
...
Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Nunmai handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Nunmai identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Nunmai Engine"` 403s, while `"Claude Code"` and `"Codex"` succeed. Nunmai auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `nunmai mcp install figma`, then `nunmai mcp login figma`.
:::
...
**Google-hosted servers (Gmail, Calendar).** Google only issues a refresh token when the authorization request carries `access_type=offline`, which MCP discovery never advertises. Nunmai adds it (plus `prompt=consent`, so a repeat login is re-granted one) whenever the discovered authorization server is `accounts.google.com`, so the connection persists across restarts and works from `nunmai gateway`. Other issuers' requests are untouched.

**Remote / headless hosts.** When Nunmai runs on a different machine than your browser, the loopback callback can't reach your laptop. Ways to complete the flow:

- **Nunmai Desktop (automatic):** when you run the OAuth sign-in from the Desktop app's MCP setup UI against a remote backend, Desktop hosts the callback listener on *your* machine and relays the authorization back to the gateway automatically — no tunnel, paste, or proxy needed. Requires both the Desktop app and the backend to be up to date.
- **Paste-back (no setup):** on an interactive terminal Nunmai prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **Device-code login (no callback at all):** if the server's authorization server advertises a device authorization endpoint, run `nunmai mcp login <server> --flow device` on the machine running Nunmai. It prints a verification URL and a short code; open the URL on any device, enter the code, and Nunmai polls for approval. No browser is launched on the host and no callback listener is needed. Set `oauth.flow: device` on the server to make `login` and `reauth` use it by default. Details: [Device-code login](../../reference/mcp-config-reference.md#device-code-login-rfc-8628).
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Nunmai on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Nunmai expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `nunmai mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `nunmai mcp login` can list the tools and look like it worked, but every real tool call later times out. `nunmai mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `nunmai mcp login googledrive` — with the pre-registered client, Nunmai skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.nunmai/config.yaml` from inside a running Nunmai session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `nunmai mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

**Need longer than 5 minutes to approve?** Set `oauth.timeout` on the server entry (seconds). `nunmai mcp login`, the dashboard and Desktop re-auth all wait `oauth.timeout` + 15 s (or the entry's `connect_timeout`, whichever is longer); a login that still runs out of time reports `Connecting to MCP server '<name>' timed out after Ns` naming both knobs instead of a blank failure line.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Nunmai passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Nunmai profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Nunmai reads MCP config from `~/.nunmai/config.yaml` under `mcp_servers`.
...
| `cwd` | string | Working directory for the stdio server process. Default: the session working directory when one is pinned (ACP/gateway sessions, `terminal.cwd`), else the Nunmai process directory |
...
| `enabled` | bool | If `false`, Nunmai skips the server entirely |
...
For well-known MCP servers, `nunmai mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
nunmai mcp add codex --preset codex
```
...
You can pick any local name (`nunmai mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Nunmai registers MCP tools

Nunmai prefixes MCP tools so they do not collide with built-in names:
...
- Nunmai only registers resource utilities if the MCP session actually supports resource operations
- Nunmai only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Nunmai, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Nunmai skips the server completely and does not even attempt a connection.
...
You can also separately disable Nunmai-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Nunmai does not create an empty runtime MCP toolset for that server.
...
Nunmai discovers MCP servers at startup and registers their tools into the normal tool registry.
...
MCP servers can notify Nunmai when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Nunmai receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
A running messaging gateway (`nunmai gateway run`) also watches `config.yaml` on its own: within about a minute of you removing an `mcp_servers` entry or setting `enabled: false`, that server's connection is torn down; a newly added entry is connected. A server whose first connect failed (an unreachable host, or an OAuth server on a headless box that had no token yet) is retried automatically on its connect cooldown schedule (30 s, doubling up to 10 min) once you fix the cause. No restart or `/reload-mcp` needed for the edit to take effect.

**Expired OAuth tokens in the background.** The gateway, `/reload-mcp`, and the periodic self-probe of a parked server never open a browser — nobody is there to complete the flow. When a refresh token dies, the server parks with a warning in `gateway.log` and you re-authorize once with `nunmai mcp login <server>` (or the Desktop/dashboard *Authorize* button); the parked server picks the new token up on its next probe.
...
For stdio servers, Nunmai does not blindly pass your full shell environment.
...
Then verify your config and restart Nunmai.
...
`nunmai mcp test <name>` reports what the server actually answered. When the MCP SDK can only say
`Server returned an error response` (a 4xx/5xx whose body is not a JSON-RPC error), Nunmai appends
...
endpoint. `nunmai logs --level debug` additionally shows the exact endpoint each connect attempt used.
...
Because Nunmai now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Nunmai may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Nunmai via the `sampling/createMessage` protocol. This allows an MCP server to ask Nunmai to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Nunmai as an MCP server

In addition to connecting **to** MCP servers, Nunmai can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Nunmai's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Nunmai
- You want a single MCP server that bridges to all of Nunmai's connected messaging platforms at once
- You already have a running Nunmai gateway with connected platforms
...
Add Nunmai to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Nunmai in a specific location:

```json
{
  "mcpServers": {
    "nunmai": {
      "command": "/home/user/.nunmai/nunmai-engine/venv/bin/nunmai",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Nunmai-specific channel browser:
...
The MCP server includes a live event bridge that polls Nunmai's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
nunmai mcp serve              # Normal mode
nunmai mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Nunmai's session store — `~/.nunmai/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `nunmai send` CLI.
...
- The embedded `nunmai mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Nunmai, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
