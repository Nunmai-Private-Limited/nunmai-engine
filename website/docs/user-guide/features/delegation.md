...
Top-level model calls run in the background automatically. Nunmai returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds that workspace's **project context files** (`.nunmai.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under the repo's own conventions without having to rediscover them.
...
When a top-level agent provides a `tasks` array, Nunmai returns one background handle and runs the subagents in parallel. By default the call returns **one** consolidated message once every task has finished. Results are delivered only between the parent's turns: the parent should finish anything that does not depend on the children, then end its turn rather than polling transcripts, artifacts, or CI while it waits.
...
When a background delegation finishes, Nunmai stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Nunmai restarts after completion but before delivery, the pending
...
Nunmai cannot prove whether its external side effects happened. Pending and
...
```
<nunmai_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.nunmai/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
- A Nunmai process restart does **not** resume a running child. Its attempt becomes `unknown` because Nunmai cannot prove which side effects happened.
...
- Each child starts its terminal in `<repo>/.worktrees/subagent-<id>` on its
  own branch `nunmai-subagent/subagent-<id>`, and its goal message tells it to
