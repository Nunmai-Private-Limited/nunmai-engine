...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `nunmai computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Nunmai host identity. |
...
| Nunmai session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Nunmai approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Nunmai risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Nunmai then launches the cua-driver runtime with the trusted-launcher grant
...
launched for bounded, unrestricted, or existing-profile access. One Nunmai
...
embedded service under the Nunmai host identity.
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Nunmai
autodetection is a planned cua-driver follow-up, so currently point Nunmai at
