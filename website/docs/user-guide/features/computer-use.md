...
Nunmai Engine can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Nunmai integration. It
...
**Fresh installs already have the driver.** The Nunmai installer
...
- **`nunmai tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
nunmai computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `nunmai computer-use
status` to verify the install.

Already have cua-driver? Nunmai reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `nunmai update`, and the first
`computer_use` call of a session, Nunmai checks the local version and
...
selected with `NUNMAI_CUA_DRIVER_CMD` stays
under your control, so Nunmai reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Nunmai autodetection is a
planned cua-driver follow-up, so currently point Nunmai at that directory or
...
interface. The built-in toolset provides Nunmai actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `nunmai computer-use doctor` (CuaDriver, `com.trycua.driver`, in every permission mode — the driver daemon always launches through `CuaDriver.app`). |
...
```
nunmai -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.nunmai/config.yaml`.

## Permission modes and logged-in browser profiles

Nunmai maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Nunmai session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Nunmai approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Nunmai risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.nunmai/cua-manifest.yaml
```
...
for the format). Nunmai launches a private runtime with
...
identity instead of resetting with every Nunmai build), and Nunmai verifies
...
resetting or closing the Nunmai session, cancellation cleanup, or process exit
closes that transport session. Nunmai also stops private runtimes that it
launched for bounded or unrestricted access. One Nunmai
...
## `nunmai computer-use doctor` — your first triage stop

`nunmai computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ nunmai computer-use doctor
...
`nunmai computer-use status` prints the same dead-daemon line and exits 1.
...
Nunmai run declares a public cua-driver **session name** (something like
...
without it. Nunmai disables it automatically where it is a known failure
...
Nunmai keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Nunmai-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.nunmai/skills/cua-driver` (Nunmai is one
...
These are **platform deep dives, not duplicates of the Nunmai skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Nunmai
...
image capture also saves a bounded, shareable copy under Nunmai' image cache
...
Nunmai applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.nunmai/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Nunmai applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Nunmai agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Nunmai agent itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `nunmai computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the Nunmai venv
...
- **Platform-specific deployment gotchas:**
  ...
  - **Linux** requires a reachable display server. Headless servers
    ...
    desktop over TigerVNC, streamed into Nunmai Desktop, where you can
    take over for logins and 2FA. You start it from the Desktop's
    Screen pane or `nunmai computer-use screen start`; it starts on
...
On Linux, native Wayland support remains an explicit opt-in. Nunmai passes the
...
```
NUNMAI_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
NUNMAI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Nunmai disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Nunmai sets
...
When it's on, `nunmai computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Nunmai at a binary you built from source instead
of the published release. Nunmai resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`NUNMAI_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Nunmai testing (see notes).
...
### Option B — point Nunmai straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`NUNMAI_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
NUNMAI_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
NUNMAI_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Nunmai is using your build

- `nunmai computer-use status` prints the resolved binary path and
  version.
- `nunmai computer-use doctor` confirms the binary is reachable and
...
- **Nunmai spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Nunmai YOLO, Nunmai instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Nunmai
  change needed either way.
- **Local builds skip the version check.** Nunmai warns when the
...
**First action when anything's off: run `nunmai computer-use doctor`.**
...
Run `nunmai computer-use install` to fetch the cua-driver binary, or
run `nunmai tools` and enable the Computer Use toolset.

**Clicks seem to have no effect** — Capture and verify. A modal you
didn't see may be blocking input. Dismiss it with `escape` or the close
button.

**macOS: System Settings shows CuaDriver ON, but `nunmai computer-use
...
nunmai computer-use permissions grant
```
...
Wayland without an XWayland bridge. `nunmai computer-use doctor` will
...
- **Nunmai-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Nunmai `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Nunmai autodetection is a
  planned follow-up; currently point Nunmai at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Nunmai among them)
