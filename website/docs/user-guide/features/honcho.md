...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Nunmai's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Nunmai instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nunmai memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `nunmai` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Automatically generated Nunmai titles (`derived` or `llm`), including lineage titles for Desktop branches, are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions without a manual mapping.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nunmai syncs them back at session init.
...
The `nunmai honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nunmai memory setup honcho` (or run `nunmai memory setup` and pick it from the list); the `nunmai honcho` subcommand then appears on the next invocation.

```bash
nunmai memory setup honcho    # Configure Honcho directly (works before activation)
nunmai honcho status          # Connection status, config, and key settings
nunmai honcho setup           # Redirects to `nunmai memory setup` (post-activation alias)
nunmai honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nunmai honcho peer            # Show or update peer names + dialectic reasoning level
nunmai honcho mode            # Show or set recall mode (hybrid/context/tools)
nunmai honcho tokens          # Show or set token budget for context and dialectic
nunmai honcho identity        # Seed or show the AI peer's Honcho identity
nunmai honcho sync            # Sync Honcho config to all existing profiles
nunmai honcho peers           # Show peer identities across all profiles
nunmai honcho sessions        # List known Honcho session mappings
nunmai honcho map             # Map current directory to a Honcho session name
nunmai honcho enable          # Enable Honcho for the active profile
nunmai honcho disable         # Disable Honcho for the active profile
nunmai honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nunmai honcho`

If you previously used the standalone `nunmai honcho setup`:
...
No re-login or re-setup needed. Run `nunmai memory setup` and select "honcho" — the wizard detects your existing config.
