...
:::tip
Hosted-mode auth uses Nunmai Portal OAuth; if you also want the dashboard to talk to a real backend, `nunmai setup --portal` wires up the model and tool gateway too. See [Nunmai Portal](../../integrations/nous-portal.md).
:::
...
```bash
# Custom port
nunmai dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
nunmai dashboard --host 0.0.0.0

# Start without opening browser
nunmai dashboard --no-open
```
...
child with the selected profile's `NUNMAI_HOME`, so the conversation runs
...
processes (manage them via `nunmai -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `nunmai-engine` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.nunmai/nunmai-engine && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.nunmai/nunmai-engine && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `nunmai dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `nunmai dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.nunmai` is nearly full
...
```ini
[Service]
EnvironmentFile=%h/.nunmai/.env
ExecStart=/path/to/venv/bin/python -m nunmai_cli.main dashboard \
...
One backend serves a whole host, so when `nunmai dashboard` finds a live backend it cannot
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `nunmai config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Nunmai instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `nunmai profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.nunmai/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `nunmai skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `nunmai mcp` reads from.
...
Connect Nunmai to any messaging platform from the browser — full parity with
`nunmai setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.nunmai/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page. On a system-scope install (`nunmai gateway install --system`) the dashboard runs the restart under `sudo -n`, so the dashboard user needs passwordless sudo; without it the request fails immediately instead of reporting a restart that the CLI then refuses.
...
This re-reads `~/.nunmai/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`NUNMAI_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.nunmai`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/nunmai/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
|-------|-----------|----------|
| `nunmai dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `nunmai dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
### Default provider: Nunmai Research
...
|---------|-----------|--------|----------------|
| `NUNMAI_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `nunmai dashboard register` |

Per the Nunmai Engine convention (`~/.nunmai/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: NUNMAI_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Nunmai Research** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://nunmai.example.com/auth/callback` and set `NUNMAI_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `NUNMAI_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `NUNMAI_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.nunmai/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.nunmai/.env <<EOF
NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME=admin
NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Nunmai Research](#default-provider-nous-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `NUNMAI_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `NUNMAI_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `NUNMAI_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-nunmai.json` — it defines a `nunmai` realm, a **public PKCE client** (`nunmai-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "nunmai-dashboard",
      "name": "Nunmai Engine Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-nunmai.json:/opt/keycloak/data/import/realm-nunmai.json:ro" \
...
`NUNMAI_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → nunmai-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `NUNMAI_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
first; without one, Nunmai fails closed at startup. This prevents the local SPA
...
Nunmai always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
nunmai dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `NUNMAI_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `NUNMAI_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `nunmai_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `nunmai_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `nunmai_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `nunmai_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$NUNMAI_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
from nunmai_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
NUNMAI_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  nunmai dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.nunmai/.env (secrets file, 0600).
cat >> ~/.nunmai/.env <<'EOF'
NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME=admin
NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
nunmai dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/nunmai` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `NUNMAI_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `NUNMAI_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `NUNMAI_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME` / `NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `NUNMAI_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `nunmai_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `nunmai update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `nunmai dashboard` will build it on first launch.
