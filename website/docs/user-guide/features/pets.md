...
Nunmai can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<NUNMAI_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Nunmai renders the sprite at full fidelity when your
...
nunmai pets list
nunmai pets list cat

# Install a pet and make it active in one step
nunmai pets install boba --select
...
## `nunmai pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `nunmai pets list [query] [--limit N]` |
| List installed pets | `nunmai pets list --installed` |
| Install a pet | `nunmai pets install <slug> [--select] [--force]` |
| Set the active pet | `nunmai pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `nunmai pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `nunmai pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `nunmai pets off` |
| Remove an installed pet | `nunmai pets remove <slug>` |
| Diagnose setup | `nunmai pets doctor` |

`nunmai pets show` flags:
...
Beyond installing pre-made pets from the gallery, Nunmai can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `nunmai pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Nunmai state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Nunmai Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nunmai run a quality-first model chain by default.

- Resolution order prefers Nunmai Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `nunmai tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `NUNMAI_PET_IMAGE_PROVIDER` env var (e.g. `NUNMAI_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Nunmai is
...
Run `nunmai pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Nunmai uses its own
  profile-scoped `<NUNMAI_HOME>/pets/` instead — install through `nunmai pets`.

## See also

- The [`nunmai-engine` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nunmai-engine.md)
