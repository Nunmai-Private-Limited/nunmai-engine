...
Run multiple independent Nunmai agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Nunmai home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Nunmai recognises a directory under `~/.nunmai/profiles/` as a profile only when it carries one of those identity files (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json`, `state.db`); a bare directory left behind by logging or cron is ignored by `profile list`, gateways and `-p`. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Nunmai state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Nunmai home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](./features/memory-providers.md) instead.
:::
...
These terms describe different parts of Nunmai:
...
- **Agent** is the running Nunmai assistant that uses that configuration and
  state. "Nunmai Engine" also names the product.
...
- **Messaging bot** is an account on a platform such as Telegram, Discord, or
  Slack, connected to Nunmai through the gateway. Its
...
```bash
nunmai profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Nunmai profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `nunmai setup --portal` inside the new profile to wire up models + tools at once. See [Nunmai Portal](../integrations/nous-portal.md).
:::

### Blank profile

```bash
nunmai profile create mybot
```
...
```bash
nunmai profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `nunmai profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
nunmai profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. When cloning from the default profile, the local-model runtime trees (`models/`, `runtimes/`, `node/` — downloaded weights and managed binaries, re-fetched on demand) are skipped too, as `nunmai backup` already does. **Cron jobs are not cloned** either: they are scheduled work bound to the source profile and its delivery channel, and a clone that inherited them would run every job twice (two gateways, same job ids). The new profile starts with an empty `cron/`. For a full backup including history and cron jobs, use `nunmai profile export` or `nunmai backup` instead.

:::note OAuth logins are shared, not copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. The new profile keeps reading the login from the root `~/.nunmai/auth.json`, and a token refresh performed inside any profile is written back to root, so all profiles stay signed in. Static API keys are copied as usual. To give a profile its own separate OAuth login, run `nunmai -p <name> auth add <provider>` inside it.
:::

### Clone from a specific profile

```bash
nunmai profile create work --clone-from coder
```
...
```bash
nunmai profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `nunmai gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`nunmai -p <name> setup` or the dashboard Messaging page.

```bash
nunmai profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `nunmai profile list` prints the same warning for any existing
...
The alias works with every nunmai subcommand — it's just `nunmai -p <name>` under the hood.
...
```bash
nunmai -p coder chat
nunmai --profile=coder doctor
nunmai chat -p coder -q "hello"    # works in any position
```

### Sticky default (`nunmai profile use`)

```bash
nunmai profile use coder
nunmai chat                   # now targets coder
nunmai tools                  # configures coder's tools
nunmai profile use default    # switch back
```

Sets a default so plain `nunmai` commands target that profile. Like `kubectl config use-context`.
...
- **`nunmai profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Nunmai its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Nunmai was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.nunmai/profiles/coder/.env

# Edit assistant's tokens
nano ~/.nunmai/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates nunmai-gateway-coder systemd/launchd service
assistant gateway install     # creates nunmai-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `nunmai profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `nunmai -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](./docker.md#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.nunmai/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `nunmai profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`nunmai update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
nunmai profile list           # show all profiles with status
nunmai profile show coder     # detailed info for one profile
nunmai profile rename coder dev-bot   # rename (updates alias + service)
nunmai profile migrate-identity coder dev-bot   # retry a rename's identity migration
nunmai profile purge-identity dev-bot   # retry a delete's identity purge
nunmai profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
nunmai profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.nunmai` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
nunmai profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `nunmai profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.nunmai/profile.yaml`; remove that line to
...
```bash
nunmai profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `nunmai profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.nunmai`). To remove everything, use `nunmai uninstall`.
:::
...
```bash
# Bash
eval "$(nunmai completion bash)"

# Zsh
eval "$(nunmai completion zsh)"
```
...
Profiles use the `NUNMAI_HOME` environment variable. When you run `coder chat`, the wrapper script sets `NUNMAI_HOME=~/.nunmai/profiles/coder` before launching nunmai. Since 119+ files in the codebase resolve paths via `get_nunmai_home()`, Nunmai state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `NUNMAI_HOME`.
...
isolated by `NUNMAI_HOME`, not by changing `HOME`. Container backends still use
`{NUNMAI_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `NUNMAI_HOME` is the profile boundary. It controls Nunmai config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Nunmai
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Nunmai keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Nunmai launches tool
subprocesses with `HOME={NUNMAI_HOME}/home`; you then need to initialize or link
...
Nunmai also exposes `NUNMAI_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.nunmai` itself. No migration needed — existing installs work identically.
...
nunmai profile export coder
nunmai profile import ./coder.tar.gz --name coder
```
...
nunmai profile install github.com/you/research-bot --alias

# Update later when the author ships a new version (keeps your memories + .env)
nunmai profile update research-bot
```
