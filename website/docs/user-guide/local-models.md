...
Nunmai can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Nunmai does the rest.
...
2. Click **Install runtime**. Nunmai downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Nunmai, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Nunmai picks what to download
...
Models ship in several quality grades (quantizations). Nunmai picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Nunmai never
...
Local models live or die by memory placement, so Nunmai manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Nunmai deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), Nunmai uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  Nunmai compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Nunmai
...
If a llama-server is already running on your machine, Nunmai detects it
...
[Run Nunmai Locally with Ollama](../guides/local-ollama-setup.md) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Nunmai.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Nunmai updates it with
...
Models and runtime builds live under the Nunmai home directory
