...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Nunmai knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
nunmai peer list
nunmai peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
nunmai peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
nunmai peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
