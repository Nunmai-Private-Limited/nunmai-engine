...
If you only run one Nunmai agent, you don't need this page — see
...
[Connecting Desktop to Many Nunmai Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Nunmai agents that should all
...
(`ai.nunmai.gateway-<name>.plist`), a systemd user service
(`nunmai-gateway-<name>.service`), a systemd **system** service when installed with
`sudo nunmai gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `nunmai serve` backend. This guide adds the patterns for managing them
collectively.
...
nunmai profile create coder
nunmai profile create personal-bot
nunmai profile create research
...
- `nunmai -p <name> gateway run` while it is live **attaches** instead of
...
- `nunmai gateway start --all` / `restart --all` mean *the one host
  ...
  `nunmai gateway migrate --multiplex` one-liner.
- `nunmai gateway run --replace` takes over the process **serving this
  ...
  instead of refusing and respawn-storming under the supervisor. An older Nunmai
  wrote a systemd drop-in (`nunmai-gateway.service.d/20-replace.conf`) that
  forced `--replace` onto the unit; `nunmai update` / `nunmai gateway restart`
  now remove that file. `nunmai gateway run --force` starts a separate gateway
...
[`nunmai gateway migrate --multiplex`](#migrating-from-per-profile-gateways) and
...
that are not served, the blocker, and the fix; the same box in the `nunmai
update` summary and `nunmai gateway status`; a banner in the dashboard
...
- `gateway.multiplex_profiles` has **one valid value right now: `true`**, and
  ...
  the next `nunmai update` summary — never a silent flip. A per-profile gateway
...
Other processes (`nunmai -p <name> gateway start`, the dashboard, `nunmai gateway
...
UNIX users, or a `NUNMAI_HOME` outside `<default home>/profiles/` — every
...
```bash
nunmai config set gateway.multiplex_profiles true
nunmai gateway restart
```

Equivalently, in the default profile's `~/.nunmai/config.yaml`:
...
```bash
nunmai -p coder gateway stop     # park coder; other profiles keep running
nunmai -p coder gateway start    # unpark coder and serve it again
nunmai -p coder gateway restart  # reconnect coder with its current configuration
```
...
`nunmai -p coder gateway status` reports
`parked (nunmai -p coder gateway start)` while the marker exists.
...
`nunmai -p coder gateway stop`), Start unparks it while a host gateway is live,
...
`nunmai -p coder gateway install` (or `start`, `run`, and
the service step of `nunmai -p coder setup`) refuses with exit 78 whether or not
...
❌ Profile 'coder' does not get a gateway of its own.
  ...
  Or fold an existing per-profile fleet onto one host gateway:
    ...
    nunmai gateway migrate --multiplex

  A separate per-profile gateway (for a fleet split across UNIX users or a
  NUNMAI_HOME outside profiles/) needs --force:  nunmai -p coder gateway install --force
  ...
  it will be removed once they are — plan to fold this profile with `nunmai gateway migrate --multiplex`.)
```
...
PID and served set, and the pointer is `nunmai -p default gateway restart`.
...
by this gateway`. `nunmai -p coder gateway install|start|run` works without
...
multiplexer or `nunmai serve`. Point webhook clients at the standalone
...
`nunmai -p coder gateway status` prints `standalone by config
(gateway.standalone: true)` before the profile's own gateway state, and
`nunmai gateway status` (default) lists it as `standalone by config: coder`
after the served set. `nunmai gateway migrate --multiplex` leaves the profile
...
`NUNMAI_HOME` outside `profiles/`): it installs a real per-profile service, and
...
`restart` commands above to manage a satellite inside the host. `nunmai gateway
...
The setup flows follow the same rule: `nunmai -p coder setup gateway`, `nunmai -p coder setup`,
`nunmai -p coder gateway setup` and `nunmai -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `nunmai gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.nunmai/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `nunmai webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`nunmai webhook ls` shows the
...
A standalone `nunmai -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ nunmai -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: nunmai gateway status
...
`nunmai gateway status` and `nunmai status` on the default profile list the same
...
There is a single process-level PID and lock (the multiplexer, under the default home). `nunmai status` on the default profile reports the multiplexer and lists the profiles it serves (`Serves: coder, research`). `nunmai -p coder status` and `nunmai -p coder gateway status` report "running via the default-profile multiplexer" instead of "stopped". The dashboard's `/api/status?profile=coder` / Channels page report the multiplexer as coder's running gateway, with coder's own adapters as its platforms. The single `gateway_state.json` lives under the default home: secondary adapters appear there as `<profile>:<platform>` entries beside `served_profiles`; no per-profile gateway status file is written.

`nunmai -p coder cron status` names the single host gateway and the profiles it serves — `Scheduler host: the host gateway (PID 4211) serving profiles default, coder` — then checks coder's own ticker heartbeat and last successful tick. A missing or stale heartbeat produces a warning rather than an unconditional running verdict. `cron list` and `cron create` also warn when a served profile has no fresh heartbeat. `cron status` adds tick-failure details that those lightweight checks do not read.

When no gateway owns the host role, `cron status` tells you to start the **one** host gateway (`nunmai --profile default gateway install` / `gateway run`) and to make sure it serves this profile. Installing a per-profile service is shown only under `LEGACY (pre-multiplex topology, not recommended)`: it would start a second gateway process on the host. `nunmai doctor` follows the same rule — under s6 it reports `Host gateway: the host gateway (PID 4211) serving profiles default, coder` instead of a per-profile slot count, flags any still-supervised per-profile slot as LEGACY, and checks the host systemd unit's linger even when you run doctor from a served profile. The `state.db` holder lines name the shared host process too, so "3 process(es) holding the DB open" says which gateway and which profiles stopping it would affect.
...
`HINDSIGHT_BANK_ID`, `NUNMAI_HONCHO_HOST`) are read from the routed profile's
...
`NUNMAI_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`trust_recent_files*`) and the Nunmai `auth.json` used for auxiliary calls are all
...
`NUNMAI_HOME`, without the default profile's secrets in their environment, and
...
| Dashboard actions (`nunmai -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `NUNMAI_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
`nunmai profile delete <name>`, or move the
...
multiplexer started as `nunmai -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`nunmai profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `nunmai -p <name> gateway
...
adapters the same way, and `nunmai profile rename` unroutes the old name before
...
`~/.local/bin/nunmai-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nunmai-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    nunmai gateway "$action"
  else
    nunmai -p "$profile" gateway "$action"
  fi
}
...
```bash
nunmai-gateways start      # start every configured profile
nunmai-gateways stop       # stop every configured profile
nunmai-gateways restart    # restart all
nunmai-gateways status     # status across all
nunmai-gateways list       # delegates to `nunmai gateway list`
```

:::tip
The `default` profile is targeted with `nunmai gateway <action>` (no `-p`),
not `nunmai -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nunmai -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nunmai.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nunmai-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nunmai.gateway.plist` /
`nunmai-gateway.service`.
...
```bash
# Default profile
tail -f ~/.nunmai/logs/gateway.log
tail -f ~/.nunmai/logs/gateway.error.log

# Named profile
tail -f ~/.nunmai/profiles/<name>/logs/gateway.log
tail -f ~/.nunmai/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nunmai/logs/gateway.log ~/.nunmai/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nunmai logs -f                  # follow default profile
nunmai -p coder logs -f         # follow one profile
nunmai logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nunmai profile list             # profiles + model + gateway state
nunmai-gateways status          # full status across every profile
launchctl list | grep nunmai    # macOS — PIDs and labels
systemctl --user list-units 'nunmai-gateway-*'   # Linux — units
```
...
```
~/.nunmai/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.nunmai/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nunmai config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nunmai/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nunmai --why="gateways running" \
  sleep infinity &
...
`nunmai-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nunmai/.env ~/.nunmai/profiles/*/.env
```
...
never double-binds a running fleet). `nunmai update` folds them for you unless a
...
```bash
nunmai gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
nunmai gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
```
...
keeps `nunmai -p <name> gateway install --force` as its path — or opts out of
the host gateway with `gateway.standalone: true` (see
[No new per-profile gateways](#no-new-per-profile-gateways)), which
`nunmai gateway migrate --multiplex` respects.

### Docker / Nunmai Cloud (s6-supervised container)
...
to converge either: `nunmai gateway migrate --multiplex` (and the hook `nunmai
...
service) and `gateway.multiplex_profiles` is off, `nunmai update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `nunmai gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `nunmai update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `nunmai update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `nunmai profile create` produces.
...
| `NUNMAI_HOME` outside `<default home>/profiles/` | a unit pinning `NUNMAI_HOME=/opt/nunmai/profiles/emma` |

In that case `nunmai update` prints the boundary it found plus
`nunmai gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `nunmai gateway migrate --multiplex --dry-run`
...
```bash
nunmai config set gateway.auto_multiplex_migration false
```

`nunmai update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/nunmai/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`nunmai gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `nunmai gateway migrate --multiplex` with
...
`nunmai profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `nunmai profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.nunmai/gateway_migration.json` for rollback.
...
| A secondary profile enables a port-binding platform that has **no** `/p/<profile>/` ingress on the default listener | The multiplexer skips that whole profile (see [rule 2](#2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix)) | Disable the platform in that profile (`platforms.<name>.enabled: false`), or run the profile standalone: set `gateway.standalone: true` in its own `config.yaml` and wait for the host to rescan (at most 30 seconds), or send its `rescan-profiles` control verb. Use `nunmai -p <name> gateway install --force` only where a boundary blocks the fold. |
...
above). `nunmai profile create` confirms this when the live multiplexer picked the
profile up; it prints the `nunmai gateway restart` reminder only when it could not
...
window, the next `nunmai gateway migrate --multiplex` sees the flag on, the
...
`nunmai update` pulls the latest code once and syncs new bundled skills into
every profile:
...
boundary (different UNIX user, `NUNMAI_HOME` outside `profiles/`) and the
...
You ran `nunmai gateway start` after a previous `nunmai gateway stop`. The
...
```bash
ps -ef | grep "nunmai_cli.*-p <profile>"
cat ~/.nunmai/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nunmai.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nunmai.gateway-<profile>.plist

# Linux
systemctl --user restart nunmai-gateway-<profile>.service
```

### Health check

```bash
nunmai doctor                  # default profile
nunmai -p <profile> doctor     # one profile
```
