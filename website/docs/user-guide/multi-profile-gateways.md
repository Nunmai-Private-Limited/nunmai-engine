...
(`ai.nunmai.gateway-<name>.plist`) or systemd user service
(`nunmai-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
You do **not** run `nunmai gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `nunmai gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.nunmai/profiles/coder/.env`; the default listener key is rejected.
...
home). `nunmai status` reports the multiplexer and the profiles it serves;
`nunmai status -p <name>` slices to one profile. Each profile still writes its
