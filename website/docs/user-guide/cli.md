...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Nunmai Engine terminal interface — commands, keybindings, personalities, and more"
---
...
One command — `nunmai setup --portal` — and you're ready to `nunmai chat`. See [Nunmai Portal](../integrations/nous-portal.md).
:::

:::tip
Nunmai also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `nunmai --tui` — see the [TUI](tui.md) guide.
:::
...
nunmai chat --query-file prompt.txt
nunmai chat --query-file - < prompt.txt
...
nunmai chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
nunmai --continue             # Resume the most recent CLI session (-c)
nunmai --resume <session_id>  # Resume a specific session by ID (-r)
nunmai --resume latest        # Resume the most recent session (same as -c)
nunmai --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
nunmai chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nunmai -w                         # Interactive mode in worktree
nunmai -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`nunmai -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
nunmai worktree list              # audit: age, size, verdict, reason per tree
nunmai worktree list --json       # machine-readable audit (trees, external trees, branches)
nunmai worktree prune             # remove safe trees + delete merged branches
nunmai worktree prune --dry-run   # show the plan without changing anything
nunmai worktree prune --older-than 7   # only reap trees idle for 7+ days
nunmai worktree prune --trees-only     # leave local branches alone
nunmai worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running nunmai session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.nunmai/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `nunmai -w` for days — no longer accumulate merged scratch trees
...
The `nunmai plugins` commands manage native Nunmai plugins and portable Agent
...
nunmai plugins disable <plugin-name>
nunmai plugins update <plugin-name>
nunmai plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Nunmai currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Nunmai CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Nunmai CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `NUNMAI_YOLO_MODE` is on (either `nunmai --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Nunmai to background (Unix only). Run `fg` in the shell to resume. |
...
- **Runs on your machine, in the session working directory.** With the default local terminal backend `!pwd` matches what the agent would see. A remote or sandboxed `terminal.backend` (`ssh`, `docker`, …) is **not** used for `!` commands — they always run on the host where Nunmai itself runs, so `!hostname` names your machine while the agent's `terminal` tool names the backend. Ask the agent (or open a shell on the target) to run something *inside* the backend. Path completion in the composer, by contrast, does follow the configured backend and lists the target's filesystem.
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.nunmai/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Nunmai supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Nunmai replies |
...
```bash
nunmai -s nunmai-engine-dev,github-auth
nunmai chat -s github-pr-workflow -s github-auth
```

Nunmai loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
`--ignore-rules` (equivalently `NUNMAI_IGNORE_RULES=1`) skips auto-load together with AGENTS.md, SOUL.md, `.cursorrules` and memory injection; explicit `-s` skills still load. The setting is profile-scoped: each profile's `config.yaml` controls its own list.

## Skill Slash Commands

Every installed skill in `~/.nunmai/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.nunmai/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Nunmai..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Nunmai also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Nunmai recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Nunmai — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Nunmai is working, Nunmai prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Nunmai to the background — just like any terminal process. The shell prints a confirmation:

```
Nunmai Engine has been suspended. Run `fg` to bring Nunmai Engine back.
```
...
Resume this session with:
  nunmai --resume 20260225_143052_a1b2c3
...
```bash
nunmai --continue                          # Resume the most recent CLI session
nunmai -c                                  # Short form
nunmai -c "my project"                     # Resume a named session (latest in lineage)
nunmai --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nunmai --resume "refactoring auth"         # Resume by title
nunmai --resume latest                     # Resume the most recent session (same as -c)
nunmai --resume latest --in ./my-project   # Latest session for ./my-project's workspace
nunmai -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nunmai sessions rename <id> <title>` from the command line. Use `nunmai sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Nunmai's SQLite state database under `~/.nunmai/state.db`. The database keeps:
...
Nunmai immediately confirms the task and gives you back the prompt:
