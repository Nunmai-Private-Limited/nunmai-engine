...
title: "Git Worktrees"
description: "Run multiple Nunmai agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Nunmai Engine is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Nunmai so each session has a clean, isolated working directory.

## Why Use Worktrees with Nunmai?

Nunmai treats the **current working directory** as the project root:

- CLI: the directory where you run `nunmai` or `nunmai chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.nunmai/config.yaml`
...
Nunmai creates `.worktrees/my-experiment/` inside the repo (branch
`nunmai/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `nunmai-<id>`
...
git worktree add ../repo-feature feature/nunmai-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nunmai-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Nunmai there:

```bash
cd ../repo-feature

# Start Nunmai in the worktree
nunmai
```
...
git worktree add ../repo-experiment-a feature/nunmai-a
git worktree add ../repo-experiment-b feature/nunmai-b
```
...
- Works on its own branch (`feature/nunmai-a` vs `feature/nunmai-b`).
...
- Nunmai checkpoint data under `~/.nunmai/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Nunmai experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nunmai-checkpoints-docs`, `feature/nunmai-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Nunmai from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nunmai -w` (Automatic Worktree Mode)

Nunmai has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nunmai/nunmai-<hash>`).
...
```bash
nunmai -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nunmai -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Nunmai session its own clean checkout.
