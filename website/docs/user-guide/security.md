...
Nunmai Engine is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Nunmai checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.nunmai/config.yaml`:
...
| `timeout` | `300` | Seconds Nunmai waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `NUNMAI_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `nunmai --yolo` or `nunmai chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `NUNMAI_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `NUNMAI_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Nunmai shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
`NUNMAI_SUPERVISED_CHILD` marker they export.
...
the job uses KeepAlive or controls Nunmai.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Nunmai refuses to run them **regardless** of:
...
Configure the timeout in `~/.nunmai/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.nunmai/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.nunmai/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` nunmai/gateway | Self-termination prevention |
...
The `NUNMAI_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.nunmai/config.yaml`:
...
unattended ones: a cron job, `nunmai chat -q` run or webhook session under
...
re-save it with `nunmai config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `nunmai config edit` to review or remove patterns from your permanent allowlist.
:::

:::caution
The list is read when Nunmai starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart Nunmai. If you
removed it for safety reasons, restart.
:::

### Mining Approval History (`nunmai approvals suggest`)
...
```bash
nunmai approvals suggest            # dry run — prints a numbered proposal
nunmai approvals suggest --apply 1,3  # merge picks into command_allowlist
nunmai approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.nunmai/state.db`) for
...
Before `write_file` or `patch` touches disk, Nunmai checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `NUNMAI_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Nunmai secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under NUNMAI_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
Sensitive paths inside the safe root are still blocked — pointing `NUNMAI_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

The `~` in the OS-credential rows means *every* home a write can land in, not just the process `HOME`: the OS user's real home, the profile home (`{NUNMAI_HOME}/home` under `TERMINAL_HOME_MODE=profile`, containers and spawned workers, where the process `HOME` is pinned), and named accounts (`~root/.ssh/authorized_keys`). An absolute path to the real home's `~/.aws/credentials` is denied even when the agent process runs with `HOME` pointed elsewhere.

Safe-root violations return `Write denied: '…' is outside NUNMAI_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### NUNMAI_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/Nunmai-Private-Limited/nunmai-engine) (`NUNMAI_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.nunmai/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.nunmai/cron/jobs.json`, profile skills, or other Nunmai state outside that prefix

To allow both a workspace and Nunmai home:

```bash
export NUNMAI_WRITE_SAFE_ROOT=/path/to/project:/home/you/.nunmai
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [NUNMAI_WRITE_SAFE_ROOT](../reference/environment-variables.md#nunmai_write_safe_root).

### Cron and other Nunmai state

Do not ask the agent to `patch` `~/.nunmai/cron/jobs.json` directly. Use the `cronjob_manage` tool, [`nunmai cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Nunmai control files when write safety blocks direct edits.
...
When running the messaging gateway, Nunmai controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.nunmai/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.nunmai/.env to allow open access,
...
For more flexible authorization, Nunmai includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `nunmai pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.nunmai/config.yaml`:
...
nunmai pairing approve telegram ABC12DEF

# Revoke a user's access
nunmai pairing revoke telegram 123456789

# Clear all pending codes
nunmai pairing clear-pending
```

:::tip Docker users: run pairing commands as the `nunmai` user
The official Docker image runs the gateway as the unprivileged `nunmai` user
...
Always pass `-u nunmai`:

```bash
docker exec -u nunmai nunmai-engine nunmai pairing approve telegram ABC12DEF
```
...
**Storage:** Pairing data is stored in `~/.nunmai/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Nunmai applies strict security hardening to every container.
...
Container resources are configurable in `~/.nunmai/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.nunmai/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `NUNMAI_HOME`. Skills declare these in frontmatter:
...
When loaded, Nunmai checks if these files exist in the active profile's `NUNMAI_HOME` and registers them for mounting:
...
With the switch off Nunmai never reads or refreshes those files: the `claude_code` credential-pool row disappears, `nunmai auth list` prints one line saying so, and the log carries one INFO line per process. Only automatic adoption is affected — `nunmai auth add openai-codex` still asks before importing an existing Codex CLI login. Automatic recovery also only repairs the credential Nunmai already holds: a Codex CLI/Desktop login into a different ChatGPT workspace is refused with a warning (re-authenticate with `nunmai auth add openai-codex`), and a login you complete while recovery is running is never overwritten. Add your own logins with `nunmai auth add anthropic` / `nunmai auth add openai-codex`.
...
| **terminal** (local) | Blocks explicit Nunmai infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Nunmai infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Nunmai integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Nunmai under WSL.
...
Your own `SOUL.md` in `NUNMAI_HOME` is treated differently: it is a file you wrote (file-tool writes to it
need your approval, and project checkouts never supply it), so a scanner hit there **does not block the
file**. Nunmai logs a warning naming the matched pattern, loads the file as usual, and `/context` lists it as
...
the file. The exception does not extend to a `SOUL.md` shipped by a profile distribution: `nunmai profile
install <git-url>` and `nunmai profile update` copy a third party's `SOUL.md` into the profile home without
...
4. **Store secrets securely** — keep API keys in `~/.nunmai/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.nunmai/logs/` for unauthorized access attempts
10. **Keep updated** — run `nunmai update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.nunmai/.env`:
...
TERMINAL_SSH_USER=nunmai
TERMINAL_SSH_KEY=~/.ssh/nunmai_agent_key
```
...
Most third-party code Nunmai can run is gated by an explicit allow-list: general plugins need `plugins.enabled`, shell hooks need a first-use approval (or `hooks_auto_accept`), MCP servers are listed in config. One surface is deliberately different:

| Extension point | Loaded from | Loaded when | Opt-in |
|-----------------|-------------|-------------|--------|
| [Gateway event hooks](./features/hooks.md#gateway-event-hooks) | `<profile home>/hooks/<name>/` (`HOOK.yaml` + `handler.py`) | Gateway startup (`HookRegistry.discover_and_load()`), per served profile | **Placing the directory.** No `plugins.enabled` entry, no prompt; `NUNMAI_SAFE_MODE` does not skip it. |

The gateway imports every valid hook directory in-process, with the gateway's own privileges. This is the documented contract (since `3988c3c245f`), not an oversight: the profile home is operator-owned configuration, and anyone who can write into it can already run code as you through `config.yaml` shell hooks or by editing `plugins.enabled`, so a separate consent gate for `hooks/` would add friction without moving the trust boundary. Treat the contents of `~/.nunmai/hooks/` like the contents of `config.yaml` — review a `handler.py` before you place it, and include `ls ~/.nunmai/hooks/` whenever you audit the rest of the profile home (the directory is not on the [protected-paths denylist](#file-write-safety), so it is ordinary writable state). Full details: [gateway hook trust model](./features/hooks.md#gateway-hook-trust).

## Supply-chain advisory checking

Nunmai ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `nunmai_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `nunmai doctor` for the full remediation.
- **`nunmai doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Nunmai installs these **lazily** on first use rather than eagerly under `nunmai-engine[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `nunmai tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `nunmai tools`.
