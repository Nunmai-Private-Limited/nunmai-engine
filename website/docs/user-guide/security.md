...
| Nunmai credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under NUNMAI_HOME (active profile and global root) |
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/hermes-agent) (`NUNMAI_WRITE_SAFE_ROOT=/opt/data`)
...
Do not ask the agent to `patch` `~/.nunmai/cron/jobs.json` directly. Use the `cronjob` tool, [`nunmai cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Nunmai control files when write safety blocks direct edits.
