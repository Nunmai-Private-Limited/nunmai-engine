...
# Connecting Desktop to Many Nunmai Instances

Register every Nunmai backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Nunmai Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Nunmai gateway…"**) deep-links straight to
...
list of Nunmai gateways. Its intro says it plainly: *"Manage this device and
every Nunmai gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Nunmai runtime managed by this app." | automatic |
| **Remote gateway** | "A Nunmai gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Nunmai install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Nunmai Cloud** | "A hosted instance discovered through your Nunmai Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Nunmai Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Nunmai Cloud** kind in the add-connection
...
The Nunmai Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Nunmai Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Nunmai Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `nunmai serve` backend,
   ...
   - *SSH only:*
     ...
     - **Nunmai path (optional)** — full path to the `nunmai` executable on
       the remote (for example `/opt/nunmai/bin/nunmai`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `nunmai` on its `PATH` and **Test** reports *"Nunmai is not
...
Gateways are machines or hosted backends; profiles are isolated Nunmai agents
...
connection is registered) dispatches `nunmai update` to every eligible
...
- **Nunmai Cloud** instances are skipped with a *"Managed by Nunmai Cloud"*
...
panel, ⌘K **Update Nunmai**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `nunmai serve` is running on the remote host, the
...
- **"Update Nunmai Desktop to chat with agents on other connections"** — the
