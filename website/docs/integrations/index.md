...
Nunmai Engine connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Nunmai can do and where it can run.

:::tip Start here
If you only have time to set up one integration, set up [Nunmai Portal](./nous-portal.md) — a single OAuth login covers 300+ models plus the four Tool Gateway tools (web search, image generation, TTS, and browser automation).
:::

## AI Providers & Routing

Nunmai supports multiple AI inference providers out of the box. Use `nunmai model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](./providers.md)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Nunmai auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](../user-guide/features/mcp.md)** — Connect Nunmai to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Nunmai tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `nunmai tools`:
...
Nunmai includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](../user-guide/features/acp.md)** — Use Nunmai Engine inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Nunmai runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](../user-guide/features/api-server.md)** — Expose Nunmai as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Nunmai as a backend with its full toolset.
...
- **[Memory Providers](../user-guide/features/memory-providers.md)** — Plug in external memory backends for deeper personalization. Seven providers ship with Nunmai: Honcho (dialectic reasoning), OpenViking (tiered retrieval), Mem0 (cloud extraction), Holographic (local SQLite), RetainDB (hybrid search), ByteRover (CLI-based), and Supermemory; Hindsight (knowledge graphs) is available from the plugin catalog via `nunmai plugins install hindsight`.

## Messaging Platforms

Nunmai runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `nunmai slack manifest --agent-view` generates |
...
- **[Buzz](./buzz.md)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Nunmai as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Nunmai identity server-side, or the native gateway platform joins Buzz channels with full Nunmai memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](../user-guide/features/plugins.md)** — Extend Nunmai with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.nunmai/plugins/`, project-local `.nunmai/plugins/`, and pip-installed entry points.
- **[Build a Plugin](../developer-guide/plugins/index.md)** — Step-by-step guide for creating Nunmai plugins with tools, hooks, and CLI commands.
