...
The same subscription unlocks the [Tool Gateway](../user-guide/features/tool-gateway.md), which routes Nunmai Engine's tool calls through Nunmai-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.nunmai/auth.json` is the only credential on disk, and Nunmai mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
Nunmai Research's own **Hermes 4** family (Hermes-4-70B, Hermes-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Nunmai Engine**, however. Hermes 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for research workflows or via the [subscription proxy](../user-guide/features/subscription-proxy.md) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Nunmai-side opinion — it's the official guidance from Nunmai Research.
...
If you already have Nunmai configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `nunmai model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Nunmai is running. For remote hosts, see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup {#profile-setup}

If you use [Nunmai profiles](../user-guide/profiles.md), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
- **CLI shortcut:** `nunmai portal open` (opens the same page in your default browser)

## Configuration reference

After `nunmai setup --portal`, `~/.nunmai/config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/.nunmai/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Nunmai mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.
...
If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Nunmai stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `nunmai auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `nunmai portal info` shows "not logged in"
...
Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `nunmai auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues) — we surface the Portal's catalog to Nunmai and gaps usually mean a routing config we can update.
...
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-Nunmai tools (other agents, scripts, third-party clients)
...
- **[Profiles](../user-guide/profiles.md)** — Multiple Nunmai configurations sharing one Portal login
