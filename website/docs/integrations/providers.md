...
This page covers setting up inference providers for Nunmai Engine — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Nunmai.

## Inference Providers

You need at least one way to connect to an LLM. Use `nunmai model` to switch providers and models interactively, or configure directly:
...
| **Anthropic** | `nunmai model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.nunmai/.env`, or `nunmai auth add openrouter --type oauth` (browser login via OpenRouter's PKCE flow; stores a key in the credential pool) |
| **Ramp Router** | `RAMP_ROUTER_API_KEY` in `~/.nunmai/.env` (provider: `router`; aliases: `ramp-router`, `ramp`, `router.com`; Responses-native gateway, live account-scoped catalog) |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.nunmai/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.nunmai/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.nunmai/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.nunmai/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.nunmai/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.nunmai/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.nunmai/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.nunmai/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **Nebius Token Factory** | `NEBIUS_API_KEY` in `~/.nunmai/.env` (provider: `nebius-token-factory`; aliases: `nebius`, `nebius-tf`, `tokenfactory`) |
| **Actual Computer** | `ACTUAL_API_KEY` in `~/.nunmai/.env` for the hosted relay; set `model.base_url` in `config.yaml` for a local daemon (no key needed on loopback). Provider: `actual`; aliases: `actual-computer`, `actualcomputer`, `aci`. |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.nunmai/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.nunmai/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.nunmai/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `nunmai model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.nunmai/.env` (provider: `alibaba`; mainland-China endpoint: `alibaba-cn`) |
...
| **Alibaba Cloud (Token Plan)** | `ALIBABA_TOKEN_PLAN_API_KEY` in `~/.nunmai/.env` (provider: `alibaba-token-plan`; mainland-China endpoint: `alibaba-token-plan-cn` with `ALIBABA_TOKEN_PLAN_CN_API_KEY`, falling back to the shared key) — Model Studio flat-token tier |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.nunmai/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.nunmai/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.nunmai/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **Tencent TokenPlan** | `TOKENPLAN_API_KEY` in `~/.nunmai/.env` (provider: `tencent-tokenplan`, aliases: `tokenplan`, `tencent-lkeap`; Anthropic Messages endpoint) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.nunmai/.env` (provider: `opencode-zen`) |
| **CommandCode** | `COMMANDCODE_API_KEY` in `~/.nunmai/.env` (provider: `commandcode`, alias: `commandcode-chat`; Claude models via `commandcode-anthropic`, alias: `commandcode-claude`). Works with GOAT/Pro/Max/Provider plans (not the $1 Go plan — no API access). |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.nunmai/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.nunmai/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.nunmai/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.nunmai/.env` (provider: `gemini`) |
| **Google Vertex AI** | `nunmai model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.nunmai/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `nunmai model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `nunmai model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.nunmai/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `nunmai model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `nunmai model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `nunmai model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.nunmai/.env` (provider: `stepfun`) |
| **LM Studio** | `nunmai model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `nunmai model` → choose "Custom endpoint" (saved in `config.yaml`) |

Both built-in OpenCode providers send an opaque, per-conversation `x-opencode-session` header on every request (main turns on every transport plus auxiliary calls such as compression, titles, approval checks, skills-hub lookups and `/btw` side questions — including the ones that run in the background after the turn has ended; headless Kanban `specify`/`decompose` and dashboard estimate calls use a per-task key; one-shots with no live session at all, such as Desktop commit-message generation from the review panel, send a fresh ephemeral key). OpenCode uses it to pin a conversation to one backend so its prompt cache stays warm; the value is derived from the Nunmai session id (or the Kanban task id) and carries no personal data.

The two built-in OpenCode providers each pin their own relay on `opencode.ai` (`opencode-zen` → `/zen/v1`, `opencode-go` → `/zen/go/v1`). A `model.base_url` left behind by the other relay is healed to the selected provider's relay, and the model you pick (`-m`, `/model`, a fallback entry or a channel override) decides which relay is used — so switching from a Zen model to a Go-only one never sends the request to Zen. The same per-model routing is re-applied when a session is resumed (`nunmai --resume`, `/resume`, the TUI and desktop resume paths): a wire format or relay URL recorded while the session ran a different OpenCode model never carries over to the model the session is reopened on. OpenCode models whose id carries a `-vision` marker (for example `deepseek-v4-flash-vision-exp`) are treated as vision-capable even before models.dev lists them, so `agent.image_input_mode: auto` attaches images natively without a `supports_vision` override. A custom provider you define under `providers:` whose name extends a family slug (for example `opencode-go-bridge`) still gets the family's per-model API-mode routing and `/v1` handling, but its `base_url` is taken as declared: name it after the relay it actually points at. Auxiliary tasks (`auxiliary.compression`, titles, vision, MoA) pointed at an OpenCode provider follow the same per-model table, so a Responses-only model such as `gpt-5.6-luna` or an Anthropic-wire one such as `minimax-m2.5` works there exactly as it does for the main conversation.
...
**For full details:** see the dedicated [Nunmai Portal integration page](./nous-portal.md) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Nunmai Engine with Nunmai Portal guide](../guides/run-nunmai-with-nous-portal.md).
...
**JWT auth (automatic).** Nunmai prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code by default (open a URL, enter a code). Organizations that disable the device-code grant can opt in to the browser authorization-code + PKCE flow instead: `nunmai auth add openai-codex --browser` (one login) or `auth.codex_login_flow: browser` in `config.yaml` (every Codex login, including `nunmai model`). That flow listens on `http://localhost:1455/auth/callback` — the redirect URI registered for the Codex client, so the port is fixed; if it is already taken (a Codex CLI sign-in in progress) Nunmai says so and falls back to device code. Over SSH the listener needs a tunnel (`ssh -N -L 1455:127.0.0.1:1455 user@host`, see [OAuth over SSH](../guides/oauth-over-ssh.md)). Nunmai stores the resulting credentials in its own auth store under `~/.nunmai/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required. Automatic adoption of the Codex CLI login (when Nunmai' own refresh fails) is controlled by `auth.adopt_external_logins` — see [Borrowed CLI logins](../user-guide/security.md#borrowed-cli-logins).

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Nunmai marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `nunmai auth add openai-codex` (or `nunmai model` → **ChatGPT or Codex Subscription**) to start a fresh login (device code, or `--browser` for the loopback PKCE flow); the quarantine clears on the next successful exchange.

Device login can fail with `[SSL: UNEXPECTED_EOF_WHILE_READING]` or a TLS handshake timeout on Python/OpenSSL 3.5+ when a middlebox rejects post-quantum groups such as X25519MLKEM768 (curl may still work). A one-off dropped connection is not fatal: while waiting for your browser approval Nunmai keeps polling through up to six consecutive transport errors (and retries the device-code request and token exchange twice) before giving up, so only a persistently broken network surfaces this error. Nunmai does not change default TLS policy. Point `OPENSSL_CONF` at a config that restricts `Groups` to classic curves before running `nunmai model`, or diagnose with TLS 1.2:
...
Paid Nunmai Portal subscribers also get access to the **[Tool Gateway](../user-guide/features/tool-gateway.md)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `nunmai setup --portal` logs you in, sets Nunmai as your provider, and turns the gateway on in one command. Existing users can enable it from `nunmai model` or per-tool from `nunmai tools`. Inspect routing at any time with `nunmai portal info`.
:::

### Two Commands for Model Management

Nunmai has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`nunmai model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Nunmai chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `nunmai model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `nunmai model`, complete the provider setup, then start a new session.


### Subscription plans: what your plan pays for

Several providers let you sign in to Nunmai with a **consumer subscription** (Claude Max, ChatGPT, SuperGrok / X Premium+, …) instead of an API key. What that subscription actually pays for — and what it doesn't — differs per provider, and it's the single most common source of billing surprises. The table below is the short version; each provider's own section has the details.

> Cells marked *not currently documented* mean exactly that: Nunmai docs do not yet specify the behavior. Don't assume — check your provider's billing dashboard, and treat these as open questions.

| Plan / path | Can Nunmai use it? | What gets consumed | What does NOT get consumed | Common surprise |
|---|---|---|---|---|
| **Anthropic — Claude Max + OAuth** | ✅ Yes — `nunmai model` → Anthropic OAuth. Requires Max **and** purchased extra usage credits | The **extra/overage credits** you've added on top of the Max plan | The **base Max plan allowance** (the usage included in Claude Code by default) | All Nunmai usage bills as "extra usage" even while your included Max allowance sits untouched |
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `nunmai model` → **ChatGPT or Codex Subscription** (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
When no explicit environment credential is selected, Nunmai-owned OAuth grants
...
unless `auth.adopt_external_logins: false` is set, in which case Nunmai never
...
When you authenticate via `nunmai model` → Anthropic OAuth (or via `nunmai auth add anthropic --type oauth`), Nunmai routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Nunmai — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
nunmai chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `nunmai model`
# Nunmai will use Claude Code's credential store directly when available
nunmai model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
nunmai chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
nunmai chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `nunmai model`, Nunmai prefers Claude Code's own credential store over copying the token into `~/.nunmai/.env`. That keeps refreshable Claude credentials refreshable.
...
Nunmai supports GitHub Copilot as a first-class provider with two modes:
...
```bash
nunmai chat --provider copilot --model gpt-5.4
```
...
If no token is found, `nunmai model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `nunmai model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `nunmai model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Nunmai
Nunmai sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Nunmai now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Nunmai therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
nunmai chat --provider copilot-acp --model copilot-acp
...
| `NUNMAI_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `NUNMAI_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
nunmai chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.nunmai/.env

# NovitaAI Model API
nunmai chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.nunmai/.env

# Ramp Router (model IDs come from your account's live catalog)
nunmai chat --provider router --model gpt-5.4-mini
# Requires: RAMP_ROUTER_API_KEY in ~/.nunmai/.env

# z.ai / ZhipuAI GLM
nunmai chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.nunmai/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
nunmai chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.nunmai/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
nunmai chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.nunmai/.env

# MiniMax (global endpoint)
nunmai chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.nunmai/.env

# MiniMax (China endpoint)
nunmai chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.nunmai/.env

# Qwen Cloud / DashScope (Qwen models)
nunmai chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.nunmai/.env

# Xiaomi MiMo
nunmai chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.nunmai/.env

# Tencent TokenHub (Hy4 preview)
nunmai chat --provider tencent-tokenhub --model hy4-preview
# Requires: TOKENHUB_API_KEY in ~/.nunmai/.env

# Tencent TokenPlan (Hy4 preview via Anthropic Messages endpoint)
nunmai chat --provider tencent-tokenplan --model hy4-preview
# Requires: TOKENPLAN_API_KEY in ~/.nunmai/.env

# Arcee AI (Trinity models)
nunmai chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.nunmai/.env

# Meta Model API (Muse Spark family)
nunmai chat --provider meta-ai --model muse-spark-1.2
# Requires: MODEL_API_KEY in ~/.nunmai/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
nunmai chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.nunmai/.env

# Nebius Token Factory
nunmai chat --provider nebius --model deepseek-ai/DeepSeek-V4-Pro
# Requires: NEBIUS_API_KEY in ~/.nunmai/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `nunmai model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, Nunmai automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.nunmai/.env` and pick xAI in `nunmai model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `nunmai model`, or run `nunmai auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Nunmai runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Nunmai automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `nunmai tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `nunmai doctor` and `nunmai chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `nunmai migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped copy of the previous config lands in `backups/config/` first).

```bash
nunmai migrate xai          # preview replacements
nunmai migrate xai --apply  # rewrite ~/.nunmai/config.yaml in place
```
...
# Short alias
nunmai chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `nunmai model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Nunmai auto-discovers the available models.
...
nunmai chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 nunmai chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Nunmai lazy-installs `google-auth` on first use; run `nunmai setup` if the managed install needs repair. See the [Google Vertex AI guide](../guides/google-vertex.md) for the full walkthrough, and the [Google Gemini guide](../guides/google-gemini.md) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `nunmai model`, sign in through the browser, and Nunmai persists the refresh token.

```bash
nunmai model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.nunmai/auth.json

nunmai chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `NUNMAI_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Nunmai exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
nunmai chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `nunmai model`, sign in through the browser, and Nunmai persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
nunmai chat   # uses api.minimax.io/anthropic endpoint
```
...
nunmai chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.nunmai/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 nunmai chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Nunmai automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
nunmai chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.nunmai/.env
```
...
- **Local daemon** — on-device at `http://127.0.0.1:8080`, fully offline. No API key needed: Nunmai detects the loopback base URL and authenticates with an internal placeholder automatically.

```bash
# Hosted relay (ACTUAL_API_KEY in ~/.nunmai/.env)
nunmai chat --provider actual --model <model-id-from-your-cluster>

# Local daemon (model.base_url in ~/.nunmai/config.yaml, no key)
nunmai chat --provider actual --model <installed-model-name>
```

Store provider settings in `~/.nunmai/config.yaml`; only the hosted API key belongs in `.env`:
```yaml
model:
  provider: "actual"
  default: "<model-id>"
...
Notes:
- Model IDs come from your cluster's `GET /v1/models` — discover with `nunmai model` or `curl -s https://api.actual.inc/v1/models -H "Authorization: Bearer $ACTUAL_API_KEY"`.
...
- Small local models: Nunmai' full default toolset plus the system prompt can exceed a 32k context window, producing an empty-stream error from llama.cpp-family servers. Restrict the toolset (`-t file,web`) or load the model with a larger context. The optional `actual-setup` skill (`nunmai skills install official/devops/actual-setup`) covers setup and troubleshooting in detail.
...
```bash
# StepFun
nunmai chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.nunmai/.env
```
...
```bash
# Use any available model
nunmai chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.nunmai/.env

# Short alias
nunmai chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Nunmai Engine works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Nunmai at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `nunmai model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `nunmai setup` or config migration.
:::
...
:::warning nunmai model vs /model
**`nunmai model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Nunmai chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `nunmai model`, set up the new provider, then start a new session.
:::
...
When switching providers, Nunmai persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Nunmai Engine requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Nunmai.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Nunmai) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `nunmai` (Qwen 2.5, Nunmai 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** Nunmai preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
If responses seem truncated, check the server's generation default and configure it on the server (for example SGLang's `--default-max-tokens`). Nunmai does not expose an output-token cap setting.
:::
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Nunmai. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below Nunmai' minimum per active session.

Then configure Nunmai to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Nunmai won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Nunmai 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Nunmai preserves the context of an already-loaded LM Studio instance. For an unloaded model in the default explicit mode, Nunmai omits `context_length` unless you configured one in Nunmai, so LM Studio can apply its own model setting. Nunmai then uses only the context length LM Studio reports after loading.
...
If you use LM Studio's Just-In-Time loading / Auto-Evict feature and want LM Studio to manage model loading and eviction from normal chat requests, skip Nunmai' explicit preload step:

```bash
nunmai config set model.lmstudio_load_mode jit
```

Set it back to the default explicit preload behavior with:

```bash
nunmai config set model.lmstudio_load_mode explicit
```

**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Nunmai) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Nunmai Engine requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Nunmai config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Nunmai config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Nunmai.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Nunmai inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser nunmai` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Nunmai's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Nunmai thinks the context is
...
Nunmai auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Nunmai uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output limit on the server** — configure the server's generation default (for example SGLang's `--default-max-tokens`). Nunmai does not expose an output-token cap setting. Response length is distinct from the conversation's context window (`context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](../user-guide/configuration.md#context-compression) in Nunmai.
...
Then configure Nunmai with `nunmai model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Nunmai with `nunmai model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `nunmai model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Nunmai uses this to decide when to compress history and to validate API requests.

Output limits govern a single generated response, not the conversation history.
Nunmai no longer reads `model.max_tokens`, `NUNMAI_MAX_TOKENS`, provider output-cap
...
`max_tokens`, so Nunmai supplies an internal value. Bedrock Converse is a separate
...
`nunmai model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
providers:
  local:
    ...
    # api_key omitted — Nunmai uses "no-key-required" for keyless local servers
  work:
    api: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    transport: chat_completions   # set explicitly by `nunmai model` → Custom Endpoint wizard; auto-detection still happens as a fallback
  anthropic-proxy:
...
`custom_providers:` entries, including `nunmai model` setup. Helpers run only when
...
:::note Legacy format
Older configs used a top-level `custom_providers:` list instead. It still works — Nunmai reads both — and `nunmai update` auto-migrates it to the `providers:` dict (config v12). Field names differ slightly in the dict format: legacy `model` is `default_model`, and legacy `api_mode` is `transport`.
:::

**Context window on `codex_responses` proxies.** A custom entry with `transport: codex_responses` (a local Codex proxy, for example) resolves the context window of Codex OAuth models (`gpt-6-astra`, `gpt-5.6-sol`/`-terra`/`-luna`, `gpt-5.5`, …) from the Codex OAuth table — 272K for most slugs — not from the 1.05M direct-API catalog, so compression fires before the Codex backend's limit and its 272K billing tier. The decision follows the transport, not the hostname; the same holds for `openai-codex` behind `NUNMAI_CODEX_BASE_URL` or `model.base_url`. A per-model `models.<id>.context_length`, an entry-level `context_length`, or `model.context_length` still wins; the opt-in `-900k` picker variants keep their verified 900K.

**Reasoning effort on custom endpoints.** The configured `reasoning_effort` (`/reasoning max`, `agent.reasoning_effort`) reaches a custom endpoint unchanged on both the `chat_completions` and the `codex_responses` transport — up to `max`; only the Nunmai-internal `ultra` is clamped to `max`. Two exceptions follow the host rather than the entry: a custom entry pointed at `api.openai.com` keeps OpenAI's per-model ladder (`max` is a gpt-5.6-only level there), and an entry pointed at a provider whose profile publishes a per-model vocabulary (Ramp Router) is clamped to that catalog. An endpoint that rejects the level answers with an HTTP 400 instead of Nunmai silently downgrading it. When no effort is configured at all, `chat_completions` requests carry `reasoning_effort: medium` — the same default the Nunmai Portal and OpenRouter routes apply — rather than leaving the endpoint's own default in charge (kimi-k3's is `max`, about 3x the reasoning tokens of `medium`); models marked `supports_reasoning: false` in the catalog or `model_overrides`, and Ollama models without the `thinking` capability, keep the field off.

Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Nunmai will merge it into each chat-completions request for that endpoint:
...
The `nunmai model` → Custom Endpoint wizard now prompts for the API mode explicitly and persists your answer to `config.yaml` (as `transport` on the provider entry). URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Nunmai routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
**Inheriting a catalogued vendor's metadata (`catalog_provider`).** When a named custom provider (a gateway, proxy or reseller) serves models that Nunmai already knows under a built-in provider, point the entry at that vendor and its models inherit the catalogued context window, output limit, vision and reasoning flags — no `model_overrides` needed:
...
`catalog_provider` accepts a Nunmai provider id (`deepseek`, `anthropic`, `openai`, …) or a models.dev id. It affects metadata lookups only — requests still go to your `api` URL with your credentials — and an explicit `model_overrides` entry for the same model still wins.
...
You can also select named custom providers from the interactive `nunmai model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under the `providers:` dict. Three worked recipes follow. Each drops into `~/.nunmai/config.yaml` and the matching API key goes in `~/.nunmai/.env`.
...
Together's `/v1/models` endpoint works, so `nunmai model` can auto-discover available models.
...
:::tip Troubleshooting
- `nunmai doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `nunmai model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `nunmai model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Nunmai at your instance (no API key needed):
   ```bash
   nunmai config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.nunmai/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.nunmai/config.yaml`:
...
Configure a chain of backup providers Nunmai tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `nunmai fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](../user-guide/features/fallback-providers.md).
:::
