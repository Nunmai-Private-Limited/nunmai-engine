...
| **Actual Computer** | `ACTUAL_API_KEY` in `~/.nunmai/.env` for the hosted relay, or `ACTUAL_BASE_URL=http://127.0.0.1:8080` for the local daemon — no key needed on loopback (provider: `actual`; aliases: `actual-computer`, `actualcomputer`, `aci`) |
...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.nunmai/.env` (provider: `alibaba`) |
...
| **OpenCode Free** | Keyless — no API key or account needed (provider: `opencode-free`, aliases: `free`, `opencode_free`). Select via `nunmai model` or `/model free`; requests are sent anonymously |
...
nunmai chat --provider tencent-tokenhub --model hy3-preview
...
Your own hardware as a private inference cluster via [Actual Computer](https://actual.inc). Two serving modes, both OpenAI-compatible (Nunmai uses the Responses API transport):
...
# Local daemon (ACTUAL_BASE_URL=http://127.0.0.1:8080 in ~/.nunmai/.env, no key)
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Nunmai with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
