...
description: "Run Nunmai Engine directly on an Android phone with Termux"
---

# Nunmai on Android with Termux
...
Nunmai Engine can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Nunmai from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Nunmai installs are marked with install method `apt`. Nunmai therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade nunmai-engine
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Nunmai runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Nunmai now ships a Termux-aware installer path:
...
- links `nunmai` into `$PREFIX/bin` so it stays on your Termux PATH
...
Nunmai requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `nunmai` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nunmai" "$PREFIX/bin/nunmai"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nunmai` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nunmai/.env`.
...
`uvloop` bundles libuv, whose `./configure` does not run on Android. Nunmai keeps it out of the core install and of the `termux` / `termux-all` extras — `uvicorn` falls back to the stdlib asyncio loop, which the dashboard and gateway run fine on. If you see this error, you are installing an extra that includes `uvloop` (for example `[all]`): install `.[termux]` or `.[termux-all]` instead.

### `nunmai doctor` says ripgrep or Node is missing
