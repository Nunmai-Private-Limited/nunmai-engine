...
- **Quick Setup (Nunmai Portal)** — OAuth login, no API keys to manage; sets up a model plus the Tool Gateway tools, billed to your [Nunmai Portal subscription](/integrations/nous-portal). The recommended fast path.
...
- **[Moving to another machine](/reference/faq#exporting-nunmai-to-another-machine)** — `nunmai backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
