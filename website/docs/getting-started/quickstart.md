---
sidebar_position: 1
title: "Nunmai Engine Quickstart"
description: "Your first conversation with Nunmai Engine — from install to chatting in under 5 minutes"
---

# Nunmai Engine Quickstart

This guide gets you from zero to a working Nunmai setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Nunmai Engine Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Nunmai Engine Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Nunmai for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Nunmai working on my machine | `nunmai setup` | Run a real chat and verify it responds |
| I already know my provider | `nunmai model` | Save the config, then start chatting |
| I want a bot or always-on setup | `nunmai gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `nunmai model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `nunmai model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Nunmai cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Nunmai Engine
### With the Nunmai Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Nunmai Desktop installer](https://nunmai.in/) from our website and run it.

### Without Nunmai Desktop:
For a command-line only install without Nunmai Desktop, run:
...
The single most important setup step. Use `nunmai model` to walk through the choice interactively:

```bash
nunmai model
```

:::tip Easiest path: Nunmai Portal
...
:::info Setup modes
On a fresh install, `nunmai setup` offers three modes:

- **Quick Setup (Nunmai Portal)** — OAuth login, no API keys to manage; sets up a model plus the Tool Gateway tools, billed to your [Nunmai Portal subscription](../integrations/nous-portal.md). The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `nunmai update`. Re-enable anything later with `nunmai tools`, seed skills with `nunmai skills opt-in --sync`, or tune settings with `nunmai setup agent`.
:::
...
|----------|-----------|---------------|
| **Nunmai Portal** | Subscription-based, zero-config | OAuth login via `nunmai model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `nunmai model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `nunmai model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `nunmai_cli/models.py` may change between releases) | `nunmai model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `nunmai model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `nunmai model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `nunmai model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `nunmai model` (requires `copilot` CLI + `copilot login`) |
...
Nunmai Engine requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `nunmai model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Nunmai separates secrets from normal config:

- **Secrets and tokens** → `~/.nunmai/.env`
- **Non-secret settings** → `~/.nunmai/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
nunmai config set model anthropic/claude-opus-4.6
...
```bash
nunmai            # classic CLI
nunmai --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Nunmai ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `nunmai` vs `nunmai --tui`.
:::
...
```bash
nunmai --continue    # Resume the most recent session
nunmai -c            # Short form
```
...
```bash
nunmai gateway setup    # Interactive platform configuration
```
...
- `nunmai tools` — tune tool access per platform
- `nunmai skills` — browse and install reusable workflows
...
```bash
nunmai config set terminal.backend docker    # Docker isolation
nunmai config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `nunmai egress setup && nunmai egress start`; `nunmai setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Nunmai install directory (the curl installer placed it at
# ~/.nunmai/nunmai-engine on Linux/macOS or %LOCALAPPDATA%\nunmai\nunmai-engine on Windows):
cd ~/.nunmai/nunmai-engine
...
Skills are on-demand instruction documents that teach Nunmai how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Nunmai ships with a catalog of bundled skills already installed in `~/.nunmai/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
nunmai skills browse                      # list everything available
nunmai skills search kubernetes           # find skills by keyword
nunmai skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `nunmai skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Nunmai ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.nunmai/nunmai-engine && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Nunmai opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `nunmai model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `nunmai gateway setup` and check `nunmai gateway status` |
| `nunmai --continue` can't find old session | Switched profiles or session never saved | Check `nunmai sessions list` and confirm you're in the right profile |
...
| `nunmai doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `nunmai model`
3. `nunmai setup`
4. `nunmai sessions list`
5. `nunmai --continue`
6. `nunmai gateway status`
...
| Command | Description |
|---------|-------------|
| `nunmai` | Start chatting |
| `nunmai model` | Choose your LLM provider and model |
| `nunmai tools` | Configure which tools are enabled per platform |
| `nunmai setup` | Full setup wizard (configures everything at once) |
| `nunmai doctor` | Diagnose issues |
| `nunmai update` | Update to latest version |
| `nunmai gateway` | Start the messaging gateway |
| `nunmai --continue` | Resume last session |
...
- **[Moving to another machine](../reference/faq.md#exporting-nunmai-to-another-machine)** — `nunmai backup` migrates your whole setup (or [a single profile](../reference/faq.md#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
