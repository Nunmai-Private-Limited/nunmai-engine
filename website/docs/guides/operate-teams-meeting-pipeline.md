...
```bash
nunmai teams-pipeline validate
```
...
```bash
nunmai teams-pipeline token-health
nunmai teams-pipeline token-health --force-refresh
```
...
```bash
nunmai teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
nunmai teams-pipeline maintain-subscriptions
nunmai teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Nunmai cron (recommended if you already run the Nunmai gateway)

Nunmai ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.nunmai/scripts/`. First create the script:

```bash
mkdir -p ~/.nunmai/scripts
cat > ~/.nunmai/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec nunmai teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.nunmai/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
nunmai cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/nunmai-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Nunmai Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=nunmai
EnvironmentFile=/etc/nunmai/env
ExecStart=/usr/local/bin/nunmai teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/nunmai-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Nunmai Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now nunmai-teams-pipeline-maintain.timer
systemctl list-timers nunmai-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/nunmai teams-pipeline maintain-subscriptions >> /var/log/nunmai/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nunmai/.env` at the top of a wrapper script that crontab calls.
...
```bash
nunmai teams-pipeline subscriptions   # should show expirationDateTime advanced
nunmai teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
nunmai teams-pipeline list
nunmai teams-pipeline list --status failed
nunmai teams-pipeline show <job-id>
```

### Replay a stored job

```bash
nunmai teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
nunmai teams-pipeline fetch --meeting-id <meeting-id>
nunmai teams-pipeline fetch --join-web-url "<join-url>"
nunmai teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `nunmai teams-pipeline maintain-subscriptions --dry-run`
- inspect `nunmai teams-pipeline list --status failed`
...
- run `nunmai teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nunmai teams-pipeline run`
...
- [ ] `nunmai teams-pipeline validate` returns an OK snapshot
- [ ] `nunmai teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Nunmai cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
