...
Run `nunmai setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Nunmai Portal](../integrations/nous-portal.md).
:::
...
For a full walkthrough, see [Use SOUL.md with Nunmai](./use-soul-with-nunmai.md).
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`nunmai prompt-size`](../reference/cli-commands.md#nunmai-prompt-size) (works offline).
:::
