...
description: "Diagnose and fix common Nunmai cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`nunmai gateway` for foreground, or `nunmai gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nunmai cron tick`.
...
```bash
date
nunmai cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nunmai/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nunmai/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nunmai/.env` |
...
| `local` | Write access to `~/.nunmai/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nunmai cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Nunmai Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nunmai skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `nunmai skills list`.
...
2. `~/.nunmai/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nunmai cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Nunmai config directory). Verify:
```bash
ls ~/.nunmai/scripts/your-script.py   # Must exist
nunmai cron edit <job_id> --script ~/.nunmai/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nunmai skills install <skill-name>`.
...
Jobs are stored in `~/.nunmai/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nunmai/cron/jobs.json
chmod 600 ~/.nunmai/cron/jobs.json   # Your user should own it
```
...
```bash
nunmai cron list                    # Show all jobs, states, next_run times
nunmai cron run <job_id>            # Schedule for next tick (for testing)
nunmai cron edit <job_id>           # Fix configuration issues
nunmai logs                         # View recent Nunmai logs
nunmai skills list                  # Verify installed skills
```
...
1. Run the job with `nunmai cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.nunmai/logs/agent.log` for scheduler messages and `~/.nunmai/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/hermes-agent](https://github.com/Nunmai-Private-Limited/nunmai-engine) with:
