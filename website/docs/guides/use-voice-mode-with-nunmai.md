---
sidebar_position: 8
title: "Use Voice Mode with Nunmai"
description: "A practical guide to setting up and using Nunmai voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Nunmai
...
:::tip
[Nunmai Portal](../integrations/nous-portal.md) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Nunmai sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Nunmai.
...
## Step 1: make sure normal Nunmai works first
...
Nunmai supports both local and cloud speech stacks.
...
### If you use `nunmai setup`

If you choose NeuTTS in the setup wizard, Nunmai checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Nunmai transcribes and responds
...
- asking Nunmai to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Nunmai loop.

## Tuning CLI behavior

### Silence threshold

If Nunmai starts/stops too aggressively, tune:
...
Nunmai stays a normal chat bot, but can speak replies.
...
- you want Nunmai to function like a portable research or ops assistant
...
Nunmai joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Nunmai detects speech boundaries
- transcripts are posted in the associated text channel
- Nunmai responds in text and audio
...
1. get text Nunmai working
2. run `nunmai setup tts` to enable voice support
