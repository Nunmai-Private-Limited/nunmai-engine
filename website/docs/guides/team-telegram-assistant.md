...
This tutorial walks you through setting up a Telegram bot powered by Nunmai Engine that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Nunmai Engine installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nunmai/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Nunmai itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Nunmai Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nunmai_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Nunmai Engine. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Nunmai Gateway...
...
# View live logs
journalctl --user -u nunmai-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nunmai gateway install` to update the plist.
:::
...
```bash
nunmai gateway stop && nunmai gateway start
```
...
nunmai pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nunmai/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nunmai/config.yaml`:
...
Customize how the bot communicates by editing `~/.nunmai/SOUL.md`:

For a full guide, see [Use SOUL.md with Nunmai](/guides/use-soul-with-nunmai).
...
```bash
# From the CLI
nunmai cron list          # View all scheduled jobs
nunmai cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nunmai-gateway` (Linux) or `~/.nunmai/logs/gateway.log` (macOS) |
| Cron job output | `~/.nunmai/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nunmai/cron/jobs.json` |
| Pairing data | `~/.nunmai/pairing/` |
| Session history | `~/.nunmai/sessions/` |
