...
sidebar_position: 16
title: "Google Gemini"
description: "Use Nunmai Engine with Google Gemini — native AI Studio API, API-key setup, tool calling, streaming, and quota guidance"
---

# Google Gemini

Nunmai Engine supports Google Gemini as a native provider using the **Google AI Studio / Gemini API** — not the OpenAI-compatible endpoint. This lets Nunmai translate its internal OpenAI-shaped message and tool loop into Gemini's native `generateContent` API while preserving tool calling, streaming, multimodal inputs, and Gemini-specific response metadata.

## Prerequisites

- **Google AI Studio API key** — create one at [aistudio.google.com/apikey](https://aistudio.google.com/apikey)
- **Billing-enabled Google Cloud project** — recommended for agent use. Gemini's free tier is too small for long-running agent sessions because Nunmai may make several model calls per user turn.
- **Nunmai installed** — no extra Python package is required for the native Gemini provider.

:::tip API key path
Set `GOOGLE_API_KEY` or `GEMINI_API_KEY`. Nunmai checks both names for the `gemini` provider.
:::
...
echo "GOOGLE_API_KEY=..." >> ~/.nunmai/.env

# Select Gemini as your provider
nunmai model
# → Choose "More providers..." → "Google AI Studio"
# → Nunmai checks your key tier and shows Gemini models
# → Select a model
...
After running `nunmai model`, your `~/.nunmai/config.yaml` will contain:
...
Nunmai detects this endpoint and creates its native Gemini adapter. Internally, Nunmai still keeps the agent loop in OpenAI-shaped messages, then translates each request to Gemini's native schema:
...
- streaming responses → OpenAI-shaped stream chunks for the Nunmai loop
...
For Gemini 3 tool use, Nunmai preserves the `thoughtSignature` values attached to function-call parts and replays them on the next tool turn. That covers the validation-critical path for multi-step agent workflows.

Gemini 3 may also attach thought signatures to other response parts. Nunmai' native adapter is optimized for agent tool loops today, so it does not yet replay every non-tool-call signature with full part-level fidelity.
:::
...
doesn't end with an API version segment (`v1beta`, `v1alpha`, `v1`, ...), Nunmai
...
prefix no longer identifies its surface. Nunmai never reroutes by key shape: the
...
express-mode key, and Nunmai completes it to the `publishers/google` form. Each
surface only accepts its own keys — a `403 PERMISSION_DENIED` usually means the
key/host pairing is crossed, and Nunmai appends guidance naming the other surface.
...
Earlier Nunmai releases detected the `AQ.` prefix and rerouted such keys to
...
`nunmai doctor`, TTS — now goes to the AI Studio host and a Vertex express key
gets `403 PERMISSION_DENIED` there. Add `GEMINI_BASE_URL=https://aiplatform.googleapis.com`
to `~/.nunmai/.env` (or set `base_url` on the provider) once and restart.
:::

## Available Models

The `nunmai model` picker shows Gemini models maintained in Nunmai' provider registry. Common choices include:
...
Model availability changes over time. If a model disappears or is not enabled for your key, run `nunmai model` again and pick one from the current list.
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Nunmai config. Note that your usage charges may be affected if newer models introduce different rates.
...
Google also exposes Gemma models through the Gemini API. Nunmai recognizes these as Google models, but hides very low-throughput Gemma entries from the default model picker so new users do not accidentally select an evaluation-tier model for a long-running agent session.
...
These models are best treated as evaluation options on Gemini API keys. Google's Gemma API pricing is free-tier-only and the usage caps are low compared with production Gemini models, so sustained Nunmai agent use should normally move to a paid Gemini model, a self-hosted deployment, or another provider with appropriate quota.
...
If you have not configured Gemini yet, exit the session and run `nunmai model` first. `/model` switches among already-configured providers and models; it does not collect new API keys.
...
Gemini works with all Nunmai gateway platforms (Telegram, Discord, Slack, WhatsApp, LINE, Feishu, etc.). Configure Gemini as your provider, then start the gateway normally:
...
Nunmai could not find a usable API key. Add one of these to `~/.nunmai/.env`:
...
Then run `nunmai model` again.

### "This Google API key is on the free tier"

Nunmai probes Gemini API keys during setup. Free-tier quotas can be exhausted after a handful of agent turns because tool use, retries, compression, and auxiliary tasks may require multiple model calls.
...
The selected model is not available for your account, region, or key. Run `nunmai model` again and pick another Gemini model from the current list.

### Gemma model is not shown in `nunmai model`

Nunmai may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.nunmai/config.yaml`.
...
Upgrade Nunmai and rerun `nunmai model`. The native Gemini adapter sanitizes tool schemas for Gemini's stricter function-declaration format; older builds or custom endpoints may not.
