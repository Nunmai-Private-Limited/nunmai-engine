...
Run `/reload-mcp` after changing the config. See [Use MCP with Nunmai](/guides/use-mcp-with-nunmai) for the full filtering model (`include`/`exclude`, `prompts`, `resources`).
...
The OAuth browser callback runs on the machine where Nunmai is running. On a remote host, forward the loopback port over SSH — the same pattern as any other OAuth login. See [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh).
