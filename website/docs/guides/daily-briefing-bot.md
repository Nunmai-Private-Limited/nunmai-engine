...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `nunmai setup --portal`. See [Nunmai Portal](../integrations/nous-portal.md).
:::
...
2. **Nunmai spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.nunmai/cron/output/` and you can read them anytime.
:::
...
Nunmai will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Nunmai Engine
```
...
Before creating cron jobs, ensure Nunmai has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Nunmai what you want:
...
Nunmai will create the cron job for you using the unified `cronjob_manage` tool.
...
For faster briefings, tell Nunmai to delegate each topic to a sub-agent:
...
Nunmai will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
