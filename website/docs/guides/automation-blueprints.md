...
Copy-paste blueprints for common automation patterns. Each blueprint uses Nunmai's built-in [cron scheduler](../user-guide/features/cron.md) for time-based triggers and [webhook platform](../user-guide/messaging/webhooks.md) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nunmai webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nunmai webhook subscribe`) |
...
```bash
nunmai cron create "0 2 * * *" \
  "You are a project manager triaging the Nunmai-Private-Limited/nunmai-engine GitHub repo.

1. Run: gh issue list --repo Nunmai-Private-Limited/nunmai-engine --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
nunmai webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
nunmai cron create "0 9 * * 1" \
  "Scan the Nunmai-Private-Limited/nunmai-engine repo for documentation drift.

1. Run: gh pr list --repo Nunmai-Private-Limited/nunmai-engine --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (nunmai_cli/commands.py, nunmai_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (nunmai_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
nunmai cron create "0 6 * * *" \
  "Run a dependency security audit on the nunmai-engine project.

1. cd ~/.nunmai/nunmai-engine && source .venv/bin/activate
...
```bash
nunmai webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
nunmai webhook subscribe alert-triage \
...
```python title="~/.nunmai/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Nunmai-Monitor/1.0"})
...
nunmai cron create "every 30m" \
  ...
  --script ~/.nunmai/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
nunmai cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Nunmai Engine development." \
...
```bash
nunmai webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
nunmai webhook subscribe stripe-payments \
...
```bash
nunmai cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the nunmai-engine codebase.
