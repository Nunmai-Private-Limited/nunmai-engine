...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.nunmai/.env` at the end.
...
4. Fill in:
   - **Name:** `Nunmai Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `nunmai-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.nunmai/.env`:
...
Nunmai ships a Graph auth smoke-test. From your Nunmai install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.nunmai/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `nunmai gateway restart`.
