...
3. Sets `model.provider: nous` in `~/.nunmai/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Nunmai is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
nunmai setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
nunmai auth add nous --type oauth
# Then re-run `nunmai setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Nunmai Engine release notes" and summarize the top 3 hits.
```

You should see Nunmai call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`nunmai setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
nunmai config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Hermes-4 for agent work

Hermes-4-70B and Hermes-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](../user-guide/features/subscription-proxy.md) from non-agent tools. For Nunmai Engine itself, stick to the frontier agentic models above.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Nunmai will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Nunmai profiles](../user-guide/profiles.md) (e.g. a separate config per project), each profile is an independent credential island: a profile that has never signed in to the Portal fails closed instead of adopting another profile's session. Sign in once per profile with `nunmai -p <name> portal` — when a shared Portal session already exists on the machine it offers to import it without a browser round-trip, and from then on the shared token store keeps that profile's token current. See [Profile setup](../integrations/nous-portal.md#profile-setup).

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.nunmai/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `nunmai portal info` shows "not logged in" after `nunmai setup --portal`
...
```bash
nunmai config set model.provider nous
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Nunmai doesn't replay it endlessly. Just log in again:

```bash
nunmai auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/Nunmai-Private-Limited/nunmai-engine/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`nunmai portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Nunmai profiles where you're using the wrong one (check `nunmai profile list`)

### Want to revoke and start clean

```bash
nunmai auth logout nous       # wipes the local refresh token
...
- **[Nunmai Portal integration page](../integrations/nous-portal.md)** — Overview of what's in the subscription
- **[Tool Gateway](../user-guide/features/tool-gateway.md)** — Full details on every gateway-routed tool
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-Nunmai tools
...
- **[Profiles](../user-guide/profiles.md)** — Share one Portal login across multiple Nunmai configurations
