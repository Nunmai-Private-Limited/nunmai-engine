...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `nunmai send`."
---

# Pipe Script Output to Messaging Platforms

`nunmai send` is a small, scriptable CLI that pushes a message to any
messaging platform Nunmai is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | nunmai send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `nunmai
...
nunmai send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
nunmai send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
nunmai send --list

# Filter by platform
nunmai send --list telegram
```
...
Any platform Nunmai ships adapters for works as a target:
...
`nunmai send` resolves the message body in this order:

1. **Positional argument** — `nunmai send --to telegram "hi"`
2. **`--file PATH`** — `nunmai send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | nunmai send --to telegram`

When stdin is a TTY (no pipe), Nunmai does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  nunmai send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `nunmai send` reuses your Nunmai config, the same script works on
any host where Nunmai is installed — no need to export bot tokens into
...
`nunmai send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  nunmai send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | nunmai send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.nunmai/bin/nunmai send \
...
```bash
./train.py --epochs 200 && \
  nunmai send --to telegram "training done" || \
  nunmai send --to telegram "training failed (exit $?)"
```
...
nunmai send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(nunmai send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `nunmai send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `nunmai send` calls
the platform's REST endpoint directly using credentials from
`~/.nunmai/.env` and `~/.nunmai/config.yaml` (or the equivalent files under
your resolved Nunmai home — `%LOCALAPPDATA%\nunmai` on Windows, or the profile
directory when `NUNMAI_HOME` / `-p` is set). It's a standalone subprocess
...
`Looked in: C:\Users\me\AppData\Local\nunmai\.env (no DISCORD_BOT_TOKEN),
C:\Users\me\AppData\Local\nunmai\config.yaml (no platforms.discord block),
...
`.env` so `nunmai send` can use it. When your shell is scoped to a profile home
(`NUNMAI_HOME=<root>/profiles/<name>`) but the connected gateway runs from the
default root, the error says so — the gateway never read the profile's `.env`,
and `nunmai send --list` points at the root's `channel_directory.json`.
...
pointing at the gateway; start it with `nunmai gateway start` and retry.
...
# Machine-readable
nunmai send --list --json
```

The listing is built from `~/.nunmai/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`nunmai gateway
...
| Approach | Multi-platform | Reuses Nunmai creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `nunmai send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`nunmai send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `nunmai send`.
...
- [Gateway Internals](../developer-guide/gateway-internals.md) —
  the delivery router that `nunmai send` shares with cron delivery.
