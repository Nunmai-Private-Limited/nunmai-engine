...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Nunmai Engine  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Nunmai instantly when PRs are opened or updated.
:::
...
- **Nunmai Engine installed** — see the [Installation guide](../getting-started/installation.md)
- **Gateway running** for cron jobs:
  ```bash
  nunmai gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.nunmai/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Nunmai can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Nunmai-Private-Limited/nunmai-engine --state open --limit 3
```
...
Still in the chat, ask Nunmai to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Nunmai-Private-Limited/nunmai-engine
```
...
A skill gives Nunmai consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.nunmai/skills/code-review
```

Create `~/.nunmai/skills/code-review/SKILL.md`:
...
Verify it loaded — start `nunmai` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Nunmai your team's standards:
...
2. Teach Nunmai your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
nunmai gateway status    # Is the gateway running?
nunmai cron list         # Is the job enabled?
```
