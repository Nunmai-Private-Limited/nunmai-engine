...
**Your data goes only where you point it.** API calls go **only to the LLM provider you configure**. Nunmai Engine does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.nunmai/`. See the [FAQ](/reference/faq#is-my-data-sent-anywhere).
...
For `ssh`, set `terminal.backend: ssh` in `config.yaml` and provide host details via `TERMINAL_SSH_HOST`, `TERMINAL_SSH_USER`, and `TERMINAL_SSH_KEY` in `~/.nunmai/.env`. See [Network Isolation](/user-guide/security#network-isolation).
