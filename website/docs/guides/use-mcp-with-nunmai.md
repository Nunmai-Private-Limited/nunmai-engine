---
sidebar_position: 6
title: "Use MCP with Nunmai"
description: "A practical guide to connecting MCP servers to Nunmai Engine, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Nunmai

This guide shows how to actually use MCP with Nunmai Engine in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Nunmai tool
- you want Nunmai to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Nunmai to internal APIs, databases, or company systems without modifying Nunmai core

Do not use MCP when:
- a built-in Nunmai tool already solves the job well
...
- Nunmai remains the agent
- MCP servers contribute tools
- Nunmai discovers those tools at startup or reload time
...
If you installed Nunmai with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Nunmai banner/status should show MCP integration when configured
- ask Nunmai what tools it has available
- use `/reload-mcp` after config changes
- check logs if the server failed to connect
- run `nunmai mcp test <server>` from a shell — it connects, lists the
...
## WSL2: bridge Nunmai in WSL to Windows Chrome

This is the practical setup when:

- Nunmai runs inside WSL2
...
In this setup, Nunmai does **not** connect to Chrome directly. Instead:

- Nunmai runs in WSL
- Nunmai starts a local stdio MCP server
...
```text
Nunmai (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Nunmai stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Nunmai core browser transport
...
```bash
nunmai mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
nunmai mcp test chrome-devtools-win
```

Then start a fresh Nunmai session or run:
...
Once loaded, Nunmai can use the MCP-prefixed browser tools directly. For example:
...
If Nunmai runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Nunmai from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Nunmai from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Nunmai:
...
2. Nunmai-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Nunmai will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Nunmai to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Nunmai to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Nunmai remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
nunmai mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
nunmai mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `nunmai mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Nunmai to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Nunmai to merge, publish, deploy, or spawn runtimes.
...
Now Nunmai can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Nunmai core.
...
Because Nunmai now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
