...
const BACKEND = process.env.NUNMAI_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `nunmai dashboard` server injects a one-shot
 * session token into `index.html` (see `nunmai_cli/web_server.py`). The
 ...
 * production builds.
 */
function nunmaiDevToken(): Plugin {
  const TOKEN_RE = /window\.__NUNMAI_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__NUNMAI_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;
  const INITIAL_PROFILE_RE =
    /window\.__NUNMAI_INITIAL_PROFILE__\s*=\s*("(?:\\.|[^"\\])*")/;

  return {
    name: "nunmai:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[nunmai] Could not find session token in ${BACKEND} — ` +
              `is \`nunmai dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__NUNMAI_SESSION_TOKEN__="${match[1]}";` +
              `window.__NUNMAI_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};` +
              `window.__NUNMAI_INITIAL_PROFILE__=${initialProfileJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[nunmai] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`nunmai dashboard\` or set NUNMAI_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@nunmai/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `nunmai dashboard` must serve these; Vite has no
