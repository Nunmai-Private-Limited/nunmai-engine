declare global {
  interface Window {
    __NUNMAI_DASHBOARD_PROFILE__?: string;
    __NUNMAI_INITIAL_PROFILE__?: string;
  }
}

export function dashboardInitialProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__NUNMAI_INITIAL_PROFILE__ ?? "";
}

/**
 * The profile this backend process itself serves, injected by the server and
 * empty when it cannot be named unambiguously (custom NUNMAI_HOME).
 *
...
export function dashboardServingProfile(): string {
  if (typeof window === "undefined") return "";
  return window.__NUNMAI_DASHBOARD_PROFILE__ ?? "";
}
