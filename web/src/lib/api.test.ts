...
beforeEach(() => {
  ...
  Object.defineProperty(window, "__NUNMAI_SESSION_TOKEN__", {
    configurable: true,
    value: "stale-token",
    writable: true,
  });
  Object.defineProperty(window, "__NUNMAI_AUTH_REQUIRED__", {
    configurable: true,
    value: false,
    writable: true,
  });
...
describe("management profile scope", () => {
  ...
  it("falls back to the profile this backend serves when nothing is selected", async () => {
    vi.stubGlobal("window", { __NUNMAI_DASHBOARD_PROFILE__: "served" });
    const fetchMock = jsonFetchMock();
    vi.stubGlobal("fetch", fetchMock);
    setManagementProfile("");
...
describe("api OAuth helpers", () => {
  it("starts OAuth login in gated mode without requiring an injected session token", async () => {
    vi.stubGlobal("window", { __NUNMAI_AUTH_REQUIRED__: true });
    const fetchMock = jsonFetchMock({
      flow: "device_code",
  ...
  it("still sends the injected session token for OAuth login in loopback mode", async () => {
    vi.stubGlobal("window", { __NUNMAI_SESSION_TOKEN__: "loopback-token" });
    const fetchMock = jsonFetchMock({
      flow: "device_code",
      session_id: "oauth-session",
    });
