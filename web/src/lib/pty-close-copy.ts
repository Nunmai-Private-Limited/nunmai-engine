...
export const PTY_TOKEN_MISSING_BANNER: PtyBanner = {
  text: "Chat can't connect because this page was opened without a login token. Reload the page, or start it again with `nunmai dashboard` in a terminal.",
  action: 'reload'
}
...
const REJECTION_BANNERS: Record<number, PtyBanner> = {
  ...
  4403: {
    text: 'The dashboard refused this chat connection because the page address does not match the server it was opened from. Open the dashboard from the address `nunmai dashboard` printed.',
    action: null
  },
  4404: {
    text: 'This Nunmai server does not offer the terminal chat. Update Nunmai (`nunmai update`) and reload the page.',
    action: 'reload'
  },
  4408: {
    text: 'This Nunmai server only accepts chat from the machine it runs on. Open the dashboard on that machine, or start it with a public bind.',
    action: null
  }
}
...
export const PTY_GAVE_UP_BANNER: PtyBanner = {
  text: 'Lost connection to the Nunmai dashboard server. If you stopped `nunmai dashboard`, start it again; otherwise click Reconnect now.',
  action: 'check-server'
}

/** Overlay copy: the nunmai --tui child exited; a crash looks identical to `/exit`. */
