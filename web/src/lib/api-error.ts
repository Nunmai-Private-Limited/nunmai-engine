...
export const API_UNREACHABLE_MESSAGE =
  "Nunmai dashboard cannot reach the Nunmai service. Is `nunmai dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Nunmai service hit an internal error.",
  502: "The dashboard proxy could not reach the Nunmai service.",
  503: "The Nunmai service is not ready yet. Try again in a moment.",
  504: "The Nunmai service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Nunmai service answered with an unexpected error (${status}).`;
}
