...
// (e.g. https://mission-control.tilos.com/nunmai/). The Python backend
// injects ``window.__NUNMAI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__NUNMAI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const NUNMAI_BASE_PATH = readBasePath();
const BASE = NUNMAI_BASE_PATH;
...
declare global {
  interface Window {
    __NUNMAI_SESSION_TOKEN__?: string;
    __NUNMAI_BASE_PATH__?: string;
    ...
    __NUNMAI_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Nunmai-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__NUNMAI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``nunmai update``, ``nunmai gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__NUNMAI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__NUNMAI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__NUNMAI_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``nunmai_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__NUNMAI_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Nunmai-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``nunmai_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__NUNMAI_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildNunmaiWebSocketUrl({
...
export const api = {
  ...
  updateNunmai: () =>
    fetchJSON<ActionResponse>("/api/nunmai/update", { method: "POST" }),
  checkNunmaiUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/nunmai/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Nunmai Portal contract V1 (the access token has no email/name claims —
...
/** Preflight for `nunmai gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** nunmai-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``nunmai update``. */
  can_update_nunmai?: boolean;
  config_path: string;
   ...
   * ``nunmai gateway migrate --multiplex`` runs. null/absent when it multiplexes or only one
   * profile exists. */
  ...
  /** NS-656: disk-usage rollup for the NUNMAI_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the NUNMAI_HOME volume — no staleness dimension, so no
 * sampled_at. */
