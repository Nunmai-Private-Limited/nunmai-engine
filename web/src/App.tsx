...
const SIDEBAR_COLLAPSED_KEY = "nunmai-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="nunmai-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="nunmai-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateNunmai = status?.can_update_nunmai === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'nunmai update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "nunmai update";
    return (
      t.status.updateNunmaiConfirmMessage ??
      `This will run 'nunmai update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateNunmaiConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateNunmai) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateNunmai,
      runningLabel: t.status.updatingNunmai,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Nunmai gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateNunmaiConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateNunmaiConfirmTitle ?? `${t.status.updateNunmai}?`}
    />
    </>
  );
}
