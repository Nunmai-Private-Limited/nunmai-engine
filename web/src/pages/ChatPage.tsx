/**
 * ChatPage — embeds `nunmai --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (nunmai_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "nunmai.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__NUNMAI_SESSION_TOKEN__ &&
    !window.__NUNMAI_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `nunmai dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("nunmai-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("nunmai-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__NUNMAI_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Nunmai TUI has enabled xterm mouse-events
    ...
    //      payloads upload to NUNMAI_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to NUNMAI_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[nunmai-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__NUNMAI_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets NUNMAI_HOME pointed at the
    ...
    // forwarding them into Nunmai. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="nunmai-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
