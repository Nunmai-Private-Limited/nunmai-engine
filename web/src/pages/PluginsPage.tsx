...
const MEMORY_PROVIDER_BUILTIN = "__nunmai_memory_builtin__";
  ...
  if (!hasDetails || !setup) {
    return (
      <p className="border border-destructive/50 px-3 py-2 text-xs text-destructive">
        This provider is installed but unavailable. It may need local dependencies or a manual setup step before Nunmai can activate it.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className={isBlocked ? "text-destructive" : "text-muted-foreground"}>
        {needsDependencySetup
          ? "Finish these setup steps before Nunmai can activate this provider."
      ...
      {setup.required_env.length && needsDependencySetup ? (
        <div className="grid gap-2">
          <p className="text-muted-foreground">
            Required environment values. Fill the matching fields below, or set them in the Nunmai environment.
          </p>
          <div className="flex flex-wrap gap-2">
                ...
                <div className="flex flex-col gap-4 min-w-0">
                  ...
                  {!selectedMemoryName && (
                    <p className="text-xs text-muted-foreground">
                      Nunmai will use the built-in MEMORY.md and USER.md files.
                    </p>
                  )}
        ...
        <div className="flex flex-col gap-3" data-testid="plugin-catalog-section">
          ...
          <p className="text-xs tracking-[0.06em] text-text-tertiary">
            {t.pluginsPage.catalogHint ??
              "Curated, Nunmai-reviewed plugins pinned to exact commits."}
          </p>
          ...
          {catalogLoading ? (
            ...
            <p className="text-xs text-text-tertiary">
              ...
              <a
                className="underline"
                href="https://nunmai.in/docs/plugins"
                target="_blank"
                rel="noreferrer"
              >
                {t.pluginsPage.catalogEmptyDocsLink ?? "Learn about Nunmai plugins"}
              </a>
            </p>
          ) : (
    ...
    <Card className={cn(busy ? "opacity-70" : undefined)}>
      <CardContent className="flex flex-col gap-3 px-6 py-4">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-text-tertiary">
          ...
          {entry.requires_nunmai ? (
            <span>nunmai {entry.requires_nunmai}</span>
          ) : null}
