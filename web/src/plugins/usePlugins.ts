...
import { api, NUNMAI_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "nunmai:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${NUNMAI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${NUNMAI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?nunmai_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-nunmai-plugin", manifest.name);
