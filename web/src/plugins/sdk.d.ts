/**
 * Nunmai Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__NUNMAI_PLUGIN_SDK__`` and
 * ``window.__NUNMAI_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__NUNMAI_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``NunmaiPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@nunmai/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__NUNMAI_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__NUNMAI_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__NUNMAI_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface NunmaiPluginSDK {
  ...
  /**
   * Shared UI primitives (Nunmai DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __NUNMAI_PLUGIN_SDK__?: NunmaiPluginSDK;
    __NUNMAI_PLUGINS__?: PluginRegistry;
  }
}
