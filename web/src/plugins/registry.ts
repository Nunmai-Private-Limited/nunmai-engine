 ...
 *
 * Plugins call window.__NUNMAI_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__NUNMAI_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__NUNMAI_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__NUNMAI_PLUGIN_SDK__ = {
    ...
    // window.__NUNMAI_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Nunmai DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
