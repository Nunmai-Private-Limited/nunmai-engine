"""Firecrawl web search + extract provider (direct SDK, keyless cloud, or Nunmai tool-gateway).
...
def _use_keyless_ring() -> bool:
    """Route via the keyless ring only with no direct credentials, when the managed Nunmai
...
def _is_tool_gateway_ready() -> bool:
    """True when gateway URL + Nunmai Subscriber token are available."""
    return _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.peek_nous_access_token) is not None


def check_firecrawl_api_key() -> bool:
    """True when the route selected via ``nunmai tools`` (or, on a never-configured
...
def _firecrawl_backend_help_suffix() -> str:
    return ", or use the Nunmai Tool Gateway via your subscription (FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN)" if _backend_helpers.managed_nous_tools_enabled() else ""
...
def _get_firecrawl_client() -> Any:
    ...
    managed fallback billed to Nunmai); never-configured → direct when present, else managed. Raises ValueError
    ...
    def _unconfigured_message() -> str:
        ...
        if _backend_helpers.managed_nous_tools_enabled():
            return message + " With your Nunmai subscription you can also use the Tool Gateway. run `nunmai tools` and select Nunmai Subscription as the web provider."
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        resolved, log, message = _managed(), "the Nunmai Subscription web selection is stored but the tool gateway is unavailable.", lambda: selection_error(
            "web", NOUS_MANAGED_PROVIDER, "the Nunmai Tool Gateway is not available (not entitled or unreachable)")
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return setup_schema(
            ...
            "Full search + extract; supports keyless cloud, direct API, and Nunmai tool-gateway routing.",
