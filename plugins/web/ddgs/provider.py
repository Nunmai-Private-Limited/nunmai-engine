...
``nunmai tools`` can prompt the user to install it.
...
GIL — so the whole Nunmai process freezes through Ctrl+C/SIGTERM. Each search
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``nunmai tools`` probes.
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        ``primp`` call cannot freeze the Nunmai process (#36776, #68096).
        """
        try:
