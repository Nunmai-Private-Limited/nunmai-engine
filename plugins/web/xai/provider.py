...
``{title, url, description, position}`` rows every other Nunmai web
provider produces.
...
prefers Nunmai-managed xAI Grok OAuth (via ``nunmai auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.nunmai/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``nunmai tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `nunmai auth` to sign in with "
