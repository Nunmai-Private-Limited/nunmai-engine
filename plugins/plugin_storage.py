"""Per-plugin persistent storage: ``<nunmai home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<nunmai home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `nunmai plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<nunmai home>/plugin-data/<name>/``; resolves ``get_nunmai_home()`` on
