...
``<nunmai home>/plugins/<name>/``. That tree is the plugin *install* dir —
``nunmai plugins remove`` deletes it and ``nunmai plugins update`` git-pulls
...
``<nunmai home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Nunmai.
...
# Mirrors the plugin-name shape `nunmai plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<nunmai home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`nunmai_constants.get_nunmai_home` on every call. Don't cache the
    result across profile switches.
    """
    from nunmai_constants import get_nunmai_home

    root = get_nunmai_home() / "plugin-data" / _validate_name(name)
