...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "nunmai-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "nunmai"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > nunmai-<profile> > "default" (API auto-creates "default").
        ...
        if not project:
            profile_name = os.path.basename(str(kwargs.get("nunmai_home", "")))
            project = f"nunmai-{profile_name}" if profile_name not in {"", ".nunmai"} else "default"
