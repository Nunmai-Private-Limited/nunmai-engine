...
| 4 | `per-session` strategy | Nunmai session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nunmai-engine`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nunmai`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-nunmai-engine`.
...
- **`per-directory`** — basename of `$PWD`. Opening nunmai in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
