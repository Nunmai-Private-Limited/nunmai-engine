...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/nunmai>
...
```bash
nunmai memory setup honcho   # configure Honcho directly (works on a fresh install)
nunmai memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
nunmai config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.nunmai/.env
```

> `nunmai honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `nunmai memory setup honcho`.
...
Set `"recallSync": true` in `$NUNMAI_HOME/honcho.json` (at the root or under
`hosts.nunmai`), or enable **Current-query recall** in the memory settings or
`nunmai honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `nunmai model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NUNMAI_HOME/honcho.json` | Profile-local (isolated Nunmai instances) |
| 2 | `~/.nunmai/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Nunmai profile: `nunmai` (default) or `nunmai_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `nunmai honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 2 | `per-session` strategy | Nunmai session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config) | `"myproject-main"` |
| 4 | Explicit `/title` command (non-automatic title) | `"refactor-auth"` |
| 5 | `per-repo` strategy | Git root directory name (`nunmai-engine`) |
...
Automatically generated Nunmai titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-nunmai-engine`.
...
- **`per-session`** — Nunmai session ID (timestamp + hex). Every `nunmai` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Nunmai profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nunmai`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Nunmai profile: `nunmai` (default) or `nunmai_<profile>` (e.g. `nunmai -p coder` -> host key `nunmai_coder`). Older `nunmai.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `NUNMAI_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `nunmai-engine`) |
...
|---------|-------------|
| `nunmai memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `nunmai honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `nunmai memory setup`) |
| `nunmai honcho status` | Show resolved config for active profile |
| `nunmai honcho enable` / `disable` | Toggle Honcho for active profile |
| `nunmai honcho mode <mode>` | Change recall or observation mode |
| `nunmai honcho peer --user <name>` | Update user peer name |
| `nunmai honcho peer --ai <name>` | Update AI peer name |
| `nunmai honcho tokens --context <N>` | Set context token budget |
| `nunmai honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nunmai honcho map <name>` | Map current directory to a session name |
| `nunmai honcho sync` | Create host blocks for all Nunmai profiles |
