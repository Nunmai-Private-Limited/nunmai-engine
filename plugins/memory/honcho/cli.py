...
def _setup_identity_mapping(cfg: dict, nunmai_host: dict, current_peer: str) -> None:
...
def _apply_grant_to_host(nunmai_host: dict, cred) -> None:
...
def _setup_device_login(nunmai_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(nunmai_host, cred)
        return True
    return False


def _setup_browser_login(nunmai_host: dict, write_path: Path) -> bool:
...
def _setup_cloud_auth(cfg: dict, nunmai_host: dict, write_path: Path) -> bool:
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(nunmai_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(nunmai_host, write_path)
...
def cmd_setup(args) -> None:
    ...
    _setup_identity_mapping(cfg, nunmai_host, current_peer)
