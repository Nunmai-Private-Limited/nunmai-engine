...
Handles: nunmai honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "nunmai" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Nunmai profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'nunmai honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'nunmai honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `nunmai update` -- no output, no exceptions.
    """
    try:
        from nunmai_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Nunmai profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $NUNMAI_HOME/honcho.json so each profile/instance gets
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        nunmai_host.get("pinUserPeer"),
        nunmai_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in nunmai_host:
        aliases_src = nunmai_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in nunmai_host:
        prefix_src = nunmai_host.get("runtimePeerPrefix")
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom nunmai_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    nunmai_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
...
def _echo_identity_mapping(nunmai_host: dict) -> None:
    ...
    aliases = nunmai_host.get("userPeerAliases")
    prefix = nunmai_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(nunmai_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _apply_runtime_prefix(
        nunmai_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/nunmai (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Nunmai persistent cross-session memory.")
    ...
    nunmai_host = hosts.setdefault(_host_key(), {})
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("  Re-run 'nunmai honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'nunmai honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'nunmai honcho setup' again once you have a key.\n")
                return
    ...
    if new_peer:
        nunmai_host["peerName"] = new_peer

    current_ai = nunmai_host.get("aiPeer") or cfg.get("aiPeer", "nunmai")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        nunmai_host["aiPeer"] = new_ai

    current_workspace = nunmai_host.get("workspace") or cfg.get("workspace", "nunmai")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        nunmai_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Nunmai GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Nunmai gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(nunmai_host)
            nunmai_host["pinUserPeer"] = False
            if prior_aliases:
                nunmai_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                nunmai_host, current_prefix, prefix_from_root,
            ...
            if merged:
                nunmai_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                nunmai_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                nunmai_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = nunmai_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        nunmai_host["observationMode"] = new_obs
    else:
        nunmai_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(nunmai_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        nunmai_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        nunmai_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = nunmai_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        nunmai_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = nunmai_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    current_dialectic = str(nunmai_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    current_reasoning = (
        nunmai_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        nunmai_host["dialecticReasoningLevel"] = new_reasoning
    else:
        nunmai_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = nunmai_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        nunmai_host["sessionStrategy"] = new_strat
    ...
    try:
        ...
        print("  Run: nunmai config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    nunmai honcho status     -- show full config")
    print("    nunmai honcho mode       -- change recall/observation mode")
    print("    nunmai honcho tokens     -- tune context and dialectic budgets")
    print("    nunmai honcho peer       -- update peer names")
    print("    nunmai honcho map <name> -- map this directory to a session name\n")
...
def _active_profile_name() -> str:
    """Return the active Nunmai profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("nunmai.work") stay readable per the README's back-compat
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'nunmai honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'nunmai honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: nunmai honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        nunmai = hosts.get(_host_key(), {})
        user = nunmai.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = nunmai.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = nunmai.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = nunmai.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Nunmai' identity in Honcho. Seed with 'nunmai honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: nunmai honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: nunmai honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = nunmai.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = nunmai.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = nunmai.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Nunmai asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'nunmai honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'nunmai honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    nunmai honcho identity --show        — show both peer representations")
        print("    nunmai honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  nunmai honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'nunmai honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'nunmai honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  nunmai honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'nunmai'")
        print("  with Honcho configured and no prior session history.")
        print("  (Nunmai calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Nunmai, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    nunmai honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  nunmai honcho identity <path/to/SOUL.md>")
    ...
    print("    Nunmai:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Nunmai:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Nunmai:   Honcho observes every message and updates representations")
    ...
    print("    Nunmai:   per-session by default — each run gets its own session")
    print("              Map a custom name:  nunmai honcho map <session-name>")
    ...
    if not has_key:
        ...
        print("  2. nunmai honcho migrate            — re-run this walkthrough")
    else:
        print("  1. nunmai honcho status             — verify Honcho connection")
        print("  2. nunmai                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. nunmai honcho identity --show    — verify AI peer representation")
        print("  4. nunmai honcho tokens             — tune context and dialectic budgets")
        print("  5. nunmai honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'nunmai memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``nunmai honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``nunmai honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to nunmai memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Nunmai Honcho",
    )
