...
``nunmai://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Nunmai OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``nunmai-desktop`` / ``nunmai-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Nunmai.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local NUNMAI_HOME override can't reach.
