...
Config file resolution: $NUNMAI_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def resolve_active_host() -> str:
    """Honcho host key: NUNMAI_HONCHO_HOST env, else the active profile. The config's
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $NUNMAI_HOME/honcho.json -> default profile's honcho.json
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Nunmai spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_nunmai_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "nunmai", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), nunmai_home=get_nunmai_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Nunmai profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, nunmai_home=get_nunmai_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_nunmai_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
