...
Before running `nunmai memory setup`, add the local endpoint to
...
Then run `nunmai memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$NUNMAI_HOME/supermemory.json`
...
| `container_tag` | `nunmai` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nunmai-{identity}` → `nunmai-coder`). |
...
`https://api.supermemory.ai`. Nunmai resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: nunmai`, and document writes stamp
`metadata.sm_source: nunmai`. This is a **functional routing key, not telemetry**:
it groups Nunmai-written memories into a dedicated "Nunmai" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Nunmai profile:

```json
{
  "container_tag": "nunmai-{identity}"
}
```

For a profile named `coder`, this resolves to `nunmai-coder`. The default profile resolves to `nunmai-default`. Without `{identity}`, all profiles share the same container.
