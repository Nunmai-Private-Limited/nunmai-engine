...
def _load_supermemory_config(nunmai_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $nunmai_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if nunmai_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(nunmai_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, nunmai_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(nunmai_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "nunmai"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Nunmai writes into the "Nunmai" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "nunmai", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, nunmai_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(nunmai_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._nunmai_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `nunmai memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, nunmai_home):
        ...
        _save_supermemory_config(sanitized, nunmai_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from nunmai_constants import get_nunmai_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_nunmai_home())))}

    def post_setup(self, nunmai_home: str, config: dict) -> None:
        from nunmai_cli.config import save_config
        from nunmai_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, nunmai_home=nunmai_home)
        ...
        status = _probe_supermemory_connection(api_key, nunmai_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from nunmai_constants import get_nunmai_home
        self._nunmai_home = kwargs.get("nunmai_home") or str(get_nunmai_home())
        self._session_id, self._turn_count, self._pending_turns = session_id, 0, []
        config = _load_supermemory_config(self._nunmai_home)
    ...
    def _write_turns(self, mode: str, new_turn: Optional[Dict[str, str]] = None) -> None:
        ...
        with self._capture_lock:
            ...
            for sid in dict.fromkeys(t["session_id"] for t in turns):
                ...
                metadata = {"type": "conversation", "session_id": sid, "timestamp": now.isoformat()}  # no sm_capture_mode: Nunmai policy
