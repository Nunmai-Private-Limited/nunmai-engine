...
def _load_supermemory_config(nunmai_home: str) -> dict:
...
class _SupermemoryClient:
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Nunmai writes into the "Nunmai" Space in the Supermemory
...
def _resolve_container_tag_for_setup(nunmai_home: str, *, identity: str = "default") -> str:
...
def _probe_supermemory_connection(api_key: str, nunmai_home: str, *, identity: str = "default") -> dict:
    ...
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(nunmai_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nunmai memory setup`.
        # All other options are documented for $NUNMAI_HOME/supermemory.json
    ...
    def post_setup(self, nunmai_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(env_writes, nunmai_home=nunmai_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nunmai_home)
