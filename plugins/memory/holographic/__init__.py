"""nunmai-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $NUNMAI_HOME/config.yaml under plugins.nunmai-memory-store: db_path ($NUNMAI_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from nunmai_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "nunmai-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nunmai_home):
        """Write config to config.yaml under plugins.nunmai-memory-store."""
        config_path = Path(nunmai_home) / "config.yaml"
        try:
            import yaml
            from nunmai_cli.config import read_user_config_raw  # raw read: merged defaults must not be persisted
            existing = read_user_config_raw(config_path)
            existing.setdefault("plugins", {})["nunmai-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_nunmai_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _nunmai_home = str(get_nunmai_home())
        db_path = self._config.get("db_path", _nunmai_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $NUNMAI_HOME so paths resolve to the active profile
            db_path = db_path.replace("$NUNMAI_HOME", _nunmai_home).replace("${NUNMAI_HOME}", _nunmai_home)
