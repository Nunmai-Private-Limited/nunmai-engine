...
_DEFAULT_DB_PATH = "$NUNMAI_HOME/memory_store.db"
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nunmai_home):
        ...
        # ``merge_existing`` keeps every other section; *nunmai_home* is the active profile already.
        ...
        if isinstance(db_path, str) and Path(db_path).expanduser() == Path(nunmai_home) / "memory_store.db":
            values["db_path"] = _DEFAULT_DB_PATH
        save_config({"plugins": {"nunmai-memory-store": values}}, merge_existing=True)
