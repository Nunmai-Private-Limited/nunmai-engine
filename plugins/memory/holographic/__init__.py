"""nunmai-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $NUNMAI_HOME/config.yaml (profile-scoped):
  plugins:
    nunmai-memory-store:
      db_path: $NUNMAI_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "nunmai-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nunmai_home):
        ...
        try:
            ...
            existing["plugins"]["nunmai-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_nunmai_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _default_db = _nunmai_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $NUNMAI_HOME in user-supplied paths so config values like
        # "$NUNMAI_HOME/memory_store.db" or "~/.nunmai/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$NUNMAI_HOME", _nunmai_home)
            db_path = db_path.replace("${NUNMAI_HOME}", _nunmai_home)
