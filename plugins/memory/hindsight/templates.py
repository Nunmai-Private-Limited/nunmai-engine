...
the ``nunmai`` integration, and applies a chosen manifest to the user's bank
...
def fetch_nunmai_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``nunmai`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "nunmai" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_nunmai_templates()
