"""`nunmai memory setup` wizard for the Hindsight provider (``post_setup``)."""
...
def _prompt_embedded_llm(llm_provider: str, provider_config: dict, env_writes: dict, nunmai_env: Path) -> None:
    ...
    env_writes["HINDSIGHT_LLM_API_KEY"] = llm_key or _load_simple_env(nunmai_env).get("HINDSIGHT_LLM_API_KEY", "")


def run_setup(provider, nunmai_home: str, config: dict) -> None:
    ...
    env_writes: dict = {}
    nunmai_env = Path(nunmai_home) / ".env"
    ...
    if mode == "cloud":
        ...
        _prompt_embedded_llm(llm_provider, provider_config, env_writes, nunmai_env)
    ...
    if env_writes:
        _write_env(nunmai_env, env_writes)
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            materialized_config = json.loads(
                (Path(nunmai_home) / "hindsight" / "config.json").read_text(encoding="utf-8")
            )
        llm_api_key = (
            env_writes.get("HINDSIGHT_LLM_API_KEY", "")
            or _load_simple_env(nunmai_env).get("HINDSIGHT_LLM_API_KEY", "")
