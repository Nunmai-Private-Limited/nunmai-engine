...
Config: $NUNMAI_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Nunmai Engine and the User"
...
def _context_thread(target, name: str) -> threading.Thread:
    ...
    closed without the profile's secret scope + NUNMAI_HOME override. (The shared
...
def _load_config() -> dict:
    """$NUNMAI_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_nunmai_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
        if path.exists():
    ...
    return {
        ...
        "banks": {"nunmai": {"bankId": os.environ.get("HINDSIGHT_BANK_ID", "nunmai"),
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Nunmai event time with an explicit UTC offset."""
    event_time = _nunmai_now()
    # nunmai_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "nunmai", "mid", ""
    ...
    def save_config(self, values, nunmai_home):
        """Merge *values* into $NUNMAI_HOME/hindsight/config.json."""
    ...
    def post_setup(self, nunmai_home: str, config: dict) -> None:
        ...
        run_setup(self, nunmai_home, config)
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "nunmai"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "nunmai"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "nunmai", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "nunmai"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            ...
            logger.warning(msg)
            # Also print: otherwise the user would only see Nunmai get sluggish.
            with contextlib.suppress(Exception):
                # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
                # the user would only see Nunmai get sluggish. (issue #13125)
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_nunmai_home() / "logs" / "hindsight-embed.log"
