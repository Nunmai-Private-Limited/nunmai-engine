...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    at import, so Nunmai degrades instead of retrying a broken backend).
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    on the Nunmai .env during post_setup, where a Notepad BOM would stick to the first key."""
    if not path.exists():
        return {}
...
def _embedded_profile_env_path(config: dict[str, Any]) -> Path:
    profile = str(config.get("profile", "nunmai") or "nunmai")
