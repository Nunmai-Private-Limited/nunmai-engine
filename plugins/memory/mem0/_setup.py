...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # nunmai_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(nunmai_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(nunmai_home) / "mem0.json"
    existing = {}
...
def _setup_platform(nunmai_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "nunmai-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "nunmai"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.nunmai/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.nunmai/.env (or unset it) or Nunmai will keep "
    ...
    provider.save_config(provider_config, nunmai_home)
...
def _setup_selfhosted(nunmai_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "nunmai-user"
    )
...
def _setup_oss(nunmai_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "nunmai-user")
    ...
    if env_writes:
        _write_env(Path(nunmai_home) / ".env", env_writes)
    _save_mem0_json(nunmai_home, {"mode": "oss", "user_id": user_id, "agent_id": "nunmai", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, nunmai_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # nunmai_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "nunmai-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "nunmai"
...
def _setup_oss_interactive(nunmai_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], nunmai_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], nunmai_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'nunmai-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "nunmai-user")

    agent_id = input("  Agent ID [nunmai]: ").strip()
    agent_id = agent_id or "nunmai"
    ...
    if env_writes:
        _write_env(Path(nunmai_home) / ".env", env_writes)
    _save_mem0_json(nunmai_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(nunmai_home: str, config: dict) -> None:
    """Entry point called by nunmai memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(nunmai_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(nunmai_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(nunmai_home, config, flags)
        return
