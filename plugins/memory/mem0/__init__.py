...
self-hosted Mem0 server (MEM0_HOST, HTTP), or OSS Memory. Secrets live in $NUNMAI_HOME/.env
(MEM0_API_KEY, MEM0_HOST); settings in $NUNMAI_HOME/mem0.json via `nunmai memory setup`:
...
def _load_config() -> dict:
    """Env vars provide defaults; $NUNMAI_HOME/mem0.json overrides individual keys.
    ...
    config = {"mode": os.environ.get("MEM0_MODE", "platform"), "api_key": get_secret("MEM0_API_KEY", ""), "host": os.environ.get("MEM0_HOST", ""), "agent_id": os.environ.get("MEM0_AGENT_ID", "nunmai"), "oss": {}}
    ...
    file_cfg = _read_mem0_json(get_nunmai_home() / "mem0.json")
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._mode, self._api_key, self._host, self._user_id, self._agent_id = "platform", "", "", _DEFAULT_USER_ID, "nunmai"
    ...
    def save_config(self, values, nunmai_home):
        """Merge-write config to $NUNMAI_HOME/mem0.json."""
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._mode, self._api_key, self._host, self._agent_id = cfg.get("mode", "platform"), cfg.get("api_key", ""), cfg.get("host", ""), cfg.get("agent_id", "nunmai")
