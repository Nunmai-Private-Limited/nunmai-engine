...
```bash
nunmai memory setup    # select "mem0"
```
...
Behavioral settings live in `$NUNMAI_HOME/mem0.json` (set them via `nunmai memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.nunmai/.env`.
...
| `user_id` | `nunmai-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Nunmai session and call `mem0_search` — it connects to your server.
...
```bash
nunmai memory setup mem0 --mode oss \
...
```bash
nunmai memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$NUNMAI_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.nunmai/mem0_qdrant"}}
  }
}
```
...
```bash
nunmai memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$NUNMAI_HOME/mem0.json`).
