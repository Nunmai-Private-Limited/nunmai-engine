...
``$NUNMAI_HOME/plugins/<name>/``, project ``./.nunmai/plugins/<name>/`` (opt-in via
NUNMAI_ENABLE_PROJECT_PLUGINS), then ``nunmai_agent.memory_providers`` entry points.
...
# Per Nunmai home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(nunmai_home_key(), {})
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.nunmai/plugins/`` or None. Gated on NUNMAI_ENABLE_PROJECT_PLUGINS like the
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``nunmai <provider>`` commands."""
...
def load_memory_provider(name: str, *, register_skills: Optional[bool] = None) -> Optional["MemoryProvider"]:
    ...
    if provider_dir is not None and _explicitly_disabled(name, provider_dir):
        # The Plugins hub / `nunmai plugins disable` park a user-installed provider in
        # ``plugins.disabled``; the loader must honour it or "disabled" is a lie in the UI.
        logger.warning("Memory provider '%s' is disabled via plugins.disabled; run `nunmai plugins enable %s` "
...
def import_memory_provider_module(name: Optional[str] = None) -> bool:
    ...
    instead of a fresh native extension load. Exists so ``nunmai acp`` can pay the heavy
...
def import_provider_module(name: str, submodule: Optional[str] = None):
    ...
    catalog install under ``$NUNMAI_HOME/plugins/`` loads under the synthetic user namespace,
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _nunmai_user_memory.<name>.cli, usually before the
