...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with nunmai-engine)
2. User-installed providers: ``$NUNMAI_HOME/plugins/<name>/``
3. Project-local providers: ``./.nunmai/plugins/<name>/``, opt-in via
   ``NUNMAI_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``nunmai_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "nunmai_agent.memory_providers"
...
_USER_NAMESPACE = "_nunmai_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_nunmai_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_nunmai_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$NUNMAI_HOME/plugins/`` or None if unavailable."""
    try:
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.nunmai/plugins/`` or None if unavailable or not opted in.

    Gated on ``NUNMAI_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from nunmai_cli.plugins import _env_enabled

        if not _env_enabled("NUNMAI_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".nunmai" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($NUNMAI_HOME/plugins/<name>/)
    # 3. Project-local providers (./.nunmai/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``nunmai <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from nunmai_cli.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$NUNMAI_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``nunmai memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every nunmai startup.
        """
        if self._context is None:
            from nunmai_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _nunmai_user_memory.<name>.cli, usually
