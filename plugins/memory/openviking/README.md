...
| `OPENVIKING_AGENT` | `nunmai` | Nunmai peer ID in OpenViking, used for peer-scoped memories |
...
current user). Nunmai caches a confirmed user only for the active connection.
If the probe fails, Nunmai uses the configured user, or `default`, for that
