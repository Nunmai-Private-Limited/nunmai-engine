...
- OpenViking server running and reachable from Nunmai

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Nunmai can identify older servers that expose the legacy status-only health
...
connection values into Nunmai, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.nunmai/.env`; for a named profile use
...
OpenViking's server config is separate from Nunmai:
...
Nunmai-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Nunmai lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Nunmai sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Nunmai also sends `User-Agent: openviking-memory-nunmai/<version>` on
OpenViking requests. This standard harness identifier contains the Nunmai
...
does not ask for a peer ID. Without a configured peer, Nunmai sends neither
...
order remains environment, linked OpenViking config, then Nunmai YAML. To use
...
on the old implicit `nunmai` peer now use user memory for new writes. Without
...
Set `agent: nunmai` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `nunmai-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Nunmai
...
Use the same OpenViking profile and credentials as Nunmai for manual recovery.
...
Nunmai built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Nunmai action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Nunmai
...
file URIs, such as
`viking://user/default/peers/nunmai/memories/preferences/mem_abc123.md` (any
