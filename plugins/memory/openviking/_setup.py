"""Interactive ``nunmai memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (Nunmai
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that Nunmai then
...
def _save_nunmai_only_config(*, config: dict, provider_config: dict, env_path: Path, values: dict) -> None:
    ov = _ov()
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    _save_nunmai_only_config(config=config, provider_config=provider_config, env_path=env_path, values=values)
    _print_openviking_ready("Connection saved to Nunmai .env.")
    return True


def run_setup(nunmai_home: str, config: dict) -> None:
