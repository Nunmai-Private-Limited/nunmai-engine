...
def _nunmai_home_path() -> Path:
    try:
        ...
        return Path(env_home).expanduser() if env_home else Path.home() / ".nunmai"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    log_path = _nunmai_home_path() / _OPENVIKING_SERVER_LOG_RELATIVE_PATH
    try:
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._nunmai_home = str(kwargs.get("nunmai_home") or "").strip() or str(_nunmai_home_path())
