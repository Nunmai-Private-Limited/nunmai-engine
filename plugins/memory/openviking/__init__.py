...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Nunmai peer ID in OpenViking (default: nunmai)
...
_DEFAULT_AGENT = "nunmai"
_OPENVIKING_USER_AGENT = f"openviking-memory-nunmai/{_NUNMAI_VERSION}"
_AGENT_PROMPT_LABEL = "Nunmai peer ID in OpenViking"
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Nunmai to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Nunmai refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("NUNMAI_HOME", "")).expanduser() if os.environ.get("NUNMAI_HOME") else Path.home() / ".nunmai"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Nunmai did not start "
    ...
    try:
        ...
        # The Nunmai Desktop backend can include the Nunmai venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Nunmai venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Nunmai venv,
        # aborting `nunmai update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Nunmai will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Nunmai session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Nunmai only", "write values only to Nunmai .env"),
    ...
    _save_nunmai_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Nunmai .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See nunmai-engine#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Nunmai peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, nunmai_home: str, config: dict) -> None:
        ...
        from nunmai_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        nunmai_home_path = Path(nunmai_home)
        env_path = nunmai_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        nunmai_home = str(kwargs.get("nunmai_home") or "").strip()
        if not nunmai_home:
            try:
                from nunmai_constants import get_nunmai_home
                nunmai_home = str(get_nunmai_home())
            except Exception:
                nunmai_home = str(Path.home() / ".nunmai")
        self._nunmai_home = nunmai_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Nunmai will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.nunmai/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts nunmai (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Nunmai will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._nunmai_home:
            return None
        return Path(self._nunmai_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._nunmai_home:
            return None
        return Path(self._nunmai_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Nunmai' full canonical transcript."""
        if not messages:
            return []
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See nunmai-engine#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/nunmai/.overview.md).
