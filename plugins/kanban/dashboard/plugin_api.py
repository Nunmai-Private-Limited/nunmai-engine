...
Every handler is a thin wrapper around ``nunmai_cli.kanban_db`` (the same code paths the CLI
...
from nunmai_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from nunmai_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    ``NUNMAI_KANBAN_BOARD`` env var (concurrent requests for different boards would cross-write)."""
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``nunmai_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"nunmai_cli.{module}"), fn)(task_id, author=(author or None))
...
def get_board(
    ...
    (``NUNMAI_KANBAN_BOARD`` env → on-disk ``current`` pointer → ``default``)."""
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different NUNMAI_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from nunmai_cli.kanban import _check_dispatcher_presence
                from nunmai_constants import get_nunmai_home
                running, message = _check_dispatcher_presence(nunmai_home=get_nunmai_home())
...
class UpdateTaskBody(BaseModel):
    ...
    # Handoff fields forwarded to complete_task on -> 'done' (parity with ``nunmai kanban complete``).
...
def list_diagnostics(
    ...
    consumed by ``nunmai kanban diagnostics`` when the dashboard runs."""
    with _board_conn(board) as (board, conn):
...
def specify_task_endpoint(task_id: str, payload: SpecifyBody, board: Optional[str] = Query(None)):
    """Flesh out a triage task via the auxiliary LLM (``nunmai kanban specify``). Non-OK is NOT
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    (``nunmai kanban reassign <task_id> <profile> [--reclaim]``)."""
    with _board_conn(board) as (board, conn):
...
def _active_profile_name() -> str:
    """Current Nunmai profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    (same substrate as the Models page) so it can't offer a pair Nunmai rejects. Skips pricing
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            ...
            profile_dir = Path(get_nunmai_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    """``nunmai profile describe <name> --auto``: persist with ``description_auto: true``.
...
def decompose_task_endpoint(task_id: str, payload: DecomposeBody, board: Optional[str] = Query(None)):
    """Fan a triage task out into child tasks via the auxiliary LLM (``nunmai kanban decompose``).
