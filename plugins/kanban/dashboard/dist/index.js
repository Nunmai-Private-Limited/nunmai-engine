/**
 * Nunmai Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.nunmai/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__NUNMAI_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__NUNMAI_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "nunmai-kanban-dot-triage",
    todo: "nunmai-kanban-dot-todo",
    ready: "nunmai-kanban-dot-ready",
    running: "nunmai-kanban-dot-running",
    blocked: "nunmai-kanban-dot-blocked",
    review: "nunmai-kanban-dot-review",
    done: "nunmai-kanban-dot-done",
    archived: "nunmai-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-nunmai-task";
  ...
  const DOCS_URL = "https://nunmai.in/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://nunmai.in/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "nunmai.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="nunmai-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])nunmai-kanban-md-code\1/i.test(attrs)) {
      return ' class="nunmai-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "nunmai-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "nunmai-kanban-md",
  ...
  // standard `drop` event and our `nunmai-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      function startDrag() {
        ...
        proxy.classList.add("nunmai-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("nunmai-kanban-column--drop");
          if (target) target.classList.add("nunmai-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (!dragging) return;
        if (lastTarget) {
          lastTarget.classList.remove("nunmai-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("nunmai-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("nunmai-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__NUNMAI_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "nunmai-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "nunmai-kanban-attention",
        "nunmai-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "nunmai-kanban-attention-bar" },
        h("span", { className: "nunmai-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "nunmai-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "nunmai-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "nunmai-kanban-attention-row",
                  "nunmai-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "nunmai-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "nunmai-kanban-attention-row-id" }, task.id),
                h("span", { className: "nunmai-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "nunmai-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "nunmai-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "nunmai-kanban-diag-action-btn",
      action.suggested ? "nunmai-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " nunmai-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".nunmai-kanban-drawer-comment-row input, .nunmai-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "nunmai-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("nunmai-kanban-diag", sevClass) },
      h("div", { className: "nunmai-kanban-diag-header" },
        h("span", { className: "nunmai-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "nunmai-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "nunmai-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "nunmai-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "nunmai-kanban-diag-data-row" },
                  h("span", { className: "nunmai-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "nunmai-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "nunmai-kanban-diag-data-row" },
                h("span", { className: "nunmai-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "nunmai-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "nunmai-kanban-diag-reassign-row" },
            h("span", { className: "nunmai-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "nunmai-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "nunmai-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "nunmai-kanban-diag-msg",
              msg.ok ? "nunmai-kanban-diag-msg--ok" : "nunmai-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "nunmai-kanban-section" },
      h("div", { className: "nunmai-kanban-section-head-row" },
        h("span", { className: "nunmai-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "nunmai-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "nunmai-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "nunmai-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "nunmai-kanban-docs-link",
      title: "Open Nunmai Kanban docs in a new tab",
      "aria-label": "Nunmai Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `nunmai kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "nunmai-kanban-msg-ok" : "nunmai-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardProjectBadge(props) {
    ...
    return h(Badge, {
      variant: "outline",
      className: "nunmai-kanban-board-project text-xs font-normal gap-1",
      ...
      h("button", {
        type: "button",
        className: "nunmai-kanban-board-project-unbind",
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "nunmai-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "nunmai-kanban-boardswitcher" },
      h("div", { className: "nunmai-kanban-boardswitcher-inner" },
  ...
  // Live (non-archived) Nunmai projects available for board scoping,
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "nunmai-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "nunmai-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "nunmai-kanban-dialog-title" },
        ...
        h("div", { className: "nunmai-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Nunmai profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "nunmai-kanban-bulk" },
      h("span", { className: "nunmai-kanban-bulk-count" },
      ...
      h("div", { className: "nunmai-kanban-bulk-priority",
      ...
      h("div", { className: "nunmai-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Nunmai profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "nunmai-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("nunmai-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("nunmai-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "nunmai-kanban-trash",
        dragOver ? "nunmai-kanban-trash--drop" : "",
        props.draggingTaskId ? "nunmai-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "nunmai-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "nunmai-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".nunmai-kanban-card")) return true;
      if (target.closest && target.closest(".nunmai-kanban-column-add")) return true;
      if (target.closest && target.closest(".nunmai-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("nunmai-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("nunmai-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".nunmai-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "nunmai-kanban-columns",
        isScrollable ? "nunmai-kanban-columns--scrollable" : "",
        isPanning ? "nunmai-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("nunmai-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("nunmai-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "nunmai-kanban-column",
        dragOver ? "nunmai-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "nunmai-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "nunmai-kanban-col-check",
        ...
        h("span", { className: cn("nunmai-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "nunmai-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "nunmai-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "nunmai-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "nunmai-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "nunmai-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "nunmai-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "nunmai-kanban-lane" },
                  h("div", { className: "nunmai-kanban-lane-head" },
                    h("span", { className: "nunmai-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "nunmai-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "nunmai-kanban-card--stale-red";
    if (age >= tier.amber) return "nunmai-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".nunmai-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "nunmai-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "nunmai-kanban-card",
        props.selected ? "nunmai-kanban-card--selected" : "",
        props.failed ? "nunmai-kanban-card--failed" : "",
        props.draggingSource ? "nunmai-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "nunmai-kanban-card-content" },
          h("div", { className: "nunmai-kanban-card-row" },
            h("label", {
              className: "nunmai-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "nunmai-kanban-card-check",
            ...
            h("span", { className: "nunmai-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or nunmai kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "nunmai-kanban-warning-badge",
                    "nunmai-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "nunmai-kanban-needs-assignee",
          ...
          h("div", { className: "nunmai-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "nunmai-kanban-card-row nunmai-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "nunmai-kanban-assignee",
                            title: `Assigned to Nunmai profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "nunmai-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "nunmai-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "nunmai-kanban-ago",
  ...
  // centered modal reusing the nunmai-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "nunmai-kanban-dialog nunmai-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Nunmai profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Nunmai profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "nunmai-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "nunmai-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "nunmai-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "nunmai-kanban-drawer-close",
        ...
        data ? h("div", { className: "nunmai-kanban-drawer-comment-foot" },
          h("div", {
            className: "nunmai-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "nunmai-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "nunmai-kanban-section" },
      h("div", { className: "nunmai-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "nunmai-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "nunmai-kanban-drawer-body" },
      h("div", { className: "nunmai-kanban-drawer-title" },
        h("span", { className: cn("nunmai-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "nunmai-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "nunmai-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "nunmai-kanban-section" },
            h("div", { className: "nunmai-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "nunmai-kanban-section" },
            h("div", { className: "nunmai-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "nunmai-kanban-done-no-result nunmai-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "nunmai-kanban-section" },
            h("div", { className: "nunmai-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "nunmai-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "nunmai-kanban-section" },
        h("div", { className: "nunmai-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "nunmai-kanban-comment" },
            h("div", { className: "nunmai-kanban-comment-head" },
              h("span", { className: "nunmai-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "nunmai-kanban-diag-action-btn",
      ...
      h("div", { className: "nunmai-kanban-section" },
        h("div", { className: "nunmai-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "nunmai-kanban-comment" },
                h("div", { className: "nunmai-kanban-comment-head" },
                  h("span", { className: "nunmai-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "nunmai-kanban-comment-ago" },
      ...
      h("div", { className: "nunmai-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "nunmai-kanban-event",
              isDiag ? "nunmai-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "nunmai-kanban-event-header" },
                  h("span", { className: "nunmai-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "nunmai-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "nunmai-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "nunmai-kanban-event-header-plain" },
                  h("span", { className: "nunmai-kanban-event-kind" }, e.kind),
                  h("span", { className: "nunmai-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "nunmai-kanban-event-phantom-row" },
                  h("span", { className: "nunmai-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "nunmai-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "nunmai-kanban-event-payload" },
    ...
    return h("div", { className: "nunmai-kanban-section" },
      h("div", { className: "nunmai-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "nunmai-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `nunmai-kanban-run--${r.outcome || r.status || "ended"}`
          : "nunmai-kanban-run--active";
        return h("div", { key: r.id, className: cn("nunmai-kanban-run", outcomeClass) },
          h("div", { className: "nunmai-kanban-run-head" },
            h("span", { className: "nunmai-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "nunmai-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "nunmai-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "nunmai-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "nunmai-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "nunmai-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "nunmai-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "nunmai-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "nunmai-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "nunmai-kanban-pre nunmai-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "nunmai-kanban-meta-row" },
      h("span", { className: "nunmai-kanban-meta-label" }, props.label),
      h("span", { className: "nunmai-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "nunmai-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nunmai-kanban-meta-row" },
        h("span", { className: "nunmai-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "nunmai-kanban-meta-value nunmai-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nunmai-kanban-meta-row" },
        h("span", { className: "nunmai-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "nunmai-kanban-meta-value nunmai-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nunmai-kanban-meta-row" },
        h("span", { className: "nunmai-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "nunmai-kanban-meta-value nunmai-kanban-editable",
    ...
    return h("div", { className: "nunmai-kanban-meta-row" },
      h("span", { className: "nunmai-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "nunmai-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "nunmai-kanban-section" },
      h("div", { className: "nunmai-kanban-section-head-row" },
        h("span", { className: "nunmai-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "nunmai-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "nunmai-kanban-section" },
      h("div", { className: "nunmai-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "nunmai-kanban-deps-row" },
        h("span", { className: "nunmai-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "nunmai-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "nunmai-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "nunmai-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "nunmai-kanban-dep-chip-x",
      ...
      h("div", { className: "nunmai-kanban-deps-row" },
        h("span", { className: "nunmai-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "nunmai-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "nunmai-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "nunmai-kanban-msg-ok"
          : "nunmai-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "nunmai-kanban-section" },
      ...
      h("div", { className: "nunmai-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "nunmai-kanban-home-sub nunmai-kanban-home-sub--on"
              : "nunmai-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__NUNMAI_PLUGINS__ && typeof window.__NUNMAI_PLUGINS__.register === "function") {
    window.__NUNMAI_PLUGINS__.register("kanban", KanbanPage);
  }
})();
