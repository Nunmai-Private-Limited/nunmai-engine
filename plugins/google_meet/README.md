...
Let the nunmai agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where nunmai runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `nunmai meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `nunmai meet` CLI |
...
| `cli.py` | `nunmai meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NUNMAI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `nunmai meet node {run,list,approve,remove,status,ping}` |
...
nunmai meet install                                      # pip + Chromium
nunmai meet setup                                        # preflight
nunmai meet auth                                         # optional
nunmai meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
nunmai meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.nunmai/.env
nunmai meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
nunmai meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
nunmai meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.nunmai/.env
nunmai meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, nunmai will **not** switch your system audio input automatically — the
...
admission the bot unmutes itself if Meet seated it muted; `status.json` / `nunmai meet
...
nunmai plugins enable google_meet
nunmai meet node run --display-name my-mac --host 0.0.0.0 --port 18789
