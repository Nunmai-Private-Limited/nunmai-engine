"""`nunmai meet node ...` subcommand tree, wired under the ``nunmai meet`` parser."""
...
def _cmd_run(args: argparse.Namespace, reg: NodeRegistry) -> int:
    ...
    print(f"[meet-node] display_name={server.display_name}\n"
          ...
          f"             nunmai meet node approve <name> ws://<host>:{args.port} {token}")
    try:
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``nunmai meet node ...``; returns a process exit code."""
