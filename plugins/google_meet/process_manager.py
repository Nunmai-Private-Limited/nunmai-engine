...
One active meeting at a time, recorded in ``$NUNMAI_HOME/workspace/meetings/.active.json``
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Nunmai Engine", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "NUNMAI_MEET_URL": url, "NUNMAI_MEET_OUT_DIR": str(out),
           "NUNMAI_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "NUNMAI_MEET_HEADED"),
        (auth_state, "NUNMAI_MEET_AUTH_STATE"),
        (duration, "NUNMAI_MEET_DURATION"),
        (mode, "NUNMAI_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "NUNMAI_MEET_REALTIME_MODEL"),
        (realtime_voice, "NUNMAI_MEET_REALTIME_VOICE"),
        (realtime_instructions, "NUNMAI_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("NUNMAI_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
