...
Standalone subprocess spawned by ``process_manager.py``; configured via ``NUNMAI_MEET_*`` env,
status + transcript written under ``$NUNMAI_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``NUNMAI_MEET_URL=... NUNMAI_MEET_OUT_DIR=/tmp/x NUNMAI_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__nunmaiMeetDrain()`` pulls new entries.
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__nunmaiMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'nunmai_meet_sink'}", str(pcm_path)]
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``NUNMAI_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("NUNMAI_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("NUNMAI_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("NUNMAI_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("NUNMAI_MEET_AUTH_STATE", "").strip(),
        guest_name=env("NUNMAI_MEET_GUEST_NAME", "Nunmai Engine"),
        duration_s=_parse_duration(env("NUNMAI_MEET_DURATION", "")),
        realtime=env("NUNMAI_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # NUNMAI_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("NUNMAI_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("NUNMAI_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("NUNMAI_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("NUNMAI_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("NUNMAI_MEET_LOBBY_TIMEOUT", "300")))
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — NUNMAI_MEET_URL must be a "
