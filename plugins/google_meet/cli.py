...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``nunmai meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="Nunmai Engine")
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"nunmai meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: nunmai meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: nunmai meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "nunmai meet join https://meet.google.com/abc-defg-hij"
        )
    else:
...
def _cmd_stop() -> int:
    res = pm.stop(reason="nunmai meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
