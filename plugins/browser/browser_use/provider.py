...
``BROWSER_USE_API_KEY`` (https://browser-use.com) or the managed Nunmai tool gateway (bills to a
Nunmai subscription). Direct first, managed second, unless ``tool_gateway.browser: gateway`` flips
...
class BrowserUseBrowserProvider(CloudBrowserProvider):
    ...
    # registered for the Nunmai gateway path and legacy cloud_provider configs.
    setup_tag = None
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Lazy: managed_tool_gateway pulls in the Nunmai auth stack direct-key users never need.
