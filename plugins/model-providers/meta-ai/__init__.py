"""Meta Model API (Muse Spark) provider plugin for Nunmai Engine.
...
Bundled from https://github.com/albertodepaola/hermes-meta-provider. Nunmai'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to nunmai' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Nunmai' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in nunmai_cli/providers.py and the retention hint in
