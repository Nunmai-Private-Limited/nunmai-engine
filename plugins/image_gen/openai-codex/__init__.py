...
_NO_AUTH = (
    ...
    "`nunmai auth add openai-codex` (or `nunmai setup` → Codex) to sign in.")
...
class OpenAICodexImageGenProvider(StaticImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            # Codex OAuth bootstrap hook (nunmai_cli/tools_config_post_setup.py) starts the sign-in (#102144).
            "post_setup": "openai_codex",
            "post_setup_hint": (
                "Sign in with `nunmai auth add openai-codex` (or `nunmai setup` → Codex) "
