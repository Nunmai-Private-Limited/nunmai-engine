...
# Nunmai' 3 abstract ratios → Krea's enum (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16).
...
_USER_AGENT = "Nunmai-Agent/1.0 (krea-image-gen)"
        ...
        if failure.kind == "http":
            ...
            if managed and 400 <= status < 500:
                hint = (
                    ...
                    f"Model '{model_id}' may not be enabled/priced on the Nunmai Portal's Krea gateway. "
...
class KreaImageGenProvider(StaticImageGenProvider):
    ...
    def is_available(self) -> bool:
        # Direct key OR managed Nunmai gateway (portal users without a Krea key).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Nunmai access token); otherwise direct Krea with a BYO ``KREA_API_KEY``.
        ...
        if managed is not None:
            for what, arg in _MANAGED_UNSUPPORTED:
                if arg in payload:
                    return fail(
                        f"Managed Krea (Nunmai Subscription) does not support {what}. "
        ...
        # 2. Poll — same principal as submit, so the managed path polls the gateway with the Nunmai token.
