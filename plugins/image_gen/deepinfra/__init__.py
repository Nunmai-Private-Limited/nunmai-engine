...
:func:`nunmai_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``nunmai tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from nunmai the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `nunmai tools` → Image "
                    "Generation → DeepInfra to configure, or `nunmai setup` "
                    "to add the key."
                ),
        ...
        from nunmai_cli.models import deepinfra_base_url
