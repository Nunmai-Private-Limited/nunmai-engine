...
def _build_client(openai: Any, base_url: str, api_key: str) -> Any:
    """``openai.OpenAI`` on Nunmai' env-only-proxy httpx client, so a local/custom endpoint never
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_factory("openai", aspect)(
                ...
                "Run `nunmai tools` → Image Generation → OpenAI to configure, or `nunmai setup` "
                "to add the key.",
                "auth_required")
        ...
        # gpt-image bills per text/image token; the tier id is a Nunmai label, the API model prices.
