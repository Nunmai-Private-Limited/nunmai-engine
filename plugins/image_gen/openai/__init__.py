...
three virtual model IDs so the ``nunmai tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `nunmai tools` → Image "
                    "Generation → OpenAI to configure, or `nunmai setup` "
                    "to add the key."
                ),
