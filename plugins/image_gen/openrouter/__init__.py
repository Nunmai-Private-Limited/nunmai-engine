...
via :func:`nunmai_cli.runtime_provider.resolve_runtime_provider`. OpenRouter alone also
...
_ATTRIBUTION_HEADERS = {
    ...
    "HTTP-Referer": "https://github.com/Nunmai-Private-Limited/nunmai-engine",
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """One instance per backend (OpenRouter, Nunmai Portal); they differ only in the runtime
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        releases selectable); Nunmai Portal = chat-completions only. Offline: default chain + snapshot."""
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return fail(
                ...
                f"Configure {self._display} in `nunmai tools` → Image Generation.",
                "missing_api_key")
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        # No picker row: Portal models are offered inside the single managed "Nunmai Subscription" row
        ...
        OpenRouterCompatImageProvider(
            provider_name="nous", display_name="Nunmai Portal", runtime_name="nous", config_key="nous",
