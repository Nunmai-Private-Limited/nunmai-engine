...
This plugin ships bundled with Nunmai but is **opt-in** — it only loads when
...
nunmai tools  # → Langfuse Observability
...
Set these in `~/.nunmai/.env` (or via `nunmai tools`):
...
nunmai chat -q "hello"              # then check Langfuse for a "Nunmai turn" trace
```

Generation observations include the Nunmai system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `NUNMAI_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
NUNMAI_LANGFUSE_ENV=production       # environment tag
NUNMAI_LANGFUSE_RELEASE=v1.0.0       # release tag
NUNMAI_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
NUNMAI_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
NUNMAI_LANGFUSE_MAX_DEPTH=4          # max payload depth (default: 4)
NUNMAI_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
NUNMAI_LANGFUSE_DEBUG=true           # verbose plugin logging
```

`NUNMAI_LANGFUSE_MAX_DEPTH` controls nested payload capture in both `sanitized`
...
set it to a higher non-negative integer (for example, `10`) in the Nunmai
...
`NUNMAI_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
nunmai plugins disable observability/langfuse
```
