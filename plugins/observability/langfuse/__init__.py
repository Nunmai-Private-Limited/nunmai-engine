"""langfuse — Nunmai plugin tracing conversations, LLM calls and tool usage to Langfuse.

Activated via ``plugins.enabled``; hooks are inert without the ``langfuse`` SDK
and credentials. Env: NUNMAI_LANGFUSE_PUBLIC_KEY / SECRET_KEY (required),
...
# Under a multiplexed profile override, one settled client (or _INIT_FAILED) per Nunmai home: the
...
def _debug(message: str) -> None:
    if _env("NUNMAI_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _settled_client() -> Any:
    ...
    return _LANGFUSE_CLIENT_BY_HOME.get(nunmai_home_key())
...
def _settle_client() -> Any:
    ...
    if get_nunmai_home_override() is None:
        ...
        _LANGFUSE_CLIENT_BY_HOME[nunmai_home_key()] = settled
...
def _build_client() -> Optional[Langfuse]:
    ...
    public_key, secret_key = (_secret(f"NUNMAI_LANGFUSE_{n}") or _secret(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _secret(f"NUNMAI_LANGFUSE_{name}") or _secret(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _secret("NUNMAI_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
...
def _resolve_max_depth(configured_depth: str) -> int:
    """Parse ``NUNMAI_LANGFUSE_MAX_DEPTH``; an invalid value warns ONCE per distinct value.
    ...
    try:
        ...
        logger.warning("Invalid NUNMAI_LANGFUSE_MAX_DEPTH=%r; use a non-negative integer. Falling back to 4.", configured_depth)
        return 4
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    if max_depth is None:
        max_depth = _resolve_max_depth(_env("NUNMAI_LANGFUSE_MAX_DEPTH", "4") or "4")
    if depth > max_depth:
        return "<max-depth>"
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    # need an explicit total (Nunmai estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "nunmai", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Nunmai turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Nunmai turn",
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Nunmai versions:
