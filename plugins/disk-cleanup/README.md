...
Auto-tracks and cleans up ephemeral files created during Nunmai Engine
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NUNMAI_HOME` and `/tmp/nunmai-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NUNMAI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NUNMAI_HOME` or `/tmp/nunmai-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NUNMAI_HOME/disk-cleanup/` is itself excluded
- `$NUNMAI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
