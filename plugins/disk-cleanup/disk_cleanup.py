"""disk_cleanup — ephemeral file cleanup for Nunmai Engine.
...
Rules:
  ...
  - empty dirs    → always delete (under NUNMAI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NUNMAI_HOME and /tmp/nunmai-*
Never touches: ~/.nunmai/logs/ or any system directory.
"""
...
try:
    ...
    def get_nunmai_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NUNMAI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nunmai").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NUNMAI_HOME/logs/``."""
    return get_nunmai_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NUNMAI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NUNMAI_HOME or ``/tmp/nunmai-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nunmai_home = get_nunmai_home()
    try:
        path.resolve().relative_to(nunmai_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nunmai-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nunmai-"):
        return True
    return False
...
# Paths under $NUNMAI_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so NUNMAI_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        nunmai_home = get_nunmai_home()
        for parent in ("cron", "cronjobs"):
            base = nunmai_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NUNMAI_HOME, but never recurse into known
    # durable state trees.  Some installs place the Nunmai checkout, venv,
    # and desktop build under NUNMAI_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    nunmai_home = get_nunmai_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in nunmai_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nunmai_home = get_nunmai_home()
    try:
        rel = path.resolve().relative_to(nunmai_home)
        ...
        # Path isn't under NUNMAI_HOME (e.g. /tmp/nunmai-*) — fall through.
        pass
