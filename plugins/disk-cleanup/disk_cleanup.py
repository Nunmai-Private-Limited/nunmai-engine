...
@functools.lru_cache(maxsize=1)  # built lazily so NUNMAI_HOME resolves once
def _protected_cron_paths() -> frozenset:
    ...
    return frozenset(str(x) for parent in ("cron", "cronjobs") for base in (get_nunmai_home() / parent,)
