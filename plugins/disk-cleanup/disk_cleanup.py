...
after 14 days; empty dirs under NUNMAI_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly NUNMAI_HOME and /tmp/nunmai-*; never ~/.nunmai/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$NUNMAI_HOME/disk-cleanup/<name>`` — deliberately outside ``$NUNMAI_HOME/logs/``."""
    return get_nunmai_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under NUNMAI_HOME or ``/tmp/nunmai-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_nunmai_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nunmai-")
...
# Top-level NUNMAI_HOME dirs whose empty subdirs are never swept (last row: user project trees).
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    "logs", "memories", "sessions", "cron", "cronjobs",
    "cache", "skills", "plugins", "disk-cleanup", "optional-skills",
    "nunmai-engine", "backups", "profiles", ".worktrees",
...
# Top-level NUNMAI_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
def _is_protected_dir(p: Path) -> bool:
    """A tracked DIRECTORY that is NUNMAI_HOME itself or lives under a protected top-level tree
    (``cache/terminal`` holds terminal snapshots) is never rmtree'd; only its files age out."""
    if not p.is_dir():
        return False
    with contextlib.suppress(ValueError, OSError):
        rel = p.resolve().relative_to(get_nunmai_home())
...
# Paths under $NUNMAI_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def _is_protected_cron_path(p: Path) -> bool:
    return str(p.resolve()) in _protected_cron_paths(get_nunmai_home())
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NUNMAI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_nunmai_home())
...
def _sweep_empty_dirs(nunmai_home: Path) -> int:
    """Remove empty dirs under NUNMAI_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under NUNMAI_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(nunmai_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def _inside_git_worktree(path: Path) -> bool:
    ...
    Only ``.git`` entries strictly BELOW ``NUNMAI_HOME`` count for in-home paths: a home kept
    in a dotfiles repo (``~/.git``) would otherwise make every scratch file look Git-owned."""
    parents = list(path.resolve().parents)
    with contextlib.suppress(ValueError):
        parents = parents[: parents.index(get_nunmai_home())]
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under NUNMAI_HOME (/tmp/nunmai-*) — name rules only
        rel = path.resolve().relative_to(get_nunmai_home())
