...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/nunmai-x/test_foo.py`
...
All operations are scoped to NUNMAI_HOME and /tmp/nunmai-*.  # no-tmp: ok — legacy scratch scope this plugin cleans up
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside NUNMAI_HOME): {path_arg}"
