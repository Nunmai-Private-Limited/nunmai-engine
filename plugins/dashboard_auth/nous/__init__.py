"""NousDashboardAuthProvider — Nunmai Portal OAuth (authorization-code + PKCE).
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      NUNMAI_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      NUNMAI_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``nunmai-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``nunmai_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set NUNMAI_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via NUNMAI_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    """Nunmai Portal OAuth via authorization-code + PKCE (S256)."""

    name = "nous"
    display_name = "Nunmai Research"
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"nunmai_session_pkce\"]``
        ...
        cookie_payload = {
            "nunmai_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Nunmai Portal
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between NUNMAI_DASHBOARD_PORTAL_URL / NUNMAI_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Nunmai Portal token missing oauth_contract_version claim "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("NUNMAI_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``NUNMAI_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("NUNMAI_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set NUNMAI_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``nunmai_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "NUNMAI_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Nunmai Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Nunmai Engine "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"NUNMAI_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Nunmai Portal "
