...
when a client_id (``dashboard.oauth.client_id`` / ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
from nunmai_cli.dashboard_auth import LoginStart, ProviderError, Session
...
class NousDashboardAuthProvider(JwtOAuthProvider):
    ...
    def _claims_for(self, access_token: str) -> Dict[str, Any]:
        ...
        if contract_version is None:
            logger.warning(
                "Nunmai Portal token missing oauth_contract_version claim (contract says it should be %d); proceeding anyway.",
...
def _settings() -> dict:
    ...
    client_id = resolve_env_or_cfg("NUNMAI_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("NUNMAI_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "NUNMAI_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The Nunmai Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Nunmai Engine instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"NUNMAI_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
            f"shape 'agent:{{instance_id}}'. The Nunmai Portal provisions this value at deploy "
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('nunmai_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('nunmai_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('nunmai_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('nunmai_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
