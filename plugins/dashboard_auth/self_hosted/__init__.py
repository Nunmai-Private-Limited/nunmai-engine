...
A standards-compliant OpenID Connect Relying Party for the ``nunmai dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes Nunmai Portal's
...
:class:`~nunmai_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``nunmai_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~nunmai_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because Nunmai Portal mints a custom JWT
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.nunmai/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    NUNMAI_DASHBOARD_OIDC_ISSUER
    NUNMAI_DASHBOARD_OIDC_CLIENT_ID
    NUNMAI_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    NUNMAI_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Nunmai does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``NUNMAI_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "NUNMAI_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "NUNMAI_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("NUNMAI_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.nunmai/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "NUNMAI_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(NUNMAI_DASHBOARD_OIDC_ISSUER + NUNMAI_DASHBOARD_OIDC_CLIENT_ID) "
