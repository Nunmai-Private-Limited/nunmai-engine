...
``dashboard.oauth.self_hosted.{issuer,client_id,scopes,client_secret}`` or ``NUNMAI_DASHBOARD_OIDC_*``.
"""
...
def _settings() -> dict:
    ...
    issuer = setting("NUNMAI_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("NUNMAI_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            "Self-hosted OIDC dashboard auth is not configured. Set both an issuer and "
            "a client_id — either as env vars (NUNMAI_DASHBOARD_OIDC_ISSUER + "
            "NUNMAI_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("NUNMAI_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.nunmai/.env. Empty ⇒ public client.
        "client_secret": setting("NUNMAI_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
