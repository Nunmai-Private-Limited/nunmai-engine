...
into the same ``DashboardAuthProvider`` framework as the Nunmai OAuth
...
mirroring the Nunmai provider's precedence convention:
  ...
  Environment overrides::
      ...
      NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      NUNMAI_DASHBOARD_BASIC_AUTH_SECRET
      NUNMAI_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
Skip reasons:
  Like the Nunmai provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "NUNMAI_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or NUNMAI_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "NUNMAI_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "NUNMAI_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
