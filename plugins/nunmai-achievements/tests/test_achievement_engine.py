...
class AchievementEngineTests(unittest.TestCase):
    ...
    def test_finished_rescan_keeps_persisted_unlock_when_live_metric_shrinks(self):
        ...
        with TemporaryDirectory() as data_dir, patch.object(plugin_api, "ACHIEVEMENTS", [definition]), patch.object(plugin_api, "_data_dir", return_value=Path(data_dir)), patch.object(plugin_api, "get_nunmai_home", return_value=Path(data_dir)):
...
class CompactionScanTests(unittest.TestCase):
    def test_scan_stats_survive_compaction_and_v1_checkpoint_is_rescanned(self):
        ...
        with TemporaryDirectory() as tmp, patch.object(plugin_api, "_data_dir", return_value=Path(tmp) / "data"), patch.object(plugin_api, "get_nunmai_home", return_value=Path(tmp)):
            ...
            with patch.object(nunmai_state, "SessionDB", lambda read_only=True: SessionDB(Path(tmp) / "state.db", read_only=read_only)):
