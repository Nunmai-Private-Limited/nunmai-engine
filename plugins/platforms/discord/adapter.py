...
def _needs_server_members_intent(
    ...
    """Return True when Nunmai must request Discord's Server Members intent.
...
def _discord_ready_timeout_seconds() -> float:
    ...
    if raw:
        try:
            ...
            logger.warning(
                "Ignoring invalid NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.nunmai/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Nunmai gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Nunmai requested (#79430): Message
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``NUNMAI_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Nunmai response. Emoji-only acks are
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "nunmai_voice",
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Nunmai session")
        ...
        @tree.command(name="status", description="Show Nunmai session status")
        ...
        @tree.command(name="stop", description="Stop the running Nunmai agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.nunmai/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Nunmai Engine to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Nunmai gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Nunmai session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Nunmai in the thread",
        ...
        # nunmai_cli/commands.py automatically appear as Discord slash
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Nunmai skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from nunmai_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Nunmai session in the thread
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Nunmai has an LLM-generated session title and can safely rename
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Nunmai needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``nunmai update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``nunmai pairing approve``).
    ...
    # so users approved via ``nunmai pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``nunmai update`` prompts.
...
# is not in this process (e.g. ``nunmai cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "NUNMAI_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "NUNMAI_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``nunmai_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # nunmai_cli/setup.py::_setup_discord function.  Same shape as Teams.
