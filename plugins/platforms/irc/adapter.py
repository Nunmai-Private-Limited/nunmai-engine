"""IRC Platform Adapter for Nunmai Engine — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (nunmai-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "nunmai-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        elif command == "433":  # ERR_NICKNAMEINUSE — retry: nunmai_, nunmai_1, nunmai_2...
...
def interactive_setup() -> None:
    """`nunmai gateway setup` flow (lazy nunmai_cli imports keep the plugin importable outside the CLI)."""
    ...
    if declines_reconfigure("IRC", "Reconfigure IRC?", "IRC_SERVER"):
        return
    info("Connect Nunmai to an IRC network. Uses Python stdlib — no extra packages needed.",
    ...
    for label, env, what in (("Bot nickname (e.g. nunmai-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #nunmai — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :Nunmai Engine (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "nunmai-bot").rstrip("_0123456789-")[:24] or "nunmai-bot"
