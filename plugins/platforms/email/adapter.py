"""Email platform adapter for the Nunmai gateway: users talk to Nunmai by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _nunmai_version = "0"
        imap.xatom("ID", f'("name" "nunmai-engine" "version" "{_nunmai_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `nunmai gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Nunmai Engine"), ("Date", formatdate(localtime=True))):
            msg[key] = value
