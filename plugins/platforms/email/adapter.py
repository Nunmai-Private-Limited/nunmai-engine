"""
Email platform adapter for the Nunmai gateway.

Allows users to interact with Nunmai by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "nunmai-engine" "version" "{_nunmai_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `nunmai config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `nunmai gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
...
# nunmai_cli/gateway.py, and the _send_email dispatch in
