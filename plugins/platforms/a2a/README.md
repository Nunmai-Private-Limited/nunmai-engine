# A2A — Agent-to-Agent protocol for Nunmai
...
peer (another Nunmai, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
nunmai gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Nunmai serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.nunmai/a2a_audit.jsonl`.
- Conversations persist to `~/.nunmai/a2a_conversations/` — they survive context
