...
In single-bot mode (what Nunmai currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("NUNMAI_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "NUNMAI_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "NUNMAI_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("NUNMAI_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("NUNMAI_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "NUNMAI_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="nunmai-feishu-sdk",
                )
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``nunmai_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"nunmai_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "nunmai_update_prompt_action": answer,
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        nunmai_action = action_value.get("nunmai_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("nunmai_update_prompt_action")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Nunmai' SessionSource fields.
...
# Called by `nunmai gateway setup` via _setup_feishu() in nunmai_cli/gateway.py.
# =============================================================================
...
# dict in nunmai_cli/gateway.py, and the _send_feishu dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in nunmai_cli/gateway.py and the static
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nunmai setup` to install Feishu support.",
