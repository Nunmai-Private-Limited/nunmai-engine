    ...
    async def send_exec_approval(
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return _card_button(label, btn_type, {"nunmai_action": action_name, "approval_id": approval_id})
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `nunmai setup` to install Feishu support."}
    try:
...
def interactive_setup() -> None:
    ...
    from nunmai_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success, print_warning
