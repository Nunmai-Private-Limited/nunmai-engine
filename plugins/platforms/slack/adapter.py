...
# identify NunmaiAgent traffic — matching other Nunmai outbound surfaces
# that already set ``NunmaiAgent/<version>`` for platform-partner attribution.
try:
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``nunmai config set`` and Railway persist YAML
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|nunmai>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `nunmai slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `nunmai gateway setup` "
                "or in the active profile's ~/.nunmai/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `nunmai gateway setup` "
                "or add it to your active profile's ~/.nunmai/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `nunmai gateway setup` "
                "or in the active profile's ~/.nunmai/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `nunmai gateway setup` "
                "or add it to your active profile's ~/.nunmai/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_NUNMAI_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # Nunmai has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Nunmai can "
            ...
            # /model work directly without /nunmai prefix). A single regex
            ...
            # manifest (see `nunmai slack manifest`). In Socket Mode, Slack
            ...
            from nunmai_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/nunmai$")
            ...
            for _action_id in (
                "nunmai_approve_once",
                "nunmai_approve_session",
                "nunmai_approve_always",
                "nunmai_deny",
            ):
            ...
            for _action_id in (
                "nunmai_confirm_once",
                "nunmai_confirm_always",
                "nunmai_confirm_cancel",
            ):
            ...
            self._app.action(
                _re.compile(r"^nunmai_clarify_choice_\d+$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'nunmai slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Nunmai handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Nunmai session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Nunmai owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_nunmai_no_thread_response"):
        ...
        # the shape produced by peer Nunmai agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "nunmai_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "nunmai_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nunmai_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "nunmai_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "nunmai_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``nunmai_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``nunmai_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"nunmai_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "nunmai_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nunmai_confirm_once": "once",
            "nunmai_confirm_always": "always",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nunmai_approve_once": "once",
            "nunmai_approve_session": "session",
            "nunmai_approve_always": "always",
            "nunmai_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/nunmai <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/nunmai
        ...
        if slash_name in {"nunmai", ""}:
            # Legacy /nunmai <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from nunmai_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/nunmai   ``).
        ...
        # "Running /cmd…" ack shown by handle_nunmai_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/nunmai <question>" must produce public replies so
...
# ``nunmai_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_nunmai_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``nunmai_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under NUNMAI_HOME, and print
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Nunmai",
                bot_description="Your Nunmai agent on Slack",
            )
            target = Path(get_nunmai_home()) / "slack-manifest.json"
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nunmai update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``nunmai_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # Interactive setup wizard — replaces nunmai_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in nunmai_cli/gateway.py.
