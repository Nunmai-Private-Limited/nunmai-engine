...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(os.getenv("NUNMAI_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nunmai setup gateway`` → SimpleX; writes ``~/.nunmai/.env``."""
    print(
        "\nSimpleX Chat setup\n------------------\nRequirements:\n"
    ...
    try:
        ...
        print("nunmai_cli.config not available; set SIMPLEX_* vars manually in ~/.nunmai/.env")
        return
