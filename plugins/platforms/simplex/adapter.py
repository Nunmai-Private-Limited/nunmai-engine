"""SimpleX Chat platform adapter (Nunmai plugin).
...
This adapter ships as a Nunmai platform plugin under
``plugins/platforms/simplex/``. The Nunmai plugin loader scans the
...
Optional environment variables:
    ...
    NUNMAI_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``nunmai setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Nunmai — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NUNMAI_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nunmai setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.nunmai/.env`` via
    ``nunmai_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "nunmai_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.nunmai/.env"
        )
        return
