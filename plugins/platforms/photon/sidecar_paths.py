...
under an immutable ``/opt/nunmai`` (read-only for the nunmai user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$NUNMAI_HOME/photon/sidecar`` (the durable data volume,
