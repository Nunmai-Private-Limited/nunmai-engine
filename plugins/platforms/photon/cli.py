"""
``nunmai photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Nunmai
...
# Writable sidecar runtime dir (mirrors to NUNMAI_HOME on immutable
...
# wiring imports this module for `nunmai --help`).
...
# when called later (gateway start, nunmai status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `nunmai photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Nunmai Engine')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``nunmai photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Nunmai Engine" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.nunmai/.env,
    ...
    try:
        from nunmai_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  nunmai gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `nunmai photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`nunmai photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.nunmai/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `nunmai photon telemetry on` / `nunmai photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.nunmai/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  nunmai gateway restart")
    return 0
...
#
# `nunmai gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``nunmai photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `nunmai gateway setup` wizard."""
