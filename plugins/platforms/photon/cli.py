"""``nunmai photon ...`` CLI subcommands (registered via ``ctx.register_cli_command()``):
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p_setup.add_argument("--project-name", default=None, help="Project name (default: 'Nunmai Engine')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    print("\n✓ Photon setup complete.\n  Start the gateway:  nunmai gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_deps_installed() else '✗ run `nunmai photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`nunmai photon telemetry on|off`)")
    return 0


def _telemetry_enabled() -> bool:
    """PHOTON_TELEMETRY from env / ~/.nunmai/.env; truthy set mirrors the sidecar's."""
    try:
...
def gateway_setup() -> None:
    """Run Photon first-time setup from the unified `nunmai gateway setup` wizard (same flow
    as ``nunmai photon setup``; phone is prompted when stdin is a TTY)."""
