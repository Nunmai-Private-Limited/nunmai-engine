...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.nunmai/.env``; management
...
# published CLI client until Nunmai gets its own client_id.
...
# -- auth.json helpers (shares the file with the rest of nunmai-engine) ------------

def _auth_json_path() -> Path:
    """``~/.nunmai/auth.json`` honouring the active Nunmai profile."""
    try:
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nunmai/.env`` (secret never bound to a printable local
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `nunmai photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `nunmai photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `nunmai photon setup`)")]
    emit("\n".join(rows))
