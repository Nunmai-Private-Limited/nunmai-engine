...
Small Node helper that bridges Nunmai Engine to Photon's Spectrum SDK
(`spectrum-ts`).  Nunmai is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Nunmai-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Nunmai can receive inbound messages
...
The Nunmai plugin's `nunmai photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Nunmai is Python, so replies go through this sidecar
