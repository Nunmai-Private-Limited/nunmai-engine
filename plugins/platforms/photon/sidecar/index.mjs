// Nunmai Engine — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Nunmai): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Nunmai -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Nunmai-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `nunmai photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Nunmai can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "nunmai-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Nunmai'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Nunmai allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Nunmai, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-nunmai-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Nunmai supplied them so a known-good
