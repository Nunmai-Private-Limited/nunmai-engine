...
// `message.content.text` before Nunmai can see it. We rewrite the two inbound
...
const MARKER = "Nunmai patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Nunmai to patch; keep the legacy v8 path below for older pinned installs.
    if (
