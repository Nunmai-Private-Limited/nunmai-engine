...
``<nunmai-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``nunmai cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Nunmai owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = _APPROVAL_CHOICES.get(nunmai_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        ...
        for label, choice, style in prompt.actions:
            ...
            actions.append(ExecuteAction(
                title=label, verb="nunmai_approve",
                data={**btn_data_base, "nunmai_action": self._EA_CARD_ACTIONS[choice]}, **kw))
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Nunmai" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from nunmai_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Nunmai venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
