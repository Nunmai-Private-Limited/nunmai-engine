        ...
        *,
        ...
        if is_final:
            try:
                ...
                # resend.  Nunmai' prior behaviour — raising RuntimeError so
...
def qr_scan_for_bot_info(
    *,
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=nunmai"

    # ── Step 1: Fetch QR code ──
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "NunmaiAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "NunmaiAgent/1.0"})
...
# wizard + _PLATFORMS["wecom"] static dict in nunmai_cli/gateway.py, and the
...
def interactive_setup() -> None:
    ...
    Replaces nunmai_cli/gateway.py::_setup_wecom and the static
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nunmai setup` to install WeCom support.",
