...
and injects content-free wake hints into Nunmai' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Nunmai only verifies that wake hints are content-free.
...
def interactive_setup() -> None:
    """Interactive ``nunmai gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Nunmai env
    ...
    print_info("Connect Nunmai to Raft as an external agent.")
