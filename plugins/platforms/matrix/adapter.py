...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Nunmai command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Nunmai accepts ``!command`` as a Matrix-friendly alias, but only for
...
# Uses get_nunmai_home() so each profile gets its own Matrix store.
from nunmai_constants import get_nunmai_dir as _get_nunmai_dir

_STORE_DIR = _get_nunmai_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Nunmai
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("NUNMAI_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("NUNMAI_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Nunmai Engine" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in nunmai_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces nunmai_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Nunmai delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via nunmai_cli.gateway.get_env_value so setup-status
