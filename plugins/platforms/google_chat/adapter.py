...
def ensure_google_chat_deps() -> bool:
    ...
    ``NUNMAI_LAZY_INSTALL_TARGET`` instead of the read-only venv. Resets the
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # The profile scope this adapter was connected under (multiplex: NUNMAI_HOME override + secret
        ...
        self._thread_count_store = _ThreadCountStore(_get_nunmai_home() / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from nunmai_constants import get_nunmai_home as _get_nunmai_home
        return _get_nunmai_home() / "google_chat_bot_id.json"
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "nunmai_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nunmai is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Nunmai is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. nunmai-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``nunmai setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.nunmai/.env``."""
    from nunmai_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``nunmai cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Nunmai is thinking…' marker message "
