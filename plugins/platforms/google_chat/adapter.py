...
# ``model_tools`` import time, so ``nunmai status``, ``nunmai chat``, etc.
...
# ``nunmai_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nunmai is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nunmai MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "nunmai-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NUNMAI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from nunmai_constants import get_nunmai_home as _get_nunmai_home
            _nunmai_home = _get_nunmai_home()
        except (ModuleNotFoundError, ImportError):
            _nunmai_home = _Path.home() / ".nunmai"
        self._thread_count_store = _ThreadCountStore(
            _nunmai_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nunmai is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NUNMAI_HOME", str(_Path.home() / ".nunmai"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NUNMAI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nunmai MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nunmai is thinking…' marker message.
        ...
        "Nunmai is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nunmai is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nunmai is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nunmai is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nunmai setup``.

    The setup wizard at ``nunmai_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nunmai/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nunmai-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nunmai/.env")
    print_info("Restart the gateway: nunmai gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``nunmai cron`` running as a
    separate process from ``nunmai gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nunmai plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nunmai setup` to install Google Chat support.",
        ...
        # the "Nunmai is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nunmai is thinking…' marker message "
