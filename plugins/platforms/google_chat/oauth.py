...
legacy single-user mode). Files under ``${NUNMAI_HOME}``: ``google_chat_user_tokens/
...
# human-readable under ``ls ~/.nunmai/google_chat_user_tokens/``.
...
def _token_rel(email: Optional[str]) -> str:
    """NUNMAI_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def _user_tokens_dir() -> Path:
    return get_nunmai_home() / "google_chat_user_tokens"


def _token_path(email: Optional[str] = None) -> Path:
    """Per-user token path for ``email``, or the legacy single-user path."""
    return get_nunmai_home() / _token_rel(email)


def _client_secret_path() -> Path:
    return get_nunmai_home() / "google_chat_user_client_secret.json"


def _pending_auth_path(email: Optional[str] = None) -> Path:
    if email:
        return get_nunmai_home() / "google_chat_user_oauth_pending" / f"{_sanitize_email(email)}.json"
    return get_nunmai_home() / "google_chat_user_oauth_pending.json"
...
def install_deps() -> bool:
    ...
    try:
        ...
        # lazy_deps honors NUNMAI_LAZY_INSTALL_TARGET on sealed hosted images;
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    print(f"Profile path: {display_nunmai_home()}/{_token_rel(email)}")
