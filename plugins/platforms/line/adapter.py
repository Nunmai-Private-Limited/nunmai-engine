...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, NUNMAI_HOME).
        ...
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), nunmai_home)):  # no-tmp: ok — macOS /private/tmp alias in the allowed-roots check, not a write target
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``nunmai status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``nunmai setup line`` wizard (writes ``~/.nunmai/.env``); CLI helpers are lazy-imported."""
    from nunmai_cli.config import get_env_value, save_env_value
    from nunmai_cli.cli_output import print_header, print_info, prompt
