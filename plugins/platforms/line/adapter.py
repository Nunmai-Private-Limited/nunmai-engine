...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), nunmai_home)):
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``nunmai status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nunmai setup line`` (writes ``~/.nunmai/.env``)."""
