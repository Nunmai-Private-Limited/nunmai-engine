"""
LINE Messaging API platform adapter for Nunmai Engine.
...
to Nunmai Engine. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NUNMAI_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nunmai_home = Path(get_nunmai_home()).resolve()
        except Exception:
            nunmai_home = Path.home().joinpath(".nunmai").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nunmai status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nunmai status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nunmai setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nunmai/.env`` via ``nunmai_cli.config``.
    """
    print()
    ...
    try:
        from nunmai_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("nunmai_cli.config not available; set LINE_* vars manually in ~/.nunmai/.env")
        return
