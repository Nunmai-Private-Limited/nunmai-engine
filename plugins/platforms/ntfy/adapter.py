"""ntfy platform adapter (Nunmai plugin).
...
``plugins/platforms/ntfy/``. The Nunmai plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "nunmai-in"              # subscribe topic (incoming)
          publish_topic: "nunmai-out"     # optional — defaults to topic
...
_ECHO_TAG = "nunmai-engine"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``nunmai cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Nunmai dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `nunmai gateway status` without
