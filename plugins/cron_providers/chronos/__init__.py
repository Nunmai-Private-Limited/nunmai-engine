...
Holds no scheduler credentials — speaks only to NAS ``agent-cron`` endpoints with the Nunmai token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def __init__(self) -> None:
        ...
        # Set when NAS answered 403 invalid_client: the Nunmai token in auth.json is not this
    ...
    def is_available(self) -> bool:
        ...
        Nunmai login; otherwise the resolver falls back to the built-in ticker."""
    ...
    def _note_identity_rejected(self) -> None:
        ...
        ``agent:*`` client or the hosted bootstrap session, and a plain ``nunmai auth`` login is
        ...
        logger.warning(
            "Chronos: NAS rejected this agent's Nunmai credential for agent-cron (403 invalid_client). "
            "The Nunmai token in auth.json is not this instance's provisioned identity (an agent:* client "
            "or the hosted bootstrap session), so no job can be armed. A normal `nunmai auth` re-login "
            "cannot fix this; the hosted credential has to be restored from the Nunmai Portal. Falling back "
            "to the built-in cron ticker for this process so scheduled jobs keep firing on time.")
        if self._stop_event is None:
            return  # start() never ran (e.g. a CLI `nunmai cron add`); nothing to tick here
