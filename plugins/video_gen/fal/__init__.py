...
Authentication via ``FAL_KEY`` or the managed Nunmai gateway. Output is an
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"Nunmai Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Nunmai Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `nunmai tools` → Video Generation."
            ) from exc
        raise


def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `nunmai tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `nunmai tools` → Video Generation → FAL to configure) "
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `nunmai tools` → Video Generation."
                    ),
