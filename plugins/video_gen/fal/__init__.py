...
path containing one) → ``DEFAULT_MODEL``. Auth via ``FAL_KEY`` or the managed Nunmai gateway; output is an HTTPS URL.
"""
...
def _resolve_managed_fal_video_gateway():
    ...
    if selected == NOUS_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error("video_gen", NOUS_MANAGED_PROVIDER, "the Nunmai Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            if billing is not None:
                raise ValueError(
                    f"Nunmai Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}): {billing}") from exc
            raise ValueError(f"Nunmai Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}). This model may not yet be enabled "
                             f"on the Nunmai Portal's FAL proxy. Either:\n  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                             f"  • Pick a different model via `nunmai tools` → Video Generation.") from exc
        raise
...
_NO_BACKEND_MSG = ("No FAL backend available. Either set FAL_KEY (run `nunmai tools` → Video Generation → FAL to configure) "
                   "or sign in to Nunmai (`nunmai setup`) for managed gateway access.")
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `nunmai tools` → Video Generation.",
