...
class OpenRouterVideoGenProvider(VideoGenProvider):
    ...
    def _headers(self, api_key: str) -> Dict[str, str]:
        return {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json",
                "HTTP-Referer": "https://github.com/Nunmai-Private-Limited/nunmai-engine", "X-Title": "Nunmai Engine"}
    ...
    def generate(
        ...
        if not api_key:
            return fail("No OpenRouter credential: set OPENROUTER_API_KEY or run `nunmai auth add openrouter`",
                        "missing_credentials")
