# Nunmai Engine Security Policy

This document describes Nunmai Engine's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Nunmai-Private-Limited/nunmai-engine/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Nunmai Engine does not operate a bug
bounty program.**
...
- Environment details (`nunmai --version`, commit SHA, OS, Python
  version).
...
Nunmai Engine is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Nunmai Engine,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Nunmai Engine access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Nunmai Engine's documentation
...
Nunmai Engine supports two OS-level isolation postures. They address
...
Nunmai Engine supports this in two ways:

- **Nunmai Engine's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Nunmai Engine's in-process heuristics
...
Nunmai Engine filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Nunmai Engine
itself. Bugs in Nunmai Engine's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Nunmai Engine does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Nunmai Engine's own documentation, or reasonable
  operator expectations would predict — including cases where
  Nunmai Engine has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  nunmai-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Nunmai Engine's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Nunmai Engine. Bugs in Nunmai
...
- Nunmai Engine includes supply-chain guards for MCP server
