"""Timezone-aware clock for Nunmai.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``NUNMAI_TIMEZONE`` env var, then ``timezone`` in ``~/.nunmai/config.yaml``, else server-local
...
# NUNMAI_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``NUNMAI_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("NUNMAI_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without nunmai_cli importable.
        try:
