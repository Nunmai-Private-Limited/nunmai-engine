...
Resolution order:
  1. ``NUNMAI_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.nunmai/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Nunmai never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Nunmai requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("NUNMAI_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``NUNMAI_TIMEZONE`` update) to force ``get_timezone()`` /
