"""Drive the real ``nunmai fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from nunmai_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="nunmai_test_fallback_") as home:
        hh = Path(home) / ".nunmai"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "NUNMAI_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "nunmai_cli.main", "fallback", "add"],
