...
Each probe runs in a FRESH subprocess with an isolated NUNMAI_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "NUNMAI_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "NUNMAI_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("NUNMAI_SESSION", "NUNMAI_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "nunmai_cli.main", "gateway.run", "tools.registry", "nunmai_state", "tui_gateway.server", "nunmai_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "nunmai-engine" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `nunmai --version`, `nunmai --help`, `nunmai doctor --help`, `nunmai config get model` (no network)
CLI = {"version": ["nunmai_cli/main.py", "--version"], "help": ["nunmai_cli/main.py", "--help"], "config_get": ["nunmai_cli/main.py", "config", "get", "model"], "tools_list": ["nunmai_cli/main.py", "tools", "--help"], "skills_help": ["nunmai_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_nunmai_default_ms", lambda: resolve_toolset("nunmai-default"), 200)
...
try:
    import nunmai_state, tempfile, uuid
    from pathlib import Path
    db=nunmai_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(nunmai_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
