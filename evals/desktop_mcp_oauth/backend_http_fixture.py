...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["NUNMAI_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(NunmaiTokenStorage("positive", nunmai_home=owner).get_tokens())
        ...
        override = set_nunmai_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from nunmai_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="nunmai-oauth-http-") as temp:
            home = Path(temp)
            (home / ".nunmai").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "NUNMAI_HOME": str(home / ".nunmai"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
