...
- **`forensics/`** reads a *copy* of a Nunmai `state.db` (plus rotated `agent.log*` and git) and
...
- A Nunmai checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.nunmai/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Nunmai is idle) and, optionally, the rotated `~/.nunmai/logs/agent.log*`.
- For `--live` probes: real credentials in `NUNMAI_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.nunmai/logs/agent.log*"
...
Nunmai recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp NUNMAI_HOME), main vs a branch or integration checkout:
...
Reference results (2026-09-05/06): Nunmai native wire 13.9% stuck (14–20% over 4 runs, unchanged by the
portal's provider pin or a 2 s settle); Nunmai chat wire 0/320; OpenRouter pinned 0/161; OpenRouter
...
Forensic lanes: five parallel Nunmai subagents (2026-09-04), rewritten here to take `--db`/`--root`
