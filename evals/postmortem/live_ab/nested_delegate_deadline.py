...
real result. Uses glm-5.3 via Nunmai for cost. Deadline shortened via config to keep the run short."""
...
# Temp NUNMAI_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["NUNMAI_HOME"] = home
real_home = os.environ.get("NUNMAI_HOME_SOURCE", os.path.expanduser("~/.nunmai"))  # credentials are copied from here into a temp home
