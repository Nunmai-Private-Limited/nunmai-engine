...
message prefix between call N and N+1. If Nunmai strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--nunmai-home DIR]   (default NUNMAI_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("NUNMAI_HOME", os.path.expanduser("~/.nunmai"))
if "--nunmai-home" in sys.argv:
    os.environ["NUNMAI_HOME"] = sys.argv[sys.argv.index("--nunmai-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.nunmai/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
