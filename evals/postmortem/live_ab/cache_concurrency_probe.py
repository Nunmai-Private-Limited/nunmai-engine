...
Runs N independent Nunmai ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated nunmai-engine #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
Fable 5.1, 20 sessions x 6 calls unless noted):
  nous, native /v1/messages         13.9% stuck (4 runs 14-20%)     -> chat is the Nunmai default
...
Usage:
  ...
  providers: nous (Portal creds from NUNMAI_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Nunmai request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="nunmai-engine checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Nunmai would pick for the provider/model)")
...
os.environ.setdefault("NUNMAI_HOME", os.path.expanduser("~/.nunmai"))
import anthropic
