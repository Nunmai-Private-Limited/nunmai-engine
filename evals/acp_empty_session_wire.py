...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='nunmai-acp-empty-'))
    nunmai = home / '.nunmai'
    nunmai.mkdir()
    (nunmai / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), NUNMAI_HOME=str(nunmai), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               NUNMAI_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(nunmai / 'state.db') as db:
