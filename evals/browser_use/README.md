...
- **Arms differ only by tree + config.** `base` runs the built-in twelve
  ...
  digest's value). Each cell gets a throwaway `NUNMAI_HOME`; web-fetch
...
- **Backend matrix.** `orchestrate.py` drives a local headless-Chrome CDP;
  `orchestrate_cloud.py --backend nunmai-cloud|browserbase` provisions a real
...
opus4.8    nunmai-cloud    12/12     33330    2.8   33.8
opus4.8    browserbase    6/6      26712    2.2   23.2
kimi-k3    local-cdp     18/18     19230    2.4   33.3
kimi-k3    nunmai-cloud    12/12     22050    2.9   41.4
