#!/usr/bin/env bash
# Live A/B for the interrupted-pull repair (nunmai_cli/_early_recovery.py::restore_interrupted_pull).
#
...
# utils.py, makes nunmai_cli/config.py and run_agent.py import a new utils name, and adds a package. Runs the REAL autostash + _pull_updates from the install's own tree and the REAL entry point
# (`python -m nunmai_cli.main config path`) under a disposable HOME/NUNMAI_HOME:
#   A  updater SIGKILLed before git wrote anything; user edits upstream-changed files, fetches, runs nunmai
#   B  custom branch whose commit conflicts upstream: update exits 1; user merges by hand, runs nunmai
...
#      the merged run_agent.py; the user edits a file git never wrote, then runs `nunmai-engine`'s import
# Fixed: A/B untouched, C/D/E restored with the user edit kept and the re-update lands; one VERDICT line.
# [python] defaults to <repo>/venv/bin/python (needs the Nunmai deps).
set -u
REPO=$1; REF=$2; LABEL=$3
PY=${4:-$REPO/venv/bin/python}
U=$(mktemp -d -t "nunmai-interrupted-pull-ab-$LABEL.XXXXXX")
export HOME=$U/fakehome NUNMAI_HOME=$U/fakehome/.nunmai
unset PYTEST_CURRENT_TEST
cat > "$U/pull.py" <<'PYEOF'
# `nunmai update` after its fetch: the REAL autostash + _pull_updates, imported from the install's tree.
import pathlib
import sys

root = pathlib.Path(sys.argv[1])
sys.path.insert(0, str(root))
import nunmai_cli.main as m  # noqa: E402

assert pathlib.Path(m.__file__).resolve().is_relative_to(root.resolve()), m.__file__
m.PROJECT_ROOT = root
from nunmai_cli import update_cmd  # noqa: E402
...
setup() {  # $1 = install kind: clone | worktree
  cd "$U"; rm -rf "$U/origin" "$U/install" "$U/wtbase"; mkdir -p "$NUNMAI_HOME"
  ...
  ( cd "$U/origin" && git checkout -q -B main "$REF" && git config user.email t@x.invalid && git config user.name t
    printf '\n\ndef _torn_probe():\n    return 1\n' >> utils.py
    sed -i 's/^from utils import atomic_replace, fast_safe_load, file_signature$/from utils import atomic_replace, fast_safe_load, file_signature, _torn_probe  # noqa: F401/' nunmai_cli/config.py
    grep -q _torn_probe nunmai_cli/config.py || { echo "SETUP: config.py import line not found"; exit 1; }
...
marker() { cat "$(git rev-parse --git-dir)/nunmai-update-pull" 2>/dev/null | tr '\n' ' '; }
nunmai() { PYTHONPATH=$U/install timeout 120 "$PY" -m nunmai_cli.main config path 2>&1 | grep -v '^$' | head -6 | sed 's/^/    nunmai> /'; }
...
echo; echo "--- A: kill before git wrote anything, then user edits + fetch, then any nunmai command"
...
echo "  before nunmai: [$(git status --porcelain | tr '\n' ' ')]"
nunmai
echo "  after nunmai:  [$(git status --porcelain | tr '\n' ' ')] marker=[$(marker)]"
...
for a in "$@"; do
  if [ "$a" = "--ff-only" ]; then
    /usr/bin/git show origin/main:nunmai_cli/config.py > nunmai_cli/config.py
...
# ---- E: the custom-branch `git merge` wrote its merged run_agent.py, SIGKILL; `nunmai-engine` launches ----
echo; echo "--- E: kill inside the custom-branch merge, then the nunmai-engine entry point (run_agent)"
setup clone >/dev/null
...
PYTHONPATH=$U/install timeout 120 "$PY" -c "import run_agent; print('run_agent imported')" 2>&1 | grep -v '^$' | tail -4 | sed 's/^/    nunmai-engine> /'
