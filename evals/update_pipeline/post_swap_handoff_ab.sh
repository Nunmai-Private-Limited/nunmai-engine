#!/usr/bin/env bash
# Live A/B for the post-swap hand-off (nunmai_cli/update_handoff.py).
#
# Installs a real clone at <installed-sha>, publishes a target commit on its origin that
# (a) adds a symbol to nunmai_logging (purge-protected on the old updater),
# (b) imports it at module level from nunmai_cli/config.py (re-imported by the old updater's
...
# then runs `nunmai update` from the installed clone and reads the receipt + output.
#
...
# Needs: uv on PATH, a Python for the venv. Runs with a disposable HOME/NUNMAI_HOME, no npm
# on PATH (skips Node/web/Desktop work) and --no-gateway-restart (never touches your fleet).
set -euo pipefail
REPO=$1; SHA=$2; LABEL=$3
ROOT=$(mktemp -d -t nunmai-post-swap-ab.XXXXXX)
...
(
  ...
  python3 - <<'PY'
import re
p = "nunmai_cli/config.py"; s = open(p).read()
s = s.replace("import logging\n", "import logging\nfrom nunmai_logging import post_swap_probe  # noqa: F401  (A/B probe: new symbol on a cached module)\n", 1)
open(p, "w").write(s)
p = "nunmai_cli/update_cmd_maint.py"; s = open(p).read()
needle = 'def _update_complete_message('
i = s.index(needle); j = s.index('\n', s.index('"""', s.index('"""', i) + 3)) + 1  # end of docstring
s = s[:j] + '    from nunmai_logging import post_swap_probe\n    print(f"  [completion printed by: {post_swap_probe()}]")\n' + s[j:]
open(p, "w").write(s)
PY
  git -c user.name=ab -c user.email=ab@example.invalid commit -qam "probe: new nunmai_logging symbol used by config.py and the completion line"
  git push -q origin main
)

# Venv for the installed clone (deps unchanged between the two commits → the updater skips the sync).
uv venv -q --python "${NUNMAI_AB_PYTHON:-3.11}" "$ROOT/install/.venv"
uv pip install -q --python "$ROOT/install/.venv/bin/python" -e "$ROOT/install"

export HOME="$ROOT/home" NUNMAI_HOME="$ROOT/home/.nunmai"
mkdir -p "$NUNMAI_HOME"
export PATH="$ROOT/install/.venv/bin:/usr/bin:/bin"
set +e
( cd "$ROOT/install" && python -m nunmai_cli.main update --yes --no-backup --no-gateway-restart ) > "$ROOT/update.out" 2>&1
CODE=$?
set -e
...
python3 - "$NUNMAI_HOME/logs/update_receipts/latest.json" <<'PY'
import json, sys
try:
