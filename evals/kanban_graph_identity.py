...
def main():
    ...
    if not args.child:
        with tempfile.TemporaryDirectory(prefix="kanban-identity-") as home:
            env = {"HOME": home, "NUNMAI_HOME": home + "/nunmai", "PATH": os.defpath,
    ...
    from nunmai_cli.kanban_db_dispatch import dispatch_once
    from tools.kanban_tools import _handle_create
    from nunmai_cli.kanban_decompose import _apply_fanout, _Routing
    graph = importlib.import_module("nunmai_cli.kanban_db_graph") if (args.repo / "nunmai_cli/kanban_db_graph.py").exists() else kb
