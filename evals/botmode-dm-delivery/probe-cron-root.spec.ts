...
test.beforeAll(async () => {
  ...
  for (const name of ['default', 'alpha', 'beta']) {
    const home = name === 'default' ? sandbox.nunmaiHome : path.join(sandbox.nunmaiHome, 'profiles', name)
  ...
  fs.mkdirSync(bin)
  fs.writeFileSync(path.join(bin, 'nunmai'), `#!/bin/sh\ncd ${repo}\nprintf '%s\\n' "$$ $NUNMAI_HOME $*" >> ${evidence}/children.log\nexec ${python} -m nunmai_cli.main "$@"\n`, { mode: 0o755 })
  env = buildAppEnv(sandbox, { HOME: sandbox.root, NUNMAI_DESKTOP_PYTHON: python,
    NUNMAI_DESKTOP_NUNMAI: path.join(bin, 'nunmai'), PATH: `${bin}:${process.env.PATH}`,
    PYTHONPATH: repo, NUNMAI_SINGLE_QUERY_LINGER_SECONDS: '1' })
...
test('ordinary cron pins an unowned named target under a custom root', async () => {
  ...
  const page = fixture.page
  const composer = await openBot('Nunmai')
  ...
  const discovery = 'from pathlib import Path; import json,os; from tools.bot_live_delivery import find_canonical_owner; h=Path(os.environ["NUNMAI_HOME"]); print(json.dumps({"default":find_canonical_owner(h),"alpha":find_canonical_owner(h/"profiles"/"alpha")}))'
  ...
  try {
    ...
    expect(launched).toContain(path.join(fixture.sandbox.nunmaiHome, 'profiles', 'alpha'))
    expect(held.default.lease_id).toBe(before.default.lease_id)
    expect(fs.existsSync(path.join(fixture.sandbox.nunmaiHome, 'cron', 'bot_chat_pending'))).toBe(false)
    ...
    const rows = probe('import sqlite3,json,os; from pathlib import Path; h=Path(os.environ["NUNMAI_HOME"]); print(json.dumps({n:sqlite3.connect(h/"state.db" if n=="default" else h/"profiles"/n/"state.db").execute("select session_id,role,content from messages").fetchall() for n in ["default","alpha"]}))')
    ...
    const missing = probe('import json,os; from pathlib import Path; from cron.scheduler_delivery import _deliver_to_bot_chat; h=Path(os.environ["NUNMAI_HOME"]); p=h/"profiles"/"beta"; p.rename(h/"profiles"/"beta-removed"); result=_deliver_to_bot_chat({"id":"removed","execution_id":"missing"},"MUST_NOT_RUN","beta"); print(json.dumps({"result":result,"recreated":p.exists(),"wrong_root":(Path.home()/".nunmai").exists()}))')
