...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), NUNMAI_HOME=str(home / "nunmai"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("nunmai-gateway*", "nunmai-serve*")] + [unit + ".service"]
...
unit = f"nunmai-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
