...
Run with the Nunmai venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Nunmai children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "nunmai_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="nunmai-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".nunmai"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "NUNMAI_HOME": str(state),
