...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="nunmai_test_notice_") as home:
        hh = Path(home) / ".nunmai"
        hh.mkdir()
        ...
        from nunmai_cli.banner import VERSION
        ...
        bootstrap = ("import nunmai_cli.main as m; import nunmai_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "nunmai_cli/banner.py") in text,
