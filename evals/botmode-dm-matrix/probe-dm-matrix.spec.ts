...
test.beforeAll(async () => {
  ...
  env = buildAppEnv(sandbox, { HOME: sandbox.root, NUNMAI_DESKTOP_PYTHON: python,
    ...
    PYTHONPATH: repo, NUNMAI_SINGLE_QUERY_LINGER_SECONDS: '30' })
  for (const name of ['alpha', 'beta', 'gamma']) {
    const h = path.join(sandbox.nunmaiHome, 'profiles', name)
    execFileSync(python, ['-c', 'import sys; from pathlib import Path; from nunmai_state import SessionDB; d=SessionDB(db_path=Path(sys.argv[1])/"state.db"); d.create_session("matrix-"+sys.argv[2],"cli",cwd=sys.argv[3]); d.set_session_title("matrix-"+sys.argv[2],"Bot Chat"); d.close()', h, name, sandbox.root], { env, cwd: repo })
  }
...
async function openBot(name: string) {
  ...
  const row = page.getByRole('button', { name: new RegExp(`^${name === 'default' ? 'nunmai' : name}\\b`, 'i') }).filter({ visible: true }).first()
...
function dbMessages(name: string) {
  const home = name === 'default' ? fixture.sandbox.nunmaiHome : path.join(fixture.sandbox.nunmaiHome, 'profiles', name)
...
test('incoming live sender card is rendered before any reload', async () => {
  ...
  const child = spawn(python, ['-m', 'nunmai_cli.main', '-p', 'beta', 'chat', '--in', '~', '-c', 'Bot Chat', '--create-if-missing', '-Q', '-q', 'E2E_DM(alpha)[matrix-live-sender-card]'], { cwd: repo, env, stdio: ['ignore', output, output] })
  try {
