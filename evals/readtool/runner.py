"""Run the read-tool eval through the REAL Nunmai AIAgent.

For each task: fresh temp NUNMAI_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    nunmai_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".nunmai"
    ...
    try:
        ...
        shutil.rmtree(nunmai_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.nunmai/.env; set +a  — then relaunch."
        )
