...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "nunmai", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="Nunmai is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("NUNMAI_HOME", os.path.join(os.path.dirname(a.out), "nunmai-home-probe"))
    os.makedirs(os.environ["NUNMAI_HOME"], exist_ok=True)
