...
const home = mkdtempSync(join(tmpdir(), 'nunmai-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, NUNMAI_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', NUNMAI_NONINTERACTIVE: '1',
  NUNMAI_DASHBOARD_SESSION_TOKEN: token, NUNMAI_SERVE_HEADLESS: '1',
  NUNMAI_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'nunmai_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-Nunmai-Session-Token': token } })
