...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "NUNMAI_HOME": str(root / "home/.nunmai"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "NUNMAI_NONINTERACTIVE": "1"}
    Path(env["NUNMAI_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from nunmai_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'nunmai')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,nunmai_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,nunmai_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Nunmai\n", b"Name=Nunmai custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["NUNMAI_HOME"]) / "config.yaml"
