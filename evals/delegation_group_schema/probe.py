...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, NUNMAI_HOME=home, PATH="/usr/bin:/bin", NUNMAI_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["nunmai-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["nunmai-cli"], quiet_mode=True)
