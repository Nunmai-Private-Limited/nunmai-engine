...
Differences from the TypeScript original are format-only: Nunmai transcripts
...
def apply_decisions(messages, decisions: List[Decision], calls: List[ToolCall], head: int):
    """Rebuild the Nunmai transcript: dropped calls vanish with their result row,
...
def openrouter_asker(api_key: Optional[str] = None, model: str = OPENROUTER_JEV_MODEL,
    ...
    def ask(state, questions):
        ...
        req = urllib.request.Request(url, data=body, headers={
            ...
            "HTTP-Referer": "https://github.com/Nunmai-Private-Limited/nunmai-engine", "X-Title": "nunmai compaction eval",
        })
        try:
