# fast-jev-compaction vs Nunmai compaction — 3-transcript scorecard (2026-09-19)
...
compaction multiplies cache breaks; it is the wrong trade for Nunmai.
...
configured `auxiliary.compression` route (gemini-3.8-flash via Nunmai). A fourth transcript
...
4. **The state ceiling does not fit Nunmai scale.** Jev's 32K window forces the whole
   ...
   all. The plugin is designed for Claude Code's ~200K compaction point; a 1M-window Nunmai
...
- Transcripts reconstructed with `scripts/reconstruct_lineage.py` from a state.db copy;
  ...
  identical exam.
- The `jev` arm counts rows (Nunmai has one `role: tool` row per result), so
...
- Eval spend for the whole run (question generation, 634 answer/judge calls): 43.5M input
  tokens ≈ $33.6 at gemini-3.8-flash list, through Nunmai inference. Jev spend across all
