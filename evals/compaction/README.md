...
cp ~/.nunmai/state.db /tmp/state_copy.db
...
Threshold 500000 ≈ Nunmai' 1M-window posture; 160000 ≈ a 200K-window host.
...
- **Default arm is `current+recovery`: the production path.** Compaction in
  Nunmai is the summary *plus* the session_search pointer it carries, so the
