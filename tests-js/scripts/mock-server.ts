 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `nunmai serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Nunmai should add
...
export const APPROVAL_COMMAND = 'rm -rf /tmp/nunmai-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}nunmai Reply with B.] E2E_SAY(nunmai)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `nunmai-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated NUNMAI_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `nunmai-dev-mock-${Date.now()}`))
  const nunmaiHome = nodePath.join(root, 'nunmai-home')
...
function writeMockConfig(nunmaiHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(nunmaiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(nunmaiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.nunmaiHome, mock.url)
  console.log(`  NUNMAI_HOME: ${sandbox.nunmaiHome}`)
  ...
  const env: Record<string, string> = {
    ...
    NUNMAI_DESKTOP_NUNMAI_ROOT: repoRoot,
    NUNMAI_DESKTOP_APP_NAME: `NunmaiDevMock-${Date.now()}`,
  }
