...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/nunmai.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/nunmai.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'nunmai-update',
  error: 'E2E ASSERTION FAILED: nunmai update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('nunmai.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'nunmai-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured nunmai desktop spec; update completed',
      logs: { desktop: '[nunmai] [updates] no staged updater; surfacing manual `nunmai update` for CLI install at C:/install\n[nunmai] [updates] manual: nunmai update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: nunmai update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> nunmai-update (v2026.3.12 -> HEAD)'
