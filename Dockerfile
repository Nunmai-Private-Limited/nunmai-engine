...
# for the same reason: it lazy-installs into /opt/nunmai/.venv at first
...
RUN set -eu; \
    ...
    mkdir -p /etc/nunmai; \
    NUNMAI_GIT_SHA="${NUNMAI_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/nunmai/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/nunmai/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "nousresearch/hermes-agent", "version": project["version"], "revision": os.environ.get("NUNMAI_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'
