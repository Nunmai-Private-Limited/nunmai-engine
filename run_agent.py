...
# IMPORTANT: nunmai_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See nunmai_bootstrap.py for full rationale.
try:
    import nunmai_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when nunmai_bootstrap isn't registered in the venv
    # yet — happens during partial ``nunmai update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``nunmai -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("NUNMAI_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("NUNMAI_SESSION_SOURCE", "")
...
from nunmai_cli.timeouts import (
...
_loaded_env_paths = load_nunmai_dotenv(nunmai_home=_nunmai_home, project_env=_project_env)
...
# (nunmai_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Nunmai-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.nunmai/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[nunmai-engine: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `nunmai --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from nunmai_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``nunmai chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``nunmai chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``NUNMAI_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``NUNMAI_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("NUNMAI_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``NUNMAI_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("NUNMAI_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``NUNMAI_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("NUNMAI_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        nunmai-engine #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See nunmai-engine#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Nunmai against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Nunmai can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("NUNMAI_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from nunmai_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``NUNMAI_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.nunmai/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Nunmai-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects NUNMAI_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Nunmai' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Nunmai model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Nunmai-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``NUNMAI_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("NUNMAI_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.nunmai/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``NUNMAI_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("NUNMAI_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Nunmai process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Nunmai process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `nunmai doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.nunmai/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.nunmai/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`nunmai doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (NUNMAI_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (NUNMAI_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset NUNMAI_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("NUNMAI_DEV_CREDITS"))
        ...
        if _dev:
            # NUNMAI_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `nunmai logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Nunmai YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``nunmai auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.nunmai/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``nunmai setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from nunmai_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from nunmai_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = nunmai_xai_default_headers()
        else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from nunmai_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from nunmai_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from nunmai_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated nunmai_logging._session_context (a
            ...
            try:
                from nunmai_logging import set_session_context
            ...
            # NUNMAI_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from nunmai_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            # Serialize the full load -> run -> flush region across Nunmai
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Nunmai process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Nunmai process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Nunmai process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Nunmai process kept this session busy too "
