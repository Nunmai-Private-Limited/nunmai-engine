...
try:
    ...
    pass  # partial `nunmai update` — only skips the Windows UTF-8 stdio setup

import sys

# `nunmai-engine` runs this module without nunmai_cli.main, which repairs a `nunmai update` killed
# while git wrote the new tree; do it here, before importing anything else from the checkout.
if "nunmai_cli.main" not in sys.modules:
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``nunmai -c`` / ``--resume``), or None.
...
# Sources that label the human conversation an interactive UI transport hosts. A finite ``nunmai chat -q`` /
# one-shot child spawned from such a session inherits NUNMAI_SESSION_SOURCE (the terminal tool bridges the
# session env into child processes) but is NOT that conversation: labelling it ``tui``/``desktop`` lists it
# in the TUI/WebUI pickers as a resumable chat and lets ``nunmai -c`` in the TUI continue it (#112550).
# Automation sources (kanban, tool, cron, a2a, ...) are inherited on purpose.
_UI_TRANSPORT_SOURCES = frozenset({"tui", "desktop"})

# Finite non-interactive CLI runs (``nunmai chat -q``/``--oneshot``, ``nunmai -z``) get their own source so human
# pickers hide them without title/cwd heuristics; ``nunmai -c`` still treats them as CLI history.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    ...
    source = str(get_session_env("NUNMAI_SESSION_SOURCE", "") or "").strip()
    single_query = get_session_env("NUNMAI_SINGLE_QUERY_SESSION", "") == "1"
    explicit = get_session_env("NUNMAI_SESSION_SOURCE_EXPLICIT", "") == "1"
...
_nunmai_home = get_nunmai_home()  # read by agent_init via _ra()._nunmai_home
_loaded_env_paths = load_nunmai_dotenv(nunmai_home=_nunmai_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from nunmai_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``NUNMAI_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("NUNMAI_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``NUNMAI_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("NUNMAI_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _provider_model_requires_responses_api(model: str, *, provider: Optional[str] = None) -> bool:
        ...
        # Nunmai serves GPT-5.x via chat completions (its /v1/responses returns 404); generic custom endpoints
