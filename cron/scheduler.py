...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``nunmai cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # (interrupted `nunmai update`, manual git pull) serves MIXED modules — old entries frozen in
...
def _snapshot_pin(job: dict, axis: str, current: str, job_id: str) -> str:
    ...
    if snapshot and current and snapshot.lower() != current.lower():
        logger.info(
            ...
            "`nunmai cron edit %s --%s <value>` or cron.%s in config.yaml moves it.",
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    creation snapshot > NUNMAI_MODEL > config ``model:``. Re-read every tick (no cache) so
    ``nunmai cron edit --model`` applies next tick."""
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_nunmai_home()))
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"NUNMAI_HOME": str(_get_nunmai_home().resolve())},
    )
...
def tick(
    ...
    try:
        # `nunmai pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
