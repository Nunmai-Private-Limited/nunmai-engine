...
A file lock (~/.nunmai/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
# `nunmai update`) otherwise fail with ModuleNotFoundError for nunmai_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from nunmai_constants import get_nunmai_home, nunmai_home_key
from cron.env_settings import cron_env_setting
from nunmai_cli._subprocess_compat import windows_hide_flags
from nunmai_cli.config import (
    load_config, load_config_readonly)
from nunmai_cli.fallback_config import get_fallback_chain, scoped_fallback_chain
from nunmai_time import now as _nunmai_now, safe_strftime
...
def _fallback_chain_phrase(job: Optional[dict] = None) -> str:
    ...
    if job is not None and _job_route_pinned(job):
        return (
            ...
            f"`fallback_providers`; `nunmai cron edit {job.get('id')} --unpin` lets it follow the "
    ...
    if chain:
        return "No backup provider succeeded either."
    return (
        "No backup provider is configured — add one with `nunmai fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `nunmai cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    yields (fail open). Raised, not returned, so ``record_ticker_error`` sees it and ``nunmai cron
    status`` isn't green.
    """
...
def stale_code_yield_labels(recorded_error: str | None) -> tuple[str, str] | None:
    ...
    ``nunmai cron status`` runs in another process and only sees the marker text; a yielding
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # underneath it (interrupted `nunmai update`, manual git pull) serves MIXED modules and every
    ...
    if not job.get("no_agent") and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`nunmai gateway restart` to fix it."
            )
...
def _repeat_alert_withheld(incident: dict) -> bool:
    ...
    try:
        ...
        return False
    return _elapsed_seconds(_nunmai_now(), last) < hours * 3600
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy nunmai_cli import at module load; shares MCP-membership with gateway/CLI
    from nunmai_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str]:
    ...
    try:
        from nunmai_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
        return sorted(_get_platform_tools(cfg or {}, "cron"))
    except Exception as exc:
        raise RuntimeError(
            "Cron toolset resolution failed, so this run was refused rather than given every "
            f"tool. Check `platform_toolsets.cron` in config.yaml (`nunmai cron doctor`): {exc}"
        ) from exc
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    from nunmai_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_key(job_id: str, home: Optional[Union[Path, str]] = None) -> tuple:
    ...
    """
    return (nunmai_home_key(home) if home is not None else nunmai_home_key(_get_nunmai_home()), job_id)


# Home key -> the real home Path that produced it. ``nunmai_home_key`` normcases (it lower-cases on
...
def _remember_inflight_home(home: Path) -> Path:
    ...
    _inflight_home_paths[nunmai_home_key(home)] = home
    return home
...
def get_running_job_details() -> list[dict]:
    ...
    runs). The drain wait publishes this so ``nunmai update`` can say WHICH job it is waiting on."""
    now = time.time()
    with _running_lock:
        return [
            {"job_id": key[1],
...
def get_wedged_job_ids() -> "frozenset[str]":
    ...
    if unresolved:
        ...
        local_home = nunmai_home_key(_get_nunmai_home())
        with _running_lock:
...
def try_register_running_job(job_id: str) -> bool:
    ...
    """
    from nunmai_cli.backend_retirement import retirement

    key = _inflight_key(job_id, _remember_inflight_home(_get_nunmai_home()))
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = cron_env_setting("NUNMAI_CRON_INFLIGHT_MAX_MINUTES").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid NUNMAI_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _nunmai_now().isoformat()}
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_nunmai_home() / "cron" / "inflight_forced_releases.jsonl"
...
def sweep_stale_inflight(due_jobs: Optional[list] = None) -> list:
    ...
    _local_home = nunmai_home_key(_get_nunmai_home())
...
def _cron_inactivity_seconds() -> float:
    """Parse NUNMAI_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the
    ...
    raw = cron_env_setting("NUNMAI_CRON_TIMEOUT").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid NUNMAI_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _get_parallel_pool(max_workers: Optional[int]) -> concurrent.futures.ThreadPoolExecutor:
    ...
    """
    home = nunmai_home_key(_remember_inflight_home(_get_nunmai_home()))
...
# Per-fire usage audit log; resolves via _get_nunmai_home() so profile-scoped paths work.
def _usage_audit_path() -> Path:
    return _get_nunmai_home() / "cron" / "usage_audit.jsonl"
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``nunmai update`` / ``nunmai
...
_nunmai_home: Path | None = None


def _get_nunmai_home() -> Path:
    """Nunmai home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active NUNMAI_HOME at call time means a profile's jobs are stored AND executed under that
    profile's home (its .env, config.yaml, scripts, skills).
    """
    return _nunmai_home or get_nunmai_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    nunmai_home = _get_nunmai_home()
    lock_dir = nunmai_home / "cron"
...
def _run_no_agent_job(
    ...
    try:
        ...
        load_nunmai_dotenv(nunmai_home=_get_nunmai_home())
    except Exception:
    ...
    now_iso = _nunmai_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _apply_monitor_gate(
    ...
    _mon_now = _nunmai_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    the main agent model (config ``model:``, then NUNMAI_MODEL). Re-read every tick (no cache) so
    ``nunmai cron edit --model`` and ``nunmai model`` both apply next tick."""
    model = job.get("model") or cron_env_setting("NUNMAI_MODEL") or ""
    _cron_default_provider = ""
    _cfg: dict = {}
    _model_cfg: Any = {}
    try:
        from nunmai_cli.config_effective import load_user_config_effective
        _cfg_path = str(_get_nunmai_home() / "config.yaml")
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"NUNMAI_MODEL={cron_env_setting('NUNMAI_MODEL')!r}, "
            "config.yaml model.default missing or empty). "
            f"Set a per-job model via "
            f"`nunmai cron edit {job_id} --model <name>` or set a "
            "default with `nunmai model <name>`."
        )
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        cron_env_setting("NUNMAI_PREFILL_MESSAGES_FILE")
    ...
    if not pfpath.is_absolute():
        pfpath = _get_nunmai_home() / pfpath
...
def _blocked_config_result(job_id: str, job_name: str, _pf_reason: str) -> tuple:
    ...
    blocked_doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {_nunmai_now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"**Status:** BLOCKED (configuration)\n\n"
        "The pre-run configuration check found a problem, so the agent did not run "
        "(nothing was charged).\n\n"
        f"**Reason:** {_pf_reason}\n\n"
        "Nunmai tries again at the next scheduled time and clears this state on the first healthy "
        "run; this alert is not repeated. Check with `nunmai cron doctor`. Set `cron.preflight: "
...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str]:
    ...
    try:
        # Do NOT pass NUNMAI_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (NUNMAI_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from NUNMAI_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override NUNMAI_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from nunmai_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # NUNMAI_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    try:
        ...
        _cron_title = f"{_title_base} · {safe_strftime(_nunmai_now(), '%b %d %H:%M')}"
    ...
    # vocabulary in nunmai_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: NUNMAI_IGNORE_USER_CONFIG=1.
    if not job.get("no_agent"):
...
_CRON_DELIVERY_VARS = (
    "NUNMAI_CRON_AUTO_DELIVER_PLATFORM",
    "NUNMAI_CRON_AUTO_DELIVER_CHAT_ID",
    "NUNMAI_CRON_AUTO_DELIVER_THREAD_ID")
...
class _CronRunScope:
    ...
    NUNMAI_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / NUNMAI_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the NUNMAI_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # NUNMAI_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["NUNMAI_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # NUNMAI_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    reset_secret_source_cache(_get_nunmai_home())
    load_nunmai_dotenv(nunmai_home=_get_nunmai_home())

    delivery_target = _resolve_delivery_target(job)
    if delivery_target:
        _VAR_MAP["NUNMAI_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
        _VAR_MAP["NUNMAI_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
        _VAR_MAP["NUNMAI_CRON_AUTO_DELIVER_THREAD_ID"].set(
...
def run_job(
    ...
    _cron_session_id = f"cron_{job_id}_{_nunmai_now().strftime('%Y%m%d_%H%M%S')}"
...
def run_one_job(
    ...
    external_owner = os.environ.get("_NUNMAI_CRON_EXTERNAL_WORKER") == execution_id
    ...
    profile_home = _get_nunmai_home().resolve()
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_nunmai_home()), profile_home=str(_get_nunmai_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_nunmai_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_nunmai_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_nunmai_home().resolve()),
    ...
    try:
        worker_env = strip_launch_profile_env(build_subprocess_env(
            ...
            extra={"NUNMAI_HOME": str(profile_home)},
        ))
    finally:
        reset_secret_scope(secret_token)
    worker_env = systemd_user_bus_env(worker_env)
    # Unattended worker: the gateway sets NUNMAI_EXEC_ASK at startup (interactive launches set
    ...
    # `-m cron.scheduler` has no nunmai_cli.main bootstrap; pin this checkout explicitly
    # (PYTHONSAFEPATH / stale editable mapping, #112729). See cron/scheduler_worker_env.py.
    from cron.scheduler_worker_env import pin_nunmai_tree_on_pythonpath
    repo_root = Path(__file__).resolve().parent.parent
    worker_env = pin_nunmai_tree_on_pythonpath(worker_env, repo_root)
    try:
    ...
    try:
        ...
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_NUNMAI_CRON_EXTERNAL_WORKER")
            os.environ["_NUNMAI_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_NUNMAI_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_NUNMAI_CRON_EXTERNAL_WORKER"] = old_external_execution
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the nunmai checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # Nunmai source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``nunmai -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `nunmai cron run` that claimed a job and
    ...
    _reap_key = nunmai_home_key(_get_nunmai_home())
...
def _resolve_max_parallel_workers() -> Optional[int]:
    """Max workers: env > config.yaml > unbounded (NUNMAI_CRON_MAX_PARALLEL=1 restores serial)."""
    try:
        _env_par = cron_env_setting("NUNMAI_CRON_MAX_PARALLEL").strip()
        if _env_par:
            return int(_env_par) or None
    except (ValueError, TypeError):
        logger.warning("Invalid NUNMAI_CRON_MAX_PARALLEL value; defaulting to unbounded")
    with contextlib.suppress(Exception):
        _ucfg = load_config() or {}
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from nunmai_logging import setup_logging

            setup_logging(nunmai_home=_get_nunmai_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
