...
Uses a file-based lock (~/.nunmai/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `nunmai update` reloads
# the module) fail with ModuleNotFoundError for nunmai_time et al.
...
from nunmai_time import now as _nunmai_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `nunmai fallback add`, "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Nunmai, pin it explicitly: "
                f"`nunmai cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `nunmai update`, manual
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy nunmai_cli import at cron module load (matches
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``nunmai tools`` config for the ``cron`` platform.
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``NUNMAI_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("NUNMAI_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_nunmai_home())
...
# (NUNMAI_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse NUNMAI_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("NUNMAI_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
...
# Resolves through _get_nunmai_home() so profile-scoped paths work correctly.
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.nunmai/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``nunmai update`` / ``nunmai gateway stop`` / systemd restart, or an
...
def _get_nunmai_home() -> Path:
    """Resolve Nunmai home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active NUNMAI_HOME at call time
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Nunmai — {task_name}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from nunmai_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``nunmai [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's NUNMAI_HOME) or a validated local profile name.  Returns None
    ...
    nunmai_bin = _shutil.which("nunmai")
    if nunmai_bin:
        argv = [nunmai_bin]
    else:
        try:
            ...
            if _ilu.find_spec("nunmai_cli") is not None:
                argv = [sys.executable, "-m", "nunmai_cli.main"]
            else:
                return "bot-chat delivery failed: nunmai CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: nunmai CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover NUNMAI_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="nunmai-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the NUNMAI_HOME
    ...
    ``~/.nunmai/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits NUNMAI_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from nunmai_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # NUNMAI_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `nunmai cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`nunmai cron run`) has no live relay adapter, so surface the
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("NUNMAI_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid NUNMAI_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    NUNMAI_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("NUNMAI_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid NUNMAI_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    NUNMAI_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("NUNMAI_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid NUNMAI_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within NUNMAI_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Nunmai-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against NUNMAI_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_nunmai_home() / "scripts"
    ...
    # escape — scripts MUST reside within NUNMAI_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("NUNMAI_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `nunmai setup`), or pin a "
            "working provider via `nunmai cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`nunmai setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        # "no delivery target resolved". load_nunmai_dotenv does not override
    ...
    # gateway message. Do not seed NUNMAI_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads NUNMAI_SESSION_PLATFORM
    #     and NUNMAI_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via NUNMAI_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     NUNMAI_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read NUNMAI_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the NUNMAI_CRON_AUTO_DELIVER_* vars set
    # below, so clearing NUNMAI_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # NUNMAI_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ NUNMAI_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_session_var = _VAR_MAP["NUNMAI_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `nunmai chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with NUNMAI_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $NUNMAI_KANBAN_TASK -- letting
        ...
        # load_nunmai_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this NUNMAI_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_nunmai_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from nunmai_cli.env_loader import (
            load_nunmai_dotenv,
        ...
        # cron-fleet default) > NUNMAI_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``nunmai cron edit --model``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"NUNMAI_MODEL={os.getenv('NUNMAI_MODEL', '')!r}, "
        ...
        prefill_file = (
            os.getenv("NUNMAI_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject NUNMAI_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Nunmai "
                        "pin it explicitly: "
                        f"`nunmai cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from NUNMAI_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # NUNMAI_HOME. When a workdir is configured, also inject project
        ...
        # override via NUNMAI_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from nunmai_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_nunmai_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_nunmai_now().strftime('%b %d %H:%M')}"
            ...
            # nunmai_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`nunmai cron run --prompt` /
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_nunmai_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `nunmai cron incidents ack`.
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`nunmai pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `nunmai resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `nunmai cron run` that claimed a job
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _nunmai_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _nunmai_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set NUNMAI_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("NUNMAI_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
