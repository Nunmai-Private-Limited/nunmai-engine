...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``nunmai://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``nunmai://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"nunmai://blueprint/{quote(blueprint.key)}{qs}"
