...
``nunmai gateway restart`` (or ``launchctl kickstart ai.nunmai.gateway``
or ``systemctl restart nunmai-gateway``).  When the cron fires, the
...
``nunmai cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``nunmai gateway``, ``launchctl ... nunmai-gateway``,
``systemctl ... nunmai-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_NUNMAI_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``nunmai gateway
...
The profile-flag form (``nunmai -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `nunmai gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `nunmai` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/nunmai gateway restart-notes.md`) from
    # matching via the `/nunmai` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])nunmai\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a nunmai-gateway label. macOS launchd
    # labels look like `ai.nunmai.gateway` / `nunmai-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated nunmai services (e.g.
    # `launchctl unload ai.nunmai.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.nunmai.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a nunmai-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bnunmai[.\-]?gateway)"
    # Branch D: pkill / kill targeting the nunmai gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.nunmai.gateway-... \` / `  -- ...`,
...
# selector — `nunmai -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `nunmai` and `gateway`
...
# `nunmai -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    Prefers the explicit ``NUNMAI_PROFILE_NAME`` / ``NUNMAI_PROFILE`` env
    (set by the profile launcher and kanban worker spawns), falling back to
    ``nunmai_cli.profiles.get_active_profile_name`` (derived from
    ``NUNMAI_HOME``, which the gateway process inherits from its launch
...
# Branch B only catches `launchctl <verb> ... nunmai[.-]?gateway` when the
...
# command — `for item in 'ai.nunmai.gateway-apollo:...' 'ai.nunmai.gateway:...';
...
# is the unexpanded variable `$label`, not the string "nunmai.gateway". This
...
# EXISTING job, so anchoring to the nunmai-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.nunmai.update-checker.plist`) to stay unblocked. The fix is checking
...
_NUNMAI_GATEWAY_LABEL_RE = re.compile(r"(?i)\bnunmai[.\-]?gateway\b")
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _NUNMAI_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``nunmai``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: nunmai gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `nunmai -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `nunmai` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (nunmai_cli.profiles
...
# for `systemctl restart nunmai-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart nunmai-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart nunmai-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.nunmai.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart nunmai-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart nunmai-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".nunmai"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<NUNMAI_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.nunmai/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_nunmai_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_nunmai_home() falls back to Path.home(), which raises when
        # neither NUNMAI_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.nunmai/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `nunmai gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `nunmai gateway restart` from a shell outside "
