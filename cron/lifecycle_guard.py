...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``NUNMAI_PROFILE_NAME``/``NUNMAI_PROFILE`` env first, then
    ``nunmai_cli.profiles.get_active_profile_name`` (from ``NUNMAI_HOME``); ``None`` if neither."""
