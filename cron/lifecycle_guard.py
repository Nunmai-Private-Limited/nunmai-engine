...
A cron job that restarts/stops the gateway from inside the gateway (``nunmai gateway restart``,
``launchctl kickstart ai.nunmai.gateway``, ``systemctl restart nunmai-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `nunmai gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `nunmai` from being a path component or word tail (`/docs/nunmai gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    # Windows spells the CLI with a launcher suffix (`nunmai.exe`, npm-style `nunmai.cmd`/`.ps1`);
    # same command, so the suffix is optional here.
    r"(?:(?<![/\w.\-])nunmai(?:\.(?:exe|cmd|bat|com|ps1))?\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a nunmai-gateway label so unrelated nunmai services stay
    ...
    # -l ai.nunmai.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
...
# gateway is literally `python.exe` / `python3.12` (`python -m nunmai_cli.main gateway run`), so
# `taskkill /F /IM python.exe`, `pkill -9 python3` or `killall python` carry no nunmai/gateway token
...
# (`python -m nunmai_cli.main gateway run` / `nunmai gateway run`) only through its own tokens;
# an unrelated script that merely contains "nunmai" (`nunmai-polis/run.sh`, `my_nunmai_bot.py`)
# cannot match it. Same nunmai+gateway pairing as Branch D, plus the module path.
_GATEWAY_CMDLINE_TOKEN_RE = re.compile(r"(?i)nunmai_cli|\bnunmai\b[^\n]*\bgateway\b|\bgateway\b[^\n]*\bnunmai\b")
...
HOST_INTERPRETER_KILL_REJECTION = (
    ...
    "for a background job Nunmai started, or kill/taskkill by its explicit PID."
)
...
def _pattern_reaches_host_interpreter(pattern: str, *, full_cmdline: bool, exact: bool) -> bool:
    ...
    command line). `python -m nunmai_cli.main …` is the gateway's own cmdline, so a `-f` pattern
    that names the interpreter and then only wildcards or a `nunmai` token reaches it, while
    ...
    return not rest.strip() or bool(_ERE_WILDCARD_ONLY.match(rest)) or "nunmai" in rest.lower()
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.nunmai.gateway-... \` / `  -- ...`,
...
# Branch A2: `nunmai -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `nunmai -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the nunmai-gateway label anchor
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard (``nunmai_cli.profiles.current_profile_name``); ``None`` if none."""
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``nunmai``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: nunmai gateway
    ...
    # Profile-flag form (#78028): `nunmai -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `nunmai` and `gateway`. It is only the same foot-gun when the named profile IS
    ...
    # Branch E (#113667): killers aimed at the interpreter image itself carry no nunmai/gateway token.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<NUNMAI_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    try:
        ...
        # get_nunmai_home() falls back to Path.home(), which raises when neither NUNMAI_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if refusal:
        ...
        # The direct command regex below still scans the full text, so a literal `nunmai gateway restart`
