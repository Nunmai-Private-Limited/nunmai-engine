...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `nunmai setup`) for that home, or pin a "
...
def _credential_store_scope_label() -> str:
    """``[profile '<name>', NUNMAI_HOME <path>]`` for the home this preflight read credentials from.

    The verdict must name the store it judged: a scheduler process whose home differs from the
    shell where "the same credential works" (Docker HOME vs NUNMAI_HOME, a multiplexed satellite
    ...
    return f"[profile '{get_active_profile_name() or 'default'}', NUNMAI_HOME {get_nunmai_home()}]"
...
def _primary_profile_routes_for_current_home() -> list:
    ...
    try:
        ...
        primary_home = get_default_nunmai_root()
        current_home = _sched.Path(get_nunmai_home())
        if (
