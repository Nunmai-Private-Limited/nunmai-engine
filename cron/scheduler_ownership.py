...
``is_gateway_runtime_lock_active`` goes through ``get_process_nunmai_home()``, which ignores the
...
def register_ticked_homes(homes) -> None:
    ...
    for home in homes:
        path = Path(home)
        resolved[nunmai_home_key(path)] = path
    with _ticked_lock:
...
def serves_profile(home: Optional[Union[Path, str]] = None) -> bool:
    ...
    key = nunmai_home_key(home if home is not None else get_nunmai_home())
    with _ticked_lock:
...
def record_serves_profile(record: Any, home: Optional[Union[Path, str]] = None) -> bool:
    ...
    from gateway.status import (
        _get_process_nunmai_home, _profile_label_for_home, _same_nunmai_home)

    try:
        target = Path(home) if home is not None else get_nunmai_home()
        record_home = record.get("nunmai_home")
        if not isinstance(record_home, str) or not record_home.strip():
            record_home = _get_process_nunmai_home()
        if _same_nunmai_home(record_home, target):
            return True
    except Exception:
        return False
