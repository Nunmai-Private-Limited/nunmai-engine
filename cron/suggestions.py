...
A *suggestion* is a ready-to-run cron job spec that Nunmai surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.nunmai/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_nunmai_home() (profile home), not the
# shared default root. See cron/jobs.py for the full rationale.
CRON_DIR = get_nunmai_home().resolve() / "cron"
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _nunmai_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
