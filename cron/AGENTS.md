...
schedule via the `cronjob` tool; users via `nunmai cron list|add|edit|pause|resume|run|remove` or
...
- **Inactivity watchdog** on cron agent sessions (`_cron_inactivity_seconds()`): default 600s idle,
  `NUNMAI_CRON_TIMEOUT` overrides, `0` = unlimited. It is idle time, not wall-clock — a stalled
...
- Per-home tick lock `<home>/cron/.tick.lock` prevents duplicate ticks across processes for
  that profile's store; never a `~/.nunmai/...` literal.
...
- **A claim is released under the key it was registered with.** The cron scope is a ContextVar:
  ...
  sees phantom work. Never rebuild a home from a key half (`Path(key[0])`): `nunmai_home_key`
...
- **The host gateway stands down for a profile that runs its OWN gateway.** `run.py::
  _cron_profile_gate` (the same gate `nunmai_cli/web_server.py` passes) keeps the launch process
...
- The cron ticker runs in the desktop-spawned backend when `NUNMAI_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `nunmai kanban
...
- **CLI:** `nunmai_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`nunmai tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/nunmai-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `NUNMAI_KANBAN_BOARD` pinned in their env and
...
- **Notifications leave through the task's owning profile.** `nunmai_cli/kanban_db_notify.py`
  ...
  Dispatched workers get `NUNMAI_KANBAN_BOARD` and the assignee's `NUNMAI_HOME` pinned in a
...
- **Prompt injection sites gate on ownership, not tool access.** Tool access (`kanban_show` visible
  via a profile's toolset) and an inherited `NUNMAI_KANBAN_TASK` (delegate children, cron runs beside
...
- **Descendant fence is a path, not a flag.** A delegated child's Kanban marker
  ...
  `kanban_path_is_fenced(path)` denies mutations only on the dispatcher-pinned `NUNMAI_KANBAN_DB`
  or under that root, so a child working against a scratch `NUNMAI_HOME` keeps a writable board.

## Tests

`tests/cron/`, `tests/nunmai_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
