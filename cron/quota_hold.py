...
``AuthError`` from ``nunmai_cli.auth_codex._codex_quota_exhausted_error``). When the whole
...
def hold_active(job: Dict[str, Any], now: Optional[datetime] = None) -> bool:
    ...
    return until is not None and _instant_after(until, now or _nunmai_now())
...
def _window_end(hold_seconds: float) -> datetime:
    ...
    return _seconds_after(_nunmai_now(), float(hold_seconds) + HOLD_SLACK_SECONDS)
