...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Nunmai — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's NUNMAI_HOME must not shadow it.
        env.pop("NUNMAI_HOME", None)
    else:
        # Multiplex workers carry the profile in a ContextVar, not os.environ.
        env["NUNMAI_HOME"] = str(source_home)
