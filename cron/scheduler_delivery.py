...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Nunmai — {_cron_display_name(job)}"
    try:
...
def _run_bot_chat_turn(argv: list, env: dict, report_path: str, timeout: float) -> subprocess.CompletedProcess:
    """Run one ``nunmai chat -Q`` delivery child; the cap bounds the TURN, not the process (#113608).
    ...
    """
    from nunmai_cli.quiet_single_query import run_reported_turn
    ...
    return run_reported_turn(argv, env=env, report_path=report_path, timeout=timeout,
                             cwd=env.get("NUNMAI_HOME") or None)
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str, *, deferred: Optional[dict] = None,
    ...
    try:
        source_home = get_nunmai_home().resolve()
    ...
    # The running install first (same trust order as gateway.run._resolve_nunmai_bin): the
    # scheduler lives in the long-running gateway, so a PATH-first lookup would hand delivery
    # to whatever `nunmai` PATH names — another install, or a planted one — instead of this one.
    try:
        import importlib.util as _ilu
        found = _ilu.find_spec("nunmai_cli") is not None
    except Exception:
        found = False
    if found:
        ...
        if not nunmai_bin:
            return ("Nunmai could not deliver this result to Bot Chat: the `nunmai` command was not found. "
                    "The result is saved; run `nunmai cron runs` to see it, or `nunmai doctor` if this keeps happening")
        argv = [nunmai_bin]
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="nunmai-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
        ...
        from nunmai_cli.quiet_single_query import TURN_REPORT_FILE_ENV
        ...
        if result.returncode != 0:
            ...
            return (
                f"Nunmai could not deliver this result to Bot Chat (profile '{profile_label}'). "
                "The result is saved; run `nunmai cron runs` to see it, or `nunmai doctor` if this keeps happening"
                f". Details: {tail}")
        ...
        if not (deferred or {}).get("degraded"):
            marker = (
                ...
                f"complete saved output with `nunmai cron runs` (job '{job_id}'). "
        ...
        hint = (
            ...
            f"session frees; full output stays saved, run `nunmai cron runs` for job '{job_id}'"
            if marker_queued else
            "the result is saved; run `nunmai cron runs` to see it, or `nunmai doctor` "
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`nunmai cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_NUNMAI_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `nunmai cron list` shows it.
    ...
    # at boot; standalone runs (`nunmai cron run`) did not, silently dropping files. Idempotent.
