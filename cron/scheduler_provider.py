...
def _guarded_store_write(action, description, *args, **kwargs):
    ...
    markers are diagnostics for ``nunmai cron status`` — losing one write to a broken store
...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``nunmai profile list`` (#47368). Filtering on directory existence leaves a deleted
...
def _profile_cron_scope(home):
    ...
    # unsuccessful (#32612).
    home_token = set_nunmai_home_override(str(home))
    try:
...
def fire_overdue_jobs(
    ...
    """
    # `nunmai pause` ESTOP: skip the sweep entirely. No state to unwind — the
    # next housekeeping pass after `nunmai resume` catches overdue work up
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _elapsed_seconds, _ensure_aware, _nunmai_now,
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global NUNMAI_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `nunmai cron status` reflects liveness for every profile independently.
        ...
        register_ticked_homes([get_process_nunmai_home()])
        ...
        try:
            ...
            # Heartbeat before the first sleep so `nunmai cron status` sees a live ticker
            # immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `nunmai cron status` (separate process) shows WHY.
                _guarded_store_write(
