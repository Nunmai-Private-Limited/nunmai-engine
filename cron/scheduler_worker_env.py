...
``cron.scheduler``, not ``nunmai_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips Nunmai-owned PYTHONPATH entries because user
children must not see our tree. This child IS Nunmai, so the pin is applied *after* the
...
def pin_nunmai_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
