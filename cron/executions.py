...
def _connect() -> sqlite3.Connection:
    ...
    # ``nunmai_cli.sqlite_util`` / ``cron.jobs`` cached, so new names must be resolved at call time,
    ...
    path = EXECUTIONS_FILE or (get_nunmai_home().resolve() / "cron" / "executions.db")
    _ensure_cron_dir(path.parent)
    return open_db(path, db_label="cron/executions.db", synchronous_full=True, initialize=_initialize_schema)


def _initialize_schema(conn: sqlite3.Connection) -> None:
    from nunmai_cli.sqlite_util import add_column_if_missing
...
def _live_owner_stale_after_seconds() -> Optional[float]:
    ...
    ``max(3 × NUNMAI_CRON_TIMEOUT, cron script timeout, 7200)``. Returns ``None`` (never reclaim
...
def _claim_age_seconds(claimed_at: str) -> float:
    """Seconds since ``claimed_at`` (NOT NULL, always the aware ISO string from nunmai_time.now)."""
    return (_nunmai_now() - datetime.fromisoformat(claimed_at)).total_seconds()
