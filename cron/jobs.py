"""Cron job storage: ~/.nunmai/cron/jobs.json; output in
~/.nunmai/cron/output/{job_id}/{timestamp}.md"""
...
from nunmai_time import get_timezone
...
# Cron is per-profile by design: anchor at get_nunmai_home() (active profile home), NOT
# get_default_nunmai_root() — the shared root would funnel every profile's jobs into one jobs.json
# and run them under the ticker's NUNMAI_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own NUNMAI_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same NUNMAI_HOME — so a job authored in profile `coder` lives in
# `~/.nunmai/profiles/coder/cron/jobs.json` and executes with `coder`'s `.env`, `config.yaml`, and skills.
# Do NOT change this to the default root: that re-breaks per-profile isolation. See also the dynamic
# `_get_nunmai_home()` / `_get_lock_paths()` resolution in cron/scheduler.py. See #4707.
...
# Heartbeat: touched every ticker loop so `nunmai cron status` can tell the ticker THREAD is alive,
# not just the gateway PROCESS; success = last tick that completed WITHOUT raising.
# The gateway process and the (separate) ``nunmai cron status`` process share it so status can tell whether
...
# threshold in `nunmai cron status` (nunmai_cli/cron.py), so they never drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_nunmai_home(), so re-pointing NUNMAI_HOME after import uses ITS OWN store rather than the
...
# Fallback stale-recovery window for a one-shot's running-claim when NUNMAI_CRON_TIMEOUT=0
...
def _oneshot_run_claim_ttl_seconds() -> float:
    """One-shot running-claim TTL from ``NUNMAI_CRON_TIMEOUT``: unset/invalid → 600s → 1800s;
...
def _secure_dir(path: Path):
    ...
    container/NUNMAI_HOME_MODE rules as the rest of NUNMAI_HOME (#10757)."""
    from nunmai_cli.config import _secure_dir as _shared_secure_dir
    _shared_secure_dir(path)


def _secure_file(path: Path):
    """Owner-only (0600) via the shared helper (managed/container skip included)."""
    from nunmai_cli.config import _secure_file as _shared_secure_file
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<nunmai_home>/profiles/<name>/`` (default/custom homes are not).
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # Naive timestamps become aware in the CONFIGURED Nunmai timezone (not server-local):
            # the due-check compares against nunmai_time.now().
            ...
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=_nunmai_now().tzinfo)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _ensure_aware(dt: datetime) -> datetime:
    """Aware datetime in the configured Nunmai timezone. Legacy naive values are read as
...
def _record_persisted_error_recovery(job: Dict[str, Any], previous_next_run: str) -> None:
    ...
    entry = {
        ...
        "rearmed_at": _nunmai_now().isoformat(),
    }
...
def _record_timezone_migration_catchup(
    ...
    entry = {
        ...
        "fired_at": _nunmai_now().isoformat(),
    }
    ...
    if kind == "cron":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall nunmai-engine or run 'pip install croniter' in your runtime env.",
                expr)
            return None
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    tick* marker. We track two distinct signals so `nunmai cron status` can tell a thread that is merely
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    critical under multiplex_profiles where ``nunmai cron status`` must report per-profile liveness
    (#69377).
    """
...
def resume_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    if (
        ...
        and _instant_at_or_before(stored_dt, _nunmai_now())
    ):
...
def _claim_owner_is_dead(claim: Dict[str, Any]) -> bool:
    ...
    ``_machine_id()`` stamps ``host:pid[:token]``; a foreign host, an explicit NUNMAI_MACHINE_ID,
...
def _claim_is_live(claim: Any, now: datetime, ttl_seconds: float) -> bool:
    ...
    after the TTL (a killed ``nunmai cron run`` otherwise blocks the next manual run for the
...
def note_fire_forward_failure(job_id: str, detail: str) -> bool:
    ...
    def apply(jobs, _i, job):
        job["last_fire_error"] = {
            "at": _nunmai_now().isoformat(), "detail": str(detail or "")[:500]}
        save_jobs(jobs)
        return True
...
def _machine_id() -> str:
    ...
    fresh-claim check): ``NUNMAI_MACHINE_ID`` if set, else hostname:pid."""
...
def _oneshot_dispatch_limit_reached(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if job.get("last_run_at") is not None:
        ...
        logger.warning(
            ...
            "'nunmai cron resume <job> --run-now' (#93524).",
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same NUNMAI_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    raw_jobs = load_jobs()
    scan = _DueScan(raw_jobs, _nunmai_now())
...
# the disk.
# Unlike the quick-snapshot store (`nunmai_cli.backup`, capped at 20) it had no retention, so a
...
def save_job_output(job_id: str, output: str):
    ...
    output_file = job_output_dir / f"{_nunmai_now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
