...
def _profile_user_agent() -> str:
    """Return a ``nunmai-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from nunmai_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"nunmai-cli/{_ver}"
    except Exception:
        return "nunmai-cli"
...
class ProviderProfile:
    ...
    # ``auth_handler(action, args) -> bool``: ``nunmai auth add|status|logout|refresh <name>`` calls it
    ...
    # Both own their own token endpoints; Nunmai passes no secrets beyond the pooled row itself.
    ...
    # describe how to launch it; nunmai_cli/auth.py's
    ...
    def default_reasoning_config(self, model: str | None = None) -> dict | None:
        ...
        where each profile already decides (Nunmai/OpenRouter fill medium; Anthropic omits). A profile
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.nunmai/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # nunmai-cli UA so the catalog endpoint is reachable.
