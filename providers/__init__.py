...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    if source == "bundled":
        ...
        module_name = f"_nunmai_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$NUNMAI_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``nunmai plugins install`` at
          ``$NUNMAI_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 2. User plugins — under $NUNMAI_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``nunmai plugins install`` / the plugin index.
    #     Those clone into $NUNMAI_HOME/plugins/<name>/ — flat, NOT under
