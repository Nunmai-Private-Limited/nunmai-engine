...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with nunmai-engine)
2. User plugins: ``$NUNMAI_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``nunmai_agent.plugins``
...
process; the ``$NUNMAI_HOME`` plugins of the profile home bound at lookup time
...
# ``$NUNMAI_HOME`` plugin dir) / "runtime" (entry point, legacy module, direct call).
...
@dataclass
class _HomeLayer:
    """The ``$NUNMAI_HOME/plugins`` providers of ONE profile home.
    ...
    ``stamps`` are the plugin dirs' mtimes: a directory added by ``nunmai plugins install`` while the
...
# The layer a ``$NUNMAI_HOME`` plugin import registers into. A ContextVar, not a module global:
# two turn threads scanning two profile homes at once must not cross-register. Never a lock held
# across the import itself — a thread mid-``import nunmai_cli.auth`` (whose import calls
...
def _sync_auth_registry() -> None:
    """Mirror profiles into the ``nunmai_cli`` snapshots (auth registry, picker catalog) that are loaded.

    ``nunmai_cli.auth`` takes its own snapshot of ``list_providers()`` when it is imported. If a
    ...
    ``sys.modules`` on purpose: this layer must never import ``nunmai_cli`` (that would run auth's
    top-level code mid-scan and risk a circular import). Never raises: registration must not fail
    because of the auth mirror.
    """
    for module, attr in (
        ("nunmai_cli.auth", "sync_plugin_provider_registry"),
        ("nunmai_cli.models_catalog_static", "sync_plugin_provider_catalog"),
    ):
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$NUNMAI_HOME/plugins/model-providers/`` can override
    bundled profiles without editing repo code. A registration made while a
    ``$NUNMAI_HOME`` plugin is being imported lands in that home's layer.
    """
...
def provider_source(name: str) -> str | None:
    ...
    ``"user"`` is what lets a ``$NUNMAI_HOME`` plugin re-registering a bundled name win in
    ``nunmai_cli.auth.PROVIDER_REGISTRY`` too — a bundled profile never rewrites a built-in row.
    """
    layer = _home_layer()
...
def routed_model_rejects_vision_tool_messages(provider: str, model: str) -> bool:
    ...
    from nunmai_cli.providers import is_routing_aggregator
...
def list_providers() -> list[ProviderProfile]:
    ...
    ``$NUNMAI_HOME`` plugins shadow process-wide profiles of the same name."""
    global _PROVIDER_LIST_CACHE
    if not _discovered:
        _discover_providers()
    layer = _home_layer()
...
def _user_plugins_dir() -> Path | None:
    """Return ``$NUNMAI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from nunmai_constants import get_nunmai_home

        d = get_nunmai_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$NUNMAI_HOME/plugins/`` if it exists.

    This is where ``nunmai plugins install`` clones a plugin — flat, one
...
def _scan_home_layer(layer: _HomeLayer, key: str) -> None:
    ...
    ``$NUNMAI_HOME/plugins/model-providers/<name>/`` first, then plugins cloned flat by
    ``nunmai plugins install`` into ``$NUNMAI_HOME/plugins/<name>/`` that declare
...
def _user_module_name(plugin_dir: Path, home_key: str) -> str:
    ...
    return f"_nunmai_user_provider_{digest}_{plugin_dir.name.replace('-', '_')}"
...
def _import_plugin_dir(plugin_dir: Path, source: str, *, home_key: str = "") -> None:
    ...
    # multiple NUNMAI_HOME profiles don't alias each other.
    if source == "bundled":
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``nunmai_agent.plugins``
    ...
    * **Provider targets only.** The ``nunmai_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$NUNMAI_HOME``) keep their
    ...
    try:
        from nunmai_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    module-level. Later steps win on name collision. ``$NUNMAI_HOME`` plugins are
    ...
    try:
        ...
        _discovering = False
        # nunmai_cli.auth may have been imported by a plugin during discovery and snapshotted a
...
def _run_discovery_steps() -> None:
    ...
    # 0. Pip-installed plugins — entry points in the ``nunmai_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$NUNMAI_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with nunmai-engine.
