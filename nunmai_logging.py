...
``~/.nunmai/logs/`` (profile-aware via ``get_nunmai_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nunmai_cli.web_server``,
    ``nunmai_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Nunmai (TUI, gateway, ``hy_memory`` server, MCP
...
from nunmai_constants import get_config_path, get_nunmai_home
...
# Used by _ComponentFilter and exposed for ``nunmai logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Nunmai logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nunmai_home
        Override for the Nunmai home directory.  Falls back to
        ``get_nunmai_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nunmai_verbose", False):
                return
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nunmai group. However, both ``_open()`` (initial creation)
...
# the module header): when several Nunmai processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="nunmai-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse nunmai_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when nunmai_cli.config isn't importable (bare
        # nunmai_logging consumers).
        try:
            from nunmai_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
