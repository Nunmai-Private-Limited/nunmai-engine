"""Centralized logging setup for Nunmai Engine.
...
# in Nunmai (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from nunmai_constants import get_config_path, get_nunmai_home, mkdir_under_nunmai_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nunmai_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.nunmai_home = str(get_nunmai_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.nunmai_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._nunmai_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``nunmai logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "nunmai_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("nunmai_cli", "cli"),
    "cron": ("cron",),
    "gui": ("nunmai_cli.web_server", "nunmai_cli.pty_bridge", "nunmai_cli.desktop", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    ...
    """Configure the Nunmai logging subsystem; returns the ``logs/`` directory.
    ...
    home = nunmai_home or get_nunmai_home()
    log_dir = mkdir_under_nunmai_home(home / "logs")
    # A second Nunmai home in a process that already logs for another one — a dashboard or
    # ``nunmai serve`` backend building agents for several profiles, a multiplexed gateway —
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_nunmai_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nunmai_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from nunmai_cli.config import is_managed
        ...
        if _is_unavailable_log_stream(exc):
            ...
            if not self._unavailable_reported:
                ...
                _quietly(lambda: print(
                    f"nunmai_logging: {self.baseFilename} unavailable ({exc}); "
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Nunmai home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._nunmai_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "nunmai_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._nunmai_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="nunmai-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_nunmai_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_nunmai_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses nunmai_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare nunmai_logging consumers.
        try:
