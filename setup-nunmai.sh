...
# 5. Symlinks the 'nunmai' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ Nunmai Engine Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "${TMPDIR:-${NUNMAI_HOME:-$HOME/.nunmai}}/nunmai-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "${TMPDIR:-${NUNMAI_HOME:-$HOME/.nunmai}}/nunmai-uv-installer.$$.sh")"
...
if is_termux; then
    # Nunmai currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Nunmai requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink nunmai into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nunmai command..."

NUNMAI_BIN="$SCRIPT_DIR/venv/bin/nunmai"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NUNMAI_BIN" "$COMMAND_LINK_DIR/nunmai"
echo -e "${GREEN}✓${NC} Symlinked nunmai → $COMMAND_LINK_DISPLAY_DIR/nunmai"
...
# ============================================================================
# Seed bundled skills into ~/.nunmai/skills/
# ============================================================================

NUNMAI_SKILLS_DIR="${NUNMAI_HOME:-$HOME/.nunmai}/skills"
mkdir -p "$NUNMAI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nunmai/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NUNMAI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nunmai status        # Check configuration"
if is_termux; then
    echo "  nunmai gateway       # Run gateway in foreground"
else
    echo "  nunmai gateway install # Install gateway service (messaging + cron)"
fi
echo "  nunmai cron list     # View scheduled jobs"
echo "  nunmai doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nunmai_cli.main setup
fi
