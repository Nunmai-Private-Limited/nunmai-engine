...
``nunmai_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import nunmai_state (cycle) — shared
module-level constants live in nunmai_state_common.
"""
...
# Moved methods logged under the "nunmai_state" logger before the split;
...
def schema_read_probe_statements() -> tuple:
    ...
    ``nunmai_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `nunmai update` until the user's
...
class SessionSchemaMixin:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`nunmai sessions optimize-storage` with the gateway stopped. "
                        "`nunmai doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from nunmai_constants import get_nunmai_home
            cache_path = get_nunmai_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `nunmai sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `nunmai db optimize`) must keep its EXISTING
