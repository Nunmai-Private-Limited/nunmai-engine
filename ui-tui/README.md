...
React + Ink terminal UI for Nunmai. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NUNMAI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `nunmai setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Nunmai remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.nunmai/.nunmai_history` or under `NUNMAI_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Nunmai remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Nunmai credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `nunmai setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/nunmai-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external nunmai setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Nunmai remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.nunmai/tui-parent.log
    ...
    types/
      nunmai-ink.d.ts            type declarations for @nunmai/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NunmaiCLI subprocess for slash commands
```
