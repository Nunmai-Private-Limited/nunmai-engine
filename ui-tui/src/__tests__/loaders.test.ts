...
import { renderToScreen } from '../../packages/nunmai-ink/src/ink/render-to-screen.js'
import { cellAtIndex } from '../../packages/nunmai-ink/src/ink/screen.js'
  ...
  it('accepts a bare row COUNT and derives widths — the generated-code shape', async () => {
    ...
    const { screen, height } = renderToScreen(
      createElement(ShimmerRows, {
        rows: 3,
        width: 20,
        t: { color: { completionBg: '#052819', label: '#D6B36C', muted: '#2E9C7C' } }
      }),
      30
    )
