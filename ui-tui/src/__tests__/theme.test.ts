...
// NUNMAI_TUI_THEME=light (or NUNMAI_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Nunmai Engine')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#6BE7C8')
...
describe('LIGHT_THEME', () => {
  it('avoids bright-yellow accents unreadable on white backgrounds (#11300)', async () => {
    ...
    expect(LIGHT_THEME.color.primary).not.toBe('#6BE7C8')
    expect(LIGHT_THEME.color.accent).not.toBe('#3FCFA9')
    expect(LIGHT_THEME.color.muted).not.toBe('#2E9C7C')
    expect(LIGHT_THEME.color.statusWarn).not.toBe('#6BE7C8')
  })
...
describe('detectLightMode', () => {
  ...
  it('honors NUNMAI_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NUNMAI_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ NUNMAI_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ NUNMAI_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ NUNMAI_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ NUNMAI_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets NUNMAI_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', NUNMAI_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors NUNMAI_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NUNMAI_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ NUNMAI_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', NUNMAI_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', NUNMAI_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses NUNMAI_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ NUNMAI_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // NUNMAI_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })
  ...
  it('normalizes non-banner foregrounds on light Apple Terminal', async () => {
    ...
    expect(theme.color.primary).toBe('#6BE7C8')
    expect(theme.color.accent).toBe('#3FCFA9')
    expect(theme.color.border).toBe('#0B6B4F')
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })
  ...
  it('keeps truecolor light Apple Terminal in truecolor (adapting, not ansi256-bucketing)', async () => {
    ...
    const theme = fromSkin({ banner_text: '#EFE9D7' }, {})
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    const light = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#CC9B1F', 'dark muted'],
      [dark.DARK_THEME.color.label, '#D6B36C', 'dark label'],
      [dark.DARK_THEME.color.statusFg, '#C0C0C0', 'dark statusFg'],
      [dark.DARK_THEME.color.completionBg, '#052819', 'dark surface'],
      [dark.DARK_THEME.color.completionCurrentBg, '#1F4A3D', 'dark chip'],
      [dark.DARK_THEME.color.selectionBg, '#1B3A32', 'dark selection'],
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })
  ...
  it('rescues near-invisible colors with a hue-preserving multiplicative lift', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })
    // The default dark cream (#EFE9D7, 1.08:1 on white) is genuinely invisible.
    const { color } = fromSkin({ banner_text: '#EFE9D7' }, {})

    expect(color.text.toLowerCase()).not.toBe('#EFE9D7')
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ NUNMAI_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late NUNMAI_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ NUNMAI_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
