import type { InputEvent, Key } from '@nunmai/ink'
import * as Ink from '@nunmai/ink'
 ...
 * but that accepts `key.meta` on macOS — and nunmai-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.NUNMAI_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which nunmai-ink decodes as meta+'d'; without this branch it
