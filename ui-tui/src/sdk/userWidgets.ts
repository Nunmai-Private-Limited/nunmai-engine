...
import { Box, Text } from '@nunmai/ink'
...
/**
 * User widget apps — Nunmai authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$NUNMAI_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.nunmai/plugins/`: files under NUNMAI_HOME execute
...
const widgetsDir = () => join(process.env.NUNMAI_HOME?.trim() || join(homedir(), '.nunmai'), 'tui-widgets')
 ...
 *  every change, so a widget Nunmai writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Nunmai) ever writes must hot-load too — a 10s poll here read
