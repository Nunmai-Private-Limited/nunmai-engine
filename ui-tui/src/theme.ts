import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@nunmai/shared/color'
import type { SkinBranding, SkinColors } from '@nunmai/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @nunmai/shared/color (the primitives,
...
// Mirrors @nunmai/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @nunmai/ink is bundled separately from packages/.
...
// The classic light-mode Nunmai look was never hand-authored: for years the
 ...
 *  answer when known (cached in NUNMAI_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.NUNMAI_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit NUNMAI_TUI_THEME / COLORFGBG signals above still win,
...
// `NUNMAI_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `NUNMAI_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `NUNMAI_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `NUNMAI_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Nunmai palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.NUNMAI_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.NUNMAI_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.NUNMAI_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into NUNMAI_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into NUNMAI_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Nunmai navy
  ...
  // the derived value already carries the identity seeds (e.g. Nunmai navy).
  const activeRow =
