import type { RunExternalProcess } from '@nunmai/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nunmai ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nunmai: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nunmai ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
