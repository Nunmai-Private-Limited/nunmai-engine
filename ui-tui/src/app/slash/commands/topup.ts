import { driveChargeSettlement, type SettlementOutcome } from '@nunmai/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Nunmai Engine page; this is NOT a per-terminal stop.
      sys(
        "Remote spending is off for this account — a billing admin can turn it on from the portal's Nunmai Engine page."
      )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into Nunmai Portal — run /portal to log in, then /topup.')
