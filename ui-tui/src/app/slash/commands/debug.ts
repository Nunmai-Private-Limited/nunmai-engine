...
import { terminalBackgroundHex } from '@nunmai/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $NUNMAI_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see NUNMAI_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['NUNMAI_TUI_BACKGROUND', process.env.NUNMAI_TUI_BACKGROUND ?? '(unset)'],
            ['NUNMAI_TUI_THEME', process.env.NUNMAI_TUI_THEME ?? '(unset)'],
