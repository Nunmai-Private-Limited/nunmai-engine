...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'session usage + Nunmai credits',
    name: 'usage',
    run: (_arg, ctx) => {
      ctx.gateway.rpc<SessionUsageResponse>('session.usage', { session_id: ctx.sid }).then(r => {
        ...
        // Nunmai balance block is agent-independent (a portal fetch), so it shows
        ...
        if (usageModel?.available && (barLines.length || usageModel.status === 'free')) {
          ...
          if (creditsLines.length) {
            ctx.transcript.panel('Nunmai balance', [{ text: creditsLines.join('\n') }])
            showedBalance = true
          }
        }
