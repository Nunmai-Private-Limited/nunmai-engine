 ...
 * `nunmai-boot-background` / `nunmai-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports NUNMAI_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.nunmai matches get_nunmai_home()'s default.
const bootFilePath = () => join(process.env.NUNMAI_HOME ?? join(homedir(), '.nunmai'), 'tui-theme-boot.json')

// Never touch the user's real ~/.nunmai from test runs (the TS suite has no
// NUNMAI_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into NUNMAI_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.NUNMAI_TUI_THEME || env.NUNMAI_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.NUNMAI_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.NUNMAI_TUI_BACKGROUND
  ) {
    env.NUNMAI_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.NUNMAI_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.NUNMAI_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into NUNMAI_TUI_THEME.
