...
const logDir = join(process.env.NUNMAI_HOME?.trim() || join(homedir(), '.nunmai'), 'logs')
...
// a real ~/.nunmai (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('nunmai-tui: parent lifecycle log unavailable\n')
    }
  }
}
