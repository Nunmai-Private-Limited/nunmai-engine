...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NUNMAI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Nunmai on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NUNMAI_TUI_TRUECOLOR=1.
