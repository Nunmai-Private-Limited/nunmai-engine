...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def _project_compression_tips(self, sessions: List[Dict[str, Any]], compact_rows: bool) -> List[Dict[str, Any]]:
        ...
        for s in sessions:
            ...
            if merged.get("title") is None:
                ...
                # between leaves it on the ended root, and exact-title lookups (`nunmai peer dm` ->
    ...
    def search_sessions(
        ...
        so ``nunmai -c``/``--resume`` picks its last session. ``source`` may be one label or several."""
        where_clauses = []
        params: list = []
        if source:
