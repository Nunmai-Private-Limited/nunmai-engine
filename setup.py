...
Nunmai Engine (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``NUNMAI_NIX_BUILD=1`` on the
Nunmai package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("NUNMAI_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for nunmai-engine is not supported.\n"
    "Nunmai is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://nunmai.in/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Nunmai Nix derivation sets NUNMAI_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``nunmai_state``, ``toolsets``...)
