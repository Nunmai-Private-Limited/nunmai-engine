...
#
# `services.nunmai-engine` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  nunmaiHome, programs.nunmai-engine (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "nunmai-config-attrs";
    description = "Nunmai YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $NUNMAI_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Nunmai Engine";
      ...
      package = mkOption {
        ...
        description = "The nunmai-engine package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Nunmai configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `nunmai config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $NUNMAI_HOME/.env. Nunmai reads that file at each start, with
          load_nunmai_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."nunmai/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Nunmai refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Nunmai reads SOUL.md and memories/ from NUNMAI_HOME, so put those
          files in `nunmaiHomeFiles`.
      ...
      nunmaiHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into NUNMAI_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Nunmai reads SOUL.md and the memory files from NUNMAI_HOME and not
          from the working directory. Declare those files here, or Nunmai
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nunmai plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Nunmai discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nunmai_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nunmai (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nunmai-engine's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nunmai gateway`.";
      };
      ...
      # ── The backend: `nunmai serve` or `nunmai dashboard` ──────────────
      # `nunmai serve` and `nunmai dashboard` are the same entry point,
      # nunmai_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Nunmai Desktop
      ...
      # controls an external gateway, with `nunmai gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Nunmai
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            NUNMAI_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Nunmai Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."nunmai/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `nunmai` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.nunmai-engine.installPackage was removed. Nunmai now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.nunmai-engine.enable = ${lib.boolToString (value != false)};  # the nunmai CLI, and NUNMAI_HOME for your shells
        programs.nunmai-engine.desktop.enable = true;  # the desktop application

      `services.nunmai-engine` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "nunmai-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "nunmai-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "nunmai-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "nunmai-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "nunmai-engine: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # NUNMAI_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Nunmai make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${nunmaiHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Nunmai
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${nunmaiHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${nunmaiHome}/config.yaml
            ${run}chmod ${modes.config} ${nunmaiHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "nunmai-managed" managedSystem} ${nunmaiHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${nunmaiHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${nunmaiHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${nunmaiHome}/auth.json"
        else
          ''
            if [ ! -e ${nunmaiHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${nunmaiHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree nunmaiHome cfg.nunmaiHomeFiles}
      ...
      ${run}find ${nunmaiHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "nunmai-engine: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${nunmaiHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/nunmai"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps nunmai as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "nunmai-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "nunmai-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "nunmai-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          NUNMAI_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export NUNMAI_DASHBOARD_SESSION_TOKEN

          if [ -z "$NUNMAI_DASHBOARD_SESSION_TOKEN" ]; then
            echo "nunmai-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "nunmai-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "nunmai-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "nunmai-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "nunmai-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "nunmai-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "nunmai-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Nunmai Engine web dashboard and desktop backend"
    else
      "Nunmai Engine backend for Nunmai Desktop";

  # The environment that each Nunmai process needs, from either module.
  #
  # managedSystem gives the value of NUNMAI_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Nunmai an identity and a memory, use
          ${optionPath}.nunmaiHomeFiles instead. Those files go to
          NUNMAI_HOME. Nunmai reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of NUNMAI_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
