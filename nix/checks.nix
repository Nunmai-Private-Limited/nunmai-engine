...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nunmai-engine = self'.packages.default;
      nunmaiVenv = nunmai-engine.nunmaiVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "nunmai-check";
                homeDirectory = "/home/nunmai-check";
                stateVersion = "24.11";
              };
            }
            { services.nunmai-engine = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.nunmai-engine = programs;
              services.nunmai-engine = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.nunmai-engine. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.nunmai-engine);
      ...
      configKeys = pkgs.runCommand "nunmai-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nunmaiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nunmai-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nunmai-build-package" { } ''
          echo "PASS: package built at ${nunmai-engine}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nunmai-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.NUNMAI_TEST = "1";
                environmentFiles = [ "/run/secrets/nunmai-env" ];
                nunmaiHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "nunmai" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "nunmai" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.nunmaiAgentSetup.data;

            failures =
              lib.optional (names != [
                "nunmai-engine"
                "nunmai-backend"
              ]) "expected nunmai-engine + nunmai-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/nunmai gateway" (argvOf "nunmai-engine")
              ) "gateway process does not run `nunmai gateway`: ${argvOf "nunmai-engine"}"
              ++ lib.optional (
                !lib.hasInfix "bin/nunmai serve" (argvOf "nunmai-backend")
              ) "backend process does not run `nunmai serve`: ${argvOf "nunmai-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "nunmai-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/nunmai-check/.nunmai" (envOf n)) names
              ) "gateway and backend must share one NUNMAI_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.NUNMAI_HOME or null != "/home/nunmai-check/.nunmai"
              ) "programs.nunmai-engine.enable must export NUNMAI_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "nunmai-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/nunmai-check/.nunmai/SOUL.md" activation
              ) "nunmaiHomeFiles must install into NUNMAI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads NUNMAI_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "NUNMAI_MANAGED=home-manager" (envOf n)) names
              ) "processes must report NUNMAI_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "nunmai-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "nunmai-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/nunmai-check";
            ...
            cases = [
              ...
              {
                name = "nunmaiHomeFiles needs no directory";
                ok = accepts { nunmaiHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "nunmai-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one NUNMAI_HOME ───────────────
        # `programs.enable` exports NUNMAI_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.nunmai
        # while the services use the NUNMAI_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/nunmai-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                nunmaiHome = "/home/nunmai-check/.nunmai-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "nunmai-desktop") cfg.home.packages;
            ...
            # second Nunmai runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "nunmai-engine") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.nunmai-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.nunmai-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one nunmai-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "NUNMAI_HOME" != "/home/nunmai-check/.nunmai-work"
              ) "the launcher must carry NUNMAI_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "NUNMAI_HOME")})"
              ++ lib.optional (
                setValue "NUNMAI_MANAGED" != "home-manager"
              ) "the launcher must report NUNMAI_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "NUNMAI_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "NUNMAI_DESKTOP_NUNMAI" != "${lib.head agentPackages}/bin/nunmai"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "NUNMAI_DESKTOP_NUNMAI")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "NUNMAI_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "NUNMAI_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "NUNMAI_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "NUNMAI_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "NUNMAI_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "nunmai-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares NUNMAI_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set NUNMAI_HOME" wrapper
              ) "the launcher must carry NUNMAI_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "NUNMAI_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "NUNMAI_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? nunmai-backend || cfg.launchd.agents ? nunmai-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "nunmai-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `nunmai`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.nunmai-engine.enable = true;";
              }
              {
                value = false;
                expect = "programs.nunmai-engine.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "nunmai-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "nunmai-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "nunmai" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."nunmai-engine-setup".text;

            failures =
              lib.optional (names != [
                "nunmai-engine"
                "nunmai-backend"
              ]) "expected nunmai-engine + nunmai-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/nunmai gateway" (execOf "nunmai-engine")
              ) "gateway unit does not run `nunmai gateway`: ${execOf "nunmai-engine"}"
              ++ lib.optional (
                !lib.hasInfix "bin/nunmai dashboard" (execOf "nunmai-backend")
              ) "backend unit does not run `nunmai dashboard`: ${execOf "nunmai-backend"}"
              ++ lib.optional (
                units.nunmai-engine.environment.NUNMAI_HOME != units.nunmai-backend.environment.NUNMAI_HOME
              ) "gateway and backend must share one NUNMAI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/nunmai/.nunmai/SOUL.md" activation
              ) "nunmaiHomeFiles must install into NUNMAI_HOME";
          ...
          in
          pkgs.runCommand "nunmai-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs nunmai, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.nunmai-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/nunmai serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec nunmai directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "nunmai-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "nunmai-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec nunmai, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/nunmai dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "nunmai-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.nunmai-engine;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.nunmai-engine;
          ...
          in
          pkgs.runCommand "nunmai-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $NUNMAI_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                NUNMAI_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "nunmai-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'NUNMAI_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--nunmai-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "nunmai-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "nunmai-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nunmai-engine}/bin/nunmai || (echo "FAIL: nunmai binary missing"; exit 1)
          test -x ${nunmai-engine}/bin/nunmai-engine || (echo "FAIL: nunmai-engine binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nunmai-engine}/bin/nunmai --version 2>&1 | grep -qi "nunmai" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nunmai-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nunmai nunmai-engine nunmai-acp; do
            test -x ${nunmai-engine}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nunmai-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nunmai --help ==="
          ${nunmai-engine}/bin/nunmai --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nunmai-engine}/bin/nunmai --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nunmai-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nunmai-engine}/share/nunmai-engine/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${nunmai-engine}/share/nunmai-engine/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NUNMAI_BUNDLED_SKILLS" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NUNMAI_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${nunmai-engine}/share/nunmai-engine/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${nunmai-engine}/share/nunmai-engine/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "NUNMAI_OPTIONAL_SKILLS" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, NUNMAI_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nunmai-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nunmai-engine}/share/nunmai-engine/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nunmai-engine}/share/nunmai-engine/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NUNMAI_BUNDLED_PLUGINS" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NUNMAI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "nunmai-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${nunmai-engine}/share/nunmai-engine/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${nunmai-engine}/share/nunmai-engine/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${nunmai-engine}/share/nunmai-engine/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "NUNMAI_BUNDLED_LOCALES" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: NUNMAI_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + NUNMAI_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && NUNMAI_BUNDLED_LOCALES=${nunmai-engine}/share/nunmai-engine/locales \
            ${nunmaiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with NUNMAI_BUNDLED_LOCALES set"; exit 1)
        ...
        # NUNMAI_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "nunmai-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${nunmai-engine}/share/nunmai-engine/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${nunmai-engine}/share/nunmai-engine/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "NUNMAI_OPTIONAL_MCPS" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: NUNMAI_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${nunmai-engine}/bin/nunmai mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: nunmai mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "nunmai-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nunmai-engine}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nunmai-engine}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NUNMAI_TUI_DIR" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NUNMAI_TUI_DIR set in wrapper"
        ...
        # Verify NUNMAI_NODE is set in wrapper and points to Node 26+
        # (Nunmai pins its toolchain to Node 26 everywhere)
        nunmai-node = pkgs.runCommand "nunmai-node-version" { } ''
          set -e
          echo "=== Checking NUNMAI_NODE in wrapper ==="
          grep -q "NUNMAI_NODE" ${nunmai-engine}/bin/nunmai || \
            (echo "FAIL: NUNMAI_NODE not set in wrapper"; exit 1)
          echo "PASS: NUNMAI_NODE present in wrapper"

          NUNMAI_NODE=$(sed -n "s/^export NUNMAI_NODE='\(.*\)'/\1/p" ${nunmai-engine}/bin/nunmai)
          test -x "$NUNMAI_NODE" || (echo "FAIL: NUNMAI_NODE=$NUNMAI_NODE not executable"; exit 1)
          echo "PASS: NUNMAI_NODE executable at $NUNMAI_NODE"

          NODE_MAJOR=$("$NUNMAI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Nunmai requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All NUNMAI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NUNMAI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nunmai-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NUNMAI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NUNMAI_MANAGED guards ==="
          check_blocked "config set" ${nunmai-engine}/bin/nunmai config set model foo
          check_blocked "config edit" ${nunmai-engine}/bin/nunmai config edit
        ...
        extra-python-packages = let
          ...
          nunmaiWithExtra = nunmai-engine.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nunmai-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nunmaiWithExtra}/bin/nunmai || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nunmaiWithExtra}/bin/nunmai || \
          ...
          if grep -q "PYTHONPATH" ${nunmai-engine}/bin/nunmai; then
        ...
        extra-dependency-groups = let
          nunmaiWithGroups = nunmai-engine.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nunmai-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nunmaiWithGroups}"
          echo "venv: ${nunmaiWithGroups.nunmaiVenv}"
        ...
        messaging-variant = pkgs.runCommand "nunmai-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nunmaiVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nunmai-config-roundtrip" {
          ...
          merge_and_load() {
            local nunmai_home="$1"
            export NUNMAI_HOME="$nunmai_home"
            ${configMergeScript} ${nixSettings} "$nunmai_home/config.yaml"
            ${nunmaiVenv}/bin/python3 -c '
import json, sys
