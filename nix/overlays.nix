# nix/overlays.nix — Expose pkgs.nunmai-engine for external NixOS configs
#
...
# `pkgs.nunmai-engine`, `nix build .#default`, and the NixOS module's
...
{ inputs, ... }:
{
  flake.overlays.default = final: _: {
    nunmai-engine = inputs.self.packages.${final.stdenv.hostPlatform.system}.default;
  };
}
