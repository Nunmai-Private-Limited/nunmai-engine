# nix/homeManagerModules.nix — the Home Manager module for nunmai-engine
#
# This module is the user-level equivalent of nixosModules.default. Nunmai is
...
#
# `services.nunmai-engine` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.nunmai-engine.enable and
#                                        home.sessionVariables
#   added     programs.nunmai-engine    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.nunmai")  -> nunmaiHome, set directly
#
# To use the module:
#   imports = [ nunmai-engine.homeManagerModules.default ];
#   programs.nunmai-engine = {
#     enable = true;          # the nunmai CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.nunmai-engine;
      ...
      # not reach it, and the application would open ~/.nunmai while the
      # services use nunmaiHome. Thus the launcher carries the value itself.
      #
      # NUNMAI_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        NUNMAI_HOME = cfg.nunmaiHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) NUNMAI_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        NUNMAI_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("NUNMAI_DESKTOP_REMOTE_URL is set
      # but NUNMAI_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          NUNMAI_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export NUNMAI_DESKTOP_REMOTE_TOKEN
        else
          echo "nunmai-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "nunmai-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.nunmai-engine — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Nunmai needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.nunmai-engine` stays the authority for the state and the
      # configuration. This module reads nunmaiHome and the backend address
      # from it, and never the reverse.
      options.programs.nunmai-engine = {
        enable = lib.mkEnableOption ''
          the Nunmai Engine command line application.

          This adds `nunmai` to home.packages, and exports NUNMAI_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.nunmai-engine`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.nunmai-engine.package";
          description = ''
            The nunmai-engine package to install.

            The default follows `services.nunmai-engine.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Nunmai Desktop application (Electron).

            This adds `nunmai-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Nunmai
            runtime that `package` gives, and reads the NUNMAI_HOME of
            `services.nunmai-engine`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Nunmai runtime with
            the usual distribution. This module gives it the Nix package
            instead, with NUNMAI_DESKTOP_NUNMAI. It installs no second copy
            of Nunmai, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.nunmaiDesktop;
            defaultText = lib.literalExpression "config.programs.nunmai-engine.package.nunmaiDesktop";
            description = ''
              The nunmai-desktop package to use.

              The default follows `package`, and thus also
              `services.nunmai-engine.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Nunmai runtime, and this module cannot make
      ...
      options.services.nunmai-engine =
        ...
        // {
          nunmaiHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.nunmai";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.nunmai"'';
            description = ''
              The value of NUNMAI_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.nunmai` to it.
              This module sets NUNMAI_HOME directly. Thus an existing
              ~/.nunmai continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.nunmai-work";
          };

          # `installPackage` moved to `programs.nunmai-engine.enable`. The
          ...
          # quiet removal gives them a machine with no `nunmai` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.nunmai-engine.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.NUNMAI_HOME = cfg.nunmaiHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # NUNMAI_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put NUNMAI_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `nunmai` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.nunmai-engine.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.nunmaiAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) nunmaiHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.nunmai-engine = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.nunmai-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.nunmai-engine = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "nunmai-engine";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.nunmai-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "nunmai-backend";
              };
            })
          ]
        ))
