# nix/web.nix — Nunmai Web Dashboard (Vite/React) frontend build
{ nunmaiNpmLib, ... }:
nunmaiNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @nunmai/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../nunmai_cli/web_dist for the
