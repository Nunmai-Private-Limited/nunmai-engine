# nix/desktop.nix — Nunmai Desktop (Electron) app build + wrapper
#
# `nunmaiAgent` is the fully-built `.#default` package — it ships the
# `nunmai` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `NUNMAI_DESKTOP_NUNMAI` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Nunmai CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # NUNMAI_HOME and NUNMAI_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "nunmai-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "nunmai-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = nunmaiNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "nunmai-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe nunmaiNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/nunmai-desktop $out/bin
    cp -r ${renderer}/* $out/share/nunmai-desktop/
    ...
    substituteInPlace $out/share/nunmai-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/nunmai-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # NUNMAI_DESKTOP_NUNMAI to the absolute path of the nix-built `nunmai`
    # binary so the desktop's resolver step 4 ("existing Nunmai CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/nunmai-desktop \
      --add-flags "$out/share/nunmai-desktop" \
      --set NUNMAI_DESKTOP_NUNMAI "${lib.getExe nunmaiAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/nunmai.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../nunmai_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/nunmai-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/nunmai.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/nunmai.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Nunmai Engine";
    homepage = "https://github.com/Nunmai-Private-Limited/nunmai-engine";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "nunmai-desktop";
  };
}
