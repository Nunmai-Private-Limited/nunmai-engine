# nix/packages.nix — Nunmai Engine package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./nunmai-engine.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.nunmaiTui;
        web = full.nunmaiWeb;
        desktop = full.nunmaiDesktop;

        update-npm-lockfile = full.nunmaiNpmLib.updateNpmLockfile;
      };
    };
}
