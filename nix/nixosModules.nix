# nix/nixosModules.nix — the NixOS module for nunmai-engine
#
...
#
# Container mode: nunmai runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NUNMAI_HOME/.env
# and read by nunmai at startup — no container recreation needed for env changes.
#
# Tool resolution: the nunmai wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.nunmai-engine;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      nunmai-engine = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      nunmaiHome = "${cfg.stateDir}/.nunmai";
      ...
      # NUNMAI_HOME via addToSystemPackages, so they can save settings through the
      ...
      # nunmai user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "nunmai-container-entrypoint" ''
        set -eu

        NUNMAI_UID="''${NUNMAI_UID:?NUNMAI_UID must be set}"
        NUNMAI_GID="''${NUNMAI_GID:?NUNMAI_GID must be set}"

        # ── Group: ensure a group with GID=$NUNMAI_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$NUNMAI_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="nunmai"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$NUNMAI_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$NUNMAI_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$NUNMAI_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$NUNMAI_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="nunmai"
          TARGET_HOME="/home/nunmai"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$NUNMAI_UID" -g "$NUNMAI_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$NUNMAI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$NUNMAI_UID:$NUNMAI_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure NUNMAI_HOME is owned by the target user.
        ...
        if [ -n "''${NUNMAI_HOME:-}" ] && [ -d "$NUNMAI_HOME" ]; then
          find "$NUNMAI_HOME" \! -user "$NUNMAI_UID" -exec chown "$NUNMAI_UID:$NUNMAI_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/nunmai-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/nunmai-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nunmai ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nunmai
          chmod 0440 /etc/sudoers.d/nunmai
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$NUNMAI_UID" --regid="$NUNMAI_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $NUNMAI_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "nunmai-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        nunmai_bin=${containerDataDir}/current-package/bin/nunmai
      '';

      # Default: /var/lib/nunmai/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the nunmai group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit nunmaiHome; };
    ...
    in
    {
      options.services.nunmai-engine =
        common.sharedOptions {
          defaultPackage = nunmai-engine;
          defaultPackageText = lib.literalExpression "nunmai-engine.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/nunmai";
              description = "State directory. Contains .nunmai/ subdir (NUNMAI_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the nunmai CLI to environment.systemPackages and export
                NUNMAI_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.nunmai symlink to the service
                  stateDir. These users are automatically added to the nunmai group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.nunmai-engine.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the nunmai CLI to system PATH and export NUNMAI_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.nunmai/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.NUNMAI_HOME = nunmaiHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.nunmai-engine";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.nunmai-engine.workingDirectory;
                optionPath = "services.nunmai-engine";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.nunmai-engine: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the nunmai user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.nunmai-engine: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed nunmai
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure nunmai is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${nunmaiHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${nunmaiHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."nunmai-engine-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${nunmaiHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${nunmaiHome} ${cfg.workingDirectory}
                  ...
                  find ${nunmaiHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${nunmaiHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${nunmaiHome}/$_subdir"
                    chmod 2770 "${nunmaiHome}/$_subdir"
                    find "${nunmaiHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg nunmaiHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${nunmaiHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${nunmaiHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${nunmaiHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.nunmai";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${nunmaiHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "nunmai-engine: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.nunmai -> stateDir/.nunmai for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.nunmai-engine = {
              description = "Nunmai Engine Gateway";
              ...
              # $NUNMAI_HOME/.env by the activation script. load_nunmai_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "nunmai-engine: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.nunmai-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.nunmai-engine = {
              description = "Nunmai Engine Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  NUNMAI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  NUNMAI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env NUNMAI_UID="$NUNMAI_UID" \
                    --env NUNMAI_GID="$NUNMAI_GID" \
                    --env NUNMAI_HOME=${containerDataDir}/.nunmai \
                    --env NUNMAI_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/nunmai gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
