...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      nunmaiNpmLib = self'.packages.default.passthru.nunmaiNpmLib;
      ...
      nunmaiAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "nunmai" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../nunmai} $out/bin/nunmai
          '')
        ...
        shellHook = ''
          ${nunmaiAgentDevShellHook}
          ${nunmaiNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export NUNMAI_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Nunmai Engine dev shell in $NUNMAI_PYTHON_SRC_ROOT"
          echo "Ready. Run 'nunmai' or 'sandbox nunmai' to start."
        '';
      };
    };
}
