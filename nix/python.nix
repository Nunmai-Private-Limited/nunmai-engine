...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Nunmai derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            nunmai-engine = prev.nunmai-engine.overrideAttrs (_old: {
              NUNMAI_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $NUNMAI_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$NUNMAI_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        nunmai-engine = prev.nunmai-engine.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "nunmai-engine-env" {
    nunmai-engine = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "nunmai-engine-editable-env" {
    nunmai-engine = dependency-groups;
  };
}
