...
from nunmai_state_errors import STORAGE_RECOVERY_DOCS_URL, classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `nunmai {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Nunmai process",
        "Wait a moment and try again; if it persists, stop the other Nunmai process "
        "(`nunmai {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        _DOCTOR + " Recovery: `nunmai {profile_arg}sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `nunmai {profile_arg}doctor --fix` (or `nunmai {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Nunmai was running",
        "Stop Nunmai (`nunmai {profile_arg}gateway stop`), run `nunmai {profile_arg}doctor`, then start it again.",
    ),
    ...
    "deleted_wal": (
        ...
        "Nothing is lost. Quit every Nunmai process on this profile (Desktop app, "
        "`nunmai {profile_arg}gateway stop`, dashboard, cron), run `nunmai {profile_arg}doctor` — it names "
        "any process still holding the log — then start Nunmai again. Do not run `doctor --fix` or delete "
    ...
    "turn_lease": (
        "storage_busy",
        "another Nunmai process took over this session",
