...
documents (stop Nunmai, recover or restore, start again). No marker is written to disk:
...
def mark_storage_corrupt(db_path, reason: object) -> None:
    ...
    logger.error(
        ...
        "until Nunmai restarts on a recovered or restored file. Stop Nunmai, then run "
        "`nunmai sessions recover --source %s --inspect-only` or restore a snapshot.",
