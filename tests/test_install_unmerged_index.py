...
The ``nunmai update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m nunmai_cli.main gateway run`` — image name
    ``pythonw``, not ``nunmai.exe`` — so the ``taskkill /IM nunmai.exe`` guard
    ...
    # The nunmai.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM nunmai.exe' in text
