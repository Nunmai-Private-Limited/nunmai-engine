...
def _drive_node_bootstrap():
    return nunmai_constants._run_node_bootstrap("ensure_node", timeout=5)
...
def test_node_bootstrap_spawns_resolved_bash(monkeypatch, tmp_path, drive):
    ...
    monkeypatch.delenv("NUNMAI_SKIP_NODE_BOOTSTRAP", raising=False)
    monkeypatch.setattr(nunmai_constants, "_NODE_BOOTSTRAP_SCRIPT", tmp_path / "node-bootstrap.sh")
