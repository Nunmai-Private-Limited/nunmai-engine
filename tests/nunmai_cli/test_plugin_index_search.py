...
(remote → cache → seed), `nunmai plugins search --json`, and install-time
...
from nunmai_cli import plugin_index
...
def nunmai_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_nunmai_home", lambda: tmp_path)
    return tmp_path
...
class TestSearch:
    ...
    def test_exact_name_ranks_first(self):
        results = search_index(self.entries, "nunmai-media-studio")
        assert results[0].name == "nunmai-media-studio"

    def test_matches_tags(self):
        results = search_index(self.entries, "telegram")
        assert results and results[0].name == "nunmai-telegram-business"

    def test_matches_description(self):
        results = search_index(self.entries, "secretary")
        assert [e.name for e in results] == ["nunmai-telegram-business"]

    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "nunmai-media-studo")
        assert results and results[0].name == "nunmai-media-studio"
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, nunmai_home, monkeypatch):
        _write_cache(nunmai_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, nunmai_home, monkeypatch):
        _write_cache(nunmai_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, nunmai_home, monkeypatch):
        _write_cache(nunmai_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, nunmai_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, nunmai_home, monkeypatch):
        _write_cache(nunmai_home, SAMPLE)
    ...
    def test_offline_skips_network(self, nunmai_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, nunmai_home, monkeypatch):
        cache = nunmai_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, nunmai_home, monkeypatch):
...
class TestResolveName:
    ...
    def test_exact_unique(self):
        entry, candidates = resolve_name(self.entries, "nunmai-media-studio")
        assert entry is not None and entry.repo == "NousResearch/hermes-media-studio"

    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "Nunmai-Media-Studio")
        assert entry is not None

    def test_unique_partial(self):
        entry, _ = resolve_name(self.entries, "telegram")
        assert entry is not None and entry.name == "nunmai-telegram-business"

    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "nunmai")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from nunmai_cli.plugins_cmd import _looks_like_bare_index_name

        assert _looks_like_bare_index_name("nunmai-media-studio")
    ...
    def test_install_resolves_name_and_pins_ref(self, nunmai_home, monkeypatch):
        from nunmai_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("nunmai-media-studio", enable=False)
        assert captured["identifier"] == "NousResearch/hermes-media-studio"
        assert captured["ref"] == "e" * 40

    def test_install_explicit_ref_beats_index_pin(self, nunmai_home, monkeypatch):
        from nunmai_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_install_plugin_core", fake_core)
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("nunmai-media-studio", enable=False, ref="d" * 40)
        assert captured["ref"] == "d" * 40

    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, nunmai_home, monkeypatch, capsys
    ):
        from nunmai_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("nunmai", enable=False)
        assert exc.value.code == 1
        assert not called
        out = capsys.readouterr().out
        assert "ambiguous" in out
        assert "nunmai-media-studio" in out
        assert "nunmai-telegram-business" in out

    def test_install_unknown_name_exits(self, nunmai_home, monkeypatch, capsys):
        from nunmai_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, nunmai_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, nunmai_home, monkeypatch, capsys):
        from nunmai_cli import plugins_cmd
        ...
        assert payload["results"][0]["name"] == "nunmai-telegram-business"
    ...
    def test_no_results_message(self, nunmai_home, monkeypatch, capsys):
        from nunmai_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, nunmai_home, monkeypatch):
        from nunmai_cli import plugins_cmd
