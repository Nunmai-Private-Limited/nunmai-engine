"""Tests for nunmai_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from nunmai_cli import relaunch as relaunch_mod


class TestResolveNunmaiBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/nunmai"
        ...
        assert relaunch_mod.resolve_nunmai_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "nunmai"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'nunmai' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_nunmai_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/nunmai" if name == "nunmai" else None
        )
        assert relaunch_mod.resolve_nunmai_bin() == "/usr/bin/nunmai"
...
class TestBuildRelaunchArgv:
    ...
    def test_preserves_inherited_flags(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_nunmai_bin", lambda: "/usr/bin/nunmai")
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/nunmai", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/nunmai", ["/usr/bin/nunmai", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        nunmai).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_nunmai_bin", lambda: r"C:\Users\test\nunmai.exe")
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_nunmai_bin", lambda: r"C:\nunmai.exe")
...
class TestResolveNunmaiBinWindowsPyGuard:
    """On Windows, resolve_nunmai_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a nunmai.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\nunmai.exe" if name == "nunmai" else None,
        )

        bin_path = relaunch_mod.resolve_nunmai_bin()
        # Must NOT be the .py — must be the nunmai.exe PATH entry.
    ...
    def test_posix_python_launcher_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        The git installer invokes ``nunmai`` with the venv interpreter while the
        ...
        script = tmp_path / "nunmai"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        wrapper = tmp_path / "bin" / "nunmai"
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: str(wrapper) if name == "nunmai" else None,
        )
        assert relaunch_mod.resolve_nunmai_bin() == str(wrapper)
    ...
    def test_posix_python_launcher_on_path_falls_back_to_current_python(
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: str(script) if name == "nunmai" else None,
        )

        assert relaunch_mod.resolve_nunmai_bin() is None

        # A console script pinned to the running interpreter keeps the venv: still exec-able.
        pinned = tmp_path / "pinned" / "nunmai"
        pinned.parent.mkdir()
        pinned.write_text(f"#!{sys.executable}\n")
        pinned.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(pinned), "chat"])
        assert relaunch_mod.resolve_nunmai_bin() == str(pinned)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_nunmai_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND nunmai.exe
