...
a step that still raises so one bad value cannot wedge ``nunmai config migrate`` /
``nunmai update`` and leave the config unversioned.
"""
...
def test_failing_step_is_skipped_with_warning_and_config_still_migrates(tmp_path, caplog):
    """``migrate_config`` (the ``nunmai config migrate`` / ``nunmai update`` path) keeps going
    ...
    from nunmai_cli import config_migrations
    ...
    with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path)}), \
            ...
            caplog.at_level(logging.WARNING, logger="nunmai_cli.config_migrations"):
