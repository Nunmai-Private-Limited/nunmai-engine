"""``nunmai update`` / ``nunmai migrate relay``: legacy ``NUNMAI_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from nunmai_cli.relay_plugin_migrate import (
...
NUNMAI_NEMO_RELAY_ATOF_ENABLED=1
NUNMAI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
NUNMAI_NEMO_RELAY_ATOF_FILENAME=nunmai-atof.jsonl
NUNMAI_NEMO_RELAY_ATOF_MODE=append
NUNMAI_NEMO_RELAY_ATIF_ENABLED=1
NUNMAI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
NUNMAI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
NUNMAI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "nunmai-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file Nunmai will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: NUNMAI_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
