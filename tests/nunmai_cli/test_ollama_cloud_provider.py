...
from nunmai_cli.auth import resolve_provider, resolve_api_key_provider_credentials
...
class TestOllamaCloudMergedDiscovery:
    ...
    def test_cache_only_serves_stale_cache_without_rewriting_disk(self, tmp_path, monkeypatch):
        ...
        with patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev), \
             patch("nunmai_cli.models.fetch_api_models", side_effect=AssertionError("network probe ran")):
        ...
        with patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev), \
             patch("nunmai_cli.models.fetch_api_models", return_value=["live-only", "shared", "new-live"]) as live:
