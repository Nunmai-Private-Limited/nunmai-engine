"""A second `nunmai serve` attaches to the host backend instead of binding a second port.
...
from nunmai_cli.main_dashboard import _attach_to_host_backend
...
def host_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sys.argv", ["nunmai", "serve"])
    return tmp_path
...
def test_unprovable_liveness_still_has_to_answer(host_dir, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.process_identity._pid_alive_matches", lambda *_a, **_k: None)
...
@pytest.mark.parametrize(
    "argv,over",
    [(["nunmai", "serve", "--port", "8899"], {"port": 8899}),
     (["nunmai", "serve", "--host", "0.0.0.0"], {"host": "0.0.0.0"})],
...
def test_dashboard_is_never_routed_to_a_headless_backend(host_dir, owner, capsys):
    """`nunmai serve` and `nunmai dashboard` publish the same host role; only one mounts the SPA,
