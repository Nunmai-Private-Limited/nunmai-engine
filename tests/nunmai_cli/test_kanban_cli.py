"""Tests for the kanban CLI surface (nunmai_cli.kanban)."""
...
def test_worker_link_preserves_foreign_child_rules(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", worker)
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", str(worker_run_id))
    ...
    # with NUNMAI_KANBAN_RUN_ID — the one path _cmd_link forwards a run id for.
    assert kc._cmd_link(argparse.Namespace(
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="nunmai", add_help=False)
