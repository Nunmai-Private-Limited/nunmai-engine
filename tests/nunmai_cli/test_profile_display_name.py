...
The canonical profile id ("default" for ~/.nunmai) is never touched —
...
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() and NUNMAI_HOME under tmp_path."""
...
class TestMetaAndValidation:
    ...
    def test_bot_title_read_from_nunmai_bots_ui_meta(self, profile_env):
        ...
        from nunmai_cli.web_routers.profiles import _profile_to_dict
...
class TestRenameDefault:
    ...
    def test_named_rename_still_real_and_keeps_display_name(
        ...
        monkeypatch.setattr(
            "nunmai_cli.profiles.check_alias_collision", lambda name: "skip"
        )
