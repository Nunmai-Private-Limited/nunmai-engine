"""#93349 — ``nunmai update`` restarts only the gateways of the home it is updating.

``nunmai-gateway*`` units and ``gateway run`` processes are host-wide namespaces shared by every
Nunmai install under the account. A scratch home's update used to drain and restart the account's
real ``nunmai-gateway.service`` and SIGTERM sibling installs' gateways because they were listed,
...
FOREIGN_HOME = "/srv/other-account-home/.nunmai"


@pytest.fixture
def own_home(monkeypatch, tmp_path):
    home = tmp_path / "homeA" / ".nunmai"
    home.mkdir(parents=True)
    monkeypatch.setenv("NUNMAI_HOME", str(home))
    # The plan inventory is this install only; the fixture home has no profiles dir.
    monkeypatch.setattr("nunmai_cli.update_receipt._profile_homes", lambda: [("default", home)])
    return home


def _pid_homes(monkeypatch, mapping: dict):
    monkeypatch.setattr(dashboard_procs, "_nunmai_home_for_pid", lambda pid: mapping.get(pid))


def test_systemd_unit_of_another_home_is_left_alone(monkeypatch, own_home):
    """user/nunmai-gateway runs on another NUNMAI_HOME → no drain, no restart, not a failure;
    ...
    fleet._restart_one_systemd_gateway_unit(
        "nunmai-gateway", scope="user", scope_cmd=["systemctl", "--user"], drain_budget=5.0,
    ...
    assert drained == [4343] and restarted == ["nunmai-gateway"] and failed == []
...
def test_manual_gateway_of_another_home_is_not_stopped(monkeypatch, own_home):
    ...
    monkeypatch.setattr("nunmai_cli.gateway._get_service_pids", lambda **k: set())
    monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda **k: [111, 222, 333])
    monkeypatch.setattr("nunmai_cli.gateway.find_profile_gateway_processes", lambda **k: [])
    monkeypatch.setattr("nunmai_cli.gateway._wait_for_gateway_exit", lambda **k: None)
