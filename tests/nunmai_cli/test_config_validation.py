...
class TestTimezoneValidation:
    ...
    schedule on server-local time (nunmai_time._get_zoneinfo falls back with
    ...
    def test_invalid_or_non_string_zone_is_an_error(self):
        ...
        assert "IANA" in issue.hint and "NUNMAI_TIMEZONE" in issue.hint
...
class TestUnknownTopLevelKeys:
    ...
    (gateway/run.py, nunmai send) so users can feed skills and external apps
...
class TestQuotedContainerValues:
    ...
    def test_quoted_list_in_container_slot_is_flagged_with_remedy(self):
        ...
        assert "nunmai config set plugins.enabled '[\"a\",\"b\"]'" in flagged["plugins.enabled"].hint
