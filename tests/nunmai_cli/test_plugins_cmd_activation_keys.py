...
Runs the real ``nunmai_cli.plugins_cmd`` commands against a temp NUNMAI_HOME and checks the verdict
...
from nunmai_cli.plugins_discovery import collect_directory_manifests, gate_manifest
...
def home(tmp_path, monkeypatch):
    ...
    (nunmai_home / "plugins").mkdir(parents=True)
    (nunmai_home / "config.yaml").write_text("plugins:\n  enabled: []\n  disabled: []\n", encoding="utf-8")
...
def test_disable_bundled_platform_by_manifest_name_gates_the_loader(home):
    """`nunmai plugins disable photon-platform` must produce a key the loader's gate matches; the
