...
from nunmai_cli.console_engine import NunmaiConsoleEngine, run_console_repl


def test_sessions_optimize_accepts_the_force_override_it_advertises(_isolate_nunmai_home):
    ...
    result = NunmaiConsoleEngine().execute("sessions optimize --force", confirmed=True)

    assert result.status == "ok", result.output
    assert "Usage:" not in result.output


def test_sessions_list_and_stats_use_isolated_session_store(_isolate_nunmai_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(nunmai_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = NunmaiConsoleEngine().execute(
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_nunmai_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_nunmai_home):
    ...
    assert code == 0
    assert "Nunmai Console" in stdout.getvalue()
    assert "nunmai>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from nunmai_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_nunmai_home):
    result = NunmaiConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_nunmai_home, monkeypatch
):
    ...
    result = NunmaiConsoleEngine().execute("checkpoints prune", confirmed=True)
