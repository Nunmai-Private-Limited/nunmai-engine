...
class TestRestartWaitsForApiServerPort:
    """Regression for #91547: ``nunmai gateway restart`` waited only for the old PID; on macOS the
...
def test_find_windows_gateway_services_ignores_task_scheduler_ancestor(monkeypatch):
    ...
    Nunmai-owned service (by binary path) stays SCM-supervised."""
    import psutil
    import nunmai_cli.gateway_windows as gateway_windows

    monkeypatch.setattr(gateway_windows, "nunmai_service_roots", lambda: (r"C:\nunmai\nunmai-engine",))
    ...
    # Nunmai's, and never a reason to abort; a Nunmai-NAMED service is settled without asking binpath.
    ...
    named = run(UnreadableConfigService("NunmaiGateway", OSError(15100, "MUI file missing")))
    assert [(s.name, s.service_pid, s.gateway_pid) for s in named] == [("NunmaiGateway", 2360, 18480)]
