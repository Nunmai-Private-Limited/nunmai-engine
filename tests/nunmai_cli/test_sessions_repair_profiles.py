"""``nunmai sessions repair-profiles``: the backward-looking half of the per-profile store model.
...
def test_apply_settles_every_repairable_crossing_and_is_idempotent(homes, monkeypatch, capsys):
    _seed_crossings(homes)
    snapshots = []
    monkeypatch.setattr("nunmai_cli.sessions_cmd_repair_profiles.default_snapshot",
...
def test_apply_refuses_while_a_gateway_owns_a_store(homes, monkeypatch, capsys):
    _seed_crossings(homes)
    monkeypatch.setattr("nunmai_cli.sessions_cmd_repair_profiles.live_gateway_homes",
...
def test_legacy_main_rekey_adopts_a_standalone_gateways_history(homes, monkeypatch):
    ...
    _seed_crossings(homes)
    monkeypatch.setattr("nunmai_cli.sessions_cmd_repair_profiles.default_snapshot", lambda store: "snap")
