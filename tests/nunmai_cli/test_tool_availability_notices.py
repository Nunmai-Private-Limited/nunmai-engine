...
from nunmai_cli.tool_availability_notices import filter_to_enabled_toolsets, tool_availability_warning_lines
...
def test_terminal_line_names_backend_reason_and_fix_commands():
    ...
    assert "nunmai doctor" in text and "nunmai setup terminal" in text
...
def test_web_collapses_env_dump_into_one_provider_sentence():
    ...
    assert "Web search is off" in text and "nunmai setup tools" in text
...
def test_generic_toolset_lists_vars_and_setup_hint():
    ...
    text = "\n".join(lines)
    assert "rl" in text and "TINKER_API_KEY" in text and "nunmai setup" in text
...
class TestFilterToEnabledToolsets:
    _BUNDLE = {"nunmai-cli": ["terminal", "process_manage", "web_search", "read_file"],
    ...
    def test_composite_bundle_keeps_terminal_notice_on_default_install(self):
        kept = filter_to_enabled_toolsets([_TERMINAL, _WEB, _RL], ["nunmai-cli"], self._resolve)
    ...
    def test_resolver_error_is_not_fatal(self):
        ...
        assert filter_to_enabled_toolsets([_TERMINAL], ["nunmai-cli"], boom) == []
