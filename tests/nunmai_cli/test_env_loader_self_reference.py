...
def test_alternating_project_env_and_none_stays_stable(home, tmp_path):
    ...
    for i in range(6):
        load_nunmai_dotenv(
            nunmai_home=home, project_env=project_env if i % 2 else None, load_external_secrets=False
        )
...
def test_alternating_homes_each_resolve_against_boot_path(tmp_path, monkeypatch):
    ...
    for h in (home_a, home_b, home_a, home_b, home_a):
        load_nunmai_dotenv(nunmai_home=h, load_external_secrets=False)
...
def test_later_layer_in_the_same_load_still_sees_the_earlier_layer(home, tmp_path):
    """Within one load_nunmai_dotenv the project layer builds on the user layer, as before; the
    ...
    for _ in range(3):
        load_nunmai_dotenv(nunmai_home=home, project_env=project_env, load_external_secrets=False)
