...
A long-lived multiplex process (the Desktop's ``nunmai serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    nunmai_logging.setup_logging(nunmai_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(nunmai_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_nunmai_home_override(profile)
    try:
        if adopt:
            nunmai_logging.setup_logging(nunmai_home=profile)
        else:
            assert nunmai_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in nunmai_logging._queued_file_handlers
            if isinstance(h, nunmai_logging._ProfileRoutingFileHandler)]
