"""Invariant: `nunmai serve`'s opportunistic auto-archive never opens a WRITABLE
...
``gateway.lock`` that ``gateway.status`` uses, runs under a ``nunmai gateway run``
...
from nunmai_cli import web_server_sessions as wss
...
def test_serve_auto_archive_defers_to_a_live_gateway_for_the_profile(archive_probe):
    ...
    # argv0 basename `nunmai` + the `gateway run` subcommand is what
    # gateway.status's process-identity check reads off /proc for this PID.
    entrypoint = tmp_path / "nunmai"
