"""Plugin catalog contracts (nunmai_cli/plugin_catalog.py): the in-tree seed is valid, bad entries are
...
def test_newer_in_tree_pin_outranks_a_fresh_live_cache(tmp_path, monkeypatch):
    """Right after `nunmai update` bumps an in-tree pin, a live cache fetched BEFORE the bump must not
