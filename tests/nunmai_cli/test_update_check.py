...
def git_repo(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("nunmai_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(monkeypatch, *, head=SHA_A, origin="https://github.com/Nunmai-Private-Limited/nunmai-engine.git"):
    calls = []
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("ghcr.io/nunmai-private-limited/nunmai-engine", "main")
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``nunmai update`` moving HEAD busts it at once;
