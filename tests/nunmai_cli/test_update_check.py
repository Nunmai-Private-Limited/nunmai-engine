"""Tests for the update check mechanism in nunmai_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "nunmai-engine"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("nunmai_cli.banner.subprocess.run") as mock_run:
...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/Nunmai-Private-Limited/nunmai-engine.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    monkeypatch.delenv("NUNMAI_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "nunmai_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "nunmai_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_nunmai_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via nunmai_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to nunmai_home / "nunmai-engine". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to nunmai_home / "nunmai-engine".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("nunmai_cli"):
