...
#88251: ``nunmai update`` treated a failed desktop pack as non-fatal, printed
...
import nunmai_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_constants.with_nunmai_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "nunmai_constants.display_nunmai_home", lambda: str(tmp_path), raising=False
    )
...
def test_lost_desktop_with_surviving_build_stamp_is_rebuilt(desktop_env, monkeypatch):
    ...
    forever; the build stamp under NUNMAI_HOME survived and is the proof Desktop was installed here."""
    desktop_dir, calls = desktop_env
    monkeypatch.setattr(
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_nunmai_home", lambda: tmp_path)
