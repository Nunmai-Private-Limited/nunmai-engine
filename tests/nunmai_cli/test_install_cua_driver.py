...
* Be supported-platform-only — no-op silently elsewhere so ``nunmai update``
...
class TestCuaDriverRuntimeContract:
    def test_current_manifest_is_ready(self):
        from nunmai_cli import tools_config_cua as tools_config
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Nunmai
    ...
    def test_incompatible_explicit_override_is_not_replaced(self, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_CUA_DRIVER_CMD", "/opt/custom/cua-driver")
        incompatible = {
            "ready": False,
            "version": "0.19.4",
            "reason": "Nunmai computer use requires cua-driver 0.20.0 or newer",
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("NUNMAI_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`nunmai update` passes require_confirmed_update=True: the full
    ...
    Explicit `nunmai computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `nunmai computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Nunmai' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `nunmai update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `nunmai update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``nunmai update``
...
class TestCuaVersionSummary:
    """`nunmai computer-use status` prints one line, whatever the binary says.

    A binary chosen by NUNMAI_CUA_DRIVER_CMD is under no obligation to answer
