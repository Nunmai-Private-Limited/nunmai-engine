...
def _patch_gateway_discovery():
    ...
    it ``ai.nunmai.gateway`` and the verify step exits 1 (#111866, #110701).
    """
    with patch("nunmai_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("nunmai_cli.update_cmd_fleet._restart_macos_launchd_gateways", lambda *a, **k: None), \
...
def test_cmd_update_ordinary_divergence_also_leaves_a_rescue_ref(monkeypatch, tmp_path, capsys):
    ...
    assert ref_name.startswith("refs/nunmai-update-backups/diverged-main-")
...
def test_stash_selector_is_a_bare_index_never_a_brace_selector(tmp_path):
    ...
    import nunmai_cli.update_cmd_stash as stash_mod
...
def test_autostash_survives_intent_to_add_entries(tmp_path):
    ...
    Reported: `nunmai update` aborted with "Entry 'tests/...' not uptodate. Cannot merge." because
    ...
    assert nunmai_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
...
def test_conflicted_restore_records_parked_step_in_receipt(monkeypatch, tmp_path):
    import subprocess
    from nunmai_cli import update_receipt
    ...
    restored = nunmai_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
