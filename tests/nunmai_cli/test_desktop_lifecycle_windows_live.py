...
def _write_ledger(entries: list[dict]) -> None:
    from nunmai_cli import process_identity as pid_mod
...
def test_holder_scan_fallback_respects_token_classifier(sleeper, monkeypatch, tmp_path):
    ...
    serve_like = sleeper("-m", "nunmai_cli.main", "serve")
    # Lookalike from the #90778 class — must NOT confer ownership.
    kanban_like = sleeper("-m", "nunmai_cli.main", "kanban", "--preserve-cache")
    ...
    monkeypatch.setattr(
        "nunmai_cli.main._detect_venv_python_processes", lambda: [serve_row, kanban_row]
    )
    ...
    monkeypatch.setattr(
        "nunmai_cli.main._detect_venv_python_processes", lambda: [kanban_row]
    )
