"""Fixtures shared across nunmai_cli tests."""
...
def _inline_post_swap_handoff(request, monkeypatch):
    """Run the post-swap tail in-process instead of re-executing ``nunmai update --post-swap``.
    ...
    if request.node.get_closest_marker("real_post_swap_handoff"):
        return
    try:
        from nunmai_cli import update_cmd, update_receipt
    except Exception:
        return
...
def _discharge_host_update_obligation():
    """Start and end every ``nunmai_cli`` test with NO host update-restart obligation.

    The record is host-scoped on purpose (one multiplexer per host), so it lives in the
    per-OS-USER host state dir — not in the per-test ``NUNMAI_HOME``. The root conftest pins
    that dir per test only when the caller supplied no ``NUNMAI_GATEWAY_LOCK_DIR`` (#118097
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from nunmai_cli import gateway, main, update_cmd, update_cmd_fleet
    from nunmai_cli import update_inventory, update_receipt
