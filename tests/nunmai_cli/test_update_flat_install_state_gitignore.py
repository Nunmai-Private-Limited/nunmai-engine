"""Flat-install runtime state must be gitignored so ``nunmai update``'s untracked
autostash cannot sweep the live state.db (#110648).

On a flat install (checkout root == $NUNMAI_HOME) the profile's runtime files
live inside the repo as untracked paths. ``git stash push --include-untracked``
(nunmai_cli/update_cmd_stash.py) moves the whole untracked set into the stash and
...
.nunmai-bootstrap-complete / .install_method precedent (#38529 / #66189).
"""
import os
import shutil
import sqlite3
...
# Runtime state that lives at $NUNMAI_HOME's root on a flat install, exactly as
# ``nunmai update`` would sweep it: one representative per ignored class. The
...
FLAT_INSTALL_RUNTIME_STATE = (
    ...
    # files from nunmai_state_dbfile / nunmai_state_repair / nunmai_state_common /
    # nunmai_cli/kanban_db_*, the repair-attempts ledger and malformed backups.
...
def flat_install_repo(tmp_path: Path) -> Path:
    ...
    Built in a subdirectory of tmp_path: the suite-wide NUNMAI_HOME isolation
    fixture (tests/conftest.py) materialises its own ``nunmai_test/`` tree in
    ...
    (repo / "app.py").write_text("print('nunmai')\n")
...
def test_flat_install_runtime_state_is_ignored(flat_install_repo):
    ...
    so `nunmai update` never enters its stash step for runtime state alone."""
    status = _run_git(
...
def test_untracked_autostash_cannot_sweep_runtime_state(flat_install_repo):
    ...
    _run_git(
        flat_install_repo,
        "stash", "push", "--include-untracked", "-m", "nunmai-update-autostash",
    )
    assert (flat_install_repo / "app.py").read_text() == "print('nunmai')\n"
    missing = [
        rel
        for rel in FLAT_INSTALL_RUNTIME_STATE
...
def test_untracked_autostash_leaves_open_wal_database_readable(flat_install_repo):
    ...
    from nunmai_cli.update_cmd_stash import _stash_local_changes_if_needed
