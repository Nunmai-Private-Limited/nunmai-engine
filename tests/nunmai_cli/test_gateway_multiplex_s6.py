...
slot, ``nunmai gateway migrate --multiplex`` folds an UP slot in-process, a guard refusal on a
...
from nunmai_cli import gateway_multiplex_s6 as s6
from nunmai_cli.container_boot import reconcile_profile_gateways
...
def test_migrate_folds_an_up_named_slot_in_process_and_boot_shares_the_rule(s6_host, tmp_path):
    ...
    actions = {a.profile: a for a in reconcile_profile_gateways(
        nunmai_home=root, scandir=tmp_path / "boot-svc", dry_run=True, container_argv=())}
...
def test_resolved_default_is_written_once_and_never_on_a_guard(s6_host):
    ...
    # gateway start (log_multiplex_decision prints it) and the next ``nunmai update`` summary.
