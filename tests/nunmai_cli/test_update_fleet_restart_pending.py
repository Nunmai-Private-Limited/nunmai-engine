...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(update_cmd, "_purge_stale_nunmai_modules", lambda: None)
    monkeypatch.setattr(nunmai_main, "_purge_stale_nunmai_modules", lambda: None)
...
def test_marker_round_trip_under_nunmai_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_nunmai_home()
