...
A ``nunmai update`` killed after git pull advanced HEAD but before the
...
- next ``nunmai update`` with git already up to date still runs the
...
import nunmai_cli.main_web_build as main_web_build
import nunmai_cli.main_install_repair as main_install_repair
from nunmai_cli import update_cmd
import nunmai_cli.update_cmd_fleet as update_cmd_fleet
...
import nunmai_cli.update_host_obligation as host_obligation
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(nunmai_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(nunmai_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(nunmai_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(nunmai_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        nunmai_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/Nunmai-Private-Limited/nunmai-engine.git",
    )
    monkeypatch.setattr(update_cmd, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        nunmai_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(nunmai_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        nunmai_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(nunmai_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        nunmai_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        nunmai_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(nunmai_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(nunmai_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(nunmai_main, "_build_web_ui", lambda *a, **k: None)
    ...
    import nunmai_cli.gateway as nunmai_gateway

    monkeypatch.setattr(
        nunmai_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(nunmai_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        nunmai_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "nunmai_cli.update_inventory.collect_runtime_inventory",
...
def test_obligation_round_trip_is_host_scoped():
    ...
    assert not (get_nunmai_home() / "fleet_restart_pending").exists()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_nunmai_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "nunmai_cli.gateway.find_gateway_pids", lambda **k: []
    )
    ...
    monkeypatch.setattr("nunmai_cli.gateway_windows.is_installed", lambda: False)
...
def test_run_pending_restart_skips_gateways_already_on_checkout_code(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda **k: [48096])
    monkeypatch.setattr(
        "nunmai_cli.update_receipt.collect_fleet_versions",
        lambda: [{"profile": "default", "pid": 48096, "code_sha": sha, "state": "current"}],
    )
    stopped = []
    monkeypatch.setattr("nunmai_cli.gateway.kill_gateway_processes", lambda **k: stopped.append(k))
    monkeypatch.setattr("nunmai_cli.gateway_windows.restart", lambda: stopped.append("windows-restart"))
    monkeypatch.setattr("nunmai_cli.gateway_windows.is_installed", lambda: True)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``nunmai-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import nunmai_cli.update_inventory as ui
    ...
    import nunmai_cli.process_identity as pi
    ...
    latest = get_nunmai_home() / "logs" / "update_receipts" / "latest.json"
...
def test_clean_update_defers_desktop_owned_serve_and_clears_marker(
    ...
    nunmai_main.cmd_update(args)  # no SystemExit(1)
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(nunmai_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "nunmai gateway restart" in err
...
#
# `systemctl --user restart nunmai-gateway` never runs this module's clear path, and an update
...
def test_startup_warn_kept_without_positive_evidence(monkeypatch, capsys, disk_sha, fleet):
    ...
    _patch_marker_sha(monkeypatch, disk_sha)
    monkeypatch.setattr("nunmai_cli.update_receipt.collect_fleet_versions", lambda **kwargs: fleet)
...
# warned and every no-op ``nunmai update`` exited 1 while the gateway verifiably served HEAD.
...
def _checkout_with_carried_commit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nunmai_main, "PROJECT_ROOT", repo)
...
def test_startup_warn_silent_when_failed_receipt_already_restarted_fleet(monkeypatch, capsys):
    ...
    restart it performed; ``nunmai update``'s catch-up still owes the fleet the checkout."""
        ...
        json.dumps(
            {
                ...
                "stop_reason": "AttributeError: module 'nunmai_cli.main_dashboard' has no attribute 'x'",
                "pre_update": {"sha": pre}, "post_update": {"sha": pulled},
                "gateway_restart": {
                    "restarted_services": ["nunmai-gateway"], "relaunched_profiles": [],
...
def test_startup_warn_silent_when_completed_update_fleet_restarted_onto_moved_checkout(monkeypatch, capsys):
    ...
    plus ``nunmai gateway restart`` leaves every owed gateway on today's checkout — newer than the
...
def test_catchup_settles_failed_receipt_from_live_fleet_instead_of_exit_1(monkeypatch, capsys):
    ...
    be matched to a live gateway, so every up-to-date `nunmai update` ran the restart, printed
    "completed" then "incomplete", exited 1 and wrote another failed receipt — even after a manual
    `nunmai gateway restart` put the fleet on the checkout code (#117051). With every live row
    ...
    monkeypatch.setattr("nunmai_cli.update_receipt.collect_fleet_versions", lambda **k: list(live))
