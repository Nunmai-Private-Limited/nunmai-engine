...
from nunmai_cli.dashboard_auth import clear_providers, get_provider
...
from nunmai_cli.dashboard_auth import registry as _auth_registry
...
def _real_ctx() -> tuple[PluginManager, PluginContext]:
    manager = PluginManager(scope_key=nunmai_home_key())
...
def test_auth_provider_kept_out_of_manager_teardown_order():
    ...
    # Still attributed to the plugin for `nunmai plugins list`.
...
def test_profile_manager_cannot_replace_launch_home_auth_provider():
    ...
    profile_scope = nunmai_home_key(Path(manager.scope_key) / "profiles" / "bot")
...
def test_profile_directory_can_be_the_process_launch_home(monkeypatch, tmp_path):
    ...
    assert manager.scope_key == nunmai_home_key(get_process_nunmai_home())
...
def test_targeted_unload_disposes_persistent_auth_provider():
    ...
    # `nunmai plugins disable basic` drives a targeted unload of that plugin.
