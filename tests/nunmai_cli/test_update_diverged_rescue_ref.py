"""`nunmai update` must not drop local commits without leaving a named way back.
...
HEAD under ``refs/nunmai-update-backups/`` first and tell the user the ref name.
...
def _rescue_refs(checkout):
    out = _git(checkout, "for-each-ref", "--format=%(refname) %(objectname)",
               "refs/nunmai-update-backups/").stdout
...
def _assert_reset_kept_local_commit(checkout, local_sha, output):
    ...
    assert refs[0].startswith("refs/nunmai-update-backups/diverged-main-")
...
def test_nunmai_update_keeps_local_commit_behind_a_rescue_ref(
