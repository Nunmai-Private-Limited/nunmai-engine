...
import nunmai_cli.doctor as doctor
import nunmai_constants
from nunmai_cli import config as config_mod
import nunmai_cli.gateway as gateway_cli
from nunmai_cli import doctor as doctor_mod
from nunmai_cli.doctor_config import _has_provider_env_config
import shutil
from nunmai_cli import doctor_tools
from nunmai_cli import doctor_state
from nunmai_cli import doctor_platform
from nunmai_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert nunmai_constants.is_termux() is True
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `nunmai doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "NUNMAI_HOME", nunmai_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("NUNMAI_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("NUNMAI_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_nunmai_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal NUNMAI_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_nunmai_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "NUNMAI_HOME", home)
        ...
        try:
            from nunmai_cli import auth as _auth_mod
...
def test_run_doctor_accepts_nunmai_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (nunmai_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".nunmai"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`nunmai doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp NUNMAI_HOME.

        ``run_doctor`` reads the module-level ``NUNMAI_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("NUNMAI_HOME")`` alone
        leaves doctor probing the REAL ~/.nunmai. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from nunmai_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("nunmai_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "nunmai_cli.doctor", raising=False)
        from nunmai_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Nunmai / Codex / Gemini /
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Nunmai / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".nunmai"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Nunmai Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale NUNMAI_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"NUNMAI_MAX_ITERATIONS={ghost}\n")
        (nunmai_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "NUNMAI_HOME", nunmai_home)
        monkeypatch.setattr(doctor_mod, "get_nunmai_home", lambda: nunmai_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("NUNMAI_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("NUNMAI_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale NUNMAI_MAX_ITERATIONS" in out
        env_after = (nunmai_home / ".env").read_text(encoding="utf-8")
        assert "NUNMAI_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_nunmai_tool_progress_warning_says_unsupported_since_floor(self):
        """NUNMAI_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"NUNMAI_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["NUNMAI_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"NUNMAI_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["NUNMAI_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        nunmai_home = tmp_path / ".nunmai"
        nunmai_home.mkdir(parents=True)
        (nunmai_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (nunmai_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"NUNMAI_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: NUNMAI_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.nunmai" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "nunmai update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.nunmai"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.nousresearch.nunmai" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (nunmai desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.nunmai" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
