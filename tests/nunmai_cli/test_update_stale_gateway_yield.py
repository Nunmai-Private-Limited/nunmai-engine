...
def cron_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    (home / ".nunmai").mkdir(parents=True)
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("NUNMAI_HOME", str(home / ".nunmai"))
    for name in list(sys.modules):
        if name.split(".")[0] in ("cron", "gateway", "nunmai_cli", "nunmai_constants"):
            monkeypatch.delitem(sys.modules, name, raising=False)
    return home / ".nunmai"


def _status_block(pids):
    from nunmai_cli.cron import _print_ticker_health
...
def test_update_signals_proven_stale_gateway_survivor(cron_home, tmp_path):
    ...
    from nunmai_cli.update_cmd_fleet import _GatewayRestartOutcome
...
def test_verify_fleet_hands_stale_rows_to_survivor_signalling(monkeypatch):
    ...
    import nunmai_cli.update_cmd_stale_survivors as surv
