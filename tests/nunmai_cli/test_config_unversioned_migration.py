...
the installers seed it from cli-config.yaml.example and targeted writers (``nunmai config set``,
...
def test_update_keeps_user_values_of_an_unversioned_config_and_migrates_legacy_keys(nunmai_home):
    from nunmai_cli.config import DEFAULT_CONFIG, set_config_value
    from nunmai_cli.personality import persist_personality
    from nunmai_cli.update_cmd import _check_and_apply_config_migration
    ...
    # Installed with `nunmai plugins install` and never enabled.
    plugin = nunmai_home / "plugins" / "notes-helper"
    ...
    (nunmai_home / "SOUL.md").write_text(soul, encoding="utf-8")
    assert persist_personality("kawaii")
    for key, value in USER_CHOICES.items():
        set_config_value(key, value)
    before = _raw(nunmai_home)

    _check_and_apply_config_migration()

    after = _raw(nunmai_home)
    ...
    assert (nunmai_home / "SOUL.md").read_text(encoding="utf-8") == soul


def test_config_seeded_from_the_template_reads_as_current(nunmai_home):
    """install.sh, install.ps1, docker/stage2-hook.sh and `nunmai doctor --fix` copy the template."""
    from nunmai_cli.config import check_config_version

    shutil.copy(TEMPLATE, nunmai_home / "config.yaml")
