"""Model pickers under a Nunmai free-tier identity.

A guest identity carrying inference shows one row, "Nunmai · free tier", with the single model
``nous/welcome``; the same guest with ``nous.guest: false`` shows no Nunmai row at all. The rule lives
in one helper (``_free_tier_nous_row``) and this file exercises it through the real row builders
against a temp ``NUNMAI_HOME`` with the network catalog fetch stubbed.
"""
...
from nunmai_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    """Seed a guest identity as the only Nunmai state and keep every row builder offline."""
    ...
    from nunmai_cli import model_switch_providers as msp
    ...
    return Path(os.environ["NUNMAI_HOME"])
...
def _cli_nous_rows(config):
    from nunmai_cli.main_provider_setup import _build_provider_picker_rows
...
def test_guest_identity_shows_free_tier_row_with_only_welcome_model(guest_home, monkeypatch):
    ...
    # The `nunmai model` provider picker applies the same rule from the same helper.
