...
Discovery is real (a plugin dir under the test NUNMAI_HOME); only the remote model probe is stubbed.
"""
...
def install_profile(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.models_validate.validate_requested_model", lambda *a, **kw: ACCEPT)
    ...
    def _install(name: str, *, aliases=(), base_url: str, env_var: str, api_mode: str = "chat_completions"):
        ...
        monkeypatch.delitem(sys.modules, f"_nunmai_user_provider_{name}", raising=False)
        plugin_dir = get_nunmai_home() / "plugins" / "model-providers" / name
...
def test_persisted_alias_config_applies_to_the_canonical_runtime_provider(install_profile):
    ...
    (get_nunmai_home() / "config.yaml").write_text(yaml.safe_dump(
