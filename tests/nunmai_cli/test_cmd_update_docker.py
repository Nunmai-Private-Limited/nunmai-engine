"""Tests for ``nunmai update`` / ``--check`` inside the Docker container.
...
fix, ``nunmai update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Nunmai, not an update to the running container, so the message
...
@patch("nunmai_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``nunmai update`` inside Docker → friendly message + exit 2, no git calls.
...
def test_format_docker_update_message_contents():
    ...
    """
    from nunmai_cli.config import format_docker_update_message
    ...
    assert "NUNMAI_HOME" in msg or "/opt/data" in msg, (
