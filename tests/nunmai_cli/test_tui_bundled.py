...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "nunmai_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from nunmai_cli.main_tui_launch import _find_bundled_tui
    result = _find_bundled_tui(nunmai_cli_dir=tmp_path / "nunmai_cli")
