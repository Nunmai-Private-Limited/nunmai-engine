"""``nunmai chat -q``/``-Q`` runs give plugins the CLI reference like the interactive loop does.

Only ``NunmaiCLI.run()`` set ``PluginManager._cli_ref``, so a plugin tool dispatched from a one-shot
