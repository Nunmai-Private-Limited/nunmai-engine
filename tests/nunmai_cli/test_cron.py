"""Tests for nunmai_cli.cron command handling."""
...
from nunmai_cli import cron as cron_cli
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``nunmai cron list`` and ``nunmai cron doctor``,
...
class TestCronDoctor:
    def test_doctor_reports_cron_health_issues(self, tmp_cron_dir, capsys):
        ...
        assert "nunmai cron edit" in out
...
class TestCronListStatusRendering:
    ...
    def test_default_list_includes_paused_jobs(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda: [1])
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "nunmai_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`nunmai cron run` must not report 'failed' when the run was dispatched
...
class TestStatusSurfacesDeadScheduler:
    ...
    def _dead_gateway(self, monkeypatch, lock_dir):
        ...
        monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda: [])
        monkeypatch.setattr(
            "nunmai_cli.gateway.named_profile_served_by_running_multiplexer", lambda: None
        )
