...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `nunmai setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    monkeypatch.setattr("nunmai_cli.auth._is_remote_session", lambda: True)

    from nunmai_cli.auth import is_source_suppressed, suppress_credential_source
...
def test_tts_xai_step_prefers_existing_api_key_over_oauth(monkeypatch):
    ...
    import nunmai_cli.setup_tts as setup_tts
