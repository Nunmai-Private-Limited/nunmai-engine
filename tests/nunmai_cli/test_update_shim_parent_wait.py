"""A child spawned off the Windows ``nunmai.exe`` shim waits for its parent before touching the venv.
...
from nunmai_cli import main_install_repair, update_handoff, update_lock
...
import psutil
from nunmai_cli import main as cli_main, update_cmd, update_handoff, update_lock
...
Path(os.environ["NUNMAI_TEST_READY"]).touch()
try:
...
def test_update_child_outwaits_shim_parent_then_owns_the_lock_and_the_resume_token(tmp_path):
    ...
    the child (a fresh ``nunmai update``) must not pause gateways, scan holders or claim the
    ...
    marker = nunmai_home / update_lock.MARKER_NAME
    marker.write_text(f"{parent.pid}\n{int(time.time())}\n", encoding="utf-8")
    env = {
        **os.environ, "NUNMAI_HOME": str(nunmai_home), "NUNMAI_TEST_READY": str(ready),
...
def test_shim_holder_is_the_launcher_ancestor_not_the_interpreter_it_ran():
    """``nunmai.exe`` spawns python.exe with the shim as argv[0] and exits only after reaping it,
