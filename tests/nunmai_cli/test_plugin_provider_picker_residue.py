...
Residue after picker admission: `nunmai model` had no flow for a plugin profile (selection was a silent
...
def plugin(monkeypatch):
    ...
    import providers
    from nunmai_cli import auth
    from nunmai_cli.auth_plugin_providers import PLUGIN_MIRRORED_PROVIDERS, register_plugin_provider
    ...
    monkeypatch.setattr("nunmai_cli.auth_plugin_providers.PLUGIN_MIRRORED_PROVIDERS", mirrored)
...
def test_nunmai_model_routes_registered_plugin_profiles_to_the_generic_flow(plugin, monkeypatch, tmp_path):
    """Selecting an admitted external-process or OAuth plugin in `nunmai model` persists config.model;
...
def test_oauth_plugin_status_follows_the_credential_pool(plugin):
    ...
    `nunmai auth add` hint. Bundled OAuth providers never reach this builder."""
    from nunmai_cli import auth, models
    ...
    assert "nunmai auth add example-oauth" in signed_out["hint"]
...
def test_any_external_process_plugin_counts_as_signed_in_when_its_binary_resolves(plugin, monkeypatch, tmp_path):
    ...
    from nunmai_cli import auth
    from nunmai_cli.inventory import _external_process_signed_in, _filter_explicit_provider_rows
