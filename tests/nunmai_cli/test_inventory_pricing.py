...
import nunmai_cli.inventory as inv
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (nunmai_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        ...
        assert observed["a"] == (
            nunmai_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            nunmai_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    token = set_nunmai_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
...
def test_prewarm_nous_rotation_when_another_provider_is_current(tmp_path, monkeypatch):
    """Nunmai endpoint identity must not depend on Nunmai being selected."""
    from nunmai_constants import (
        reset_nunmai_home_override,
        set_nunmai_home_override,
    )
