"""Tests for nunmai_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import nunmai_cli.gui_uninstall as gu


def _make_agent(nunmai_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = nunmai_home / "nunmai-engine"
    (agent_root / "nunmai_cli").mkdir(parents=True)
    (agent_root / "nunmai_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(nunmai_home: Path) -> None:
    """Create the source-built GUI artifacts a `nunmai desktop` run produces."""
    desktop = nunmai_home / "nunmai-engine" / "apps" / "desktop"
    ...
    (nunmai_home / "nunmai-engine" / "node_modules").mkdir(parents=True)
    (nunmai_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(nunmai_home: Path) -> None:
    (nunmai_home / "config.yaml").write_text("x: 1\n")
    (nunmai_home / ".env").write_text("KEY=secret\n")
    (nunmai_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(nunmai_home)
    _make_gui_build(nunmai_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(nunmai_home)
    ...
    assert summary["nunmai_home"] == str(nunmai_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `nunmai desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    nunmai_home = tmp_path / ".nunmai"
    _make_agent(nunmai_home)
    icon = lde.icon_path(nunmai_home / "nunmai-engine")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(nunmai_home)
    ...
    assert lde.icon_path(nunmai_home / "nunmai-engine").exists()
    # The agent itself survives a GUI uninstall.
    assert (nunmai_home / "nunmai-engine" / "nunmai_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".nunmai")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import nunmai_cli.uninstall as uninstall
