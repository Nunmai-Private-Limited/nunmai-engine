...
The bug class: on Windows, when `nunmai.exe`/sibling shims could not be
...
import nunmai_cli._install_repair as ir
import nunmai_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("nunmai", "nunmai-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(tmp_path, monkeypatch):
    ...
    # nunmai.exe cannot be renamed; nunmai-gateway.exe can
    real_rename = Path.rename

    def deny_nunmai(self, target):
        if self.name == "nunmai.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_nunmai)
    monkeypatch.setattr(main_install_repair, "_nunmai_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "nunmai.exe" in exc_info.value.failed_shims
    # The successful rename (nunmai-gateway.exe) was rolled back.
    assert (scripts / "nunmai-gateway.exe").exists()
    assert not list(scripts.glob("nunmai-gateway.exe.old.*"))


@pytest.mark.windows_only
def test_non_strict_keeps_warn_and_try(tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("nunmai",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("nunmai",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["nunmai.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    assert "nunmai.exe" in capsys.readouterr().out
