"""Tests for nunmai_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate NUNMAI_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from nunmai_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "Nunmai Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from nunmai_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://nunmai.in/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/Nunmai-Private-Limited/nunmai-engine"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import nunmai_cli.model_catalog as model_catalog
        from nunmai_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("nunmai_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("nunmai_cli.models.fetch_openrouter_models") as orm, \
             patch("nunmai_cli.models.fetch_nous_recommended_models") as nous:
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in nunmai_cli.models."""
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``nunmai model`` CLI — not the live
        ...
        # ``NUNMAI_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` NUNMAI_HOME directly instead.
        import importlib
        from nunmai_cli import model_catalog
        from nunmai_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from nunmai_cli.model_switch_providers import list_picker_providers

            active_home = Path(os.environ["NUNMAI_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("nunmai_cli.models.check_nous_free_tier", return_value=False), patch(
                "nunmai_cli.models.union_with_portal_free_recommendations",
