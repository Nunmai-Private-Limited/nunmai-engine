...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_PROFILE", "probe")
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: "nunmai gateway run")
...
def test_status_preserves_profile_health_contract(served_root, capsys, monkeypatch, mode):
    ...
    if mode == "local":
        monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda: [os.getpid()])
    if mode == "external":
    ...
    if mode in {"missing", "stale"}:
        assert "nunmai --profile default gateway restart" in output
    if mode == "missing":
    ...
    if mode in {"disabled", "excluded", "unrelated_pid"}:
        ...
        assert "nunmai --profile default gateway install" in output
        assert "sudo nunmai --profile default gateway install --system" in output
        assert "nunmai --profile default gateway run" in output
        ...
        assert "nunmai gateway install   # starts a SECOND gateway" not in output
    if mode == "external":
...
def test_host_record_rung_names_the_roster_and_a_runnable_restart(served_root, capsys, monkeypatch):
    ...
    assert "nunmai --profile default gateway restart" in output
    assert "\n  If heartbeat never appears, restart: nunmai gateway restart" not in output
...
def test_satellite_list_and_create_require_own_heartbeat(served_root, capsys, monkeypatch, heartbeat):
    ...
    for output in (created, listed):
        assert ("Check status:  nunmai cron status" in output) == (heartbeat != "fresh")
    cron.cron_status()
...
def test_doctor_bounds_persisted_fire_errors(served_root, capsys, detail):
    from cron import jobs
    from nunmai_cli.cron import cron_doctor
    ...
    if detail:
        ...
        assert f"nunmai cron run {records[0]['id']}" in line
...
def test_doctor_reports_persisted_dispatch_health(served_root, capsys, dispatch):
    ...
    assert "Review the findings above, then run `nunmai cron doctor` again." in output
