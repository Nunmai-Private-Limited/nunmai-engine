"""``/api/status`` agrees with ``nunmai gateway status`` on the two #113372 shapes:
...
def test_status_keeps_watchdog_degraded_verdict_and_reason_for_dead_pid(client, monkeypatch):
    ...
    # ``nunmai gateway stop`` afterwards records the operator's intent: no longer a current failure.
