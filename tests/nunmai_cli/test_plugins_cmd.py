"""Tests for nunmai_cli.plugins_cmd — the ``nunmai plugins`` CLI subcommand."""
...
class TestGitPullPluginDirAutostash:
    ...
    def test_dirty_checkout_pulls_and_reapplies_local_edit(self, tmp_path):
        import nunmai_cli.plugins_cmd as pc
    ...
    def test_conflicting_local_edit_is_preserved_in_stash(self, tmp_path):
        ...
        assert "nunmai-plugin-update-autostash" in stash_list
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from nunmai_cli.plugins_cmd import cmd_install

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("nunmai_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("nunmai_cli.plugins_cmd._display_after_install")
    @patch("nunmai_cli.plugins_cmd.shutil.move")
    @patch("nunmai_cli.plugins_cmd.rmtree_readonly")
    @patch("nunmai_cli.plugins_cmd._plugins_dir")
    @patch("nunmai_cli.plugins_cmd._read_manifest")
    @patch("nunmai_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("nunmai_cli.plugins_cmd._sanitize_plugin_name")
    @patch("nunmai_cli.plugins_cmd._plugins_dir")
    def test_update_plugin_not_found(self, mock_plugins_dir, mock_sanitize):
        from nunmai_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_plugin_not_found(self, mock_plugins_dir, mock_sanitize):
        from nunmai_cli.plugins_cmd import cmd_remove
    ...
    def test_remove_plugin_core_deletes_read_only_git_tree(self, tmp_path):
        ...
        from nunmai_cli.plugins_cmd import _remove_plugin_core
...
class TestCopyExampleFiles:
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "nunmai_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_prompts_for_missing_var_rich_format(self):
        from nunmai_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock, patch
        ...
        with patch("nunmai_cli.config.get_env_value", return_value=None), \
             patch("builtins.input", return_value="pk-lf-123"), \
             patch("nunmai_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_masked_prompt(self):
        ...
        with patch("nunmai_cli.config.get_env_value", return_value=None), \
             patch("nunmai_cli.plugins_cmd.masked_secret_prompt", return_value="s3cret") as mock_prompt, \
             patch("nunmai_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
...
class TestProviderDiscovery:
    ...
    def test_save_context_engine(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli.plugins_cmd import _save_context_engine
    ...
    def test_discover_context_engines_empty(self):
        ...
        with patch("plugins.context_engine.discover_context_engines",
            ...
            from nunmai_cli.plugins_cmd import _discover_context_engines
...
class TestSubdirInstallE2E:
    ...
    def test_installs_only_the_subdir_plugin(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli import plugins_cmd as pc
...
def test_portable_manifest_is_visible_to_plugin_cli(tmp_path):
    ...
    from nunmai_cli.agent_plugins import PLUGIN_SCHEMA_V1
    from nunmai_cli.plugins_cmd import _read_manifest_info
...
def test_autostash_dirty_tree_promotes_intent_to_add_entries(tmp_path):
    ...
    Same class as the `nunmai update` autostash: an intent-to-add entry is never "uptodate", so
    ...
    from nunmai_cli.plugins_cmd import _autostash_dirty_tree
...
def test_toggle_plugin_toolset_rewrites_a_list_literal_string_platform_entry(tmp_path, monkeypatch):
    """``nunmai plugins enable`` must reach a platform whose ``platform_toolsets`` entry is the
    list-literal string an older ``nunmai config set`` stored, and re-save it as a real list —
    ...
    (tmp_path / "config.yaml").write_text(
        yaml.safe_dump({"platform_toolsets": {"cli": '["web", "terminal"]', "telegram": ["nunmai-telegram"]}}),
        encoding="utf-8")
    ...
    assert saved["telegram"] == ["nunmai-telegram", "my-plugin"]
