"""Tests for issue #26670 — concurrent nunmai.exe detection and improved
quarantine retry / reboot-deferred fallback during `nunmai update` on Windows.
...
def _make_proc(pid: int, exe: str, name: str = "nunmai.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "nunmai.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_nunmai_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "nunmai.exe")]
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    pairs = main_install_repair._quarantine_running_nunmai_exe(tmp_path)
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="NunmaiGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="NunmaiGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    captured = capsys.readouterr().out
    assert "Another nunmai.exe is running" not in captured
