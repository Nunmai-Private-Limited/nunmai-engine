"""`nunmai update` concurrent-instance gate and Windows gateway service helpers
...
# Tests in this module either exercise the REAL _detect_concurrent_nunmai_instances
# helper (and need the autouse stub in tests/nunmai_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_nunmai_instances — parent-chain exclusion
#
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(tmp_path, monkeypatch):
    ...
    rows = [
        ...
        (launcher_pid, str(shim), "nunmai.exe"),
        (other_pid, str(shim), "nunmai.exe"),
    ...
    result = dashboard_procs._detect_concurrent_nunmai_instances(tmp_path)

    assert result == [(other_pid, "nunmai.exe")]


# ---------------------------------------------------------------------------
# _quarantine_running_nunmai_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(tmp_path):
    ...
    pairs = main_install_repair._quarantine_running_nunmai_exe(
    ...
    assert orig == shim
    assert quarantine.name.startswith("nunmai.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_nunmai_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "nunmai_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "nunmai_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import nunmai_cli.gateway as gateway_mod
    import nunmai_cli.update_cmd_windows as update_cmd_windows
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["NunmaiGateway"],
        "expected_services": ["NunmaiGateway"],
        "restarted_services": [],
        "service_profiles": {"NunmaiGateway": "default"},
    }
    assert stopped == ["NunmaiGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["NunmaiGateway"]
    assert token["resume_needed"] is False


def _two_services():
    return [
        SimpleNamespace(name="NunmaiGateway", profile="default", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="NunmaiGatewayPicasso", profile="picasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    with pytest.raises(RuntimeError, match="NunmaiGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["NunmaiGatewayPicasso", "NunmaiGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: NunmaiGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="NunmaiGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["NunmaiGateway"]
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("NunmaiGateway")

    assert restarted == ["NunmaiGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m nunmai_cli.main gateway run"
...
# when every concurrent nunmai.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, nunmai.exe shim, nunmai-gateway.exe,
    gateway/run.py, bare `nunmai gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "nunmai_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\nunmai.exe", "gateway", "run"],
        [r"C:\venv\Scripts\nunmai-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["nunmai.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["nunmai.exe", "gateway"],  # bare `nunmai gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["nunmai.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\nunmai.exe"],  # interactive REPL
        [r"C:\venv\Scripts\nunmai.exe", "dashboard"],
        ["nunmai.exe", "gateway", "status"],  # management, not runtime
        ["nunmai.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["nunmai.exe", "gateway", "run"],
                200: ["nunmai.exe"],  # REPL — keep
                300: ["nunmai.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "nunmai.exe"), (300, "nunmai.exe"), (400, "nunmai.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["nunmai.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\nunmai-gateway.exe"],
            }
        ),
    )
    matches = [(111, "nunmai.exe"), (222, "nunmai-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                1000: ["nunmai.exe", "gateway", "run"],
                2000: [r"C:\venv\Scripts\nunmai-gateway.exe"],
            }
        ),
    )
    ...
    with patch.object(
        ...
        return_value=[(1000, "nunmai.exe"), (2000, "nunmai-gateway.exe")],
    ), patch.object(
    ...
    assert "Another nunmai.exe is running" not in capsys.readouterr().out
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    monkeypatch.setitem(
        ...
        "psutil",
        _fake_psutil_classify(
            {1000: ["nunmai.exe", "gateway", "run"], 3000: ["nunmai.exe"]}
        ),
    )
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_nunmai_instances",
        return_value=[(1000, "nunmai.exe"), (3000, "nunmai.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m nunmai_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "NunmaiGateway", expected_service_identity=(11, 11.0)
        )
