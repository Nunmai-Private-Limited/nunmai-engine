...
``_write_profile_model`` used to enter only the NUNMAI_HOME override. Once the dashboard
...
def homes(tmp_path, monkeypatch):
    """A throwaway NUNMAI_HOME whose named profile carries its own ``.env`` credential.
...
def probe(monkeypatch):
    ...
    import nunmai_cli.models_validate as mv
...
def test_model_pick_resolves_key_env_from_profile_scope(client, homes, probe):
    ...
    with _nunmai_home_scope(homes[1]):
...
def test_model_pick_for_default_from_named_profile_launch(homes, probe, monkeypatch):
    ...
    root, demo = homes
    monkeypatch.setenv("NUNMAI_HOME", str(demo))
    (root / ".env").write_text("ACME_RELAY_KEY=root-key\n", encoding="utf-8")
    from nunmai_cli.config import invalidate_env_cache, load_config
    from nunmai_cli.web_server_profiles import _nunmai_home_scope
    invalidate_env_cache()
    from nunmai_cli import web_server
    ...
    with _nunmai_home_scope(root):
        assert (load_config().get("model") or {}).get("default") == "acme/mini"
    with _nunmai_home_scope(demo):
...
def test_model_set_names_target_from_a_backend_launched_as_another_profile(homes, probe, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(demo))  # launched as A = demo
    invalidate_env_cache()
    from nunmai_cli import web_server
