...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_nunmai_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from nunmai_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """nunmai -z bypasses NunmaiCLI, but recall still needs SessionDB."""
    from nunmai_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "nunmai_state_registry", mod("nunmai_state_registry", acquire=fake_acquire))
    monkeypatch.setitem(
        sys.modules,
        "nunmai_cli.config",
        mod("nunmai_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "nunmai_cli.models",
        mod("nunmai_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("nunmai_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["NUNMAI_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/nunmai-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["NUNMAI_MODEL"] == "nous/nunmai-test"
    assert env["NUNMAI_INFERENCE_MODEL"] == "nous/nunmai-test"
    assert env["NUNMAI_TUI_PROVIDER"] == "nous"
    assert env["NUNMAI_INFERENCE_PROVIDER"] == "nous"
    assert env["NUNMAI_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["NUNMAI_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("nunmai-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_nunmai_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("NUNMAI_TUI_DIR", raising=False)
