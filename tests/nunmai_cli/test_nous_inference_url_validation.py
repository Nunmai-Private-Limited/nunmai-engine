...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.auth"):
            assert (
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        # The Nunmai refresh sites live in auth_nous.py (split out of auth.py);
        # read both so the guard tolerates relocation but still fires on deletion.
        import nunmai_cli.auth as _auth_mod
        import nunmai_cli.auth_nous as _nous_mod
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import nunmai_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestHealsPoisonedStoredValue:
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        import nunmai_cli.auth_nous as nunmai_cli_auth_nous
        ...
        monkeypatch.setattr(nunmai_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(nunmai_cli_auth_nous, "_assert_nous_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_nous_invoke_jwt", lambda *a, **k: None)
        monkeypatch.setattr(nunmai_cli_auth_nous, "_select_nous_invoke_jwt", lambda *a, **k: None)
...
class TestEnvOverrideWins:
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(nunmai_cli_auth_nous, "_nous_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(nunmai_cli_auth_nous, "_write_shared_nous_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
        monkeypatch.setattr(nunmai_cli_auth_nous, "_sync_nous_pool_from_auth_store", lambda *a, **k: None)
