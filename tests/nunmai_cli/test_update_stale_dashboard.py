"""Tests for the stale-dashboard handling run at the end of ``nunmai update``.

``nunmai update`` detects ``nunmai dashboard`` processes left over from the
...
from nunmai_cli.main_dashboard import _find_stale_dashboard_pids
from nunmai_cli.dashboard_procs import _kill_stale_dashboard_processes
from nunmai_cli import dashboard_procs
from nunmai_cli import main_dashboard
from nunmai_cli import update_cmd
from nunmai_cli.update_cmd import _finish_dashboard_update_cleanup
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "nunmaiPath": "/opt/nunmai/bin/nunmai",
        "nunmaiHome": str(tmp_path),
    ...
    monkeypatch.delenv("NUNMAI_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "nunmai_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m nunmai_cli.main dashboard"),
                    _ps_line(12345, "nunmai dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("nunmai_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "nunmai-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="nunmai-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "nunmai-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "nunmai-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "nunmai-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("nunmai_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]), \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert ["systemctl", "--user", "restart", "nunmai-dashboard.service"] in calls
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("nunmai_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("nunmai_cli._subprocess_compat.pid_is_nunmai", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(own_home))
        with patch(
            "nunmai_cli.main._kill_stale_dashboard_processes",
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/nunmai-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="nunmai-serve.service"), \
        ...
        restart.assert_called_once_with(
            "nunmai-serve.service", "/system.slice/nunmai-serve.service"
        )
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: nunmai update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"nunmai-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["nunmai", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("nunmai_cli.dashboard_procs._nunmai_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["nunmai", "dashboard", "--port", "8300"],
                ["nunmai", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["nunmai", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["nunmai", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["nunmai", "serve"]])

        assert failed == [["nunmai", "serve"]]
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from nunmai_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.nunmai/profiles/mini-cat"),
        ]) == []
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "nunmai_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "nunmai_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["nunmai", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["nunmai", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["nunmai", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_nunmai_home(self):
        from nunmai_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/nunmai-home-a"
        a = ["nunmai", "dashboard", "--port", "8300"]
        b = ["nunmai", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.nunmai/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["nunmai", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["nunmai", "dashboard", "--port", "8301"]
        home = "/home/u/.nunmai"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another NUNMAI_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.nunmai"),
                (2, b, "/work/project/.nunmai"),
            ],
            own_home="/home/u/.nunmai",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.nunmai-sidecar/nunmai-engine/venv/bin/python",
            "-m", "nunmai_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.nunmai-lifeos")],
            own_home="/Users/u/.nunmai",
        )
        assert out == []

    def test_matching_nunmai_home_is_kept(self):
        from nunmai_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["nunmai", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.nunmai")],
            own_home="/Users/u/.nunmai",
        )
        assert out == [argv]

    def test_symlinked_nunmai_home_compares_equal(self, tmp_path):
        from nunmai_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable NUNMAI_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from nunmai_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_nunmai_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            nunmai_constants, "get_nunmai_home", lambda: Path("/home/u/.nunmai")
        )
        argv = ["nunmai", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.nunmai-lifeos"),
        ])
        assert foreign == []
        own = _filter_dashboard_respawn_candidates([
            (1, argv, "/home/u/.nunmai"),
        ])
        assert own == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00nunmai_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "nunmai_cli.main", "serve"]
...
class TestPostUpdateDashboardCleanupIsolation:
    ...
    def test_cleanup_failure_is_isolated_and_recorded(self, capsys):
        ...
        import nunmai_cli.update_receipt as ur
        from nunmai_cli import update_cmd
        ...
        try:
            ...
            with patch(
                ...
                side_effect=AttributeError(
                    "module 'nunmai_cli.main_dashboard' has no attribute '_loaded_launchd_backend_jobs'"
                ),
            ):
...
class TestLaunchdSupervisedBackends:
    ...
    ARGV = [
        "/opt/nunmai/venv/bin/python", "-m", "nunmai_cli.main",
    ...
    def _run(self, pid, jobs, *, restart_ok=True, restart_managed=True):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("nunmai_cli.dashboard_procs._process_ancestors", return_value=[]), \
             patch("nunmai_cli.dashboard_procs._nunmai_home_for_pid", return_value=None), \
             patch.object(live, "_respawn_dashboard_processes", return_value=[]) as respawn, \
    ...
    def test_launchd_owned_backend_restarts_through_launchd_never_as_a_detached_respawn(self, capsys):
        ...
        job = ("system", "ai.nunmai.dashboard", list(self.ARGV), None)
        result, restart, respawn = self._run(9102, [job])
        respawn.assert_not_called()
        restart.assert_called_once_with("system", "ai.nunmai.dashboard", None)
        assert result["killed"] == [9102] and result["unrecovered"] == []
        assert "when you're ready" not in capsys.readouterr().out

        result, restart, respawn = self._run(9103, [("gui/501", "ai.nunmai.dashboard", list(self.ARGV), 9103)],
                                             restart_ok=False)
        respawn.assert_not_called()
        assert result["unrecovered"] == [9103]
        assert "run: launchctl kickstart -k gui/501/ai.nunmai.dashboard" in capsys.readouterr().out

        # A LaunchDaemon lives in the system domain: kickstart needs root, so a non-root hint says sudo.
        with patch("nunmai_cli.dashboard_procs.os.geteuid", return_value=501, create=True):  # windows-footgun: ok — patch target string, create=True
            self._run(9105, [("system", "ai.nunmai.serve", list(self.ARGV), None)], restart_ok=False)
        assert "run: sudo launchctl kickstart -k system/ai.nunmai.serve" in capsys.readouterr().out

        other = ("gui/501", "ai.nunmai.other", ["nunmai", "dashboard", "--port", "8300"], 777)
    ...
    def test_launchd_job_attribution_is_by_live_pid_ancestor_or_exact_argv(self):
        ...
        never otherwise."""
        uid = 501
        backend_argv = ["/opt/nunmai/venv/bin/python", "-m", "nunmai_cli.main", "dashboard", "--port", "9119"]
        serve_argv = ["/opt/nunmai/venv/bin/python", "-m", "nunmai_cli.main", "serve", "--port", "8642"]
        jobs = [
            (f"user/{uid}", "ai.nunmai.dashboard", backend_argv, 4242),
            ("system", "ai.nunmai.serve", serve_argv, None),
        ]

        owning = main_dashboard._launchd_job_owning_backend
        assert owning(4242, ["something", "else"], jobs) == (f"user/{uid}", "ai.nunmai.dashboard", 4242)
        assert owning(9999, ["python"], jobs, ancestors=[4242, 1]) == (f"user/{uid}", "ai.nunmai.dashboard", 4242)
        assert owning(9999, serve_argv, jobs) == ("system", "ai.nunmai.serve", None)
        ...
        assert owning(9999, backend_argv + ["--no-open"], jobs) == (f"user/{uid}", "ai.nunmai.dashboard", 4242)
