...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``nunmai_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``nunmai_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from nunmai_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"nunmai_cli.main.X\")``
    ...
    live = sys.modules.get("nunmai_cli.main")
    if live is None:
        live = importlib.import_module("nunmai_cli.main")
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "nunmai_cli.main._find_stale_dashboard_pids",
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("nunmai_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["nunmai_cli.main"]
