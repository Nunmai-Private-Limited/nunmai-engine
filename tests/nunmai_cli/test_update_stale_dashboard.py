...
from nunmai_cli.main_dashboard import _restart_managed_dashboard_service
from nunmai_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
class TestKillStaleDashboardPosix:
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("nunmai_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "nunmai-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "nunmai-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "nunmai-dashboard.service"],
            ["systemctl", "--user", "restart", "nunmai-dashboard.service"],
        ]
        ...
        assert "✓ restarted nunmai-dashboard.service" in capsys.readouterr().out
...
class TestWindowsWmicEncoding:
    ...
    `nunmai update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("nunmai_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m nunmai_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("nunmai_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "✓ restarted systemd service nunmai-serve.service" in out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["nunmai", "dashboard", "--port", "9119"]
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["nunmai", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``nunmai update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``nunmai_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from nunmai_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("nunmai_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import nunmai_cli._subprocess_compat as compat
        from nunmai_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["nunmai_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["nunmai_cli._subprocess_compat"])
            importlib.reload(sys.modules["nunmai_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "nunmai_cli._subprocess_compat" in reloaded
        assert "nunmai_cli.dashboard_procs" in reloaded
