...
from nunmai_cli import main as nunmai_main
import nunmai_cli.main_provider_setup as nunmai_cli_main_provider_setup
from nunmai_cli import model_switch
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.config.load_config", lambda: config)
    monkeypatch.setattr("nunmai_cli.runtime_provider.load_config", lambda: config)

    shell = cli.NunmaiCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "nunmai_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("nunmai_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr("run_agent.AIAgent", _DummyAgent)

    shell = cli.NunmaiCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_ensure_runtime_credentials_passes_cli_model_as_target_model(monkeypatch):
    """`nunmai -m mimo-v2.5 --provider opencode-go` must resolve credentials for the model the
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.NunmaiCLI(model="mimo-v2.5", provider="opencode-go", compact=True, max_turns=1)
...
def _quota_auth_error():
    from nunmai_cli.auth import CODEX_RATE_LIMITED_CODE, AuthError
    return AuthError(
...
@pytest.mark.parametrize(("exc_factory", "expected", "absent"), [
    ...
    (lambda: __import__("nunmai_cli.auth", fromlist=["AuthError"]).AuthError(
...
def test_fallback_runtime_labels_quota_outage_and_bad_credentials_distinctly(monkeypatch, tmp_path, exc_factory, expected, absent):
    ...
    monkeypatch.setattr("nunmai_cli.fallback_config.resolve_entry_api_key", lambda entry: "k")
...
def test_ensure_runtime_credentials_records_quota_vs_bad_key(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _raise_quota)
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _raise_missing)
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "nunmai"
    config_home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "nunmai_cli.auth.get_provider_auth_state",
        lambda provider: {
    ...
    monkeypatch.setattr(
        "nunmai_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "nunmai_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("nunmai_cli.models_pricing.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("nunmai_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "nunmai_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "nunmai_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "nunmai_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    nunmai_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.NunmaiCLI(compact=True, max_turns=1)
...
def test_startup_fallback_re_resolves_reasoning_for_the_fallback_model(monkeypatch, reasoning_flag, expected_effort):
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.NunmaiCLI(compact=True, max_turns=1, reasoning=reasoning_flag)
...
def test_custom_entry_model_swap_re_resolves_reasoning(monkeypatch):
    """`nunmai chat --model <custom-provider-name>`: the runtime's explicit `model` replaces the
    ...
    shell = cli.NunmaiCLI(model="my-lan", compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch):
    monkeypatch.setattr(
        "nunmai_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("nunmai_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("nunmai_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("nunmai_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("nunmai_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("nunmai_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    caller_cfg = {}
    nunmai_main._model_flow_custom(caller_cfg)
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("nunmai_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "nunmai_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("nunmai_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("nunmai_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "nunmai_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "nunmai_cli.main_provider_setup._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("nunmai_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    nunmai_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(nunmai_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "nunmai_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "nous"}},
    )
    monkeypatch.setattr("nunmai_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("nunmai_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("nunmai_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("nunmai_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("nunmai_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(nunmai_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    monkeypatch.setattr(nunmai_cli_main_provider_setup, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("nunmai_cli.auth._login_nous", _fake_login)

    nunmai_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="nunmai-local",
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from nunmai_cli.main_provider_setup import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "nunmai_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("nunmai_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("nunmai_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="NUNMAI_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "NUNMAI_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from nunmai_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
