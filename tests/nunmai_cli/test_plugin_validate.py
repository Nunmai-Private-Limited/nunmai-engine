"""Tests for ``nunmai plugins validate`` (nunmai_cli/plugin_validate.py).
...
from nunmai_cli.plugin_validate_desktop import desktop_surface_hits, is_desktop_surface
...
def _portable_plugin(root: Path, servers: dict, declarations: dict) -> Path:
    ...
    (root / "plugin.json").write_text(json.dumps({
        ...
        "name": "example-plugin",
        "extensions": {"com.nousresearch.nunmai": {"servers": declarations}},
    }), encoding="utf-8")
...
def test_requires_nunmai_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_nunmai=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert any(name == "requires_nunmai" and ok for name, ok, _ in report.checks)
...
def test_config_schema_admits_every_type_the_loader_and_renderer_accept(tmp_path):
    ...
    from nunmai_cli.plugins_settings import _FIELD_TYPES
...
class TestRequiresNunmaiSpec:
    """A typo'd ``requires_nunmai`` clause must fail admission, not silently gate nothing."""

    def test_typoed_clause_fails_admission(self, tmp_path):
        d = _make_plugin(
            tmp_path, manifest={**BASE_MANIFEST, "requires_nunmai": ">=0.21.1,<0.x"}
        )
        report = validate_plugin_dir(d)
        assert not report.ok
        assert any(
            "requires_nunmai" in f and "does not parse" in f for f in report.failures
        ), report.failures
...
class TestDesktopSurface:
    ...
    def test_sdk_only_plugin_passes(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            "import { definePlugin } from '@nunmai/plugin-sdk'\n"
    ...
    def test_prototype_patch_and_chunk_import_fail(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            ...
            "const sdk = await import('@nunmai/plugin-sdk')\n"
        ))
        report = validate_plugin_dir(d)
        failed = {name: detail for name, ok, detail in report.checks if not ok}
        assert "desktop surface" in failed
