"""Operator authority over Nunmai routing overrides and Portal-returned inference hosts.
...
* Profile authority — ``NOUS_INFERENCE_BASE_URL`` / ``NUNMAI_PORTAL_BASE_URL`` are per-profile
...
OTHER_NOUS_HOST = "https://portal.nousresearch.com/v1"  # Nunmai-owned, but not an inference gateway
...
@pytest.mark.parametrize("helper, var", [
    ...
    (auth_nous._nous_portal_env_override, "NUNMAI_PORTAL_BASE_URL"),
])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Another Nunmai-owned host is not the named recipient — refused, DNS suffix or not.
...
def test_portal_override_alone_grants_nothing(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_PORTAL_BASE_URL", NONPROD_PORTAL)
    monkeypatch.delenv("NOUS_INFERENCE_BASE_URL", raising=False)
