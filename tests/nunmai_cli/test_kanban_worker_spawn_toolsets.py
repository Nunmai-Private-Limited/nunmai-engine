...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    NUNMAI_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["NUNMAI_HOME"] == str(profile)
    assert captured["env"]["NUNMAI_KANBAN_TASK"] == "t_spawn_tools"
...
""".lstrip(),
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "t_spawn_tools")
