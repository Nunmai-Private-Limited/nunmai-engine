"""`nunmai sessions set-journal-mode` converts an existing WAL store offline and refuses under a foreign holder.
...
from nunmai_cli.sessions_cmd_journal_mode import _refusal
...
def test_set_journal_mode_converts_wal_store_offline(tmp_path, monkeypatch, capsys):
    ...
    _wal_store(db)
    monkeypatch.setattr("nunmai_state.DEFAULT_DB_PATH", db)
...
@pytest.mark.parametrize("target,current,platform,cross_vm,force,expected", [
    ...
    ("delete", "not-a-database", "linux", False, False, "not a Nunmai SQLite store"),
    ("delete", "unknown(3/3)", "linux", False, True, "not a Nunmai SQLite store"),
])
