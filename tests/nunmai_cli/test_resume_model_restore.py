...
SessionDB.session_gateway_runtime (nunmai_state.py) — the round trip that
makes `nunmai --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare NunmaiCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.NunmaiCLI)
...
def test_restore_llamacpp_session_keeps_launch_base_url_for_same_provider(monkeypatch):
    """`nunmai --provider llamacpp --base-url X --resume` is user intent for the SAME provider the
