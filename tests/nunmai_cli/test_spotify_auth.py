...
import nunmai_cli.auth_spotify as auth_spotify
from nunmai_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from nunmai_cli.auth_commands import auth_status_command
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    Mirrors Nunmai / xAI-OAuth / Codex-OAuth / MiniMax quarantine pattern.
    """
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
