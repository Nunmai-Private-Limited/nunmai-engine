...
reset).  Nunmai must detect that and lift the stale local cooldown instead
...
def _write_auth_store(nunmai_home, payload):
...
def test_resolver_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    _write_auth_store(nunmai_home, _pool_only_rate_limited_store())
    ...
    store = json.loads((nunmai_home / "auth.json").read_text())
...
def test_resolver_selects_entry_with_expired_millisecond_reset(tmp_path, monkeypatch):
    ...
    nunmai_home = tmp_path / "nunmai"
    _write_auth_store(nunmai_home, store)
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
...
def test_pool_probe_not_fired_for_non_quota_exhaustion(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path / "nunmai", store)
...
def test_resolver_refreshes_expired_token_before_probe(tmp_path, monkeypatch):
    ...
    now = time.time()
    nunmai_home = tmp_path / "nunmai"
    _write_auth_store(nunmai_home, _expired_jwt_pool_store(now))
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    ...
    entry = json.loads((nunmai_home / "auth.json").read_text())["credential_pool"]["openai-codex"][0]
...
def test_pool_selection_refreshes_expired_token_before_probe(tmp_path, monkeypatch):
    ...
    disk = json.loads((nunmai_home / "auth.json").read_text())
