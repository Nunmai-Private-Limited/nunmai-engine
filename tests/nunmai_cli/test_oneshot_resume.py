"""Tests for `nunmai -z --resume <session>` (#105892).
...
from nunmai_cli.oneshot import (
...
class TestLoadResumeTarget:
    ...
    def test_empty_session_keeps_resolved_id(self, tmp_path):
        ...
        # re-minted a session for `nunmai -z "hello" -c <title> --create-if-missing`,
...
class TestRunAgentResumeRuntime:
    ...
    def test_run_agent_uses_stored_runtime_and_reopens_row(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.config.load_config", lambda: {"model": {"default": "ambient-model", "provider": "openrouter"}})
        monkeypatch.setattr(
...
class TestRunOneshotForwardsResume:
    def test_resume_kwarg_reaches_run_agent(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.oneshot._run_agent", _fake_run_agent)
