"""Tests for nunmai_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                ...
                # nunmai_constants._get_platform_default_nunmai_home.
                ...
                assert home == base / "nunmai"
            else:
                assert home == Path.home() / ".nunmai"


class TestEnsureNunmaiHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from nunmai_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Nunmai Engine, an intelligent AI assistant created by Nunmai "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``nunmai logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from nunmai_cli.config_read_errors import _CONFIG_PARSE_WARNED
...
class TestLoadEnvInlineComments:
    def test_unquoted_hash_is_a_comment_quoted_hash_is_data(self, tmp_path):
        ...
        is a comment, a quoted value keeps its hash. Nunmai' own writer (_quote_env_value) always quotes
        values containing `#`, so a saved secret round-trips."""
        from nunmai_cli.config import invalidate_env_cache
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("nunmai_cli.config.sanitize_env_file", return_value=2),
            patch(
                "nunmai_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("nunmai_cli.config.read_raw_config", return_value={}),
            patch("nunmai_cli.config.get_missing_env_vars", return_value=[]),
            patch("nunmai_cli.config.get_missing_config_fields", return_value=[]),
            patch("nunmai_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """NUNMAI_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `nunmai setup`) lets a
        ...
        via config.yaml; NUNMAI_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from nunmai_cli.config import OPTIONAL_ENV_VARS
        assert "NUNMAI_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestConfigMigrationSecretPrompts:
    def test_required_secret_env_prompt_uses_masked_prompt(self, tmp_path, monkeypatch):
        from nunmai_cli import config as cfg_mod
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `nunmai setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_nunmai_home(self):
        from nunmai_cli.config_migrations import support_floor_message
        from nunmai_constants import display_nunmai_home

        msg = support_floor_message()
        assert f"{display_nunmai_home()}/config.yaml" in msg
...
class TestRetiredMultiplexAllowlist:
    def test_v43_drops_multiplex_profile_allowlist_from_user_config(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Nunmai runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Nunmai
    process on next startup via the dotenv → ``os.environ`` chain in
    ``nunmai_cli/env_loader.py``.
    ...
    def _nunmai_home(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
        ensure_nunmai_home()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NUNMAI_SPOTIFY_CLIENT_ID",
            "NUNMAI_QWEN_BASE_URL",
            "NUNMAI_MAX_ITERATIONS",
        ],
    )
    def test_nunmai_integration_keys_still_writable(self, allowed_key):
        """``NUNMAI_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NUNMAI_CRON_SESSION",
            "NUNMAI_SINGLE_QUERY_SESSION",
            "NUNMAI_SESSION_KEY",
            "NUNMAI_SESSION_PLATFORM",
        ],
    )
    def test_nunmai_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        (tmp_path / ".env").write_text(
            f"NUNMAI_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["NUNMAI_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Nunmai_Yolo_Mode", "NUNMAI_YOLO_MODE"),
            ("Nunmai_Optional_Mcps", "NUNMAI_OPTIONAL_MCPS"),
            ("Nunmai_Copilot_Acp_Command", "NUNMAI_COPILOT_ACP_COMMAND"),
            ("Nunmai_Copilot_Acp_Args", "NUNMAI_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from nunmai_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from nunmai_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from nunmai_cli.config import _persist_migration
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: nunmai\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "nunmai"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import nunmai_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`nunmai config set/unset` must exit cleanly (no traceback) when the
    ...
    def test_config_command_set_exits_cleanly_on_broken_yaml(self, tmp_path, capsys):
        ...
        err = capsys.readouterr().err
        assert "formatting error" in err and "`nunmai config edit`" in err
        assert config_path.read_text(encoding="utf-8") == original
...
def test_gateway_multiplex_keys_are_recognized_config_keys():
    """``nunmai config set gateway.multiplex_profiles true`` used to warn 'not a recognized config
    ...
    from nunmai_cli.config_defaults import DEFAULT_CONFIG
...
def test_lsp_root_policy_keys_are_recognized_and_off_by_default():
    ...
    ``nunmai config set`` and must default to today's behaviour (no grace, lifetime broken set, no exclusion)."""
    from nunmai_cli.config import _validate_config_key
...
class TestCompatibleProvidersMalformedLegacyKey:
    ...
    def test_string_custom_providers_keeps_providers_view_and_warns(self, caplog):
        from nunmai_cli.config_providers import get_compatible_custom_providers

        config = {
            "custom_providers": "- name: broken",
            "providers": {"exl3": {"api": "http://127.0.0.1:8290/v1", "default_model": "m"}},
        }
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.config"):
