"""Contract tests for nunmai_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp NUNMAI_HOME (the autouse
...
from nunmai_cli.local_runtime.binaries import (
    AssetPlan,
    BinaryResolutionError,
    resolve_assets,
    select_backend,
)
from nunmai_cli.local_runtime.detect import DetectedServer, probe_port
...
def test_sha256_mismatch_rejects(tmp_path, monkeypatch):
    ...
    from nunmai_cli.local_runtime import binaries
...
def _make_supervisor(tmp_path, port):
    ...
    from nunmai_cli.local_runtime.supervisor import LlamaServerSupervisor
...
def test_llamacpp_endpoint_resolution_prefers_managed(tmp_path, monkeypatch, stub_server):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    port, handler = stub_server
    from nunmai_cli.local_runtime import endpoint as ep
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from nunmai_cli.local_runtime import endpoint as ep
    from nunmai_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from nunmai_cli.local_runtime.supervisor import state_path
    ...
    monkeypatch.setattr("nunmai_cli.local_runtime.detect.DEFAULT_PROBE_PORTS",
                        (dead_port,))
...
def test_llamacpp_dead_server_raises_friendly_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.local_runtime.endpoint.resolve_llamacpp_endpoint",
        lambda *a, **k: None)
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    from nunmai_cli.local_runtime import bootstrap as bs
    from nunmai_cli.local_runtime import endpoint as ep
    from nunmai_cli.local_runtime.supervisor import state_path
...
def test_staged_models_requires_every_split_part(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    import nunmai_cli.local_runtime.bootstrap as bs
...
def test_bootstrap_skips_boot_with_no_staged_models(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.local_runtime.binaries.ensure_runtime_installed", _boom)
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    from nunmai_cli.local_runtime import supervisor as sup_mod
    from nunmai_cli.local_runtime.supervisor import LlamaServerSupervisor
    ...
    key_file = tmp_path / ".nunmai" / "runtimes" / "llamacpp" / ".api_key"
...
def test_runtime_provider_seam_llamacpp_alias(tmp_path, monkeypatch, stub_server):
    ...
    from nunmai_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_staged_local_model_resolves_without_a_running_server(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    _stage_local_model(tmp_path / ".nunmai", "Qwen3.8-27B-IQ3_S-mtp")
    monkeypatch.setattr("nunmai_cli.local_runtime.detect.DEFAULT_PROBE_PORTS", ())

    from nunmai_cli.providers import LLAMACPP_PROVIDER_ID, resolve_provider_full
...
def test_bootstrap_disabled_is_noop(tmp_path, monkeypatch):
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
    from nunmai_cli.local_runtime import bootstrap
...
def test_bootstrap_reuses_running_server(tmp_path, monkeypatch, stub_server):
    ...
    called = []
    monkeypatch.setattr(
        "nunmai_cli.local_runtime.binaries.ensure_runtime_installed",
...
def test_bootstrap_failure_never_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.local_runtime.binaries.ensure_runtime_installed", boom)
...
def test_ensure_local_runtime_serializes_racing_callers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bootstrap, "_detect_gpu_vendor", lambda: None)
    monkeypatch.setattr("nunmai_cli.local_runtime.binaries.installed_tags", lambda: ["b1"])
    monkeypatch.setattr("nunmai_cli.local_runtime.binaries.default_tag", lambda: "b1")
    monkeypatch.setattr("nunmai_cli.local_runtime.binaries.ensure_runtime_installed",
...
def test_ensure_local_runtime_proceeds_when_boot_lock_is_unwritable(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr("nunmai_cli.local_runtime.endpoint._state_endpoint", lambda: None)
    monkeypatch.setattr("nunmai_cli.local_runtime.binaries.installed_tags", lambda: [])
...
def test_manifest_verified_tolerates_non_dict_manifest(tmp_path):
    ...
    from nunmai_cli.local_runtime.binaries import manifest_verified
