...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from nunmai_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "nunmai" / "auth.json").read_text())
...
def test_auth_add_migrates_legacy_prefixed_key_for_configured_provider(
    ...
    payload = json.loads((nunmai_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("nunmai_cli.models.clear_provider_models_cache"):
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from nunmai_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_normalizes_display_name_to_provider_key(
    ...
    from nunmai_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `nunmai auth add nous` must also populate providers.nous so the
    ...
    # raises "Nunmai is not logged into Nunmai Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`nunmai auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``nunmai auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``nunmai auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("nunmai_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_auth_add_codex_warns_when_login_is_same_account_as_pooled_entry(tmp_path, monkeypatch, capsys):
    """A second ``nunmai auth add openai-codex`` for the SAME OpenAI account must tell the user
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from nunmai_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from nunmai_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``nunmai auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "nunmai" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`nunmai logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from nunmai_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Nunmai reads from has a
...
def test_seed_from_singletons_respects_nunmai_pkce_suppression(tmp_path, monkeypatch):
    """anthropic nunmai_pkce must not re-seed from ~/.nunmai/.anthropic_oauth.json when suppressed."""
    nunmai_home = tmp_path / "nunmai"
    nunmai_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))

    import yaml
    (nunmai_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (nunmai_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["nunmai_pkce"]},
    }))

    # Stub the readers so only nunmai_pkce is "available"; claude_code returns None
    import agent.anthropic_credentials as aa
    monkeypatch.setattr(aa, "read_nunmai_oauth_credentials", lambda: {
    ...
    # nunmai_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "nunmai_pkce" not in active
...
def test_auth_remove_env_seeded_dotenv_with_bom_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    assert "DEEPSEEK_API_KEY" not in (nunmai_home / ".env").read_text(encoding="utf-8-sig")
...
def test_auth_add_openrouter_oauth_persists_pkce_key_without_touching_api_key_default(tmp_path, monkeypatch):
    """`nunmai auth add openrouter --type oauth` stores the PKCE-minted key as an ``api_key`` pool row
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("nunmai_cli.auth._openrouter_pkce_login", lambda **kw: {"api_key": "sk-or-v1-from-pkce"})
...
def test_openrouter_loopback_callback_binds_nonce_path_and_rejects_forged_redirect(monkeypatch):
    ...
    import nunmai_cli.auth_openrouter as orm
