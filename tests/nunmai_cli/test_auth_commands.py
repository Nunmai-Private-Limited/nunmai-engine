...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("nunmai_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """nunmai auth add xai-oauth must set active_provider and write a pool entry.
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import nunmai_cli.auth_commands as auth_commands
    import nunmai_cli.auth_qwen as auth_qwen
