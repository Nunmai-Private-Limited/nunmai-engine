"""Tests for nunmai_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/nunmai_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_nunmai_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Nunmai"):
                check_nunmai_requires(spec, cur)
...
class TestLooksLikeGitUrl:
    ...
    def test_git_source_removes_read_only_git_metadata(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.profile_distribution._git_clone", clone_local)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from nunmai_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_nunmai_requires(self, profile_env, monkeypatch):
        # Pin current Nunmai version to something well below the requirement
        import nunmai_cli
        monkeypatch.setattr(nunmai_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            nunmai_requires=">=99.0.0",
        )
...
class TestDescribe:
    ...
    def test_describe_missing_profile_raises(self, profile_env):
        with pytest.raises(DistributionError, match="No profile named .*nunmai profile list"):
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from nunmai_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from nunmai_cli.profiles import create_profile, list_profiles, get_profile_dir
