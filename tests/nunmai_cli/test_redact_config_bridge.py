...
`NUNMAI_REDACT_SECRETS` at module-import time. `nunmai_cli/main.py` at
...
in .env), the toggle was silently ignored in both `nunmai chat` and
`nunmai gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `NUNMAI_REDACT_SECRETS`
env var in `nunmai_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports nunmai_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("NUNMAI_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import nunmai_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('NUNMAI_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["NUNMAI_HOME"] = str(nunmai_home)
    env.pop("NUNMAI_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env NUNMAI_REDACT_SECRETS takes precedence over config.yaml."""
    nunmai_home = tmp_path / ".nunmai"
    nunmai_home.mkdir()
    (nunmai_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (nunmai_home / ".env").write_text("NUNMAI_REDACT_SECRETS=true\n")
