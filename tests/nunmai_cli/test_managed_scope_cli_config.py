...
nunmai_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `nunmai config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_nunmai_home = get_nunmai_home()`` at import time (module
    singleton), so monkeypatching NUNMAI_HOME after import doesn't move it.
    ...
    this only matters for tests that swap NUNMAI_HOME.)
    """
    import cli

    cli._nunmai_home = home
