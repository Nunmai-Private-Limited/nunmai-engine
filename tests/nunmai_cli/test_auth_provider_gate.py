...
def test_vertex_adc_counts_as_explicit_when_config_present(tmp_path, monkeypatch):
    ...
    Nunmai at it (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH),
    ...
    # vertex.project_id in config.yaml is a deliberate, Nunmai-scoped signal.
    ...
    # No Nunmai-scoped Vertex config at all → stays hidden.
...
def test_vertex_ambient_google_creds_env_does_not_count_as_explicit(tmp_path, monkeypatch):
    ...
    unrelated GCP work) must NOT mark Vertex explicit — only Nunmai-scoped
...
def test_profile_dotenv_key_counts_as_explicit_when_process_env_lacks_it(tmp_path, monkeypatch):
    ...
    from nunmai_cli.auth import is_provider_explicitly_configured
    from nunmai_cli.web_server_profiles import _config_profile_scope
...
def test_dotenv_key_counts_when_shell_exports_the_var_empty(tmp_path, monkeypatch):
    ...
    (tmp_path / "nunmai" / ".env").write_text("DEEPSEEK_API_KEY=sk-dotenv-only-secret\n")

    from nunmai_cli.auth import is_provider_explicitly_configured, resolve_api_key_provider_credentials
...
def test_bedrock_not_explicit_without_aws_env(tmp_path, monkeypatch, _clean_aws_env):
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "nunmai"))
    (tmp_path / "nunmai").mkdir(parents=True, exist_ok=True)
...
def test_bedrock_ambient_aws_profile_is_not_explicit(tmp_path, monkeypatch, _clean_aws_env):
    ...
    not an explicit Nunmai provider choice — same principle as gh_cli-seeded
