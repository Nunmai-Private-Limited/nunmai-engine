"""Verify `nunmai -c` picks the session the user most recently used."""
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = nunmai_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    real_db = nunmai_state.SessionDB
...
def test_resolve_last_session_cli_continues_a_oneshot(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nunmai_cli.main._resolve_workspace_key", lambda: None)
    monkeypatch.setattr("nunmai_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
