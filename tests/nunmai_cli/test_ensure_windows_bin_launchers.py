"""``nunmai`` must survive git operations on the checkout (launcher layout).

The Windows ``nunmai`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``NUNMAI_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``nunmai-engine\\bin``) was swept by ``nunmai update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``nunmai`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``nunmai update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``nunmai_constants.venv_bin_dir``), so these
...
from nunmai_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: NUNMAI_HOME/nunmai-engine/venv/Scripts + launchers."""
    home = tmp_path / "nunmai"
    root = home / "nunmai-engine"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under NUNMAI_HOME gains nothing anywhere."""
    home = tmp_path / "nunmai-home"
    home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(home))
    root = tmp_path / "src" / "nunmai-engine"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``nunmai -p <name>`` NUNMAI_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `nunmai update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "nunmai.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "nunmai.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        ...
        == _normalize_windows_path("c:/users/me/appdata/local/NUNMAI/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``nunmai update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/nunmai.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/nunmai.exe is not gitignored — nunmai update's autostash "
