...
from nunmai_cli.config import DEFAULT_CONFIG, load_config, migrate_config, read_raw_config, save_config
...
def test_transient_read_error_is_not_recorded_as_a_corrupt_config(read, home, monkeypatch, capsys):
    ...
    from nunmai_cli.auth import _refuse_env_adoption_if_config_corrupt
    from nunmai_cli.config_read_errors import _CONFIG_PARSE_WARNED, get_active_config_parse_failure
    path = home / "config.yaml"
    _fresh_process(home, _CONFIG)
...
def test_unreadable_config_serves_one_cached_fallback_until_it_reads(home, monkeypatch):
    ...
    import builtins
    from nunmai_cli import config_backups
...
def test_save_refusal_for_bad_yaml_asks_for_an_edit_not_a_retry(home):
    ...
    with pytest.raises(RuntimeError, match="has a formatting error") as refusal:
        save_config(cfg)
    assert "nunmai config edit" in str(refusal.value) and "Try again" not in str(refusal.value)
