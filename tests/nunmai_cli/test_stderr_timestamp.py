"""Tests for nunmai_cli.stderr_timestamp."""
...
from nunmai_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.nunmai.gateway-butler"}
...
def test_child_launchd_label_env_exports_only_nunmai_job_labels():
    assert stderr_timestamp._child_launchd_label_env(_LAUNCHD_ENV) == {LAUNCHD_LABEL_ENV: "ai.nunmai.gateway-butler"}
    # Interactive shells and the grandchild itself read "0": nothing to export. App-coalition labels
    # (IDE integrated terminals) are not a Nunmai job identity either.
...
@pytest.mark.parametrize("xpc, expected", [("ai.nunmai.gateway-butler", "ai.nunmai.gateway-butler"), ("0", "unset")],
...
def test_main_forwards_launchd_label_to_child_only_under_launchd(tmp_path, monkeypatch, xpc, expected):
    """The gateway grandchild must resolve its job (drain cap, restart route) from NUNMAI_LAUNCHD_LABEL;
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.nunmai.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "nunmai_cli.main", "gateway", "run", "--replace"]
...
def test_main_maps_gateway_ex_config_to_clean_stop(tmp_path):
    ...
    log_path = tmp_path / "gateway.error.log"
    gateway_tail = ["-m", "nunmai_cli.main", "gateway", "run"]
...
def test_wrapper_forwards_sigusr1_restart_request_to_child(tmp_path):
    """Regression for #101426: launchd owns the wrapper's PID, so ``nunmai update`` sends its
    ...
    wrapper = subprocess.Popen(
        [sys.executable, "-m", "nunmai_cli.stderr_timestamp", "--error-log", str(log_path), "--",
