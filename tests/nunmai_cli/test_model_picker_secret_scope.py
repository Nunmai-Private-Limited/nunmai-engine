...
class TestSwitchModelKeyEnvScope:
    ...
    def _run_switch(self, monkeypatch, user_cfg):
        ...
        monkeypatch.setattr(
            "nunmai_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
        )
...
class TestPickerKeyEnvDotenv:
    ...
    that lives only in ``$NUNMAI_HOME/.env`` authenticates the ``/model`` verification probe, and
    ...
    def test_switch_probe_uses_dotenv_key_over_stale_process_env(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _fake_runtime)
        monkeypatch.setattr(ms, "resolve_alias", lambda *a, **k: None)
