"""Catalog-aware ``nunmai plugins`` surface (nunmai_cli/plugins_cmd_catalog.py): a bare catalog name installs
...
from nunmai_cli import plugin_catalog as pc_cat
from nunmai_cli import plugins_cmd as pc
from nunmai_cli import plugins_cmd_catalog as cat
...
def test_owner_repo_hash_subdir_shorthand_resolves_like_the_catalog_spelling():
    from nunmai_cli.plugins_cmd import _resolve_git_url
    assert _resolve_git_url("plastic-labs/honcho#nunmai-plugin-honcho") == (
        "https://github.com/plastic-labs/honcho.git", "nunmai-plugin-honcho")
...
def test_in_tree_sidecar_cannot_forge_catalog_provenance(world, tmp_path):
    ...
    its own .nunmai-catalog.json is not a catalog install and does not mark the real entry installed."""
...
def test_kill_list_covers_update_enable_and_load_of_an_installed_plugin(world, tmp_path, monkeypatch):
    ...
    from nunmai_cli.plugins_discovery import gate_manifest
