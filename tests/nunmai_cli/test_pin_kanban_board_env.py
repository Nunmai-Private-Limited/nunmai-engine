...
board into `NUNMAI_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`nunmai kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `nunmai kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `NUNMAI_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("NUNMAI_KANBAN_BOARD")
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("NUNMAI_KANBAN_BOARD") == "preset"
