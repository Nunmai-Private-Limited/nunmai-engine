...
When ``NUNMAI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.nunmai/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.nunmai            (has its own auth.json fixture)
    * Profile     -> tmp_path/.nunmai/profiles/coder   (active, NUNMAI_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Nunmai is not
# logged into Nunmai Portal"`` even though the user has a valid global Nunmai
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from nunmai_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from nunmai_cli.auth import _save_auth_store, read_credential_pool
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_nunmai_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (NUNMAI_HOME != ~/.nunmai, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    nunmai_home = tmp_path / "classic"
    nunmai_home.mkdir()
