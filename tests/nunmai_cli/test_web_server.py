"""Tests for nunmai_cli.web_server and related config utilities."""
...
import nunmai_cli.config as _cfg_mod
import nunmai_cli.web_routers.chat_ws as _rt_chat_ws
import nunmai_cli.web_server_chat as _web_server_chat
import nunmai_cli.web_server_config as _web_server_config
import nunmai_cli.web_server_dashboard as _web_server_dashboard
import nunmai_cli.web_server_files as _web_server_files
import nunmai_cli.web_server_gateway as _web_server_gateway
import nunmai_cli.web_server_lifecycle as _web_server_lifecycle
import nunmai_cli.web_server_memory as _web_server_memory
import nunmai_cli.web_server_messaging as _web_server_messaging
import nunmai_cli.web_server_sessions as _web_server_sessions
...
@pytest.fixture
def _install_example_plugin(_isolate_nunmai_home):
    """Drop the example-dashboard fixture into the per-test NUNMAI_HOME
    ...
    isolated ``NUNMAI_HOME``.

    The user-plugin source is preferred over a transient
    ``NUNMAI_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, nunmai-achievements, model providers
    ...
    from nunmai_cli import web_server

    user_plugins_dir = get_nunmai_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `nunmai plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down NUNMAI_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Nunmai vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints NUNMAI_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import nunmai_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_session_token_resolution_preserves_loaded_app_auth(self, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        assert ws._resolve_session_token() == "desktop-seeded-token"
        monkeypatch.delenv("NUNMAI_DASHBOARD_SESSION_TOKEN", raising=False)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nunmai_home):
        """Create a TestClient and isolate the state DB under the test NUNMAI_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import nunmai_state
        from nunmai_constants import get_nunmai_home
        from nunmai_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(nunmai_state, "DEFAULT_DB_PATH", get_nunmai_home() / "state.db")
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `nunmai update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `nunmai update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_is_read_only_on_a_healthy_store(self, monkeypatch):
        ...
        SessionDB(db_path=get_nunmai_home() / "state.db").close()
        ...
        monkeypatch.setattr(nunmai_state.SessionDB, "__init__", spy)
        _web_server_lifecycle._eager_reconcile_own_session_db()
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = nunmai_state.SessionDB.__init__
        ...
        # the class on nunmai_state covers every open the helper makes.
        monkeypatch.setattr(nunmai_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(nunmai_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        NUNMAI_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import nunmai_cli.web_server as web_server
...
class FlatProvMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "label": "Bank", "default": "nunmai"},
            {"key": "recall_budget", "label": "Budget", "choices": ["low", "mid", "high"], "default": "mid"},
        ]

    def save_config(self, values, nunmai_home):
        path = Path(nunmai_home) / "flatprov" / "config.json"
...
"""
    ...
    def _install_flatprov(self):
        ...
        plugin_dir = get_nunmai_home() / "plugins" / "flatprov"
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from nunmai_constants import get_nunmai_home
        from nunmai_cli.config import load_env
        ...
        config_path = get_nunmai_home() / "flatprov" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from nunmai_constants import get_nunmai_home
        from nunmai_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide NUNMAI_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from nunmai_constants import get_nunmai_home

        path = get_nunmai_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_nunmai_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["nunmai"]["workspace"] == "myws"
        assert cfg["hosts"]["nunmai"]["peerName"] == "eri"
        assert cfg["hosts"]["nunmai"]["environment"] == "local"
        assert cfg["hosts"]["nunmai"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["nunmai"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from nunmai_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_nunmai_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn nunmai update")
        ...
        # method through nunmai_cli.config directly.
        monkeypatch.setattr(
            "nunmai_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_nunmai_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("nunmai-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("nunmai-update", None)

        resp = self.client.post("/api/nunmai/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "nunmai-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert spawned is False

        status = self.client.get("/api/actions/nunmai-update/status")
    ...
    def test_update_nunmai_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn nunmai update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through nunmai_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "nunmai_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        check = self.client.get("/api/nunmai/update/check")
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "nunmai-update.log").write_text(
            "=== nunmai-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== nunmai update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== nunmai-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/nunmai-update/status?lines=2000")
        ...
        assert f"=== nunmai-update completed {action_id} ===" in data["lines"]

    def test_update_nunmai_spawns_with_action_id(self, monkeypatch):
        import nunmai_cli.web_server as web_server
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_nunmai_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("nunmai-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("nunmai-update", None)
        ...
        assert calls == [
            (["update"], "nunmai-update", {"NUNMAI_ACTION_ID": "a" * 32})
        ]

    def test_update_nunmai_reuses_running_action(self, monkeypatch):
        ...
        monkeypatch.setattr(
            _web_server_gateway,
            "_spawn_nunmai_action",
            lambda *_args, **_kwargs: pytest.fail("must not spawn a duplicate update"),
        )
        _web_server_gateway._ACTION_PROCS["nunmai-update"] = Proc()
        _web_server_gateway._ACTION_IDS["nunmai-update"] = "b" * 32

        try:
            resp = self.client.post("/api/nunmai/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("nunmai-update", None)
            _web_server_gateway._ACTION_IDS.pop("nunmai-update", None)
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Nunmai provider — keep the user's aggregator instead of writing a
    ...
    def test_model_set_flips_a_stale_setup_record(self, monkeypatch):
        ...
        fb.reset_for_tests()
        monkeypatch.setattr("nunmai_cli.model_cost_guard.expensive_model_warning", lambda *_a, **_k: None)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from nunmai_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from nunmai_cli.config import save_env_value
        from nunmai_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "nunmai_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/NunmaiSetupBot/nunmai_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/NunmaiSetupBot/nunmai_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "nunmai_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_nunmai_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from nunmai_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from nunmai_cli.web_server_profiles import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``nunmai model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from nunmai_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from nunmai_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from nunmai_cli import profiles as profiles_mod
        from nunmai_cli.config import custom_endpoint_key_env
        from nunmai_constants import get_nunmai_home

        default_home = get_nunmai_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
        (worker_home / "config.yaml").touch()  # identity marker: bare dirs are not profiles
    ...
    def test_custom_endpoint_validate_keeps_model_alias_metadata(self, monkeypatch):
        ...
        from nunmai_cli.web_routers import config_env
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from nunmai_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from nunmai_cli.config import custom_endpoint_key_env, get_env_value
    ...
    def test_env_rejects_its_redacted_preview(self):
        ...
        from nunmai_cli.config import load_env, save_env_value
    ...
    def test_messaging_and_custom_endpoint_reject_stale_previews(self):
        ...
        from nunmai_cli.config import load_config, load_env, save_config, save_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from nunmai_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from nunmai_cli.config import read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_nunmai_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from nunmai_constants import get_nunmai_home
        import nunmai_cli.profiles as profiles_mod
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        import nunmai_cli.tools_config_post_setup as tools_config_post_setup
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "nunmai"
        save_config(config)
        ...
        assert "nunmai@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # NUNMAI_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestDesktopHostRendezvousIsolation:
    ...
    def test_desktop_backend_does_not_claim_the_host_serve_record(self, monkeypatch, tmp_path):
        ...
        terminal `nunmai plugins install` on a Desktop-only box must still find it (#119644):
        it publishes under its OWN role, which the attach ladder never reads."""
        import io
        import urllib.request
        from gateway import host_rendezvous as hr
        import nunmai_cli.web_server as web_server
        from nunmai_cli.main_dashboard import _host_backend_attachment
        from nunmai_cli.plugins_activation import notify_serve_backend

        monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("NUNMAI_DESKTOP", "1")
        monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "desktop-spawn-token")
        ...
        def _fake_urlopen(request, timeout=None):
            dialed.append((request.full_url, request.get_header("X-nunmai-session-token")))
        ...
        try:
            ...
            # ...but a terminal `nunmai plugins install` still lights up its open chats.
    ...
    def test_standalone_backend_still_claims_the_host_serve_record(self, monkeypatch):
        ...
        supervised service whose shell merely inherited NUNMAI_DESKTOP=1 without the token."""
        from gateway import host_rendezvous as hr
        import nunmai_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from nunmai_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from nunmai_cli.web_server_config import _denormalize_config_from_web
        from nunmai_cli.config import save_config
...
class TestDenormalizeProviderSwitch:
    ...
    def test_vendor_slug_switches_off_non_aggregator_provider(self):
        ...
        with _patch("nunmai_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter"):
    ...
    def test_rejected_switch_is_400_and_leaves_the_model_block_byte_identical(self, monkeypatch):
        ...
        from starlette.testclient import TestClient
        from nunmai_constants import get_nunmai_home
        from nunmai_cli.model_switch import ModelSwitchResult
        from nunmai_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        cfg_path = get_nunmai_home() / "config.yaml"
        cfg_path.write_text(
            "model:\n"
            "  default: llama3.2\n"
            "  provider: ollama-local\n"
        ...
        monkeypatch.setattr("nunmai_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter")
        monkeypatch.setattr("nunmai_cli.model_switch.switch_model",
...
class TestStatusInstallId:
    ...
    Nunmai home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_nunmai_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """NUNMAI_HOME=<root> and NUNMAI_HOME=<root>/profiles/<name> resolve to
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from nunmai_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.nunmai/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_nunmai_home_override(str(other))
        try:
            results = _web_server_dashboard._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(nunmai_home, name="ocean"):
        themes_dir = nunmai_home / "dashboard-themes"
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nunmai_home):
        ...
        monkeypatch.setattr(
            nunmai_state, "DEFAULT_DB_PATH", get_nunmai_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``nunmai_cli/web_server.py``).
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nunmai_home):
        ...
        # Pin the SessionDB to the isolated NUNMAI_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_nunmai_home, _install_example_plugin):
        ...
        installs it into the per-test ``NUNMAI_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into NUNMAI_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, nunmai-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (nunmai-achievements is loaded by default).
        resp = self.client.get("/api/plugins/nunmai-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        NUNMAI_HOME override scopes a request to another profile."""
        from nunmai_constants import (
            reset_nunmai_home_override,
            set_nunmai_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets NUNMAI_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the nunmai root's plugins/ directory."""
        root = tmp_path / "nunmai-root"
    ...
    def test_unreadable_plugin_paths_do_not_block_discovery(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli import web_server_dashboard
...
# or ``sh -c 'printf …'``) instead of the real ``nunmai --tui`` binary.  The
# endpoint resolves its argv through ``_resolve_chat_argv``, so tests
# monkeypatch that hook.
# ---------------------------------------------------------------------------

from nunmai_cli import main_tui_launch
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"nunmai-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "NUNMAI_CWD": str(tmp_path),
            "NUNMAI_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_tui_launch._apply_tui_python_env(env)

        assert env["NUNMAI_PYTHON"] == command


    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from nunmai_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from nunmai_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import nunmai_cli.main_tui_launch as tui_launch
    import nunmai_cli.web_server as ws
    ...
    gateway_url = env.get("NUNMAI_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nunmai_home, _install_example_plugin):
        ...
        the per-test ``NUNMAI_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_nunmai_home):
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("NUNMAI_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        _web_server_dashboard.mount_spa(spa_app)
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__NUNMAI_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `nunmai serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__NUNMAI_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("NUNMAI_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__NUNMAI_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__NUNMAI_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/nunmai"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/nunmai/ds-assets/bg.png)" in prefixed.text
        assert "url(/nunmai/fonts-terminal/x.woff2)" in prefixed.text
...
class TestSubmittedCustomEndpointSurvivesAssignment:
    ...
    def test_submitted_custom_endpoint_wins_over_an_env_endpoint(self, monkeypatch):
        from nunmai_cli.web_server_config import _apply_main_model_assignment, _validated_main_model_selection

        monkeypatch.setenv("CUSTOM_BASE_URL", "http://127.0.0.1:9999/v1")
        monkeypatch.setattr(
            "nunmai_cli.models_validate.validate_requested_model",
            lambda *a, **k: {"accepted": True, "persist": True, "recognized": True, "message": None})
        monkeypatch.setattr("nunmai_cli.model_switch.get_model_info", lambda *a, **k: None)
        monkeypatch.setattr("nunmai_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
