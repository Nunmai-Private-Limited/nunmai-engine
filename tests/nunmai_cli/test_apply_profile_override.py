...
def _run_apply_profile_override(
    ...
    if active_profile and active_profile != "default":
        ...
        (nunmai_root / "profiles" / active_profile / "config.yaml").write_text("{}\n")  # identity marker
...
class TestApplyProfileOverrideNunmaiHomeGuard:
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli.main import _apply_profile_override, _resolve_sudo_user_profile_env
        _apply_profile_override()
        ...
        (user_home / ".nunmai" / "profiles" / "ghost" / "cron").mkdir(parents=True)
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        for name in ("briefer", "coder"):
            (nunmai_root / "profiles" / name).mkdir(parents=True, exist_ok=True)
            (nunmai_root / "profiles" / name / "config.yaml").write_text("{}\n")  # identity marker
...
class TestS6ContainerGatewayRun:
    ...
    def test_the_redirected_run_keeps_the_root_home_despite_the_active_profile(
        ...
        monkeypatch.setattr("nunmai_cli.service_manager._s6_running", lambda: True)
        root = tmp_path / ".nunmai"
        result = _run_apply_profile_override(
            tmp_path, monkeypatch, nunmai_home=str(root), active_profile="coder",
            argv=["nunmai", "gateway", "run"],
        )
    ...
    def test_a_foreground_run_and_other_verbs_still_follow_the_active_profile(
        ...
        monkeypatch.setattr("nunmai_cli.service_manager._s6_running", lambda: True)
        root = tmp_path / ".nunmai"
        for argv in (["nunmai", "gateway", "run", "--no-supervise"], ["nunmai", "chat"]):
            result = _run_apply_profile_override(
                tmp_path, monkeypatch, nunmai_home=str(root), active_profile="coder", argv=argv,
            )
