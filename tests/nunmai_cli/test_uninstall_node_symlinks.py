"""Tests for nunmai_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $NUNMAI_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Nunmai-managed node dir.
"""
...
def _make_nunmai_node(nunmai_home: Path) -> Path:
    """Create a fake $NUNMAI_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = nunmai_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    nunmai_home = fake_home / ".nunmai"
    _make_nunmai_node(nunmai_home)
    ...
    removed = uninstall.remove_node_symlinks(nunmai_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    nunmai_home = fake_home / ".nunmai"
    node_bin = _make_nunmai_node(nunmai_home)
