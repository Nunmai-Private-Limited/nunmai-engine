"""Invariant tests for the plain-language startup messages of the ``nunmai`` CLI.

Contracts (not snapshots): a mistyped subcommand names the typo, offers the closest
match and ``nunmai --help`` without dumping the 70-entry choice list; a rejected
profile name explains the rule; missing optional dependencies point at ``nunmai update``.
"""
...
def test_unknown_subcommand_names_typo_suggests_closest_and_hides_choice_list():
    ...
    assert "'sesions' is not a `nunmai` command" in text
    assert "Did you mean: sessions" in text
    assert "nunmai --help" in text
...
def test_nested_group_typo_names_the_group_and_suggests():
    ...
    assert "'stat' is not a `nunmai gateway` command" in text
...
def test_invalid_profile_flag_value_explains_rule_and_exits(monkeypatch):
    from nunmai_cli import main as _main

    monkeypatch.setattr(sys, "argv", ["nunmai", "-p", "Work Bot", "status"])
    err = io.StringIO()
    with redirect_stderr(err), pytest.raises(SystemExit) as exc:
...
def test_option_looking_dash_p_value_is_a_silent_skip_even_under_nunmai(monkeypatch):
    from nunmai_cli import main as _main

    # `-p no:xdist` reaching us through a differently named runner (tox, nox, python -m) must not exit.
    monkeypatch.setattr(sys, "argv", ["nunmai", "-p", "no:xdist", "tests/"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
    monkeypatch.setattr(sys, "argv", ["nunmai", "-p", "--flag"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_invalid_dash_p_after_a_subcommand_is_left_to_that_subcommand(monkeypatch):
    ...
    # A plugin/subcommand flag such as `nunmai kanban serve -p "Work Bot"` is not our profile selector.
    monkeypatch.setattr(sys, "argv", ["nunmai", "kanban", "serve", "-p", "Work Bot"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
