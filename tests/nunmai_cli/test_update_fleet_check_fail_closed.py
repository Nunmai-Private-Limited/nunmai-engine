...
``nunmai update`` exited 0 with zero rows — even when a gateway was
...
from nunmai_cli.main import _fleet_probe_expected_runtimes
from nunmai_cli.update_inventory import RuntimeRecord
...
class TestEmptySnapshotFailClosed:
    ...
    def test_incomplete_when_restart_phase_touched_gateways(self):
        ...
        assert (
            _fleet_probe_expected_runtimes(None, [], None, ["nunmai-gateway"], set())
            is True
        )
...
class TestCallSiteWiring:
    ...
    def test_settle_sleep_gated_on_expected_runtimes(self):
        ...
        from nunmai_cli import update_cmd_fleet
