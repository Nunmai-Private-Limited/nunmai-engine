...
Desktop sidebar lists); the pick reaches the TUI as ``NUNMAI_CWD``/``NUNMAI_TUI_CWD`` and a
...
from nunmai_cli import web_server, web_server_chat
from nunmai_cli.web_routers import chat_workspaces
...
def test_resolve_chat_cwd_fails_closed_and_reaches_the_tui_env(monkeypatch, tmp_path):
    """A picked directory becomes NUNMAI_CWD + NUNMAI_TUI_CWD on the PTY child; a missing one is a
    ...
    assert env["NUNMAI_CWD"] == str(tmp_path)
    assert env["NUNMAI_TUI_CWD"] == str(tmp_path)

    _argv, _cwd, env = web_server_chat._resolve_chat_argv()
    assert "NUNMAI_TUI_CWD" not in env
