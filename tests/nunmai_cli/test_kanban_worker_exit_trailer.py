...
the process running the sweep; a per-tick ``nunmai kanban dispatch`` process finds it
...
from nunmai_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER, exit_single_query
...
def test_exit_single_query_writes_trailer_only_for_kanban_workers(monkeypatch, capsys):
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "t_1")
    with pytest.raises(SystemExit) as exc:
