"""A `$NUNMAI_HOME` provider plugin re-registering a bundled name reaches the runtime (#48450).
...
reads ``nunmai_cli.auth.PROVIDER_REGISTRY`` though, so the mirror has to carry the override across.
...
def _run(tmp_path: Path, plugin_source: str) -> dict:
    ...
    env = {**os.environ, "NUNMAI_HOME": str(home), "PYTHONPATH": str(REPO), "STEPFUN_API_KEY": "sk-fixture"}
