...
from nunmai_cli import update_cmd_fleet, update_receipt


def _checkout(tmp_path: Path, name: str) -> Path:
    root = tmp_path / name
    module = root / "nunmai_cli" / "main.py"
...
def test_code_root_requires_absolute_path(tmp_path, monkeypatch):
    ...
    assert update_receipt._code_root_for_path("nunmai_cli/main.py") is None
...
def test_gateway_root_uses_pid_guarded_status_argv(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _path: {"pid": 42, "argv": [str(root / "nunmai_cli" / "main.py")]},
    )
...
def test_gateway_root_uses_virtualenv_when_module_path_is_absent(tmp_path, monkeypatch):
    ...
    fake_process = SimpleNamespace(
        ...
        cmdline=lambda: ["python", "-m", "nunmai_cli.main"],
    )
    ...
    failed = update_receipt.print_fleet_version_matrix([
        {
            ...
            "state": "external", "code_root": "/srv/nunmai-pinned",
        }
    ])
    output = capsys.readouterr().out
    assert failed is False
    assert "separate checkout" in output
    assert "/srv/nunmai-pinned" in output
...
def test_collect_fleet_versions_classifies_separate_checkout_gateway(tmp_path, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(json.dumps({
        "gateway_state": "running", "kind": "nunmai-gateway", "pid": 4242,
        "argv": [str(pinned / "nunmai_cli" / "main.py"), "gateway", "run"],
    ...
    monkeypatch.setattr("nunmai_cli.profiles._get_default_nunmai_home", lambda: home)
    monkeypatch.setattr("nunmai_cli.profiles._get_profiles_root", lambda: tmp_path / "no_profiles")
