"""`nunmai sessions optimize|optimize-storage|prune` refuse while another process holds state.db (#110054).
...
def test_store_rewrites_refuse_and_name_the_holder_until_forced(action, state_db, foreign_holder, capsys):
    ...
    assert f"Refusing `nunmai sessions {action}`" in out and "--force" in out
...
def test_prune_preview_passes_the_delete_waits_for_a_quiet_store(state_db, foreign_holder, capsys):
    ...
    assert "Refusing `nunmai sessions prune`" in capsys.readouterr().out
