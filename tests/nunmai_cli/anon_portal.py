...
exercised through Nunmai' real client code, never mocked away. Scenarios flip its behaviour
...
def install_portal(monkeypatch, tmp_path, fake: FakePortal | None = None) -> FakePortal:
    """Route every Nunmai HTTP client at *fake*, isolate the stores, and reset the per-process memos.

    One transport seam: ``httpx.Client`` itself, which ``auth_nous._nous_http_client`` and
    ``resolve_nous_access_token`` both construct."""
    from nunmai_cli import anon_auth, free_tier_bootstrap
