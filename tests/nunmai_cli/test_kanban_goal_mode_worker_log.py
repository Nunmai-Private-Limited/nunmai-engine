...
from nunmai_cli import kanban_db_dispatch as dispatch
...
def test_goal_mode_worker_takes_the_same_stdout_rich_path_as_a_one_shot_worker(monkeypatch):
    monkeypatch.setattr(dispatch, "_resolve_nunmai_argv", lambda: ["nunmai"])
    ...
    # The mode travels in NUNMAI_KANBAN_GOAL_MODE; the argv must not opt the worker out of
