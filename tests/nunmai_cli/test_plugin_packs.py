...
def _fake_catalog_entry(name):
    from nunmai_cli.plugin_catalog import CatalogCapabilities, PluginCatalogEntry
...
def test_resolve_pack_plugins_uses_catalog_for_bare_names():
    ...
    with mock.patch(
        "nunmai_cli.plugin_catalog.load_catalog_live",
...
def test_resolve_pack_plugins_carries_catalog_miss_as_error():
    ...
    with mock.patch(
        "nunmai_cli.plugin_catalog.load_catalog_live", return_value=[]
    ):
        resolved = resolve_pack_plugins(pack)
