"""A Nunmai-authored systemd ``--replace`` drop-in must not survive unit refresh.

An older Nunmai wrote ``<unit>.d/20-replace.conf`` to end a respawn storm; it overrides ExecStart
with ``gateway run --replace``. Combined with the cross-profile ownership guard, that override turns
a per-profile fleet's unit into one that can never start (#119467). Refresh retires the Nunmai file,
...
def _current_unit_with_dropin(tmp_path, monkeypatch, dropin_text: str):
    unit = tmp_path / "nunmai-gateway.service"
    ...
    monkeypatch.setattr(gateway_cli, "_sync_nunmai_home_from_systemd_unit", lambda **_: None)
    calls = []
...
def test_refresh_retires_the_nunmai_replace_dropin_even_when_the_unit_text_is_current(tmp_path, monkeypatch, capsys):
    ...
    unit, dropin, calls = _current_unit_with_dropin(tmp_path, monkeypatch, NUNMAI_DROPIN)
...
def test_refresh_keeps_an_operator_written_dropin_of_the_same_name(tmp_path, monkeypatch):
    """Only the file Nunmai authored (recognised by its own comment) is retired."""
    unit, dropin, calls = _current_unit_with_dropin(
        tmp_path, monkeypatch, "[Service]\nExecStart=\nExecStart=/opt/nunmai gateway run --replace\n")
