...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_nunmai_home", lambda: nunmai_home)
    monkeypatch.setattr(uninstall, "_is_default_nunmai_home", lambda home: False)
    ...
    assert str(nunmai_home) in output
    assert project_root.exists()
    assert nunmai_home.exists()
