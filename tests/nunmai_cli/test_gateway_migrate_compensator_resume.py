...
def _write_live_gateway(home: Path, served: list[str] | None = None) -> None:
    record = {"pid": os.getpid(), "nunmai_home": str(home), "gateway_state": "running"}
    if served is not None:
        record["served_profiles"] = served
    (home / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "nunmai_home": str(home)}))
...
def stranded(tmp_path, monkeypatch):
    ...
    def _service_op(kind, system, verb, home, *, run_as_user=None):
        name = nunmai_constants.profile_name_for_home(home) or "default"
