...
from nunmai_cli.agent_plugins import (
    MCP_SCHEMA_V1,
    PLUGIN_SCHEMA_V1,
    AgentPluginError,
...
def test_server_declaration_joins_mcp_and_preserves_liveness(tmp_path: Path) -> None:
    ...
    _write_json(
        ...
        _manifest(extensions={
            "com.nousresearch.nunmai": {"servers": {"worker": {
...
def test_orphan_server_declaration_disables_package(tmp_path: Path) -> None:
    _write_json(
        tmp_path / "plugin.json",
        _manifest(extensions={
            "com.nousresearch.nunmai": {"servers": {"orphan": {"requires": {"app": False}}}}
        }),
    )
...
def test_liveness_without_declaration_disables_package(tmp_path: Path) -> None:
    _write_json(
        tmp_path / "plugin.json",
        _manifest(extensions={
            "com.nousresearch.nunmai": {"servers": {"worker": {"liveness": {"kind": "static"}}}}
        }),
    )
    _write_json(
        tmp_path / "mcp.json",
