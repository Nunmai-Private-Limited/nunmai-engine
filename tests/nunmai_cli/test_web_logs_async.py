...
from nunmai_cli.web_routers import status
...
def test_get_logs_yields_while_reading_and_filtering(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(status, "get_nunmai_home", lambda: tmp_path)
    ...
    import nunmai_cli.logs
    monkeypatch.setattr(nunmai_cli.logs, "_read_tail", blocking_read)
