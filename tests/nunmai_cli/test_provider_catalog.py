"""Tests for the unified provider catalog (nunmai_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``nunmai model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from nunmai_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
