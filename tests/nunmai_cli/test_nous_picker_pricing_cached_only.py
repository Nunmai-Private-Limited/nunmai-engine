"""The Nunmai picker row never starts a pricing fetch: the picker only uses the ids the Portal
unions append, and a cold pricing cache must not hold the picker open (salvage of #102099)."""

import nunmai_cli.models_pricing as mp
...
def test_nous_picker_model_ids_reads_pricing_cache_only(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.models.check_nous_free_tier", lambda **kw: False)
    monkeypatch.setattr("nunmai_cli.models.fetch_nous_recommended_models", lambda *a, **kw: None)
