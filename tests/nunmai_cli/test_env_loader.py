...
def test_recovered_update_retry_skips_external_secret_sources(tmp_path, monkeypatch):
    ...
    loaded = load_nunmai_dotenv(nunmai_home=home)
...
def test_bom_first_key_is_seen_by_installer_and_scrub_alike(tmp_path, monkeypatch):
    ...
    the key set the installers define (``load_nunmai_dotenv`` into os.environ, ``load_env_file`` into a
    ...
    from nunmai_cli.env_loader import _env_keys_defined_in_dotenv
...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from nunmai_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from nunmai_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "NUNMAI_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="nunmai_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    """Known Nunmai keys inherited from parent process are removed when absent
    ...
    monkeypatch.setenv("NUNMAI_ACP_AUTH_METHOD", "cursor_login")
    ...
    # NUNMAI_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "NUNMAI_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``NUNMAI_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "nunmai"
    home.mkdir()
    (home / ".env").write_text("NUNMAI_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "NUNMAI_ACP_AUTH_METHOD" in os.environ
    assert os.environ["NUNMAI_ACP_AUTH_METHOD"] == ""
    # COPILOT_CLI_PATH is absent from .env → cleared
    assert "COPILOT_CLI_PATH" not in os.environ


def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    """When no profile .env exists (bare profile), load_nunmai_dotenv must not
    ...
    assert os.getenv("NUNMAI_ACP_AUTH_METHOD") == "cursor_login"
    assert os.getenv("PATH") == "/usr/bin:/bin"


def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Nunmai key that IS explicitly set in the profile .env survives
    the cleanup (overrides the inherited value).
    """
    home = tmp_path / "nunmai"
    home.mkdir()
    (home / ".env").write_text(
        "NUNMAI_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("NUNMAI_ACP_AUTH_METHOD") == "claude_code_cli"


def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Nunmai key defined with the bash-compatible ``export KEY=value``
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export NUNMAI_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("NUNMAI_ACP_AUTH_METHOD", "cursor_login")
    load_nunmai_dotenv(nunmai_home=home)
    assert os.getenv("NUNMAI_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from nunmai_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
#
# load_nunmai_dotenv loads .env with override=True, so a stale
...
# the shared nunmai_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process NUNMAI_HOME (a different profile's
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.nunmai/.env
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_nunmai_dotenv(nunmai_home=other_home)
...
def test_parent_injected_dashboard_session_token_survives_dotenv(tmp_path, monkeypatch):
    """A parent that spawns `nunmai dashboard` mints NUNMAI_DASHBOARD_SESSION_TOKEN and keeps it for
    its own /api probes; a persisted token in ~/.nunmai/.env must not replace it, or the parent gets
    HTTP 401 from its own child (#115955). Ordinary keys keep the documented .env-wins precedence."""
    home = tmp_path / "nunmai"
    home.mkdir()
    (home / ".env").write_text(
        "NUNMAI_DASHBOARD_SESSION_TOKEN=persisted-token\nNUNMAI_DASHBOARD_PUBLIC_URL=http://127.0.0.1:1\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("NUNMAI_DASHBOARD_SESSION_TOKEN", "link-token")
    monkeypatch.setenv("NUNMAI_DASHBOARD_PUBLIC_URL", "http://127.0.0.1:43123")

    load_nunmai_dotenv(nunmai_home=home)
    assert os.environ["NUNMAI_DASHBOARD_SESSION_TOKEN"] == "link-token"
    assert os.environ["NUNMAI_DASHBOARD_PUBLIC_URL"] == "http://127.0.0.1:1"  # control: .env still wins
...
def test_dotenv_published_dashboard_session_token_still_reloads(tmp_path, monkeypatch):
    ...
    (home / ".env").write_text("NUNMAI_DASHBOARD_SESSION_TOKEN=first\n", encoding="utf-8")
    load_nunmai_dotenv(nunmai_home=home)
    assert os.environ["NUNMAI_DASHBOARD_SESSION_TOKEN"] == "first"

    (home / ".env").write_text("NUNMAI_DASHBOARD_SESSION_TOKEN=second\n", encoding="utf-8")
    load_nunmai_dotenv(nunmai_home=home)
    assert os.environ["NUNMAI_DASHBOARD_SESSION_TOKEN"] == "second"
