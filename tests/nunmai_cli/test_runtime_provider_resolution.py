...
def test_runtime_selected_copilot_exchanges_ambient_pool_token(tmp_path, monkeypatch):
    ...
    (nunmai_home / "auth.json").write_text(json.dumps({"version": 1, "credential_pool": {}}))
    ...
    monkeypatch.setattr("nunmai_cli.copilot_auth.resolve_copilot_token", lambda: ("ghu_raw_gh_token", "gh auth token"))
    monkeypatch.setattr("nunmai_cli.copilot_auth.get_copilot_api_token",
...
def test_codex_pool_honors_nunmai_codex_base_url(monkeypatch):
...
def test_codex_pool_honors_model_base_url(monkeypatch):
    ...
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openai-codex")
    monkeypatch.setattr(rp, "load_pool", lambda provider: _Pool())
    monkeypatch.delenv("NUNMAI_CODEX_BASE_URL", raising=False)
...
def test_openai_key_bound_to_another_host_never_reaches_openrouter(monkeypatch, openai_base_url, expected_key):
    ...
    from nunmai_cli.runtime_provider_backends import _resolve_openrouter_runtime
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
...
def test_codex_app_server_opt_in_routes_only_named_custom_providers(monkeypatch):
    ...
    assert resolved["api_key"] == "test-key"  # Nunmai' own aux/fallback client keeps the credential
...
def test_explicit_openrouter_config_mirror_bypasses_pool(monkeypatch):
    ...
    # The canonical URL that `nunmai setup` persists is NOT a mirror: the pool must still serve it.
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from nunmai_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from nunmai_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_bare_custom_resolves_model_key_env_for_configured_base_url(monkeypatch):
    ...
    # Direct-alias rung (`nunmai --resume` provider change, `/model` direct aliases pass explicit_base_url):
...
def test_configured_key_env_resolving_empty_is_logged(monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="nunmai_cli.runtime_provider"):
...
_CODEX_STORE_CREDS = {"base_url": "https://chatgpt.com/backend-api/codex", "api_key": "tok",
                      "source": "nunmai-auth-store", "last_refresh": 1}
