...
base deps) during ``nunmai update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point nunmai_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("nunmai_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("nunmai_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("nunmai_cli.main_install_repair._run_install_with_heartbeat"):

            from nunmai_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("nunmai_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("nunmai_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from nunmai_cli.main_install_repair import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `nunmai update`.
        ...
        with patch("nunmai_cli.main_install_repair._is_windows", return_value=True):
            from nunmai_cli.main_install_repair import _resolve_install_target_python
