"""Tests for IRC gateway configuration via `nunmai setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Nunmai install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Nunmai install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nunmai")
            monkeypatch.setenv("IRC_NICKNAME", "nunmai-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nunmai setup gateway` experience with IRC present."""
