...
to resolve ``get_service_name()``. If that switch were invisible to ``get_nunmai_home()`` the name
would resolve to the launch home's bare ``nunmai-gateway`` and the cleanup would disable, stop and
...
def two_homes(tmp_path, monkeypatch):
    ...
    host_unit = unit_dir / "nunmai-gateway.service"
    victim_unit = unit_dir / "nunmai-gateway-victim.service"
...
def test_delete_from_a_pinned_multiplexer_targets_the_victims_unit_only(two_homes, monkeypatch):
    ...
    assert ["systemctl", "--user", "disable", "nunmai-gateway-victim"] in calls
    assert not any(c[:3] == ["systemctl", "--user", "disable"] and c[3] == "nunmai-gateway" for c in calls)
    # The caller's process home is restored: the process still is the launch profile afterwards.
    assert profiles.os.environ["NUNMAI_HOME"] == str(launch)
