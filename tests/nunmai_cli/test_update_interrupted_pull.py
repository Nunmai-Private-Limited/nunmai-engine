"""A `nunmai update` killed while git writes the new tree must leave a recoverable install.
...
checkout import) puts the old tree back so ``nunmai update`` can simply run again.
"""
...
# far (the entry module, its package's __init__ and what nunmai_bootstrap needs excluded: those run
...
importlib.import_module(entry.rpartition(".")[0] or "nunmai_cli")
...
def checkout(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.main.PROJECT_ROOT", root)
...
def test_killed_pull_is_restored_on_next_launch_and_update_reruns(checkout, monkeypatch):
    ...
    # Another `nunmai` launched while an update is mid-pull must not race its git.
    ...
    _pull(root)  # `nunmai update` again: a normal fast-forward
    assert _git(root, "rev-parse", "HEAD") == b and not marker.exists()

    # Every console script (`nunmai`, `nunmai-engine`, `nunmai-acp`) repairs before its entry module imports
    # any other checkout module past nunmai_bootstrap: any of them may be a half-written file.
    repo = os.path.realpath(Path(er.__file__).parent.parent)
    for entry in ("nunmai_cli.main", "agent.legacy_cli", "run_agent", "acp_adapter.entry"):
