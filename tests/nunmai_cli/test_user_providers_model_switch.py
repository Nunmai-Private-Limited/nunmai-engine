...
from nunmai_cli import runtime_provider as rp
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical Nunmai write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though Nunmai's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("nunmai_cli.providers.NUNMAI_OVERLAYS", {})
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical Nunmai shape
    ...
    ``default_model``, so new-shape entries written by Nunmai's own writer
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("nunmai_cli.model_switch.resolve_alias", return_value=None), \
         patch("nunmai_cli.model_switch.list_provider_models", return_value=[]), \
         patch("nunmai_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("nunmai_cli.models_validate.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("nunmai_cli.models.detect_provider_for_model", return_value=None), \
         patch("nunmai_cli.model_switch.get_model_info", return_value=None), \
         patch("nunmai_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("nunmai_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.models.fetch_api_models", _fake_fetch)
...
def test_current_custom_model_is_surfaced_in_builtin_provider_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.models.cached_provider_model_ids",
...
def test_overlay_provider_row_merges_configured_models(monkeypatch):
    """A ``providers.<overlay>.models`` block extends a Nunmai-overlay row (azure-foundry) the way
    ...
    monkeypatch.setattr("nunmai_cli.providers.NUNMAI_OVERLAYS", {"azure-foundry": NUNMAI_OVERLAYS["azure-foundry"]})
    monkeypatch.setattr("nunmai_cli.models.cached_provider_model_ids", lambda *_a, **_k: ["gpt-5.6-sol", "shared"])
...
def test_entra_only_azure_foundry_row_is_listed_without_api_key(monkeypatch, base_url, listed):
    ...
    from nunmai_cli.model_switch_providers import _collect_authed_provider_slugs
    from nunmai_cli.providers import NUNMAI_OVERLAYS

    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("agent.models_dev.PROVIDER_TO_MODELS_DEV", {})
    monkeypatch.setattr("nunmai_cli.providers.NUNMAI_OVERLAYS", {"azure-foundry": NUNMAI_OVERLAYS["azure-foundry"]})
    monkeypatch.setattr("nunmai_cli.models.cached_provider_model_ids", lambda *_a, **_k: ["gpt-5.6-sol"])
    monkeypatch.setattr("nunmai_cli.models._get_model_config_dict",
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider_backends._azure_entra_credentials",
...
def test_cli_picker_provider_select_reads_the_disk_cached_catalog(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr("nunmai_cli.models.cached_provider_model_ids",
                        lambda slug, *_a, **_k: seen.append(slug) or ["gpt-5.4"])
    monkeypatch.setattr("nunmai_cli.models.provider_model_ids",
    ...
    cli_mod.NunmaiCLI._handle_model_picker_selection.__get__(self_, SimpleNamespace)(persist_global=True)
