...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        message = self._msg(r"C:\v\Scripts\python.exe -m nunmai_cli.main dashboard")
        assert "close the desktop app" not in message.lower()
        assert "nunmai dashboard" in message

    def test_preserve_cache_not_labeled_serve(self):
        message = self._msg(r"python -m nunmai_cli.main kanban --preserve-cache")
    ...
    def test_serve_gets_backend_hint(self):
        message = self._msg(r"python -m nunmai_cli.main serve --host 127.0.0.1 --port 0")
        assert "Nunmai backend" in message

    def test_gateway_hint(self):
        message = self._msg(r"python -m nunmai_cli.main gateway run")
