"""Tests for the per-profile Projects store (nunmai_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Nunmai Engine", folders=["/tmp/nunmai"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "nunmai-engine"
    assert proj.name == "Nunmai Engine"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/nunmai"
    assert [f.path for f in proj.folders] == ["/tmp/nunmai"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "nunmai-engine").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' NUNMAI_HOME.
