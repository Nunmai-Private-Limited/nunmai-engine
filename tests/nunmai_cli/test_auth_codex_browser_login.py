"""``nunmai auth add openai-codex --browser``: opt-in loopback authorization-code + PKCE (#95743).
...
def test_browser_flag_runs_loopback_pkce_and_stores_loopback_source(tmp_path, monkeypatch, fake_openai):
    ...
    (tmp_path / "nunmai" / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    from nunmai_cli import auth_codex_browser as browser_mod
    ...
    monkeypatch.setattr(
        "nunmai_cli.auth._codex_device_code_login",
...
def test_default_is_device_code_and_busy_callback_port_falls_back(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.auth._codex_device_code_login", _device)
    bind_attempts = []
    ...
    sources = [e["source"] for e in json.loads((tmp_path / "nunmai" / "auth.json").read_text())["credential_pool"]["openai-codex"]]
