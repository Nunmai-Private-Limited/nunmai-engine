"""Tests for the shared tar.gz writer (``nunmai_cli.archive_safe.make_targz``).

``make_targz`` backs both ``nunmai profile export`` and ``nunmai kanban
...
from nunmai_cli.archive_safe import make_targz
