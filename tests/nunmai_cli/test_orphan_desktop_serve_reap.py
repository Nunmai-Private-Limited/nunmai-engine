"""Orphan Desktop-local ``nunmai serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    assert _is_desktop_local_serve_cmdline(
        "python -m nunmai_cli.main serve --host 127.0.0.1 --port 0"
    )
    assert _is_desktop_local_serve_cmdline(
        "nunmai serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/nunmai serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "nunmai serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "nunmai serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("nunmai gateway run --replace")
    # "serve" inside another token is not the serve subcommand (token matcher, not substring).
    assert not _is_desktop_local_serve_cmdline(
        "nunmai kanban --preserve-cache --host 127.0.0.1 --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "nunmai serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "nunmai serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "nunmai serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "nunmai serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        patch(
            "nunmai_cli.dashboard_procs._scan_dashboard_processes",
            return_value=scanned,
        ),
        patch(
            "nunmai_cli.dashboard_procs._process_ppid",
            side_effect=lambda pid: ppids.get(pid),
        ),
        patch("os.kill", side_effect=fake_kill),
    ):
        os.environ.pop("NUNMAI_DESKTOP_CHILD_PID", None)
...
def test_reap_passes_child_pid_exclude_to_scan():
    with (
        ...
        patch.dict(os.environ, {"NUNMAI_DESKTOP_CHILD_PID": "999,111"}, clear=False),
    ):
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "nunmaiHome": "~/.nunmai",
        "logPath": f"~/.nunmai/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_lock_owned_serve_pids_sees_root_home_locks_from_a_profile_home(tmp_path, monkeypatch):
    """A profile backend (``NUNMAI_HOME=<root>/profiles/<name>``) must still see the Desktop's SSH
    ...
    monkeypatch.setattr(nunmai_constants, "_get_platform_default_nunmai_home", lambda: root)
    monkeypatch.setattr(nunmai_constants, "_default_nunmai_root_memo", None, raising=False)
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.nunmai/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "nunmai serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "nunmai serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "nunmai serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
    ...
    with (
        ...
        patch("nunmai_cli.dashboard_procs._process_ppid", return_value=1),
        patch("os.kill", side_effect=fake_kill),
    ):
        result = _reap_orphaned_desktop_local_serves(
            sleep_fn=lambda _s: None,
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "nunmai serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "nunmai serve --isolated --host 127.0.0.1 --port 0"),
        (780, "nunmai serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when NUNMAI_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "nunmai serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_kills_descendants_of_killed_roots_but_spares_a_failed_roots_subtree():
    ...
    scanned = [
        (111, "nunmai serve --host 127.0.0.1 --port 0"),  # ours: killed, child 1111 swept
        (444, "nunmai serve --host 127.0.0.1 --port 0"),  # not ours: EPERM, child 4444 spared
    ]
    ...
    with (
        patch("nunmai_cli.dashboard_procs._scan_dashboard_processes", return_value=scanned),
        patch("nunmai_cli.dashboard_procs._process_ppid", return_value=1),
        patch("nunmai_cli.dashboard_procs._posix_descendants", return_value=descendants) as snap,
