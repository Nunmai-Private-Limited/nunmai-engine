...
def _switch(raw_input, profile):
    with patch("nunmai_cli.model_switch.list_provider_models", return_value=[]), \
         patch("providers.get_provider_profile", return_value=profile), \
         patch("nunmai_cli.model_switch.get_authenticated_provider_slugs", return_value=["copilot", "anthropic"]), \
         patch("nunmai_cli.models_validate.validate_requested_model", return_value=_ACCEPTED), \
         patch("nunmai_cli.models.detect_provider_for_model", return_value=("copilot", "claude-haiku-4.5")), \
...
def test_declared_process_model_validates_without_endpoint_warning():
    ...
    with patch("providers.get_provider_profile", return_value=profile), \
         patch("nunmai_cli.models.fetch_api_models", side_effect=AssertionError("no HTTP probe for process://")):
