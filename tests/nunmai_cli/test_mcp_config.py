"""
Tests for nunmai_cli.mcp_config — ``nunmai mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.config.get_nunmai_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "nunmai_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "nunmai_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    ...
    def test_list_with_servers(self, tmp_path, capsys):
        ...
        from nunmai_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from nunmai_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_nunmai_home in the mcp_config module namespace
        monkeypatch.setattr(
            "nunmai_cli.mcp_config.get_nunmai_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.mcp_config._probe_single_server", mock_probe
        )
        # No auth, accept all tools
        inputs = iter(["n", ""])  # no auth needed, enable all
        monkeypatch.setattr("builtins.input", lambda _: next(inputs))

        from nunmai_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    ...
    def test_exit_codes_distinguish_failure_from_unknown_server(self, tmp_path, capsys, monkeypatch):
        ...
        from nunmai_cli.mcp_config import cmd_mcp_test

        monkeypatch.setattr("nunmai_cli.mcp_config._probe_single_server", lambda name, cfg, **kw: [])
        ...
        monkeypatch.setattr("nunmai_cli.mcp_config._probe_single_server", failing_probe)
    ...
    def test_cli_dispatcher_forwards_test_exit_code(self, tmp_path, monkeypatch):
        """``nunmai mcp test`` reaches ``main()`` with the handler's code (the dispatcher used to drop it)."""
        _seed_config(tmp_path, {"ink": {"url": "https://mcp.ml.ink/mcp"}})
        from nunmai_cli.main import cmd_mcp
    ...
    def test_probe_uses_configured_connect_timeout(self, monkeypatch):
        ...
        import asyncio
        from nunmai_cli import mcp_config
...
class TestProbeEnvResolution:
    ...
    where `nunmai mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from nunmai_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import nunmai_cli.mcp_config as mc
    ...
    def test_probe_propagates_explicit_connect_timeout_to_config(self, monkeypatch):
        """An explicit `connect_timeout=` override (e.g. `nunmai mcp login`'s 315s, extended so a
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from nunmai_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from nunmai_cli.mcp_config import _save_mcp_server, _get_mcp_servers
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from nunmai_cli.mcp_config import cmd_mcp_login
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.mcp_config._probe_single_server",
    ...
    def test_login_clears_tokens_but_keeps_discovered_server_metadata(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.mcp_config._probe_single_server", mock_probe)
        from nunmai_cli.mcp_config import cmd_mcp_login
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "nunmai_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from nunmai_cli.mcp_config import cmd_mcp_reauth
...
def test_tool_filters_keeps_explicit_empty_include():
    ...
    from nunmai_cli.mcp_config import _tool_filters
