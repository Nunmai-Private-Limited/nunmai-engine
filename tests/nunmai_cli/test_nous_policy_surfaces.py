"""Every Nunmai model list is narrowed to the org's policy before it is shown.
...
class TestRecommendedDefaultEndpoint:
    ...
    def _call(self, monkeypatch):
        import nunmai_cli.auth as auth_mod
        from nunmai_cli.web_routers.models import get_recommended_default_model
...
class TestNousPrefetch:
    ...
    def test_nous_is_not_collected_for_prefetch(self, monkeypatch):
        import nunmai_cli.auth as auth_mod
        import nunmai_cli.model_switch_providers as ms
...
class TestPolicyNoticeIsShown:
    ...
    def test_login_prints_it(self, monkeypatch, tmp_path, policy, capsys):
        import nunmai_cli.nous_account as account_mod
...
def test_titling_seeds_the_shared_catalog_entry_like_the_pickers(monkeypatch):
    ...
    without the Nunmai-only arguments costs the picker its sale chrome and leaves
