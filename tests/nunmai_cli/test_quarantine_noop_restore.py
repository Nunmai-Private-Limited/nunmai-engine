...
``nunmai*.exe`` shims aside (``nunmai.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``nunmai`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("nunmai", "nunmai-engine", "nunmai-acp", "nunmai-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "nunmai.exe" in names, "nunmai.exe must be restored after a no-op install"
    assert "nunmai-acp.exe" in names
    assert "nunmai-gateway.exe" in names
    assert (scripts / "nunmai.exe").read_bytes() == b"MZ-old-nunmai"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "nunmai.exe").read_bytes() == b"MZ-new-nunmai"
...
# ---------------------------------------------------------------------------
# nunmai_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "nunmai.exe" in names, "nunmai.exe must be restored after a no-op recovery install"
    assert (scripts / "nunmai.exe").read_bytes() == b"MZ-old-nunmai"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "nunmai.exe").write_bytes(b"MZ-new-nunmai")
