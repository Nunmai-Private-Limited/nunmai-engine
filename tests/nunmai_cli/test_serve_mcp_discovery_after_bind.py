...
`nunmai dashboard` arms it at boot and the first /api/ws client (or agent build) fires it (#58733).
...
import nunmai_cli.mcp_startup as mcp_startup
import nunmai_cli.web_server as web_server
import nunmai_cli.web_server_lifecycle as web_server_lifecycle
from tests.nunmai_cli.test_dashboard_auth_gate import _stub_uvicorn_run
...
def _stub_dashboard_runtime(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "nunmai_logging", types.SimpleNamespace(setup_logging=lambda **_k: None))
    monkeypatch.setitem(sys.modules, "nunmai_cli.plugins", types.SimpleNamespace(discover_plugins=lambda: None))
    return main_mod


def test_standalone_dashboard_boot_arms_discovery_without_starting_it(monkeypatch):
    """#58733: an idle, unvisited `nunmai dashboard` must not spawn the configured MCP servers."""
    _reset_discovery_state(monkeypatch)
...
def test_first_gateway_ws_client_starts_the_armed_discovery_once(monkeypatch):
    ...
    import nunmai_cli.web_routers.chat_ws as chat_ws
