"""Regression tests for #34107 — Docker UID/GID handling in ensure_nunmai_home.

When Nunmai runs in Docker with ``NUNMAI_UID=1000`` / ``NUNMAI_GID=911``,
the entrypoint chowns the top-level ``NUNMAI_HOME`` once at startup. But
...
The fix is a ``_chown_to_nunmai_uid`` helper (``nunmai_constants``, the single home of the
managed/container/NUNMAI_UID policy) that reads the env vars and applies chown after
...
# ---------------------------------------------------------------------------
# _resolve_nunmai_uid_gid
# ---------------------------------------------------------------------------


class TestResolveNunmaiUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from nunmai_constants import _resolve_nunmai_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_nunmai_uid
# ---------------------------------------------------------------------------


class TestChownToNunmaiUid:
    ...
    def test_eperm_is_silently_swallowed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_UID", "1000")
        monkeypatch.setenv("NUNMAI_GID", "911")
        import nunmai_constants as cfg
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("NUNMAI_UID", raising=False)
        monkeypatch.delenv("NUNMAI_GID", raising=False)
        from nunmai_cli import config as cfg
