...
`nunmai config set platforms.<name>.<display_setting> <value>` must write to
...
def _write_config(nunmai_home: Path, data: dict) -> Path:
...
def _set(monkeypatch, nunmai_home, key, value, force=False):
    """Isolated call to set_config_value against a temp NUNMAI_HOME."""
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    # set_config_value resolves the home live via get_config_path()/get_nunmai_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, nunmai_home, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, nunmai_home, "platforms.telegram.streaming", "false")

        result = yaml.safe_load((nunmai_home / "config.yaml").read_text())
    ...
    def test_show_reasoning_redirects(self, nunmai_home, monkeypatch):
        _set(monkeypatch, nunmai_home, "platforms.telegram.show_reasoning", "false")
        result = yaml.safe_load((nunmai_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["telegram"]["show_reasoning"] is False

    def test_tool_progress_redirects(self, nunmai_home, monkeypatch):
        ...
        # is the bool.
        _set(monkeypatch, nunmai_home, "platforms.discord.tool_progress", "off")
        result = yaml.safe_load((nunmai_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["discord"]["tool_progress"] is False

    def test_connection_key_not_redirected(self, nunmai_home, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, nunmai_home, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((nunmai_home / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, nunmai_home, monkeypatch, capsys):
        ...
        raw = yaml.safe_load((nunmai_home / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, nunmai_home, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, nunmai_home, monkeypatch):
        from nunmai_cli.config import unset_config_value
    ...
    def test_set_prints_redirect_note(self, nunmai_home, monkeypatch, capsys):
        _set(monkeypatch, nunmai_home, "platforms.telegram.streaming", "false")
