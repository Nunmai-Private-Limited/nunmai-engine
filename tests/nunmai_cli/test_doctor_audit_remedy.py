"""Doctor npm-audit remedy guidance — the fix must persist across `nunmai update`.

Regression for #116774: `npm audit fix` on a managed install is reverted by the
next `nunmai update`, because `_run_npm_install_deterministic` runs `npm ci`
...
def test_root_remedy_never_prescribes_local_audit_fix(capsys):
    ...
    next `nunmai update` reinstalls from the committed lockfile and reverts it
    (#116774)."""
