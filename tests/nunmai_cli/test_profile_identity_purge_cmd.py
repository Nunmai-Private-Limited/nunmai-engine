"""The `nunmai profile purge-identity` retry path: dispatched, and honest about failure.

`nunmai profile delete` reports a pending identity settlement when it cannot purge and names this
...
def test_purge_identity_exits_nonzero_when_settlement_stays_pending(
    ...
    monkeypatch.setattr("nunmai_cli.profile_identity.purge_profile_identity", lambda name: False)
...
def test_purge_identity_refuses_a_same_name_profile_created_after_the_delete(profile_env, capsys):
    ...
    scope = str(profile_env / ".nunmai" / "sessions")
    db = SessionDB(profile_env / ".nunmai" / "state.db")
    ...
    assert exc.value.code not in (0, None)
    check = SessionDB(profile_env / ".nunmai" / "state.db")
    try:
