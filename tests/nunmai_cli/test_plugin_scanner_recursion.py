...
def _enable(nunmai_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<nunmai_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        nunmai_home = Path(os.environ["NUNMAI_HOME"])  # set by hermetic conftest fixture
        user_plugins = nunmai_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(nunmai_home, "image_gen/openai")
...
class TestForeignHarnessManifestDirs:
    def test_foreign_harness_dirs_skipped_without_warnings(
        ...
        They must be skipped silently; the plugin's real Nunmai manifest
        (``.nunmai-plugin/plugin.yaml``) is still discovered."""
        import os
        nunmai_home = Path(os.environ["NUNMAI_HOME"])  # set by hermetic conftest fixture
        sp = nunmai_home / "plugins" / "superpowers"
        (sp / ".nunmai-plugin").mkdir(parents=True)
        (sp / ".nunmai-plugin" / "plugin.yaml").write_text(
            yaml.dump(
                {
                    "name": "superpowers",
                    "version": "6.3.0",
        ...
        with caplog.at_level("WARNING", logger="nunmai_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()

        assert "superpowers/.nunmai-plugin" in mgr._plugins
        parse_warnings = [
    ...
    def test_broken_portable_plugin_still_warns(self, tmp_path, monkeypatch, caplog):
        ...
        import os
        nunmai_home = Path(os.environ["NUNMAI_HOME"])  # set by hermetic conftest fixture
        broken = nunmai_home / "plugins" / "broken-portable"
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(nunmai_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(nunmai_home, "my-img-plugin")
