...
def test_config_get_unset_subcommands_parse():
    """`nunmai config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="nunmai")
...
# ── deprecated `nunmai login` fails gracefully, not with argparse error ────
#
# `nunmai login` is a removed command; its handler (`login_command` in
# `nunmai_cli/auth.py`) prints a deprecation notice pointing at `nunmai auth` /
# `nunmai model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `nunmai --help`.
