...
when the terminal backend was already running containers via NUNMAI_DOCKER_BINARY
...
from nunmai_cli.web_routers import tools as tools_mod
...
def _hide_host_runtimes(monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_DOCKER_BINARY", raising=False)
...
def test_probe_ready_when_only_podman_is_configured(tmp_path, monkeypatch):
    """NUNMAI_DOCKER_BINARY=podman + working `version` is ready; never `docker info`."""
    _hide_host_runtimes(monkeypatch)
    fake = tmp_path / "podman"
    fake.write_text("#!/bin/sh\nexit 0\n")
    fake.chmod(0o755)
    monkeypatch.setenv("NUNMAI_DOCKER_BINARY", str(fake))
