...
Real processes with real Nunmai-shaped argv, real inventory collection
...
reconciliation. No mocks on the components under test.
 ...
 1. Spawn a real process with a `gateway run` command line registered as the
    profile gateway (state file in a temp NUNMAI_HOME) — real
...
def test_plan_reconciliation_live_windows(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(profiles_mod, "_get_default_nunmai_home", lambda: home)
