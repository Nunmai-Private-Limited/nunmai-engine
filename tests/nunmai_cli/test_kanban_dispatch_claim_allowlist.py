...
On a board shared across Nunmai homes (one kanban.db mounted in several
...
def test_present_blank_or_null_allowlist_skips_all_cards(
    ...
    with kbc.connect() as conn, caplog.at_level("WARNING", logger="nunmai_cli.kanban_db"):
        tid = kb.create_task(conn, title="foreign card", assignee="default")
        res = kbd.dispatch_once(conn, dry_run=True)
    assert res.spawned == []
    assert res.skipped_nonspawnable == [tid]
...
def test_allowlist_config_read_failure_skips_all_cards(
    ...
    from nunmai_cli import config_effective
...
def test_diagnostics_reports_resolved_allowlist(kanban_home, capsys, config, expected):
    """`nunmai kanban diagnostics` (text and --json) shows what this home may claim."""
    import argparse
    import json

    from nunmai_cli import kanban as kanban_cli
