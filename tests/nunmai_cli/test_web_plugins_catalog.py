...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_nunmai_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    ...
    installer-owned ``.install-metadata.json`` record (an in-tree ``.nunmai-catalog.json`` is inert)."""
    from nunmai_constants import get_nunmai_home
    from nunmai_cli.plugins_cmd import _read_install_metadata, _write_install_metadata
    d = get_nunmai_home() / "plugins" / name
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Nunmai plugin catalog" in resp.json()["detail"]
