"""Tests for ``nunmai dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``nunmai update``
...
from nunmai_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("nunmai_cli.dashboard_procs._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No nunmai dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "nunmai dashboard --port 9119"),
            (12346, "python -m nunmai_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "nunmai serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("nunmai_cli.dashboard_procs._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("nunmai_cli.main_dashboard._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids",
             ...
             patch("nunmai_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        # for the post-`nunmai update` path).
        kwargs = mock_kill.call_args.kwargs
        assert "reason" in kwargs
        assert "stop" in kwargs["reason"].lower()
        assert exc.value.code == 0

    def test_stop_scopes_scan_and_kill_to_the_invoking_nunmai_home(self, tmp_path, monkeypatch, capsys):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids", return_value=[12345]) as scan, \
             patch("nunmai_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids", return_value=[]), \
             patch("nunmai_cli.dashboard_procs._kill_stale_dashboard_processes") as kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids", return_value=[12345]), \
             patch("nunmai_cli.dashboard_procs._kill_stale_dashboard_processes",
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``nunmai dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"nunmai_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
