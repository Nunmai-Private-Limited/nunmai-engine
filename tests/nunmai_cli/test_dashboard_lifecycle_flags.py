...
class TestDashboardStatus:
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 nunmai dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("nunmai_cli.main._find_stale_dashboard_pids",
             ...
             patch("nunmai_cli.dashboard_procs._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
...
class TestArgparseWiring:
    ...
    ``nunmai dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from nunmai_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("nunmai_cli.main")
