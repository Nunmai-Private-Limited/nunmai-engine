...
from nunmai_cli import process_identity
from nunmai_cli import update_cmd_fleet as fleet
from nunmai_cli import update_receipt
from nunmai_cli.update_inventory import RuntimeRecord, UpdatePlan
from nunmai_cli.update_serve_obligations import defer_manual_serve, retain_receipt_manual_serves
...
def test_manual_deferral_survives_receipt_rotation(monkeypatch, capsys, kind, condition):
    ...
    monkeypatch.setattr("nunmai_cli.update_cmd._finish_dashboard_update_cleanup", lambda *a, **k: None)
    monkeypatch.setattr("nunmai_cli.gateway_migrate.maybe_auto_migrate_after_update", lambda: None)
    ...
    if condition == "unknown":
        ...
        (get_nunmai_home() / "serve_restart_pending").write_text("not a directory")
    elif condition == "missing-identity":
        runtime.detail.clear()
    elif condition == "failed-unit":
        restart.failed_or_stale_units.append("nunmai-serve-work.service")
    ...
    assert "nunmai-serve.service" not in capsys.readouterr().out
    update_receipt.begin_update_receipt()
    ...
    assert "nunmai gateway restart" not in warning
    monkeypatch.setattr(process_identity, "_pid_alive_matches", lambda *a: None)
    fleet._warn_pending_fleet_restart_on_startup()
...
def test_historical_manual_obligation_does_not_block_healthy_gateway(monkeypatch, capsys, alive, historical_fleet, marker, gateway_present):
    ...
    if not gateway_present:
        ...
        receipt["fleet"] = []
    root = get_nunmai_home() / "logs" / "update_receipts"
    root.mkdir(parents=True, exist_ok=True)
    (root / "latest.json").write_text(json.dumps(receipt))
    monkeypatch.setattr(fleet, "_current_checkout_sha", lambda: "new")
    monkeypatch.setattr("nunmai_cli.update_cmd._current_checkout_sha", lambda: "new")
    ...
    monkeypatch.setattr("nunmai_cli.update_inventory.collect_runtime_inventory", lambda: UpdatePlan(runtimes=[runtime] if alive is not False else []))
    if marker:
        fleet._write_fleet_restart_pending_marker(expected_sha="new")
...
def test_stamped_manual_only_history_has_no_gateway_obligation(monkeypatch, capsys, marker):
    ...
    monkeypatch.setattr(fleet, "_current_checkout_sha", lambda: "new")
    monkeypatch.setattr(update_receipt, "collect_fleet_versions", lambda **k: [])
    monkeypatch.setattr("nunmai_cli.update_inventory.collect_runtime_inventory", lambda: UpdatePlan(runtimes=[RuntimeRecord(**runtime)]))
    if marker:
        fleet._write_fleet_restart_pending_marker(expected_sha="new")
...
def test_historical_retention_is_independent_of_plan_order(monkeypatch, capsys, manual_first, unsupported):
    ...
    assert list((get_nunmai_home() / "serve_restart_pending").glob("*.json"))
    update_receipt.begin_update_receipt()
    update_receipt.finalize_update_receipt("success", fleet=[])
    fleet._warn_pending_fleet_restart_on_startup()
...
def test_historical_retention_failure_warns_and_survives_rotation(monkeypatch, capsys, failure, gateway_state):
    ...
    from nunmai_cli import update_serve_obligations as obligations
    ...
    directory = get_nunmai_home() / "serve_restart_pending"
