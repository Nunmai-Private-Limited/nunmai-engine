...
#    reported as added/removed by `nunmai tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `nunmai tools` printed ``- kanban``
...
# `nunmai tools post-setup <key>` child. On Windows each console child (npm,
