"""Tests for nunmai_cli.tools_config platform tool persistence."""
...
from nunmai_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'nunmai' instead of 'nunmai-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import nunmai_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["nunmai"]}}

    with caplog.at_level(logging.WARNING, logger="nunmai_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "nunmai" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["nunmai-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["nunmai-cli", "bogus"]}}
...
def test_enable_on_string_platform_toolsets_keeps_listed_entries():
    """#115866: `nunmai tools enable` must operate on the selection a list-literal
    ...
    with patch("nunmai_cli.tools_config.save_config"):
...
def test_malformed_list_string_platform_toolsets_warns_then_falls_back(caplog):
    ...
    with caplog.at_level("WARNING", logger="nunmai_cli.tools_config"):
        enabled = _get_platform_tools(config, "cli", include_default_mcp_servers=False)
...
# via the platform's native composite. The composite ``nunmai-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["nunmai-telegram", "discord"]}}
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("nunmai_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "nunmai_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
    ...
    configured = []
    monkeypatch.setattr(
        "nunmai_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from nunmai_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """nunmai-gateway includes list should cover all messaging platforms."""
        from nunmai_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["nunmai-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from nunmai_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from nunmai_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool_install._find_agent_browser (PATH -> Homebrew/Nunmai-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("nunmai_cli.tools_config_post_setup._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("nunmai_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nunmai_cli.tools_config_post_setup._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nunmai_cli.tools_config_post_setup._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Nunmai-managed-Node-only setups: npx resolves via
        ...
        nunmai_npx = "/home/user/.nunmai/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool_install._chromium_installed", return_value=False
            ...
            "tools.browser_tool_install._resolve_npx_bin", return_value=nunmai_npx
        ), patch(
            "nunmai_cli.tools_config_post_setup._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            nunmai_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nunmai_cli.tools_config_post_setup._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("nunmai_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("nunmai_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
            "nunmai_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $NUNMAI_HOME/bin when only side installs exist."""
        with patch(
            "nunmai_cli.tools_config_post_setup.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from nunmai_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "nunmai_cli.tools_config_post_setup.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("nunmai_cli.tools_config_post_setup._print_warning") as warn:
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_registered_backend(self):
        ...
        from nunmai_cli.tools_config import IMAGEGEN_BACKENDS, TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from nunmai_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("nunmai_cli.tools_config._prompt_choice", return_value=1):
            _configure_imagegen_model("fal", config)
        # ordered[0] == current (default klein), ordered[1] == first non-default
        from nunmai_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("nunmai_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        from nunmai_cli.tools_config import IMAGEGEN_BACKENDS, _configure_imagegen_model
        config = {"image_gen": "some-garbage-string"}
        with patch("nunmai_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from nunmai_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "nunmai_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("nunmai_cli.tools_config._prompt_choice", return_value=0),
        ):
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `nunmai tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from nunmai_cli.tools_config import _get_effective_configurable_toolsets
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    import nunmai_cli.tools_config as tc
    import nunmai_cli.tools_config_providers as tcp
...
#
# FAL, Krea and Portal models all live behind the single "Nunmai Subscription" row; the stored
...
def test_exactly_one_image_row_is_active_for_a_managed_selection(monkeypatch):
    import nunmai_cli.tools_config as tools_config
    from nunmai_cli.tools_config_providers import _plugin_image_gen_providers
...
def test_gui_model_catalog_for_the_managed_row_spans_every_managed_gateway(monkeypatch):
    import nunmai_cli.tools_config as tools_config
    from nunmai_cli.web_routers.tools import _resolve_toolset_model_plugin, _toolset_model_catalog
...
def test_pool_only_account_is_offered_fal_models_only(monkeypatch):
    import nunmai_cli.tools_config as tools_config
    from nunmai_cli.tools_config_providers import _managed_image_catalog
...
# ── Toolsets that shipped after a platform's last `nunmai tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[nunmai-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["nunmai-cli"]}},
        "cli",
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `nunmai config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_disabled_composite_debugging_prunes_constituent_platform_toolsets():
    ...
    toolsets on ``nunmai tools --summary``, not only strip them at runtime."""
    config = {
        "platform_toolsets": {"cli": ["nunmai-cli"]},
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (nunmai-acp, nunmai-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"nunmai-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"nunmai-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``nunmai-cli``), it MUST be admitted as a configurable key instead of
    ...
    import nunmai_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want nunmai-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["nunmai-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [nunmai-cli, a2a]``, ``a2a``
    ...
    config = {"platform_toolsets": {"cli": ["nunmai-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
...
class TestLightpandaPostSetup:
    ...
    def test_reports_binary_when_found(self, _stub_browser_use_install):
        from nunmai_cli.tools_config import _run_post_setup

        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value="/opt/lightpanda"), \
             patch("nunmai_cli.tools_config_post_setup._print_success") as ok, \
             patch("nunmai_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_prints_install_hint_when_missing(self):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value=None), \
             patch("nunmai_cli.tools_config_post_setup._print_warning") as warn, \
             patch("nunmai_cli.tools_config_post_setup._print_info") as info:
            _run_post_setup("lightpanda")
