...
from nunmai_cli import gateway, main, update_cmd_fleet as fleet
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    monkeypatch.setattr(main, "_purge_stale_nunmai_modules", lambda: None)
    stopped = []
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                ...
                "nunmai-gateway-two.service loaded failed failed\n"), stderr="")
