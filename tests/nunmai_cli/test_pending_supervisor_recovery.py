...
from nunmai_cli import gateway, main, update_cmd_fleet as fleet, update_receipt


@pytest.fixture(autouse=True)
def _units_belong_to_this_update(monkeypatch):
    """The fake ``nunmai-gateway-one/two`` units carry no home; ownership (#93349,
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "nunmai-gateway-one.service loaded active running\n"
                + ("" if failure == "missing-owned" else "nunmai-gateway-two.service loaded failed failed\n")), stderr="")
        bad = cmd[-1] == "nunmai-gateway-two"
        if "restart" in cmd:
    ...
    monkeypatch.setattr(update_receipt, "collect_fleet_versions", lambda **kw: [
        {"profile": name.removeprefix("nunmai-gateway-"), "state": "current", "code_sha": "pending"}
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"nunmai-gateway-one", "nunmai-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.nunmai.gateway", "ai.nunmai.gateway-two"
