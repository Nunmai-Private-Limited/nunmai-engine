"""#81209: ``nunmai -z`` must consult the fallback chain at *resolution* time.
...
class TestResolveRuntimeWithFallback:
    def test_auth_error_walks_chain_in_order_and_re_raises_primary(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", all_fail)
    ...
    def test_misconfiguration_is_never_rerouted(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", typo)
        with pytest.raises(ValueError):
            resolve_runtime_with_fallback(_CFG, requested="antropic")
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", lambda **kw: {"provider": "p"})
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", primary_down_first_entry_typo)
...
def test_run_agent_falls_back_when_primary_resolution_raises_auth_error(monkeypatch):
    ...
    import nunmai_cli.oneshot as oneshot_mod
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", fake_resolve)
    monkeypatch.setattr("nunmai_cli.tools_config._get_platform_tools", lambda _cfg, _p: [])
