...
from nunmai_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``nunmai_session_at`` / ``nunmai_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    provider first, which tries to reach Nunmai Portal's JWKS; if that's
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["nunmai_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
