...
def test_sessions_export_md_writes_single_session(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(nunmai_state, "SessionDB", lambda *args, **kwargs: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_sessions_export_redact_scrubs_secrets(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "nunmai", "sessions", "export", "--format", "md",
...
def _real_store(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nunmai_state, "SessionDB", StoreAtTmp)
    return StoreAtTmp
...
def _export_delete(monkeypatch, out_dir, session_id, *extra):
    ...
    monkeypatch.setattr(sys, "argv", [
        "nunmai", "sessions", "export", "--format", "md", "--session-id", session_id,
...
def test_delete_after_verified_rejects_same_count_content_change(monkeypatch, tmp_path, capsys):
    ...
    import nunmai_cli.session_export_md as session_export_md
