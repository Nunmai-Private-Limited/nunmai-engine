...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp NUNMAI_HOME IS the root here).
...
def test_not_ours_returns_none(nunmai_home):
    from nunmai_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(nunmai_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from nunmai_cli.local_runtime.bootstrap import assets_dir
    from nunmai_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(nunmai_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(nunmai_home):
...
def test_lookup_chain_consults_managed_runtime(nunmai_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(nunmai_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(nunmai_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(nunmai_home, monkeypatch, tmp_path):
