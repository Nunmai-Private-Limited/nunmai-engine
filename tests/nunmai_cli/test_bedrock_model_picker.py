...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (NUNMAI_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via NUNMAI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in NUNMAI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from nunmai_cli.providers import NUNMAI_OVERLAYS
        assert "bedrock" in NUNMAI_OVERLAYS
