"""Tests for the Phase 4 s6 dispatch helper in nunmai_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`nunmai gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_all_handles_partial_failure(
    ...
    monkeypatch.setattr(
        "nunmai_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    monkeypatch.setattr(
        "nunmai_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "nunmai_cli.service_manager.detect_service_manager",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("nunmai_cli.gateway._profile_suffix", lambda: "")

    monkeypatch.setattr("nunmai_cli.gateway.os.execvp", _raise_missing_sleep)
    block_calls: list[bool] = []
    monkeypatch.setattr(
        "nunmai_cli.gateway._block_until_terminated",
        lambda: block_calls.append(True),
    )
    monkeypatch.delenv("NUNMAI_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("NUNMAI_GATEWAY_NO_SUPERVISE", raising=False)
...
def _armed_watchdog(monkeypatch: pytest.MonkeyPatch):
    """Arm the real watchdog as nunmai_cli.main's argv fast-path does; the long timeout keeps the
    deadline out of the test, only the handle state at handoff is under test."""
    import nunmai_startup_watchdog as sw
...
class _UnregisteredRecorder(_CallRecorder):
    ...
    def _svc(self, action: str, name: str) -> None:
        from nunmai_cli.service_manager import GatewayNotRegisteredError
...
def _arrange(monkeypatch, tmp_path, mgr, *, profile: str, seed_soul: bool):
    """Force the s6 branch and make ``tmp_path`` the shared NUNMAI_HOME the slot maps back to."""
    from nunmai_cli import gateway as gw
    from nunmai_cli import service_manager as sm
