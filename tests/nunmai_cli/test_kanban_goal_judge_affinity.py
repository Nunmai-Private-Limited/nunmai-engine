...
Both terminal-handoff gates (``nunmai_cli.kanban`` for the CLI, ``tools.kanban_tools`` for
...
def test_cli_gate_transport_failure_fails_open_but_genuine_verdict_rejects():
    with _aux_client(), patch("nunmai_cli.goals.judge_goal", return_value=_TRANSPORT_FAILED):
        assert kanban_cli._goal_mode_handoff_rejection(_task(), "evidence") == ("done", None)
    with _aux_client(), patch("nunmai_cli.goals.judge_goal", return_value=_GENUINE_CONTINUE):
...
def test_cli_gate_binds_per_task_affinity_scope():
    ...
    seen = []
    with _aux_client(), patch("nunmai_cli.goals.judge_goal", side_effect=_recording_judge(seen)):
