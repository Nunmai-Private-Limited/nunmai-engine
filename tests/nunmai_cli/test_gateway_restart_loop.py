...
Covers:
- Defense 1: gateway stop/restart refuse when _NUNMAI_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no nunmai/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m nunmai_cli.main"',
        "pkill -f nunmai_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the nunmai-gateway forms (salvaged from #94379).
        "nunmai.exe gateway restart",
        "nunmai.cmd gateway stop",
        r'"C:\Program Files\nunmai.exe" gateway restart',
        "taskkill /F /IM nunmai-gateway.exe",
        "Stop-Process -Name nunmai-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "nunmai" cannot match the gateway cmdline.
        "pkill -f 'nunmai-polis/run.sh'",
        "pkill -f my_nunmai_bot.py",
        "pgrep python",
        "nunmai.exe gateway start",
        "my-nunmai.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.nunmai.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nunmai/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l nunmai-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nunmai.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.nunmai.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.nunmai.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.nunmai/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.nunmai.gateway-apollo:/a.plist' "
            "'ai.nunmai.gateway-cronus:/c.plist' 'ai.nunmai.gateway-plutus:/p.plist' "
            "'ai.nunmai.gateway:/Users/botuser/Library/LaunchAgents/ai.nunmai.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated nunmai note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.nunmai.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.nunmai.gateway',
        "launchctl kick'start' -k gui/501/ai.nunmai.gateway",
        "launchctl kick\\start -k gui/501/ai.nunmai.gateway",
        'launchctl "kickstart" -k gui/501/ai.nunmai.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.nunmai.gateway',
        "launchctl dis\\able gui/501/ai.nunmai.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.nunmai."gateway"',
        # Same class on the systemctl and nunmai-CLI branches.
        'systemctl re"start" nunmai-gateway',
        'nunmai gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.nunmai.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.nunmai.update-checker',
        'systemctl re"start" nunmai-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nunmai cron list",
        "nunmai update",
        "nunmai config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `nunmai gateway start` is benign — starting a gateway from inside a
        ...
        "nunmai gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway nunmai
        # services must not be falsely blocked (the old `.*nunmai` matched any
        # nunmai token).
        "launchctl unload ai.nunmai.update-checker.plist",
        "launchctl restart ai.nunmai.daemon",
        ...
        "systemctl restart nunmai-meta.service",
        "systemctl restart nunmai-cron-helper",
        ...
        "echo after the nunmai gateway restarted cleanly",
        "the nunmai gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "nunmai skill view gateway-notes && echo nunmai gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `nunmai` is a path component
        # there, not a command.
        "cat '/docs/nunmai gateway restart-notes.md'",
        "less /home/user/notes/nunmai gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nunmai gateway restart; echo done",
        "nunmai gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;nunmai gateway restart",
        "true && nunmai gateway stop",
        "echo $(nunmai gateway restart)",
        "echo `nunmai gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.nunmai.gateway"])',
        'subprocess.run(["nunmai", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.nunmai.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["nunmai", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: nunmai gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `nunmai -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry NUNMAI_HOME/NUNMAI_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("NUNMAI_PROFILE", "zeus")
        monkeypatch.delenv("NUNMAI_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "nunmai -p zeus gateway stop",
        "nunmai -p zeus gateway restart",
        "nunmai --profile zeus gateway restart",
        "nunmai --profile zeus gateway stop",
        "nunmai --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "nunmai -v -p zeus gateway restart",
        "nunmai -p zeus -v gateway restart",
        "nunmai --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "nunmai -p venus gateway stop",
        "nunmai -p venus gateway restart",
        "nunmai --profile venus gateway restart",
        "nunmai --profile=venus gateway stop",
        "nunmai -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "nunmai -p zeus gateway start",
        "nunmai -p zeus gateway start --all",
    ])
    ...
    def test_nunmai_home_derived_profile(self, monkeypatch):
        # Without NUNMAI_PROFILE the guard falls back to the NUNMAI_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("NUNMAI_PROFILE", raising=False)
        monkeypatch.delenv("NUNMAI_PROFILE_NAME", raising=False)
        import nunmai_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("nunmai -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("nunmai -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("nunmai -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("nunmai gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_nunmai_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade nunmai then run nunmai gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_nunmai_gateway_once.sh). The script must live under
        # NUNMAI_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
        scripts_dir = tmp_path / ".nunmai" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_NUNMAI_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _NUNMAI_GATEWAY=1.

    Issue #37453: systemctl --user restart nunmai-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart nunmai-gateway",
        "systemctl --user restart nunmai-gateway",
        "systemctl stop nunmai-gateway.service",
        "nunmai gateway restart",
        "nunmai gateway uninstall",
        "launchctl kickstart gui/501/ai.nunmai.gateway",
        "launchctl bootout gui/501/ai.nunmai.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.nunmai.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nunmai/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nunmai.gateway.restart-once.plist",
        "pkill -f nunmai.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart nunmai-gateway", force=True
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-nunmai label: label-independent detection is the point
        # (#62891 second reproduction used `ai.nunmai.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.nunmai.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _NUNMAI_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _NUNMAI_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_NUNMAI_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="nunmai gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["nunmai gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.nunmai.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.nunmai.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-nunmai systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("please run nunmai gateway restart", None)
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfenunmai gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under NUNMAI_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("nunmai gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 nunmai gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_nunmai_home()` falls back to Path.home(), which raises
        RuntimeError when neither NUNMAI_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart nunmai-gateway' .",
        "grep -rn 'nunmai gateway restart' ./logs",
        "rg 'nunmai gateway restart' ../archive",
        "grep -c 'systemctl stop nunmai-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM nunmai_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell nunmai gateway restart"',
        'sqlite3 ./db ".system systemctl restart nunmai-gateway"',
        'psql ./x -c "\\! systemctl restart nunmai-gateway"',
        "grep -r 'nunmai gateway restart' . | sh",
        "grep -r 'nunmai gateway restart' ./logs | bash",
        "grep -r 'nunmai gateway restart' . | sudo sh",
        "grep -r 'x' . ; nunmai gateway restart",
        "grep -r 'x' . && systemctl restart nunmai-gateway",
        'grep -r "$(nunmai gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart nunmai-gateway",
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    nunmai_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run nunmai gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai"))
        (tmp_path / ".nunmai").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run nunmai gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop nunmai-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart nunmai-gateway' /var/log/syslog",
        "rg 'nunmai gateway restart' /home/user/.nunmai/logs/",
        "journalctl -u nunmai-gateway --grep 'systemctl restart nunmai-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'nunmai_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell nunmai gateway restart"',
        'psql -c "\\! systemctl restart nunmai-gateway"',
        "grep 'systemctl restart nunmai-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart nunmai-gateway",
        'grep "$(systemctl restart nunmai-gateway)" f',
        "grep 'restart' log; systemctl restart nunmai-gateway",
        'sqlite3 db "SELECT 1"; nunmai gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "nunmai gateway restart",
        "sudo systemctl stop nunmai-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart nunmai-gateway%'\""
        )
