...
def test_message_pages_identify_the_serving_profile(tmp_path, monkeypatch, serving_profile):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(work_home))
    # conftest redirects this legacy override independently of NUNMAI_HOME.
    monkeypatch.setattr("nunmai_state.DEFAULT_DB_PATH", work_home / "state.db")
    ...
    from nunmai_cli.web_routers.sessions import manage_router
...
def test_message_pages_type_untyped_failed_turn_rows(tmp_path, monkeypatch):
    ...
    try:
        ...
        db.append_messages_batch("s", [
            ...
            {"role": "user", "content": "c"},
            {"role": "assistant", "content": f"Quoting Nunmai: {FAILED_TURN_NOTICE}"},
        ])
    finally:
        db.close()
