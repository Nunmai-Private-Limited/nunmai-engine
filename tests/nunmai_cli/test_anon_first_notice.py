"""CLI one-time notice: an explicit-provider install learns once that the Nunmai free tier exists."""
...
def test_signed_in_account_is_never_told_about_the_free_tier():
    with _auth_store_lock():
        store = _load_auth_store()
        store.setdefault("providers", {})["nous"] = {
            "auth_method": "oauth", "access_token": _jwt(client_id="nunmai-cli", account_tier="pro"),
