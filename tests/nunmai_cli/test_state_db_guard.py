...
from nunmai_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    NUNMAI_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def nunmai_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("nunmai_cli.config") or mod == "nunmai_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, nunmai_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, nunmai_home, capsys, monkeypatch):
    ...
    def test_failed_snapshot_is_loud_and_update_continues(self, nunmai_home, capsys, monkeypatch):
