...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("nunmai_cli.relaunch.resolve_nunmai_bin", lambda: "/usr/bin/nunmai")
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(tmp_path, xdg_home, monkeypatch):
    ...
    nunmai_bin = tmp_path / "bin" / "nunmai"
    nunmai_bin.parent.mkdir()
    nunmai_bin.write_text("#!/usr/bin/env python3\nimport nunmai_cli\n", encoding="utf-8")
    nunmai_bin.chmod(0o755)
    monkeypatch.setattr("nunmai_cli.relaunch.resolve_nunmai_bin", lambda: str(nunmai_bin))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
