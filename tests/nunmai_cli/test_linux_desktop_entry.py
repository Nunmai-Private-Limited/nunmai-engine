"""Tests for the Linux XDG desktop entry installed by ``nunmai desktop``."""
...
from nunmai_cli import linux_desktop_entry as lde
...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/nunmai on the dev machine.
...
def _stub_install(tmp_path, monkeypatch) -> None:
    nunmai_bin = tmp_path / "bin" / "nunmai"
    nunmai_bin.parent.mkdir(exist_ok=True)
    nunmai_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "nunmai_cli.relaunch.resolve_nunmai_bin", lambda: str(nunmai_bin)
    )
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    assert entry == xdg_home / "applications" / "nunmai.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `nunmai` would not resolve.
    assert values["Exec"] == f"{nunmai_bin} desktop"
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nunmai.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "nunmai_cli.relaunch.resolve_nunmai_bin", lambda: "/usr/bin/nunmai"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("nunmai_cli.relaunch.resolve_nunmai_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m nunmai_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `nunmai`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(
    ...
    nunmai_bin = tmp_path / "bin" / "nunmai"
    nunmai_bin.parent.mkdir()
    nunmai_bin.write_text(
        "#!/usr/bin/env python3\nimport nunmai_cli\n", encoding="utf-8"
    )
    nunmai_bin.chmod(0o755)
    monkeypatch.setattr(
        "nunmai_cli.relaunch.resolve_nunmai_bin", lambda: str(nunmai_bin)
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
    ...
    assert str(nunmai_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    nunmai_bin.parent.mkdir()
    nunmai_bin.write_text(
        '#!/bin/bash\nexec /opt/nunmai/venv/bin/python "$@"\n', encoding="utf-8"
    )
    nunmai_bin.chmod(0o755)
    monkeypatch.setattr(
        "nunmai_cli.relaunch.resolve_nunmai_bin", lambda: str(nunmai_bin)
    ...
    assert exec_line == f"{nunmai_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    nunmai_bin.parent.mkdir()
    interpreter = os.path.abspath(sys.executable)
    nunmai_bin.write_text(f"#!{interpreter}\nimport nunmai_cli\n", encoding="utf-8")
    nunmai_bin.chmod(0o755)
    monkeypatch.setattr(
        "nunmai_cli.relaunch.resolve_nunmai_bin", lambda: str(nunmai_bin)
    )
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/nunmai (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "nunmai"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport nunmai_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "nunmai"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "nunmai" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m nunmai_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m nunmai_cli.main desktop` fallback. Persisting the
    ...
    def fake_resolve():
        # Mirror resolve_nunmai_bin's chain: argv[0] → relative → PATH → None.
        return sys.argv[0] if sys.argv[0] else None

    monkeypatch.setattr("nunmai_cli.relaunch.resolve_nunmai_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's nunmai script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "nunmai"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "nunmai"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_never_persists_a_checkout_internal_path_hit(tmp_path, xdg_home, monkeypatch):
    ...
    cycle), and every flip rewrites nunmai.desktop. A rewrite landing
...
def test_exec_finds_known_wrapper_when_resolver_has_no_candidate(
    ...
    `python -m`, and PATH has no `nunmai`) makes resolve_nunmai_bin return
    ...
    `<python> -m nunmai_cli.main desktop` module form. Each flip between the
    wrapper and module forms rewrites nunmai.desktop on the next launch; any
    ...
    known_wrapper = tmp_path / "cold-home" / ".local" / "bin" / "nunmai"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "nunmai"} "$@"\n',
        encoding="utf-8",
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/nunmai may
    ...
    other_root.mkdir()
    foreign_wrapper = tmp_path / "known-home" / ".local" / "bin" / "nunmai"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/nunmai",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/nunmai",
            id="termux-prefix-first",
        ),
        pytest.param(
            "root-fhs",
            {"__EUID0__": "1"},
            "/usr/local/bin/nunmai",
            id="root-fhs",
        ),
        pytest.param(
            "non-root-no-fhs",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/nunmai") < candidates.index(
            f"{sentinel_home}/.local/bin/nunmai"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/nunmai" not in candidates
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when Nunmai is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "nunmai"
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_nunmai_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "nunmai-engine"
    (home_real / "nunmai-engine").mkdir()

    shim = home_link / ".local" / "bin" / "nunmai"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/nunmai "$@"\n',
        encoding="utf-8",
    )
...
def test_needs_interpreter_case_insensitive_match(tmp_path, monkeypatch):
    ...
    console_script = venv_bin / "nunmai"
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "nunmai"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "nunmai"
...
def test_install_places_1024_png_in_256x256_not_scalable(
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nunmai.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nunmai.png"
    assert values["Icon"] == "nunmai"
    assert dest.is_file()
    assert dest.read_bytes() == lde.icon_path(root).read_bytes()
...
def test_install_removes_stale_scalable_png(tmp_path, xdg_home, monkeypatch):
    """v2026.8.31 wrote the PNG into scalable/. A later nunmai desktop
...
def test_install_exact_48_png_uses_48x48_dir(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "48x48" / "apps" / "nunmai.png"
    assert dest.is_file()
    assert not (
        xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nunmai.png"
    ).exists()
...
def test_install_resizes_decodable_png_to_panel_sizes(
    ...
    dest_24 = xdg_home / "icons" / "hicolor" / "24x24" / "apps" / "nunmai.png"
    dest_256 = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nunmai.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nunmai.png"
