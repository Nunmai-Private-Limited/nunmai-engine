"""Tests for ``nunmai doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from nunmai_cli import doctor_live
from nunmai_cli.doctor_live import (
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `nunmai doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("nunmai_cli.doctor.NUNMAI_HOME", tmp_path / "home")
        monkeypatch.setattr("nunmai_cli.doctor.PROJECT_ROOT", tmp_path / "root")
