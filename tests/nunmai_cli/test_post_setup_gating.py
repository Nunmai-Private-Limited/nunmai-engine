...
empty `env_vars` — causing `nunmai tools` to write the toolset to config
...
class TestPostSetupGate:
    ...
    def test_incompatible_cua_driver_forces_setup(self, monkeypatch):
        from nunmai_cli import tools_config, tools_config_post_setup
...
class TestBrowserBackendPrompt:
    ...
    This made the provider picker re-appear every time `nunmai tools` was
...
class TestBrowserBackendPromptThroughLoader:
    ...
    behaviour against the merged dict a real ``nunmai tools`` run feeds the gate.
    """
    ...
    def test_unset_browser_still_prompts(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(self._home(tmp_path, "cli: {}\n")))
        config = load_config()
    ...
    def test_explicit_backend_skips_prompt(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(self._home(tmp_path, "browser:\n  backend: browser-use\n")))
        config = load_config()
        assert tools_config._toolset_needs_configuration_prompt("browser", config) is False
