"""Tests for the post-update *import* guard in ``nunmai update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not nunmai_main's. Both modules expose
...
def test_import_guard_does_not_accept_forged_static_marker(monkeypatch, tmp_path):
    ...
    (tmp_path / "consumer.py").write_text(
        "import os, sys\n"
        "sys.stdout.write('__NUNMAI_IMPORT_HEALTH__[]')\n"
        "sys.stdout.flush()\n"
        "os._exit(7)\n"
    )
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("consumer",))
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``nunmai update`` can run under a different Python than the install's.
...
def test_import_guard_rejects_module_satisfied_only_by_inherited_pythonpath(
    ...
    stale.mkdir()
    (stale / "nunmai_stale_supply.py").write_text("VALUE = 'from the stale tree'\n")

    monkeypatch.setattr(
        update_cmd, "_UPDATE_CRITICAL_MODULES", ("nunmai_stale_supply",)
    )
    monkeypatch.setattr(
        update_cmd_deps, "_UPDATE_CRITICAL_MODULES", ("nunmai_stale_supply",)
    )
    ...
    assert ok is False
    assert module == "nunmai_stale_supply"
    assert error is not None and "nunmai_stale_supply" in error
...
def test_import_guard_accepts_candidate_with_foreign_pythonpath(monkeypatch, tmp_path):
    ...
    stale = tmp_path / "stale"
    stale.mkdir()
    (stale / "nunmai_stale_supply.py").write_text("VALUE = 'stale'\n")
    (tmp_path / "nunmai_stale_supply.py").write_text("VALUE = 'candidate'\n")
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "nunmaix"])
...
@pytest.mark.parametrize("modname", ["tools.todo_tool", "agent.context_compressor",
                                     "nunmai_constants", "nunmai_cli.config", "cli"])
...
def test_import_probe_sees_a_stale_editable_finder_instead_of_the_checkout_cwd(monkeypatch, tmp_path):
    ...
    (tmp_path / "nunmai_probe_pkg").mkdir()
    (tmp_path / "nunmai_probe_pkg" / "__init__.py").write_text("")
    venv = tmp_path / "venv"
    python = venv / "bin" / "python"
    python.parent.mkdir(parents=True)
    python.symlink_to(sys.executable)
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("nunmai_probe_pkg",))
    monkeypatch.setattr(update_cmd_deps, "_UPDATE_CRITICAL_MODULES", ("nunmai_probe_pkg",))
    ...
    finder = site / "__editable___nunmai_agent_0_21_0_finder.py"
    finder.write_text("MAPPING = {}\n", encoding="utf-8")  # stale: no nunmai_probe_pkg
    ...
    assert set(failures) == {"nunmai_probe_pkg"}
    assert failures["nunmai_probe_pkg"][0] == "ModuleNotFoundError"
