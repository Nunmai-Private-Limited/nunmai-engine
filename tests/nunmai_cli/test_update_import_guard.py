...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    syntax_ok, _, _ = nunmai_main._validate_critical_files_syntax(tmp_path)
    ...
    ok, module, error = nunmai_main._validate_critical_modules_import(tmp_path)
    assert ok is False
...
def test_import_guard_passes_on_consistent_tree(monkeypatch, tmp_path):
    ...
    assert nunmai_main._validate_critical_modules_import(tmp_path) == (True, None, None)
...
def test_import_guard_ignores_non_import_errors(monkeypatch, tmp_path):
    ...
    ok, _, _ = nunmai_main._validate_critical_modules_import(tmp_path)
    assert ok is True
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("nunmai update" in line for line in hint)
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `nunmaix`
    (third-party) matched the probe's loose `startswith("nunmai")`. A user
    ...
    """
    from nunmai_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "nunmaix"):
