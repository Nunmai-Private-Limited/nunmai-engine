"""Tests for nunmai_cli.process_identity — spawn tags, the machine spawn
...
(NUNMAI_SPAWN), long-lived processes self-register (pid, create_time,
purpose, spawner) in spawn-ledger.json, and `nunmai update` reaps holders the
...
def test_install_id_stable_and_path_scoped():
    a = pi.install_id(Path("/opt/nunmai"))
    assert a == pi.install_id(Path("/opt/nunmai"))
...
def test_register_self_survives_non_utf8_argv(tmp_path):
    ...
    bad_argv = ["nunmai", "serve", os.fsdecode(b"/tmp/project-\xff")]  # surrogate-escaped path
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.object(pi, "_ledger_path", return_value=ledger), \
         patch.object(pi.os, "getpid", return_value=999), \
...
def test_register_self_falls_back_to_desktop_parent_env(tmp_path):
    # Legacy Desktop: no NUNMAI_SPAWN, but NUNMAI_PARENT_PID + winms marker.
    ledger = tmp_path / "spawn-ledger.json"
    fake = _fake_psutil({999: 50.0})
    env = {
        "NUNMAI_PARENT_PID": "888",
        "NUNMAI_PARENT_START_MARKER": "winms:1755689000123",
    }
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.dict(pi.os.environ, env, clear=False), \
...
def _holders(*pids):
    return [(p, "python.exe", f"python.exe -m nunmai_cli.main --profile p{p} serve") for p in pids]
...
def test_desktop_ssh_backend_spawn_shape_is_desktop_owned(monkeypatch):
    """Desktop's SSH spawn is ``env NUNMAI_DESKTOP=1 nunmai serve --isolated ... --ssh-session-token-file F``
    ...
    ssh_argv = ["serve", "--isolated", "--host", "127.0.0.1", "--port", "0",
                "--ssh-session-token-file", "/home/u/.nunmai/desktop-ssh/abc.token"]

    assert pi.is_desktop_owned_backend(ssh_argv) is True
    monkeypatch.setattr(sys, "argv", ["nunmai", *ssh_argv])
    assert pi.is_desktop_owned_backend() is True
    # The bare inherited flag (a Desktop terminal pane running `nunmai serve`) is still not ownership.
