"""Tests for nunmai_cli.gateway_windows."""
...
import nunmai_cli.gateway as gateway
import nunmai_cli.gateway_windows as gateway_windows
import nunmai_cli.setup as setup


_BREAKAWAY_MARKER = "_NUNMAI_GATEWAY_BREAKAWAY"
...
def test_exec_schtasks_decodes_ansi_output_under_utf8_mode(monkeypatch):
    ...
    xml = r'<Arguments>//B //Nologo "C:\Users\方舟\AppData\Local\nunmai\gateway-service\Nunmai_Gateway.vbs"</Arguments>'
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Nunmai_Gateway", "/XML"])
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda nunmai_home: "")
    monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: str(nunmai_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "nunmai_cli.main"]
    assert cwd == str(nunmai_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "nunmai_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda home=None: (argv, cwd, {"NUNMAI_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda home=None: (
            argv,
            cwd,
            {"NUNMAI_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_nunmai_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".nunmai"
        home.mkdir()
        monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".nunmai"
        project = tmp_path / "checkout"
        monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: missing)
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Nunmai\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Nunmai\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    """
    calls = []
    script_path = tmp_path / "Nunmai_Gateway_alice.cmd"
    xml_seen = {}
    ...
    ok, detail = gateway_windows._install_scheduled_task("Nunmai_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Nunmai_Gateway_alice")
    ...
    assert "Nunmai_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "nunmai_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("NUNMAI_HOME", "PYTHONIOENCODING", "NUNMAI_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_atomic_write_leaves_no_staging_file_when_swap_fails(monkeypatch, tmp_path):
    ...
    startup.mkdir()
    entry, staging = startup / "Nunmai_Gateway.vbs", startup / "Nunmai_Gateway.tmp"
...
def test_uninstall_and_reinstall_sweep_stale_startup_staging_file(monkeypatch, tmp_path):
    ...
    startup = tmp_path / "Startup"
    startup.mkdir()
    entry, staging = startup / "Nunmai_Gateway_alice.vbs", startup / "Nunmai_Gateway_alice.tmp"
    script = tmp_path / "task" / "Nunmai_Gateway_alice.cmd"

    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Nunmai_Gateway_alice")
    ...
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Nunmai_Gateway_alice.cmd")
...
def test_status_names_and_uninstall_removes_pre_suffix_launchers(monkeypatch, tmp_path, capsys):
    """#116157: a Scheduled Task ``Nunmai_Gateway`` and a Startup ``Nunmai_Gateway.vbs`` left from before
    ...
    startup.mkdir()
    legacy_vbs = startup / "Nunmai_Gateway.vbs"
    legacy_vbs.write_text(gateway_windows._build_startup_launcher(home / "gateway-service" / "Nunmai_Gateway.cmd"), encoding="utf-8")
    legacy_pair = home / "gateway-service" / "Nunmai_Gateway.cmd"
    legacy_pair.write_text("legacy", encoding="utf-8")
    schtasks_calls = []
    registered = {"Nunmai_Gateway"}
    task_xml = gateway_windows._build_scheduled_task_xml("Nunmai_Gateway", home / "gateway-service" / "Nunmai_Gateway.vbs", None)
    ...
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Nunmai_Gateway_alice")
    monkeypatch.setattr(gateway_windows, "get_task_script_path", lambda: home / "gateway-service" / "Nunmai_Gateway_alice.cmd")
    monkeypatch.setattr(gateway_windows, "get_startup_entry_path", lambda: startup / "Nunmai_Gateway_alice.vbs")
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Nunmai_Gateway_alice.cmd")
    monkeypatch.setattr(gateway_windows, "_startup_dir", lambda: startup)
    monkeypatch.setattr(gateway_windows, "_nunmai_home", lambda: home)
    monkeypatch.setattr(gateway_windows, "_exec_schtasks", fake_schtasks)
    ...
    assert "legacy pre-suffix Scheduled Task still installed: Nunmai_Gateway" in out

    gateway_windows.uninstall()
    out = capsys.readouterr().out
    assert "Removed legacy pre-suffix Scheduled Task 'Nunmai_Gateway'" in out
    assert not legacy_vbs.exists() and not legacy_pair.exists()
    assert ["/Delete", "/F", "/TN", "Nunmai_Gateway"] in schtasks_calls
    gateway_windows.status()
...
def test_secondary_profile_leaves_default_profiles_bare_launchers_alone(monkeypatch, tmp_path, capsys):
    """The bare ``Nunmai_Gateway`` task and Startup entry are the LIVE identity of the default ``~/.nunmai``
    ...
    startup.mkdir()
    default_vbs = startup / "Nunmai_Gateway.vbs"
    default_vbs.write_text(gateway_windows._build_startup_launcher(default_home / "gateway-service" / "Nunmai_Gateway.cmd"), encoding="utf-8")
    task_xml = gateway_windows._build_scheduled_task_xml("Nunmai_Gateway", default_home / "gateway-service" / "Nunmai_Gateway.vbs", None)
    schtasks_calls = []

    def fake_schtasks(args):
        schtasks_calls.append(args)
        if args[0] == "/Query" and args[args.index("/TN") + 1] == "Nunmai_Gateway":
            return (0, task_xml, "")
        return (1, "", "ERROR: The system cannot find the file specified.")

    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Nunmai_Gateway_work")
    monkeypatch.setattr(gateway_windows, "get_task_script_path", lambda: home / "gateway-service" / "Nunmai_Gateway_work.cmd")
    monkeypatch.setattr(gateway_windows, "get_startup_entry_path", lambda: startup / "Nunmai_Gateway_work.vbs")
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Nunmai_Gateway_work.cmd")
    monkeypatch.setattr(gateway_windows, "_startup_dir", lambda: startup)
    monkeypatch.setattr(gateway_windows, "_nunmai_home", lambda: home)
    monkeypatch.setattr(gateway_windows, "_exec_schtasks", fake_schtasks)
    monkeypatch.setattr(gateway_windows, "_gateway_pids", lambda *a, **k: [])
    ...
    assert not any(call[0] == "/Delete" and "Nunmai_Gateway" in call for call in schtasks_calls)
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>"C:\\Users\\me\\.nunmai\\gateway-service\\Nunmai_Gateway.vbs"</Arguments>
    </Exec>
  </Actions>
</Task>
"""
...
def test_scheduled_task_drift_names_missing_hardening_leaves(monkeypatch):
    ...
    ``nunmai gateway status`` prints together with the ``nunmai gateway install`` repair hint."""
    launcher = Path(r"C:\Users\me\.nunmai\gateway-service\Nunmai_Gateway.vbs")
    template = gateway_windows._build_scheduled_task_xml("Nunmai_Gateway", launcher, r"PC\me")
...
def test_reconcile_scheduled_task_reregisters_only_on_drift(monkeypatch, tmp_path):
    ...
    assert gateway_windows.reconcile_scheduled_task("Nunmai_Gateway") is True
    ...
    calls.clear()
    assert gateway_windows.reconcile_scheduled_task("Nunmai_Gateway") is False
...
def _arrange_uninstalled_start(monkeypatch):
    ...
    installs, spawns = [], []
    monkeypatch.delenv("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN", raising=False)
    monkeypatch.delenv("NUNMAI_NONINTERACTIVE", raising=False)
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "_print_start_attestation_warning", lambda: None)
...
def test_stdin_interactive_only_when_isatty_and_a_console_answers_get_console_mode():
    """Windows CRT isatty() is True for the NUL device (`nunmai gateway start < NUL`, stdin=DEVNULL), so
...
def test_start_with_nul_stdin_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    ...
    assert "nunmai gateway install" in capsys.readouterr().out


def test_start_without_tty_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    """`nunmai gateway start < /dev/null` must not answer the persistence question with a default Yes
    ...
    assert installs == [] and spawns == [1]
    out = capsys.readouterr().out
    assert "nunmai gateway install" in out and "did not complete" not in out
...
def test_start_on_tty_hands_both_answers_to_install_and_honours_the_env_opt_out(monkeypatch):
    ...
    again). NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN=0 → no question, no install, a plain start."""
    installs, spawns = _arrange_uninstalled_start(monkeypatch)
    monkeypatch.setattr(setup, "is_interactive_stdin", lambda: True)
    ...
    installs.clear()
    monkeypatch.setenv("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN", "0")
...
# resume_pending=True) never fires when `nunmai gateway stop` kills the
...
def test_nunmai_owns_windows_service_requires_name_or_binary_under_a_nunmai_root():
    """Task Scheduler (``Schedule`` in svchost) above a task-launched gateway is never its supervisor;
    a service is Nunmai-owned only by a ``nunmai*`` name or a binary under the install (#97208)."""
    roots = (
        r"C:\Users\kaize\AppData\Local\nunmai\nunmai-engine",
        r"C:\Users\kaize\AppData\Local\nunmai\nunmai-engine\venv\Scripts",
        r"C:\Users\kaize\AppData\Local\nunmai\gateway-service",
    )
    owns = gateway_windows.nunmai_owns_windows_service
    ...
    assert not owns("Other", r"C:\Users\kaize\AppData\Local\nunmai\nunmai-engine-fork\run.exe", roots)

    assert owns("NunmaiGateway", r"C:\nssm\nssm.exe", roots)
    assert owns("Nunmai_Gateway_derek", "", roots)
    assert owns("gw", r'"C:\Users\KAIZE\AppData\Local\nunmai\nunmai-engine\venv\Scripts\nunmai.exe" gateway run', roots)
    assert owns("gw", r"C:\Users\kaize\AppData\Local\nunmai\gateway-service\Nunmai_Gateway.cmd", roots)
