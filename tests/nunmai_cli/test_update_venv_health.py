...
def test_venv_dependency_set_stale_compares_installed_distribution_with_checkout(tmp_path):
    ...
    with patch.object(cli_main, "PROJECT_ROOT", tmp_path), patch.object(cli_main, "_is_windows", return_value=False), \
        ...
        assert update_cmd._venv_dependency_set_stale() == (True, "installed nunmai-engine 0.20.6, checkout is 0.21.2")
...
def test_current_checkout_with_stale_dependency_set_runs_the_sync(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nunmai_cli.managed_uv.update_managed_uv", lambda **kwargs: None)
    monkeypatch.setattr("nunmai_cli.managed_uv.ensure_uv", lambda **kwargs: "uv")
    ...
    assert run((True, "installed nunmai-engine 0.20.6, checkout is 0.21.2")) is True
    assert calls == ["sync"]
