"""ensure_nunmai_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".nunmai"
    second = tmp_path / "b" / ".nunmai"

    monkeypatch.setenv("NUNMAI_HOME", str(first))
    cfg.ensure_nunmai_home()

    # Profile switch: NUNMAI_HOME moves → the new path is ensured too.
