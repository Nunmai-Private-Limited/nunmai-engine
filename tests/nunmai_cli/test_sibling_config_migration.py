...
`nunmai update` migrated only the active profile's config.yaml; sibling
...
context-local NUNMAI_HOME override.
...
import nunmai_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        nunmai_constants, "get_process_nunmai_home", lambda: active_home
    )
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_nunmai_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_nunmai_home_override() == before
