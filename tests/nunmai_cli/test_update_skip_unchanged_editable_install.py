"""``nunmai update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``nunmai.exe`` gets quarantined,
...
from nunmai_cli.update_cmd import _editable_install_is_current
...
def _finder(repo, mapping, *, layout="lib/python3.11/site-packages", annotated=False):
    ...
    (site / "__editable___nunmai_agent_0_21_4_finder.py").write_text(body, encoding="utf-8")
    return site
...
def test_stale_or_unreadable_finder_map_refuses_the_skip(repo, monkeypatch, annotated):
    ...
    monkeypatch.setattr("nunmai_cli.update_cmd_deps.project_venv_dir", lambda _: repo / "venv")
    assert _editable_install_is_current(GIT, repo, before) is False
    (site / "__editable___nunmai_agent_0_21_4_finder.py").write_text("MAPPING = (\n", encoding="utf-8")
    assert _editable_install_is_current(GIT, repo, before) is False
