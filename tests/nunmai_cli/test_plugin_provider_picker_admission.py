...
was invisible to ``/model``, ``nunmai model`` and ``list_available_providers()`` — while the in-tree
...
def test_plugin_profiles_are_admitted_by_slug_not_auth_type(auth_type):
    ...
    from nunmai_cli.models_catalog_static import CANONICAL_PROVIDERS, _plugin_provider_enters_picker
...
def test_external_process_plugin_authenticated_flag_tracks_binary_and_catalog_uses_fallback(
    ...
    from providers.base import ProviderProfile
    from nunmai_cli import models, models_catalog_static
