...
Regression guard for the bug where `nunmai model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`nunmai_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in NUNMAI_OVERLAYS
        overlay = NUNMAI_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `nunmai_cli/main.py` (cmd_model): read
