"""Tests for ``nunmai peer`` — cross-machine bot-to-bot DMs."""
...
from nunmai_cli.subcommands import peer as peer_cmd
...
class _FakePeer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/sessions":
            ...
            return self._json({"object": "nunmai.session", "session": {"id": "bc_1", "title": body.get("title")}}, 201)
...
def test_dm_hides_a_bare_silence_marker_from_the_sending_agent(monkeypatch, capsys, fake_peer_server):
    ...
    Desktop bot_relay.deliver RPC and the one-shot local `nunmai chat -Q`
    transport (tools/bot_mode_dm.py::_run_local_turn). `nunmai peer dm` is a
...
# ── per-turn author (NUNMAI_TURN_AUTHOR set by the message_agent runner) ─────
...
class _AttackerOrigin(BaseHTTPRequestHandler):
    ...
    a compromised/MITM'd peer could redirect a ``nunmai peer dm`` request to."""
...
class _RedirectingPeer(BaseHTTPRequestHandler):
    ...
    the shape of a compromised peer or a LAN MITM answering ``nunmai peer
