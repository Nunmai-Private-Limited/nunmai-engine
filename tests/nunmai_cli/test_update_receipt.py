"""Tests for nunmai_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
from nunmai_cli import update_cmd, update_cmd_maint


@pytest.fixture()
def receipt_home(tmp_path, monkeypatch):
    """Isolated NUNMAI_HOME for receipt writes."""
    home = tmp_path / ".nunmai"
    home.mkdir()
    # ``_receipt_dir`` resolves through ``nunmai_constants.get_nunmai_home`` (env var), not
    # ``nunmai_cli.config`` — patch where production reads.
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["nunmai-gateway"],
        ...
        assert gr["restarted_services"] == ["nunmai-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another nunmai.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "nunmai.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            nunmai_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            nunmai_main,
        ...
        monkeypatch.setattr(
            nunmai_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import nunmai_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "nunmai_cli.profiles._get_default_nunmai_home", lambda: home
        )
        monkeypatch.setattr(
            "nunmai_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
