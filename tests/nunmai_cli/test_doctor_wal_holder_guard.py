"""Regression: ``nunmai doctor --fix`` must not checkpoint the live WAL under a running gateway.
...
# that depend on Nunmai *choosing* WAL mode (declined on vulnerable SQLite
...
def test_doctor_names_retired_wal_holders_instead_of_healthy_state_db(tmp_path, monkeypatch, capsys):
    ...
    import nunmai_cli.doctor_state as doctor_state
    import nunmai_state_dbfile

    db = tmp_path / "state.db"
    db.write_bytes(b"")
    monkeypatch.setattr(doctor, "NUNMAI_HOME", tmp_path)
    monkeypatch.setattr(nunmai_state_dbfile, "iter_deleted_sqlite_sidecar_holders",
