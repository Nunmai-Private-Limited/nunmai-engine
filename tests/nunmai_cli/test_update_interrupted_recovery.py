...
Covers the breadcrumb lifecycle so a ``nunmai update`` killed mid-install
...
from nunmai_cli import main_install_repair as mir
...
def pending_core_install(tmp_path, monkeypatch):
    """A source checkout whose last ``nunmai update`` (owner now dead) left the core install
    ...
    monkeypatch.setattr(mir, "_windows_running_nunmai_launcher_locked", lambda: False)
    monkeypatch.setattr("nunmai_cli.managed_uv.ensure_uv", lambda *a, **k: None)
    installs: list[str] = []
    outcome = {"fail": True}
...
def _launch(root):
    """One ``nunmai`` boot: the pre-import early pass, then main()'s post-import recovery."""
