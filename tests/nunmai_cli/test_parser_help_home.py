...
``get_nunmai_home()`` printed ``[NUNMAI_HOME fallback] ... wrong profile`` on every ``nunmai``
command for users of ``nunmai profile use`` (#112319, also reported in #112839).
"""
...
def test_help_text_names_the_profile_config_once_the_process_is_re_homed(sticky_profile_home, monkeypatch):
    from nunmai_cli._parser import _cfg_path

    monkeypatch.setenv("NUNMAI_HOME", str(sticky_profile_home / "profiles" / "coder"))

    assert _cfg_path() == "~/.nunmai/profiles/coder/config.yaml"
