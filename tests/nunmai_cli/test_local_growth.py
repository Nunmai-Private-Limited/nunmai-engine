...
def test_overrides_roundtrip_and_clear(nunmai_home):
    from nunmai_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(nunmai_home):
    from nunmai_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(nunmai_home):
    ...
    from nunmai_cli.local_runtime.growth import maybe_grow_window
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from nunmai_cli.local_runtime.context_policy import growth_decision
    from nunmai_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Nunmai' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from nunmai_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(nunmai_home, tmp_path, monkeypatch):
    ...
    import nunmai_cli.local_runtime.presets as presets_mod

    from nunmai_cli.local_runtime.catalog import CATALOG
    from nunmai_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(nunmai_home, tmp_path, monkeypatch):
    ...
    from nunmai_cli.local_runtime.estimator import HardwareBudget
    from nunmai_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(nunmai_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(nunmai_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import nunmai_cli.local_runtime.presets as presets_mod

    from nunmai_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from nunmai_cli.local_runtime.growth import save_window_override
...
def test_mtp_plan_matches_cost_at_initial_and_restored_windows(nunmai_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from nunmai_cli.local_runtime import presets
    from nunmai_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from nunmai_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
    from nunmai_cli.local_runtime.growth import save_window_override
...
def test_growth_requires_an_admissible_materialized_preset(nunmai_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from nunmai_cli.local_runtime import bootstrap, catalog, growth, hardware, presets
    from nunmai_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from nunmai_cli.local_runtime.estimator import HardwareBudget, ctx_bytes
    ...
    from nunmai_cli.local_runtime import gguf, estimator
    ...
    from nunmai_cli.local_runtime.binaries import runtimes_root
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(nunmai_home, tmp_path, monkeypatch):
