"""Tests for nunmai -z --usage-file (per-run JSON usage report)."""

import json

import pytest

from nunmai_cli.oneshot import _write_usage_file
...
class TestAuxiliaryLedger:
    ...
    def test_aux_usage_is_a_separate_breakdown_and_main_keys_unchanged(self, tmp_path):
        from nunmai_cli.oneshot import _auxiliary_usage, _attach_auxiliary_usage
    ...
    def test_waits_for_in_flight_title_thread(self, tmp_path):
        ...
        from nunmai_cli.oneshot import _attach_auxiliary_usage
        from nunmai_state import SessionDB
