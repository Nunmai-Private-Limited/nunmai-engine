"""``nunmai usage`` — the non-interactive /usage surface (issue #33094).

Drives the real ``nunmai`` argparse entrypoint; only the network fetch is replaced with a snapshot
...
def _run(argv, fetch):
    with patch.object(nunmai_main, "_plugin_cli_discovery_needed", return_value=False), \
         patch("agent.account_usage.fetch_account_usage", fetch), \
         patch.object(sys, "argv", ["nunmai", *argv]):
        try:
            nunmai_main.main()
...
def test_nunmai_usage_json_is_one_stable_document(capsys):
    calls = []
...
def test_nunmai_usage_without_credential_exits_nonzero_with_one_stderr_line(capsys):
