"""OAuth-shaped model-provider plugins are first-class in `nunmai auth` (#116408).

The seam: ``ProviderProfile.auth_handler``. These tests drive the real ``nunmai auth`` argparse
surface against a plugin discovered from an isolated NUNMAI_HOME, so they fail if the dispatch is
...
def _rediscover() -> None:
    """Point the next profile lookup at the (new) NUNMAI_HOME user plugin dir.
    ...
    for mod in [m for m in sys.modules if m.startswith("_nunmai_user_provider")]:
        del sys.modules[mod]


@pytest.fixture
def install_provider(tmp_path, monkeypatch):
    """Write a model-provider plugin into an isolated NUNMAI_HOME and discover it."""
    installed: list[str] = []

    def _install(name: str = "fake-auth", *, with_handler: bool = True) -> Path:
        """Write (or rewrite) the fixture plugin and re-run discovery."""
        plugin_dir = tmp_path / "nunmai" / "plugins" / "model-providers" / name
...
def _parse_auth_args(argv: list[str]) -> argparse.Namespace:
    """Parse `nunmai auth <argv>` through the real subcommand parser."""
...
def test_oauth_plugin_owns_every_auth_action(tmp_path, install_provider, capsys, argv, expected):
    """An oauth_external plugin registers and `nunmai auth <action>` reaches its handler, args included,
    ...
    assert not (tmp_path / "nunmai" / "auth.json").exists()
...
def test_oauth_plugin_without_handler_fails_loud_and_builtins_are_untouched(tmp_path, install_provider):
    ...
    pool = json.loads((tmp_path / "nunmai" / "auth.json").read_text(encoding="utf-8"))["credential_pool"]
