"""Tests for nunmai_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $NUNMAI_HOME under tmp_path; no real
...
from nunmai_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running nunmai containers, those
    ...
    """
    monkeypatch.setattr(
        "nunmai_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under nunmai_home/profiles/<name>/."""
    p = nunmai_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `nunmai profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    NUNMAI_HOME root (the implicit default profile)."""
    if state is not None:
        (nunmai_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (nunmai_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (nunmai_home / "processes.json").write_text("[]")
...
def test_a_named_profile_slot_is_registered_but_never_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        nunmai_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_the_retired_opt_out_cannot_boot_a_second_gateway_in_the_container(
    ...
    actions = reconcile_profile_gateways(nunmai_home=tmp_path, scandir=scandir, dry_run=False)
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from nunmai_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/nunmai/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
...
def test_a_named_slots_autostart_intent_boots_the_root_slot(tmp_path: Path) -> None:
    """An image only ever driven as `nunmai -p coder gateway start` has no root state at all.
    ...
    nunmai_home = tmp_path / "data"
    nunmai_home.mkdir()
    _make_profile(nunmai_home, "coder", state=None, desired_state="running")

    actions = reconcile_profile_gateways(
        nunmai_home=nunmai_home, scandir=tmp_path / "svc", dry_run=True, container_argv=())
...
def test_a_stopped_fleet_still_boots_nothing(tmp_path: Path) -> None:
    ...
    nunmai_home = tmp_path / "data"
    nunmai_home.mkdir()
    _seed_default_root(nunmai_home, state="stopped")
    _make_profile(nunmai_home, "coder", state=None, desired_state="stopped")
    _make_profile(nunmai_home, "ops", state=None, desired_state="startup_failed")
