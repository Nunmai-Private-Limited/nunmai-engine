...
def _run(monkeypatch, capsys, argv_tail, db):
    ...
    monkeypatch.setattr(nunmai_state, "SessionDB", lambda *args, **kwargs: db)
    monkeypatch.setattr(sys, "argv", ["nunmai", "sessions", *argv_tail])
    try:
        main_mod.main()
        code = 0
...
def test_pin_multiple_ids_one_missing(monkeypatch, capsys):
    ...
    assert "No session 'nope'" in out and "nunmai sessions list" in out
    assert code == 1
