...
from nunmai_cli import gateway as nunmai_gateway
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\Nunmai\\.venv\\Scripts\\python.exe -m nunmai_cli.main serve --host 127.0.0.1"
    run = "C:\\Nunmai\\.venv\\Scripts\\python.exe -m nunmai_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\Nunmai\\.venv\\Scripts\\python.exe -m nunmai_cli.main"
...
def _running_beta_pause_fixture(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nunmai_gateway, "launch_detached_profile_gateway_restart", lambda p, o: True)
    ready_probes: list = []
...
def test_service_supervised_running_profile_is_not_cold_started(monkeypatch, tmp_path):
    ...
    service = SimpleNamespace(name="NunmaiGw-beta", profile="beta", service_pid=800, gateway_pid=900,
    ...
    assert token["services"] == ["NunmaiGw-beta"]
    assert token["profiles"] == {}
    assert "cold_start_profiles" not in token
