"""One backend serves every profile, so a REST handler that reads ``get_nunmai_home()``
...
@pytest.fixture
def homes(tmp_path, monkeypatch, _isolate_nunmai_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_nunmai_home", lambda: launch_home)
...
def client(monkeypatch, homes):
    ...
    monkeypatch.setattr(nunmai_state, "DEFAULT_DB_PATH", homes["launch"] / "state.db")
    c = TestClient(app)
...
def seams(monkeypatch):
    ...
    ``spawn`` = the argv of every backgrounded ``nunmai`` action, ``db`` = the profile every
    ...
    from nunmai_cli import web_server_gateway, web_server_sessions
    ...
    class _Pool:
        def remove_index(self, _index):
            record["pool_home"].append(str(get_nunmai_home()))
            return type("_Entry", (), {"source": ""})()

        def entries(self):
            return []

    monkeypatch.setattr(web_server_gateway, "_spawn_nunmai_action", _spawn)
...
# Routes whose effect is a backgrounded ``nunmai`` subprocess: the profile must reach its argv,
...
def test_unnamed_profile_still_means_the_launch_profile_on_a_single_profile_host(
    ...
    """A plain ``nunmai serve`` has nothing to confuse: `curl` with no profile is unchanged."""
    from agent.secret_scope import is_multiplex_active
    from nunmai_cli import profiles
    from tui_gateway.launch_profile_policy import activate_multi_profile_hosting_eagerly
...
def readiness_only_in_beta(homes, monkeypatch):
    ...
    """
    from nunmai_cli import web_server_memory
    from nunmai_cli.config import get_nunmai_home

    def _statuses():
        ready = get_nunmai_home().resolve() == homes["worker_beta"].resolve()
...
def test_local_models_quickstart_activates_into_the_named_profile(client, homes, monkeypatch):
    ...
    from nunmai_cli.config import get_nunmai_home
    from nunmai_cli.web_routers import local_models as lm
    ...
    monkeypatch.setattr(lm, "_assign_default", lambda *_a: seen.append(str(get_nunmai_home())))
...
def test_forced_update_check_busts_the_named_profiles_cache(client, homes, monkeypatch):
    from nunmai_cli import banner
    from nunmai_cli.web_routers import actions
    ...
    resp = client.get("/api/nunmai/update/check?force=true&profile=worker_beta")
...
def test_egress_status_reads_the_named_profiles_config(client, homes, monkeypatch):
    from nunmai_cli import proxy_cli
...
def test_memory_provider_setup_runs_in_the_named_profiles_home(client, homes, monkeypatch):
    from nunmai_cli.config import get_nunmai_home
    from nunmai_cli.web_routers import memory_providers as mp
    ...
    monkeypatch.setattr(mp, "_install_memory_provider_setup",
                        lambda name: {"ok": True, "provider": name, "home": str(get_nunmai_home())})
