...
One gateway per host serves every profile. ``nunmai -p X gateway install|start`` used to refuse only
...
``--force`` and pointed at ``nunmai gateway install`` (default) / ``nunmai gateway migrate --multiplex``;
...
def test_named_profile_install_and_start_refuse_without_force_when_no_multiplexer_runs(quiet_host):
    ...
    for profile in ("coder", "ops", "coder"):
        ...
        for verb in ("install", "start"):
            ...
            assert "nunmai gateway install" in out and "nunmai gateway migrate --multiplex" in out
            assert f"nunmai -p {profile} gateway install --force" in out
