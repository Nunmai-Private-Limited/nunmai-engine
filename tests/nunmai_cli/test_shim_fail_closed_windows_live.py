...
holding ``nunmai.exe`` open WITHOUT FILE_SHARE_DELETE, exactly like a
...
def held_shim(tmp_path: Path):
    ...
    shim = scripts / "nunmai.exe"
    shim.write_bytes(b"MZ fake shim")
    (scripts / "nunmai-gateway.exe").write_bytes(b"MZ fake shim")
...
def test_locked_shim_really_cannot_be_renamed(held_shim):
    ...
    with pytest.raises(OSError):
        os.rename(shim, shim.with_name("nunmai.exe.old.premise"))
...
def test_strict_quarantine_refuses_against_real_lock(held_shim, monkeypatch):
    ...
    monkeypatch.setattr(
        nunmai_cli_main_install_repair,
        "_run_install_with_heartbeat",
        lambda cmd, env=None: install_ran.append(cmd),
    )
...
def test_release_then_strict_quarantine_succeeds(tmp_path, monkeypatch):
    ...
    scripts = tmp_path / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    (scripts / "nunmai.exe").write_bytes(b"MZ fake shim")

    holder = subprocess.Popen(
        [sys.executable, "-c", _HOLDER_CODE, str(scripts / "nunmai.exe")],
        stdout=subprocess.PIPE,
        text=True,
    )
    ...
    assert _wait_until(lambda: _rename_round_trips(scripts / "nunmai.exe")), "holder handle never released"
