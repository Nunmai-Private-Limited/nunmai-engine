...
class TestCodexOAuthBootstrapHook:
    ...
    def test_hook_starts_codex_oauth_only_when_credentials_missing(self, monkeypatch, logged_in):
        from nunmai_cli import auth, tools_config_post_setup

        monkeypatch.setattr(auth, "get_codex_auth_status", lambda: {"logged_in": logged_in})
        monkeypatch.setattr("nunmai_cli.setup.prompt_choice", lambda *a, **kw: 0)
        started, saved = [], []
    ...
    def test_hook_prints_auth_command_instead_of_device_login_when_noninteractive(self, monkeypatch, capsys):
        """Desktop's PostSetupRunner spawns `nunmai tools post-setup openai_codex` with stdin=DEVNULL and
        NUNMAI_NONINTERACTIVE=1: nobody can complete a device-code login there, so the hook must name
        the real command and return instead of starting one."""
        from nunmai_cli import auth, tools_config_post_setup

        monkeypatch.setenv("NUNMAI_NONINTERACTIVE", "1")
        ...
        assert "nunmai auth add openai-codex" in capsys.readouterr().out

    def test_readiness_reports_codex_row_from_auth_store(self, monkeypatch):
        from nunmai_cli import auth, tools_config
