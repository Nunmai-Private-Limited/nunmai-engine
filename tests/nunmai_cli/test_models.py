"""Tests for the nunmai_cli models module."""
...
import nunmai_cli.models as _models_mod
from nunmai_cli import models_local
from nunmai_cli import models_validate
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("nunmai_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("nunmai_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        nunmai-engine is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("nunmai_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("nunmai_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_empty_supported_parameters_list_drops_model(self):
        ...
        from nunmai_cli.models import _openrouter_model_supports_tools
...
class TestDetectProviderForModel:
    ...
    def test_short_alias_resolves_to_static_model(self):
        ...
        ), patch("nunmai_cli.models_detect.provider_has_credentials", return_value=True):
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "nunmai_cli.models.fetch_nous_recommended_models",
...
class TestCheckNousFreeTierCache:
    ...
    @patch("nunmai_cli.nous_account.get_nous_portal_account_info")
    ...
    def test_entitlement_cache_is_profile_scoped(self, mock_account, tmp_path):
        ...
        def account_for_active_profile(*, force_fresh=False):
            is_free = nunmai_home_key() == nunmai_home_key(tmp_path / "free")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from nunmai_cli.models import fetch_nous_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("nunmai_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("nunmai_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("nunmai_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("nunmai_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from nunmai_cli.models_pricing import _format_price_per_mtok
...
class TestLocalOllamaModelDiscovery:
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from nunmai_cli.models_local import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("nunmai_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from nunmai_cli.models import _get_ollama_base_url
        from nunmai_cli.models_local import should_use_ollama_native_catalog

        with patch("nunmai_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "nunmai_cli.models_local.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from nunmai_cli.models_local import probe_ollama_local_models
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("nunmai_cli.models_local.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
            assert should_use_ollama_native_catalog("custom", "192.168.1.5:11434/v1") is True
        with patch("nunmai_cli.models_local.probe_ollama_local_models", return_value=None):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("nunmai_cli.models_local.fetch_ollama_local_models") as fetch_local, patch(
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from nunmai_cli.models import _credential_fingerprint
        ...
        ), patch("nunmai_cli.models_local.probe_ollama_local_models") as probe_ollama:
    ...
    def test_clear_provider_models_cache_clears_ollama_native_tags_cache(self):
        import nunmai_cli.models as models
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from nunmai_cli.model_setup_flows import _model_flow_named_custom
        ...
        try:
            with patch("nunmai_cli.config.load_config", return_value=config), patch(
                "nunmai_cli.config.save_config"
            ), patch("nunmai_cli.auth._save_model_choice"), patch(
                "nunmai_cli.auth.deactivate_provider"
            ), patch("nunmai_cli.main_provider_setup._save_custom_provider"), patch(
                "nunmai_cli.curses_ui.curses_radiolist",
    ...
    def test_picker_custom_provider_group_verifies_ambiguous_ollama_port(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value={"providers": {}}), patch(
            "nunmai_cli.models_local.probe_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("nunmai_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
                user_providers={},
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("nunmai_cli.models_local.probe_ollama_local_models") as probe_ollama, patch(
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        try:
            with patch(
                ...
                "nunmai_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from nunmai_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "nunmai_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import nunmai_cli.model_switch as model_switch
...
class TestOpenRouterCatalogDiskCache:
    ...
    def _install_network(monkeypatch, calls):
        ...
        monkeypatch.setattr("nunmai_cli.model_catalog.get_curated_openrouter_models",
                            lambda: [("a/one", "")])
...
class TestAzureFoundryPickerCatalog:
    ...
    def test_provider_model_ids_probes_the_configured_resource(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider._resolve_azure_foundry_runtime", fake_runtime)
        monkeypatch.setattr("nunmai_cli.azure_detect._probe_openai_models", fake_probe)
    ...
    def test_probe_miss_or_resolver_error_keeps_the_empty_static_catalog(self, monkeypatch):
        monkeypatch.setattr("nunmai_cli.azure_detect._probe_openai_models", lambda *a, **k: (False, []))
        monkeypatch.setattr("nunmai_cli.runtime_provider._resolve_azure_foundry_runtime",
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider._resolve_azure_foundry_runtime", raising)
        assert _models_mod.provider_model_ids("azure-foundry", force_refresh=True) == []
