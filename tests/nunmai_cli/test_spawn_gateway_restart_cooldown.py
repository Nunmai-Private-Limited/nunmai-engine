...
class TestRepeatRestartWithinCooldown:
    ...
    @patch(
        "nunmai_cli.web_server._gateway_subcommand",
        return_value=["gateway", "restart"],
    )
    @patch("nunmai_cli.web_server._spawn_nunmai_action")
    @patch("nunmai_cli.web_server._ACTION_PROCS", {})
...
class TestExistingBehaviourIsPreserved:
    ...
    def test_live_child_is_still_reused_without_consulting_the_clock(
        ...
        with patch(
            "nunmai_cli.web_server._ACTION_PROCS", {"gateway-restart": live}
        ), patch(
            "nunmai_cli.web_server._ACTION_COMMANDS",
