"""Tests for nunmai_cli.mcp_catalog and nunmai_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("NUNMAI_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_nunmai_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp NUNMAI_HOME."""
    hh = tmp_path / "nunmai-home"
    hh.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(hh))
    monkeypatch.setattr(
        "nunmai_cli.config.get_nunmai_home", lambda: hh
    )
    monkeypatch.setattr(
        "nunmai_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "nunmai_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_nunmai_home() lazily through nunmai_constants
    monkeypatch.setattr(
        "nunmai_constants.get_nunmai_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from nunmai_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from nunmai_cli.mcp_catalog import list_catalog
    ...
    def test_connector_slug_metadata_reaches_the_catalog_payload(self, catalog_dir):
        from nunmai_cli.mcp_catalog import _parse_manifest
        from nunmai_cli.web_routers.mcp import _catalog_entry_json
    ...
    def test_suggest_onboarding_metadata_is_additive(self, catalog_dir):
        from nunmai_cli.mcp_catalog import _build_server_config, _parse_manifest
        from nunmai_cli.web_routers.mcp import _catalog_entry_json
    ...
    def test_suggest_discovery_metadata_is_bounded_data(self, catalog_dir):
        from nunmai_cli.mcp_catalog import CatalogError, _parse_manifest
        from nunmai_cli.web_routers.mcp import _catalog_entry_json
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from nunmai_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from nunmai_cli.mcp_catalog import _build_server_config
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nunmai_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from nunmai_cli.config import load_config
        from nunmai_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from nunmai_cli.mcp_catalog import install_entry
        from nunmai_cli.config import get_env_value, load_config
    ...
    def test_install_oauth_preregistered_client_writes_oauth_block(self, catalog_dir, monkeypatch):
        ...
        from nunmai_cli import mcp_catalog
        from nunmai_cli.config import get_config_path, get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nunmai_cli.mcp_catalog import install_entry, uninstall_entry
        from nunmai_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from nunmai_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from nunmai_cli import mcp_catalog
        from nunmai_cli.mcp_catalog import _do_git_install
        ...
        from nunmai_cli import git_credentials
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "nunmai_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    ...
    def test_manifest_connector_slugs_are_valid_and_unique(self, monkeypatch):
        from nunmai_cli.mcp_catalog import catalog_diagnostics, list_catalog

        source_catalog = Path(__file__).parents[2] / "optional-mcps"
        monkeypatch.setattr("nunmai_cli.mcp_catalog._catalog_root", lambda: source_catalog)
    ...
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no NUNMAI_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("NUNMAI_OPTIONAL_MCPS", raising=False)
        from nunmai_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Nunmai fetches/launches is pinned
        to an exact version.
