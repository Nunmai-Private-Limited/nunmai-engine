...
Multiplex-only (Teknium ruling): exactly one ``nunmai gateway run`` per host serves every
profile. The obligation used to live in ONE profile's ``NUNMAI_HOME``, so ``nunmai -p coder
...
import nunmai_cli.update_cmd_fleet as fleet
import nunmai_cli.update_host_obligation as host_obligation
import nunmai_cli.update_restart_recovery as recovery
...
def two_profiles(tmp_path, monkeypatch):
    """Two profile NUNMAI_HOMEs behind ONE host state dir — the real multiplex topology."""
...
def no_live_fleet(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.update_receipt.collect_fleet_versions", lambda: [])
...
def test_host_gateway_restarts_once_when_two_profiles_run_the_catch_up(
    ...
    """``nunmai -p coder update`` then ``nunmai -p writer update`` stops the host gateway ONCE."""
    monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda **k: [4242])
    monkeypatch.setattr("nunmai_cli.gateway.supports_systemd_services", lambda: False)
    monkeypatch.setattr("nunmai_cli.gateway.is_macos", lambda: False)
    monkeypatch.setattr("nunmai_cli.gateway.is_windows", lambda: False)
    monkeypatch.setattr("nunmai_cli.gateway._wait_for_gateway_exit", lambda **k: True)
    monkeypatch.setattr(fleet, "_restart_macos_launchd_gateways", lambda *a, **k: None)
    kills: list = []
    monkeypatch.setattr("nunmai_cli.gateway.kill_gateway_processes", lambda **k: kills.append(k))
...
def test_leftover_per_profile_units_restart_their_one_host_process_once(monkeypatch, capsys):
    ...
    fleet._restart_systemd_gateway_units_best_effort(
        failed, _listing(["nunmai-gateway.service", "nunmai-gateway-coder.service", "nunmai-gateway-writer.service"]))

    assert restarted == ["nunmai-gateway"]
    assert failed == []
...
def test_units_with_distinct_live_pids_are_each_restarted(monkeypatch):
    ...
    pids = {"nunmai-gateway": 1, "nunmai-gateway-coder": 2}
    ...
    fleet._restart_systemd_gateway_units_best_effort(
        [], _listing(["nunmai-gateway.service", "nunmai-gateway-coder.service"]))

    assert sorted(restarted) == ["nunmai-gateway", "nunmai-gateway-coder"]


def _host_record(tmp_path, monkeypatch, profiles: list[str]) -> None:
    lock_dir = tmp_path / "gateway-locks"
    lock_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(lock_dir))
...
def test_recovery_keeps_separate_processes_separate(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(tmp_path / "empty-locks"))
    argvs: list[list[str]] = []
...
def test_unwritable_host_state_dir_still_arms_the_obligation(two_profiles, no_live_fleet, monkeypatch, tmp_path):
    ...
    The host record moved out of ``$NUNMAI_HOME`` (writable by construction) into the host state
...
def test_restart_runs_once_per_host_on_a_non_git_install(two_profiles, monkeypatch, capsys):
    ...
    ``mark_host_restart_completed("")`` can never match, so every profile's ``nunmai update``
