...
Regression for the cross-profile leak class in ``nunmai dashboard`` / ``nunmai serve``:
``_config_profile_scope`` bound only NUNMAI_HOME, so ``GET /api/config?profile=B`` expanded B's
``${VAR}`` refs to the DEFAULT profile's plaintext credentials (its ``os.environ``), and console
``send`` for B (``send_cmd._load_nunmai_env``) copied B's ``.env`` into the shared process env with
...
def test_console_send_for_named_profile_does_not_write_process_env(two_homes, monkeypatch):
    ...
    from nunmai_cli.web_routers.chat_ws import _execute_console_line

    root, b = two_homes
    seen = {}

    def fake_send(args):
        import nunmai_cli.send_cmd as send_cmd
        ...
        seen["home"] = Path(os.environ.get("NUNMAI_HOME", ""))
        return '{"success": true}'
