"""``nunmai chat -Q`` as a dispatcher's re-run of a failed bot delivery (#111721).

A failed delivery turn persists its user row before the provider call; the policy-gated re-run
replays the same session and payload in a fresh process. Told so via NUNMAI_RESUME_UNANSWERED_TURN,
...
def test_turn_report_is_written_before_the_exit_linger_and_the_path_is_not_inherited(monkeypatch, tmp_path):
    ...
    NUNMAI_QUIET_TURN_REPORT_FILE: written the moment the turn ends — before the one-shot exit
