...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nunmai_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("nunmai_cli.main._desktop_build_needed", return_value=True), \
         patch("nunmai_cli.main._write_desktop_build_stamp"), \
         patch("nunmai_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("nunmai_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("nunmai_cli.main._register_linux_desktop_entry"), \
         patch("nunmai_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("nunmai_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("nunmai_cli.main._desktop_build_needed", return_value=True), \
         patch("nunmai_cli.main._write_desktop_build_stamp"), \
         patch("nunmai_cli.main._desktop_macos_relaunchable_fixup"), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nunmai_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("nunmai_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("nunmai_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("nunmai_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("nunmai_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("Nunmai Local Signing") is False
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("Nunmai Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("nunmai_cli.main._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main._desktop_build_needed", return_value=False), \
         patch("nunmai_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("nunmai_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("nunmai_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nunmai_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("nunmai_cli.main._desktop_build_needed", return_value=True), \
         patch("nunmai_cli.main._write_desktop_build_stamp"), \
         patch("nunmai_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("nunmai_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         ...
         patch("nunmai_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nunmai_cli.config.load_config", return_value=cfg), \
         patch("nunmai_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("nunmai_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nunmai_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("nunmai_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("nunmai_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nunmai_cli.config.load_config", return_value={}), \
         patch("nunmai_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("nunmai_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("nunmai_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nunmai_cli.config.load_config", return_value=cfg), \
         patch("nunmai_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("nunmai_cli.main._detect_linux_password_store") as mock_detect, \
         patch("nunmai_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
