...
class TestConfigYamlRouting:
    ...
    def test_terminal_image_goes_to_config(self, _isolated_nunmai_home):
    ...
    def test_cron_script_timeout_is_recognized(self, _isolated_nunmai_home, capsys):
        ...
        assert "script_timeout_seconds: 600" in _read_config(_isolated_nunmai_home)

    def test_memory_nudge_interval_is_recognized(self, _isolated_nunmai_home, capsys):
        """The documented background-memory review interval is runtime config."""
        set_config_value("memory.nudge_interval", "0")

        assert "not a recognized config key" not in capsys.readouterr().out
        assert "nudge_interval: 0" in _read_config(_isolated_nunmai_home)

    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_nunmai_home):
    ...
    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_nunmai_home):
...
class TestCronModelChangeNotice:
    ...
    def test_notice_says_jobs_keep_running_and_names_the_user_owned_pin_path(
        ...
        assert "nunmai cron edit <job_id> --provider <provider> --model <model>" in notice
...
class TestSecretRedactionInDisplay:
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_nunmai_home, capsys):
...
class TestSchemaValidation:
    """#34067: ``nunmai config set`` must not report bare success for
    ...
    a post-write notice warns that Nunmai may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_nunmai_home, capsys):
        """The documented TCC signing identity setting is part of the schema."""
        set_config_value("desktop.macos_signing_identity", "Nunmai Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_nunmai_home))
        assert saved["desktop"]["macos_signing_identity"] == "Nunmai Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out
