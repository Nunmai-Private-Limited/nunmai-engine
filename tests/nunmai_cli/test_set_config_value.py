...
def _isolated_nunmai_home(tmp_path):
    """Point NUNMAI_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"NUNMAI_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_nunmai_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_nunmai_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_nunmai_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_nunmai_home):
...
class TestGatewayPlatformsPrefixRedirect:
    ...
    def test_set_lands_on_top_level_platforms_block_the_loader_reads(self, _isolated_nunmai_home, capsys):
        (_isolated_nunmai_home / "config.yaml").write_text(
            "platforms:\n  telegram:\n    enabled: false\n", encoding="utf-8")
        set_config_value("gateway.platforms.telegram.enabled", "true")
        out = capsys.readouterr().out
        assert "saved as platforms.telegram.enabled" in out
        loaded = yaml.safe_load(_read_config(_isolated_nunmai_home))
    ...
    def test_nested_display_setting_still_reaches_display_platforms(self):
        from nunmai_cli.config import _redirect_platform_display_key
        key, _ = _redirect_platform_display_key("gateway.platforms.telegram.streaming")
        assert key == "display.platforms.telegram.streaming"

    def test_get_and_unset_still_reach_a_legacy_nested_only_value(self, _isolated_nunmai_home, capsys):
        ...
        from nunmai_cli.config import get_config_value, unset_config_value

        legacy = "gateway:\n  platforms:\n    telegram:\n      enabled: true\n"
        (_isolated_nunmai_home / "config.yaml").write_text(legacy, encoding="utf-8")
        ...
        assert "gateway" not in (yaml.safe_load(_read_config(_isolated_nunmai_home)) or {})
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_nunmai_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_nunmai_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_nunmai_home)





    def test_tool_search_defer_is_recognized(self, _isolated_nunmai_home, capsys):
...
class TestFalsyValues:
    ...
    def test_config_command_accepts_empty_string(self, _isolated_nunmai_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_nunmai_home, capsys):
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_nunmai_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_nunmai_home)
        capsys.readouterr()
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_nunmai_home)) or {}
        assert reloaded == {}
        assert "TERMINAL_ENV=" not in _read_env(_isolated_nunmai_home)
        assert "Unset terminal.backend" in capsys.readouterr().out


    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_nunmai_home, capsys):
        (_isolated_nunmai_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_nunmai_home))
...
class TestConfigGetPhantomKeyNotice:
    ...
    def test_unseeded_live_key_notice_hedges_instead_of_asserting_unread(
        ...
        (_isolated_nunmai_home / "config.yaml").write_text("browser:\n  cloud_provider: local\n")
    ...
    def test_recognized_and_custom_keys_are_not_flagged(
        self, _isolated_nunmai_home, capsys, key, body
    ):
        (_isolated_nunmai_home / "config.yaml").write_text(body)
...
class TestListNavigation:
    """nunmai config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_nunmai_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_nunmai_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_nunmai_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_nunmai_home):
...
class TestStringTypedConfigValues:
    ...
    def test_string_typed_values_are_not_coerced(self, _isolated_nunmai_home, value):
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_nunmai_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_nunmai_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from nunmai_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_nunmai_home, capsys):
...
class TestSchemaValidation:
    ...
    def test_unknown_subkey_under_known_section_refused_before_write(
        self, key, suggestion, _isolated_nunmai_home, capsys
    ):
        config_path = _isolated_nunmai_home / "config.yaml"
    ...
    def test_unknown_leaf_under_known_section_is_written_with_notice(
        self, key, value, expected, suggestion, _isolated_nunmai_home, capsys
    ):
    ...
    def test_unknown_top_level_key_still_written_with_notice(self, _isolated_nunmai_home, capsys):
        set_config_value("brand_new_future_key", "value")
        assert "brand_new_future_key" in _read_config(_isolated_nunmai_home)
    ...
    def test_force_suppresses_notice(self, _isolated_nunmai_home, capsys):
        ...
        content = _read_config(_isolated_nunmai_home)
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from nunmai_cli.config import _validate_config_key
...
class TestDisplaySkinTouch:
    ...
    (`nunmai config set display.skin X` while it is already X — the recovery
    ...
    def test_reaffirming_same_skin_moves_the_watcher_signature(self, _isolated_nunmai_home):
        import os as _os
        skins = _isolated_nunmai_home / "skins"
        skins.mkdir()
    ...
    def test_builtin_or_missing_skin_file_is_fine(self, _isolated_nunmai_home):
        """Built-ins have no user file — the set must still succeed cleanly."""
        set_config_value("display.skin", "mono")
        assert "skin: mono" in _read_config(_isolated_nunmai_home)

    def test_touch_preserves_skin_file_contents(self, _isolated_nunmai_home):
        skins = _isolated_nunmai_home / "skins"
        skins.mkdir()
        body = "name: neon\ncolors:\n  ui_accent: '#ff33aa'\n"
...
class TestMappingGuard:
    """``nunmai config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_nunmai_home):
        """nunmai config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_nunmai_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
        ...
        config_text = _read_config(_isolated_nunmai_home)
        import yaml as _yaml
    ...
    def test_bare_model_shorthand_creates_default_when_none(self, _isolated_nunmai_home):
        """Bare model shorthand still works when config is empty (legacy behaviour)."""
        set_config_value("model", "gpt-5.6-sol")
        assert "gpt-5.6-sol" in _read_config(_isolated_nunmai_home)

    def test_non_model_mapping_is_refused(self, _isolated_nunmai_home):
        """nunmai config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_nunmai_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_nunmai_home):
        """nunmai config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_nunmai_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
        ...
        import yaml as _yaml
        parsed = _yaml.safe_load(_read_config(_isolated_nunmai_home))
        assert parsed["terminal"] == "zsh"

    def test_model_default_dotted_path_is_not_guarded(self, _isolated_nunmai_home):
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_nunmai_home):
        """nunmai config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_nunmai_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestScalarModelSubKeyPreservation:
    ...
    def test_scalar_model_id_preserved_after_provider_write(self, _isolated_nunmai_home):
        ...
        raw = _read_config(_isolated_nunmai_home)
        parsed = yaml.safe_load(raw)
        model = parsed["model"]
        assert model["default"] == "gpt-4o", f"model.default lost: {model}"
        assert model["provider"] == "openai"

    def test_scalar_model_id_preserved_after_api_key_write(self, _isolated_nunmai_home):
...
class TestMalformedYAMLConfigPreservation:
    ...
    def test_set_config_value_refuses_broken_yaml(self, _isolated_nunmai_home, capsys):
        """set_config_value must raise, not overwrite the broken config."""
        self._write_broken_config(_isolated_nunmai_home)
        ...
        assert "formatting error" in combined and "`nunmai config edit`" in combined
        # Original config must remain intact
        raw = _read_config(_isolated_nunmai_home)
        assert raw == self.BROKEN_CONFIG, f"Config was overwritten:\n{raw}"

    def test_unset_config_value_refuses_broken_yaml(self, _isolated_nunmai_home, capsys):
        """unset_config_value must raise, not overwrite the broken config."""
        from nunmai_cli.config import unset_config_value
...
class TestLiteralDotKeyEscaping:
    """``nunmai config set/unset/get`` must not split a key segment on a
    ...
    def test_split_key_path_escaped_dot(self):
        from nunmai_cli.config import _split_key_path
    ...
    def test_set_preserves_literal_dot_in_provider_key(self, _isolated_nunmai_home, capsys):
        self._write_config(_isolated_nunmai_home, {
            "providers": {
    ...
    def test_unset_removes_literal_dot_provider_key(self, _isolated_nunmai_home, capsys):
        self._write_config(_isolated_nunmai_home, {
            "providers": {
                "qwen3.5-397b-wafer-non-zdr": {"api": "https://pass.wafer.ai/v1"},
                "openrouter": {"api_key": "or-keep"},
    ...
    def test_unset_nested_field_under_literal_dot_key(self, _isolated_nunmai_home, capsys):
        self._write_config(_isolated_nunmai_home, {
            "providers": {
    ...
    def test_get_reads_literal_dot_provider_key(self, _isolated_nunmai_home, capsys):
        self._write_config(_isolated_nunmai_home, {
    ...
    def test_unescaped_dotted_path_unchanged(self, _isolated_nunmai_home):
...
class TestConfigGetRedaction:
    ...
    def test_config_get_masks_every_credential_path(self, _isolated_nunmai_home, capsys, monkeypatch, key):
        self._seed(_isolated_nunmai_home, monkeypatch)
        from nunmai_cli.config import get_config_value
    ...
    def test_config_get_raw_prints_the_real_value(self, _isolated_nunmai_home, capsys, monkeypatch):
        self._seed(_isolated_nunmai_home, monkeypatch)
        from nunmai_cli.config import get_config_value
    ...
    def test_config_get_classifies_env_header_and_enum_keys(
            self, _isolated_nunmai_home, capsys, monkeypatch, key, env_line, yaml_line, masked):
        monkeypatch.delenv("UNSET_THING_API_KEY", raising=False)
        (_isolated_nunmai_home / "config.yaml").write_text(
            "mcp_servers:\n  s:\n    url: https://x.example\n" + yaml_line, encoding="utf-8")
        (_isolated_nunmai_home / ".env").write_text(env_line + "\n", encoding="utf-8")
        from nunmai_cli.config import get_config_value
...
class TestContainerTypeRefusal:
    ...
    def test_string_where_schema_wants_list_is_refused(self, _isolated_nunmai_home, capsys):
        self._write_config(_isolated_nunmai_home, {"model": {"default": "m"}})
        ...
        assert "custom_providers" not in _read_config(_isolated_nunmai_home)

    def test_valid_literal_and_scalar_keys_still_write(self, _isolated_nunmai_home):
        self._write_config(_isolated_nunmai_home, {"model": {"default": "m", "aliases": {"a": "p/m"}}})
        ...
        import yaml as _yaml
        saved = _yaml.safe_load(_read_config(_isolated_nunmai_home))
    ...
    def test_unseeded_or_top_level_container_key_is_refused_without_on_disk_value(
            self, _isolated_nunmai_home, key):
    ...
    def test_bare_name_for_string_list_slot_is_stored_as_one_item_list(self, _isolated_nunmai_home):
...
class TestProviderSwitchClearsBaseUrl:
    ...
    def test_switching_provider_clears_foreign_route(self, _isolated_nunmai_home, capsys, target, seed_route):
        self._seed(_isolated_nunmai_home, {"provider": "opencode-go", "default": "gpt-5.3-codex", **seed_route})
        set_config_value("model.provider", target)
        model = yaml.safe_load(_read_config(_isolated_nunmai_home))["model"]
    ...
    def test_route_that_belongs_to_target_is_kept(self, _isolated_nunmai_home, capsys, key, target, seed):
        self._seed(_isolated_nunmai_home, {**seed, "default": "m"})
