...
def worker_setup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, kb.Task]:
    ...
    monkeypatch.setattr(kbd, "_resolve_nunmai_argv", lambda: ["nunmai"])
...
def test_managed_gateway_worker_is_spawned_in_restart_safe_scope(
    ...
    assert captured_cmd[unit_index + 1] == "nunmai-worker-kanban-t_candidate_restart-run-23"
    assert "MemoryMax=536870912" in captured_cmd
    separator = captured_cmd.index("--")
    assert captured_cmd[separator + 1 : separator + 4] == ["nunmai", "-p", "coder"]
    assert captured_cwd == str(workspace)
    assert captured_env["NUNMAI_KANBAN_TASK"] == task.id
    assert captured_env["NUNMAI_KANBAN_RUN_ID"] == "23"
...
def test_standalone_dispatcher_keeps_direct_worker_spawn(
    ...
    assert captured_cmd[:3] == ["nunmai", "-p", "coder"]
...
def test_oneshot_unit_dispatcher_scope_wraps_or_warns_never_dooms_silently(
    ...
    user bus the worker gets its own ``nunmai-worker-*`` scope; without one the
    ...
    monkeypatch.setattr(
        ...
        lambda cmd, unit_suffix: ["systemd-run", "--user", "--scope", "--unit", f"nunmai-worker-{unit_suffix}", *cmd],
    )

    kbd._default_spawn(task, str(workspace))
    assert spawned[-1][:3] == ["systemd-run", "--user", "--scope"]
    assert f"nunmai-worker-kanban-{task.id}-run-{task.current_run_id}" in spawned[-1]
    ...
    with caplog.at_level("WARNING", logger=process_registry.logger.name):
        kbd._default_spawn(task, str(workspace))
    assert spawned[-1][:3] == ["nunmai", "-p", "coder"]
    ...
    cron = process_registry.restart_safe_gateway_child_argv(
        ["nunmai", "cron"], unit_suffix="cron-job-1", require_restart_safe_scope=True,
    )
...
def test_real_user_systemd_scope_preserves_worker_context(
    ...
    script = (
        ...
        "'task': os.environ.get('NUNMAI_KANBAN_TASK'), "
        "'run': os.environ.get('NUNMAI_KANBAN_RUN_ID'), "
        "'cgroup': pathlib.Path('/proc/self/cgroup').read_text()})); os.replace(t, p); time.sleep(0.5)"
    )
    monkeypatch.setattr(kbd, "_resolve_nunmai_argv", lambda: [sys.executable, "-c", script, str(receipt)])
    monkeypatch.setenv("INVOCATION_ID", "managed-gateway-test")
    ...
    assert "nunmai-gateway.service" not in payload["cgroup"]
