...
class TestNoSkillsOptOut:
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["nunmai-engine"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["nunmai-engine"]
        assert len(called) == 1
...
class TestWrapperScript:
    ...
    def test_remove_finds_bat_on_windows(self, profile_env):
        from nunmai_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestInternalHelpers:
    ...
    def test_default_nunmai_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_nunmai_home() returns NUNMAI_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NUNMAI_HOME", str(docker_home))
        home = _get_default_nunmai_home()
