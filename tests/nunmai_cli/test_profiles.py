"""Comprehensive tests for nunmai_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NUNMAI_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.nunmai/profiles)
    * NUNMAI_HOME  -> tmp_path/.nunmai  (so get_nunmai_home() agrees)
    * Creates the bare-minimum ~/.nunmai directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".nunmai"
...
class TestGetProfileDir:
    ...
    def test_default_returns_nunmai_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_fresh_profile_inherits_a_usable_model(self, profile_env):
        ...
        """
        default_home = profile_env / ".nunmai"
    ...
    def test_clone_config_copies_only_the_active_memory_providers_config(self, profile_env):
        ...
        payload = '{"mode": "local_embedded", "bank_id": "nunmai", "apiKey": "hs-secret"}'
        (default_home / "hindsight" / "config.json").write_text(payload)
        (default_home / "mem0.json").write_text('{"agent_id": "nunmai"}')
    ...
    def test_clone_sync_imports_carries_manifest_but_never_links_profiles(self, profile_env):
        ...
        from nunmai_cli.agent_import_sync import SYNC_MANIFEST_NAME, load_sync_manifest
...
class TestNoSkillsOptOut:
    """Tests for `nunmai profile create --no-skills` and the opt-out marker."""
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `nunmai update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".nunmai" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("nunmai_cli.profiles._cleanup_gateway_service"), \
             patch("nunmai_cli.profiles.time.sleep"), \
             patch("nunmai_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_delete_purges_profile_keyed_identity(self, profile_env):
        ...
        scope = str(tmp_path / ".nunmai" / "sessions")
        db = SessionDB(tmp_path / ".nunmai" / "state.db")
        ...
        with patch("nunmai_cli.profiles._cleanup_gateway_service"), \
             patch("nunmai_cli.profiles._live_default_multiplexer", return_value=False):
            delete_profile("gone", yes=True)

        check = SessionDB(tmp_path / ".nunmai" / "state.db")
        try:
    ...
    def test_delete_reports_pending_settlement_for_a_live_multiplexer(self, profile_env, capsys):
        ...
        """
        from nunmai_state import SessionDB
        from nunmai_cli.profiles import ProfileIdentitySettlementPending
        ...
        with patch("nunmai_cli.profiles._cleanup_gateway_service"), \
             patch("nunmai_cli.profiles._live_default_multiplexer", return_value=True):
        ...
        assert ei.value.retry_command == "nunmai profile purge-identity gone"
        assert not ei.value.path.exists()
        assert isinstance(ei.value, RuntimeError)  # the CLI handler catches RuntimeError

        assert "nunmai profile purge-identity gone" in capsys.readouterr().err
        check = SessionDB(tmp_path / ".nunmai" / "state.db")
        try:
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "nunmai_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "nunmai_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "nunmai_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "nunmai_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_nunmai_shim(self, profile_env, monkeypatch):
        """A `nunmai` console-script shim spawned directly (e.g. Electron's
        findOnPath('nunmai') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "nunmai" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "nunmai".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/nunmai", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/nunmai", "--profile", "other", "serve"]),
            # Non-nunmai script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_nunmai_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "nunmai" (e.g.
        nunmai-notes.py, nunmai-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "nunmai" -- only the actual known
        console-script entry points (nunmai, nunmai-engine, nunmai-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/nunmai-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/nunmai-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (nunmai-engine,
        nunmai-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "nunmai"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/nunmai-engine",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/nunmai-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestListProfiles:
    ...
    def test_lazy_skill_count_never_walks_in_the_polled_request(self, profile_env, monkeypatch):
        ...
        skills = profile_env / ".nunmai" / "skills" / "cat"
        for i in range(3):
        ...
        first = _rpc()
        assert walks == [] or set(walks) == {"nunmai-skill-count"}
        assert first[0]["skill_count"] in (0, 3)  # 0 until the refresh lands, never a stall
        for t in threading.enumerate():
            if t.name == "nunmai-skill-count":
                t.join(timeout=10)
        assert walks == ["nunmai-skill-count"]
        assert _rpc()[0]["skill_count"] == 3
        assert list_profiles(lazy_skill_count=True)[0].skill_count == 3
        assert walks == ["nunmai-skill-count"]  # a second poll inside the window schedules nothing
        ...
        from nunmai_cli.web_routers import profiles as profiles_router
    ...
    def test_skill_count_survives_subtree_vanishing_mid_walk(self, profile_env, monkeypatch):
        ...
        assert profiles._count_skills(profile_env / ".nunmai") == 3
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".nunmai" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom NUNMAI_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("NUNMAI_HOME", str(custom))
        # With Docker-aware roots, a custom NUNMAI_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("nunmai_cli.profiles.shutil.which", lambda name: "/opt/nunmai/bin/nunmai")
        from nunmai_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/nunmai/bin/nunmai -p mybot" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from nunmai_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from nunmai_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "nunmai.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nunmai_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nunmai_heimdall"]["peerName"] == "user-peer"
    ...
    def test_multiplexed_rename_unroutes_old_then_hot_serves_new(self, profile_env):
        ...
        create_profile("oldname", no_alias=True)
        old_dir = tmp_path / ".nunmai" / "profiles" / "oldname"
        new_dir = tmp_path / ".nunmai" / "profiles" / "newname"
        ...
        def _record_notify(name):
            ...
            if name == "oldname" and old_dir.exists():
                ...
                with pytest.raises(FileNotFoundError):
                    mkdir_under_nunmai_home(old_dir / "logs")

        with patch("nunmai_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("nunmai_cli.profiles._live_default_multiplexer", return_value=True), \
             patch("nunmai_cli.profiles._notify_multiplexer", side_effect=_record_notify):
    ...
    def test_unmultiplexed_rename_does_not_signal_multiplexer(self, profile_env):
        ...
        with patch("nunmai_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("nunmai_cli.profiles._live_default_multiplexer", return_value=False), \
             patch("nunmai_cli.profiles._notify_multiplexer") as notify:
            new_dir = rename_profile("oldname", "newname")

        notify.assert_not_called()
        assert not (tmp_path / ".nunmai" / "profiles" / ".deleted").exists()
        assert not old_dir.exists() and new_dir.is_dir()
    ...
    def test_rename_migrates_session_identity_without_live_gateway(self, profile_env):
        ...
        pdb.close()
        root_db = SessionDB(tmp_path / ".nunmai" / "state.db")
        ...
        moved_db.close()
        root_db2 = SessionDB(tmp_path / ".nunmai" / "state.db")
    ...
    def test_migrate_identity_command_repairs_a_failed_live_migration(self, profile_env, capsys):
        """The failed-live-migration end state must be recoverable: `nunmai profile
        ...
        with patch("nunmai_cli.profiles.check_alias_collision", return_value="skip"), \
            ...
            rename_profile("oldname", "newname")
        assert "nunmai profile migrate-identity oldname newname" in capsys.readouterr().err

        # Gateway restarted/stopped → the retry command repairs both stores.
        with patch("nunmai_cli.profiles._live_default_multiplexer", return_value=False):
        ...
        moved_db = SessionDB(tmp_path / ".nunmai" / "profiles" / "newname" / "state.db")
        row = moved_db._read_one(
            "SELECT session_key, profile_name FROM sessions WHERE id = ?", ("sess1",))
    ...
    def test_rename_succeeds_when_previous_name_write_fails(self, profile_env):
        ...
        with patch("nunmai_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("nunmai_cli.profiles.write_profile_meta", side_effect=OSError("disk full")):
            new_dir = rename_profile("oldname", "newname")
...
class TestExportImport:
    ...
    def test_export_default_includes_profile_data(self, profile_env, tmp_path):
        ...
        # Write through NUNMAI_HOME, not get_profile_dir("default"): the latter resolves to the
        ...
        default_dir = profile_env / ".nunmai"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultNunmaiHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_nunmai_home()."""
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under NUNMAI_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NUNMAI_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="nunmai gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with NUNMAI_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("NUNMAI_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_nunmai_home()
    ...
    def test_default_profile_with_key_still_served_with_one_warning(self, profile_env, caplog):
        ...
        caplog.clear()
        with caplog.at_level("WARNING", logger="nunmai_cli.profiles"):
            serve = dict(profiles_to_serve(multiplex=True))
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured NUNMAI_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("NUNMAI_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No NUNMAI_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("NUNMAI_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_nunmai_home()
...
def test_profile_delete_and_rename_stop_the_profiles_bot_desktop(profile_env, op):
    ...
    try:
        with patch("nunmai_cli.profiles._cleanup_gateway_service"), \
             patch("nunmai_cli.profiles.check_alias_collision", return_value="skip"):
            if op == "delete":
                delete_profile("coder", yes=True)
            else:
...
class TestCloneAllExcludesRuntimeTrees:
    ...
    runtime trees the local-models flow puts under ``~/.nunmai``: ``models/``
...
def test_count_skills_publishes_timestamp_after_the_walk(tmp_path, monkeypatch):
    ...
    from nunmai_cli import profiles as mod
