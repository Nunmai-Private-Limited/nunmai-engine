...
def test_unit_transaction_budget_preserves_scope_and_health(monkeypatch, graceful, retry):
    ...
    def systemctl(cmd, *, timeout):
        ...
        if "list-units" in cmd:
            return subprocess.CompletedProcess(cmd, 0, "nunmai-serve-test.service loaded active running", "")
        if "show" in cmd:
    ...
    name = "nunmai-gateway-test" if graceful else "nunmai-serve-test"
    restarted, failed = [], []
...
def test_budget_fallback_keeps_real_errors(monkeypatch, limits, outcome):
    ...
    if outcome == "timeout":
        with pytest.raises(subprocess.TimeoutExpired):
            fleet._systemctl_reset_and_restart(["systemctl"], "nunmai-serve-test")
    else:
        result = fleet._systemctl_reset_and_restart(["systemctl"], "nunmai-serve-test")
...
def test_fleet_restart_repairs_a_system_unit_that_cannot_park_on_exit_78(monkeypatch, tmp_path, capsys, root):
    ...
    from nunmai_cli import gateway as gateway_cli

    unit_dir = tmp_path / "system"
    unit_dir.mkdir()
    stale = unit_dir / "nunmai-gateway.service"
    stale.write_text("[Service]\nRestart=on-failure\nRestartSec=10\n", encoding="utf-8")
    current = unit_dir / "nunmai-gateway-ops.service"
    ...
    for name in ("nunmai-gateway", "nunmai-gateway-ops"):
        fleet._restart_one_systemd_gateway_unit(
    ...
    if root:
        ...
        assert refreshed == []
        assert out.count("RestartPreventExitStatus=78") == 1 and "nunmai-gateway lacks" in out
        assert "sudo nunmai gateway install --system" in out
