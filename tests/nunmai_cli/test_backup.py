...
class TestShouldExclude:
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from nunmai_cli.backup import _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestIterBackupFiles:
    ...
    def test_prunes_browser_use_cli_profiles_at_home_roots_only(self, tmp_path):
        """The Browser Use CLI backend writes ``NUNMAI_HOME/browser_profiles/`` (underscore) — a
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert (nunmai_home / "config.yaml").read_text() == "model: test\n"
...
class TestBackupEdgeCases:
    ...
    def test_incomplete_archive_is_kept_but_reported_as_failure(self, tmp_path, monkeypatch, capsys):
        ...
        nunmai_home.mkdir()
        _make_nunmai_tree(nunmai_home)
        unreadable = nunmai_home / "skills" / "locked.md"
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        from nunmai_cli.backup import _RUN_BACKUP_PREFIX, run_backup
        from nunmai_cli.main import cmd_backup
