"""#103339 item 2: ``nunmai doctor --fix`` never checkpoints state.db through a bare writable ``sqlite3.connect``
...
from nunmai_cli.doctor_report import Finding
from nunmai_cli.doctor_state import _state_db_wal
...
def test_doctor_checkpoint_runs_only_on_the_exclusive_repair_guard(tmp_path, monkeypatch):
    ...
    real_durable = nunmai_state_repair._connect_repair_durable

    def _durable(path, **kwargs):
        guard_connects.append(Path(path))
        return real_durable(path, **kwargs)

    monkeypatch.setattr(nunmai_state_repair, "_connect_repair_durable", _durable)
...
def test_session_count_reads_a_home_with_uri_reserved_characters(tmp_path):
    ...
    from nunmai_cli.doctor_state import _session_count
...
def test_large_wal_warning_under_a_live_writer_never_suggests_a_bare_fix(tmp_path, monkeypatch, capsys):
    """`nunmai doctor` (no --fix) on a large WAL while Desktop/gateway hold the DB must say it is normal and
    order "stop" before any `--fix` — the bare "run 'nunmai doctor --fix'" nudge is how users became the
    second writer (#110054)."""
    import nunmai_state_holders

    monkeypatch.setattr(nunmai_state_holders, "live_writer_holds_db", lambda *a, **k: True)
    finding = Finding()
    _state_db_wal(finding, False, _large_wal_db(tmp_path))
    assert len(finding.issues) == 1 and not finding.fixed
    assert finding.issues[0].index("stop the profile's gateway") < finding.issues[0].index("nunmai doctor --fix")


def test_large_wal_warning_without_a_holder_still_orders_stop_before_fix(tmp_path, monkeypatch):
    import nunmai_state_holders

    monkeypatch.setattr(nunmai_state_holders, "live_writer_holds_db", lambda *a, **k: False)
    finding = Finding()
    _state_db_wal(finding, False, _large_wal_db(tmp_path))
    assert len(finding.issues) == 1 and not finding.fixed
    assert finding.issues[0].index("stop the profile's gateway") < finding.issues[0].index("nunmai doctor --fix")
