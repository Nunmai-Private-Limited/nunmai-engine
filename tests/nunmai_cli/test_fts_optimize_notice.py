...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            nunmai_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(nunmai_constants, "get_nunmai_home", lambda: tmp_path)
    monkeypatch.setattr(nunmai_state, "SessionDB", FakeSessionDB)
    ...
    assert "nunmai sessions optimize-storage" in capsys.readouterr().out
    conn.close()
