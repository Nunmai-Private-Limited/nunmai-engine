...
no longer pick the surface: ``nunmai doctor`` and the dashboard key test probe the default Studio
...
def test_doctor_gemini_probe_keeps_aq_keys_on_the_studio_host(monkeypatch):
    from nunmai_cli.doctor_connectivity import _apikey_request
...
def test_dashboard_gemini_key_probe_keeps_aq_keys_on_the_studio_host(monkeypatch):
    import nunmai_cli.web_routers.config_env as mod
    from nunmai_cli.web_models import EnvVarUpdate
