...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from nunmai_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # nunmai model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in NUNMAI_OVERLAYS
        overlay = NUNMAI_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a NunmaiAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("NunmaiAgent/")

    def test_gmi_profile_declares_nunmai_user_agent(self):
        """The GMI plugin sets a NunmaiAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("NunmaiAgent/"), (
            f"expected GMI profile User-Agent to start with 'NunmaiAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "nunmai_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["nunmai", "chat", "--provider", "gmi"])

        from nunmai_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("nunmai_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("nunmai_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from nunmai_cli.model_setup_flows import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_nunmai_home() / "config.yaml").read_text()) or {}
