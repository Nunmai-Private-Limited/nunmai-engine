"""Dashboard Nunmai Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_nunmai_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from nunmai_cli.console_engine import ConsoleResult, NunmaiConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(NunmaiConsoleEngine, "execute", slow_execute)
...
def test_console_cancel_stops_forked_agent_request_before_reporting(console_client, monkeypatch, blocking_provider):
    ...
    from nunmai_cli.web_routers import chat_ws
    ...
    skill_dir = get_nunmai_home() / "skills" / "console-cancel-probe"
