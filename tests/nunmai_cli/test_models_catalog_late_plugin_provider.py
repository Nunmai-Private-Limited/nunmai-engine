"""A provider registered after ``nunmai_cli.models`` was imported still reaches the picker catalog.

``CANONICAL_PROVIDERS`` admitted plugin providers once, at import. A plugin whose imports pull
``nunmai_cli.models`` in mid-discovery, or a profile registered at runtime, never reached
...
def test_late_registered_provider_reaches_picker_catalog(monkeypatch):
    import nunmai_cli.models_catalog_static as catalog
    from nunmai_cli.models import list_available_providers
    ...
    monkeypatch.setattr("nunmai_cli.models.CANONICAL_PROVIDERS", catalog.CANONICAL_PROVIDERS)
    monkeypatch.setattr("nunmai_cli.models._PROVIDER_LABELS", catalog._PROVIDER_LABELS)
