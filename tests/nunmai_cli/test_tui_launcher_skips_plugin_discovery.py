...
`nunmai --tui` just spawns a Node process; the spawned tui_gateway backend
performs its own plugin discovery. Running discover_plugins() in the
launcher added ~0.5s to every `nunmai --tui` startup for work the backend
...
def _install_discover_spy(monkeypatch):
    ...
    # ``nunmai_cli.plugins`` (replaced by the stub above), fails, and falls
