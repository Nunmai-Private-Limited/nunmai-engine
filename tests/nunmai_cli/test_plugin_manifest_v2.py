...
def nunmai_home(tmp_path, monkeypatch):
    home = tmp_path / "nunmai_home"
    (home / "plugins").mkdir(parents=True)
    monkeypatch.setenv("NUNMAI_HOME", str(home))
    monkeypatch.setenv("NUNMAI_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "NUNMAI_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "oldie",
            manifest_extra={"mystery_field": True},
        )
        _enable(nunmai_home, ["oldie"])
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.plugins"):
            mgr = PluginManager()
...
class TestV2Parsing:
    def test_v2_fields_parse(self, nunmai_home):
        _write_plugin(
            nunmai_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(nunmai_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_malformed_v2_fields_warn_and_degrade(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, nunmai_home):
        ...
        _write_plugin(
            nunmai_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            nunmai_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(nunmai_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(nunmai_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(nunmai_home, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, nunmai_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
    ...
    def test_satisfied_pip_dep_is_quiet(self, nunmai_home, caplog):
        _write_plugin(
            nunmai_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(nunmai_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, nunmai_home):
        _write_plugin(nunmai_home / "plugins", "probe-target")
        _write_plugin(
            nunmai_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(nunmai_home, ["probe-target", "prober"])
        import sys
...
class TestRequiresNunmai:
    def test_gate_reads_the_running_code_version_not_dist_metadata(self, monkeypatch):
        ...
        from nunmai_cli import plugins_manifest
        monkeypatch.setattr(importlib.metadata, "version", lambda name: "0.21.0")
        monkeypatch.setattr("nunmai_cli.__version__", "0.21.4")
        assert plugins_manifest.running_nunmai_version() == "0.21.4"
        assert plugins_manifest.version_satisfies(">=0.21.4", plugins_manifest.running_nunmai_version())
    ...
    def test_prerelease_spellings_gate(self, spec, current, expected):
        from nunmai_cli.plugins_manifest import version_satisfies
        assert version_satisfies(spec, current) is expected

    def test_unsatisfied_requires_nunmai_skips_without_importing(self, nunmai_home, monkeypatch):
        """A too-new ``requires_nunmai`` records an error and never runs register(); a satisfied one loads."""
        import sys
        from nunmai_cli import plugins_manifest
        monkeypatch.setattr(plugins_manifest, "running_nunmai_version", lambda: "1.2.3")
        _write_plugin(nunmai_home / "plugins", "future", manifest_extra={"requires_nunmai": ">=99.0"},
                      register_body="import sys; sys._rh_future = True")
        _write_plugin(nunmai_home / "plugins", "current", manifest_extra={"requires_nunmai": ">=1.2,<2"},
                      register_body="import sys; sys._rh_current = True")
        _enable(nunmai_home, ["future", "current"])
        try:
            mgr = PluginManager()
            mgr.discover_and_load()
            assert not hasattr(sys, "_rh_future")
            assert "requires nunmai >=99.0" in (mgr._plugins["future"].error or "")
...
class TestLoadIsolation:
    def test_sys_exit_in_plugin_is_isolated_and_named(self, nunmai_home, caplog):
        """A plugin calling ``sys.exit()`` at import used to propagate SystemExit out of discovery: the whole
        registry emptied, ``_discovered`` reset and ``nunmai chat`` exited 3 with no output. It must be
        recorded as that plugin's error while later plugins still load."""
        _write_plugin(nunmai_home / "plugins", "b_exit")
        (nunmai_home / "plugins" / "b_exit" / "__init__.py").write_text("import sys\nsys.exit(0)\n")
        _write_plugin(nunmai_home / "plugins", "c_after")
        _enable(nunmai_home, ["b_exit", "c_after"])
        mgr = PluginManager()
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.plugins"):
    ...
    def test_keyboard_interrupt_still_propagates(self, nunmai_home):
        _write_plugin(nunmai_home / "plugins", "ctrlc")
        (nunmai_home / "plugins" / "ctrlc" / "__init__.py").write_text("raise KeyboardInterrupt\n")
        _enable(nunmai_home, ["ctrlc"])
        with pytest.raises(KeyboardInterrupt):
            PluginManager().discover_and_load()

    def test_register_overrunning_load_timeout_skips_only_that_plugin(self, nunmai_home, caplog):
        ...
        _write_plugin(nunmai_home / "plugins", "b_slow", register_body=(
        ...
        _write_plugin(nunmai_home / "plugins", "c_after")
        _enable(nunmai_home, ["b_slow", "c_after"])
    ...
    def test_load_timeout_zero_runs_register_inline(self, nunmai_home):
        """``plugins.load_timeout_seconds: 0`` disables the deadline: register() runs on the calling thread."""
        import sys
        import threading
        _write_plugin(nunmai_home / "plugins", "inline",
...
class TestBundledKeyShadowing:
    def test_impostor_dir_cannot_claim_a_bundled_key(self, tmp_path, monkeypatch, caplog):
        """``~/.nunmai/plugins/impostor_dir/plugin.yaml`` with ``name: <bundled key>`` used to displace the
        bundled plugin silently, so ``nunmai plugins enable <key>`` enabled unrelated code. The bundled
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(home))
        monkeypatch.setenv("NUNMAI_ENABLE_PROJECT_PLUGINS", "0")
        monkeypatch.setenv("NUNMAI_BUNDLED_PLUGINS", str(bundled))
        ...
        import sys
        try:
            with caplog.at_level(logging.INFO, logger="nunmai_cli.plugins"):
                mgr = PluginManager()
                mgr.discover_and_load()
...
class TestManifestParsingRobustness:
    def test_list_manifest_is_rejected_with_a_clear_reason_and_hooks_alias(self, nunmai_home, caplog):
        ...
        from nunmai_cli.plugins_discovery import scan_directory
        bad = nunmai_home / "plugins" / "listy"
        bad.mkdir()
        (bad / "plugin.yaml").write_text("- name: listy\n")
        good = _write_plugin(nunmai_home / "plugins", "hooky", manifest_extra={"hooks": ["pre_tool_call"]})
        with caplog.at_level(logging.WARNING, logger="nunmai_cli.plugins"):
            manifests = {m.name: m for m in scan_directory(nunmai_home / "plugins", "user")}
...
class TestDirectoryPluginKeepsIdentityOverEntryPoint:
    """A pyproject-wrapper plugin depends on a pip package that ships a ``nunmai_agent.plugins``
    ...
    def test_loader_and_listing_prefer_the_installed_directory(self, nunmai_home, monkeypatch):
        from nunmai_cli.plugins_manifest import PluginManifest
        _write_plugin(nunmai_home / "plugins", "twin")
        _enable(nunmai_home, ["twin"])
        ...
        monkeypatch.setattr("nunmai_cli.plugins_cmd.discover_entrypoint_manifests", lambda: [twin_ep], raising=False)
        monkeypatch.setattr("nunmai_cli.plugins.discover_entrypoint_manifests", lambda: [twin_ep])

        mgr = PluginManager()
        mgr.discover_and_load()
        assert mgr._plugins["twin"].manifest.source == "user"

        from nunmai_cli.plugins_cmd import _discover_all_plugins
