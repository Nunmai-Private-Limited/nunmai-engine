"""A Desktop-pooled ``nunmai serve`` child must PROVE it is idle before the Desktop may retire it
...
from nunmai_cli.web_server_idle_proof import idle_proof, pending_human_input
...
def _spawn_desktop_child(tmp_path: Path, name: str, *, busy: bool) -> subprocess.Popen:
    """Mirror the Desktop pool spawn: ``NUNMAI_DESKTOP=1`` (in-process cron ticker), a per-child
    NUNMAI_HOME, an ephemeral port, the session token the Desktop probes with. The busy child
    ...
    for k in ("NUNMAI_PARENT_PID", "NUNMAI_PARENT_START_MARKER", "NUNMAI_PARENT_NONCE"):
        env.pop(k, None)
    env.update(
...
def _probe(port: int, token: str | None = TOKEN) -> tuple[int, dict]:
    req = urllib.request.Request(f"http://127.0.0.1:{port}/api/health/idle",
                                 headers={"X-Nunmai-Session-Token": token} if token else {})
    try:
