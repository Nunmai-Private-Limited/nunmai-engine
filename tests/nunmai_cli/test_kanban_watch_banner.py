...
from nunmai_cli import kanban_ops
from nunmai_cli.kanban import kanban_command
from nunmai_cli.kanban_parser import build_parser
...
def test_watch_names_resolved_board(tmp_path, monkeypatch, capsys, environment, explicit, expected):
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_HOME", str(tmp_path))
    ...
    if environment:
        monkeypatch.setenv("NUNMAI_KANBAN_BOARD", environment)
