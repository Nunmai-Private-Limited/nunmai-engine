"""`nunmai sessions export` single-file formats accept a directory as OUTPUT."""
...
def _export(monkeypatch, *argv):
    monkeypatch.setattr(nunmai_state, "SessionDB", lambda *args, **kwargs: _FakeDB())
    monkeypatch.setattr(sys, "argv", ["nunmai", "sessions", "export", "--session-id", "sess", *argv])
    main_mod.main()
...
def test_jsonl_export_into_directory_writes_default_named_file(monkeypatch, tmp_path):
    ...
    written = out_dir / "nunmai_session_sess-123.jsonl"
...
def test_jsonl_export_to_file_path_is_unchanged(monkeypatch, tmp_path):
    ...
    assert target.is_file() and not (tmp_path / "nunmai_session_sess-123.jsonl").exists()
