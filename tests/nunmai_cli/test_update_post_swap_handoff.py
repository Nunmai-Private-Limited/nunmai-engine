"""``nunmai update`` finishes in an interpreter born on the pulled code (nunmai_cli/update_handoff.py).
...
stops at the swap and re-executes ``nunmai update --post-swap <file>``; the child resumes the
...
from nunmai_cli import update_cmd, update_handoff, update_receipt
...
def test_parent_uses_handoff_module_loaded_before_checkout_changes(monkeypatch):
    ...
    pulled_handoff = ModuleType("nunmai_cli.update_handoff")
    ...
    monkeypatch.setitem(sys.modules, "nunmai_cli.update_handoff", pulled_handoff)
...
def test_parent_reexecs_tail_on_pulled_tree_and_relays_exit_code(monkeypatch, tmp_path):
    """The pre-swap interpreter runs NOTHING after the swap: it spawns ``python -m nunmai_cli.main
    ...
    import nunmai_cli.update_cmd_config as cfg
    ...
    assert spawned["cmd"][:4] == [sys.executable, "-m", "nunmai_cli.main", "update"]
    assert spawned["cmd"][4:] == ["--yes", "--no-gateway-restart", "--branch", "main", "--post-swap", spawned["cmd"][-1]]
    env = spawned["env"]
    assert env[update_handoff.POST_SWAP_ENV] == "1" and env["NUNMAI_UPDATE_REEXEC"] == "1"
...
def test_child_resumes_receipt_and_runs_git_tail_from_payload(monkeypatch, tmp_path):
    ...
    handoff.write_text(json.dumps({
        ...
        "receipt": {"schema": 1, "started_at": started, "outcome": "running", "steps": [{"name": "pre_update_backup", "ok": True}],
                    "skips": [], "gateway_restart": {}, "fleet": [], "pre_update": {"sha": "a" * 40}, "argv": ["nunmai", "update"]},
    }), encoding="utf-8")
    import nunmai_cli.update_cmd_config as cfg
    seen = {}
