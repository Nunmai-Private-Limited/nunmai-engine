...
def test_identical_hook_failure_warns_once_then_debug(manager, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="nunmai_cli.plugins"):
        for i in range(5):
...
def test_execution_chain_middleware_failure_warns_once(manager, caplog, monkeypatch):
    ...
    from nunmai_cli import middleware as mw

    monkeypatch.setattr("nunmai_cli.plugins._plugin_manager", manager)
