...
from nunmai_cli.main_web_build import _build_web_ui, _run_npm_install_deterministic
from nunmai_cli.main_web_build import _web_ui_build_needed, _compute_web_ui_content_hash, _missing_web_build_tool, _web_ui_stamp_path, _write_web_ui_build_stamp
from nunmai_cli.update_cmd import _web_build_toolchain_ready, _web_toolchain_roots
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nunmai_cli.main.subprocess.run", return_value=install_cp), \
             patch("nunmai_cli.main_web_build._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("nunmai_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("nunmai_cli.main_web_build._time.sleep") as mock_sleep, \
