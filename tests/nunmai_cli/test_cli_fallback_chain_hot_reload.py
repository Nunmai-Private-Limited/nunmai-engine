...
``NunmaiCLI`` holds one long-lived agent and read the chain once in ``__init__``; ``nunmai fallback
add`` from another terminal never reached the open chat. The turn loop (``NunmaiCLI.chat``) now
...
def _chat_turn(monkeypatch, shell, config_text: str) -> None:
    """Drive ``NunmaiCLI.chat`` past the fallback sync against ``config_text`` as the live config.yaml."""
