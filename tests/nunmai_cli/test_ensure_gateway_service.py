"""Tests for nunmai_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `nunmai setup` and `nunmai import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("nunmai_constants.is_container", lambda: container)
