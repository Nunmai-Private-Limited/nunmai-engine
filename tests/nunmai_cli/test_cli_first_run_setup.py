...
def test_credentials_ready_false_when_no_provider(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _raise)
...
def test_offer_first_run_setup_re_resolves_reasoning_for_picked_model(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.main.select_provider_and_model", lambda: None)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
...
def test_empty_key_error_names_actual_provider(monkeypatch, capsys):
    ...
    assert "nunmai model" in out or "nunmai setup" in out
...
def _forbid_wizard(monkeypatch, shell):
    monkeypatch.setattr("nunmai_cli.main.select_provider_and_model",
...
def test_benched_credential_prints_cooldown_instead_of_wizard(monkeypatch, capsys):
    ...
    def _raise(**kwargs):
        raise AuthError("Nunmai is not logged into Nunmai Portal.", provider="nous",
    ...
    assert "re-authenticate" not in out and "nunmai model" not in out


def test_auth_json_only_login_explains_instead_of_wizard(monkeypatch, capsys, tmp_path):
    """auth.json-only shape: logged into Nunmai but no ``model.provider`` (requested "auto"). The
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", lambda **kw: (_ for _ in ()).throw(
        AuthError("Nunmai is not connected to any AI provider yet.", code="no_provider_configured")))
    shell._maybe_offer_first_run_setup()
    assert offered == [True]
