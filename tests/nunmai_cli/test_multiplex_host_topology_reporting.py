...
* ``nunmai -p <served> cron status`` names the HOST gateway as the ticker and never tells the
  user to start a second process.
* ``nunmai -p <served> doctor`` reports the single host gateway + its roster, not
  "Per-profile gateways: up/total", and still checks the HOST systemd unit's linger.
* ``nunmai claw``'s destructive-action warning FIRES for a served profile.
...
def served_host(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_PROFILE", "served")
    monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(locks))
    monkeypatch.setattr("nunmai_constants.get_default_nunmai_root", lambda: root)
    monkeypatch.setattr("nunmai_cli.gateway.find_gateway_pids", lambda: [])
...
def test_cron_status_names_the_host_gateway_for_a_served_profile(served_host, capsys, monkeypatch):
    ...
    assert "nunmai gateway install" not in out
    assert "sudo nunmai gateway install --system" not in out


def test_claw_warning_fires_for_a_served_profile(served_host, capsys, monkeypatch):
    from nunmai_cli import claw
...
def test_doctor_reports_the_single_host_gateway_not_per_profile_slots(served_host, capsys, monkeypatch):
    ...
    class _Mgr:
        def is_running(self, name):
            return name in ("main-nunmai", "gateway-legacy")
    ...
    monkeypatch.setattr("nunmai_cli.service_manager.S6ServiceManager", _Mgr)
    issues: list[str] = []
...
def test_doctor_checks_host_unit_linger_under_a_served_profile(served_host, tmp_path, capsys, monkeypatch, xdg):
    ...
    (unit_dir / "nunmai-gateway.service").write_text("[Unit]\n")  # the HOST unit; no per-profile unit
    ...
    monkeypatch.setattr("nunmai_cli.gateway.is_linux", lambda: True)
    monkeypatch.setattr("nunmai_cli.service_manager.detect_service_manager", lambda: "systemd")
    monkeypatch.setattr("nunmai_cli.gateway.get_systemd_linger_status", lambda *a, **k: (False, "off"))
