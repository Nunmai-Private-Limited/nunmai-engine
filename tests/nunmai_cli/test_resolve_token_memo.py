...
def _fresh_memo(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NUNMAI_PORTAL_BASE_URL", raising=False)
...
def test_memo_expires_after_ttl(monkeypatch, tmp_path):
    ...
    cache_key = auth.nunmai_home_key()
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    nunmai_constants.set_nunmai_home_override (gateway/run.py,
    tui_gateway/server.py), not the NUNMAI_HOME env var — the memo must key
    on that same resolved home, or one profile's context can read another
    profile's already-cached Nunmai access token for up to the TTL window.
    """
    ...
    reset_token = nunmai_constants.set_nunmai_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        nunmai_constants.reset_nunmai_home_override(reset_token)
    ...
    reset_token = nunmai_constants.set_nunmai_home_override(str(profile_b))
    try:
