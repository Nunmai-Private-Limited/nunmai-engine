...
@pytest.mark.parametrize("exc, pointer, absent", [
    (_StatusError("HTTP 401: Invalid API key", 401), "nunmai model", "HTTP 401"),
    ...
    (Exception("Unknown error"), "nunmai doctor", "Unknown error"),
])
...
def test_chat_error_response_accepts_plain_string_summary():
    ...
    assert "nunmai model" in text.splitlines()[0]
...
# ── cli-30: nunmai sessions with a bad id / unopenable DB ──────────────────
...
def test_sessions_db_open_failure_points_to_repair(monkeypatch, capsys):
    ...
    db_path = nunmai_state._default_db_path()
    ...
    assert code == 1
    assert "nunmai sessions repair" in out
