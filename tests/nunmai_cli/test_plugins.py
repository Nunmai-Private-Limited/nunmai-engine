...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        assert "Relay lifecycle is owned by Nunmai core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Nunmai Relay plugin" in caplog.text
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under nunmai_plugins.<name>."""
        plugins_dir = tmp_path / "nunmai_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "nunmai_test"))

        # Clean up any prior namespace module
        sys.modules.pop("nunmai_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "nunmai_plugins.ns_plugin" in sys.modules
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.config.load_config", lambda: {"hooks": {}}
        )
        home_key = str(get_nunmai_home().expanduser().resolve())
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("nunmai_cli.plugins.PluginContext.dispatch_tool.__module__", "nunmai_cli.plugins"):
...
class TestPluginDebugLogging:
    """NUNMAI_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("NUNMAI_PLUGINS_DEBUG", raising=False)
        from nunmai_cli import plugins as plugins_mod
