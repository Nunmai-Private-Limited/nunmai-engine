"""Tests for the Nunmai plugin system (nunmai_cli.plugins)."""
...
from nunmai_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV
from nunmai_cli.middleware import (
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<nunmai_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<nunmai_home>/plugins/``; we derive
    ``<nunmai_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Nunmai home for this
    plugin isn't necessarily the current ``NUNMAI_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``NUNMAI_HOME``/``set_nunmai_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <NUNMAI_HOME>/config.yaml.
        # Config is always read from NUNMAI_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            nunmai_home = Path(home)
        else:
            import os
            nunmai_home_str = os.environ.get("NUNMAI_HOME")
            if nunmai_home_str:
                nunmai_home = Path(nunmai_home_str)
            else:
                nunmai_home = base.parent
        nunmai_home.mkdir(parents=True, exist_ok=True)
        cfg_path = nunmai_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        from nunmai_cli import plugins as plugins_mod
    ...
    def test_enabled_portable_plugin_registers_components(
        ...
        from nunmai_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
        from nunmai_cli import plugins as plugins_mod
        ...
        (plugin / "plugin.json").write_text(
            json.dumps({
                ...
                "name": "portable.test",
                "extensions": {"com.nousresearch.nunmai": {"servers": {"worker": {
        ...
        from nunmai_cli.agent_plugins import liveness_for
        from nunmai_platform import declaration
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from nunmai_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from nunmai_cli import plugins as plugins_mod
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "nunmai_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "nunmai_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_entry_point_function_form_registers(self, tmp_path, monkeypatch):
        ...
        """
        nunmai_home = tmp_path / "nunmai_test"
        nunmai_home.mkdir(parents=True, exist_ok=True)
...
class TestPluginLoading:
    ...
    def test_bundled_cron_provider_is_not_loaded_by_general_manager(self, tmp_path, monkeypatch):
        ...
        nunmai_home = tmp_path / "nunmai_test"
        nunmai_home.mkdir(exist_ok=True)
        (nunmai_home / "config.yaml").write_text(yaml.safe_dump({"plugins": {"enabled": ["chronos"]}}))
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        from nunmai_cli import plugins as plugins_mod
    ...
    def test_user_cron_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        nunmai_home = tmp_path / "nunmai_test"
        plugin_dir = nunmai_home / "plugins" / "mycron"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin.yaml").write_text(yaml.dump({"name": "mycron"}), encoding="utf-8")
        (plugin_dir / "__init__.py").write_text(
            "def register(ctx):\n    ctx.register_cron_scheduler(object())\n", encoding="utf-8")
        (nunmai_home / "config.yaml").write_text(yaml.safe_dump({"plugins": {"enabled": ["mycron"]}}))
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``nunmai_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "nunmai_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $NUNMAI_HOME/plugins/.
        nunmai_home = tmp_path / "nunmai_test"
...
class TestDeliveryParity:
    ...
    def _fresh_manager(self, monkeypatch, register_body):
        ...
        import nunmai_cli.plugins as plugins_mod
...
class TestForceReloadSymmetry:
    ...
    def test_hook_timeout_does_not_block_caller(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.plugins._resolve_hook_callback_timeout", lambda: 0.15
        )
    ...
    def test_hook_exception_still_isolated_under_timeout_path(self, monkeypatch):
        monkeypatch.setattr(
            "nunmai_cli.plugins._resolve_hook_callback_timeout", lambda: 1.0
        )
    ...
    def test_pre_tool_call_callback_exception_fails_closed(self, monkeypatch, timeout):
        ...
        monkeypatch.setattr(
            "nunmai_cli.plugins._resolve_hook_callback_timeout", lambda: timeout
        )
    ...
    def test_hung_callback_suppresses_repeat_fires(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.plugins._resolve_hook_callback_timeout", lambda: 0.1
        )
    ...
    def test_concurrent_same_tool_calls_with_distinct_ids_both_run(self, monkeypatch):
        ...
        import time
        monkeypatch.setattr(
            "nunmai_cli.plugins._resolve_hook_callback_timeout", lambda: 5.0
        )
    ...
    def test_hung_worker_caps_new_call_identities_after_suppression(self, monkeypatch, caplog):
        ...
        the callback (#105223)."""
        import nunmai_cli.plugins_dispatch as dispatch
    ...
    def test_hung_worker_does_not_fail_closed_forever(self, monkeypatch):
        ...
        from nunmai_cli.plugins import _PRE_TOOL_CALL_TIMEOUT_BLOCK_MESSAGE
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "nunmai_cli.plugins.invoke_hook",
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from nunmai_cli import observability
        from nunmai_cli.plugins import get_pre_tool_call_directive
    ...
    def test_later_block_outranks_earlier_approve(self, monkeypatch):
        ...
        from nunmai_cli.plugins import _get_pre_tool_call_directive_details
        monkeypatch.setattr(
            "nunmai_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from nunmai_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from nunmai_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("nunmai_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `nunmai plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_nunmai_home_override(self, tmp_path):
        """set_nunmai_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``NUNMAI_HOME`` via a
        context-local ContextVar, which — per
        ``nunmai_constants.set_nunmai_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``NUNMAI_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('NUNMAI_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['NUNMAI_HOME'] itself (simulating
        # a real plugin like nunmai-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_nunmai_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_nunmai_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``nunmai_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``nunmai_plugins.<slug>.state``. If a profile switch replaces only
        ...
        def write_stateful_plugin(home: Path, marker: str) -> None:
            ...
            # `nunmai_plugins.stateful_plugin.state`.
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("nunmai_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from NUNMAI_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """NUNMAI_HOME at the root resolves to 'default'."""
        home = tmp_path / ".nunmai"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """NUNMAI_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".nunmai" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NUNMAI_HOME", str(prof))
