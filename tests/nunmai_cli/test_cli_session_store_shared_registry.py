...
managers acquire the same path through ``nunmai_state_registry`` from the REPL thread a
...
import nunmai_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = nunmai_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(nunmai_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        NunmaiCLI._init_session_store(cli)
        ...
        nunmai_state_registry.close_all()
