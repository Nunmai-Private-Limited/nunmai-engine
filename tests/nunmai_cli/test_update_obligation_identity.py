...
from nunmai_cli import update_cmd, update_cmd_fleet, update_receipt
...
def test_supervised_serve_row_never_vetoes_gateway_discharge(monkeypatch, supervisor):
    """A host that also runs a supervised ``nunmai dashboard``/``serve`` carries that row in every
    ...
    (directory / "latest.json").write_text(json.dumps({
        ...
        "gateway_restart": {"restarted_services": ["nunmai-gateway"], "incomplete": False},
        "plan": {"runtimes": [
