"""Tests for kanban board export / import (``nunmai_cli.kanban_transfer``).
...
from nunmai_cli import kanban_transfer as kt
from nunmai_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("NUNMAI_KANBAN_HOME", str(root))
        for var in ("NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_WORKSPACES_ROOT",
                    "NUNMAI_KANBAN_ATTACHMENTS_ROOT", "NUNMAI_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Nunmai"):
