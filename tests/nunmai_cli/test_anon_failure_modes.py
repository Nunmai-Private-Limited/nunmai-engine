"""Nunmai free tier: every way the account service or the wire can refuse the free tier, and what
Nunmai does with each (the failure-mode contract behind the desktop's onboarding copy).

Driven through a fake NAS whose responses are the ones the real service sends (see the code table
in ``nunmai_cli.anon_auth``), never through mocked-away client code.
"""
...
from nunmai_cli import anon_auth, anon_sign_in, free_tier_bootstrap
from nunmai_cli.auth import _load_auth_store

from tests.nunmai_cli.anon_portal import PORTAL, WELCOME, install_portal  # noqa: F401
...
class TestNasRefusalCodes:
    def test_surface_not_enabled_is_a_terminal_gate(self, nas):
        ...
        assert "Nunmai account" in str(err) and "free" in str(err)
    ...
    def test_a_locked_account_is_never_replaced_through_connectors_either(self, nas):
        ...
        from tests.nunmai_cli.anon_portal import make_jwt
...
class TestSignInFailures:
    @pytest.mark.parametrize("code,retry_after,retryable,needle", [
        ...
        (anon_auth.ANON_GATE_CLOSED, 0, False, "Nunmai account"),
    ...
    def test_a_service_verdict_keeps_its_code_wait_and_copy(self, code, retry_after, retryable, needle):
        ...
        assert "Nunmai " not in state.copy and "http" not in state.copy
...
def test_extra_welcome_hosts_make_a_local_stand_in_the_welcome_host(monkeypatch):
    """``NUNMAI_EXTRA_WELCOME_HOSTS`` (env-only) extends the ROUTE predicate so a rehearsal against a
    local stand-in gets the free tier's rules — including the route-keyed dark-tier 403."""
    monkeypatch.delenv("NUNMAI_EXTRA_WELCOME_HOSTS", raising=False)
    assert anon_auth.route_is_welcome_host("http://127.0.0.1:8765/v1") is False
    monkeypatch.setenv("NUNMAI_EXTRA_WELCOME_HOSTS", "127.0.0.1, localhost")
