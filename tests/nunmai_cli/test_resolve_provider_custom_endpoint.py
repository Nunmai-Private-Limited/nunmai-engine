...
``nunmai chat`` (which resolves the runtime directly) worked against the same config.
"""
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_GUEST_ONBOARDING", raising=False)
    for var in ("OPENAI_API_KEY", "OPENROUTER_API_KEY", "ANTHROPIC_API_KEY", "OPENAI_BASE_URL",
                "OPENROUTER_BASE_URL", "NUNMAI_INFERENCE_PROVIDER", "NOUS_API_KEY"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # has_named_custom_provider() already routes it at runtime (``nunmai chat`` works), so the
...
def test_a_provider_configured_after_boot_flips_the_stale_setup_record(isolated_home, monkeypatch):
    ...
    ``nunmai setup`` in another process. A ``False`` record is reconciled with the config files
    ...
    token = set_nunmai_home_override(str(profile_b))
    try:
