"""``nunmai update`` refreshes the installed macOS ``Nunmai.app`` from the rebuilt bundle (#52339).

``nunmai desktop --build-only`` only packages into ``apps/desktop/release/``; Finder launches the
...
def _bundle(root: Path, asar: bytes) -> Path:
    ...
    (app / "Contents" / "MacOS" / "Nunmai").write_bytes(b"\xcf\xfa\xed\xfe")
...
def test_stale_bundle_is_replaced_current_and_running_are_left_alone(rebuilt, tmp_path):
    ...
    installed, problems = main_desktop._install_rebuilt_macos_bundles(
        rebuilt, [stale, current, running, tmp_path / "missing" / "Nunmai.app"],
        running={running.resolve()})

    assert installed == [stale]
    assert _asar(stale) == b"rebuilt"
    assert not (stale.parent / "Nunmai.app.nunmai-update-old").exists()
    assert not (stale.parent / "Nunmai.app.nunmai-update-new").exists()
...
def test_failed_swap_keeps_the_previous_bundle_launchable(rebuilt, tmp_path, monkeypatch):
    ...
    def fail_final_rename(self, target):
        if self.name.endswith(".nunmai-update-new"):
