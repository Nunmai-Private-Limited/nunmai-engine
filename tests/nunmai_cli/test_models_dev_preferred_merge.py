...
These guard the contract:
  ...
  * OpenRouter and Nunmai Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Nunmai) semantics.
...
appear in ``/model`` without a Nunmai release.
"""
...
class TestMergeHelper:
    def test_deepseek_picker_ignores_models_dev_retired_ids(self):
        ...
        ), patch("nunmai_cli.models._PROVIDER_CATALOG_FETCHERS", {}), \
                patch("nunmai_cli.models._profile_live_catalog", return_value=None):
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("nunmai_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from nunmai_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("nunmai_cli.main_provider_setup._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("nunmai_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("nunmai_cli.config.get_env_value", return_value=""),
