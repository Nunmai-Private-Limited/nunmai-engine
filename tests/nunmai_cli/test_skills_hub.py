...
from nunmai_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import tools.skills_hub_install as hub_install
    import nunmai_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`nunmai skills list` reads the active profile's config via
    NUNMAI_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from NUNMAI_PLATFORM env."""
...
def test_inspect_reuses_one_ssrf_safe_client_for_metadata_and_bundle(monkeypatch, tmp_path):
    ...
    class Source:
        def inspect(self, _identifier):
            ...
            # The Nunmai-index fetch must ride the same pool (no cache → real GET).
            assert search._load_nunmai_index() == {"skills": []}
    ...
    monkeypatch.setattr(search, "_nunmai_index_cache_file", lambda: tmp_path / "nunmai-index.json")
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `nunmai skills search`
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    from nunmai_cli.skills_hub import do_search
