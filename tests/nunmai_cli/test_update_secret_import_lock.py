...
External secret backends are useful during normal Nunmai startup, but the
...
""",
    ...
    dispatch = (
        "nunmai_main.cmd_update = lambda _args: 0\n"
        "nunmai_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import nunmai_cli.main as nunmai_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "NUNMAI_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["nunmai", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_nunmai_dotenv(
        nunmai_home=home,
...
def test_update_probe_children_skip_external_secret_sources(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import sys; sys.argv = ['nunmai', 'update']\n"
         "from nunmai_cli.update_cmd_deps import _validate_critical_modules_import\n"
