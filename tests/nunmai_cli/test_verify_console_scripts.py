...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("nunmai_cli.main_install_repair._is_windows", return_value=True), \
             patch("nunmai_cli.main_install_repair._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("nunmai_cli.main_install_repair._run_quarantined_install") as mock_install:
            from nunmai_cli.main_install_repair import _verify_console_scripts_installed
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("nunmai_cli.main_install_repair._is_windows", return_value=True):
            names = {path.name for path in main_install_repair._nunmai_exe_shims(fake_scripts_dir)}
