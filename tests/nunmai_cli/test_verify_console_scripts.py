...
``uv pip install -e .`` can record ``nunmai.exe`` in the wheel RECORD while the
file never lands, so ``nunmai`` drops off PATH after a "successful" install.
...
_SCRIPTS = ("nunmai", "nunmai-engine", "nunmai-acp")
...
def test_missing_nunmai_exe_is_reinstalled(scripts_dir, monkeypatch, capsys):
    """The #52931 shape: nunmai-engine/nunmai-acp landed, nunmai.exe did not."""
    for name in ("nunmai-engine", "nunmai-acp"):
        (scripts_dir / f"{name}.exe").write_bytes(b"shim")
    calls = _record_installs(monkeypatch, lands=("nunmai",))

    main_install_repair._verify_console_scripts_installed(["uv", "pip"], env={})

    assert calls == [(["uv", "pip", "install", "--reinstall", "-e", "."], scripts_dir)]
    assert (scripts_dir / "nunmai.exe").is_file()
    out = capsys.readouterr().out
    assert "nunmai" in out and "missing" in out
...
def test_repair_that_still_leaves_a_shim_missing_is_reported(scripts_dir, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Still missing after repair: nunmai" in out
    assert "python -m nunmai_cli.main" in out
