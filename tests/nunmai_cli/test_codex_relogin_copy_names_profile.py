...
Profiles are islands (93889b770da): a bare ``nunmai auth`` / ``nunmai model`` from a named
profile's error text re-signs the ROOT store, which is exactly the loop #114012 measured.
Every relogin hint raised by ``nunmai_cli/auth_codex.py`` and appended by
``format_auth_error`` must carry the ``-p <profile>`` selector under a profile NUNMAI_HOME.
"""
...
from nunmai_cli.auth import AuthError, _read_codex_tokens, format_auth_error
from nunmai_cli.auth_codex import _codex_refresh_failure_error


@pytest.fixture
def codex_profile_home(tmp_path, monkeypatch):
    profile_home = tmp_path / ".nunmai" / "profiles" / "codex"
...
def test_missing_and_reused_codex_credential_copy_names_the_profile(codex_profile_home):
    ...
    for err in (missing.value, reused):
        text = str(err)
        assert "`nunmai -p codex auth add openai-codex --type oauth`" in text, text
        assert "`nunmai auth`" not in text, text
...
def test_format_auth_error_relogin_suffix_names_the_profile(codex_profile_home):
    ...
    assert "`nunmai -p codex model`" in rendered, rendered
    assert "`nunmai model`" not in rendered, rendered
