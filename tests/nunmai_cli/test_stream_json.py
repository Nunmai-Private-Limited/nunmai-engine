"""``nunmai chat -q … --format stream-json`` emits a parseable JSONL event stream and nothing else on stdout."""
...
def _run_stream_json_chat(monkeypatch, capsys, run_conversation, credentials_ok=True):
    ...
    import cli
    import nunmai_cli.main as cli_entry
    ...
    monkeypatch.setattr(cli, "NunmaiCLI", FakeCLI)
    ...
    monkeypatch.setattr("nunmai_cli.free_tier_bootstrap.run_bootstrap", lambda **_k: None)
    monkeypatch.setattr("nunmai_cli.quiet_single_query.continue_quiet_notify_completions", lambda *_a, **_k: None)
