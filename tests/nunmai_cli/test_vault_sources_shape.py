"""`nunmai vault sources --disable` must not crash on a hand-edited non-dict
...
from nunmai_cli.vault import _cmd_sources, vault_command
...
def test_vault_command_surfaces_corrupt_store_as_clean_error(tmp_path, monkeypatch, capsys):
    ...
    vault_command(SimpleNamespace())  # bare `nunmai vault` -> _cmd_list
