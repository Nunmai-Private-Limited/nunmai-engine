"""Tests for the Command Installation check in nunmai doctor."""
...
import nunmai_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal NUNMAI_HOME + PROJECT_ROOT for doctor tests."""
    ...
    nunmai_bin = venv_bin_dir / "nunmai"
    nunmai_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    ...
    return home, project, nunmai_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, nunmai_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "nunmai"
        wrong_target = tmp_path / "wrong_nunmai"
        ...
        assert cmd_link.resolve() == nunmai_bin.resolve()
