...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated NUNMAI_HOME.
        import nunmai_constants
        stamp = nunmai_constants.get_nunmai_home() / "cache" / ".uv_self_update_stamp"
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("nunmai_cli.managed_uv._install_uv_posix") as mock_posix:
            from nunmai_cli.managed_uv import _install_uv
            _install_uv(target)
