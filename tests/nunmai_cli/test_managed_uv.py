...
from nunmai_cli.managed_uv import _CandidateStageError
...
class TestPipInstallHint:
    ...
    def test_names_the_managed_uv_when_present(self, tmp_path):
        ...
        _make_executable(uv)
        with patch("nunmai_cli.managed_uv.get_nunmai_home", return_value=tmp_path):
            from nunmai_cli.managed_uv import pip_install_hint
...
class TestEnsureUv:
    ...
    def test_dead_managed_launcher_is_reinstalled(self, tmp_path):
        ...
        with patch("nunmai_cli.managed_uv.get_nunmai_home", return_value=tmp_path), \
             patch("nunmai_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("nunmai_cli.managed_uv._uv_version", return_value="uv 0.1.2"), \
             patch("nunmai_cli.managed_uv._install_uv", side_effect=_make_executable) as mock_install:
            from nunmai_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("nunmai_cli.managed_uv.get_nunmai_home", return_value=tmp_path), \
             patch("nunmai_cli.managed_uv._uv_runs", return_value=True), \
             patch("nunmai_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
...
class TestStageCandidateVenvCrossPlatform:
    ...
    def test_sync_keeps_uv_project_config_and_merges_stderr(self, tmp_path):
        ...
        with patch("nunmai_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("nunmai_cli.managed_uv.subprocess.Popen", side_effect=fake_popen), \
             patch(
                 "nunmai_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_sync_failure_reports_the_child_reason(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING), \
             patch("nunmai_cli.managed_uv.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch(
                 "nunmai_cli.managed_uv.subprocess.Popen",
        ...
        assert not list((root / ".nunmai-runtime").glob("venv-candidate-*"))
