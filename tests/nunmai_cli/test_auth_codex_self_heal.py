...
Nunmai keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Nunmai process) rotates the shared
...
from nunmai_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Nunmai auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Nunmai auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "nunmai-auth-store"
    stored = json.loads((nunmai_home / "auth.json").read_text())
...
def test_opt_out_never_adopts_codex_cli_login(tmp_path, monkeypatch):
    ...
    family the user did not hand to Nunmai. Both automatic recovery paths must leave it (and Nunmai' own
    auth.json) untouched and surface the real error instead."""
    nunmai_home = tmp_path / "nunmai"
    codex_home = tmp_path / "codex"
    nunmai_home.mkdir()
    codex_home.mkdir()
    (nunmai_home / "config.yaml").write_text("auth:\n  adopt_external_logins: false\n")
    nunmai_auth = {"version": 1, "providers": {"openai-codex": {
        "tokens": {"refresh_token": "stale-refresh"}, "auth_mode": "chatgpt"}}}
    (nunmai_home / "auth.json").write_text(json.dumps(nunmai_auth))
    (codex_home / "auth.json").write_text(json.dumps({
    ...
    assert json.loads((nunmai_home / "auth.json").read_text()) == nunmai_auth
...
def _seed_homes(tmp_path, monkeypatch, nunmai_tokens, cli_tokens):
    nunmai_home, codex_home = tmp_path / "nunmai", tmp_path / "codex"
    nunmai_home.mkdir()
    codex_home.mkdir()
    (nunmai_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {"openai-codex": {
        "tokens": nunmai_tokens, "auth_mode": "chatgpt"}}}))
    (codex_home / "auth.json").write_text(json.dumps({"tokens": cli_tokens}))
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    monkeypatch.setenv("CODEX_HOME", str(codex_home))
    return nunmai_home / "auth.json"
...
def test_recovery_refuses_codex_cli_login_from_another_workspace(tmp_path, monkeypatch, caplog):
    ...
    Nunmai credential it is supposed to repair — the store stays byte-identical and the log says why."""
    ...
    with caplog.at_level("WARNING", logger="nunmai_cli.auth"), pytest.raises(AuthError) as info:
