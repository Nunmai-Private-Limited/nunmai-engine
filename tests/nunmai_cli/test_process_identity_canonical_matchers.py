...
from nunmai_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from nunmai_cli.update_cmd_windows import _nunmai_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m nunmai_cli.main` spawn shape —
# a user-launched `nunmai serve` / `nunmai dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m nunmai_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m nunmai_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/nunmai serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\nunmai\.venv\Scripts\nunmai.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("nunmai.exe dashboard", "dashboard", False, False),
    ("nunmai --profile ops serve " + LOOPBACK, "serve", True, False),
    ("nunmai -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m nunmai_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("nunmai --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("nunmai gateway run --replace", "gateway", False, False),
    ("nunmai chat --model serve", "chat", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _nunmai_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("nunmai serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("nunmai serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("nunmai serve --host localhost --port 0")
