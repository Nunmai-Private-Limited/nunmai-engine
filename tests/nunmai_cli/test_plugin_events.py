...
``nunmai_cli.plugin_events`` instead of importing
...
from nunmai_cli import plugin_events
...
def test_broadcast_from_a_turn_isolation_child_reaches_the_parent_gateways_clients(monkeypatch):
    ...
    ``nunmai serve`` and fan out there instead of dying at ``logger.debug``."""
    import io
    import json
    import os
    import threading
    ...
    monkeypatch.setenv("NUNMAI_COMPUTE_HOST_HEARTBEAT_SECS", "0")
    monkeypatch.setenv("NUNMAI_COMPUTE_HOST_CHILD", "0")
...
def test_broadcast_without_a_gateway_module_is_a_logged_no_op(monkeypatch, caplog):
    """A plugin-only process (``nunmai plugins validate`` importing the backend) has no
    ...
    with caplog.at_level(logging.WARNING, logger="nunmai_cli.plugin_events"):
