...
from nunmai_cli.web_routers.sessions import _project_for_display
...
def test_rest_pages_bind_the_history_owner_for_messages_and_around(
    ...
    from nunmai_cli import config as config_mod
    from nunmai_cli.web_routers import sessions
    import nunmai_state_timeline
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "display": {
                "show_commentary": str(get_nunmai_home_override())
                == str(homes["visible"])
            }
        },
    )
...
def test_unscoped_rest_history_uses_custom_home_of_its_database(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "display": {
                "show_commentary": str(get_nunmai_home_override()) != str(custom)
            }
        },
    )
    row = _row()
    ...
    token = set_nunmai_home_override(str(custom))
    try:
        asyncio.run(exercise())
    finally:
...
def test_cold_resume_uses_the_resumed_session_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "load_config",
        lambda: {
            "display": {"show_commentary": str(get_nunmai_home_override()) != str(home)}
        },
    )
