...
profile switcher can target any profile's NUNMAI_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via NUNMAI_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_nunmai_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_nunmai_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import nunmai_cli.mcp_config as mcp_config
    ...
    def test_mcp_test_resolves_profile_secret_source_scope(
        ...
        def _worker_sources(nunmai_home):
            if Path(nunmai_home).resolve() == worker_home.resolve():
    ...
    def test_mcp_list_expands_url_ref_from_profile_secret_scope(
        ...
        monkeypatch.setattr(
            ...
            lambda nunmai_home: {"MCP_GH_URL": "http://worker/mcp"}
            if Path(nunmai_home).resolve() == worker_home.resolve() else {},
        )
...
class TestProfileScopedModel:
    ...
    def _accept_any_model(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.model_switch.switch_model", _switch)
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same NUNMAI_HOME the rest
        ...
        monkeypatch.setattr(
            "nunmai_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_nunmai_home()))
            return None
        ...
        assert resp.json()["nunmai_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_nunmai_home(self, isolated_profiles, monkeypatch):

        monkeypatch.setattr(
            "nunmai_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["NUNMAI_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "NUNMAI_TUI_GATEWAY_URL" not in env
