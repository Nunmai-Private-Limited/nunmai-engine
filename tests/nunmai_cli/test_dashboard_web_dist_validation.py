"""Regression tests: `nunmai dashboard` validates NUNMAI_WEB_DIST before serving.

A custom NUNMAI_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "nunmai_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "nunmai_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """NUNMAI_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("NUNMAI_WEB_DIST", str(empty_dist))
    ...
    builds = []
    monkeypatch.setattr(
        "nunmai_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "NUNMAI_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("NUNMAI_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "nunmai_cli" / "web_dist"
    ...
    monkeypatch.setattr("nunmai_cli.main_web_build._build_web_ui", fake_build)
...
def test_desktop_child_dashboard_drops_packaged_renderer(main_mod, monkeypatch):
    """#116107: every desktop-spawned process inherits NUNMAI_DESKTOP=1 with the
    packaged dist; a browser `nunmai dashboard` from it must not keep serving the
    IPC-only desktop renderer."""
    packaged = "/Applications/Nunmai.app/Contents/Resources/app.asar.unpacked/dist"
    monkeypatch.setenv("NUNMAI_DESKTOP", "1")
    monkeypatch.setenv("NUNMAI_WEB_DIST", packaged)
    monkeypatch.setenv("NUNMAI_SERVE_HEADLESS", "1")

    main_mod._dashboard_sanitize_desktop_env(headless_backend=False)

    assert "NUNMAI_WEB_DIST" not in os.environ
    assert "NUNMAI_SERVE_HEADLESS" not in os.environ
...
def test_desktop_headless_serve_keeps_packaged_renderer(main_mod, monkeypatch):
    ...
    assert os.environ["NUNMAI_WEB_DIST"] == packaged
