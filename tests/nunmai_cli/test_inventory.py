"""Behavior tests for nunmai_cli.inventory.
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Nunmai' own device flow
    ...
    with (
        ...
        patch(
            "nunmai_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from nunmai_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "nunmai_cli.auth.read_credential_pool",
            return_value=[
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("nunmai_cli.models.clear_provider_models_cache") as clear:
...
def test_picker_metadata_uses_one_config_read_for_real_models_dev_lookups(tmp_path, monkeypatch):
    ...
    def _build_and_count(model_ids):
        ...
        with (
            ...
            patch("nunmai_cli.inventory._local_runtime_row", return_value=None),
            patch("nunmai_cli.inventory._moa_provider_row", return_value=None),
            patch("nunmai_cli.models.model_supports_fast_mode", return_value=False),
            patch("nunmai_cli.inventory._reasoning_catalog_reader", return_value=None),
