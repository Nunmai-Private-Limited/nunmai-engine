"""Tests for nunmai_cli/goals.py — persistent cross-turn goals."""
...
def nunmai_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves NUNMAI_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from nunmai_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, nunmai_home):
    ...
    def test_continuation_prompt_shape(self, nunmai_home):
        ...
        mgr.set("port goal command to nunmai")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to nunmai" in prompt
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, nunmai_home):
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from nunmai_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, nunmai_home):
        from nunmai_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, nunmai_home):
        from nunmai_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestSessionDbCacheAfterProfileDelete:
    """``nunmai profile delete`` force-closes every registry handle under the profile home
    ...
    def test_delete_then_recreate_gets_a_live_store(self, nunmai_home):
        import shutil

        import nunmai_state
        import nunmai_state_registry as registry
        from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override
        from nunmai_cli.goals import GoalState, _get_session_db, load_goal, save_goal
        ...
        with patch.object(nunmai_state, "DEFAULT_DB_PATH", nunmai_state._IMPORT_DEFAULT_DB_PATH):
            profile = nunmai_home / "profiles" / "p1"
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, nunmai_home):
        from nunmai_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, nunmai_home):
        import pytest
        from nunmai_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, nunmai_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli import goals
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, nunmai_home):
        from nunmai_cli import goals
        from nunmai_cli.goals import GoalManager
    ...
    def test_wait_on_rejects_a_pid_not_alive_on_this_host(self, nunmai_home, monkeypatch):
    ...
    def test_stop_waiting_clears_barrier(self, nunmai_home):
        from nunmai_cli.goals import GoalManager
    ...
    def test_barrier_on_a_process_that_never_exits_expires(self, nunmai_home):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_on_dead_pid_continues_instead_of_parking(self, nunmai_home):
    ...
    def test_judge_wait_on_pid_dying_between_check_and_park_continues(self, nunmai_home):
    ...
    def test_judge_wait_pid_parks_loop(self, nunmai_home):
        from nunmai_cli import goals
        from nunmai_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, nunmai_home):
        from nunmai_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, nunmai_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, nunmai_home):
    ...
    def test_wait_on_session_validation(self, nunmai_home):
        from nunmai_cli.goals import GoalManager
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from nunmai_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from nunmai_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, nunmai_home):
        from nunmai_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from nunmai_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, nunmai_home):
        from nunmai_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli import goals
        from nunmai_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli import goals
        from nunmai_cli.goals import GoalContract
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_pauses_goal_instead_of_done(self, nunmai_home):
        from unittest.mock import patch
        from nunmai_cli.goals import GoalManager
...
def test_goal_session_db_is_the_registry_shared_handle(nunmai_home):
