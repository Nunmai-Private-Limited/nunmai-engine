...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NUNMAI_HOME with .container-mode file."""
    nunmai_home = tmp_path / ".nunmai"
    nunmai_home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    monkeypatch.delenv("NUNMAI_DEV", raising=False)

    container_mode = nunmai_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nunmai-engine\n"
        "exec_user=nunmai\n"
        "nunmai_bin=/data/current-package/bin/nunmai\n"
    )
    return nunmai_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nunmai_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nunmai-engine"
    assert info["exec_user"] == "nunmai"
    assert info["nunmai_bin"] == "/data/current-package/bin/nunmai"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nunmai_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nunmai"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nunmai-engine" in cmd
    assert "/data/current-package/bin/nunmai" in cmd
    assert "chat" in cmd
