...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``nunmai -p X``);
...
- ``nunmai status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    (root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "nunmai_home": str(root)}))
    (root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "nunmai_home": str(root), "gateway_state": "running",
         "served_profiles": ["default", "coder"]}))
    monkeypatch.setenv("NUNMAI_HOME", str(root / "profiles" / "coder"))
    ...
    monkeypatch.setattr(nunmai_constants, "_default_nunmai_root_memo", None)
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: (
        "python -m nunmai_cli.main gateway run" if pid == os.getpid() else real_cmdline(pid)))
    return root
...
def test_probe_falls_back_to_config_only_without_recorded_key(served_root):
    ...
    (served_root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "nunmai_home": str(served_root), "gateway_state": "running"}))
...
def test_probe_survives_a_missing_default_pid_file(served_root):
    ...
    (``nunmai -p X status``, ``cron list``, the dashboard ladder) say "not running" about the gateway
...
def test_setup_gateway_service_step_skips_install_for_served_profile(served_root, monkeypatch, capsys):
    """``nunmai -p <profile> setup gateway`` (and ``nunmai setup`` / ``nunmai import``) reach the service
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(served_root / "profiles" / "other"))  # not in the live record
    assert gw.ensure_gateway_service(context="setup") is True
    assert calls == []
...
def test_recycled_pid_does_not_lend_a_stale_record_its_served_profiles(served_root):
    ...
    once did, so `nunmai -p coder gateway start` exited 78 for a multiplexer that was long gone."""
    import subprocess
    import gateway.status as status
    from nunmai_cli.gateway import named_profile_served_by_running_multiplexer
    ...
    try:
        ...
        for name in ("gateway.pid", "gateway_state.json"):
            (served_root / name).write_text(json.dumps({
                "pid": child.pid, "nunmai_home": str(served_root), "gateway_state": "running",
...
def test_satellite_gateway_identity_does_not_imply_cron_health(served_root, monkeypatch):
    import nunmai_cli.gateway as gw
    import nunmai_cli.status as st
    import nunmai_cli.cron as cr
    ...
    assert f"Scheduler host: the host gateway (PID {os.getpid()}) serving profiles default, coder" \
        in buf.getvalue()
    # The remediation this rung prints must run for a served NAMED profile (`nunmai gateway
    # restart` exits 78 there).
    assert "restart: nunmai --profile default gateway restart" in buf.getvalue()
...
def test_dashboard_liveness_ladder_reports_served_profile_running(served_root):
    ...
    "stopped" while `nunmai -p X status` said running. Alpha's `<X>:<platform>` entries project as its own."""
...
def test_dashboard_lifecycle_verbs_target_the_multiplexer(served_root, monkeypatch):
    ...
    managed normally."""
    from nunmai_cli import web_server_gateway
    from nunmai_cli.web_server_gateway import _gateway_subcommand, _profile_action_environment, multiplexed_profile_refusal
    ...
    # This process's own NUNMAI_HOME is coder's; the restart child must still run under the DEFAULT
    ...
    assert _profile_action_environment(restart)["NUNMAI_HOME"] == str(served_root)
    assert multiplexed_profile_refusal("coder", "stop") is None  # parks via `nunmai -p coder gateway stop`
...
def test_the_multiplexer_restart_names_the_root_even_under_a_sticky_active_profile(served_root, monkeypatch):
    ...
    from nunmai_cli.web_server_gateway import _gateway_subcommand, _profile_action_environment
    monkeypatch.setenv("NUNMAI_HOME", str(served_root))  # the dashboard runs as the default profile
    ...
    for var in ("NUNMAI_SUPERVISED_CHILD", "NUNMAI_S6_SUPERVISED_CHILD", "INVOCATION_ID",
                "NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr("sys.argv", ["nunmai", *restart])
    _apply_profile_override()  # what the spawned child does first
    assert os.environ["NUNMAI_HOME"] == str(served_root)
...
def test_the_topology_lists_a_served_profile_under_the_multiplexer_not_as_its_own_gateway(served_root, monkeypatch):
    ...
    from nunmai_cli.web_server_gateway import _collect_profile_gateway_topology
    monkeypatch.setenv("NUNMAI_HOME", str(served_root))
