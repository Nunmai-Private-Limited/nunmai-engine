"""Regression for #41403 — ``nunmai update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.nunmai.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed NUNMAI_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.nunmai.gateway",
            "ai.nunmai.gateway-merit-ops",
            "ai.nunmai.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(nunmai_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.nunmai.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.nunmai.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.nunmai.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.nunmai.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.nunmai.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.nunmai.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.nunmai.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.nunmai.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # box with a live ai.nunmai.gateway* fleet would leak its PIDs into the assertion.
        monkeypatch.setattr(gw.subprocess, "run", lambda *a, **k: _completed(0, ""))
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.nunmai.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.nunmai.gateway", "ai.nunmai.gateway-a", "ai.nunmai.gateway-b"],
        )
        located = {
            "ai.nunmai.gateway": (f"gui/{UID}", 100),
            "ai.nunmai.gateway-a": (f"gui/{UID}", 200),
            "ai.nunmai.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through nunmai_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.nunmai.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.nunmai.gateway", current, "ai.nunmai.gateway-user-scoped"],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.nunmai.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.nunmai.gateway",
            f"user/{UID}/ai.nunmai.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.nunmai.gateway",
            f"user/{UID}/ai.nunmai.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.nunmai.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.nunmai.gateway-a"],
            located={"ai.nunmai.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.nunmai.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nunmai.gateway",
            labels=["ai.nunmai.gateway", "ai.nunmai.gateway-idle"],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.nunmai.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nunmai.gateway",
            labels=["ai.nunmai.gateway", "ai.nunmai.gateway-dormant"],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.nunmai.gateway-dormant"]
        assert restarted == ["ai.nunmai.gateway", "ai.nunmai.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nunmai.gateway",
            labels=["ai.nunmai.gateway", "ai.nunmai.gateway-a"],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.nunmai.gateway-a"]
        assert restarted == ["ai.nunmai.gateway", "ai.nunmai.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.nunmai.gateway",
                "ai.nunmai.gateway-bad",
                "ai.nunmai.gateway-good",
            ],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-bad": (f"gui/{UID}", 200),
                "ai.nunmai.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.nunmai.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.nunmai.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.nunmai.gateway-good"]
        assert restarted == ["ai.nunmai.gateway", "ai.nunmai.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.nunmai.gateway",
                "ai.nunmai.gateway-wedged",
                "ai.nunmai.gateway-after",
            ],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.nunmai.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.nunmai.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.nunmai.gateway-after"]
        assert restarted == ["ai.nunmai.gateway", "ai.nunmai.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-wedged": (f"gui/{UID}", 200),
                "ai.nunmai.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.nunmai.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nunmai.gateway",
            labels=["ai.nunmai.gateway", "ai.nunmai.gateway-zombie"],
            located={
                "ai.nunmai.gateway": (f"gui/{UID}", 100),
                "ai.nunmai.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.nunmai.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.nunmai.gateway"]
        assert failed == ["ai.nunmai.gateway-zombie"]


def _write_launchd_plist(agents_dir: Path, label: str, *, argv=(), nunmai_home=None, raw=None):
    ...
    if nunmai_home is not None:
        data["EnvironmentVariables"] = {"NUNMAI_HOME": str(nunmai_home)}
...
class TestLegacyLaunchdLabelsForInstall:
    ...
    from the plist's pinned NUNMAI_HOME, fail-closed, so another install's fleet stays untouched
    (the #41403 boundary) and a sandboxed NUNMAI_HOME never enumerates the account's real units."""
    ...
    def test_only_units_pinned_to_this_installs_homes_are_credited(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(nunmai_constants, "get_default_nunmai_root", lambda: root)
        venv_python = str(root.parent / "install" / ".venv" / "bin" / "python")
        _write_launchd_plist(agents, "ai.nunmai.gateway-398559f7", argv=[venv_python], nunmai_home=root / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.nunmai.gateway-1a2b3c4d", argv=[venv_python], nunmai_home=root)
        _write_launchd_plist(agents, "ai.nunmai.gateway-gopod", argv=[venv_python], nunmai_home=root / "profiles" / "gopod")
        # Another install's unit: same venv on disk, but its home lives under a different root.
        _write_launchd_plist(agents, "ai.nunmai.gateway-5db8084b", argv=[venv_python], nunmai_home=tmp_path / "other-root" / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.nunmai.gateway-nohome", argv=[venv_python])  # no pinned home: unattributable
        _write_launchd_plist(agents, "ai.nunmai.gateway-broken", raw="not a plist at all")
        assert gw.legacy_launchd_labels_for_install(exclude={"ai.nunmai.gateway-gopod", "ai.nunmai.gateway"}) == [
            "ai.nunmai.gateway-1a2b3c4d", "ai.nunmai.gateway-398559f7",
        ]
...
class TestRestartMacosLaunchdGatewaysLegacyUnits:
    def test_legacy_labelled_sibling_is_restarted_like_any_other(self, monkeypatch, tmp_path, capsys):
        ...
        current = "ai.nunmai.gateway"
        legacy = "ai.nunmai.gateway-398559f7"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.nunmai.gateway-gopod"],
            legacy_labels=[legacy],
            located={
                current: (f"gui/{UID}", 100),
                "ai.nunmai.gateway-gopod": (f"gui/{UID}", 200),
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.nunmai.gateway-gopod",
            f"user/{UID}/{legacy}",
        ]
        assert restarted == [current, "ai.nunmai.gateway-gopod", legacy]
        assert failed == []
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.nunmai.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.nunmai.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
