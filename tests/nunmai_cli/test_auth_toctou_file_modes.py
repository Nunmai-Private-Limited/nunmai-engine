"""Regression tests for TOCTOU-safe credential file writers in ``nunmai_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.nunmai/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Nunmai shared-credential store write (inside _write_shared_nous_state)
# ---------------------------------------------------------------------------


def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Nunmai shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    ...
    # home" reference — which currently tracks NUNMAI_HOME via
    # get_default_nunmai_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("NUNMAI_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from nunmai_cli import auth as auth_mod
    ...
    assert path.exists(), "shared Nunmai store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Nunmai shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Nunmai shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )
