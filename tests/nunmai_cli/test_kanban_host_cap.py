...
1. The standalone daemon path (``nunmai kanban daemon --force`` /
   :func:`nunmai_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``nunmai kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import nunmai_cli.config as cfgmod
    import nunmai_cli.profiles as profmod
