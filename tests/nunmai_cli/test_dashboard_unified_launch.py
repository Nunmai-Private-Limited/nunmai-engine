...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # NUNMAI_HOME.  For a standard install (NUNMAI_HOME unset) that root is
        # the platform-native default (~/.nunmai), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from nunmai_constants import get_default_nunmai_root
        assert env.get("NUNMAI_HOME") == str(get_default_nunmai_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (NUNMAI_DESKTOP=1) must NOT
