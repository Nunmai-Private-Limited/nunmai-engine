...
(release/win-unpacked/Nunmai.exe), its renderer bundle (dist/), its own
node_modules and the dashboard assets (nunmai_cli/web_dist/) exist only in
the live tree. Swapping `apps` / `nunmai_cli` without grafting them deletes
...
def test_zip_swap_keeps_every_nested_build_output_and_the_guard_admits_them(tmp_path):
    from nunmai_cli.update_cmd_zip import (
    ...
    outputs = {
        "apps/desktop/release/win-unpacked/Nunmai.exe": b"MZbuilt",
        "apps/desktop/node_modules/electron/index.js": b"electron",
        "apps/desktop/dist/index.html": b"live renderer",
        "nunmai_cli/web_dist/index.html": b"<dashboard>",
    }
    for rel, data in outputs.items():
        (root / rel).parent.mkdir(parents=True)
        (root / rel).write_bytes(data)
    (root / "nunmai_cli" / "__pycache__").mkdir()
    (root / "nunmai_cli" / "x.py").write_text("old", encoding="utf-8")
    ...
    (extracted / "nunmai_cli").mkdir()
    (extracted / "nunmai_cli" / "x.py").write_text("new", encoding="utf-8")

    _commit_staged_replacements(_stage_entries(str(extracted), ["apps", "nunmai_cli"], str(root)))

    assert (root / "nunmai_cli" / "x.py").read_text(encoding="utf-8") == "new"
    for rel, data in outputs.items():
    ...
    assert not (root / "nunmai_cli" / "__pycache__").exists()  # regenerable, dropped with the old tree
    ...
    for line in ("!! apps/desktop/release/", "!! apps/desktop/dist/", "!! apps/desktop/node_modules/",
                 "!! apps/desktop/build/", "!! nunmai_cli/web_dist/", "!! nunmai_cli/__pycache__/",
                 "!! __pycache__/", "!! ui-tui/dist/", "!! ui-tui/packages/nunmai-ink/dist/",
    ...
    for line in ("!! apps/desktop/notes.local", "?? apps/desktop/release/", "!! nunmai_cli/web_dist_backup/",
...
def test_guard_admits_a_real_installs_ignored_set_and_blocks_only_what_the_swap_destroys(tmp_path):
    ...
    from nunmai_cli.update_cmd_zip import _zip_overlay_block_reason
    ...
    for tracked in ("apps/desktop/package.json", "nunmai_cli/main.py", "scripts/whatsapp-bridge/index.js",
    ...
    subprocess.run(["git", "-C", str(root), "-c", "user.email=t@t", "-c", "user.name=t", "commit", "-qm", "init"],
                   check=True)
    for ignored in (".bytecode-fingerprint", ".nunmai-bootstrap-complete", ".install_method",
                    "nunmai_agent.egg-info/PKG-INFO", "nunmai_cli/__pycache__/main.pyc", "__pycache__/x.pyc",
                    "apps/desktop/release/win-unpacked/Nunmai.exe", "apps/desktop/dist/index.html",
                    "apps/desktop/build/icon.ico", "apps/desktop/node_modules/electron/index.js",
                    "nunmai_cli/web_dist/index.html", "ui-tui/dist/entry.js", "ui-tui/node_modules/x/index.js",
                    "ui-tui/packages/nunmai-ink/dist/index.js", "web/node_modules/x/index.js",
    ...
    shipped = {"apps", "nunmai_cli", "scripts", "ui-tui", "web", "tests-js", "run_agent.py"}
    assert _zip_overlay_block_reason(root, shipped=shipped) is None
    assert _zip_overlay_block_reason(root, shipped=shipped | {"nunmai_agent.egg-info"}) is not None
