...
Nunmai ``/v1/models`` answers each caller with the catalog their org may reach,
...
from nunmai_cli.models_pricing import fetch_models_with_pricing, peek_cached_pricing
...
class TestNousCatalogExpiry:
    """A Nunmai catalog reflects the org's policy, which an admin can change while
