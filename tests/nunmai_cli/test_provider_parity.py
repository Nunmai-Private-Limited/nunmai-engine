...
the SAME provider universe as ``nunmai model`` (the CLI/TUI picker).
...
def test_every_nunmai_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `nunmai model` universe."""
    ...
    assert not missing, (
        "providers shown in `nunmai model` but not configurable in the desktop "
