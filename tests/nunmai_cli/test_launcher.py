"""Tests for the top-level `./nunmai` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nunmai` should use `nunmai_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nunmai"
    called = []

    fake_main_module = types.ModuleType("nunmai_cli.main")

    def fake_main():
        called.append("nunmai_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nunmai_cli.main", fake_main_module)
