...
``nunmai_cli.gateway_multiplex_mode.resolve_multiplex_mode`` — the same preflight
``nunmai gateway migrate --multiplex`` runs — so flipping the default can never make a default
...
from nunmai_cli import gateway_multiplex_mode as mode
...
def test_migration_plan_treats_the_unset_default_as_not_yet_multiplexed(fleet):
    """The fleet the boot guard refuses is exactly the one ``nunmai gateway migrate --multiplex`` folds:
...
def test_live_record_outranks_the_raw_flag_for_other_processes(fleet, monkeypatch):
    ...
    record = {"pid": os.getpid(), "nunmai_home": str(root), "gateway_state": "running", "served_profiles": []}
...
def test_guard_refusal_is_recorded_in_runtime_status_and_cleared_on_default(tmp_path, monkeypatch):
    """A guard refusal must be visible to `nunmai gateway status`, not only in the boot log; a later
    boot that multiplexes clears it (a stale reason would misdescribe the live gateway)."""
    from gateway import status as gw_status
    from nunmai_cli.gateway_multiplex_mode import MultiplexDecision, record_multiplex_decision
