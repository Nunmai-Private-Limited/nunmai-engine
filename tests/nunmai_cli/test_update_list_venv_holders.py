"""``nunmai update --list-venv-holders``: read-only JSON twin of the Windows venv-holder refusal.
...
from nunmai_cli import update_cmd_windows
...
def test_list_venv_holders_json_and_exit_3_when_holders_present(monkeypatch, capsys, _quiet_preflight):
    holders = [
        (4242, "python.exe", r"C:\nunmai\venv\Scripts\python.exe -m nunmai_cli.main serve --port 8642"),
        (4343, "python.exe", r"C:\nunmai\venv\Scripts\python.exe -m nunmai_cli.main gateway run"),
        (4444, "python.exe", r"C:\nunmai\venv\Scripts\python.exe -m nunmai_cli.main -p work kanban list"),
        (4545, "python.exe", r"C:\nunmai\venv\Scripts\python.exe some_script.py"),
    ]
    ...
    assert [(h["pid"], h["kind"]) for h in payload] == [
        (4242, "backend"), (4343, "gateway"), (4444, "nunmai:kanban"), (4545, "python")]
...
def test_list_venv_holders_empty_list_exits_zero_without_updating(monkeypatch, capsys, _quiet_preflight):
    ...
    monkeypatch.setattr("nunmai_cli.update_cmd._cmd_update_impl", _boom)
