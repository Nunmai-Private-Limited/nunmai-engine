...
from nunmai_cli import nous_auth_keepalive as keepalive
...
def test_keepalive_binds_launch_scope_for_multiplexed_singleton_refresh(tmp_path, monkeypatch, caplog):
    ...
    from nunmai_cli.auth_nous import _nous_inference_env_override, _nous_portal_env_override

    launch_home = tmp_path / ".nunmai"
    launch_home.mkdir()
    (launch_home / ".env").write_text(
        "NOUS_INFERENCE_BASE_URL=https://inference.example/v1\n"
        "NUNMAI_PORTAL_BASE_URL=https://portal.example\n",
        encoding="utf-8",
    )
