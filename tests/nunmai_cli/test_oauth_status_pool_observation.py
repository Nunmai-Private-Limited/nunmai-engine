...
(``/model`` picker, ``nunmai doctor``, dashboard cards). When that read ran ``pool.select()`` it
...
from nunmai_cli.auth import AuthError, DEFAULT_CODEX_BASE_URL, get_codex_auth_status
...
def _pool_only_codex_home(tmp_path, monkeypatch, *, access_tokens: list):
    """NUNMAI_HOME whose only Codex credentials live in ``credential_pool.openai-codex``; the token
    endpoint is a transient failure (the credential itself is still good)."""
    import nunmai_cli.auth as auth
    import nunmai_cli.codex_models as codex_models
...
def _singleton_only_codex_home(tmp_path, monkeypatch, *, tokens: dict, codex_cli_tokens: dict):
    """NUNMAI_HOME whose Codex credentials are the ``providers.openai-codex`` singleton only, with a
    valid Codex CLI login sitting beside it in ``CODEX_HOME``."""
    home, codex_home = tmp_path / "nunmai", tmp_path / "codex"
    home.mkdir()
    codex_home.mkdir()
...
def test_status_snapshot_never_adopts_codex_cli_tokens(tmp_path, monkeypatch):
    """#68004: a Nunmai store missing its refresh_token is recovery-eligible on the runtime path, but
    ``nunmai status`` / ``nunmai doctor`` must not import the Codex CLI's single-use token family."""
    from nunmai_cli.auth import resolve_codex_runtime_credentials
    ...
    assert resolve_codex_runtime_credentials()["source"] == "nunmai-auth-store"
...
def test_status_snapshot_never_refreshes_an_expired_singleton(tmp_path, monkeypatch):
    ...
    assert status["source"] == "nunmai-auth-store", "the status read never reached the singleton resolver"
...
def test_status_snapshot_leaves_the_auth_store_manifest_byte_identical(tmp_path, monkeypatch):
    ...
    assert status["source"] == "nunmai-auth-store"
...
def test_model_picker_catalog_never_refreshes_the_stored_codex_login(tmp_path, monkeypatch):
    ...
    import nunmai_cli.auth as auth
    import nunmai_cli.codex_models as codex_models
    from nunmai_cli.models import _codex_catalog
