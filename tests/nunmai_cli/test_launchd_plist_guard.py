...
whole ``nunmai update`` post-pull cleanup instead of being skipped.
"""
import os
...
MALFORMED_PLIST = (
    ...
    "    <string>/usr/local/bin/nunmai</string>\n    <string>&&</string>\n"
    "  </array>\n</dict>\n</plist>\n"
)

GOOD_PLIST = (
    '<?xml version="1.0" encoding="UTF-8"?>\n<plist version="1.0">\n<dict>\n'
    "  <key>Label</key>\n  <string>ai.nunmai.dashboard.test</string>\n"
    "  <key>ProgramArguments</key>\n  <array>\n"
...
def test_malformed_plist_is_skipped_not_fatal(tmp_path):
    ...
    with mock.patch(
        "nunmai_cli.gateway._launchd_print_service_pid", return_value=(False, None)
    ) as probe:
...
def test_malformed_sibling_does_not_hide_the_good_job(tmp_path):
    ...
    (tmp_path / "ai.nunmai.dashboard.test.plist").write_text(GOOD_PLIST, encoding="utf-8")
    with mock.patch(
        "nunmai_cli.gateway._launchd_print_service_pid", return_value=(True, 4321)
    ) as probe:
        jobs = main_dashboard._loaded_launchd_backend_jobs([("agent", tmp_path)])
    assert jobs == [
        (
            f"gui/{os.getuid()}",  # windows-footgun: ok — macos_only file
            "ai.nunmai.dashboard.test",
            ["/usr/local/bin/nunmai", "dashboard", "--port", "9119"],
            4321,
        )
    ]
    # Only the well-formed label was probed against launchd.
    assert [c.args[1] for c in probe.call_args_list] == ["ai.nunmai.dashboard.test"]
