"""A CONVERGED host is a no-op for ``nunmai gateway migrate --multiplex``, run after run.
...
def converged_host(tmp_path, monkeypatch):
    ...
    assert str(nunmai_constants.get_default_nunmai_root()).startswith(str(tmp_path))
    ...
    (root / "gateway.pid").write_text(json.dumps({"pid": pid, "nunmai_home": str(root)}))
    (root / "gateway_state.json").write_text(json.dumps(
        {"pid": pid, "nunmai_home": str(root), "gateway_state": "running", "served_profiles": served}))
    for name in ("coder", "ops"):
        home = root / "profiles" / name
        (home / "gateway_state.json").write_text(json.dumps(
            {"pid": pid, "nunmai_home": str(home), "gateway_state": "running"}))

    import gateway.status as status
    from gateway import host_attach, host_rendezvous as hr
    monkeypatch.setattr(status, "_read_process_cmdline", lambda p: "nunmai gateway run")
    ...
    monkeypatch.setattr("gateway.control_socket.identify_gateway",
                        lambda home: {"pid": pid, "nunmai_home": str(root), "served_profiles": served})
