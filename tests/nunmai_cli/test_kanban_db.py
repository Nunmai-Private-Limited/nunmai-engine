...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nunmai_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import nunmai_state as _nunmai_state
    monkeypatch.setattr(
        _nunmai_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _nunmai_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    nunmai_state._wal_fallback_warned_paths.clear()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        nunmai_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from nunmai_cli.kanban_db import connect
