...
def test_resolve_nunmai_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m nunmai_cli.main`.

    Pins the correct module name (NOT `nunmai` — there is no top-level
    `nunmai` package). Regression for #23198: the original PR shipped
    `python -m nunmai` which fails with `No module named nunmai` on every
    invocation.
    """
    import shutil
    import sys
    import nunmai_cli.kanban_db as kb
    from nunmai_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("NUNMAI_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_nunmai_argv()
    assert argv == [sys.executable, "-m", "nunmai_cli.main"]
...
def test_resolve_nunmai_argv_module_actually_runs():
    ...
    assert "Nunmai Engine" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="nunmai_cli.kanban_db"):
        with kbc.connect() as conn:
