"""Tests for the Kanban DB layer (nunmai_cli.kanban_db)."""
...
from nunmai_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated NUNMAI_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import nunmai_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import nunmai_cli.kanban_db as _kb
    from nunmai_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("NUNMAI_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_blocker_auth_curated_not_open_stem(
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
...
def test_infrastructure_spawn_refusal_never_charges_the_card(
    ...
    def spawn_via_real_boundary(task, workspace, board=None):
        kbd._restart_safe_worker_argv(task, ["nunmai", "chat"])  # raises: real probe verdict, real _degrade()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Nunmai' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Nunmai' DB/metadata/logs on task completion.
    """
...
# spawned with `nunmai -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's NUNMAI_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's NUNMAI_HOME."""

    def _set_home(self, monkeypatch, tmp_path, nunmai_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        monkeypatch.delenv("NUNMAI_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.nunmai/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.nunmai/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.nunmai/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # NUNMAI_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile NUNMAI_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # NUNMAI_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is NUNMAI_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["NUNMAI_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["NUNMAI_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["NUNMAI_KANBAN_TASK"] == "t_dispatch_env"
        assert env["NUNMAI_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "NUNMAI_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nunmai_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import nunmai_state_wal as _nunmai_state_wal
    monkeypatch.setattr(
        _nunmai_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _nunmai_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    nunmai_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("nunmai_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="nunmai_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "nunmai_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'nunmai kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_nunmai_argv()
#
# Workers spawned by the dispatcher must use a `nunmai` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["nunmai", ...]`
...
# only falls back to the PATH shim when ``nunmai_cli`` is not importable.
# ---------------------------------------------------------------------------


def test_resolve_nunmai_argv_prefers_module_form_over_path_shim(monkeypatch):
    """A `nunmai` on PATH must not shadow the running install (#111569):
    the module argv wins whenever ``nunmai_cli`` is importable; only an
    explicit ``$NUNMAI_BIN`` overrides it."""
    import shutil
    import sys
    from nunmai_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("NUNMAI_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/tmp/planted/nunmai")
    monkeypatch.setattr(kbd, "_safe_which_no_cwd", lambda name: "/tmp/planted/nunmai")
    assert kbd._resolve_nunmai_argv() == [sys.executable, "-m", "nunmai_cli.main"]

    monkeypatch.setenv("NUNMAI_BIN", "/opt/nunmai/bin/nunmai")
    assert kbd._resolve_nunmai_argv() == ["/opt/nunmai/bin/nunmai"]




def test_resolve_nunmai_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `nunmai_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m nunmai_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("NUNMAI_BIN", None)
        with mock.patch.object(shutil, "which", return_value=None):
            argv = kbd._resolve_nunmai_argv()
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from nunmai_cli.kanban_db_connect import connect
