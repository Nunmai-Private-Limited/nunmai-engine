...
def test_exhausted_pool_provider_is_not_authenticated(monkeypatch):
    ...
    from nunmai_cli.model_switch import get_authenticated_provider_slugs
...
def test_opaque_legacy_pool_value_stays_visible(monkeypatch):
    ...
    from nunmai_cli.model_switch_providers import _credential_pool_is_usable
...
def test_aux_task_picker_requests_exhausted_pool_visibility(monkeypatch):
    """The ``nunmai model`` auxiliary-task picker (``_aux_select_for_task``)
