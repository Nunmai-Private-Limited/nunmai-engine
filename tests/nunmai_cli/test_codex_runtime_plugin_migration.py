...
from nunmai_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_nunmai_tools_mcp_entry,
...
# ---- end-to-end migrate(, expose_nunmai_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_nunmai_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(nunmai_cfg, codex_home=tmp_path, expose_nunmai_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Nunmai' managed block, re-running
        ...
        migrate({"mcp_servers": {"nunmai-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.nunmai-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    ``nunmai codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_nunmai_tools=False)
...
# ---- Bug C: NUNMAI_HOME tempdir leak into ~/.codex/config.toml ----


class TestNunmaiHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_nunmai_tools_mcp_entry()`` read ``NUNMAI_HOME``
    ...
    once codex spawned the nunmai-tools MCP subprocess.
    """




    def test_real_nunmai_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate NUNMAI_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.nunmai"
        monkeypatch.setenv("NUNMAI_HOME", real_path)
        entry = _build_nunmai_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("NUNMAI_HOME") == real_path

    def test_unset_nunmai_home_omits_env_key(self, monkeypatch):
        """When NUNMAI_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever NUNMAI_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_nunmai_tools_mcp_entry()
        env = entry.get("env", {})
        assert "NUNMAI_HOME" not in env, (
            f"NUNMAI_HOME should not be set when env var is unset, got: "
            f"{env.get('NUNMAI_HOME')!r}"
        )
...
class TestSameNameUserMcpTable:
    """Issue #79023: a Nunmai server whose name the user already declares outside the managed
    ...
    def test_user_table_wins_and_output_stays_valid_toml(self, tmp_path):
        ...
        report = migrate(
            ...
            codex_home=tmp_path, discover_plugins=False, expose_nunmai_tools=False,
            default_permission_profile=None)
    ...
    def test_cli_migrate_dry_run_json_reports_without_writing(self, tmp_path, monkeypatch, capsys):
        """`nunmai codex-runtime migrate --dry-run --json` is the supported automation seam:
        drive it through the real ``nunmai`` argparse tree so the subcommand registration in
        nunmai_cli/main.py stays pinned, and honour ``CODEX_HOME`` like every codex sibling."""
        import json

        import nunmai_cli.main as main
