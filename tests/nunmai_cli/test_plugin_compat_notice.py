"""nunmai_cli.plugin_compat: detect plugins on old import paths, tell the user, disable after the date.

Kept with the compat layer (tests/nunmai_cli/test_compat_manifest_targets.py); deleted with it.
"""
...
from nunmai_cli import plugin_compat as pc
...
MANIFEST = {"tools.web_tools": {"prefers_gateway": "tools.tool_backend_helpers.prefers_gateway"},
            "nunmai_cli.kanban_db": {"connect": "nunmai_cli.kanban_db_connect.connect"}}
...
@pytest.mark.parametrize("src, expect", [
    ...
    ("from unittest.mock import patch\npatch('nunmai_cli.kanban_db.connect')\n", ["nunmai_cli.kanban_db.connect"]),
...
def test_scan_plugin_walks_dir_and_skips_tests(tmp_path):
    ...
    (tmp_path / "sub").mkdir(); (tmp_path / "sub" / "m.py").write_text("import nunmai_cli.kanban_db as k\nk.connect()\n")
...
def test_disable_only_after_the_date_and_not_when_allowed(tmp_path, monkeypatch):
    ...
    assert reason and pc.COMPAT_REMOVAL in reason and "nunmai plugins compat" in reason
