...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 nunmai-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("nunmai-engine-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from nunmai_cli.web_server import should_require_auth
...
def test_start_server_insecure_public_no_longer_bypasses_gate(monkeypatch):
    ...
    """
    from nunmai_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_trusted_container_proxy_controls_https_detection():
    ...
    from nunmai_cli.dashboard_auth.cookies import detect_https
...
def test_desktop_ssh_backend_serves_session_token_requests_despite_public_url(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_DASHBOARD_PUBLIC_URL", "https://dashboard.example.test:9443")
    ...
    try:
        ...
        with_token = client.get("/api/profiles", headers={"X-Nunmai-Session-Token": ssh_token})
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("NUNMAI_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("NUNMAI_DASHBOARD_PUBLIC_URL", public_url)
