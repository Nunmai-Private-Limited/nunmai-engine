...
import nunmai_cli.update_cmd_fleet as fleet_mod
...
def _fleet_homes(monkeypatch, tmp_path, records: dict[str, dict]) -> None:
    ...
    monkeypatch.setattr("nunmai_cli.build_info.get_code_identity", lambda refresh=False: {"sha": HEAD, "version": "1.0"})
    monkeypatch.setattr("nunmai_cli.profiles._get_default_nunmai_home", lambda: root)
    monkeypatch.setattr("nunmai_cli.profiles._get_profiles_root", lambda: root / "profiles")
...
def test_restart_phase_records_accepted_self_restart_and_verify_exits_clean(monkeypatch, tmp_path):
    ...
    restart = fleet_mod._GatewayRestartOutcome(
        incomplete=False, phase_errors=[], pre_restart_gateway_pids=[ancestor], restarted_services=["nunmai-gateway"],
