...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "nunmai_version"):
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_nunmai_home
):
    ...
    startup under NUNMAI_DESKTOP=1 so the stale orphan is cleared first.
    """
    import nunmai_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from nunmai_cli.gateway, so patch it
    # on that module.
    import nunmai_cli.gateway as g
