"""Redaction-safe forensic logging at the Nunmai OAuth quarantine path.

A NAS-hosted Fly agent's Nunmai bootstrap session can take a terminal
...
from nunmai_cli.auth import AuthError, _quarantine_nous_oauth_state
...
def test_raw_refresh_token_never_logged(caplog):
    ...
    state = _make_state()
    with caplog.at_level(logging.DEBUG, logger="nunmai_cli.auth"):
        _quarantine_nous_oauth_state(state, _error(), reason="unit_test_quarantine")
