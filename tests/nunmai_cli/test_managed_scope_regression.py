...
def nunmai_home(tmp_path, monkeypatch):
    ...
    # /etc/nunmai on the dev/CI box can't influence the test.
    monkeypatch.setenv("NUNMAI_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(nunmai_home, monkeypatch):
...
def test_env_expansion_in_user_config(nunmai_home, monkeypatch):
    from nunmai_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_nunmai_dotenv(nunmai_home=str(home))
