"""A memory provider installed under ``$NUNMAI_HOME/plugins/`` (catalog install) keeps the
...
def resolve_config_path():
    return Path(os.environ["NUNMAI_HOME"]) / "honcho.json"
...
@pytest.fixture
def user_dir_honcho(monkeypatch, tmp_path, _isolate_nunmai_home):
    ...
    plugin_dir = get_nunmai_home() / "plugins" / "honcho"
    ...
    (get_nunmai_home() / "honcho.json").write_text(
        json.dumps({"hosts": {"nunmai": {"workspace": "from-user-dir"}}}), encoding="utf-8"
    )
