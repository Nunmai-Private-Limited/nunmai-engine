...
``nunmai gateway status`` / ``gateway list`` / ``profile list`` keyed liveness
...
def _fake_multiplexer(monkeypatch, tmp_path, *, multiplex: bool, pid_file: bool = True):
    ...
    (tmp_path / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "kind": "nunmai-gateway", "gateway_state": "running",
        "start_time": status._get_process_start_time(os.getpid()), "nunmai_home": str(tmp_path),
    }))
    ...
    monkeypatch.setattr(
        status, "_read_process_cmdline", lambda pid: "python -m nunmai_cli.main gateway run --replace"
    )
...
def test_standalone_profile_status_reports_standalone_by_config(monkeypatch, tmp_path):
    """`nunmai -p X gateway status` on a standalone X says so and never claims the multiplexer."""
    _fake_multiplexer(monkeypatch, tmp_path, multiplex=True)
