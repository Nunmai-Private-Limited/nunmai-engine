...
Relay runs its finalizer as soon as the provider stream ends — concurrently with Nunmai'
...
def _stream_through_relay(tmp_path, monkeypatch, response_body: bytes, *, finalize_before):
    ...
    Returns ``(nunmai_result, relay_llm_end_event)``."""
...
def test_openai_stream_final_tool_call_delta_reaches_relay_parent_event(tmp_path, monkeypatch):
    ...
    nunmai_call = result.choices[0].message.tool_calls[0]
    assert (nunmai_call.function.name, nunmai_call.function.arguments) == ("read_file", '{"path": "/tmp/x"}')
