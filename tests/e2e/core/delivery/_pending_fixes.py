...
``HOME``/``NUNMAI_HOME`` (no state leaks into the suite process). ``expect_gap`` applies a
...
jobs._nunmai_now = lambda: datetime.fromisoformat("2026-03-07T09:00:30-05:00")
...
def gap_open(pr: int) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix=f"gap-{pr}-") as tmp:
        ...
        env = {k: v for k, v in os.environ.items()
               if not k.startswith(("PYTEST_", "NUNMAI_")) and not k.endswith("_API_KEY")}
        env.update({"HOME": str(home), "NUNMAI_HOME": str(home / ".nunmai"),
