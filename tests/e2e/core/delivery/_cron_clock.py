...
* ``VirtualClock`` is ONE epoch value in a file shared by every process of a scenario. It is
  installed at the single wall-clock seam — ``nunmai_time.datetime`` (every ``_nunmai_now``
  binding in cron/jobs.py, scheduler.py, executions.py, occurrences via jobs, ... resolves
  ``nunmai_time.now()`` -> ``datetime.now(tz)`` at call time) — plus the ``time.time()`` reads of
...
files, so two processes can share one ``NUNMAI_HOME`` and one can be SIGKILLed mid-run. Two
...
def install(clock: VirtualClock, control: Control, setattr_fn=setattr) -> None:
    ...
    setattr_fn(nunmai_time, "datetime", _virtual_datetime_class(clock))
    ...
    import nunmai_cli.sqlite_util as sqlite_util
...
class ChildHost:
    """Same real ticker loop in a separate OS process sharing the scenario's NUNMAI_HOME."""
