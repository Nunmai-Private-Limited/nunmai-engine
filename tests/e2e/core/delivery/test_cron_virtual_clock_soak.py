...
executions ledger / delivery routing / ``mark_job_run``, a second OS process sharing NUNMAI_HOME
...
over the job's configured zone (nunmai ``timezone`` if set, else the process zone — "server
...
class Soak:
    def __init__(self, sc: Scenario, home: Path, control: H.Control, env: Dict[str, str]):
        ...
        self.zone = ZoneInfo(sc.nunmai_tz or sc.process_tz)
...
def soak_env(tmp_path, monkeypatch):
    for key in list(os.environ):
        if key.endswith("_API_KEY") or key in ("INVOCATION_ID", "NUNMAI_TIMEZONE"):
            monkeypatch.delenv(key, raising=False)
    home_root = tmp_path / "home"
    nunmai_home = home_root / ".nunmai"
    nunmai_home.mkdir(parents=True)
    monkeypatch.setenv("HOME", str(home_root))
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    original_tz = os.environ.get("TZ")
    yield tmp_path, nunmai_home, monkeypatch
...
def _run_scenario(sc: Scenario, soak_env) -> Dict[str, int]:
    ...
    soak = Soak(sc, nunmai_home, control, env)
    try:
        soak.run()
    finally:
...
def test_two_replicas_contend_for_every_fire(soak_env):
    ...
    try:
        ...
        peer.stop()
    rows = H.ledger_rows(nunmai_home)
