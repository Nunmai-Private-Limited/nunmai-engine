...
``kill -9`` + restart on the same NUNMAI_HOME possible.
"""
...
# Per-platform send/edit semantics (what the real service enforces, not what Nunmai believes).
...
class GatewayProcess:
    ...
    def __init__(self, root: Path, *, platforms: Dict[str, str], llm_base_url: str,
        ...
        from tests.fakes.fake_llm_provider import write_nunmai_home
        ...
        write_nunmai_home(self.nunmai_home, llm_base_url, extra_config=extra_config)
    ...
    def db_path(self) -> Path:
        return self.nunmai_home / "state.db"
    ...
    def env(self) -> Dict[str, str]:
        ...
        env.update({
            "HOME": str(self.home), "NUNMAI_HOME": str(self.nunmai_home),
            # The child's HOME is itself a throwaway tmp dir, so ~/.nunmai IS the temp home here; the
            # live-system guard (armed by the inherited isolation marker) would refuse it.
            "NUNMAI_STATE_DB_GUARD_BYPASS": "1",
            "NUNMAI_GATEWAY_LOCK_DIR": str(self.root / "gateway-locks"),
            "TZ": "UTC", "PYTHONHASHSEED": "0", "PYTHONUNBUFFERED": "1", "C12_PARENT_PID": str(os.getpid()),
            "NUNMAI_DISABLE_LAZY_INSTALLS": "1", "TIRITH_ENABLED": "false",
            "AWS_EC2_METADATA_DISABLED": "true", "NUNMAI_HONCHO_HOST": "nunmai",
