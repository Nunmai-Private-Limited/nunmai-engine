...
Real chain: a real ``nunmai chat --cli`` / ``nunmai --tui`` process (the TUI spawns its real Node
...
from tests.e2e.core.terminal._pty import REPO_ROOT, PtyNunmai, canon
...
def test_terminal_transcript_integrity(surface: str, scenario: str, tmp_path: Path) -> None:
    if surface == "tui" and not _tui_available():
        if os.environ.get("NUNMAI_E2E_REQUIRE_TUI") == "1":
            pytest.fail("ui-tui/dist/entry.js or node missing but NUNMAI_E2E_REQUIRE_TUI=1")
    ...
    with FakeLLMServer(script, aux=lambda _req: Text(TITLE)) as llm:
        term = PtyNunmai(tmp_path, SURFACES[surface], llm, rows=rows, cols=cols)
        try:
            term.wait_ready()
            expected_main = 0
