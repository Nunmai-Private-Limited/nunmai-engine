...
Harness: one real ``nunmai serve`` (the Desktop's backend argv) per module, 3 Desktop-shaped
...
GRACE_S = 2  # NUNMAI_TUI_WS_ORPHAN_REAP_GRACE_S: short so owner-death paths finish inside the test
STEP_TIMEOUT = 90.0
...
def backend(tmp_path_factory):
    ...
    be = Backend(root, _responder, extra_config="display:\n  busy_input_mode: queue\n",
                 env={"NUNMAI_TUI_WS_ORPHAN_REAP_GRACE_S": str(GRACE_S)})
    be.start()
    try:
        yield be
    finally:
