"""Drive a real ``nunmai`` CLI / Ink TUI process under a real PTY, rendered through :mod:`._vt`.

The child runs with an isolated HOME/NUNMAI_HOME (only the fake LLM provider configured, no
...
class PtyNunmai:
    """One interactive ``nunmai`` process on a PTY with an emulated screen."""
    ...
    def __init__(self, root: Path, argv_tail: list[str], llm: FakeLLMServer, *, rows: int, cols: int,
        ...
        operator = (_operator_home() / ".nunmai").resolve()
        assert operator not in (self.nunmai_home.resolve(), *self.nunmai_home.resolve().parents), (
            f"sandbox {self.nunmai_home} sits inside the operator's Nunmai home")
        write_nunmai_home(self.nunmai_home, llm.base_url, extra_config=extra_config)
        self.llm = llm
        ...
        env.update(
            HOME=str(self.home), NUNMAI_HOME=str(self.nunmai_home), PYTHONPATH=str(REPO_ROOT),
            TMPDIR=str(root / "tmp"), TERM="xterm-256color", COLORTERM="truecolor",
            PYTHONUNBUFFERED="1", NUNMAI_STATE_DB_GUARD_BYPASS="1",
        )
        ...
        self.proc = subprocess.Popen(
            [sys.executable, "-c", _SESSION_LEADER, sys.executable, "-m", "nunmai_cli.main", *argv_tail],
    ...
    def wait_for_text(self, needle: str, timeout: float = 60.0) -> None:
        ...
        assert self.proc.poll() is None, f"nunmai exited ({self.proc.returncode}) waiting for {needle[:40]!r}\n{self.dump()}"
    ...
    def wait_ready(self, timeout: float = 120.0) -> None:
        ...
        while True:
            ...
            assert self.proc.poll() is None, f"nunmai exited ({self.proc.returncode}) during startup\n{self.dump()}"
    ...
    def exit(self, command: str = "/exit", timeout: float = 60.0) -> int:
        ...
        if self.proc.poll() is None:
            raise AssertionError(f"nunmai did not exit within {timeout:.0f}s of {command}\n{self.dump()}")
    ...
    def wait_turns_persisted(self, n_replies: int, timeout: float = 90.0) -> None:
        ...
        assert self.proc.poll() is None, f"nunmai exited ({self.proc.returncode}) mid-turn\n{self.dump()}"
