"""Real ``nunmai serve`` backend + WebSocket JSON-RPC clients for the terminal lane E2E suites.

One backend per test module: a real ``python -m nunmai_cli.main serve --port 0`` subprocess (the
exact argv Desktop spawns, see ``apps/desktop/electron/backend-command.ts``) with an isolated
HOME/NUNMAI_HOME, no inherited provider credentials, and only the recording fake LLM provider
...
from tests.fakes.fake_llm_provider import FakeLLMServer, write_nunmai_home

REPO_ROOT = Path(__file__).resolve().parents[4]
READY_RE = re.compile(r"NUNMAI_BACKEND_READY port=(\d+)")
...
def _minimal_env(home: Path, nunmai_home: Path, token: str, extra: dict[str, str] | None) -> dict[str, str]:
    """A clean child env: nothing Nunmai-, provider- or credential-shaped leaks in from the test runner."""
    env = {k: os.environ[k] for k in ("PATH", "LANG", "LC_ALL", "TERM", "SYSTEMROOT") if k in os.environ}
    tmpdir = home.parent / "tmp"
    tmpdir.mkdir(parents=True, exist_ok=True)
    env.update(
        HOME=str(home), NUNMAI_HOME=str(nunmai_home), PYTHONPATH=str(REPO_ROOT),
        TMPDIR=str(tmpdir), NUNMAI_DASHBOARD_SESSION_TOKEN=token, PYTHONUNBUFFERED="1",
        NO_COLOR="1",
        # The child's HOME *is* the sandbox, so its "real" root (expanduser('~')/.nunmai) is the tmp
...
class Backend:
    """One real ``nunmai serve`` process wired to a :class:`FakeLLMServer`."""

    def __init__(self, root: Path, responder, *, extra_config: str = "", env: dict[str, str] | None = None,
                 aux=None) -> None:
        self.root = root
        self.home = root / "home"
        self.nunmai_home = self.home / ".nunmai"
    ...
    def start(self, *, timeout: float = 90.0) -> "Backend":
        operator_root = (Path(_operator_home()) / ".nunmai").resolve()
        assert operator_root not in (self.nunmai_home.resolve(), *self.nunmai_home.resolve().parents), (
            f"sandbox {self.nunmai_home} sits inside the operator's Nunmai home {operator_root}")
        self.llm.start()
        write_nunmai_home(self.nunmai_home, self.llm.base_url, extra_config=self.extra_config)
        ...
        self.proc = subprocess.Popen(
            [sys.executable, "-m", "nunmai_cli.main", "serve", "--host", "127.0.0.1", "--port", "0"],
            cwd=str(self.root), env=_minimal_env(self.home, self.nunmai_home, self.token, self.extra_env),
        ...
        def ready() -> int | None:
            if proc.poll() is not None:
                raise AssertionError(f"nunmai serve exited {proc.returncode} before ready:\n{self.logs()}")
            match = READY_RE.search((self.root / "serve.stdout.log").read_text(errors="replace"))
            return int(match.group(1)) if match else None

        try:
            self.port = poll_until(ready, timeout=timeout, interval=0.1, what="NUNMAI_BACKEND_READY")
    ...
    def logs(self, tail: int = 60) -> str:
        ...
        for path in (self.root / "serve.stdout.log", self.root / "serve.stderr.log",
                     self.nunmai_home / "logs" / "errors.log"):
            name = path.name
            if path.exists():
    ...
    def db_rows(self, sql: str, args: Iterable[Any] = ()) -> list[tuple]:
        path = self.nunmai_home / "state.db"
        if not path.exists():
            return []
