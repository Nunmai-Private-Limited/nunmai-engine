...
Harness: a real ``nunmai gateway run`` child process with ``gateway.multiplex_profiles: true`` serving
...
class MultiplexGateway:
    ...
    def _spawn(self) -> None:
        ...
        self.proc = subprocess.Popen(
            [sys.executable, "-m", "nunmai_cli.main", "gateway", "run"], cwd=str(self.home),
    ...
    def chat(self, prefix: str, api_key: str, text: str) -> tuple[int, Any]:
        req = urllib.request.Request(
            ...
            data=json.dumps({"model": "nunmai", "messages": [{"role": "user", "content": text}]}).encode(),
...
def test_multiplexed_gateway_never_crosses_tenants(fleet, request: pytest.FixtureRequest) -> None:
    ...
    created = H.run_nunmai(["profile", "create", "gamma"], home)
    assert created.returncode == 0, created.stderr[-2000:]
    attach = H.run_nunmai(["-p", "gamma", "gateway", "run"], home, timeout=150)
    ...
    deleted = H.run_nunmai(["profile", "delete", "gamma", "--yes"], home)
