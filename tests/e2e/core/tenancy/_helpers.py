...
Nunmai runs for real in child processes with HOME=<tmp>/home and NUNMAI_HOME=<tmp>/home/.nunmai
...
_STRIP_PREFIXES = ("NUNMAI_", "OPENAI", "ANTHROPIC", "OPENROUTER", "AWS_", "AZURE_", "GOOGLE_", "GEMINI",
...
def hermetic_env(home: Path, extra: dict[str, str] | None = None) -> dict[str, str]:
    """Child env: fake HOME (profile root anchor) + NUNMAI_HOME under it, nothing credential-shaped."""
    home = home.resolve()
    assert home != real_user_home() and home / ".nunmai" != real_user_home() / ".nunmai", home
    ...
    for var in ("HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy", "XDG_STATE_HOME",
                ...
                # live nunmai-gateway unit, whatever it decides about service management.
                "DBUS_SESSION_BUS_ADDRESS", "XDG_RUNTIME_DIR"):
        env.pop(var, None)
    env.update(
        HOME=str(home),
        NUNMAI_HOME=str(home / ".nunmai"),
        ...
        no_proxy="127.0.0.1,localhost",
        # The live-DB guard treats $HOME/.nunmai/state.db of a pytest descendant as production;
        # this HOME is the test's own tmp dir (asserted above).
        NUNMAI_STATE_DB_GUARD_BYPASS="1",
        NUNMAI_ACCEPT_HOOKS="1",
    )
    env.update(extra or {})
    return env
...
class Tenant:
    name: str
    home: Path  # the profile's NUNMAI_HOME
    workdir: Path
...
def make_tenants(root: Path, names: Iterable[str], launch: str = "default") -> dict[str, Tenant]:
    """Build tenant homes on disk: launch profile at HOME/.nunmai, the rest under profiles/<name>."""
    nunmai_home = root / "home" / ".nunmai"
    tenants: dict[str, Tenant] = {}
    for name in names:
        home = nunmai_home if name == launch else nunmai_home / "profiles" / name
...
def assert_profiles_root_under(root: Path, home: Path) -> None:
    ...
    probe = subprocess.run(
        [sys.executable, "-c", "from nunmai_cli.profiles import _get_profiles_root as r; print(r())"],
...
def seed_due_cron_job(t: Tenant, home: Path) -> str:
    ...
    r = subprocess.run([sys.executable, "-c", _SEED_CRON, t.cron_prompt], cwd=str(home),
                       env=hermetic_env(home, {"NUNMAI_HOME": str(t.home)}), capture_output=True, text=True,
...
def run_nunmai(argv: list[str], home: Path, timeout: float = 120.0,
               extra_env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    proc = subprocess.Popen(
        [sys.executable, "-m", "nunmai_cli.main", *argv], cwd=str(home), env=hermetic_env(home, extra_env),
...
class ServeBackend(TuiBackend):
    """The real Desktop backend: ``nunmai serve --port 0`` with the JSON-RPC surface on ``/api/ws``.

    ``NUNMAI_DESKTOP=1`` marks it app-spawned, which is also what runs the cron ticker in-process.
    """
    ...
    def __init__(self, home: Path, log_path: Path, extra_env: dict[str, str] | None = None) -> None:
        ...
        self._log = open(log_path, "a", encoding="utf-8")  # noqa: SIM115 - closed in close()
        env = {"NUNMAI_DASHBOARD_SESSION_TOKEN": self.token, "NUNMAI_DESKTOP": "1", **(extra_env or {})}
        self.proc = subprocess.Popen(
        ...
        self.port = poll(lambda: port_box[0] if port_box else (self.proc.poll() is not None and -1), 120,
                         "nunmai serve to report its port")
        assert self.port > 0, f"nunmai serve exited rc={self.proc.returncode}"
