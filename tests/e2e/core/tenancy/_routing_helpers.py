...
Nunmai itself runs for real in child processes (``nunmai chat -q`` / ``nunmai -z`` /
``python -m tui_gateway.entry``) with HOME and NUNMAI_HOME inside the test's tmp dir,
...
# Env that could route or authenticate a child Nunmai outside the fake fleet.
_STRIP_SUFFIXES = ("_API_KEY", "_TOKEN", "_BASE_URL", "_SECRET", "_ACCESS_KEY", "_KEY_ID")
_STRIP_PREFIXES = ("NUNMAI_", "OPENAI", "ANTHROPIC", "OPENROUTER", "AWS_", "AZURE_", "GOOGLE_",
                   "GEMINI", "PYTEST_", "NOUS_", "XAI_", "LLM_", "CUSTOM_")


def hermetic_env(home: Path, nunmai_home: Path | None = None, proxy: str | None = None) -> dict[str, str]:
    ...
    assert fake != real_home and real_home / ".nunmai" not in (fake, *fake.parents), fake
    env = {k: v for k, v in os.environ.items()
           if not (k.endswith(_STRIP_SUFFIXES) or k.startswith(_STRIP_PREFIXES))}
    env.update(
        HOME=str(home),
        NUNMAI_HOME=str(nunmai_home or home / ".nunmai"),
        PYTHONPATH=str(REPO_ROOT),
        NO_COLOR="1",
        TERM="dumb",
        # A pytest-descendant child treats $HOME/.nunmai/state.db as production; that HOME is ours.
...
def inference_hosts() -> frozenset[str]:
    """Hostnames of every inference API Nunmai itself knows (its provider registry + OpenRouter).
...
def write_home(nunmai_home: Path, config: dict[str, Any], env: dict[str, str],
               auth: dict[str, Any] | None = None) -> Path:
    nunmai_home.mkdir(parents=True, exist_ok=True)
    (nunmai_home / "config.yaml").write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    (nunmai_home / ".env").write_text("".join(f"{k}={v}\n" for k, v in env.items()), encoding="utf-8")
    if auth is not None:
        (nunmai_home / "auth.json").write_text(json.dumps(auth), encoding="utf-8")
    return nunmai_home
...
def run_nunmai(argv: list[str], home: Path, *, nunmai_home: Path | None = None, proxy: str | None = None,
               timeout: float = 240.0) -> RunResult:
    """Run the real ``nunmai`` entry point in its own process group; kill only that group."""
    start = time.monotonic()
    proc = subprocess.Popen(
        [sys.executable, "-m", "nunmai_cli.main", *argv],
        cwd=str(home), env=hermetic_env(home, nunmai_home, proxy), stdin=subprocess.DEVNULL,
