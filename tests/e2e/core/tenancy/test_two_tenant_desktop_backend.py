"""C7 two-tenant canary: ONE Desktop backend (``nunmai serve``) serving three profiles over /api/ws.
...
Harness: the real app backend, ``nunmai serve --port 0`` with ``NUNMAI_DESKTOP=1`` (which also runs
...
def test_desktop_backend_never_crosses_tenants(fleet, request: pytest.FixtureRequest) -> None:
    ...
    deleted = H.run_nunmai(["profile", "delete", "gamma", "--yes"], root / "home")
