...
selection, so a misroute always has somewhere to land and be caught. Nunmai runs for real in a
child process (``nunmai chat -q``, ``nunmai -z``, the stdio ``tui_gateway`` the TUI/Desktop
drive) with a tmp HOME/NUNMAI_HOME and every credential env var stripped.
...
class Leg:
    ...
    profile: str | None = None                 # run with NUNMAI_HOME = profiles/<name>
...
def _bare_custom(c: dict[str, Any], e: dict[str, str], _f: Fleet) -> None:
    ...
    # tests/nunmai_cli/test_runtime_provider_resolution.py); with neither, nothing may be reached
...
def _run_case(case: Case, root: Path) -> tuple[Fleet, list[LegOutcome]]:
    ...
    try:
        ...
        write_home(home / ".nunmai", config, env, auth)
        for name, (pconfig, penv) in case.profiles(fleet).items():
            write_home(home / ".nunmai" / "profiles" / name, pconfig, penv)
        outcomes = []
        for leg in case.legs:
            marks, egress_mark = fleet.marks(), len(trap.attempts)
            nunmai_home = home / ".nunmai" / "profiles" / leg.profile if leg.profile else None
            run = run_nunmai(leg.argv, home, nunmai_home=nunmai_home, proxy=trap.url, timeout=leg.timeout)
...
def test_cli_routing_truth_table(case: Case, cli_outcomes: dict[str, Any]) -> None:
    ...
    for i, out in enumerate(outcomes):
        ...
        ctx = (f"[{case.id} leg {i}: nunmai {' '.join(leg.argv)}] rc={run.rc} ({run.seconds:.1f}s)\n"
               f"requests:\n{describe(out.log)}\nstdout tail:\n{run.stdout[-1500:]}\nstderr tail:\n{run.stderr[-1500:]}")
...
def test_tui_gateway_model_switch_routing(tmp_path: Path, request: pytest.FixtureRequest) -> None:
    ...
    write_home(home / ".nunmai", config, env, pool_auth("pool-host", fleet.keys["pool"]))
