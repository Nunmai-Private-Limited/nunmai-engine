...
The chaos suites drive a REAL Nunmai surface (AIAgent child process, GatewayRunner,
...
def write_chaos_home(root: Path, base_url: str, **cfg: Any) -> tuple[Path, Path]:
    """Create ``root/home`` (fake $HOME) and ``root/home/.nunmai`` (NUNMAI_HOME)."""
    home = root / "home"
    nunmai_home = home / ".nunmai"
    nunmai_home.mkdir(parents=True, exist_ok=True)
    (nunmai_home / "config.yaml").write_text(chaos_config(base_url, **cfg), encoding="utf-8")
    (nunmai_home / ".env").write_text("OPENAI_API_KEY=sk-fake-chaos\n", encoding="utf-8")
    return home, nunmai_home


def hermetic_env(home: Path, nunmai_home: Path, tag: str) -> dict[str, str]:
    """Child env: fake HOME/NUNMAI_HOME, no real credentials, repo importable, and a
    tag every descendant inherits so the orphan scan can find it after reparenting."""
    env = {
        k: v for k, v in os.environ.items()
        if not (
            k.endswith(("_API_KEY", "_TOKEN", "_SECRET"))
            or k.startswith(("NUNMAI_", "OPENROUTER", "ANTHROPIC", "OPENAI", "NOUS_"))
