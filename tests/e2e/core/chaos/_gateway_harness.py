...
class GatewayProc:
    ...
    def __post_init__(self) -> None:
        self.home, self.nunmai_home = write_chaos_home(
            self.root, self.base_url, extra=gateway_extra_config() + self.extra_config, **self.cfg)
        plugin_dir = self.nunmai_home / "plugins" / fake_platform.PLUGIN_DIR_NAME
        ...
        self.state_db = self.nunmai_home / "state.db"
    ...
    def start(self) -> None:
        ...
        env = hermetic_env(self.home, self.nunmai_home, self.tag)
        ...
        env.update({
            ...
            "NUNMAI_GATEWAY_LOCK_DIR": str(self.root / "locks"),
