...
def run_scenario(scn: Scenario, srv: FakeLLMServer, root: Path) -> dict[str, Any]:
    ...
    home, nunmai_home = write_chaos_home(root, srv.base_url, **cfg)
    tag = new_tag()
    work = gateway_cwd(root)
    env = env_for_gateway(hermetic_env(home, nunmai_home, tag), work)
    ...
    try:
        ...
        if scn.action in EXIT_ACTIONS:
            return _exit_mid_turn(scn, gw, hb, tag, nunmai_home / "state.db", stored, stats)
