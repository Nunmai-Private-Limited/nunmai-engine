...
def make_scenario(provider, tmp_path, monkeypatch):
    """Build a Scenario with its own NUNMAI_HOME/state.db; closes every agent + DB at teardown."""
    server, dispatch = provider
    made: list[Scenario] = []

    def make(mode: str = "good", *, extra: str = "", platform: str = "cli", window: int | None = None,
             threshold_tokens: int = THRESHOLD_TOKENS, name: str = "nunmai_home") -> Scenario:
        # The hermetic root conftest already isolates NUNMAI_HOME per test; HOME stays put because the
        # state.db live-system guard treats $HOME/.nunmai as production.
        nunmai_home = tmp_path / name
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        import nunmai_state

        monkeypatch.setattr(nunmai_state, "DEFAULT_DB_PATH", nunmai_home / "state.db")
        slow = mode == "slow"
        if slow:
        ...
        write_home(nunmai_home, server.base_url,
                   compaction_config(extra=extra, slow=slow, threshold_tokens=threshold_tokens))
        sc = Scenario(server=server, dispatch=dispatch, home=nunmai_home, mode=mode, platform=platform, window=window)
        made.append(sc)
        return sc
