...
def _child_env(home: Path) -> dict:
    # Probe hygiene: tmp HOME/NUNMAI_HOME, no provider keys. The in-test db guard detects pytest by
    # process ancestry and would read this tmp $HOME/.nunmai as the production root; the documented
    # child-process escape hatch is safe because every path here lives under tmp_path.
    env = {k: v for k, v in os.environ.items() if not k.endswith("_API_KEY")}
    env.update(HOME=str(home), NUNMAI_HOME=str(home / ".nunmai"), PYTHONPATH=str(REPO), PYTHONUNBUFFERED="1",
...
def test_kill9_mid_compaction_leaves_state_consistent_and_resumable(make_scenario, provider, tmp_path, point):
    ...
    home = tmp_path / "home"
    sc = make_scenario("good", threshold_tokens=HIGH_THRESHOLD, name="home/.nunmai")
