...
* P2  "set one key" changes exactly one key, on every writer: in-process ``set_config_value`` over
      EVERY documented scalar leaf of ``DEFAULT_CONFIG``, the real ``nunmai config set`` CLI
...
* P4  Nunmai's ``.env`` loaders are idempotent (self-references, placeholders, quoting), the
...
def _value_for(default: Any, rng: random.Random, *, long: bool, env: dict[str, str], seed: int) -> Any:
    ...
    if long and roll < 0.35:
        ...
        return rng.choice([
            ...
            "x" * 150 + " " + "C:\\Users\\me\\.nunmai\\scripts\\" + "y" * 90,
        ])
    if roll < 0.08:
        return None
    if roll < 0.18:
...
def _write_file(path: Path, text: str) -> None:
    """Atomic replace (new inode), like every Nunmai writer, so no cache can serve stale bytes."""
...
def home(monkeypatch):
    """The per-test NUNMAI_HOME the repo conftest sandboxes; config caches reset around the test."""
    hh = Path(os.environ["NUNMAI_HOME"])
    hh.mkdir(parents=True, exist_ok=True)
    assert str(_cfg_path()).startswith(str(hh))
    _reset_config_caches()
    with contextlib.suppress(Exception):
        from tui_gateway import server
        monkeypatch.setattr(server, "_nunmai_home", hh)
    yield hh
    _reset_config_caches()
...
class ReadFaults:
    ...
    (``nunmai_cli.config.fast_safe_load`` — every config.yaml reader in nunmai_cli.config,
...
def _cli(env: dict, *args: str, timeout: float = 120) -> subprocess.CompletedProcess:
    return subprocess.run([sys.executable, "-m", "nunmai_cli.main", *args], cwd=str(WORKTREE), env=env,
...
def _cli_home(tmp_path: Path, case: Case) -> tuple[dict, Path]:
    ...
    cfg = Path(env["NUNMAI_HOME"]) / "config.yaml"
...
def test_p2_env_lock_refusal_is_not_reported_as_success(key, tmp_path):
    ...
    env = isolated_env(tmp_path / "cli", pythonpath=WORKTREE, extra={"NUNMAI_MANAGED_DIR": str(managed)})
    hh = Path(env["NUNMAI_HOME"])
    ...
    with known_failure(r"^`config set \w+` exit=0 but \.env unchanged|^a refused env write still rewrote config\.yaml",
                       "#119928 (fix PR #119929): when the managed-scope .env pins a key, `nunmai config set` "
...
class _RpcProc:
    def __init__(self, root: Path):
        ...
        self.home = Path(self.env["NUNMAI_HOME"])
...
def web_app():
    ...
    from nunmai_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
...
def env_restore(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_MULTIPLEX_PROFILES", "0")
    yield
    os.environ.clear()
...
def test_p4_load_nunmai_dotenv_is_idempotent(seed, home, env_restore, monkeypatch):
    ...
    snaps = []
    for _ in range(4):
        load_nunmai_dotenv(nunmai_home=home, load_external_secrets=False)
...
def _migration_start_versions() -> list[int | None]:
    from nunmai_cli.config_migrations import MIGRATIONS, SUPPORT_FLOOR_VERSION
