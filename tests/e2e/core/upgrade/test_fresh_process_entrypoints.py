...
The ``nunmai update`` failure class this guards (issue_classes.md C6): after the pull every
...
* **Stale graph** – the newest release whose updater still finished in the pre-pull
  interpreter (no ``nunmai_cli/update_handoff.py``) is extracted from git, its updater graph
...
* **Entrypoints** – ``python -m nunmai_cli.main`` and every ``[project.scripts]`` console
...
from tests.fakes.fake_llm_provider import FakeLLMServer, Text, write_nunmai_home
...
# its purge in the pre-pull interpreter: the restart phase (``nunmai_cli.gateway`` and what
# it pulls), the maintenance/summary steps, and the atexit browser cleanup that re-imports
# ``tools.browser_tool`` -> ``nunmai_cli.config`` (#112522). Entry points that only ever start
# in a NEW process (gateway.run, tui_gateway.entry, cron.scheduler) are covered by the
# fresh-interpreter smoke instead.
_POST_PURGE_IMPORTS = (
    "nunmai_cli.config", "nunmai_cli.managed_scope", "nunmai_cli.gateway", "gateway.status",
    "nunmai_cli.gateway_migrate", "nunmai_cli.profiles", "nunmai_cli.backup", "nunmai_cli.model_catalog",
    "nunmai_cli.plugin_compat", "agent.curator", "tools.skills_sync", "tools.browser_tool",
)
# What the pre-hand-off ``nunmai update`` process had imported before the pull.
_OLD_UPDATER_GRAPH = ("nunmai_cli.main", "nunmai_cli.update_cmd", "nunmai_cli.config", "nunmai_cli.gateway")

_READY_RE = re.compile(r"^NUNMAI_(?:BACKEND|DASHBOARD)_READY port=(\d+)", re.M)  # electron/backend-ready.ts
...
def _requirements():
    ...
    extras = {_canonical(Requirement(r).name) for reqs in PYPROJECT["project"]["optional-dependencies"].values()
              for r in reqs if not r.startswith("nunmai-engine")}
    return core, extras
...
def _root_py_modules() -> list[str]:
    ...
    spec = importlib.util.spec_from_file_location("_nunmai_setup_py_c6", WORKTREE / "setup.py")
...
def _shipped_modules() -> tuple[list[dict], set[str]]:
    ...
    are imported the way ``nunmai_cli.plugins_loader`` imports them.
    """
    tracked = _tracked()
    ...
    first_party = set(roots) | {p.split(".")[0] for p in pkgs} | {"nunmai_plugins"}
...
def _import(entry):
    if entry["kind"] == "name":
        ...
        return
    ns = types.ModuleType("nunmai_plugins")  # plugins_loader's synthetic namespace parent
    ns.__path__ = []
    ns.__package__ = "nunmai_plugins"
    sys.modules.setdefault("nunmai_plugins", ns)
    name = "nunmai_plugins." + entry["slug"]
...
for name in spec["old_graph"]:
    importlib.import_module(name)
main = sys.modules["nunmai_cli.main"]
...
for step in ("_reload_updated_runtime_modules", "_purge_stale_nunmai_modules"):
...
def test_every_shipped_module_imports_from_a_clean_first_party_graph(tmp_path):
    ...
    assert {"nunmai_cli.main", "run_agent", "gateway.run", "tui_gateway.entry", "acp_adapter.entry"} <= set(ids)
...
def _pre_handoff_tag() -> tuple[str, str] | None:
    ...
    for tag in cp.stdout.split()[:15] if cp.returncode == 0 else []:
        if _git("cat-file", "-e", f"{tag}:nunmai_cli/update_handoff.py").returncode == 0:
            continue
        grep = _git("grep", "-l", "def _purge_stale_nunmai_modules", tag, "--", "nunmai_cli")
...
def test_pre_handoff_updater_stale_graph_imports_post_update_modules(tmp_path):
    ...
    modules (``utils``, ``nunmai_constants``) cached, then imports new restart-phase code.
    Retire this leg together with ``nunmai_cli/stale_modules.py``."""
    _sandbox_or_skip()
...
class Entry:
    via: str                 # "module" (python -m nunmai_cli.main) or a [project.scripts] name
    args: tuple[str, ...]
    rcs: frozenset[int]
...
_ENTRIES = {
    ...
    "nunmai-version": Entry("nunmai", ("--version",), frozenset({0}), prints_version=True),
    "nunmai-help": Entry("nunmai", ("--help",), frozenset({0})),
    "nunmai-acp-version": Entry("nunmai-acp", ("--version",), frozenset({0}), prints_version=True),
    "nunmai-acp-help": Entry("nunmai-acp", ("--help",), frozenset({0})),
    "nunmai-engine-help": Entry("nunmai-engine", ("--help",), frozenset({0})),
}
...
def test_entrypoint_in_a_fresh_process(case, tmp_path):
    ...
    nunmai_home = Path(env["NUNMAI_HOME"])
    with FakeLLMServer([Text(reply)]) as srv:
        write_nunmai_home(nunmai_home, srv.base_url)
        config_before = (nunmai_home / "config.yaml").read_bytes()
        args = [*entry.args, canary] if entry.turn else list(entry.args)
        if entry.via == "module":
            argv = [PY, "-m", "nunmai_cli.main", *args]
        else:
    ...
    if case == "module-doctor":
        ...
        assert (nunmai_home / "config.yaml").read_bytes() == config_before, "doctor without --fix rewrote config.yaml"
    if not entry.turn:
...
def test_serve_announces_ready_and_stops_cleanly_on_sigterm(tmp_path):
    ...
    env = isolated_env(tmp_path, pythonpath=WORKTREE, extra={"NUNMAI_GATEWAY_LOCK_DIR": str(lock_dir)})
    ...
    with FakeLLMServer() as srv, open(stdout_path, "w", encoding="utf-8") as out, open(stderr_path, "w", encoding="utf-8") as err:
        write_nunmai_home(Path(env["NUNMAI_HOME"]), srv.base_url)
        argv = [PY, str(reaper), str(report_path),
                PY, "-m", "nunmai_cli.main", "serve", "--host", "127.0.0.1", "--port", "0"]
        ...
        try:
            ...
            port = wait_for(_ready, timeout=180, what="NUNMAI_BACKEND_READY on stdout")
            ...
            serve = next(p for p in reaper_proc.children() if p.cmdline()[1:3] == ["-m", "nunmai_cli.main"])
