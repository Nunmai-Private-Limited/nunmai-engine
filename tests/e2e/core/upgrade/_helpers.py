...
Every Nunmai process these suites spawn runs:

* with a HOME/NUNMAI_HOME under the test's tmp dir and an environment built from an
  allowlist (no inherited ``*_API_KEY`` / ``NUNMAI_*``), so only the fake provider is
  configured;
* inside a ``bwrap`` sandbox when bubblewrap is usable: its own PID namespace (the
  ...
  ``~/.nunmai`` bind-mounted read-only, and ``--die-with-parent`` so killing the
...
def isolated_env(
    ...
    *,
    ...
    """Allowlisted environment with HOME/NUNMAI_HOME under ``root``."""
    home = root / "home"
    ...
    env.update(
        ...
        DBUS_SESSION_BUS_ADDRESS="unix:path=/nonexistent/nunmai-test-bus",
        NO_COLOR="1",
        TERM="dumb",
        PYTHONUNBUFFERED="1",
        PYTHONHASHSEED="0",
        NUNMAI_DISABLE_LAZY_INSTALLS="1",
    ...
    uv = shutil.which("uv") or (str(REAL_HOME / ".nunmai" / "bin" / "uv") if (REAL_HOME / ".nunmai" / "bin" / "uv").exists() else None)
...
def sandbox_argv(argv: Sequence[str], *, writable: Iterable[Path]) -> list[str]:
    ...
    if not BWRAP_OK:
        return list(argv)
    cmd = ["bwrap", "--dev-bind", "/", "/"]
    real_nunmai = REAL_HOME / ".nunmai"
    if real_nunmai.is_dir():
        cmd += ["--ro-bind", str(real_nunmai), str(real_nunmai)]
    for w in writable:
        w = Path(w)
