...
class LiveCase:
    id: str
    provider: str  # nunmai provider id passed to resolve_runtime_provider
    key_env: str
...
# Override with NUNMAI_LIVE_MODEL_<ID> (upper-cased, '-' -> '_').
...
def model_override(case: LiveCase) -> str | None:
    return os.environ.get("NUNMAI_LIVE_MODEL_" + case.id.upper().replace("-", "_")) or None
...
@dataclass
class WireLog:
    """Every HTTP exchange made through httpx (all SDKs Nunmai uses sit on it)."""
...
def write_live_home(nunmai_home: Path, provider: str, model: str) -> None:
...
def usage_line(case: LiveCase, model: str, agent: Any, wire: WireLog) -> dict:
    ...
    return {
        ...
        "nunmai_est_usd": round(float(agent.session_estimated_cost_usd or 0.0), 6),
        "nunmai_cost_status": str(getattr(agent, "session_cost_status", "")),
...
def emit_usage(line: dict) -> None:
    ...
    target = os.environ.get("NUNMAI_LIVE_USAGE_FILE")
    if target:
