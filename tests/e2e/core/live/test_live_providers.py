...
(``LIVE-USAGE {...}``) and appended to ``$NUNMAI_LIVE_USAGE_FILE`` when set.
"""
...
def live_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    # $HOME/.nunmai as production), so nothing can resolve the developer's real home.
    home = tmp_path / "home"
...
def _live_listing(provider: str, runtime: dict) -> list[str]:
    ...
    if provider == "nous":
        from nunmai_cli.auth import fetch_nous_models
