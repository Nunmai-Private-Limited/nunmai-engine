"""Entrypoint driver for the parity matrix: the ACP stdio server (``nunmai acp``).
...
from tests.e2e.core.parity._helpers import TURN_TIMEOUT, DriveResult, ParityHome, nunmai_argv, terminate
from tests.fakes.fake_llm_provider import FakeLLMServer

ACP_TOOLSET = "nunmai-acp"  # acp_adapter/session.py::_expand_acp_enabled_toolsets default
...
def drive_acp(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    with open(stderr_path, "w", encoding="utf-8") as stderr_fh:
        proc = subprocess.Popen(
            nunmai_argv("acp"), cwd=ph.project, env=ph.env(), stdin=subprocess.PIPE,
