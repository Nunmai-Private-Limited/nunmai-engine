...
The Desktop spawns ``nunmai serve`` with stdout piped and learns the port ONLY from
...
# HOME/NUNMAI_HOME (asserted in ParityHome.env), so no real state is reachable.
pytestmark = [
...
def _check_serve_ready(home) -> None:
    ...
    try:
        ...
        req = urllib.request.Request(f"http://127.0.0.1:{port}/api/status",
                                     headers={"X-Nunmai-Session-Token": sp.token})
