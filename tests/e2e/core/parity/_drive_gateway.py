...
Both stop through the normal operator path of ``nunmai gateway stop``: write the
...
def _stop(ph: ParityHome, proc: subprocess.Popen) -> bool:
    """``nunmai gateway stop`` semantics (planned-stop marker for the PID, then SIGTERM),
...
def drive_gateway(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    return DriveResult(
        final_text=_collect_delivered(events), toolset="nunmai-telegram", cwd_channel="terminal.cwd",
...
def drive_api_server(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    env_before = (ph.nunmai_home / ".env").read_text(encoding="utf-8") if (ph.nunmai_home / ".env").exists() else ""
    for _attempt in range(PORT_ATTEMPTS):
        port = _free_loopback_port()
        (ph.nunmai_home / ".env").write_text(env_before, encoding="utf-8")
    ...
    try:
        status, raw = _http(
            ...
            body={"model": "nunmai-engine", "messages": [{"role": "user", "content": prompt}],
                  "stream": False},
        )
    ...
    return DriveResult(
        final_text=text, toolset="nunmai-api-server", cwd_channel="terminal.cwd", graceful_exit=graceful,
