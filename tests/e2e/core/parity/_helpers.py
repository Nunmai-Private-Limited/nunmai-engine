"""Shared fixture NUNMAI_HOME + invariant checks for the entrypoint-parity suite.
...
from tests.fakes.fake_llm_provider import FakeLLMServer, Text, ToolCall, write_nunmai_home
...
class ParityHome:
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        """Hermetic env for a subprocess Nunmai: fake HOME, no real credentials."""
        ...
        # A tmp_path under ``~/.nunmai/cache/scratch`` (TMPDIR when Nunmai itself runs the suite)
        # is fine: the child's HOME is the fixture home, so its ``~/.nunmai`` never resolves there.
        real_root = Path(pwd.getpwuid(os.getuid()).pw_dir, ".nunmai").resolve()
        fixture = self.nunmai_home.resolve()
        assert fixture != real_root and fixture.parent != real_root / "profiles", (
            f"fixture NUNMAI_HOME {self.nunmai_home} is the real install's live home")
        assert fixture == (self.home / ".nunmai").resolve(), (
            f"fixture NUNMAI_HOME {self.nunmai_home} is not <fixture HOME>/.nunmai")
        # Allowlist, not denylist: the runner may itself be a Nunmai process whose
        # TERMINAL_CWD / NUNMAI_* / credential env would silently reroute the child.
        env = {
        ...
        env.update({
            "HOME": str(self.home),
            "NUNMAI_HOME": str(self.nunmai_home),
            ...
            # The child's HOME is the fixture home, so its ``~/.nunmai/state.db`` IS
            # the tmp NUNMAI_HOME's db; under a pytest ancestor the live-DB guard
            # (nunmai_state_guard) would refuse it. This is the guard's documented
            # child-process escape hatch; the path is tmp_path by construction.
            "NUNMAI_STATE_DB_GUARD_BYPASS": "1",
        })
        env.update(extra or {})
        return env
    ...
    def update_config(self, mutate: Callable[[dict], None]) -> None:
        ...
        path = self.nunmai_home / "config.yaml"
...
def build_parity_home(root: Path, base_url: str, *, grandchild: bool = True,
    ...
    home = root / "home"
    nunmai_home = home / ".nunmai"
    project = root / "project"
    markers = root / "markers"
    for d in (nunmai_home, project, markers):
        d.mkdir(parents=True, exist_ok=True)
    tag = uuid.uuid4().hex
    c = {name: f"{name.upper()}-{uuid.uuid4().hex[:12]}" for name in (
        "context", "skill", "memory", "shell_hook", "plugin_hook", "mcp")}
    ph = ParityHome(root=root, home=home, nunmai_home=nunmai_home, project=project,
                    markers=markers, pid_log=root / "mcp_pids.log", tag=tag, canaries=c)

    write_nunmai_home(nunmai_home, base_url)
    cfg = yaml.safe_load((nunmai_home / "config.yaml").read_text(encoding="utf-8"))
    cfg["agent"]["disabled_toolsets"] = [DISABLED_TOOLSET]
    cfg["hooks"] = {"pre_llm_call": [{
        "command": f"{sys.executable} {nunmai_home / 'agent-hooks' / 'shell_hook.py'}",
        "timeout": 30,
    }]}
    ...
    (nunmai_home / "config.yaml").write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")

    hooks_dir = nunmai_home / "agent-hooks"
    hooks_dir.mkdir()
    ...
    plugin_dir = nunmai_home / "plugins" / PLUGIN_NAME
    ...
    skill_dir = nunmai_home / "skills" / "parity-skill"
...
def check_invariants(obs: Observation, ph: ParityHome, *, toolset: str = "nunmai-cli",
...
def nunmai_argv(*args: str) -> list[str]:
    return [sys.executable, "-m", "nunmai_cli.main", *args]
