...
* ``drive_serve`` — ``nunmai serve --host 127.0.0.1 --port 0`` spawned the way the
  Desktop spawns it (``TERMINAL_CWD`` + ``NUNMAI_DASHBOARD_SESSION_TOKEN`` +
  ``NUNMAI_DESKTOP=1`` env, stdin closed), port read from the READY sentinel on
...
def drive_tui_gateway(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    try:
        ...
        try:
            ...
            terminate(proc)
    return DriveResult(final_text=text, toolset="nunmai-cli", graceful_exit=graceful,
                       extra={"exit_code": proc.returncode, "stderr_tail": cap.stderr[-2000:]})


# nunmai serve (Desktop backend) ------------------------------------------------------
...
def spawn_serve(ph: ParityHome) -> ServeProcess:
    """Spawn ``nunmai serve`` with the Desktop's argv/env/stdio shape (electron/main.ts)."""
    ...
    proc = subprocess.Popen(
        nunmai_argv("serve", "--host", "127.0.0.1", "--port", "0"), cwd=ph.home, env=env,
...
def first_stdout_line(sp: ServeProcess, timeout: float = READY_TIMEOUT) -> str:
    ...
    while True:
        ...
        if line:
            return line
        if "NUNMAI_BACKEND_READY" in sp.cap.stderr:
            raise AssertionError(
...
def drive_serve(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    return DriveResult(final_text=text, toolset="nunmai-cli", cwd_channel="TERMINAL_CWD + session.create cwd",
