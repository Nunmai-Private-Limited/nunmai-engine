"""Entrypoint driver for the parity matrix: cron run-now (``nunmai cron run <id>``).

Follows the driver contract in ``_drive_cli``. ``nunmai cron create`` writes the
job (``--deliver local``: no platform; ``--workdir``: cron's documented
per-job cwd/context channel, see ``nunmai cron create --help``), then
``nunmai cron run <id>`` executes it. With no gateway owning the store the CLI
runs the job synchronously through ``cron.scheduler.run_job`` — the same agent
build the ticker uses (``nunmai_cli/cron.py::_job_action`` forces the
...
from tests.e2e.core.parity._helpers import TURN_TIMEOUT, DriveResult, ParityHome, nunmai_argv
from tests.fakes.fake_llm_provider import FakeLLMServer

# cron/scheduler.py::_resolve_cron_enabled_toolsets -> _get_platform_tools(cfg, "cron") default.
CRON_TOOLSET = "nunmai-cron"
...
def _cron(ph: ParityHome, *args: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        nunmai_argv("cron", *args), cwd=ph.home, env=ph.env(), capture_output=True, text=True,
...
def drive_cron(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    assert created.returncode == 0 and match, (
        f"nunmai cron create exited {created.returncode}: {created.stdout[-1000:]} {created.stderr[-2000:]}")
    job_id = match.group(1)

    ran = _cron(ph, "run", job_id)
    assert ran.returncode == 0, f"nunmai cron run exited {ran.returncode}: {ran.stderr[-2000:]}"
    ...
    assert "Ran now:" in ran.stdout, f"nunmai cron run did not execute synchronously: {ran.stdout[-1000:]}"

    outputs = sorted((ph.nunmai_home / "cron" / "output" / job_id).glob("*.md"))
    final_text = None
    if outputs:
