...
def _run_cli(ph: ParityHome, *args: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        nunmai_argv(*args), cwd=ph.project, env=ph.env(), capture_output=True, text=True,
...
def drive_oneshot(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    ...
    assert proc.returncode == 0, f"nunmai -z exited {proc.returncode}: {proc.stderr[-2000:]}"
    return DriveResult(final_text=proc.stdout.strip(), toolset="nunmai-cli")


def drive_chat_q(ph: ParityHome, srv: FakeLLMServer, prompt: str) -> DriveResult:
    proc = _run_cli(ph, "chat", "-q", prompt, "-Q")
    assert proc.returncode == 0, f"nunmai chat -q exited {proc.returncode}: {proc.stderr[-2000:]}"
    return DriveResult(final_text=proc.stdout.strip(), toolset="nunmai-cli")
