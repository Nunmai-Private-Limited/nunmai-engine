...
One real ``state.db`` under ``tmp_path``, in either journal mode Nunmai deploys (see ``JOURNAL_MODES``); every
...
* journal-mode selection through the PRODUCTION decision path: the ``delete`` arm pins the version the
  children's ``nunmai_state_wal.is_sqlite_wal_reset_vulnerable()`` probe sees to a WAL-reset-vulnerable
...
* the invariant checks, done in the test process with short-lived bare ``sqlite3`` connections only (the
  test process never imports ``nunmai_state``, so it never becomes a foreign holder of the file).
...
from nunmai_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable
...
SEED_ENV = "NUNMAI_SQLITE_TORTURE_SEED"

JOURNAL_MODES = ("wal", "delete")
# Read ONLY by _roles.py in each child: the SQLite version its production version probe reports.
SQLITE_PIN_ENV = "NUNMAI_E2E_SQLITE_VERSION_PIN"
VULNERABLE_SQLITE = "3.50.4"  # bundled by uv's CPython 3.11.14 (the unit CI job): Nunmai runs DELETE there
...
def skip_unless_deployable(journal: str) -> None:
    """WAL is not what Nunmai runs on a vulnerable SQLite, so that arm is not deployable there; DELETE always is."""
    if journal == "wal" and not linked_sqlite_is_wal_capable():
        import pytest

        pytest.skip(f"linked SQLite {sqlite3.sqlite_version} runs Nunmai in DELETE mode; the delete arm covers it")
...
def child_env(home: Path, nunmai_home: Path) -> dict:
    """Probe hygiene: private HOME/NUNMAI_HOME, no provider credentials, repo importable."""
    env = {k: v for k, v in os.environ.items() if not k.endswith("_API_KEY")}
    env.update({
        "HOME": str(home),
        "NUNMAI_HOME": str(nunmai_home),
        "PYTHONUNBUFFERED": "1",
        # The child's NUNMAI_HOME *is* this chamber's private home, which the live-DB guard reads as
        # "the real Nunmai root"; HOME/NUNMAI_HOME above already keep it off the production install.
...
class Chamber:
    """A private NUNMAI_HOME + state.db plus the processes playing roles against it."""
    ...
    def __init__(self, root: Path, *, journal: str = "wal"):
        ...
        self.nunmai_home.mkdir(parents=True, exist_ok=True)
        # The config default in BOTH arms: the delete arm is a default-config user on a vulnerable SQLite.
        (self.nunmai_home / "config.yaml").write_text("database:\n  journal_mode: wal\n", encoding="utf-8")
        self.db = self.nunmai_home / "state.db"
        self.work = root / "work"
        self.work.mkdir(exist_ok=True)
        self.env = child_env(self.home, self.nunmai_home)
    ...
    def spawn_cli(self, name: str, *argv: str) -> subprocess.Popen:
        """A real `nunmai …` CLI subprocess against this NUNMAI_HOME (``nunmai_cli.main`` run as ``__main__``
