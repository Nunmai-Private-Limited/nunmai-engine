...
provider on ONE shared ``state.db`` — once per journal mode Nunmai deploys (WAL, and the DELETE mode the
...
def rig(request, tmp_path_factory):
    ...
    write_nunmai_home(ch.nunmai_home, server.base_url, extra_config=MICRO_CONFIG + DB_CONFIG)
    tui_home = ch.root / "tui-home"
    write_nunmai_home(tui_home / ".nunmai", server.base_url, extra_config=DB_CONFIG)
    for home in (ch.nunmai_home, tui_home / ".nunmai"):
        cfg = home / "config.yaml"
        cfg.write_text(cfg.read_text(encoding="utf-8").replace(ctx, "  context_length: 64000\n"), encoding="utf-8")
    yield Rig(ch, server, aux, {"HOME": str(tui_home), "NUNMAI_HOME": str(tui_home / ".nunmai")})
    ch.shutdown()
    server.stop()
