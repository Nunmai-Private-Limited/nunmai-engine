...
``state.db`` through the production ``SessionDB`` (or, for the non-nunmai opener, a bare ``sqlite3``
...
Journal-mode seam: with ``NUNMAI_E2E_SQLITE_VERSION_PIN`` set, the production version probe
``nunmai_state_wal.is_sqlite_wal_reset_vulnerable()`` reports that SQLite version instead of the linked one;
...
def _apply_sqlite_version_pin() -> None:
    pin = os.environ.get("NUNMAI_E2E_SQLITE_VERSION_PIN")
    if not pin:
        return
    import nunmai_state_wal

    pinned = tuple(int(p) for p in pin.split("."))
    probe = nunmai_state_wal.is_sqlite_wal_reset_vulnerable

    def is_sqlite_wal_reset_vulnerable(version_info=None):
        return probe(pinned if version_info is None else version_info)

    nunmai_state_wal.is_sqlite_wal_reset_vulnerable = is_sqlite_wal_reset_vulnerable
...
def _patient(a: dict, out: Out, op: str, fn, *, deadline: float = 90.0):
    ...
    retried. DELETE mode is documented to block readers on writes (nunmai_state_wal), so a busy read/open is an
...
def _stray_close(db_path: Path) -> None:
    """What a plugin, a tool read of ~/.nunmai or a header probe does: open + close the live files in THIS
...
def role_churn(a: dict, out: Out) -> int:
    """`nunmai sessions list` / doctor / cron-guard shape: open, read, close — ``iterations`` times in one
    ...
    try:
        for i in range(int(a["iterations"])):
            _patient(a, out, "churn", _raw_cycle if i % 2 else _nunmai_cycle)
            if i == 3:
...
def role_cli(a: dict, out: Out) -> int:
    """``nunmai <argv>``: ``nunmai_cli.main`` run as ``__main__``, i.e. ``python -m nunmai_cli.main <argv>``."""
    import runpy

    sys.argv = ["nunmai", *a["argv"]]
    try:
