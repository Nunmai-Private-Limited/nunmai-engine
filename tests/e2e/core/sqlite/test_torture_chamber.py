...
One ``state.db`` per journal mode Nunmai deploys — WAL, and DELETE (what it runs on a WAL-reset-vulnerable
...
``nunmai sessions list`` / ``sessions stats`` CLI), FTS rebuild/optimize maintenance, and
...
``NUNMAI_SQLITE_TORTURE_SEED`` replays a run.
"""
...
def ep_lock_cancellation(ch, ep, rng):
    """Plugins, tool reads of ~/.nunmai and header probes open+close state.db inside the writer process,
