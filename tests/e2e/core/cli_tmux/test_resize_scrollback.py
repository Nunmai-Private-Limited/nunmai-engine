...
A real ``nunmai chat --cli`` runs in a private tmux server against the scripted fake provider.
...
def test_resizes_keep_each_transcript_line_once_in_tmux_scrollback(tmp_path: Path) -> None:
    sock = f"nunmai-e2e-{uuid.uuid4().hex[:8]}"
    home = tmp_path / "home"
    (tmp_path / "work").mkdir()
    ...
    with FakeLLMServer(script, aux=lambda _r: Text("Scripted session title")) as llm:
        write_nunmai_home(home / ".nunmai", llm.base_url)
        env = {k: v for k, v in os.environ.items() if not k.startswith(("NUNMAI_", "TMUX"))}
        env.update(HOME=str(home), NUNMAI_HOME=str(home / ".nunmai"), PYTHONPATH=str(REPO_ROOT),
                   TERM="xterm-256color")
        argv = [sys.executable, "-m", "nunmai_cli.main", "chat", "--cli", "--yolo"]
        ...
        try:
            ...
            wait_for("Welcome to Nunmai", timeout=120)
            time.sleep(2.0)
