...
``nunmai chat -q --resume`` oneshot CLI — switching process and working directory between turns,
...
def world(tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    nunmai_home = Path(os.environ["NUNMAI_HOME"])  # hermetic per-test tmp dir from the conftest
...
def run_journey(world: dict, hops: list[Hop]) -> tuple[str, list[tuple[int, str]], int | None]:
    ...
    srv, script, home = world["srv"], world["script"], world["nunmai_home"]
    sid: str | None = None
...
def test_request_prefix_is_byte_stable_across_processes(world, journey):
    ...
    srv, home = world["srv"], world["nunmai_home"]
    main = srv.main_requests()
    assert len(main) >= 10
