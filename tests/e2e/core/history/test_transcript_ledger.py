...
and at the end of every scenario a FRESH ``nunmai chat --resume`` process replays the persisted
...
def world(tmp_path, monkeypatch):
    home = tmp_path / "home"
    home.mkdir()
    # The hermetic conftest's per-test NUNMAI_HOME (a tmp dir): the in-process live-system guard
    # refuses a <HOME>/.nunmai layout, and the children must share this very state.db.
    ...
    with FakeLLMServer(script) as srv:
        write_nunmai_home(nunmai_home, srv.base_url, extra_config=OFFLINE_CONFIG + NO_BACKGROUND_REVIEW)
        monkeypatch.setenv("OPENAI_API_KEY", "sk-fake-e2e")
        yield {"srv": srv, "script": script, "home": home, "nunmai_home": nunmai_home,
...
def test_transcript_ledger(world, name):
    srv, script, nunmai_home = world["srv"], world["script"], world["nunmai_home"]
    ...
    session = InProcessSession(srv.base_url, nunmai_home, f"ledger-{name}")
    script.session = session
    ledger = Ledger(srv, nunmai_home)
    try:
    ...
    assert_usage_matches(nunmai_home, lineage(nunmai_home, sid), srv.requests, f"scenario {name}")
    integrity_ok(nunmai_home)

    # (d) resume in a FRESH `nunmai chat --resume` process: its first request replays the persisted
    ...
    persisted, _ = views(nunmai_home, sid)
    n_before = len(main)
    env = child_env(world["home"], nunmai_home, world["workdir"])
    ...
    assert_usage_matches(nunmai_home, lineage(nunmai_home, sid), srv.requests, f"scenario {name} after resume")
...
def test_micro_compaction_resumed_display_shows_each_input_once(world):
    ...
    session = InProcessSession(srv.base_url, nunmai_home, "ledger-micro-display")
    inputs: list[str] = []
    try:
        ...
        session.close()
    model, display = views(nunmai_home, sid)
