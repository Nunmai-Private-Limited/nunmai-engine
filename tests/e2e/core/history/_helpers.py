...
real ``tui_gateway`` stdio JSON-RPC server and the real ``nunmai chat -q`` oneshot CLI as
...
# NUNMAI_SESSION_*, AUXILIARY_*, provider keys ... any of which changes the child's prompt or
...
def db_connect(nunmai_home: Path) -> sqlite3.Connection:
    con = sqlite3.connect(f"file:{nunmai_home / 'state.db'}?mode=ro", uri=True, timeout=30)
    con.row_factory = sqlite3.Row
    return con


def views(nunmai_home: Path, sid: str) -> tuple[list[dict], list[dict]]:
    """(model_history, display_history) exactly as a resuming surface loads them."""
    from nunmai_state import SessionDB

    db = SessionDB(db_path=nunmai_home / "state.db")
    try:
...
def model_only_identities(nunmai_home: Path, model: list[dict]) -> set[tuple]:
    ...
    if not ids:
        return set()
    with db_connect(nunmai_home) as con:
...
def row_counts(nunmai_home: Path, sid: str) -> dict[str, int]:
    with db_connect(nunmai_home) as con:
...
def integrity_ok(nunmai_home: Path) -> None:
    with db_connect(nunmai_home) as con:
        assert con.execute("PRAGMA integrity_check").fetchone()[0] == "ok"


def lineage(nunmai_home: Path, sid: str) -> list[str]:
...
def assert_rows_exactly_once(nunmai_home: Path, sid: str, where: str) -> None:
...
def assert_usage_matches(nunmai_home: Path, sids: list[str], records: list[dict[str, Any]], where: str) -> None:
...
def child_env(home: Path, nunmai_home: Path, workdir: Path) -> dict[str, str]:
    env = {k: os.environ[k] for k in _ENV_ALLOW if k in os.environ}
    env.update(
        HOME=str(home), NUNMAI_HOME=str(nunmai_home), PWD=str(workdir),
        NUNMAI_TEST_ISOLATION=str(nunmai_home),
...
def run_oneshot(env: dict[str, str], cwd: Path, prompt: str, spawned: Spawned, *,
    ...
    """``nunmai chat -q PROMPT -Q [--resume SID]`` in a fresh process -> (stdout, durable sid)."""
    cmd = [sys.executable, "-m", "nunmai_cli.main", "chat", "-q", prompt, "-Q"]
    if resume:
        cmd += ["--resume", resume]
...
class InProcessSession:
    ...
    def __init__(self, base_url: str, nunmai_home: Path, sid: str, *, platform: str = "cli") -> None:
        from nunmai_cli.config import load_config
        from nunmai_cli.tools_config import _get_platform_tools
        from nunmai_state import SessionDB
        from run_agent import AIAgent

        self.db = SessionDB(db_path=nunmai_home / "state.db")
    ...
    def compress(self, args: str = "") -> Any:
        """Mirror ``nunmai_cli.cli_session_mixin`` ``/compress``: install ``after_messages``, follow a
...
class Ledger:
    ...
    def __init__(self, srv: Any, nunmai_home: Path, *, check_inputs: bool = True) -> None:
        self.srv, self.home, self.check_inputs = srv, nunmai_home, check_inputs
