...
The defect: nunmai_state already knows macOS `fsync()` does not guarantee
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in nunmai_state_repair; nunmai_state only
    # re-imports them.
    import nunmai_state_repair

    source = Path(nunmai_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(nunmai_state_repair.__file__))
