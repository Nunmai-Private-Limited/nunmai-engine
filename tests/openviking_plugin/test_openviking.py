...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/nunmai/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/peers/nunmai/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "nunmai" for headers in normalized_headers)
        assert all(
...
class TestEnsureClientReloadsEnv:
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/default/peers/nunmai/memories/"
        )
