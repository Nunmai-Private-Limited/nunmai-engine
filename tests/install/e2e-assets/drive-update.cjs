// drive-update.cjs — launch the INSTALLED Nunmai.exe (real Electron desktop
...
//
//   node <this file> <path-to-Nunmai.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Nunmai.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: NUNMAI_HOME (isolated install) and
  ...
  // The real backend (`nunmai serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Nunmai... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.nunmaiDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Nunmai — this window will close" overlay should appear,
  ...
  // the staged binary), which claims NUNMAI_HOME/.nunmai-update-in-progress
  ...
  // proved the desktop had exited and `nunmai update` was already running).
  //
  ...
  const nunmaiHome = process.env.NUNMAI_HOME
  const markerPath = nunmaiHome ? path.join(nunmaiHome, '.nunmai-update-in-progress') : null
  const resultPath = nunmaiHome ? path.join(nunmaiHome, '.nunmai-update-result.json') : null
