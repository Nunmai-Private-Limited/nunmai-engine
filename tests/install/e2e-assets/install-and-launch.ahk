...
; Drive the REAL Nunmai-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Nunmai.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-nunmai-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Nunmai-Setup.exe"
...
Log("Launch clicked; waiting for the Nunmai desktop app window")
...
try {
    ...
    throw Error("Nunmai.exe real-sized window did not appear within 300s of clicking Launch")
}
