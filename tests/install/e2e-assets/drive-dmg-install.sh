#!/usr/bin/env bash
# Drive the Nunmai-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install Nunmai ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "Nunmai")
...
NUNMAI_BIN="$INSTALL_DIR/venv/bin/nunmai"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$NUNMAI_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.nunmai/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Nunmai.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "Nunmai-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
