...
#   mock_start <workroot>      start, write config into $NUNMAI_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, NUNMAI_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$NUNMAI_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $NUNMAI_HOME (api $url/v1)"
}
