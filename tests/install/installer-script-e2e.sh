...
#              install landed on OLD with a working `nunmai`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `nunmai`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method nunmai-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  nunmai-update|installer-script|installer-script+desktop|nunmai-desktop-app-update) ;;
  *) echo "error: --update-method must be nunmai-update, installer-script, installer-script+desktop or nunmai-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/nunmai-installer-script-e2e"
LOG_DIR="${NUNMAI_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled nunmai or a
# developer config, and old installer scripts hardcode $HOME/.nunmai (the
# NUNMAI_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export NUNMAI_HOME="$HOME/.nunmai"
mkdir -p "$NUNMAI_HOME"
...
assert_desktop_artifact() {
  ...
  for cand in \
    ...
    "$release_dir/linux-unpacked/nunmai" \
    "$release_dir/mac-arm64/Nunmai.app" \
    "$release_dir/mac/Nunmai.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local nunmai="$INSTALL_DIR/venv/bin/nunmai"
  [ -x "$nunmai" ] || fail "no nunmai console script at $nunmai"
  "$nunmai" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "nunmai --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "nunmai --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `nunmai desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `nunmai update` itself makes. Probe the
  # INSTALLED nunmai for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `nunmai desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local nunmai="$INSTALL_DIR/venv/bin/nunmai"
  if ! "$nunmai" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "nunmai desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$nunmai" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "nunmai desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "nunmai desktop --build-only ($1) exited $rc; transcript above"
  ok "nunmai desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  nunmai-update)
    ...
    # installed nunmai; older ones read the prompt from stdin, so close it.
    NUNMAI="$INSTALL_DIR/venv/bin/nunmai"
    if "$NUNMAI" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$NUNMAI" update --yes)
    else
      update_cmd=("$NUNMAI" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "nunmai update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "nunmai update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  nunmai-desktop-app-update)
    # The real user surface: `nunmai desktop` launches the app, the user
    ...
    # suite's own) is configured into NUNMAI_HOME exactly like the dev:mock
    ...
    step "capturing the nunmai desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      NUNMAI_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$NUNMAI" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "nunmai desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "nunmai desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "nunmai desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$NUNMAI_HOME/logs" "$ildest/nunmai-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/nunmai/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$NUNMAI_HOME/.nunmai-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$NUNMAI_HOME" > "$ildest/nunmai-home-ls.txt" 2>/dev/null || true
