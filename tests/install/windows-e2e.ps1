...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Nunmai the way
...
# actually touches:
#
#   INSTALL   - downloads the production Nunmai-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Nunmai.exe must appear.
...
#   installer's `git clone` and `nunmai update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and nunmai-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "nunmai-desktop-app-update", "nunmai-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Nunmai-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:NUNMAI_E2E_WORKROOT) { $env:NUNMAI_E2E_WORKROOT } else { Join-Path $env:TEMP "nunmai-desktop-gui-e2e" }),
...
$NunmaiHome  = Join-Path $WorkRoot "nunmai-home"
$InstallDir  = Join-Path $NunmaiHome "nunmai-engine"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if nunmai ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit nunmai thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Nunmai.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Nunmai.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-NunmaiRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $NunmaiHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "nunmai-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $NunmaiHome ".nunmai-update-result.json"
    ...
    Get-ChildItem -LiteralPath $NunmaiHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "nunmai-home-ls.txt")
}

function Test-NunmaiRuns([string]$Label) {
    Save-InstallSideState $Label
    $nunmaiExe = Join-Path $InstallDir "venv\Scripts\nunmai.exe"
    Assert-True (Test-Path -LiteralPath $nunmaiExe) "$Label -- venv\Scripts\nunmai.exe exists"
    & $nunmaiExe --version 2>&1 | ForEach-Object { Write-Host "    nunmai --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- nunmai --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-NunmaiHome", $NunmaiHome, "-InstallDir", $InstallDir)
...
function Invoke-NunmaiUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $nunmaiExe = Join-Path $InstallDir "venv\Scripts\nunmai.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $nunmaiExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $nunmaiExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "nunmai update transcript" $log
    Assert-True ($updateExit -eq 0) "nunmai update exited $updateExit (expected 0)"
}

function Invoke-NunmaiDesktopAppUpdate([string]$TargetSha) {
    # The nunmai-desktop launch surface: `nunmai desktop` runs its whole
    ...
    $prevCap = $env:NUNMAI_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:NUNMAI_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $nunmaiExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:NUNMAI_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "nunmai desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "nunmai desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $NunmaiHome ".nunmai-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured nunmai desktop spec; update completed"
}
...
function Stop-NunmaiAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Nunmai.exe (Electron) -- never nunmai.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Nunmai" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Nunmai.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `nunmai update`/desktop builds use the Nunmai-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $NunmaiHome "node\node.exe"),
        (Join-Path $NunmaiHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Nunmai-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Nunmai-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Nunmai-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Nunmai-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:NUNMAI_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $NunmaiHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Nunmai-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Nunmai.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Nunmai-Setup.exe", (Join-Path $NunmaiHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Nunmai" -ErrorAction SilentlyContinue)) "Nunmai.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Nunmai-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $NunmaiHome "logs\bootstrap-installer.log"
    ...
    Stop-NunmaiAppProcesses "post-install"
    ...
    Test-NunmaiRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Nunmai.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Nunmai.exe present before update"
    ...
    try {
        ...
        # `nunmai update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .nunmai-update-result.json on every exit;
        #   * older checkouts -> the staged nunmai-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/nunmai/relaunch
        ...
        $updateLog = Join-Path $NunmaiHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-NunmaiRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Nunmai.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Nunmai.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Nunmai" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Nunmai window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Nunmai" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $NunmaiHome "logs\desktop-update-handoff.log"
        ...
        Stop-NunmaiAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, nunmai runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-NunmaiRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-NunmaiRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $NunmaiHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "nunmai-desktop-app-update" {
            Invoke-NunmaiDesktopAppUpdate $state.current
        }
        "nunmai-update" {
            Invoke-NunmaiUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading Nunmai-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $NunmaiHome "logs\desktop.log"))) {
