...
#
# Installs a real, earlier Nunmai the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `nunmai`.
#
...
# Keep sandbox state out of the default .nunmai-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".nunmai-sandbox-e2e-$ROUTE"
export NUNMAI_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/nunmai/.nunmai/nunmai-engine"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/nunmai-engine.git"
...
# both together.
UPSTREAM_URL="${NUNMAI_DEV_SANDBOX_UPSTREAM:-https://github.com/Nunmai-Private-Limited/nunmai-engine.git}"
...
LOG_DIR="${NUNMAI_E2E_LOG_DIR:-$(mktemp -d -t nunmai-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED nunmai accept FLAG on `nunmai update`?
#
...
# that silently rots. `nunmai update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "nunmai update --help 2>&1" | grep -qF -- "$flag"
}
...
require_nunmai_works() {
  local when="$1" out
  out="$(in_sandbox "nunmai --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "nunmai --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "nunmai runs $when"
}

# ── install the earlier Nunmai ─────────────────────────────────────────────
...
require_nunmai_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: nunmai update'
    ...
    # nunmai which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="nunmai update --yes"
    else
      update_cmd="nunmai update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "nunmai update failed ($update_cmd)"
    fi
    require_landed_on_target 'nunmai update'
    require_nunmai_works 'after nunmai update'
    ;;
  installer)
    ...
    require_nunmai_works 'after installer re-run'
    ;;
esac
