...
# (Nunmai-Setup.dmg from the website) can reach HEAD.
#
...
#              nunmai-desktop-app-update  capture `nunmai desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              nunmai-update              CLI update from the installed venv
...
#     --update-method open-app-update|nunmai-desktop-app-update
...
case "$UPDATE_METHOD" in
  open-app-update|nunmai-desktop-app-update|nunmai-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${NUNMAI_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/nunmai-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$NUNMAI_HOME/nunmai-engine"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$NUNMAI_HOME"
  touch "$NUNMAI_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Nunmai.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Nunmai.app" \
    "/Applications/Nunmai.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Nunmai-Setup.dmg"

  local dmg="$WORK_ROOT/Nunmai-Setup.dmg"
  ...
  # "Install Nunmai" button with native input.
  local rc=0
  ...
  log_group "Nunmai-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$nunmai" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "nunmai --version failed after install"
  ok "nunmai --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Nunmai.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    nunmai-update)
      ...
      # update subcommand only in later releases; ask the installed nunmai.
      local nunmai="$INSTALL_DIR/venv/bin/nunmai"
      local update_cmd=("$nunmai" update)
      ...
      [ "$rc" -eq 0 ] || fail "nunmai update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Nunmai.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Nunmai.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Nunmai.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, nunmai update) need.
      local app app_bin
...
PYEOF
    ...
    nunmai-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "nunmai desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "nunmai desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.nunmai/logs" "$ildest/nunmai-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Nunmai"
  ...
  "$INSTALL_DIR/venv/bin/nunmai" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "nunmai --version failed after update"
  ok "nunmai --version works post-update"
