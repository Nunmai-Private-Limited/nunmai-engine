...
branch must survive behind a ``refs/nunmai-update-backups/`` rescue ref, the same
namespace ``nunmai update`` uses, and the installer must print the ref.
"""
...
def _diverged_managed_checkout(tmp_path: Path) -> tuple[Path, str]:
    ...
    managed = tmp_path / "nunmai-engine"
...
def _assert_local_commit_parked(repo: Path, local_sha: str, output: str) -> None:
    ...
    refs = dict(
        line.split()[::-1] for line in _git(
            ...
            "refs/nunmai-update-backups/").splitlines())
    ref = refs.get(local_sha)
    assert ref and ref.startswith("refs/nunmai-update-backups/diverged-main-"), refs
...
def test_install_sh_repository_stage_parks_local_commits_before_reset(tmp_path: Path) -> None:
    ...
    env = os.environ | {
        ...
        "NUNMAI_INSTALL_DIR": str(managed),
    }
...
def test_install_ps1_repository_stage_parks_local_commits_before_reset(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-NunmaiHome", str(tmp_path / "nunmai-home"),
        ],
