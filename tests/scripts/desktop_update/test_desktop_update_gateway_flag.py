...
`nunmai update --gateway` (re)starts the local messaging gateway after the
...
# Stands in for `nunmai`: answers the `update --help` probe (so --keep-stash
...
def _run_handoff(tmp_path: Path, extra_args: list[str]) -> list[str]:
    """Run the real hand-off end to end; return the argv of each nunmai call."""
    ...
    env = {**os.environ, "TMPDIR": str(tmp_path), "NUNMAI_TEST_ARGV": str(argv_log)}
    subprocess.run(
...
def test_default_handoff_asks_for_the_local_gateway(tmp_path):
    ...
    assert update_calls, "hand-off never ran nunmai update"
