...
def test_packaging_declared_as_core_dependency():
    ...
    nunmai_cli/main.py) yet was undeclared, so it only reached users
...
_UPDATE_DOWNGRADE_GUARD_FLOORS = {
    # `nunmai update` reinstalls exact pins from pyproject/lazy_deps. These
...
def _extra_closure(extras: dict, name: str) -> set:
    """Names of every extra reachable from ``nunmai-engine[name]`` self-references."""
    seen, todo = set(), [name]
    while todo:
        cur = todo.pop()
        if cur in seen:
        ...
        for spec in extras.get(cur, ()):
            if _distribution_name(spec) == "nunmai-engine":
...
def test_termux_install_paths_never_request_uvloop():
    ...
    opt-in ``uvloop`` extra through any chain of ``nunmai-engine[...]``
