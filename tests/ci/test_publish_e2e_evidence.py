...
def test_upload_evidence_accepts_only_attachment_urls(tmp_path, monkeypatch):
    ...
    result = _mod.upload_evidence(
        ...
        tmp_path,
        "Nunmai-Private-Limited/nunmai-engine",
        "bot-session-token",
    )

    assert result == {"shot.png": "https://github.com/user-attachments/assets/12345678-1234-1234-1234-123456789abc"}
    assert calls[0][0] == ["gh", "image", "--repo", "Nunmai-Private-Limited/nunmai-engine", str(shot)]
...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- nunmai-e2e-evidence:start -->\npending\n<!-- nunmai-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- nunmai-ci-review-bot -->\n<!-- nunmai-e2e-evidence:start -->\npending\n<!-- nunmai-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- nunmai-ci-review-bot --> no evidence"}]) is None
