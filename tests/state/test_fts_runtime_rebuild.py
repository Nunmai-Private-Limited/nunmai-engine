...
class TestRuntimeFtsRebuild:
    ...
    def test_foreign_holder_detection_includes_deleted_wal(
        ...
        monkeypatch.setattr(nunmai_state, "psutil", FakePsutil)
        monkeypatch.setattr(nunmai_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(nunmai_state.os, "getpid", lambda: 111)
        # Force the macOS/psutil path even on Linux test runners
        monkeypatch.setattr(nunmai_state.sys, "platform", "darwin")
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(nunmai_state, "_IS_WINDOWS", False)
        monkeypatch.setattr(nunmai_state.os, "getpid", lambda: 111)
        monkeypatch.setattr(nunmai_state.sys, "platform", "linux")
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(nunmai_state.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(nunmai_state.os, "readlink", _readlink)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        cmdline_path.write_bytes(b"python3\x00nunmai_cli.main\x00chat\x00")
        ...
        def _fake_cmdline(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(nunmai_state, "_read_proc_cmdline", _fake_cmdline)
