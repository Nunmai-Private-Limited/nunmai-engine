...
work on a file several Nunmai processes share (gateway service, the Desktop
app's `nunmai serve` backend, CLI sessions, the TUI slash worker):

* `nunmai_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `nunmai_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
