"""display_nunmai_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom NUNMAI_HOME under
the user profile (e.g. AppData/Local/nunmai), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\nunmai`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\nunmai/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayNunmaiHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "nunmai"
        with patch.object(hc, "get_nunmai_home", return_value=nested):
            out = hc.display_nunmai_home()
        self.assertEqual(out, "~/AppData/Local/nunmai")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import nunmai_constants as hc

        with patch.object(hc, "get_nunmai_home",
                          return_value=Path.home() / ".nunmai"):
            out = hc.display_nunmai_home()
        self.assertEqual(out, "~/.nunmai")

    def test_outside_home_falls_back_to_absolute(self):
        import nunmai_constants as hc

        outside = Path("/opt/nunmai-custom") if os.name != "nt" else Path("C:/opt/nunmai-custom")
        with patch.object(hc, "get_nunmai_home", return_value=outside):
            out = hc.display_nunmai_home()
