...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_nunmai_attribution(mock_openai):
    """Keyless requests still identify as Nunmai (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Nunmai Engine"
    assert str(headers.get("User-Agent", "")).startswith("NunmaiAgent/")
