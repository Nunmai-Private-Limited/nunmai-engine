...
Reported: "switched from openrouter provider to anthropic api key via nunmai
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("nunmai_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("nunmai_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
