...
Build the real image and verify: a config.yaml present in $NUNMAI_HOME
is migrated by docker_config_migrate.py on boot, running as the nunmai
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $NUNMAI_HOME must be migrated on boot by
    docker_config_migrate.py, running as the nunmai user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $NUNMAI_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/nunmai/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by nunmai (migration ran as nunmai)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "nunmai", (
        f"config.yaml not owned by nunmai (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
