...
Multiplex-only (#118273, #118433): ONE gateway per container serves every profile. ``nunmai
profile create`` still registers the profile's s6 slot (so ``--force`` has something to drive),
but ``nunmai -p <profile> gateway start`` without ``--force`` refuses, names the escape, and
...
Every ``docker exec`` here runs as the unprivileged ``nunmai`` user (via
...
def test_named_profile_gateway_start_refuses_without_force(
    ...
    r = _sh(container_name, f"nunmai profile create {PROFILE}")
    ...
    r = _sh(container_name, f"nunmai -p {PROFILE} gateway start", timeout=60)
...
def test_named_profile_gateway_force_start_then_stop(
    ...
    r = _sh(container_name, f"nunmai -p {PROFILE} gateway start --force", timeout=60)
    assert r.returncode == 0, (
        f"--force gateway start failed: stderr={r.stderr!r} stdout={r.stdout!r}"
    )
    _wait_for_want_state(container_name, want_up=True)

    r = _sh(container_name, f"nunmai -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
