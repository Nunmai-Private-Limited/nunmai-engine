"""Runtime smoke test for Docker $NUNMAI_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the nunmai user after container boot.
...
registered later runs its log service as the dropped nunmai user and
...
def test_logs_gateways_seeded_and_nunmai_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by nunmai after boot."""
    ...
    assert "logs=nunmai" in r.stdout, (
        f"logs/ not owned by nunmai: {r.stdout}"
    )
    assert "gateways=nunmai" in r.stdout, (
        f"logs/gateways/ not owned by nunmai: {r.stdout}"
    )
