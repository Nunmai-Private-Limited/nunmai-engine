...
Override the image with ``NUNMAI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("NUNMAI_TEST_IMAGE", "nunmai-engine-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``NUNMAI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NUNMAI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged nunmai user
# ---------------------------------------------------------------------------
#
# Background: every Nunmai runtime path inside the container drops to UID
# 10000 (the ``nunmai`` user) via ``s6-setuidgid nunmai``. ``docker exec``
...
# fails (PermissionError swallowed) for nunmai, so a test that ran as root
...
# helpers below run every probe under ``-u nunmai`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: nunmai).
