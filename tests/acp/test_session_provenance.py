...
def test_root_session_no_compression(db):
    ...
    assert prov["currentNunmaiSessionId"] == "root1"
    assert prov["rootNunmaiSessionId"] == "root1"
    assert prov["parentNunmaiSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_nunmai_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentNunmaiSessionId"] == "old"
    assert prov["rootNunmaiSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousNunmaiSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootNunmaiSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"nunmai"}
    assert "sessionProvenance" in meta["nunmai"]
    assert meta["nunmai"]["sessionProvenance"]["currentNunmaiSessionId"] == "root1"
