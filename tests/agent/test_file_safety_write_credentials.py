"""Secret stores under NUNMAI_HOME are write-denied; control files stay writable (#110464).
...
@pytest.fixture()
def nunmai_layout(tmp_path, monkeypatch):
    """Profile NUNMAI_HOME plus a distinct global root, both patched."""
    root = tmp_path / "nunmai_root"
...
def test_read_denied_secret_stores_are_write_denied_on_profile_and_root(nunmai_layout):
    root, profile = nunmai_layout
...
def test_control_files_and_lookalikes_outside_home_stay_writable(nunmai_layout, tmp_path):
    root, profile = nunmai_layout
    for base in (profile, root):
...
class TestProfileHomeProcessHome:
    """With the process HOME pinned to ``{NUNMAI_HOME}/home`` (TERMINAL_HOME_MODE=profile,
    ...
    def profile_home_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_nunmai_home_path", lambda: profile)
        monkeypatch.setattr(fs, "_nunmai_root_path", lambda: profile.parent)
        return profile
