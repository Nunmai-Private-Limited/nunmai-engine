...
def test_connection_staggers_past_blackholed_ipv6(monkeypatch):
    ...
    monkeypatch.setattr(nunmai_bootstrap.socket, "socket", FakeSocket)
    monkeypatch.setattr(
        nunmai_bootstrap.selectors, "DefaultSelector", FakeSelector
    )
    monkeypatch.setattr(
        nunmai_bootstrap.time, "monotonic", lambda: clock[0]
    )

    winner = nunmai_bootstrap._happy_eyeballs_create_connection(
        ("chatgpt.com", 443),
        timeout=10.0,
    )
    ...
    assert clock[0] == nunmai_bootstrap._HAPPY_EYEBALLS_DELAY_SECONDS
...
def test_codex_auth_http_client_uses_happy_eyeballs_backend(no_proxy_env):
    from nunmai_cli.auth import _codex_http_client
