...
def _make_codex_agent(
    ...
    *,
    ...
    for name in ("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "NUNMAI_CODEX_TTFB_MAX_SECONDS",
...
def _shorten_implicit_idle_watchdog(monkeypatch, helpers, timeout=2.0):
    ...
    monkeypatch.delenv("NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", raising=False)
...
def test_local_endpoint_ttfb_default_uses_local_stale_ceiling(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_LOCAL_STREAM_STALE_TIMEOUT", "600")
...
def test_local_endpoint_ttfb_explicit_env_still_wins(tmp_path, monkeypatch):
    """An operator-set NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS is honoured verbatim on local endpoints."""
    from agent import chat_completion_helpers as h

    monkeypatch.setenv("NUNMAI_LOCAL_STREAM_STALE_TIMEOUT", "600")
    local = _make_codex_agent(tmp_path, monkeypatch, provider="custom", base_url="http://127.0.0.1:11434/v1")
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "45")
...
def test_ttfb_includes_silent_hang_hint_for_gpt_5_5(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "0.4")
...
def test_ttfb_installs_and_retires_the_codex_request_token(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "1")
...
def test_idle_phase_policy_is_narrow_and_preserves_operator_overrides(
    ...
    if idle_env is None:
        ...
        monkeypatch.setenv("NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_env)
...
def test_event_stale_phase_is_scoped_to_physical_stream_attempt(
    ...
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "2")
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_STRICT", "1")
    monkeypatch.setenv("NUNMAI_CODEX_HARD_TIMEOUT_SECONDS", "5")
...
def test_large_codex_request_hard_ceiling_reclaims_silent_stall(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    # Real default TTFB threshold (no NUNMAI_CODEX_TTFB_* override) → for a
    # >10k-token request the no-event TTFB watchdog is auto-disabled.
    monkeypatch.setenv("NUNMAI_CODEX_HARD_TIMEOUT_SECONDS", "3")
...
def test_explicit_ttfb_max_seconds_still_caps(tmp_path, monkeypatch):
    """An explicit NUNMAI_CODEX_TTFB_MAX_SECONDS override still bounds the
    scaled cutoff."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    agent.reasoning_config = {"enabled": False}
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_MAX_SECONDS", "90")
