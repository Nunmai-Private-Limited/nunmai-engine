...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.moa_config.resolve_moa_preset",
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp NUNMAI_HOME
...
class TestNormalizeAuxProvider:
    ...
    def test_covers_every_alias_the_main_path_resolves(self):
        """Every alias nunmai_cli.auth resolves also resolves in aux — drift becomes a red test (#115006)."""
        from nunmai_cli.auth import _PROVIDER_ALIASES as auth_table
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``nunmai auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        monkeypatch.setenv("NUNMAI_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestBuildCodexClient:
    ...
    def test_profile_codex_base_url_overrides_pool_endpoint(self, monkeypatch):
        ...
        with (
            ...
            monkeypatch.setenv("NUNMAI_CODEX_BASE_URL", "http://127.0.0.1:8787/v1")
            mock_openai.return_value = MagicMock()
            from agent.auxiliary_client import _build_codex_client
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``nunmai model``, no per-task override is
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_allows_builtin_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nunmai_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
            client, model = _try_openrouter()
    ...
    def test_free_only_skips_paid_configured_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nunmai_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("nunmai_cli.config.load_config_readonly", return_value=_paid_cfg), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("nunmai_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_nous_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("nunmai_cli.models.get_nous_recommended_aux_model", return_value=None),
        ):
...
class TestRefreshNousRecommendedModel:
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.models.get_nous_recommended_aux_model", _boom)
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.models.get_nous_recommended_aux_model",
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Nunmai auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Nunmai owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    ...
    def test_generic_reasoning_fallback_clamps_ultra_for_auxiliary_and_moa_calls(self, task, monkeypatch):
        """The OpenAI-compatible fallback must never put Nunmai-only ``ultra`` on the wire."""
    ...
    def test_profile_projection_receives_wire_clamped_effort(self, monkeypatch):
        ...
        an OpenRouter/Nunmai slot 400'd otherwise (#112010)."""
        import agent.auxiliary_client as aux
    ...
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value=config), patch("nunmai_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("nunmai_cli.config.load_config", return_value=config), patch("nunmai_cli.config.load_config_readonly", return_value=config), \
    ...
    def test_bare_custom_auth_error_does_not_fall_back_to_env_base_url(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        with patch("nunmai_cli.runtime_provider.resolve_runtime_provider",
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Nunmai reasoning.

    MoA slot reasoning is carried through call_llm as a Nunmai
...
class TestCodexAuxiliaryAdapterReservedToolAliases:
    ...
    reversed on the parsed tool_calls before Nunmai dispatch (#114260)."""
    ...
    def test_wire_tools_match_main_transport_aliases_and_strict(self, base_url, aliased, monkeypatch):
        ...
        # Deterministic xAI branch: a non-xAI web backend keeps client dispatch under ``nunmai_web_search``.
        ...
        assert {t["name"] for t in resp_kwargs["tools"]} == {
            f"nunmai_{n}" if n in aliased else n
        ...
        assert history_names == ["nunmai_search_files" if "search_files" in aliased else "search_files"]
        assert resp_kwargs.get("_wire_aliases", {}) == {f"nunmai_{n}": n for n in aliased}
    ...
    def test_create_maps_aliases_back_and_never_sends_alias_map(self):
            ...
            def create(self, **kwargs):
                ...
                return SimpleNamespace(
                    ...
                    output=[SimpleNamespace(type="function_call", call_id="c9", id="fc_9",
                                            name="nunmai_search_files", arguments='{"pattern": "x"}')],
                )
        ...
        assert "nunmai_search_files" in {t["name"] for t in wire_tools}
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "NunmaiAgent"
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("nunmai_cli.config.load_config", return_value=fake_config), patch("nunmai_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("nunmai_cli.config.load_config", return_value={"model": {}}), patch("nunmai_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("nunmai_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            "nunmai_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "sk-test", "base_url": "https://api.example.com/v1"},
        ), patch(
            "nunmai_cli.models_pricing.fetch_models_with_pricing", return_value={}
        ) as fetch:
