...
refreshed client also fails (e.g. an out-of-credit 404 on a stale Nunmai runtime token),
...
exercises the real path end to end: a temp ``NUNMAI_HOME`` whose ``config.yaml``
declares a ``fallback_chain``, the real client construction and HTTP layer against a
local endpoint, with only the credential sources stubbed (the Nunmai portal account
...
def nous_ladder_endpoint(monkeypatch):
    """A local endpoint that answers the Nunmai host, recording every request.

    The routing decision that selects the auth-refresh rung matches on the base URL
    host, so the endpoint is addressed as the real Nunmai host and ``getaddrinfo`` is
