...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Nunmai Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = nunmai_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.nunmai.
    assert "/root/.nunmai/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.nunmai/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.nunmai/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.nunmai/auth.json` and read the
    ...
    oauth = nunmai_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = nunmai_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.nunmai/auth.json and @file:.nunmai/.anthropic_oauth.json "
        "and @file:.nunmai/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Nunmai/composer-images/a.png",
...
async def test_side_thread_expansion_guards_the_served_profile_home(tmp_path: Path, monkeypatch):
    ...
    token = set_nunmai_home_override(served_home)
    try:
        result = preprocess_context_references(
    ...
    assert any("internal Nunmai path" in w for w in result.warnings)


@pytest.mark.asyncio
async def test_composer_paste_outside_workspace_is_attached_but_sibling_dir_is_not(tmp_path, monkeypatch):
    """Desktop saves a large paste under <NUNMAI_HOME>/composer-pastes and attaches it
    ...
    nunmai_home = tmp_path / ".nunmai"
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    paste = nunmai_home / "composer-pastes" / "pasted_content_1.txt"
    paste.parent.mkdir(parents=True)
    paste.write_text("PASTED-BODY-MARKER\n", encoding="utf-8")
    lookalike = nunmai_home / "my-composer-pastes-backup" / "secret.txt"
