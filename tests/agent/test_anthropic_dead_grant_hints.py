"""Nunmai credential hints and the Anthropic token-endpoint error shape (#113023).

A dead Nunmai login must be repaired with ``nunmai auth add <provider>``; hints that send the user to
an external CLI's login command do not touch Nunmai' own credentials. The token endpoint's
