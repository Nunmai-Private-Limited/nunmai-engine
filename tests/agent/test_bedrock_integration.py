...
Note: Tests that import ``nunmai_cli.auth`` or ``nunmai_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from nunmai_cli.models import _PROVIDER_ALIASES
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        # The Nunmai free tier counts as a configured provider and sits above the Bedrock chain
        # (NS-829); this test's contract is the chain itself, so switch the free tier off.
        monkeypatch.setattr("nunmai_cli.anon_auth.guest_enabled", lambda: False)
        # Mock the auth store to have no active provider
        with patch("nunmai_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("nunmai_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("nunmai_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("nunmai_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_openai_models_route_to_mantle_responses(self, monkeypatch):
        ...
        for model_id in BEDROCK_OPENAI_RESPONSES_MODEL_IDS:
            with patch("nunmai_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
                 patch("nunmai_cli.runtime_provider._get_model_config", return_value={
                     "provider": "bedrock",
                     "default": model_id,
                 }):
                result = resolve_runtime_provider(requested="bedrock")
...
class TestProvidersModule:
    """Verify bedrock is wired into nunmai_cli/providers.py."""
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "nunmai-engine[bedrock]" not in extras["all"]
