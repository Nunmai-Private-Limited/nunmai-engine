...
def test_hosted_context_rejection_far_below_the_known_window_compresses():
    ...
    route's real window is below the one Nunmai assumes, so /retry would fail forever. Compress."""
...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("NUNMAI_HOME", "/srv/nunmai-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "nunmai gateway stop" in text and "nunmai doctor" in text
    assert "~/.nunmai" not in text and "/srv/nunmai-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at nunmai_state
