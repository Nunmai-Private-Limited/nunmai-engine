...
def _agent(**overrides):
    base = dict(_codex_session=None, session_cwd="/tmp", tool_progress_callback=None,
                _cached_system_prompt="SOUL: you are Nunmai", ephemeral_system_prompt=None)
    base.update(overrides)
    return SimpleNamespace(**base)


_HISTORY = [
    {"role": "system", "content": "SOUL: you are Nunmai"},
...
def test_fresh_thread_is_seeded_with_prior_turns_but_not_the_current_one(monkeypatch):
    ...
    assert instructions.startswith("SOUL: you are Nunmai")
    ...
    assert instructions.count("SOUL: you are Nunmai") == 1  # system rows are not re-rendered as history
    # The seed is not part of the recorded composition: the next turn keeps the thread.
    assert agent._codex_session_prompt == "SOUL: you are Nunmai"
