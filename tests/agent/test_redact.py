...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure NUNMAI_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.nunmai/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.nunmai/config.yaml", "api_key: nunmaiConfigSecret123", "nunmaiConfigSecret123"),
            (
                "head ~/.nunmai/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.nunmai/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.nunmai/config.example.yaml",
            "cat ~/.nunmai/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestNunmaiHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED Nunmai home: a managed Windows home
    (``%LOCALAPPDATA%\\nunmai``) has no ``.nunmai`` segment and a resolved path never spells
    ``$NUNMAI_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "nunmai"  # no ".nunmai" segment
