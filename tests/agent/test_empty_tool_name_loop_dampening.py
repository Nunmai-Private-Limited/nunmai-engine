...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated NUNMAI_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="nunmai_e2e_47967_")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("nunmai_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into nunmai_cli.*/tools.* and their monkeypatches would otherwise
