"""Nunmai scope cwd export through the real NeMo Relay ATOF plugin."""
...
def test_run_conversation_exports_session_and_turn_cwds(tmp_path, monkeypatch):
    ...
    for directory in (nunmai_home, session_cwd, turn_cwd, atof_dir):
        directory.mkdir()
...
""",
    ...
    session_start = only_event("nunmai.session", "start")
    turn_start = only_event("nunmai.turn", "start")
    session_end = only_event("nunmai.session", "end")
    turn_end = only_event("nunmai.turn", "end")
