...
_CONFIG = {"custom_providers": [{
    ...
    "extra_headers": {"User-Agent": "NunmaiAgent/1.0", "X-Privacy-Tier": "enterprise"},
}]}


def _build(route):
    with patch("agent.anthropic_adapter._require_sdk") as sdk, patch("nunmai_cli.config.load_config", return_value=_CONFIG):
