...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"nunmai-engine-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "nunmai-engine-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "nunmai-engine-dev"
...
# `nunmai curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `nunmai curator pin <umbrella>` in the rename block so
