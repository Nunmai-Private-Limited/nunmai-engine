"""One-shot sessions (``nunmai chat -q``) drop the self-improvement footprint.
...
``NUNMAI_SINGLE_QUERY_SESSION`` the approval gate and delegation dispatcher read, so an interactive
...
def test_oneshot_delegation_budget_charges_total_children_then_refuses(oneshot, monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_SINGLE_QUERY_SESSION")
