...
"I signed in once and Nunmai keeps failing" (#113023). Two invariants: the WARNING carries the
``nunmai auth add <provider>`` hint, and a row the quarantine does not drop (an independent
...
@pytest.mark.parametrize(
    ...
    [
        ("openai-codex", "_is_terminal_codex_oauth_refresh_error", "_sync_entry_from_auth_store",
         "_clear_terminal_tokens_state", "nunmai auth add openai-codex"),
        ("nous", "_is_terminal_nous_refresh_error", "_sync_nous_entry_from_auth_store",
         "_clear_terminal_nous_state", "nunmai auth add nous"),
    ],
)
...
def test_anthropic_dead_grant_warns_and_marks_dead(monkeypatch, caplog):
    ...
    entry = _entry("anthropic", source="manual:nunmai_pkce")
    pool._entries = [entry]
    ...
    warnings = [r for r in caplog.records if r.levelno == logging.WARNING and "terminally invalid" in r.getMessage()]
    assert len(warnings) == 1 and "nunmai auth add anthropic" in warnings[0].getMessage()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"client_id": "nunmai-cli", "scope": "inference:invoke", "access_token": _expired_invoke_jwt(),
...
def test_nous_login_missing_refresh_failure_is_terminal(tmp_path, monkeypatch, caplog, nous_state, expected_code):
    ...
    NUNMAI_HOME so the production raise site's code is what reaches the classifier."""
    ...
    assert "nunmai auth add nous" in warnings[0].getMessage()
    row = pool._entries[0]
