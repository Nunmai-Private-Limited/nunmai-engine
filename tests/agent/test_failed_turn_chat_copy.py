...
def test_oauth_rejection_chat_text_names_the_provider_slug_and_the_failing_profile(tmp_path, monkeypatch):
    ...
    text = result["final_response"]
    assert "`nunmai -p codex auth add openai-codex --type oauth`" in text
    assert "<provider>" not in text
    assert "token_revoked" in text  # the raw error survives for the judge to quote
    # The CLI 💡 hint names the same command; it no longer sends the user to a bare `nunmai auth`.
    cli_hint = "\n".join(hints)
    assert "`nunmai -p codex auth add openai-codex --type oauth`" in cli_hint, cli_hint
    assert "`nunmai auth`" not in cli_hint, cli_hint
...
def test_interpreter_shutdown_copy_substitutes_the_real_session_id():
    ...
    assert "nunmai --resume 20260914_abc" in verdict.final_response
...
def test_model_caused_codes_stay_on_the_provider_layer_and_runtime_codes_on_gateway():
    ...
    per-code copy applies); a busy session or loop bug is Nunmai-side (gateway layer, so the
