"""Tests for agent/nous_rate_guard.py — cross-session Nunmai Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    nunmai_home = str(tmp_path / ".nunmai")
    os.makedirs(nunmai_home, exist_ok=True)
...
class TestIsGenuineNousRateLimit:
    ...
    Nunmai Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Nunmai).
...
class TestWelcomeRouteCopy:
    ...
    def test_the_welcome_host_rate_limit_message_names_the_slash_command(self, monkeypatch):
        ...
        assert "Nunmai Portal" not in expected
    ...
    def test_a_non_welcome_route_keeps_todays_sentence(self, monkeypatch):
        ...
        expected = "Nunmai Portal rate limit active — resets in 10m."
        assert verdict.action == "return"
        assert statuses == [f"⏳ {expected}"]
