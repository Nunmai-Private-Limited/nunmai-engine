...
NUNMAI_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every NUNMAI_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["NUNMAI_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["NUNMAI_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("NUNMAI_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("NUNMAI_DEV_CREDITS", None)
...
def _seed(agent, fixture):
    ...
    os.environ["NUNMAI_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["NUNMAI_DEV_CREDITS_FIXTURE"] = fixture
    try:
...
def _run_bg_seed(monkeypatch, agent, *, warm):
    ...
    import nunmai_cli.nous_account as nous_account
    from agent import credits_tracker

    monkeypatch.delenv("NUNMAI_DEV_CREDITS", raising=False)  # fixtures would take the sync path
...
# ── inference-header path: the catalog goes cold again after the Nunmai TTL ────
...
def test_header_after_ttl_expiry_rewarms_instead_of_flashing_the_banner(monkeypatch):
    """The session-start warm is one-shot; a Nunmai catalog expires after _NOUS_CATALOG_TTL_SECONDS. A
