...
def test_client_cache_key_is_scoped_per_profile_home(tmp_path):
    """Callers that omit api_key (pool / Nunmai auth.json paths) must not share a client across
    multiplex profiles: the per-turn NUNMAI_HOME override has to participate in the key."""
