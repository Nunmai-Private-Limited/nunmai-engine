...
from nunmai_cli.auth_plugin_providers import is_refreshable_oauth_provider
...
def test_plugin_refresh_outcome(plugin_profiles, caplog, hook, status, tokens, extra_key):
    ...
    if status == STATUS_DEAD:
        assert "nunmai auth add example-oauth" in caplog.text
    else:
        assert "nunmai auth add" not in caplog.text


def test_plugin_refresh_adopts_peer_rotation_without_spending_token(plugin_profiles):
    """Two Nunmai processes share one auth.json: the second refresh adopts the first's rotated pair
