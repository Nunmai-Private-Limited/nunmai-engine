...
@pytest.mark.parametrize("api_key", [make_jwt(account_tier="free", client_id="nunmai-cli"), "sk-named"],
...
def test_signing_in_does_not_inherit_anonymous_cooldown(tmp_path, monkeypatch):
    ...
    guest.api_key = make_jwt(account_tier="free", client_id="nunmai-cli")
...
def test_auxiliary_anonymous_cooldown_does_not_outlive_signing_in(tmp_path, monkeypatch):
    ...
    runtime[:] = [make_jwt(account_tier="free", client_id="nunmai-cli"), NAMED]
...
def test_401_diagnostics_follow_request_identity_on_welcome_host(tier, capsys, monkeypatch):
    ...
    assert ("Nunmai couldn't start a new one" in output) == (tier == "anonymous")
    assert ("nunmai auth add nous" in output) == (tier != "anonymous")
...
def test_named_account_on_welcome_host_gets_reconnect_copy_without_signin_card():
    ...
    message = "This endpoint serves anonymous Nunmai Engine accounts only. Use https://inference-api.nousresearch.com with your API key or signed-in account."
    error = Exception(message)
    error.status_code, error.body = 400, {"status": 400, "message": message}
    agent = agent_for(make_jwt(account_tier="free", client_id="nunmai-cli"), WELCOME)
