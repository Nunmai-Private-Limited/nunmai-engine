...
def test_default_when_nothing_set(monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_LANGUAGE", raising=False)
...
def test_language_is_per_profile_under_multiplex(monkeypatch, tmp_path):
    """NUNMAI_LANGUAGE in the DEFAULT profile's environ must not leak into a secondary profile's
    ...
    monkeypatch.setenv("NUNMAI_LANGUAGE", "zh")  # default profile's .env, bridged into environ
    ...
    try:
        ...
        home_token = set_nunmai_home_override(prof_b)
        try:
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus NUNMAI_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("NUNMAI_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
