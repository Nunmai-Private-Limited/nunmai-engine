...
def test_kanban_guidance_requires_worker_task_at_agent_init(monkeypatch, task_id, expected):
    ...
    if task_id is None:
        ...
        monkeypatch.setenv("NUNMAI_KANBAN_TASK", task_id)
    monkeypatch.setattr("nunmai_cli.plugins.discover_plugins", lambda: None)
    monkeypatch.setattr(
        model_tools,
...
def test_kanban_guidance_fallback_requires_owned_worker_task(monkeypatch, task_id, owner, expected):
    ...
    is not identity, and an inherited NUNMAI_KANBAN_TASK is not ownership (#112486)."""
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_nunmai_home``,
    ``get_default_nunmai_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``NUNMAI_HOME`` and no resolver mocks.
    """

    def test_real_nunmai_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_nunmai_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Nunmai profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """NUNMAI_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".nunmai"
        ...
        assert "Active Nunmai profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "NUNMAI_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "NUNMAI_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_nunmai_home", lambda: Path("/nunmai"))

    # Production renders this as str(get_nunmai_home()) + "/profiles/<name>/",
    # and str(Path("/nunmai")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/nunmai".
    _home_str = str(Path("/nunmai"))
    expected_profile = (
    ...
    with (
        ...
        patch("nunmai_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("nunmai_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under NUNMAI_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from nunmai_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("nunmai_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from nunmai_time import now as nunmai_now
        sid = nunmai_now().strftime("%Y%m%d_%H%M%S_fresh")
    ...
    def test_surrogate_zone_name_does_not_abort_prompt(self):
        ...
        with patch("nunmai_time.now", return_value=current):
