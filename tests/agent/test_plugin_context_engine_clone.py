...
SQLite connections — nunmai-lcm) stay selectable and a child's model never leaks into the parent
(#99640, #42449)."""
...
class _LockedEngine(_Engine):
    """Holds a lock (deepcopy raises) and hands out per-agent clones like nunmai-lcm does."""
...
def _select(engine):
    with (patch("plugins.context_engine.load_context_engine", return_value=None),
          patch("nunmai_cli.plugins.get_plugin_context_engine", return_value=engine)):
