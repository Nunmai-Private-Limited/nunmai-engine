"""load_nunmai_dotenv() reloads must not clobber values an external secret source resolved (#74265).

The gateway module import, per-turn reloads and cron fires all call ``load_nunmai_dotenv()`` again;
...
def test_source_secret_survives_second_load_nunmai_dotenv(tmp_path, monkeypatch):
    ...
    env_loader.load_nunmai_dotenv(nunmai_home=home)
    assert os.environ["GLM_API_KEY"] == "vault-value", (
        "second load_nunmai_dotenv() wrote the .env placeholder back over the source-resolved value"
    )
