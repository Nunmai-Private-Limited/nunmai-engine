...
conversation-affinity hint Nunmai sends — ``prompt_cache_key`` on both
OpenAI-wire transports, the OpenRouter/Nunmai sticky ``session_id``, and xAI's
``x-grok-conv-id`` — so the conversation never lands back on the routing
bucket it warmed. Nunmai cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Nunmai-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Nunmai-internal token; a session KEY embeds the
...
class TestProviderStickyKeys:
    """OpenRouter / Nunmai sticky ids and x-grok-conv-id read the same scope."""
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Nunmai-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``NUNMAI_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_SESSION_SOURCE", "override-src")
