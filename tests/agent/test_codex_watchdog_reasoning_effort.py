...
def _codex_agent(tmp_path: Path, monkeypatch, effort: str, *, enabled: bool = True):
    ...
    for var in ("NUNMAI_API_CALL_STALE_TIMEOUT", "NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS",
                "NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "NUNMAI_CODEX_TTFB_MAX_SECONDS"):
...
def test_default_effort_tiers_and_explicit_operator_values_are_untouched(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "20")
    monkeypatch.setenv("NUNMAI_CODEX_TTFB_TIMEOUT_SECONDS", "45")
    monkeypatch.setenv("NUNMAI_API_CALL_STALE_TIMEOUT", "75")
