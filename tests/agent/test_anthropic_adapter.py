...
class TestBuildAnthropicClient:
    ...
    def test_opencode_endpoint_gets_attribution_headers(self):
        ...
        with patch("agent.anthropic_adapter._anthropic_sdk") as mock_sdk:
            ...
            assert headers["HTTP-Referer"] == "https://nunmai.in"
            assert headers["X-Title"] == "Nunmai Engine"
            assert headers["User-Agent"].startswith("NunmaiAgent/")
    ...
    def test_disables_sdk_retries_for_api_key(self):
        ...
        double-retries inside nunmai's outer loop. We delegate retry entirely
...
class TestResolveAnthropicToken:
    ...
    def test_falls_back_to_anthropic_credential_pool_oauth(self, monkeypatch, tmp_path):
        ...
        # returns nothing, mirroring a Nunmai-PKCE-only setup.
...
def test_oauth_system_prompt_sanitizer_preserves_docs_url():
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Nunmai Engine by Nunmai Research uses nunmai-engine skills. "
                    "Docs: https://nunmai.in/docs ; "
                    "interpreter ~/.nunmai/nunmai-engine/venv/bin/python ; "
                    "source github.com/NousResearch/hermes-agent ; mail nunmai-engine@example.com ; "
                    "skill_view(name='nunmai-engine') ; nunmai-engine's docs ; built by nunmai-engine."
                ),
            },
            {"role": "user", "content": "Hi"},
        ],
    ...
    assert "https://nunmai.in/docs" in system_text
    # Paths and repo slugs are addresses too: a subagent told to run
    # ``~/.nunmai/claude-code/venv/bin/python`` fails on a file that does not exist.
    assert "~/.nunmai/nunmai-engine/venv/bin/python" in system_text
    assert "github.com/NousResearch/hermes-agent" in system_text
    assert "nunmai-engine@example.com" in system_text
    assert "skill_view(name='nunmai-engine')" in system_text  # a quoted slug is an identifier
