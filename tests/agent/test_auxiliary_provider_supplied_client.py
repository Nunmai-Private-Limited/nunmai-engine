...
def registered(monkeypatch):
    ...
    Mirrors the import-time synthesis ``nunmai_cli.auth`` performs for plugin ``api_key``
    profiles, which is what routes them into ``_resolve_api_key_branch`` in the first place.
    """
    import nunmai_cli.auth as _auth
    from nunmai_cli.auth_plugin_providers import register_plugin_provider
