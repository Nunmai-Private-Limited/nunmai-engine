"""The codex_app_server runtime hands Nunmai' composed system prompt to the codex thread (#74712, #26035).
...
def _agent(**overrides):
    base = dict(_codex_session=None, session_cwd="/tmp", tool_progress_callback=None,
                _cached_system_prompt="SOUL: you are Nunmai", ephemeral_system_prompt="Always start with ZZZ")
...
def test_runtime_sends_composed_prompt_once_per_thread(monkeypatch):
    ...
    assert len(starts) == 1
    assert starts[0]["developerInstructions"] == "SOUL: you are Nunmai\n\nAlways start with ZZZ"
...
def test_runtime_retires_thread_when_prompt_composition_changes(monkeypatch):
    ...
    assert starts == ["SOUL: you are Nunmai\n\nAlways start with ZZZ", "SOUL: you are Nunmai\n\nPersonality: pirate"]
