...
class TestCacheFileReadBlocking:
    """Internal Nunmai cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        nunmai_home = tmp_path / ".nunmai"
        cache = nunmai_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._nunmai_home_path", return_value=nunmai_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Nunmai cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        nunmai_home = tmp_path / ".nunmai"
        hub = nunmai_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_nunmai_home(self, tmp_path):
        """The env basename guard does not depend on NUNMAI_HOME resolution."""
        nunmai_home = tmp_path / ".nunmai"
        nunmai_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        nunmai_home = tmp_path / ".nunmai"
        cache = nunmai_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
