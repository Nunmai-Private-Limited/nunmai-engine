"""Regression for #115702: a paid Nunmai model behind an empty credit balance answers HTTP 404
...
def test_billing_fallback_warning_names_failing_profile_and_remedy(tmp_path, caplog):
    ...
    for name in ("alpha", "beta"):
        token = set_nunmai_home_override(tmp_path / ".nunmai" / "profiles" / name)
        try:
            caplog.clear()
    ...
    assert "Profile alpha:" in seen["alpha"] and "nunmai -p alpha model" in seen["alpha"]
