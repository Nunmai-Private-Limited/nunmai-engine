...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("nunmai_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated NUNMAI_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="nunmai_api_content_")
    os.makedirs(os.path.join(test_home, ".nunmai"))
    prev_home = os.environ.get("NUNMAI_HOME")
    os.environ["NUNMAI_HOME"] = os.path.join(test_home, ".nunmai")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["NUNMAI_HOME"] = prev_home
...
class TestSessionRowExistsBeforePreflightCompaction:
    ...
    def test_in_place_compaction_multimodal_context_part_survives_reload(self, tmp_path):
        ...
        try:
            ...
            with patch("nunmai_cli.plugins.invoke_hook", return_value=[{"context": "PLUGIN-CTX"}]):
                ctx = _build(
