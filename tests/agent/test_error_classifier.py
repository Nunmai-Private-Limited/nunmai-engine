...
class TestServerInjectedParameterRejection:
    ...
    so an identical request succeeds on retry ~80% of the time.  Nunmai never
    ...
    def test_retention_rejection_from_meta_host_stays_non_retryable(self):
        """Boundary: on api.meta.ai / Bedrock Mantle Nunmai DOES send
...
# ── Test: Nunmai welcome tier (free tier) refusals ───────────────────────

class TestNousWelcomeTier:
    """The Nunmai gateway's welcome-tier contract: a structured 429 body carries ``reason`` /
    ...
    def test_named_caller_on_the_welcome_host_is_deterministic(self):
        body = {"status": 400, "message": "This endpoint serves anonymous Nunmai Engine accounts only. Use https://inference-api.nousresearch.com with your API key or signed-in account."}
        err = MockAPIError(f"Error code: 400 - {body}", status_code=400, body=body)
