...
def test_oneshot_run_gets_distinct_source(monkeypatch, inherited):
    """A finite `nunmai chat -q` / `nunmai -z` run is tagged `oneshot`, whether launched from a plain shell
    or spawned inside a TUI/Desktop session (whose transport label it inherits but is not) (#112550)."""
    monkeypatch.setenv("NUNMAI_SESSION_SOURCE", inherited)
...
def test_oneshot_keeps_explicit_source_flag(monkeypatch, explicit):
    """`nunmai chat -q --source tui` is a documented flag, not an inherited transport label: main.py
    marks it NUNMAI_SESSION_SOURCE_EXPLICIT=1 and the one-shot drop must not override it."""
    monkeypatch.setenv("NUNMAI_SESSION_SOURCE", explicit)
    monkeypatch.setenv("NUNMAI_SESSION_SOURCE_EXPLICIT", "1")
    monkeypatch.setenv("NUNMAI_SINGLE_QUERY_SESSION", "1")
