...
def secondary_scope(monkeypatch):
    for name in ("OPENAI_BASE_URL", "XAI_BASE_URL", "NUNMAI_XAI_BASE_URL", "NOUS_INFERENCE_BASE_URL",
...
def test_base_urls_follow_the_scoped_key_not_default_environ(monkeypatch, secondary_scope, tmp_path):
    ...
    monkeypatch.setattr("nunmai_cli.runtime_provider._get_named_custom_provider", lambda name: None)
