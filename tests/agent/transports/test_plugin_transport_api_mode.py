...
Both tests drive a REAL plugin discovered from an isolated NUNMAI_HOME rather than the gates in
...
def install_dialect_plugin(tmp_path, monkeypatch):
    """Write a model-provider plugin (optionally registering its transport) under a temp NUNMAI_HOME."""
...
def test_registered_plugin_dialect_reaches_every_gate(install_dialect_plugin):
    ...
    from nunmai_cli.runtime_provider import _parse_api_mode, resolve_runtime_provider
