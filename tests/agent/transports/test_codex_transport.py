...
class TestCodexBuildKwargs:
    ...
    def test_900k_context_variant_suffix_stripped_on_wire(self, transport):
        """``-900k`` large-context picker variants are Nunmai-side aliases —
    ...
    def test_xai_injects_native_web_search_when_client_web_search_present(self, transport, monkeypatch):
        ...
        assert "nunmai_web_search" not in names

    def test_xai_renames_client_web_search_when_firecrawl_configured(self, transport, monkeypatch):
        """Configured Firecrawl (or any non-xai backend) must keep Nunmai
        ...
        assert "read_file" in names
        assert "nunmai_web_search" in names
        assert "web_search" not in names
    ...
    def test_xai_does_not_inject_native_web_search_without_client_web_search(self, transport):
        ...
        would silently bypass Nunmai's web-provider config and tool-trace
    ...
    def test_openai_native_not_selected_keeps_client_web_search(self, transport, monkeypatch):
        """A Codex turn that has not selected ``openai-native`` keeps Nunmai
...
class TestResponsesReservedToolAliases:
    ...
    def test_builtin_opencode_go_aliases_reserved_names(self, transport):
        ...
        assert "nunmai_search_files" in names
        assert "nunmai_web_search" in names
    ...
    def test_non_opencode_backend_keeps_original_names(self, transport):
        ...
        assert "web_search" in names
        assert "nunmai_search_files" not in names
    ...
    def test_perplexity_agent_api_aliases_reserved_names(self, transport, monkeypatch):
        ...
        assert "search_files" not in names
        assert "web_search" not in names
        assert "nunmai_fetch_url" in names
        assert "nunmai_people_search" in names
        assert "nunmai_finance_search" in names
        assert "fetch_url" not in names
        assert "people_search" not in names
        assert "finance_search" not in names
        assert "read_file" in names
        assert transport._last_wire_aliases == {
            "nunmai_search_files": "search_files",
            "nunmai_web_search": "web_search",
            "nunmai_fetch_url": "fetch_url",
            "nunmai_people_search": "people_search",
            "nunmai_finance_search": "finance_search",
        }
        ...
        msg = SimpleNamespace(
            ...
            tool_calls=[
                ...
                SimpleNamespace(
                    ...
                    function=SimpleNamespace(
                        name="nunmai_web_search",
                        arguments='{"query":"nunmai"}',
                    ),
                ),
            ],
            codex_reasoning_items=None,
...
class TestXaiReservedToolSearchAlias:
    ...
    def test_xai_aliases_reserved_tool_search(self, transport):
        ...
        names = self._names(kw)
        assert "nunmai_tool_search" in names
    ...
    def test_openai_responses_aliases_reserved_tool_search(self, transport):
        ...
        both the ChatGPT Codex backend and api.openai.com get the bridge under ``nunmai_tool_search``."""
        for extra in ({"is_codex_backend": True}, {"base_url": "https://api.openai.com/v1"}):
            ...
            names = self._names(kw)
            assert "nunmai_tool_search" in names and "tool_search" not in names, extra
            assert transport._last_wire_aliases == {"nunmai_tool_search": "tool_search"}
    ...
    def test_other_responses_backend_keeps_tool_search_name(self, transport):
        ...
        assert "nunmai_tool_search" not in names
    ...
    def test_no_alias_emitted_means_no_reverse_rewrite(self, transport, monkeypatch):
        ...
        aliases must not have a real ``nunmai_tool_search`` tool rewritten."""
        real_tool = {"type": "function", "function": {
            "name": "nunmai_tool_search", "description": "A real MCP tool.",
            "parameters": {"type": "object", "properties": {}}}}
        transport.build_kwargs(
            model="grok-4.6",
            messages=[{"role": "user", "content": "hi"}],
        ...
        normalized = self._normalize_named_call(
            transport, monkeypatch, "nunmai_tool_search"
        )
        assert [tc.name for tc in normalized.tool_calls] == ["nunmai_tool_search"]

    def test_alias_collision_takes_suffix_no_duplicates(self, transport, monkeypatch):
        """A real tool already named ``nunmai_tool_search`` keeps its wire
        name; the bridge is suffixed and both round-trip independently."""
        tools = [
            {"type": "function", "function": {
                "name": "nunmai_tool_search", "description": "Real tool.",
                "parameters": {"type": "object", "properties": {}}}},
            {"type": "function", "function": {
        ...
        names = self._names(kw)
        assert names == ["nunmai_tool_search", "nunmai_tool_search_2"]
        assert len(names) == len(set(names))
        assert transport._last_wire_aliases == {"nunmai_tool_search_2": "tool_search"}
        # Bridge alias maps back; the real tool's name is untouched.
        normalized = self._normalize_named_call(
            transport, monkeypatch, "nunmai_tool_search_2"
        )
        assert [tc.name for tc in normalized.tool_calls] == ["tool_search"]
        normalized2 = self._normalize_named_call(
            transport, monkeypatch, "nunmai_tool_search"
        )
        assert [tc.name for tc in normalized2.tool_calls] == ["nunmai_tool_search"]
...
class TestCodexTransportXaiReasoningEffort:
    ...
    def test_grok_46_clamps_nunmai_aliases_to_model_ceiling(self, transport, effort):
        """Nunmai ladder aliases mean "this model's ceiling" — on grok-4.6
