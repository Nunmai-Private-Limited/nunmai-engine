...
Covers:
  * NUNMAI_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestNunmaiAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import NUNMAI_AGENT_HELP_GUIDANCE
        assert "skill_view(name='nunmai-engine')" in NUNMAI_AGENT_HELP_GUIDANCE
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "nunmai-engine" not in disabled
    ...
    def test_skill_manage_delete_refused(self):
        ...
        msg = _pinned_guard("nunmai-engine")
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the nunmai-engine skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "nunmai-engine"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_nunmai_home", lambda: home)
        ...
        assert result["copied"] == ["nunmai-engine"]
        assert (home / "skills" / "autonomous-ai-agents" / "nunmai-engine" / "SKILL.md").exists()
