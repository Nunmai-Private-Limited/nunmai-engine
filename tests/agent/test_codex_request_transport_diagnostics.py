...
def _oversized_codex_kwargs(size: int) -> dict:
    return {"model": "gpt-5-codex", "instructions": "You are Nunmai.", "store": False, "tools": None,
...
def test_zero_event_retry_prunes_oversized_tool_output_and_logs_size_delta(monkeypatch, tmp_path, caplog):
    ...
    monkeypatch.setattr("nunmai_constants.get_nunmai_home", lambda: tmp_path, raising=False)
