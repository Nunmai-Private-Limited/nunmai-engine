"""Tests for the cross-Nunmai-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Nunmai profile's
...
Reference: May 2026 incident — a nunmai-security profile session
accidentally edited skills under both ~/.nunmai/profiles/nunmai-security/skills/
AND ~/.nunmai/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Nunmai root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_nunmai(tmp_path, monkeypatch):
    """Build a fake Nunmai layout:
    ...
    sec_home = root / "profiles" / "nunmai-security"
    ...
    monkeypatch.setattr(fs, "_nunmai_root_path", lambda: root)
...
def _set_active_home(monkeypatch, nunmai_home: Path):
    """Point file_safety._nunmai_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_nunmai_home_path", lambda: nunmai_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_nunmai, monkeypatch):
        _set_active_home(monkeypatch, fake_nunmai["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_nunmai, monkeypatch):
        """If NUNMAI_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_nunmai_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_nunmai, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_nunmai["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_nunmai["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "nunmai-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_nunmai, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_nunmai["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_nunmai["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "nunmai-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_nunmai, monkeypatch, area):
        _set_active_home(monkeypatch, fake_nunmai["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_nunmai["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    ...
    def test_in_profile_returns_none(self, fake_nunmai, monkeypatch):
        from agent.file_safety import get_cross_profile_warning
        assert get_cross_profile_warning(
            str(fake_nunmai["root"] / "skills" / "a" / "SKILL.md")) is None

    def test_cross_profile_returns_none_guard_retired(self, fake_nunmai, monkeypatch):
        from agent.file_safety import get_cross_profile_warning
        target = fake_nunmai["root"] / "profiles" / "security" / "skills" / "x" / "SKILL.md"
