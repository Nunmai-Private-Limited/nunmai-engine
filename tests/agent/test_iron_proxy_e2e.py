...
Gated on the network: skipped unless NUNMAI_RUN_E2E=1 (no CI lane sets it;
...
pytestmark = pytest.mark.skipif(
    os.environ.get("NUNMAI_RUN_E2E", "0") != "1",
    reason="E2E proxy test — set NUNMAI_RUN_E2E=1 to run (requires network + curl + openssl)",
)
...
def test_iron_proxy_swaps_authorization_header_end_to_end(nunmai_home, monkeypatch):
...
def test_iron_proxy_swaps_x_api_key_header_end_to_end(nunmai_home, monkeypatch):
...
def test_iron_proxy_management_reload_end_to_end(nunmai_home, monkeypatch):
