...
def test_credential_rotation_does_not_carry_global_headers_across_routes():
    ...
    with (
        patch("nunmai_cli.config.load_config_readonly", return_value=config),
        patch(
...
def test_codex_rotation_keeps_proxy_override(monkeypatch):
    ...
    endpoint; with NUNMAI_CODEX_BASE_URL set the rotated client must keep targeting the proxy."""
    from agent.credential_pool import PooledCredential

    monkeypatch.setenv("NUNMAI_CODEX_BASE_URL", "http://127.0.0.1:8787/backend-api/codex/")
