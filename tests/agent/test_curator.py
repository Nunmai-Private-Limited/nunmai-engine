...
def test_curator_slot_is_canonical_aux_task():
    ...
    from nunmai_cli.main_provider_setup import _AUX_TASKS
    from nunmai_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. nunmai_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. nunmai_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.nunmai/skills/<umbrella>/references/ && mv ...`` its
