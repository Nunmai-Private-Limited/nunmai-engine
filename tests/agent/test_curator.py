...
def curator_env(tmp_path, monkeypatch):
    """Isolated NUNMAI_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from NUNMAI_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.nunmai once monkeypatch
    ...
    # thread here, while NUNMAI_HOME is still pinned to this test's tmp home
...
def test_bundled_skills_are_off_limits_unless_opted_in(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.config.load_config", lambda: {"curator": {}})
    assert usage._prune_builtins_enabled() is False
    assert DEFAULT_CONFIG["curator"]["prune_builtins"] is False
    monkeypatch.setattr("nunmai_cli.config.load_config", lambda: {"curator": {"prune_builtins": True}})
...
def test_cli_pin_refuses_bundled_skill(curator_env):
    from nunmai_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `nunmai model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_fork_receives_configured_reasoning(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.config.load_config_readonly", lambda: cfg)
    monkeypatch.setattr(
        "nunmai_cli.runtime_provider.resolve_runtime_provider",
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``nunmai curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``nunmai curator rollback`` restores a
