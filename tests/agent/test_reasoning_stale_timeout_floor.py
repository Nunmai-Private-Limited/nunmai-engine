...
thinking phase.  Nunmai's default cloud-stream stale detector
(``NUNMAI_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``NUNMAI_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NUNMAI_API_CALL_STALE_TIMEOUT", raising=False)
...
def test_explicit_provider_stale_timeout_wins_over_context_tier_and_reasoning_floor(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    monkeypatch.delenv("NUNMAI_STREAM_STALE_TIMEOUT", raising=False)
