"""The goal judge names a Nunmai auxiliary auth failure instead of an opaque judge error (#42177).

``_resolve_nous_runtime_api`` swallows the Nunmai resolver's ``AuthError`` so the ladder can fall
...
from nunmai_cli.auth_constants import AuthError
...
def test_goal_judge_reason_names_nous_auth_failure_and_still_fails_open(tmp_path, monkeypatch):
    """Real judge_goal → call_llm → ladder with goal_judge pinned to nous and no Nunmai login."""
    _reset(monkeypatch)
    ...
    assert reason.startswith("goal_judge auxiliary client unavailable: Nunmai Portal runtime credentials unavailable:")
    assert "nunmai model" in reason, reason
...
def test_nous_credential_failure_is_remembered_and_warned_once(caplog, monkeypatch):
    ...
    assert detail.startswith("Nunmai Portal runtime credentials unavailable: ")
    assert "invalid_grant" in detail and "nunmai model" in detail
...
def test_never_logged_in_is_debug_but_a_dead_credential_warns(caplog, monkeypatch, tmp_path):
    """The auto-route walk resolves Nunmai on every pass; users who never chose Nunmai must not be nagged."""
    _reset(monkeypatch)
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    not_logged_in = AuthError("Nunmai is not logged into Nunmai Portal.", provider="nous", relogin_required=True)
    ...
    assert "nunmai model" in quiet  # the goal judge still gets the remediation text
