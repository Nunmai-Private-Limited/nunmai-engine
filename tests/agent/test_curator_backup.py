...
def backup_env(monkeypatch, tmp_path):
    """Isolate NUNMAI_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_nunmai_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd nunmai_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under NUNMAI_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level NUNMAI_DIR picks up the tmp HOME."""
    import nunmai_constants
    importlib.reload(nunmai_constants)
