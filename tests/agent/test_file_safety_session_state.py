...
manipulation. Nunmai keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point NUNMAI_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_nunmai_home /
    get_default_nunmai_root) instead of monkeypatching private helpers —
    ...
    NUNMAI_HOME=<root>/profiles/<name> makes get_default_nunmai_root()
