...
def test_auth_surface_names_oauth_vs_api_key_recovery():
    ...
    assert oauth["provider_label"] == "Nunmai Portal"
...
def test_free_tier_block_gets_its_own_code_and_carries_the_sentence():
    ...
    result = _failed_result("auth_permanent", error="HTTP 403: no permissions",
                            free_tier={"kind": "disabled", "message": "Using Nunmai without signing in is switched off."})
    ...
    assert surface["message"] == "Using Nunmai without signing in is switched off."
