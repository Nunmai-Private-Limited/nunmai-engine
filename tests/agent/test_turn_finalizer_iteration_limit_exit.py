...
def test_pending_response_does_not_mask_later_terminal_exit(
    ...
    monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    agent = _LimitAgent()
...
def test_pending_response_records_kanban_timeout(monkeypatch):
    monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "task-123")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("nunmai_cli.kanban_db_connect.connect", lambda: conn)
    monkeypatch.setattr("nunmai_cli.kanban_db_dispatch._record_task_failure", record)
    agent = _LimitAgent()
...
def test_bounded_fallback_records_kanban_failure_when_interrupted(monkeypatch):
    ...
    """
    monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "task-456")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("nunmai_cli.kanban_db_connect.connect", lambda: conn)
    monkeypatch.setattr("nunmai_cli.kanban_db_dispatch._record_task_failure", record)
...
def test_bounded_fallback_records_kanban_failure_when_failed(monkeypatch):
    ...
    """
    monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "task-789")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("nunmai_cli.kanban_db_connect.connect", lambda: conn)
    monkeypatch.setattr("nunmai_cli.kanban_db_dispatch._record_task_failure", record)
...
def test_bounded_fallback_does_not_fire_when_budget_not_exhausted(monkeypatch):
    ...
    """
    monkeypatch.setattr("nunmai_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "task-999")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("nunmai_cli.kanban_db_connect.connect", lambda: conn)
    monkeypatch.setattr("nunmai_cli.kanban_db_dispatch._record_task_failure", record)
...
def test_budget_exhausted_child_does_not_record_parent_kanban_timeout(monkeypatch, scope):
    """An in-process delegate_task child (or cron run) inherits ``NUNMAI_KANBAN_TASK`` from
