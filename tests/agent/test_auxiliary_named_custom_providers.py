...
def _isolate(tmp_path, monkeypatch):
    """Redirect NUNMAI_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test NUNMAI_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("nunmai_cli.auth.resolve_api_key_provider_credentials", return_value={
...
class TestResolveProviderClientNamedCustom:
    ...
    def test_providers_dict_uses_durable_pool_when_no_inline_key(self, tmp_path):
        ...
        auth_path = tmp_path / ".nunmai" / "auth.json"
...
class TestBareNamedAuxCredentialSurvivesAsyncRebuild:
    ...
    def _cfg(self):
        import sys
        return {
            "model": {"provider": "nunmai-gw", "default": "main-model"},
            "providers": {
                "nunmai-gw": {
            ...
            "auxiliary": {"compression": {"provider": "nunmai-gw", "model": "aux-model"}},
        }
    ...
    def test_async_task_client_carries_key_cmd_bearer_and_extra_headers(self, tmp_path):
        ...
        from agent.auxiliary_client import resolve_provider_client
        client, _model = resolve_provider_client("nunmai-gw", "aux-model", async_mode=True, task="compression")
        assert client is not None
    ...
    def test_sync_to_async_rebuild_keeps_credential_and_headers(self, tmp_path):
        ...
        sync_client, model = resolve_provider_client("nunmai-gw", "aux-model", task="compression")
