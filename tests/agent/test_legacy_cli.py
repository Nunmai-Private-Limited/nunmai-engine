"""The packaged ``nunmai-engine`` console script honours argv (#54648).
...
def _run_console_script(monkeypatch, *argv: str):
    ...
    module, func = tomllib.loads(pyproject.read_text(encoding="utf-8"))["project"]["scripts"]["nunmai-engine"].split(":")
    monkeypatch.setattr(sys, "argv", ["nunmai-engine", *argv])
    try:
...
def test_metadata_invocations_never_start_an_agent(argv, monkeypatch, capsys):
    ...
    assert "usage: nunmai-engine" in out or out.startswith("Nunmai Engine v")
