...
pytestmark = pytest.mark.skipif(
    os.environ.get("NUNMAI_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "NUNMAI_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    # Listed in preference order: Nunmai Portal before OpenAI.
