...
class TestParseConfigStringList:
    """#86661: `nunmai config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `nunmai config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    nunmai_home = tmp_path / "da\\ta"
    subprocess_home = nunmai_home / "home"
    subprocess_home.mkdir(parents=True)
    (nunmai_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    monkeypatch.setenv("HOME", str(nunmai_home))
