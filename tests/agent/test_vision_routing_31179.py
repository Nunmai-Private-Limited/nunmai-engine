...
def isolated_home(monkeypatch):
    """Temp NUNMAI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="nunmai_test_31179_")
    nunmai_home = os.path.join(test_home, ".nunmai")
...
def _fresh_modules():
    """Drop cached nunmai modules so each test reloads against current env."""
    _drop_reload_targets()
