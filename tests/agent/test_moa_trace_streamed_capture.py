...
path via ``nunmai chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp NUNMAI_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point NUNMAI_HOME at a temp dir and turn moa.save_traces on."""
    nunmai_home = tmp_path / ".nunmai"
    nunmai_home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))

    # save_moa_turn reads config via nunmai_cli.config.load_config; stub it to
