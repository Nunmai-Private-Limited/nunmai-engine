...
class TestGenerateTitle:
    ...
    def test_expanded_paste_ref_footer_does_not_demote_the_preview(self):
        ...
        ref = "@file:/home/u/.nunmai/attachments/pasted_content_2026-09-18_14-09-43-735_d0ee85.txt"
    ...
    def test_title_language_reads_config(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value=cfg), patch("nunmai_cli.config.load_config_readonly", return_value=cfg):
            assert _title_language() == "French"
        with patch("nunmai_cli.config.load_config", return_value={}), patch("nunmai_cli.config.load_config_readonly", return_value={}):
            assert _title_language() == ""
        with patch("nunmai_cli.config.load_config", side_effect=RuntimeError("bad config")), \
         patch("nunmai_cli.config.load_config_readonly", side_effect=RuntimeError("bad config")):
            assert _title_language() == ""
...
class TestMaybeAutoTitle:
    ...
    def test_kanban_worker_is_named_after_its_card_without_the_llm_thread(self, tmp_path, monkeypatch):
        ...
        from nunmai_cli import kanban_db, kanban_db_connect
    ...
    def test_kanban_worker_with_unreadable_card_falls_back_to_the_task_id(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NUNMAI_KANBAN_TASK", "t_missing")
        db = SessionDB(tmp_path / "state.db")
        db.create_session(session_id="sess-1", source="kanban")
    ...
    def test_delegated_child_of_a_worker_is_not_named_after_the_card(self, tmp_path, monkeypatch):
        """A delegate_task child inherits ``NUNMAI_KANBAN_TASK`` but is not the card's session;
    ...
    def test_model_upgrade_disabled_keeps_derived_title_and_spawns_no_thread(self, tmp_path):
        ...
        with patch("nunmai_cli.config.load_config_readonly", return_value=config), \
        ...
        # (``nunmai sessions retitle-skills``) still asks the model.
        resp = MagicMock()
        resp.choices = [MagicMock()]
