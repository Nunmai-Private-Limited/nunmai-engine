...
def test_httpx_hook_captures_litellm_header_and_clears_when_absent():
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={
            "id": "c", "object": "chat.completion", "created": 0, "model": "nunmai-router",
    ...
    client.chat.completions.create(model="nunmai-router", messages=[{"role": "user", "content": "hi"}])
    assert result_model_fields(agent) == {"requested_model": "nunmai-router", "served_model": "gpt-4o-2024-11-20"}

    served["value"] = ""  # next response has no routing header: the stale value must not survive
    client.chat.completions.create(model="nunmai-router", messages=[{"role": "user", "content": "hi"}])
    assert result_model_fields(agent) == {"requested_model": "nunmai-router", "served_model": None}

    # Nunmai' own fallback route surfaces the same way when no proxy header is present.
