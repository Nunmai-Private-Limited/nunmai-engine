...
A custom provider entry may name a header that carries Nunmai' conversation id so a
...
def _providers(with_header: bool):
    ...
    return patch("nunmai_cli.config.get_compatible_custom_providers", return_value=[entry])
