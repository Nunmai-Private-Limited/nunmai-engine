"""A host that mirrors the served profile into NUNMAI_HOME must not flip launch-home identity.

Nunmai WebUI serves several profiles from one process and, for legacy readers, mirrors the active
turn's profile into ``os.environ["NUNMAI_HOME"]`` while also installing the context-local override.
...
``nunmai_constants.pin_process_nunmai_home`` gives such hosts one stable anchor for all of them.
"""
...
from nunmai_cli.env_loader import _process_nunmai_home
from tools.environments.local import _is_routed_home
from tools.mcp_tool_scope import _server_key


def _under(home, fn):
    """Run *fn* with *home* installed as the task's Nunmai-home override."""
    token = nunmai_constants.set_nunmai_home_override(home)
    try:
        return fn()
    finally:
        nunmai_constants.reset_nunmai_home_override(token)
...
ROUTED = {
    ...
    "env_loader._process_nunmai_home": lambda home: _process_nunmai_home().resolve() != Path(home).resolve(),
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(nunmai_constants, "_PINNED_PROCESS_NUNMAI_HOME", None)
    return launch, served


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_pinned_launch_home_survives_a_mirrored_nunmai_home(homes, monkeypatch, decision):
    launch, served = homes
    ...
    assert _under(served, lambda: _server_key("atlassian")) == (nunmai_constants.nunmai_home_key(served), "atlassian")
    # Process-asset readers keep following the env var: only routing decisions use the pin.
    assert nunmai_constants.get_process_nunmai_home() == served
    assert nunmai_constants.get_routing_process_nunmai_home() == launch


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_unpinned_or_cleared_pin_keeps_nunmai_home_semantics(homes, monkeypatch, decision):
    launch, served = homes
    routed = ROUTED[decision]
    for _ in ("never pinned", "pinned then cleared"):
        ...
        assert routed(served) is True
        assert routed(launch) is False
        monkeypatch.setenv("NUNMAI_HOME", str(served))  # mirrored: the env var IS the launch home
        assert routed(served) is False
        nunmai_constants.pin_process_nunmai_home(launch)
        nunmai_constants.pin_process_nunmai_home(None)
