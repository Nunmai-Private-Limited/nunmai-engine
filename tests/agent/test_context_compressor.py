...
class TestSummarizeToolResultClarify:
    ...
    def test_live_oneshot_producer_is_recognized_as_sentinel(self):
        ...
        from nunmai_cli.oneshot import _oneshot_clarify_callback
...
class TestAuthFailureAborts:
    ...
    def test_missing_provider_api_key_is_terminal_access_failure(self):
        ...
        oauth_err = RuntimeError(
            ...
            "Run `nunmai auth add minimax-oauth` to sign in, or switch to a different provider "
            "with `nunmai model`."
        )
    ...
    def test_missing_provider_api_key_preserves_original_messages(self):
        ...
        err = RuntimeError(
            ...
            "to a different provider with nunmai model."
        )
        with patch(
...
class TestTruncateToolCallArgsJson:
    ...
    def test_pass3_emits_valid_json_for_downstream_provider(self):
        ...
        args_payload = _json.dumps({
            "path": "~/.nunmai/skills/shopping/browser-setup-notes.md",
        ...
        assert parsed["path"] == "~/.nunmai/skills/shopping/browser-setup-notes.md"
