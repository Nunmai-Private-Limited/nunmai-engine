...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("nunmai_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "nunmai doctor" not in lower
...
def test_explanation_persistence_corrupt_cause_never_says_free_space():
    ...
    assert "nunmai doctor" in lower
    assert "free some space" not in lower
    assert "full disk" not in lower


def test_explanation_persistence_corrupt_backups_dir_follows_nunmai_home(monkeypatch, tmp_path):
    """Step 3 must name the backups dir under the ACTIVE home, not ~/.nunmai (#104250).

    Pre-update backups live at ``<nunmai_root>/backups`` (``nunmai_cli/backup.py``), so a
    custom-NUNMAI_HOME deployment told to restore from ``~/.nunmai/backups/`` is misdirected
    mid data-loss incident: that directory may not exist at all, or may hold an unrelated
    install's backups.
    """
    custom_home = tmp_path / "custom-nunmai-home"
    monkeypatch.setenv("NUNMAI_HOME", str(custom_home / "profiles" / "research"))
    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "corrupt"
    )
    assert f"{custom_home / 'backups'}" in out
    assert "~/.nunmai/backups" not in out
...
def test_deleted_wal_cause_is_plain_first_steps_not_a_forensic_runbook():
    """The WAL-generation runbook lives in the logger.error at nunmai_state; the chat reply
    gives the two steps a user can take (stop, doctor) and points at the log."""
    from nunmai_state_errors import PERSISTENCE_ERROR_CAUSES

    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "deleted_wal"
    ).lower()
    assert "deleted_wal" in PERSISTENCE_ERROR_CAUSES
    assert "nunmai gateway stop" in out and "nunmai doctor" in out
    for jargon in ("manifest", "state.db-wal", "sidecar", "header_only", "--inspect-only", "generation"):
        assert jargon not in out, jargon
    assert "~/.nunmai" not in out  # display_nunmai_home(), never a hardcoded path
...
def test_persistence_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path, cause):
    """Every copy-pasteable ``nunmai`` command in a persistence explanation names the profile
    whose store failed — a multi-profile backend serves sessions whose state.db is not the
    process default, and a bare ``nunmai`` follows the sticky active_profile (#105887). The
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / ".nunmai" / "profiles" / "research"))
    ...
    assert f"`nunmai {selector}doctor" in out
    assert "`nunmai doctor" not in out and "`nunmai gateway" not in out
...
def test_explanation_persistence_one_arg_backward_compat():
    ...
    assert out.strip() != ""
    assert "nunmai doctor" in out.lower()
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to nunmai_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from nunmai_state import SessionCompressionInProgressError
    from nunmai_state_errors import CompressionSessionBusyError
    from nunmai_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from nunmai_state import classify_persistence_error
    from nunmai_state_errors import SessionTurnLeaseLostError
...
def test_classify_persistence_error_fts_provenance_order():
    ...
    from nunmai_state import SessionDB, classify_persistence_error
    from nunmai_state_errors import SQLITE_CORRUPT_VTAB, is_fts_scoped_corruption_error
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("NUNMAI_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"NUNMAI_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from nunmai_state import StateDbCorruptError, classify_persistence_error
