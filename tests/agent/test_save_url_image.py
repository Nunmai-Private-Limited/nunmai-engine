...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NUNMAI_HOME under tmp_path.

    ``NUNMAI_ALLOW_PRIVATE_URLS`` opts the loopback test server into private-IP
    ...
    monkeypatch.setenv("NUNMAI_ALLOW_PRIVATE_URLS", "1")
    from tools import url_safety
    url_safety._reset_allow_private_cache()
    (tmp_path / ".nunmai").mkdir()

    # Force the constants/image cache helpers to re-read NUNMAI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("nunmai_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_nunmai_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NUNMAI_HOME — gateway cleanup
