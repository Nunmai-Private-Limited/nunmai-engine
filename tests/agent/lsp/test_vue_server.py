...
(``tsserver/request`` notifications) that Nunmai's generic client does not
run, so it never publishes diagnostics.  Nunmai pins the self-hosting 2.x line
...
def _stage(tmp_path, monkeypatch, vue_version: str, *, js_sdk: bool):
    """Fake ``<NUNMAI_HOME>/lsp`` tree: a staged launcher, the Vue package, optionally a JS TypeScript SDK."""
