...
def test_workspace_removal_releases_only_its_language_servers(entry, mock_pyright, project, tmp_path,
                                                                 monkeypatch):
    """Every in-process workspace-removal path (kanban worktree/scratch cleanup, ``nunmai -w`` exit, the
...
def test_reaper_shuts_down_client_whose_root_was_deleted(mock_pyright, tmp_path):
    """A root deleted outside Nunmai is reaped on the next sweep even though the client is not idle."""
