"""``auth.adopt_external_logins: false`` keeps Nunmai off the Claude Code login (#113023).

Claude Code's OAuth refresh token is single-use: once Nunmai borrows and refreshes it, Nunmai and
Claude Code hold one token family and whichever refreshes first logs the other out. With the opt-out
set, Nunmai must neither read nor refresh ``~/.claude/.credentials.json``, must drop the pool row an
earlier adopting process persisted, and must say so in ``nunmai auth list``.
"""
...
def test_opt_out_never_reads_or_refreshes_claude_code_login(tmp_path, monkeypatch):
    ...
    _write_config(nunmai_home, adopt=None)
    ...
    _write_config(nunmai_home, adopt=False)
    ...
    _write_config(nunmai_home, adopt=True)
