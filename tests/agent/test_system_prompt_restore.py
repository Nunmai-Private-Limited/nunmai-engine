...
class TestStoredPromptReuse:
    ...
    def test_a_swept_pin_row_is_re_pinned_on_the_next_turn(self, tmp_path):
        """``nunmai sessions recover`` from an older build deleted pin rows it did not know about,
...
class TestPerResponseSessionWritePath:
    ...
    Nunmai Studio group chat pre-creates the SQLite row and pre-persists the
