...
* ``urllib``-based ``/models`` catalog discovery probe
  (``nunmai_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from nunmai_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        clean_env.setenv("NUNMAI_CA_BUNDLE", bundle_file)
        probe = MagicMock(return_value=[])
        with patch("nunmai_cli.config.get_compatible_custom_providers", probe):
