...
def _default_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.config_effective.load_user_config_effective", lambda *_a, **_k: {}
    )
...
def _set_segments(monkeypatch, *, on_compaction=False, max_turns=0):
    monkeypatch.setattr(
        "nunmai_cli.config_effective.load_user_config_effective",
        lambda *_a, **_k: {
            "gateway": {
                "telemetry": {
                    "session_segments": {
...
class TestCompactionRotation:
    ...
    def test_segment_metadata_on_rotated_scope(self, coordinator, monkeypatch):
        ...
        assert new_seg.get("nunmai.session.segment") == 1
        assert new_seg.get("nunmai.session.segment_reason") == "compaction"
...
class TestMaxTurnsRotation:
    def test_rotates_after_cap(self, coordinator, monkeypatch):
        ...
        assert sessions[-1]["metadata"].get("nunmai.session.segment_reason") == "max_turns"
...
class TestGatewayRunStaysUnimported:
    ...
    import-time env setup (_NUNMAI_GATEWAY, NUNMAI_QUIET, TERMINAL_CWD := home)
    hangs CLI approvals (#87183) and runs ``nunmai -z`` in $HOME (#95577). A
    ...
    def test_relay_runtime_never_imports_gateway_run(self, monkeypatch) -> None:
        for var in ("TERMINAL_CWD", "NUNMAI_QUIET", "_NUNMAI_GATEWAY"):
...
leaked = {v: os.environ[v] for v in ("TERMINAL_CWD", "NUNMAI_QUIET", "_NUNMAI_GATEWAY") if v in os.environ}
