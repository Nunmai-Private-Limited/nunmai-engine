...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # _get_platform_tools (a non-configurable platform toolset in nunmai-cli's
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("NUNMAI_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown NUNMAI_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("NUNMAI_MODEL", "seed/model")
    monkeypatch.setenv("NUNMAI_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from nunmai_state_common import _RECOVERABLE_END_REASONS
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Nunmai TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("nunmai_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "nunmai_cli.commands_completion.SlashCommandCompleter",
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("nunmai_state_registry.acquire", ProfileDB2)
...
def test_reload_env_rpc_calls_nunmai_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.nunmai/.env
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from nunmai_cli.mem_trim import trim_memory` at call time.
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from get_nunmai_home() (live
    # env var), always targeting NUNMAI_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    monkeypatch.setattr(cli, "_nunmai_home", tmp_path)
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch, tmp_path):
    ...
    try:
        ...
        monkeypatch.setattr("nunmai_cli.mem_trim.trim_memory", _inspect_trim_frame)
