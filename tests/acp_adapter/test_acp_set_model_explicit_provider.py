...
def test_explicit_provider_prefix_skips_detection(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.models.detect_provider_for_model", hijack)
    assert NunmaiACPAgent._resolve_model_selection("anthropic:claude-sonnet-5", "anthropic") == (
        "anthropic", "claude-sonnet-5")
    assert calls == []


def test_bare_name_still_uses_detection(monkeypatch):
    monkeypatch.setattr("nunmai_cli.models.detect_provider_for_model", lambda m, c: ("deepseek", m))
    assert NunmaiACPAgent._resolve_model_selection("deepseek-flash", "anthropic") == ("deepseek", "deepseek-flash")
