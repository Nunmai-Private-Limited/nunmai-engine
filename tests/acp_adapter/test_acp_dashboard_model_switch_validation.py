...
def _acp_agent():
    ...
    return NunmaiACPAgent(session_manager=_SM()), made
...
def test_acp_and_dashboard_reject_what_switch_model_rejects(monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.model_switch.switch_model", lambda **_kw: rejected)
...
def test_acp_switch_model_carries_the_live_agent_toolsets_into_the_rebuild(monkeypatch):
    ...
    agent, made = _acp_agent()
    agent._switch_model(_state(enabled_toolsets=["nunmai-acp", "mcp-demo-search"], disabled_toolsets=["browser"]),
                        "claude-sonnet-5")

    assert made["enabled_toolsets"] == ["nunmai-acp", "mcp-demo-search"]
