...
→ ``resolve_reasoning_config`` chain on the per-test ``NUNMAI_HOME``; only the agent constructor and
...
def acp_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nunmai_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kwargs: None)

    def _write_config(cfg: dict) -> None:
        (Path(os.environ["NUNMAI_HOME"]) / "config.yaml").write_text(yaml.safe_dump(cfg), encoding="utf-8")
