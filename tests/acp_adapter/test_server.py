"""Tests for acp_adapter.server — NunmaiACPAgent ACP server."""
...
def agent(mock_manager):
    """NunmaiACPAgent backed by a mock session manager."""
    return NunmaiACPAgent(session_manager=mock_manager)
...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        acp_agent = NunmaiACPAgent(session_manager=manager)
        ...
        with (
            patch("nunmai_cli.inventory.load_picker_context", return_value=picker_context),
            patch("nunmai_cli.inventory.build_models_payload", return_value=payload),
        ):
...
class TestPrompt:
    ...
    async def test_prompt_binds_session_id_into_subprocess_env(self, agent, mock_manager):
        ...
        leaving the ``NUNMAI_SESSION_ID`` ContextVar bound to the explicit ""
        ...
        empty ``NUNMAI_SESSION_ID`` instead of the session's own id.
        """
        ...
        def _run(*args, **kwargs):
            ...
            captured["child"] = _make_run_env({}).get("NUNMAI_SESSION_ID")
    ...
    async def test_prompt_fails_tool_calls_left_open_before_responding(self, agent, mock_manager, executor_raises):
        ...
        with patch.object(NunmaiACPAgent, "_run_agent_turn", side_effect=_turn):
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["nunmai-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["nunmai-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["nunmai-acp", "mcp-srv"]
