...
class TestCreateSession:
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_platform.host.runtime._wsl_detected", True)
        monkeypatch.setattr(
    ...
    def test_make_agent_uses_session_cwd_during_init_and_stamps_runtime(
        ...
        monkeypatch.setattr("nunmai_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
    ...
    def test_make_agent_prefers_passed_toolsets_over_config_servers(self, monkeypatch):
        ...
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        monkeypatch.setattr("nunmai_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        ...
        manager._make_agent(
            session_id="rebuilt", cwd=".", enabled_toolsets=["nunmai-acp", "mcp-acp-server"], disabled_toolsets=["browser"],
        )

        assert "mcp-cfg-server" in seen[0]["enabled_toolsets"] and seen[0]["disabled_toolsets"] is None
        assert (seen[1]["enabled_toolsets"], seen[1]["disabled_toolsets"]) == (["nunmai-acp", "mcp-acp-server"], ["browser"])

    @pytest.mark.parametrize("config, offered, withheld", [
        # agent.disabled_toolsets, in the JSON-string shape `nunmai config set` stores (#74582).
    ...
    def test_fresh_agent_tool_surface_honours_toolset_config(self, monkeypatch, config, offered, withheld):
        ...
        monkeypatch.setattr("run_agent.AIAgent", FakeAgent)
        monkeypatch.setattr("nunmai_cli.config.load_config", lambda: {"model": {"default": "m"}, **config})
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        monkeypatch.setattr("nunmai_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        ...
        assert set(resolve_toolset(withheld)) <= surface(["nunmai-acp"])  # non-vacuous: offered by default
    ...
    @pytest.mark.parametrize("acp_toolsets, expected_mcp", [
        ...
        (["nunmai-acp", "alpha"], {"mcp-alpha"}),       # listed server names are an allowlist
        (["nunmai-acp", "no_mcp"], set()),              # the no_mcp sentinel drops them all
    ])
    ...
    def test_make_agent_surfaces_the_provider_resolution_failure(self, monkeypatch):
        ...
        def _no_creds(**_kw):
            raise RuntimeError("No Codex credentials stored. Run `nunmai auth add openai-codex`")

        class BareFails:
            def __init__(self, **kwargs):
                raise RuntimeError("No LLM provider configured. Run `nunmai setup`")

        class BareWorks:
            def __init__(self, **kwargs):
                self.kwargs = kwargs

        monkeypatch.setattr("nunmai_cli.config.load_config", lambda: {"model": {"default": "m", "provider": "openai-codex"}})
        monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _no_creds)
        monkeypatch.setattr("nunmai_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        manager = SessionManager(db=None)
...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self):
        ...
        assert acp_session._normalize_cwd_for_compare(
            "/nonexistent-nunmai-test/x/../y"
        ) == "/nonexistent-nunmai-test/y"
