...
Driven through the real ``NunmaiACPAgent.prompt()`` with a real ``AIAgent`` and ``SessionDB``
...
def acp(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_DISABLE_PLUGINS", "1")
    monkeypatch.setenv("NO_PROXY", "127.0.0.1,localhost")
    provider = _LoopbackProvider()

    import acp_adapter.session as acp_session
    import nunmai_cli.config as cli_config
    import nunmai_cli.mcp_startup as mcp_startup
    import nunmai_cli.runtime_provider as runtime_provider
    ...
    from acp_adapter.server import NunmaiACPAgent, TextContentBlock
    from acp_adapter.session import SessionManager
    from nunmai_state import SessionDB

    db_path = tmp_path / "state.db"
    db = SessionDB(db_path)
    server = NunmaiACPAgent(session_manager=SessionManager(db=db))
    conn = _RecordingConn()
...
def test_acp_refusal_closes_the_turn_and_is_not_replayed_into_the_next_prompt(acp):
    ...
    Invariant: the durable tail after a failed turn is a Nunmai-authored assistant row (never
