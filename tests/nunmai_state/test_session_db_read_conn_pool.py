...
``nunmai_cli.sqlite_safe_read``'s registry permanently over-counted as well.
...
def _live_count(path) -> int:
    ...
    import nunmai_cli.sqlite_safe_read as mod
...
def test_read_open_failure_backs_off_but_recovers(db):
    ...
    from nunmai_state import _READ_OPEN_RETRY_SECONDS
...
def test_peak_live_connections_bounded_under_simultaneous_burst(db):
    ...
    """
    from nunmai_state import _READ_POOL_MAX
...
def test_peak_is_bounded_across_two_SessionDBs_on_one_path(db):
    ...
    from nunmai_state import SessionDB, _READ_POOL_MAX
...
# And Nunmai's SQLite descriptors are only ever a share of the fd table: the
...
def test_peak_is_bounded_across_many_database_files(tmp_path):
    ...
    from nunmai_state import SessionDB, _READ_POOL_MAX
    from nunmai_state_readpool import _READ_POOL_PROCESS_MAX
    ...
    try:
        ...
        for d in dbs:
            d.close()
        assert nunmai_state_readpool._process_read_permits.acquire(blocking=False), (
            "close() stranded a process permit"
        )
        nunmai_state_readpool._process_read_permits.release()
...
def test_no_read_connection_is_opened_without_descriptor_headroom(db, monkeypatch):
    ...
    """
    import nunmai_state_readpool as readpool
...
def test_fd_soft_limit_fails_open_for_importable_resource_stub(monkeypatch):
    ...
    assert nunmai_state_readpool._fd_soft_limit() is None
    assert nunmai_state_readpool._fd_headroom_ok() is True
...
def test_duplicate_handles_on_one_path_are_reported(db, caplog):
    ...
    from nunmai_state import SessionDB
    from nunmai_state_readpool import _HANDLES_PER_PATH_WARN
...
def test_handle_diagnostics_unavailable_does_not_block_database(tmp_path, monkeypatch):
    ...
    module_sys = SimpleNamespace(**vars(nunmai_state.sys))
    module_sys._getframe = deny_frame_access
    monkeypatch.setattr(nunmai_state, "sys", module_sys)
...
def test_failed_initialization_does_not_register_duplicate_writer_handle(db, monkeypatch):
    ...
    budget = nunmai_state_readpool._read_budget_for(db.db_path)
    registered = []
