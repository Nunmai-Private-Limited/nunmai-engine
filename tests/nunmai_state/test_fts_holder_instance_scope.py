...
Nunmai instances: a main gateway (user ``ubuntu``,
NUNMAI_HOME=/home/ubuntu/.nunmai) and a demo gateway (user ``demo``,
NUNMAI_HOME=/home/demo/.nunmai).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_nunmai``.
The demo process's argv matches the Nunmai patterns exactly, so the fallback
...
rebuild in ``nunmai_state_schema._recover_stale_fts`` was deferred 42 times
...
nunmai); a genuine second instance with a DIFFERENT NUNMAI_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Nunmai
...
# token lives under /home/demo/.nunmai, the binary name matches the Nunmai
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.nunmai/nunmai-engine/nunmai",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + nunmai launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.nunmai/nunmai-engine/venv/bin/python",
    "/home/demo/.nunmai/nunmai-engine/nunmai_cli/main.py",
    "gateway",
]

# A Nunmai-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["nunmai", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Nunmai home → not a holder of our state.db."""
        # A real ``.nunmai`` home, as on the field host this test is drawn from: the install
        # location can only identify a home that is itself part of an install layout.
        db_path = tmp_path / ".nunmai" / "state.db"
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["nunmai", f"--db={db_path}", "gateway"],
            ["nunmai", "checkpoint", f"{db_path}-wal"],
            ["nunmai", "--home", our_home, "gateway"],
        ):
            assert nunmai_state_holders._looks_like_nunmai(argv) or argv[0] == "nunmai"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
    ...
    def test_install_root_argv_still_holds_a_profile_store_under_it(self, tmp_path, monkeypatch):
        ...
        multiplexer = ["nunmai", "--home", str(root), "gateway", "run"]
        other_install = ["nunmai", "--home", "/home/demo/.nunmai", "gateway", "run"]

        _install_fake_proc(monkeypatch, db_path.parent, unreadable_pids=(222,))
        _install_fake_argv(monkeypatch, {222: multiplexer})
        assert [pid for pid, _ in nunmai_state_holders.foreign_state_db_holders(db_path)] == [222]
    ...
    def test_shared_binary_plus_another_profile_selection_is_dismissed(self, tmp_path, monkeypatch):
        ...
        Every nunmai process on a normal host runs ``<root>/venv/bin/nunmai``; counting that token
        as proof made ``nunmai -p other chat -q`` an uninspectable holder of every OTHER profile's
        ...
        db_path = root / "profiles" / "b" / "state.db"
        db_path.parent.mkdir(parents=True)
        shared_binary = str(root / "venv" / "bin" / "nunmai")
    ...
    def test_token_naming_another_profiles_store_is_dismissal_evidence(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["nunmai", f"--db={other_store}", "sessions", "optimize"],
            [str(root / "venv" / "bin" / "nunmai"), "sessions", str(other_store)],
        ):
            _install_fake_proc(monkeypatch, db_path.parent, unreadable_pids=(222,))
            _install_fake_argv(monkeypatch, {222: argv})
            assert nunmai_state_holders.foreign_state_db_holders(db_path) == [], argv

    def test_unrelated_install_under_a_non_nunmai_profiles_tree_stays_dismissed(
        ...
        unrelated Nunmai install living under it was counted as a holder — the literal two-instance
        shape #92401 was filed about. The canonical ``named_profile_home`` predicate requires the
        parent to be a real Nunmai home.
        """
        work = tmp_path / "work"
        db_path = work / "profiles" / "b" / "state.db"
        db_path.parent.mkdir(parents=True)
        unrelated_home = work / "demo" / ".nunmai"
        unrelated_home.mkdir(parents=True)

        _install_fake_proc(monkeypatch, db_path.parent, unreadable_pids=(222,))
        _install_fake_argv(
            monkeypatch, {222: ["nunmai", "--nunmai-home", str(unrelated_home), "gateway", "run"]})
        assert nunmai_state_holders.foreign_state_db_holders(db_path) == []

    def test_custom_home_is_not_dismissed_by_the_install_location(self, tmp_path, monkeypatch):
        """A store at a custom NUNMAI_HOME is SERVED BY the binary under ``~/.nunmai``.
        ...
        _install_fake_proc(monkeypatch, db_path.parent, unreadable_pids=(222,))
        _install_fake_argv(
            monkeypatch, {222: ["/home/u/.nunmai/venv/bin/nunmai", "gateway", "run"]})
        holders = nunmai_state_holders.foreign_state_db_holders(db_path)
