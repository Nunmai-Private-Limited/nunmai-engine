...
def write_second_generation(db_path: Path, n_rows: int) -> int:
    ...
    (as any non-nunmai opener would), write ``n_rows`` messages and checkpoint them into the main file.
...
_GATEWAY_CHILD = textwrap.dedent(
    ...
    from pathlib import Path
    repo, nunmai_home, db_path = sys.argv[1], sys.argv[2], sys.argv[3]
    sys.path.insert(0, repo)
    os.environ["NUNMAI_HOME"] = nunmai_home
    import nunmai_state_wal
    if nunmai_state_wal.is_sqlite_wal_reset_vulnerable():
        nunmai_state_wal.is_sqlite_wal_reset_vulnerable = lambda version_info=None: False
    nunmai_state_wal.resolve_journal_mode = lambda: "wal"
    from nunmai_state import DeletedWalGenerationError, SessionDB
...
def gateway_writer(tmp_path: Path):
    ...
    repo_root = os.path.dirname(os.path.abspath(nunmai_state.__file__))
    ...
    with stderr_path.open("w", encoding="utf-8") as stderr:
        proc = subprocess.Popen(
            [sys.executable, "-c", _GATEWAY_CHILD, repo_root, str(nunmai_home), str(path)],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr,
            text=True, encoding="utf-8", bufsize=1,
            env={**os.environ, "NUNMAI_STATE_DB_GUARD_BYPASS": "1"},
        )
