...
Before, ``database is locked (another Nunmai process held the state.db write lock for over 60s)``
identified the victim only; every Nunmai process has the DB open, so the descriptor scan could not
...
from nunmai_state_lockowners import parse_proc_locks, state_db_write_lock_holders
