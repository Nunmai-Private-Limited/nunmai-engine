...
unguarded ``fcntl.F_RDLCK`` read at import time. ``nunmai_state`` imports this module at module
level, so the ``AttributeError`` killed every importer — ``nunmai serve`` (Desktop backend
"exited before port announcement (1)"), ``nunmai doctor``, the gateway, cron — before
...
def _reimport_with_fcntl(monkeypatch, stub: types.ModuleType | None):
    ...
    monkeypatch.delitem(sys.modules, "nunmai_state_lockguard", raising=False)
    ...
    return importlib.import_module("nunmai_state_lockguard")
