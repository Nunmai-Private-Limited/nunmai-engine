...
A raw single-process ``pytest tests/nunmai_cli/`` used to accumulate every
...
* ``nunmai_state_guard._register_test_instance`` adds every successfully
  constructed SessionDB to a WeakSet registry when the
  ``NUNMAI_TEST_ISOLATION`` marker is set (test-isolation runs only).
...
def test_constructed_sessiondb_is_registered(tmp_path):
    ...
    try:
        assert db in nunmai_state_guard._test_instance_registry
    finally:
        db.close()
