"""Plain-language contracts for nunmai_state user-facing errors (CLI UX message campaign, cluster D)."""
...
def test_db_unavailable_is_one_line_for_chat_surfaces_by_default():
    nunmai_state._set_last_init_error("OperationalError: database is locked")
    try:
...
def test_db_unavailable_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    for text in (no_cause, network):
        assert f"`nunmai {selector}doctor`" in text and "`nunmai doctor`" not in text
...
def test_db_unavailable_network_drive_cause_does_not_offer_doctor_fix():
    ...
    cannot host SQLite's WAL; `nunmai doctor --fix` cannot repair a mount, so it
