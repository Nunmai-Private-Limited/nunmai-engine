...
``nunmai_state_ids.new_session_id`` and the id it produces is what lost-and-found salvage classifies
...
from nunmai_state_ids import SESSION_ID_PATTERN, new_session_id
...
def test_minted_ids_are_what_salvage_classifies_as_session_ids(hex_len, expected_re):
    from nunmai_cli.session_lost_and_found import _is_session_id
    ...
    assert SESSION_ID_PATTERN is importlib.import_module("nunmai_cli.session_lost_and_found").SESSION_ID_PATTERN
