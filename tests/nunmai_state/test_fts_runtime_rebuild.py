...
import nunmai_state
import nunmai_state_holders
import nunmai_state_schema
from nunmai_state import SessionDB
from nunmai_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from nunmai_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m nunmai_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "nunmai-engine.service"),
            ("grep", "nunmai-engine", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m nunmai_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/nunmai-engine/.venv/bin/nunmai gateway"),
            ("python3", "/opt/nunmai-engine/tools/check_state.py"),
            ("nunmai-monitor", "gateway"),
            ("nunmaictl", "serve"),
            ("python3", "worker.py", "nunmai_cli.main"),
            ("python3", "-m", "other.module", "nunmai_cli.main"),
            ("python3", "-c", "nunmai_cli.main"),
            ("python3", "-Icprint('nunmai_cli.main')", "nunmai_cli/main.py"),
        ),
    )
    def test_uninspectable_non_nunmai_process_is_not_a_holder(self, argv):
        assert not nunmai_state_holders._looks_like_nunmai(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/nunmai", "gateway"),
            ("/usr/local/bin/nunmai-engine", "serve"),
            ("/usr/local/bin/nunmai-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "nunmai_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "nunmai_cli.main", "gateway"),
            ("/usr/bin/python3", "-mnunmai_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "nunmai_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "nunmai_cli.main"),
            (
                "/opt/nunmai-engine/.venv/bin/python",
                "/opt/nunmai-engine/nunmai_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "nunmai_cli/main.py", "gateway"),
            ("python3", "/opt/nunmai-engine/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_nunmai_process_remains_a_holder(self, argv):
        assert nunmai_state_holders._looks_like_nunmai(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(nunmai_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(nunmai_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(nunmai_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(nunmai_state_holders.os, "stat", _stat)

        holders = nunmai_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert nunmai_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Nunmai process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Nunmai
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00nunmai_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(nunmai_state_holders, "_read_proc_argv", _fake_argv)

        holders = nunmai_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "nunmai_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from nunmai_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(nunmai_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            nunmai_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "nunmai_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(nunmai_state_schema.time, "time", lambda: clock[0])

        from nunmai_cli.doctor_state import _render_state_db_stats
        from nunmai_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(nunmai_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(nunmai_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m nunmai_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + nunmai_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = nunmai_state_schema._FTS_STALE_RETRY_MAX_SECONDS
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="nunmai_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
