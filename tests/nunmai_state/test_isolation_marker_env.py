...
child at the same moment the child lost the ``NUNMAI_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``NUNMAI_TEST_ISOLATION`` is Nunmai's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``nunmai_state`` honors it as a
...
try:
    ...
    fired = True
print(json.dumps({{
    "armed": nunmai_state_guard._running_under_pytest(),
    "fired": fired,
}}))
"""
...
def test_conftest_exports_the_marker():
    ...
    assert os.environ.get("NUNMAI_TEST_ISOLATION"), (
        "NUNMAI_TEST_ISOLATION must be exported by tests/conftest.py so "
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_TEST_ISOLATION", "/tmp/some-isolation-root")
    assert nunmai_state_guard._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    monkeypatch.delenv("PYTEST_VERSION", raising=False)
    monkeypatch.delenv("NUNMAI_TEST_ISOLATION", raising=False)
    assert nunmai_state_guard._running_under_pytest() is False
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, NUNMAI_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(NUNMAI_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: NUNMAI_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        NUNMAI_TEST_ISOLATION="/tmp/pytest-isolation-root",
