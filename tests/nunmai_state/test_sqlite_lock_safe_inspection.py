"""POSIX advisory locks must survive Nunmai' own database inspection.
...
Nunmai used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `nunmai sessions
...
out across Nunmai' inspection calls.
"""
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_nunmai_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import nunmai_cli.sqlite_safe_read as ssr
...
def test_repair_connections_are_tracked_for_byte_probe_safety(tmp_path, clean_registry, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    from nunmai_state import SessionDB
    from nunmai_state_repair import _connect_repair_durable, _repair_conn
...
def test_byte_probe_never_cancels_the_repair_exclusion(tmp_path, clean_registry):
    """A live repair's EXCLUSIVE lock must survive Nunmai' own inspection (#63386).
    ...
    """
    import nunmai_state_repair as repair
