...
history. ``nunmai_state_health`` is now the one latch: readers, writers and the readiness
...
from nunmai_state import SessionDB, StateDbCorruptError
from nunmai_state_errors import classify_persistence_error
...
def test_readiness_state_db_and_session_store_agree_on_corrupt(_isolate_nunmai_home):
    from gateway.readiness import collect_runtime_readiness
    from nunmai_constants import get_nunmai_home

    path = get_nunmai_home() / "state.db"
    _seed(path, count=1)
...
def test_readiness_probe_latches_a_file_that_is_not_a_database(_isolate_nunmai_home):
...
@pytest.fixture
def client(monkeypatch, _isolate_nunmai_home):
    try:
    ...
    import nunmai_state
    from nunmai_cli.web_routers import profiles as profiles_routes
    ...
    (get_nunmai_home() / "config.yaml").write_text("{}\n", encoding="utf-8")
    c = TestClient(app)
