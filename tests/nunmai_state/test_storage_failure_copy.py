...
@pytest.mark.parametrize(
    ...
    [
        ...
        (sqlite3.OperationalError("attempt to write a readonly database"), "storage_readonly", "nunmai doctor --fix"),
        (sqlite3.DatabaseError("database disk image is malformed"), "storage_corrupt", "nunmai doctor --fix"),
        (OSError(28, "No space left on device"), "disk_full", "Free some disk space"),
        (None, "storage_unavailable", "nunmai doctor --fix"),
    ],
)
...
def test_action_command_is_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    whose state.db is not the process default; a bare ``nunmai`` follows active_profile)."""
    ...
    for exc in (sqlite3.OperationalError("database is locked"), sqlite3.DatabaseError("malformed"), None):
        ...
        assert f"`nunmai {selector}" in action and "`nunmai doctor" not in action and "`nunmai gateway" not in action
