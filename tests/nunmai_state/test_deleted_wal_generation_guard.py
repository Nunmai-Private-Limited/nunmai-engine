...
from nunmai_state_dbfile import _pread_db_header, iter_deleted_sqlite_sidecar_holders
from tests.nunmai_state._wal_generation_harness import (
...
def test_second_sessiondb_open_refuses_through_symlinked_home(tmp_path, force_wal):
    ...
    real_home = tmp_path / "nunmai-real"
    real_home.mkdir()
    link_home = tmp_path / "nunmai-link"
...
def test_iter_holders_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(nunmai_state_dbfile.os, "readlink", fake_readlink)
    try:
...
def test_iter_holders_ignores_descriptor_closed_during_scan(tmp_path, monkeypatch, vanish_errno):
    ...
    monkeypatch.setattr(nunmai_state_dbfile.sys, "platform", "linux")
    monkeypatch.setattr(
        nunmai_state_dbfile,
    ...
    monkeypatch.setattr(nunmai_state_dbfile.os, "stat", stat_vanished)
...
def test_write_path_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(nunmai_state_readpool.os, "readlink", fake_readlink)
    try:
