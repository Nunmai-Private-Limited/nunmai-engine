...
Regression: the old implementation wrote ``nunmai_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``nunmai sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.nunmai/sessions/saved/`` and prints
...
def nunmai_home(tmp_path, monkeypatch):
    ...
    # Clear any cached nunmai_home computation
    import nunmai_constants
    if hasattr(nunmai_constants, "_nunmai_home_cache"):
        nunmai_constants._nunmai_home_cache = None
    return home
...
def test_save_conversation_writes_under_nunmai_home(nunmai_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.nunmai/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the NUNMAI_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "nunmai_constants"]:
    ...
    cli.NunmaiCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("nunmai_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.nunmai/sessions/saved/
    saved_dir = nunmai_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("nunmai_conversation_*.json"))
    ...
    assert "nunmai --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(nunmai_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "nunmai_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(nunmai_home, capsys):
    ...
    cli.NunmaiCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(nunmai_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "nunmai_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.NunmaiCLI.save_conversation(stub, "/save pdf")
