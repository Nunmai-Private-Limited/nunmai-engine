...
minimal ``NunmaiCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def nunmai_home(tmp_path, monkeypatch):
    """Isolated NUNMAI_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".nunmai"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("NUNMAI_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves NUNMAI_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal NunmaiCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, nunmai_home):
        ...
        with patch("nunmai_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, nunmai_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, nunmai_home):
        ...
        src = inspect.getsource(NunmaiCLI.chat)
