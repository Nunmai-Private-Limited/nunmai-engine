...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_nunmai_home(), so
        # point NUNMAI_HOME at the temp dir (the _nunmai_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "nunmai_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in NUNMAI_HOME/config.yaml
    ...
    install whose NUNMAI_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read NUNMAI_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh NUNMAI_HOME with NO config.yaml (managed/desktop first launch).
        nunmai_home = tmp_path / ".nunmai"
        nunmai_home.mkdir()
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
