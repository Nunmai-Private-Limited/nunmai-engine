...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = NunmaiCLI.__new__(NunmaiCLI)
        host.model = "opus"
        ...
        with patch.object(NunmaiCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(NunmaiCLI, "_is_session_yolo_active", return_value=False):
            text = NunmaiCLI._build_status_bar_text(host, width=width)
...
class TestCommandRegistration:
    ...
    def test_verbose_cycle_releases_focus_view(self):
        ...
        with patch("cli.save_config_value", return_value=True), patch("cli._cprint"):
            NunmaiCLI._toggle_verbose(host)
