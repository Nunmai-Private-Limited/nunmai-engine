...
def _isolate(tmp_path, monkeypatch):
    """Isolate NUNMAI_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal NunmaiCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("nunmai_cli.banner.build_welcome_banner"):
        from cli import NunmaiCLI
        obj = NunmaiCLI.__new__(NunmaiCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nunmai-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Nunmai' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Nunmai' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Nunmai' minimum context length."""
