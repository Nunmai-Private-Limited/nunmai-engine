...
from nunmai_cli import model_setup_flows
...
def test_nunmai_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in NunmaiCLI.__init__")

    monkeypatch.setattr("nunmai_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    nunmai_main._model_flow_custom({})
...
def test_auto_provider_name_localhost():
    from nunmai_cli.main_provider_setup import _auto_provider_name
