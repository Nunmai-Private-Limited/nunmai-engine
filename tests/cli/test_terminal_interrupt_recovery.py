...
The recovery path lives in ``NunmaiCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A NunmaiCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(NunmaiCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise NunmaiCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("nunmai_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        bare_cli._force_full_redraw = MagicMock()
        with patch(
            "nunmai_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "nunmai_cli.curses_ui.flush_stdin",
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        # process_loop's post-turn finally block is NunmaiCLI._tui_after_turn.
        src = inspect.getsource(NunmaiCLI._tui_after_turn)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(NunmaiCLI, "_recover_terminal_after_interrupt")
        assert callable(NunmaiCLI._recover_terminal_after_interrupt)
