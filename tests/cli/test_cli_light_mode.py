...
class TestLightModeDetection:
    def test_nunmai_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("NUNMAI_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_nunmai_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("NUNMAI_LIGHT", "0")
        # Also blank out other signals so nothing else flips it light.
        monkeypatch.delenv("NUNMAI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("NUNMAI_TUI_THEME", raising=False)
        monkeypatch.delenv("NUNMAI_TUI_BACKGROUND", raising=False)
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False

    def test_theme_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("NUNMAI_LIGHT", raising=False)
        monkeypatch.delenv("NUNMAI_TUI_LIGHT", raising=False)
        monkeypatch.setenv("NUNMAI_TUI_THEME", "light")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_light(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("NUNMAI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("NUNMAI_TUI_THEME", raising=False)
        monkeypatch.setenv("NUNMAI_TUI_BACKGROUND", "#FFFFFF")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_dark(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("NUNMAI_TUI_LIGHT", raising=False)
        monkeypatch.delenv("NUNMAI_TUI_THEME", raising=False)
        monkeypatch.setenv("NUNMAI_TUI_BACKGROUND", "#052819")
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False
...
class TestLightModeRemap:
    def test_remap_no_op_in_dark_mode(self, cli_mod, monkeypatch):
        ...
        assert cli_mod._maybe_remap_for_light_mode("#EFE9D7") == "#EFE9D7"
    ...
    def test_remap_known_dark_color(self, cli_mod, monkeypatch):
        ...
        assert cli_mod._maybe_remap_for_light_mode("#EFE9D7") == "#1A1A1A"
        assert cli_mod._maybe_remap_for_light_mode("#6BE7C8") == "#9A6B00"
...
class TestSkinConfigHook:
    ...
    def test_badge_preserves_its_paired_colors_in_light_mode(
        ...
        from nunmai_cli.skin_engine import (
        ...
        try:
            ...
            background = skin.colors.get("status_bar_strong", skin.colors.get("banner_title", "#6BE7C8"))
            foreground = skin.colors.get("status_bar_bg", "#052819")
    ...
    def test_hook_installed(self, cli_mod):
        from nunmai_cli.skin_engine import SkinConfig

        assert getattr(SkinConfig, "_nunmai_light_mode_hook_installed", False) is True
    ...
    def test_skin_color_remaps_through_wrapper_in_light_mode(
        ...
        skin = SkinConfig(
            name="test",
            colors={"banner_text": "#EFE9D7", "response_border": "#6BE7C8"},
        )
    ...
    def test_skin_color_passthrough_in_dark_mode(self, cli_mod, monkeypatch):
        ...
        skin = SkinConfig(name="test", colors={"banner_text": "#EFE9D7"})
        assert skin.get_color("banner_text") == "#EFE9D7"
...
_CHILD_SRC = r"""
import sys, os
sys.path.insert(0, os.environ["NUNMAI_REPO"])
import cli
...
def _run_osc11_child(reply_fn, repo_root, timeout=8.0):
    ...
    env = dict(_os.environ, NUNMAI_REPO=str(repo_root))
