"""Tests for nunmai_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import nunmai_logging
# Use whatever RotatingFileHandler class nunmai_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. nunmai_logging
...
from nunmai_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    nunmai_logging._logging_initialized = False
    ...
    nunmai_logging._reset_queued_handlers()
    ...
    nunmai_logging._install_session_record_factory()
    yield
    ...
    nunmai_logging._logging_initialized = False
    nunmai_logging.clear_session_context()


@pytest.fixture
def nunmai_home(tmp_path, monkeypatch):
    """Provide an isolated NUNMAI_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_nunmai_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["NUNMAI_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, nunmai_home):
        log_dir = nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        assert log_dir == nunmai_home / "logs"
        assert log_dir.is_dir()



    def test_idempotent_no_duplicate_handlers(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)  # second call — should be no-op

        agent_handlers = [
            h for h in nunmai_logging._queued_file_handlers
    ...
    def test_writes_to_agent_log(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)

        test_logger = logging.getLogger("test_nunmai_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        nunmai_logging.flush_log_queue()

        agent_log = nunmai_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, nunmai_home, tmp_path):
        ...
        profile_home.mkdir()
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        assert nunmai_logging.enable_profile_log_routing(
            [nunmai_home, profile_home]
        ) is True
    ...
    def test_routed_records_follow_their_own_profiles_redaction_policy(
            self, nunmai_home, tmp_path, monkeypatch, launch_redacts, routed_opt_out, routed_redacted):
        ...
        if routed_opt_out == "config":
            ...
            (routed / ".env").write_text("NUNMAI_REDACT_SECRETS=false\n", encoding="utf-8")
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        assert nunmai_logging.enable_profile_log_routing([nunmai_home, routed]) is True
        ...
        token = set_nunmai_home_override(routed)
        try:
        ...
        assert (launch_secret not in (nunmai_home / "logs" / "agent.log").read_text()) is launch_redacts

    def test_release_profile_log_handlers_closes_only_deleted_profile(self, nunmai_home, tmp_path):
        ...
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        assert nunmai_logging.enable_profile_log_routing(
            [nunmai_home, deleted_home, other_home]
        ) is True

        logger = logging.getLogger("agent.profile-delete-log-release")
        token = set_nunmai_home_override(deleted_home)
        try:
            logger.warning("deleted profile log handles")
        finally:
            reset_nunmai_home_override(token)
        token = set_nunmai_home_override(other_home)
        try:
        ...
        routers = [
            handler for handler in nunmai_logging._queued_file_handlers
            if isinstance(handler, nunmai_logging._ProfileRoutingFileHandler)
        ]
        ...
        assert nunmai_logging.release_profile_log_handlers(deleted_home) == 2
    ...
    def test_a_second_home_routes_instead_of_stacking_an_unfiltered_handler(self, nunmai_home, tmp_path):
        ...
        profile_home.mkdir()
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        nunmai_logging.setup_logging(nunmai_home=profile_home)

        assert not [h for h in nunmai_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)], (
        ...
        a_log = (nunmai_home / "logs" / "agent.log").read_text()
    ...
    def test_setup_for_an_already_routed_home_adds_no_duplicate_writer(self, nunmai_home, tmp_path):
        ...
        profile_home.mkdir()
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        assert nunmai_logging.enable_profile_log_routing([nunmai_home, profile_home]) is True
        nunmai_logging.setup_logging(nunmai_home=profile_home)
        ...
        assert "once please" not in (nunmai_home / "logs" / "agent.log").read_text()

    def test_a_component_log_added_after_routing_is_routed_too(self, nunmai_home, tmp_path):
        ...
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        nunmai_logging.setup_logging(nunmai_home=profile_home)
        nunmai_logging.setup_logging(nunmai_home=nunmai_home, mode="gateway")
        ...
        a_log = (nunmai_home / "logs" / "gateway.log").read_text()
    ...
    def test_explicit_params_override_config(self, nunmai_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (nunmai_home / "config.yaml").write_text(yaml.dump(config))

        nunmai_logging.setup_logging(nunmai_home=nunmai_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_receives_gateway_records(self, nunmai_home):
        ...
        gw_log = nunmai_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, nunmai_home):
...
class TestGuiMode:
    ...
    def test_gui_log_receives_only_gui_components(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home, mode="gui")

        logging.getLogger("nunmai_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        nunmai_logging.flush_log_queue()

        gui_log = nunmai_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, nunmai_home):
        """When session context is set, log lines include [session_id]."""
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        nunmai_logging.set_session_context("abc123")
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        nunmai_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_nunmai_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        nunmai_logging._add_rotating_handler(
            log_path,
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("nunmai_cli.config.is_managed", return_value=True):
                nunmai_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = nunmai_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert nunmai_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not nunmai_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, nunmai_home):
        level, max_size, backup = nunmai_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, nunmai_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> nunmai_logging._ManagedRotatingFileHandler:
        handler = nunmai_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that nunmai_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        nunmai_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        nunmai_logging.setup_logging(nunmai_home=nunmai_home, mode="gateway")
        gw_path = nunmai_home / "logs" / "gateway.log"
        rotated = nunmai_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = nunmai_logging._safe_stderr()
