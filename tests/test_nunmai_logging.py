...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, nunmai_home):
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, nunmai_home):
    ...
    def test_gateway_log_not_created_in_cli_mode(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home, mode="cli")
        root = logging.getLogger()
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, nunmai_home):
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = nunmai_logging._ComponentFilter(("gateway",))
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in nunmai_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        nunmai_logging.set_session_context("factory_test")
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, nunmai_home):
        nunmai_logging.setup_logging(nunmai_home=nunmai_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_nunmai_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in nunmai_logging._queued_file_handlers
        )
