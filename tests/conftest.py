...
# nunmai_state._running_under_pytest() as a test-context signal. A child
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from nunmai_cli import kanban_db as _kb_mod
    except Exception:
        return
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``nunmai_cli.kanban_db`` is *already imported* — a
