"""Shared fixtures for the nunmai-engine test suite.
...
2. **Isolated NUNMAI_HOME.** NUNMAI_HOME points to a per-test tempdir so
   code reading ``~/.nunmai/*`` via ``get_nunmai_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".nunmai"`` instead of the canonical
   ``get_nunmai_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NUNMAI_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox NUNMAI_HOME before ANY test module is imported ──────────────────
# `nunmai_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_nunmai_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.nunmai/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes NUNMAI_HOME, but fixtures run
...
# the REAL Nunmai root — capture it BEFORE the sandbox rewires NUNMAI_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.nunmai (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("NUNMAI_KANBAN_HOME", "").strip()
_PRE_SANDBOX_NUNMAI_HOME = os.environ.get("NUNMAI_HOME", "")


def _nunmai_home_points_at_production(value: str) -> bool:
    """True when a pre-set NUNMAI_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    NUNMAI_HOME=~/.nunmai``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``nunmai_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.nunmai`` — the escape vector that landed pytest
    ...
    NUNMAI_HOME is honored now.
    """
    if not value:
        return True
    try:
        # The platform-default root, not a hardcoded ``~/.nunmai``: Windows installs live under
        # ``%LOCALAPPDATA%\nunmai``, and a dev shell exporting that path used to be honored as
        # "custom", pinning import-time paths (``tui_gateway.server._nunmai_home``) to the live
        # install so the state.db guard tripped on every store-touching test (#112692).
        from nunmai_state_guard import _real_platform_state_root

        resolved = Path(value).expanduser().resolve()
        real_root = _real_platform_state_root() or (Path.home() / ".nunmai").resolve()
    except Exception:
        return True
...
if _nunmai_home_points_at_production(os.environ.get("NUNMAI_HOME", "")):
    _SESSION_NUNMAI_HOME = tempfile.mkdtemp(prefix="nunmai-test-home-")
    os.environ["NUNMAI_HOME"] = _SESSION_NUNMAI_HOME
    atexit.register(shutil.rmtree, _SESSION_NUNMAI_HOME, True)
...
# like a real CLI") — which used to disarm nunmai_state's live-DB guard in
# the child at the same moment the child lost the NUNMAI_HOME redirect.
# NUNMAI_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# nunmai_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export NUNMAI_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["NUNMAI_TEST_ISOLATION"] = os.environ.get("NUNMAI_HOME", "") or "1"
...
os.environ["NUNMAI_DISABLE_LAZY_INSTALLS"] = "1"

#: NUNMAI_HOME as it stood when conftest was imported - i.e. before any test
...
NUNMAI_HOME_AT_CONFTEST_IMPORT = os.environ.get("NUNMAI_HOME", "")

# ── Host-rendezvous isolation ───────────────────────────────────────────────
# ``gateway/host_rendezvous.py`` publishes ONE record per role per OS USER, in
# ``$NUNMAI_GATEWAY_LOCK_DIR`` else ``$XDG_STATE_HOME/nunmai/gateway-locks`` —
# deliberately outside NUNMAI_HOME, because the host singleton spans profiles.
...
HOST_LOCK_DIR_AT_CONFTEST_IMPORT = os.environ.get("NUNMAI_GATEWAY_LOCK_DIR", "")
if not HOST_LOCK_DIR_AT_CONFTEST_IMPORT:
    ...
    _LOCK_DIR_PREFIX = "nunmai-test-gateway-locks-"
    ...
    os.environ["NUNMAI_GATEWAY_LOCK_DIR"] = _SESSION_LOCK_DIR
...
# NUNMAI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NUNMAI_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``nunmai_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Nunmai agent session inherits them and
    # nunmai_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "NUNMAI_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "NUNMAI_INTERACTIVE",
    "NUNMAI_QUIET",
    "NUNMAI_TOOL_PROGRESS",
    "NUNMAI_TOOL_PROGRESS_MODE",
    "NUNMAI_MAX_ITERATIONS",
    "NUNMAI_SESSION_PLATFORM",
    "NUNMAI_SESSION_CHAT_ID",
    "NUNMAI_SESSION_CHAT_NAME",
    "NUNMAI_SESSION_CHAT_TYPE",
    "NUNMAI_SESSION_THREAD_ID",
    "NUNMAI_SESSION_SOURCE",
    "NUNMAI_SESSION_KEY",
    "NUNMAI_GATEWAY_SESSION",
    "NUNMAI_CRON_SESSION",
    "_NUNMAI_GATEWAY",
    "NUNMAI_PLATFORM",
    "NUNMAI_MODEL",
    "NUNMAI_INFERENCE_MODEL",
    "NUNMAI_INFERENCE_PROVIDER",
    "NUNMAI_TUI_PROVIDER",
    "NUNMAI_MANAGED",
    "NUNMAI_MANAGED_DIR",
    "NUNMAI_DEV",
    ...
    "NUNMAI_TIMEZONE",
    "NUNMAI_REDACT_SECRETS",
    "NUNMAI_BACKGROUND_NOTIFICATIONS",
    "NUNMAI_EXEC_ASK",
    "NUNMAI_HOME_MODE",
    "NUNMAI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.nunmai/kanban.db instead of the per-test NUNMAI_HOME.
    "NUNMAI_KANBAN_DB",
    "NUNMAI_KANBAN_BOARD",
    "NUNMAI_KANBAN_HOME",
    "NUNMAI_KANBAN_WORKSPACES_ROOT",
    "NUNMAI_KANBAN_LOGS_ROOT",
    "NUNMAI_KANBAN_TASK",
    "NUNMAI_KANBAN_WORKSPACE",
    "NUNMAI_KANBAN_RUN_ID",
    "NUNMAI_KANBAN_CLAIM_LOCK",
    "NUNMAI_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "NUNMAI_DELEGATED_CHILD_CONTEXT",
    "NUNMAI_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "nunmai" host.
    "NUNMAI_HONCHO_HOST",
    ...
    "NUNMAI_DASHBOARD_OAUTH_CLIENT_ID",
    "NUNMAI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NUNMAI_HOME to per-test tempdirs so code that
    reads ``~/.nunmai/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral NUNMAI_* vars that could change test semantics.
    for name in _NUNMAI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("NUNMAI_HONCHO_HOST", "nunmai")

    # 3. Redirect NUNMAI_HOME to a per-test tempdir. Code that reads
    #    ``~/.nunmai/*`` via ``get_nunmai_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.nunmai/*`` via
    #    ``Path.home() / ".nunmai"`` instead of ``get_nunmai_home()``
    #    is a bug to fix at the callsite.
    fake_nunmai_home = tmp_path / "nunmai_test"
    fake_nunmai_home.mkdir()
    (fake_nunmai_home / "sessions").mkdir()
    (fake_nunmai_home / "cron").mkdir()
    (fake_nunmai_home / "memories").mkdir()
    (fake_nunmai_home / "skills").mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(fake_nunmai_home))
    # A test that pins the process home (nunmai_constants.pin_process_nunmai_home) must not
    # leak that module-global into the next test's routed-profile decisions.
    try:
        import nunmai_constants as _hc
        monkeypatch.setattr(_hc, "_PINNED_PROCESS_NUNMAI_HOME", None, raising=False)
    except Exception:
        pass
    ...
    # write the developer's live ~/.local/state/nunmai/gateway-locks.
    ...
    if not HOST_LOCK_DIR_AT_CONFTEST_IMPORT:
        ...
        monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(tmp_path / "gateway-locks"))
    ...
    # nunmai_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("NUNMAI_TEST_ISOLATION", str(fake_nunmai_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("NUNMAI_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. nunmai_state computes ``DEFAULT_DB_PATH = get_nunmai_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from nunmai_state import ...``) that
    ...
    nunmai_state_mod = sys.modules.get("nunmai_state")
    if nunmai_state_mod is not None and hasattr(nunmai_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            nunmai_state_mod, "DEFAULT_DB_PATH", fake_nunmai_home / "state.db"
        )
    ...
    monkeypatch.setenv("NUNMAI_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.nunmai/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import nunmai_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path NUNMAI_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_nunmai_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from nunmai_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from nunmai_cli import _subprocess_compat
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/nunmai_cli/
    ...
    ``pytest tests/nunmai_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``nunmai_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``NUNMAI_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``nunmai_state_registry.acquire()`` are skipped:
    ...
    if wait is not None:
        wait()
    try:
        from nunmai_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.nunmai. This autouse
...
# allow-list because test-level fixtures legitimately move NUNMAI_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session NUNMAI_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``NUNMAI_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Nunmai root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_NUNMAI_HOME and not _nunmai_home_points_at_production(
        _PRE_SANDBOX_NUNMAI_HOME
    ):
        # NUNMAI_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_nunmai_root().resolve()
    # No pre-existing NUNMAI_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".nunmai").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.nunmai`` captured at import time. Hermetic tests that legitimately
    move NUNMAI_HOME to sibling tempdirs are unaffected.

    Only patches when ``nunmai_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("nunmai_cli.kanban_db")
    _kdbc = sys.modules.get("nunmai_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import nunmai_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.nunmai. See #69283."
        )
...
# ``nunmai_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Nunmai root. This fixture wires the
# test-side knobs:
...
#     installs where NUNMAI_HOME is not ~/.nunmai) into the guard's
...
# so subprocess children that import nunmai_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("nunmai_state")
    ...
    if _PRE_SANDBOX_NUNMAI_HOME and not _nunmai_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_NUNMAI_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Nunmai home override redirects every later
    # ``get_nunmai_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from nunmai_constants import get_nunmai_home_override, set_nunmai_home_override

        if get_nunmai_home_override() is not None:
            set_nunmai_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal nunmai config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``nunmai-gateway`` process
...
# correlated with a ``tests/nunmai_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> nunmai-gateway``
...
def _wal_is_usable() -> bool:
    """True when Nunmai will actually put a database into WAL mode here.

    Nunmai refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Nunmai can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Nunmai managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``nunmai_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_nunmai_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_nunmai_home`` fixture
    redirects ``NUNMAI_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.nunmai/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``nunmai_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["NUNMAI_VOICE_TTS"] = "1"``. The
...
#      ``nunmai_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``nunmai_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``nunmai_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``nunmai_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Nunmai runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.nunmai``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native Nunmai home.

    Every per-test sandbox is ``<basetemp>/.../nunmai_test``. ``get_default_nunmai_root()``
    prefers the platform-native home whenever ``NUNMAI_HOME`` sits *under* it, so a basetemp
    inside ``~/.nunmai`` (or ``%LOCALAPPDATA%\\nunmai``, where ``TEMP`` commonly lives on
    ...
    """
    from nunmai_constants import _get_platform_default_nunmai_home

    native = _get_platform_default_nunmai_home().resolve()
    ...
    # Nunmai home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.nunmai/nunmai-engine). The relocated basetemp goes
    # into ONE prunable root outside the home, never loose into the operator's
    # $HOME (123 ``nunmai-pytest-basetemp-*`` dirs piled up there in a day, one per
    ...
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's Nunmai home {native}; "
    ...
    config._nunmai_relocated_basetemp = safe
...
def _pytest_disk_temp_root(native: Path) -> Path:
    ...
    if os.name != "nt" and os.path.isdir("/var/tmp"):  # no-tmp: ok — disk-backed FHS root
        root = Path("/var/tmp/nunmai-pytest")  # no-tmp: ok — /var/tmp is disk-backed by FHS, never tmpfs
    else:
        root = native.parent / "nunmai-pytest"
...
def _remove_relocated_basetemp(config) -> None:
    safe = getattr(config, "_nunmai_relocated_basetemp", None)
    if safe is not None:
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Nunmai falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Nunmai uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart nunmai-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting nunmai/python patterns are also blocked.
    """
    ...
    def _matches_nunmai_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _NUNMAI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_nunmai_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch nunmai-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m nunmai_cli.main".
                if (
                    "nunmai" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live nunmai-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting nunmai/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `nunmai update` (or the
        # equivalent `python -m nunmai_cli.main update`).  These commands
        ...
        if "update" in low and (
            # nunmai update / nunmai update --gateway / setsid bash -c ... nunmai update
            ("nunmai" in low and "update" in low.split())
            or
            # python -m nunmai_cli.main update --gateway
            ("nunmai_cli" in low and "update" in low.split())
            or
            # venv/bin/nunmai update  (absolute path variant used in tests)
            (".venv/bin/nunmai" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`nunmai update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m nunmai_cli.main
        # gateway run|start|restart``). ``_spawn_nunmai_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp NUNMAI_HOME, resolves the DEVELOPER's
        # ``nunmai-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … nunmai gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "nunmai gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (nunmai_cli.web_server_gateway."
                "_spawn_nunmai_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_NUNMAI_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import nunmai_cli.voice as _voice
    except Exception:
