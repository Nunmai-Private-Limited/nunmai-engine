"""Tests for nunmai_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Nunmai entry point
(nunmai, nunmai-engine, nunmai-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Nunmai entry point imports nunmai_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nunmai_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nunmai_bootstrap", None)
    import nunmai_bootstrap  # noqa: WPS433
    return nunmai_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Nunmai CLI
...
class TestEntryPointsImportBootstrap:
    """Every Nunmai entry point must import nunmai_bootstrap as its
    ...
    # Entry points that invoke Nunmai as a process.  Each one must
    # import nunmai_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nunmai_cli/main.py",   # nunmai CLI (console_script)
        "run_agent.py",          # nunmai-engine (console_script)
        "acp_adapter/entry.py",  # nunmai-acp (console_script)
    ...
    def test_library_imports_of_dual_use_entry_modules_stay_side_effect_free(self, tmp_path):
        # The codex nunmai-tools MCP server and the compute host are ``python -m`` entry points
        # that agent/gateway code and tests also import; there the bootstrap exported TMPDIR and
        # NUNMAI_SCRATCH_DIR into every importer (gateway.relay's read-only routing included).
        code = textwrap.dedent("""
            ...
            print(json.dumps({"bootstrapped": "nunmai_bootstrap" in sys.modules, "changed": changed}))
        """)
        ...
        env.update({"HOME": str(tmp_path), "USERPROFILE": str(tmp_path), "PYTHONPATH": str(repo),
                    "NUNMAI_HOME": str(tmp_path / "home"), "PYTHONDONTWRITEBYTECODE": "1"})
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nunmai_bootstrap' and that
        ...
        half-finished ``nunmai update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nunmai_bootstrap`` as a top-level module) leaves nunmai
        ...
        """
        # Resolve relative to the nunmai-engine repo root.  Tests live
        # at tests/test_nunmai_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nunmai start even when nunmai_bootstrap hasn't been
        ...
        assert first_import_name == "nunmai_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nunmai_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nunmai_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Nunmai's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("NUNMAI_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["NUNMAI_PYTHON_SRC_ROOT"] = env
            elif "NUNMAI_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["NUNMAI_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/nunmai")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NUNMAI_PYTHON_SRC_ROOT", None)
            else:
                os.environ["NUNMAI_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/nunmai", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Nunmai root.  The guard must relocate Nunmai to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/nunmai"])
        assert result[0] == "/opt/nunmai"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Nunmai root.
        assert result.index("/opt/nunmai") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["NUNMAI_PYTHON_SRC_ROOT"] = "/env/nunmai"
            hb.harden_import_path()
            assert sys.path[0] == "/env/nunmai"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NUNMAI_PYTHON_SRC_ROOT", None)
...
class TestHappyEyeballsSocketConnect:
    ...
    def test_import_routes_http_client_and_urllib3_connects_through_the_racer(self):
        ...
        assert getattr(urllib3_connection.create_connection, "_nunmai_happy_eyeballs", False)
    ...
    def test_racer_bug_raises_instead_of_falling_back_to_the_serial_walk(self, monkeypatch):
        ...
        for racer in (socket.create_connection, urllib3_connection.create_connection):
            assert getattr(racer, "_nunmai_happy_eyeballs", False)
...
class TestNeverFreeEnviron:
    ...
    def test_arrays_superseded_by_new_names_stay_intact(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import ctypes, os, nunmai_bootstrap
            ...
            for i in range(80):
                ...
                os.environ[f"NUNMAI_ENVIRON_PROBE_{i}"] = "1"
            ...
            assert os.environ["NUNMAI_ENVIRON_PROBE_79"] == "1"
    ...
    def test_concurrent_writers_lose_no_name_in_the_c_environ(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import ctypes, os, sys, threading, nunmai_bootstrap
            getenv = ctypes.CDLL(None).getenv
            getenv.restype, getenv.argtypes = ctypes.c_char_p, [ctypes.c_char_p]
            sys.setswitchinterval(1e-6)
            batches = [[f"NUNMAI_ENVIRON_RACE_{t}_{i}" for i in range(200)] for t in range(8)]
    ...
    def test_set_del_churn_of_the_same_names_keeps_memory_bounded(self):
        ...
        subprocess.run([sys.executable, "-c", textwrap.dedent("""
            import os, tracemalloc, nunmai_bootstrap
            def churn(cycles):
                for _ in range(cycles):
                    for k in range(4):
                        os.environ[f"NUNMAI_ENVIRON_CHURN_{k}"] = "1"
                    for k in range(4):
                        del os.environ[f"NUNMAI_ENVIRON_CHURN_{k}"]
            churn(50)
            tracemalloc.start()
