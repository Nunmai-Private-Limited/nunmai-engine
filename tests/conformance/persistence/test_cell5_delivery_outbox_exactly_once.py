...
Nunmai' outbox is ``gateway/delivery_ledger.py``. Because a send and its
...
* The ledger module, on a real ``state.db`` under an isolated
  ``HOME=<tmp>/home``, ``NUNMAI_HOME=<tmp>/home/.nunmai`` (every child).
...
class Cell:
    ...
    def __init__(self, tmp_path: Path):
        ...
        self.nunmai_home.mkdir(parents=True)
        (self.nunmai_home / "config.yaml").write_text(
            "gateway:\n  delivery_ledger: true\n", encoding="utf-8"
        )
        self.journal = tmp_path / "transport.jsonl"
        self.ids_file = tmp_path / "ids.json"
        self.db_path = self.nunmai_home / "state.db"
        (tmp_path / "tmp").mkdir()
        self._boots = 0
    ...
    def spawn(self, script: str, role: str, extra: dict | None = None) -> subprocess.Popen:
        ...
        env.update({
            ...
            # HOME/.nunmai IS the temp home here, so the live-system guard would
