...
from nunmai_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(nunmai_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <nunmai_home>/plugins/<name> and opt it in."""
    plugin_dir = nunmai_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "nunmai_empty"))
