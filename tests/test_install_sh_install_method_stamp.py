...
($INSTALL_DIR), not into the shared $NUNMAI_HOME.

Background (shared-$NUNMAI_HOME bug)
------------------------------------
$NUNMAI_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.nunmai:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$NUNMAI_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``nunmai update``).
...
``~/.nunmai/nunmai-engine``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$NUNMAI_HOME/\.install_method"', text), (
        "install.sh must not stamp $NUNMAI_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `nunmai update`"
    )
