...
These run without Docker: the shim's NUNMAI_TINI_SHIM_TARGET /
NUNMAI_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["NUNMAI_TINI_SHIM_TARGET"] = str(init)
    env["NUNMAI_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "NUNMAI_TINI_SHIM_TARGET" in text
