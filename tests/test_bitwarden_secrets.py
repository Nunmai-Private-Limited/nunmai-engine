...
binary download are exercised manually by `nunmai secrets bitwarden
...
def nunmai_home(tmp_path, monkeypatch):
    """Point Nunmai at an isolated home directory."""
    home = tmp_path / ".nunmai"
    home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(home))
    # Some modules cache get_nunmai_home; clear if needed.
    import nunmai_constants
    if hasattr(nunmai_constants, "_NUNMAI_HOME_CACHE"):
        nunmai_constants._NUNMAI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(nunmai_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from nunmai_cli.env_loader import _apply_external_secret_sources
