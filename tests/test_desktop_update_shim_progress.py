...
def progress(tmp_path):
    ...
    status = tmp_path / "nunmai-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Nunmai to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `nunmai update`, and reports the stage that was on screen
...
n="$(cat "$NUNMAI_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$NUNMAI_TEST_CALLS"
for f in "$TMPDIR"/nunmai-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$NUNMAI_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$NUNMAI_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "nunmai-engine"
    (install_root / "venv" / "bin").mkdir(parents=True)
    nunmai = install_root / "venv" / "bin" / "nunmai"
    nunmai.write_text(FAKE_NUNMAI)
    nunmai.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "NUNMAI_TEST_CAPTURE": str(capture),
        "NUNMAI_TEST_CALLS": str(calls),
        "NUNMAI_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".nunmai-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`nunmai update` is the longest wait in the hand-off and the one the
