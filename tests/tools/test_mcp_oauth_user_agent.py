...
shared ``Nunmai-Agent/<version>`` default is stamped instead: those requests
...
def test_unconfigured_user_agent_falls_back_to_the_nunmai_default(
    builder, tmp_path, monkeypatch
):
    """No config → the shared ``Nunmai-Agent/<version>`` default, never a header-less request.
...
# ---------------------------------------------------------------------------
# Device flow: the token poll is sent by Nunmai itself, so watch the socket
# ---------------------------------------------------------------------------
...
def test_device_flow_token_poll_carries_a_user_agent_on_the_wire(
    ...
    from tools.mcp_oauth_manager import NunmaiMCPOAuthProvider
    ...
    provider = NunmaiMCPOAuthProvider(
    ...
    assert all(agents), seen  # nothing leaves Nunmai header-less
