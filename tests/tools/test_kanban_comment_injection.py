...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_WORKSPACES_ROOT", "NUNMAI_KANBAN_HOME", "NUNMAI_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_PROFILE", "worker-bot")
    agent = FakeAgent()
...
def test_delegated_child_in_worker_process_neither_receives_nor_consumes_notes(worker_home, monkeypatch):
    """A delegate_task child inherits the worker's ``NUNMAI_KANBAN_TASK``; operator notes
...
def test_skips_own_authored_comments_without_env_profile(worker_home, monkeypatch):
    """Own comments are skipped even when ``NUNMAI_PROFILE`` is absent: the
    injection filter resolves the worker's identity the same way the persisted
    write side does (env → ``NUNMAI_HOME``-derived active profile), so a worker's
    ...
    assert identity != "worker"  # resolved from NUNMAI_HOME, not the generic label
