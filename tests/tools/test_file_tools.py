...
class TestSensitivePathCheck:
    ...
    def test_nunmai_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved_loaded", True)

        from tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "Nunmai config" in result["error"]



    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved", "/home/user/.nunmai/config.yaml")
        monkeypatch.setattr("tools.file_tools_write_guards._nunmai_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
...
class TestSSHConfigWriteGate:
    ...
    def test_single_query_session_denies_with_the_q_mode_message(self, ssh_config, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_INTERACTIVE", "1")  # `nunmai chat -q` exports it too
        from tools.file_tools import write_file_tool
...
class TestSecretFileReadRedaction:
    ...
    def nunmai_home(self, tmp_path, monkeypatch):
        """A Nunmai home with no ``.nunmai`` segment, like ``%LOCALAPPDATA%\\nunmai``."""
        import agent.file_safety as file_safety

        home = tmp_path / "nunmai"
        home.mkdir()
        monkeypatch.setattr(file_safety, "_nunmai_home_path", lambda: home)
        monkeypatch.setattr(file_safety, "_nunmai_root_path", lambda: home)
        return home
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_read_file_of_nunmai_config_masks_opaque_token(self, mock_get, nunmai_home):
        ...
        from tools.file_tools import read_file_tool
        out = json.loads(read_file_tool(str(nunmai_home / "config.yaml"), task_id="secret-read"))
        ...
        out = json.loads(read_file_tool(str(nunmai_home.parent / "proj-config.yaml"), task_id="plain-read"))
        assert self.SYNTH in out["content"]

    @patch("tools.file_tools._get_file_ops")
    def test_search_in_nunmai_home_masks_opaque_token(self, mock_get, nunmai_home):
        config = nunmai_home / "config.yaml"
        ops = MagicMock()
        ...
        from tools.file_tools import search_tool
        raw = search_tool(pattern="ADS_API_TOKEN", path=str(nunmai_home),
