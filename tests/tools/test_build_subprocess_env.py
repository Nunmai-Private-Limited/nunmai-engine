...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_nunmai_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["NUNMAI_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_nunmai_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see NUNMAI_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('NUNMAI_HOME'), "
    ...
    assert result["home"] == str(nunmai_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `nunmai` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_nunmai_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no nunmai console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``nunmai``.
    ...
    bin_dir = local_mod._resolve_nunmai_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "nunmai.exe" if os.name == "nt" else "nunmai")
    ):
        pytest.skip("no real nunmai console-script install available")

    # Simulate the service-manager minimal PATH: nunmai dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("nunmai", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("nunmai", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'nunmai' must resolve from the child PATH {env.get('PATH')!r}"
    )
