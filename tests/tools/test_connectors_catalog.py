...
def _entry(name, *, platforms=(), requires_env=()):
    return SimpleNamespace(
        ...
        description=f"Drives {name}. More text.", requires_nunmai="", platforms=list(platforms),
...
def test_only_a_setup_profile_session_selects_the_tool(tmp_path, monkeypatch):
    ...
    homes = {}
    for name, role in (("default", None), ("nunmai-setup", "setup")):
        homes[name] = tmp_path / name
        homes[name].mkdir()
        if role:
            write_profile_meta(homes[name], role=role)
    for name, expected in (("default", False), ("nunmai-setup", True)):
        token = set_nunmai_home_override(str(homes[name]))
        try:
...
def test_an_approved_row_installs_into_default_and_lists_the_live_tools(tmp_path):
    from nunmai_cli.profiles import get_profile_dir, write_profile_meta
    from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override

    setup_home = tmp_path / "nunmai-setup"
    setup_home.mkdir()
    ...
    token = set_nunmai_home_override(str(setup_home))  # the call comes from the setup chat
    try:
