...
class TestStaleSyncBackTempCleanup:
    ...
    def test_removes_only_stale_prefixed_entries(self, tmp_path, monkeypatch):
        stale_tar = tmp_path / "nunmai-sync-back-stale.tar"
        stale_dir = tmp_path / "nunmai-sync-back-stale-staging"
        recent = tmp_path / "nunmai-sync-back-recent.tar"
    ...
    def test_sync_back_sweeps_leaked_entry_and_uses_identifiable_tar(self, tmp_path, monkeypatch):
        ...
        leaked = tmp_root / "nunmai-sync-back-leaked.tar"
        ...
        def download(dest: Path):
            seen["tar"] = dest
            _make_tar({"root/.nunmai/x.txt": b"hi"}, dest)
...
class TestSyncBackAppliesChanged:
    ...
    def test_sync_back_applies_changed_file(self, tmp_path):
        ...
        mgr.sync_back(nunmai_home=tmp_path / ".nunmai")
...
class TestSyncBackConflict:
    ...
    def test_sync_back_conflict_warns(self, tmp_path, caplog):
        ...
        remote_path = "/root/.nunmai/config.json"
        mapping = [(str(host_file), remote_path)]
        ...
        download_fn = _make_download_fn({
            "root/.nunmai/config.json": remote_content,
        })
...
class TestSyncBackSizeCap:
    ...
    def test_sync_back_refuses_oversized_tar(self, tmp_path, caplog):
        ...
        files = {"root/.nunmai/skill.md": b"remote_version"}
        download_fn = _make_download_fn(files)

        mgr = _make_manager(
            tmp_path,
            file_mapping=[(skill_host, "/root/.nunmai/skill.md")],
            bulk_download_fn=download_fn,
        )

        # Cap at 1 byte so any non-empty tar exceeds it
        with caplog.at_level(logging.WARNING, logger="tools.environments.file_sync"):
            with patch("nunmai_cli.config.load_config", return_value={"terminal": {"sync_back_max_bytes": 1}}):
                mgr.sync_back(nunmai_home=tmp_path / ".nunmai")
    ...
    def test_cap_override_config_key_raises_the_cap(self, tmp_path, monkeypatch, caplog):
        ...
        files = {"root/.nunmai/skill.md": b"remote_version"}
        mgr = _make_manager(tmp_path, file_mapping=[(host_file, "/root/.nunmai/skill.md")],
                            bulk_download_fn=_make_download_fn(files))

        monkeypatch.setenv("NUNMAI_SYNC_BACK_MAX_BYTES", "1")  # the first cut's env var: must be ignored
        monkeypatch.setattr("nunmai_cli.config.load_config",
...
class TestSyncBackWindowsHost:
    ...
    def test_backend_can_reopen_the_tar_path_for_writing(self, tmp_path):
        ...
        def download(dest: Path) -> None:
            ...
            with tarfile.open(fileobj=buf, mode="w") as tar:
                info = tarfile.TarInfo(name="root/.nunmai/skill.py")
                info.size = 2
        ...
        mgr = _make_manager(tmp_path, [(str(host_file), "/root/.nunmai/skill.py")], bulk_download_fn=download)
        mgr._pushed_hashes["/root/.nunmai/skill.py"] = _sha256_bytes(b"v1")
        mgr.sync_back(nunmai_home=tmp_path / ".nunmai")
    ...
    def test_posix_remote_keys_match_on_windows(self, tmp_path):
        host_file = tmp_path / "host" / "skill.py"
        _write_file(host_file, b"v1")
        mapping = [(str(host_file), "/root/.nunmai/skills/a/skill.py")]
        mgr = _make_manager(tmp_path, mapping, bulk_download_fn=_make_download_fn({
            "root/.nunmai/skills/a/skill.py": b"v2", "root/.nunmai/skills/a/new.md": b"new"}))
        mgr._pushed_hashes["/root/.nunmai/skills/a/skill.py"] = _sha256_bytes(b"v1")
        mgr.sync_back(nunmai_home=tmp_path / ".nunmai")
        assert host_file.read_bytes() == b"v2"  # relpath key was 'root\\.nunmai\\...' → skipped
