...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "nunmai", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@nunmai", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("nunmai@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("nunmai@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "nunmai" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "nunmai", "connection_id": "cloud-1",
         "connection_label": "Nunmai Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="nunmai", message="ping", agent=agent))
    assert out.get("status") == "queued"
    assert "Nunmai Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 nunmai (@nunmai): ping")
...
def test_remote_default_is_addressable_by_its_title_slug(tmp_path, monkeypatch):
    """A remote ``default`` is ``@nunmai`` like every gateway's own, so its Bot Mode title is the only
    bare form that singles it out: ``message_agent`` accepts the slug, the prompt roster offers it
    (never bare ``@nunmai``, which is the LOCAL default), and the form does not depend on roster order."""
    from tools import bot_mode_probe

    home = _managed_home(tmp_path)
    rows = [
        {"profile": "default", "handle": "nunmai", "connection_id": "vps-1", "title": "CoS Bot"},
        {"profile": "default", "handle": "nunmai", "connection_id": "cloud-1", "title": "Ops Bot"},
    ...
    for order in (rows, rows[::-1]):
        ...
        assert forms == {"cloud-1/cos-bot": "cos-bot", "vps-1/default": "nunmai@vps-1", "cloud-1/default": "ops-bot"}
    ...
    # a bare @nunmai from the local default is the two remote defaults — offered under their reply-safe forms
    err = json.loads(message_agent_tool(target="nunmai", message="ping", agent=agent))["error"]
    assert "nunmai@vps-1" in err and "ops-bot" in err
    section = bot_mode_probe.get_bot_mode_protocol_section(home)
    assert "`@ops-bot`" in section and "`@nunmai@vps-1`" in section and "- `@nunmai` —" not in section
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    # Offered under its title slug: bare `@nunmai` is THIS gateway's own default (#103731).
    assert "`@moxie`" in section and "Nunmai Cloud" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="nunmai",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "nope"))
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "nunmai", "connection_id": "cloud-1",
         "connection_label": "Nunmai Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "queued"}),
...
# ── delivery turn author (NUNMAI_TURN_AUTHOR on the recipient turn) ──────────
...
def test_delivery_env_carries_only_the_given_author(monkeypatch):
    """The dispatcher's own NUNMAI_TURN_AUTHOR never reaches the child: dropped without an author, replaced with one."""
    from agent.turn_author import TURN_AUTHOR_ENV

    monkeypatch.setenv("NUNMAI_RELAY_TEST_MARKER", "kept")
    monkeypatch.setenv(TURN_AUTHOR_ENV, json.dumps({"id": "bot:previous", "name": "previous", "is_bot": True}))
    monkeypatch.setenv("NUNMAI_SESSION_KEY", "session-A")
    monkeypatch.setenv("NUNMAI_UI_SESSION_ID", "ui-A")
    monkeypatch.setenv("NUNMAI_SESSION_ID", "session-A")
    monkeypatch.setenv("NUNMAI_SESSION_PROFILE", "profile-A")
    # A session-* knob, not identity: stripping it would break the child's watcher tuning.
    monkeypatch.setenv("NUNMAI_SESSION_STALL_TIMEOUT", "97")
    ...
    assert env["NUNMAI_RELAY_TEST_MARKER"] == "kept"
    assert "NUNMAI_SESSION_KEY" not in env
    assert "NUNMAI_UI_SESSION_ID" not in env
    assert "NUNMAI_SESSION_ID" not in env
    assert "NUNMAI_SESSION_PROFILE" not in env
    assert env["NUNMAI_SESSION_STALL_TIMEOUT"] == "97"
