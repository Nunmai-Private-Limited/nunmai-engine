"""Regression tests for the stage2 Nunmai routing-override sync.

Hosted deploys carry ``NUNMAI_PORTAL_BASE_URL`` / ``NOUS_INFERENCE_BASE_URL`` only in the container
...
NAMES = ("NUNMAI_PORTAL_BASE_URL", "NOUS_PORTAL_BASE_URL", "NOUS_INFERENCE_BASE_URL")
...
def _sync_block(text: str) -> str:
    start = text.index("# --- Sync deploy-injected Nunmai routing overrides")
...
def test_container_values_reach_home_and_every_profile_env(stage2_text: str, tmp_path: Path) -> None:
    """All three overrides land in $NUNMAI_HOME/.env and each profiles/*/.env (created 0600 when
    ...
    result = _run_sync(stage2_text, home, {"NUNMAI_PORTAL_BASE_URL": PORTAL, "NOUS_PORTAL_BASE_URL": PORTAL,
    ...
    for env_file in (home / ".env", home / "profiles" / "work" / ".env", home / "profiles" / "ops" / ".env"):
        ...
        assert parsed["NUNMAI_PORTAL_BASE_URL"] == parsed["NOUS_PORTAL_BASE_URL"] == PORTAL, env_file
        assert parsed["NOUS_INFERENCE_BASE_URL"] == INFERENCE, env_file
        assert len(_assignments(env_file, "NUNMAI_PORTAL_BASE_URL")) == 1, env_file
...
def test_container_wins_over_stale_line_then_idempotent_then_removed_when_unset(stage2_text: str, tmp_path: Path) -> None:
    ...
    home.mkdir()
    env_file = home / ".env"
    env_file.write_text("NUNMAI_PORTAL_BASE_URL=https://portal.nousresearch.com\nNOUS_INFERENCE_BASE_URL=https://by-hand/v1\nOTHER=1\n")

    first = _run_sync(stage2_text, home, {"NUNMAI_PORTAL_BASE_URL": PORTAL})
    assert first.returncode == 0, first.stderr
    assert load_env_file(env_file)["NUNMAI_PORTAL_BASE_URL"] == PORTAL
    assert len(_assignments(env_file, "NUNMAI_PORTAL_BASE_URL")) == 1
    assert "Synced NUNMAI_PORTAL_BASE_URL" in first.stdout
    ...
    second = _run_sync(stage2_text, home, {"NUNMAI_PORTAL_BASE_URL": PORTAL})
    ...
    with env_file.open("a") as fh:
        fh.write("NUNMAI_PORTAL_BASE_URL=https://by-hand\n")  # operator line beside the managed one
    third = _run_sync(stage2_text, home, {})
    assert third.returncode == 0, third.stderr
    parsed = load_env_file(env_file)
    assert _assignments(env_file, "NUNMAI_PORTAL_BASE_URL") == ["NUNMAI_PORTAL_BASE_URL=https://by-hand"], \
    ...
    assert "Removed NUNMAI_PORTAL_BASE_URL" in third.stdout

    if os.geteuid() != 0:
        env_file.chmod(0o444)
        frozen = env_file.read_text()
        readonly = _run_sync(stage2_text, home, {"NUNMAI_PORTAL_BASE_URL": PORTAL})
        env_file.chmod(0o600)
        assert readonly.returncode == 0, "a read-only volume degrades to a warning under set -e"
        assert "could not write NUNMAI_PORTAL_BASE_URL" in readonly.stdout
    ...
    fourth = _run_sync(stage2_text, home, {"NUNMAI_PORTAL_BASE_URL": PORTAL})
