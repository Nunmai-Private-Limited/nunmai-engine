...
def _seed(tmp_path: Path, fake_bins: list[str], browser_exec: str = "", profile_dir: str = "") -> Path:
    ...
    env = {
        "PATH": str(bindir),
        "HOME": str(tmp_path),
        "NUNMAI_BD_PROFILE": "t", "NUNMAI_BD_DISPLAY_NUM": "99",
        "NUNMAI_BD_SOCKET": str(tmp_path / "rfb.sock"), "NUNMAI_BD_XAUTH": str(tmp_path / "Xauthority"),
        "NUNMAI_BD_ENV_FILE": str(tmp_path / "env"), "NUNMAI_BD_CONFIG_HOME": str(cfg),
        "NUNMAI_BD_SEED_ONLY": "1",
        **({"NUNMAI_BD_BROWSER_EXEC": browser_exec, "NUNMAI_BD_BROWSER_EXEC_LINE": f"Exec={browser_exec} --user-data-dir={profile_dir or f'{tmp_path}/bp'}"} if browser_exec else {}),
    }
...
def test_dock_lists_only_programs_present_on_path(tmp_path):
    ...
    execs = sorted(
        ...
        for line in (cfg / "xfce4/panel" / pid.replace("plugin-", "launcher-") / "nunmai.desktop").read_text(encoding="utf-8").splitlines()
...
def test_browser_launcher_follows_the_profile_without_reseeding_the_panel(tmp_path):
    ...
    binds the bot's user-data-dir by absolute path: after `nunmai profile rename` the dock must open the
    ...
    execs = [
        line for d in (cfg / "xfce4/panel").glob("launcher-*/nunmai.desktop")
...
def test_browser_launcher_seeded_before_the_marker_still_follows_a_rename(tmp_path):
    """Profiles seeded by an earlier launcher have the Browser entry but no `.nunmai-browser-launcher`;
    ...
    chrome = tmp_path / "bin" / "chrome"
    cfg = _seed(tmp_path, ["xfce4-terminal", "chrome"], browser_exec=str(chrome), profile_dir="/old name/bp")
    (cfg / "xfce4/panel/.nunmai-browser-launcher").unlink()
    shutil.rmtree(tmp_path / "bin")
    _seed(tmp_path, ["xfce4-terminal", "chrome"], browser_exec=str(chrome), profile_dir="/new name/bp")
    execs = [
        line for d in (cfg / "xfce4/panel").glob("launcher-*/nunmai.desktop")
