    ...
    def test_extracts_quoted_absolute_path(self):
        ...
        assert (
            _extract_screenshot_path_from_text(
                "Screenshot saved to '/Users/david/.nunmai/browser_screenshots/shot.png'"
            )
            == "/Users/david/.nunmai/browser_screenshots/shot.png"
        )
...
class TestInactivityJanitorMultiplex:
    ...
    def test_janitor_tears_down_under_owner_profile_scope(self, tmp_path, monkeypatch):
        ...
        from nunmai_constants import (
            get_nunmai_home, reset_nunmai_home_override, set_nunmai_home_override,
        )
        ...
        home_tok = set_nunmai_home_override(str(p1))
...
class TestAtexitOriginUnimportable:
    def test_hooks_stay_silent_when_origin_fresh_import_fails(self, monkeypatch):
        """Mid-`nunmai update` the on-disk tree can be half-new (new config.py importing
