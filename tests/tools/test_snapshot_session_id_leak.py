...
run stamps per command (NUNMAI_DELEGATED_CHILD_CONTEXT, NUNMAI_CRON_SESSION), or
...
def test_export_dump_drops_every_bridged_var_and_the_delegation_marker():
    ...
    exports = "; ".join([f'export {n}="x"' for n in scoped] + ['export NUNMAI_HOME="/h"', 'export MYVAR="keep"'])
    out = subprocess.run(
    ...
    assert 'declare -x NUNMAI_HOME="/h"' in out
