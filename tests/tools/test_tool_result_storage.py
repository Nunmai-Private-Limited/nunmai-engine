...
class TestWriteToSandbox:
    ...
    def test_size_probe_decides_lossless(self, stdin_mode, probed, ok):
        ...
        assert _write_to_sandbox(content, "/tmp/nunmai-results/p.txt", env) is ok
        if not ok:
            rm_cmd = env.execute.call_args_list[2][0][0]
            assert rm_cmd.startswith("rm -f ") and "/tmp/nunmai-results/p.txt" in rm_cmd
        else:
    ...
    def test_unprobeable_backend_is_best_effort_success(self):
        ...
        assert _write_to_sandbox("data", "/tmp/nunmai-results/np.txt", env) is True
