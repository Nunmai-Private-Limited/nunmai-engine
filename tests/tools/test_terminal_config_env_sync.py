...
def test_save_config_set_bridges_every_cli_terminal_key():
    """``nunmai config set terminal.X`` must propagate every key the CLI
    ...
    assert not missing, (
        f"`nunmai config set terminal.X` doesn't sync these keys to .env: "
        f"{sorted(missing)}.  Add them to TERMINAL_CONFIG_ENV_MAP in "
        f"nunmai_cli/config.py (set_config_value bridges through it)."
    )
