...
def test_the_image_bakes_the_same_apt_packages_the_runtime_would_install() -> None:
    ...
    assert "ARG NUNMAI_BOT_DESKTOP" in text, "the Bot Screen apt layer is gone from the Dockerfile"
    body = text.split("ARG NUNMAI_BOT_DESKTOP", 1)[1].split("--no-install-recommends", 1)[1].split("rm -rf", 1)[0]
...
: > "$NUNMAI_BD_XLOCK_DIR/spawned.$$"
sleep 0.4
echo $$ > "$NUNMAI_BD_XLOCK_DIR/.X${NUNMAI_BD_DISPLAY_NUM}-lock"
: > "$NUNMAI_BD_SOCKET"
printf 'DISPLAY=:%s\\n' "$NUNMAI_BD_DISPLAY_NUM" > "$NUNMAI_BD_ENV_FILE"
sleep 30
"""

# One start() per process: state_dir() is NUNMAI_HOME-scoped and process-global, so two profiles need two
...
os.environ["NUNMAI_BD_XLOCK_DIR"] = str(scratch / "xlocks")
try:
...
def start_in_fresh_process(tmp_path):
    ...
    def launch(home: Path) -> subprocess.Popen:
        env = {**os.environ, "NUNMAI_HOME": str(home)}
...
def in_process_runtime(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_BD_XLOCK_DIR", str(tmp_path / "xlocks"))
    yield tmp_path
