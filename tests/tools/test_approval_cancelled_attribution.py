...
def gateway_session(monkeypatch):
    ...
    for k in ("NUNMAI_CRON_SESSION", "NUNMAI_YOLO_MODE", "NUNMAI_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("NUNMAI_GATEWAY_SESSION", "1")
    monkeypatch.setenv("NUNMAI_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from NUNMAI_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("NUNMAI_CRON_SESSION", "NUNMAI_YOLO_MODE", "NUNMAI_GATEWAY_SESSION", "NUNMAI_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
