...
def _orphan(home: Path) -> str:
    ...
    env = {**os.environ, "NUNMAI_HOME": str(home), "PYTHONPATH": REPO}
...
class _Home:
    ...
    def __enter__(self):
        self._token = set_nunmai_home_override(str(self.home))
        return self

    def __exit__(self, *exc):
        reset_nunmai_home_override(self._token)
...
def test_gateway_watcher_sweeps_each_served_ledger_in_its_own_scope(tmp_path, monkeypatch):
    ...
    runner._served_profile_homes = {"default": get_nunmai_home(), "b": home_b}
...
def test_tui_notification_poller_sweeps_under_its_session_profile(tmp_path, monkeypatch):
    ...
    def fake_sweep(target_queue):
        seen.append((str(get_nunmai_home()), target_queue))
        stop.set()
        return 0
...
def test_throttle_runs_at_most_one_sweep_per_home_per_interval(tmp_path, monkeypatch):
    calls = []
    monkeypatch.setattr(ad, "sweep_orphaned_completions", lambda q, **kw: calls.append(str(get_nunmai_home())) or 0)
    q = queue.Queue()
