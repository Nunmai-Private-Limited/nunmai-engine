...
def test_resume_on_another_surface_restores_the_pinned_tool_bytes(monkeypatch, tmp_path):
    ...
    one written by other code (``nunmai update``) takes the current definitions instead."""
    ...
    sent.tools = [_tool("read_file"), _described("delegate_task", "delegate", group={"type": "string"}),
                  _described("skill_manage", "lands in /home/u/.nunmai/skills"),
    ...
    with SessionDB(db_path=tmp_path / "state.db") as db:
        ...
        monkeypatch.setattr(_mcp_agent, "tool_pin_version", lambda: "sha-after-nunmai-update")
        updated = _agent([])
    ...
    assert updated.tools == [*this_surface[:2], {"type": "function", "function": {**static["skill_manage"]}},
                             this_surface[2]]
    assert upgraded_pin == {"version": "sha-after-nunmai-update", "tools": updated.tools}
...
def test_a_pin_never_re_adds_a_tool_this_sessions_config_excludes(monkeypatch):
    ...
    agent = _agent(["read_file"], enabled=["nunmai-cli"], disabled=["terminal"])
