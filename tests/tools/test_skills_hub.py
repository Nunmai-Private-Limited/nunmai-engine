...
from tools.skills_hub_search import (
    NUNMAI_INDEX_TTL, _load_nunmai_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  nunmai:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["nunmai"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `nunmai skills browse`."
            )
...
def _make_index_source(skills):
    """Build a NunmaiIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import NunmaiIndexSource
    src = NunmaiIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestNunmaiIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "nunmai-index"
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "Nunmai-Private-Limited/nunmai-engine"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `nunmai update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``nunmai skills install … --name
...
class TestIndexMissFallback:
    """An available nunmai-index stands in for the external registries; when it
    ...
    def _sources(self, index_results):
        index = _FakeSource("nunmai-index", results=index_results)
    ...
    def test_index_miss_consults_displaced_registries_but_not_github(self):
        ...
        assert source_counts == {"nunmai-index": 0, "skills-sh": 1}
        assert timed_out == []
    ...
    def test_index_hit_leaves_registries_untouched(self):
        index, skills_sh, github = self._sources([self._meta("nunmai-index")])

        results, source_counts, _ = parallel_search_sources(
            [index, skills_sh, github], query="humanizar", overall_timeout=5.0)

        assert [r.identifier for r in results] == ["nunmai-index/humanizar"]
        assert source_counts == {"nunmai-index": 1}
    ...
    def test_provider_filter_miss_skips_registries_without_provider_data(self):
        ...
        assert source_counts == {"nunmai-index": 0}
...
# ---------------------------------------------------------------------------
# _load_nunmai_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadNunmaiIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "nunmai-index.json"
        monkeypatch.setattr("tools.skills_hub_search._nunmai_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (NUNMAI_INDEX_TTL + 100)
        import os
