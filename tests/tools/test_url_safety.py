...
class TestSSRFGuardedHttpxClient:
    ...
    async def test_async_backend_blocks_unix_socket_connects(self):
        ...
        with pytest.raises(SSRFConnectionBlocked, match="Unix socket"):
            await backend.connect_unix_socket("/tmp/nunmai.sock")
...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        with patch("nunmai_cli.config.read_raw_config", side_effect=Exception("no config")):
    ...
    def test_config_security_string_false_stays_disabled(self, monkeypatch):
        ...
        with patch("nunmai_cli.config.read_raw_config", return_value=cfg):
            assert _global_allow_private_urls() is False
...
class TestDeclaredFakeIpSentinelRanges:
    ...
    def test_declaration_excuses_only_the_declared_block(self, declared):
        ...
        _reset_allow_private_cache()
        with patch("nunmai_cli.config.read_raw_config", lambda: {}), _resolves_to("198.18.0.23"):
            assert is_safe_url("https://example.com/file.jpg") is False
