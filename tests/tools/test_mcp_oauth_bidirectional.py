"""Regression test for the ``NunmaiMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
@pytest.mark.asyncio
async def test_nunmai_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    assert _NUNMAI_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_nunmai_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _NUNMAI_PROVIDER_CLS is not None
...
async def test_long_lived_resource_request_does_not_block_concurrent_post(
    ...
    acquiring its Bearer token.  Nunmai narrows that lock around resource I/O
