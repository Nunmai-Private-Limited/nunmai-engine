...
2. ``local_delivery_command`` hardcoded ``"nunmai"``, relying on PATH —
   ...
   basename so resolved absolute paths (and ``nunmai.exe``) still take
...
def test_waiter_argv_uses_forward_slashes_on_windows():
    ...
    parts = shlex.split(bot_relay.waiter_command("C:\\Users\\joshu\\.nunmai", ENV))

    assert "-c" not in parts and "--wait-reply" in parts
    assert parts[parts.index("--wait-reply") + 1] == f"C:/Users/joshu/.nunmai/bot_relay/replies/{ENV['id']}.json"
    assert not any("\\" in part for part in parts)


def test_local_delivery_resolves_sibling_nunmai(tmp_path, monkeypatch):
    bin_dir = tmp_path / "venv" / "bin"
    bin_dir.mkdir(parents=True)
    sibling = bin_dir / "nunmai"
    sibling.touch()
    sibling.chmod(0o755)
...
def test_local_delivery_uses_shutil_which_when_no_sibling(tmp_path, monkeypatch):
    ...
    which_hit = str(tmp_path / "usr-local-bin" / "nunmai")
    monkeypatch.setattr(
        bot_relay.shutil, "which", lambda name: which_hit if name == "nunmai" else None
    )
...
def test_delivery_lock_recognizes_resolved_cli_paths(tmp_path, monkeypatch):
    ...
    now that argv[0] may be a resolved absolute path (or nunmai.exe)."""
    acquired = []
    ...
    with bot_mode_dm._delivery_lock(
        [str(tmp_path / "venv" / "bin" / "nunmai"), "-p", "ops", "chat"],
        stdin_file=False,
    ):
        pass
    with bot_mode_dm._delivery_lock(["nunmai", "-p", "ops", "chat"], stdin_file=False):
        pass
    with bot_mode_dm._delivery_lock(
        ["C:\\venv\\Scripts\\nunmai.exe", "-p", "ops", "chat"], stdin_file=False
    ):
        pass
