...
``nunmai_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``NUNMAI_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # NUNMAI_* handling (#27303): only the operational allowlist passes;
        # every other NUNMAI_* is dropped (the broad prefix was removed).
        "NUNMAI_HOME": "/home/alice/.nunmai",        # allowlisted → kept
        "NUNMAI_PROFILE": "default",                 # allowlisted → kept
        "NUNMAI_INTERACTIVE": "1",                   # not allowlisted → dropped
        "NUNMAI_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "NUNMAI_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``nunmai_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``nunmai_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_nunmai_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_nunmai_tools_module(["terminal"], transport="uds")
...
# let the child import nunmai_tools, a user script that printed non-ASCII
