...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad NUNMAI_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: NUNMAI_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_nunmai_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "NUNMAI_HOME": "/h", "NUNMAI_PROFILE": "p",
        "NUNMAI_CONFIG": "/c.yaml", "NUNMAI_ENV": "/e",
        "NUNMAI_DELEGATED_CHILD_CONTEXT": "1",
        # other NUNMAI_* → dropped (broad prefix removed)
        "NUNMAI_BASE_URL": "https://x", "NUNMAI_INTERACTIVE": "1",
        "NUNMAI_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "NUNMAI_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "NUNMAI_BASE_URL", "NUNMAI_INTERACTIVE", "NUNMAI_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped NUNMAI_* vars."""
    import logging
