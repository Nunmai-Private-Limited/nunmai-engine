...
Nunmai accepts exactly this shape — the document fetched from the well-known URL derived from the advertised
...
async def test_origin_issued_metadata_shim_does_not_read_non_discovery_response(url):
    ...
    provider = NunmaiProviderMixin.__new__(NunmaiProviderMixin)
    response = ResourceResponse()

    assert await provider._nunmai_accept_origin_issued_metadata(response) is response
