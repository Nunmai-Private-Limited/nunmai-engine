...
def _restore_tool_modules():
    original_nunmai_home = os.environ.get("NUNMAI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_nunmai_home is None:
            os.environ.pop("NUNMAI_HOME", None)
        else:
            os.environ["NUNMAI_HOME"] = original_nunmai_home
        _reset_modules(("tools", "nunmai_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    nunmai_cli = types.ModuleType("nunmai_cli")
    nunmai_cli.__path__ = [str(REPO_ROOT / "nunmai_cli")]  # type: ignore[attr-defined]
    sys.modules["nunmai_cli"] = nunmai_cli
    nunmai_home = tmp_path / "nunmai-home"
    os.environ["NUNMAI_HOME"] = str(nunmai_home)
    sys.modules["nunmai_cli.config"] = types.SimpleNamespace(
        get_nunmai_home=lambda: nunmai_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="nunmai-engine", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": nunmai_home / "modal_snapshots.json",
