...
def worker_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", str(run_id))
    return tid
...
def test_request_review_rejects_unknown_reviewer_without_mutation(monkeypatch, worker_env, tmp_path):
    ...
    (tmp_path / ".nunmai" / "profiles" / "verifier" / "config.yaml").write_text("{}\n")  # identity marker
    with kbc.connect() as conn:
...
def test_unbound_worker_cannot_mutate_card(monkeypatch, worker_env):
    ...
    # Worker is scoped to the task (NUNMAI_KANBAN_TASK set by the fixture) but
    # has NO run id — the unbound state the dispatcher never produces.
    monkeypatch.delenv("NUNMAI_KANBAN_RUN_ID", raising=False)
...
def test_malformed_run_id_refused_but_nonlifecycle_allowed(monkeypatch, worker_env):
    """A malformed (non-integer) NUNMAI_KANBAN_RUN_ID is treated as unbound and
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", "not-an-int")
...
def test_link_running_child_allows_owner_but_rejects_foreign(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", str(own_run_id))
