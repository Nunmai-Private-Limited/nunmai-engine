...
_KILL_SWITCH_AT_COLLECTION = os.environ.get("NUNMAI_DISABLE_LAZY_INSTALLS")
...
class TestSecurityGating:
    ...
    def test_config_failure_fails_open(self, monkeypatch):
        ...
        monkeypatch.delenv("NUNMAI_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.setattr(
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A — nunmai update wiring)
# ---------------------------------------------------------------------------
...
class TestActiveFeatures:
    ...
    def test_shared_dependency_does_not_activate_feature(self, monkeypatch):
        ...
        # reasons. It must not make nunmai update try to refresh Matrix unless
...
class TestRefreshActiveFeatures:
    ...
    def test_windows_matrix_refresh_is_skipped_before_pip(self, monkeypatch):
        ...
        # path. `nunmai update` must not retry that doomed install every run.
        #
    ...
    def test_matrix_probe_reports_unsupported_on_real_windows(self):
        ...
        # actually fires on a real Windows host, so `nunmai update` skips the
...
class TestInstallSpecs:
    ...
    def test_plugin_dependency_installs_do_not_inherit_nunmai_exclude_newer(self, monkeypatch, tmp_path):
    ...
    def test_nunmai_own_lazy_installs_keep_the_checkout_quarantine(self, monkeypatch, tmp_path):
        """Nunmai's OWN optional deps (LAZY_DEPS via ``ensure``) stay under the 14-day quarantine even when
    ...
    def test_never_raises_on_unexpected_error(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nunmai_cli.config.load_config", lambda: {}, raising=False
        )
...
class TestInstallWarmsBytecode:
    ...
    def _install(monkeypatch, returncode):
        ...
        monkeypatch.setattr(
            "nunmai_cli.managed_uv.resolve_uv", lambda *a, **kw: "uv", raising=False
        )
