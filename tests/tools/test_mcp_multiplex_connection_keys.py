...
def two_profiles(tmp_path, monkeypatch):
    ...
    def enter(which):
        tokens.append(set_nunmai_home_override(homes[which]))
        return nunmai_home_key(homes[which])

    yield enter
    for tool_name in list(registry.get_tool_names_for_toolset("mcp-x")):
        for home in homes.values():
            registry.deregister(tool_name, scope=nunmai_home_key(home))
...
def test_served_profile_without_multiplex_flag_gets_its_own_connection(two_profiles, monkeypatch):
    """A dashboard/desktop backend serves profiles through the NUNMAI_HOME override with
    ...
    with patch("nunmai_constants.get_nunmai_home_override", return_value=None):
...
def test_adopter_scope_setup_failure_leaks_no_override_and_continues(two_profiles, monkeypatch):
    ...
    not leak that adopter's NUNMAI_HOME override into the caller's context, and the
