...
def test_pinned_profile_honours_tilde_and_resolves_relative_paths_against_nunmai_home(tmp_path, monkeypatch):
    ...
    of this profile's screen state lives (its NUNMAI_HOME), so two profiles never share one 'pin'."""
    home = tmp_path / "nunmai-home"
    monkeypatch.setenv("NUNMAI_HOME", str(home))
    monkeypatch.setattr(runtime, "get_nunmai_home", lambda: home)
...
def test_dock_exec_line_survives_spaces_in_the_executable_and_profile_paths():
    ...
    Chromium under '/opt/Google Chrome/' or a profile under a spaced NUNMAI_HOME broke the dock icon.
    ...
    profile = '/home/a b/.nunmai/browser "x"/profile'
    line = browser.dock_exec_line(exe, profile)
    assert line.startswith('Exec="/opt/Google Chrome/chrome" ')
    assert r'"--user-data-dir=/home/a b/.nunmai/browser \\"x\\"/profile"' in line  # spec: \" quoted, then \ string-escaped
