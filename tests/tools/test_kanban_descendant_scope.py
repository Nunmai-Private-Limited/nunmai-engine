...
def _worker_board(tmp_path, monkeypatch):
    ...
    for key, value in {
        "NUNMAI_KANBAN_DB": str(db), "NUNMAI_KANBAN_BOARD": "default",
        "NUNMAI_KANBAN_TASK": own, "NUNMAI_KANBAN_RUN_ID": str(task.current_run_id),
        "NUNMAI_KANBAN_CLAIM_LOCK": task.claim_lock, "HOME": str(tmp_path),
    }.items():
...
def test_terminal_descendants_cannot_mutate_even_after_task_is_removed(tmp_path, monkeypatch):
    ...
    script.write_text(
        ...
        " 'db': os.getenv('NUNMAI_KANBAN_DB'), 'board': os.getenv('NUNMAI_KANBAN_BOARD')}\n"
        ...
        "os.environ.pop('NUNMAI_KANBAN_TASK', None)\n"
        f"p = subprocess.run([sys.executable, '-m', 'nunmai_cli.main', 'kanban', 'complete', foreign, '--result', 'must refuse'], cwd={str(ROOT)!r}, capture_output=True, text=True, stdin=subprocess.DEVNULL, timeout=45)\n"
    ...
    child_envs = [
        ...
        _build_safe_env({"NUNMAI_HOME": os.environ["NUNMAI_HOME"]}),
    ]
...
def test_worker_cli_cannot_use_foreign_task_to_drop_run_scope(tmp_path, monkeypatch):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "nunmai_cli.main", "kanban", "complete", foreign, "--result", "no"],
    ...
    for arguments in (["attach", foreign, str(attachment)], ["unblock", foreign]):
        proc = subprocess.run([sys.executable, "-m", "nunmai_cli.main", "kanban", *arguments],
                              cwd=ROOT, env=dict(os.environ), stdin=subprocess.DEVNULL,
                              capture_output=True, text=True, timeout=45)
...
def test_child_shell_can_write_a_kanban_board_outside_its_lineage_root(tmp_path, monkeypatch):
    ...
    DB its shell touches: a repro run against a scratch NUNMAI_HOME got a silently read-only board
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(lineage_home))
    monkeypatch.delenv("NUNMAI_DELEGATED_CHILD_CONTEXT", raising=False)
    for key in ("NUNMAI_KANBAN_DB", "NUNMAI_KANBAN_BOARD", "NUNMAI_KANBAN_TASK", "NUNMAI_KANBAN_HOME"):
    ...
    script.write_text(
        ...
        "from nunmai_cli import kanban_db as kb\n"
        "from nunmai_cli.kanban_db_connect import connect\n"
        "out = {'marker': os.environ.get('NUNMAI_DELEGATED_CHILD_CONTEXT')}\n"
        "try:\n"
        "    conn = connect(kb.kanban_db_path()); kb.create_task(conn, title='lineage'); out['lineage'] = 'WROTE'\n"
        "except PermissionError as exc:\n"
        "    out['lineage'] = 'fenced: ' + str(exc)\n"
        f"os.environ['NUNMAI_HOME'] = {str(scratch_home)!r}\n"
        "import nunmai_constants; nunmai_constants._default_nunmai_root_memo = None\n"
