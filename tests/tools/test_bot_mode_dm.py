...
def test_delivery_command_author_json_survives_quoting_and_windows_slash_rewrite(tmp_path, monkeypatch):
    ...
    command = bot_mode_dm._delivery_command(["nunmai", "-p", "x"], str(tmp_path / "dm.txt"),
    ...
    assert _runner_parts(command) == ("query-file", str(tmp_path / "dm.txt"), ["nunmai", "-p", "x"])

    monkeypatch.setattr(sys, "platform", "win32")
    command = bot_mode_dm._delivery_command(["nunmai", "-p", "x"], "C:\\Users\\me\\dm.txt",
    ...
    command = bot_mode_dm._delivery_command(["nunmai"], "dm.txt", stdin_file=True)
...
def test_delivery_runner_unlinks_when_child_launch_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="child launch failed"):
        bot_mode_dm._run_delivery(["nunmai"], str(dm_file), stdin_file=False)
    assert not dm_file.exists()
...
def test_sweeper_removes_only_stale_dm_files(tmp_path, monkeypatch):
    ...
    legacy_stale = tmp_path / "nunmai-dm-stale.txt"
