...
def test_cannot_message_self(tmp_path):
    ...
    result = json.loads(
        bot_mode_dm.message_agent_tool(target="nunmai", message="hi", agent=agent)
    )
    assert "error" in result
...
def test_local_delivery_command_and_ack(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bot_relay, "_nunmai_cli", lambda: "nunmai")
    ...
    assert _runner_author(command) == {"id": "bot:default", "name": "nunmai", "is_bot": True}

    # attribution prefix applied server-side; body verbatim inside the file
    content = Path(dm_file).read_text(encoding="utf-8")
    assert content.startswith("Message from 🤖 nunmai (@nunmai): ")
...
def test_relay_ack_is_queued_with_the_envelope_id(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "nunmai", "connection_id": "cloud-1", "connection_label": "Nunmai Cloud"},
    ])
    result = json.loads(bot_mode_dm.message_agent_tool(target="nunmai", message="ping", agent=_FakeAgent(home)))
...
def _rename(home: Path, folder: str, *, display_name: str = "", title: str = "") -> None:
    lines = ["description: teammate for tests", "ui_meta:", "  nunmai-bots:", "    shape: cloud"]
    if title:
...
def test_ambiguous_friendly_name_fails_closed(tmp_path, monkeypatch):
    ...
    reserved @nunmai alias can never be hijacked by a rename."""
    calls = _capture_spawn(monkeypatch)
...
def test_peer_delivery_command_pins_registry_profile_for_secondary_bots(
    ...
    profile (#93935). `nunmai peer` resolves bot_peers through
    ...
    assert transport_argv == ["nunmai", "-p", "default", "peer", "dm", "spark"]
...
def test_peer_delivery_command(tmp_path, monkeypatch):
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[0]["command"])
    assert mode == "stdin"
    assert transport_argv == ["nunmai", "-p", "default", "peer", "dm", "spark/researcher"]
    # the peer child reads the author from its env and forwards it in the request body
    assert _runner_author(calls[0]["command"]) == {"id": "bot:eri-mac.local/default", "name": "nunmai", "is_bot": True}
...
def test_delivery_pins_the_nunmai_entrypoint_beside_this_interpreter(tmp_path, monkeypatch):
    """A background delivery must not rely on PATH: the runner's service context
    lacks the gateway's venv bin dir, so a bare ``nunmai`` resolves to a system
    ...
    nunmai_entry = venv_bin / ("nunmai.exe" if sys.platform == "win32" else "nunmai")
    nunmai_entry.write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[0]["command"])
    assert mode == "query-file"
    assert transport_argv[0] == str(nunmai_entry)
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[1]["command"])
    assert mode == "stdin"
    assert transport_argv == [str(nunmai_entry), "-p", "default", "peer", "dm", "spark"]
...
def test_renamed_primary_signs_with_its_friendly_name_and_is_reachable_by_it(tmp_path, monkeypatch):
    """#89720: `nunmai profile rename default Maia` writes profile.yaml ``display_name`` (no Bot Mode
    title). The primary must then sign `Maia (@nunmai)`, not `nunmai (@nunmai)`, and a teammate must
    reach it as `maia` / `@maia` — the tag the Desktop roster inserts — while `@nunmai` keeps resolving.
    ...
    assert Path(dm_file).read_text(encoding="utf-8").startswith("Message from 🤖 Maia (@nunmai): ")

    coder = _FakeAgent(home / "profiles" / "coder")
    for target in ("maia", "@maia", "@nunmai"):
    ...
    assert Path(dm_file).read_text(encoding="utf-8").startswith("Message from 🤖 Maia Prime (@nunmai): ")


def test_named_profile_sender_prefix(tmp_path, monkeypatch):
    """A named-profile bot signs with its own handle, not @nunmai."""
    calls = _capture_spawn(monkeypatch)
    home = _managed_home(tmp_path, teammates=("researcher", "coder"))
...
def test_live_dm_admitted_before_waiter_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "wrong-home"))
    ...
    assert record["message"] == "Message from 🤖 nunmai (@nunmai): hello"
    assert record["author"] == {"id": "bot:default", "name": "nunmai", "is_bot": True}
...
def test_local_turn_reemits_empty_stdout_for_a_bare_silence_marker(tmp_path, capsys):
    """#110782: the one-shot ``nunmai chat -c "Bot Chat"`` transport applies the gateway's
...
def test_query_file_delivery_closes_stdin_for_initial_attempt_and_retry(
    ...
    returncode = bot_mode_dm._run_delivery(
        ["nunmai", "-p", "researcher"], str(dm_file), stdin_file=False
    )
...
def test_delivery_main_child_env_carries_only_the_argv_author(tmp_path, monkeypatch, mode, author):
    """The ``--author`` payload becomes NUNMAI_TURN_AUTHOR on the child. Without it the runner drops the
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    monkeypatch.setenv("NUNMAI_DM_TEST_MARKER", "kept")
    ...
    returncode = bot_mode_dm._delivery_main(
        ["--run-delivery", *author_args, mode, str(dm_file), "nunmai", "-p", "researcher"])

    assert returncode == 0
    [(argv, kwargs)] = calls
    assert argv[:3] == ["nunmai", "-p", "researcher"]
    assert kwargs["env"]["NUNMAI_DM_TEST_MARKER"] == "kept"
...
def test_real_delivery_command_round_trip_carries_author(tmp_path):
    """Through a real subprocess, the runner argv built by ``_delivery_command`` sets NUNMAI_TURN_AUTHOR on the child."""
    dm_file = tmp_path / "message.txt"
    dm_file.write_text("secret", encoding="utf-8")
    ...
    child.write_text(
        ...
        "pathlib.Path(sys.argv[1]).write_text(os.environ.get('NUNMAI_TURN_AUTHOR', 'unset'), encoding='utf-8')\n",
        encoding="utf-8",
    )
    author = {"id": "bot:default", "name": "nunmai", "is_bot": True}
...
def test_local_turn_relays_utf8_reply_under_a_gbk_default_codec(tmp_path, monkeypatch, capsys):
    """#83851: the transport is a Nunmai CLI child, which always writes UTF-8 stdio. Decoding it with
