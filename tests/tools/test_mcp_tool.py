...
class TestLoadMCPConfig:
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value={"mcp_servers": servers}):
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("nunmai_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool_config import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
    ...
    def test_portable_servers_merge_after_native_interpolation(self):
        ...
        with (
            patch("nunmai_cli.config.load_config", return_value={"mcp_servers": native}),
            patch("nunmai_cli.plugins.discover_plugins"),
            patch("nunmai_cli.plugins.get_plugin_manager", return_value=manager),
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_nunmai_schema(self):
...
class TestMCPServerTask:
    ...
    def test_start_defaults_stdio_cwd_to_session_cwd(self, tmp_path, monkeypatch):
        ...
        server spawned there inherits the Nunmai process dir instead, so
...
class TestToolsetInjection:
    ...
    def test_partial_failure_retry_on_second_call(self):
        ...
        fake_toolsets = {
            "nunmai-cli": {"tools": [], "description": "CLI", "includes": []},
        }
...
class TestDiscoveryConnectConcurrency:
    ...
    def _run_pass(server_names, cap):
        ...
        with patch("tools.mcp_tool_config._load_mcp_config", return_value=server_names), \
             patch("nunmai_cli.config.load_config", return_value={"mcp": {"discovery_concurrency": cap}}), \
