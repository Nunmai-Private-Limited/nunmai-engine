...
def test_worker_create_keeps_durable_origin(tmp_path, monkeypatch, linked, explicit):
    from nunmai_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kn
    ...
    if explicit:
        ...
        state.close()
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", owner)
    monkeypatch.setenv("NUNMAI_SESSION_ID", "ephemeral")
...
def test_tool_create_only_stamps_persisted_ambient_session(tmp_path, monkeypatch, session_id, persisted):
    ...
    from nunmai_cli import kanban_db as kb, kanban_db_connect as kbc
    from tools import kanban_tools as kt
    ...
    monkeypatch.setenv("NUNMAI_SESSION_ID", session_id)
...
def test_tool_create_stamps_request_scoped_session_over_process_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
    monkeypatch.delenv("NUNMAI_KANBAN_TASK", raising=False)
    monkeypatch.setenv("NUNMAI_SESSION_ID", "other-session")
    kb.init_db()
    state = SessionDB(db_path=tmp_path / "state.db")
