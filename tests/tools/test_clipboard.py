...
class TestMacosClipboardFileUrl:
    ...
    Other apps still paste the image; Nunmai must treat a local image file-url
    ...
    def test_only_copied_image_files_are_clipboard_images(self, tmp_path, name, expected):
        ...
        with patch("nunmai_cli.clipboard.subprocess.run", side_effect=self._furl_run(src)):
            assert _macos_has_image() is expected
