...
Older images made /opt/nunmai/ui-tui and /opt/nunmai/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV NUNMAI_TUI_DIR=/opt/nunmai/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R nunmai:nunmai /opt/nunmai/ui-tui" not in text
    assert "chown -R nunmai:nunmai /opt/nunmai/node_modules" not in text
