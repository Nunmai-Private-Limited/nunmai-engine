...
class TestBackendSelection:
    ...
    ``nunmai tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_managed_gateway_does_not_preempt_explicit_exa(self):
        """Regression: a Nunmai OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured EXA_API_KEY in the fallback path.
        Free Nunmai tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """A Nunmai OAuth token must not beat an explicit TAVILY_API_KEY."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}), \
             patch("tools.web_tools._is_tool_gateway_ready", return_value=True), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}):
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a Nunmai-managed
...
class TestParallelClientConfig:
    ...
    def test_client_follows_the_key_reload_applies(self):
        ...
        from nunmai_cli.config import get_env_path, reload_env
        from plugins.web.parallel.provider import _get_sync_client as _get_parallel_client
...
class TestCheckWebApiKey:
    ...
    def test_xai_only_env_end_to_end_toolset_gate(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))  # isolate auth.json / credential pool
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from nunmai_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.nunmai/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.nunmai/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "nunmai-engine"
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "nunmai-engine"
