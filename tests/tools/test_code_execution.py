...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_generated_module_accepts_all_params(self):
        """Executing the generated nunmai_tools module: every stub accepts all of
        ...
        for transport in ("uds", "file"):
            src = generate_nunmai_tools_module(list(SANDBOX_ALLOWED_TOOLS), transport=transport)
            namespace = {"__name__": "nunmai_tools"}
            exec(compile(src, "nunmai_tools.py", "exec"), namespace)
            calls = []
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("nunmai_cli.config.load_config_readonly", return_value={}):
            result = _load_config()
