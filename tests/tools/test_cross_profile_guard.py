...
def fake_nunmai(tmp_path, monkeypatch):
    """Build a two-profile Nunmai layout and point NUNMAI_HOME at
    the nunmai-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_nunmai):
        from tools.file_tools import write_file_tool
        target = fake_nunmai["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_nunmai):
        ...
        from tools.file_tools import write_file_tool
        target = fake_nunmai["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_nunmai_path_unaffected(self, fake_nunmai, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_nunmai):
    ...
    def test_cross_profile_patch_bypass(self, fake_nunmai):
        from tools.file_tools import patch_tool
        target = fake_nunmai["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_nunmai):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_nunmai, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_nunmai["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up NUNMAI_HOME (set in
        ...
        assert "not found in active profile 'nunmai-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.nunmai/profiles/<name>/."""
        # Don't set NUNMAI_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_nunmai_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_nunmai_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_nunmai):
        """When active profile is 'nunmai-security', the prompt warns
        ...
        assert "Active Nunmai profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.nunmai/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Nunmai profile: default" in src
        assert "Active Nunmai profile: {active_profile}" in src
