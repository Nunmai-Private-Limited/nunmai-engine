...
def test_timeout_returns_and_frees_the_slot_even_when_a_descendant_survives(monkeypatch):
    """From an unprivileged Nunmai, killpg reaches the sudo leader but not a root-owned apt child; that
...
def test_root_installs_without_sudo_and_without_asking(monkeypatch):
    """The official Docker image runs Nunmai as uid 0 with no sudo binary: the package manager must be
