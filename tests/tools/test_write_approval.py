...
and the shared slash-command handlers (nunmai_cli/write_approval_commands.py).
...
@pytest.fixture
def nunmai_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="nunmai_wa_test_")
    home = os.path.join(d, ".nunmai")
    os.makedirs(home)
    monkeypatch.setenv("NUNMAI_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import nunmai_cli.config as cfg
    c = cfg.load_config()
...
def test_list_pending_skips_non_dict_record(nunmai_home):
...
def test_memory_gate_off_allows_write(nunmai_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(nunmai_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(nunmai_home, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.config.load_config", _boom)
...
def test_handle_approve_all(nunmai_home):
...
def test_handle_approve_surfaces_overwritten_entry(nunmai_home):
...
def test_approve_refuses_staged_remove_whose_entry_changed(nunmai_home, shape):
...
@pytest.mark.parametrize("shape", ["single", "batch"])
def test_approve_names_the_entry_a_remove_deleted(nunmai_home, shape):
...
def test_approve_refuses_unpinned_legacy_remove(nunmai_home):
...
def test_handle_approval_on(nunmai_home):
    from nunmai_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(nunmai_home):
    from nunmai_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(nunmai_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(nunmai_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(nunmai_home):
