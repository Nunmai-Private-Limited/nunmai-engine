...
def _assert_nunmai_identity_headers(headers):
    """Both Perplexity endpoints carry the Nunmai identity headers (same set as Kimi/OpenCode)
    ...
    assert headers["X-Pplx-Integration"] == "nunmai-engine"
...
def test_search_dispatch_maps_search_api_shape():
    ...
    _assert_nunmai_identity_headers(post.call_args.kwargs["headers"])
