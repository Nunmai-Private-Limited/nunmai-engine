...
async def _run_browser_flow(tmp_path, monkeypatch, *, issuer, authorization_servers, scope=None):
    ...
    from tools.mcp_oauth import NunmaiTokenStorage, _authorization_code_result
    from tools.mcp_oauth_manager import _NUNMAI_PROVIDER_CLS, reset_manager_for_tests
    ...
    metadata = OAuthClientMetadata(redirect_uris=[AnyUrl("http://127.0.0.1:1/cb")], client_name="Nunmai Engine")
    if scope is not None:
        metadata.scope = scope
    provider = _NUNMAI_PROVIDER_CLS(
        server_name="srv", server_url=RESOURCE, storage=NunmaiTokenStorage("srv"), client_metadata=metadata,
...
async def test_device_flow_normalizes_issuer_and_asks_google_for_offline_access(capsys):
    ...
    for issuer, expects_offline in ((GOOGLE, True), (OTHER_AS, False)):
        ...
        provider = _NUNMAI_PROVIDER_CLS(
            server_name="srv", server_url=RESOURCE, storage=NunmaiTokenStorage("srv"),
            client_metadata=OAuthClientMetadata(redirect_uris=[AnyUrl("http://127.0.0.1:1/cb")], client_name="Nunmai Engine"))
