...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects NUNMAI_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("NUNMAI_INTERACTIVE", "NUNMAI_GATEWAY_SESSION", "NUNMAI_EXEC_ASK", "NUNMAI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["NUNMAI_INTERACTIVE"] = "1"
        session_key = os.getenv("NUNMAI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No NUNMAI_INTERACTIVE or NUNMAI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
