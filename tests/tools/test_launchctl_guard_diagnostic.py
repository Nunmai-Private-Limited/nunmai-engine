...
def test_interpreter_kill_rejection_names_the_owned_process_route(tmp_path, monkeypatch):
    """Scheduled-Task topology (#113667): only ``NUNMAI_SUPERVISED_CHILD`` marks the launch, and an
    ...
    monkeypatch.setenv("NUNMAI_SUPERVISED_CHILD", "1")
    for marker in ("INVOCATION_ID", "XPC_SERVICE_NAME", "NUNMAI_S6_SUPERVISED_CHILD", "NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR"):
    ...
    # Absence on the wrong side: a plain foreground `nunmai gateway run` carries no launch marker.
    monkeypatch.delenv("NUNMAI_SUPERVISED_CHILD")
