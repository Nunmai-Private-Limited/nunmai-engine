...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_nunmai_cli_expands_web(self):
        """nunmai-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"nunmai-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("nunmai-cli", expanded)
    ...
    def test_included_toolsets_of_composite_parent_are_grantable(self):
        ...
        self.assertNotIn("nunmai-cli", expanded)

    def test_composites_with_allowed_included_tools_are_not_stripped(self):
        toolsets = ["safe", "nunmai-gateway", "nunmai-cli", "delegation", "kanban"]

        self.assertEqual(
            _strip_blocked_tools(toolsets),
            ["safe", "nunmai-gateway", "nunmai-cli"],
        )
