...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = NunmaiTokenStorage("shared")
...
def test_nunmai_provider_subclass_exists():
    """NunmaiMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _NUNMAI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _NUNMAI_PROVIDER_CLS is not None
    assert issubclass(_NUNMAI_PROVIDER_CLS, OAuthClientProvider)
