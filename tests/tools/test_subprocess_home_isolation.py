...
Nunmai state stays profile-scoped through NUNMAI_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for nunmai_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(nunmai_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("NUNMAI_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        nunmai_home = real_home / ".nunmai" / "profiles" / "coder"
        profile_home = nunmai_home / "home"
    ...
    def test_host_auto_repairs_missing_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_REAL_HOME", str(real_home))
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        from nunmai_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from nunmai_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("NUNMAI_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["NUNMAI_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(nunmai_home / "home")
        assert result["NUNMAI_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_nunmai_home_unset(self, monkeypatch):
