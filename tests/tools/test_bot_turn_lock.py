...
def test_run_delivery_holds_profile_lock_during_turn(root, tmp_path, monkeypatch):
    """The local `nunmai -p <profile>` turn runs UNDER the profile lock."""
    home = root / ".nunmai"
    home.mkdir()
    ...
    rc = bot_mode_dm._run_delivery(
        ["nunmai", "-p", "ops", "chat"], str(dm), stdin_file=False
    )
    assert rc == 0
    assert observed["argv"][:3] == ["nunmai", "-p", "ops"]
...
def test_delivery_main_reports_target_busy_json(root, tmp_path, monkeypatch, capsys):
    ...
    try:
        rc = bot_mode_dm._delivery_main(
            ["--run-delivery", "query-file", str(dm), "nunmai", "-p", "ops", "chat"]
        )
        assert rc == 1
...
def test_peer_stdin_delivery_skips_local_lock(root, tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(bot_mode_dm.subprocess, "run", _fake_run)
        rc = bot_mode_dm._run_delivery(
            ["nunmai", "peer", "dm", "spark/ops"], str(dm), stdin_file=True
        )
...
def test_local_delivery_command_never_reenters_the_lock():
    ...
    the profile lock. That argv must stay a raw nunmai CLI invocation:
    routing it through the --run-delivery wrapper would make the child hit
    _delivery_lock (nunmai CLI + '-p'), burn the full wait
    ...
    assert Path(argv[0]).name in ("nunmai", "nunmai.exe")
...
def test_relay_deliver_serializes_then_succeeds(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", lambda *a, **k: _Proc())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (subprocess.TimeoutExpired(["nunmai"], 600), 5093, "delivery_timeout"),
...
def test_every_relay_refusal_carries_its_typed_reason(tmp_path, monkeypatch, failure, code, reason):
    ...
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", _raise)
...
def test_delivery_main_reports_every_failure_as_typed_json(tmp_path, monkeypatch, capsys, failure, reason):
    ...
    rc = bot_mode_dm._delivery_main(["--run-delivery", "query-file", str(dm), "nunmai", "-p", "ops", "chat"])
