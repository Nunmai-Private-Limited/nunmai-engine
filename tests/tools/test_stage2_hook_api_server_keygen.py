...
was gated on ``[ -f "$NUNMAI_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'NUNMAI_HOME="{home}"\n'
        # In tests we run unprivileged; as_nunmai is a passthrough then.
        'as_nunmai() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Nunmai loads $NUNMAI_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/nunmai/.env.example -> $NUNMAI_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $NUNMAI_HOME/.env from it on first boot"
    )
