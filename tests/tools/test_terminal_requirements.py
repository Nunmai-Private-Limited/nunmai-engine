...
def _clear_terminal_env(monkeypatch):
    ...
    # Default: no Nunmai subscription — patch both the terminal_tool local
...
def test_local_terminal_requirements(monkeypatch, caplog):
    """Local backend uses Nunmai' own LocalEnvironment wrapper."""
...
def test_modal_backend_managed_mode_without_feature_flag_logs_clear_error(monkeypatch, caplog, tmp_path):
    ...
    assert ok is False
    assert any(
        "Nunmai Tool Gateway access is not currently available" in record.getMessage()
        for record in caplog.records
    )
