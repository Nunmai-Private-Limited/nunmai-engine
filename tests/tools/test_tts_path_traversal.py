...
def test_output_path_rejects_nunmai_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    nunmai_home = tmp_path / "nunmai-home"
    nunmai_home.mkdir()
    monkeypatch.setattr(file_safety, "_nunmai_home_path", lambda: nunmai_home)
    monkeypatch.setattr(file_safety, "_nunmai_root_path", lambda: nunmai_home)

    target = nunmai_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    nunmai_home = tmp_path / "nunmai-home"
    token_dir = nunmai_home / "mcp-tokens"
