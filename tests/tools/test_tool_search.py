...
class TestConfigParsing:
    ...
    def test_defer_default_is_the_registered_list_and_a_user_list_replaces_it(self, caplog):
        """#116404: the curated deferral set lives in DEFAULT_CONFIG (so ``nunmai config set``
...
class TestClassification:
    def test_core_tools_never_defer(self):
        ...
        assert _NUNMAI_CORE_TOOLS
        for core_name in _NUNMAI_CORE_TOOLS:
    ...
    def test_gui_surface_tools_never_defer(self):
        ...
        for name in surface:
            ...
            assert name not in _NUNMAI_CORE_TOOLS
