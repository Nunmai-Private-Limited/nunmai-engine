...
# cwd. Without an explicit PYTHONPATH the child resolves ``nunmai_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_PROFILE", "parent-worker")
    monkeypatch.setenv("NUNMAI_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("NUNMAI_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with NUNMAI_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", "123")
    home = tmp_path / ".nunmai"
    home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote NUNMAI_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".nunmai"
    home.mkdir()
    ...
    monkeypatch.setenv("NUNMAI_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("NUNMAI_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("NUNMAI_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("NUNMAI_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("NUNMAI_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("NUNMAI_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("NUNMAI_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("NUNMAI_DELEGATED_CHILD_CONTEXT") is None
    assert env["NUNMAI_HOME"] == str(home)
    assert env["NUNMAI_DELEGATED_CHILD_CONTEXT"]  # fenced board root (path), not a bare flag
    assert "NUNMAI_KANBAN_TASK" not in env
    assert "NUNMAI_KANBAN_RUN_ID" not in env
    assert "NUNMAI_KANBAN_CLAIM_LOCK" not in env
    # Board location and workspace routing ride along with the fence marker.
    assert env["NUNMAI_KANBAN_DB"] == str(home / "kanban.db")
    assert env["NUNMAI_KANBAN_WORKSPACE"] == str(tmp_path / "parent-workspace")
...
def test_auto_heartbeat_reports_failure_without_mutating_fenced_child_board(
    ...
    try:
        ...
        monkeypatch.setenv("NUNMAI_DELEGATED_CHILD_CONTEXT", str(tmp_path / ".nunmai"))
        ...
        with caplog.at_level(logging.WARNING, logger="tools.kanban_tools"):
            ...
            monkeypatch.setattr(kanban_tools, "_auto_heartbeat_last_attempt", 0.0)
            assert kanban_tools.heartbeat_current_worker_from_env() is False
        fence_warnings = [r for r in caplog.records if "NUNMAI_DELEGATED_CHILD_CONTEXT" in r.getMessage()]
        assert len(fence_warnings) == 1 and tid in fence_warnings[0].getMessage()

        monkeypatch.delenv("NUNMAI_DELEGATED_CHILD_CONTEXT")
        monkeypatch.setattr(kanban_tools, "_auto_heartbeat_last_attempt", 0.0)
