...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "nunmai-in"})
    ...
    def test_missing_media_is_reported_to_the_caller_and_nunmai_send_exits_nonzero(self, tmp_path, monkeypatch):
        """#115908: a MEDIA path that does not exist on the host was dropped with only a host-side
        warning while ``nunmai send`` printed success:true and exited 0. The surviving attachment is
        still sent; the payload names the drop and the CLI exit code follows it."""
        from nunmai_cli.send_cmd import _emit_result
...
class TestSendToPlatformChunking:
    ...
    def test_signal_long_message_is_chunked(self, monkeypatch):
        ...
        The standalone path (nunmai send / cron / MCP) speaks raw JSON-RPC via
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@nunmai:matrix.org", "@nunmai:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``nunmai cron``
    runs separately from ``nunmai gateway``), ``_send_via_adapter`` should
...
def test_not_configured_error_names_resolved_home_and_consulted_sources(tmp_path, monkeypatch):
    ...
    hardcoded ``~/.nunmai``) and what each source held, so a Windows/profile home user can fix the right file."""
...
def test_not_configured_error_names_default_root_gateway_and_secret_sources(tmp_path, monkeypatch):
    """Under ``NUNMAI_HOME=<root>/profiles/<p>`` the error says a live gateway from the default root has the
