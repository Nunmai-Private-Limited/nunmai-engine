...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from nunmai"
...
class TestCheckSendMessage:
    ...
    1. ``NUNMAI_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``NUNMAI_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``NUNMAI_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
