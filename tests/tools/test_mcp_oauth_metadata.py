...
Covers:
- :class:`NunmaiTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.NunmaiMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _NUNMAI_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# NunmaiTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = NunmaiTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
        storage = NunmaiTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (NunmaiMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: NunmaiTokenStorage, **context_attrs):
    ...
    provider = _NUNMAI_PROVIDER_CLS.__new__(_NUNMAI_PROVIDER_CLS)
    provider._nunmai_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
        storage = NunmaiTokenStorage("mgr-srv")
        ...
        with patch.object(
            _NUNMAI_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
        storage = NunmaiTokenStorage("flow-srv")
        ...
        with patch.object(
            _NUNMAI_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
