"""pytest's basetemp must never sit inside the operator's platform-native Nunmai home.

Every per-test sandbox is ``<basetemp>/.../nunmai_test`` and ``get_default_nunmai_root()``
...
def test_basetemp_inside_the_native_home_is_relocated_outside_it(tmp_path, monkeypatch):
    ...
    native.mkdir()
    monkeypatch.setattr(nunmai_constants, "_get_platform_default_nunmai_home", lambda: native)
    ...
    assert relocated.parent.name == "nunmai-pytest" and relocated.parent != Path.home()
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(relocated / "t0" / "nunmai_test"))
    assert nunmai_constants.get_default_nunmai_root() == relocated / "t0" / "nunmai_test"
...
def test_fallback_root_escapes_a_repo_checked_out_inside_the_native_home(tmp_path, monkeypatch):
    # Default install: repo at ~/.nunmai/nunmai-engine and TEMP under the home (Windows).
    native = tmp_path / "native-home"
    (native / "nunmai-engine").mkdir(parents=True)
    monkeypatch.setattr(nunmai_constants, "_get_platform_default_nunmai_home", lambda: native)
    monkeypatch.setattr(suite_conftest, "PROJECT_ROOT", native / "nunmai-engine")
