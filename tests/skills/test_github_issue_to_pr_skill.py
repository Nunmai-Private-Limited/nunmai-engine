...
def test_frontmatter_required_fields():
    ...
    nunmai = fm["metadata"]["nunmai"]
    assert nunmai["tags"]
    assert "related_skills" in nunmai
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Nunmai Engine"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["nunmai"]["related_skills"]:
        hits = (
