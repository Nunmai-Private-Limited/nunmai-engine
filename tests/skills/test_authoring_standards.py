...
def test_required_frontmatter_fields(p):
    ...
    nunmai = (fm.get("metadata") or {}).get("nunmai") or {}
    if not (nunmai.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.nunmai.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    nunmai = (fm.get("metadata") or {}).get("nunmai") or {}
    dangling = [
        rs for rs in (nunmai.get("related_skills") or []) if rs not in _all_names()
    ]
