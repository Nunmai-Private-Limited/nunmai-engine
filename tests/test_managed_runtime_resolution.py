"""Guard: Nunmai-owned subprocesses must not resolve managed runtimes by bare PATH.

Nunmai installs runtimes for itself — ``uv`` at ``$NUNMAI_HOME/bin/uv``, Node at
``$NUNMAI_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Nunmai's own
...
* when a system copy also exists, the one Nunmai does not own wins — which is
...
``iter_nunmai_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Nunmai provisions into NUNMAI_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Nunmai-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("nunmai_cli/update_cmd_deps.py", "uv"): (
    ...
    ("nunmai_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (nunmai_agent-<version>/) gets created and deleted concurrently, and
...
def _resolution_sites() -> frozenset[tuple[str, str, str]]:
    ...
    for path in _source_files():
        rel = path.relative_to(REPO_ROOT).as_posix()
        if rel == "nunmai_platform" or rel.startswith("nunmai_platform/"):
            continue
        try:
            source = path.read_text(encoding="utf-8")
        except (OSError, UnicodeDecodeError):
...
def test_bare_which_and_known_path_tables_are_allowlisted():
    ...
    assert not unlisted, (
        ...
        + "\nuse a nunmai_platform resolver or add a justified allowlist row"
    )


def test_resolution_allowlist_has_no_stale_rows():
    """Remove bootstrap rows as their call sites move to nunmai_platform."""
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Nunmai-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$NUNMAI_HOME/bin (uv) and $NUNMAI_HOME/node are not on an "
        ...
        "  node/npm -> nunmai_constants.find_node_executable()\n"
        "  PATH env -> nunmai_constants.iter_nunmai_node_dirs()\n"
