"""Scratch dir contract: TMPDIR/TMP/TEMP follow NUNMAI_HOME/cache/scratch unless the user set them."""
...
from nunmai_constants import apply_scratch_tmp_env, get_scratch_dir, prune_scratch_dir


def test_scratch_env_follows_home_and_respects_user_tmpdir(tmp_path):
    """Unset temp vars → scratch of env NUNMAI_HOME; a Nunmai-exported value re-derives for a routed
    home; a user/OS-set value (macOS ``/var/folders``, ``%TEMP%``) is never touched."""
    home_a, home_b = tmp_path / "a", tmp_path / "b"
    env = {"NUNMAI_HOME": str(home_a)}
    assert apply_scratch_tmp_env(env) is True
    scratch_a = str(home_a / "cache" / "scratch")
    assert env["TMPDIR"] == env["TMP"] == env["TEMP"] == env["NUNMAI_SCRATCH_DIR"] == scratch_a
    assert os.path.isdir(scratch_a)

    env["NUNMAI_HOME"] = str(home_b)  # child served under another profile's home
    assert apply_scratch_tmp_env(env) is True
    assert env["TMPDIR"] == str(home_b / "cache" / "scratch")

    user_env = {"NUNMAI_HOME": str(home_a), "TMPDIR": "/var/folders/zz"}
    assert apply_scratch_tmp_env(user_env) is False
    assert user_env["TMPDIR"] == "/var/folders/zz" and "NUNMAI_SCRATCH_DIR" not in user_env
...
def test_bootstrap_import_exports_scratch_to_process_and_children(tmp_path):
    """``import nunmai_bootstrap`` alone makes ``tempfile`` (this process AND a child) land in scratch."""
    env = {k: v for k, v in os.environ.items() if k not in ("TMPDIR", "TMP", "TEMP", "NUNMAI_SCRATCH_DIR")}
    env["NUNMAI_HOME"] = str(tmp_path)
    code = ("import tempfile, os, subprocess, sys; import nunmai_bootstrap; "
...
class TestScratchDirPermissionPolicy:
    ...
    an explicit NUNMAI_HOME_MODE and a managed/shared home win (#117347)."""
    ...
    def _isolate_env(self, monkeypatch, tmp_path):
        ...
        for var in ("NUNMAI_HOME_MODE", "NUNMAI_MANAGED", "NUNMAI_CONTAINER",
                    "NUNMAI_SKIP_CHMOD", "NUNMAI_UID", "NUNMAI_GID"):
    ...
    def test_explicit_home_mode_retained_with_setgid(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("NUNMAI_HOME_MODE", "2770")
        scratch = get_scratch_dir(tmp_path, prune=False)
    ...
    def test_managed_env_leaves_preexisting_mode_untouched(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("NUNMAI_MANAGED", "nixos")
    ...
    def test_effective_home_marker_does_not_govern_another_homes_scratch(self, tmp_path, monkeypatch):
        ...
        # ``nunmai doctor`` have already resolved theirs, so the marker lookup must not fall back
        # to ``get_nunmai_home()``: for a sticky profile with NUNMAI_HOME unset that lookup warns
    ...
    def test_canonical_container_signal_without_env_override_keeps_operator_mode(self, tmp_path, monkeypatch):
        # Podman/containerd/K8s runtimes often don't export NUNMAI_CONTAINER; the canonical
        # _detect_container breadth (not the narrower legacy signal set) must skip the chmod.
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setattr("nunmai_constants._detect_container", lambda: True)
        pre = tmp_path / "cache" / "scratch"
        pre.mkdir(parents=True)
        os.chmod(pre, 0o750)
        scratch = get_scratch_dir(tmp_path, prune=False)
    ...
    def test_container_keeps_operator_mode_but_honors_explicit(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("NUNMAI_CONTAINER", "1")
        pre = tmp_path / "cache" / "scratch"
        pre.mkdir(parents=True)
        os.chmod(pre, 0o750)
    ...
    def test_nunmai_uid_gid_applied_to_scratch(self, tmp_path, monkeypatch):
        self._isolate_env(monkeypatch, tmp_path)
        monkeypatch.setenv("NUNMAI_UID", "1000")
        monkeypatch.setenv("NUNMAI_GID", "911")
...
def test_secure_file_skips_chmod_on_canonical_container_signal(tmp_path, monkeypatch):
    ...
    get_scratch_dir already honor (one policy implementation in nunmai_constants)."""
    from nunmai_cli import config

    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "home"))
    for var in ("NUNMAI_MANAGED", "NUNMAI_CONTAINER", "NUNMAI_SKIP_CHMOD"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr("nunmai_constants._detect_container", lambda: True)
