...
def test_tui_import_exposes_auth_registry_to_provider_plugins(tmp_path):
    ...
    plugin_dir = nunmai_home / "plugins" / "model-providers" / "import-order-probe"
    ...
    (plugin_dir / "__init__.py").write_text(
        ...
        "\n"
        "from nunmai_cli.auth import PROVIDER_REGISTRY, ProviderConfig\n"
    ...
    probe = subprocess.run(
        [
            ...
            "assert 'IMPORT_ORDER_PROBE_KEY' in _NUNMAI_PROVIDER_ENV_BLOCKLIST",
        ],
        cwd=REPO_ROOT,
        env=env,
    ...
    assert "partially initialized module 'nunmai_cli.auth'" not in probe.stderr
