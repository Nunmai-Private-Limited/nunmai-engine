...
import nunmai_cli.auth
import providers
from nunmai_cli import auth_oauth_pkce_plugin as pkce
...
def _config(idp: FakeIdP, **overrides) -> pkce.OAuthPKCEConfig:
    base = pkce.OAuthPKCEConfig(client_id="nunmai-example", authorize_url=f"{idp.base}/authorize",
...
def test_pkce_handler_add_status_refresh_logout_against_fake_idp(idp, monkeypatch, tmp_path, request):
    ...
    monkeypatch.setattr("nunmai_cli.auth_device_flow._can_open_graphical_browser", lambda: True)
    cfg = _config(idp)
    ...
    request.addfinalizer(lambda: (providers._REGISTRY.pop(PROVIDER, None),
                                  nunmai_cli.auth.PROVIDER_REGISTRY.pop(PROVIDER, None)))
    ...
    rows = json.loads((tmp_path / "nunmai" / "auth.json").read_text())["credential_pool"][PROVIDER]
    assert [(r["auth_type"], r["source"], r["oauth_pkce"]["client_id"]) for r in rows] == [
        ("oauth", pkce.POOL_SOURCE, "nunmai-example")]
...
def test_spent_refresh_token_is_grant_dead_and_marks_the_pool_row_dead(idp, monkeypatch, tmp_path, request):
    ...
    disk = json.loads((tmp_path / "nunmai" / "auth.json").read_text())["credential_pool"][PROVIDER][0]
...
def test_alias_login_stores_the_row_under_the_canonical_profile_name(idp, monkeypatch, tmp_path, request):
    """nunmai auth add <alias> must write the pool under ProviderProfile.name, not the typed alias."""
    from agent.credential_pool import load_pool
    ...
    request.addfinalizer(lambda: (
        ...
        nunmai_cli.auth.PROVIDER_REGISTRY.pop(PROVIDER, None),
        nunmai_cli.auth.PROVIDER_REGISTRY.pop(alias, None)))
    args = SimpleNamespace(provider=alias, no_browser=False)
    assert handler("add", args) is True
    pool = json.loads((tmp_path / "nunmai" / "auth.json").read_text())["credential_pool"]
