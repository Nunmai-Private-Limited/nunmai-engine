"""Regression tests for #102123: plugins discovered after ``nunmai_cli.auth`` is
first imported must still reach ``PROVIDER_REGISTRY``.

``nunmai_cli.auth`` mirrors provider-plugin profiles into ``PROVIDER_REGISTRY``
when it is imported.  If a plugin's own imports pull ``nunmai_cli.auth`` in
...
def _run_probe(nunmai_home: Path, code: str) -> subprocess.CompletedProcess:
    env = os.environ.copy()
    env["NUNMAI_HOME"] = str(nunmai_home)
    env.pop("NUNMAI_PROFILE", None)
...
def test_plugins_discovered_after_auth_import_resolve(tmp_path):
    ...
    # Sorted first: a plugin whose imports drag nunmai_cli.auth in mid-discovery
    # (any plugin importing agent.credential_pool or similar does this).
    _write_plugin(
        nunmai_home,
        "aaa-early-probe",
        "import nunmai_cli.auth  # noqa: F401 — simulate a core-importing plugin\n"
...
# never imports nunmai_cli.auth on its own).
# ---------------------------------------------------------------------------
...
def test_user_plugin_alias_repoints_and_display_name_follows(_isolated_registries, monkeypatch, tmp_path):
    """A $NUNMAI_HOME plugin owns the aliases it declares and the display name of a same-name row.
