...
class TestToolsetConsistency:
    ...
    def test_nunmai_platforms_share_core_tools(self):
        """All nunmai-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nunmai-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nunmai-cli", "nunmai-telegram", "nunmai-discord", "nunmai-whatsapp", "nunmai-slack", "nunmai-signal", "nunmai-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_nunmai_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("nunmai-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("nunmai-cli")
        second = resolve_toolset("nunmai-cli")
        ...
        assert (
            "nunmai-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("nunmai-cli", include_registry=False)
        second = resolve_toolset("nunmai-cli", include_registry=False)
        assert first == second
