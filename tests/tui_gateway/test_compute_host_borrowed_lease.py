...
def _registry() -> list[dict]:
    path = os.path.join(os.environ["NUNMAI_HOME"], "runtime", "active_sessions.json")
...
def isolated_env(monkeypatch, tmp_path):
    ...
    conftest-sandboxed NUNMAI_HOME registry while nothing calls a provider."""
...
def test_isolated_turn_without_matching_vouch_still_fails_closed(isolated_env):
    ...
    try:
        ...
        assert errors and "open in another Nunmai window" in errors[0]
