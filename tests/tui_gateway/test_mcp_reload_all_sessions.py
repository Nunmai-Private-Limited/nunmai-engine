...
def reload_env(monkeypatch, tmp_path):
    ...
    def _discover():
        discovered_homes.append(nunmai_constants.nunmai_home_key())
        if current_secret_scope() is not None:
            scoped_homes.append(nunmai_constants.nunmai_home_key())
...
def test_reload_rediscovers_under_each_live_profile_scope(reload_env):
    ...
    assert nunmai_constants.nunmai_home_key() in reload_env.discovered_homes
    assert nunmai_constants.nunmai_home_key(reload_env.profile_b) in reload_env.discovered_homes
...
def test_reload_rediscovers_the_launch_profile_under_its_own_secret_scope(reload_env):
    ...
    assert nunmai_constants.nunmai_home_key() in reload_env.scoped_homes
