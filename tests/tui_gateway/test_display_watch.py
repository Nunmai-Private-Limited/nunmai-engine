"""Lease transitions made by ANOTHER process reach `nunmai serve` clients as ``display.lease``.

The agent lives in whatever process hosts it (messaging gateway, ``nunmai chat``, a cron worker);
...
def _watching(server, home: Path, monkeypatch) -> list:
    ...
    monkeypatch.setattr(server, "_nunmai_home", str(home))
    server._ensure_lease_watcher()
    assert _wait_for(lambda: nunmai_home_key(home) in server._lease_epochs), "watcher never seeded the home"
    return events
...
def test_takeover_in_another_process_is_broadcast(tmp_path, monkeypatch):
    ...
    assert payload["profile_key"] == nunmai_home_key(home)
