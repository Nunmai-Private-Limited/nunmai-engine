    ...
    with patch.dict(
        "sys.modules",
        {
            "nunmai_constants": MagicMock(
                get_nunmai_home=MagicMock(return_value="/tmp/nunmai_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nunmai-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nunmai-release' patched"
    }
