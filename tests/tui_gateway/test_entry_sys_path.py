...
Nunmai's own top-level modules and crashed the backend on import
(``ImportError: cannot import name ... from 'utils'``). entry.py must run
``nunmai_bootstrap.harden_import_path()`` before its first non-stdlib import.
...
def test_entry_imports_from_cwd_with_colliding_packages(tmp_path):
    ...
    (and friends) must succeed — the guard strips CWD so Nunmai's modules win."""
    ...
    env["NUNMAI_HOME"] = str(tmp_path / "nunmai_home")
