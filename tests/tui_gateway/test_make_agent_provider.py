...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("nunmai_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("nunmai_cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("nunmai_cli.model_switch.switch_model", return_value=_FakeResult()),
        ...
        patch("nunmai_cli.model_switch.persist_model_selection") as mock_persist,
        patch(
...
def test_resumed_row_cannot_pin_stale_wire_onto_per_model_provider():
    ...
    with patch("nunmai_cli.runtime_provider.resolve_runtime_provider", side_effect=fake_resolve):
