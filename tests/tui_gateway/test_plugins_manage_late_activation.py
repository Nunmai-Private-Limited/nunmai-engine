...
def _fake_install_core(identifier, *, force=False, ref=None):
    ...
    from nunmai_cli.plugins_cmd import _plugins_dir, _read_manifest
...
def test_plugins_manage_install_rescans_fires_on_plugin_loaded_and_exposes_mcp_servers():
    (Path(os.environ["NUNMAI_HOME"]) / "plugins").mkdir(exist_ok=True)
    manager = get_plugin_manager()
    manager.discover_and_load()  # the serve process booted long before the install
    events: list = []
    manager.on_plugin_loaded(events.append)
    with patch("nunmai_cli.plugins_cmd._install_plugin_core", _fake_install_core), \
         patch("nunmai_cli.plugins_cmd._install_python_dependencies_quietly", return_value=[]), \
