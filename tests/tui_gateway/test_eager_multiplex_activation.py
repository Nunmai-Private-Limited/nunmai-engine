...
empty ``.env``, which is all a crashed ``nunmai profile create`` leaves behind. Both made a host
...
def two_profile_host(tmp_path, monkeypatch):
    """Fake HOME so ``profiles/`` never resolves to the live install (see nunmai-engine-dev)."""
    home = tmp_path / "fakehome" / ".nunmai"
    ...
    from nunmai_cli.profiles import _get_profiles_root
