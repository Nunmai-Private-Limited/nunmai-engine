...
launch home, not whatever ``get_nunmai_home()`` resolves to at first-touch
...
def launch_db_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_nunmai_home", str(launch_home))
...
def test_get_db_first_touch_under_foreign_override_uses_launch_path(launch_db_env):
    ...
    token = set_nunmai_home_override(str(foreign_home))
    try:
        db = server._get_db()
...
def test_get_db_follows_a_process_home_redirected_after_import(monkeypatch, tmp_path):
    """The launch handle resolves ``NUNMAI_HOME`` at first use, not at import (#112692): a harness
    ...
    monkeypatch.setattr(server, "_db", None)
    monkeypatch.setattr(server, "_db_error", None)
    monkeypatch.setenv("NUNMAI_HOME", str(redirected))
    try:
        db = server._get_db()
        assert db is not None
...
def test_insights_get_reads_the_requested_profile_store_not_the_launch_handle(launch_db_env, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nunmai_cli.profiles.get_profile_dir", lambda name: profiles_root / name)
