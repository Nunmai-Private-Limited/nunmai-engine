...
guard is covered in ``tests/nunmai_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``nunmai_cli.model_switch_providers.prewarm_picker_cache_async``
...
import nunmai_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from nunmai_cli.model_switch,
