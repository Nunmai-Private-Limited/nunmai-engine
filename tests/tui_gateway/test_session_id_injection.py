...
session id into NUNMAI_SESSION_ID so terminal/execute_code subprocesses can
...
saw an empty NUNMAI_SESSION_ID even though agent_init had set it via
...
def test_set_session_context_injects_agent_session_id(monkeypatch):
    """NUNMAI_SESSION_ID must equal the live agent.session_id after binding."""
    _install_session(
    ...
    assert get_session_env("NUNMAI_SESSION_ID") == "20260722_deadbeef"
...
def test_set_session_context_falls_back_to_session_key(monkeypatch):
    ...
    (never leave NUNMAI_SESSION_ID empty for an identified session)."""
    _install_session(monkeypatch, session_key="skey-xyz", agent_session_id=None)

    server._set_session_context("skey-xyz")

    assert get_session_env("NUNMAI_SESSION_ID") == "skey-xyz"
...
def test_set_session_context_injects_session_profile(tmp_path, monkeypatch):
    ...
    assert get_session_env("NUNMAI_SESSION_PROFILE") == "work"
...
def test_set_session_context_uses_launch_profile_without_override(monkeypatch):
    ...
    assert get_session_env("NUNMAI_SESSION_PROFILE") == "default"
