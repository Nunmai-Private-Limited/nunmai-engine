...
def test_settled_discussion_retry_publishes_once(tmp_path, monkeypatch, later_messages):
    ...
    (tmp_path / ".nunmai" / "profiles" / "reviewer").mkdir(parents=True)
    (tmp_path / ".nunmai" / "profiles" / "reviewer" / "config.yaml").write_text("{}\n")  # identity marker
    server = SimpleNamespace(_methods={}, _sessions={}, _sessions_lock=threading.Lock())
    service = HostedRoomService(server, db_path=tmp_path / ".nunmai" / "state.db")
