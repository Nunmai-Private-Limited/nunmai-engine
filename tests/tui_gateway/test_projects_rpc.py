...
def test_record_repos_persists_and_shows_zero_session_repo(tmp_path):
    ...
    # Repo-first: a scanned repo with no nunmai sessions still surfaces.
...
def test_scan_time_is_not_treated_as_session_activity(tmp_path):
    ...
    in Nunmai outranked the ones they actually work in.
    """
...
def test_terminal_session_persists_its_launch_cwd():
    ...
    The user cd'd into that directory before running nunmai. Dropping it left
...
def test_desktop_agent_rebuild_preserves_workspace_provenance(
    ...
    session = {
        "agent": object(),
        "attached_images": [],
        "cwd": "/picked/repo" if explicit_cwd else "/opt/nunmai",
        "edit_snapshots": {},
...
def _bind_profiles(monkeypatch, tmp_path: Path, homes: dict[str, Path]) -> None:
    ...
    """
    monkeypatch.setattr(
        "nunmai_cli.profiles.get_profile_dir",
...
def _serving_launch_profile(launch_home: Path):
    ...
    Both the ambient override AND ``server._nunmai_home`` point at it: once the process
    ...
    token = set_nunmai_home_override(launch_home)
    prev_db, prev_error, prev_home = server._db, server._db_error, server._nunmai_home
    server._nunmai_home = launch_home
    ...
    try:
        ...
        server._db.close()
        server._db, server._db_error, server._nunmai_home = prev_db, prev_error, prev_home
        reset_nunmai_home_override(token)
...
def test_projects_without_a_profile_stay_on_the_launch_home(monkeypatch, tmp_path):
    ...
    assert not (Path(os.environ["NUNMAI_HOME"]) / "projects.db").exists()
