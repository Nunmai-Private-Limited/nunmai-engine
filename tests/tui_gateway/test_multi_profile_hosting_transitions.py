...
* releasing a runtime scope is per-reset best-effort: a failing terminal reset must not leave the
  previous profile's secrets / NUNMAI_HOME installed for the next body in that context.
"""
...
def test_send_keeps_external_source_value_over_raw_dotenv(two_homes, monkeypatch):
    ...
    wins) survives ``_load_nunmai_env`` for the routed ``send``."""
    from nunmai_cli import env_loader
    from nunmai_cli.send_cmd import _load_nunmai_env
...
def test_launch_body_survives_first_secondary_activation_on_the_dashboard(two_homes, monkeypatch):
    ...
    from nunmai_cli import web_server_profiles as wsp
...
def test_release_resets_every_scope_when_one_reset_fails(two_homes, monkeypatch):
    ...
    assert current_secret_scope() is not None and get_nunmai_home_override() == str(b)
    ...
    assert Path(server._nunmai_home) == root
