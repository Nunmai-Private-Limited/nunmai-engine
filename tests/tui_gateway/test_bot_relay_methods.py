...
from nunmai_cli.dashboard_auth.ws_tickets import INTERNAL_PROVIDER, INTERNAL_USER_ID
...
def home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(h))
    return h
...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="nunmai"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", _fake_run)
    out = _result(
    ...
    assert argv[0].rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("nunmai", "nunmai.exe")
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'nunmai' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "nunmai", "message": "x"}))
...
def test_deliver_restamps_relayed_sender_with_a_reply_safe_handle(home, monkeypatch):
    ...
    ``@nunmai`` — the recipient's OWN default. The delivered text names the sender by the form this
    ...
    # (#114980) — patching only the first would spawn a real ``nunmai chat -Q`` child there.
    monkeypatch.setattr("subprocess.run", _fake_run)
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", _fake_run, raising=False)
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "nunmai", "connection_id": "vps-1", "title": "CoS Bot"},
    ])
    stamp = "Message from 🤖 CoS Bot (@nunmai): are we done?"
    sender = {"from_profile": "default", "from_handle": "nunmai", "from_connection": "vps-1"}
    ...
    _result(srv._methods["bot_relay.deliver"](3, {"profile": "ops", "message": "plain text (@nunmai): x", **sender}))
    assert seen == ["Message from 🤖 CoS Bot (@cos-bot): are we done?",
                    "Message from 🤖 CoS Bot (@nunmai@lan-2): are we done?",
                    "plain text (@nunmai): x"]
...
def test_deliver_relays_empty_reply_for_a_bare_silence_marker(home, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", lambda *_a, **_k: _Proc())
...
def test_deliver_lands_in_live_bot_chat_instead_of_subprocess(home, monkeypatch):
    ...
    def _fake_run(argv, *a, **k):
        ...
        # daemon thread; only the relay's `nunmai` CLI spawn is under test.
...
def _no_cli_transport(monkeypatch, spawned):
    ...
    # quiet_single_query.run_reported_turn once the relay books turns from their report
    # (#114980) — guarding only the first would let a real ``nunmai chat -Q`` child spawn there.
    monkeypatch.setattr("subprocess.run", _fake_run)
    monkeypatch.setattr("nunmai_cli.quiet_single_query.run_reported_turn", _fake_run, raising=False)
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "nunmai-relay-dm-*")), "tempfile leaked"
...
def test_deliver_child_env_carries_the_envelope_sender_on_every_attempt(home, monkeypatch, fake_runs, sender, expected):
    """NUNMAI_TURN_AUTHOR on the child comes from the envelope's sender fields alone: the retry gets the same
    ...
    assert all(e["NUNMAI_RELAY_TEST_MARKER"] == "kept" for e in envs)
...
def test_deliver_accepts_a_sender_from_an_admitted_non_login_client(home, fake_runs, bound_client, identity):
    ...
    gateway that requires sign-in (``nunmai_cli/dashboard_auth/routes.py``), so it is a login
...
def test_gateway_drains_the_mailbox_the_tools_write_to(tmp_path, monkeypatch, subdir):
    """Both ends of the relay mailbox derive the install root from NUNMAI_HOME with ONE formula.
    The writer side (``message_agent``'s ``_nunmai_root``) and the drain side
    (``methods_bot_relay._relay_root``) must agree for a ``profiles/<name>`` home AND for an
    arbitrary subdir of the native ``~/.nunmai`` — a split here is silent non-delivery."""
    from tools.bot_mode_probe import _default_home, _nunmai_root
    from tui_gateway import methods_bot_relay

    monkeypatch.setenv("HOME", str(tmp_path))
    home = tmp_path / ".nunmai" / subdir
    home.mkdir(parents=True)
    monkeypatch.setenv("NUNMAI_HOME", str(home))

    writer_root = _nunmai_root(Path(_default_home()))
    target = {"profile": "scout", "handle": "scout", "connection_id": "cloud-1",
              "connection_label": "", "title": "", "description": ""}
    env = bot_relay.enqueue_envelope(
        writer_root, target=target, message="m", sender_profile="default", sender_handle="nunmai")
...
def _child_argv(monkeypatch, body: str) -> dict:
    """Stand in a Python child for the ``nunmai`` transport; it imports ``nunmai_cli`` from this checkout."""
...
def test_reported_turn_still_lingering_at_the_cap_is_booked_from_its_latest_report_not_killed(tmp_path, monkeypatch):
    ...
    env = _child_argv(monkeypatch, """
        import os, time
        from nunmai_cli.quiet_single_query import TURN_REPORT_FILE_ENV, write_turn_report
