...
def test_the_cas_writer_round_trips_through_the_listing(home):
    ...
    before = _row()["ui_meta_revisions"]["nunmai-bots"]

    envelope = srv._methods["profiles.configure"](2, {
        "name": "bob",
        "ui_meta": {"nunmai-bots": {"title": "Bobby"}},
        "ui_meta_expected_revisions": {"nunmai-bots": before},
    })
    assert envelope["result"]["applied"]["ui_meta"] is True

    row = _row()
    assert row["ui_meta"]["nunmai-bots"]["title"] == "Bobby"
    assert row["ui_meta_revisions"]["nunmai-bots"] == before + 1
