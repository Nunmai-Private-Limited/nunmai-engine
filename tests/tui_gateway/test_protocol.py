...
def server():
    ...
    # e.g. nunmai_cli.active_sessions would bind the mocked get_nunmai_home
    ...
    with patch.dict("sys.modules", {
        "nunmai_constants": MagicMock(get_nunmai_home=MagicMock(return_value="/tmp/nunmai_test")),
        "nunmai_cli.env_loader": MagicMock(),
        "nunmai_cli.banner": MagicMock(),
        "nunmai_state": MagicMock(),
    }):
        import importlib
...
def test_config_roundtrip(server, tmp_path, monkeypatch):
    # monkeypatch, not assignment: a bare ``server._nunmai_home = tmp_path`` outlives this test and every
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/nunmai-engine-dev": {"name": "nunmai-engine-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "nunmai-engine-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient NUNMAI_HOME
    ...
    # The gateway's own NUNMAI_HOME (the test-isolation tempdir, no
...
def test_slash_exec_routes_a_secondary_only_bundle_to_dispatch(server, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_BUNDLES_DIR", raising=False)
    profile_b = tmp_path / "profile_b"
    external_b = tmp_path / "external_b"
    for name in ("one", "two"):
        d = external_b / name
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `nunmai config set` does)
    ...
    import nunmai_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_nunmai_home", lambda: tmp_path)
    monkeypatch.setattr(server, "_nunmai_home", tmp_path)
    ...
    # Activate midnight, as `nunmai config set display.skin midnight` would.
...
def test_peerless_global_broadcast_never_reaches_stdout_in_ws_backend(capture, monkeypatch):
    """`nunmai serve` / dashboard speak JSON-RPC over WS only; Desktop captures their stdout
    ...
    printed (~1100 `[nunmai] {"jsonrpc": ...}` lines in desktop.log)."""
    server, buf = capture
