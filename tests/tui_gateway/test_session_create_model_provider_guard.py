...
def _create(monkeypatch, tmp_path):
    monkeypatch.setattr("nunmai_cli.banner.prefetch_update_check", lambda: None)
...
@pytest.mark.parametrize("params", [
    ...
    {"model": "gpt-5.5", "provider": "custom:local"},  # custom endpoint: Nunmai cannot know its models
