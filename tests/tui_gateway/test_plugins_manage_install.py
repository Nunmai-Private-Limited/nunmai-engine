...
def test_plugins_manage_install_failure():
    with patch(
        "nunmai_cli.plugins_cmd.dashboard_install_plugin",
...
def test_plugins_manage_update_requires_catalog_sidecar(tmp_path, monkeypatch):
    ...
    import nunmai_cli.plugins_cmd as plugins_cmd
...
def test_plugins_manage_list_reports_desktop_half(tmp_path):
    ...
    with patch("nunmai_cli.plugins_cmd._discover_all_plugins", return_value=rows), \
         ...
         patch("nunmai_cli.plugins_cmd_catalog.catalog_pins", return_value={}):
