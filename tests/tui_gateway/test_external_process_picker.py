...
Drives a fake process profile (registered like a ``$NUNMAI_HOME/plugins/<name>`` model-provider plugin
would) through ``list_available_providers``, ``provider_model_ids``, the TUI/Desktop ``model.options``
RPC, the ``nunmai model`` setup picker and a session-scoped ``config.set model`` switch.
"""
import shutil
import sys
...
def picker_env(monkeypatch, tmp_path):
    ...
    import nunmai_cli.inventory as inventory
...
def test_process_provider_reaches_every_shared_picker(picker_env, monkeypatch):
    ...
    from nunmai_cli.models import _PROVIDER_LABELS, list_available_providers, provider_model_ids
    ...
    from nunmai_cli import main, model_setup_flows
