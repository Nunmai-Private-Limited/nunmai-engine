...
Behavior contracts, exercised against a real temp NUNMAI_HOME (no mocks on the
...
def watcher_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_nunmai_home", str(tmp_path))
...
def test_served_profile_store_move_broadcasts_sessions_changed(watcher_home, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.profiles.get_profile_dir", lambda name: home / "profiles" / name)
...
def test_pairing_probe_reuses_live_profile_roots_until_the_profile_set_moves(watcher_home, monkeypatch):
    ...
    home, events = watcher_home
    live_calls = []
    real_live = nunmai_constants.named_profile_is_live
    monkeypatch.setattr(nunmai_constants, "named_profile_is_live",
