...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """A temp NUNMAI_HOME root with one named profile, ``bot``, at ``<root>/profiles/bot``."""
...
def test_describe_matches_the_runtime_once_legacy_disabled_is_migrated(profile_dir):
    ...
    _write_mcp(profile_dir, {
        ...
        # `nunmai mcp add` writes `enabled: true`; the old editor then only added `disabled: true`.
