...
def nunmai_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves NUNMAI_HOME.
...
def server(nunmai_home, monkeypatch):
    ...
    # Pin config resolution to the isolated NUNMAI_HOME. Sibling test
    ...
    # module-level ``_nunmai_home = get_nunmai_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, nunmai_home):
    _write_moa_config(nunmai_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_nunmai_home())
        started.set()
