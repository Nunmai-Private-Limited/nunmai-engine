...
Regression for #118538: routed turns bind ``_profile_runtime_scope(home)`` (NUNMAI_HOME override +
...
def two_homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_nunmai_home", launch)
    ...
    def stub_pre_tool_call(**_kw):
        ...
        seen.append({"home": get_nunmai_home().name, "x": (entry.get("settings") or {}).get("x"),
                     "bound": get_nunmai_home_override() is not None})
        return None
...
def _fire_off_turn_workers(launch_manager_hook_seen):
    ...
    def observer(**_kw):
        observed.append({"home": get_nunmai_home().name, "bound": get_nunmai_home_override() is not None})
...
def test_launch_profile_turn_hooks_see_bound_scope_a_b_a(two_homes):
    ...
    assert [(r["home"], r["x"]) for r in seen] == [
        ("nunmai_home", "alpha"), ("beta", "beta"), ("nunmai_home", "alpha")]
    assert all(r["bound"] for r in seen), seen
    assert off_turn_a == [{"home": "nunmai_home", "bound": True}] * 2, off_turn_a
    assert off_turn_b == [{"home": "beta", "bound": True}] * 2, off_turn_b
    assert get_nunmai_home_override() is None  # every scope released
...
def test_single_profile_launch_scope_binds_no_override(two_homes):
    ...
    try:
        ...
        server._release_profile_runtime_scope_tokens(scopes)
    assert seen == [{"home": "nunmai_home", "x": "alpha", "bound": False}]
    assert Path(get_nunmai_home()) == launch
