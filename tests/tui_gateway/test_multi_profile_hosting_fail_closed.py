...
Regression for the silent cross-profile secret leak class: ``nunmai serve`` hosted many profile
...
only NUNMAI_HOME. And the launch-profile asymmetry: a default-member hosted-room turn in a
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        f"A_ONLY_TOKEN={A_VAL}\nNUNMAI_API_KEY={A_API_KEY}\nNUNMAI_BASE_URL={A_BASE_URL}\n"
        f"NUNMAI_CODEX_BASE_URL={A_CODEX_URL}\n",
        encoding="utf-8")
    (b / ".env").write_text(
        f"B_ONLY_TOKEN={B_VAL}\nNUNMAI_API_KEY={B_API_KEY}\nNUNMAI_BASE_URL={B_BASE_URL}\n"
        f"NUNMAI_CODEX_BASE_URL={B_CODEX_URL}\n",
        encoding="utf-8")
    for home in (root, b):
    ...
    monkeypatch.setenv("NUNMAI_API_KEY", A_API_KEY)
    monkeypatch.setenv("NUNMAI_BASE_URL", A_BASE_URL)
    monkeypatch.setenv("NUNMAI_CODEX_BASE_URL", A_CODEX_URL)
    monkeypatch.setenv("INJECTED_TOKEN", ENV_VAL)  # systemd / op run credential injection
    monkeypatch.setattr(server, "_nunmai_home", root)
...
def test_rpc_scope_reaches_llm_oneshot_and_model_options(two_homes, monkeypatch):
    ...
    def fake_oneshot(**kwargs):
        seen["oneshot"] = (Path(os.environ.get("NUNMAI_HOME", "")), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        from nunmai_constants import get_nunmai_home
        seen["oneshot_home"] = Path(get_nunmai_home())
        return "t"
    ...
    def build_payload(ctx, **kwargs):
        from nunmai_constants import get_nunmai_home
        seen["options"] = (Path(get_nunmai_home()), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        return {"providers": []}

    monkeypatch.setattr("nunmai_cli.inventory.build_model_options_payload", build_payload)
...
def test_manual_compress_routes_bind_the_sessions_full_runtime_scope(two_homes, monkeypatch, route):
    ...
    def observe_scope():
        seen.append((Path(get_nunmai_home()), get_secret("A_ONLY_TOKEN"), get_secret("B_ONLY_TOKEN")))
...
def test_live_review_binds_runtime_scope_under_multiplex(two_homes, monkeypatch):
    ...
    def fake_start_review(agent, snapshot, prompt):
        seen.append((
            Path(get_nunmai_home()),
            get_secret("A_ONLY_TOKEN"),
            get_secret("B_ONLY_TOKEN"),
            get_secret("NUNMAI_CODEX_BASE_URL"),
        ))
    ...
    assert os.environ["NUNMAI_CODEX_BASE_URL"] == A_CODEX_URL
...
def test_launch_profile_agent_build_is_scoped_once_multiplexing(two_homes, monkeypatch):
    ...
    try:
        ...
        assert Path(get_nunmai_home()) == root
    finally:
...
class _MemoryManager:
    ...
    def build_system_prompt(self):
        from agent.secret_scope import get_secret
        from nunmai_constants import get_nunmai_home
        return f"{get_nunmai_home()}|{get_secret('MEM_PROVIDER_KEY')}"
