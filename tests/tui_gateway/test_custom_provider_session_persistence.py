...
"No LLM provider configured. Run `nunmai model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real NUNMAI home.
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("nunmai-ultra-sft")
            == "custom:nunmai-ultra"
        )
...
class TestFollowProfileConfigRuntimeOverrides:
    def test_composer_pick_on_bot_chat_survives_resume_until_profile_model_changes(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(server, "_nunmai_home", str(launch))
        ...
        try:
            with (
                patch("nunmai_cli.model_switch.parse_model_flags", return_value=("glm-5.1", None, False, False, None)),
                patch("nunmai_cli.model_switch.resolve_persist_behavior", return_value=False),
                patch("nunmai_cli.model_switch.switch_model", return_value=result),
