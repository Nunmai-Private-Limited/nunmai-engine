...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nunmai_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("nunmai_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_nunmai_home() == secondary
    assert get_nunmai_home() == launch
