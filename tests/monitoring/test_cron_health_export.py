...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("nunmai.gateway.up"), _M("nunmai.gateway.active_agents"),
        _M("nunmai.gateway.busy"), _M("nunmai.gateway.drainable"),
        _M("nunmai.gateway.restart_requested"),
        _M("nunmai.platform.up"), _M("nunmai.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("nunmai.cron.scheduler.heartbeat_age_seconds"),
        _M("nunmai.cron.scheduler.last_success_age_seconds"),
        _M("nunmai.cron.scheduler.catch_up_occurrences"),
        _M("nunmai.cron.jobs.enabled"), _M("nunmai.cron.jobs.running"),
        _M("nunmai.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Nunmai Engine-owned Relay transport health" in text
