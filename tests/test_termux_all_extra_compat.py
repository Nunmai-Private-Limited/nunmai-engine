...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"nunmai-engine[termux]"' in text
    assert '"nunmai-engine[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"nunmai-engine[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
