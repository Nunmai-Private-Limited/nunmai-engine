...
NUNMAI_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env NUNMAI_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``nunmai config get terminal.backend`` still says local.
    """
    nunmai_home = tmp_path / ".nunmai"
    nunmai_home.mkdir()
    (nunmai_home / "config.yaml").write_text(
    ...
    (nunmai_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
