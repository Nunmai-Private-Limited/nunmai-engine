...
def test_supervised_child_marker_is_a_launch_not_a_restart_route():
    """The Windows Scheduled-Task launcher exports only ``NUNMAI_SUPERVISED_CHILD``: that must make
    ...
    from nunmai_cli.gateway_windows import _GATEWAY_ENV

    task_env = dict(_GATEWAY_ENV)
    assert task_env["NUNMAI_SUPERVISED_CHILD"] == "1"
