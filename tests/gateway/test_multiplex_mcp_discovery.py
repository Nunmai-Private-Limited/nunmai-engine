...
async def test_gateway_boot_discovers_mcp_under_every_profile_home(
    ...
    def fake_discover() -> list[str]:
        seen.append((get_nunmai_home(), threading.current_thread().name))
        return []
...
async def test_reload_mcp_only_touches_requesting_profile(
    ...
    worker_scope = nunmai_home_key(worker_home)
    ...
    monkeypatch.setattr(
        ...
        {"default-srv": nunmai_home_key(tmp_path), "worker-srv": worker_scope},
    )
    seen: list[tuple] = []

    def fake_shutdown(*, scope=None) -> None:
        seen.append(("shutdown", scope, get_nunmai_home()))

    def fake_discover() -> list[str]:
        seen.append(("discover", get_nunmai_home()))
        return []
...
async def test_reload_mcp_reports_a_shared_server_to_a_non_owner_profile(
    ...
    worker_home.mkdir(parents=True)
    worker_scope = nunmai_home_key(worker_home)
    launch_scope = nunmai_home_key(tmp_path / "default")
...
def test_scope_visibility_rejects_a_foreign_or_differently_authenticated_route(
    ...
    worker_scope = nunmai_home_key(tmp_path / "worker")
    launch_scope = nunmai_home_key(tmp_path / "default")
...
def test_shared_server_tools_are_callable_and_removed_on_non_owner_reload(
    ...
    worker_home.mkdir(parents=True)
    launch_home.mkdir()
    worker_token = set_nunmai_home_override(worker_home)
    previous_multiplex = set_multiplex_active(True)
    worker_scope = nunmai_home_key()
    launch_scope = nunmai_home_key(launch_home)
    ...
    try:
        ...
        reset_nunmai_home_override(worker_token)
...
def test_deregister_scope_kwarg_targets_overlay_and_keeps_plugin_confinement() -> None:
    ...
    reg._plugin_module_scopes["nunmai_plugins.p"] = {"/home/p1"}
    reg._caller_module = staticmethod(lambda: "nunmai_plugins.p")
