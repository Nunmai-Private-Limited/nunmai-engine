...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under NUNMAI_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.nunmai/data/watchlist.json",
                            },
                            {
...
class TestCrossPlatformDeliveryMirror:
    ...
    def homes(self, tmp_path, monkeypatch):
        ...
        # The hermetic conftest pins DEFAULT_DB_PATH when nunmai_state is already imported; un-pin it so
...
class TestMultiplexProfileWebhookAuthentication:
    ...
    async def test_routed_profile_skills_resolve_under_that_profile(
        ...
        monkeypatch.setattr(
            "nunmai_cli.profiles.get_profile_dir", lambda name: tmp_path / "profiles" / name
        )
        route_secret = "worker-route-secret-abc123"
        adapter = _make_adapter(
            routes={
