...
``gateway.multiplex_profiles`` the skills-sync pulls resolved Nunmai credentials through the
...
def two_homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(a))
    monkeypatch.delenv("NOUS_INFERENCE_BASE_URL", raising=False)
    # The hermetic conftest pins ``nunmai_state.DEFAULT_DB_PATH`` at one sandbox store whenever
    # nunmai_state is already imported, and that pin WINS over ``get_nunmai_home()`` inside
    ...
    resolved = Path(nunmai_state._default_db_path())
...
def _record_credential_chores(monkeypatch):
    """Replace the three credential-reading chores with recorders of (home, Nunmai override) they see."""
    import agent.curator as curator
    import tools.skills_sync_client as ssc
    import tools.skills_sync_client_org as sso
    from nunmai_cli.auth_nous import _nous_inference_env_override
    from nunmai_constants import get_nunmai_home

    seen: dict = {"sync": [], "org": [], "curator": []}

    def _rec(key):
        return lambda *a, **k: seen[key].append((get_nunmai_home().name, _nous_inference_env_override()))
...
def test_multiplexed_auto_archive_tick_sweeps_every_served_profile_store(two_homes, monkeypatch):
    ...
    ``acquire()`` resolves through ``get_nunmai_home()``, so an unscoped tick archived the
    launch profile's store only — and `nunmai serve`/the dashboard defer to the gateway for
...
def test_profile_scope_setup_failure_restores_the_callers_home(two_homes, monkeypatch):
    ...
    resolving ``get_nunmai_home()`` to the failed profile for every later unscoped read.
    """
