...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.nunmai."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /nunmai, /btw, /stop,
        ...
        for expected in ("/nunmai", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``nunmai gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "nunmai_feedback" in created_apps[0].registered_actions
        assert "nunmai_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("nunmai_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("nunmai_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestSlashCommands:
    ...
    # instead of as /nunmai subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_nunmai_prefix_still_works(self, adapter):
        """Backward compat: /nunmai btw foo must still route to /btw foo.

        Old workspace manifests only declared /nunmai as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`nunmai config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "nunmai gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_nunmai_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/nunmai",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/nunmai-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/nunmai-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_NUNMAI_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_nunmai_slack_user_agent_prefix_format(self):
        """Module constant matches the NunmaiAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._NUNMAI_SLACK_USER_AGENT_PREFIX.startswith("NunmaiAgent/")
