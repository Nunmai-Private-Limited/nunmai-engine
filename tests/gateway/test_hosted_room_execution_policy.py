...
def _policy(*, approval_mode: str = "manual", max_turns: int = 12) -> dict:
    return execution_policy_mapping(
        ...
        config={
            ...
            "platform_toolsets": {"api_server": ["nunmai-api-server", "web"]},
        },
    )
...
def _two_profile_homes(tmp_path, monkeypatch) -> None:
    """alpha is the launch/active home (``NUNMAI_PROFILE=alpha``); beta is a served sibling."""
    ...
    for name, turns in (("alpha", 12), ("beta", 7)):
        ...
        (home / "config.yaml").write_text(
            f"agent:\n  max_turns: {turns}\napprovals:\n  mode: manual\n"
            "platform_toolsets:\n  api_server: [nunmai-api-server, web]\n")
        (home / ".env").write_text("")
    from nunmai_cli import profiles
    import gateway.run as gateway_run
    alpha = root / "profiles" / "alpha"
    monkeypatch.setattr(profiles, "_get_default_nunmai_home", lambda: root)
    monkeypatch.setattr(gateway_run, "_nunmai_home", alpha)
    monkeypatch.setenv("NUNMAI_HOME", str(alpha))
    monkeypatch.setenv("NUNMAI_PROFILE", "alpha")
...
def test_catalog_requires_the_served_profile_and_never_reads_nunmai_profile(monkeypatch):
