...
async def test_estop_refuses_fire_before_admission_and_lifts_on_resume(adapter, monkeypatch, tmp_path):
    """`nunmai pause` must stop the NAS-driven door too: 503 + Retry-After (NAS redelivers
    after `nunmai resume`) and no ``claim_fire`` — a claim followed by a 503 would leave a
