...
(/opt/nunmai/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable NUNMAI_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_nunmai_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable NUNMAI_HOME."""
    ...
    monkeypatch.setattr(
        "nunmai_constants.get_nunmai_home", lambda: nunmai_home
    )
    ...
    expected = nunmai_home / "scripts" / "whatsapp-bridge"
