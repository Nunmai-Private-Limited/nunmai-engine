...
async def test_restart_command_while_busy_requests_drain_without_interrupt(monkeypatch):
    ...
    monkeypatch.delenv("NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR", raising=False)
...
def test_load_busy_text_mode_follows_input_mode_and_honors_legacy(tmp_path, monkeypatch):
    monkeypatch.setattr(gateway_run, "_nunmai_home", tmp_path)
    monkeypatch.delenv("NUNMAI_GATEWAY_BUSY_TEXT_MODE", raising=False)
    monkeypatch.delenv("NUNMAI_GATEWAY_BUSY_INPUT_MODE", raising=False)
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_TEXT_MODE", "queue")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "queue"

    # Bogus legacy value is ignored → falls through to busy_input_mode (interrupt).
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_TEXT_MODE", "bogus")
    assert gateway_run.GatewayRunner._load_busy_text_mode() == "interrupt"
...
async def test_restart_from_served_profile_chat_restarts_the_host_gateway(monkeypatch):
    ...
    detached watcher relaunches `nunmai gateway restart` under the launch home (under a named
    profile's home it exits 78 and nothing comes back), and stop() - which flushes pending
    messages under get_nunmai_home() - runs outside the requester's profile scope."""
    ...
    async def _recording_stop(**_kwargs):
        seen["stop_home"] = get_nunmai_home()
        seen["stop_secret_scope"] = current_secret_scope()

    runner.stop = _recording_stop
    watcher_envs = []
    monkeypatch.setattr(gateway_run, "_resolve_nunmai_bin", lambda: ["nunmai"])
    monkeypatch.setattr(
    ...
    assert [env.get("NUNMAI_HOME") for env in watcher_envs] == [str(launch_home)]
...
async def test_windows_detached_restart_scrubs_gateway_marker(monkeypatch, tmp_path):
    ...
    assert cmd[-3:] == ["nunmai", "gateway", "restart"]
    assert kwargs["env"].get("_NUNMAI_GATEWAY") is None
...
async def test_request_restart_skips_wait_when_only_wedged_turns(monkeypatch):
    ...
    Regression: a WhatsApp turn wedged for 30+ min pinned `nunmai update`
    ...
    """
    monkeypatch.delenv("NUNMAI_AGENT_TIMEOUT", raising=False)
    runner, _adapter = make_restart_runner()
    runner.stop = AsyncMock()
...
def test_wedged_agent_count_disabled_timeout_counts_nothing(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_AGENT_TIMEOUT", "0")
    runner, _adapter = make_restart_runner()
...
async def test_request_restart_skips_wait_for_cron_run_past_inflight_allowance(monkeypatch, tmp_path):
    ...
    #115469 Defect B: a no-agent job whose delivery hung pinned ``nunmai update`` in "draining" for the
