...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``NUNMAI_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.profiles.profiles_to_serve", return_value=served), \
            patch("nunmai_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
