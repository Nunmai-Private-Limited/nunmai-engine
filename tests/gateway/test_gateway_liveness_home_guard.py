"""A profile directory copied out of another NUNMAI_HOME must never read as running.
...
def _live_record(nunmai_home):
    return {
        "pid": _LIVE_PID, "kind": "nunmai-gateway", "gateway_state": "running", "start_time": 1000,
        "argv": ["python", "-m", "nunmai_cli.main", "-p", "eagle", "gateway", "run"],
        "nunmai_home": str(nunmai_home),
    }


def _alive(monkeypatch, cmdline="python -m nunmai_cli.main -p eagle gateway run"):
