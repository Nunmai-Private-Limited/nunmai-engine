"""Agent status lines reach the OpenAI-compatible SSE writers as ``nunmai.status`` events (#85426):
...
async def test_chat_completions_stream_forwards_agent_status_callback(adapter, monkeypatch):
    ...
    as an ``event: nunmai.status`` frame and never as answer ``content``."""
    ...
    with patch.object(api_mod.web, "StreamResponse", return_value=fake_response):
        await adapter._write_sse_chat_completion(
            request, "chatcmpl-x", "nunmai-engine", int(time.time()), stream_q, agent_task, agent_ref)
    frames = _frames(written)
    assert [d for e, d in frames if e == "nunmai.status"] == [{"kind": "lifecycle", "text": _LADDER}]
...
async def test_responses_stream_emits_status_event_outside_output_items(adapter):
    ...
    with patch.object(api_mod.web, "StreamResponse", return_value=fake_response):
        await adapter._write_sse_responses(
            request=request, response_id=f"resp_{uuid.uuid4().hex[:28]}", model="nunmai-engine",
