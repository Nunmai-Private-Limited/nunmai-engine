...
async def test_secondary_profile_busy_mode_controls_live_busy_behavior(
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_ACK_ENABLED", "false")
...
async def test_secondary_profile_busy_mode_controls_priority_path(
    ...
    monkeypatch.setenv("NUNMAI_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "0")
    runner = _runner(default_mode="interrupt")
    adapter = await _load_profile_snapshot(
        runner,
        tmp_path / "research",
...
async def test_primary_adapter_busy_origin_uses_routed_privacy(
    ...
    monkeypatch.setattr("gateway.run._nunmai_home", home)
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_ACK_ENABLED", "false")
    ...
    ambient = get_nunmai_home_override()
    ...
    assert get_nunmai_home_override() == ambient
...
async def test_secondary_busy_text_timing_follows_profile_config_not_process_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", "9.0")
    monkeypatch.setenv("NUNMAI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", "9.0")
    runner = _runner()
...
async def test_secondary_human_delay_follows_profile_config_not_process_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HUMAN_DELAY_MODE", "custom")
    monkeypatch.setenv("NUNMAI_HUMAN_DELAY_MIN_MS", "1")
    monkeypatch.setenv("NUNMAI_HUMAN_DELAY_MAX_MS", "2")
    runner = _runner()
