...
(no patched predicates) against a temp ``NUNMAI_HOME`` with two served profiles.
"""
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("nunmai_cli.profiles.profiles_to_serve", return_value=served), \
            patch("nunmai_cli.profiles.get_profile_dir", side_effect=lambda n: home if n == "default" else home / "profiles" / n), \
            patch("nunmai_cli.profiles.profile_exists", return_value=True):
        yield rig
...
def test_unresolved_under_multiplex_raises_and_never_means_default(mux, tmp_path, monkeypatch):
    ...
    solo_home.mkdir()
    monkeypatch.setenv("NUNMAI_HOME", str(solo_home))
