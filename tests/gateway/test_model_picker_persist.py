...
closure the PR changed, against a real temp ``NUNMAI_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "nunmai_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_nunmai_home() / config.yaml`` — point it here.
    monkeypatch.setattr("nunmai_constants.get_nunmai_home", lambda: nunmai_home)
    monkeypatch.setattr("nunmai_cli.config.get_nunmai_home", lambda: nunmai_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_nunmai_home", default_home)
    _stub_picker_dependencies(monkeypatch)
...
def _make_store_runner(adapter, sessions_dir, monkeypatch):
    ...
    monkeypatch.setattr(nunmai_state, "SessionDB", _no_sqlite)
