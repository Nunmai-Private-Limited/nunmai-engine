...
def _mention_entity(text, mention="@nunmai_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@nunmai_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @nunmai_bot", entities=[_mention_entity("hi @nunmai_bot")])) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="nunmai_bot")
    ...
    assert adapter._should_process_message(_group_message("hi @nunmai_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @nunmai_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @nunmai_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @nunmai_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @nunmai_bot")])
    ) is False
...
def test_config_bridges_telegram_group_settings(monkeypatch, tmp_path):
    ...
    # from a developer's real ~/.nunmai/.env, defeating the env-over-YAML bridge
...
class _IdentityBot:
    ...
    def __init__(self, bot_id=999, cached="nunmai_bot", server=None):
        self.id = bot_id
        self._cached = cached
...
def test_bot_never_adopts_another_accounts_username():
    ...
    adapter = _make_adapter(require_mention=True)
    adapter._bot = _IdentityBot(cached="nunmai_bot")
    ...
    assert adapter._current_bot_username() == "nunmai_bot"
...
def test_sibling_bot_wake_word_message_is_observed_not_dropped():
    ...
    async def _run():
        ...
        store = _FakeSessionStore()
        adapter._session_store = store
        msg = _bot_sender_message("nunmai, can you take this one?")
        ...
        assert message["observed"] is True
        assert message["content"].endswith("nunmai, can you take this one?")
...
def test_sibling_bot_explicit_mention_still_dispatches_and_is_not_observed():
    ...
    human = _group_message("nunmai, hello")
