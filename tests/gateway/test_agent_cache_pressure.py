...
class TestPressureSignalScope:
    ...
    def _cgroup(self, monkeypatch, tmp_path, stat_text):
        ...
        monkeypatch.setattr(cleanup, "_own_cgroup_path", lambda: "/nunmai.service")
        real_read_text = acp.Path.read_text

        def read_text(self, *args, **kwargs):
            if str(self) == "/sys/fs/cgroup/nunmai.service/memory.high":
                return "6979321856\n"
            if str(self) == "/sys/fs/cgroup/nunmai.service/memory.stat":
    ...
    def test_same_cgroup_child_anon_counts_against_the_budget(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("nunmai_cli.mem_trim.collect_memory_snapshot", lambda: {"rss_anon_kib": 1_600 * 1024})
