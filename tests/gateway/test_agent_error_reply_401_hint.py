...
sign-in (93889b770da: a bare ``nunmai auth add <provider>`` from a named profile re-signs the
...
def test_gateway_401_reply_names_the_provider_and_the_profile(monkeypatch, tmp_path):
    ...
    assert "`nunmai -p codex auth add openai-codex --type oauth`" in known, known
    ...
    assert "`nunmai -p codex auth add <provider>`" in unknown, unknown
