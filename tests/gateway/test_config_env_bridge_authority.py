...
Guards against the 60-vs-500 bug where a stale `.env NUNMAI_MAX_ITERATIONS=60`
...
def _run_gateway_import(
    nunmai_home: Path, initial_env: dict[str, str], routed_home: Path | None = None
) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        if {str(routed_home or "")!r}:
            ...
            set_nunmai_home_override({str(routed_home or "")!r})
        ...
        for k in (
            ...
            "NUNMAI_AGENT_TIMEOUT",
            "NUNMAI_AGENT_TIMEOUT_WARNING",
            "NUNMAI_TURN_LEASE_TIMEOUT",
            "NUNMAI_SESSION_STALL_TIMEOUT",
            "NUNMAI_GATEWAY_BUSY_INPUT_MODE",
            "NUNMAI_GATEWAY_BUSY_TEXT_MODE",
    ...
    # otherwise intentionally clean so stale Nunmai settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def nunmai_home(tmp_path: Path) -> Path:
    home = tmp_path / ".nunmai"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(nunmai_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(nunmai_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(nunmai_home, {
        "NUNMAI_AGENT_TIMEOUT": "60",
        "NUNMAI_AGENT_TIMEOUT_WARNING": "30",
        "NUNMAI_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(nunmai_home, initial_env={})

    assert env.get("NUNMAI_AGENT_TIMEOUT") == "1800"
    assert env.get("NUNMAI_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("NUNMAI_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(nunmai_home: Path) -> None:
    ...
    _write_env(
        nunmai_home,
        {"NUNMAI_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(nunmai_home, initial_env={})

    assert env.get("NUNMAI_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert float(env.get("NUNMAI_TURN_LEASE_TIMEOUT")) == float(
...
def test_config_platform_connect_timeout_supplies_env_when_unset(nunmai_home: Path) -> None:
    ...
    _write_config(nunmai_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(nunmai_home, initial_env={})

    assert env.get("NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(nunmai_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        nunmai_home,
        initial_env={"NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
...
def test_first_import_under_a_routed_override_bridges_the_process_home(tmp_path: Path) -> None:
    ...
    assert env.get("NUNMAI_MAX_ITERATIONS") == "111"
