...
async def test_run_blocking_propagates_caller_contextvars(tmp_path, monkeypatch):
    ...
    multiplexed profile's NUNMAI_HOME override is a contextvar, and a worker that lost it
    ...
    adapter = FeishuAdapter(PlatformConfig())
    token = set_nunmai_home_override(str(tmp_path / "profiles" / "secondary"))
    try:
        assert await adapter._run_blocking(get_nunmai_home_override) == str(tmp_path / "profiles" / "secondary")
    finally:
