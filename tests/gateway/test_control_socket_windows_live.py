...
os.environ["NUNMAI_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "nunmai-gateway"
    ...
    monkeypatch.setattr(
        "nunmai_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
            ...
            json.dumps(
                {
                    ...
                    "code_sha": "OLD", "kind": "nunmai-gateway",
                }
            ),
            encoding="utf-8",
        )
    ...
    standin = subprocess.Popen(
        [sys.executable, "-c", "import time; time.sleep(120)", "nunmai", "gateway", "run"],
