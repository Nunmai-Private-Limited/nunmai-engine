    ...
    async with client:
        ...
        with patch.object(adapter, "_run_agent", side_effect=_fake_run_agent):
            r = await client.post("/v1/chat/completions", json={
                "model": "nunmai-engine", "messages": [{"role": "user", "content": "q"}]})
            assert r.status == 200
            message = (await r.json())["choices"][0]["message"]
            assert message["reasoning_content"] == REASONING
            assert REASONING not in message["content"]

            r = await client.post("/v1/responses", json={"model": "nunmai-engine", "input": "q", "store": True})
            assert r.status == 200
            data = await r.json()
    ...
    async with client:
        ...
        with patch.object(adapter, "_run_agent", side_effect=_fake_run_agent):
            r = await client.post("/v1/responses", json={
                "model": "nunmai-engine", "store": False,
