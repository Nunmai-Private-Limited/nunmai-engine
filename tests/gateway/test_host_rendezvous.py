...
def test_two_homes_take_their_own_per_home_lock_but_only_one_host_lock(host_dir, monkeypatch):
    """The per-home lock is per NUNMAI_HOME (N profiles = N locks); the host lock is not.
    ...
    env = {**os.environ, "NUNMAI_HOME": str(home_a), "NUNMAI_GATEWAY_LOCK_DIR": str(host_dir / "locks")}
...
def test_sigterm_removes_the_record_and_its_live_session_token(host_dir):
    ...
    env = {**os.environ, "NUNMAI_GATEWAY_LOCK_DIR": str(host_dir / "locks")}
    child = subprocess.Popen(
...
def test_unopenable_lock_dir_is_not_reported_as_another_owner(host_dir, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(blocked / "locks"))
...
def test_lock_cache_follows_the_lock_directory(host_dir, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_GATEWAY_LOCK_DIR", str(moved))
...
def test_relative_xdg_state_home_is_ignored(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NUNMAI_GATEWAY_LOCK_DIR", raising=False)
    monkeypatch.setenv("XDG_STATE_HOME", "relative/state")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))

    assert status._get_lock_dir().is_absolute()
    assert status._get_lock_dir() == tmp_path / ".local" / "state" / "nunmai" / status._LOCKS_DIRNAME
