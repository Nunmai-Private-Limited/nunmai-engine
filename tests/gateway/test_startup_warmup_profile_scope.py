...
``get_tool_definitions`` runs every ``check_fn``; the vision probe resolves Nunmai runtime
...
def multiplex_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    home.mkdir()
    (home / ".env").write_text(f"NUNMAI_PORTAL_BASE_URL={PORTAL}\n")
...
def _run_warmup(monkeypatch, *, multiplex: bool) -> dict:
    ...
    async def drive() -> None:
        ...
        seen["home_override_after"] = get_nunmai_home_override()
