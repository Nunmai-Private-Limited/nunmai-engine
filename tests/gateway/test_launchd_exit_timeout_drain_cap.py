...
def _grandchild(label):
    ...
    in the wrapper's NUNMAI_LAUNCHD_LABEL re-export."""
...
@pytest.mark.parametrize(
    ...
    [
        ("darwin", _direct("ai.nunmai.gateway"), _CAPPED),
        ("darwin", _grandchild("ai.nunmai.gateway"), _CAPPED),
        ...
        ("linux", _direct("ai.nunmai.gateway"), 180.0),
        ("linux", _grandchild("ai.nunmai.gateway"), 180.0),
    ],
...
def test_launchd_reader_yields_a_budget_only_for_nunmai_jobs(platform, environ, expected):
    """End to end from the process environment to the stop drain: the reader sizes the drain to the
    live ExitTimeOut only for an ``ai.nunmai`` job on darwin, seen directly or through the wrapper's
...
def test_forwarded_label_marks_grandchild_supervised_for_every_reader():
    ...
    grandchild_env = _grandchild("ai.nunmai.gateway")
    assert launchd_service_label(grandchild_env, platform="darwin") == "ai.nunmai.gateway"
