"""The messaging gateway is a boot owner of the Nunmai free tier.

Rung 5 made every demand-time site a read (provider resolution, ``/login``, the connector token), so a
process that never runs the bootstrap can never have an identity. `cmd_chat` and `nunmai serve` run it;
this file pins that `nunmai gateway run` does too, and does it BEFORE any adapter connects, so a fast
...
def test_gateway_bootstrap_seam_calls_the_one_creator(monkeypatch):
    ...
    import nunmai_cli.free_tier_bootstrap as ftb
    calls: list[dict] = []
