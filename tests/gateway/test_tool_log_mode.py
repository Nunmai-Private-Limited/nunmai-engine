...
lines to ~/.nunmai/logs/tool_calls.log via write_tool_log's rotating handler.
...
async def test_write_tool_log_shares_one_logger_across_turns(tmp_path, monkeypatch):
    ...
    shared = logging.getLogger("nunmai.tool_calls")
    ...
    try:
        ...
        assert set(logging.Logger.manager.loggerDict) - before <= {"nunmai.tool_calls"}
