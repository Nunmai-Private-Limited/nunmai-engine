...
``get_nunmai_home()``, which follows the override: the row landed in ``profiles/<name>/state.db``,
...
@pytest.fixture(params=["nunmai-home-env", "platform-default"])
def launch_home(request, tmp_path, monkeypatch):
    root = tmp_path / "root"
    (root / "profiles" / "research").mkdir(parents=True)
    if request.param == "nunmai-home-env":
        monkeypatch.setenv("NUNMAI_HOME", str(root))
    else:
        # A default gateway run in the foreground has no NUNMAI_HOME at all.
