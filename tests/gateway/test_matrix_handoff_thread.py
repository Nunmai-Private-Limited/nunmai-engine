...
class TestHandoffThread:
    ...
    def test_seed_event_becomes_the_thread_id(self):
        ...
        thread_id = asyncio.run(adapter.create_handoff_thread("!room1:example.org", "Nunmai — daily brief"))

        assert thread_id == "$root_evt"
        adapter.send.assert_awaited_once_with("!room1:example.org", "Nunmai — daily brief")
