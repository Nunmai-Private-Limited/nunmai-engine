...
from nunmai_cli.config import TURN_LIMIT_UNLIMITED
...
def test_agent_budget_line_matches_enforced_limit(monkeypatch, max_turns, expected):
    monkeypatch.setenv("NUNMAI_MAX_ITERATIONS", "90")  # .env value: config wins when set, else it is the legacy bridge
    monkeypatch.setattr("gateway.run._reload_runtime_env_preserving_config_authority", lambda: None)
    monkeypatch.setattr("gateway.run.get_nunmai_home_override", lambda: None)
