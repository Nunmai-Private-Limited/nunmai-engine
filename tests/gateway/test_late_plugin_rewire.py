...
from nunmai_cli.plugins import PluginContext, PluginManager, PluginManifest, discover_plugins, get_plugin_manager
...
def test_rewire_is_idempotent_per_native_client():
    ...
    with patch("nunmai_cli.plugins.get_plugin_manager", return_value=_factory_manager(calls)):
...
def test_telegram_late_plugin_handler_precedes_core_catch_all():
    ...
    with patch("nunmai_cli.plugins.get_plugin_manager", return_value=mgr):
...
def test_runner_rewires_live_adapters_on_the_loop_when_plugins_load():
    ...
    _write_plugin(Path(os.environ["NUNMAI_HOME"]))  # something must actually load for the event to fire
    mgr = PluginManager()
