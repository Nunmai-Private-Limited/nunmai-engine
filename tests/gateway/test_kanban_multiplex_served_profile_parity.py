...
``nunmai -p X`` dispatcher would build, and X's notifications are rendered/filtered under X's
config.
"""
...
def served(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text("NUNMAI_MODEL=default-model\nTERMINAL_ENV=docker\n")
    ...
    monkeypatch.setenv("NUNMAI_MODEL", "default-model")
    monkeypatch.setenv("TERMINAL_ENV", "docker")
    monkeypatch.setenv("NUNMAI_KANBAN_DB", str(tmp_path / "board.db"))
...
def test_worker_for_served_profile_gets_its_own_env_and_toolset_pin(served, monkeypatch):
    ...
    monkeypatch.setattr(kbd, "_nunmai_argv", lambda: ["nunmai"], raising=False)
    kbd._default_spawn(task, str(served.alpha), board=None)

    env = spawned["env"]
    assert env["NUNMAI_HOME"] == str(served.alpha)
    assert "NUNMAI_MODEL" not in env and "TERMINAL_ENV" not in env
...
class RecordingAdapter:
    ...
    async def send(self, chat_id, text, metadata=None):
        ...
        self.sent.append({"text": text, "home": str(get_nunmai_home()), "strict": media_delivery_strict()})
