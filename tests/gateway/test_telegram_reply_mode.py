...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Nunmai-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Nunmai topic the session runs
    in. The no-anchor branch must prefer the Nunmai topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_nunmai_topic_thread_id(self):
