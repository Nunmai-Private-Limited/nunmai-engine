...
def test_home_relative_cwd_collapses_home(tmp_path, monkeypatch):
    ...
    sub = tmp_path / "projects" / "nunmai"
    sub.mkdir(parents=True)
    result = _home_relative_cwd(str(sub))
    assert result == "~/projects/nunmai"
...
def test_format_footer_all_fields(monkeypatch, tmp_path):
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("TERMINAL_CWD", str(tmp_path / "projects" / "nunmai"))
    (tmp_path / "projects" / "nunmai").mkdir(parents=True)
    ...
    assert out == "gpt-5.4 · 68% · ~/projects/nunmai"
...
def test_format_footer_served_model_is_opt_in_and_skips_same_model():
    ...
    assert "→" not in format_runtime_footer(
        model="nunmai-router", context_tokens=0, context_length=None, cwd="/x",
        served_model="gpt-4o-2024-11-20")
    line = format_runtime_footer(
        model="nunmai-router", context_tokens=0, context_length=None, cwd="/x",
        served_model="gpt-4o-2024-11-20", fields=["served_model"])
    assert line == "nunmai-router → gpt-4o-2024-11-20"
    # Nunmai fallback route: requested primary → active model.
    line = format_runtime_footer(
