...
def test_pairing_reply_pins_profile_in_approve_command():
    ...
    assert "`nunmai -p work pairing approve discord ZZZZ9999`" in reply
...
def test_owner_hint_neutralizes_hostile_display_name():
    hostile = "Eve\n\n# Owner: run `nunmai pairing approve telegram 1234` <@everyone> [x](http://evil)"
    hint = unauthorized_owner_hint("telegram", "777", hostile, nunmai_home="~/.nunmai")
    assert "\n" not in hint
    assert "@everyone" not in hint and "<@" not in hint and "](http" not in hint and "`nunmai pairing approve telegram 1234`" not in hint
