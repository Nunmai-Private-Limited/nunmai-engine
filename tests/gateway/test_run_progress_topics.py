...
class LongPreviewAgent:
    ...
    LONG_CMD = "cd /home/teknium/.nunmai/nunmai-engine/.worktrees/nunmai-d8860339 && source .venv/bin/activate && python -m pytest tests/gateway/test_run_progress_topics.py -n0 -q"
...
def test_tool_progress_mode_reads_profile_scope_not_process_environ(monkeypatch, tmp_path):
    """NUNMAI_TOOL_PROGRESS_MODE must resolve through the active profile's secret scope, not
    ...
    monkeypatch.setenv("NUNMAI_TOOL_PROGRESS_MODE", "off")
    # This profile's OWN scoped value, which must win over the leaked process env.
    token = secret_scope.set_secret_scope({"NUNMAI_TOOL_PROGRESS_MODE": "all"})
    try:
...
def test_tool_progress_mode_follows_profile_through_the_real_scoping_seam(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_TOOL_PROGRESS_MODE", "off")
    (root / ".env").write_text("NUNMAI_TOOL_PROGRESS_MODE=log\n")
    (beta / ".env").write_text("NUNMAI_TOOL_PROGRESS_MODE=verbose\n")
...
async def test_run_agent_progress_uses_event_message_id_for_slack_dm(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_TOOL_PROGRESS_MODE", "all")
...
async def test_slack_null_tool_progress_is_inheritance_not_explicit_off(monkeypatch, tmp_path, display_cfg, env_mode):
    ...
    monkeypatch.delenv("NUNMAI_TOOL_PROGRESS_MODE", raising=False)
    if env_mode is not None:
        monkeypatch.setenv("NUNMAI_TOOL_PROGRESS_MODE", env_mode)
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        DuplicateNativeToolsAgent,
...
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_TOOL_PROGRESS_MODE", "verbose")
