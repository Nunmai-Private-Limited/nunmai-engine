...
def test_served_profile_completion_wakes_in_process_only_for_the_session_it_owns(served, monkeypatch):
    ...
    adapter._api_key, adapter._host, adapter._port, adapter._model_name = "k" * 20, "127.0.0.1", 8642, "nunmai"
    ...
    assert [c["headers"]["X-Nunmai-Session-Id"] for c in _FakeHttpSession.calls] == ["default-owned"]
...
def test_single_profile_gateway_keeps_the_http_self_post(served, monkeypatch):
    ...
    assert adapter.turns == []
    assert [c["headers"]["X-Nunmai-Session-Id"] for c in _FakeHttpSession.calls] == [SESSION]
