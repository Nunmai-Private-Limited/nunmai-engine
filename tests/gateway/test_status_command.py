from nunmai_state import AsyncSessionDB, SessionDB
...
async def test_first_run_slack_home_channel_onboarding_uses_parent_command(monkeypatch):
    ...
    assert "/nunmai sethome" in onboarding
...
async def test_profile_command_reports_source_stamped_profile(monkeypatch, tmp_path):
    ...
    profile_home = nunmai_home / "profiles" / "milo"
    ...
    # The reply renders display_nunmai_home() for the routed profile, which abbreviates a home
    ...
    with _profile_runtime_scope(profile_home):
        expected_home = display_nunmai_home()
...
async def test_context_command_keeps_configured_window_without_resident_agent():
    ...
    with patch("gateway.run._load_gateway_config", return_value=config), patch(
        ...
        "nunmai_cli.config.get_compatible_custom_providers",
...
async def test_context_all_appends_expanded_listings():
    ...
    fake_details = {
        "skills": [
            {"name": "nunmai-engine", "index_tokens": 30, "skill_md_tokens": 2_500},
        ],
        "toolsets": [
