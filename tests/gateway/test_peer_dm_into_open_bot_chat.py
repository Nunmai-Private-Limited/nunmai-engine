...
``nunmai peer dm`` posts to ``/api/sessions/{id}/chat`` on the peer. When the peer's canonical Bot
...
from nunmai_cli.subcommands import peer as peer_mod
...
async def test_a_peer_turn_into_an_open_bot_chat_is_answered_by_its_live_owner(
    ...
    try:
        ...
        if status == 200:
            ...
            assert (body["object"], body["status"]) == ("nunmai.session.chat.queued", "queued")
    finally:
...
def test_peer_dm_reports_a_turn_queued_in_the_open_bot_chat_as_delivered(monkeypatch, capsys, as_json):
    ...
    monkeypatch.setattr(peer_mod, "_request", lambda url, key, **kw: {
        "object": "nunmai.session.chat.queued", "session_id": "bot-chat", "status": "claimed", "delivery_id": "d" * 32})
