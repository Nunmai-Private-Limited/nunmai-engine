...
@pytest.mark.parametrize(
    ("cmdline", "expected"),
    [
        ("/v/python -m nunmai_cli.main -p ops-2 gateway run", False),
        ("/v/python -m nunmai_cli.main --profile ops2 gateway run", False),
        ("/v/python -m nunmai_cli.main -p ops gateway run", True),
        ("/v/python -m nunmai_cli.main --profile=ops gateway run", True),
    ],
)
...
@pytest.mark.parametrize("cmdline", [
    "/v/python -m nunmai_cli.main --profile=ops gateway run",
    "/v/python -m nunmai_cli.main -p ops gateway run",
    "/v/python -m nunmai_cli.main --profile ops gateway run",
])
...
@pytest.mark.parametrize(
    ("cmdline", "expected"),
    [
        ("NUNMAI_HOME={home}2 nunmai gateway run", []),        # longer sibling home
        ("NUNMAI_HOME={home}/ nunmai gateway run", [424242]),  # trailing-separator spelling
        ("NUNMAI_HOME={home} nunmai gateway run", [424242]),   # exact home
    ],
)
def test_scan_gateway_pids_claims_own_home_spellings_not_the_sibling(
    tmp_path, cmdline, expected, monkeypatch
):
    """``_scan_gateway_pids`` drives the mirrored NUNMAI_HOME predicate: the process-table
    fallback must not sweep a longer sibling home's live gateway, while the supervisor
    trailing-separator spelling (``NUNMAI_HOME=/root/.nunmai/``) is still its own home."""
    import nunmai_cli.gateway as gw
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
