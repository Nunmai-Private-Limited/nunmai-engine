...
def test_cold_fronted_platforms_is_read_only(tmp_path, case):
    ...
    # pytest's imported modules. HOME and cwd are temporary as well as NUNMAI_HOME.
    ...
    env.update({"HOME": str(tmp_path), "USERPROFILE": str(tmp_path),
                "NUNMAI_HOME": str(home), "NUNMAI_MANAGED_DIR": str(managed),
    ...
    code = textwrap.dedent("""
        ...
        if sys.argv[1] == "scoped":
            set_nunmai_home_override(sys.argv[2])
