...
context: a caller sitting inside a served profile's scope would otherwise hand its NUNMAI_HOME
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(tmp_path / "launch"))
...
async def test_teardown_thread_does_not_inherit_the_callers_home_override(tmp_path, monkeypatch):
    ...
    from nunmai_constants import (
        get_nunmai_home_override, reset_nunmai_home_override, set_nunmai_home_override)
    ...
    monkeypatch.setattr(
        ...
        lambda **_kw: seen.append(get_nunmai_home_override()))

    token = set_nunmai_home_override(str(poison))
    try:
