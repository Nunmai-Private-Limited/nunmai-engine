"""E2E: ``nunmai peer dm`` against a REAL api_server gateway whose canonical
Bot Chat session is HIDDEN (issue #91583).

Two real NUNMAI homes in spirit: the "peer" side is a real
:class:`APIServerAdapter` bound to a real loopback TCP socket over a real
SQLite ``state.db`` (its own tmp NUNMAI_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``nunmai peer dm`` client code (``nunmai_cli.subcommands.peer``),
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own NUNMAI_HOME + state.db) on a real socket."""
