...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("NUNMAI_SESSION_ID")
            observed["context_platform"] = get_session_env("NUNMAI_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("NUNMAI_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("NUNMAI_SESSION_ID")
...
async def test_session_chat_treats_pre_existing_poisoned_row_as_no_model(session_db):
    ...
    virtual model alias (e.g. "nunmai-engine") persisted literally as its
    ...
    with patch.object(adapter, "_create_agent", side_effect=fake_create_agent):
        async with TestClient(TestServer(app)) as cli:
            ...
            responses = await (await cli.post(
                "/v1/responses", json={"model": "nunmai-engine", "input": "go", "stream": True})).text()
