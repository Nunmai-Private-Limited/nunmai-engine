...
class TestBuildSessionContextPrompt:
    ...
    def test_slack_tools_loaded_scope_failure_fails_closed(self, monkeypatch):
        ...
        try:
            with patch("tools.mcp_tool_discovery.get_registered_mcp_server_names", return_value=[]), \
                    ...
                    patch("nunmai_cli.tools_config._get_platform_tools", return_value=["slack"]):
    ...
    def test_local_delivery_path_uses_display_nunmai_home(self):
        config = GatewayConfig()
        source = SessionSource(
        ...
        with patch("nunmai_constants.display_nunmai_home", return_value="~/.nunmai/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.nunmai/profiles/coder/cron/output/" in prompt
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_home))
