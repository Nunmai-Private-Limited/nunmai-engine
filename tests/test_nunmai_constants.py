...
class TestGetDefaultNunmaiRoot:
    ...
    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; NUNMAI_HOME / home changes invalidate.

        get_default_nunmai_root() resolves NUNMAI_HOME against the native
        ...
        (native home, NUNMAI_HOME) compared for free each call.
        """
        # NUNMAI_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_nunmai_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            nunmai_constants, "_default_nunmai_root_memo", None, raising=False
        )

        first = get_default_nunmai_root()
        ...
        # NUNMAI_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("NUNMAI_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_nunmai_root() == docker_root
        assert resolve_calls["n"] > before, (
            "NUNMAI_HOME change must force a fresh resolution"
        )
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_constants, "_container_detected", None)
    ...
    def test_cgroup_v2_fallback_inspects_only_the_root_mount(self, tmp_path):
        ...
        from nunmai_constants import _root_mount_has_marker
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_constants, "_container_detected", True)
        assert is_container() is True
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import nunmai_cli._subprocess_compat as subprocess_compat
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(nunmai_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(nunmai_constants.sys, "platform", "darwin")
        monkeypatch.setenv("NUNMAI_HOME", str(tmp_path))
