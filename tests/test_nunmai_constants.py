"""Tests for nunmai_constants module."""
...
from nunmai_platform.host import runtime as host_runtime
from nunmai_platform.host import facts as host_facts
from nunmai_constants import (
    agent_browser_runnable,
    find_nunmai_node_executable,
    ...
    with_nunmai_node_path,
)


class TestGetDefaultNunmaiRoot:
    """Tests for get_default_nunmai_root() — Docker/custom deployment awareness."""

    @pytest.mark.linux_only
    def test_no_nunmai_home_returns_native(self, tmp_path, monkeypatch):
        """When NUNMAI_HOME is not set, returns ~/.nunmai."""
        monkeypatch.delenv("NUNMAI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_nunmai_root() == tmp_path / ".nunmai"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NUNMAI_HOME=<root>/profiles/<name>),
        ...
        assert get_default_nunmai_root() == docker_root
    ...
    def test_expanded_custom_profile_returns_custom_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv(
            "NUNMAI_HOME", f"{home_token}/deployment/profiles/research"
        )
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "native-home")

        assert get_default_nunmai_root() == custom_root

    @pytest.mark.windows_only
    def test_no_nunmai_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        ...
        assert get_default_nunmai_root() == local_appdata / "nunmai"
...
class TestGetNunmaiHome:
    """Tests for get_nunmai_home() platform-aware fallback."""
    ...
    def test_warn_once_latch_engages_on_first_check_even_without_warning(self, tmp_path, monkeypatch):
        ...
        nothing to warn about, otherwise every get_nunmai_home() call re-stats active_profile."""
        monkeypatch.delenv("NUNMAI_HOME", raising=False)
        monkeypatch.setattr(nunmai_constants, "_profile_fallback_warned", False)
        monkeypatch.setattr(nunmai_constants, "_get_platform_default_nunmai_home", lambda: tmp_path)

        get_nunmai_home()
        assert nunmai_constants._profile_fallback_warned is True

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When NUNMAI_HOME is unset on Windows, use %LOCALAPPDATA%\\nunmai."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NUNMAI_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_nunmai_home() == local_appdata / "nunmai"


class TestGetProcessNunmaiHome:
    """Tests for get_process_nunmai_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_nunmai_home() == home
    ...
    def test_process_and_context_homes_expand_environment_and_user_syntax(
        ...
        for syntax in (home_token, "~"):
            ...
            monkeypatch.setenv("NUNMAI_HOME", f"{syntax}/process-home")
            assert get_process_nunmai_home() == process_home

            override_home = tmp_path / "override-home"
            token = set_nunmai_home_override(f"{syntax}/override-home")
            try:
                assert get_nunmai_home() == override_home
                assert get_process_nunmai_home() == process_home
            finally:
...
class TestNunmaiManagedNode:
    ...
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_nunmai_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(home))
        monkeypatch.setattr(nunmai_constants, "node_tool_runnable", lambda path: True)

        assert find_nunmai_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(nunmai_constants, "heal_nunmai_managed_node", lambda: False)
        monkeypatch.setattr(
            nunmai_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert nunmai_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Nunmai-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_constants, "heal_nunmai_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = nunmai_constants._NUNMAI_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = nunmai_constants.find_nunmai_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert nunmai_constants.find_nunmai_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert nunmai_constants.find_nunmai_node_executable("node") == str(node)
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from nunmai_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".nunmai"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/nunmai to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (nunmai user) from traversing the install dir.
        """
        install_root = Path(nunmai_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/nunmai/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/nunmai-data/...).
...
class TestGetNunmaiDir:
    """Tests for ``get_nunmai_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_nunmai_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_nunmai_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert nunmai_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nunmai_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert nunmai_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert nunmai_constants.windows_path_to_wsl("/home/alex") is None
        assert nunmai_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(nunmai_constants, "is_wsl", lambda: True)
        assert nunmai_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nunmai_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert nunmai_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert nunmai_constants.managed_node_tree_in_use() is True
    ...
    def test_unrelated_process_does_not_count(self, tmp_path, monkeypatch):
        ...
        assert nunmai_constants.managed_node_tree_in_use() is False
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(home))
        monkeypatch.setenv(
            "NUNMAI_NODE_TARGET_MAJOR",
            str(nunmai_constants._NUNMAI_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(nunmai_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            nunmai_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            nunmai_constants,
        ...
        monkeypatch.setattr(
            nunmai_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(nunmai_constants._NUNMAI_NODE_TARGET_MAJOR)
        ...
        result = nunmai_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nunmai_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_nunmai_managed_node() is False
        assert nunmai_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_nunmai_managed_node() is False
        assert nunmai_constants._managed_node_heal_attempted is True
        # The flag is set, so the once-per-process budget is spent.
        assert heal_nunmai_managed_node() is False
        assert calls["n"] == 1

class TestProjectVenvDirOutOfTree:
    """#116148: a checkout with no in-tree venv whose interpreter lives in ``$NUNMAI_HOME/venvs/<name>``
    ...
    def _running_from(monkeypatch, checkout, venv):
        monkeypatch.setattr(nunmai_constants, "__file__", str(checkout / "nunmai_constants.py"))
    ...
    def test_out_of_tree_install_resolves_the_running_interpreter_venv(self, monkeypatch, tmp_path):
        checkout = tmp_path / "nunmai-engine"
        checkout.mkdir()
        venv = tmp_path / "venvs" / "nunmai"
        nunmai_constants.venv_python_path(venv).parent.mkdir(parents=True)
        nunmai_constants.venv_python_path(venv).write_text("", encoding="utf-8")
        self._running_from(monkeypatch, checkout, venv)

        assert nunmai_constants.project_venv_dir(checkout) == venv
    ...
    def test_foreign_root_and_in_tree_venv_are_unchanged(self, monkeypatch, tmp_path):
        ...
        assert nunmai_constants.project_venv_dir(other) is None
        (checkout / ".venv").mkdir()
        assert nunmai_constants.project_venv_dir(checkout) == checkout / ".venv"
