...
- files inside the Nunmai home tree are repaired with ``chmod u+rw``
...
import nunmai_state
from nunmai_state import SessionDB, preflight_db_writability
...
def nunmai_home(tmp_path, monkeypatch):
    """Isolated NUNMAI_HOME so the repair scope covers tmp DBs."""
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, nunmai_home):
    ...
    def test_repairs_readonly_sidecars(self, nunmai_home):
        db = nunmai_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, nunmai_home):
        sub = nunmai_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, nunmai_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, nunmai_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, nunmai_home):
        db = nunmai_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, nunmai_home):
        db_path = nunmai_home / "state.db"
