"""The conftest WAL gate must agree with nunmai_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``nunmai_state``. That is deliberate: importing
``nunmai_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.nunmai`` before the per-test ``NUNMAI_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_nunmai_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and nunmai_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_nunmai_state_at_collection():
    """The gate must stay import-free of nunmai_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.nunmai, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``nunmai_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "nunmai_state" or name.startswith("nunmai_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported nunmai_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.nunmai during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("nunmai_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["nunmai_state"] = saved
    assert not blocked
