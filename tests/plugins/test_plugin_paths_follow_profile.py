"""Plugin data paths follow the active profile's NUNMAI_HOME, including the ContextVar override.

Several plugins carried a ``~/.nunmai`` fallback (guarding an ImportError of ``nunmai_constants``
...
def _openviking_log(home):
    ...
    return ov.get_nunmai_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
