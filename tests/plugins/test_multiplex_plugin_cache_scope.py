...
NUNMAI_HOME override (``nunmai_constants.set_nunmai_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``NUNMAI_HOME``) and profile B with different config/.env values."""
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from nunmai_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", nunmai_home=str(home))
