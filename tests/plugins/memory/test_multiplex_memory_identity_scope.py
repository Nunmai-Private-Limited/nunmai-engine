...
OPENVIKING_* / NUNMAI_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "NUNMAI_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def secondary_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(prof_b))
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("nunmai", "", "platform")
    ...
    provider.initialize("s1", nunmai_home=str(secondary_profile))
    assert provider._client.project == "nunmai-b"
