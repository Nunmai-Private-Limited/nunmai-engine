...
Profile isolation is a ContextVar-scoped NUNMAI_HOME override; a plain ``threading.Thread`` starts
...
def _retaindb(seen, tmp_path):
    ...
    p._client = MagicMock()
    p._context_overlay = lambda query: {"context": seen.setdefault("home", get_nunmai_home()) and "ctx"}
