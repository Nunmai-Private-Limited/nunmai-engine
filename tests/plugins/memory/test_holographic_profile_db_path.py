...
Setup used to offer, and store, the active profile's concrete path (``~/.nunmai/memory_store.db``).
...
import nunmai_cli.memory_setup as memory_setup
from nunmai_cli.profiles import create_profile, rename_profile
...
def _setup_with_defaults(monkeypatch, home: Path) -> dict:
    """``nunmai memory setup`` for holographic with Enter on every field, saved as ``cmd_setup`` does."""
    monkeypatch.setenv("NUNMAI_HOME", str(home))
...
def _opened_db(monkeypatch, home: Path) -> Path:
    """The DB a session started with ``nunmai -p <profile>`` opens."""
    monkeypatch.setenv("NUNMAI_HOME", str(home))
...
def test_default_db_path_follows_the_profile_that_opens_it(root, monkeypatch, lifecycle):
    ...
    assert answers["db_path"] == "$NUNMAI_HOME/memory_store.db"
...
def test_save_config_stores_only_this_profiles_default_path_as_the_placeholder(root, monkeypatch, spelling):
    ...
    given = {
        "display": "~/.nunmai/profiles/work/memory_store.db",
        "absolute": str(profile / "memory_store.db"),
        "elsewhere": str(root / "shared" / "facts.db"),
    }[spelling]
    expected = given if spelling == "elsewhere" else "$NUNMAI_HOME/memory_store.db"

    HolographicMemoryProvider(config={"hrr_dim": 64}).save_config({"db_path": given, "hrr_dim": "64"}, str(profile))

    stored = yaml.safe_load((profile / "config.yaml").read_text(encoding="utf-8"))["plugins"]["nunmai-memory-store"]
