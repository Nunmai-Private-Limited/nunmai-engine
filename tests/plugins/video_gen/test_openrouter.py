...
def test_pooled_credential_enables_the_backend(monkeypatch, tmp_path):
    """A key added only with ``nunmai auth add openrouter`` serves chat and image_gen; video must use it too."""
...
def test_one_job_keeps_one_credential_while_the_pool_rotates(monkeypatch, tmp_path):
    ...
    (get_nunmai_home() / "config.yaml").write_text("credential_pool_strategies:\n  openrouter: round_robin\n")
...
def test_register_exposes_openrouter_in_the_video_gen_picker(monkeypatch):
    from nunmai_cli import plugins as plugin_loader, tools_config
