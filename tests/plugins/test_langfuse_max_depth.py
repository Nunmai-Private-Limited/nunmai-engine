...
def test_capture_respects_configured_depth_for_tool_inputs_and_outputs(
    ...
    plugin = importlib.import_module("plugins.observability.langfuse")
    monkeypatch.setenv("NUNMAI_LANGFUSE_CAPTURE", mode)
    if configured_depth is None:
        monkeypatch.delenv("NUNMAI_LANGFUSE_MAX_DEPTH", raising=False)
    else:
        monkeypatch.setenv("NUNMAI_LANGFUSE_MAX_DEPTH", configured_depth)
...
def test_invalid_max_depth_warns_and_preserves_default_capture(monkeypatch, caplog, invalid_depth):
    ...
    monkeypatch.setenv("NUNMAI_LANGFUSE_MAX_DEPTH", invalid_depth)

    assert plugin._capture_content(payload) == default_capture
    assert len(caplog.records) == 1
    assert "NUNMAI_LANGFUSE_MAX_DEPTH" in caplog.text
...
def test_invalid_max_depth_warns_once_per_value_not_per_payload(monkeypatch, caplog):
    """A bad ``NUNMAI_LANGFUSE_MAX_DEPTH`` must not log one warning per captured prompt/tool payload
    for the life of the process; a changed (still bad) value gets its own single warning."""
    plugin = importlib.import_module("plugins.observability.langfuse")
    monkeypatch.setenv("NUNMAI_LANGFUSE_CAPTURE", "full")
    monkeypatch.setenv("NUNMAI_LANGFUSE_MAX_DEPTH", "abc")
    payload = {"result": {"data": [{"index": 0}]}}

    for _ in range(5):
        plugin._capture_content(payload)
        plugin._capture_content(payload, tool_result_of=("example", {}))
    assert sum("NUNMAI_LANGFUSE_MAX_DEPTH" in r.getMessage() for r in caplog.records) == 1

    monkeypatch.setenv("NUNMAI_LANGFUSE_MAX_DEPTH", "-7")
    for _ in range(3):
        plugin._capture_content(payload)
    assert sum("NUNMAI_LANGFUSE_MAX_DEPTH" in r.getMessage() for r in caplog.records) == 2
