...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate NUNMAI_HOME for each test.

    The global hermetic fixture already redirects NUNMAI_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    NUNMAI_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "nunmai_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["nunmai_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_nunmai_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_nunmai_home(self, _isolate_env):
        dg = _load_lib()
...
class TestProfileUserTreesNeverCleaned:
    ...
    def test_session_end_hook_leaves_workspace_files_alone(self, _isolate_env):
        ...
        the NUNMAI_HOME root is the control: it is still tracked and removed."""
...
class TestGitWorktreeFilesNeverCleaned:
    ...
    def test_quick_drops_stale_tracked_worktree_entry_instead_of_deleting(self, _isolate_env):
        """A test_* file inside a linked git worktree ($NUNMAI_HOME/worktrees/, .git is a
    ...
    def test_scratch_outside_git_trees_still_cleaned(self, _isolate_env):
        """Control: root-level test_* scratch is still auto-deleted — even when NUNMAI_HOME
        itself lives inside a git checkout (dotfiles repo); only .git entries strictly below
        NUNMAI_HOME mark a file as git-owned."""
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, nunmai_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from nunmai_cli import plugins as pmod
