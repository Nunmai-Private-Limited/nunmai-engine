...
The contract under test: data lives under ``<nunmai home>/plugin-data/<name>/``
...
def test_data_dir_lives_outside_the_install_tree(nunmai_home):
    root = plugin_data_dir("my-plugin")

    assert root == nunmai_home / "plugin-data" / "my-plugin"
    assert root.is_dir()
    # The invariant that motivated the module: data must not live under the
    # install tree that `nunmai plugins remove` deletes.
    assert (nunmai_home / "plugins") not in root.parents


@pytest.mark.parametrize("bad", ["", ".", "..", "../escape", "a/b", "a\\b", "x" * 65])
def test_hostile_names_are_rejected(nunmai_home, bad):
    with pytest.raises(ValueError):
        plugin_data_dir(bad)


def test_plugin_db_journal_mode_is_the_shared_fallback_verdict(nunmai_home):
    ...
    assert (nunmai_home / "plugin-data" / "board" / "data.db").exists()


def test_plugin_db_rejects_path_shaped_filenames(nunmai_home):
    with pytest.raises(ValueError):
