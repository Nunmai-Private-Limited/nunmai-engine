"""Tests for the bundled nunmai-achievements dashboard plugin.
...
``plugins/nunmai-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the nunmai-engine level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.nunmai so state/snapshot files don't collide.
    ...
    # swap ``sys.modules['nunmai_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from nunmai_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for nunmai_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['nunmai_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("nunmai_state")
    fake_module.SessionDB = lambda **_kw: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "nunmai_state", fake_module)
...
def test_scan_sessions_never_opens_a_writable_session_db(plugin_api, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(nunmai_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
