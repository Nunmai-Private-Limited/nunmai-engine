...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure NUNMAI_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", nunmai_home=str(tmp_path / ".nunmai"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", nunmai_home=str(nunmai_home))
        return p
