...
Run from the PR worktree:
    ...
    cd ~/.nunmai/nunmai-engine/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.nunmai/nunmai-engine``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.nunmai/nunmai-engine
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a nunmai-engine checkout"
)
...
# Isolated NUNMAI_HOME for the config write.
