...
def _tmp_nunmai_home(tmp_path, monkeypatch):
...
class TestMetadata:
    ...
    def test_setup_schema_has_no_required_env_vars(self, provider):
        ...
        assert "nunmai auth add openai-codex" in schema["post_setup_hint"]
        assert "nunmai auth codex`" not in schema["post_setup_hint"]
