...
Uses a throwaway NUNMAI_HOME so it never touches ~/.nunmai. No real names.
"""
...
def _fresh_home():
    """Point NUNMAI_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_nunmai_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["NUNMAI_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER NUNMAI_HOME is set.
...
# a different get_nunmai_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"NUNMAI_HOME (throwaway): {HOME}")
