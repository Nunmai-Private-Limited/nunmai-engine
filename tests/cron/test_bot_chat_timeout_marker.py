...
1. A timed-out CLI delivery queues exactly one SHORT degraded-delivery marker via the
   deferred lane (flagged on the record, references `nunmai cron runs`, never repeats the
...
def cli_lane(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        Mock(side_effect=subprocess.TimeoutExpired(["nunmai"], 600)))
    return tmp_path
...
def test_timeout_queues_degraded_marker(cli_lane, monkeypatch):
    ...
    assert "nunmai cron runs" in marker
...
def test_late_turn_report_books_delivery(tmp_path, monkeypatch):
    ...
    result = delivery._run_bot_chat_turn(["nunmai"], {}, str(tmp_path / "r.json"), 0.4)
