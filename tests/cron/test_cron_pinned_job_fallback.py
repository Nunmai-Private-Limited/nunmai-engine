...
children via ``nunmai_cli.fallback_config.scoped_fallback_chain``):
...
def _run(tmp_path, job, *, primary_error=None):
    ...
    with patch("cron.scheduler._nunmai_home", tmp_path), \
         ...
         patch("nunmai_state_registry.acquire", return_value=MagicMock()), \
         patch("nunmai_cli.runtime_provider.resolve_runtime_provider", side_effect=resolve), \
...
def test_resolve_job_runtime_does_not_walk_the_chain_for_a_pinned_job(pin):
    ...
    with patch("nunmai_cli.runtime_provider.resolve_runtime_provider", resolve):
...
def test_resolve_job_runtime_walks_the_chain_for_an_unpinned_job():
    ...
    with patch("nunmai_cli.runtime_provider.resolve_runtime_provider", side_effect=resolve):
