"""
Tests for timezone support (nunmai_time module + integration points).
...
def _reset_nunmai_time_cache():
    """Reset the nunmai_time module cache."""
    nunmai_time.reset_cache()


# =========================================================================
# nunmai_time.now() — core helper
# =========================================================================

class TestNunmaiTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_nunmai_time_cache()

    def teardown_method(self):
        _reset_nunmai_time_cache()
        os.environ.pop("NUNMAI_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["NUNMAI_TIMEZONE"] = "Asia/Kolkata"
        result = nunmai_time.now()
...
class TestSafeStrftime:
    def test_valid_locale_text_is_byte_identical_to_strftime(self):
        ...
        for value in (
            ...
            assert nunmai_time.safe_strftime(value, fmt) == value.strftime(fmt)
    ...
    def test_surrogate_zone_name_renders_json_safe(self):
        ...
        rendered = nunmai_time.safe_strftime(value, "%a %Y-%m-%d %H:%M %Z %z %%Z")
        ...
        assert nunmai_time._repair_surrogates(_ESCAPED_ZONE, "cp1252") == "Paris, Madrid (heure d'été)"
...
class TestGetTimezone:
    ...
    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching NUNMAI_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("NUNMAI_TIMEZONE", raising=False)

        monkeypatch.setenv("NUNMAI_HOME", str(first_home))
        assert str(nunmai_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch NUNMAI_HOME in one process.
        monkeypatch.setenv("NUNMAI_HOME", str(second_home))
        assert str(nunmai_time.get_timezone()) == "America/New_York"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway NUNMAI_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("NUNMAI_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert nunmai_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert nunmai_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_nunmai_home_override(str(routed_home))
            try:
                assert nunmai_time.get_timezone_name() == "Asia/Tokyo"
                assert str(nunmai_time.get_timezone()) == "Asia/Tokyo"
            finally:
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_nunmai_home_override(str(homes[key]))
                try:
                    tz = nunmai_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When NUNMAI_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) NUNMAI_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("NUNMAI_TIMEZONE=" + os.environ.get("NUNMAI_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "NUNMAI_TIMEZONE=NOT_SET" in result["output"], (
            "NUNMAI_TIMEZONE should not leak into child env (only TZ)"
        )
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=nunmai_tz) which shifted
        absolute time when system-local tz != Nunmai tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Nunmai tz
        ...
        # Use a Nunmai timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _nunmai_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["NUNMAI_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_nunmai_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Nunmai timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["NUNMAI_TIMEZONE"] = "US/Eastern"
        _reset_nunmai_time_cache()
