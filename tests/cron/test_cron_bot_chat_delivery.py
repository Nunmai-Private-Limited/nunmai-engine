...
def test_unknown_profile_resolves_to_none():
    with mock.patch("nunmai_cli.profiles.profile_exists", return_value=False):
...
def test_create_validation_accepts_bare_and_existing():
    ...
    with mock.patch("nunmai_cli.profiles.profile_exists", return_value=True):
...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    with mock.patch.object(sched_delivery, "_run_bot_chat_turn", side_effect=fake_run), \
         mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/nunmai"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "Daily digest"}, "the output", "")

    assert err is None
    argv = calls["argv"]
    # The running install's interpreter, not whatever `nunmai` PATH names (same order as /update).
    assert argv[:3] == [sys.executable, "-m", "nunmai_cli.main"]
...
def test_deliver_timeout_returns_error_string():
    with mock.patch.object(
        sched_delivery, "_run_bot_chat_turn",
        side_effect=subprocess.TimeoutExpired(cmd="nunmai", timeout=600),
    ), mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/nunmai"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "")
    assert err is not None
...
def _child_env() -> dict:
    """The stand-in child imports ``nunmai_cli`` from this checkout, like the real ``-m nunmai_cli.main``."""
...
def test_delivery_child_runs_in_the_target_home_not_the_schedulers_cwd(tmp_path, monkeypatch):
    ...
    child = textwrap.dedent("""
        ...
        write_turn_report(os.environ.pop(TURN_REPORT_FILE_ENV), exit_code=0)
        """)
    env = {**_child_env(), "NUNMAI_HOME": str(home), TURN_REPORT_FILE_ENV: str(report)}
...
def test_bot_chat_turn_roundtrips_accented_utf8_reply(tmp_path):
    """The delivery child writes UTF-8 unconditionally — nunmai_cli reconfigures its
    own streams via nunmai_bootstrap on Windows even under PYTHONIOENCODING=cp1252 —
    ...
    raw UTF-8 bytes through sys.stdout.buffer the way the bootstrapped nunmai_cli
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("nunmai_cli.profiles.list_profile_names",
