...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    assert "-p" not in argv  # own profile: subprocess inherits NUNMAI_HOME
    assert "chat" in argv
...
def test_deliver_named_profile_uses_p_flag_and_clears_home():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/nunmai"), \
         mock.patch.dict(sched.os.environ, {"NUNMAI_HOME": "/tmp/other-profile"}):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "research")

    assert err is None
    argv = calls["argv"]
    assert argv[1:3] == ["-p", "research"]
    # -p owns resolution; the scheduler's own NUNMAI_HOME must not leak in.
    assert "NUNMAI_HOME" not in calls["kwargs"]["env"]
