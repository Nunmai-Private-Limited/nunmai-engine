...
class TestSlackSyntheticThreadCapture:
    def test_synthetic_slack_thread_not_captured(self):
        ...
        env = {
            "NUNMAI_SESSION_PLATFORM": "slack",
            "NUNMAI_SESSION_CHAT_ID": "D0BJTDCSR7C",
            "NUNMAI_SESSION_THREAD_ID": "1755043010.123456",
            "NUNMAI_SESSION_MESSAGE_ID": "1755043010.123456",
        }
    ...
    def test_genuine_slack_thread_preserved(self):
        ...
        env = {
            "NUNMAI_SESSION_PLATFORM": "slack",
            "NUNMAI_SESSION_CHAT_ID": "C0AGENERAL",
            "NUNMAI_SESSION_THREAD_ID": "1755040000.000100",
            "NUNMAI_SESSION_MESSAGE_ID": "1755043010.123456",
        }
        with _session_env(env):
            origin = _origin_from_env()
    ...
    def test_non_slack_platform_thread_untouched(self):
        ...
        env = {
            ...
            "NUNMAI_SESSION_CHAT_ID": "-1003941067111",
            "NUNMAI_SESSION_THREAD_ID": "2203",
            "NUNMAI_SESSION_MESSAGE_ID": "2203",
        }
        with _session_env(env):
            origin = _origin_from_env()
        assert origin is not None
...
def _run_at_in(minutes: int) -> str:
    return (nunmai_time.now() + timedelta(minutes=minutes)).isoformat()
...
class TestNearHorizonSlackThreadKept:
    ...
    def test_oneshot_firing_now_keeps_thread(self):
        ...
        frozen = nunmai_time.now()
        with (
            _session_env(_TOP_LEVEL_SLACK),
            patch("tools.cronjob_job_args.nunmai_time") as frozen_clock,
        ):
