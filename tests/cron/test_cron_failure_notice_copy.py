"""User-facing cron failure notices: plain words, the real output path, and the exact `nunmai cron`
...
def test_auth_failure_names_the_pinned_provider_and_the_failing_profile(monkeypatch, tmp_path):
    ...
    profile_home = tmp_path / ".nunmai" / "profiles" / "ops"
    ...
    assert "`nunmai -p ops auth add openai-codex --type oauth`" in msg, msg
    assert "<provider>" not in msg
    unpinned = _summarize_cron_failure_for_delivery(JOB, "Error code: 401 - Unauthorized")
    assert "`nunmai -p ops auth add <provider>`" in unpinned, unpinned
...
def test_rate_and_usage_limit_phrases_still_yield_a_provider_notice(monkeypatch):
    ...
    Nunmai Portal limit turns into a raw generic notice."""
    _no_chain(monkeypatch)
    for text in (
        "Nunmai Portal rate limit active until 15:00",
        ...
        assert "`nunmai cron run ab12cd34`" in msg or "`nunmai cron edit ab12cd34" in msg, msg
...
def test_cron_cause_gloss_is_the_shared_table():
    ...
    for reason in FAILURE_CAUSE_GLOSS:
        ...
        assert notice is not None and "`nunmai cron" in notice, reason
