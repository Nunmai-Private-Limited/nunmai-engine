...
class _Scope:
    ...
    def __enter__(self):
        self._token = set_nunmai_home_override(self._home)
        return self
...
class TestPoolReaping:
    def test_pool_of_an_unticked_home_is_reaped(self, tmp_path):
        ...
        try:
            ...
            assert nunmai_home_key(home_b) not in sched._parallel_pools, (
                "an unticked home's ThreadPoolExecutor leaked for the process's lifetime"
            )
            assert nunmai_home_key(home_b) not in sched._parallel_pool_max_workers
            # The home still being ticked keeps its pool.
            assert nunmai_home_key(home_a) in sched._parallel_pools
        finally:
