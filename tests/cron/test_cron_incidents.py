...
def test_redaction_applied_to_incident_error(monkeypatch, tmp_path):
    # agent.redact snapshots _REDACT_ENABLED from NUNMAI_REDACT_SECRETS at
    ...
    # so a developer shell exporting NUNMAI_REDACT_SECRETS=false freezes
...
def test_repeat_failure_alerts_once_then_reminds_after_cooldown(monkeypatch, tmp_path):
    ...
    with cron_jobs.use_cron_store(tmp_path):
        ...
        stale = (_nunmai_now() - timedelta(hours=7)).isoformat()
...
def test_cli_list_and_ack(monkeypatch, tmp_path, capsys):
    from nunmai_cli.cron import cron_incidents
