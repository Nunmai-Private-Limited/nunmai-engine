...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_nunmai_home_override(str(root))
    ...
    def run(argv, env, report_path, timeout):
        ...
        code = ('import json,sys; sys.argv=["nunmai"]+json.loads(sys.argv[1]); '
                'import nunmai_cli.main; from nunmai_constants import get_nunmai_home; '
                'print(json.dumps(str(get_nunmai_home())))')
        # Everything after the launcher (binary or ``python -m nunmai_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "nunmai_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".nunmai").exists()
    finally:
