...
the shared /blueprint command handler (nunmai_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated NUNMAI_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("nunmai://blueprint/morning-brief?")
