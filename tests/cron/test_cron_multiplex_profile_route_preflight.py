...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_nunmai_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "nunmai_constants.get_default_nunmai_root", lambda: root
    )
    token = set_nunmai_home_override(str(grant_home))
