...
def test_external_worker_adopts_execution_and_runs_payload_once(
    ...
    adopted = Mock(
        side_effect=lambda execution_id: (
            observed_homes.append(get_nunmai_home().resolve())
            or {"id": execution_id, "status": "running"}
        )
    )
    run = Mock(
        side_effect=lambda *_args, **_kwargs: (
            observed_homes.append(get_nunmai_home().resolve()) or True
        )
    )
...
def test_scoped_wrapper_exit_without_user_bus_names_the_cause_and_invalidates_probe(
    ...
    monkeypatch.setattr(scheduler, "_get_nunmai_home", lambda: tmp_path)
    monkeypatch.setattr(
...
def test_launch_external_worker_pins_the_gateways_tree_on_pythonpath(
    ...
    """#112729: the worker starts in ``cron.scheduler`` (no ``nunmai_cli.main`` bootstrap),
...
def test_launch_external_worker_pin_extends_the_sanitized_env_not_os_environ(
    ...
    assert worker_env_mod.pin_nunmai_tree_on_pythonpath(dict(untouched), repo_root) == untouched
    assert "PYTHONPATH" not in worker_env_mod.pin_nunmai_tree_on_pythonpath({}, repo_root)
...
def test_worker_delivery_queue_is_keyed_by_the_delivering_jobs_own_execution(
    ...
    ``nunmai cron run <other>``) must not queue under the OUTER execution id."""
    import cron.scheduler as scheduler
    ...
    monkeypatch.setenv("_NUNMAI_CRON_EXTERNAL_WORKER", "exec-outer")
...
def test_managed_gateway_restart_preserves_active_worker_and_single_side_effect(
    ...
    harness = (
        ...
        f"os.environ['NUNMAI_HOME'] = {str(home)!r}\n"
