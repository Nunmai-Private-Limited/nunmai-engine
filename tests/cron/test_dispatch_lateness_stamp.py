...
kind) on every recurring dispatch so `nunmai cron list` and
`nunmai cron status` can surface late catch-ups.
"""
...
def cron_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cron.jobs._nunmai_now", lambda: FIXED_NOW)
    return tmp_path
...
class TestDueScanDispatchStamp:
    def test_catch_up_beyond_grace_stamped_and_persisted(self, cron_store):
        ...
        # Persisted, so a separate `nunmai cron list` process can read it.
