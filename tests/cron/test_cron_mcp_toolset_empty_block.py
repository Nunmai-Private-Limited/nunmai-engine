...
def _run(job, tmp_path):
    ...
    with patch("run_agent.AIAgent") as agent_cls, \
         ...
         patch("nunmai_cli.runtime_provider.resolve_runtime_provider", return_value=dict(_RUNTIME)):
...
def test_requested_mcp_server_owned_by_other_profile_blocks_run(tmp_path):
    ...
    token = set_nunmai_home_override(tmp_path / "other")
    try:
        undo = _register_notion_in_scope(nunmai_home_key())
    finally:
