"""Regression for #86721 — a one-shot `nunmai cron run` invocation's
...
def test_one_shot_cli_path_reaps_dead_owner_row_before_returning_sync(tmp_path, monkeypatch):
    """The real one-shot `nunmai cron run` shape (``_SESSION_ASYNC_DELIVERY`` scoped to False,
    as nunmai_cli/cron.py::_job_action does) must still reap a 'running' row whose owner pid
