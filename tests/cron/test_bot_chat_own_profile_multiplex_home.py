...
The multiplexed ticker binds the served profile as a ``NUNMAI_HOME`` override that never reaches
...
def test_an_explicit_target_profile_still_wins_over_the_ticking_profile(multiplexer):
    ...
    root, captured = multiplexer
    token = set_nunmai_home_override(str(root / "profiles" / "alpha"))
    try:
        assert delivery._deliver_to_bot_chat({"id": "j", "name": "nightly"}, "the brief", "beta") is None
    finally:
        reset_nunmai_home_override(token)
    assert captured["env"]["NUNMAI_HOME"] == str(root / "profiles" / "beta")
