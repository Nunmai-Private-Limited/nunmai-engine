...
def test_cronjob_tool_create_no_agent_without_script_errors(nunmai_env):
...
def test_timed_out_no_agent_script_delivery_is_not_mislabeled_as_provider_failure(
    ...
    (nunmai_env / "scripts" / "slow.py").write_text("import time; time.sleep(999)\n")
    job = create_job(
        prompt=None,
        schedule="every 5m",
        script="slow.py",
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(nunmai_env, monkeypatch):
...
def test_run_job_script_path_traversal_still_blocked(nunmai_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(nunmai_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(nunmai_env):
    ...
    script = nunmai_env / "scripts" / "probe.py"
