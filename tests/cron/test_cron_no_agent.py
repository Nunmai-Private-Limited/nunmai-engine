...
def test_create_job_no_agent_requires_script(nunmai_env):
...
def test_update_job_roundtrips_no_agent_flag(nunmai_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = nunmai_env / "scripts" / "w.sh"
...
def test_run_job_no_agent_success_returns_script_stdout(nunmai_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = nunmai_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(nunmai_env, monkeypatch):
    ...
    def fake_load(*, nunmai_home=None, project_env=None):
        loaded_homes.append(nunmai_home)
        return []

    monkeypatch.setattr(env_loader, "load_nunmai_dotenv", fake_load)

    script_path = nunmai_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_nunmai_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(nunmai_env)
...
def test_no_agent_script_gets_owning_profiles_declared_secret_never_launch_residue(
    nunmai_env, monkeypatch, tmp_path,
):
    ...
    (nunmai_env / ".env").write_text("LAUNCH_ONLY_TOKEN=launch-secret\n", encoding="utf-8")
    monkeypatch.setenv("LAUNCH_ONLY_TOKEN", "launch-secret")  # what load_nunmai_dotenv() did at startup
    ...
    home_token = set_nunmai_home_override(str(routed))
...
def test_no_agent_script_of_launch_profile_keeps_its_own_env_credential(nunmai_env, monkeypatch):
    ...
    monkeypatch.delenv("JOB_SVC_TOKEN", raising=False)
    (nunmai_env / "scripts" / "probe.sh").write_text(_PRESENCE_PROBE, encoding="utf-8")
...
def test_run_job_script_nul_path_fails_cleanly(nunmai_env):
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.nunmai/scripts/nightly.sh",
