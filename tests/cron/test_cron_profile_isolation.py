...
profile's NUNMAI_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.nunmai/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active NUNMAI_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever NUNMAI_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    NUNMAI_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import nunmai_constants

    monkeypatch.setattr(
        nunmai_constants, "_get_platform_default_nunmai_home", lambda: root
    )
    monkeypatch.setenv("NUNMAI_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile NUNMAI_HOME (<root>/profiles/<name>), the cron store
    ...
    assert nunmai_constants.get_nunmai_home().resolve() == profile_home.resolve()
    assert nunmai_constants.get_default_nunmai_root().resolve() == root.resolve()

    # cron/jobs.py computes NUNMAI_DIR from get_nunmai_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.NUNMAI_DIR.resolve() == profile_home.resolve()
        assert (
