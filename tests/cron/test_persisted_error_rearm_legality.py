...
def cron_store(tmp_path, monkeypatch):
    ...
    (nunmai_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("NUNMAI_HOME", str(nunmai_home))
    monkeypatch.setattr(J, "NUNMAI_DIR", nunmai_home)
    monkeypatch.setattr(J, "CRON_DIR", nunmai_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", nunmai_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", nunmai_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_nunmai_now", lambda: now):
            due = J.get_due_jobs()
