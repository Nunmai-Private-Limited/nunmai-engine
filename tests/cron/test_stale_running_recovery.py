...
(``max(3 × NUNMAI_CRON_TIMEOUT, script timeout, 7200)``) and fails closed when the inactivity
...
from cron.executions import _nunmai_now, _transaction, recover_interrupted_executions
...
def _seed_running(job_id: str, *, age_seconds: float, pid: int = 99999) -> str:
    claimed_at = (_nunmai_now() - timedelta(seconds=age_seconds)).isoformat()
...
def test_live_owner_stale_claim_gets_recovered(monkeypatch):
    ...
    monkeypatch.setenv("NUNMAI_CRON_TIMEOUT", "600")
    ...
    monkeypatch.setenv("NUNMAI_CRON_TIMEOUT", "3600")
...
def test_live_owner_recent_claim_not_recovered(monkeypatch, timeout):
    ...
    monkeypatch.setenv("NUNMAI_CRON_TIMEOUT", timeout)
