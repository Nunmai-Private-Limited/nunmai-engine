...
def test_deferred_destination_does_not_follow_root_changes(tmp_path, monkeypatch, recipient):
    ...
    monkeypatch.setenv("NUNMAI_HOME", str(source))
    monkeypatch.setattr("nunmai_cli.profiles.get_profile_dir", lambda _: home)
    ...
    try:
        ...
        lease.release()
        db.close()
    monkeypatch.setattr("nunmai_cli.profiles.get_profile_dir", lambda _: other)
    ...
    try:
        with monkeypatch.context() as changed:
            changed.setenv("NUNMAI_HOME", str(tmp_path / "new-source"))
        ...
        if recipient == "cli":
            ...
            assert Path(run.call_args.args[1]["NUNMAI_HOME"]) == home
