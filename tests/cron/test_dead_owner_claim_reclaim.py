...
A one-shot ``nunmai cron run <job_id>`` used to background-dispatch the run
...
1. ``nunmai_cli.cron._job_action("run", ...)`` declares the channel stateless
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``nunmai cron run`` leaves behind: a row stuck in
...
class TestTickReapsDeadOwnerClaims:
    ...
    def test_reap_is_throttled_between_ticks(self, monkeypatch, executions):
        ...
        monkeypatch.setattr(
            ...
            {nunmai_home_key(scheduler_mod._get_nunmai_home()):
...
class TestOneShotCliRunIsSynchronous:
    ...
    def test_cli_run_declares_stateless_channel_before_dispatch(self, monkeypatch):
        """`nunmai cron run` must gate off async delivery so the run executes
    ...
    def test_background_dispatch_refused_when_channel_stateless(self, monkeypatch):
        ...
        monkeypatch.setenv("NUNMAI_SESSION_KEY", "inherited-gateway-session")
...
def test_reap_throttle_is_profile_scoped(monkeypatch, tmp_path):
    ...
    home_b.mkdir()
    monkeypatch.setattr(scheduler_mod, "_last_dead_owner_reap_at", {})
    monkeypatch.setattr(scheduler_mod, "_get_nunmai_home", lambda: home_a)
    scheduler_mod._maybe_reap_dead_owners()
    monkeypatch.setattr(scheduler_mod, "_get_nunmai_home", lambda: home_b)
    scheduler_mod._maybe_reap_dead_owners()
    assert len(calls) == 2, (
