#!/usr/bin/env python3
"""SQLite state store for Nunmai Engine: session metadata, message history, model
...
from nunmai_constants import get_nunmai_home, mkdir_under_nunmai_home
from typing import Any, Callable, Dict, Iterator, List, Optional, Tuple, TypeVar, cast

from nunmai_state_common import (
...
from nunmai_state_holders import read_only_db_uri
from nunmai_state_health import (
    STORAGE_CORRUPT, mark_storage_corrupt, note_storage_error, storage_corrupt_reason, storage_state,
)
from nunmai_state_errors import (
...
from nunmai_state_guard import (
...
from nunmai_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from nunmai_state_sessions import SessionSessionsMixin
from nunmai_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from nunmai_state_portability import SessionPortabilityMixin
from nunmai_state_telegram import SessionTelegramTopicsMixin
from nunmai_state_profile_repair import SessionProfileRepairMixin
from nunmai_state_schema import SessionSchemaMixin
import nunmai_state_holders as _state_holders
import nunmai_state_lockguard as _lockguard
from nunmai_state_lockowners import log_write_lock_holders
from nunmai_state_dbfile import (
...
from nunmai_state_messages import SessionMessagesMixin
from nunmai_state_rewind import SessionRewindMixin
from nunmai_state_wal import (
    _WAL_INCOMPAT_MARKERS, _on_disk_journal_mode, apply_database_pragmas, apply_wal_with_fallback,
)
from nunmai_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from nunmai_state_titles import SessionTitlesMixin
from nunmai_state_usage import SessionUsageMixin
from nunmai_state_maintenance import SessionMaintenanceMixin
from nunmai_state_gateway import SessionGatewayMixin
from nunmai_state_compression import SessionCompressionMixin
from nunmai_state_search import SessionSearchMixin
...
class SessionResumeTooLargeError(ValueError):
    def __init__(
        ...
        super().__init__(
            ...
            "Start a fresh chat and use `nunmai sessions export` to keep a copy, or raise the limit "
            "with `nunmai config set sessions.max_resume_messages 0`."
        )
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_nunmai_home() / "state.db"
...
# into "database is locked" (dashboard 503s, `nunmai sessions list` crashes) under a busy gateway.
_READ_BUSY_TIMEOUT_S = 5.0


def _default_db_path() -> Path:
    """Default state DB path at CALL time: a re-pointed ``DEFAULT_DB_PATH`` wins, else
    ``get_nunmai_home()`` is resolved fresh (a runtime NUNMAI_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_nunmai_home() / "state.db"


# Live-DB guard knobs live HERE (not in nunmai_state_guard): the hermetic conftest monkeypatches
# ``nunmai_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-NUNMAI_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``NUNMAI_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Nunmai root {root}). "
                "Tests must run against a temporary NUNMAI_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "NUNMAI_HOME. If this test genuinely needs the live database, mark it with "
...
def _secure_state_db_files(db_path: Path, *, create_main: bool = False) -> None:
    ...
    (gateway, desktop ``nunmai serve``) keep using the deleted inodes.
    """
    if os.name == "nt":
        return
...
_NETWORK_DRIVE_ACTION = (
    "Move it to a local disk (`nunmai {profile_arg}doctor` shows where it is), then start Nunmai again."
)


def format_session_db_unavailable(
    prefix: str = "Nunmai can't open its session history right now",
    *,
    details: bool = False,
) -> str:
    """User-facing one-liner: ``<prefix>: <gloss>. <consequence> <action>[ network hint]``.

    The cause table lives in ``nunmai_state_user_copy`` so CLI, gateway and TUI agree. Chat
    ...
    message names the network-drive suspicion, because ``nunmai doctor --fix`` cannot repair a
    ...
    if not cause:
        return (
            f"{prefix}. {_SESSION_DB_CONSEQUENCE} Run `nunmai {profile_arg}doctor` to check the "
            "storage location."
        )
    from nunmai_state_user_copy import describe_storage_failure
    ...
    if details:
        from nunmai_state_user_copy import storage_failure_details
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to NUNMAI_HOME/sessions/<id>.jsonl (state.db was replaced under a
...
# connection per resolved path via nunmai_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in nunmai_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via nunmai_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # See #60609.  `recovered` = placeholders `nunmai sessions recover` synthesizes for
    ...
    # SQLite's deterministic busy handler convoys under many nunmai processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        self._wal_lock_guard: dict = {}  # nunmai_state_lockguard.hold() record, see _open_writer
        ...
        # Opened via nunmai_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via nunmai_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        mkdir_under_nunmai_home(self.db_path.parent)
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`nunmai db optimize`); no background worker races session lifecycle.
    ...
    def _open_read_only(self) -> None:
        ...
        for attempt in range(_READ_ONLY_IOERR_RETRY_ATTEMPTS + 1):
            try:
                ...
                # and a retry would multiply that wait on blocking callers (TUI, `nunmai status`).
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            f"state.db was empty or damaged ({zsize} bytes) and has been {where}; Nunmai started with a "
            "fresh, empty session database. To bring old sessions back, run "
            f"`nunmai sessions recover --source {qpath or self.db_path} --inspect-only`, or restore a "
            "snapshot with `/snapshot list` then `/snapshot restore <id>` (terminal `nunmai` chat only)."
        )
        logger.error(msg)
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if is_sqlite_lock_error(exc):
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Nunmai process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `nunmai sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through nunmai_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see nunmai_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``nunmai_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from nunmai_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                ...
                if quarantine_reason is not None:
                    logger.warning(
                        ...
                        "before restarting, then run `nunmai sessions recover --source %s --inspect-only`.",
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('nunmai_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('nunmai_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('nunmai_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('nunmai_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('nunmai_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('nunmai_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('nunmai_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('nunmai_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('nunmai_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('nunmai_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('nunmai_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('nunmai_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('nunmai_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('nunmai_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('nunmai_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('nunmai_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('nunmai_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('nunmai_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('nunmai_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('nunmai_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('nunmai_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('nunmai_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('nunmai_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('nunmai_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('nunmai_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('nunmai_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('nunmai_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('nunmai_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('nunmai_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('nunmai_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('nunmai_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('nunmai_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('nunmai_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('nunmai_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('nunmai_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('nunmai_state_registry', 'release_or_close'),
    'report_startup_progress': ('nunmai_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('nunmai_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('nunmai_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('nunmai_state_wal', 'sqlite_source_id'),
    'workspace_key': ('nunmai_state_sessions', 'workspace_key'),
}
