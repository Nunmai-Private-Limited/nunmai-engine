...
from nunmai_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
...
class SessionDB(
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `nunmai snapshot list` / "
            f"`nunmai snapshot restore <id>` if available, or salvage the preserved bytes with "
            f"`nunmai sessions recover --source {qpath or self.db_path}`. "
