#!/usr/bin/env python3
"""
SQLite State Store for Nunmai Engine.
...
# transitive dependency via nunmai_state_common). run_agent keeps its own
# predating copy — nunmai_state cannot import run_agent (circular) — guarded
...
from nunmai_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from nunmai_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``nunmai -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_nunmai_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.nunmai`` even when a test fixture later
    redirects ``NUNMAI_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_nunmai_home()`` fresh so a runtime
       ``NUNMAI_HOME`` redirect takes effect regardless of import order.
    """
...
# ~/.nunmai/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects NUNMAI_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without NUNMAI_HOME, a stale worktree
# without the re-pin, or a developer shell that exports NUNMAI_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "NUNMAI_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.nunmai``).  The test conftest injects the pre-sandbox production
#: root here so custom-``NUNMAI_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Nunmai root for the guard.

    Deliberately avoids ``Path.home()`` / ``nunmai_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``nunmai_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "nunmai"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "nunmai"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".nunmai"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``NUNMAI_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Nunmai test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``NUNMAI_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "NUNMAI_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``NUNMAI_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``nunmai`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Nunmai home *root*.
    ...
    under ``~/.nunmai/nunmai-engine/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``NUNMAI_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``NUNMAI_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "NUNMAI_HOME. If this test genuinely needs the live "
...
# nunmai_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``nunmai sessions optimize`` (FTS merge + VACUUM)
    ...
    ``nunmai_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``nunmai_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``nunmai update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Nunmai-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Nunmai"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Nunmai-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `nunmai doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``nunmai_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see nunmai_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with nunmai_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``nunmai doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Nunmai host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``nunmai serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Nunmai's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``nunmai_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship nunmai_state without nunmai_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``nunmai_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Nunmai home tree (``get_nunmai_home()``) — the safe repair scope:
      Nunmai owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``nunmai_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_nunmai_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Nunmai needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (nunmai_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_nunmai_state.py:600-632)
        ...
        probe_session_id = f"_nunmai_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Nunmai deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (nunmai gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.nunmai/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``nunmai sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("NUNMAI_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_nunmai_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("NUNMAI_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship nunmai_state without nunmai_cli). A
    ...
    try:
        ...
        logger.debug(
            "nunmai_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch nunmai_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``nunmai_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `nunmai snapshot list` / "
                "`nunmai snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (nunmai doctor, dashboards) ──────────
...
_NUNMAI_CMDLINE_MARKERS = ("nunmai_cli.main", "nunmai_cli/main", "nunmai serve",
                           "nunmai-engine", "nunmai gateway", "nunmai chat")


def _looks_like_nunmai(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Nunmai process?
    ...
    We only flag processes that look like Nunmai, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _NUNMAI_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple nunmai processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Nunmai
    ...
    # `nunmai update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `nunmai snapshot list` / "
                    f"`nunmai snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`nunmai db optimize`),
            ...
            # ``nunmai_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `nunmai update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `nunmai sessions optimize-storage` on a host "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Nunmai process —
                        # only flag uninspectable holders that look like
                        # another Nunmai instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_nunmai(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from nunmai_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `nunmai sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`nunmai sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``nunmai sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``nunmai --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``nunmai serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: nunmai_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Nunmai) receive
        ...
        if repair_alternation and messages:
            # Lazy import: nunmai_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Nunmai Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``nunmai -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``nunmai sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``NUNMAI_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Nunmai, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Nunmai session.

        A Nunmai session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Nunmai session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `nunmai sessions vacuum` runs in a transient
