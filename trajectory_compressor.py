...
from nunmai_constants import OPENROUTER_BASE_URL, get_nunmai_home
from agent.retry_utils import jittered_backoff
from nunmai_cli.env_loader import load_nunmai_dotenv

# Load .env from NUNMAI_HOME first, then project root as a dev fallback.
load_nunmai_dotenv(nunmai_home=get_nunmai_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: nunmai setup")
