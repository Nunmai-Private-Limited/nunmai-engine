# Nunmai Plugin Catalog

Curated, Nunmai-approved Nunmai plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`nunmai plugins catalog` / `nunmai plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `nunmai-engine` repository, reviewed and merged by a maintainer. There is
...
3. **No self-updating code.** A listed plugin must not fetch and replace
   ...
   `nunmai plugins update <name>`. Keep the updater in the standalone
...
5. **Owner-or-major-contributor submissions, or a maintainer-curated sweep.**
   ...
   Nunmai maintainers may also add entries in batches from a reviewed sweep
...
7. **The install scanner runs at admission.** `nunmai plugins validate` includes
...
8. **Desktop plugins stay inside the SDK surface.** A `desktop/plugin.js` runs
   ...
   no `eval`/`new Function`, no `import()` of anything but `@nunmai/plugin-sdk`
   / `react` (app bundle chunks, blob or http URLs included), no script-tag
   injection, no reaching into the app's internal stores. `nunmai plugins
   ...
   patching around it.
9. **Dependency security policy is the plugin's.** Nunmai's 14-day
   `exclude-newer` quarantine covers Nunmai's own dependencies only; a plugin's
   `python_dependencies` / `pyproject.toml` install under the plugin's policy
   (no quarantine, still inside Nunmai's core constraints). Reviewers read the
...
Every entry gets a page at `https://nunmai.in/docs/plugins/<name>`
