...
(``nunmai_cli.web_routers.display``) to stream raw RFB; ``display.lease.acquire`` / ``release`` are
...
def _display_snapshot() -> dict:
    ...
    return {**st.as_dict(), "lease": _bd_lease.public_view(_bd_lease.get()), "profile_key": nunmai_home_key()}
...
def _(rid, params: dict) -> dict:
    ...
    from nunmai_cli.dashboard_auth.ws_tickets import mint_ticket
    from tools.bot_desktop import runtime as _bd_runtime
    try:
        ...
        ticket = mint_ticket(user_id=f"display:{viewer_id}", provider="bot-desktop",
                             extra={"nunmai_home": str(get_nunmai_home()), "viewer_id": viewer_id})
