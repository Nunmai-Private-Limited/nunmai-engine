...
def _profile_build_scope(profile_home):
    """Bind NUNMAI_HOME + secret scope for an agent build (home alone leaves get_secret() on the LAUNCH .env)."""
    if not profile_home:
        yield
        return
...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    try:
        ...
        from nunmai_state_errors import is_disk_full_error
...
def _(rid, params: dict) -> dict:
    ...
    # ``profile`` (app-global remote mode): stored so the build and every turn re-bind NUNMAI_HOME.
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from nunmai_state import SessionResumeTooLargeError, resolved_max_resume_messages
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if (session := _sessions.get(sid)) is not None:
                ...
                # Each turn re-binds NUNMAI_HOME (mid-turn memory/skills reads); lease claimed lazily on turn 1.
...
def _(rid, params: dict) -> dict:
    ...
    # the WS-detached sentinel: detached is attachable until grace-reap, and ``nunmai --tui`` rides stdio.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        try:
            home = Path(profile_home) if profile_home is not None else get_nunmai_home()
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    # Nunmai credits are agent-independent (portal fetch); fail-open when absent.
    with contextlib.suppress(Exception):
...
def _pet_config_followup(what: str, fn, *args) -> None:
    """Best-effort ``nunmai_cli.pets`` active-slug update after a store op that already succeeded."""
    try:
        fn(*args)
    except Exception as exc:  # noqa: BLE001
...
def _(rid, params: dict) -> dict:
    """Read-only billing view (no scope required); fail-open. The Nunmai free tier has no account to
...
def _(rid, params: dict) -> dict:
    ...
    from nunmai_cli.nous_billing import post_subscription_preview
...
def _billing_route(name: str, call, *, invalid=None, message: str = "", error: str = "invalid_request",
    ...
    """Portal write route on ``nunmai_cli.nous_billing`` (lazy; tests patch its functions): ``invalid(params)``
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Nunmai TUI Status", "", f"Session ID: {key}", f"Path: {display_nunmai_home()}",
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    path = saved_dir / f"nunmai_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
