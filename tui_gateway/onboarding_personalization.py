...
def remember_onboarding(answers: dict) -> dict:
    ...
    for key, label in (('focus', 'Focus areas'), ('connectors', 'Tools the user uses (not connection status)'),
                       ('plugins', 'Nunmai plugins the user picked during onboarding (not install status)')):
    ...
    # backend or under a custom Nunmai home.
    token = set_nunmai_home_override(get_profile_dir('default'))
    try:
