...
def _format_live_review_output(sid: str, session: Optional[dict], arg: str) -> str:
    ...
    # (NUNMAI_UI_SESSION_ID + steer authority), or delegate_task registers the reviewer with no owner
    ...
    try:
        ...
        # reads NUNMAI_CODEX_BASE_URL via get_secret; under multiplex that raises
...
def _format_live_history_output(sid: str, session: dict, arg: str) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = {"user": "You", "assistant": "Nunmai"}.get(role, role.title())
...
def _mirror_personality(sid, session, agent, arg) -> None:
    if arg and agent:
        ...
        from nunmai_cli.personality import persist_personality  # single owner: no surface drift
