...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from nunmai_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (nunmai_cli.personality owns this)."""
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    if env_val := os.environ.get("NUNMAI_TUI_MAX_TURNS"):
...
def _load_fallback_model():
    ...
    NunmaiCLI/gateway: ``fallback_providers`` first, legacy ``fallback_model`` merged after)."""
...
def _sync_agent_fallback_with_config(sid: str, session: dict) -> None:
    ...
    opened before ``nunmai fallback add`` kept an empty chain forever and a provider-quota 429 ended in
