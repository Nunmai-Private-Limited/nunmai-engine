...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.nunmai/.env`` into the gateway process via
    ``nunmai_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from nunmai_cli.config import reload_env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "nunmai_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m nunmai_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind NUNMAI_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_nunmai_home_override(_profile_home) if _profile_home else None
        )
        try:
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_nunmai_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off NUNMAI_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from nunmai_constants import set_nunmai_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from nunmai_constants import reset_nunmai_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``nunmai journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from nunmai_cli.skills_hub import do_install
        ...
        if action == "browse":
            from nunmai_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from nunmai_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `nunmai mcp` offers, so
    ...
    try:
        ...
        from nunmai_cli import mcp_catalog
...
# (nunmai_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes NUNMAI_HOME via set_nunmai_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses nunmai_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from nunmai_cli.config import load_config, save_config, save_env_value
        from nunmai_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from nunmai_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.nunmaiDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``nunmai mcp login`` uses
    ...
    try:
        ...
        nunmai_home = str(get_nunmai_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(nunmai_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``nunmai plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.nunmai/plugins/`` (non-interactive).
    ...
    plugins live under each profile's NUNMAI_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Nunmai plugin.
