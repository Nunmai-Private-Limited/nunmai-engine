...
def _profile_scoped_rpc(
    ...
    """Wrap a handler body with the optional ``profile`` NUNMAI_HOME scope. Order: ``required``
    ...
    def deco(body):
        def handler(rid, params: dict) -> dict:
            ...
            if profile := _str_arg(params, "profile") if scoped else "":
                try:
                    profile_dir = _tools_mod("nunmai_cli.profiles").get_profile_dir(profile)
                    if not profile_dir or not profile_dir.is_dir():
                        return _err(rid, 4064, f"profile '{profile}' not found")
                    token = _tools_mod("nunmai_constants").set_nunmai_home_override(str(profile_dir))
...
def _mcp_named_server(rid, params):
    ...
    name, servers = _str_arg(params, "name"), _tools_mod("nunmai_cli.mcp_config")._get_mcp_servers()
...
def _run_plugin_command(handler, arg: str) -> str:
    return str(_tools_mod("nunmai_cli.plugins").resolve_plugin_command_result(handler(arg)) or "")


def _is_profile_skill_command(session: dict, base: str) -> bool:
    """True when ``/base`` is a skill command of the session's profile (NUNMAI_HOME bound to it so
...
def _(rid, params: dict) -> dict:
    cfg = _load_cfg()
    api_key = _tools_mod("agent.secret_scope").get_secret("NUNMAI_API_KEY", "") or cfg.get("api_key", "")
    masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
    base_url = os.environ.get("NUNMAI_BASE_URL", "") or cfg.get("base_url", "")
    sections = [
        {"title": "Model", "rows": [
        ...
        {"title": "Environment", "rows": [["Working Dir", os.getcwd()], ["Config File", str(_nunmai_home / "config.yaml")]]},
    ]
...
def _(rid, params: dict) -> dict:
    ...
    enabled, tools}]}``"""
    servers = _tools_mod("nunmai_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    import time
    hc = _tools_mod("nunmai_constants")
    configured = _tools_mod("nunmai_cli.mcp_config")._get_mcp_servers()
