...
def _profile_scoped_rpc(
    ...
    (``_session_profile_runtime_scope``), not NUNMAI_HOME alone: these bodies read config.yaml,
...
def _scoped_rpc(name: str, fail_code: int = 5024, **kw):
    """``@method(name)`` + ``_profile_scoped_rpc`` (optional ``profile`` NUNMAI_HOME scope)."""
...
def _mcp_server_rows():
    config_servers = _tools_mod("nunmai_cli.mcp_config")._get_mcp_servers()
...
def _capture_run_kwargs(timeout: int) -> dict:
    ...
    return dict(
        ...
        stdin=subprocess.DEVNULL, creationflags=_tools_mod("nunmai_cli._subprocess_compat").windows_hide_flags())
...
_SIMPLE_RPCS = {
    ...
    # Re-read ``~/.nunmai/.env`` (CLI ``/reload`` parity); built agents keep their pool, ``/new`` resolves fresh.
    "reload.env": (5015, lambda params: {"updated": int(_tools_mod("nunmai_cli.config").reload_env())}),
    "plugins.list": (5032, lambda params: {"plugins": [
        {"name": n, "version": getattr(i, "version", "?"), "enabled": getattr(i, "enabled", True)}
        for n, i in _tools_mod("nunmai_cli.plugins").get_plugin_manager()._plugins.items()]}),
...
def _mcp_reload_confirm_required() -> bool:
    ...
    try:
        cfg = _tools_mod("nunmai_cli.config").load_config()
...
def _refresh_live_sessions(home=None, *, preserve_prefix: bool = False, note: str = "") -> None:
    ...
    want = nunmai_home_key(home) if home is not None else None
    with _sessions_lock:
        live = [(sid, sess) for sid, sess in _sessions.items() if sess.get("agent") is not None and (
            want is None or nunmai_home_key(sess.get("profile_home") or get_process_nunmai_home()) == want)]
...
def refresh_plugin_sessions(home, note: str) -> None:
    ...
    turn. Called by ``nunmai_cli.plugins_activation_live``."""
...
def _catalog_registry(cat: _Catalog) -> None:
    commands = _tools_mod("nunmai_cli.commands")
...
def _catalog_plugin_commands(cat: _Catalog) -> None:
    plugin_cmds = _tools_mod("nunmai_cli.plugins").get_plugin_commands() or {}
    if plugin_cmds:
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {
        "pairs": cat.pairs, "sub": {k: v[:] for k, v in _tools_mod("nunmai_cli.commands").SUBCOMMANDS.items()},
        "canon": cat.canon,
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m nunmai_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    return _captured_exec(
        rid, [sys.executable, "-m", "nunmai_cli.main", *argv], min(int(params.get("timeout", 240)), 600),
        ...
        env=nunmai_subprocess_env(inherit_credentials=True))


@_rpc("command.resolve", 5012)
def _(rid, params: dict) -> dict:
    r = _tools_mod("nunmai_cli.commands").resolve_command(params.get("name", ""))
    if r:
...
def _plugin_command_handler(name: str):
    try:
        return _tools_mod("nunmai_cli.plugins").get_plugin_command_handler(name)
    except Exception:
        return None


def _run_plugin_command(handler, arg: str, session=None) -> str:
    """Run a plugin slash-command handler under the session's ``NUNMAI_SESSION_*`` binding.
    ...
    plugins = _tools_mod("nunmai_cli.plugins")
...
def _session_home_scope(session, cwd: str | None = None):
    """Bind NUNMAI_HOME and the logical cwd to the session for the block.
    ...
    hc = _tools_mod("nunmai_constants")
    ...
    token = hc.set_nunmai_home_override(profile_home) if profile_home else None
    ...
    try:
        ...
        if token is not None:
            hc.reset_nunmai_home_override(token)
...
def _bundle_key_for(name: str):
    ...
    try:
        if _tools_mod("nunmai_cli.commands").resolve_command(name) is None:
...
_cmd_init = _prompt_builtin("nunmai_cli.init_command", "build_init_prompt_for_cwd", kw="extra")
...
def _cmd_moa(rid, params, session, name, arg):
    ...
    try:
        moa = _tools_mod("nunmai_cli.moa_config")
        if not arg:
...
def _cmd_focus(rid, params, session, name, arg):
    ...
    fv = _tools_mod("nunmai_cli.focus_view")
...
def _cmd_goal(rid, params, session, name, arg):
    with _session_profile_runtime_scope(session or {}):
        sid_key, goals, err = _session_key_or_err(rid, session, "nunmai_cli.goals", "goals")
        if err:
            return err
        try:
...
def _cmd_loop(rid, params, session, name, arg):
    sid_key, loops, err = _session_key_or_err(rid, session, "nunmai_cli.loops", "loops")
    if err:
        return err
...
def _(rid, params: dict) -> dict:
    ...
    api_key = get_secret("NUNMAI_API_KEY", "") or cfg.get("api_key", "")
    masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
    base_url = get_secret("NUNMAI_BASE_URL", "") or cfg.get("base_url", "")
    sections = [
        {"title": "Model", "rows": [
...
def _configure_session_tools(rid, params: dict, sid: str, session) -> dict:
    ...
    hc, tc = _tools_mod("nunmai_cli.config"), _tools_mod("nunmai_cli.tools_config")
    cfg = hc.load_config()
...
def _(rid, params: dict) -> dict:
    """cronjob() keys off NUNMAI_HOME, so ``profile`` reaches a per-profile cron store."""
...
def _skills_install(rid, params, query):
    ...
    _tools_mod("nunmai_cli.skills_hub").do_install(query, skip_confirm=True, console=quiet)
    return _ok(rid, {"installed": True, "name": query})


def _skills_browse(rid, params, query):
    pg = int(params.get("page", 0) or 0) or (int(query) if query.isdigit() else 1)
    browse = _tools_mod("nunmai_cli.skills_hub").browse_skills
    return _ok(rid, browse(page=pg, page_size=int(params.get("page_size", 20))))


_SKILLS_ACTIONS = {
    "list": lambda rid, params, query: _ok(rid, {"skills": _tools_mod("nunmai_cli.banner").get_available_skills()}),
    "search": _skills_search, "install": _skills_install, "browse": _skills_browse,
    "inspect": lambda rid, params, query: _ok(
        rid, {"info": _tools_mod("nunmai_cli.skills_hub").inspect_skill(query) or {}})}
...
# Gateway mirrors of the dashboard REST surface (nunmai_cli/web_routers/mcp.py) so a
# desktop plugin can manage MCP servers for ANY profile. Persistence: nunmai_cli/mcp_config.py.
@_scoped_rpc("mcp.catalog")
def _(rid, params: dict) -> dict:
    """``{servers: [{name, description, installed, enabled, requires: [env keys], transport}]}`` per profile."""
    mcp_catalog = _tools_mod("nunmai_cli.mcp_catalog")
    out = []
...
def _(rid, params: dict) -> dict:
    ...
    include_runtime = (_tools_mod("agent.secret_scope").is_multiplex_active()
                       or hc.nunmai_home_key() == hc.nunmai_home_key(hc.get_process_nunmai_home()))
...
def _(rid, params: dict) -> dict:
    ...
    mc = _tools_mod("nunmai_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    hc, mc = _tools_mod("nunmai_cli.config"), _tools_mod("nunmai_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    if err := _mcp_plugin_write_error(rid, name, _mcp_server_rows()[1]):
        return err
    if not _tools_mod("nunmai_cli.mcp_config")._remove_mcp_server(name):
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        cfg["auth"] = "oauth"
        nunmai_home = str(_tools_mod("nunmai_constants").get_nunmai_home().expanduser().resolve(strict=False))
        result = _tools_mod("tui_gateway.mcp_oauth_sessions").start_flow(
            nunmai_home, name, cfg, client_redirect_uri=client_redirect_uri)
...
def _(rid, params: dict) -> dict:
    ...
    home = str(_tools_mod("nunmai_constants").get_nunmai_home().expanduser().resolve(strict=False))
...
def _plugin_server_rows(plugin_dir: Path | None, key: str, *, portable: bool) -> list[dict]:
    if not portable or plugin_dir is None:
        return []
    package = _tools_mod("nunmai_cli.agent_plugins").load_agent_plugin(plugin_dir, plugin_dir)
    namespace = package.manifest.get("extensions", {}).get("com.nousresearch.nunmai", {})
    declared = namespace.get("servers", {})
    if not isinstance(declared, dict):
        return []
    server_name_for = _tools_mod("nunmai_cli.plugins_manifest").portable_mcp_server_name
    ...
    for name in sorted(declared):
        ...
        if connected:
            ...
            continue
        decl = _tools_mod("nunmai_platform.declaration").lookup(internal_name)
...
def _plugin_rows() -> list[dict]:
    pc = _tools_mod("nunmai_cli.plugins_cmd")
    cat = _tools_mod("nunmai_cli.plugins_cmd_catalog")
    ...
    for name, version, desc, source, _dir, key in sorted(pc._discover_all_plugins()):
        ...
        out.append({
            ...
            "settings_schema": _tools_mod("nunmai_cli.plugins_settings").plugin_settings_fields(key, _dir_path),
...
def _plugins_toggle(rid, params):
    ...
    toggle = _tools_mod("nunmai_cli.plugins_cmd").dashboard_set_agent_plugin_enabled
...
def _plugins_install(rid, params):
    ...
    _ensure_plugin_activation_listener()
    result = _tools_mod("nunmai_cli.plugins_cmd").dashboard_install_plugin(
...
def _plugins_update(rid, params):
    ...
    pc, cat = _tools_mod("nunmai_cli.plugins_cmd"), _tools_mod("nunmai_cli.plugins_cmd_catalog")
    ...
    if result.changed:
        _ensure_plugin_activation_listener()
        activate = _tools_mod("nunmai_cli.plugins_activation").activate_plugin_now
        payload = _with_activation({**payload, **activate(result.installed_name)}, result.installed_name)
    return _ok(rid, payload)


def _plugins_remove(rid, params):
    """Uninstall a user install (``<NUNMAI_HOME>/plugins/<name>``) — the same core as ``nunmai plugins
    ...
    result = _tools_mod("nunmai_cli.plugins_cmd").dashboard_remove_user_plugin(name)
...
def _plugins_settings(rid, params):
    ...
    try:
        written = _tools_mod("nunmai_cli.plugins_settings").save_plugin_settings(
...
def _plugins_onboarding(rid, params):
    ...
    return _ok(rid, {"onboarding": _tools_mod("nunmai_cli.plugin_catalog_presence").onboarding_entries()})
...
def _(rid, params: dict) -> dict:
    """TUI Plugins Hub backend (shares primitives with ``nunmai plugins`` / the dashboard):
