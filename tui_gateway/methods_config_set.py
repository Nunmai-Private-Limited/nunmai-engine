...
def _set_model(rid, params, key, value, session):
    ...
    if session:
        ...
        if parse_model_switch_args(str(value)).is_once:
            ...
            # One string for every client: the Ink TUI (dashboard /chat, `nunmai --tui`) has no
...
def _set_focus(rid, params, key, value, session):
    ...
    from nunmai_cli.focus_view import FOCUS_TOOL_PROGRESS_MODE, normalize_tool_progress_mode, resolve_focus_arg
    d_f = _display_cfg()
...
def _set_yolo(rid, params, key, value, session):
    ...
    if scope == "global":
        ...
        enable = _BOOL_WORDS.get(raw, not is_truthy_value(os.environ.get("NUNMAI_YOLO_MODE")))
        if enable:
...
def _set_cwd(rid, params, key, value, session):
    ...
    if Path(get_nunmai_home()).resolve() == Path(_nunmai_home).resolve():
...
def _set_personality(rid, params, key, value, session):
    ...
    # Persists via nunmai_cli.personality (single owner), never the user-owned system prompt.
