"""Onboarding JSON-RPC handlers: the backend owns the setup profile (``nunmai_cli.setup_profile``).
...
def _(rid, params: dict) -> dict:
    ...
    from nunmai_cli.setup_profile import ensure_setup_profile
    try:
...
def _(rid, params: dict) -> dict:
    ...
    from nunmai_cli.setup_profile import find_setup_profile, reset_setup_profile
...
def _clear_setup_sessions(profile_dir) -> None:
    ...
    with _sessions_lock:
        live = [sid for sid, sess in _sessions.items()
                if Path(sess.get("profile_home") or _nunmai_home).resolve() == target]
    for sid in live:
