...
def _connector_rpc(rid, params, action):
    ...
    try:
        ...
        with _session_profile_runtime_scope({"profile_home": profile_home or str(_nunmai_home)}):
...
def _(rid, params):
    ...
    with _session_profile_runtime_scope({"profile_home": session.get("profile_home") or str(_nunmai_home)}):
        return _apply_connection_answer(rid, answer, operation)
    ...
    with server._sessions_lock:
        sid = next(
            (
                ...
                and nunmai_home_key(session.get("profile_home") or get_process_nunmai_home()) == operation.profile_key
            ),
            None,
        )
    if sid is not None:
