...
class HostSupervisor:
    ...
    def __init__(
        ...
        expected_nunmai_home: str | None = None, autostart: bool = True) -> None:
        self.registry_path = (
            Path(registry_path) if registry_path is not None
            else get_nunmai_home() / "state" / _REGISTRY_NAME)
        ...
        self.expected_nunmai_home = (
            str(get_nunmai_home()) if expected_nunmai_home is None else expected_nunmai_home)
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        self._hello = {}
        env = {**nunmai_subprocess_env(inherit_credentials=True), **os.environ, **(self.env or {})}
        env["NUNMAI_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
        root = str(_repo_root())
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("nunmai_home") or "")
        if got_home and got_home != self.expected_nunmai_home:
            raise RuntimeError(
                f"compute host NUNMAI_HOME mismatch: {got_home} != {self.expected_nunmai_home}")
