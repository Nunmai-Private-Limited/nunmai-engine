...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    hung SQLite write can't block exit past ``NUNMAI_TUI_EXIT_FLUSH_BUDGET_S`` (default 5s). Running sessions
    ...
    if result["flushed"] < flushable:
        logger.warning(
            ...
            "may have been lost (NUNMAI_TUI_EXIT_FLUSH_BUDGET_S)",
...
def _transport_is_dead(transport) -> bool:
    ...
    # standalone `nunmai --tui` and must NOT count as dead.
...
_HEARTBEAT_REFRESH_S = max(0.0, env_float("NUNMAI_GATEWAY_HEARTBEAT_REFRESH_S", 60.0))
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    threading.Thread(target=_loop, name="nunmai-gateway-heartbeat", daemon=True).start()
