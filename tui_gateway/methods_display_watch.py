"""Cross-process lease watcher: ``display.lease`` for transitions made OUTSIDE ``nunmai serve``.

The lease (``tools.bot_desktop.lease``) lives on disk and is changed by whatever process hosts the
agent — a ``nunmai computer-use screen`` takeover from the CLI, a cron worker's release.
...
def _watched_lease_homes() -> list[Path]:
    return [Path(_nunmai_home), *_served_profile_homes]
...
def _poll_runtime_files() -> None:
    ...
    from nunmai_constants import nunmai_home_key, reset_nunmai_home_override, set_nunmai_home_override
...
def _ensure_lease_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="nunmai-lease-watcher", daemon=True).start()
