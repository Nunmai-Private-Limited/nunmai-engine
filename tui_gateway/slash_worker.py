...
# from shadowing Nunmai's own top-level modules.  This worker is spawned as
...
# and crash the child in a retry loop (issue #51286).  ``nunmai_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``NUNMAI_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("NUNMAI_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("NUNMAI_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before NunmaiCLI snapshots tools.

    Each slash_worker child is its own process — the parent ``nunmai serve``
...
def main():
    ...
    # Start before the (hundreds-of-ms) NunmaiCLI build — that window is itself
