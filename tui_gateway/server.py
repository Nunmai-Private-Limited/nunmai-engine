...
from nunmai_constants import (
    get_nunmai_home, get_nunmai_home_override, profile_name_for_home,
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # launch NUNMAI_HOME (#40677). The override goes through the build_subprocess_env factory's `extra`
        # (applied last, always wins) instead of a hand-rolled env["NUNMAI_HOME"] assignment.
        ...
        env = _prepend_tool_paths(build_subprocess_env(
            nunmai_subprocess_env(inherit_credentials=True), scrub_secrets=False,
            inherit_profile_home=False, extra={"NUNMAI_HOME": str(profile_home)} if profile_home else None))
...
def _get_db():
    ...
    if _db is None:
        ...
        try:
            ...
            # get_nunmai_home(), which the desktop multiplex cron ticker temporarily
            ...
            _db, _db_error = acquire(Path(_nunmai_home) / "state.db"), None
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s NUNMAI_HOME around a handler (pets/projects resolve via
    ``get_nunmai_home``, so app-global remote mode still hits the focused profile). No-op for launch.
...
def _bind_build_profile_scopes(profile_home: str) -> "_TurnScopes":
    """Bind a session profile's NUNMAI_HOME / secret / terminal scopes for an agent build. Fail-open per
...
def _release_build_profile_scopes(scopes: "_TurnScopes") -> None:
    if scopes.home is not None:
        reset_nunmai_home_override(scopes.home)
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    room bots "out of Nunmai credits" after a profile switch); signals: ``room_plumbing`` /
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    if live is None or not hasattr(live[0], "_build_system_prompt") or not hasattr(live[2], "update_system_prompt"):
        return
    agent, session_key, db = live
    # Re-bind the session's profile NUNMAI_HOME (the build's finally reset it → root profile's SOUL.md/skills)
    ...
    home_token = set_nunmai_home_override(profile_home) if profile_home else None
