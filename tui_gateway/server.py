...
from nunmai_constants import (
    get_nunmai_home, get_nunmai_home_override, get_process_nunmai_home, profile_name_for_home,
    reset_nunmai_home_override, set_nunmai_home_override)
from nunmai_cli.env_loader import load_nunmai_dotenv
from utils import file_signature, is_truthy_value
from nunmai_state_ids import new_session_id
...
_nunmai_home = _NUNMAI_HOME_AT_IMPORT = get_nunmai_home()
load_nunmai_dotenv(nunmai_home=_nunmai_home, project_env=Path(__file__).parent.parent / ".env")
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("NUNMAI_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("NUNMAI_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("NUNMAI_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("NUNMAI_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "nunmai_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("nunmai_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Nunmai-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Nunmai agent → needs provider credentials. Tier-1 secrets
        ...
        # `nunmai -p X` would load itself. The launch profile is a profile too: once the process hosts
        ...
        env = _prepend_tool_paths(served_profile_child_env(
            target_home=profile_home or (_nunmai_home if is_multiplex_active() else None),
        ...
        # Prepend the Nunmai venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Nunmai-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("NUNMAI_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("NUNMAI_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("NUNMAI_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _launch_state_db_path() -> Path:
    """Launch profile's ``state.db`` at call time: the patched ``_nunmai_home`` when a test changed
    it, else the live process home — resolved through :func:`get_process_nunmai_home`, which honours
    ``NUNMAI_HOME`` but ignores the context-local override. The desktop multiplex cron ticker sets
    ...
    import time lets a harness that redirects ``NUNMAI_HOME`` after import be honoured (#112692)."""
    home = _nunmai_home if _nunmai_home != _NUNMAI_HOME_AT_IMPORT else get_process_nunmai_home()
...
def _profile_db(params: dict | None = None, *, writer: bool = False):
    ...
    nunmai_cli.web_routers.profiles._read_profile_db."""
    ...
    if (profile_home := _profile_home(profile)) is None:
        ...
        try:
            if writer:
                ...
                from nunmai_cli.web_server_sessions import _open_session_db_at_path
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``nunmai`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".nunmai", "nunmai"}:
        from nunmai_cli import profiles as profiles_mod
        # Check the profiles root directly: get_profile_dir rejects "nunmai" as a
...
def _db_unavailable_error(rid, *, code: int):
    from nunmai_state_user_copy import describe_storage_failure, storage_failure_details
...
# backend, so calls carry ``profile`` → open that profile's db and bind its NUNMAI_HOME (ContextVar
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s full runtime scope (NUNMAI_HOME + secrets + terminal policy) around a
...
def _launch_configured_cwd() -> str | None:
    ...
    # Read the launch file by path. ``_load_cfg`` follows the active NUNMAI_HOME
    # override, which may belong to a different profile-scoped RPC.
    return _profile_configured_cwd(Path(_nunmai_home))
...
# `nunmai serve` / dashboard processes speak JSON-RPC over WS only: their stdout is captured into
...
def _bind_build_profile_scopes(profile_home: "str | None") -> "_TurnScopes | None":
    """Bind a session profile's NUNMAI_HOME / secret / terminal scopes for an agent build. ``None`` is the
    ...
    if profile_home:  # secret/terminal helper failed: keep at least the home + terminal refusal scope
        scopes.home = set_nunmai_home_override(profile_home)
        with contextlib.suppress(Exception):
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's NUNMAI_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _nunmai_home) / "config.yaml"
...
def _load_cfg_raw() -> dict:
    ...
    from nunmai_cli.config_read_errors import FailedConfigRead
    try:
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess NUNMAI_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(NUNMAI_GATEWAY_SESSION="1", NUNMAI_EXEC_ASK="1", NUNMAI_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Nunmai Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Nunmai Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``nunmai --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("NUNMAI_MODEL", "") or os.environ.get("NUNMAI_INFERENCE_MODEL", "")).strip()
...
def _resolve_session_platform() -> str:
    """``NUNMAI_DESKTOP=1`` without ``NUNMAI_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``nunmai --tui``)."""
    desktop = is_truthy_value(os.environ.get("NUNMAI_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("NUNMAI_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the NUNMAI_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("NUNMAI_TUI_PROVIDER", "").strip():
    ...
    with contextlib.suppress(Exception):
        ...
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("NUNMAI_INFERENCE_PROVIDER", "").strip().lower() or "auto")
        # Same owner as NunmaiCLI/oneshot: ``custom:<name>:<model>`` selects that provider (#73943).
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    "out of Nunmai credits" after a profile switch). A canonical Bot Chat may instead restore an explicit
    ...
    from nunmai_cli.runtime_provider import is_foreign_provider_endpoint
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # persist). The full scope, not NUNMAI_HOME alone: the external memory provider's system_prompt_block()
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`nunmai_constants.resolve_reasoning_config` (per-model override >
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_NUNMAI_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``NUNMAI_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a NUNMAI_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] NUNMAI_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown NUNMAI_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in NUNMAI_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit NUNMAI_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("NUNMAI_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        fallback_notice = "[tui] no valid NUNMAI_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from nunmai_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid NUNMAI_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on NUNMAI_DEV_CREDITS so the payload stays clean otherwise.
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if provider == "custom" and "provider" not in mirror and agent is not None:
        ...
        with _profile_build_scope(sess.get("profile_home") or _nunmai_home):
    ...
    # Nunmai-internal step (#61634) as a wire level the route does not have.
    ...
    try:
        ...
        for entry in _load_fallback_model() or []:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from nunmai_cli.fallback_config import effective_runtime_provider, resolve_entry_api_key
                ...
                from nunmai_cli.auth import primary_failure_wording
...
def _rederive_per_model_route(model: str, runtime: dict) -> None:
    ...
    that pick the wire per model (OpenCode Zen/Go, Copilot, Nunmai) must re-derive both from the target model,
    ...
    from nunmai_cli.model_switch import model_derived_api_mode
    from nunmai_cli.models import normalize_opencode_base_url
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + NUNMAI_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `nunmai skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("nunmai_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("NUNMAI_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped NUNMAI_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str, *, model_history_only: bool = False) -> tuple[list, list, list]:
    ...
    from nunmai_state import SessionResumeTooLargeError
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("NUNMAI_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`nunmai setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`nunmai gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`nunmai sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`nunmai config edit` needs $EDITOR in a real terminal",
}
