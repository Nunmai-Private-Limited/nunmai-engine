...
activate with `nunmai --tui` or `NUNMAI_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `NUNMAI_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.nunmai`. The first non-launch home hosted flips
...
**A method that sets only `get_nunmai_home_override()` is half-bound**: config paths resolve to the
...
(`nunmai -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `nunmai_cli/commands.py` (`nunmai_cli/AGENTS.md`).
