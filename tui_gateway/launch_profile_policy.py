"""Launch-profile policy for a process that hosts several profile homes (``nunmai serve`` /
``nunmai dashboard`` pooling, ``?profile=``, hosted rooms; the multiplexed gateway's own worker).
...
def _servable_profile_homes() -> set:
    ...
    ``nunmai profile create`` leaves behind — counting it would flip a single-profile host
    fail-closed at its next boot.
    """
    from nunmai_constants import named_profile_has_servable_identity
    ...
    homes.add(Path(os.environ.get("NUNMAI_HOME") or Path.home() / ".nunmai").resolve())
    return homes
...
def activate_multi_profile_hosting_eagerly() -> bool:
    ...
    polluted snapshot. One ``nunmai serve`` / ``nunmai gateway run`` per host means the process
...
def launch_profile_runtime_scope(launch_home: "str | Path") -> Iterator[None]:
    """Bind the launch profile's own runtime scope for one body: NUNMAI_HOME override naming the
...
def launch_profile_scope_if_multiplexed():
    ...
    return launch_profile_runtime_scope(get_process_nunmai_home())
