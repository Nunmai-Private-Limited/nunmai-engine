...
# from shadowing Nunmai's own top-level modules.  ``nunmai_bootstrap`` lives at
...
# shared owner in nunmai_cli.mcp_startup — the wait/in-flight/join helpers
...
# nunmai_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `NUNMAI_TUI_SIDECAR_URL`, set by the dashboard's
...
# tested; override via ``NUNMAI_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("NUNMAI_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``NUNMAI_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``nunmai --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``nunmai_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from nunmai_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → nunmai_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's NUNMAI_HOME first), so a launch profile with no
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``nunmai --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``nunmai dashboard`` spawn theirs via
    ``nunmai_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``nunmai_cli.mcp_startup`` thread used by the
    ...
    try:
        from nunmai_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's NUNMAI_HOME override — the shared owner in
    ``nunmai_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    try:
        from nunmai_cli.model_switch import prewarm_picker_cache_async
