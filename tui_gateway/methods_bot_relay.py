...
def _relay_root() -> Path:
    ...
    every NUNMAI_HOME, including non-``profiles/`` subdirs of ``~/.nunmai``."""
    from tools.bot_mode_probe import _default_home, _nunmai_root
    return _nunmai_root(Path(_default_home()))
...
def _run_delivery(profile: str, tmp: str, env: dict | None = None, *,
    ...
    try:
        # The relay pins UTF-8 on every platform (#93590): its child is the bootstrapped nunmai_cli
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery,
    ...
    GATEWAY via the one-turn ``nunmai -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "nunmai" else profile
        if resolved not in known:
            return _err(rid, 4092, f"no profile '{profile}' on this gateway")
        # The sender stamped itself with its bare @handle; a relayed "@nunmai" is ANOTHER machine's
        ...
        if _is_authenticated_identity(identity):
            ...
            # nunmai_cli/dashboard_auth/routes.py), so refusing took cross-connection relay offline
        ...
        owner_home = live_home if live_home is not None else Path(_nunmai_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="nunmai-relay-dm-", suffix=".txt", text=True)
        try:
