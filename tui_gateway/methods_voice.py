...
# ── Voice state: NUNMAI_VOICE / NUNMAI_VOICE_TTS are runtime-only env flags (never config.yaml)
...
def _end_voice_chat(*, stop_loop: bool, stop_tts: bool) -> None:
    ...
    os.environ["NUNMAI_VOICE"] = os.environ["NUNMAI_VOICE_TTS"] = "0"
    if stop_loop:
...
# ── Wake word ("Hey Nunmai"): process-global detector (one mic). The first eligible transport
...
def _(rid, params: dict) -> dict:
    ...
    from nunmai_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
...
def _set_voice_tts(on: bool) -> None:
    ...
    os.environ["NUNMAI_VOICE_TTS"] = "1" if on else "0"
    if not on:
        _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        import nunmai_cli.voice  # noqa: F401  (a missing module must answer 5026, not die in a thread)
    except Exception as e:
