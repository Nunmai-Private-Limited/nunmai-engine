...
def _is_repo_junk(root: str) -> bool:
    ...
    NUNMAI_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
...
def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of NUNMAI_HOME may be an
    intentional prose/data workspace, so only NUNMAI_HOME itself is excluded here."""
    if not cwd:
        return True
    from nunmai_constants import get_nunmai_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Nunmai sessions still appear in the sidebar (#81723).
