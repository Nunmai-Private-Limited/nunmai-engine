...
def _session_home(session: dict) -> Path:
    ...
    return Path(session.get("profile_home") or _nunmai_home)
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # Ownership admission BEFORE message.start: a sibling backend sharing this NUNMAI_HOME may have written the
