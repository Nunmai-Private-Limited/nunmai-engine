...
class _TurnScopes:
    ...
    home: Any = None  # per-turn NUNMAI_HOME override for a resumed remote profile
    secret: Any = None
    terminal: Any = None
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if isinstance(run_kwargs, dict):
        run_kwargs.clear()
    try:  # while the profile NUNMAI_HOME override is still active (session's own config)
