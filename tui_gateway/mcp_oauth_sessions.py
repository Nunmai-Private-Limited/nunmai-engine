...
The underlying token machinery is the *same* one the CLI ``nunmai mcp login``
uses — ``nunmai_cli.mcp_config._probe_single_server`` under
...
def _worker(session_id: str, nunmai_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    NUNMAI_HOME override + secret-scope + force_interactive_oauth +
...
def start_flow(
    ...
    *,
    ...
    OAuth-capable). ``nunmai_home`` is the already-resolved profile home dir
    ...
    threading.Thread(
        target=_worker,
        args=(session_id, nunmai_home, server_name, dict(cfg), reconnect_live),
        daemon=True,
