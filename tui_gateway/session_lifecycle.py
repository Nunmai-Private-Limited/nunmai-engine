...
_SESSION_OWNERSHIP_UNAVAILABLE = "Nunmai could not safely reserve this session. Try again."
...
def _install_borrowed_lease(sid: str, session: dict, frame: dict) -> None:
    ...
    if not key or str(vouch.get("session_id") or "") != key:
        return
    from nunmai_cli.active_sessions import ActiveSessionLease
...
def _ensure_active_session_slot(sid: str, session: dict) -> str | None:
    ...
    if limit_message is None:
        ...
        return None
    from nunmai_cli.active_sessions import SESSION_NOT_OWNED
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    try:
        from nunmai_cli.active_sessions import active_session_liveness_guard, release_active_session_liveness_guard
    except Exception as exc:
...
def _pop_session_by_id(sid: str) -> dict | None:
    ...
    with _sessions_lock:
        ...
        if session is not None:
            ...
            home = str(Path(session.get("profile_home") or get_nunmai_home()).resolve())
    ...
    for sid, session in clientless:
        ...
        with _session_resume_lock, _sessions_lock:
            ...
            if current.get("close_on_disconnect"):
                ...
                # `nunmai --tui` keeps real _stdio. UNLESS another window (pop-out viewer) still shows the session:
