...
handlers delegate to the same `nunmai_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Nunmai profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (nunmai-engine#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (nunmai-engine#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's nunmai-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``nunmai setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (nunmai_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local NUNMAI_HOME override — the same
    ...
    if model and provider:
        try:
            from nunmai_cli.web_routers.profiles import _write_profile_model
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the NUNMAI_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `nunmai tools`
            ...
            # composites (nunmai-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from nunmai_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `nunmai tools` / Settings is where
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from nunmai_cli.profiles import write_profile_meta
