...
class ConfigProviderRef(OpenModel):
    """``nunmai_cli/models.py::list_available_providers`` row."""
...
method("diagnostics.share_nous", params=DiagnosticsShareNousParams, result=DiagnosticsShareNousResult,
       doc="Upload a force-redacted debug bundle to Nunmai-internal diagnostics storage.")
...
class ModelPricing(Result):
    """``nunmai_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
    ``"free"``); the sale fields are Nunmai Portal-only."""
...
class ModelCapabilities(Result):
    """``nunmai_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``nunmai_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class GoalContractSnapshot(Result):
    """``nunmai_cli/goals.py::GoalContract.to_dict``."""
