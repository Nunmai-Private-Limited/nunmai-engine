...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.nunmai/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``nunmai_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``nunmai_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
...
class PluginSettingField(Result):
    ...
    (``nunmai_cli.plugins_settings.plugin_settings_fields``). ``secret`` fields carry no value: ``env``
...
class PluginActivation(Result):
    """What a plugin loaded mid-run does NOW vs later (``nunmai_cli.plugins_activation``). ``activated_now``
...
class PluginsManageResult(Result):
    ...
    (the canonical key written)/``plugin``; ``install`` → ``nunmai_cli.plugins_cmd.dashboard_install_plugin``'s
