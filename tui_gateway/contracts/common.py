...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: nunmai_state rows, provider inventories)."""
...
class SessionLiveInfo(OpenModel):
    ...
    # equal when verbatim). Lets clients label a clamped Nunmai step ("ultra sends max on this route").
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``nunmai_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``nunmai_state_messages.set_message_reaction``); ``seen`` is
