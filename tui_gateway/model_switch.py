...
def _profile_runtime_scope_tokens(profile_home, *, hydrate_secrets: bool = True) -> "_TurnScopes":
    """Bind NUNMAI_HOME + secret + terminal scope for ``profile_home`` (None = launch profile) and
    ...
    try:
        if profile_home:
            ...
            overlay = None
            scopes.home = set_nunmai_home_override(str(home))
        else:
            # The launch home IS get_nunmai_home() (``_profile_home`` answers None for "already the
...
def _release_profile_runtime_scope_tokens(scopes: "_TurnScopes | None") -> None:
    ...
    not leave the previous profile's secrets / NUNMAI_HOME installed for the next body in this
    ...
    for token, reset in ((scopes.terminal, reset_terminal_scope), (scopes.secret, reset_secret_scope),
                         (scopes.home, reset_nunmai_home_override)):
        if token is None:
            continue
        try:
            reset(token)
...
def _apply_model_switch(
    ...
    # Deliberately NOT written to process-global env (NUNMAI_MODEL & co.): the desktop hosts
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        ...
        # how `nunmai --tui -m` once leaked into config.yaml).
        _apply_model_switch(
