...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'nunmai' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nunmai_cli.commands import SlashCommandCompleter
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `nunmai model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.nunmai/.env via the unified credential lifecycle
