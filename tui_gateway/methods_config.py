...
from nunmai_constants import display_nunmai_home as _display_nunmai_home
...
def _cfg_get_provider(params):
    from nunmai_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_nunmai_home), "display": _display_nunmai_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's NUNMAI_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``NUNMAI_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    added after boot — the Models page, a picker key, ``nunmai setup`` from a shell — flips it
    ...
    try:
        ...
        from nunmai_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Nunmai provider is configured.", source)
            ...
            # on profile state: a paid Nunmai key beside a free-tier identity must not read as free.
...
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Nunmai-internal diagnostics storage — same collection +
    force-redaction pipeline as ``nunmai debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from nunmai_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
