"""Windows UTF-8 bootstrap for Nunmai entry points.
...
should be imported at the very top of every Nunmai entry point
(``nunmai``, ``nunmai-engine``, ``nunmai-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``nunmai_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``nunmai_bootstrap``, never ``nunmai_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Nunmai ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Nunmai source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``NUNMAI_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
...
# Apply on import — entry points just need ``import nunmai_bootstrap``
# (or ``from nunmai_bootstrap import apply_windows_utf8_bootstrap``) at
