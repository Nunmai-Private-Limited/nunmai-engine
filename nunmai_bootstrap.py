"""Process bootstrap for Nunmai entry points: Windows UTF-8 stdio, import-path
...
point (``nunmai``, ``nunmai-engine``, ``nunmai-acp``, ``gateway.run``, ``batch_runner``,
...
here may pull in a Nunmai package that a project-local directory could shadow.
"""
...
def _patch_urllib3_create_connection(module) -> None:
    ...
    if getattr(module.create_connection, "_nunmai_happy_eyeballs", False):
        return
    ...
    _urllib3_racer._nunmai_happy_eyeballs = True  # type: ignore[attr-defined]
...
class _Urllib3ConnectionPatcher(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    Importing urllib3 eagerly costs ~50 ms on every CLI start, and ``nunmai`` / the TUI
...
def install_happy_eyeballs_socket_connect() -> None:
    ...
    """
    if getattr(socket.create_connection, "_nunmai_happy_eyeballs", False):
        return
    ...
    _socket_racer._nunmai_happy_eyeballs = True  # type: ignore[attr-defined]
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``nunmai_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``nunmai_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def install_never_free_environ() -> None:
    ...
    whole process. Nunmai writes new names at runtime from many places (``session.create``
    turns on gateway prompts, the agent build sets ``NUNMAI_SESSION_ID``) while background
    ...
    """
    if getattr(os.putenv, "_nunmai_never_free_environ", False) or not _glibc_frees_environ():
        return
    import _thread
    import ctypes
    ...
    _putenv._nunmai_never_free_environ = True  # type: ignore[attr-defined]
    _unsetenv._nunmai_never_free_environ = True  # type: ignore[attr-defined]
    os.putenv, os.unsetenv = _putenv, _unsetenv


def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Nunmai modules.

    Nunmai ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Nunmai root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("NUNMAI_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``NUNMAI_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("NUNMAI_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
def export_scratch_tmp_env() -> None:
    """Point ``TMPDIR``/``TMP``/``TEMP`` at ``NUNMAI_HOME/cache/scratch`` unless the user set them.

    System temp is tmpfs on most Linux hosts and containers; Nunmai' browser profiles, PTY
    probes and every ``tempfile`` default a child script makes would eat RAM there. Runs at
    import so every entry point and every child they spawn inherits it; ``nunmai_cli.main``
    re-runs it after ``--profile`` re-homes the process. Never raises.
    """
    try:
        from nunmai_constants import export_scratch_tmp_env as _export
        _export()
    except Exception:
        pass  # a missing/unwritable home just leaves the system temp dir in place


# Apply on import — entry points only need ``import nunmai_bootstrap`` first.
