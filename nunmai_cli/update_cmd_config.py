"""Post-``nunmai update`` config-schema migration for the active profile and every sibling.
Names are re-imported by ``update_cmd`` (``nunmai_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _reload_config_modules() -> None:
    ...
    for mod_name in (
        "nunmai_cli.config_defaults", "nunmai_cli.config", "nunmai_cli.config_migrations",
...
def _run_config_check_fresh() -> tuple:
    ...
    from nunmai_cli.update_cmd import _reload_config_modules
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local NUNMAI_HOME override (never ``os.environ``)
    ...
    profile, but ``nunmai update`` historically migrated only the active profile's config — siblings drifted
    versions until their gateway hit a config the new code couldn't read.
    """
    from nunmai_cli.update_cmd import _run_config_check_fresh, _run_migrate_config_fresh
    migrated: list[tuple[str, int, int]] = []
    with _best_effort('Sibling profile enumeration failed: %s'):
        from nunmai_constants import (
            get_process_nunmai_home, reset_nunmai_home_override, set_nunmai_home_override)
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from nunmai_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from nunmai_cli.update_cmd import _gateway_prompt
    if assume_yes:
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    _reload_config_modules()
    from nunmai_cli.config import get_missing_env_vars, get_missing_config_fields
    ...
    # migration per sibling home via the context-local NUNMAI_HOME override (never os.environ).
