...
3. Running inside a container with no persistent volume mount over the
   NUNMAI_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Nunmai as an unprivileged user (or in a "
...
def run_security_audit(
    *, nunmai_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(nunmai_home)
        if r:
            findings.append(r)
    except Exception:
        pass
