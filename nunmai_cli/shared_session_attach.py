...
from nunmai_cli.active_sessions import active_session_registry_snapshot, session_owner_details
...
def discover_attach_url(session_id: str, *, registry_home: str | Path | None = None) -> str | None:
    ...
    """
    home = Path(registry_home if registry_home is not None else get_nunmai_home()).resolve()
    ...
    if not isinstance(endpoint, str) or not endpoint:
        raise ValueError("This chat is open in another Nunmai window/terminal, and attaching "
                         "this terminal to it is not available in this build. Close the chat "
                         "there and run nunmai --resume " + session_id + " here to take it over.\n"
    ...
    try:
        ...
        raise ValueError("This chat is open in another Nunmai window/terminal, and attaching "
                         ...
                         "close it there and run nunmai --resume " + session_id + " here.\n"
...
def configure_tui_attachment(env: dict[str, str], session_id: str | None, *,
    ...
    if not session_id or env.get("NUNMAI_TUI_GATEWAY_URL", "").strip():
        return
    url = discover_attach_url(session_id, registry_home=registry_home)
    if url is not None:
        env["NUNMAI_TUI_GATEWAY_URL"] = url
