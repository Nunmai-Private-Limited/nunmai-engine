...
Pure logic: walks a Nunmai config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nunmai doctor` and a future `nunmai migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nunmai config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nunmai config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `nunmai migrate xai` reports nothing to migrate
