...
- ``nunmai_cli.providers``        -- canonical provider identity + overlays
- ``nunmai_cli.model_normalize``  -- per-provider name formatting
...
from nunmai_cli.model_normalize import (
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Nunmai wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Nunmai.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Nunmai versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``nunmai model`` wizard — not a
    ...
    while ``nunmai model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Nunmai itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Nunmai itself discovered (``models_discovered: true``
...
_NUNMAI_MODEL_WARNING = (
    "Nunmai Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Nunmai Engine. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nunmai Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"nunmai" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``nunmai-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nunmai" in their tag but are fully tool-capable.
#
...
def is_nous_nunmai_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_NUNMAI_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nunmai config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nunmai_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nunmai model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nunmai doctor' — config issues detected:"
        ...
        from nunmai_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nunmai_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from nunmai_cli.config import load_config
            from nunmai_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from nunmai_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from nunmai_cli.models import should_use_ollama_native_catalog
    ...
    # nunmai_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from nunmai_cli.models import opencode_provider_family as _oc_family_fn
    ...
    nunmai_warn = _check_nunmai_model_warning(new_model)
    if nunmai_warn:
        warnings.append(nunmai_warn)
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Nunmai provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from nunmai_cli.models import cached_provider_model_ids
...
def _collect_authed_provider_slugs(
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from nunmai_cli.providers import NUNMAI_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from nunmai_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Nunmai-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for nunmai_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(nunmai_id)
        if (
            _alias_target
            and _alias_target != nunmai_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = nunmai_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(nunmai_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != nunmai_id:
            continue
        if nunmai_id.lower() in seen:
            continue
        if nunmai_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from nunmai_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(nunmai_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(nunmai_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        nunmai_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(nunmai_id)
            seen.add(nunmai_id.lower())

    # --- Section 2: Nunmai-only providers (NUNMAI_OVERLAYS) ---
    _mdev_to_nunmai = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in NUNMAI_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        nunmai_slug = _mdev_to_nunmai.get(pid, pid)
        if nunmai_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or nunmai_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or nunmai_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(nunmai_slug)
            seen.add(pid.lower())
            seen.add(nunmai_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from nunmai_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against nunmai_id / mdev_id (section 1), pid / nunmai_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nunmai_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nunmai provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://nunmai.in/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nunmai release. Falls back to the in-repo
    ...
    if "lmstudio" not in curated and (
        ...
        from nunmai_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Nunmai-mapped providers ---
    from nunmai_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nunmai_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for nunmai_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip nunmai_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose nunmai_id matches the canonical profile name so
        ...
        # same nunmai_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if nunmai_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (NUNMAI_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Nunmai may not route yet.
        ...
        # /model picker sees the SAME list `nunmai model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(nunmai_id)
        if not model_ids:
            model_ids = curated.get(nunmai_id, [])
            if nunmai_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nunmai_id, model_ids)
    ...
    # --- 2. Check Nunmai-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nunmai_cli.providers import NUNMAI_OVERLAYS
    from nunmai_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Nunmai provider ID.
    # NUNMAI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Nunmai IDs ("copilot").
    _mdev_to_nunmai = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in NUNMAI_OVERLAYS.items():
        ...
        # Resolve Nunmai slug — e.g. "github-copilot" → "copilot"
        nunmai_slug = _mdev_to_nunmai.get(pid, pid)
        if nunmai_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or nunmai_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(nunmai_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nunmai_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds and nunmai_slug == "anthropic":
            try:
                ...
                if (nunmai_creds and nunmai_creds.get("accessToken")) or \
        ...
        if nunmai_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(nunmai_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(nunmai_slug)
                model_ids = _ids if _ids else (curated.get(nunmai_slug, []) or curated.get(pid, []))
            except Exception:
            ...
            try:
                ...
                from nunmai_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(nunmai_slug, []) or curated.get(pid, [])
                if nunmai_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(nunmai_slug, model_ids)
        total = len(model_ids)
        if nunmai_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": nunmai_slug,
            "name": get_label(nunmai_slug),
            "is_current": nunmai_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nunmai",
        ...
        seen_slugs.add(nunmai_slug.lower())
        _record_builtin_endpoint(nunmai_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NUNMAI_OVERLAYS (keeps /model in sync
    # with `nunmai model`).
    try:
        from nunmai_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Nunmai writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Nunmai's own writer uses (mirrors section-4 grouping).
            ...
            # ``nunmai model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``nunmai model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Nunmai's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nunmai_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``nunmai model``. Pin a dict catalog with
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nunmai_cli.models.fetch_openrouter_models`, which filters the
