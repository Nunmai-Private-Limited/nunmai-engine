...
# Re-exported: callers/tests patch nunmai_cli.model_switch.<name>.
from nunmai_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Nunmai wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Nunmai itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
# Match only the real Hermes 3 / 4 chat families; a bare substring check
# false-positived on tool-capable local Modelfiles like ``nunmai-brain:qwen3-14b-ctx16k``.
#   match:    NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
#   no match: nunmai-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_NUNMAI_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nous_nunmai_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Hermes 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _NOUS_NUNMAI_NON_AGENTIC_RE.search(model_name))


def _check_nunmai_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Hermes 3/4 chat model, else ""."""
    return _NUNMAI_MODEL_WARNING if is_nous_nunmai_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from nunmai_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``NunmaiCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    from nunmai_cli.models import parse_model_input
    ...
    if current_provider:
        try:
            from nunmai_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
            if is_routing_aggregator(_norm_prov(current_provider)):
                from nunmai_cli.models import _find_openrouter_slug
    ...
    try:
        from nunmai_cli.models import normalize_provider
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    is the raw level word (validated by :func:`nunmai_constants.parse_reasoning_effort` at apply
...
def _configured_provider_identity(slug: str, cfg: dict) -> tuple[str, str, str, str]:
    ...
    from nunmai_cli.config_providers import _canonical_api_mode, _normalize_custom_provider_entry
...
def _ollama_configured_base() -> tuple[dict, str]:
    from nunmai_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'nunmai model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'nunmai doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    from nunmai_cli.models_local import _same_ollama_native_root
...
def _convert_vendor_colon_slug(st: _Switch) -> None:
    ...
    ``left`` names a provider Nunmai knows, so ``/model alibaba:qwen3.6-plus`` routes like
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from nunmai_cli.models import detect_provider_for_model
    ...
    if current_provider == "nous":
        ...
        if route_is_welcome_host(st.current_base_url) and st.new_model != GUEST_MODEL:
            return st.fail(
                f"{st.new_model} needs a Nunmai account or an API key. "
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from nunmai_cli.providers import resolve_user_provider
    ...
    if user_pdef is not None and user_pdef.base_url:
        ...
        from nunmai_cli.runtime_provider import _resolves_to_custom
        ...
        try:
            ...
            return st.fail_on_target(
                ...
                f"`nunmai auth add {st.target_provider}`, or pick a connected provider in /model.\n"
                f"  Details: {e}")
    return None
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from nunmai_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
    keep_current_ollama_endpoint = False
    ollama_headers: dict[str, str] = {}
    if st.current_provider == "custom" and st.current_base_url:
        try:
            from nunmai_cli.models_local import should_use_ollama_native_catalog
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from nunmai_cli.models_local import _get_ollama_request_headers
    from nunmai_cli.models_validate import validate_requested_model
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from nunmai_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from nunmai_cli.models import opencode_model_api_mode
...
def model_derived_api_mode(provider: str, model: str, api_key: str = "") -> Optional[str]:
    ...
    endpoint (OpenCode Zen/Go and custom providers extending a family slug, Copilot, Nunmai); None when the
    ...
    from nunmai_cli.models import opencode_provider_family
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from nunmai_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_nunmai_model_warning(st.new_model)) if w]
    ...
    try:
        from nunmai_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
def persist_model_selection(result: ModelSwitchResult, config_path: Any = None) -> None:
    ...
    ``NUNMAI_HOME/config.yaml`` — the context override or ``NUNMAI_HOME`` at call time).
    ...
    from nunmai_cli.config import get_config_path, read_user_config_raw
...
def _scoped_key_env(name: str) -> str:
    ...
    ``$NUNMAI_HOME/.env`` authenticates the ``/model`` verification probe (#109315) and a rotated
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('nunmai_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('nunmai_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
