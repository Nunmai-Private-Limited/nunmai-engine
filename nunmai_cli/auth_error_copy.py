...
DEVICE_FLOW_ERROR_COPY = {
    ...
    "access_denied": (
        "Sign-in was declined in the browser. Run `{retry}` to try again, or `nunmai model` to pick a "
        "different provider."),
    "invalid_grant": (
        "The sign-in code was not accepted by the server. Run `{retry}` to get a new code."),
    "invalid_client": (
        "The server did not recognize this copy of Nunmai. Run `nunmai update`, then `{retry}` again."),
}
...
def device_flow_error(code: str, description: str, *, retry_command: str = "nunmai portal") -> SignInCopyError:
...
def sign_in_failure_lines(
    exc: BaseException, *, service_host: str = "portal.nousresearch.com", retry_command: str = "nunmai portal",
) -> list:
    ...
    rules: Sequence[_Rule] = (
        ...
        (is_network_error,
         "Could not sign in: Nunmai could not reach {host}. Check your internet connection or proxy, "
         "then run `{retry}` again."),
    )
    lead = _classify(
        exc, rules,
        "Could not sign in. Run `{retry}` to try again, or `nunmai model` to pick a different provider.")
...
def provider_setup_failure_lines(exc: BaseException, *, retry_command: str = "nunmai model") -> list:
