"""Provider-specific runtime builders for :mod:`nunmai_cli.runtime_provider`: Azure Foundry, the
...
``nunmai_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _resolve_openrouter_runtime(
    *, requested_provider: str, explicit_api_key: Optional[str] = None, explicit_base_url: Optional[str] = None
    ...
    if is_openrouter_context:
        ...
        from nunmai_cli.runtime_provider_custom import _model_cfg_key_env_for
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from nunmai_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
