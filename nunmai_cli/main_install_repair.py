...
def _stdout_to_stderr():
    ...
    a protocol stream (``nunmai acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_core_update_marker_locked() -> None:
    ...
    if attempts >= max_attempts:
        print(
            f"✗ Finishing an interrupted `nunmai update` failed {attempts} times; automatic "
            "retries are paused so Nunmai stops reinstalling on every launch.")
    ...
    # Windows: a ``nunmai.exe`` launch has the launcher as an ancestor; the quarantined full
...
def _manual_core_recovery_lines(root: Path, marker: Path, *, windows: bool) -> tuple[str, ...]:
    ...
    On Windows another Nunmai process (Desktop backend, gateway, a second terminal) mapping a
    venv ``.pyd`` is what keeps the automatic install failing, so they must be closed first.
    """
    if windows:
        return (
            "  Close every Nunmai window (Desktop app, gateways, other terminals), then from a "
            "new terminal run:",
            f'    cd /d "{root}"',
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\nunmai.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``nunmai.exe`` never matches.
...
def _windows_shim_holder_pid() -> int:
    """Pid a detached child must outwait before touching the venv: the ``nunmai.exe`` launcher
...
def _windows_running_nunmai_launcher_locked() -> bool:
    """True when a venv ``nunmai*.exe`` shim is this process or an ancestor (best-effort)."""
...
def _reexec_dependency_sync_off_windows_shim(gateway_resume: dict | None = None) -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``nunmai update`` so the sync and its tail happen
    ...
    ``venv\\Scripts\\nunmai.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    from nunmai_cli.update_handoff import detached_shim_child_env
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `nunmai update` starts, so an update running
...
def _quarantine_running_nunmai_exe(
    ...
    """Pre-empt the Windows file lock on the running ``nunmai.exe``.
    ...
    transient; Nunmai Desktop backend child: until closed) — retry with backoff, then warn
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Nunmai versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _nunmai_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_nunmai_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``nunmai``
...
class ShimQuarantineError(RuntimeError):
    """A live ``nunmai*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    Every editable install rewrites the entry-point shims; on Windows the live ``nunmai.exe``
    can be neither deleted nor overwritten, so without quarantine ``nunmai`` drops off PATH.
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `nunmai` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    (1) orphan rescue — ``nunmai.exe`` missing while ``nunmai.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``nunmai.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``nunmai.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``nunmai*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves nunmai.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``nunmai.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``nunmai`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``nunmai-engine.exe`` and ``nunmai-acp.exe`` present but ``nunmai.exe`` missing, so
    ``nunmai`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    _report_still_missing(
        _missing(), "Workaround: python -m nunmai_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``nunmai update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `nunmai update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        return
    _report_still_missing(
        _missing_deps(), "Run `nunmai update --force` after closing other nunmai processes.",
