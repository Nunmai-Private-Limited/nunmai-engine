"""``nunmai sessions`` command — extracted from ``nunmai_cli/main.py``.
...
Helpers that stay in ``nunmai_cli.main`` (``get_nunmai_home``,
...
``nunmai_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `nunmai sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`nunmai sessions prune --never-active` — drop leaked/dead keyed rows.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    nunmai sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        from nunmai_state import workspace_key as _ws_key
        ...
        from nunmai_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from nunmai_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_nunmai_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from nunmai_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_nunmai_home() / "session-exports"
        ...
        # `nunmai sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `nunmai sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `nunmai sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `nunmai sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: nunmai sessions pin <session_id>"
            )
            return
        ...
        # Launch nunmai --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`nunmai sessions optimize` later to reclaim freed space.)")
