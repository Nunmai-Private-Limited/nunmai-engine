...
``_DB_HANDLERS`` (everything else, sharing one ``SessionDB``). ``get_nunmai_home`` is resolved through
``nunmai_cli.main`` at call time so monkeypatches keep working. Picker: :mod:`nunmai_cli.sessions_cmd_browse`.
"""
...
from nunmai_cli.cli_output import print_truncated
from nunmai_cli.sessions_cmd_browse import _relative_time, _session_browse_picker


def get_nunmai_home():
    from nunmai_cli import main
    return main.get_nunmai_home()


def _sessions_dir() -> Path:
    return get_nunmai_home() / "sessions"
...
def _not_found(session_id) -> int:
    print(f"No session '{session_id}'. Run: nunmai sessions list to find the id.")
    return 1


def _print_dry_run_preview(candidates, filters) -> None:
    from nunmai_cli.session_filters import describe_filters
...
def _export_dir(output) -> Path:
    """``--output`` dir for multi-file exports; ``~/.nunmai/session-exports`` when empty or ``-``."""
    return Path(output).expanduser() if output and output != "-" else get_nunmai_home() / "session-exports"
...
def _cmd_repair(args):
    from nunmai_state import DEFAULT_DB_PATH as db_path, SessionDB
    ...
    print(
        ...
        f"    nunmai sessions recover --source {source_hint} \\\n"
        "        --inspect-only\n"
...
def _cmd_recover(args):
    ...
    import sqlite3
    from nunmai_cli.session_recovery import (
...
def _cmd_import(args):
    from nunmai_cli.foreign_sessions import run_sessions_import
...
def _cmd_list(db, args):
    from nunmai_state_sessions import workspace_key as _ws_key
...
def _cmd_export(db, args):
    from nunmai_cli.session_filters import build_prune_filters
    filters = None
    ...
    from nunmai_cli.session_export import SAVE_TRANSCRIPT_FORMATS
...
def _render_only(args, sessions):
    ...
    from nunmai_cli.session_export import export_record_count, render_sessions_export
...
def _render_html(args, sessions):
    ...
    from nunmai_cli.session_export_html import generate_html_export, generate_multi_session_html_export
...
def _export_flat(kind, args, collect):
    ...
    if sessions is not None:
        from nunmai_cli.session_export import default_save_filename
        name = (default_save_filename(sessions[0].get("id", ""), args.format) if len(sessions) == 1
                else f"nunmai_sessions.{args.format}")
...
def _export_markdown(db, args, filters, redact):
    ...
    from nunmai_cli.session_export_md import append_manifest_entry, write_session_markdown
    if args.output == "-":
...
def _export_markdown_single(db, args, export_one, output_dir, lineage_is_logical):
    ...
    from nunmai_cli.session_export_md import verify_export_file
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from nunmai_cli.session_filters import format_epoch, parse_duration_seconds
...
def _note_pinned_skipped(db, filters, action):
    ...
    if action == "prune":
        verb = "deleted"
        optin = "Pass --include-pinned to delete them anyway, or unpin first with `nunmai sessions unpin <id>`."
    else:
        verb, optin = "archived", "Unpin first with `nunmai sessions unpin <id>` to include them."
...
def _cmd_prune_or_archive(db, args, action):
    ...
    from nunmai_cli.session_filters import build_prune_filters, describe_filters, format_epoch
    ...
    if skipped_open:
        print(f"Note: {skipped_open} open session{'' if skipped_open == 1 else 's'} also match these filters but "
              "will be skipped because prune only deletes ended sessions. Use `nunmai sessions delete <id>` "
...
def _cmd_pinned(db, args):
    ...
    if getattr(args, "json", False):
        ...
        return
    if not pinned_rows:
        print("No pinned sessions. Pin one with: nunmai sessions pin <session_id>")
        return
...
def _cmd_optimize_storage(db, args):
    ...
    if result.get("vacuumed") is False:
        print("  (VACUUM was skipped or failed — run `nunmai sessions optimize` later to reclaim freed space.)")
...
def _cmd_repair_profiles(args):
    from nunmai_cli.sessions_cmd_repair_profiles import cmd_repair_profiles
    return cmd_repair_profiles(args)


def _cmd_set_journal_mode(args):
    from nunmai_cli.sessions_cmd_journal_mode import cmd_set_journal_mode
...
def _print_empty_store(action: str, args) -> None:
    ...
    if action == "stats":
        ...
        print("[]" if getattr(args, "json", False) else "No pinned sessions. Pin one with: nunmai sessions pin <session_id>")
    else:
        print("No sessions found.")
...
def cmd_sessions(args, sessions_parser=None):
    ...
    try:
        ...
        print("Could not open your session history database. "
              "Run: nunmai sessions repair to fix it (a backup is made first).")
        print(f"Details: {e}")
        return 1
    try:
        ...
        try:
            ...
            from nunmai_state_repair import _schema_not_built
            ...
            print(f"Error: session database needs migration — run any writing nunmai command first ({e})")
            return 1
    finally:
        db.close()
