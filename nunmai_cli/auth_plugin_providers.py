"""Model-provider plugins as first-class citizens of ``nunmai auth`` and the credential pool.

A ``plugins/model-providers/<name>/`` profile (``providers.base.ProviderProfile``) is mirrored into
``nunmai_cli.auth.PROVIDER_REGISTRY`` with the ``auth_type`` it declares, so ``resolve_provider()``
...
- ``auth_handler(action, args) -> bool`` for ``nunmai auth add|status|logout|refresh <name>``
...
Every function here late-imports ``nunmai_cli.auth`` names: this module is imported by the auth facade
...
def register_plugin_provider(pp: Any) -> None:
    ...
    """
    from nunmai_cli.auth import PROVIDER_REGISTRY, ProviderConfig, _api_key_provider
...
def _user_owns_alias(pp: Any, alias: str) -> bool:
    """True when *alias* resolves to the ``$NUNMAI_HOME`` plugin *pp* in the ``providers`` layer."""
    try:
...
def mirror_aliases(pconfig: Any, pp: Any) -> None:
    ...
    A bundled plugin never steals an alias another row already holds; a ``$NUNMAI_HOME`` plugin
...
def override_registry_row(pconfig: Any, pp: Any) -> None:
    """A ``$NUNMAI_HOME`` plugin re-registering a name that already has a row wins for the fields
...
def sync_plugin_provider_registry() -> int:
    ...
    whose own imports pull ``nunmai_cli.auth`` in mid-``_discover_providers()`` sees only what was
    registered so far, and every later plugin would otherwise fail with "Unknown provider" (#102123).
    """
    from nunmai_cli.auth import BUILTIN_PROVIDER_IDS, PROVIDER_REGISTRY
...
def dispatch_plugin_auth(action: str, args: Any, provider: str) -> bool:
    """Offer ``nunmai auth <action> <provider>`` to the provider's ``auth_handler``.
...
def plugin_missing_auth_handler_error(provider: str, action: str) -> Optional[SystemExit]:
    ...
    return SystemExit(
        ...
        f"auth_handler, so `nunmai auth {action} {provider}` cannot be handled. Add "
...
def get_plugin_oauth_auth_status(provider_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "hint": "" if live else f"Run `nunmai auth add {provider_id}` to sign in."}
