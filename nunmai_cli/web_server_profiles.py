...
from nunmai_cli.config import DEFAULT_CONFIG, get_process_nunmai_home
from nunmai_cli.web_models import MCPServerCreate
from nunmai_cli.web_server_gateway import _ACTION_LOG_FILES
from nunmai_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _nunmai_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_nunmai_home()`` at call
    time) to ``path`` for the block via the context-local NUNMAI_HOME override."""
...
def serving_profile_name() -> str:
    ...
    ELSEWHERE, so a custom NUNMAI_HOME outside ``profiles/`` (``get_active_profile_name()``
    ...
    try:
        ...
        if not name or name == "custom":
            return ""
        return name if profiles_mod.profile_matches_home(name, get_process_nunmai_home()) else ""
    except Exception:
        return ""
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (NUNMAI_HOME-scoped).
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_nunmai_home()`` at call
    ...
    ``get_nunmai_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late NUNMAI_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_nunmai_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.NUNMAI_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.NUNMAI_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.NUNMAI_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe profile scope: the task-local NUNMAI_HOME contextvar PLUS the profile's secret
    ...
    process_home = get_process_nunmai_home()
    if _is_current_profile(profile):
    ...
    with (_nunmai_home_scope(profile_dir) if profile_dir is not None else nullcontext()):
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``nunmai`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_nunmai_action``
