...
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local NUNMAI_HOME
    ...
    with _nunmai_home_scope(profile_dir):
        yield None if profile_dir.resolve() == get_process_nunmai_home().resolve() else profile_dir
