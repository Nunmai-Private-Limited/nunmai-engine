...
on Japanese locales, etc.).  Nunmai's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nunmai_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nunmai is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NUNMAI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NUNMAI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nunmai-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nunmai
    ...
    # install without Nunmai-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nunmai's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nunmai's
    ``nunmai config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nunmai (User env var in Windows Settings, or
    export in a PowerShell profile) and Nunmai picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nunmai-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nunmai\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nunmai`` from right after install) retain their old PATH.

    Any subprocess Nunmai spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nunmai-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nunmai", "git", "cmd"),
        os.path.join(local_appdata, "nunmai", "git", "bin"),
        os.path.join(local_appdata, "nunmai", "git", "usr", "bin"),
        # Nunmai venv Scripts directory — host of the nunmai.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nunmai, but harmless to include.
        os.path.join(local_appdata, "nunmai", "nunmai-engine", "venv", "Scripts"),
