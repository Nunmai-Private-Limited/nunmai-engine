"""API connectivity probes for ``nunmai doctor`` (split out of ``doctor.py``).
...
def _anthropic_messages_probe(base: str, key: str):
    ...
    headers = {"anthropic-version": "2023-06-01", "User-Agent": _NUNMAI_USER_AGENT, **auth}
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _NUNMAI_USER_AGENT}
...
def _probe_github_token() -> ProbeResult:
    ...
    """
    name = "GitHub token"
    from nunmai_cli.config import get_env_value, load_env
    ...
    try:
        ...
        r = httpx.get(GITHUB_API_PROBE_URL, timeout=10, headers={
            "Authorization": f"Bearer {get_env_value(var)}", "User-Agent": _NUNMAI_USER_AGENT,
