"""Bounded product contract for the first Nunmai shared-metrics slice."""
...
SCHEMA_KEY = "nunmai.metrics.schema_version"
SCHEMA_VERSION = "nunmai.metrics.event.v2"
MODEL_CALL_SCOPE = "nunmai.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "nunmai.task_run"
TOOL_CALL_SCOPE = "nunmai.tool_call"
CLIENT_ACTIVE_MARK = "nunmai.client.active"
TOOL_APPROVAL_MARK = "nunmai.tool_approval"
SKILL_LIFECYCLE_MARK = "nunmai.skill.lifecycle"
SKILL_LOAD_MARK = "nunmai.skill.load"
SUBSCRIBER_NAME = "nunmai.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "nunmai.client.active"
LEGACY_MODEL_CALL_METRIC = "nunmai.model_call.count"
MODEL_ROUTE_METRIC = "nunmai.model_route.count"
TASK_STARTED_METRIC = "nunmai.task_run.started"
TASK_FINISHED_METRIC = "nunmai.task_run.finished"
TOOL_CALL_METRIC = "nunmai.tool_call.count"
TOOL_APPROVAL_METRIC = "nunmai.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "nunmai.skill.lifecycle.count"
SKILL_LOAD_METRIC = "nunmai.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "nunmai_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Nunmai installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    nunmai_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(nunmai_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "nunmai_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("nunmai_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Nunmai logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "nunmai.call_role"
    )
    call_role = (metadata or {}).get("nunmai.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from nunmai_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Nunmai terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "nunmai-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Nunmai registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Nunmai tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Nunmai."""
