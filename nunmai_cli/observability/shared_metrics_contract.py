...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(nunmai_version or "").strip()
    ...
    return {
        ...
        "nunmai_version": normalized_version,
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("nunmai.call_role")
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Nunmai terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "nunmai-yuanbao", "slack", "sms")
    ):
