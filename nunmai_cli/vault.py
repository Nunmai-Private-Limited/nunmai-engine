"""``nunmai vault`` — manage the local encrypted autofill vault.

Subcommands:
- ``nunmai vault add``   interactive wizard; the password is read via
  getpass (never echoed, never accepted as argv). The login identifier is
  visible metadata and prompted normally.
- ``nunmai vault list``  metadata — labels, kinds, identifiers, origins,
  handles. Passwords are never shown.
- ``nunmai vault rm``    remove an item by handle/id.
...
def _cmd_list(args) -> None:
    ...
    `nunmai vault list` unlock does not carry into a chat session — unlock there when asked)."""
    ...
    if not rows and not locked:
        c.print("[dim]Vault is empty. Add an item with `nunmai vault add`.[/]")
        return
    if rows:
...
def _cmd_sources(args) -> None:
    ...
    from nunmai_cli.config import _ensure_dict, load_config, save_config
    ...
    for name, cls in classes.items():
        if name in enabled:
            ...
            status = "[dim]turned off[/] (`nunmai vault sources --enable {name}` to use it)".format(name=name)
        else:
...
def register_cli(subparser) -> None:
    """Build the ``nunmai vault`` argparse tree (called from main.py)."""
    ...
    p_rm.add_argument("handle", help="Item handle (see `nunmai vault list`)")
