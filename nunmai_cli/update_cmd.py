"""Nunmai update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``nunmai_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``nunmai_cli.main`` at call time.
"""
...
from nunmai_cli.config import get_nunmai_home  # noqa: F401  (re-exported; patched via update_cmd)
from nunmai_cli import update_handoff as _update_handoff
from nunmai_cli.update_cmd_common import _best_effort
from nunmai_cli._early_recovery import interrupted_pull_marker
from nunmai_constants import get_default_nunmai_root, project_venv_dir, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from nunmai_cli.update_abort_recovery import (  # noqa: F401
...
from nunmai_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _nunmai_holder_subcommand, _holder_value_flags,
...
from nunmai_cli.update_cmd_fleet import (  # noqa: F401
...
from nunmai_cli.update_cmd_zip import (  # noqa: F401
...
from nunmai_cli.update_cmd_stash import (  # noqa: F401
...
from nunmai_cli.update_cmd_config import (  # noqa: F401
...
from nunmai_cli.update_cmd_deps import (  # noqa: F401
...
from nunmai_cli.update_cmd_git import (  # noqa: F401
...
from nunmai_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``nunmai_cli.main`` handle: keeps main-side test patches effective, import one-way."""
...
_UPDATE_CRITICAL_FILES = (
    "nunmai_cli/main.py", "nunmai_cli/config.py", "nunmai_cli/__init__.py",
    "nunmai_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "nunmai_constants.py")
...
# otherwise leaves `nunmai update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from nunmai_constants import get_nunmai_home  # noqa: F811  (deliberate: constants variant)
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-nunmai.exe gate."""
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_nunmai_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from nunmai_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``nunmai update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from nunmai_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    if is_shallow:
        ...
        from nunmai_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    if handed_off_sync or not healthy or stale:
        ...
        if runtime_repaired is not None:
            ...
            # imports pass while lazily-installed backends and ``nunmai tools`` dependencies
            ...
            # must leave the breadcrumb so the next `nunmai` run finishes the repair.
            ...
            if _m()._refresh_active_lazy_features(
                ...
                print("  ⚠ Lazy-refresh recovery incomplete — run `nunmai` again "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    # back — an expiring log the user has to know to reach for, in a directory Nunmai updates
    ...
    if pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/nunmai-update-backups/{kind}-{branch}-"
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in nunmai_cli/config.py bricked
    # every user who ran ``nunmai update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``nunmai update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway nunmai.exe
    holds the venv shim."""
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Nunmai runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from nunmai_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another nunmai.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_nunmai_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://nunmai-engine.nunmai.in/install.sh | bash")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``nunmai update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``nunmai
...
def _handle_update_called_process_error(
    ...
    if _should_zip_fallback_on_update_error(e):
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            print()
            print("  Nunmai may not start until the dependencies are installed. Fix the error above")
            print("  (usually network or disk space), then run `nunmai update` again.")
            if _m()._is_windows():
                print("  If `nunmai update` itself will not start, retry through the venv interpreter:")
                print(
...
def _post_swap_payload(
    *, swap: str, branch: str, opts, gateway_mode: bool, had_desktop_app_before_update: bool,
    ...
    from nunmai_cli.update_receipt import detach_update_receipt
...
def _sibling_snapshots_module():
    ...
    import nunmai_cli.update_cmd_config as _cfg
    return _cfg


def _hand_off_post_swap(args, **payload_kwargs) -> None:
    """Re-execute ``nunmai update --post-swap`` on the pulled tree and exit with its code.

    The parent detaches from the receipt and its Windows resume hook — the child owns both —
    and only relays the exit code (``nunmai_cli/update_handoff.py``).
    """
    from nunmai_cli.update_receipt import resume_update_receipt
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    import nunmai_cli.update_cmd_config as _cfg
    from nunmai_cli.update_inventory import UpdatePlan
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # A child spawned off nunmai.exe already outwaited its parent in ``cmd_update`` (before the
    # update lock, so the lock it now holds is its own — the parent's marker left with it).
    from nunmai_cli.update_handoff import adopt_handed_off_gateway_resume
    ...
    # Any venv python still running (typically the Desktop `nunmai serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `nunmai
