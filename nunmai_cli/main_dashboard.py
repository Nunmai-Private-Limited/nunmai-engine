...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None,
    ...
    *scope_home*: keep only backends whose resolved Nunmai home (see
    ``_nunmai_home_for_pid``) is this home; unreadable ownership is spared, never guessed.
    ...
    # 'nunmai dashboard --stop'``); killing it takes down the invoking terminal.
    ancestors = _caller_ancestor_pids()
    pids = [pid for pid in pids if not _is_caller_wrapper_shell(pid, ancestors)]
    return _pids_owned_by_nunmai_home(pids, scope_home) if scope_home else pids
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"nunmai {candidate}", f"nunmai_cli.main {candidate}", f"nunmai_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Nunmai installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``nunmai-serve.service``), or None.
...
# launchd plist directories that can supervise a ``nunmai dashboard`` / ``nunmai serve`` backend on
...
def _loaded_launchd_backend_jobs(
    ...
    ``ProgramArguments`` is a ``nunmai dashboard`` / ``nunmai serve`` backend. macOS only (empty
    ...
    from nunmai_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from nunmai_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``nunmai update``, detached, logging to
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``nunmai update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        stage = "continued on the pulled code" if os.environ.get("NUNMAI_UPDATE_POST_SWAP") == "1" else "started"
        log_file.write(f"\n=== nunmai update {stage} {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    """
    from nunmai_cli.dashboard_procs import _scan_dashboard_processes
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under the account's $HOME/.nunmai/
    # desktop-ssh, independent of NUNMAI_HOME and the active profile. Anchor
    # validation there, NOT get_nunmai_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_nunmai_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".nunmai" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``nunmai dashboard`` inheriting that ``NUNMAI_WEB_DIST`` would
...
def _attach_to_host_backend(args, headless_backend: bool) -> None:
    """Multiplex-only: a second `nunmai serve`/`dashboard` attaches to the host backend.
    ...
    Desktop pool backends (NUNMAI_DESKTOP=1) keep their own lifecycle.
    ...
    * the owner must ANSWER on its recorded port and identify itself (a record alone cannot see a
      ...
      `nunmai update` relaunch landing in that window would otherwise exit 0 with NOTHING
    ...
    * a `nunmai dashboard` user is never handed a headless backend's URL (no SPA behind it).
    ...
    if not headless_backend and not identity.get("servesSpa"):
        print(f"Refusing to start: this host is already served by {hr.describe(record)}, "
              "which is a headless `nunmai serve` backend with no dashboard UI.")
        print("  Stop it and run `nunmai dashboard`, or use --isolated for a dedicated server.")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
    ...
    print(f"Nunmai {kind} already running on this host: PID {record.pid}, port {record.port}.")
...
def _route_named_profile_dashboard(
    ...
    opts out; Desktop pool backends (NUNMAI_DESKTOP=1) stay per-profile. Returns normally when no
    routing applies.
    """
    try:
        from nunmai_cli.profiles import get_active_profile_name
    ...
    # NUNMAI_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping NUNMAI_HOME: in the Docker layout the root is /opt/data, and an
    # unset NUNMAI_HOME would fall back to $HOME/.nunmai = /opt/data/.nunmai — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_nunmai_root() strips a trailing profiles/<name> for both layouts.
    try:
        from nunmai_constants import get_default_nunmai_root
        env["NUNMAI_HOME"] = str(get_default_nunmai_root())
    except Exception:
        env.pop("NUNMAI_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets NUNMAI_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless NUNMAI_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed NUNMAI_WEB_DIST can't be populated.
    """
    from nunmai_cli.main import PROJECT_ROOT
    from nunmai_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["NUNMAI_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "NUNMAI_WEB_DIST" not in os.environ and not skip_build:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # --build-mode skip after a wipe of web_dist); a custom NUNMAI_WEB_DIST
        ...
        # NUNMAI_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # NUNMAI_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        if not (_dist_root / "index.html").exists():
            ...
            sys.exit(1)
        # web_server reads NUNMAI_WEB_DIST raw at import (no expanduser), so a
