...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        log_file.write(f"\n=== nunmai update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _route_named_profile_dashboard(
    ...
    (NUNMAI_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("NUNMAI_DESKTOP") == "1"
    ):
        return
