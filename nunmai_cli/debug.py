...
_USER_AGENT = "nunmai-engine/debug-share"
...
# ``nunmai debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``nunmai debug delete`` manually
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``nunmai dump`` is run internally.
    """
    buf = io.StringIO()
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip a :func:`collect_share_bundle` mapping into the Nunmai envelope (shape parsed by the
...
def build_debug_share(
    ...
    Shared by ``nunmai debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``nunmai debug share --nous``: gzip the same bundle into the Nunmai envelope → Nous-S3."""
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("nunmai debug share --nous: applied force-mode redaction before upload")
    ...
    try:
        ...
        print(f"\nNous upload failed: {exc}\n"
              "\nThe Nunmai diagnostics service may be unavailable or not yet provisioned.\n"
              "Run `nunmai debug share --local` to print the report instead, "
    ...
    print("\nShare this private link with the Nunmai team — only Nunmai staff "
          ...
          "  GitHub Issues        https://github.com/Nunmai-Private-Limited/nunmai-engine/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nunmai debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'nunmai debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
...
Options (share):
  ...
  --nous       Upload to Nunmai-internal storage (private, staff-only,
