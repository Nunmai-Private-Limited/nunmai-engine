"""``nunmai debug`` debug tools for Nunmai Engine.

Currently supports:
    nunmai debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[nunmai debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.nunmai/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``nunmai debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``nunmai debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `nunmai debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nunmai-engine/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``nunmai debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.nunmai/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``nunmai debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nunmai-engine/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----NunmaiDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from nunmai_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_nunmai_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `nunmai debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Nunmai Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``nunmai dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``nunmai dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "nunmai-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``nunmai debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "nunmai debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    print("To delete now:  nunmai debug delete <url>")

    print("\nShare these links with the Nunmai team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Nunmai version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from nunmai_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "nunmai debug share --nous: applied force-mode redaction before upload"
        )
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nunmai debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'nunmai debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``nunmai debug`` call.
    ...
    # best-effort — any failure is swallowed so ``nunmai debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: nunmai debug <command>")
        print()
        print("Commands:")
