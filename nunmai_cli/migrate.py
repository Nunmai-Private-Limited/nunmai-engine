"""CLI handlers for ``nunmai migrate ...``.

Currently exposes only ``nunmai migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``nunmai migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: nunmai migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `nunmai migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `nunmai doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
