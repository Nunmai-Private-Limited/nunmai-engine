"""Quiet ``nunmai chat -Q`` helpers: bind this session's key and resume nested notifies.

Bot Mode delivers a local DM as ``nunmai -p <bot> chat -Q --query-file``. Interactive
...
``NUNMAI_SESSION_KEY`` and never woke the recipient. Quiet also printed and exited
...
# ``nunmai kanban dispatch`` process never reaped the worker, so ``os.waitpid`` cannot tell it how
...
# contract as NUNMAI_TURN_AUTHOR): nothing the turn spawns inherits it, and a nested one-shot
# never writes over its host's report — the record also carries the writer's pid.
TURN_REPORT_FILE_ENV = "NUNMAI_QUIET_TURN_REPORT_FILE"
...
def run_reported_turn(argv: list, *, env: MutableMapping[str, str], report_path: str, timeout: float,
    ...
    """Run one ``nunmai chat -Q`` delivery child; *timeout* bounds the TURN, not the process.
    ...
    *cwd* pins the child's directory (a spawner sitting in a reaped scratch workspace must not
    ...
    the child is guaranteed UTF-8 (nunmai_bootstrap reconfigures its streams even under
