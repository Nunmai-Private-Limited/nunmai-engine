...
Mixed into :class:`nunmai_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from nunmai_cli.middleware import OBSERVER_SCHEMA_VERSION
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from nunmai_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"nunmai-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="nunmai-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from nunmai_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
