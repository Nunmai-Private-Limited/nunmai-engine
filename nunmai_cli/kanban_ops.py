"""Dispatcher and maintenance verbs for ``nunmai kanban``: ``dispatch``,
...
from nunmai_cli.kanban_output import _err, _fmt_ts, _print_json
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"successfully.{held} Check profile health (venv, PATH, credentials) and `nunmai "
                    f"kanban list --status ready` / `nunmai kanban list --status blocked` for "
