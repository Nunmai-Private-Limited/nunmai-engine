"""Normalized Nunmai Portal account entitlement helpers."""
...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Nunmai account. Run `nunmai auth upgrade`."
FREE_TIER_NEEDS_ACCOUNT_CHAT = "This needs a Nunmai account. Use /login to sign in."
...
class NousPortalAccountInfo:
    ...
    # NAS account tier claim; ``"anonymous"`` is the free tier (no Nunmai account behind it).
    ...
    def is_anonymous_tier(self) -> bool:
        """The free tier: no Nunmai account, so no billing, credits, or entitlement to speak of."""
...
def nous_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Nunmai account snapshot."""
    try:
...
def format_nous_portal_entitlement_message(
    ...
    """User-facing guidance for a missing Nunmai tool-gateway entitlement; ``None`` when entitled.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                return (
                    f"{capability} isn't included with your current Nunmai Portal access. "
    ...
    if account_info is None:
        return (
            f"Nunmai could not verify your Nunmai Portal entitlement, so {capability} is unavailable. "
            f"Run `nunmai model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nunmai inference credentials are configured, but Nunmai cannot verify your Nunmai Portal "
                f"paid access for {capability}. Log in with `nunmai model` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nunmai Portal to use {capability}: run `nunmai model`. "
            f"Billing and credits are managed at {billing_url}."
        )
    if account_info.paid_service_access is None:
        detail = f"Nunmai could not verify your Nunmai Portal paid access, so {capability} is unavailable."
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `nunmai model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Nunmai could not find a Nunmai Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `nunmai model` to authenticate again; if the problem persists, contact Nunmai support."
        )
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `nunmai model` to refresh Nunmai."
        return message
    return (
        f"Your Nunmai Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Nunmai Portal access is paused because you've exceeded the per-member spend cap set by "
            f"your organisation.{cap_detail}{credit_detail} Ask your organisation admin to raise the "
            f"member spend cap at {billing_url}, then run `nunmai model` to refresh."
        )
    if active and paid:
        return (
            f"Your Nunmai Portal credits are exhausted{credit_detail}, so {capability} is unavailable. "
            f"Top up or renew credits at {billing_url}."
        )
    if active and paid is False:
        return (
            f"Your current Nunmai Portal plan does not include paid service access, "
            f"so {capability} is unavailable. Upgrade or add credits at {billing_url}."
        )
    if active is False:
        return (
            f"Your Nunmai Portal account has no active subscription or usable credits{credit_detail}, "
            f"so {capability} is unavailable. Subscribe or add credits at {billing_url}."
        )
    return (
        f"Your Nunmai Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(*, force_fresh: bool = False, min_jwt_ttl_seconds: int = 60) -> NousPortalAccountInfo:
    """Normalized Nunmai Portal account entitlement.
...
def nous_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from nunmai_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NousPortalAccountInfo:
    ...
    try:
        from nunmai_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_inference_key_pool(portal_base_url: Optional[str]) -> Optional[NousPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Nunmai keys."""
    try:
...
def _info_from_valid_jwt(
    ...
    try:
        from nunmai_cli.auth import _decode_jwt_claims
    except Exception:
        return None
...
def resolve_nous_portal_base_url() -> str:
    from nunmai_cli.auth import _nous_portal_base_url, get_provider_auth_state
