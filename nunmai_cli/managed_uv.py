...
``<checkout>/.nunmai-runtime/python``. A vulnerable interpreter is never reinstalled in place: a
...
def pip_install_hint(package: str) -> str:
    ...
    Names Nunmai' own uv when it exists: the installer drops it in ``$NUNMAI_HOME/bin``
...
def _uv_runs(uv_bin: str) -> bool:
    ...
    ``$NUNMAI_HOME/bin``: it is a file with the executable bit that never runs, so is_file()+X_OK
...
def _record_runtime_repair(repair: RuntimeRepairResult) -> None:
    """Put the repair outcome into the update receipt (no-op outside ``nunmai update``).
    ...
    """
    from nunmai_cli.update_receipt import record_skip, record_step
...
def _stream_sync(argv: list[str], *, cwd: Path, env: dict[str, str]) -> tuple[int, str]:
    ...
    scrollback only, and the rejection carried a bare exit code — "nunmai update says the SQLite
