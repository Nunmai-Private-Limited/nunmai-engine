...
The Python backing the install is shared by every Nunmai profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.nunmai-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
def managed_uv_path() -> Path:
    """Path of Nunmai' own uv binary (``$NUNMAI_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_nunmai_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Nunmai keeps databases "
...
def _uv_self_update_stamp() -> Path:
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``nunmai update`` daily; a blocking network
...
def _reload_nunmai_constants():
    """Re-execute ``nunmai_constants`` from disk (the imported one may predate venv_python_path)."""
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Nunmai generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `nunmai update`. The
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``nunmai update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``nunmai.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\nunmai.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `nunmai update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m nunmai_cli.main update",
...
def _install_uv(target: Path) -> None:
    ...
    into ``$NUNMAI_HOME/bin/`` instead of ``~/.local/bin/``.
    """
