...
from nunmai_cli.auth_plugin_providers import (
...
# ``nunmai auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
_OAUTH_DEFAULT_PROVIDERS = _OAUTH_CAPABLE_PROVIDERS - {"openrouter"}
# Providers whose sibling CLI login Nunmai may borrow (``auth.adopt_external_logins``).
...
def _unknown_provider_exit(provider: str) -> SystemExit:
    ...
    return SystemExit(
        f"Unknown provider '{provider}'.{hint} Run `nunmai auth` to see the provider list, or "
        "`nunmai model` to pick one interactively.")
...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_nunmai_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from nunmai_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``nunmai auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:nunmai_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> PooledCredential:
    """``nunmai auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nunmai credential at <nunmai-root>/shared/nous_auth.json
    # (written by any previous login) makes `nunmai --profile <name> auth add nous --type oauth`
    # a one-tap operation for multi-profile users.
    if auth_mod._read_shared_nous_state():
        try:
            found = f"Found existing Nunmai OAuth credentials at {auth_mod._nous_shared_store_path()}"
        except RuntimeError:
            found = "Found existing shared Nunmai OAuth credentials"
        print()
        print(found)
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if spec is None:
        raise SystemExit(f"`nunmai auth add {provider}` is not implemented for auth type {requested_type} yet.")
    ...
    for position, sibling in enumerate(existing, start=1):
        if _codex_principal_identity(sibling.access_token) == identity:
            print(f'warning: this login is the same OpenAI account as openai-codex credential #{position} '
                  ...
                  f"(`nunmai auth remove openai-codex {position}`).", file=sys.stderr)
            return
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `nunmai auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`nunmai auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def _free_tier_lines() -> tuple[str, str]:
    ...
    from nunmai_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Nunmai reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`nunmai auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `nunmai auth list {provider}`).")
    ...
    if (not is_refreshable_oauth_provider(provider) or matched.auth_type != AUTH_TYPE_OAUTH
        ...
        raise SystemExit(
            ...
            f"credential.")
    # Nunmai's resolver is singleton-bound, not an independent-account refresher.
    if provider == "nous" and matched.source != "device_code":
        raise SystemExit(
            ...
            "Reauthenticate with `nunmai auth add nous --type oauth`.")
    ...
    if refreshed is None:
        ...
        raise SystemExit(
            ...
            f"Sign in again with `nunmai auth add {provider} --type oauth`.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `nunmai auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `nunmai auth` is called bare."""
...
def auth_upgrade_command(args) -> None:
    """``nunmai auth upgrade``: sign the free tier into a Nunmai account, keeping its connectors."""
    from nunmai_cli.anon_auth import upgrade_guest
