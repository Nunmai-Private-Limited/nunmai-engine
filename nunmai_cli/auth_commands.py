...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        # Codex-style auto-import: if a shared Nunmai credential lives at
        # <nunmai-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `nunmai --profile <name>
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing Nunmai OAuth credentials at {path}")
            else:
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``nunmai auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``nunmai auth add xai-oauth`` overwrote the
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Nunmai reads from
