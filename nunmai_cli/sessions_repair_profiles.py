"""``nunmai sessions repair-profiles`` — settle crossed-profile durable state across every store.
...
class RepairPlan:
        ...
        if key_profile not in self.claimed:
            self._add(Finding(
                ...
                reason=f"no live profile named {key_profile!r}; create it, then rerun, or "
                       f"`nunmai profile migrate-identity {key_profile} <target>`"))
            return
        moving.add(sid)
        self._add(Finding(
    ...
    def _scan_routing(self, session: _Session, store: Store) -> None:
        from nunmai_state_profile_repair import session_key_profile
...
def default_snapshot(store: Store) -> Optional[str]:
    ...
    try:
        return create_quick_snapshot(label="repair-profiles", nunmai_home=store.home)
    except Exception as exc:
