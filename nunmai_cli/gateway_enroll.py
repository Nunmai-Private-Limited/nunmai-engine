"""``nunmai gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nunmai Portal access token from the existing login
     (``~/.nunmai/auth.json``) — the same path ``nunmai dashboard register``
     uses (``resolve_nous_access_token``). This proves *which Nunmai org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.nunmai/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Nunmai Portal).
    ...
    self-hosted-IdP path; otherwise Nunmai Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Nunmai Portal "
                "token could not be verified — try `nunmai auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `nunmai gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Nunmai Portal); otherwise the Nunmai Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.nunmai/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_nunmai_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in nunmai_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_nunmai_root()).resolve()
