...
def cmd_gateway_enroll(args) -> None:
    ...
    # is set (air-gapped / self-hosted IdP), otherwise Nunmai Portal.
    try:
        ...
        if getattr(exc, "relogin_required", False):
            _fail(
                "✗ You're not logged into Nunmai Portal.",
                "  Run `nunmai setup` (or `nunmai auth add nous`) first, then retry.",
            )
        _fail(f"✗ Could not resolve a Nunmai Portal access token: {exc}")
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.nunmai/.env and never
