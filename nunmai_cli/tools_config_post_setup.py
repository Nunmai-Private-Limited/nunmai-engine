"""Post-setup install hooks and installed-state predicates for `nunmai tools` provider rows."""
...
from nunmai_cli.tools_config_cua import (
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Nunmai-managed ``$NUNMAI_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Nunmai under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Nunmai-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung nunmai_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Nunmai-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Nunmai-managed node -> npx) rather than
    # a bare shutil.which — Nunmai-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.nunmai/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.nunmai/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `nunmai auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.nunmai/.env)
    ...
    try:
        ...
        _info_lines("Run manually: nunmai auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: nunmai auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        ...
        _info_lines("Run manually: nunmai plugins enable observability/langfuse")
    _info_lines("Restart Nunmai for tracing to take effect.", "Verify: nunmai plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from nunmai_cli.setup import prompt_choice, prompt as _setup_prompt
        from nunmai_cli.setup_tts import _run_xai_oauth_login_from_setup
        from nunmai_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: nunmai auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            ...
            _print_warning("    xAI Grok OAuth login did not complete. Run later: nunmai auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: nunmai auth add xai-oauth")
    else:
...
def _post_setup_openai_codex() -> None:
    ...
    relogin = "nunmai auth add openai-codex"
    _print_info("    OpenAI (Codex auth) needs credentials.")
    try:
        from nunmai_cli.auth import _codex_device_code_login, _save_codex_tokens
        from nunmai_cli.setup import is_noninteractive, prompt_choice
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
...
def _xai_credentials_ready() -> bool:
    from nunmai_cli.tools_config import _xai_credentials_present  # facade binding: tests patch it there
...
def run_post_setup_command(args) -> int:
    """``nunmai tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
...
# Python deps installed via ``nunmai tools`` aren't in the managed runtime's locked ``all`` sync, so a
