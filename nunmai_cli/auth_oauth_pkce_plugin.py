...
Nunmai owns the security boundary: HTTPS-only endpoints (plain HTTP only for a loopback-literal host,
...
Lives in ``nunmai_cli`` because everything it drives (loopback helpers, the auth store lock, the pool)
does; every core import is deferred into the callables so a plugin may import this module while
provider discovery is still running inside ``nunmai_cli.auth``'s own import.
"""
...
def _post_token(provider: str, cfg: OAuthPKCEConfig, data: Dict[str, str], *, code: str) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import _coerce_ttl_seconds, _default_verify, _utc_now_z
...
def _token_http_error(provider: str, response: Any, fallback_code: str):
    ...
    from nunmai_cli.auth import _OAUTH_GRANT_DEAD_CODES
...
def login(provider: str, cfg: OAuthPKCEConfig, *, open_browser: bool = True) -> Dict[str, Any]:
    ...
    print(f"\nOpen this URL to authorize Nunmai with {cfg.label or provider}:\n  {authorize_url}\n")
    ...
    def handler(action: str, args: Any) -> bool:
        ...
        if action == "status":
            ...
            if not entries:
                ...
                print(f"{provider}: expired (needs refresh) — run `nunmai auth refresh {provider}`")
            return True
        if action == "logout":
...
def pkce_refresh_credential(cfg: OAuthPKCEConfig) -> Callable[[Any], Mapping[str, Any]]:
    ...
    def refresh(entry: Any) -> Mapping[str, Any]:
        from nunmai_cli.auth import _auth_store_lock, read_credential_pool
