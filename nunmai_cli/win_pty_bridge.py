"""Windows ConPTY bridge for the `nunmai dashboard` chat tab.

Drop-in counterpart to ``nunmai_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``nunmai_cli.web_server`` consumes: ``spawn``, ``read``,
