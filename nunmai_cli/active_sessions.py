...
from nunmai_constants import get_default_nunmai_root, get_nunmai_home, named_profile_is_live
...
def active_session_limit_message(
    ...
    return (
        f"Nunmai is at the active session limit ({active_count}/{max_sessions})."
...
def format_refusal_stderr(message: str) -> str:
    ...
    return f"nunmai-refusal-reason: {reason}\n{message}" if reason else str(message)
...
def session_already_owned_message(session_id: str, entry: dict[str, Any]) -> str:
    ...
    """
    return (
        "This chat is open in another Nunmai window/terminal. Use it there, or start a new chat here.\n"
        + session_owner_details(session_id, entry)
    )


def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_nunmai_home())
...
class ActiveSessionLease:
    ...
    enabled: bool = True
    released: bool = False
    # Pinned at acquisition: a lease taken under the root NUNMAI_HOME must release
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "Nunmai could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile NUNMAI_HOME override.
    # See #85431.
