...
class ActiveSessionLease:
    ...
    # root ``NUNMAI_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile NUNMAI_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``nunmai dashboard`` / ``serve``) never trips that
