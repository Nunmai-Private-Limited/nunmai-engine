...
``nunmai cron``. Invariants (mirrors goals.py): injection is a plain user message — no system-prompt
...
def _get_session_db() -> Optional[Any]:
    """Persistence goes through the goals module's per-NUNMAI_HOME cached SessionDB (one shared connection)."""
    try:
