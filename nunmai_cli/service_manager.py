...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``nunmai gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from nunmai_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `nunmai gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged nunmai user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Nunmai runtime call
    inside the container drops to nunmai via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``nunmai_cli.gateway`` (systemd/launchd) and
# ``nunmai_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``nunmai -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``nunmai_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import nunmai_cli
        importlib.import_module(f"nunmai_cli.{self._backend}")
        return getattr(nunmai_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in nunmai_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in nunmai_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``nunmai_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `nunmai profile create` inside the container. Static services (main-nunmai, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's NUNMAI_HOME blindly: derive the shared profile root and map the suffix to the root
    ...
    root = nunmai_home.parent.parent if nunmai_home.parent.name == "profiles" else nunmai_home
...
def register_unregistered_profile_gateway(mgr: ServiceManager, profile: str) -> bool:
    ...
    `nunmai profile create` can only register a slot when it runs inside the container; created
...
# UID/GID of the in-image ``nunmai`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_NUNMAI_UID = 10000
_NUNMAI_GID = 10000


def _chown_nunmai(path: Path) -> None:
    try:
        os.chown(path, _NUNMAI_UID, _NUNMAI_GID)
    except PermissionError:
        # Already running as nunmai → the dir is nunmai-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create nunmai-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    nunmai user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources NUNMAI_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to nunmai.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root NUNMAI_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``NUNMAI_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``nunmai_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        # nunmai_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export NUNMAI_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its NUNMAI_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``nunmai gateway run``, an agent action, the Open
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock AS NUNMAI when starting as root. Never
            # chown/unlink nunmai-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded nunmai-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed nunmai-owned supervise/ BEFORE publishing so the nunmai-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is nunmai-owned (see ``_seed_supervise_skeleton``)
