...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``nunmai model`` -> xAI Grok OAuth."""
    try:
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via nunmai setup model "
...
def _setup_tts_provider(config: dict):
    ...
    if tool_backend_helpers.managed_nous_tools_enabled() and nous_subscription.get_nous_subscription_features(config).nous_auth_present:
        options.insert(0, ("nous-openai",
                           "Nunmai Subscription (managed OpenAI TTS, billed to your subscription)"))
    ...
    if selected == "nous-openai":
        selected = "openai"
        _setup.print_info("OpenAI TTS will use the managed Nunmai gateway and bill to your subscription.")
