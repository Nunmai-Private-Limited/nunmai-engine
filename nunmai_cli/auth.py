...
from nunmai_cli.config import (
    get_nunmai_home, get_config_path, read_raw_config, require_readable_config_before_write)
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # is a 401), so there is no secret to configure. Select via `nunmai model` / `/model free`.
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # enforced); ignore failures. secure_parent_dir refuses to chmod /, top-level dirs, or the nunmai-engine
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from nunmai_cli.config import get_active_config_parse_failure
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("Nunmai is not logged into Nunmai Portal.", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise _nous_err("No access token found for Nunmai Portal login.", relogin=True)
...
def _provider_is_keyless(provider_id: str) -> bool:
    """NunmaiOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from nunmai_cli.providers import NUNMAI_OVERLAYS
        return bool(getattr(NUNMAI_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Nunmai can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
