...
- The auth store (``~/.nunmai/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``nunmai_cli.auth.<name>`` stays the public/patchable surface."""
...
from nunmai_constants import OPENROUTER_BASE_URL, nunmai_home_key, secure_parent_dir
...
from nunmai_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from nunmai_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_nous import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_minimax import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_xai import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_codex import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from nunmai_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from nunmai_cli.auth_qwen import (  # noqa: F401  re-exported
...
from nunmai_cli.auth_constants import (  # noqa: F401  re-exported
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ProviderConfig(
        "nous", "Nunmai Portal", "oauth_device_code", portal_base_url=DEFAULT_NOUS_PORTAL_URL,
...
# ``nunmai_cli.config`` discovers model-provider plugins while importing, and a plugin may read this
...
from nunmai_cli.config import (  # noqa: E402
    atomic_config_write, get_nunmai_home, get_config_path, read_raw_config, require_readable_config_before_write)
...
from nunmai_cli.auth_plugin_providers import (  # noqa: E402
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.nunmai/.env`` so a deliberate
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$NUNMAI_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    # Prefer ~/.nunmai/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $NUNMAI_HOME/.env (e.g. NUNMAI_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``nunmai auth``."""
...
# Entitlement failures: Nunmai gets a Portal-aware message; other providers a fixed generic one (or
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        # Profile-aware: a bare `nunmai model` from a named profile re-signs the ROOT store (#114012).
        from nunmai_constants import profile_cli_selector

        return f"{error} Run `nunmai {profile_cli_selector()}model` to re-authenticate."
...
def _auth_file_path() -> Path:
    ...
    # monkeypatch NUNMAI_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".nunmai" / "auth.json")):
        raise RuntimeError(
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        return None
    return None if _same_path(get_nunmai_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".nunmai" / "auth.json"
        try:
...
def _file_lock(
    ...
    shared Nunmai store) track reentrancy separately."""
...
def _save_private_json(target: Path, data: Any, *, fsync_dir: bool = False, **dump_kwargs: Any) -> None:
    ...
    mkdir_under_nunmai_home(target.parent)
...
def _save_provider_state_to_source(
    ...
    left as it is (a Nunmai free-tier identity refreshed for a connector call must not become the
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``nunmai auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``nunmai auth add`` in the profile shadows global)."""
...
def _merge_disk_cooldown_state(
    ...
    ``nunmai auth reset`` that postdates the snapshot's cooldown (``status_cleared_at`` newer than
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``nunmai auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Nunmai flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Nunmai.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "nunmai_pkce", "manual"})
...
def _env_secret(name: str) -> bool:
    ...
    Must not read raw ``os.getenv``: under ``nunmai serve`` / Desktop multiplex the
    ...
    NUNMAI_HOME ``.env`` first, then the scope-checked environ) so the gate and the key that
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Nunmai flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Nunmai-scoped routing config is present.
    ...
    Nunmai-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Nunmai-initiated flow, or Nunmai-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``nunmai logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    if str(provider_name or "").strip().lower() in {"opencode-free", "free", "opencode_free"}:
        return ("OpenCode discontinued anonymous free-tier access outside its own client "
                ...
                "($10/mo subscription, OPENCODE_GO_API_KEY) via 'nunmai model'.")
    try:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from nunmai_cli.config_read_errors import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run nunmai setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `nunmai auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `nunmai auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `nunmai auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    dashboard's Ink chat parked every session on Setup Required while ``nunmai chat`` worked
    (#108383)."""
    try:
        from nunmai_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if provider:
            from nunmai_cli.runtime_provider_custom import has_named_custom_provider
        ...
        if base_url:
            from nunmai_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.nunmai/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.nunmai/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    7. Nunmai free tier when it is on and its identity exists (never created here);
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'nunmai model' for available providers, "
                "or run 'nunmai doctor' to diagnose config issues.")
    ...
    # Nunmai free tier, when it is on and its identity already exists. This rung sits ABOVE the Bedrock
    ...
    raise AuthError(
        "Nunmai is not connected to any AI provider yet. Run `nunmai model` to pick one (the free "
        "Nunmai tier needs no API key), type `/login` in chat, or add a key with "
        f"`nunmai auth add <provider>`. (Advanced: put an API key such as OPENROUTER_API_KEY in "
        f"{display_nunmai_home()}/.env.)",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the Nunmai / xAI / Codex pattern)."""
...
# Valid Nunmai Portal hosts; a stored portal_base_url outside this set is a misconfiguration and falls
...
# Keyed by nunmai_home_key(): the resolution itself is profile-scoped (_auth_file_path reads the
# per-turn NUNMAI_HOME override a multiplex gateway sets), so a single slot would hand profile A's
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """NUNMAI_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("Nunmai is not logged into Nunmai Portal.", "nous_auth_missing", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            from nunmai_cli.anon_auth import is_guest_state, refresh_guest_state
            ...
            if not isinstance(access_token, str) or not access_token:
                raise _nous_err(
                    "No access token found for Nunmai Portal login.", "nous_auth_missing_access_token", relogin=True)
...
def invalidate_nous_auth_status_cache() -> None:
    """Clear the get_nous_auth_status() memo (for code paths that mutate Nunmai auth state without
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for Nunmai auth, memoised ~15s keyed on the auth.json mtime.
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("nunmai_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``nunmai logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
# Nunmai state-shape failures raised BEFORE any refresh POST (no login, no token pair): retrying
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from nunmai_cli.providers import normalize_provider
...
def _external_process_auth_evidence(provider_id: str, resolved_command: Optional[str]) -> tuple[bool, Optional[str]]:
    ...
    for any external-process profile is its binary resolving: the subprocess owns real auth and Nunmai
...
def _copilot_acp_auth_evidence() -> tuple[bool, Optional[str]]:
    ...
    try:
        from nunmai_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: NUNMAI_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / NUNMAI_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``nunmai_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``nunmai doctor`` runs the live probe). ``api_key`` (default): usable
...
def login_command(args) -> None:
    ...
    print("The 'nunmai login' command has been removed.\nUse 'nunmai auth' to manage credentials,\n"
          "'nunmai model' to select a provider, or 'nunmai setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nous":
        from nunmai_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
    ...
    if target == "nous":
        ...
        from nunmai_cli.auth_nous import _clear_shared_nous_state
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('nunmai_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('nunmai_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('nunmai_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('nunmai_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('nunmai_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('nunmai_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('nunmai_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('nunmai_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('nunmai_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('nunmai_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('nunmai_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('nunmai_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('nunmai_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('nunmai_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('nunmai_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('nunmai_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('nunmai_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
