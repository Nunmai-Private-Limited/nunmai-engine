...
# that process is ``nunmai update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``nunmai_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``nunmai_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import nunmai_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]nunmai secrets bitwarden status[/cyan]\n"
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `nunmai secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next Nunmai invocation."
    )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`nunmai secrets bitwarden setup` first.[/yellow]"
        )
        return 1
