...
Split out of :mod:`nunmai_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``nunmai``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "nunmai", "depends",
    "requires_nunmai", "python_runtime",
}
...
_CONFIG_SCHEMA_TYPES: Dict[str, tuple] = {
    ...
    # ``secret`` values live in ``.env`` (see nunmai_cli.plugins_settings), so a config.yaml copy is
...
def _plugins_debug() -> bool:
    from nunmai_cli import plugins as _origin
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `nunmai plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `nunmai plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Nunmai version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_nunmai_error``.
    requires_nunmai: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Nunmai never auto-installs these (isolation design for the install seam
...
# ── requires_nunmai version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_nunmai_version() -> str:
    """Version of the Nunmai code that is running: ``nunmai_cli.__version__``. Distribution metadata is only
    ...
    try:
        ...
        pass
    return importlib.metadata.version("nunmai-engine")
...
def requires_nunmai_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_nunmai`` rejects the running version."""
    if not manifest.requires_nunmai:
        return None
    current = running_nunmai_version()
    if version_satisfies(manifest.requires_nunmai, current):
        return None
    return f"requires nunmai {manifest.requires_nunmai}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_nunmai=str(data.get("requires_nunmai") or "").strip(),
