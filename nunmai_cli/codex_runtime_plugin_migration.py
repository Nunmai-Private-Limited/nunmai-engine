"""Migrate Nunmai MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
from agent.transports.nunmai_tools_mcp_server import NUNMAI_TOOLS_MCP_SERVER_NAME
...
MIGRATION_END_MARKER = "# end nunmai-engine managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Nunmai config.")
        ...
        if self.preserved_user_servers:
            lines.append(
                ...
                f"config.toml (Nunmai projection skipped): {', '.join(self.preserved_user_servers)}")
        lines.extend(f"⚠ {err}" for err in self.errors)
        return "\n".join(lines)


# Nunmai MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Nunmai keys.
_KNOWN_NUNMAI_KEYS = {
...
# (nunmai key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, nunmai_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Nunmai MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Nunmai' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(nunmai_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if nunmai_cfg.get("command"):
        if nunmai_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(nunmai_cfg["command"])
        if nunmai_cfg.get("args"):
            out["args"] = [str(a) for a in nunmai_cfg["args"]]
        if nunmai_cfg.get("env"):
            out["env"] = _str_map(nunmai_cfg["env"])
        if nunmai_cfg.get("cwd"):
            out["cwd"] = str(nunmai_cfg["cwd"])
    elif nunmai_cfg.get("url"):
        out["url"] = str(nunmai_cfg["url"])
        if nunmai_cfg.get("headers"):
            out["http_headers"] = _str_map(nunmai_cfg["headers"])
        if nunmai_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for nunmai_key, codex_key, note in _TIMEOUT_KEYS:
        if nunmai_key in nunmai_cfg:
            try:
                out[codex_key] = float(nunmai_cfg[nunmai_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if nunmai_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in nunmai_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_NUNMAI_KEYS:
            skipped.append(f"{key} (unknown Nunmai key)")
    return out, skipped
...
# env-var passthrough (NUNMAI_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Nunmai)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _unmanaged_mcp_server_names(toml_text: str) -> set[str]:
    ...
    block is the user's. A Nunmai server whose name is already declared by the user is therefore
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_bin=codex_bin, codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="nunmai-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a NUNMAI_HOME pointing there burned into
...
def _build_nunmai_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Nunmai' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    NUNMAI_HOME passes through only IF SET, read from os.environ (not get_nunmai_home()): when
    unset the codex subprocess must inherit its launcher's runtime NUNMAI_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched NUNMAI_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched nunmai finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # NUNMAI_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_nunmai_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever NUNMAI_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's NUNMAI_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("NUNMAI_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    nunmai_home = os.environ.get("NUNMAI_HOME") or ""
    if nunmai_home and not _looks_like_test_tempdir(nunmai_home):
        env["NUNMAI_HOME"] = nunmai_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["NUNMAI_QUIET"] = "1"
    env["NUNMAI_REDACT_SECRETS"] = env.get("NUNMAI_REDACT_SECRETS", "true")
    return {
...
def migrate(
    nunmai_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_nunmai_tools: bool = True) -> MigrationReport:
    """Translate Nunmai mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_nunmai_tools`` registers Nunmai' own tool surface
    (agent/transports/nunmai_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    nunmai_servers = (nunmai_config or {}).get("mcp_servers") or {}
    if not isinstance(nunmai_servers, dict):
        report.errors.append("mcp_servers in Nunmai config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in nunmai_servers.items():
        name = str(raw_name)
    ...
    if discover_plugins and not dry_run:
        from nunmai_cli.codex_runtime_switch import get_configured_codex_binary
        plugins, plugin_err = _query_codex_plugins(
            codex_home=codex_home, codex_bin=get_configured_codex_binary(nunmai_config))
        if plugin_err:
    ...
    if expose_nunmai_tools:
        translated[NUNMAI_TOOLS_MCP_SERVER_NAME] = _build_nunmai_tools_mcp_entry()
        if NUNMAI_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(NUNMAI_TOOLS_MCP_SERVER_NAME)
    without_managed = ""
    if target.exists():
        try:
        ...
        if report.plugin_query_error:
            try:
                ...
                report.plugin_query_error += (
                    "; existing config.toml was unloadable — re-run `nunmai codex-runtime migrate` "
                    "to migrate plugins")
