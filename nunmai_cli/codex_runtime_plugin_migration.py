"""Migrate Nunmai' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Nunmai' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Nunmai mcp_servers.<n>.command/args/env  → codex stdio transport
  Nunmai mcp_servers.<n>.url/headers       → codex streamable_http transport
  Nunmai mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Nunmai mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Nunmai-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Nunmai' own
...
# Nunmai keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Nunmai' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, nunmai_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Nunmai MCP server config to the codex inline-table dict
    ...
    has_command = bool(nunmai_cfg.get("command"))
    has_url = bool(nunmai_cfg.get("url"))
    ...
    # Timeouts
    if "timeout" in nunmai_cfg:
        try:
            out["tool_timeout_sec"] = float(nunmai_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in nunmai_cfg:
        try:
            out["startup_timeout_sec"] = float(nunmai_cfg["connect_timeout"])
        except (TypeError, ValueError):
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (NUNMAI_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Nunmai)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Nunmai' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Nunmai' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Nunmai
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a NUNMAI_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed nunmai-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.nunmai`` symlink that happens to live under ``/private/var/folders``
...
def _build_nunmai_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Nunmai' own
    ...
    so a nunmai installed under /opt/, /usr/local/, or a venv all work.
    NUNMAI_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # NUNMAI_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_nunmai_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever NUNMAI_HOME its launcher sets
    ...
    # — that would override the launcher's NUNMAI_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("NUNMAI_HOME", tmp_path) would
    ...
    if nunmai_home and _looks_like_test_tempdir(nunmai_home):
        nunmai_home = ""
    if nunmai_home:
        env["NUNMAI_HOME"] = nunmai_home
    # PYTHONPATH passes through so a worktree-launched nunmai finds the
...
def migrate(
    ...
    *,
    ...
    expose_nunmai_tools: bool = True,
) -> MigrationReport:
    """Translate Nunmai mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        nunmai_config: full ~/.nunmai/config.yaml dict
        ...
        expose_nunmai_tools: when True (default), register Nunmai' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Nunmai for tools
    ...
    if not isinstance(nunmai_servers, dict):
        report.errors.append(
            "mcp_servers in Nunmai config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in nunmai_servers.items():
    ...
    # Inject Nunmai' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Nunmai for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/nunmai_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_nunmai_tools:
        translated["nunmai-tools"] = _build_nunmai_tools_mcp_entry()
        if "nunmai-tools" not in report.migrated:
            report.migrated.append("nunmai-tools")
