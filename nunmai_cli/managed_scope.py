...
DISTINCT from ``nunmai_cli.config.is_managed()`` / ``NUNMAI_MANAGED`` (a coarse package-manager
...
# unless the stale ``utils`` is dropped first. See nunmai_cli.stale_modules.
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/nunmai`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``NUNMAI_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$NUNMAI_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/nunmai`` when it exists.
