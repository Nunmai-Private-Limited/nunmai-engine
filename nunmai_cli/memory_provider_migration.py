...
"the code now lives under ``NUNMAI_HOME/plugins/<name>``". Two hooks call :func:`migrate_home`:

* ``nunmai update`` — for every profile home that shares the venv (primary; runs where the venv was
  just rebuilt anyway).
* agent init — when the configured provider cannot be found at all, once per process (Desktop
  users update through the app and never run ``nunmai update`` by hand).
...
def configured_provider(home: Path) -> str:
    ...
    from nunmai_cli.plugin_python_deps import _read_home_config
...
def provider_present(name: str, home: Path) -> bool:
    """True when the provider resolves anywhere Nunmai looks for *home* (bundled, that home's user
...
def catalog_source(name: str) -> Optional[str]:
    ...
    from nunmai_cli.plugin_catalog import get_live_catalog_entry
...
def migrate_home(home: Path, *, install: Callable[[str], dict], say: Callable[[str], None] = print) -> Optional[str]:
    ...
    if catalog_source(name) is None:
        say(f"  ⚠ Memory provider '{name}' is configured but not installed and not in the plugin catalog. "
            f"Install it with `nunmai plugins install <source>` or change memory.provider.")
        return None
    try:
        result = install(name)
    ...
    say(f"  ⚠ Memory provider '{name}' moved out of core and could not be installed automatically: "
        f"{result.get('error') or 'unknown error'}. Run `nunmai plugins install {name}`.")
    return None
...
def migrate_all_homes(*, say: Callable[[str], None] = print) -> list[str]:
    """``nunmai update`` hook: every profile home sharing this venv. Returns installed plugin names."""
    from nunmai_cli.plugin_python_deps import dependency_homes
    installed: list[str] = []
...
def recover_at_startup(name: str) -> bool:
    ...
    if not _allow_lazy_installs():
        logger.warning("Memory provider '%s' is not installed; security.allow_lazy_installs is off — "
                       "run `nunmai plugins install %s`.", name, name)
        return False
