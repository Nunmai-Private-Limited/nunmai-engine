...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`nunmai_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from nunmai_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `nunmai model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Nunmai device flow or Claude Code
    ...
    try:
        ...
        readers = (read_nunmai_oauth_credentials, read_claude_code_credentials)
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Nunmai overlay declares it keyless."""
    try:
        from nunmai_cli.providers import NUNMAI_OVERLAYS
        overlay = NUNMAI_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `nunmai model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, force_fresh_nous_tier: bool = False, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``; for Nunmai also
    ``free_tier`` (account is free-tier) and ``unavailable_models`` (paid models a free user can't pick).
    ``cached_only`` never hits the network: unknown Nunmai entitlement fails closed (``free_tier_pending``,
    ...
    for row in rows:
        ...
        if slug == "nous" and cached_only:
            ...
            if cached_nous_tier is None:
                ...
                if not row.get("warning"):  # say why every model renders locked
                    row["warning"] = ("Checking Nunmai plan entitlement… models unlock on the "
        ...
        for mid in models:
            ...
            # Sale chrome is Nunmai Portal-only (other catalogs' nested pricing.original is ignored); free
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        ...
        if not current:
            ...
            try:
                from nunmai_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from nunmai_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (nunmai_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="nunmai-picker-pricing-prewarm", daemon=True)
