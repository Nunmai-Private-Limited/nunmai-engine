...
def build_models_payload(
    ...
    *,
    ...
    try:
        from nunmai_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``nunmai_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`nunmai_cli.providers.custom_provider_aliases` — lets the frontend
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from nunmai_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `nunmai model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Nunmai' own Anthropic device flow
    ...
    try:
        ...
        nunmai_creds = read_nunmai_oauth_credentials() or {}
        if nunmai_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Nunmai.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Nunmai device flow / Claude Code) are
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`nunmai_cli.model_switch.list_picker_providers`) so the
