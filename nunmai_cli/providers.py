"""Single source of truth for provider identity in Nunmai Engine."""
...
# -- Nunmai overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class NunmaiOverlay:
    """Nunmai-specific provider metadata layered on top of models.dev."""
...
NUNMAI_OVERLAYS: Dict[str, NunmaiOverlay] = {
    "moa": NunmaiOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": NunmaiOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "nous": NunmaiOverlay(auth_type="oauth_device_code", base_url_override="https://inference-api.nousresearch.com/v1"),
    "openai-codex": NunmaiOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": NunmaiOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": NunmaiOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": NunmaiOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="NUNMAI_QWEN_BASE_URL"),
    "lmstudio": NunmaiOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": NunmaiOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": NunmaiOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": NunmaiOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": NunmaiOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": NunmaiOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": NunmaiOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": NunmaiOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": NunmaiOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": NunmaiOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": NunmaiOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": NunmaiOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": NunmaiOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": NunmaiOverlay(is_aggregator=True),
    "opencode": NunmaiOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": NunmaiOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "kilo": NunmaiOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": NunmaiOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": NunmaiOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": NunmaiOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": NunmaiOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": NunmaiOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": NunmaiOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": NunmaiOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": NunmaiOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": NunmaiOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": NunmaiOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": NunmaiOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": NunmaiOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": NunmaiOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": NunmaiOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": NunmaiOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": NunmaiOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": NunmaiOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "nunmai", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "moa": "Nunmai Agent", "nous": "Nunmai Portal", "openai-codex": "ChatGPT or Codex Subscription",
...
def _overlay_pdef(canonical, ov: NunmaiOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Nunmai overlay;
    Nunmai-only overlay (nous, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = NUNMAI_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or NunmaiOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "nunmai")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # NUNMAI_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def nous_api_mode(model: str = "") -> str:
    """Wire protocol for a Nunmai Portal model. Portal serves its ``anthropic/*`` catalog on a native
    ...
    Empty/unknown model defaults to ``chat_completions`` (the historical Nunmai transport)."""
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode (wire protocol) for a provider/endpoint: host-mandated mode, then Nunmai dual-wire
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Nunmai registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from nunmai_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
# below and the picker's Local row (``nunmai_cli/inventory.py``) — the two drifting apart is what
...
def _has_staged_local_models() -> bool:
    """True when GGUFs are staged under the Nunmai home's ``models/`` — the model the picker's Local
    row offers, which the runtime seam serves by booting/attaching a server on selection."""
    try:
        from nunmai_cli.local_runtime.bootstrap import staged_model_ids
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from nunmai_cli.config import load_config_readonly
        from nunmai_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
...
def resolve_provider_full(name: str, user_providers: Optional[Dict[str, Any]] = None,
    ...
    # and `nunmai model` must not reject them as unknown. Last rung, so every user-configured
