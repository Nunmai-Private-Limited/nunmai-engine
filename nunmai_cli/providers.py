...
2. **Nunmai overlays** — transport type, auth patterns, aggregator flags,
...
# -- Nunmai overlay ----------------------------------------------------------
# Nunmai-specific metadata that models.dev doesn't provide.
...
NUNMAI_OVERLAYS: Dict[str, NunmaiOverlay] = {
    ...
    "openai-codex": NunmaiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": NunmaiOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": NunmaiOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": NunmaiOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NUNMAI_QWEN_BASE_URL",
    ),
    "lmstudio": NunmaiOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": NunmaiOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": NunmaiOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": NunmaiOverlay(
    ...
    "kimi-for-coding": NunmaiOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": NunmaiOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": NunmaiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": NunmaiOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": NunmaiOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": NunmaiOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": NunmaiOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": NunmaiOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": NunmaiOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": NunmaiOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": NunmaiOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": NunmaiOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": NunmaiOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": NunmaiOverlay(
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Nunmai overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Nunmai overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + nunmai extra
    ...
    if overlay is not None:
        # Nunmai-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and NUNMAI_OVERLAYS, so
...
def nous_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Nunmai Portal model.
    ...
    historical Nunmai transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Nunmai Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Nunmai) whose
    ...
    # Nunmai is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Nunmai overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in NUNMAI_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Nunmai provider IDs must win over LOSSY alias collapsing.
