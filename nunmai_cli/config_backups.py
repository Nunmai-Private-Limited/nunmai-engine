...
``<NUNMAI_HOME>/backups/config/`` — ``backups/`` is already excluded from full backups, so they
...
``nunmai setup`` runs or a gateway restarting against broken YAML cannot litter the home dir.
"""
