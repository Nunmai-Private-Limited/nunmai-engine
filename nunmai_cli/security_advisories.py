...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``nunmai doctor --ack <id>``; the ack is persisted to
...
1. ``nunmai doctor`` (and ``nunmai doctor --ack <id>``)
...
run in environments where the rest of Nunmai failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.nunmai/.env (OpenRouter, Anthropic, OpenAI, "
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Nunmai requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.nunmai/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `nunmai doctor` on the gateway host for details.")
