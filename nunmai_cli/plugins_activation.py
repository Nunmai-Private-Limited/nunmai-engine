...
def activate_plugin_now(name: str, *, in_process: bool = True) -> Dict[str, Any]:
    ...
    in another process (``nunmai plugins install``) passes ``in_process=False``; the running Desktop /
    ...
    if not in_process:
        activation = (notify_serve_backend(name, Path(get_nunmai_home())) or {}).get("activation")
    answer = None
    try:
        ...
        if answer is None:
            root = Path(get_default_nunmai_root())
...
def _go_live(name: str) -> Optional[Dict[str, Any]]:
    from nunmai_cli.plugins_activation_live import connect_plugin_mcp, live_notice, plugin_skills
    try:
        from nunmai_cli.plugins import _join_background_discovery, get_plugin_manager
    ...
    if server is not None and (servers or note):
        from nunmai_constants import get_nunmai_home
        try:
            server.refresh_plugin_sessions(Path(get_nunmai_home()), note)
        except Exception:
...
def notify_serve_backend(name: str, home: Path) -> Optional[Dict[str, Any]]:
    """Ask the running dashboard / Desktop backend (``nunmai serve``, found through its host record) to
    ...
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json", "X-Nunmai-Session-Token": token})
