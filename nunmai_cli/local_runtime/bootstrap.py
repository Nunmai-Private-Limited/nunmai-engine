...
from nunmai_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Nunmai process
...
def _detect_gpu_vendor() -> str | None:
    ...
    from nunmai_cli.local_runtime.hardware import _nvidia_smi_path
...
def models_dir() -> Path:
    ...
    return get_default_nunmai_root() / "models"
...
def _presets_stale() -> bool:
    ...
    with suppress(Exception):
        from nunmai_cli.local_runtime.presets import read_preset_decisions
...
def _stop_state_server(state: dict) -> None:
    ...
    server."""
    from nunmai_cli.local_runtime.endpoint import _pid_alive
...
def _admitted_models_max(mdir: Path, configured: int) -> int:
    ...
    """
    try:
        from nunmai_cli.local_runtime.hardware import probe_budget
        from nunmai_cli.local_runtime.presets import admitted_residency_count
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from nunmai_cli.local_runtime.hardware import probe_budget
    from nunmai_cli.local_runtime.presets import generate_presets
...
def _cross_process_boot_lock(timeout_s: float = 130.0):
    """Serialize the state-check-then-spawn sequence across every Nunmai process on this
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Nunmai process may already be supervising — reuse via state, but ONLY while its
