"""NunmaiCLI constructor phases: display options, model/provider routing, turn limits, toolsets, checkpoints, prompt/reasoning, runtime state, session store and UI state.

Mixin split out of ``cli.py``; bound onto ``NunmaiCLI`` via the MRO.
"""
...
class CLIInitMixin:
    ...
    def _init_checkpoints_and_rules(self, checkpoints, pass_session_id, ignore_rules):
        ...
        self.ignore_rules = ignore_rules or is_truthy_value(os.environ.get("NUNMAI_IGNORE_RULES"))
