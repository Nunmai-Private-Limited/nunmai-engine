...
A pack is a single YAML file (``nunmai-pack.yaml``) that pins a set of
...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``nunmai plugins install`` (#64228).
    ...
    for entry in pack.plugins:
        ...
        try:
            if index_entries is None:
                from nunmai_cli.plugin_index import load_index

                index_entries, _src = load_index()
            from nunmai_cli.plugin_index import resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `nunmai skills install <id>`.[/dim]"
        )
    console.print(
...
def install_pack_plugins(
    ...
    *,
    ...
    single install (:func:`nunmai_cli.plugins_cmd._run_capability_consent`).
...
def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``nunmai plugins pack install <path-or-url>``.
...
def pack_command(args) -> None:
    ...
    if action == "install":
        ...
        Console().print(
            "[red]Error:[/red] Usage: nunmai plugins pack {install|export|show}"
        )
        sys.exit(1)
