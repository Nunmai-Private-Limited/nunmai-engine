"""Plugin catalog — curated, Nunmai-approved Nunmai plugins shipped with the repo.
...
an installed Nunmai sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://nunmai.in/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_nunmai": self.requires_nunmai,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_nunmai=str(data.get("requires_nunmai") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_nunmai_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``NUNMAI_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_nunmai_home(cache.parent)
...
def load_catalog_live() -> List[PluginCatalogEntry]:
    ...
    different pins the NEWER source supplies it — right after ``nunmai update`` bumps an in-tree pin,
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_nunmai:
        bits.append(f"Requires Nunmai {entry.requires_nunmai}.")
    return " ".join(bits)
