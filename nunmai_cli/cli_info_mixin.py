...
Mixin split out of ``cli.py``; bound onto ``NunmaiCLI`` via the MRO. cli.py-internal symbols are
...
from nunmai_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from nunmai_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Nunmai needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        if is_nous_nunmai_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with Nunmai Engine.[/]")
            self._console_print(
        ...
        # `nunmai skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `nunmai skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from nunmai_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from nunmai_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/nunmai_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from nunmai_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_nunmai_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_nunmai_home()}/config.yaml")
            print()
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from nunmai_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `nunmai auth` first.")
            return
    ...
    def _show_usage(self):
        ...
        The Nunmai credits block is agent-independent (portal fetch), so it runs even with no live
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.nunmai/skills/ and queue a note for the next user turn.
