...
Mixin on ``NunmaiCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from nunmai_cli.main import _print_version_info
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `nunmai plugins list`, so
            ...
            # on one line (full catalog behind `nunmai plugins list`).
            user_entries = [e for e in entries if e[3] != "bundled"]
            bundled_count = len(entries) - len(user_entries)
            if not user_entries:
                print("No user plugins installed.")
                print("  Install one: nunmai plugins install owner/repo")
                print(f"  Or drop a plugin directory into {display_nunmai_home()}/plugins/")
                if bundled_count:
                    print(f"  ({bundled_count} bundled plugins available — see: nunmai plugins list)")
                return
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: nunmai plugins list)")
            print("  Enable/disable: nunmai plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from nunmai_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from nunmai_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        _active_deleg = 0
        try:
            from nunmai_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
