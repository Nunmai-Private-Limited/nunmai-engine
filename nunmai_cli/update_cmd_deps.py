...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``nunmai.exe`` quarantine, ``os error 32``
...
def _npm_lockfile_changed(nunmai_root: Path) -> bool:
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(nunmai_root)
...
def _npm_lock_cache_file(nunmai_root: Path) -> Path:
...
def _record_npm_lockfile_hash(nunmai_root: Path) -> None:
    ...
    if digest is None:
        return
    try:
        _npm_lock_cache_file(nunmai_root).write_text(digest, encoding="utf-8")
    except OSError:
...
# canonical case: ``nunmai_cli.main`` used to import it at startup while resolving external secret sources;
