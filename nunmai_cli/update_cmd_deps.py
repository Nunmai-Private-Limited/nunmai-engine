"""Post-``nunmai update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``nunmai_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
from nunmai_constants import project_venv_dir, venv_python_path
...
def _editable_finder_files(venv: Path) -> list[Path]:
    ...
    return [
        ...
        for site in sites
        if site.is_dir()
        for finder in site.glob("__editable__*nunmai_agent*finder.py")
    ]
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    quarantines the running ``nunmai.exe``, and a lost race is the ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "nunmai_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from nunmai_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from nunmai_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__NUNMAI_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing nunmai_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from nunmai_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('nunmai_') or %r:\n"
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``nunmai tools`` dependencies (from a pre-rebuild probe) into a rebuilt
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Nunmai extras / ``LAZY_DEPS``), so the core reinstall can strip them;
...
def _reapply_plugin_python_dependencies() -> None:
    ...
    ``uv sync``/reinstall strips anything Nunmai' own lock does not know). Non-memory plugins whose
    deps no longer resolve are disabled loudly, memory providers last. Never raises."""
    from nunmai_cli.plugin_python_deps import reapply_all
    from nunmai_cli.update_cmd import _m
    ...
    for name, reason in report.dropped:
        print(f"  ✗ Plugin '{name}' DISABLED: {reason}. Fix the plugin's declared dependencies, "
              f"then `nunmai plugins enable {name}`.")
    if report.failed:
...
def _migrate_removed_memory_providers() -> None:
    ...
    try:
        from nunmai_cli.memory_provider_migration import migrate_all_homes
        migrate_all_homes()
...
def _npm_lockfile_changed(nunmai_root: Path) -> bool:
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    return not _npm_stamp_matches(nunmai_root, current)


def _npm_lock_cache_file(nunmai_root: Path, scope: str = "") -> Path:
    ...
    *scope* separates install closures that share the digest (workspace-scoped vs. full desktop)."""
    ...
    return nunmai_root / f".npm_lock_hash_{cache_key}{scope}"


def _npm_stamp_matches(nunmai_root: Path, current: str, scope: str = "") -> bool:
    """True when the recorded digest for *scope* equals *current*; a missing/unreadable stamp never matches."""
    try:
        return _npm_lock_cache_file(nunmai_root, scope).read_text(encoding="utf-8").strip() == current
    except OSError:
        return False


def _clear_npm_lockfile_hash(nunmai_root: Path, scope: str = "") -> None:
    ...
    with suppress(OSError):
        _npm_lock_cache_file(nunmai_root, scope).unlink()


def _record_npm_lockfile_hash(nunmai_root: Path, scope: str = "") -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(nunmai_root, scope).write_text(digest, encoding="utf-8")
    except OSError:
...
def _desktop_deps_changed(nunmai_root: Path) -> bool:
    ...
    current = _npm_manifests_digest()
    return current is None or not _npm_stamp_matches(nunmai_root, current, DESKTOP_NPM_SCOPE)
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run nunmai
...
def _update_node_dependencies() -> list[str]:
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `nunmai update` case.
    # See #43564.
    ...
    # The chatty npm-deprecation noise during `nunmai update` comes from the *desktop* build, not this step;
    # that one is captured to update.log. See #18840.
    _clear_npm_lockfile_hash(shared_nunmai_root)
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".nunmai-bootstrap-complete", _m()._update_marker_path())
...
def _venv_dependency_set_stale() -> tuple[bool, str]:
    """Whether the venv's ``nunmai-engine`` distribution was installed from an OLDER checkout than
    ...
    from nunmai_cli.update_cmd import _m, _read_project_version
    ...
    try:
        result = subprocess.run(
            [str(venv_python), "-c", "import importlib.metadata as m; print(m.version('nunmai-engine'))"],
    ...
    return stale, f"installed nunmai-engine {installed}, checkout is {expected}" if stale else ""
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``nunmai.exe`` shim
    ...
    if _m()._reexec_dependency_sync_off_windows_shim(gateway_resume):
        ...
        # (and nunmai.exe locked) exactly while the child needs it gone (#101600).
        sys.exit(0)
...
def _rebuild_desktop_after_update(
    ...
    # build stamp under NUNMAI_HOME: it outlives a swap that lost the artifacts in an earlier run, and
    ...
    # rebuild window), then surface the tail. Put Nunmai-managed Node on PATH: the desktop
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo nunmai`` dies mid-update with ``venv/bin/nunmai`` already
    ...
    See #83529.
    A later normal ``nunmai update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/nunmai`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from nunmai_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Nunmai broken.")
    print("  This usually happens after running nunmai or pip with sudo. Offending paths:")
