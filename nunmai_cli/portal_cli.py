...
already in ``nunmai auth`` or ``nunmai tools``. It's the onboarding + discovery
...
def portal_command(args) -> int:
    ...
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `nunmai portal` is the
        # human-readable alias for `nunmai auth add nous --type oauth` /
        # `nunmai setup --portal`.
