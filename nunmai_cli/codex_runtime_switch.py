...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Nunmai'
...
The actual runtime resolution happens in nunmai_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "nunmai"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, nunmai-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Nunmai' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Nunmai for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from nunmai_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the nunmai-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "nunmai-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Nunmai tool callback are always-on production
            ...
            if "nunmai-tools" in mig_report.migrated:
                msg_lines.append(
