...
:mod:`nunmai_cli.runtime_provider`; origin-internal collaborators
...
logger = logging.getLogger("nunmai_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``nunmai_cli.runtime_provider.*`` apply."""
    import nunmai_cli.runtime_provider as origin
    return origin
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from nunmai_cli.models import normalize_opencode_base_url, opencode_model_api_mode
