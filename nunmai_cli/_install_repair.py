...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``nunmai_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``nunmai_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
def _stdout_to_stderr():
    ...
    ``nunmai acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (nunmai_constants is stdlib-only)."""
    # nunmai_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Nunmai root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
...
def _default_nunmai_root() -> Path | None:
    """The DEFAULT Nunmai root (not ``get_nunmai_home()``, which under ``nunmai -p <name>`` is
    ...
    try:
        return Path(get_default_nunmai_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``nunmai`` launchers when they vanish.

    On Windows, ``nunmai`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Nunmai root's ``bin``
    (``%LOCALAPPDATA%\\nunmai\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_nunmai_home()``
    would point inside ``profiles\\<name>`` under ``nunmai -p``, so the anchor here is
    :func:`nunmai_constants.get_default_nunmai_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_nunmai_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every nunmai_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored nunmai launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``NUNMAI_HOME\\bin`` launcher layout (``nunmai update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ nunmai launchers now resolve from {home_bin} "
...
def _quarantine_running_nunmai_exe(
    ...
    names = set(_load_console_script_names(scripts_dir.parent.parent)) or {
        "nunmai", "nunmai-engine", "nunmai-acp"}
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``nunmai acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_nunmai_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `nunmai` gone from PATH. Restore only renames back when the
