"""Telegram Managed Bot onboarding client: creates a user-owned child bot via the Nunmai onboarding
...
# Nunmai-hosted pairing API; override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.nunmai.in"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Nunmai Engine"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "NunmaiSetupBot",
    ...
    print(f"\n  Contacting Nunmai Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Nunmai Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "NunmaiSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``nunmai_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``nunmai_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"nunmai_{generate_username_slug()}_bot"
