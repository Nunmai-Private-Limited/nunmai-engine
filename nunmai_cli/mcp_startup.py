...
# Discovery slot per profile home (``nunmai_home_key()`` follows the context-local NUNMAI_HOME
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # the context-local NUNMAI_HOME override (multi-profile dashboard/desktop backends, #67605)
...
def defer_background_mcp_discovery(*, logger, thread_name: str, delay: float | None) -> None:
    ...
    with _mcp_discovery_lock:
        if nunmai_home_key() in _mcp_discovery_started or _mcp_discovery_deferred is not None:
            return
...
def _current_home_thread() -> Optional[threading.Thread]:
    ...
    return _mcp_discovery_thread.get(nunmai_home_key())
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``nunmai -z``) can construct ``AIAgent`` before any
