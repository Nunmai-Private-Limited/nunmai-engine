...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError("Not logged into Nunmai Portal. Run `nunmai auth add nous` first.")
            try:
                ...
                raise RuntimeError(f"Failed to refresh Nunmai Portal credentials: {exc}") from exc
    ...
    # auth.json access — kept local so nunmai_cli.auth's public surface does not grow.
