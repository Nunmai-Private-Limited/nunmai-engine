...
# xAI's public API is OpenAI-compatible for the endpoints Nunmai commonly
# uses. The Responses endpoint is included because Nunmai' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Nunmai-managed OAuth credentials."""

    auth_hint = "nunmai auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nunmai auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nunmai auth reset xai-oauth` or re-authenticate with "
                    "`nunmai auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nunmai auth add xai-oauth --type oauth`."
            )
