"""``nunmai doctor`` — diagnose (and with --fix, repair) a Nunmai install.
...
from nunmai_cli.config import get_env_path, get_nunmai_home, get_project_root
...
from nunmai_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from nunmai_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from nunmai_cli.doctor_tools import _safe_which

from nunmai_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from nunmai_cli.doctor_platform import (
    _check_certificates,
...
from nunmai_cli.doctor_tools import (
    _check_git_and_rg,
...
from nunmai_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from nunmai_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nunmai Portal auth", get_nous_auth_status_local())
        # Native OAuth is Nunmai' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def _ack_advisory(ack_target: str) -> None:
    """`nunmai doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from nunmai_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Could not save the acknowledgement for {ack_target}. Make sure {_DHH}/config.yaml is "
                    f"writable (`nunmai config path` prints the exact file), then re-run "
                    f"`nunmai doctor --ack {ack_target}`.", Colors.RED))
        sys.exit(1)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('nunmai_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('nunmai_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('nunmai_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('nunmai_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('nunmai_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('nunmai_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('nunmai_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('nunmai_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('nunmai_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('nunmai_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('nunmai_cli.vercel_auth', 'describe_vercel_auth'),
    ...
    'managed_scope_check': ('nunmai_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('nunmai_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('nunmai_cli.doctor_config', 'report_deprecated_config_and_env'),
}
