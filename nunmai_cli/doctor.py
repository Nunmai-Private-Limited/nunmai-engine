"""
Doctor command for nunmai CLI.

Diagnoses issues with Nunmai Engine setup.
"""
...
from nunmai_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
NUNMAI_HOME = get_nunmai_home()
_DHH = display_nunmai_home()  # user-facing display path (e.g. ~/.nunmai or ~/.nunmai/profiles/coder)

# Load environment variables from ~/.nunmai/.env so API key checks work
_env_path = get_env_path()
load_nunmai_dotenv(nunmai_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from nunmai_cli.colors import Colors, color
from nunmai_cli.models import _NUNMAI_USER_AGENT
from nunmai_cli.vercel_auth import describe_vercel_auth
from nunmai_constants import OPENROUTER_MODELS_URL
...
from nunmai_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Nunmai containers"
...
def _nunmai_database_paths(nunmai_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Nunmai-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from nunmai_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, nunmai_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((nunmai_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(nunmai_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``nunmai_cli.sqlite_safe_read``).
    ...
    """
    from nunmai_cli.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from nunmai_cli.backup import _format_size
...
def _report_database_journal_modes(
    nunmai_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from nunmai_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = nunmai_home if nunmai_home is not None else NUNMAI_HOME
    try:
        databases = _nunmai_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Nunmai databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.nunmai/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("NUNMAI_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from nunmai_cli.auth import get_minimax_oauth_auth_status
...
from nunmai_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'nunmai sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'nunmai sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. NUNMAI_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("NUNMAI_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("NUNMAI_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Nunmai Relay plugin."""
    from nunmai_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches nunmai_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``nunmai --version`` reporting a stale version while
    ...
    """
    try:
        from nunmai_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != nunmai_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'nunmai update', or set "
            "nunmai_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-nunmai and dashboard static services are up
    ...
    for static in ("main-nunmai", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `nunmai profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `nunmai doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from nunmai_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the NUNMAI_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("NUNMAI_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via NUNMAI_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.nousresearch.nunmai"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `nunmai update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (nunmai desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `nunmai desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.nousresearch.nunmai` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Nunmai once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Nunmai.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Nunmai.app``
    is deliberately not probed: it is the separately-signed Nunmai-Setup
    launcher (``com.nousresearch.nunmai.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Nunmai.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from nunmai_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Nunmai will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Nunmai.app if you use Desktop), "
        "and restart them once. With Nunmai' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `nunmai`.
    os.environ.setdefault("NUNMAI_INTERACTIVE", "1")

    # Handle `nunmai doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from nunmai_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.nunmai/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Nunmai Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`nunmai doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from nunmai_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Nunmai already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and nunmai_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.nunmai/.env (primary location for user config)
    env_path = NUNMAI_HOME / '.env'
    if env_path.exists():
        ...
        # matches nunmai_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nunmai setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nunmai setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'nunmai setup' to create one")
                issues.append("Run 'nunmai setup' to create .env")
    
    # Check ~/.nunmai/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = NUNMAI_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from nunmai_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from nunmai_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'nunmai config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.nunmai/.env or run 'nunmai setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nunmai setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nunmai config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from nunmai_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'nunmai setup' to migrate config")
                else:
                    issues.append("Run 'nunmai doctor --fix' or 'nunmai setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from nunmai_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'nunmai doctor --fix'")
        except Exception:
            pass

        # Detect stale NUNMAI_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives NUNMAI_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from nunmai_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            if drift:
                check_warn(
                    f"NUNMAI_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `nunmai setup` run)",
                )
                if should_fix:
                    if remove_env_value("NUNMAI_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale NUNMAI_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove NUNMAI_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the NUNMAI_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale NUNMAI_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'nunmai doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from nunmai_cli.config import load_env as _load_env_depr
            from nunmai_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Nunmai' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = nunmai_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Nunmai a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Nunmai Engine Persona\n\n"
                "<!-- Edit this file to customize how Nunmai communicates. -->\n\n"
                "You are Nunmai, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = nunmai_home / "memories"
    ...
    state_db_path = nunmai_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from nunmai_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'nunmai doctor --fix' "
                        "(or 'nunmai sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from nunmai_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'nunmai doctor --fix' "
                        "(or 'nunmai sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Nunmai
        ...
        try:
            from nunmai_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'nunmai sessions optimize-storage' "
    ...
    wal_path = nunmai_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'nunmai doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nunmai"
        ...
        _cmd_link = _cmd_link_dir / "nunmai"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(nunmai not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/nunmai → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/nunmai points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/nunmai → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/nunmai — run 'nunmai doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/nunmai exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/nunmai not found",
                    "(nunmai command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/nunmai → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/nunmai symlink — run 'nunmai doctor --fix'")
    ...
    try:
        from nunmai_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'nunmai-engine[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Nunmai-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `nunmai update` does, so a session's first browser call
            ...
            # `nunmai update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `nunmai doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable NUNMAI_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Nunmai one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'nunmai config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``nunmai doctor``.
    ...
    # never the legitimate source for `nunmai doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'nunmai setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = NUNMAI_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: nunmai skills list)")
    ...
    try:
        from nunmai_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = NUNMAI_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: nunmai memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: nunmai memory setup",
                    "No Honcho API key — run 'nunmai memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run nunmai memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: nunmai memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: nunmai memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from nunmai_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "nunmai -p" in content:
                            _m = _re.search(r"nunmai -p (\S+)", content)
    ...
    # `nunmai doctor --live` (real network calls; bounded + read-only).
    try:
        from nunmai_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'nunmai doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
