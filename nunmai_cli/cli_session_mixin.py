...
Mixin bound onto ``NunmaiCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from nunmai_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_status(self):
        ...
        try:
            ...
            separator_color = skin.get_color("banner_dim", "#2E9C7C")
            accent_color = skin.get_color("ui_accent", "#3FCFA9")
            label_color = skin.get_color("ui_label", "#D6B36C")
        except Exception:
            separator_color, accent_color, label_color = "#2E9C7C", "#3FCFA9", "cyan"
    ...
    def _show_session_status(self):
        ...
        from nunmai_cli.status_report import build_status_fields, status_lines
        session_meta = {}
        if self._session_db:
        ...
        lines = ["Nunmai CLI Status", "", *status_lines(fields, "session_id", "path", "title", "model")]
        try:
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from nunmai_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from nunmai_cli.timefmt import relative_time as _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Nunmai #{visible_index}]{_ts_suffix(msg)}")
    ...
    def new_session(self, silent=False, title=None):
        ...
        from nunmai_cli.cli_model_switch_mixin import _resolve_cli_reasoning
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        saved_dir = get_nunmai_home() / "sessions" / "saved"
        try:
        ...
        if filename:
            ...
            path = saved_dir / f"nunmai_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve NUNMAI_SESSION_ID to the child id after an
            # out-of-place rotation (idempotent when no rotation happened).
            if self.session_id:
                print(f"       Resume the live session with: nunmai --resume {self.session_id}")
        except Exception as e:
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``nunmai -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from nunmai_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``NUNMAI_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / NUNMAI_YOLO_MODE)."
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        # the hint ("default"/"custom" use the standard NUNMAI_HOME).
        try:
        ...
        print(f"  nunmai --resume {self.session_id}{profile_flag}")
        if session_title:
            print(f"  nunmai -c \"{session_title}\"{profile_flag}")
        print()
