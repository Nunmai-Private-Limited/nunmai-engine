"""Nunmai Portal OAuth: device-code login, refresh, shared-store mirroring, JWT selection, status.

Split out of ``nunmai_cli/auth.py``; origin helpers are imported lazily inside each function
so ``nunmai_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from nunmai_cli.auth_codex import _pool_entries
from nunmai_cli.auth_constants import (
...
if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from nunmai_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "nunmai_cli.auth").
logger = logging.getLogger("nunmai_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: nunmai auth add nous"
...
def _unusable_invoke_jwt_error(reason: str, *, no_refresh_token: bool = False) -> AuthError:
    ...
    return _nous_err(
        f"Nunmai Portal access token is not a usable inference JWT ({reason}){detail}. "
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("NUNMAI_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
# Nunmai agent-key slots; a fresh login persists them as None, quarantine strips them.
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    with suppress(Exception):
        if message := _portal_entitlement_message("Nunmai model access"):
            return message
...
# Allowlist of hosts the Nunmai Portal proxy will forward inference JWTs to — a bearer sent anywhere
...
def _nous_inference_host_allowed(hostname: Optional[str]) -> bool:
    ...
    its own — not even for a Nunmai-owned host: the operator's explicit override is the authority,
...
def _scoped_operator_override(*names: str) -> Optional[str]:
    ...
    ``NUNMAI_PORTAL_BASE_URL`` / its ``NOUS_PORTAL_BASE_URL`` alias), resolved through the profile
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    """
    from nunmai_cli.auth import _optional_base_url
    return _optional_base_url(_scoped_operator_override("NUNMAI_PORTAL_BASE_URL", "NOUS_PORTAL_BASE_URL"))
...
def _nous_invoke_jwt_status(
    ...
    from nunmai_cli.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from nunmai_cli.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from nunmai_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from nunmai_cli.auth import _nonempty_str
...
def _nous_shared_auth_dir() -> Path:
    """Directory of the shared Nunmai token store: ``NUNMAI_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``nunmai --profile X auth add nous --type
...
def _nous_shared_store_path() -> Path:
    ...
    # user's Nunmai root so a test that forgot NUNMAI_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nunmai_constants import get_default_nunmai_root
        real_home_shared = (
            get_default_nunmai_root() / "shared" / NOUS_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    """
    from nunmai_cli.auth import _file_lock
    try:
        lock_path = _nous_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No NUNMAI_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
...
# OAuth fields mirrored between a profile's Nunmai state and the shared cross-profile store.
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from nunmai_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    """
    from nunmai_cli.auth import _nonempty_str, _save_private_json
...
def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    """Shared Nunmai OAuth state when present and well-formed, else None.
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    provenance is client_id + agent_key_id (Nunmai state has no session_id).
    """
...
def _try_import_shared_nous_state(*, timeout_seconds: float = 15.0) -> Optional[Dict[str, Any]]:
    """Rehydrate Nunmai OAuth state from the shared store via a forced refresh.
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    # self-heal hook) redeemed Nunmai's refresh_token without persisting the rotated token, so the
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from nunmai_cli.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    """Write a successful Nunmai token-refresh payload into *state* (tokens + expiry fields).
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from nunmai_cli.auth import _coerce_ttl_seconds
...
def fetch_nous_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    for item in data:
        ...
        # Nunmai models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "nunmai" not in model_id.lower():
...
def refresh_nous_oauth_from_state(
    ...
    on_state_update: Optional[Callable[[Dict[str, Any], str], None]] = None) -> Dict[str, Any]:
    """Refresh Nunmai OAuth from a state dict (defaults filled in) without mutating auth.json."""
    from nunmai_cli.auth import (
    ...
    with _nous_http_client(timeout_seconds or 15.0, verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise _nous_err(
                    "No refresh token is available for Nunmai Portal.", "nous_auth_missing_refresh_token",
                    relogin=True)
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    ...
    Nunmai credentials are read from ``providers.nous`` (401 recovery, pool seeding) AND
    ...
    it.
    """
    from nunmai_cli.auth import _save_active_provider_state, _write_shared_nous_state
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from nunmai_cli.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
    ...
    from nunmai_cli.anon_auth import is_guest_state
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from nunmai_cli.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def resolve_nous_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """Resolve Nunmai inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    """
    from nunmai_cli.anon_auth import AnonCredentialDead, clear_dead_guest, ensure_portal_identity
    try:
        ...
        from nunmai_cli.anon_auth import ANON_ACCOUNT_LOCKED
...
def _resolve_nous_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    if not isinstance(api_key, str) or not api_key:
        raise _nous_err("Failed to resolve a Nunmai inference API key", "server_error")
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    """
    from nunmai_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("nous")
...
def _nous_status_from_state(
    ...
    """Auth-store-backed Nunmai status snapshot (shared by the live and refresh-free variants)."""
    from nunmai_cli.anon_auth import is_guest_state
    access_token = state.get("access_token")
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _pool_first_oauth_status(
    ...
    Pool first (where `nunmai auth` / `nunmai model` store device_code tokens), then
...
def _nous_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    portal_base_url = (
        portal_base_url or os.getenv("NUNMAI_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from nunmai_cli.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    """Fetch the curated Nunmai model list (tier/policy-filtered) and run the interactive picker.
    ...
    from nunmai_cli.models_pricing import (
    ...
    if model_ids:
        ...
        if free_tier:
            with suppress(Exception):
                unavailable_message = _portal_entitlement_message("paid Nunmai models")
...
def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    """Codex-style auto-import: offer to rehydrate a Nunmai credential from another profile.
    ...
    """
    from nunmai_cli.auth import _prompt_yes_no, _read_shared_nous_state
    from nunmai_cli.anon_auth import is_guest_state
    shared = _read_shared_nous_state()
    ...
    try:
        ...
        shared_path = None
    print()
    print(f"Found existing Nunmai OAuth credentials at {shared_path}" if shared_path
          else "Found existing shared Nunmai OAuth credentials")
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    ...
    from nunmai_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("NUNMAI_CA_BUNDLE")
    ...
    try:
        ...
        # switch to Nunmai with a mismatched model; the Nunmai tokens stay saved for future use.
        ...
        from nunmai_cli.auth_error_copy import sign_in_failure_lines
