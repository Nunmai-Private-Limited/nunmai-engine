...
tests/nunmai_cli/test_commands_execute.py).
"""
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    home_display = str(ctx.options.get("home_display") or "").strip() or display_nunmai_home()
    # Presentation-only display name (profile.yaml); `data.profile` stays the canonical id.
    label = profile_name
    try:
        from nunmai_cli.profiles import format_profile_label, get_profile_dir, read_profile_meta
