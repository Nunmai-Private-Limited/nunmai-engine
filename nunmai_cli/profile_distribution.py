"""Profile distributions — shareable, packaged Nunmai profiles via git.
...
def _is_distribution_runtime_path(parts: Tuple[str, ...]) -> bool:
    ...
    # Root-level dot entries under skills are Nunmai bookkeeping (.hub,
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            nunmai_requires=_str(data, "nunmai_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("nunmai_requires", self.nunmai_requires),
...
def check_nunmai_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Nunmai {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Nunmai profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from nunmai_cli.profiles import _canon_valid, get_profile_dir
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Nunmai distribution."
        )
    check_nunmai_requires(manifest.nunmai_requires, nunmai_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.nunmai).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _merge_cron_store(src: Path, home: Path) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="nunmai_dist_cron_") as tmp:
...
def _copy_dist_payload(staged: Path, target: Path, manifest: DistributionManifest, preserve_config: bool) -> None:
    ...
    if any(rel_parts == ("profile.yaml",) for _, rel_parts in entries):
        from nunmai_cli.profiles import drop_profile_role
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from nunmai_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from nunmai_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="nunmai_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nunmai profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from nunmai_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nunmai profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="nunmai_dist_update_") as tmp:
