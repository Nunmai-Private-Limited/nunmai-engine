...
A distribution is a Nunmai profile published as a git repository (or
...
* ``nunmai profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nunmai skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nunmai profile``, not a parallel tree):

    nunmai profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nunmai_requires=str(data.get("nunmai_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nunmai_requires:
            out["nunmai_requires"] = self.nunmai_requires
        if self.author:
...
def check_nunmai_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nunmai {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nunmai profile update`` can
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nunmai distribution."
        )

    # Version check up-front so we fail fast
    check_nunmai_requires(manifest.nunmai_requires, nunmai_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nunmai).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="nunmai_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nunmai profile update` to upgrade in place, "
