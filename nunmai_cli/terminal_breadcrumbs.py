"""Per-terminal session breadcrumbs for ``nunmai -c`` / ``--continue``. Strictly best-effort: no
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_nunmai_home() / "terminal-sessions"
...
def write_breadcrumb(session_id: str, cwd: Optional[str] = None) -> None:
    ...
    try:
        ...
        mkdir_under_nunmai_home(directory)
        now = time.time()
