"""Helpers for loading Nunmai .env files consistently across entrypoints."""
...
# Once-per-process guards: load_nunmai_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `nunmai
...
# NUNMAI_HOME paths already pulled external secrets for: load_nunmai_dotenv() runs at import time from
...
# a link-style launcher spawns `nunmai dashboard` with a fresh NUNMAI_DASHBOARD_SESSION_TOKEN and keeps
# the same token for its own /api probes). They are never .env configuration, so a persisted value in
# ~/.nunmai/.env must not replace an injected one — the parent would then 401 against its own child
# (#115955). A value an earlier dotenv pass published is still reloaded normally.
_SPAWN_CREDENTIAL_KEYS: frozenset[str] = frozenset({"NUNMAI_DASHBOARD_SESSION_TOKEN"})

# Behavioral routing keys a parent Nunmai process injects into child env that silently redirect a profile
...
def get_secret_source_values(nunmai_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``nunmai_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(nunmai_home).resolve()), {}))


def hydrate_profile_secret_sources(nunmai_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(nunmai_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_nunmai_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["NUNMAI_HOME"] = str(home)
...
def reset_secret_source_cache(nunmai_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``nunmai_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if nunmai_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        _SECRET_SOURCE_RESTORE_BY_HOME.clear()
        return
    home_key = str(Path(nunmai_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `nunmai setup` (or edit the "
...
def _load_dotenv_with_fallback(path: Path, *, override: bool, load_pass: int | None = None) -> None:
    ...
    ``load_pass`` groups the layered files of one ``load_nunmai_dotenv`` call: within a pass a later layer
    (project, managed) still sees the earlier layer's output, as it always did; only OTHER passes' output
    is peeled. A bare call (``nunmai send``'s direct reload) is its own pass."""
    raw = path.read_bytes()
    try:
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from nunmai_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_nunmai_dotenv(
    *,
    nunmai_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Nunmai env files: ``~/.nunmai/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(nunmai_home) if nunmai_home else get_process_nunmai_home()
    ...
    if is_multiplex_active() and get_nunmai_home_override() is not None:
        ...
        if load_external_secrets:
            from nunmai_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``nunmai update`` retry just completed a deferred dependency
    ...
    # ~/.nunmai/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.nunmai/.env (e.g. written by an older
    # `nunmai setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_nunmai_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_nunmai_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Nunmai reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and nunmai_logging reuse one parse instead of 3-4.
    if home_path == _process_nunmai_home():
        try:
...
def _process_nunmai_home() -> Path:
    """The NUNMAI_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_nunmai_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``nunmai_constants.get_routing_process_nunmai_home()`` is the override-immune
    resolver built for exactly this; delegate to it. It is also immune to a host
    that mirrors the served profile into the live ``NUNMAI_HOME`` env var
    (``pin_process_nunmai_home``): without that, the mirrored profile satisfied
    the guard above and bridged ITS ``terminal.*`` into the shared env.
    """
    try:
        from nunmai_constants import get_routing_process_nunmai_home

        return get_routing_process_nunmai_home()
    except Exception:
