...
# load_nunmai_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_nunmai_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_nunmai_dotenv().
# Used by setup / `nunmai model` flows to label detected credentials so
...
# NUNMAI_HOME paths we've already pulled external secrets for during this
# process.  ``load_nunmai_dotenv()`` is called at module-import time from
# several hot modules (cli.py, nunmai_cli/main.py, run_agent.py,
...
def _known_nunmai_env_keys() -> set[str]:
    """Return the combined set of known Nunmai env-var keys.
    ...
    ``load_nunmai_dotenv()`` calls.
    """
    from nunmai_cli.config import _EXTRA_ENV_KEYS
    from nunmai_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Nunmai process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Nunmai keys absent from ``.env``.
    ...
    parent Nunmai process injects and that silently change *which provider
    ...
    ``tests/nunmai_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_nunmai_dotenv()`` call.  Returns
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_nunmai_dotenv()'s .op.env bootstrap: the 1Password
...
def reset_secret_source_cache() -> None:
    """Forget which NUNMAI_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``nunmai_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_nunmai_dotenv(
    *,
    ...
    """Load Nunmai environment files with user config taking precedence.

    Behavior:
    - `~/.nunmai/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(nunmai_home or os.getenv("NUNMAI_HOME", Path.home() / ".nunmai"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Nunmai keys
    ...
    #   EnvironmentFile=-/path/to/.nunmai/.op.env
    ...
    # 2. A fresh ``nunmai update`` retry just completed a deferred dependency
    ...
    # TERMINAL_ENV=docker left in ~/.nunmai/.env (e.g. written by an older
    # `nunmai setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_nunmai_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``nunmai_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process NUNMAI_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_nunmai_dotenv(nunmai_home=...)`` call would bridge the wrong
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of NUNMAI_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Nunmai reads
    ...
    owns the once-per-NUNMAI_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_nunmai_dotenv()`` runs at import
    time from several hot modules (cli.py, nunmai_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_nunmai_dotenv() call instead of never.
        return
    ...
    # a generic dict entry must not force crypto load on every nunmai launch.
    ...
    # so the 3-5 import-time load_nunmai_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `nunmai model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `nunmai` startup, so populating the
    # shared cache here lets main.py's early bridge and nunmai_logging reuse
...
def _process_nunmai_home() -> Path:
    """The NUNMAI_HOME the shared config cache is keyed to."""
    try:
