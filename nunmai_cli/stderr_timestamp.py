...
def _is_nunmai_gateway_run_argv(command: Sequence[str]) -> bool:
    ...
    The wrapper is generic. Only historical/current Nunmai gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``nunmai update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_nunmai_gateway_run_argv(argv):
        return argv
