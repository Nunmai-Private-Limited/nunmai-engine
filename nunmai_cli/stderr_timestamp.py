...
def _install_signal_forwarders(proc: subprocess.Popen[bytes]) -> dict[int, object]:
    ...
    # so `nunmai update` signals us, not the gateway; an unforwarded SIGUSR1 kills the wrapper
...
def _is_nunmai_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Nunmai ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Nunmai gateway shapes get ``--external-
...
def _child_launchd_label_env(environ: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    via ``gateway.restart.launchd_job_label``). Only ``ai.nunmai.*`` labels are exported;
...
def _prepare_child_command(command: Sequence[str], environ: Mapping[str, str] | None = None) -> list[str]:
    ...
    plists put ``--external-supervisor`` on the inner ``gateway run`` so ``nunmai update`` can see
    the flag on the live process argv.
    """
    argv = [str(part) for part in command]
    env = os.environ if environ is None else environ
    xpc_service = str(env.get("XPC_SERVICE_NAME", "")).strip()
    if EXTERNAL_SUPERVISOR_FLAG not in argv and xpc_service and xpc_service != "0" and _is_nunmai_gateway_run_argv(argv):
...
def _child_returncode_for_supervisor(command: Sequence[str], returncode: int) -> int:
    ...
    if returncode == GATEWAY_FATAL_CONFIG_EXIT_CODE and _is_nunmai_gateway_run_argv(command):
