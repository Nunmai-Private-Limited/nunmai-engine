...
Usage:
    nunmai claw migrate              # Preview then migrate (always shows preview first)
    nunmai claw migrate --dry-run    # Preview only, no changes
    nunmai claw migrate --yes        # Skip confirmation prompt
    nunmai claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    nunmai claw migrate --no-backup  # Skip pre-migration snapshot
    nunmai claw cleanup              # Archive leftover OpenClaw directories
    nunmai claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Nunmai gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Nunmai gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'nunmai gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Nunmai")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'nunmai claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Nunmai migration."""
    ...
    # migrate-nunmai posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: nunmai claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: nunmai claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Nunmai home ─────────
    # Delegates to nunmai_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `nunmai import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from nunmai_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(nunmai_home=nunmai_home)
            if backup_archive:
    ...
    # 'nunmai claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: nunmai claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: nunmai claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  nunmai claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  nunmai claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  nunmai config set OPENROUTER_API_KEY sk-or-v1-...")
