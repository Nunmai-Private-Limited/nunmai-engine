...
Source installs report their git revision live via ``git rev-parse`` (``nunmai_cli/dump.py``,
``nunmai_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.nunmai_build_sha`` file written by the Dockerfile.
"""
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.nunmai_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``NUNMAI_GIT_SHA`` build-arg, full 40-char hash on one
