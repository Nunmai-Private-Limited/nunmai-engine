...
def warn_pending_manual_serves(*, startup: bool = False, pending_manual: list[dict] | None = None) -> None:
    ...
    for row in pending_manual:
        ...
        if type(detail.get("create_time")) in (int, float):
            print("    Ask its owner to relaunch `nunmai serve` / `nunmai dashboard`; check reminder storage permissions and free space.", file=stream)
        else:
            # No usable creation time means identity, not storage, blocked the durable reminder.
            print("    This host could not read the process creation time, so no durable reminder could be filed; ask its owner to relaunch `nunmai serve` / `nunmai dashboard`, and the warning clears once the pid is confirmed gone.", file=stream)
    directory = get_nunmai_home() / "serve_restart_pending"
    for path in sorted(directory.glob("*.json")):
        try:
            ...
            print("    Ask its owner to relaunch `nunmai serve` / `nunmai dashboard` (reconnect Desktop for an SSH backend).", file=stream)
