"""nunmai fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `nunmai model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_add(args) -> None:
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `nunmai model` — select the provider + model you want as a fallback.\n")
