"""
nunmai fallback — manage the fallback provider chain.
...
Subcommands:
  nunmai fallback [list]   Show the current fallback chain (default when no subcommand)
  nunmai fallback add      Pick provider + model via the same picker as `nunmai model`,
                           then append the selection to the chain
  nunmai fallback remove   Pick an entry to delete from the chain
  nunmai fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nunmai/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `nunmai model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nunmai fallback add")
        print()
        return
    ...
    print("  Docs: https://nunmai.in/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nunmai model`, then append the selection to the chain."""
    from nunmai_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nunmai model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nunmai fallback list` to view, or `nunmai fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nunmai fallback [subcommand]``."""
