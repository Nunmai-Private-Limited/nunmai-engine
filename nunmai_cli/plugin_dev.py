"""Runtime-backed validation behind ``nunmai plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary NUNMAI_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="nunmai-plugin-doctor-")
    stack = ExitStack()
    ...
    modules_before = {
        ...
        if name == "nunmai_plugins" or name.startswith("nunmai_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Nunmai discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "nunmai_plugins" or name.startswith("nunmai_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_nunmai_home() / "plugins"
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Nunmai never auto-installs plugin dependencies; "
