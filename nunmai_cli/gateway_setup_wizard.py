"""Gateway platform setup wizard (nunmai gateway setup): platform registry/status table, per-platform setup prompts, service offer.
...
import subprocess
import sys
from nunmai_cli.setup import print_success  # def-time binding (table value)
from nunmai_cli.setup import print_warning  # def-time binding (table value)
...
def _prompt_unauthorized_access(platform_key: str) -> None:
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Approve with: nunmai pairing approve <platform> <code>")
    elif access_idx == 2:
        ...
        _gw().print_info("  Skipped — configure later with 'nunmai gateway setup'")
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        _gw().print_info("  Install them, then rerun `nunmai gateway setup`.")
        return
    ...
    try:
        credentials = _gw().asyncio.run(qr_login(str(_gw().get_nunmai_home())))
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        _gw().print_info("  Unknown users can request access; approve with `nunmai pairing approve`.")
    elif access_idx == 1:
...
def _configure_platform(platform: dict) -> None:
    ...
    if required:
        _gw().print_info(f"  Set these env vars in ~/.nunmai/.env: {', '.join(required)}")
    else:
...
def _setup_service_action(
    ...
    try:
        ...
        if backend is not None:
            ...
            _gw().print_info("Start manually: nunmai gateway")
...
def _wizard_service_status_block() -> None:
    ...
    if _gw().supports_systemd_services() and _gw().has_legacy_nunmai_units():
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        _gw().print_info("  You can install later: nunmai gateway install")
        if _gw().supports_systemd_services():
            _gw().print_info("  Or as a boot-time service: sudo nunmai gateway install --system")
        _gw().print_info("  Or run in foreground:  nunmai gateway run")
        return
    try:
        ...
        _gw().print_info("  You can try manually: nunmai gateway install")
...
def gateway_setup():
    ...
    if any(_is_progress(_gw()._platform_status(p)) for p in _gw()._all_platforms()):
        ...
        print()
        _gw().print_info("No platforms configured. Run 'nunmai gateway setup' when ready.")
