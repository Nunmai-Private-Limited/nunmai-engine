...
Split out of ``nunmai_cli.models``. OpenRouter and
Nunmai Portal share one implementation parametrized by :class:`_CapsSource`; the per-source module
globals (``_openrouter_reasoning_caps_cache``, ``_nous_caps_disk_checked``, ...) stay defined on
``nunmai_cli.models`` — tests reset them there — and are read/written by attribute name.
...
logger = logging.getLogger("nunmai_cli.models")

Caps = dict[str, Optional[dict[str, Any]]]


def _origin():
    from nunmai_cli import models
    return models
...
# that must never block on HTTP — so without a disk copy, `nunmai -p`, a cron job, or a freshly
...
def _read_reasoning_caps_disk() -> dict[str, Any]:
    from nunmai_cli.models import _read_json_cache
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._NUNMAI_USER_AGENT})
...
# ── Per-source cache (OpenRouter, Nunmai Portal) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``nunmai_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from nunmai_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from nunmai_cli.models_profile_cache import profile_slot_set
...
# Nunmai Portal serves OpenRouter's catalog schema, so the same parser and contract apply. Its own
...
def nous_model_reasoning_capabilities(
    ...
    """Nunmai Portal counterpart of :func:`openrouter_model_reasoning_capabilities`; warm the cache
