"""Persistent session goals — the Ralph loop for Nunmai.
...
# (schema DDL, FTS tables, first nunmai_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per NUNMAI_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``nunmai profile delete`` force-closes every handle under the profile home
        # (``nunmai_state_registry.close_all_under``) before rmtree; a same-name recreate in this
...
def judge_goal(
    ...
    *,
    ...
    try:
        ...
        # No client at all (e.g. a dead Nunmai refresh token): name the cause so the user is sent to
