...
Three layers make every long-lived Nunmai process positively identifiable, so reapers (``nunmai
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``NUNMAI_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from nunmai_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``NUNMAI_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``NUNMAI_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_nunmai_root()) / LEDGER_FILENAME
    except Exception:
        from nunmai_cli.config import get_nunmai_home

        return Path(get_nunmai_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `nunmai serve --host X --port N --profile P` while bounding
...
def is_desktop_owned_backend(argv: Optional[Sequence[str]] = None) -> bool:
    ...
    ``NUNMAI_DESKTOP=1`` is inherited by every shell and agent child the app launches, so the
    ...
    strips from agent children): the local pool spawn mints ``NUNMAI_DASHBOARD_SESSION_TOKEN``,
    ...
    if os.environ.get("NUNMAI_DESKTOP") != "1":
        return False
    if os.environ.get("NUNMAI_DASHBOARD_SESSION_TOKEN"):
        return True
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's NUNMAI_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("NUNMAI_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("NUNMAI_PARENT_START_MARKER", "")
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import Nunmai code). Records the child's
