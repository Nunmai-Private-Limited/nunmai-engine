...
Three layers that make every long-lived Nunmai process positively
identifiable, so reapers (``nunmai update``, Desktop startup sweeps) never
have to guess lineage from PPID archaeology or cmdline pattern-matching:

1. **Spawn tag** (``NUNMAI_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Nunmai root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Nunmai install on the
...
def register_self(
    ...
    *,
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `nunmai serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Nunmai code). The entry records the
