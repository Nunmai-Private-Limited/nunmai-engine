...
Outputs a compact, plain-text summary of the user's Nunmai setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.nunmai/.env.
    ...
    exports. ``nunmai debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.nunmai_build_sha`` by the Dockerfile's
    ``NUNMAI_GIT_SHA`` build-arg (see ``nunmai_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nunmai dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    lines.append(f"nunmai_home:      {display_nunmai_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.nunmai/.env: a managed
        ...
        # A credential added via `nunmai auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `nunmai auth list` shows it (#42130).
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nunmai_home)}")
    lines.append(f"  skills:             {_count_skills(nunmai_home)}")
