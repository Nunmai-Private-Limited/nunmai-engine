...
``nunmai_cli.config.migrate_config``. Each step is a function
...
``nunmai_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``nunmai_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``nunmai_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``nunmai_cli.config`` (e.g.
``patch("nunmai_cli.config.read_raw_config", ...)``) keep working, because
...
#: NUNMAI_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    """Return the live ``nunmai_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_nunmai_home = _c.get_nunmai_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$NUNMAI_HOME/plugins/`` for currently installed user plugins.
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `nunmai curator status` has no
    ...
    #   3. Creates `~/.nunmai/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nunmai_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_nunmai_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `nunmai config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `nunmai config set`"
                )
...
# behavior set it to true explicitly via `nunmai config set`.
