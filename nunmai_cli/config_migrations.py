...
in :func:`run_migrations`. Every write goes through ``nunmai_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_nunmai_home()}/config.yaml and run `nunmai setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``nunmai_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$NUNMAI_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_nunmai_home() / "plugins"
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    message = (
        ...
        "Use `nunmai plugins enable <name>` to activate.")
    _commit(
        config, results, quiet,
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `nunmai curator status`
    # had no stable logs dir. Only keys the user hasn't set are written.
    _c = _cfg()
    DEFAULT_CONFIG = _c.DEFAULT_CONFIG

    try:
        curator_dir = _c.get_nunmai_home() / "logs" / "curator"
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `nunmai config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from nunmai_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "NUNMAI_NEMO_RELAY_PLUGINS_TOML.")
...
def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from tools.bot_mode_probe import _PROTOCOL_HEADING, _nunmai_root, _roster, strip_legacy_protocol

    cleaned: List[str] = []
    for name, profile_dir in _roster(_nunmai_root(get_nunmai_home())):
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from nunmai_cli.tools_config import _configurable_keys, _get_plugin_toolset_keys
    ...
    for platform, toolsets in saved.items():
        ...
        if not toolset_allowed_for_platform("connections", platform):
            continue
        # A composite like [nunmai-cli] already inherits every core tool at read time.
    ...
    _commit(
        ...
        "Uncheck Connections in `nunmai tools` to turn it off.")
...
def _migrate_to_46(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # `disabled: true` wins over an explicit `enabled: true`: `nunmai mcp add` writes that, and the
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `nunmai curator restore`). Set it back to 90 to keep the old window."))),
...
#: out: it clears OPENAI_MODEL from .env, a generic name Nunmai never reads but the user's tools may.
