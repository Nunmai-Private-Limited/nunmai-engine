"""nunmai webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.nunmai/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _nunmai_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nunmai gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nunmai webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nunmai webhook {subscribe|list|remove|test}")
        print("Run 'nunmai webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (nunmai gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nunmai webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from nunmai webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nunmai gateway run)")
