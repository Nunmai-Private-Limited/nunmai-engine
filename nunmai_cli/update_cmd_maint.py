"""Post-update maintenance for ``nunmai update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``nunmai_cli.update_cmd.<name>``
...
_UPDATE_RUNTIME_RELOAD_MODULES = "nunmai_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "nunmai_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"nunmai_cli", "nunmai_cli.main", "nunmai_cli.nunmai_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "nunmai_cli.update_"
...
def _purge_stale_nunmai_modules() -> None:
    """Evict every cached Nunmai module after the checkout changed in-place. Never raises.
    ...
    with _best_effort('Could not purge stale Nunmai modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Nunmai module(s) after checkout update", len(purged))
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Docs:         https://nunmai.in/docs/user-guide/features/curator")
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: nunmai curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from nunmai_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("nunmai_cli._subprocess_compat", "nunmai_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from nunmai_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from nunmai_cli.update_cmd import _branch_head_suffix
...
def _post_update_sqlite_runtime_status():
    ...
    from nunmai_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Nunmai runtime."""
    from nunmai_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Nunmai venv with a uv-managed Python, restart Nunmai, then verify with `nunmai doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from nunmai_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `nunmai "
                "update` again; if SQLite is unchanged, rebuild the Nunmai venv "
                "with a uv-managed Python, restart Nunmai, then verify with "
                "`nunmai doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    from nunmai_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Nunmai) and retry."
        )
        return False
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from nunmai_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: nunmai skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v nunmai'`` resolves."""
    from nunmai_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/nunmai).
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `nunmai gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``nunmai-acp`` launcher next to ``nunmai`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``nunmai acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$NunmaiHome\bin``, never
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_nunmai_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_nunmai_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from nunmai_cli.update_cmd import _record_update_step
...
def _run_full_backup() -> None:
    """Zip NUNMAI_HOME under ``backups/`` (restorable via ``nunmai import``). Never raises."""
    try:
    ...
    # display_nunmai_home so the user sees ~/.nunmai/...
    try:
        from nunmai_constants import get_nunmai_home, display_nunmai_home
        display_path = f"{display_nunmai_home()}/{out_path.relative_to(get_nunmai_home())}"
    except Exception:
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of NUNMAI_HOME under ``backups/`` (``nunmai import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in nunmai_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # NUNMAI_HOME, so sync_skills()'s module-level NUNMAI_HOME cache can't skew it.
    with suppress(Exception):
        from nunmai_cli.profiles import list_profiles, seed_profile_skills
...
def _print_plugin_compat_notice() -> None:
    ...
    from nunmai_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from nunmai_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    for message, step in (
        ...
        ('nunmai-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from nunmai_cli.update_cmd import _check_and_apply_config_migration, _m
