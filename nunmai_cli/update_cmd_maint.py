"""Post-update maintenance for ``nunmai update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary.
...
_REINSTALL_ONE_LINER = {
    True: "iex (irm https://nunmai-engine.nunmai.in/install.ps1)",
    False: "curl -fsSL https://nunmai-engine.nunmai.in/install.sh | bash",
}
...
def _sqlite_partial_completion_lines(sqlite_version: str) -> list[str]:
    ...
    return [
        ...
        "corruption bug. Nunmai works, but sessions could be damaged.",
        f"  Fix: run the installer again ({_REINSTALL_ONE_LINER[bool(_m()._is_windows())]}) "
        "which installs a safe Python, then run `nunmai doctor` to confirm.",
    ]
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    try:
        ...
        stop_result = _m()._kill_stale_dashboard_processes(
            ...
            scope_home=str(get_nunmai_home()),
        )
        ...
        print("    nunmai dashboard --port <port>   (or: systemctl --user restart nunmai-dashboard)")
        return
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    with suppress(Exception):
        from nunmai_cli.gateway_multiplex_mode import consume_rewritten_notice, recorded_standalone_warning_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    for message, step in (
        ...
        # Legacy NUNMAI_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from nunmai_cli.relay_plugin_migrate import run_relay_migration_after_update
