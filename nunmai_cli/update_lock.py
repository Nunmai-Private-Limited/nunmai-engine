...
# Set by an orchestrating updater (Tauri `nunmai-setup --update`) to its own pid before
# spawning `nunmai update` as a child stage; the parent holds the marker for its whole run,
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Nunmai is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Nunmai home (never the context-local profile override): the Rust
    updater resolves ``$NUNMAI_HOME`` or the platform default and the desktop pins that same
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``nunmai update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Nunmai update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `nunmai logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `nunmai update` again."
    )
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        # nothing pre-writes a marker for `nunmai update` (it always runs under a parent's claim).
