"""Cross-process mutual exclusion for in-flight Nunmai updates.

Three different surfaces can start an update of the same install tree:

* ``nunmai update`` from a terminal,
* the dashboard's Update button (``POST /api/nunmai/update`` →
  ``_spawn_nunmai_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``nunmai-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``nunmai update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <NUNMAI_HOME>/.nunmai-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``nunmai update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Nunmai is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``nunmai-setup`` binary under ``~/.nunmai`` is only refreshed by a full
  installer run (``copy_self_to_nunmai_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Nunmai is still running") on every
  GUI update forever, with no Nunmai process actually running.
"""
...
MARKER_NAME = ".nunmai-update-in-progress"

# Set by an orchestrating updater (the Tauri `nunmai-setup --update` flow) to
# its own pid before spawning `nunmai update` as a child stage. The parent
...
HANDOFF_PID_ENV = "NUNMAI_UPDATE_HANDOFF_PID"
...
# to show "Nunmai is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Nunmai home (never the context-local profile override):
    the Rust updater resolves ``$NUNMAI_HOME`` or the platform default, and the
    ...
    return get_process_nunmai_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``nunmai update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``nunmai-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Nunmai update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `nunmai update` as a stage): we run under ITS claim
