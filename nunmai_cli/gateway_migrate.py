"""``nunmai gateway migrate --multiplex``: converge a per-profile-gateway install onto the ONE
host gateway, with a table-driven preflight.

Multiplex-only (Teknium ruling): exactly one ``nunmai gateway run`` per host, serving every
...
what the multiplexer would do at startup. ``nunmai update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = "nunmai gateway migrate --multiplex"
...
class ProfileGateway:
    ...
    runtime_home: Optional[Path] = None  # NUNMAI_HOME the installed unit pins, when it differs from ``home``
...
def _remove_verb(service: tuple[str, bool]) -> str:
    """An s6 slot is parked down (it stays registered as the `nunmai -p X gateway start` target), every
...
class MigrationPlan:
    ...
    def expected_served_names(self) -> set[str]:
        from nunmai_cli.profiles import profile_is_parked
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``nunmai update``) bar; the explicit command also proceeds with
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active NUNMAI_HOME. Both the contextvar
    override (``get_nunmai_home``) and ``os.environ`` (``gateway.status`` identity files, unit
    ...
    previous = os.environ.get("NUNMAI_HOME")
    token = set_nunmai_home_override(str(home))
    os.environ["NUNMAI_HOME"] = str(home)
    nunmai_constants._default_nunmai_root_memo = None
    try:
        yield
    finally:
        ...
        if previous is None:
            ...
            os.environ["NUNMAI_HOME"] = previous
        nunmai_constants._default_nunmai_root_memo = None
...
def _own_gateway_pid(home: Path, owner) -> Optional[int]:
    ...
    PID, so the plan called the host half-migrated and each `nunmai update` SIGTERMed the only
    ...
    if pid is None or owner is None or pid != owner.pid:
        return pid
    with contextlib.suppress(Exception):
        from gateway.status import _same_nunmai_home
        return pid if _same_nunmai_home(Path(owner.home), Path(home)) else None
    return None


def _gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    from nunmai_cli.gateway_migrate_guards import gateway_identity
...
def _installed_services(home: Path) -> list[tuple[str, bool]]:
    ...
    from nunmai_cli import gateway as gw
    found: list[tuple[str, bool]] = []
    if gw._running_under_s6():
        from nunmai_cli.gateway_multiplex_s6 import named_slot_name, slot_is_up
        from nunmai_cli.service_manager import S6ServiceManager
...
def _windows_task_installed() -> bool:
    """Is a per-profile Windows gateway installed for the ACTIVE ``NUNMAI_HOME``?

    ``get_task_name()`` is home-suffixed, so this answers per profile exactly the way the systemd
    unit path does. Either half counts: ``nunmai gateway install`` falls back to a Startup-folder
    ...
    """
    from nunmai_cli import gateway_windows as gww
    with contextlib.suppress(Exception):
...
def _stop_gateway_process(home: Path) -> None:
    from nunmai_cli.profiles import _stop_gateway_process
...
def _s6_slot_op(verb: str, home: Path) -> None:
    ...
    registered DOWN (``down`` file) as the target of a later ``nunmai -p X gateway start``, exactly
    the shape the container's boot produces. The root slot is (re)started so it re-reads its config."""
    from nunmai_cli.gateway_multiplex_s6 import bring_root_slot_up, park_named_slot
    from nunmai_constants import profile_name_for_home
    name = profile_name_for_home(home) or "default"
    if name == "default":
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``nunmai config set`` uses; no raw YAML edits)."""
    from nunmai_cli.config import _set_nested, _write_user_config, require_readable_config_before_write
...
def _credential_key_names(platform_value: str) -> str:
    ...
    from nunmai_cli.profile_channels import credential_env_keys
...
def duplicate_credential_lines(configs: list[tuple[str, object]]) -> list[str]:
    ...
    source for the migrate preflight, ``nunmai doctor`` and ``nunmai gateway status``, so all three
...
def build_migration_plan() -> MigrationPlan:
    ...
    for check in _PREFLIGHT_CHECKS:
        check(plan, configs)
    from nunmai_cli.gateway_migrate_guards import auto_migration_blockers
    ...
    plan.notices.extend(f"Not migrated automatically by `nunmai update`: {b}" for b in auto_migration_blockers(plan))
    plan.notices.append(
...
def format_update_warning(plan: MigrationPlan, auto_blockers: list[str]) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in (*plan.blockers, *auto_blockers)],
        ...
        "  (`nunmai update` will migrate automatically once nothing blocks it.)",
    ]
...
def _preflight_apply(plan: MigrationPlan, target: Optional[tuple[str, bool]], run_as_user: Optional[str]) -> Optional[str]:
    ...
    from nunmai_cli import gateway as gw
    from nunmai_cli.config import require_readable_config_before_write
    try:
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    **Resume beats preflight.** The manifest is read BEFORE the blocker gate: a manifest means an
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `nunmai gateway status` and the gateway log; the flag and manifest are in place.",
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    the next ``nunmai gateway migrate --multiplex`` resumes.
    """
    ...
    if live is None:
        print(f"  ✗ default: no gateway confirmed serving this host within {_COMPENSATOR_WAIT_SECONDS:.0f}s "
              f"(check `nunmai gateway status` and the gateway log)")
        print(incomplete)
        return False
    _manifest_path(default_home).unlink(missing_ok=True)
...
def _host_supports_migration() -> Optional[str]:
    ...
    same way the container's boot does it (``nunmai_cli.gateway_multiplex_s6``). What cannot be
    done from here is register a slot the boot never created — that is the one refusal left.
    """
    from nunmai_cli import gateway as gw
    if not gw._running_under_s6():
        return None
    from nunmai_cli.gateway_multiplex_s6 import named_slot_name
    from nunmai_cli.service_manager import S6ServiceManager
...
def cmd_migrate(args) -> None:
    """``nunmai gateway migrate [--multiplex] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``nunmai update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,
    ...
    service-domain / UNIX-user / NUNMAI_HOME boundary blocks this path only (the explicit command decides)."""
    from nunmai_cli.gateway_migrate_guards import auto_migration_blockers, auto_migration_opted_out
