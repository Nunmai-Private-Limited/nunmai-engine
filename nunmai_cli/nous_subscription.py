...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "nunmai-cli"}
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Nunmai-managed Node dirs ($NUNMAI_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_nunmai_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("NUNMAI_LOCAL_STT_COMMAND"):
        return True
    try:
...
def _fal_feature(key: str, tool_enabled: bool, direct: bool, managed: bool, selected: Optional[str]) -> NousFeatureState:
    ...
    if selected not in (None, "nous") or (selected is None and direct):
        ...
        label = "Nunmai Subscription" if (fal_managed or selected == "nous") else ""
    return _state(
...
def _audio_features(
    ...
    # STT default is "local" (faster-whisper, needs a pip install); Nunmai subscribers are routed to the
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from nunmai_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    # every Nunmai model swap. See #92647.
    ...
    title = (
        "Your free Nunmai tool pool — pick the tools to enable:" if pool_only
        else "Your Nunmai subscription includes the Tool Gateway — pick the tools to enable:"
    )
    try:
...
def ensure_nous_portal_access(*, capability: str = "the Nunmai Tool Gateway", coverage_category: Optional[str] = None) -> bool:
    """Make sure the user is entitled to the Nunmai Tool Gateway, logging in if needed.
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the Nunmai Portal device-code OAuth and persist credentials only (no model selection, no
    ...
    proceed = _confirm("  Log in to Nunmai Portal now? [Y/n]: ")
    if proceed is None:
        print()
        return False
    if not proceed:
        ...
        return False
    try:
        # Snapshot active_provider so a tool-config login never silently switches inference to Nunmai.
        ...
        if auth._read_shared_nous_state() and _confirm("  Found existing Nunmai OAuth credentials. Import them? [Y/n]: ") is not False:
