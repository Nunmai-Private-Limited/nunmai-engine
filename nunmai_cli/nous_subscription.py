...
# coverage category (nunmai_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `nunmai tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``nunmai status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Nunmai-managed Node dirs (Windows installer / POSIX $NUNMAI_HOME/node)
        ...
        managed_path = with_nunmai_node_path().get("PATH", "")
        if managed_path:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Keyless Tavily is opt-in: selecting it in `nunmai tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("NUNMAI_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("NUNMAI_LOCAL_STT_COMMAND"))
