...
``<root>`` is the **shared Nunmai root** (the parent of any active
...
``nunmai -p <profile>`` joins the same board as the dispatcher that
...
the profile's Nunmai home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NUNMAI_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NUNMAI_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nunmai kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nunmai``. In Docker / custom
deployments where ``NUNMAI_HOME`` points outside ``~/.nunmai`` (e.g.
``/opt/nunmai``), ``<root>`` is ``NUNMAI_HOME``. Legacy env-var
overrides still work:

* ``NUNMAI_KANBAN_DB`` — pin the database file path directly.
* ``NUNMAI_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NUNMAI_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NUNMAI_KANBAN_DB``,
``NUNMAI_KANBAN_WORKSPACES_ROOT``, and ``NUNMAI_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nunmai-kanban-v1-spec.pdf`` for the full
...
from nunmai_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``nunmai_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from nunmai_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("NUNMAI_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active NUNMAI_HOME so dispatcher- and
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from nunmai_cli.lifecycle import has_hook
...
# ``NUNMAI_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NUNMAI_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NUNMAI_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NUNMAI_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NUNMAI_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "nunmai_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``nunmai-engine``
...
def kanban_home() -> Path:
    """Return the shared Nunmai root that anchors the kanban board.

    Resolution order:

    1. ``NUNMAI_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_nunmai_root()``, which already returns ``<root>``
       when ``NUNMAI_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NUNMAI_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NUNMAI_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NUNMAI_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nunmai kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NUNMAI_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nunmai kanban boards
    ...
    env = os.environ.get("NUNMAI_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nunmai kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NUNMAI_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NUNMAI_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NUNMAI_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NUNMAI_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NUNMAI_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NUNMAI_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nunmai kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``nunmai_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``NUNMAI_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (nunmai_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NUNMAI_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NUNMAI_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``nunmai_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from nunmai_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``nunmai gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from nunmai_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see nunmai_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from nunmai_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Nunmai treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``nunmai kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NUNMAI_KANBAN_DB`` env → ``NUNMAI_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `nunmai kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from nunmai_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from nunmai_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See nunmai_state._WAL_INCOMPAT_MARKERS for detection logic.
                from nunmai_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nunmai kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NUNMAI_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from nunmai_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"nunmai_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``nunmai --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from nunmai_cli import projects_db as _pdb
    ...
    # `nunmai --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``nunmai kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `nunmai kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``nunmai kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nunmai kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nunmai kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``nunmai kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``nunmai kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NUNMAI_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Nunmai' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Nunmai-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``nunmai kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Nunmai checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Nunmai treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Nunmai creates/reuses that linked worktree. With
      no ``workspace_path``, Nunmai anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Nunmai uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Nunmai
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``nunmai kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nunmai kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Nunmai profile.
    ...
    if not rows:
        return False
    try:
        from nunmai_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Nunmai ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Nunmai
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``nunmai kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``NUNMAI_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from nunmai_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from nunmai_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Nunmai profile.
        # `_default_spawn` invokes ``nunmai -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Nunmai
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `nunmai kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Nunmai bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from nunmai_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_nunmai_argv() -> list[str]:
    """Return the interpreter-bound Nunmai CLI invocation."""
    # ``nunmai_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``nunmai`` package — there is no
    # ``nunmai`` package to import.
    return [sys.executable, "-m", "nunmai_cli.main"]


def _absolute_nunmai_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Nunmai shim."""
...
def _nunmai_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Nunmai executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_nunmai_argv()
    return [_absolute_nunmai_path(path)]


def _resolve_nunmai_argv() -> list[str]:
    """Resolve the ``nunmai`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NUNMAI_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("nunmai")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\nunmai.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m nunmai_cli.main`` — fallback for setups where
       Nunmai is launched from a venv and the ``nunmai`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_nunmai_bin`` for the same reason. Kept
    local (not imported from gateway) because ``nunmai_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("NUNMAI_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _nunmai_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _nunmai_path_argv(resolved_env_bin)
        return _module_nunmai_argv()

    nunmai_bin = _safe_which_no_cwd("nunmai") if _IS_WINDOWS else shutil.which("nunmai")
    if nunmai_bin:
        return _nunmai_path_argv(nunmai_bin)
    return _module_nunmai_argv()
...
def _resolve_worker_cli_toolsets(nunmai_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``NUNMAI_KANBAN_TASK`` is set.
    """
    if not nunmai_home:
        return None
    try:
        ...
        token = set_nunmai_home_override(nunmai_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for NUNMAI_HOME=%r (%s)",
            nunmai_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nunmai -p <profile> chat -q ...`` subprocess.
    ...
    ``NUNMAI_KANBAN_DB`` / ``NUNMAI_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NUNMAI_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `nunmai -p <name>` the
    # _apply_profile_override() runs *before* nunmai_constants is imported.
    # If NUNMAI_HOME is absent from the child's env, get_nunmai_home() falls
    # back to Path.home() / ".nunmai" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from nunmai_cli.profiles import resolve_profile_env
    try:
        env["NUNMAI_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NUNMAI_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NUNMAI_HOME never had profiles created.
        pass
    if task.tenant:
        env["NUNMAI_TENANT"] = task.tenant
    env["NUNMAI_KANBAN_TASK"] = task.id
    env["NUNMAI_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `nunmai kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["NUNMAI_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["NUNMAI_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NUNMAI_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NUNMAI_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["NUNMAI_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["NUNMAI_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`nunmai -p <name>`
    # rewrites NUNMAI_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_nunmai_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NUNMAI_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NUNMAI_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["NUNMAI_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NUNMAI_KANBAN_BOARD"] = resolved_board
    # NUNMAI_PROFILE is the author the kanban_comment tool defaults to.
    # `nunmai -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NUNMAI_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited NUNMAI_TUI=1
    ...
    # older nunmai builds on PATH that predate the flag's precedence.
    env.pop("NUNMAI_TUI", None)

    cmd = [
        *_resolve_nunmai_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped NUNMAI_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("NUNMAI_HOME"))
    if worker_toolsets:
    ...
    # `nunmai kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nunmai` executable not found on PATH. "
            "Install Nunmai Engine or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nunmai kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``nunmai kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `nunmai kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NUNMAI_PROFILE values like "nunmai-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Nunmai root exists

    Reads profile paths directly so this module has no import dependency on
    ``nunmai_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from nunmai_constants import get_default_nunmai_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nunmai kanban assignees`` for the terminal.
