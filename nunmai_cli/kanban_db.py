...
Lives under the shared Nunmai root: ``default`` board DB at ``<root>/kanban.db`` (pre-boards
back-compat), other boards at ``<root>/kanban/boards/<slug>/``; a worker on one board never sees
another. Board resolution: ``board=`` arg > ``NUNMAI_KANBAN_BOARD`` > ``NUNMAI_KANBAN_DB`` (pins the
...
# ``heartbeat_claim`` or raise it via NUNMAI_KANBAN_CLAIM_TTL_SECONDS.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    """Explicit ``ttl_seconds`` > ``NUNMAI_KANBAN_CLAIM_TTL_SECONDS`` > default."""
    if ttl_seconds is not None:
        return max(1, int(ttl_seconds))

    return _env_int("NUNMAI_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    """``NUNMAI_KANBAN_CRASH_GRACE_SECONDS`` (0 = immediate, for tests) else default."""
    return _env_int("NUNMAI_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("NUNMAI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "nunmai_kanban_current_board_override", default=None,
)
...
def kanban_home() -> Path:
    """``NUNMAI_KANBAN_HOME`` else ``get_default_nunmai_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's NUNMAI_HOME would
...
def get_current_board() -> str:
    """Active slug: context override -> ``NUNMAI_KANBAN_BOARD`` -> ``<root>/kanban/current``
    ...
    for candidate in (
        ...
        os.environ.get("NUNMAI_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """``kanban.db`` path: ``NUNMAI_KANBAN_DB`` pins it (injected into workers);
    ``default`` -> ``<root>/kanban.db`` (back-compat), else the board dir."""
    return _board_path("NUNMAI_KANBAN_DB", board, ("kanban.db",), "kanban.db")


def workspaces_root(board: Optional[str] = None) -> Path:
    """Per-board scratch workspace root (``NUNMAI_KANBAN_WORKSPACES_ROOT`` wins);
    ``default`` keeps the legacy ``<root>/kanban/workspaces/``."""
    return _board_path("NUNMAI_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")


def attachments_root(board: Optional[str] = None) -> Path:
    """Per-board attachments root (``NUNMAI_KANBAN_ATTACHMENTS_ROOT`` wins). Workers
    read attachments by absolute path, so remote terminal backends must mount it."""
    return _board_path("NUNMAI_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``nunmai kanban log``
...
class Task:
    ...
    session_id: Optional[str] = None         # originating NUNMAI_SESSION_ID; NULL from CLI/dashboard
...
def create_task(
    ...
    """
    from nunmai_cli.kanban_db_graph import initial_task_state, inherit_creator_origin
    from nunmai_cli.kanban_pr_acceptance import validate_contract
...
# dispatcher tick or a manual `nunmai kanban recompute` (issue #22459).
...
def complete_task(
    ...
    if not _parents_satisfied(conn, task_id):
        return False
    from nunmai_cli.kanban_pr_acceptance_store import prepare_acceptance, record_acceptance
...
def promote_task(
    ...
    if unsatisfied:
        return False, (
            ...
            f"`nunmai kanban unlink <parent_id> {task_id}`)"
        )
...
def _ctx_comments(lines: list[str], comments: list[Comment], now: int) -> None:
    ...
    worker" framing stops an operator-controlled NUNMAI_PROFILE like
    "nunmai-system" being read as a system directive above an
    ...
    if omitted_note:
        lines.append(omitted_note)
    for c in shown:
        # Render author with explicit "comment from worker" framing so operator-controlled NUNMAI_PROFILE
        # values like "nunmai-system" or "operator" can't be misread by the next worker as a system
...
def list_profiles_on_disk() -> list[str]:
    ...
    directly to avoid importing ``nunmai_cli.profiles`` at startup."""
    try:
        from nunmai_constants import get_default_nunmai_root
...
from nunmai_cli.kanban_db_connect import (  # noqa: E402
    _INITIALIZED_PATHS,
    init_db,
    write_txn,
)
from nunmai_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from nunmai_cli.kanban_db_dispatch import (  # noqa: E402
...
_PLUGIN_COMPAT_LAZY = {
    'DEFAULT_BUSY_TIMEOUT_MS': ('nunmai_cli.kanban_db_connect', 'DEFAULT_BUSY_TIMEOUT_MS'),
    'DEFAULT_LOG_BACKUP_COUNT': ('nunmai_cli.kanban_db_dispatch', 'DEFAULT_LOG_BACKUP_COUNT'),
    'DEFAULT_LOG_ROTATE_BYTES': ('nunmai_cli.kanban_db_dispatch', 'DEFAULT_LOG_ROTATE_BYTES'),
    'DERIVED_MAX_IN_PROGRESS_CEILING': ('nunmai_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_CEILING'),
    'DERIVED_MAX_IN_PROGRESS_FLOOR': ('nunmai_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_FLOOR'),
    'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS': ('nunmai_cli.kanban_db_dispatch', 'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS'),
    'KanbanDbCorruptError': ('nunmai_cli.kanban_db_connect', 'KanbanDbCorruptError'),
    'MEMORY_GUARD_MB_PER_WORKER': ('nunmai_cli.kanban_db_dispatch', 'MEMORY_GUARD_MB_PER_WORKER'),
    'RepairResult': ('nunmai_cli.kanban_db_connect', 'RepairResult'),
    'add_notify_sub': ('nunmai_cli.kanban_db_notify', 'add_notify_sub'),
    'advance_notify_cursor': ('nunmai_cli.kanban_db_notify', 'advance_notify_cursor'),
    'check_respawn_guard': ('nunmai_cli.kanban_db_dispatch', 'check_respawn_guard'),
    'claim_unseen_events_for_sub': ('nunmai_cli.kanban_db_notify', 'claim_unseen_events_for_sub'),
    'configured_max_in_progress': ('nunmai_cli.kanban_db_dispatch', 'configured_max_in_progress'),
    'connect': ('nunmai_cli.kanban_db_connect', 'connect'),
    'connect_closing': ('nunmai_cli.kanban_db_connect', 'connect_closing'),
    'count_notify_subs': ('nunmai_cli.kanban_db_notify', 'count_notify_subs'),
    'count_running_tasks': ('nunmai_cli.kanban_db_dispatch', 'count_running_tasks'),
    'count_running_tasks_other_boards': ('nunmai_cli.kanban_db_dispatch', 'count_running_tasks_other_boards'),
    'derive_default_max_in_progress': ('nunmai_cli.kanban_db_dispatch', 'derive_default_max_in_progress'),
    'detect_crashed_workers': ('nunmai_cli.kanban_db_dispatch', 'detect_crashed_workers'),
    'detect_stale_running': ('nunmai_cli.kanban_db_dispatch', 'detect_stale_running'),
    'dispatch_once': ('nunmai_cli.kanban_db_dispatch', 'dispatch_once'),
    'enforce_max_runtime': ('nunmai_cli.kanban_db_dispatch', 'enforce_max_runtime'),
    'has_spawnable_ready': ('nunmai_cli.kanban_db_dispatch', 'has_spawnable_ready'),
    'has_spawnable_review': ('nunmai_cli.kanban_db_dispatch', 'has_spawnable_review'),
    'heartbeat_worker': ('nunmai_cli.kanban_db_dispatch', 'heartbeat_worker'),
    'list_notify_subs': ('nunmai_cli.kanban_db_notify', 'list_notify_subs'),
    'purge_stale_done_notify_subs': ('nunmai_cli.kanban_db_notify', 'purge_stale_done_notify_subs'),
    'reap_worker_zombies': ('nunmai_cli.kanban_db_dispatch', 'reap_worker_zombies'),
    'reconcile_orphaned_running': ('nunmai_cli.kanban_db_dispatch', 'reconcile_orphaned_running'),
    'remove_notify_sub': ('nunmai_cli.kanban_db_notify', 'remove_notify_sub'),
    'repair_db': ('nunmai_cli.kanban_db_connect', 'repair_db'),
    'resolve_max_in_progress': ('nunmai_cli.kanban_db_dispatch', 'resolve_max_in_progress'),
    'resolve_workspace': ('nunmai_cli.kanban_db_workspace', 'resolve_workspace'),
    'review_dispatch_enabled': ('nunmai_cli.kanban_db_dispatch', 'review_dispatch_enabled'),
    'rewind_notify_cursor': ('nunmai_cli.kanban_db_notify', 'rewind_notify_cursor'),
    'run_daemon': ('nunmai_cli.kanban_db_dispatch', 'run_daemon'),
    'set_branch_name': ('nunmai_cli.kanban_db_workspace', 'set_branch_name'),
    'set_workspace_path': ('nunmai_cli.kanban_db_workspace', 'set_workspace_path'),
    'unseen_events_for_sub': ('nunmai_cli.kanban_db_notify', 'unseen_events_for_sub'),
    'worker_log_rotation_config': ('nunmai_cli.kanban_db_dispatch', 'worker_log_rotation_config'),
}
