...
_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $NUNMAI_HOME/node is
...
def _has_nunmai_agent_browser() -> bool:
    ...
    # install.sh installs globally into $NUNMAI_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-NunmaiHome", str(get_nunmai_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = nunmai_subprocess_env(inherit_credentials=False)
