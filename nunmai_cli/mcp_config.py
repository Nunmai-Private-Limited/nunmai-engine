"""MCP Server Management CLI — ``nunmai mcp`` subcommand."""
...
from nunmai_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_nunmai_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.nunmai/.env`` into ``os.environ`` and
...
def _probe_single_server(
    ...
    # an OAuth browser flow (e.g. `nunmai mcp login`'s 315s) left that inner bound at the
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return NunmaiTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see nunmai_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  nunmai mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  nunmai mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  nunmai mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: nunmai mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_nunmai_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `nunmai mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  nunmai mcp add <name> --url <endpoint>')
        _info('  nunmai mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _probe_failure_next_step(name: str, exc: BaseException) -> str:
    ...
    if _is_auth_error(root) or getattr(getattr(root, "response", None), "status_code", None) in (401, 403):
        return f"The server rejected the sign-in. Run: nunmai mcp login {name}"
    if "missing executable" in _format_connect_error(exc):
        return (f"Install that command, or set mcp_servers.{name}.command in {display_nunmai_home()}/config.yaml "
                "to its full path.")
    return f"Check the server is running and the URL/command in its config, then run: nunmai mcp test {name}"
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `nunmai mcp remove` + `nunmai mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if selected_flow == "browser":
            ...
            NunmaiTokenStorage(name).remove(keep_metadata=True)
    except Exception as exc:
    ...
    # web_server_mcp.py and tui_gateway/mcp_oauth_sessions.py. force_interactive_oauth: `nunmai mcp
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `nunmai mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: nunmai mcp reauth <name>   |   nunmai mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'nunmai mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "nunmai mcp                                    Open the catalog picker (default)",
    "nunmai mcp catalog                            List Nunmai-approved MCPs",
    "nunmai mcp install <name>                     Install a catalog MCP",
    "nunmai mcp serve                              Run as MCP server",
    "nunmai mcp add <name> --url <endpoint>        Add a custom MCP server",
    "nunmai mcp add <name> --command <cmd>         Add a stdio server",
    "nunmai mcp add <name> --preset <preset>       Add from a known preset",
    "nunmai mcp remove <name>                      Remove a server",
    "nunmai mcp list                               List configured servers",
    "nunmai mcp test <name>                        Test connection",
    "nunmai mcp configure <name>                   Toggle tools",
    "nunmai mcp login <name>                       Re-authenticate OAuth",
    "nunmai mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``nunmai mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from nunmai_cli import mcp_picker
    ...
    if handler:
        ...
        return handler(args)
    # No subcommand — drop the user into the catalog picker (same UX as `nunmai plugin`).
    from nunmai_cli.mcp_picker import run_picker
    run_picker()
