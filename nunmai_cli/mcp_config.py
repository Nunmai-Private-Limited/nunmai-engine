...
Implements ``nunmai mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.nunmai/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from nunmai_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.nunmai/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``nunmai mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # nunmai_cli/main.py for why the dest is renamed.
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_nunmai_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `nunmai mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``nunmai mcp login`` and
    ``nunmai mcp reauth`` so both behave identically for a single server.
    """
    ...
    #
    # force_interactive_oauth: `nunmai mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Nunmai desktop / agent-
...
def cmd_mcp_reauth(args):
    ...
    ``nunmai mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``nunmai mcp reauth --all`` discovers every ``auth: oauth`` server in
...
def mcp_command(args):
    ...
    if action == "picker":
        ...
        return
    if action == "catalog":
        from nunmai_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from nunmai_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `nunmai plugin`.
        from nunmai_cli.mcp_picker import run_picker
        run_picker()
        ...
        _info("nunmai mcp add <name> --command <cmd>         Add a stdio server")
        _info("nunmai mcp add <name> --preset <preset>       Add from a known preset")
        _info("nunmai mcp remove <name>                      Remove a server")
        _info("nunmai mcp list                               List configured servers")
        _info("nunmai mcp test <name>                        Test connection")
        _info("nunmai mcp configure <name>                   Toggle tools")
        _info("nunmai mcp login <name>                       Re-authenticate OAuth")
        _info("nunmai mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
