"""External-tool checks for nunmai doctor: terminal backends, git/rg, Node + agent-browser, npm audit, tool availability.
...
import subprocess
import sys
from nunmai_cli.doctor_platform import _system_package_install_cmd
from nunmai_cli.doctor_report import Finding, _fail_and_issue, check_bool, check_info, check_ok, check_warn, doctor_check
from nunmai_cli.vercel_auth import describe_vercel_auth
from nunmai_constants import agent_browser_runnable, is_termux as _is_termux
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("NUNMAI_KANBAN_TASK")
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from nunmai_cli.doctor_state import _honcho_is_configured_for_doctor
...
_TOOLSET_SETUP_HINTS: dict[str, str] = {
    "image_gen": "(image generation unavailable — check the provider selection and its key or SDK with 'nunmai tools')",
}
    ...
    if terminal_env == "docker":
        if not docker_exe:
            _fail_and_issue("Docker or Podman not installed", "(needed for the 'docker' terminal backend)",
                            "Install Docker or Podman, or run `nunmai setup terminal` to switch backend.", issues)
        else:
            ...
            _require(_run_ok([docker_exe, "version"], timeout=10),
                     ...
                     f"{hint[0].upper()}{hint[1:]}, or run `nunmai setup terminal` to switch backend.", issues)
    elif docker_exe:
...
def _check_ssh_backend(issues: list[str]) -> None:
    ...
    if not ssh_host:
        return _fail_and_issue("SSH host not configured", "(needed for the 'ssh' terminal backend)",
                               "run `nunmai setup terminal` and enter the SSH host and user.", issues)
...
def _check_daytona_backend(issues: list[str]) -> None:
    _require(os.getenv("DAYTONA_API_KEY"), ("Daytona API key", "(configured)"),
             ...
             "run `nunmai setup terminal` (Daytona) to enter it.", issues)
    try:
...
def _check_vercel_backend(issues: list[str]) -> None:
    ...
    _require(importlib.util.find_spec("vercel") is not None, ("vercel SDK", "(installed)"),
             ("vercel SDK not installed", "(pip install 'nunmai-engine[vercel]')"),
             "Install the Vercel optional dependency: pip install 'nunmai-engine[vercel]'", issues)
...
def _check_agent_browser(should_fix: bool) -> bool:
    ...
    Mirrors ``tools.browser_tool_install._find_agent_browser``'s own cascade (lazy npx or a global/Nunmai-managed
    ...
    try:
        ...
        # (or a global/Nunmai-managed install) at first use.
    ...
    if resolved and _is_npx_agent_browser_sentinel(resolved):
        ...
        if should_fix:
            # Can't tell whether npx's cache is warm — fire the same warm-up `nunmai update` does.
    ...
    if resolved and agent_browser_runnable(resolved):
        ...
        return True
    if resolved:
        # Almost always a dangling global symlink left by npm postinstall after `nunmai update` wiped node_modules.
...
def _check_lightpanda() -> None:
    ...
    if not used:
        ...
        check_info("Fix: pick Lightpanda in `nunmai tools` → Browser Automation, or set browser.engine: auto")
...
def _audit_one(npm_bin: str, npm_dir, label: str, audit_extra: list[str], issues: list[str]) -> None:
    ...
    every `nunmai update`), so a local `npm audit fix` never persists — the next
    ...
    try:
        ...
        if total == 0:
            ...
            remedy = ("fix is an upstream lockfile bump — a local manual fix does not persist"
                      " (the next `nunmai update` reinstalls from the committed lockfile)")
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    NUNMAI_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    from nunmai_cli.doctor import PROJECT_ROOT
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable NUNMAI_HOME mirror instead of the
...
def _check_tool_availability(should_fix: bool, f: Finding) -> None:
    ...
    if api_disabled or any(status != "ok" for status, _, _ in web_rows):
        f.issues.append("Run 'nunmai setup' to configure missing API keys for full tool access")
