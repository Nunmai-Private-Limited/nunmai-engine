...
from nunmai_state_common import _RESET_CHILD_SQL, _sql_json_extract
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    ``nunmai serve`` runs neither CLI nor gateway startup hooks, so this
    ...
    try:
        ...
        # load_config() resolves through the PROCESS NUNMAI_HOME, so the dashboard swept every
        ...
        _home_token = set_nunmai_home_override(str(profile_home))
        try:
        ...
        # multiplexed secondary's store is swept too). Opening it WRITABLE from `nunmai
...
def _maybe_run_skill_maintenance(started_at: float) -> None:
    ...
    if _check_gateway_running(get_nunmai_home()):
        return
