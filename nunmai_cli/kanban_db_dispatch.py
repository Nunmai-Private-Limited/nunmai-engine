...
Split out of ``nunmai_cli.kanban_db``; origin-resident helpers are reached
...
from nunmai_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from nunmai_cli.kanban_db import Task
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Nunmai profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``nunmai kanban tail`` is the only place the
...
# children): a per-tick ``nunmai kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``nunmai_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*Nunmai")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``nunmai kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``nunmai_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several Nunmai homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``nunmai kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Nunmai profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Nunmai ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``NUNMAI_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``nunmai -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``nunmai kanban tail`` shows why the task looks stuck.
...
def _module_nunmai_argv() -> list[str]:
    """Interpreter-bound Nunmai CLI invocation (``nunmai_cli.main`` is the
    console-script target — there is no top-level ``nunmai`` package)."""
...
def _nunmai_path_argv(path: str) -> list[str]:
    """argv for a resolved Nunmai executable path. Windows batch shims
...
def _resolve_nunmai_argv() -> list[str]:
    """Resolve the ``nunmai`` invocation as argv for ``Popen``: ``$NUNMAI_BIN``
    ...
    nunmai_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("nunmai")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``nunmai_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``nunmai`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_nunmai_bin``; local because ``nunmai_cli``
    ...
    nunmai_bin = _safe_which_no_cwd("nunmai") if _kb._IS_WINDOWS else shutil.which("nunmai")
    if nunmai_bin:
...
def _worker_profile_scope(nunmai_home: str, *, bind_home: bool = True):
    ...
    from nunmai_constants import get_process_nunmai_home, reset_nunmai_home_override, set_nunmai_home_override
    ...
    home = Path(nunmai_home)
    is_launch_home = str(home.resolve()) == str(Path(get_process_nunmai_home()).resolve())
    home_token = secret_token = terminal_token = None
    try:
        home_token = set_nunmai_home_override(str(home)) if bind_home else None
...
def _resolve_worker_cli_toolsets(nunmai_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``NUNMAI_KANBAN_TASK`` is set.
    """
    if not nunmai_home:
        return None
    try:
        ...
        with _worker_profile_scope(nunmai_home):
            cfg = load_config()
...
def _worker_argv(task: Task, profile_arg: str, nunmai_home: Optional[str]) -> list[str]:
    """Build the ``nunmai -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_nunmai_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped NUNMAI_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(nunmai_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `nunmai kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``NUNMAI_KANBAN_DB`` / ``NUNMAI_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from nunmai_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # NUNMAI_PROFILE (set below) instead.
        profile_home = None
    ...
    # without it the child's get_nunmai_home() falls back to the DEFAULT
    # profile root because `nunmai -p` applies its override before
    # nunmai_constants is imported.
    if profile_home:
        env["NUNMAI_HOME"] = profile_home
    ...
    # match after `nunmai -p` rewrites NUNMAI_HOME (symlink / Docker layouts).
    env["NUNMAI_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["NUNMAI_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    ...
    env["NUNMAI_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads NUNMAI_PROFILE for its default author; `-p` alone
    ...
    # `--cli` is the highest-precedence TUI override; dropping NUNMAI_TUI covers
    # older nunmai builds on PATH that predate the flag's precedence.
    env.pop("NUNMAI_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("NUNMAI_HOME"))
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``nunmai kanban dispatch`` — the standalone
...
from nunmai_cli import kanban_db as _kb  # noqa: E402
from nunmai_cli import kanban_db_connect as _kbc  # noqa: E402
from nunmai_cli import kanban_db_workspace as _kbw  # noqa: E402
