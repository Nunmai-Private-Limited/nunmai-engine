...
def _resolve_nunmai_argv() -> list[str]:
    ...
    same-directory file), then ``which("nunmai")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    nunmai_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_nunmai_bin``; local because
    ``nunmai_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil
