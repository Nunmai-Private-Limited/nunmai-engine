"""``nunmai plugins`` CLI subcommand — install, update, remove, and list plugins."""
...
from nunmai_cli._subprocess_compat import noninteractive_git_env
from nunmai_cli.cli_output import line_input
from nunmai_cli.config import cfg_get
from nunmai_cli.plugin_capabilities import _child_dict
...
def _plugins_dir() -> Path:
    ...
    plugins = get_nunmai_home() / "plugins"
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.nunmai/.env``."""
    env_specs = [
...
def _refuse_conflicting_python_deps(tmp_target: Path, plugin_name: str) -> None:
    ...
    from nunmai_cli.plugin_python_deps import DependencyConflict, refuse_conflicting_candidate
    try:
        refuse_conflicting_candidate(tmp_target, home=get_nunmai_home())
        ...
        raise PluginOperationError(
            ...
            "its Python packages yourself: nunmai plugins install <identifier> --no-deps") from exc
...
def _install_python_dependencies_quietly(target: Path, warnings: list[str]) -> list[str]:
    ...
    from nunmai_cli.plugin_python_deps import install_for_plugin_dir
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_nunmai_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_nunmai_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_nunmai_home()}/.env later)[/dim]")
...
def _clone_failure_message(git_url: str, git_error: str) -> str:
    ...
    return (f"Could not download the plugin from {git_url}. Check the address (browse the catalog "
            "with `nunmai plugins search`), check your internet connection, or, if the repository "
...
def _unknown_plugin_message(name: str, *, downloaded_only: bool = False) -> str:
    ...
    return (f"[red]No plugin named '{name}'.[/red] Run `nunmai plugins list` to see the exact names "
            f"(nested plugins use their full key, e.g. web/firecrawl).{scope} "
            "To add one: `nunmai plugins install <owner/repo>`.")
...
def _install_metadata_path() -> Path:
    return get_nunmai_home() / "plugins" / ".install-metadata.json"
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    from nunmai_cli.plugins_manifest import SUPPORTED_MANIFEST_VERSION
    if mv_int > SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"but this Nunmai supports up to {SUPPORTED_MANIFEST_VERSION}. "
            f"Run {recommended_update_command()} to update Nunmai.",
        ) from None
...
def _ensure_tree_readable(root: Path, plugins_dir: Path) -> None:
    """Refuse to ship a tree Nunmai cannot read back. A clone can land unreadable (Windows ACL
...
def _refuse_unavailable_portable_plugin(plugin_name: str, tree: Path) -> None:
    ...
    from nunmai_platform.resolver.availability import availability

    try:
        package = load_agent_plugin(tree, tree.parent / ".nunmai-install-data")
    except ValueError as exc:
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `nunmai plugins update {plugin_name}`.")
...
def cmd_install(
    ...
    a ``.nunmai-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from nunmai_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the Nunmai catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Nunmai plugin.")
    ...
    if enable:
        ...
        console.print(
            ...
            f"Run `nunmai plugins enable {installed_name}` to activate.[/dim]")
    ...
    if enable:
        ...
        from nunmai_cli.plugins_activation import activate_plugin_now, activation_hint
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`nunmai plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from nunmai_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `nunmai plugins enable {name}` "
            f"if you trust them.")
...
def cmd_remove(name: str) -> None:
    ...
    if result.get("cleared_memory_provider"):
        console.print("[yellow]memory.provider pointed at this plugin and was reset; "
                      "run `nunmai memory setup` to pick another.[/yellow]")
    console.print()
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``nunmai setup`` / ``nunmai plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from nunmai_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Nunmai core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from nunmai_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from nunmai_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from nunmai_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`nunmai plugins capabilities {plugin_id}` to review and "
            f"`nunmai plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`nunmai plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``nunmai plugins capabilities [<id>]`` — declared vs granted."""
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`nunmai plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from nunmai_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``nunmai plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] nunmai plugins install owner/repo")
        return
    ...
    for line in removed_lines:
        console.print(line)
    console.print()
    console.print("[dim]Compact view:[/dim] nunmai plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] nunmai plugins")
    console.print("[dim]Enable/disable:[/dim] nunmai plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from nunmai_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] nunmai plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from nunmai_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the Nunmai plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the Nunmai catalog.")
    try:
    ...
    from nunmai_cli.plugins_activation import activate_plugin_now
...
def _toggle_plugin_toolset(name: str, *, enable: bool) -> None:
    ...
    if not toolset_key:
        return
    from nunmai_cli.config import load_config, save_config
    from nunmai_cli.toolset_validation import parse_platform_toolsets_value
    config = load_config()
    platform_toolsets = _child_dict(config, "platform_toolsets")
    changed = False
    for platform, raw in list(platform_toolsets.items()):
        # A list-literal string (older `nunmai config set`) is the user's real selection; toggling
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.nunmai/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str, *, accept_capabilities: bool = False) -> dict[str, Any]:
    """``git pull`` inside ``~/.nunmai/plugins/<name>``; catalog installs re-pin instead. A re-pin that
    ...
    try:
        ...
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `nunmai plugins install {rec.get('source', '<source>')} --force "
...
def _run_plugin_git(
    ...
    from nunmai_cli.git_credentials import run_git_with_credential_fallback
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[str, str]:
    ...
    from nunmai_cli.update_cmd_stash import _intent_to_add_paths
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "nunmai-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``nunmai
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.nunmai/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from nunmai_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from nunmai_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from nunmai_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``nunmai plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from nunmai_cli.plugin_compat import (
...
def plugins_command(args) -> None:
    """Dispatch nunmai plugins subcommands."""
