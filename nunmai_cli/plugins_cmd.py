...
Plugins are installed from Git repositories into ``~/.nunmai/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Nunmai subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.nunmai/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Nunmai never auto-installs plugin pip
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from nunmai_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_metadata_path() -> Path:
    return get_nunmai_home() / "plugins" / _INSTALL_METADATA_FILE
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from nunmai_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `nunmai plugins search <term>` to "
...
def cmd_install(
    ...
    console.print("[dim]  nunmai gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`nunmai plugins install {recorded_source} --force "
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``nunmai setup`` / ``nunmai plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``nunmai plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Nunmai core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from nunmai_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from nunmai_cli.plugin_capabilities import granted_capabilities
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``nunmai_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.nunmai/plugins``. Include package metadata here
    so ``nunmai plugins list`` can show and enable them.
    """
    from nunmai_cli.plugins import ENTRY_POINTS_GROUP
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Nunmai
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] nunmai plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``nunmai plugins enable web/firecrawl`` cleared
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected,
    ...
    from nunmai_cli.curses_ui import flush_stdin
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.nunmai/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `nunmai plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``nunmai plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``nunmai_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``nunmai update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "nunmai-plugin-update-autostash",
            )
...
def cmd_search(
    ...
    *,
    ...
    from nunmai_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: nunmai plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")
