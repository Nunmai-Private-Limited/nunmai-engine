...
maps. Split out of ``nunmai_cli.models``.
"""
...
# OpenRouter entries the Nunmai Portal does not carry (routing/fast variants, free tier —
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``nunmai model`` flow share one source."""
...
# Mirrors the xAI-direct IDs from $NUNMAI_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nunmai-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $NUNMAI_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see nunmai_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (nunmai model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `nunmai model` picker.
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [ProviderEntry(*row) for row in (
    ("nous", "Nunmai Portal", "Nunmai Portal (Everything your agent needs, 300+ models with bundled tool use)"),
    ...
    ("moa", "Nunmai Agent", "Nunmai Agent (named presets; aggregator acts after reference models)"),
...
def sync_plugin_provider_catalog() -> int:
    ...
    plugin whose own imports pull ``nunmai_cli.models`` in mid-``_discover_providers()``, or a
    profile registered later at runtime, would otherwise never reach the picker, ``nunmai model``,
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``nunmai model``, setup wizard,
...
# Offline/fresh-install fallback for the model Nunmai silently lands on when the user never picked
...
# (GUI onboarding / ``nunmai model``) uses the tier-aware ``get_recommended_default_model`` in
# nunmai_cli/web_server.py + ``partition_nous_models_by_tier``, which may hit the Portal.
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Nunmai' default
