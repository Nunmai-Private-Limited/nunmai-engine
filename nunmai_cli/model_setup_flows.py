"""Per-provider model-selection wizard flows for ``nunmai setup`` / ``nunmai model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``nunmai_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`nunmai_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from nunmai_cli.model_setup_flows_common import (
...
from nunmai_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from nunmai_cli.model_setup_flows_azure import _model_flow_azure_foundry
from nunmai_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from nunmai_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from nunmai_cli.models import model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from nunmai_cli.main_provider_setup import _prompt_api_key
    from nunmai_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from nunmai_cli.config import get_env_value
    ...
    from nunmai_cli.models import ai_gateway_model_ids
    from nunmai_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `nunmai moa configure <name>` first.")
        return
    ...
    _say(
        "",
        f"Default model set to: {selected_name} (via Nunmai Agent)",
...
def _nous_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    """Free/paid-tier catalog for the Nunmai picker: ``(model_ids, pricing, unavailable_models,
    ...
    if free_tier:
        try:
            from nunmai_cli.nous_account import format_nous_portal_entitlement_message, get_nous_portal_account_info
            _account_info = get_nous_portal_account_info(force_fresh=True)
            unavailable_message = format_nous_portal_entitlement_message(_account_info, capability="paid Nunmai models") or ""
        except Exception:
            unavailable_message = ""
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from nunmai_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _nous_verified_credentials(creds_or_none=None):
    """Resolve Nunmai runtime credentials; on failure print the diagnosis (re-login when the
    ...
    try:
        ...
        if relogin:
            _say(f"Session expired: {msg}", "Re-authenticating with Nunmai Portal...\n")
            try:
...
def _nous_persist_selection(selected: str, creds: dict) -> dict:
    """Nunmai persist step: model choice + provider state, then rewrite ``model`` on a fresh
    ...
    from nunmai_cli.auth import _save_model_choice, _update_config_for_provider
    from nunmai_cli.config import get_env_value, load_config, save_config, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from nunmai_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_nous, PROVIDER_REGISTRY
    from nunmai_cli.config import load_config
    from nunmai_cli.nous_subscription import prompt_enable_tool_gateway
    state = get_provider_auth_state("nous")
    if not state or not state.get("access_token"):
        _say("Not logged into Nunmai Portal. Starting login...", "")
    ...
    from nunmai_cli.models import check_nous_free_tier, get_curated_nous_model_ids
    from nunmai_cli.models_pricing import get_pricing_for_provider
    from nunmai_cli.model_switch_providers import _free_tier_nous_row
    tier_row = _free_tier_nous_row({"name": "Nunmai Portal", "models": []})
    if tier_row is None:
        print("The Nunmai free tier is off for this install; sign in with `nunmai auth upgrade` to use Nunmai models.")
        return
    if tier_row["models"]:
    ...
    from nunmai_cli.nous_account import nous_policy_notice
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `nunmai auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from nunmai_cli.models import provider_model_ids
    ...
    # credentials may live only in the pool (``nunmai auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from nunmai_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from nunmai_cli.auth import (
        get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, DEFAULT_QWEN_BASE_URL)
    from nunmai_cli.models import fetch_api_models
...
def _copilot_catalog(api_key: str):
    ...
    from nunmai_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from nunmai_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from nunmai_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from nunmai_cli.models import fetch_api_models, copilot_model_api_mode
    provider_id = "copilot"
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Nunmai turns to `copilot --acp`.",
         "  Nunmai currently starts its own ACP subprocess for each request.",
         "  Nunmai uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set NUNMAI_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from nunmai_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from nunmai_cli.auth import PROVIDER_REGISTRY
    from nunmai_cli.config import save_env_value
    from nunmai_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from nunmai_cli.config import load_config, get_env_value
    from nunmai_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.nunmai/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``nunmai_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from nunmai_cli.main_provider_setup import _prompt_provider_choice
    from nunmai_cli.auth import ZAI_ENDPOINTS
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Nunmai typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Nunmai, enable billing on your Google Cloud project and regenerate",
...
def _api_key_provider_model_list(provider_id: str, pconfig, existing_key: str, key_env: str, effective_base: str) -> list:
    ...
    from nunmai_cli.config import get_env_value
    from nunmai_cli.models import _PROVIDER_MODELS, fetch_api_models, probe_profile_catalog
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY
    from nunmai_cli.config import save_env_value, load_config
    from nunmai_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from nunmai_cli.main_provider_setup import _run_anthropic_oauth_flow
    from nunmai_cli.config import save_env_value, save_anthropic_api_key
...
# ``_model_flow_api_key_provider``; ``nunmai_cli.main`` routes every registered profile missing
...
def _plugin_flow_external_process(provider_id: str, profile) -> tuple[str, str] | None:
    from nunmai_cli.auth import get_external_process_provider_status, resolve_external_process_provider_credentials
    status = get_external_process_provider_status(provider_id)
    _say(f"  {profile.display_name or provider_id} delegates Nunmai turns to a local `{status.get('command') or profile.process_command}` process.",
...
def _plugin_flow_oauth(provider_id: str, profile) -> tuple[str, str] | None:
    ...
    from nunmai_cli.auth_plugin_providers import plugin_missing_auth_handler_error
    ...
    if not status.get("logged_in"):
        ...
        _say(f"  ⚠ Not signed in to {profile.display_name or provider_id}.",
             f"  {missing.code if missing else status.get('hint') or f'Run `nunmai auth add {provider_id}` to sign in.'}")
        return None
...
def _is_profile_plugin_flow_provider(provider_id: str) -> bool:
    ...
    from nunmai_cli.auth_plugin_providers import plugin_profile
...
def _model_flow_plugin_provider(config, provider_id, current_model=""):
    """Generic ``nunmai model`` flow for a registered plugin profile: credential step by auth_type,
    catalog via ``merge_profile_catalog`` (live ``fetch_models()`` ⊕ ``fallback_models``), persist."""
    from nunmai_cli.auth_plugin_providers import plugin_profile
    from nunmai_cli.models import merge_profile_catalog
    del config
    profile = plugin_profile(provider_id)
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('nunmai_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('nunmai_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('nunmai_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('nunmai_cli.providers', 'custom_provider_slug'),
