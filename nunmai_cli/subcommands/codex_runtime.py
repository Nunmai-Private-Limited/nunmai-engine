"""``nunmai codex-runtime`` — noninteractive counterpart of the ``/codex-runtime`` slash command.

``nunmai codex-runtime migrate [--dry-run] [--json]`` runs the same ``~/.codex/config.toml``
migration the slash command triggers when the codex app-server runtime is enabled, on the
selected profile home (``nunmai -p NAME codex-runtime migrate``), so automation no longer has to
import ``nunmai_cli.codex_runtime_plugin_migration`` directly (issue #79023).
"""
...
def cmd_codex_runtime_migrate(args: argparse.Namespace) -> int:
    """Project Nunmai MCP servers (+ codex plugins) into ~/.codex/config.toml; 1 on any error."""
...
def build_codex_runtime_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        ...
        description="Noninteractive counterpart of the /codex-runtime slash command. Toggling the "
            ...
            "re-projects Nunmai' mcp_servers + installed codex plugins into the managed block of "
    ...
    migrate_parser = actions.add_parser(
        "migrate", help="Regenerate the nunmai-managed block in codex's config.toml",
        description="Idempotent: replaces the managed block, keeps user text verbatim, skips Nunmai "
    ...
    def _print_help(args):  # noqa: ANN001 — bare `nunmai codex-runtime` lists the actions
        parser.print_help()
        return 0
