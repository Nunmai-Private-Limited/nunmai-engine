"""``nunmai import-agent`` subcommand parser (logic lives in ``nunmai_cli/agent_import.py``)."""

from __future__ import annotations

from typing import Callable

from nunmai_cli.subcommands._shared import add_yes_flag
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent", help="Import a Claude Code or Codex CLI setup into Nunmai",
        description="One-command import of another coding agent's setup into Nunmai. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Nunmai equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'nunmai setup' for those.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Overwrite existing Nunmai items on name conflicts (default: skip)")
    ...
    parser.add_argument(
        ...
        help="Re-import every previously imported source whose files changed since the last "
             "import (registered in NUNMAI_HOME/import-sync.json). Runs without prompts; "
