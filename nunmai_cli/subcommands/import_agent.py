"""``nunmai import-agent`` subcommand parser.

Follows the ``nunmai claw`` pattern (see ``nunmai_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``nunmai_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Nunmai",
        description=(
            "One-command import of another coding agent's setup into Nunmai. "
