"""``nunmai setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Nunmai Engine with an interactive wizard. "
        "Run a specific section: "
        "nunmai setup model|tts|terminal|gateway|tools|telemetry|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'nunmai setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot Nunmai Portal setup: log in via OAuth, pick a Nunmai "
        "model, set Nunmai as the inference provider, and opt into the Tool "
