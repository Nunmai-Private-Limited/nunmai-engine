"""``nunmai sync`` subcommand parser — Skill Sync.

Cloned from ``nunmai_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    nunmai sync status                 show gate/opt-in/head state
    nunmai sync pull                   pull and materialize opted-in skills
    nunmai sync push                   push opted-in skills
    nunmai sync now                    reconcile: pull then push
    nunmai sync enable <skill>         opt a skill into sync
    nunmai sync disable <skill>        opt a skill out of sync
    nunmai sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    nunmai sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Nunmai token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  nunmai sync status            what is synced, and from where\n"
            "  nunmai sync enable my-skill   include a skill in your sync\n"
            "  nunmai sync now               pull, then push\n"
            "  nunmai sync propose my-skill  share a skill with your team\n"
        ),
