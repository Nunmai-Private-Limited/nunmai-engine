"""``nunmai usage`` — the account-limits block of the REPL ``/usage`` without starting a session.
...
def usage_snapshot_document(snapshot) -> dict:
    """``nunmai usage --json`` document. Schema is documented in website/docs/reference/cli-commands.md —
