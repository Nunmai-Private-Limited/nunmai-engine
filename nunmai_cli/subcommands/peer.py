...
A *peer* is another Nunmai gateway (any machine: homelab, Spark, Nunmai
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    nunmai peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``nunmai -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.nunmai/.env`` as
  ``NUNMAI_PEER_<NAME>_KEY``.
...
def cmd_peer(args) -> int:
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: nunmai peer list", file=sys.stderr)
            return 1
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Nunmai gateways)",
        description=(
            "Register other Nunmai gateways as peers and message their agents. "
