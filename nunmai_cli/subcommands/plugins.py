"""``nunmai plugins`` subcommand parser."""
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native Nunmai plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Bare plugin catalog entry name (see `nunmai plugins search`), Git URL, or owner/repo "
            "shorthand (e.g. anpicasso/nunmai-plugin-chrome-profiles)")
    plugins_install.add_argument(
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `nunmai plugins enable <name>`",
    )

    plugins_search = plugins_subparsers.add_parser(
        "search", help="Search the curated Nunmai plugin catalog")
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (nunmai-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a nunmai-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-nunmai-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a nunmai-pack.yaml file, or an https:// URL")
