"""``nunmai mcp`` subcommand parser.

Extracted from ``nunmai_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Nunmai as an MCP server",
        description=(
            "Manage MCP server connections and run Nunmai as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'nunmai mcp add' to connect to a new server, or\n"
            "'nunmai mcp serve' to expose Nunmai conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Nunmai as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `nunmai mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Nunmai-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `nunmai mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Nunmai-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `nunmai mcp install n8n`)",
    )
