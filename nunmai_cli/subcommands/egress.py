"""``nunmai egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `nunmai proxy` is the separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``nunmai proxy`` (dest='proxy_command').
