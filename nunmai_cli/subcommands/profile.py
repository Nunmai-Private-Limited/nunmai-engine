"""``nunmai profile`` subcommand parser."""
...
def build_profile_parser(subparsers, *, cmd_profile: Callable) -> None:
    ...
    profile_parser = subparsers.add_parser(
        "profile", help="Manage profiles — multiple isolated Nunmai instances")
    ...
    profile_create.add_argument(
        ...
        help="With --clone/--clone-from: also carry over the `nunmai import-agent` sync manifest so "
             "the new profile stays registered against the same Claude Code / Codex trees "
             "(`nunmai -p <name> import-agent --sync`). Never syncs config from the source profile.")
    profile_create.add_argument(
        "--no-alias", action="store_true", help="Skip wrapper script creation")
    profile_create.add_argument(
        "--no-skills", action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `nunmai update` skill sync)",
    )
    profile_create.add_argument(
        ...
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `nunmai profile describe`.")
    ...
    profile_purge = profile_subparsers.add_parser(
        ...
        description="Re-run the session/routing identity purge that `nunmai profile delete` performs "
    ...
    profile_migrate = profile_subparsers.add_parser(
        ...
        description="Re-run the session/routing identity migration that `nunmai profile rename` "
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description="Install a Nunmai profile distribution. SOURCE can be a git URL "
