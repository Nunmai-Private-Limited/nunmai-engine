"""``nunmai sessions`` subcommand parser."""
...
from nunmai_cli.subcommands._shared import add_json_flag, add_yes_flag
...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_export.add_argument("output", nargs="?", metavar="OUTPUT",
        help="Where to write. jsonl/html/trace: a file path, or a directory (existing, or ending in /) "
            ...
            "md/qmd: a directory, one file per session (default: <nunmai home>/session-exports)")
    ...
    _flag(sessions_prune, "--force",
        help="Run even while another Nunmai process (gateway, Desktop, dashboard, cron) holds state.db — rewriting the store under a live writer can leave every agent refusing turns until all writers are stopped")
    ...
    sessions_set_journal_mode = sessions_subparsers.add_parser(
        ...
        description="Switch the on-disk journal mode of the session store. Nunmai never "
    ...
    sessions_set_journal_mode.add_argument("--db", default=None, metavar="PATH",
        help="Convert another Nunmai SQLite store (e.g. kanban.db) instead of the profile's state.db")
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import", help="Import a Claude Code or Codex CLI session into Nunmai",
    ...
    # cmd_sessions lives in nunmai_cli/sessions_cmd.py; the parser is threaded
