"""``nunmai computer-use screen`` — the Bot Desktop screen a profile's ``computer_use`` drives on a
headless Linux gateway host, viewable from Nunmai Desktop. ``status`` / ``start`` / ``stop`` /
...
def _screen_status(args) -> int:
    ...
    if not st.installed:
        ...
        print("  Install: " + (st.install_command or "nunmai computer-use screen install"))
        return 1
    if st.running:
        ...
        print("  View it: Nunmai Desktop → Bots → this bot → Screen")
        return 0
    if st.blocker:
        print(f"Bot Desktop [{st.profile}]: installed, not running. {st.blocker}")
        return 1
    print(f"Bot Desktop [{st.profile}]: installed, not running. Start: nunmai computer-use screen start")
    return 1
...
def _screen_install(args) -> int:
    ...
    if missing:
        ...
        return 1
    print("Bot Desktop: ready. Start with `nunmai computer-use screen start` or from Nunmai Desktop.")
    return 0
...
def build_screen_parser(computer_use_sub, add_json_flag) -> None:
    screen = computer_use_sub.add_parser(
        ...
        description="On a headless Linux gateway host Nunmai gives each profile its own Xfce screen\n"
            "(TigerVNC Xvnc on a private Unix socket). The agent's computer_use and headed\n"
            "browser act on it; Nunmai Desktop shows it live and lets a human take over for\n"
    ...
    stop.add_argument("--force", action="store_true", help="Stop even while a human holds control from Nunmai Desktop")
