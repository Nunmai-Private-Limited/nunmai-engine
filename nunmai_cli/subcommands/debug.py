"""``nunmai debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nunmai Engine. Use 'nunmai debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nunmai debug share              Upload debug report (asks for confirmation)
    nunmai debug share --yes        Skip confirmation (for scripts/CI)
    nunmai debug share --lines 500  Include more log lines
    nunmai debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
...
""")
    ...
    share_parser.add_argument(
        ...
        help="Upload the debug bundle to Nunmai-internal storage (AWS S3) instead "
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'nunmai debug share'")
