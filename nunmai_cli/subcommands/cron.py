...
Extracted verbatim from ``nunmai_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.nunmai/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.nunmai/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        ...
        dest="monitor_script",
        help=(
            "Set/replace the monitor source script (see `nunmai cron create "
