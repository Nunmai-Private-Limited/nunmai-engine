"""``nunmai update`` subcommand parser."""
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update Nunmai Engine to the latest version",
    ...
    update_parser.add_argument(
        ...
        help="Show the update plan and exit without changing anything: install "
            "kind (git/docker/nix), every running Nunmai service across all "
    ...
    update_parser.add_argument(
        ...
        help="Print the processes the Windows venv-holder guard would refuse on as a JSON list "
            ...
            "Read-only; kind is gateway / backend (Desktop serve) / nunmai:<subcommand> / python, so a "
    ...
    update_parser.add_argument(
        ...
        help="Force a FULL pre-update backup (quick state snapshot + NUNMAI_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
        "--yes", "-y", action="store_true", default=False,
        help="Run without blocking on prompts: accepts the config-migration and stash-restore prompts, skips the fork-upstream prompt without adding a remote. API-key entry is skipped; run 'nunmai config migrate' separately for those.",
    )
    update_parser.add_argument(
    ...
    update_parser.add_argument(
        ...
        help="Update against this branch instead of the default (main). "
            "If the local checkout is on a different branch, nunmai will "
    ...
    update_parser.add_argument(
        ...
        help="Windows: proceed with the update even when another nunmai.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
    ...
    update_parser.add_argument(
        ...
        # rest of the update runs on the pulled code (nunmai_cli/update_handoff.py).
    )
