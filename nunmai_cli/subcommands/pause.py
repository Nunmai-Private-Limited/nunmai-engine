"""``nunmai pause`` / ``nunmai resume`` — the global emergency stop.

``nunmai pause`` writes the ESTOP sentinel at ``$NUNMAI_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``nunmai resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Nunmai paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `nunmai resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Nunmai resumed — dispatch picks up on the next tick.")
    else:
        print(f"Nunmai is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`nunmai resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `nunmai pause`",
