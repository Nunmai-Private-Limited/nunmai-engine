...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``invalid choice``. Registered WITHOUT ``help=`` so it is omitted from ``nunmai --help``
    ...
    login_parser = subparsers.add_parser(
        "login",
        description="Deprecated. Use `nunmai auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider", default=None, help="(deprecated) Provider name; ignored — see `nunmai model`")
