...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Nunmai log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nunmai logs                    Show last 50 lines of agent.log
    nunmai logs -f                 Follow agent.log in real time
    nunmai logs errors             Show last 50 lines of errors.log
    nunmai logs gateway -n 100     Show last 100 lines of gateway.log
    nunmai logs gui -f             Follow gui.log in real time
    nunmai logs desktop -f         Follow desktop.log (Electron app boot/backend)
    nunmai logs --level WARNING    Only show WARNING and above
    nunmai logs --session abc123   Filter by session ID
    nunmai logs --component tools  Only show tool-related lines
    nunmai logs --since 1h         Lines from the last hour
    nunmai logs --since 30m -f     Follow, starting from 30 min ago
    nunmai logs list               List available log files with sizes
""")
