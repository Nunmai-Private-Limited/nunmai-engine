"""``nunmai dashboard`` / ``nunmai serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `nunmai dashboard|serve`
    # cmdlines and SIGTERM them — the same path `nunmai update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Nunmai web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Nunmai web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="nunmai serve",
        description="Run the Nunmai backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nunmai Engine web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically")
    # Compat shim: desktop shells <= 0.15.x spawn `nunmai dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Nunmai backend server (headless; powers the desktop app and remote backends)",
        description="Run the Nunmai backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    _configure_serve_parser(serve_parser, cmd_dashboard=cmd_dashboard)

    # `register` is nested so bare `nunmai dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description="Register this install as a self-hosted dashboard with your Nunmai "
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Optional public HTTPS OAuth redirect URI for the dashboard, e.g. "
            "https://nunmai.example.com/auth/callback. Omit for localhost-only use.")
    dashboard_register_parser.add_argument(
        "--portal-url", dest="portal_url", default=None,
        help="Override the Nunmai Portal base URL for registration (default: the "
