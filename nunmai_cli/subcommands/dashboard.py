...
``nunmai_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `nunmai dashboard` / `nunmai serve` cmdlines and SIGTERM them directly —
    # the same path `nunmai update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Nunmai web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Nunmai web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Nunmai desktop app shells (<= 0.15.x) spawn the
    # backend as `nunmai dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Nunmai couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Nunmai Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        ...
        description=(
            "Run the Nunmai backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports NUNMAI_SERVE_HEADLESS=1). `dashboard` leaves it
