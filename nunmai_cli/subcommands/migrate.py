"""``nunmai migrate`` subcommand parser."""
...
def build_migrate_parser(subparsers) -> None:
    ...
    migrate_relay = migrate_subparsers.add_parser(
        "relay", help="Convert legacy NUNMAI_NEMO_RELAY_ATIF_*/ATOF_* exporter vars into relay-plugins.toml",
        description="The NeMo Relay cutover stopped reading the legacy exporter variables; a .env that still "
            "carries them (and no NUNMAI_NEMO_RELAY_PLUGINS_TOML) exports nothing. Generate "
            "<nunmai home>/relay-plugins.toml from them, point NUNMAI_NEMO_RELAY_PLUGINS_TOML at it, "
            "and comment the legacy lines out. `nunmai update` runs this for every profile automatically.")
    migrate_relay.add_argument(
        "--all-profiles", action="store_true",
        help="Migrate the default home and every named profile (what `nunmai update` does)")
    migrate_relay.add_argument(
...
def _cmd_migrate_relay(args) -> None:
    from nunmai_cli.relay_plugin_migrate import cmd_migrate_relay
