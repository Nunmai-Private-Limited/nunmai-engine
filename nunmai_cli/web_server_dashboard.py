...
from nunmai_cli.config import cfg_get, get_process_nunmai_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``nunmai_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Nunmai Teal until
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /nunmai``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__NUNMAI_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from nunmai_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from nunmai_cli.web_deps import _server

    # `nunmai serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("NUNMAI_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__NUNMAI_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `nunmai update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__NUNMAI_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `nunmai
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__NUNMAI_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__NUNMAI_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        ...
        from nunmai_cli.web_server_profiles import serving_profile_name as _serving_profile_name
        ...
        bootstrap_script = (
            ...
            f"window.__NUNMAI_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"window.__NUNMAI_INITIAL_PROFILE__={initial_profile_js};"
            f"window.__NUNMAI_DASHBOARD_PROFILE__={serving_profile_js};"
            f"</script>"
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.nunmai/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_nunmai_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``NUNMAI_HOME``.
    """
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``NUNMAI_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the nunmai root — so the default
    ...
    # context-local NUNMAI_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``NUNMAI_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the nunmai root (``~/.nunmai/plugins``). Scan the default root as well (``get_default_nunmai_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``NUNMAI_HOME`` unchanged when it
    # *is* the root), mirroring how ``nunmai_cli.plugins`` resolves plugin install locations. The
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``nunmai_cli/plugins.py`` and the documented user
    # contract.
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``nunmai_cli.plugins``."""
    plugins = []
    seen_names: set = set()
    ...
    try:
        ...
        for tname in provides_tools:
            ...
            if cached_result is None:
                ...
                return True, f"nunmai auth {name}"
    except Exception:
        pass
    return False, ""
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from nunmai_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from nunmai_cli.web_server import _get_dashboard_plugins
    from nunmai_cli.config import get_nunmai_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from nunmai_cli.plugins_cmd_catalog import removed_annotation
    from nunmai_cli.plugin_catalog import resolved_removed_entries
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        # Same verdict as `nunmai plugins list` / the TUI hub: name+key aliases for the lists, bundled
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.nunmai/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.nunmai/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from nunmai_cli.web_server import _get_dashboard_plugins, app
    try:
