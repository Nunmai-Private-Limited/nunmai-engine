...
``nunmai mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nunmai plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nunmai mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nunmai mcp install <name>    Picker: nunmai mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Nunmai is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Nunmai — run `nunmai update` "
...
def run_picker() -> None:
    """`nunmai mcp picker` (and default `nunmai mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        print(color(
            ...
            "Run `nunmai mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
