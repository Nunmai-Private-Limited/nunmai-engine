...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``nunmai memory setup`` — if the update-time refresh only reinstalled the declared ``hindsight-client``,
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    no-ops when nothing drifted) — how ``nunmai update`` heals a provider after a venv rebuild.
    ...
    satisfied ranges. This is how ``nunmai update`` heals the active memory provider after a venv
    ...
    # install_specs routes to the durable target on sealed hosted images (NUNMAI_LAZY_INSTALL_TARGET)
...
def _post_setup_hook(provider, config: dict) -> bool:
    ...
    if hasattr(provider, "post_setup"):
        provider.post_setup(str(get_nunmai_home()), config)
        return True
    return False
...
def cmd_setup(args) -> None:
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, str(get_nunmai_home()))
        except Exception as e:
...
def _write_env_vars(
    env_writes: dict, nunmai_home: str | os.PathLike[str] | None = None) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``NUNMAI_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``nunmai_home`` is applied via the
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        if url and not is_set:
                            line += f"  → {url}"
                        print(line)
                print("  Note: systemd/gateway services do not inherit ~/.nunmai/.env —")
