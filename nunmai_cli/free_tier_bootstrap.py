"""Serve-start bootstrap for the Nunmai free tier: the ONE place a free-tier identity is created.

Every Nunmai process that may need the free tier runs this once at boot (``nunmai serve`` on a
...
(:func:`nunmai_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
class SetupRecord:
    ...
    has_identity: bool             # a Nunmai identity (free tier or account) is on disk
...
def reconcile_record() -> Optional[SetupRecord]:
    ...
    writes this process never saw (``nunmai setup`` / ``nunmai model`` from a shell, a hand edit).
    ...
    if record is None or record.provider_configured:
        return record
    from nunmai_constants import get_process_nunmai_home, nunmai_home_key
    if nunmai_home_key() != nunmai_home_key(get_process_nunmai_home()) or _inventory_stamp == _config_stamp():
        return record
...
def _build_record(*, other: bool, force: bool) -> SetupRecord:
    ...
    if anon_auth.guest_enabled():
        try:
            ...
            error = str(exc)
            logger.info("Nunmai free tier not set up at boot: %s", exc)
        if state is None:
...
def _retry_until_settled() -> None:
    ...
    for _ in range(BOOTSTRAP_RETRY_ATTEMPTS):
        ...
        if record.has_identity:
            logger.info("Nunmai free tier set up after a boot-time retry")
            return
...
def start_background_bootstrap() -> threading.Thread:
    """``nunmai serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
