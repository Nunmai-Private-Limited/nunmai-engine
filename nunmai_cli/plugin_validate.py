"""``nunmai plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``NUNMAI_HOME``), so a crashing or
...
from nunmai_cli.plugin_validate_desktop import check_desktop_surface
from nunmai_cli.plugins_manifest import _CONFIG_SCHEMA_TYPES
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "NUNMAI_VALIDATE_JSON:"
...
def _requires_nunmai_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_nunmai`` spec.

    Unlike :func:`nunmai_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from nunmai_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_nunmai(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_nunmai") or "").strip()
    if not spec:
        report.add("requires_nunmai", True, "not declared")
        return
    if _requires_nunmai_spec_valid(spec):
        report.add("requires_nunmai", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_nunmai",
            False,
            f"requires_nunmai spec {spec!r} does not parse "
...
# from nunmai so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "nunmai_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="nunmai-validate-") as scratch:
        env = dict(os.environ)
        env["NUNMAI_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_nunmai(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that Nunmai can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from nunmai_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from nunmai_cli.plugin_python_deps import applicable_specs, unsupported_specs
