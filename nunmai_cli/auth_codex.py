...
# ``nunmai auth`` from a named profile re-signs the ROOT store (93889b770da, #114012).
...
def _recover_codex_tokens_from_cli(
    ...
    from nunmai_cli.auth import _import_codex_cli_tokens, _provider_state_transaction, _save_codex_tokens
    ...
    with _provider_state_transaction("openai-codex") as (_store, state, _source):
        ...
        if known and _codex_principal_identity(imported["access_token"]) not in (None, known):
            logger.warning(
                ...
                "workspace than the Nunmai credential. Run `%s` to re-authenticate it.",
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    """
    from nunmai_cli.auth import _provider_state_transaction, _save_codex_tokens, refresh_codex_oauth_pure
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another Nunmai process) rotates the shared token this frozen copy fails with a
...
def _refresh_expired_codex_probe_token(
    ...
    """
    from nunmai_cli.auth import _codex_quota_probe_cache
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    from nunmai_cli.auth import _nonempty_str, read_credential_pool
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    ...
    ``auth.codex_login_flow``). Tokens stored in ~/.nunmai/auth.json."""
    from nunmai_cli.auth import (
    ...
    from nunmai_cli.auth_codex_browser import codex_oauth_login
    ...
    # Run a fresh OAuth flow — Nunmai gets its own session (device code unless the user opted in
    # to the browser flow).
    print()
