...
Tokens live in ~/.nunmai/auth.json, NOT ~/.codex/: Nunmai keeps its own Codex OAuth session
...
_NO_CREDENTIALS_MSG = "No Codex credentials stored. Run `nunmai auth` to authenticate."
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from nunmai_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("NUNMAI_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import _load_provider_state, _nonempty_str
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``nunmai setup`` / the model picker) is always
    synced. ``manual:device_code`` (``nunmai auth add openai-codex``) is synced only when its
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    ...
    from nunmai_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def _ssl_interop_hint(exc: BaseException) -> str:
    ...
    return (
        ...
        " https://nunmai.in/docs/integrations/providers"
    )
...
def refresh_codex_oauth_pure(
    ...
    from nunmai_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    from nunmai_cli.auth import _save_codex_tokens, refresh_codex_oauth_pure
    try:
        ...
        # another Nunmai process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from nunmai_cli.auth import _codex_access_token_is_expiring
    ...
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    if _should_refresh(access_token):
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="nunmai-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from nunmai_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _codex_login_rate_limited_error(response: "httpx.Response", *, during: str = "") -> AuthError:
    ...
    # callers surface a "retry later" notice instead of a misleading "run nunmai auth" prompt (see issue
    # #32790).
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import _utc_now_z
