"""``nunmai kanban …`` — dispatch (``kanban_command``), task-verb handlers, ``run_slash`` for ``/kanban``.
...
from nunmai_cli.kanban_output import (
...
from nunmai_cli.kanban_boards import _dispatch_boards
from nunmai_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from nunmai_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: nunmai_cli.main, run_slash)
...
def _check_dispatcher_presence(nunmai_home: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this NUNMAI_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``nunmai_home`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different NUNMAI_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=nunmai_home, use_cache=False)
    except Exception:
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``nunmai kanban …``; returns a shell-style exit code."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: nunmai kanban <action> [options]\n"
                  "Run 'nunmai kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    # `--board <slug>` pins NUNMAI_KANBAN_BOARD for the duration of this call so it inherits the
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh NUNMAI_HOME.
        try:
            kb.init_db()
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.nunmai/profiles/.\n"
              "Create one with `nunmai -p <name> setup` before assigning tasks.")
    print(
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            creator_task_id=(os.environ.get("NUNMAI_KANBAN_TASK")
...
def _cmd_link(args: argparse.Namespace) -> int:
    ...
    expected_child_run_id = (
        ...
        if args.child_id == os.environ.get("NUNMAI_KANBAN_TASK") else None)
    with kbc.connect_closing() as conn:
    ...
    if gated:
        print(
            ...
            f"completes; use `nunmai kanban unlink {args.parent_id} {args.child_id}` "
            f"to run it now."
        )
    return 0
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    ...
    if os.environ.get("NUNMAI_KANBAN_TASK") != task_id or not raw:
        return None
    try:
        return int(raw)
    except ValueError:
        ...
        def op(tid):
            ...
            try:
                ...
                fail_msg[tid] = (f"cannot complete {tid}: a live worker is running it. Wait for the "
                                 f"worker, `nunmai kanban reclaim {tid}` to release it, or re-run with "
            ...
            if not done:
                ...
                if blockers:
                    ...
                    fail_msg[tid] = (f"cannot complete {tid}: unsatisfied parent dependencies: {detail}; "
                                     f"complete the parents first, or `nunmai kanban unlink <parent> {tid}`.")
            return done
