"""
Interactive setup wizard for Nunmai Engine.
...
from nunmai_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
_DOCS_BASE = "https://nunmai.in/docs"
...
# display_nunmai_home imported lazily at call sites (stale-module safety during nunmai update)
...
from nunmai_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nunmai_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Nunmai using environment variables or config commands:")
    print_info("  nunmai config set model.provider custom")
    print_info("  nunmai config set model.base_url http://localhost:8080/v1")
    print_info("  nunmai config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nunmai setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "nunmai_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``NUNMAI_NONINTERACTIVE=1`` (see ``nunmai_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("NUNMAI_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``NUNMAI_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nunmai_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nunmai setup')")


def _print_setup_summary(config: dict, nunmai_home):
    ...
    try:
        from nunmai_cli.auth import resolve_provider
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nunmai setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from nunmai_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `nunmai tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from nunmai_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nunmai setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nunmai setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'nunmai tools' → Speech-to-Text")
            )
    ...
    # Spotify (OAuth via nunmai auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nunmai setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nunmai_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nunmai setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nunmai setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nunmai setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nunmai setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nunmai setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nunmai config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nunmai config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nunmai config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nunmai', Colors.GREEN)}              Start chatting")
    print(f"   {color('nunmai gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nunmai doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nunmai tools` and `nunmai setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nunmai model``)
    ...
    provider added to ``nunmai model`` is automatically available here.
    ...
    # Delegate to the shared nunmai model flow — handles provider picker,
    ...
    # on demand via `nunmai auth add`, `nunmai setup` vision, and
    # `nunmai setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from nunmai_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nunmai model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Nunmai auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nunmai setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nunmai setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nunmai runs shell commands and code.")
    ...
    # under ~/.nunmai/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nunmai setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nunmai setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `nunmai egress setup` then `nunmai egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `nunmai egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'nunmai-engine[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $NUNMAI_HOME/bin is never on PATH, so a bare
            # which() misses the uv Nunmai installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'nunmai-engine[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NUNMAI_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NUNMAI_MAX_ITERATIONS")
    ...
    print_info("  Run `nunmai setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NUNMAI_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.nunmai/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from nunmai_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_nunmai_home(Path(get_nunmai_home()))
    except Exception:
        pass
...
def _profile_name_from_nunmai_home(nunmai_home) -> str | None:
    """Return the active profile name when NUNMAI_HOME is a profile dir."""
    if nunmai_home.parent.name == "profiles":
        return nunmai_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nunmai delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nunmai to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nunmai_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nunmai.in/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nunmai.in/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nunmai config edit")
    print_info("   Open config in your editor:  nunmai config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nunmai_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nunmai from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nunmai setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nunmai config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `nunmai import` or `nunmai setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nunmai setup tools` and `nunmai tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Nunmai profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from nunmai_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nunmai_cli.config
    ...
    if section_key == "model":
        ...
        from nunmai_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_nunmai.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_nunmai migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nunmai", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Nunmai.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nunmai to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nunmai at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nunmai at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nunmai at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nunmai at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nunmai equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nunmai config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nunmai.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nunmai's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nunmai_home: Path) -> bool:
    ...
    print_info("Nunmai can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nunmai claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=nunmai_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nunmai claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nunmai configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nunmai config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nunmai (use nunmai claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Nunmai session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``nunmai setup`` and hunt for the quick-setup option.
    ...
    print_info("  Run `nunmai portal info` to inspect routing.")
    print_info("  Run `nunmai` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from nunmai_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``nunmai model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      nunmai setup           — full or quick (auto-detected)
      nunmai setup model     — just model/provider
      nunmai setup tts       — just text-to-speech
      nunmai setup terminal  — just terminal backend
      nunmai setup gateway   — just messaging platforms
      nunmai setup tools     — just tool configuration
      nunmai setup telemetry — just local shared metrics
      nunmai setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│  Let's configure your Nunmai Engine installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, nunmai_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Nunmai configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nunmai setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nunmai_home)
        if migration_ran:
            config = load_config()
    ...
    print_info(f"Data folder:  {nunmai_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nunmai config edit'")
    ...
    # Tune later with `nunmai setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from nunmai_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, nunmai_home)
...
def _run_first_time_quick_setup(config: dict, nunmai_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nunmai setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `nunmai import` from another machine).
        from nunmai_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    nunmai setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nunmai setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Nunmai.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Nunmai update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``nunmai-engine`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Nunmai) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``nunmai tools`` (which
    ...
    try:
        ...
        from nunmai_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``nunmai setup agent`` /
    ``nunmai config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, nunmai_home, is_existing: bool):
    ...
    print_info("essential `nunmai-engine` skill is always kept so the agent")
    print_info("can help you drive and configure Nunmai itself.")
    print()
    ...
    if path == 0:
        ...
        # `nunmai update` runs don't re-inject them. Essential skills (the
        # `nunmai-engine` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        nunmai tools")
        print_info("  Seed skills:         nunmai skills opt-in --sync")
        print_info("  Add MCP servers:     nunmai mcp add")
        print_info("  Enable plugins:      nunmai plugins")
        print_info("  Tune agent settings: nunmai setup agent")
        print()
        _print_setup_summary(config, nunmai_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, nunmai_home)


def _blank_slate_walkthrough(config: dict, nunmai_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `nunmai-engine` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `nunmai-engine`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`nunmai update` runs from re-injecting them. Opt back in any")
            print_info("time with `nunmai skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `nunmai tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `nunmai plugins list` / `nunmai plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `nunmai plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `nunmai mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `nunmai mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   nunmai tools")
    print_info("  Seed skills:         nunmai skills opt-in --sync")
    print_info("  Add MCP servers:     nunmai mcp add")
    print_info("  Tune agent settings: nunmai setup agent")
    print()
...
def _run_quick_setup(config: dict, nunmai_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nunmai setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nunmai to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nunmai setup gateway'.")
