"""Interactive setup wizard for Nunmai Engine (config lives in ~/.nunmai/).
...
``nunmai_cli.setup.<name>`` keep working.
"""
...
# Config helpers are re-exported (tests patch them on this module). display_nunmai_home is
# imported lazily at call sites (stale-module safety during nunmai update).
from nunmai_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_nunmai_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_nunmai_home,
)
from nunmai_cli.colors import Colors, color
from nunmai_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Nunmai using environment variables or config commands:",
          "  nunmai config set model.provider custom",
          "  nunmai config set model.base_url http://localhost:8080/v1",
          "  nunmai config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'nunmai setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("nunmai_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from nunmai_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``nunmai model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``NUNMAI_NONINTERACTIVE=1`` (``nunmai_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("NUNMAI_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``NUNMAI_NONINTERACTIVE=1`` or a
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'nunmai setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``nunmai model`` flow (one code
    ...
    try:
        ...
        from nunmai_cli.auth_error_copy import provider_setup_failure_lines
        lead, *rest = provider_setup_failure_lines(exc, retry_command="nunmai model")
        print_warning(lead)
        for line in rest:
            print_info(line)
    ...
    # keep safe defaults (configure via `nunmai auth add` / `nunmai setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into NUNMAI_MAX_ITERATIONS);
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `nunmai setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.nunmai/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives NUNMAI_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`nunmai setup tools` == `nunmai tools`: platform selection → toolset toggles → provider keys.
...
_SEND_CONSENT_EXPLAINER = (
    "", "Sending uploads each daily package to the Nunmai telemetry",
...
def setup_telemetry(config: dict):
    ...
    _info("Shared metrics contain only bounded counters and histograms.",
          "Collection is local. Sending them to Nunmai is a separate opt-in.")
    ...
    shared_metrics["send"] = prompt_yes_no("Send shared metrics to Nunmai?", default=shared_metrics.get("send") is True)
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from nunmai_cli.observability.shared_metrics import SharedMetricsStore
        from nunmai_cli.observability.shared_metrics_sender import reconcile_send_consent
        from nunmai_cli.sqlite_util import write_txn
...
# nunmai_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from nunmai_cli.setup_tts import setup_tts  # noqa: E402
from nunmai_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from nunmai_cli.setup_platforms import setup_gateway  # noqa: E402
from nunmai_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from nunmai_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from nunmai_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``nunmai setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ☤ Nunmai Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, nunmai_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {nunmai_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'nunmai config edit'")
    if migration_ran:
...
_FIRST_TIME_MODES = (
    ("Quick Setup (Nunmai Portal) — free OAuth login, no API keys, model + tools (recommended)",
...
def _run_setup_wizard_impl(args):
    ...
    ``nunmai setup <section>`` from SETUP_SECTIONS."""
    ...
    if getattr(args, 'non_interactive', False) or not is_interactive_stdin():
        ...
        return
    if getattr(args, "portal", False):  # one-shot Nunmai Portal setup; skips the rest
    ...
    _print_banner("│             ☤ Nunmai Engine Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Nunmai Engine installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, nunmai_home))])
            return
        ...
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'nunmai setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(nunmai_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Nunmai?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from nunmai_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, nunmai_home, is_existing))])
            return
    _run_full_setup(config, nunmai_home, is_existing=is_existing, migration_ran=migration_ran)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('nunmai_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('nunmai_constants', 'get_optional_skills_dir'),
