"""PTY bridge for `nunmai dashboard` chat tab.
...
``nunmai_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/nunmai-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "NUNMAI_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Nunmai Engine supports Windows only via WSL.")
