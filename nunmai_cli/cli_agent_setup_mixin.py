"""Agent construction + session-resume display for ``NunmaiCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``nunmai chat -q``.
...
def _credential_pool_notice(provider: str) -> tuple:
    ...
    if dead:
        ...
        lines.append(f"The {provider} sign-in was lost ({reason}); run `nunmai auth add {provider}` "
                     "to sign in again.")
...
_RESUME_SKIN_COLORS = (
    ("banner_text", "#EFE9D7"), ("session_label", "#D6B36C"), ("session_border", "#8B8682"),
    ("ui_ok", "#8FBC8F"))
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``NunmaiCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        try:
            # target_model: the ladder's model-keyed rungs (Zen/Go api_mode, Copilot/Nunmai
            # api_mode) must see the model this CLI will actually send, not config's `default`,
            # or `nunmai -m mimo-v2.5 --provider opencode-go` resolves an api_mode/base_url the
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'nunmai model' to choose a provider, or "
                      "'nunmai setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: nunmai setup")
            return False
        ...
        # is just the provider slug/display name (`nunmai chat --model <provider-name>`
        ...
        # Still empty (e.g. `nunmai auth add` without `nunmai model`): fall back to the
    ...
    def _explain_unusable_credentials(self, error) -> bool:
        ...
        from nunmai_cli.auth import format_auth_error
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(nunmai chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (nunmai sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        try:
            ...
            console = ChatConsole()
            from nunmai_cli.cli_chat_error_copy import agent_init_failure_message
            console.print(f"[bold red]{_escape(agent_init_failure_message(e))}[/]")
            from nunmai_constants import partial_update_hint
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (nunmai sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ Nunmai: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ Nunmai: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
