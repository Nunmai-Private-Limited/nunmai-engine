"""Agent-construction and session-resume display methods for ``NunmaiCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.nunmai/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``NunmaiCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        # `nunmai chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `nunmai auth add openai-codex`
        # without `nunmai model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (nunmai sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (nunmai sessions list).{_RST}")
                return False
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "event":
                ...
                lines.append("  ◆ Nunmai: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Nunmai: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
