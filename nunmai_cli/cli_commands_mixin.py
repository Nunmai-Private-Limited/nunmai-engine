...
``cli.py``'s ``NunmaiCLI`` class. ``NunmaiCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Nunmai did)
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: nunmai --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Nunmai changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Nunmai hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Nunmai config/state.
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from nunmai_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `nunmai profile import <archive>`.")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `nunmai journey`).
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from nunmai_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run nunmai chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in nunmai_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `nunmai gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `nunmai sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `nunmai sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `nunmai -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``nunmai -w``).

        ``prune`` is the same attended reclaim as ``nunmai worktree prune``
        (nunmai_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `nunmai -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `nunmai -w`.
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=os.environ.get("NUNMAI_SESSION_SOURCE", "cli"),
                model=self.model,
                model_config={
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through nunmai_cli.personality —
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from nunmai_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to nunmai_cli.curator so the CLI and the `nunmai curator`
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to nunmai_cli.skills_hub."""
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``nunmai bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: nunmai bundles create "
        ...
        _cprint(
            ...
            f"Manage with `nunmai bundles`.{_RST}"
        )
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `nunmai cron`.
        """
        ...
        _cprint(
            ...
            f"lives only while this Nunmai process runs — use `nunmai cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from nunmai_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Nunmai keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from nunmai_cli.goals import draft_contract
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_nunmai_home()}/skins/\n")
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Nunmai is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Nunmai is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Nunmai is busy; /stop still cancels it."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from nunmai_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Nunmai Engine to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``nunmai update`` so the user sees update output directly and gets
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nunmai Engine"),
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Nunmai' hotword listener.
