...
disk cache). Split out of ``nunmai_cli.models``; helpers still defined there are looked up on
``nunmai_cli.models`` at call time so ``patch("nunmai_cli.models.<name>")`` mocks keep intercepting.
"""
...
from nunmai_cli.urllib_security import url_origin
...
def probe_ollama_local_models(
    ...
    from nunmai_cli.models import _NUNMAI_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _NUNMAI_USER_AGENT, **(headers or {})}
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from nunmai_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    return {"User-Agent": _NUNMAI_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
