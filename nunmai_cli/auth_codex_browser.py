...
``nunmai auth add openai-codex --browser`` (or ``auth.codex_login_flow: browser``) sends the
...
from nunmai_cli.auth_constants import AuthError, CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL, _codex_err
...
def codex_oauth_login(args: Any) -> Dict[str, Any]:
    ...
    from nunmai_cli import auth as auth_mod  # late: ``nunmai_cli.auth.<name>`` patches must intercept
...
def _codex_browser_exchange_code(code: str, *, redirect_uri: str, code_verifier: str) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth_codex import _codex_login_post, _codex_login_rate_limited_error
...
def _codex_browser_login(
    *, open_browser: bool = True, timeout_seconds: Optional[float] = None) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth_codex import _codex_base_url
    ...
    print("(Nunmai creates its own session — won't affect Codex CLI or VS Code)")
    print()
    print(f"Open this URL to authorize Nunmai:\n  {auth_url}\n")
