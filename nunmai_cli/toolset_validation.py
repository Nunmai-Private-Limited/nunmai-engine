...
the tool registry or launching Nunmai (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``nunmai-cli`` to the non-existent ``nunmai``. ``resolve_toolset('nunmai')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``nunmai-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``nunmai`` instead of ``nunmai-cli``),
