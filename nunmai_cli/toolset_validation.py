...
from nunmai_cli.platforms import PLATFORMS
from nunmai_cli.toolset_scope import toolset_allowed_for_platform

_NO_TOOLS = "the agent will have no tools on this platform. Run `nunmai tools` to reconfigure."
...
def parse_platform_toolsets_value(value: object) -> Optional[List[str]]:
    ...
    Older ``nunmai config set`` builds stored a bare ``[...]`` argument as a plain string, so an
    ...
    ``nunmai doctor`` and ``nunmai plugins enable`` agree on what the user configured. Any other
...
def _platform_default_toolset(platform: object) -> str:
    ...
    return info.default_toolset if info is not None else f"nunmai-{platform}"
...
def _platform_default_is_valid(
    ...
    # nunmai-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    Reports: a toolset name ``is_valid_toolset`` rejects (suggesting ``nunmai-<platform>`` when that
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if toolsets is None:
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `nunmai tools` to configure explicitly.")
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `nunmai tools` to reconfigure.")
    return warnings
