...
Wraps the same config layer the CLI uses (nunmai_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from nunmai_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import NunmaiTokenStorage, force_interactive_oauth, login_connect_timeout
        ...
        try:
            ...
            secret_token = set_secret_scope(
                build_profile_secret_scope(Path(flow.nunmai_home)), profile_home=flow.nunmai_home)
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, nunmai_home=flow.nunmai_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, nunmai_home=flow.nunmai_home)
                    raise
        finally:
