...
# ``nunmai update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``nunmai update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``nunmai serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``nunmai_constants`` (stdlib-only), never ``nunmai_cli.config``: the receipt must be
    ...
    return get_nunmai_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``nunmai update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def settle_latest_receipt_fleet(fleet: list[dict[str, Any]], *, discharges) -> bool:
    ...
    pre-pull SHAs) keeps ``nunmai update`` exiting 1 and every CLI start warning about mixed
    modules, long after the operator's ``nunmai gateway restart`` fixed the fleet (#117051). The
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from nunmai_cli.profiles import _get_default_nunmai_home, _get_profiles_root, _PROFILE_ID_RE
...
def _code_root_for_path(raw: Any) -> Optional[Path]:
    """Return the Nunmai checkout containing an absolute process path."""
    ...
    with suppress(Exception):
        ...
        for parent in [candidate, *candidate.parents][:_CODE_ROOT_MAX_DEPTH]:
            if (parent / "nunmai_cli" / "main.py").is_file():
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `nunmai gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if RESTART_PENDING_STATE in states:
        ...
        print("    verify afterwards with `nunmai gateway status`.")
    ...
    if stale_or_down:
        ...
        print("  Run `nunmai gateway restart` (or `nunmai -p <profile> gateway restart` for a named")
        print("  profile), then `nunmai gateway status` to confirm.")
