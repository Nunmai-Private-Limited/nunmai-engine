...
1. **Update receipt** — a machine-readable JSON record of what one
   ``nunmai update`` run discovered, did, skipped (and why), written to
   ``<NUNMAI_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``nunmai_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``nunmai update`` is a single-threaded CLI
...
def _receipt_dir() -> Path:
    ...
    return get_nunmai_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``nunmai update`` has many early-termination paths (Windows
