...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(
        ...
        f"Nunmai is waiting for the OAuth callback on {redirect_uri}\n"
...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from nunmai_cli.auth import _default_verify
    ...
    effective_ca = (
        ca_bundle or tls_state.get("ca_bundle") or os.getenv("NUNMAI_CA_BUNDLE")
...
def _poll_device_token_generic(
    ...
    """RFC 8628 device-code polling loop shared by the Nunmai and xAI flows.
...
def _poll_for_token(
    ...
    """Poll the Nunmai token endpoint until the user approves or the code expires."""
    ...
    def _error(_response, error_payload) -> Exception:
        ...
        from nunmai_cli.auth_error_copy import device_flow_error
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    try:
        ...
        if isinstance(api_key, str) and api_key and not is_expiring(api_key, 60):
            print(f"Existing {display_name} credentials found in Nunmai auth store.")
