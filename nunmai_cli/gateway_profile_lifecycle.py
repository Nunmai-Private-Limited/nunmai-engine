...
from nunmai_constants import get_nunmai_home, get_default_nunmai_root
from nunmai_cli.profiles import parked_marker_path, profile_is_parked, profile_is_standalone, profiles_to_serve
...
def profile_lifecycle(command: str, args) -> bool:
    ...
    if command == "start":
        ...
        host_home = owner.home if owner is not None else get_default_nunmai_root()
    ...
    if command == "stop":
        ...
        if _confirmed(answer, "unserved", name):
            print(f"Profile '{name}' parked; its bots and cron are stopped. "
                  f"Start again with: nunmai -p {name} gateway start")
        else:
...
def print_parked_status() -> bool:
    ...
    from nunmai_cli import gateway as gw
    name = gw._current_profile_name()
    if name and name != "default" and profile_is_parked(get_nunmai_home()):
        print(f"Profile '{name}': parked (nunmai -p {name} gateway start)")
        return True
    if not name or name == "default":
        ...
        if parked:
            ...
            for profile in parked:
                print(f"Profile '{profile}': parked (nunmai -p {profile} gateway start)")
    return False
