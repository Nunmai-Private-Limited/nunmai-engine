...
- **gateway profiles**, relaunched through the existing per-profile
  ``nunmai_cli.main -p <profile> gateway restart`` command; and
- **``nunmai-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``nunmai serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``nunmai serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/nunmai-serve``,
``system/nunmai-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "NUNMAI_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_NUNMAI_GATEWAY", "NUNMAI_GATEWAY", "NUNMAI_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
from nunmai_constants import PROFILE_ID_RE as _PROFILE_ID_RE

_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^nunmai-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "nunmai-serve*"
...
def _launch_profile() -> str:
    """Profile this recovery process was launched as: ``<root>/profiles/<name>`` in ``NUNMAI_HOME`` names it,
    anything else is the default profile. Mirrors ``nunmai_cli.profiles.profile_root_for_env_home`` without
    importing it — this module stays stdlib-only at import time."""
    home = os.environ.get("NUNMAI_HOME", "").strip()
    if home:
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "nunmai_cli.main", "-p", profile, "gateway", "restart"]
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return ("nunmai-gateway.service", "gateway.service", "gateway-default.service")
    return (f"nunmai-gateway-{profile}.service", f"gateway-{profile}.service")
...
def _host_state_dir() -> str:
    ...
    This module imports no Nunmai code at runtime — importing the freshly pulled tree is exactly
    what aborted the phase that calls us — so the rule is duplicated here rather than shared.
    """
    override = os.environ.get("NUNMAI_GATEWAY_LOCK_DIR")
    if override:
        return override
    ...
    return os.path.join(state_home, "nunmai", "gateway-locks")
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    ``systemctl`` comes from ``shutil.which`` so this module never imports a Nunmai platform helper —
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    if result is None:
        return []
    # The glob is a systemd pattern, not a name gate (`nunmai-serve*` also matches
    # `nunmai-server.service`): require the exact base unit or the profile family.
    units: list[str] = []
...
def _normalized_skips(skip_units: Iterable[Any]) -> tuple[set[tuple[str, str]], set[str]]:
    """Split already-settled units into scope-qualified and legacy entries (a bare ``"nunmai-serve"``
...
def restart_serve_units(
    *, skip_units: Iterable[Any] = (), run: Callable[..., Any] = subprocess.run, sleep: Callable[[float], Any] = time.sleep
    ...
    """Restart every active ``nunmai-serve*`` systemd unit from this process.

    Units are enumerated from systemd, never from the update inventory, so a manually launched or
    Desktop-owned ``nunmai serve`` (no unit) structurally cannot be touched here.
    """
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # scope (`nunmai-serve.service` can exist in BOTH managers; settling one says nothing about
