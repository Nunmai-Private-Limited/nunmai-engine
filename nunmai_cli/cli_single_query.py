...
def _run_kanban_goal_loop_q(cli: "NunmaiCLI", first_response: str, run_turn=None, log=None) -> None:
...
def _run_kanban_goal_loop_chat(cli: "NunmaiCLI", first_response: str) -> None:
...
def _single_query_exit_code(result, *, credentials_rate_limited: bool = False) -> int:
    ...
    failed, so ``result`` is not a dict). A Kanban worker (``NUNMAI_KANBAN_TASK`` set) that
    ...
    if not isinstance(result, dict):
        if credentials_rate_limited and os.environ.get("NUNMAI_KANBAN_TASK"):
            from nunmai_cli.kanban_db import KANBAN_RATE_LIMIT_EXIT_CODE
    ...
    if os.environ.get("NUNMAI_KANBAN_TASK"):
        ...
        if reason in _TERMINAL_PROVIDER_REASONS:
            from nunmai_cli.kanban_db import KANBAN_TERMINAL_PROVIDER_EXIT_CODE
...
def _run_quiet_single_query(cli, effective_query, emitter=None):
    ...
    from nunmai_cli.quiet_single_query import (
...
def _run_single_query_mode(cli, query, image, quiet, oneshot, stream_json: bool = False):
    ...
    from nunmai_cli.quiet_single_query import exit_single_query
    ...
    try:
        ...
        if quiet:
            ...
            if stream_json:
                ...
                from nunmai_cli.stream_json import StreamJsonEmitter
