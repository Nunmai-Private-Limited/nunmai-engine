"""ZIP-download fallback for ``nunmai update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``nunmai_cli.update_cmd.<name>`` keeps
...
_ZIP_PRESERVED_NESTED = {
    ...
    "ui-tui": ("dist", "node_modules", "packages/nunmai-ink/dist"),
    "web": ("node_modules",),
}

_STASH_HINT = "  Stash or commit your changes, then rerun `nunmai update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``nunmai_cli.main`` re-export surface.
...
def _zip_overlay_block_reason(
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.nunmai-update-staging`` siblings that git
...
def _is_zip_preserved_entry_status_line(line: str, shipped: Optional[Collection[str]] = None) -> bool:
    ...
    ship (``.bytecode-fingerprint``, ``.nunmai-bootstrap-complete``, ``nunmai_agent.egg-info/`` — the swap
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``nunmai-engine-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, nunmai_constants.py, ...). Check up front
...
def _link_or_copy_artifact(source: str, destination: str) -> None:
    ...
    valid after the swap unlinks the old tree; on Windows a link also succeeds on a locked Nunmai.exe
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    try:
        ...
        print("  Re-run `nunmai update` to retry; if the agent won't start, reinstall from https://nunmai.in")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False, _windows_gateway_resume=None) -> bool:
    ...
    from nunmai_cli.update_cmd import _hand_off_post_swap, _m, _read_project_version, _resolve_update_options, _sweep_bytecode_after_update
    ...
    _download_and_swap_zip(branch, f"https://github.com/Nunmai-Private-Limited/nunmai-engine/archive/refs/heads/{branch}.zip")
