"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `nunmai tools`."""
...
from nunmai_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from nunmai_cli.nous_account import format_nous_portal_entitlement_message
from nunmai_cli.nous_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, NousSubscriptionFeatures
from tools.tool_backend_helpers import NOUS_MANAGED_PROVIDER, fal_key_is_configured
from utils import base_url_hostname, is_truthy_value

logger = logging.getLogger("nunmai_cli.tools_config")
...
def _visible_providers(
    ...
    Nunmai-managed rows (``managed_nous_feature``) are always shown, even logged-out/unentitled, to
    advertise the capability."""
    from nunmai_cli.tools_config import get_nous_subscription_features
...
def provider_readiness_status(provider: dict, config: dict, *, features=None, is_active: Optional[bool] = None) -> str:
    ...
    from nunmai_cli.tools_config import _POST_SETUP_READY, _provider_env_ready, get_nous_subscription_features
    from nunmai_cli.tools_config_post_setup import _POST_SETUP_AUTH_READY
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from nunmai_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note / skip row / Nunmai marker, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from nunmai_cli.tools_config import _prompt_choice, _provider_env_ready, get_nous_subscription_features
    ...
    # Logged-in Nunmai users get a marker on rows included in their subscription (cost-extra vs. included).
    ...
    for p in providers:
        ...
        if not reconfigure and p.get("managed_nous_feature"):
            sub_marker = "  ★ Included with your Nunmai subscription" if _nous_logged_in else "  ★ via Nunmai Portal (login on select)"
...
def _managed_provider_active(provider: dict, config: dict, managed_feature: str, force_fresh: bool) -> bool:
    """Active check for a Nunmai-managed row: the feature must be managed AND the category's selected
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from nunmai_cli.tools_config import _provider_env_ready
...
IMAGEGEN_BACKENDS = {
    ...
    "nous": {"display": "Nunmai Subscription", "config_key": "image_gen", "catalog_fn": _managed_image_catalog}}
...
def _pick_model_from_catalog(
    ...
    selection."""
    from nunmai_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    ...
    from nunmai_cli.tools_config import _cfg_section
...
def _select_plugin_gen_provider(section: str, plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    """Persist a plugin-backed image/video gen provider selection (``nous`` for a Nunmai-managed pick, else the
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    Pure, non-interactive core of :func:`_configure_provider` (no env prompts, post-setup hooks, Nunmai
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed rows without a marker above (image_gen/video_gen "Nunmai Subscription" rows carry only
...
def _nous_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    """Return False (after printing why) when a Nunmai-gated row cannot be selected.
    Managed Tool Gateway rows are always listed but only *activate* with paid Nunmai Portal access —
    ...
    if managed_feature:
        from nunmai_cli.nous_subscription import ensure_nous_portal_access
        ...
        if not ensure_nous_portal_access(
            ...
            _print_warning("  Not enabled — Nunmai Portal access is required for this backend.")
            return False
        return True
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present or not entitled:
            message = format_nous_portal_entitlement_message(
                features.account_info, capability=f"{provider.get('name', 'Nunmai Subscription')}")
            _print_warning(f"  {message or 'Nunmai Subscription is only available after logging into Nunmai Portal.'}")
            return False
    return True
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    """True when a BYOK row shares its category with a Nunmai-managed sibling and the user is not authed to
    Nunmai — a single dim hint tells them the key is avoidable via a Portal subscription."""
    from nunmai_cli.tools_config import TOOL_CATEGORIES, get_nous_subscription_features
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``nunmai model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``nunmai model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from nunmai_cli.tools_config import _prompt_choice
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from nunmai_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
