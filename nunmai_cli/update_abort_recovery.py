...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``nunmai-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``nunmai-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``nunmai_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "nunmai_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``nunmai-serve.service`` can exist in both the user and the system
    ...
    """
    from nunmai_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    """Name the serve/dashboard processes still on pre-update code: ``nunmai serve`` hosts
    ...
    print("    Ask their owner to relaunch `nunmai serve` / `nunmai dashboard`, or reconnect Desktop for an SSH backend.")
    if sys.platform == "linux" and any(row.get("supervisor") == "systemd" for row in rows):
        print("    For unit-managed backends: `systemctl --user restart nunmai-serve.service`.")
