...
One read-only pass answering, BEFORE any mutation: which Nunmai runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``nunmai update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Nunmai runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "nunmai update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from nunmai_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"nunmai -p {profile} gateway restart" if profile != "default" else "nunmai gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from nunmai_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from nunmai_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from nunmai_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from nunmai_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from nunmai_cli.update_receipt import _socket_identity
        ...
        for profile, home in profile_homes:
            ...
            if sock is not None:
                ...
                # touch, so `nunmai update` exited partial (#109680).
    ...
    with _probe("PID-file gateway inventory"):
        from nunmai_cli.gateway import find_profile_gateway_processes
...
def _launchd_owner_for_ledger_entry(entry: dict, pid: int, jobs: list) -> "tuple[str, str, int | None] | None":
    ...
    with suppress(Exception):
        from nunmai_cli import main_dashboard as _dash
        from nunmai_cli.dashboard_procs import _process_ancestors
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `nunmai serve --host <ip>` for a remote Desktop, a long-lived `nunmai dashboard`).
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Nunmai services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``nunmai-serve*``/``nunmai-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``nunmai-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``nunmai-serve-workbench`` — and a scope prefix
    (``user/nunmai-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"nunmai-serve{suffix}", f"nunmai-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``nunmai-gateway-foobar.service``.
    Launchd labels are ``ai.nunmai.gateway`` / ``ai.nunmai.gateway-<profile>`` — they do
    not contain the substring ``nunmai-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/nunmai-gateway``,
    ``gui/501/ai.nunmai.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"nunmai-gateway", "ai.nunmai.gateway", "gateway", "gateway-default"}
    return name in {f"nunmai-gateway-{profile}", f"ai.nunmai.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``nunmai-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if manual:
        ...
        for o in manual:
            print(f"    • {o['kind']} [{o['profile']}] pid {o['pid']}: relaunch `nunmai serve` / `nunmai dashboard`, or reconnect Desktop for an SSH backend")
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      nunmai gateway restart                # active profile")
        print("      nunmai -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        if sys.platform == "linux":
            print("      systemctl --user restart nunmai-serve.service   # unit-managed serve")
        print("      relaunch `nunmai serve` / `nunmai dashboard`")
    return True
