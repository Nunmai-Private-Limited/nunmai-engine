...
One read-only pass that answers, BEFORE any mutation: what Nunmai runtimes
...
`detect_install_method`) — so `nunmai update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `nunmai update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `nunmai serve`; it respawns
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"nunmai -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    # launched `nunmai serve --host <ip>` for a remote Desktop, or a
    # long-lived `nunmai dashboard`. Every serve/dashboard registers itself
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "nunmai-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "nunmai-gateway" in svc)
