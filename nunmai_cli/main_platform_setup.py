...
from nunmai_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``nunmai whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Nunmai?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``nunmai whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``nunmai whatsapp``: ``npm install`` the bridge when needed. False = stop."""
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", env=with_nunmai_node_path())
...
def cmd_whatsapp(args):
    ...
    from nunmai_cli.main import _require_tty, get_nunmai_home
    ...
    # every `nunmai gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: nunmai gateway")
            return
    ...
    with contextlib.suppress(KeyboardInterrupt):
        subprocess.run(
            ...
            cwd=str(bridge_dir), env=with_nunmai_node_path())
    ...
    if wa_mode == "bot":
        _say("  Next steps:", "    1. Start the gateway:  nunmai gateway",
             "    2. Send a message to the bot's WhatsApp number",
             "    3. The agent will reply automatically", "",
             "  Tip: Agent responses are prefixed with '☤ Nunmai Engine'")
    else:
        _say("  Next steps:", "    1. Start the gateway:  nunmai gateway",
    ...
    _say("", "  Or install as a service: nunmai gateway install")


def cmd_whatsapp_cloud(args):
    """Set up WhatsApp Business Cloud API (official Meta integration) — complementary to the
    ``nunmai whatsapp`` Baileys bridge wizard. See ``nunmai_cli/setup_whatsapp_cloud.py``."""
    from nunmai_cli.main import _require_tty
...
def _sync_status(ssc) -> int:
    ...
    if not status.get("logged_in"):
        _err("\nNot logged into Nunmai Portal — sync is inert.")
        ...
        _err("\nNo sync base URL configured (config.yaml sync.base_url or NUNMAI_SYNC_BASE_URL). Sync is inert.")
    return 0
...
_SYNC_GATED = {
    "pull": _sync_pull,
    "push": lambda ssc, identity: ssc.push_skills(identity=identity, message="nunmai sync push"),
    "now": lambda ssc, identity: {"pull": ssc.pull_skills(identity=identity),
                                  "push": ssc.push_skills(identity=identity, message="nunmai sync now")}}
...
def cmd_sync(args):
    ...
    if not ssc.resolve_sync_base_url():
        _err("sync inert: no sync base URL configured (config.yaml sync.base_url or NUNMAI_SYNC_BASE_URL).")
        return 1
...
def cmd_slack(args):
    """``nunmai slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: nunmai slack <subcommand>\n"
             "\n"
             "subcommands:\n"
