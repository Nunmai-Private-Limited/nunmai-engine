"""User-facing copy for ``nunmai gateway start/stop/restart`` failures on systemd hosts.

``nunmai_cli/gateway.py`` is a facade; this sibling owns the small exception -> guidance table so
...
_JOURNAL_HINT = 'journalctl --user -u nunmai-gateway --since "5 min ago"'

_SYSTEMCTL_FAILED_LINES = (
    "Could not {verb} the gateway service; systemd reported an error.",
    "See why with `nunmai gateway status --deep` or `{journal}`.",
    "To reinstall the service run `nunmai gateway install --force`.",
)

_NO_SYSTEMCTL_LINES = (
    "This system has no systemd, so Nunmai cannot install a background service here.",
    "Run the gateway directly with `nunmai gateway run` (keep it alive with tmux or screen).",
)
