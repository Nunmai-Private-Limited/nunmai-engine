...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nunmai -c.",
    ...
    "Ctrl+Z suspends Nunmai to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nunmai -c resumes your most recent CLI session. nunmai -c \"project name\" resumes by title.",
    "nunmai -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nunmai -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nunmai chat -t web,terminal enables only specific toolsets for a focused session.",
    "nunmai chat -s github-pr-workflow preloads a skill at launch.",
    "nunmai chat -q \"query\" runs a single non-interactive query and exits.",
    "nunmai chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "nunmai chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nunmai --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nunmai chat --source telegram tags the session for filtering in nunmai sessions list.",
    "nunmai -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nunmai doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nunmai dump outputs a compact setup summary — great for bug reports.",
    "nunmai config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nunmai config edit opens config.yaml in your default editor.",
    "nunmai config check scans for missing or stale configuration options.",
    "nunmai sessions browse opens an interactive session picker with search.",
    "nunmai sessions stats shows session counts by platform and database size.",
    "nunmai sessions prune --older-than 30 cleans up old sessions.",
    "nunmai skills search react --source skills-sh searches the skills.sh public directory.",
    "nunmai skills check scans installed hub skills for upstream updates.",
    "nunmai skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nunmai skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nunmai mcp add github --command npx adds MCP servers from the command line.",
    "nunmai mcp serve runs Nunmai itself as an MCP server for other agents.",
    "nunmai auth add lets you add multiple API keys for credential pool rotation.",
    "nunmai completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nunmai logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nunmai backup creates a zip backup of your entire Nunmai home directory.",
    "nunmai profile create coder creates an isolated profile that becomes its own command.",
    "nunmai profile create work --clone copies your current config and keys to a new profile.",
    "nunmai update syncs new bundled skills to ALL profiles automatically.",
    "nunmai gateway install sets up Nunmai as a system service (systemd/launchd).",
    "nunmai memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nunmai webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nunmai tools disables unused tools, nunmai skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nunmai models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nunmai -w).",
    ...
    "NUNMAI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Nunmai tools programmatically — results stay out of context.",
    ...
    "/moa (alias /nunmai) routes one hard prompt through your configured Nunmai Agent model set.",
    ...
    "Profile names become shell commands — 'nunmai profile create coder' creates the 'coder' command.",
    "nunmai profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Nunmai prints a resume command with session ID and stats.",
    "nunmai sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nunmai -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nunmai skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nunmai/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nunmai cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Nunmai runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "nunmai gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Nunmai via a local macOS server.",
    ...
    "Context files (.nunmai.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Nunmai loads project context from .nunmai.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "nunmai mcp opens an interactive picker of Nunmai-approved MCPs you can install in one keystroke.",
    "nunmai mcp catalog lists Nunmai-approved MCP servers shipped with the repo.",
    "nunmai mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — nunmai tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nunmai/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nunmai chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nunmai plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nunmai acp runs Nunmai as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NUNMAI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "nunmai auth add nous or nunmai auth add openai-codex sets up OAuth-based providers.",
    ...
    "nunmai status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.nunmai/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Nunmai your own.",
    ...
    "Before a gateway session reset, Nunmai auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nunmai/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Nunmai state via NUNMAI_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "NUNMAI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NUNMAI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nunmai/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nunmai profile create ls' would be rejected.",
    "nunmai profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nunmai claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NUNMAI_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via nunmai tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.nunmai/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.nunmai/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Nunmai auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Nunmai config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Nunmai is working.',
    ...
    '/reload reloads ~/.nunmai/.env into the running session — pick up new API keys without restarting.',
    ...
    "NUNMAI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.nunmai/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.nunmai/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'nunmai curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "nunmai curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'nunmai curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'nunmai auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Nunmai subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'NUNMAI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NUNMAI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'nunmai dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'nunmai dashboard embeds the full Nunmai TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.nunmai/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.nunmai/dashboard-plugins/ — no npm build required.',
    ...
    'NUNMAI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'NUNMAI_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'NUNMAI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NUNMAI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.nunmai/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.nunmai/skills/ so drop-in skills appear without restarting the session.',
    ...
    'nunmai -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'nunmai chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'nunmai chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'nunmai chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "nunmai chat --source tool tags programmatic chats so they don't clutter nunmai sessions list.",
    'nunmai dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'nunmai sessions rename <ID> "new title" renames any past session; nunmai sessions delete <ID> removes one.',
    'nunmai import restores a full Nunmai backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'nunmai fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'nunmai pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'nunmai setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'nunmai status --deep runs the full health sweep across every component; plain nunmai status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NUNMAI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NUNMAI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.nunmai/plugins/ — trust-gated by design.',
    "NUNMAI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NUNMAI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NUNMAI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NUNMAI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NUNMAI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NUNMAI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NUNMAI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NUNMAI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NUNMAI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NUNMAI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NUNMAI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Nunmai block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from nunmai sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.nunmai/dashboard-plugins/.',
]
...
# any project or none — Nunmai is not a coding-only agent.
# ---------------------------------------------------------------------------
