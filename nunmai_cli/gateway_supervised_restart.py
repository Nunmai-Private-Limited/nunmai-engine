"""``nunmai gateway restart`` for a gateway whose supervisor Nunmai did not install.
...
def gateway_declares_external_supervisor(pid: int, home: Path | None = None) -> bool:
    ...
    from gateway.status import _get_process_nunmai_home, read_runtime_status
    from nunmai_cli.gateway import _capture_gateway_argv

    home = home or _get_process_nunmai_home()
...
def restart_externally_supervised_gateway(supervised_pid: int) -> None:
    ...
    """
    from nunmai_cli.gateway import _get_restart_exit_wait_budget, _graceful_restart_via_sigusr1, _print_lines
    ...
    _print_lines(
        ...
        "  then rerun once it is healthy: nunmai gateway restart",
    )
    sys.exit(1)
