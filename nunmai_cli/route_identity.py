...
def provider_owns_route(provider: Any, base_url: Any, config: Any = None) -> Optional[bool]:
    ...
    """
    from nunmai_cli.providers import get_provider, normalize_provider, resolve_custom_provider, resolve_user_provider
...
def drop_stale_model_route(model_cfg: Any, provider: Any, config: Any = None) -> "tuple[dict[str, Any], bool]":
    ...
    when the block is re-pointed at *provider* without a fresh route (``nunmai config set
