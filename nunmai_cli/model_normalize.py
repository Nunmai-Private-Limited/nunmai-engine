...
- **Aggregators** (OpenRouter, Nunmai, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  Nunmai revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, Nunmai, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Nunmai' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Nunmai provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``nunmai_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from nunmai_cli.models import normalize_copilot_model_id
