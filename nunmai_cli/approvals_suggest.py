...
Nunmai has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.nunmai/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, nunmai self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    ...
    from tools.approval import (
        _rewrite_resolved_nunmai_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_nunmai_home(command))
