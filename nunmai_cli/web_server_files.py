...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``nunmai update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the nunmai-webui image), where
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Nunmai
    # root actually IS /opt/data or NUNMAI_DASHBOARD_FILES_ROOT is set.
    if _default_nunmai_root_is_opt_data():
