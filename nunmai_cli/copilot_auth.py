...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `nunmai model` to authenticate via OAuth\n"
...
def _gh_cli_candidates() -> list[str]:
    ...
    from nunmai_platform.resolver import locate_command
    from nunmai_platform.resolver.known_dirs import homebrew_dirs, user_local_bin
...
def _post_form(url: str, fields: dict, timeout: float) -> dict:
    req = urllib.request.Request(
        ...
        headers={"Accept": "application/json", "User-Agent": "NunmaiAgent/1.0",
...
def _jwt_disk_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_nunmai_home()) / _JWT_DISK_FILENAME
    except Exception:
        return None
...
def copilot_request_headers(
    *, is_agent_turn: bool = True, is_vision: bool = False) -> dict[str, str]:
    ...
    headers: dict[str, str] = {"Editor-Version": _EDITOR_VERSION, "User-Agent": "NunmaiAgent/1.0",
