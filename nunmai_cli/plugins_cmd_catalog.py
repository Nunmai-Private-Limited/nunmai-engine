"""``nunmai plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
catalog-aware update, plus the dashboard/TUI-facing catalog payload helpers.

Sibling of :mod:`nunmai_cli.plugins_cmd` (the installer core, enable/disable state and console helpers
...
from nunmai_cli.plugin_catalog import (
...
CATALOG_SIDECAR = ".nunmai-catalog.json"
...
def looks_like_catalog_name(identifier: str) -> bool:
    ...
    from nunmai_cli.plugins_cmd import _URL_SCHEMES
...
def raise_if_removed(*candidates: str) -> None:
    ...
    from nunmai_cli.plugins_cmd import PluginOperationError
    for candidate in candidates:
        ...
        if removed is not None:
            ...
            raise PluginOperationError(
                f"Plugin '{removed.name}' was removed from the Nunmai plugin catalog and is blocked from "
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    from nunmai_cli.plugins_cmd import _fail
    entry = get_live_catalog_entry(identifier)
    if entry is None:
        _fail(console, (
            f"[red]Error:[/red] '{identifier}' is not in the Nunmai plugin catalog and is not a Git URL or "
            "owner/repo shorthand. Browse entries with `nunmai plugins search`."))
...
def write_catalog_sidecar(target: Path, entry: PluginCatalogEntry, sha: Optional[str] = None) -> None:
    """Human/Desktop-readable ``.nunmai-catalog.json`` inside the install dir. It is a CONVENIENCE COPY:
...
def _install_record(plugin_dir: Path) -> Optional[dict]:
    ...
    from nunmai_cli.plugins_cmd import PluginOperationError, _plugins_dir, _read_install_metadata
...
def _adopt_legacy_sidecar(plugin_dir: Path, record: dict) -> Optional[dict]:
    ...
    from nunmai_cli.plugins_cmd import _update_install_record
...
def read_catalog_sidecar(plugin_dir) -> Optional[dict]:
    ...
    tree: a URL-installed repo that ships its own ``.nunmai-catalog.json`` must not render as a reviewed
...
def _refuse_unsupported_catalog_platform(entry: PluginCatalogEntry) -> None:
    if not entry.platforms:
        return
    from nunmai_cli.plugins_cmd import PluginOperationError
    from nunmai_platform.host.facts import os_family
...
def install_catalog_entry(entry: PluginCatalogEntry, *, force: bool, ref: Optional[str] = None,
    ...
    from nunmai_cli.plugins_cmd import _install_plugin_core, pinned_revision
...
def refuse_if_installed_removed(name: str, plugin_dir) -> None:
    ...
    if removed is not None:
        raise PluginOperationError(
            f"Plugin '{name}' was removed from the Nunmai plugin catalog: "
            f"{removed.reason or 'no reason recorded'}. Remove it with `nunmai plugins remove {name}`, "
            "or reinstall with `nunmai plugins install <source> --force --allow-removed` if you trust it.")
...
def _local_changes(target: Path) -> tuple[list[str], list[str]]:
    ...
    from nunmai_cli.plugins_cmd import _resolve_git_executable, _run_plugin_git
...
def plugin_surface(manifest: dict, tree: Path) -> Dict[str, set]:
    ...
    from nunmai_cli.plugins_cmd import _declared_capabilities_from_manifest
...
def repin_catalog_plugin(target: Path, sidecar: dict, *, consent_cb=None) -> RepinResult:
    ...
    ``<NUNMAI_HOME>/plugins-backup/<name>-<sha8>/`` with a warning. A manifest rename between pins moves the
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `nunmai plugins info` and the removed blocklist.")
...
def cmd_update_catalog(name: str, target: Path, sidecar: dict, console) -> None:
    ...
    def _confirm_widening(delta: Dict[str, List[str]]) -> bool:
        ...
        if not _is_tty():
            console.print("  [yellow]Non-interactive session: update NOT applied (fail closed). "
                          "Re-run `nunmai plugins update` in a terminal to review and confirm.[/yellow]")
            return False
    ...
    if result.changed:
        from nunmai_cli.plugins_cmd import _install_python_dependencies
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    from nunmai_cli.plugins_cmd import _table
    table = _table(((("Name", "bold")), ("Category", None), ("Tier", None), ("Description", None),
                    ("Pinned", "dim"), ("Capabilities", "dim")), title="Nunmai Plugin Catalog (curated)")
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] nunmai plugins info <name>    [dim]Install:[/dim] nunmai plugins install <name>")
...
def cmd_search(term: str = "", *, json_output: bool = False) -> None:
    ...
    from nunmai_cli.plugins_cmd import _console
...
def cmd_info(name: str) -> None:
    ...
    from nunmai_cli.plugins_cmd import _console, cmd_show
    ...
    rows = [("Repo", entry.repo), ("Subdir", entry.subdir), ("Version", entry.version), ("Pinned SHA", entry.sha),
            ("Image", entry.image),
            ("Maintainer", entry.maintainer), ("Requires", f"nunmai {entry.requires_nunmai}" if entry.requires_nunmai else ""),
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     nunmai plugins install {entry.name}")
    console.print()
...
def cmd_validate(path: str, as_json: bool = False, install_deps: bool = False) -> None:
    ...
    from nunmai_cli.plugin_validate import validate_plugin_dir
    from nunmai_cli.plugins_cmd import _console
    if install_deps:
...
def installed_catalog_state(installed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    equals the manifest name (``nunmai-plugin-x`` vs ``x``), so installs are matched through the
