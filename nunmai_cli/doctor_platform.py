"""Host-platform checks for nunmai doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``nunmai_cli/doctor.py``, which re-exports every name so ``nunmai_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from nunmai_cli.config import is_nix_install_method, recommended_update_command_for_method
from nunmai_cli.doctor_report import (
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from nunmai_cli.doctor import PROJECT_ROOT
    from nunmai_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Nunmai containers", "apt": f"run `{cmd}`"}.get(method, "run `nunmai update`")
...
def _nunmai_database_paths(nunmai_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Nunmai-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from nunmai_cli.backup import _QUICK_STATE_FILES
    entries = [(name, nunmai_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``nunmai_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from nunmai_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_holders(name: str, db_path: Path) -> None:
    ...
    from nunmai_state_holders import describe_holder_pid, foreign_state_db_holders
...
def _report_database_journal_modes(nunmai_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from nunmai_cli.doctor import NUNMAI_HOME
    ...
    try:
        databases = _nunmai_database_paths(nunmai_home if nunmai_home is not None else NUNMAI_HOME)
    ...
    for name, path in databases:
        ...
        if error is None and mode == "wal" and configured == "delete":
            ...
            check_warn(f"{name} is in WAL mode ({size}) despite database.journal_mode=delete",
                       ...
                       + ". Stop every Nunmai process for this profile, then run "
                       f"`nunmai sessions set-journal-mode delete{'' if name == 'state.db' else f' --db {path}'}`)")
            ...
            check_warn(f"{name} is in WAL mode on a cross-VM filesystem (virtiofs/9p, {size})",
                       "(WAL can silently corrupt across the VM boundary; stop every Nunmai process and run "
                       f"`nunmai sessions set-journal-mode delete{'' if name == 'state.db' else f' --db {path}'}`, then "
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs nunmai_cli.__version__ drift (a conflict resolution can revert one but not the
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != nunmai_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'nunmai update', or set nunmai_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from nunmai_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-nunmai", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
...
def _report_host_gateway_slot(mgr, issues: list[str]) -> None:
    ...
    if topology is None:
        if not slots:
            return check_info("No gateway registered yet — run `nunmai gateway install`")
        ...
        issues.append("No host gateway owns the gateway role — start the ONE host multiplexer: "
                      "nunmai --profile default gateway start")
    ...
    if legacy_up:
        ...
        issues.append("Fold the legacy per-profile gateways into the host gateway: "
                      "nunmai --profile default gateway migrate --multiplex")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `nunmai doctor --fix`, or `{pip_cmd}`")
        return
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `nunmai update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `nunmai desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.nousresearch.nunmai` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Nunmai once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Nunmai.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Nunmai.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Nunmai-Setup launcher.
    """
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Nunmai "
                   ...
                   "    then enable your terminal (and Nunmai.app if you use Desktop), and restart them once. "
                   "With Nunmai' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from nunmai_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `nunmai doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from nunmai_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Nunmai already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "nunmai" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(nunmai not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "nunmai"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/nunmai → correct target")
        check_warn(f"{display}/nunmai points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/nunmai — run 'nunmai doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/nunmai exists (non-symlink)")
    else:
        check_fail(f"{display}/nunmai not found", "(nunmai command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/nunmai symlink — run 'nunmai doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/nunmai → {venv_bin}")
    f.fixed += 1
