...
def _report_database_journal_modes(nunmai_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from nunmai_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `nunmai profile create <name>`")
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from nunmai_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
