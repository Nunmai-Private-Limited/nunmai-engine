"""Pydantic request/response models for the Nunmai dashboard web server.

Extracted verbatim from ``nunmai_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from nunmai_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``nunmai model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `nunmai import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`nunmai -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the NUNMAI_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under NUNMAI_HOME.
    output: str = ""
