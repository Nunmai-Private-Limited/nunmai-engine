...
class ImportRequest(BaseModel):
    archive: str
    # --force: the spawned `nunmai import` has stdin=DEVNULL, so its "Continue? [y/N]" prompt would
...
class ProfileCreate(BaseModel):
    ...
    # Installed async via `nunmai -p <name> skills install` (skills_hub.SKILLS_DIR is import-time-bound,
    # so NUNMAI_HOME can't redirect it); PIDs go back for the UI to poll.
    hub_skills: List[str] = []
...
class ProfileExport(BaseModel):
    ...
    output: str = ""  # archive path; empty → a staging path under NUNMAI_HOME
