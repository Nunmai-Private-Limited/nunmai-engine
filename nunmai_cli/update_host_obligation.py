"""Host-scoped update→restart obligation for ``nunmai update``.
...
profile's ``NUNMAI_HOME``. The legacy ``$NUNMAI_HOME/fleet_restart_pending`` marker was
per-home: ``nunmai -p coder update`` armed and cleared coder's copy while restarting the
...
:func:`gateway.host_rendezvous.host_state_dir` (``$NUNMAI_GATEWAY_LOCK_DIR`` else
``$XDG_STATE_HOME/nunmai/gateway-locks``) — the one cross-profile, per-OS-user state root the
...
per-profile ``nunmai-gateway-<p>.service`` units on a multiplexed host all point at the same
...
def host_restart_already_completed(sha: Optional[str]) -> bool:
    ...
    ``nunmai update`` must attach to the first restart's outcome, never kill the shared
    multiplexer again.
    """
    record = read_host_obligation()
