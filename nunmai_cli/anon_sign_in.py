...
from nunmai_cli.auth_constants import httpx


UPGRADE_START = "Sign in with a Nunmai account to unlock more models and tools."
...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `nunmai auth add nous` to sign in."
UPGRADE_REASON_COPY = {
    "user_declined": "No problem, you're still on the free Nunmai service. Sign in whenever you're ready.",
...
UPGRADE_SERVICE_BUSY = ("Signing in couldn't finish because the Nunmai service is busy. "
                        "Try again in {wait}. Your session is still here in the meantime.")
UPGRADE_SERVICE_UNREACHABLE = ("The Nunmai service couldn't be reached to finish signing you in. "
                               "Check your internet connection and try again. Your session is still here.")

UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `nunmai model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with Nunmai."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this Nunmai. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this Nunmai can sign it in."
...
# the in-chat form (never a raw exception, a URL or a ``nunmai`` verb) and ``.copy_terminal`` the
...
class Failed(SignInState):
    ...
    def retryable(self) -> bool:
        from nunmai_cli import anon_auth as _core
...
def run_sign_in(
    *,
    ...
    """Sign the free tier into a Nunmai account, keeping its connectors. Yields :class:`SignInState`s.
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from nunmai_cli import anon_auth as _core
    from nunmai_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from nunmai_cli.auth_device_flow import _request_device_code
    from nunmai_cli.auth_nous import _nous_http_client
