...
Nunmai Portal sale chrome and org-policy filtering, and the Vercel AI Gateway / Novita / Fireworks /
DeepInfra pricing adapters. Split out of ``nunmai_cli.models``; helpers still defined there are
looked up on ``nunmai_cli.models`` at call time so ``patch("nunmai_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from nunmai_cli.models_reasoning_caps import _seed_reasoning_caps
...
def compute_sale_discount(prompt: str, completion: str, original: Any) -> tuple[int, str, str] | None:
    """Sale chrome from gateway ``pricing.original`` (Nunmai Portal only; callers gate on the provider
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from nunmai_cli.models import _urlopen_model_catalog_request
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the nunmai names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    another's read. *include_sale_original* (Nunmai Portal only) copies the gateway's pre-discount
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Nunmai Portal-only; never copy pricing.original for other catalogs.
            ...
            # Nunmai Portal-only: the gateway bills this row to a subscription the account holds, not to credits.
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """``(api_key, base_url)`` for Nunmai Portal pricing; base_url is the bare origin (no ``/v1``).
...
# How long a Nunmai catalog stays trusted. Its contents depend on the org's policy, which an admin
...
def _fetch_nous_pricing(api_key: str, base_url: str, *, force_refresh: bool) -> dict[str, dict[str, Any]]:
    ...
    return fetch_models_with_pricing(
        ...
        include_sale_original=True,  # Sale chrome (pricing.original) is Nunmai Portal-only.
        cache_ttl_seconds=_NOUS_CATALOG_TTL_SECONDS,
    )


def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    """The Nunmai model ids the caller's org may reach (keys of an authenticated ``GET /v1/models``,
    ...
    try:
        from nunmai_cli.nous_account import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from nunmai_cli.models import _pricing_profile_key
...
def get_cached_nous_inference_base_url() -> str:
    """The profile's persisted Nunmai endpoint (bare origin, no ``/v1``) without refreshing auth."""
    try:
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from nunmai_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from nunmai_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _NUNMAI_USER_AGENT}
