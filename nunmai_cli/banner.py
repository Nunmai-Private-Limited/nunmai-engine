...
Pure display functions with no NunmaiCLI state dependency.
"""
import json
import logging
import os
...
_GOLD = "\033[1;38;2;107;231;200m"  # True-color #6BE7C8 bold
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `nunmai ... > file`) raises
...
# (e.g. nix-built nunmai — no local git history to count against).
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/Nunmai-Private-Limited/nunmai-engine/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "nunmai-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `nunmai update`,
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``NUNMAI_REVISION`` is set (nix builds embed it), compare
    ...
    if embedded_rev:
        ...
        # $NUNMAI_HOME/nunmai-engine/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = nunmai_home / "nunmai-engine"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Nunmai git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NUNMAI_HOME/nunmai-engine/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``nunmai_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Nunmai checkout. Cached per-process. Release URL always points at the
...
def _format_update_notice(behind: int) -> str:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built nunmai; we know an update
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_nunmai_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else NUNMAI_CADUCEUS
    except Exception:
        _bskin = None
        _hero = NUNMAI_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Nunmai Research[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Nunmai Research[/]")
    ...
    title_color = _skin_color("banner_title", "#6BE7C8")
    border_color = _skin_color("banner_border", "#0B6B4F")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else NUNMAI_AGENT_LOGO
        console.print(_logo)
        console.print()
