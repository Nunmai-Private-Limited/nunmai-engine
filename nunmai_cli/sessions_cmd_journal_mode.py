"""`nunmai sessions set-journal-mode delete|wal` — the offline self-service path for #100896.
...
def _refusal(target: str, current: str, platform: str, *, on_cross_vm_fs: bool, force: bool) -> Optional[str]:
    ...
    if current == "not-a-database" or current.startswith("unknown("):
        return (f"its file header reads {current}, so it is not a Nunmai SQLite store this command can convert "
...
def cmd_set_journal_mode(args) -> int:
    ...
    from nunmai_state_wal import (_path_on_cross_vm_fs, _set_journal_mode_no_wait, is_sqlite_wal_reset_vulnerable,
