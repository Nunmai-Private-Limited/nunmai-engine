"""``nunmai slack ...`` CLI subcommands.

Today only ``nunmai slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ nunmai slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Nunmai. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Nunmai deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Nunmai on Slack's older Assistant messaging
    ...
    """
    from nunmai_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Nunmai in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Nunmai in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``nunmai_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NUNMAI_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Nunmai")
    ...
    """
    name = getattr(args, "name", None) or "Nunmai"
    description = getattr(args, "description", None) or "Your Nunmai agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "nunmai slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"nunmai slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "nunmai slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "nunmai slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Nunmai app\n"
