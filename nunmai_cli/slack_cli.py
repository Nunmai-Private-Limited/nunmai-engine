"""``nunmai slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Nunmai defaults, tweakable in the Slack
    UI after pasting.
    """
    from nunmai_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        ...
        features["agent_view"] = {"agent_description": "Chat with Nunmai in Slack Messages."}
        bot_scopes.append("assistant:write")
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``nunmai_cli/main.py``)."""
    ...
    def fail(msg: str) -> int:
        print(f"nunmai slack manifest: {msg}", file=sys.stderr)
        return 2
