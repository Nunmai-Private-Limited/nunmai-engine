...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`nunmai tools/setup`, most subcommands, `nunmai_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `nunmai-gateway*` unit /
  `ai.nunmai.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
`_apply_profile_override()` in `nunmai_cli/main.py` sets `NUNMAI_HOME` before any module import, so
every `get_nunmai_home()` scopes to the active profile (rules in root). Profiles are independent
