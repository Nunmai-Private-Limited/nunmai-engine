# nunmai_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `NunmaiCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`nunmai_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`nunmai_cli/gateway.py` is the `nunmai gateway` facade (process discovery, systemd backend, command
...
(`nunmai gateway setup`: `_PLATFORMS` registry, status table, per-platform prompts, service offer),
...
(late binding on `nunmai_cli.gateway`), so monkeypatch on the facade; mutable state such as
...
`NunmaiCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.nunmai/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`nunmai_cli/curses_ui.py`; example
`nunmai_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
`nunmai_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`nunmai_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `nunmai config
...
- **One writer.** Every write of a `config.yaml` (main or profile) goes through
  `nunmai_cli.config.atomic_config_write` (→ `utils.atomic_roundtrip_yaml_save`, ruamel
  ...
  (CI lint) rejects it, and `tests/nunmai_cli/test_config_yaml_comment_preservation.py` guards each
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`nunmai tools/setup`, most subcommands, `nunmai_cli/config.py`, merges `DEFAULT_CONFIG`);
  `nunmai_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `nunmai send`, doctor,
  `nunmai_time`/`nunmai_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' nunmai_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`nunmai_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.nunmai/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`nunmai update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `nunmai update --plan`): read-only inventory — install kind, all
...
- **Apply**: git pull, or the Windows ZIP fallback — which fires ONLY when git itself failed
  ...
  root entry the ZIP does not ship (`.bytecode-fingerprint`, `.nunmai-bootstrap-complete`,
  `nunmai_agent.egg-info/`; tracked root entries stand in for the ZIP set before the download, the
  ...
  `nunmai_cli/web_dist`, `ui-tui/{dist,node_modules,packages/nunmai-ink/dist}`, `web/node_modules`,
  ...
  rebuild decision also trusts the build stamp under NUNMAI_HOME, so an install that already lost
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE within the updating install (every
  `nunmai-gateway*` unit / `ai.nunmai.gateway*` LaunchAgent whose home is the updating root or one of its
  ...
  `nunmai_cli/update_fleet_scope.py` judges every unit/label/process by the home it actually runs on
  (live environ, unit `Environment=`, plist `NUNMAI_HOME`), and a runtime of another `NUNMAI_HOME` on the
  same account — a sibling install, the real `nunmai-gateway.service` seen from a scratch home — is named and
...
- **Report**: every run writes a machine-readable receipt to `~/.nunmai/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `nunmai update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `nunmai update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `nunmai_cli/main.py` sets `NUNMAI_HOME` before any module import for
single-profile commands (`nunmai -p x <cmd>`), so there `get_nunmai_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["NUNMAI_HOME"]` keeps the launch
...
carries an identity marker (`nunmai_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_nunmai_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `nunmai_constants.nunmai_home_key()`, never a single flag.
...
Migration from per-profile gateways: `nunmai_cli/gateway_migrate.py` (`nunmai gateway migrate
...
other service domain / UNIX user / NUNMAI_HOME outside `profiles/` — notices for the explicit command,
...
`nunmai gateway restart` for a gateway Nunmai did not install (custom launchd agent / unit running
...
sibling and read facade helpers late-bound through `nunmai_cli.gateway`, so patch them on the facade; user unit AND `--system` unit with `User=`; an unresolvable `User=` is a blocker,
...
profile's `NUNMAI_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nunmai free tier (`nunmai_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `nunmai` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`nunmai auth upgrade`), the CLI `/login` handler and the desktop
promotion poller are renderers over it; a surface that needs the cancel check and the save to be
atomic passes `persist_guard`. The desktop's plain "connect another Nunmai account" device-code login
