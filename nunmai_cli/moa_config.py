...
MOA_MARKER_PREFIX = "__NUNMAI_MOA_TURN_V1__"
...
def validate_moa_payload(raw: Any) -> list[str]:
    ...
    for name, preset in presets.items():
        ...
        # An ``auto: true`` preset with no slots at all resolves at read time from the user's
        # credentials (see build_auto_default_slots); it is a complete, intentional configuration,
...
def _normalize_preset(raw: Any) -> dict[str, Any]:
    ...
    # ``auto: true`` with no explicit slots -> credential-aware default (aggregator = the main model,
    # references = main + other authenticated providers). Explicit slots always win over auto.
    ...
    return {
        ...
        "aggregator": aggregator or deepcopy(DEFAULT_MOA_AGGREGATOR),
...
def resolve_moa_preset(config: Any, name: str | None = None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(
            ...
            f"{available}. Run `nunmai moa list`.")
