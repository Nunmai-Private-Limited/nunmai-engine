...
``nunmai desktop`` builds and launches the Electron app. On Linux, a
...
def desktop_entry_path() -> Path:
    """Where the ``nunmai.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    ...
    Prefer the real ``nunmai`` executable (argv[0] or PATH). When Nunmai
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `nunmai` script with
            ...
            # sys.executable is the interpreter actually running Nunmai (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "nunmai_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Nunmai' venv (rather than its own shebang)."""
    try:
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Nunmai desktop entry. Return its path.
