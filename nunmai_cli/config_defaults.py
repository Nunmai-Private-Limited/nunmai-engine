"""Default configuration data for Nunmai Engine: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from nunmai_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["nunmai-cli"],
    # journal_mode: SQLite journal mode for every Nunmai DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `nunmai -c`: each CLI session writes a breadcrumb under
        # $NUNMAI_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `nunmai
        ...
        # Nunmai-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Nunmai (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env NUNMAI_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `nunmai doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`nunmai status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # NUNMAI_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else NUNMAI_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.nunmai tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # NUNMAI_HOME/home; "real" = force real HOME; "profile" = force NUNMAI_HOME/home when it
        ...
        # ["/home/user/.nunmai/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Nunmai image,
        # which drops to `nunmai` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `nunmai tools`, not a ring
        # member). Set by the `nunmai tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Nunmai-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Nunmai' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.nunmai/browser-profile/ so credentials
        ...
        # runs `nunmai browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `nunmai chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.nunmai/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `nunmai checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .nunmai.md,
    ...
    # Same bound for single-query mode (``nunmai -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Nunmai' summarizer cannot shrink it. native = codex decides; nunmai = Nunmai' threshold
        ...
        # scope it to one route ({"openai-codex:astra": 0.85} leaves Astra on OpenRouter/Nunmai at the
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `nunmai model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `nunmai`/`nunmai chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / NUNMAI_TUI=1 forces the TUI.
        "interface": "cli",
        # `nunmai --tui` auto-resumes the most recent human-facing session (like `nunmai -c`).
        # NUNMAI_TUI_RESUME=<id> always wins.
        ...
        # Nunmai credits status-bar notices (usage bands, grant-spent, depleted/restored). False mutes
        ...
        # `path`/`paths` = NUNMAI_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/nunmai`.
        ...
        "runtime_footer": {
            ...
            # deployment a routing proxy reported / Nunmai' fallback route).
        ...
        # CLI/TUI/desktop, managed with `nunmai pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_nunmai_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: NUNMAI_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than NUNMAI_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not), read by the Nunmai Portal
        # plugin. Env NUNMAI_DASHBOARD_OAUTH_CLIENT_ID / NUNMAI_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # NUNMAI_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.nunmai/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Nunmai turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Nunmai (any model / provider you have
        ...
        "auto_tts": False,
        # When a voice reply is delivered (auto-TTS on voice input, /voice on), suppress the separate
        # text message: the user hears the answer and the chat is not doubled with a text wall. The
        # full text stays in session history. Telegram captions still ride on the audio when eligible.
    ...
    # "Hey Nunmai" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_nunmai" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.nunmai/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.nunmai/cache/delegation/ with a head+tail window + read_file offset footer, nothing
        ...
        # Total subagents a finite one-shot run (nunmai chat -q / --oneshot) may spawn; 0 = unlimited.
    ...
    "moa": {
        ...
        # <nunmai_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.nunmai/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # NUNMAI_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.nunmai/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `nunmai skills trust` / `untrust`.
        ...
        # skip; NUNMAI_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${NUNMAI_SKILL_DIR} / ${NUNMAI_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `nunmai skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.nunmai/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.nunmai/.curator_backups/blobs/); powers `nunmai
    ...
    # daemon. `nunmai curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `nunmai curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `nunmai update` restoring them);
        ...
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `nunmai curator purge`
        ...
        # ~/.nunmai/skills/ to ~/.nunmai/skills/.curator_backups/<utc-iso>/skills.tar.gz (`nunmai curator
    ...
    # peerName, sessions, enabled); nunmai-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *Nunmai Engine*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (NUNMAI_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export NUNMAI_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. NUNMAI_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `nunmai plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `nunmai plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.nunmai/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # NUNMAI_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        ...
        # logins automatically when Nunmai has no usable login of its own. Their refresh tokens are single-use
        # and rotate, so two programs on one login can log each other out; set false to make Nunmai use only
        # its own logins (`nunmai auth add <provider>`). `nunmai auth add openai-codex` still offers the import
        # interactively.
        "adopt_external_logins": True,
        # How `nunmai auth add openai-codex` / `nunmai model` sign in to OpenAI Codex.
        ...
        # to device code when that port is busy. `nunmai auth add openai-codex --browser` opts in
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .nunmai config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `nunmai doctor --ack <id>`; remove by editing the list. Catalog:
        # nunmai_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $NUNMAI_HOME/plugins/ <name>/),
        ...
        # holds NO scheduler credentials (provision reuses the Nunmai Portal token).
        "chronos": {
        ...
        # Env override: NUNMAI_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: NUNMAI_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: NUNMAI_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: NUNMAI_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `nunmai cron incidents` shows it).
        # A green run or a different error alerts again immediately; `nunmai cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `nunmai -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across Nunmai homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `nunmai kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with nunmai-engine's own python
    ...
    "tools": {
        ...
        # Remote connector discovery/lifecycle through the Nunmai tool gateway.
        # The flag is the user's off switch; availability additionally requires
        # the portal sign-in every managed tool gates on.
        "connectors": {"enabled": True},
    },
    "logging": {  # File logging to ~/.nunmai/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter / Nunmai Portal model lists fetched from this
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `nunmai model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: Nunmai
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "nunmai-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to NUNMAI_STARTUP_WATCHDOG / NUNMAI_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.nunmai/sessions/sessions.json mirror of the routing index
        ...
        # `nunmai gateway migrate --multiplex` folds any per-profile fleet that is left.
        ...
        # adapter is parked; `nunmai profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `nunmai update` fold this install onto a multiplexed default gateway by itself?
        ...
        # AUTOMATIC path reads this: `nunmai gateway migrate --multiplex` is explicit and proceeds.
        ...
        # (NUNMAI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # NUNMAI_GATEWAY_MAX_STARTS / NUNMAI_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nunmai/.env, auth.json). True:
        # files must be under the Nunmai cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to NUNMAI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Nunmai cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to NUNMAI_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to NUNMAI_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # NUNMAI_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.nunmai/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `nunmai sessions prune`).
        ...
        # stay until `nunmai sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `nunmai update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.nunmai/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to NUNMAI_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`nunmai sessions export`); checked
    ...
    # and transmission to Nunmai (`send`) are SEPARATE opt-ins; see
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `nunmai doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `nunmai update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <NUNMAI_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``nunmai backup`` zip in
        # <NUNMAI_HOME>/backups/ (``nunmai import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `nunmai update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `nunmai update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        # skipped. 0 = for the process lifetime (until `nunmai lsp restart`); N = retried after N
        ...
        "exclude_roots": [],
        # Missing server binaries: auto = install via npm/go/pip into <NUNMAI_HOME>/lsp/bin/ on
        ...
        # <NUNMAI_HOME>/lsp/node_modules; a configured manager that is not installed, or an unknown
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `nunmai tools`.
    "x_search": {
    ...
    # encrypted vault (`nunmai vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.nunmai/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.nunmai/cache/,
            ...
            # Auto-download bws into ~/.nunmai/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    # Bot Desktop: a headless Xfce screen per profile on the gateway host (Linux), streamed to Nunmai
    # Desktop where a human can watch, take over (logins, 2FA, CAPTCHAs) and hand back. `nunmai computer-use screen`.
    "bot_desktop": {
        "geometry": "1440x900",
        ...
        # for; Nunmai Desktop's Screen pane offers Start and this toggle.
        "auto_start": False,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Nunmai sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Nunmai also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.nunmai/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Nunmai Desktop (Electron) launch options; only affect `nunmai desktop`.
        ...
        # Bridged to NUNMAI_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to NUNMAI_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        # Upper bound (seconds) on the Nunmai auth keepalive tick, which derives from the
        ...
        "anthropic_wire": "chat",
        # Nunmai free tier: with no other provider configured, Nunmai sets up a free Nunmai identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `nunmai auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Nunmai still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Nunmai releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": _base_url("Nunmai Portal"),
    "NUNMAI_ANON_API_SECRET": _env(
        "Shared secret for the Nunmai free-tier sign-up endpoints while they are in their gated "
        "integration phase (not needed once the gate is removed)",
        "Nunmai free-tier shared secret (leave empty unless given one)", password=True,
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Nunmai mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'nunmai model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": _tool(
        "Exact Firecrawl tool-gateway origin override for Nunmai Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": _tool(
        "Shared tool-gateway domain suffix for Nunmai Subscribers only, used to derive vendor "
    ...
    "TOOL_GATEWAY_SCHEME": _tool(
        "Shared tool-gateway URL scheme for Nunmai Subscribers only, used to derive vendor hosts "
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        "Explicit Nunmai Subscriber access token for tool-gateway requests (optional; otherwise "
        "read from the Nunmai auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Nunmai' wake word engine (optional; "
    ...
    "NUNMAI_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "NUNMAI_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "NUNMAI_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Nunmai, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @nunmai:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. NUNMAI_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #nunmai)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: nunmai-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'nunmai-engine' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Nunmai API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Nunmai API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Nunmai API server (proxy mode). Must "
    ...
    # NUNMAI_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. NUNMAI_TOOL_PROGRESS is unsupported.
    "NUNMAI_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "NUNMAI_EPHEMERAL_SYSTEM_PROMPT": _setting(
