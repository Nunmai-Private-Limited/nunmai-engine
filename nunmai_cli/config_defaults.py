"""Default configuration data for Nunmai Engine.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from nunmai_cli/config.py. Must not import from nunmai_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["nunmai-cli"],
    # SQLite journal mode used by every Nunmai database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Nunmai server processes.
    ...
    "session": {
        # Per-terminal `nunmai -c`: each CLI session drops a breadcrumb file
        # under $NUNMAI_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by nunmai_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `nunmai chat --run-budget N`.
        ...
        # an interactive `nunmai gateway restart` must never block for hours
        ...
        # the Nunmai-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nunmai
        ...
        # identity slot (SOUL.md). Empty by default. The NUNMAI_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nunmai adds a coding operating brief
        ...
        # `nunmai status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``NUNMAI_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Nunmai' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Nunmai uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at NUNMAI_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `nunmai -p <bot> chat -Q` recipient
        ...
        #             NUNMAI_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NUNMAI_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nunmai auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nunmai sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nunmai
        ...
        # bundled Nunmai image, which drops to the `nunmai` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `nunmai tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Nunmai-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Nunmai' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.nunmai/browser-profile/) so copied
        ...
        # approval does it run `nunmai browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Nunmai sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nunmai should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nunmai chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nunmai/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nunmai sweeps the checkpoint base at startup
        ...
        # ``nunmai checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .nunmai.md, or .cursorrules before Nunmai applies
    ...
    # Single-query (``nunmai -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Nunmai truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Nunmai' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); nunmai =
                                      # Nunmai' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Nunmai' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Nunmai does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``nunmai kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nunmai kanban decompose`` and the kanban
        ...
        # ``nunmai profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nunmai model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `nunmai` (and `nunmai chat`) launches by default:
        ...
        # REPL and `--tui` (or NUNMAI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `nunmai --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nunmai -c` muscle memory.  Default off so existing
        # users aren't surprised.  NUNMAI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nunmai --tui` drops a one-time hint
        ...
        # Nunmai credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see nunmai_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to NUNMAI_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/nunmai`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `nunmai pets`. Disabled until
        ...
        "pet": {
            ...
            # get_nunmai_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see nunmai_cli/web_server.py — an event-loop stall
        ...
        # NUNMAI_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (NUNMAI_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``--insecure`` is not). The bundled Nunmai Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NUNMAI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``NUNMAI_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``NUNMAI_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``NUNMAI_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nunmai appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nunmai/cache/piper-voices/
    ...
    # "Hey Nunmai" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey nunmai",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_nunmai" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nunmai/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Nunmai, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.nunmai/cache/delegation/
    ...
    # response. If not, Nunmai feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nunmai auto-pauses the goal and
    ...
    "moa": {
        ...
        # <nunmai_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.nunmai/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.nunmai/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``nunmai skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``nunmai skills trust`` / ``nunmai skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${NUNMAI_SKILL_DIR} and ${NUNMAI_SESSION_ID} in SKILL.md
        ...
        # (`nunmai skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.nunmai/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.nunmai/.curator_backups/blobs/. Enables `nunmai curator ledger`
        # and single-mutation `nunmai curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `nunmai curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `nunmai curator run --consolidate` overrides
        ...
        # normally restored on every `nunmai update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `nunmai curator purge` deletes
        ...
        # skipped), snapshot ~/.nunmai/skills/ into
        # ~/.nunmai/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nunmai curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nunmai-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "◆ *Nunmai Engine*" header.
    ...
    # dangerous command. -q runs export NUNMAI_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; NUNMAI_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Nunmai' built-in platform hint for a single messaging platform
    ...
    # ``nunmai plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.nunmai/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NUNMAI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .nunmai config) always require human
        ...
        # longer trigger the startup banner. Add via `nunmai doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nunmai_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nunmai to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NUNMAI_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Nunmai Portal token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via NUNMAI_CRON_MAX_PARALLEL env var.
        ...
        # NUNMAI_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # NUNMAI_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via NUNMAI_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `nunmai -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nunmai` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nunmai kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nunmai-engine's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nunmai tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nunmai/logs/.
    ...
    # curated model lists for OpenRouter and Nunmai Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a nunmai-engine release.
    ...
    # #8731).
    #
    # Provider keys accept the Nunmai provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal NUNMAI_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.nunmai/sessions/sessions.json entirely.
        ...
        # the NUNMAI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.nunmai.gateway` that defenses 1-2 don't
        ...
        # `nunmai update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nunmai/.env,
        ...
        # behavior: files must live under the Nunmai cache, under
        ...
        # user. Bridged to NUNMAI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nunmai
        # cache (~/.nunmai/cache/{documents,images,audio,video,screenshots})
        ...
        # to NUNMAI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NUNMAI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NUNMAI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nunmai/state.db.
    ...
    "sessions": {
        ...
        # the default of ``nunmai sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nunmai/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `nunmai sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `nunmai update` prints a one-line notice with
        ...
        # ~/.nunmai/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to NUNMAI_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # NUNMAI_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `nunmai sessions export`. Checked
    ...
    # ``nunmai doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `nunmai doctor --live`
    ...
    "updates": {
        ...
        #     DBs) into <NUNMAI_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``nunmai backup``-style zip
        #     of NUNMAI_HOME into <NUNMAI_HOME>/backups/, restorable with
        #     ``nunmai import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `nunmai update` does with uncommitted local changes to the
        ...
        # When `nunmai update` finds the source checkout parked on a feature
        ...
        #                         merge. `nunmai update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<NUNMAI_HOME>/lsp/bin/`` on first use.
    ...
    # `nunmai tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nunmai/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.nunmai/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.nunmai/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Nunmai may use this encrypted
            ...
            # ~/.nunmai/bin/ on first use.  When False you must install
            ...
            # `nunmai secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.nunmai/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Nunmai disables it for our users unless they
        # explicitly opt in here. When false (default), Nunmai sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Nunmai also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Nunmai computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Nunmai passes the
    ...
    #
    # Configure with `nunmai egress setup`.  Disabled by default — the rest of
    # Nunmai works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.nunmai/bin/ on
        ...
        # Together, DeepSeek, Nunmai).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Nunmai Desktop (Electron app) launch options. These only affect
    # `nunmai desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the NUNMAI_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the NUNMAI_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Nunmai Portal base URL override",
        "prompt": "Nunmai Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Nunmai mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nunmai model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Nunmai Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nunmai Subscribers only, used to derive vendor hosts, e.g. nousresearch.com -> firecrawl-gateway.nousresearch.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Nunmai Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nunmai Subscriber access token for tool-gateway requests (optional; otherwise read from the Nunmai auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Nunmai' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Nunmai, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nunmai:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NUNMAI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nunmai)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nunmai-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nunmai-engine' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nunmai API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nunmai API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nunmai API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NUNMAI_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to NUNMAI_TOOL_PROGRESS_MODE for backward
    ...
    # NUNMAI_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "NUNMAI_PREFILL_MESSAGES_FILE": {
    ...
    "NUNMAI_EPHEMERAL_SYSTEM_PROMPT": {
