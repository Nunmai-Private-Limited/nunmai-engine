...
dependency/config-schema warnings. Mixed into :class:`nunmai_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``nunmai_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from nunmai_constants import get_nunmai_home, reset_nunmai_home_override, set_nunmai_home_override
from registration_lifecycle import replacement_coordinator
from nunmai_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from nunmai_cli.plugins_manifest import PluginManifest, manifest_key, portable_mcp_server_name, validate_config_schema
from nunmai_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from nunmai_cli.plugins import LoadedPlugin, PluginContext

logger = logging.getLogger("nunmai_cli.plugins")
...
def _reserve_abandoned_loader_slot() -> None:
    ...
    raise PluginLoadTimeout(
        ...
        f"(plugins.load_timeout_seconds); restart Nunmai to retry"
    )
...
def run_with_load_deadline(plugin_key: str, ctx: "PluginContext", fn: Callable[[], Any]) -> Any:
    ...
    The worker inherits the caller's context (the Nunmai-home override is a ContextVar). On timeout the
...
class PluginLoaderMixin:
    def on_plugin_loaded(self, callback: Callable[[List[Dict[str, Any]]], Any]) -> Callable[[], None]:
        ...
        just the newcomer after a mid-run ``nunmai plugins install/enable``, Desktop / dashboard /
        ...
        :func:`nunmai_cli.plugins_activation.plugin_activation_summary` for the payload: ``activated_now``
    ...
    def _notify_plugin_loaded(self, loaded_before: frozenset) -> None:
        ...
        if not self._plugin_loaded_listeners:
            return
        from nunmai_cli.plugins_activation import activation_summaries
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``nunmai plugins list`` until then."""
        from nunmai_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``nunmai tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``nunmai tools`` checklist, and even an explicit
        ...
        """
        from nunmai_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `nunmai plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        ``nunmai plugins install``/``enable`` and after ``nunmai update`` (``nunmai_cli.plugin_python_deps``)
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Run `nunmai plugins enable %s` to install them, "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from nunmai_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_nunmai gate: skip cleanly (no import, no traceback) on a version mismatch.
        from nunmai_cli.plugins_manifest import requires_nunmai_error
        reason = requires_nunmai_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (nunmai_cli.plugin_compat).
        from nunmai_cli.plugin_compat import disable_reason
        ...
        try:
            if run_with_load_deadline(plugin_key, ctx, _import_and_register):
                ...
                loaded.enabled = True
                from nunmai_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from nunmai_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `nunmai plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        try:
            ...
            package = load_agent_plugin(
                Path(manifest.path), get_nunmai_home() / "plugin-data" / manifest.skill_namespace)
            ctx = PluginContext(manifest, self)
            ...
            from nunmai_cli.agent_plugins import _clear_liveness, _set_liveness
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``nunmai_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``nunmai_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Nunmai home, or an earlier force
