"""Backup and import commands for nunmai CLI."""
...
from nunmai_constants import (
    LOCAL_RUNTIME_ROOT_DIRS, _get_platform_default_nunmai_home, get_default_nunmai_root, get_nunmai_home,
    display_nunmai_home,
)
from nunmai_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
# Where ``nunmai backup --quick`` / ``/snapshot`` / the pre-update safety net write state
...
def _snapshot_recovery_hint() -> str:
    """How to restore a state snapshot. There is no `nunmai snapshot` subcommand — only the /snapshot
    slash command inside a `nunmai` session (nunmai_cli/commands.py)."""
    return ("To restore a newer snapshot, start `nunmai` in a terminal and run `/snapshot list`, then "
            "`/snapshot restore <id>` (CLI only).")

# Directory names to skip (matched against each path component). ``nunmai-engine`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../nunmai-engine/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under NUNMAI_HOME walked
...
_EXCLUDED_DIRS = {
    "nunmai-engine",     # the codebase repo — re-clone instead
...
# Nunmai-managed runtime downloads (see ``LOCAL_RUNTIME_ROOT_DIRS``). Matched ONLY at the root of
# NUNMAI_HOME and at ``profiles/<name>/`` — a deeper dir of the same name (a skill's ``models/``)
# is user data.
...
# Files ``nunmai import`` must never overwrite, matched by basename so root and named profiles are
...
# disconnected from the Nunmai portal); PID/lock/registry files reference source PIDs. Mirrors
...
# Reserved archive subtree for memory-provider state OUTSIDE NUNMAI_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Nunmai backup slot."""
...
@contextmanager
def _backup_operation_lock(nunmai_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = nunmai_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another Nunmai backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nunmai root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``nunmai-engine`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "nunmai-engine" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(nunmai_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``nunmai-engine`` carve-out, root runtime trees, per-file rules),
    shared by ``nunmai backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(nunmai_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(nunmai_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "nunmai-engine" and not is_root))
        ...
        for fname in filenames:
            rel = rel_dir / fname
            fpath = nunmai_root / rel
            # zipfile.write() follows file symlinks, so skip links before any archive write can
            # copy data from outside NUNMAI_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``nunmai update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Nunmai) and retry.", dst, exc2)
        return False
...
_RUN_BACKUP_PREFIX = "nunmai-backup-"
...
def run_backup(args) -> bool:
    """Create a zip backup of the Nunmai home directory.
    ...
    if not nunmai_root.is_dir():
        print(f"Error: Nunmai home directory not found at {nunmai_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(nunmai_root):
            return _run_backup_locked(args, nunmai_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, nunmai_root: Path) -> bool:
    ...
    print(f"Scanning {display_nunmai_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(nunmai_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_nunmai_home()}.")
    if skipped_external:
    ...
    if errors:
        ...
        print(f"\nRestore with: nunmai import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Nunmai backup."""
    names = zf.namelist()
    if not names:
        return False, "zip archive is empty"
    # Telltale files a nunmai home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Nunmai backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".nunmai", "nunmai"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Nunmai session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(nunmai_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((nunmai_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Nunmai configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, nunmai_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under NUNMAI_HOME; provider configs commonly hold credentials, so tighten to 0600.
        ...
        if external:
            ...
            if rel.endswith(_SQLITE_SIDECAR_SUFFIXES):
                skipped_runtime.append(rel)
                continue
            target = nunmai_root / rel
            root = nunmai_root.resolve()
...
def run_import(args) -> None:
    ...
    # ``get_default_nunmai_root()`` would silently retarget a profile restore at the live root.
    nunmai_root = get_nunmai_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_nunmai_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(nunmai_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, nunmai_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_nunmai_home()}.")
        if errors:
        ...
        restored_profiles = _restore_profile_wrappers(nunmai_root)
        print()
        if not (nunmai_root / "nunmai-engine").is_dir():
            print("Note: The nunmai-engine codebase was not included in the backup.\n"
                  "  If this is a fresh install, run: nunmai update")
        if restored_profiles:
            print("\nTo re-enable gateway services for profiles:")
            for pname in restored_profiles:
                print(f"  nunmai -p {pname} gateway install")
        _revive_gateway_after_import(nunmai_root)
        print("Done. Your Nunmai configuration has been restored.")


def _restore_profile_wrappers(nunmai_root: Path) -> List[str]:
    """Re-create shell wrapper scripts for restored named profiles; return the profile names seen."""
    profiles_dir = nunmai_root / "profiles"
    ...
    except ImportError:  # nunmai_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: nunmai profile list  (after installing nunmai)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(nunmai_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_nunmai_home()
    if nunmai_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  nunmai gateway install")
        return
    try:
        from nunmai_cli.gateway import ensure_gateway_service, _is_service_running
...
# --- Quick state snapshots (used by /snapshot slash command and nunmai backup --quick) ---

# Critical state files (relative to NUNMAI_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``nunmai update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside NUNMAI_HOME.
...
def _quick_snapshot_root(nunmai_home: Optional[Path] = None) -> Path:
    home = nunmai_home or get_nunmai_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, nunmai_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``nunmai update`` while the small files are always captured.
    """
...
def list_quick_snapshots(limit: int = 20, nunmai_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(nunmai_home)):
...
def restore_quick_snapshot(snapshot_id: str, nunmai_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, nunmai_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent cron-job loss across ``nunmai update``.
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_nunmai_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", nunmai_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, nunmai_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``nunmai update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, nunmai_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, nunmai_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, nunmai_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(nunmai_home)), keep, shutil.rmtree, "snapshot")


def run_quick_backup(args) -> None:
    """CLI entry point for nunmai backup --quick."""
    snap_id = create_quick_snapshot(label=getattr(args, "label", None))
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_nunmai_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, nunmai_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``nunmai_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(nunmai_root):
            return _write_full_zip_backup_locked(out_path, nunmai_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, nunmai_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(nunmai_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    nunmai_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<NUNMAI_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    nunmai_root = nunmai_home or get_default_nunmai_root()
    if not nunmai_root.is_dir():
        return None
    backup_dir = nunmai_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, nunmai_root) is None:
        return None
...
def create_pre_update_backup(
    nunmai_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``nunmai update`` continues anyway."""
    return _create_prefixed_full_backup(nunmai_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    nunmai_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``nunmai claw migrate``
    (same dir as update backups so listings/``nunmai import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        nunmai_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
