...
`nunmai backup` creates a zip archive of the entire ~/.nunmai/ directory
(excluding the nunmai-engine repo and transient files).

`nunmai import` restores from a backup zip, overlaying onto the current
NUNMAI_HOME root.
"""
...
from nunmai_constants import get_default_nunmai_root, get_nunmai_home, display_nunmai_home
from utils import (
    _preserve_file_mode,
...
# Where ``nunmai backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``nunmai-engine`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/nunmai-engine/``
...
# NUNMAI_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under NUNMAI_HOME) —
...
# File names that ``nunmai import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Nunmai portal
...
# Reserved archive subtree for provider state that lives OUTSIDE NUNMAI_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside NUNMAI_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``nunmai-engine`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/nunmai-engine/`` — must be preserved.
        if part == "nunmai-engine" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NUNMAI_HOME.
...
# to the user as a hung `nunmai update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # nunmai_cli.sqlite_safe_read). Verification targets snapshots and
...
def _run_backup_locked(args, nunmai_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"nunmai-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"nunmai-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(nunmai_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nunmai_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``nunmai-engine`` is only pruned at the root level; nested dirs
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nunmai_root)
    ...
    # outside NUNMAI_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_nunmai_home()}."
        )
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a nunmai home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Nunmai backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nunmai/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nunmai dir name
        if prefix in {".nunmai", "nunmai"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``nunmai import``, which is the
    ...
    so without the chown a ``sudo nunmai import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``nunmai import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Nunmai' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nunmai_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nunmai_root / "config.yaml").exists()
        has_env = (nunmai_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Nunmai configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under NUNMAI_HOME.
            ...
            # Nunmai portal. Matched by basename so both the root profile and
            ...
            try:
                target.resolve().relative_to(nunmai_root.resolve())
            except ValueError:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # nunmai_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: nunmai profile list  (after installing nunmai)")

        # Guidance
        print()
        if not (nunmai_root / "nunmai-engine").is_dir():
            print("Note: The nunmai-engine codebase was not included in the backup.")
            print("  If this is a fresh install, run: nunmai update")
...
# Critical state files to include in quick snapshots (relative to NUNMAI_HOME).
...
# — `nunmai update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $NUNMAI_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside NUNMAI_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: nunmai snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nunmai_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside NUNMAI_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        nunmai_home: Override for the Nunmai home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, nunmai_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nunmai_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_nunmai_home()}/state-snapshots/")
...
def _write_full_zip_backup_locked(out_path: Path, nunmai_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nunmai_root`` to ``out_path``.
...
def _pre_update_backup_dir(nunmai_home: Optional[Path] = None) -> Path:
    home = nunmai_home or get_nunmai_home()
...
def create_pre_update_backup(
    ...
    result = _write_full_zip_backup(out_path, nunmai_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nunmai claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NUNMAI_HOME under ``backups/`` before a
    ``nunmai claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nunmai import <archive>``.  Writes to
    ``<NUNMAI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nunmai import` and the
