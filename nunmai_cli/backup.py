...
from nunmai_constants import (
    _get_platform_default_nunmai_home, get_default_nunmai_root, get_nunmai_home, display_nunmai_home,
)
...
# Nunmai-managed runtime downloads (GGUF models, llama.cpp runtimes, managed Node): re-downloaded
# on demand and routinely tens to hundreds of GB. Matched ONLY at the root of NUNMAI_HOME and at
...
def run_backup(args) -> None:
    ...
    try:
        with _backup_operation_lock(nunmai_root):
            _run_backup_locked(args, nunmai_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, nunmai_root: Path) -> None:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(f"  ⚠ CRITICAL: could not snapshot DB file(s): {', '.join(failed_dbs)}\n"
              f"  ⚠ If sessions disappear after update, check {root} and run: nunmai snapshot list")
