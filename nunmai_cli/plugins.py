"""
Nunmai Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with nunmai-engine;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.nunmai/plugins/<name>/``
3. **Project plugins** – ``./.nunmai/plugins/<name>/`` (opt-in via
   ``NUNMAI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``nunmai_agent.plugins``
   entry-point group.
...
from nunmai_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from nunmai_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from nunmai_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``NUNMAI_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("NUNMAI_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``NUNMAI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.nunmai/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NUNMAI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NUNMAI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Nunmai loggers.
    """
    ...
    logger.debug(
        "NUNMAI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Nunmai' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by nunmai_cli.kanban_db when a task
    ...
    # separate `nunmai -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `nunmai kanban dispatch`),
    ...
    # ``nunmai kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: nunmai_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "nunmai_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "nunmai_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``nunmai_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- nunmai-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- nunmai-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- nunmai-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``nunmai:<event>``.
NUNMAI_EVENT_NAMESPACE = "nunmai"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Nunmai home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "nunmai", "depends",
}

# Highest manifest schema version this Nunmai understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Nunmai "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `nunmai plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.nunmai/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``nunmai plugins list``. For a flat plugin at
    ...
    # ``nunmai_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Nunmai never
    ...
    # (``nunmai plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Nunmai paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_nunmai_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from nunmai_cli import config as config_mod
        ...
        # threads. Other Nunmai config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from nunmai_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Nunmai profile name (e.g. ``"default"``).

        Derived from ``NUNMAI_HOME`` via
        :func:`nunmai_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.nunmai/profiles/<name>``, or ``"custom"`` when
        ``NUNMAI_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Nunmai core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``nunmai honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``nunmai <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`nunmai_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from nunmai_cli.dashboard_auth import DashboardAuthProvider
        from nunmai_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_nunmai_dotenv()`` startup — after
        ``~/.nunmai/.env`` loads, before Nunmai reads credentials — when
        ...
        NOTE ON TIMING: ``load_nunmai_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_nunmai_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Nunmai' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Nunmai' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``nunmai model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: nunmai_cli.main imports plugins indirectly
        from nunmai_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``nunmai:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``nunmai:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, NUNMAI_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{NUNMAI_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``nunmai:<event>``
    ...
    def register_skill(
        ...
        ``~/.nunmai/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or nunmai_home_key()
        ...
        # by resolved nunmai home). The ledger is therefore keyed per manager
        # — i.e. per (nunmai_home, plugin_id) — and every release/restore
        ...
        # contributed, so `nunmai plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("NUNMAI_SAFE_MODE"):
                logger.info("NUNMAI_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_nunmai_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from nunmai_cli.env_loader import load_nunmai_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Nunmai plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Nunmai core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Nunmai core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Nunmai Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with nunmai and must
            ...
            # all ~20 of them added several seconds to every `nunmai`
            # invocation — including plain `nunmai chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Nunmai ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `nunmai plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from nunmai_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_nunmai_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_nunmai_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.nunmai/plugins/)
        user_dir = get_nunmai_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.nunmai/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("NUNMAI_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".nunmai" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NUNMAI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("NUNMAI_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from nunmai_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from nunmai_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Nunmai process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``nunmai_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``nunmai plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`nunmai plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``nunmai tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `nunmai plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Nunmai validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Nunmai does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `nunmai plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from nunmai_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_nunmai_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``nunmai_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``nunmai_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Nunmai home (profile switch reusing a slug
        # like "nunmai-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"nunmai-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Nunmai home -> PluginManager. Nunmai supports
# multiple profiles via different NUNMAI_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_nunmai_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# nunmai_constants.set_nunmai_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_nunmai_home() / "plugins"`` and some
    plugins (notably context engines such as nunmai-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Nunmai home (env var *or* the context-local
    ``set_nunmai_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Nunmai home instead of
    being one process-wide singleton.
    """
    try:
        return get_nunmai_home().expanduser().resolve()
    except Exception:
        return get_nunmai_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``nunmai_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``nunmai_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Nunmai profile/home.
    ...
    profile switch — via ``NUNMAI_HOME`` or the context-local
    ``set_nunmai_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=nunmai_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_nunmai_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from nunmai_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nunmai-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``nunmai:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``nunmai tools`` TUI so plugin-provided toolsets appear
