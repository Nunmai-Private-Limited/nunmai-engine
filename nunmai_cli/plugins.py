...
class PluginContext:
    ...
    def register_skill(
        ...
        Not in ``~/.nunmai/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    threading.Thread(target=_runner, name="nunmai-plugin-command-await", daemon=True).start()
