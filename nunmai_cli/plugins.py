"""Nunmai Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.nunmai/plugins/<name>/``, project
``./.nunmai/plugins/<name>/`` (opt-in via ``NUNMAI_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``nunmai_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from nunmai_constants import get_nunmai_home, get_process_nunmai_home, nunmai_home_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from nunmai_cli.config import load_config_readonly
from nunmai_cli.middleware import VALID_MIDDLEWARE
from nunmai_cli.plugin_capabilities import plugin_capability_granted
from nunmai_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from nunmai_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from nunmai_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from nunmai_cli.plugins_loader import (
...
from nunmai_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, NUNMAI_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from nunmai_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from nunmai_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``NUNMAI_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
...
# ``NUNMAI_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("NUNMAI_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NUNMAI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("NUNMAI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (nunmai_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: nunmai_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
    ...
    "pre_command",
    # Per-turn model routing (used by the bundled model-router plugin). Fired right before an agent turn
    # is built — CLI (cli.py, before run_conversation) and gateway
    # (GatewayRunner._resolve_turn_agent_config, executor thread) — with the user's text and the
    # model/runtime that would otherwise be used. First valid dict wins and applies to THIS turn only; the
    # configured default and any /model session override are untouched:
    #   {"model": str, "runtime": {"provider", "api_key", "base_url", "api_mode"}, "reason": str}
    #        -> run this turn on that model/runtime
    #   None / {}  -> keep the configured model
    # Kwargs: surface ("cli" | "gateway"), text, model, runtime (dict), session_key,
    #   has_session_override (True while a /model session override is active — routers should normally
    #   respect it).
    "resolve_turn_model",
}
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.nunmai/profiles/<name>`` id, or ``"custom"``),
        derived from ``NUNMAI_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Nunmai core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``nunmai <name> ...``). *setup_fn* receives the argparse
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`nunmai_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        ...
        from nunmai_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
        if self._wrong_type(provider, DashboardAuthProvider, "dashboard-auth provider"):
            return
        launch_scope = nunmai_home_key(get_process_nunmai_home())
    ...
    def register_auxiliary_task(
        ...
        from nunmai_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``nunmai:x`` is reserved for core,
        ...
        if ":" in event:
            ...
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{NUNMAI_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        and listed by ``skills_list``. Not copied into ``~/.nunmai/skills/`` and not in the system
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_nunmai_dotenv()`` "
     "(after ``~/.nunmai/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # is keyed per (nunmai_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `nunmai plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            try:
                ...
                # load_nunmai_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_nunmai_dotenv() already
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            load_nunmai_dotenv(nunmai_home=home)
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Nunmai plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh NUNMAI_HOME/.plugin-compat-report.json from this discovery pass (nunmai_cli.plugin_compat).
        ...
        """
        try:
            from nunmai_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_nunmai_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_nunmai_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                ...
                if _discover_mcp(Path(manifest.path), get_nunmai_home() / "plugin-data"
...
# Resolved Nunmai home -> PluginManager. A process can switch profiles via
# ``set_nunmai_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    """Resolved active Nunmai home — the key for per-profile plugin managers (plugins capture the
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Nunmai profile/home (cached per resolved home; a
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from nunmai_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    # async hook under a running loop reads the default NUNMAI_HOME and get_secret raises.
    threading.Thread(target=contextvars.copy_context().run, args=(_runner,),
                     name="nunmai-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``nunmai tools`` TUI."""
    manager = get_plugin_manager()
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('nunmai_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('nunmai_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('nunmai_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('nunmai_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('nunmai_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('nunmai_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('nunmai_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('nunmai_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_nunmai_home_override': ('nunmai_constants', 'reset_nunmai_home_override'),
    'set_nunmai_home_override': ('nunmai_constants', 'set_nunmai_home_override'),
}
