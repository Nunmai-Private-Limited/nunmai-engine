"""Post-swap hand-off: finish ``nunmai update`` in an interpreter born on the pulled code.

``nunmai update`` starts in an interpreter that imported the PRE-pull tree. Once ``git merge``
...
into a hand-off file and re-executes ``nunmai update --post-swap <file>`` under the venv
...
POST_SWAP_ENV = "NUNMAI_UPDATE_POST_SWAP"
# Set on a child spawned DETACHED off the Windows console shim: the pid of the process that
# still holds ``nunmai.exe`` open (the launcher, or the interpreter it ran). The child waits
...
SHIM_PARENT_PID_ENV = "NUNMAI_UPDATE_SHIM_PARENT_PID"
...
GATEWAY_RESUME_ENV = "NUNMAI_UPDATE_GATEWAY_RESUME"
...
def write_handoff(payload: dict[str, Any]) -> Path:
    """Persist the post-swap payload under NUNMAI_HOME; returns its path."""
    from nunmai_constants import get_nunmai_home

    directory = get_nunmai_home() / "logs" / "update_receipts"
...
def _running_from_windows_shim() -> bool:
    from nunmai_cli.main_install_repair import _windows_shim_in_process_chain
...
def post_swap_command(handoff_path: Path, argv_tail: list[str]) -> list[str]:
    """``python -m nunmai_cli.main update <original flags> --post-swap <file>``."""
    return [str(post_swap_python()), "-m", "nunmai_cli.main", "update", *argv_tail, "--post-swap", str(handoff_path)]


def post_swap_child_env() -> dict[str, str]:
    """Environment for the child. ``NUNMAI_UPDATE_REEXEC`` marks it as already off the Windows
    ...
    from nunmai_cli.main_install_repair import _UPDATE_REEXEC_ENV
    from nunmai_cli.update_lock import HANDOFF_PID_ENV
...
def detached_shim_child_env(env: dict[str, str], gateway_resume: dict | None = None) -> dict[str, str]:
    ...
    open (the ``nunmai.exe`` launcher above us when psutil sees it, else this interpreter) and,
    for the legacy re-exec (no hand-off file), carries the Windows pause token."""
    from nunmai_cli.main_install_repair import _windows_shim_holder_pid
    ...
    try:
        ...
        while parent.is_running() and parent.status() != psutil.STATUS_ZOMBIE:
            if time.monotonic() >= deadline:
                print(f"  ⚠ The nunmai.exe process that started this update (PID {pid}) is still "
...
def _post_swap_cwd() -> str:
    ...
    ``nunmai update`` from elsewhere then resolves new top-level names only through the
    installed finder. If that map is stale, the child dies in ``import nunmai_cli.main``
...
def continue_update_in_fresh_interpreter(payload: dict[str, Any], *, argv_tail: list[str]) -> int | None:
    ...
    ``nunmai.exe``, the child cannot be awaited: the shim is one of the files the dependency
    ...
    if _running_from_windows_shim():
        try:
            ...
            return None
        print("→ Windows: nunmai.exe cannot replace itself while it runs; the update")
