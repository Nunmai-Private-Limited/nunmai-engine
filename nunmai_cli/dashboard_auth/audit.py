...
Profile-aware location: ``$NUNMAI_HOME/logs/dashboard-auth.log``.
...
from ``nunmai_constants`` or other nunmai_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NUNMAI_HOME/logs/dashboard-auth.log``.

    Uses ``nunmai_constants.get_nunmai_home()`` (a leaf module — no import
    ...
    return get_nunmai_home() / "logs" / "dashboard-auth.log"
