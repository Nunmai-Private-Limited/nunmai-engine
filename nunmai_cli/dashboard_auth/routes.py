...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``NUNMAI_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /nunmai`` (Mission Control deploys) — we
...
def _prefix(request: Request) -> str:
    ...
    ``nunmai_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("nunmai_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from nunmai_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from nunmai_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    from nunmai_cli.dashboard_auth.base import RefreshExpiredError
