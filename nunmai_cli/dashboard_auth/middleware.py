...
from nunmai_cli.dashboard_auth import list_session_providers
from nunmai_cli.dashboard_auth.audit import AuditEvent, audit_log
from nunmai_cli.dashboard_auth.base import (
...
from nunmai_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /nunmai``, the
    ``login_url`` is prefixed (``/nunmai/login?next=...``) so the
    ...
    """
    from nunmai_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["nunmai.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``nunmai_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``nunmai_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Nunmai Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from nunmai_cli.dashboard_auth.cookies import clear_session_cookies
        from nunmai_cli.dashboard_auth.prefix import prefix_from_request
