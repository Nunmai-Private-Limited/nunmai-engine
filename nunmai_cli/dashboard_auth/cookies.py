...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``nunmai_session_at``
(access token; Max-Age = token TTL), ``nunmai_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``nunmai_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``nunmai_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``nunmai_sso_attempt`` (auto-SSO loop guard, 60 s).
...
def _cookie_path(prefix: str) -> str:
    """``Path=/nunmai`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nunmai Portal issues a 24h rotating refresh token (nunmai #37247); a provider that omits it returns
