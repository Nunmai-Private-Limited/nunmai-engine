...
Three cookies in play:
  - nunmai_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - nunmai_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Nunmai Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / nunmai
  ...
  - nunmai_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/nunmai) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/nunmai`` covers
...
setter wrote ``__Secure-nunmai_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "nunmai_session_at"
SESSION_RT_COOKIE = "nunmai_session_rt"
...
# auth plugins are enabled (for example Basic + Nunmai OAuth).
SESSION_PROVIDER_COOKIE = "nunmai_session_provider"
PKCE_COOKIE = "nunmai_session_pkce"
...
SSO_ATTEMPT_COOKIE = "nunmai_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /nunmai`` we want ``Path=/nunmai`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Nunmai Portal
    issues a 24h rotating refresh token (nunmai #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/nunmai``)
