...
Proxies mounting the dashboard at a path inject ``X-Forwarded-Prefix: /nunmai``
so the backend can build prefixed URLs (Location headers, OAuth redirect_uri,
cookie Path, SPA asset URLs). An operator-declared ``NUNMAI_DASHBOARD_PUBLIC_URL``
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    _warn_once(
        ...
        source, cleaned, cleaned.split("://")[-1] or "nunmai.example.com")
...
def normalise_prefix(raw: Optional[str]) -> str:
    """``"/nunmai"`` form (no trailing slash) or ``""`` when unset/malformed. ``..``, ``//`` and
...
def resolve_public_url() -> str:
    ...
    ``NUNMAI_DASHBOARD_PUBLIC_URL`` env (blank counts as unset so a provisioned-but-blank secret
