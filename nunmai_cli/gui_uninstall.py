"""Nunmai Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Nunmai" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nunmai"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Nunmai"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Nunmai"


def source_built_gui_artifacts(nunmai_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``nunmai desktop`` inside the checkout (same ``nunmai-engine/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Nunmai.app"), home / "Applications" / "Nunmai.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Nunmai", local_base / "nunmai-desktop"] + (
            [Path(program_files) / "Nunmai"] if program_files else [])
    ...
    return [desktop_entry_path(), data_base / "applications" / "Nunmai.desktop"] + [
        icons / size / "apps" / "nunmai.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(nunmai_home: Path) -> bool:
    """True when a usable Python agent install exists under NUNMAI_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((nunmai_home / "nunmai-engine" / sub).is_dir() for sub in ("nunmai_cli", "venv", ".venv"))


def gui_is_installed(nunmai_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(nunmai_home), *packaged_gui_app_paths(), desktop_userdata_dir()))
...
def gui_install_summary(nunmai_home: "Path | None" = None) -> dict:
    ...
    return {"nunmai_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(nunmai_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Nunmai
        # disappears from the launcher.
        try:
            from nunmai_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove nunmai' or 'sudo dnf remove nunmai'). AppImage builds are a single "
