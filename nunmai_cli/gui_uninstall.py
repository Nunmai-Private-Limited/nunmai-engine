"""
Nunmai Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``nunmai desktop`` / ``nunmai gui``)
     Built inside the agent checkout under ``$NUNMAI_HOME/nunmai-engine/``:
       ...
       - ``$NUNMAI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Nunmai.app`` or ``~/Applications/Nunmai.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Nunmai`` (NSIS per-user)
...
the app name ("Nunmai"), separate from ``$NUNMAI_HOME``:
  - macOS:   ``~/Library/Application Support/Nunmai``
  - Windows: ``%APPDATA%\\Nunmai``
  - Linux:   ``$XDG_CONFIG_HOME/Nunmai`` (default ``~/.config/Nunmai``)
...
``nunmai uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(nunmai_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return nunmai_home / "nunmai-engine"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Nunmai"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nunmai"
...
def source_built_gui_artifacts(nunmai_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``nunmai desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``nunmai-engine/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(nunmai_home)
    ...
    return [
        ...
        nunmai_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Nunmai" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Nunmai.app"),
            home / "Applications" / "Nunmai.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Nunmai).
            local_base / "Programs" / "Nunmai",
            # Older / alternate layout some builds used.
            local_base / "nunmai-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Nunmai")
    else:
        ...
        # ``uninstall_gui``.
        from nunmai_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `nunmai desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Nunmai.desktop",
        ]
    return paths


def agent_is_installed(nunmai_home: Path) -> bool:
    """Return True when a usable Python agent install exists under NUNMAI_HOME.
    ...
    if (agent_root / "nunmai_cli").is_dir():
        return True
...
def gui_is_installed(nunmai_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(nunmai_home):
        if p.exists():
            return True
...
def gui_install_summary(nunmai_home: "Path | None" = None) -> dict:
...
def uninstall_gui(nunmai_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``nunmai-engine/nunmai_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$NUNMAI_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Nunmai disappears from the launcher.
        try:
            from nunmai_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove nunmai' or "
            "'sudo dnf remove nunmai'). AppImage builds are a single file you "
