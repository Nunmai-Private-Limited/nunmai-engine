"""Slash command definitions and autocomplete for the Nunmai CLI.
...
from nunmai_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``nunmai_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``nunmai_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Nunmai config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Nunmai works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Nunmai' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Nunmai is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.nunmai/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Nunmai plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Nunmai balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Nunmai Engine to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Nunmai Engine version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nunmai_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nunmai`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Nunmai ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Nunmai
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nunmai skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nunmai skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nunmai skills list`` and
...
# unrelated command landed. These claim their slots right after /nunmai,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /nunmai <command>).
...
# native slot, the alias spelling stays reachable via /nunmai reset).
...
# ``/nunmai <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/nunmai" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /nunmai topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /nunmai moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /nunmai debug on Slack.
#   - egress: Docker-only proxy status; reachable as /nunmai egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /nunmai init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /nunmai version on
...
#   - diff: git working-tree diff; reached via /nunmai diff on Slack so it
...
#     /nunmai update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /nunmai heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /nunmai refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /nunmai pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /nunmai whoami on Slack.
...
#     /nunmai platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_NUNMAI_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nunmai <verb>`` subcommand.
    ...
    can still reach them via ``/nunmai <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nunmai`` is always reserved as the first entry so the
    legacy ``/nunmai <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nunmai as the catch-all top-level command.
    entries.append(("nunmai", "Talk to Nunmai or run a subcommand", "[subcommand] [args]"))
    seen.add("nunmai")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NUNMAI_ONLY:
            # Intentionally Slack-via-/nunmai only (see _SLACK_VIA_NUNMAI_ONLY).
            return
    ...
    # everything except /nunmai, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nunmai handler.

    Maps both canonical names and aliases so /nunmai bg do stuff works
    the same as /nunmai background do stuff.

    Plugin-registered slash commands are included so ``/nunmai <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nunmai_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via nunmai_cli.personality."""
        try:
