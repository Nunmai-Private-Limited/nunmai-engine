...
from nunmai_cli.auth_constants import DEFAULT_NOUS_PORTAL_URL
...
def _confirm_selection_guards(
    ...
    """
    try:
        from nunmai_cli.model_selection_guards import combined_message, selection_warnings
...
class _ModelPickerRows:
    """Column-aligned picker rows (name + $/Mtok + Nunmai sale chrome).
    ...
    def __init__(
        ...
        from nunmai_cli.models_pricing import _format_price_per_mtok, compute_sale_discount
...
def _prompt_model_selection(
    ...
    *unavailable_models* render grayed out and unselectable with an upgrade link to *portal_url*.
    ...
    # Sale chrome is Nunmai Portal-only, even if pricing.original is present for another provider.
