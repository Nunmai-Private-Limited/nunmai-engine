...
def _raise_for_anon_status(response: httpx.Response, *, action: str) -> Dict[str, Any]:
    ...
    if response.status_code == 404 and error == "unknown_token":
        raise AnonCredentialDead("Nunmai free-tier credential is no longer valid.", code="anon_credential_dead")
    if response.status_code == 401 and error == "invalid_shared_secret":
        raise _anon_err("Nunmai free tier is not open on this portal.", "anon_gate_closed")
    if response.status_code == 401:
        raise AnonCredentialDead("Nunmai free-tier credential was revoked.", code="anon_credential_dead")
    if response.status_code == 429:
        raise _anon_err("Nunmai free tier is rate limited; try again shortly.", "anon_rate_limited")
    if response.status_code == 403 and error in {"anonymous_accounts_disabled", "circuit_open"}:
        raise _anon_err("Nunmai free tier is currently disabled.", "anon_gate_closed")
    raise _anon_err(
        f"Nunmai free tier {action} failed ({response.status_code}{': ' + error if error else ''}).",
        "anon_server_error")
...
def mint_guest(client: httpx.Client, portal_base_url: str) -> Dict[str, Any]:
    ...
    if not isinstance(token, str) or not token.startswith("anon_"):
        raise _anon_err("Nunmai free tier sign-up returned no credential.", "anon_server_error")
    return payload
...
def exchange_anon_jwt(client: httpx.Client, portal_base_url: str, anon_token: str) -> Dict[str, Any]:
    ...
    if not isinstance(payload.get("access_token"), str) or not payload["access_token"]:
        raise _anon_err("Nunmai free tier token exchange returned no token.", "anon_server_error")
    return payload
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    if not isinstance(anon_token, str) or not anon_token:
        raise AnonCredentialDead("Nunmai free-tier credential is missing.", code="anon_credential_dead")
...
_WELCOME_ROUTE_COPY = {
    "anon_on_paid_host": "The Nunmai free tier must use its own inference host ({host}); "
                         "Nunmai is pointed at the paid one. Restart Nunmai to re-read the route, "
                         "or unset NOUS_INFERENCE_BASE_URL if you set it.",
    "named_on_welcome_host": "This Nunmai account must use the Nunmai Portal inference host, "
                             "not the free tier's. Run /model and pick the Nunmai row again.",
    "tier_disabled": "The Nunmai free tier is switched off right now. {signin}",
}
_SIGNIN_CHAT = "Sign in with a Nunmai account for the full catalog: /login."
_SIGNIN_TERMINAL = "Sign in with a Nunmai account for the full catalog: `nunmai auth upgrade`."
...
def welcome_refusal_copy(refusal: Dict[str, Any], *, model: str = "", in_chat: bool = True) -> str:
    ...
    if reason == "model_not_free":
        what = f"{model} isn't on the Nunmai free tier" if model else "That model isn't on the Nunmai free tier"
        return f"{what}; it serves {serves} only. {signin}"
    if reason == "feature_not_free":
        return f"This feature isn't on the Nunmai free tier. {signin}"
    if reason == "at_capacity":
        return f"The Nunmai free tier is at capacity and briefly paused. {wait} {signin}"
    if reason == "admission_closed":
        return f"The Nunmai free tier isn't admitting new sessions right now. {wait} {signin}"
    if reason == "rate_limited":
        return f"Nunmai free tier rate limit active \u2014 resets in {retry}s. {signin}"
    return f"The Nunmai free tier refused this request ({reason}). {signin}"
...
def welcome_route_refusal_copy(kind: str, *, in_chat: bool = True) -> str:
    template = _WELCOME_ROUTE_COPY.get(kind) or "The Nunmai inference gateway refused this route."
...
def register_promotion_intent(
    ...
    if not isinstance(payload.get("claim_code"), str) or not payload["claim_code"]:
        raise _anon_err("Nunmai free tier sign-in returned no transfer code.", "anon_server_error")
    return payload
