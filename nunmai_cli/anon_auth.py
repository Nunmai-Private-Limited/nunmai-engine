"""Nunmai free-tier identity: the ``anonymous`` auth method of the ``nous`` provider.

The identity is created in exactly one place, at boot (``nunmai_cli.free_tier_bootstrap``), and only
while ``NUNMAI_GUEST_ONBOARDING=1`` (see ``guest_enabled``). The bootstrap mints an anonymous Nunmai
...
"Nunmai · free tier". Two user-facing verbs reach the same flow, both keeping the identity's
connectors: ``nunmai auth upgrade`` in a terminal and ``/login`` inside a chat.
...
ANON_SECRET_ENV = "NUNMAI_ANON_API_SECRET"
...
GUEST_ONBOARDING_ENV = "NUNMAI_GUEST_ONBOARDING"
...
FREE_TIER_LABEL = "Nunmai · free tier"
UPGRADE_HINT = "Run `nunmai auth upgrade` to sign in with a Nunmai account, or /login inside a chat."
FREE_TIER_NOT_SIGNED_IN = (
    "You're not signed in. Free inference and connectors are always on. "
    "Run `nunmai auth` to sign in with a Nunmai account.")
...
# fallback (no ``/login``, no ``nunmai`` verb, no guest / anonymous / credential). ``retryable``
...
# what is unavailable is using Nunmai without signing in, and signing in is free.
_SIGNIN_IS_FREE = "Signing in is free."
ANON_FAILURE_COPY = {
    ANON_GATE_CLOSED: f"This version can't be used without a Nunmai account. {_SIGNIN_IS_FREE}",
    ANON_GATE_PAUSED: f"Using Nunmai without signing in is paused for a moment. {_SIGNIN_IS_FREE}",
    ANON_RATE_LIMITED: "Lots of people are getting started right now. Try again in {wait}. "
                       "Signing in is free and skips the wait.",
    ANON_POW_REQUIRED: "The Nunmai server asked for a proof of work, but that isn't implemented in your "
                       "Agent yet. Sign in with a Nunmai account to continue.",
    ...
    ANON_UNREACHABLE: "The Nunmai service couldn't be reached. Check your internet connection and try again.",
    ANON_SERVER_ERROR: "The Nunmai service had a hiccup. Try again in a moment.",
}
...
def is_anonymous_request(provider: Any, api_key: Any) -> bool:
    ...
    """
    from nunmai_cli.auth_constants import _decode_jwt_claims
...
def current_nous_state() -> Optional[Dict[str, Any]]:
    ...
    from nunmai_cli.auth import _load_auth_store, _load_provider_state
    try:
...
def guest_carries_inference() -> bool:
    """True when the profile's Nunmai identity is the free tier and the free tier is on.
    ...
    credential-pool entry can pick a paid Nunmai key while the profile singleton is still a guest.
    """
...
EXTRA_WELCOME_HOSTS_ENV = "NUNMAI_EXTRA_WELCOME_HOSTS"


def welcome_hosts() -> frozenset[str]:
    """``WELCOME_HOSTS`` plus any ``NUNMAI_EXTRA_WELCOME_HOSTS`` entries (lowercased hostnames)."""
...
def pin_model_for_route(provider: Any, base_url: Any, model: Any) -> Any:
    """Model policy at agent START: on the Nunmai welcome host the model is ``nous/welcome``; anywhere
    ...
    if provider == "nous" and route_is_welcome_host(base_url):
        if model and model != GUEST_MODEL:
            logger.info("Nunmai free tier: using %s instead of configured model %s", GUEST_MODEL, model)
        return GUEST_MODEL
    return model
...
def route_can_serve_model(provider: Any, base_url: Any, model: Any) -> bool:
    ...
    may move to the portal host, which serves it too). Non-Nunmai routes are always eligible."""
...
def _raise_for_anon_status(response: httpx.Response, *, action: str) -> Dict[str, Any]:
    ...
    if code == ANON_SERVER_ERROR:
        logger.info("Nunmai free tier %s failed (%s%s)", action, status, f": {error}" if error else "")
...
def mint_guest(client: httpx.Client, portal_base_url: str) -> Dict[str, Any]:
    ...
    if not isinstance(token, str) or not token.startswith("anon_"):
        logger.info("Nunmai free tier sign-up returned no credential")
...
def exchange_anon_jwt(client: httpx.Client, portal_base_url: str, anon_token: str) -> Dict[str, Any]:
    ...
    if not isinstance(payload.get("access_token"), str) or not payload["access_token"]:
        logger.info("Nunmai free tier token exchange returned no token")
        raise _anon_err(ANON_FAILURE_COPY[ANON_SERVER_ERROR], ANON_SERVER_ERROR)
    return payload
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from nunmai_cli.auth_nous import _validate_nous_inference_url_from_network
...
def _portal_base_url() -> str:
    from nunmai_cli.auth_nous import _nous_portal_env_override
    return (_nous_portal_env_override() or DEFAULT_NOUS_PORTAL_URL).rstrip("/")


def _shared_identity_key(state: Any) -> Optional[str]:
    """Stable identity of a Nunmai credential: the anon_ token for a guest, the refresh token for an
...
def _mint_locked(
    ...
    from nunmai_cli.auth import _store_provider_state, _save_auth_store
    from nunmai_cli.auth_nous import _write_shared_nous_state
    ...
    logger.info("Nunmai free tier ready (identity minted)")
    return state
...
def _mint_memo_key() -> str:
    ...
    return "" if get_nunmai_home_override() is None else nunmai_home_key()
...
def _reconcile_and_provision(*, timeout_seconds: float, carries_inference: bool = True) -> Optional[Dict[str, Any]]:
    ...
    1. The shared store is the identity of record for this Nunmai root. If it holds an identity
    ...
    from nunmai_cli.auth_nous import (
    ...
    with _auth_store_lock():
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds, 5.0)):
            ...
            if shared and _shared_identity_key(shared) != _shared_identity_key(profile_state):
                ...
                _save_auth_store(auth_store)
                logger.debug("Nunmai identity adopted from the shared store")
                return state
            if profile_state:
                if not shared:
...
def ensure_portal_identity(
    *, explicit: bool, timeout_seconds: float = GUEST_MINT_TIMEOUT_SECONDS,
    ...
    """Make sure this profile has a Nunmai identity (guest or account); mint a guest only if the shared
    ...
    first, then shared, matching every other Nunmai path. ``carries_inference=False`` leaves
    ...
    try:
        ...
        logger.info("Nunmai free tier not set up (%s, attempt %d%s)", noted.code, noted.attempts,
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    from nunmai_cli.auth import _nous_portal_base_url
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from nunmai_cli.auth_nous import _clear_shared_nous_state, _nous_shared_store_lock, _read_shared_nous_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
    ...
    _clear_mint_failure()
    logger.info("Nunmai free-tier identity retired (%s); a new one is set up on next use", reason)
...
_WELCOME_ROUTE_REFUSALS = (
    ...
    ("serves anonymous nunmai agent accounts only", "named_on_welcome_host"),
...
_WELCOME_ROUTE_COPY = {
    ...
    "anon_on_paid_host": "This install is set to use a different Nunmai server (NOUS_INFERENCE_BASE_URL). "
                         "Unset it to use the free model, or sign in. {signin}",
    "named_on_welcome_host": "This Nunmai account needs to reconnect. {model_hint}",
    "tier_disabled": "Using Nunmai without signing in is switched off right now. "
...
_SIGNIN_TERMINAL = "To sign in: `nunmai auth upgrade`."
_MODEL_HINT_CHAT = "Run /model and pick the Nunmai row again."
_MODEL_HINT_TERMINAL = "Run `nunmai model` and pick the Nunmai row again."
...
def welcome_refusal_copy(refusal: Dict[str, Any], *, model: str = "", in_chat: bool = True, door: bool = True) -> str:
    ...
    if reason == "model_not_free":
        ...
        return (f"{what} available without signing in, so Nunmai uses {serves} for now. "
    ...
    return f"Nunmai couldn't send that without signing in. Signing in is free. {signin}".rstrip()
...
def welcome_route_refusal_copy(kind: str, *, in_chat: bool = True, door: bool = True) -> str:
    template = _WELCOME_ROUTE_COPY.get(kind) or "Nunmai couldn't reach the free model on this route."
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    map. The config write is the same one a sign-in completion uses, so ``nunmai model`` and the
    ...
    logger.info("Nunmai gateway asked to switch %s -> %s; applied for this session", requested, backing)
    try:
        from nunmai_cli.config import load_config_readonly
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            from nunmai_cli.auth import _update_config_for_provider
...
FREE_TIER_AVAILABLE_NOTICE = (
    "Free Nunmai inference and connectors are now available. "
...
# --- ``nunmai auth upgrade``: sign the guest into a real Nunmai account, keeping its connectors ---------
#
...
def register_promotion_intent(
    ...
    if not isinstance(payload.get("claim_code"), str) or not payload["claim_code"]:
        logger.info("Nunmai free tier sign-in returned no transfer code")
        raise _anon_err(ANON_FAILURE_COPY[ANON_SERVER_ERROR], ANON_SERVER_ERROR)
    return payload
...
def _account_state_from_token(
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from nunmai_cli.auth_nous import _NOUS_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_nous_oauth_from_state
    now = datetime.now(timezone.utc)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default for the account's tier (:func:`nunmai_cli.models.recommended_nous_default_model`, the
    same pick as ``GET /api/model/recommended-default``), through the same config write a plain Nunmai
    login uses. A config on the user's own model and host is left alone.

    Every sign-in completion (CLI ``nunmai auth upgrade``, the desktop poller) calls this once, after
    ...
    model = current
    if on_welcome_model:
        from nunmai_cli.models import recommended_nous_default_model
        try:
    ...
    try:
        ...
        # same write; the runtime's silent default applies until the user picks one with `nunmai model`.
        _update_config_for_provider(
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth_device_flow import _poll_for_token as poll
...
def persist_nous_credentials(*args, **kwargs):
    ...
    from nunmai_cli.auth_nous import persist_nous_credentials as persist
...
from nunmai_cli.anon_sign_in import (  # noqa: E402
...
from nunmai_cli.anon_sign_in_cli import (  # noqa: E402
