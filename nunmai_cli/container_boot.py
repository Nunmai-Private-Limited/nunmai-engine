...
01-nunmai-setup (the stage2 hook) has chowned the volume and seeded $NUNMAI_HOME, but
...
from nunmai_cli.gateway_multiplex_s6 import AUTOSTART_STATES as _AUTOSTART_STATES, fold_named_slot_intent
...
def _named_profile_dirs(nunmai_home: Path) -> list[tuple[str, Path]]:
    """Every real named profile under ``$NUNMAI_HOME/profiles`` (``SOUL.md`` is the marker)."""
...
def reconcile_profile_gateways(
    *, nunmai_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$NUNMAI_HOME``): ``nunmai_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``nunmai gateway start`` (no ``-p``) targets.

    Without it, bare ``nunmai gateway start`` inside the container would land on ``s6-svc -u
    /run/service/gateway-default`` → uncaught ``CalledProcessError`` → traceback to the user (PR #30136
    review).
    """
    actions: list[ReconcileAction] = []
    # ONE gateway per container: named slots are registered (so `nunmai -p X gateway start` has a
    ...
    # as a topology switch (nunmai_cli/gateway_multiplex_mode.py), so there is nothing to read.
    named = [(name, entry, _read_desired_state(entry)) for name, entry in _named_profile_dirs(nunmai_home)]
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        nunmai_home, container_argv=container_argv, dry_run=dry_run)
    ...
    # that serves them. Without this an image only ever driven as `nunmai -p coder gateway start`
    ...
    actions.append(_slot_action("default", nunmai_home, default_prior_state, default_should_start,
...
def _maybe_migrate_legacy_gateway_run_state(
    nunmai_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "nunmai":  # the wrapper re-execs `nunmai <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `nunmai -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with nunmai ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the nunmai user won't EACCES.
...
def _write_reconcile_log(nunmai_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $NUNMAI_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted NUNMAI_HOME both
    ...
    actions = reconcile_profile_gateways(nunmai_home=nunmai_home, scandir=scandir)
