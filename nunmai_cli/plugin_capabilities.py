...
this. Capabilities govern the *host API surfaces* Nunmai hands out (which
...
until the user re-consents (``nunmai plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Nunmai may declare ids this build doesn't know; they can
