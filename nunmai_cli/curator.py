...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`nunmai curator adopt <name>` hands one over"
    )
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`nunmai curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`nunmai curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.nunmai/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.nunmai/skills/ from a curator snapshot, or a single "
