"""Migrate legacy ``NUNMAI_NEMO_RELAY_ATIF_*`` / ``ATOF_*`` exporter vars into a Relay ``plugins.toml``.

The Relay cutover (Aug 2026) stopped honouring the legacy exporter variables: a profile that still
carries them and no ``NUNMAI_NEMO_RELAY_PLUGINS_TOML`` logs one warning and initialises NO exporters,
...
the file is exactly what Relay validates), points ``NUNMAI_NEMO_RELAY_PLUGINS_TOML`` at it, and
comments the legacy lines out. It runs from ``nunmai update`` for every profile home and from
``nunmai relay migrate`` for the active one.
"""
...
def relay_plugin_payload_from_legacy_env(env: Mapping[str, Any]) -> dict[str, Any]:
    ...
    atif = atof = None
    if _truthy(env.get("NUNMAI_NEMO_RELAY_ATIF_ENABLED")):
        kwargs: dict[str, Any] = {
            "enabled": True,
            "agent_name": _s(env, "NUNMAI_NEMO_RELAY_ATIF_AGENT_NAME", "Nunmai Engine"),
            "model_name": _s(env, "NUNMAI_NEMO_RELAY_ATIF_MODEL_NAME", "unknown"),
            "filename_template": _s(env, "NUNMAI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE", "nunmai-atif-{session_id}.json"),
        }
        if _s(env, "NUNMAI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"):
            kwargs["output_directory"] = _s(env, "NUNMAI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY")
        if _s(env, "NUNMAI_NEMO_RELAY_ATIF_AGENT_VERSION"):
            kwargs["agent_version"] = _s(env, "NUNMAI_NEMO_RELAY_ATIF_AGENT_VERSION")
        atif = obs.AtifConfig(**kwargs)
    if _truthy(env.get("NUNMAI_NEMO_RELAY_ATOF_ENABLED")):
        mode = _s(env, "NUNMAI_NEMO_RELAY_ATOF_MODE", "append").lower()
        sink = obs.AtofFileSinkConfig(
            output_directory=_s(env, "NUNMAI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY") or None,
            filename=_s(env, "NUNMAI_NEMO_RELAY_ATOF_FILENAME", "nunmai-atof.jsonl"),
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    from nunmai_cli.config import _env_line_defines_key, _quote_env_value, _read_env_lines, _write_env_lines
    ...
    if env.get(RELAY_PLUGINS_CONFIG_ENV, "").strip():
        ...
        return result
    if not (_truthy(env.get("NUNMAI_NEMO_RELAY_ATIF_ENABLED")) or _truthy(env.get("NUNMAI_NEMO_RELAY_ATOF_ENABLED"))):
    ...
    header = (
        "# NeMo Relay plugin configuration for Nunmai (selected via "
        f"{RELAY_PLUGINS_CONFIG_ENV} in .env).\n"
        "# Generated by `nunmai update` from the legacy NUNMAI_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def migrate_all_profile_relay_envs(*, validate: bool = True) -> list[RelayMigrationResult]:
    ...
    from nunmai_cli.profiles import _get_default_nunmai_home, _iter_named_profile_dirs
    homes = [_get_default_nunmai_home(), *_iter_named_profile_dirs()]
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    """Loud, actionable notice for `nunmai update` / `nunmai relay migrate`."""
    ...
    if migrated:
        ...
        print("   The legacy NUNMAI_NEMO_RELAY_ATIF_*/ATOF_* variables stopped producing traces after the")
...
def run_relay_migration_after_update() -> None:
    """`nunmai update` hook: migrate every profile home, print the notice. Best-effort by contract."""
    print_relay_migration_report(migrate_all_profile_relay_envs())


RELAY_MIGRATE_COMMAND = "nunmai migrate relay"


def cmd_migrate_relay(args) -> None:
    """``nunmai migrate relay [--all-profiles] [--no-validate]``."""
    ...
    if getattr(args, "all_profiles", False):
        ...
        results = [migrate_profile_relay_env(get_nunmai_home(), validate=validate)]
