"""Nunmai Engine — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m nunmai_cli.main web [--port 8080]``.
"""
...
from nunmai_cli.install_identity import get_install_id as _shared_get_install_id
from nunmai_cli.process_identity import is_desktop_owned_backend
from nunmai_cli.pty_session import run_reaper
...
from nunmai_cli import __version__
...
from nunmai_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``nunmai dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            ...
            if initial_profile_homes:
                ...
                from nunmai_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `nunmai update` otherwise 500s
    ...
    # Import nunmai_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `nunmai update` replaced the code
    ...
    # gateway running the scheduler. Server `nunmai dashboard` is unaffected —
    ...
    def _boot_local_runtime():
        try:
            from nunmai_cli.config import load_config
            from nunmai_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    # Nunmai free tier: the ONE place its identity is created. Inventories credentials, mints only
    # when NUNMAI_GUEST_ONBOARDING=1, records the answer for setup.status / free_tier.status and
    ...
    from nunmai_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from nunmai_cli.local_runtime.bootstrap import shutdown_local_runtime
...
# NUNMAI_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
...
from nunmai_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 nunmai-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``NUNMAI_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("NUNMAI_DESKTOP") == "1"
        and bool(os.environ.get("NUNMAI_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
from nunmai_cli import web_server_gateway as _gateway_mod  # noqa: E402
from nunmai_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from nunmai_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from nunmai_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from nunmai_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Nunmai home (not the profile NUNMAI_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``nunmai gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_nunmai_action(subcommand, "gateway-restart")
...
from nunmai_cli.web_routers import (  # noqa: E402
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 nunmai-0day
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from nunmai_cli.web_server_idle_exit import (
...
def _publish_host_rendezvous(host: str, port: int) -> None:
    ...
    # only. They still publish under their own role so `nunmai plugins install` from a terminal
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from nunmai_cli.dashboard_procs import _reap_orphaned_desktop_local_serves
    ...
    # No-op for standalone `nunmai serve` (no NUNMAI_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from nunmai_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # Published by /api/host/identity: an attaching `nunmai dashboard` must never be routed to a
    ...
    # ACTUAL port — what lets `nunmai update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from nunmai_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    # Host rendezvous (multiplex-only): the host lock + record that let a SECOND `nunmai serve`
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from nunmai_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``NUNMAI_SERVE_HEADLESS``).
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('nunmai_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('nunmai_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('nunmai_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('nunmai_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('nunmai_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('nunmai_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('nunmai_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('nunmai_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('nunmai_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('nunmai_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('nunmai_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('nunmai_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('nunmai_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('nunmai_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('nunmai_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('nunmai_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('nunmai_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('nunmai_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('nunmai_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('nunmai_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('nunmai_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('nunmai_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('nunmai_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('nunmai_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('nunmai_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('nunmai_cli.web_models', 'HookCreate'),
    'HookDelete': ('nunmai_cli.web_models', 'HookDelete'),
    'ImportRequest': ('nunmai_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('nunmai_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('nunmai_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('nunmai_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('nunmai_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('nunmai_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('nunmai_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('nunmai_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('nunmai_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('nunmai_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('nunmai_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('nunmai_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('nunmai_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('nunmai_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('nunmai_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('nunmai_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('nunmai_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('nunmai_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('nunmai_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('nunmai_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('nunmai_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('nunmai_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('nunmai_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('nunmai_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('nunmai_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('nunmai_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('nunmai_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('nunmai_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('nunmai_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('nunmai_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('nunmai_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('nunmai_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('nunmai_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('nunmai_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('nunmai_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('nunmai_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('nunmai_cli.web_models', 'SessionImport'),
    'SessionPrune': ('nunmai_cli.web_models', 'SessionPrune'),
    'SessionRename': ('nunmai_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('nunmai_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('nunmai_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('nunmai_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('nunmai_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('nunmai_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('nunmai_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('nunmai_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('nunmai_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('nunmai_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('nunmai_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('nunmai_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('nunmai_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('nunmai_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('nunmai_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('nunmai_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('nunmai_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('nunmai_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('nunmai_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('nunmai_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('nunmai_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('nunmai_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('nunmai_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('nunmai_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('nunmai_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('nunmai_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('nunmai_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('nunmai_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('nunmai_cli.web_routers.mcp', 'auth_mcp_server'),
    'bulk_delete_sessions_endpoint': ('nunmai_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('nunmai_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('nunmai_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('nunmai_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('nunmai_cli.config', 'cfg_get'),
    'check_config_version': ('nunmai_cli.config', 'check_config_version'),
    'check_nunmai_update': ('nunmai_cli.web_routers.actions', 'check_nunmai_update'),
    'clear_model_endpoint_credentials': ('nunmai_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('nunmai_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('nunmai_cli.config', 'coerce_provider_id'),
    'console_ws': ('nunmai_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('nunmai_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('nunmai_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('nunmai_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('nunmai_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('nunmai_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('nunmai_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('nunmai_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('nunmai_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('nunmai_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('nunmai_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('nunmai_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('nunmai_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('nunmai_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('nunmai_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('nunmai_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('nunmai_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('nunmai_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('nunmai_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('nunmai_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('nunmai_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('nunmai_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('nunmai_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('nunmai_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('nunmai_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('nunmai_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('nunmai_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('nunmai_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('nunmai_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('nunmai_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('nunmai_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('nunmai_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('nunmai_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('nunmai_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('nunmai_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('nunmai_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('nunmai_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('nunmai_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('nunmai_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('nunmai_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('nunmai_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('nunmai_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('nunmai_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('nunmai_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('nunmai_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('nunmai_cli.config', 'get_config_path'),
    'get_config_raw': ('nunmai_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('nunmai_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('nunmai_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('nunmai_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('nunmai_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('nunmai_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('nunmai_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('nunmai_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('nunmai_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('nunmai_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('nunmai_cli.config', 'get_env_path'),
    'get_env_vars': ('nunmai_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('nunmai_cli.web_routers.status', 'get_health'),
    'get_nunmai_home': ('nunmai_cli.config', 'get_nunmai_home'),
    'get_learning_graph': ('nunmai_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('nunmai_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('nunmai_cli.web_routers.status', 'get_logs'),
    'get_media': ('nunmai_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('nunmai_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('nunmai_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('nunmai_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('nunmai_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('nunmai_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('nunmai_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('nunmai_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('nunmai_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('nunmai_cli.web_routers.status', 'get_portal_status'),
    'get_process_nunmai_home': ('nunmai_cli.config', 'get_process_nunmai_home'),
    'get_profile_setup_command': ('nunmai_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('nunmai_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('nunmai_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('nunmai_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('nunmai_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('nunmai_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('nunmai_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('nunmai_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('nunmai_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('nunmai_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('nunmai_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('nunmai_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('nunmai_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('nunmai_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('nunmai_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('nunmai_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('nunmai_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('nunmai_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('nunmai_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('nunmai_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('nunmai_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('nunmai_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('nunmai_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('nunmai_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('nunmai_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('nunmai_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('nunmai_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('nunmai_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('nunmai_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('nunmai_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('nunmai_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('nunmai_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('nunmai_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('nunmai_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('nunmai_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('nunmai_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('nunmai_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('nunmai_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('nunmai_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('nunmai_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('nunmai_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('nunmai_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('nunmai_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('nunmai_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('nunmai_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('nunmai_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('nunmai_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('nunmai_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('nunmai_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('nunmai_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('nunmai_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('nunmai_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('nunmai_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('nunmai_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('nunmai_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('nunmai_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('nunmai_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('nunmai_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('nunmai_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('nunmai_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('nunmai_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('nunmai_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('nunmai_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('nunmai_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('nunmai_cli.config', 'load_env'),
    'mcp_oauth_callback': ('nunmai_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('nunmai_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('nunmai_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('nunmai_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('nunmai_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('nunmai_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('nunmai_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('nunmai_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('nunmai_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('nunmai_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('nunmai_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('nunmai_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('nunmai_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('nunmai_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('nunmai_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('nunmai_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('nunmai_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('nunmai_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('nunmai_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('nunmai_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('nunmai_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('nunmai_cli.config', 'remove_env_value'),
    'remove_env_var': ('nunmai_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('nunmai_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('nunmai_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('nunmai_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('nunmai_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('nunmai_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('nunmai_cli.web_routers.ops', 'reset_memory'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('nunmai_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('nunmai_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('nunmai_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('nunmai_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('nunmai_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('nunmai_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('nunmai_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('nunmai_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('nunmai_cli.doctor', 'run_doctor'),
    'run_dump': ('nunmai_cli.dump', 'run_dump'),
    'run_import': ('nunmai_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('nunmai_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('nunmai_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('nunmai_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('nunmai_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('nunmai_cli.config', 'save_config'),
    'save_env_value': ('nunmai_cli.config', 'save_env_value'),
    'save_toolset_env': ('nunmai_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('nunmai_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('nunmai_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('nunmai_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('nunmai_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('nunmai_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('nunmai_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('nunmai_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('nunmai_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('nunmai_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('nunmai_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('nunmai_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('nunmai_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('nunmai_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('nunmai_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('nunmai_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('nunmai_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('nunmai_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('nunmai_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('nunmai_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('nunmai_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('nunmai_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('nunmai_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('nunmai_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('nunmai_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('nunmai_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('nunmai_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('nunmai_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('nunmai_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('nunmai_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('nunmai_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('nunmai_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('nunmai_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('nunmai_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('nunmai_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('nunmai_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('nunmai_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('nunmai_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('nunmai_cli.web_routers.cron', 'update_cron_job'),
    'update_nunmai': ('nunmai_cli.web_routers.actions', 'update_nunmai'),
    'update_learning_node': ('nunmai_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('nunmai_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('nunmai_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('nunmai_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('nunmai_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('nunmai_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('nunmai_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('nunmai_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('nunmai_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('nunmai_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('nunmai_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('nunmai_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('nunmai_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('nunmai_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('nunmai_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('nunmai_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('nunmai_cli.config', 'write_platform_config_field'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from nunmai_cli.plugin_compat import warn_once
