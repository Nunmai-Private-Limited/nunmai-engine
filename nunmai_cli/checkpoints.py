"""`nunmai checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nunmai/checkpoints/``.  Actions:

    nunmai checkpoints               # same as `status`
    nunmai checkpoints status        # total size, project count, breakdown
    nunmai checkpoints list          # per-project checkpoint counts + workdir
    nunmai checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nunmai checkpoints clear [-f]    # nuke the entire base (asks first)
    nunmai checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nunmai checkpoints
    nunmai checkpoints prune --retention-days 3 --max-size-mb 200
    nunmai checkpoints clear -f
...
from nunmai_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nunmai checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nunmai checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nunmai checkpoints` → status
