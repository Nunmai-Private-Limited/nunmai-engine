...
Split out of ``nunmai_cli.models``. Catalog fetchers defined in ``nunmai_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("nunmai_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``nunmai_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `nunmai moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from nunmai_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from nunmai_cli import models as _m
    from nunmai_cli import models_local as _ml
    from nunmai_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Nunmai picker convention, so an unknown `*-900k` can never be a real
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # Nunmai: the Portal's recommended-models feed can list a model before the curated list or the
    # docs-hosted manifest catches up; `nunmai chat` already accepts those at model-list build
    ...
    if req.normalized == "nous" and req.lookup.lower() in _nous_portal_recommended_names():
        return _accept_with_note(f"Note: `{req.requested}` was not found in the live /v1/models listing "
                                 "but is a current Nunmai Portal recommendation — accepted.")
