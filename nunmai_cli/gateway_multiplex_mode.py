...
``nunmai gateway migrate --multiplex`` runs and multiplexes only when the fold would have been
...
default flipped, and the log names the blocker plus ``nunmai gateway migrate --multiplex``.
"""
...
#: One-time marker the gateway leaves after rewriting a retired ``false``; ``nunmai update``'s summary
...
def explicit_multiplex_flag(default_home: Path) -> Optional[bool]:
    ...
    other processes (``nunmai -p X ...`` has X's config loaded) asking about the default's file."""
...
def default_gateway_multiplexes(default_home: Optional[Path] = None) -> bool:
    ...
    root = Path(default_home) if default_home is not None else get_default_nunmai_root()
...
def implicit_multiplex_blocker() -> Optional[str]:
    ...
    Mirrors what makes ``nunmai gateway migrate --multiplex`` refuse or leave a per-profile gateway
    ...
    from nunmai_cli.gateway_migrate import MIGRATE_COMMAND, _host_supports_migration, build_migration_plan
...
def persist_resolved_default(decision: MultiplexDecision, default_home: Optional[Path] = None) -> bool:
    ...
    try:
        ...
        if in_file is True:
            return False
        from nunmai_cli.gateway_migrate import _write_multiplex_flag
...
def consume_rewritten_notice(default_home: Optional[Path] = None) -> list[str]:
    """``nunmai update``'s summary: print the rewrite notice ONCE more, then clear the marker."""
    default_home = Path(default_home) if default_home is not None else _default_profile_home()
...
def resolve_multiplex_mode(config) -> MultiplexDecision:
    ...
    says why, and it converges by itself once ``nunmai gateway migrate --multiplex`` has run.
    """
...
def record_multiplex_decision(decision: MultiplexDecision) -> None:
    """Persist a guard refusal into ``gateway_state.json`` so `nunmai gateway status` can show why this
...
def unserved_profiles() -> list[str]:
    ...
    from nunmai_constants import get_nunmai_home, profile_name_for_home
    from nunmai_cli.profiles import profiles_to_serve
    me = profile_name_for_home(get_nunmai_home()) or "default"
...
def standalone_warning_lines(decision: MultiplexDecision, unserved: Optional[list[str]] = None) -> list[str]:
    ...
    gateway start, in the ``nunmai update`` summary and (as text) in the dashboard banner.
    """
...
def recorded_standalone_warning_lines() -> list[str]:
    ...
    make the decision (``nunmai update``'s summary, ``nunmai gateway status``)."""
    try:
