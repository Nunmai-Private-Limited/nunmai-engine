"""Shared helpers for attaching Nunmai to a local Chromium-family CDP port.
...
# the real profile into ``~/.nunmai/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the nunmai-owned snapshot dir for ``browser``'s real profile."""
    return str(get_nunmai_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, NUNMAI_UID/GID) as every Nunmai secret dir — via ``_secure_dir``/
    ...
    ``NUNMAI_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from nunmai_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
def _unavailable_auth_dbs_error(browser: str, failed: dict[str, str]) -> str:
    ...
    if all(reason == _AUTH_DB_LOCKED for reason in failed.values()):
        return (f"{browser} is running and holds the profile's {names} with a write lock, so their "
                "SQLite backup made no progress within five seconds. Nunmai does not fall back to a "
...
_SNAPSHOT_DONE_MARKER = ".nunmai-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Nunmai can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Nunmai "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the nunmai copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_nunmai_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_nunmai_home() / "chrome-debug")
