...
Mixin split out of ``cli.py``; bound onto ``NunmaiCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``NunmaiCLI.<name>(self, ...)``.
"""
...
def stored_session_route(session_meta, *, current_model, current_provider):
    ...
    if not stored_model:
        return None
    from nunmai_state import SessionDB as _SessionDB
    ...
    # pick the wire per model (OpenCode Zen/Go, Copilot, Nunmai) re-derive both from the stored model, or a
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from nunmai_cli.model_switch import format_model_for_display, resolve_display_context_length
    ...
    # Provider-aware context chain: Codex OAuth / Copilot / Nunmai caps win over the raw
...
def _commit_model_switch(
    ...
    from cli import NunmaiCLI, _cprint
    old_model = cli.model
    ...
    if persist_global:
        from nunmai_cli.model_switch import persist_model_selection
    ...
    if not one_turn:
        NunmaiCLI._persist_model_switch_to_session(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from nunmai_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from nunmai_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if opencode_provider_family(resolved_provider) is not None:
            from nunmai_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
        ...
        if self._model_is_default:
            from nunmai_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from nunmai_cli.local_runtime.endpoint import LLAMACPP_ALIASES
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current Nunmai session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `nunmai model` / gateway pickers); live catalog only when
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from nunmai_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Nunmai default (chat_completions)
