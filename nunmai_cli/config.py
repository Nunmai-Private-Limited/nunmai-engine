...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (nunmai config edit) and retry.")}
...
def _chown_to_nunmai_uid(path) -> None:
    ...
    :func:`ensure_nunmai_home` on Docker deployments. See #34107.
    """
...
def _secure_dir(path):
    ...
    NUNMAI_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse NUNMAI_HOME to
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("NUNMAI_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or NUNMAI_CONTAINER / NUNMAI_SKIP_CHMOD opt-out).
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_nunmai_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_nunmai_dotenv() pins unexpanded literals
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nunmai writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.nunmai/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``NUNMAI_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("NUNMAI_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`nunmai cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `nunmai config set cron.model <model>`.")
...
_USAGE_GET = ("Usage: nunmai config get <key> [--json]", [
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``nunmai config`` UI without the core knowing they exist.
