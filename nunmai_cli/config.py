"""Configuration management for Nunmai Engine: config.yaml / .env loading, saving,
validation, migration, and the ``nunmai config`` command."""
...
# dies with ImportError before any later heal point is reached. See nunmai_cli.stale_modules.
...
# interpreter init (PYTHON*, NODE_*: Nunmai restarts through them), PATH (fix tool lookup
...
# and Nunmai runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``NUNMAI_*`` overall is NOT blocked — many integration credentials use that prefix
# (NUNMAI_LANGFUSE_PUBLIC_KEY, NUNMAI_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if policy_name in _ENV_VAR_NAME_DENYLIST or policy_name.startswith(_ENV_VAR_NAME_DENY_PREFIXES):
        raise ValueError(
            ...
            "or Nunmai runtime location and security policy (NUNMAI_HOME, NUNMAI_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.nunmai/.env "
            "directly.")
...
# acquires it again (nunmai_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean NUNMAI_TOOL_PROGRESS variant is unsupported (its only
    ...
    "NUNMAI_LANGFUSE_ENV", "NUNMAI_LANGFUSE_RELEASE", "NUNMAI_LANGFUSE_SAMPLE_RATE",
    "NUNMAI_LANGFUSE_MAX_CHARS", "NUNMAI_LANGFUSE_CAPTURE", "NUNMAI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "NUNMAI_ACP_AUTH_METHOD", "NUNMAI_ACP_AUTO_APPROVE", "NUNMAI_COPILOT_ACP_COMMAND",
    "NUNMAI_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# NUNMAI_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nunmai was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$NUNMAI_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because NUNMAI_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``nunmai update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``ghcr.io/nunmai-private-limited/nunmai-engine`` image bakes a ``docker`` stamp into
    ``/opt/nunmai`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of nunmai_cli/), NOT of $NUNMAI_HOME,
    ...
    method = _install_method_stamp(get_nunmai_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``nunmai_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull ghcr.io/nunmai-private-limited/nunmai-engine:latest",
    "apt": "pkg upgrade nunmai-engine",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "nunmai update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (nunmai_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Nunmai.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``nunmai update`` doesn't apply inside the Docker container.

Nunmai Engine runs as a published image (ghcr.io/nunmai-private-limited/nunmai-engine), not a
...
Verify the new version after restart:
  docker run --rm ghcr.io/nunmai-private-limited/nunmai-engine:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://github.com/Nunmai-Private-Limited/nunmai-engine/pkgs/container/nunmai-engine
...
def get_container_exec_info() -> Optional[dict]:
    ...
    already inside the container, or when NUNMAI_DEV=1 is set. Only FileNotFoundError is
    ...
    try:
        info = {}
        with open(get_nunmai_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
# ---- Config paths / NUNMAI_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / NUNMAI_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("NUNMAI_IGNORE_USER_CONFIG") == "1":
        return
    ...
    from nunmai_cli.config_backups import backup_config
    backup_path = backup_config(config_path, "corrupt")
    where = _yaml_error_location(parse_error)
    message = (
        f"Nunmai stopped because your settings file ({config_path}) has a formatting error"
        f"{f' at {where}' if where else ''}. Fix it with `nunmai config edit` and check with "
        "`nunmai config check`, or add --ignore-user-config to run once with default settings.")
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply NUNMAI_UID/GID ownership. No-op when managed;
    in a container only an explicit NUNMAI_HOME_MODE is applied. NUNMAI_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse NUNMAI_HOME to a served subdirectory without
    directory listings.

    Also applies ``NUNMAI_UID``/``NUNMAI_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    Delegates to the canonical import-safe primitive ``nunmai_constants.apply_secure_dir_policy``
...
_NUNMAI_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_nunmai_home():
    """Ensure the ~/.nunmai directory skeleton exists with secure permissions.
    ...
    if key in _NUNMAI_HOME_ENSURED and home.is_dir():
        return
    from nunmai_cli.config_home import initialize_home
    initialize_home(home, _NUNMAI_HOME_SUBDIRS, _NUNMAI_HOME_ENSURED)


# ---- Config loading/saving ----

from nunmai_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from nunmai_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use nunmai_cli.config.<name>)
...
# Back-compat re-exports — :mod:`nunmai_cli.personality` owns personality/overlay semantics.
from nunmai_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``nunmai config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `nunmai config set` routes this credential-shaped key to .env through the
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `nunmai update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "known_plugin_toolsets", # nunmai_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``nunmai_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. NUNMAI_TIMEZONE overrides "
            "this key when set.")
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'nunmai tools' and pick a different Web Search & Extract provider")
...
def _validate_quoted_containers(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    for key, kind in _container_slots().items():
        ...
        if isinstance(parsed, (list, dict)):
            _issue(issues, "warning",
                   f"{key} is the quoted string {value!r} — Nunmai expects a YAML {kind} here "
                   "and every reader ignores the string",
                   f"Run: nunmai config set {key} {shlex.quote(value)}  (stores a real {kind}), "
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from nunmai_cli.config_home import config_load_issue
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from nunmai_cli.toolset_scope import toolset_allowed_for_platform
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_nunmai_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``nunmai config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``nunmai status``/``dump``) read ``name`` and
...
# the NUNMAI_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Nunmai host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `nunmai config get`): nothing loaded yet in this process, so
        ...
        from nunmai_cli.config_backups import load_newest_good_backup
...
def _load_config_cache_hit(path_key: str, cache_sig: Any) -> Optional[Dict[str, Any]]:
    ...
    ``None``. Signatures matching is not enough: a load before load_nunmai_dotenv() would otherwise
...
def load_env() -> Dict[str, str]:
    """Load ~/.nunmai/.env as a dict. Memoised inside ``load_env_file`` (``get_env_value()`` runs
...
def sanitize_env_file() -> int:
    """Rewrite ~/.nunmai/.env with normalized line formatting; returns the number of changed lines."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nunmai/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``NUNMAI_CUSTOM_`` prefix keeps the
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nunmai/.env and os.environ; True if it was found and removed."""
...
def reload_env() -> int:
    """Re-read ~/.nunmai/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Nunmai (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.nunmai/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Nunmai-managed credential preferring ``~/.nunmai/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale NUNMAI_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("NUNMAI_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale NUNMAI_MAX_ITERATIONS={env_ghost} "
                    f"(run 'nunmai doctor --fix' to remove)", Colors.YELLOW))
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``nunmai config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``nunmai config set platforms.telegram.streaming false`` landed on a key
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Nunmai may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from nunmai_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (NUNMAI_YOLO_MODE, PATH, ...)
    ...
    if key == "model.provider" and _old_provider.lower() != str(value).strip().lower():
        from nunmai_cli.route_identity import drop_stale_model_route
        ...
        if _popped:
            ...
            _route_notice = color(
                ...
                "`nunmai config unset model.base_url` (and model.api_mode).", Colors.YELLOW)
    # api_base -> base_url alias at set-time too (mirrors _normalize_root_model_keys).
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `nunmai config
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from nunmai_cli.config_env_routing import is_env_setting_key, read_env_setting
    ...
    if _split_key_path(key)[0] in _known_top_level_keys():
        ...
        if not is_known:
            print(color(
                f"⚠ '{key}' is not a recognized config key — Nunmai may not read it; the value "
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        return
    from nunmai_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
_USAGE_GET = ("Usage: nunmai config get <key> [--json] [--raw]", [
    "nunmai config get model", "nunmai config get terminal.backend",
    "nunmai config get skills.config --json"], None)
_USAGE_SET = ("Usage: nunmai config set [--force] <key> <value>", [
    "nunmai config set model anthropic/claude-sonnet-4", "nunmai config set terminal.backend docker",
    "nunmai config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: nunmai config unset <key>", [
    "nunmai config unset model", "nunmai config unset terminal.backend",
    "nunmai config unset OPENROUTER_API_KEY"], None)
...
_CONFIG_USAGE = """Available commands:
  ...
  nunmai config get <key>          Print a resolved config value
  nunmai config set <key> <value>   Set a config value
  nunmai config unset <key>        Remove a config value
  nunmai config check     Check for missing/outdated config
  nunmai config migrate   Update config with new options
  nunmai config path      Show config file path
  nunmai config env-path  Show .env file path"""
...
def _platform_plugin_manifests():
    ...
    ``plugins/platforms/*``, the user's ``<NUNMAI_HOME>/plugins/platforms/*`` category dir, and flat
    user installs ``<NUNMAI_HOME>/plugins/*`` that declare ``kind: platform`` (#46600)."""
    user_plugins = get_nunmai_home() / "plugins"
    roots = (
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Teams / IRC / Google Chat and third-party platforms are configurable in the ``nunmai config`` /
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('nunmai_cli.route_identity', 'normalize_route_base_url'),
}
