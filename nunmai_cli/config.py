"""
Configuration management for Nunmai Engine.

Config files are stored in ~/.nunmai/ for easy access:
- ~/.nunmai/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nunmai/.env         - API keys and secrets

This module provides:
- nunmai config          - Show current configuration
- nunmai config edit     - Open config in editor
- nunmai config get      - Print a resolved configuration value
- nunmai config set      - Set a specific value
- nunmai config unset    - Remove a user configuration value
- nunmai config wizard   - Re-run setup wizard
"""
...
from nunmai_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nunmai config set`` (which rewrites
    ...
    ``config.yaml`` in place: nunmai never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.nunmai/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nunmai logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (nunmai config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nunmai config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nunmai makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nunmai itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nunmai update``, the TUI build.
...
#   on every plugin install / ``nunmai update``.
...
# * ``NUNMAI_HOME`` / ``NUNMAI_PROFILE`` / ``NUNMAI_CONFIG`` /
#   ``NUNMAI_ENV`` — Nunmai runtime location flags. Writing these into
...
#
# IMPORTANT: ``NUNMAI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NUNMAI_LANGFUSE_PUBLIC_KEY,
# NUNMAI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nunmai runtime location — never via dashboard env writer.
    # NOT a NUNMAI_* blanket: integration credentials (NUNMAI_GEMINI_*,
    # NUNMAI_LANGFUSE_*, NUNMAI_SPOTIFY_*, ...) ARE allowed.
    "NUNMAI_HOME", "NUNMAI_PROFILE", "NUNMAI_CONFIG", "NUNMAI_ENV",
    "NUNMAI_CONFIG_PATH", "NUNMAI_ENV_PATH",
    ...
    "NUNMAI_COPILOT_ACP_COMMAND", "NUNMAI_COPILOT_ACP_ARGS",
    # Nunmai security policy / approval-routing context. These remain available
    ...
    "NUNMAI_YOLO_MODE", "NUNMAI_ACCEPT_HOOKS", "NUNMAI_REDACT_SECRETS",
    "NUNMAI_INTERACTIVE", "NUNMAI_EXEC_ASK", "NUNMAI_GATEWAY_SESSION",
    "NUNMAI_CRON_SESSION", "NUNMAI_SINGLE_QUERY_SESSION",
    "NUNMAI_SESSION_KEY", "NUNMAI_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nunmai runtime location "
            "and security policy (NUNMAI_HOME, NUNMAI_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # NUNMAI_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `nunmai plugins enable
    # observability/langfuse` or `nunmai tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NUNMAI_LANGFUSE_ENV",
    "NUNMAI_LANGFUSE_RELEASE",
    "NUNMAI_LANGFUSE_SAMPLE_RATE",
    "NUNMAI_LANGFUSE_MAX_CHARS",
    "NUNMAI_LANGFUSE_CAPTURE",
    "NUNMAI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "NUNMAI_ACP_AUTH_METHOD",
    "NUNMAI_ACP_AUTO_APPROVE",
    "NUNMAI_COPILOT_ACP_COMMAND",
    "NUNMAI_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from nunmai_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set NUNMAI_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NUNMAI_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_nunmai_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # NUNMAI_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Nunmai is running in package-manager-managed mode.

    Two signals: the NUNMAI_MANAGED env var (set by the systemd service),
    or a .managed marker file in NUNMAI_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Nunmai through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``nunmai_cli/`` — i.e. the git checkout for source
    installs, ``/opt/nunmai`` inside the published image. It is a property of
    the running interpreter, NOT of ``$NUNMAI_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nunmai was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$NUNMAI_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NUNMAI_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.nunmai``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NUNMAI_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.nunmai:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NUNMAI_HOME``, a
    ...
    ``docker`` and ``nunmai update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/hermes-agent`` image bakes a ``docker``
        stamp into ``/opt/nunmai`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NUNMAI_HOME.
    try:
    ...
    #    host install whose shared $NUNMAI_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``nunmai update``.
    try:
        method = (
            (get_nunmai_home() / ".install_method")
    ...
    # detect Nix installs that don't set NUNMAI_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``nunmai_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NUNMAI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/nunmai`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "apt":
        ...
        return "pkg upgrade nunmai-engine"
    return "nunmai update"
...
# Long-form text for ``nunmai update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nunmai_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nunmai, it doesn't update
...
✗ ``nunmai update`` doesn't apply inside the Docker container.

Nunmai Engine runs as a published image (nousresearch/hermes-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/hermes-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nunmai-engine
...
Notes:
  ...
  • Your config and session history live under ``$NUNMAI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nunmai update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nunmai installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Nunmai installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nunmai."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NUNMAI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nunmai_bin
    or None if container mode is not active, we're already inside the
    container, or NUNMAI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NUNMAI_DEV") == "1":
        return None
    ...
    container_mode_file = get_nunmai_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nunmai-engine")
    exec_user = info.get("exec_user", "nunmai")
    nunmai_bin = info.get("nunmai_bin", "/data/current-package/bin/nunmai")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "nunmai_bin": nunmai_bin,
    }
...
# Re-export from nunmai_constants — canonical definition lives there.
from nunmai_constants import get_nunmai_home, get_process_nunmai_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nunmai_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_nunmai_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_nunmai_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NUNMAI_UID / NUNMAI_GID env vars set by Docker deployments.

    Docker containers running Nunmai commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NUNMAI_HOME once, but
    subdirectories created at runtime by ``ensure_nunmai_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NUNMAI_UID", "").strip()
    gid_str = os.environ.get("NUNMAI_GID", "").strip()
    try:
...
def _chown_to_nunmai_uid(path) -> None:
    """Chown ``path`` to ``NUNMAI_UID:NUNMAI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nunmai_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_nunmai_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nunmai group can
    share state with the gateway service.

    The mode can be overridden via the NUNMAI_HOME_MODE environment variable
    (e.g. NUNMAI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NUNMAI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NUNMAI_UID``/``NUNMAI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NUNMAI_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nunmai_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nunmai runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NUNMAI_CONTAINER") or os.environ.get("NUNMAI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NUNMAI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NUNMAI_HOME, upgrading legacy empty templates.
...
# ensure_nunmai_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_NUNMAI_HOME_ENSURED: set = set()


def ensure_nunmai_home():
    """Ensure ~/.nunmai directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``NUNMAI_HOME`` we only re-run
    ...
    ``get_nunmai_home()`` and therefore re-run for the new path.
    """
    home = get_nunmai_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``nunmai profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from nunmai_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _NUNMAI_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_nunmai_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _NUNMAI_HOME_ENSURED.add(key)


def _ensure_nunmai_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NUNMAI_HOME {home} does not exist."
        )
...
from nunmai_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Nunmai Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `nunmai config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nunmai.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nunmai update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Nunmai' own config writer has historically emitted it
    ...
    # Nunmai (``_save_discovered_models_to_config``), not hand-curated.
    # Older Nunmai versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nunmai versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nunmai_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (nunmai_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (nunmai_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (nunmai_cli/setup.py)
    "known_plugin_toolsets", # written/read by nunmai_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (nunmai_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nunmai setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nunmai won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, nunmai send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nunmai doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "nunmai update / nunmai -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from nunmai_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ nunmai config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in nunmai_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nunmai_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from nunmai_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nunmai config set <key> <value>")
    ...
    # `nunmai update`, but only the version bump is persisted.
    ...
    # metadata.nunmai.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.nunmai/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_nunmai_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``NunmaiCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``nunmai config set``
    ...
    backends) — while display paths (``nunmai status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the NUNMAI_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nunmai_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in nunmai_cli.personality.
from nunmai_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`nunmai_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from nunmai_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from nunmai_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``NUNMAI_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`nunmai_cli.personality` (single owner).
    """
    from nunmai_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nunmai/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_nunmai_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nunmai/config.yaml.
    ...
    (which change ``NUNMAI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Nunmai host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/nunmai/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_nunmai_dotenv()
...
#   openai-codex (OAuth — nunmai auth) — OpenAI Codex
#   nous         (OAuth — nunmai auth) — Nunmai Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.nunmai/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nunmai writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nunmai/.env.
    ...
    interactive menu render (`nunmai tools`, `nunmai setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nunmai tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nunmai/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nunmai/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``NUNMAI_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"NUNMAI_CUSTOM_{slug}_API_KEY" if slug else "NUNMAI_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nunmai/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nunmai/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nunmai — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nunmai vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.nunmai/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.nunmai/.env`` over ``os.environ``.

    Used for Nunmai-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ◆ Nunmai Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from nunmai_cli.auth import get_anthropic_key
    ...
    # Warn on stale NUNMAI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NUNMAI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NUNMAI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'nunmai doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from nunmai_cli.personality import active_personality_name
    ...
    print(color("  nunmai config edit     # Edit config file", Colors.DIM))
    print(color("  nunmai config set <key> <value>", Colors.DIM))
    print(color("  nunmai setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``NUNMAI_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("NUNMAI_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `nunmai cron list`, then pin the "
        "intended values with `nunmai cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (nunmai_cli/plugins_cmd.py, nunmai_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `nunmai config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``nunmai config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``nunmai config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``nunmai config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   nunmai config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   nunmai config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``nunmai config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``nunmai config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    nunmai config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    nunmai config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `nunmai config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_nunmai_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `nunmai config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `nunmai config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: nunmai config get <key> [--json]")
            print()
            print("Examples:")
            print("  nunmai config get model")
            print("  nunmai config get terminal.backend")
            print("  nunmai config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: nunmai config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  nunmai config set model anthropic/claude-sonnet-4")
            print("  nunmai config set terminal.backend docker")
            print("  nunmai config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: nunmai config unset <key>")
            print()
            print("Examples:")
            print("  nunmai config unset model")
            print("  nunmai config unset terminal.backend")
            print("  nunmai config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'nunmai config migrate' to add them")
        ...
        print("  nunmai config           Show current configuration")
        print("  nunmai config edit      Open config in editor")
        print("  nunmai config get <key>          Print a resolved config value")
        print("  nunmai config set <key> <value>   Set a config value")
        print("  nunmai config unset <key>        Remove a config value")
        print("  nunmai config check     Check for missing/outdated config")
        print("  nunmai config migrate   Update config with new options")
        print("  nunmai config path      Show config file path")
        print("  nunmai config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nunmai config`` UI so users
