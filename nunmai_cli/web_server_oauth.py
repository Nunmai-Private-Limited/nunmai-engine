...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: Nunmai-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_nunmai_oauth_credentials, _get_nunmai_oauth_file
        nunmai_creds = read_nunmai_oauth_credentials()
    except Exception:
        nunmai_creds = None
    if nunmai_creds and nunmai_creds.get("accessToken"):
        return _token_status("nunmai_pkce", f"Nunmai PKCE ({_get_nunmai_oauth_file()})", nunmai_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Nunmai can't read, so the unverified
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``NUNMAI_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
        from nunmai_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "nous", "name": "Nunmai Portal", "flow": "device_code", "cli_command": "nunmai auth add nous",
     "docs_url": "https://portal.nousresearch.com", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "nunmai auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "nunmai auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "nunmai auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     ...
     "docs_url": "https://nunmai.in/docs/guides/xai-grok-oauth", "status_fn": None},
    ...
    # Login works via the terminal (`nunmai auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "nunmai auth add anthropic",
...
def _nous_plain_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """Background poller for a plain Nunmai device-code login (no free-tier identity to transfer).

    A sign-in that carries the free tier's connectors runs through ``anon_auth.run_sign_in`` and
    ``_nous_promotion_poller`` instead; this is the "connect another Nunmai account" path.
    """
    from nunmai_cli.web_server_profiles import _profile_scope
    from nunmai_cli.auth import _poll_for_token, persist_nous_credentials, refresh_nous_oauth_from_state
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller: PKCE-style ``code_verifier`` + ``user_code`` instead of Nunmai's
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``nunmai auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `nunmai auth add xai-oauth`: first credential may become active; never overwrite.
