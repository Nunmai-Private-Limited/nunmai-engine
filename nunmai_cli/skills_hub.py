...
Powers both:
  - `nunmai skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `nunmai skills install`.
...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``nunmai skills inspect owner/repo/skills/foo`` showed
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.nunmai/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.nunmai/skills/<name>/)[/]"
        )
    from nunmai_cli.cli_output import line_input
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `nunmai skills install` fail.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "nunmai-index".
    ...
    # The identifier is what you pass to `nunmai skills install`. Browse used
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_nunmai_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.nunmai.blueprint block, it is a runnable automation. Register it as
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "nunmai-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"nunmai-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying nunmai-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``nunmai -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``NUNMAI_HOME`` at process
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``nunmai update`` (ported from
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nunmai skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nunmai skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_nunmai_home()}/.env or run 'gh auth login'.\n")
            return
