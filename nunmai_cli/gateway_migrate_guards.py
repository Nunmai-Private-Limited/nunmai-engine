"""Boundaries the AUTOMATIC multiplex migration (``nunmai update``) must not cross, and the opt-out.

The multiplexer replaces a kernel-enforced boundary (separate UNIX users, separate service domains,
separate NUNMAI_HOME trees) with in-process isolation. An operator may choose that with
``nunmai gateway migrate --multiplex``; an unattended update hook must not choose it for them.
...
if TYPE_CHECKING:
    from nunmai_cli.gateway_migrate import MigrationPlan, ProfileGateway
...
def _system_unit_uid(unit_path: Path) -> Optional[int]:
    ...
    from nunmai_cli.gateway import _read_systemd_user_from_unit
...
def gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    ...
    NUNMAI_HOME an installed unit pins, which is where the gateway really runs; ``home`` otherwise.
    """
    from nunmai_cli.gateway import _nunmai_home_pinned_by_unit, get_systemd_unit_path
    from nunmai_cli.gateway_migrate import _home_env
    ...
    for kind, system in services:
        ...
        pinned = _nunmai_home_pinned_by_unit(unit_path)
...
def _guard_service_domain(plan: MigrationPlan, profile: ProfileGateway) -> Optional[str]:
    ...
    if set(profile.services) == set(reference):
        return None
    from nunmai_cli.gateway_migrate import _service_label as _kind_label
...
def _guard_home_tree(plan: MigrationPlan, profile: ProfileGateway) -> Optional[str]:
    ...
    if runtime_home.is_relative_to(profiles_root):
        return None
    return (f"Profile '{profile.name}' runs with NUNMAI_HOME={runtime_home}, outside {profiles_root}: "
...
def auto_migration_blockers(plan: MigrationPlan) -> list[str]:
    ...
    domain, one profiles/ tree — the only shape ``nunmai update`` may fold on its own."""
    findings = [
        finding
