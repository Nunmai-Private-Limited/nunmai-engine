...
def _strip_nunmai_path_lines(content: str) -> str:
    """Drop the ``# Nunmai Engine`` marker (+ its PATH line) and any nunmai PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
...
def remove_path_from_shell_configs():
    ...
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            ...
            new_content = _strip_nunmai_path_lines(content)
...
def remove_wrapper_script():
    ...
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a nunmai_cli / nunmai-engine reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('nunmai_cli' in content or 'nunmai-engine' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("nunmai", "nunmai-acp", "nunmai-engine"))
...
def _remove_windows_gateway() -> bool:
    ...
    from nunmai_cli import gateway_windows as gw
...
# NUNMAI_HOME / NUNMAI_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\nunmai\git\{cmd,bin,usr\bin}, ...\nunmai\node), PortableGit + Node copies
...
def _nunmai_path_markers(nunmai_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Nunmai-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    subs = ("nunmai-engine", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
...
def remove_path_from_windows_registry(nunmai_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Nunmai-owned entries from User-scope PATH in the registry (see ``_nunmai_path_markers``)."""
    markers = tuple(m.lower() for m in _nunmai_path_markers(nunmai_home, include_managed_bin=include_managed_bin))
...
def remove_portable_tooling_windows(nunmai_home: Path) -> list[Path]:
    ...
    ``nunmai_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (nunmai_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``nunmai`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``nunmai -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current NUNMAI_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m nunmai_cli.main` (the `nunmai` wrapper may be half-gone).
    nunmai_invocation = [sys.executable, "-m", "nunmai_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nunmai_invocation + ["gateway", subcmd], capture_output=True, text=True,
...
def run_gui_uninstall(args):
    """``nunmai uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from nunmai_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    nunmai_home = get_nunmai_home()
    summary = gui_install_summary(nunmai_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ◆ Nunmai Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()
...
def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.nunmai/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, nunmai_home=nunmai_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie NUNMAI_HOMEs and systemd units behind.
    ...
    print()
    _print_box("│            ◆ Nunmai Engine Uninstaller                  │", Colors.MAGENTA)
    print()
    ...
    # services, alias wrappers, NUNMAI_HOME dirs) — otherwise they leave zombie services behind.
    ...
    _perform_uninstall(
        project_root=project_root, nunmai_home=nunmai_home, full_uninstall=full_uninstall,
...
def _print_uninstall_dry_run(*, project_root: Path, nunmai_home: Path, full_uninstall: bool) -> None:
    ...
    if not full_uninstall:
        ...
        profiles = _discover_named_profiles() if _is_default_nunmai_home(nunmai_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$NUNMAI_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: nunmai_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; nunmai\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_nunmai_home(nunmai_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(nunmai_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Nunmai-owned PATH entries in User environment"),
        (windows, "Removing NUNMAI_HOME / NUNMAI_GIT_BASH_PATH User env vars...",
         remove_nunmai_env_vars_windows, "Removed User env var: {}", "No Nunmai-set User env vars to remove"),
        (True, "Removing nunmai command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows nunmai launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows nunmai launchers found"),
        (True, "Removing Nunmai-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(nunmai_home), "Removed {}", "No Nunmai-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE NUNMAI_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nunmai_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(nunmai_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(nunmai_home), "Removed {}",
...
def main(argv=None) -> int:
    """``python -m nunmai_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``nunmai_constants`` + ``nunmai_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
