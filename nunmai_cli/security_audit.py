...
def _discover_plugins(nunmai_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nunmai/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, nunmai_home: Optional[Path] = None
) -> list[Component]:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(nunmai_home=home, **skips)
    ...
    try:
        findings = run_audit(nunmai_home=home, components=components)
