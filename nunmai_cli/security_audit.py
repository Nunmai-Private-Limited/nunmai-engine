"""On-demand supply-chain audit for Nunmai Engine installs.

Scans three surfaces a Nunmai user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Nunmai venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.nunmai/plugins``
...
def _discover_plugins(nunmai_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nunmai/plugins``.
...
def _discover_components(
    *,
    ...
    home = nunmai_home or Path(get_nunmai_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `nunmai security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, nunmai_home=home
    )
