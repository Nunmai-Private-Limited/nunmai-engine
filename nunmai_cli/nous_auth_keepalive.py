...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("NUNMAI_NOUS_TIMEOUT_SECONDS", "15"))
...
def _observed_lifetime_seconds() -> Optional[int]:
    """Server-issued lifetime (seconds) of the current Nunmai credentials; the shorter of the access
...
def _refresh_selected_pool_entry(*, min_key_ttl_seconds: int, min_access_ttl_seconds: Optional[int] = None) -> Optional[bool]:
    ...
    no usable entry; None = no Nunmai pool.
    """
    try:
...
def refresh_nous_auth_keepalive_once(
    *, min_key_ttl_seconds: int = NOUS_INVOKE_JWT_MIN_TTL_SECONDS,
    ...
    """Refresh Nunmai auth once if credentials are configured (pool entry first, then singleton state)."""
...
def start_nous_auth_keepalive(
    *, interval_seconds: Optional[int] = None,
    ...
    """Start the process-wide Nunmai auth keepalive thread (idempotent; None when disabled)."""
    ...
    try:
        if is_guest_state(get_provider_auth_state("nous")):
            logger.debug("Nunmai auth keepalive skipped: free tier has no refresh token")
            return None
    except Exception:
        pass
