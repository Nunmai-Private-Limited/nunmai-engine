...
``nunmai logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``nunmai logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    print(f"--- {display_nunmai_home()}/logs/{filename}{filter_desc} ({mode}) ---")
