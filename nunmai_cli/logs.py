"""``nunmai logs`` — view and filter Nunmai log files.
...
Usage examples::
    ...
    nunmai logs                    # last 50 lines of agent.log
    nunmai logs -f                 # follow agent.log in real time
    nunmai logs errors             # last 50 lines of errors.log
    nunmai logs gateway -n 100    # last 100 lines of gateway.log
    nunmai logs gui -f            # follow gui.log (dashboard/pty/ws)
    nunmai logs desktop -f        # follow desktop.log (Electron app boot/backend)
    nunmai logs --level WARNING    # only WARNING+ lines
    nunmai logs --session abc123   # filter by session ID substring
    nunmai logs --component tools  # only tool-related lines
    nunmai logs --since 1h         # lines from the last hour
    nunmai logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nunmai_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Nunmai runs — try 'nunmai chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nunmai_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nunmai_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nunmai_home()}/logs/")
        return

    print(f"Log files in {display_nunmai_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nunmai chat' to generate logs)")
