...
configured for "cli" in `nunmai tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (NUNMAI_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `nunmai chat`:
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to NUNMAI_INFERENCE_MODEL
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``nunmai -z`` / oneshot mode.

    Oneshot bypasses ``NunmaiCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when nunmai is invoked for
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("NUNMAI_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # NunmaiCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                NUNMAI_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via NUNMAI_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via NUNMAI_YOLO_MODE=1
