...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `nunmai tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(NUNMAI_YOLO_MODE=1). Model/provider mirror `nunmai chat`: both optional; only --model → auto-detect
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"nunmai -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in _ALL_TOOLSETS for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "nunmai -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"nunmai -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "nunmai -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )
    if not valid:
        return None, "nunmai -z: --toolsets did not contain any valid toolsets.\n"
    return valid, None
...
def run_oneshot(
    ...
    Model/provider fall back to ``NUNMAI_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    env_model_early = os.getenv("NUNMAI_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "nunmai -z: --provider requires --model (or NUNMAI_INFERENCE_MODEL). "
    ...
    # Same finite-chat marker as `nunmai chat -q` (cli.py): the session-source resolver uses it to drop an
    ...
    if failure is not None:
        ...
        real_stderr.write(f"nunmai -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if exit_code == 1:
        real_stderr.write("nunmai -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
    return exit_code


def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``NunmaiCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("NUNMAI_INFERENCE_MODEL", "").strip()
    ...
    from nunmai_cli import model_switch as _ms
    try:
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("NUNMAI_INFERENCE_PROVIDER", "").strip().lower() or "auto"
        # Same owner as NunmaiCLI startup: a provider-qualified string (``custom:<name>:<model>``,
...
def _load_resume_target(session_db, resume: Optional[str]) -> tuple[Optional[str], list, Optional[dict]]:
    ...
    nothing but is recorded under the requested session — ``nunmai -z "hello" -c <title>
...
def _apply_stored_session_runtime(
    ...
    if explicit_model:
        return choice
    from nunmai_cli.cli_model_switch_mixin import stored_session_route
...
def _run_agent(
    ...
    from nunmai_cli.runtime_provider import resolve_runtime_with_fallback
    ...
    from nunmai_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        agent = AIAgent(
            ...
            # NUNMAI_INTERACTIVE (never set), hook approval via NUNMAI_ACCEPT_HOOKS=1, dangerous
            # commands via NUNMAI_YOLO_MODE=1, skill secret capture degrades gracefully.
