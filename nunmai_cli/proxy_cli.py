"""CLI handlers for ``nunmai egress ...``.
...
The top-level command is ``nunmai egress``.  Note that the inbound OAuth
reverse-proxy command (``nunmai proxy``) lives elsewhere in
``nunmai_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from nunmai_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``nunmai_cli.main`` as part of building the top-level
    ``nunmai egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``nunmai proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`nunmai egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `nunmai secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.nunmai/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `nunmai egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.nunmai/.env for discovery.[/dim]"
            )
    ...
    # explicitly requested rotation.  This prevents re-running `nunmai
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Nunmai sandbox.  They will start "
    ...
    # `nunmai egress reload` apply future ruleset changes without a
    ...
    # rotation), running `nunmai egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]nunmai egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]nunmai egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]nunmai egress start[/cyan]\n"
        "  Restart: [cyan]nunmai egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]nunmai egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]nunmai egress status[/cyan]\n"
        "  Stop:    [cyan]nunmai egress stop[/cyan]\n"
        "  Disable: [cyan]nunmai egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `nunmai egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `nunmai egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`nunmai egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `nunmai secrets bitwarden setup` to configure the "
                "project, or switch back via `nunmai egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``nunmai egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `nunmai egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `nunmai egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `nunmai egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]nunmai egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `nunmai egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.nunmai/.env`` into ``os.environ``.

    ``nunmai egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.nunmai/.env`` (which the agent
