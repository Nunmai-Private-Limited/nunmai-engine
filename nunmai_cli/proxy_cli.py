...
from nunmai_cli.config import load_config, load_env, save_config


def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    """Attach the egress subcommand tree to a parent parser."""
    # dest='egress_command' keeps this tree disjoint from the inbound OAuth ``nunmai proxy``
...
def _setup_mint_tokens(console: Console, args: argparse.Namespace):
    ...
    if args.from_bitwarden:
        ...
        # Operators commonly keep provider keys only in ~/.nunmai/.env (loaded when the agent
        # runs, NOT exported into an interactive shell); backfill so discovery sees them.
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f"  [dim]Loaded {loaded} provider key name(s) from ~/.nunmai/.env for discovery.[/dim]")
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Nunmai sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _setup_restart_daemon(console: Console, args: argparse.Namespace, proxy_cfg: dict) -> None:
    ...
    if do_restart:
        try:
            ...
            console.print("  Run [cyan]nunmai egress start[/cyan] manually before launching new Docker sandboxes.")
        else:
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print("[yellow]proxy.enabled is false — run `nunmai egress setup` first.[/yellow]")
        return 1
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            return _refuse(
                ...
                "`nunmai egress setup --no-bitwarden` to switch back to env-based credentials.",
            )
        if not (bw_cfg or {}).get("project_id"):
            return _refuse(
                ...
                "project, or switch back via `nunmai egress setup --no-bitwarden`.",
            )
    try:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    New upstream SECRETS still need ``nunmai egress restart``: the daemon reads credentials from
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            "  iron-proxy is still running — stop it with [cyan]nunmai egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print("[yellow](no config generated — run `nunmai egress setup`)[/yellow]")
        return 1
...
def _bitwarden_env_names(console: Console) -> Optional[List[str]]:
    ...
    if not bw_cfg.get("enabled"):
        ...
        console.print("  Run `nunmai secrets bitwarden setup` first, or omit --from-bitwarden.")
        return None
    try:
...
def _load_env_file_into_environ() -> int:
    """Backfill known provider keys from ``~/.nunmai/.env`` into ``os.environ``; returns the count.
