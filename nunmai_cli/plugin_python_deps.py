"""Python dependencies declared by user plugins: read, resolve against Nunmai' own ranges, install,
and re-apply after ``nunmai update`` rebuilds the venv.
...
is resolved together with Nunmai' declared ranges; a candidate that has no solution is refused without
...
disabled with a loud warning, because a Nunmai that boots without memory reads as data loss.
...
def applicable_requirement(spec: str) -> Optional[str]:
    """``spec`` reduced to ``name[extras]specifier`` when Nunmai should install it here; ``None`` when
    its environment marker excludes this platform, when it names Nunmai itself (always satisfied by the
    running checkout — installing ``nunmai-engine`` from an index would clobber it), or when it points
...
def unsupported_specs(specs: Iterable[str]) -> list[str]:
    """Direct URL/path requirements: never installed by Nunmai (arbitrary git/tarball sources are a
...
def core_constraints(project_root: Path) -> list[str]:
    """Nunmai' own declared ranges (``[project].dependencies`` + every extra), as constraint lines.
...
def resolve(specs: list[str], constraints: list[str], *, dry_run: bool, timeout: int = 600):
    ...
    # A plugin's declared deps follow the plugin's own security policy; Nunmai's exclude-newer quarantine
    # covers Nunmai's packages only (core_constraints still keeps them in range).
...
class DepsOutcome:
    ...
    specs: tuple[str, ...] = ()
    detail: str = ""
    skipped: tuple[str, ...] = ()  # direct-URL requirements Nunmai never installs
...
def refuse_conflicting_candidate(plugin_dir: Path, *, home: Path) -> None:
    ...
    try:
        ...
        raise DependencyConflict(
            f"its Python dependencies conflict with Nunmai or an enabled plugin:\n{exc}") from exc
...
def install_for_plugin_dir(plugin_dir: Path) -> DepsOutcome:
    ...
    try:
        specs = install_declaration(decl, home=get_nunmai_home(), project_root=project_root())
...
def reapply_all(*, project_root: Path, disable: Callable[[Path, str], None]) -> ReapplyReport:
    ...
    for victim in dropped:
        ...
        reason = "its Python dependencies no longer resolve against this Nunmai"
