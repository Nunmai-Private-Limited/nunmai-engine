...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'nunmai doctor'") and last.startswith("Run 'nunmai setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `nunmai ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from nunmai_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from nunmai_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        main_handler = (
            (lambda: getattr(importlib.import_module("nunmai_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"nunmai_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"nunmai_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "nunmai_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "nunmai_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "nunmai_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "nunmai_cli.send_cmd", "register_send_subparser")


def _register_command_family(
    engine: "NunmaiConsoleEngine", root: str, surface: _CliSurface, paths: str) -> None:
    ...
    for child_path, mutating in _paths(paths):
        ...
        def handler(_engine: NunmaiConsoleEngine, args: list[str], fixed=child_path) -> str:
            return _dispatch(surface, root, fixed, args, namespace_update)

        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `nunmai {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `nunmai {usage}`?")
...
_BLOCKED_PAIRS = {
    ...
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Nunmai Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Nunmai Console.",
}
...
class NunmaiConsoleEngine:
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Nunmai Console", "", "Supported commands:"]
...
def _captured(fn):
    ...
    @functools.wraps(fn)
    def wrapper(engine: NunmaiConsoleEngine, args: list[str]) -> str:
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Nunmai Console.")
...
_version = _simple_command(
    "version", "nunmai_cli._startup_fast", "print_fast_version_info", check_updates=True)
...
_doctor = _simple_command(
    "doctor", "nunmai_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "nunmai_cli.config", "show_config")
_cron_status = _simple_command("cron status", "nunmai_cli.cron", "cron_status")
...
def _sessions_list(_engine: NunmaiConsoleEngine, args: list[str]) -> str:
    ...
    from nunmai_state_sessions import INTERNAL_LISTING_SOURCES
...
@_captured
def _config_migrate(_engine: NunmaiConsoleEngine, args: list[str]) -> None:
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from nunmai_state import SessionExportTooLargeError, resolved_max_export_messages
...
@_captured
def _sessions_export(_engine: NunmaiConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_rename(_engine: NunmaiConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_optimize(_engine: NunmaiConsoleEngine, args: list[str]) -> None:
    ...
    with _session_db(read_only=False) as db:
        from nunmai_state_holders import held_store_refusal
...
@_captured
def _sessions_repair(_engine: NunmaiConsoleEngine, args: list[str]) -> int | None:
    ns = _parse(
        "sessions repair", args, (("--check-only",), dict(action="store_true")),
        (("--no-backup",), dict(action="store_true")))
    from nunmai_state import DEFAULT_DB_PATH
    from nunmai_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _cron_pause(_engine: NunmaiConsoleEngine, args: list[str]) -> str:
    ...
    return _cron_job_action(
        ...
        lambda ref: pause_job(ref, reason="paused from nunmai console"))
...
def _cron_resume(_engine: NunmaiConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = (
            rearm_oneshot(ns.job, _nunmai_now().isoformat() if ns.run_now else ns.at)
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Nunmai component status.", _status, ""),
    (("version",), "version", "Show Nunmai version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Nunmai logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Nunmai configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    ...
    say("Nunmai Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("nunmai> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
