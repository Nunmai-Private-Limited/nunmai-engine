"""Directory initialization and storage diagnostics for the active Nunmai home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool, home: Path) -> None:
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Nunmai directory {path}: {exc}. "
            ...
            "Nunmai has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from nunmai_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Nunmai home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from nunmai_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Nunmai storage is unavailable: {exc}",
