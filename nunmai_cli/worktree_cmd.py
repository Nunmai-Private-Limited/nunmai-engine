"""``nunmai worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``nunmai_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    nunmai worktree list             # audit: verdict + reason per tree
    nunmai worktree prune            # reap safe trees + merged branches
    nunmai worktree prune --dry-run  # show the plan, change nothing
    nunmai worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `nunmai worktree prune`."
        )
