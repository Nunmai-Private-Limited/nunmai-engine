...
from nunmai_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from nunmai_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from nunmai_cli.profiles import _check_gateway_running, profiles_to_serve, profile_is_parked
    ...
    from nunmai_cli.gateway_multiplex_mode import SINGLE_PROFILE_REASON
...
# ~/.nunmai/logs/<action>.log which the dashboard tails.
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from nunmai_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``nunmai <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``nunmai -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_nunmai_dotenv``
    ...
    Named-profile actions therefore start from Nunmai' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``NUNMAI_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from nunmai_cli.web_server_profiles import _resolve_profile_dir
        from nunmai_constants import apply_subprocess_home_env, get_default_nunmai_root
        ...
        try:
            source_homes = {str(get_default_nunmai_root()), str(get_nunmai_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["NUNMAI_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["NUNMAI_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _NUNMAI_GATEWAY=1;
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``nunmai update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from nunmai_cli.update_cmd_fleet import _needs_sudo
    ...
    from nunmai_cli.gateway import _select_systemd_scope
    from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from NUNMAI_HOME, so a selector-bearing action must be resolved
    ...
    from nunmai_cli.web_server_profiles import _resolve_profile_dir
    token = set_nunmai_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_nunmai_action(
    ...
    """Spawn ``nunmai <subcommand>`` detached (via ``nunmai_cli.main``) and record the handle."""
    from nunmai_cli.web_server import PROJECT_ROOT
    ...
    if _action_targets_system_gateway(subcommand):
        ...
        # NUNMAI_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``nunmai update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from nunmai_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo nunmai {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _NUNMAI_GATEWAY).
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``nunmai --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_nunmai_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``nunmai [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``NUNMAI_HOME`` and hit the same exit-78 refusal."""
    from nunmai_cli.web_server_profiles import _profile_cli_args
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    ``nunmai -p X gateway <verb>`` runs ``gateway_profile_lifecycle``), so neither is refused;
    ...
    from nunmai_cli.profiles import profile_is_parked, profile_is_standalone
    from nunmai_cli.web_server_profiles import _resolve_profile_dir
    ...
    from nunmai_cli.gateway_migrate import _installed_services
    ...
    return (f"Profile '{requested}' does not get a gateway of its own: one host gateway serves every "
            f"profile. Install or start it from the default profile (nunmai gateway install), or fold an "
            f"existing per-profile fleet with `nunmai gateway migrate --multiplex`; "
            f"`nunmai -p {requested} gateway install --force` starts a separate one anyway.")
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from nunmai_cli.web_server import _spawn_gateway_restart
    try:
