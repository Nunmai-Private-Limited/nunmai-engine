...
def projects_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: nunmai project <action> [options]\nRun 'nunmai project --help' for the full list.", file=sys.stderr)
        return 0
...
def _cmd_list(args, conn):
    ...
    if not projs:
        return "No projects yet. Create one with `nunmai project create <name>`."
    for p in projs:
...
def _cmd_set_primary(args, conn, proj):
    if not pdb.set_primary(conn, proj.id, args.path):
        return _err(f"'{args.path}' is not a folder of {proj.slug}; add it first with `nunmai project add-folder`.")
