...
Backs ``nunmai kanban export|import``, the ``/boards/{slug}/export`` and
...
ARCHIVE_FORMAT = "nunmai-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "nunmai_version": str(nunmai_version),
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError("archive is not a Nunmai kanban board export (no manifest.json)")
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Nunmai kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Nunmai "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Nunmai and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "nunmai_version": manifest.get("nunmai_version"),
        },
        "counts": counts,
