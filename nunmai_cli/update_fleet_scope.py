"""Home scoping for ``nunmai update``'s fleet restart (#93349).

The restart phase enumerates ``nunmai-gateway*``/``nunmai-serve*`` units, ``ai.nunmai.gateway*``
LaunchAgents and every ``gateway run`` process on the host. Those are HOST-wide namespaces: a
second Nunmai install (another ``NUNMAI_HOME`` root under the same account, its own checkout and
venv) shares them, and the update used to restart that install's gateway too — including the
account's real ``nunmai-gateway.service`` when a scratch home ran ``nunmai update``.
...
process environment (``NUNMAI_HOME``/``HOME``, via ``_nunmai_home_for_pid``), the unit's declared
``Environment=`` or the plist's pinned ``NUNMAI_HOME`` — never from a unit label or an argv
...
def update_scope_homes() -> set[Path]:
    ...
    with suppress(Exception):
        ...
        if (home := _resolved(get_nunmai_home())) is not None:
            homes.add(home)
    with suppress(Exception):
...
def gateway_pid_in_update_scope(pid: int, scope: set[Path] | None = None) -> bool | None:
    ...
    from nunmai_cli.dashboard_procs import _nunmai_home_for_pid
    try:
        home = _nunmai_home_for_pid(pid)
    except Exception:
        return None
    if home is None:
        return None
...
def systemd_unit_nunmai_home(scope_cmd: list, svc_name: str) -> str | None:
    """Home the systemd unit *svc_name* runs on: its live MainPID's environment first, then the
    unit's declared ``Environment=NUNMAI_HOME``; for a user-scope unit that declares none, the
    user's own default home. ``None`` when nothing readable names a home."""
    from nunmai_cli.update_cmd_fleet import _systemctl, _unit_main_pid

    pid = _unit_main_pid(scope_cmd, svc_name)
    if pid > 0:
        with suppress(Exception):
            from nunmai_cli.dashboard_procs import _nunmai_home_for_pid
            if (home := _nunmai_home_for_pid(pid)) is not None:
                return home
    try:
    ...
    for token in tokens:
        ...
        if sep and key == "NUNMAI_HOME" and value.strip():
            return value.strip()
    if "--user" in scope_cmd:
        return str(Path.home() / ".nunmai")
    return None
...
def launchd_label_foreign_home(label: str, scope: set[Path] | None = None) -> str | None:
    """The NUNMAI_HOME a derived launchd label's installed plist pins when that home is NOT one of
    this update's — labels are account-global, so root B's default profile derives the same bare
    ``ai.nunmai.gateway`` root A installed. ``None`` = ours, or no/unreadable plist (the locate step
    ...
    with suppress(Exception):
        ...
        pinned = str(data["EnvironmentVariables"]["NUNMAI_HOME"])
...
def describe_skipped_runtime(kind: str, name: str, home: str | None) -> str:
    ...
    if home is None:
        return f"  ↷ {name}: {kind} whose Nunmai home could not be read — left alone (not restarted)"
    return f"  ↷ {name}: {kind} of another Nunmai home ({home}) — left alone (not restarted)"
