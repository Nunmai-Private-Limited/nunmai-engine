"""Configuration-file checks for nunmai doctor: .env, config.yaml validation, drift, deprecations.
Split out of ``nunmai_cli/doctor.py``."""
...
def _has_provider_env_config(content: str) -> bool:
    ...
    from nunmai_cli.doctor import _PROVIDER_ENV_HINTS
...
# TERMINAL_CWD do not false-positive). NUNMAI_TOOL_PROGRESS is silently ignored since the v12 config floor
# removed its only consumer; NUNMAI_TOOL_PROGRESS_MODE is still read by the gateway as a back-compat fallback.
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    ...
    from nunmai_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
    ...
    if not str(effective_env.get(RELAY_PLUGINS_CONFIG_ENV, "")).strip():
        findings += [(name, f"run `nunmai migrate relay` to generate relay-plugins.toml and set {RELAY_PLUGINS_CONFIG_ENV}; "
...
def managed_scope_check() -> None:
    """Report the active managed scope (resolved dir + pinned key counts); silent when none. A NUNMAI_MANAGED_DIR
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    """Managed scope plus ~/.nunmai/.env presence and provider credentials."""
    from nunmai_cli.doctor import NUNMAI_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
    ...
    if env_path.exists():
        ...
        if not check_bool(_has_provider_env_config(content), "API key or custom endpoint configured", f"No API key found in {_DHH}/.env"):
            f.issues.append("Run 'nunmai setup' to configure API keys")
        ...
        if should_fix:
            ...
            f.issues.append("Run 'nunmai setup' to create .env")
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from nunmai_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
        known = set(PROVIDER_REGISTRY.keys()) | {"openrouter", "custom", "auto", "moa"}
    with warn_on_error(""):
        from nunmai_cli.config import get_compatible_custom_providers
        from nunmai_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if provider and provider != "auto" and (catalog_provider is None or (known_providers and not (accept & valid_provider_ids))):
        ...
        _fail_and_issue(f"model.provider '{provider_raw}' is not a recognised provider", f"(known: {known_list})",
                        ...
                        f"Fix: run 'nunmai config set model.provider <valid_provider>'", issues)
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from nunmai_cli.doctor import _DHH
        with warn_on_error(""):
            if not _provider_has_credentials(runtime_provider):
                _fail_and_issue(f"model.provider '{runtime_provider}' is set but no API key is configured",
                                f"(add it to {_DHH}/.env or run 'nunmai setup')",
                                f"No credentials found for provider '{runtime_provider}'. Run 'nunmai setup' or set the provider's "
                                f"API key in {_DHH}/.env, or switch providers with 'nunmai config set model.provider <name>'", issues)
...
def _drift_config_version(f: Finding, should_fix: bool, config_path) -> None:
    ...
    if check_bool(current_ver >= latest_ver, f"Config version up to date (v{current_ver})", outdated):
        return
    if not should_fix:
        f.issues.append("Run 'nunmai doctor --fix' or 'nunmai setup' to migrate config")
        return
    try:
        ...
        f.issues.append("Run 'nunmai setup' to migrate config")
...
def _drift_stale_root_keys(f: Finding, should_fix: bool, config_path) -> None:
    ...
    if not should_fix:
        f.issues.append("Stale root-level provider/base_url in config.yaml — run 'nunmai doctor --fix'")
        return
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    """A stale NUNMAI_MAX_ITERATIONS in .env shadows agent.max_turns in config.yaml.

    The setup wizard used to dual-write the budget. The gateway bridge derives NUNMAI_MAX_ITERATIONS from
    ...
    """
    from nunmai_cli.doctor import _DHH
    # Detect stale NUNMAI_MAX_ITERATIONS ghost in .env shadowing agent.max_turns in config.yaml (issue
    # #17534). The setup wizard used to dual-write the iteration budget to both stores; users who later edit
    # only config.yaml are left with a .env ghost. The gateway bridge normally derives NUNMAI_MAX_ITERATIONS
    ...
    if cfg_max_turns is None or env_ghost is None or str(cfg_max_turns).strip() == str(env_ghost).strip():
        return
    check_warn(f"NUNMAI_MAX_ITERATIONS={env_ghost} in .env shadows agent.max_turns={cfg_max_turns} in config.yaml",
               "(stale ghost from an earlier `nunmai setup` run)")
    if not should_fix:
        f.issues.append("Stale NUNMAI_MAX_ITERATIONS in .env shadows config.yaml — run 'nunmai doctor --fix'")
    elif remove_env_value("NUNMAI_MAX_ITERATIONS"):
        check_ok(f"Removed stale NUNMAI_MAX_ITERATIONS from .env (config.yaml agent.max_turns={cfg_max_turns} is now authoritative)")
        f.fixed += 1
    else:
        check_warn("Could not remove NUNMAI_MAX_ITERATIONS from .env")
        f.manual_issues.append(f"Manually delete the NUNMAI_MAX_ITERATIONS line from {_DHH}/.env — config.yaml agent.max_turns is authoritative.")
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from nunmai_cli.config import load_env, read_user_config_raw
...
def _check_config_drift(should_fix: bool, f: Finding) -> None:
    """Config version, stale root keys, NUNMAI_MAX_ITERATIONS ghost, deprecations, structure.
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from nunmai_cli.config import load_config
    from nunmai_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from nunmai_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
    ...
    check_info("Details: nunmai plugins compat")
    f.manual_issues.append(
        f"Update {len(report)} plugin(s) still importing pre-decomposition paths (nunmai plugins compat) — "
