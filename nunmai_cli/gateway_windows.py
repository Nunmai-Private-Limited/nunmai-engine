...
_TASK_NAME_DEFAULT = "Nunmai_Gateway"
_TASK_DESCRIPTION = "Nunmai Engine Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("NUNMAI_GATEWAY_DETACHED", "1"), ("NUNMAI_SUPERVISED_CHILD", "1"))
...
def _nunmai_home() -> Path:
    from nunmai_cli.config import get_nunmai_home

    return Path(get_nunmai_home())


def nunmai_service_roots() -> tuple[str, ...]:
    """Directories a Nunmai-owned SCM service binary lives under: the checkout (its ``venv`` included),
    the running interpreter's ``Scripts`` dir (``nunmai.exe`` shim) and the ``gateway-service`` launcher dir."""
    project_root = Path(__file__).resolve().parent.parent
    return (str(project_root), str(Path(sys.executable).parent), str(_nunmai_home() / "gateway-service"))
...
def nunmai_owns_windows_service(name: str, binpath: str, nunmai_roots: tuple[str, ...]) -> bool:
    """Positive ownership of an SCM service: Nunmai-named (``nunmai*``) or its binary path starts under a
    Nunmai root. Pure so it is testable off-Windows. A Scheduled-Task-launched gateway descends from
    ...
    if normalized_name.startswith("nunmai"):
        return True
    candidate = _normalize_windows_path(binpath)
    return any(candidate.startswith(_normalize_windows_path(root) + "/") for root in nunmai_roots if root)


def _preserve_nunmai_home_path(path: str | Path) -> str:
    r"""Render Nunmai-owned paths under the configured NUNMAI_HOME spelling.

    ``%LOCALAPPDATA%\nunmai`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under NUNMAI_HOME.
    """
    candidate = Path(path)
    try:
        home = _nunmai_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Nunmai profile."""
    from nunmai_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nunmai_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"NUNMAI_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "NUNMAI_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from nunmai_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<NUNMAI_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _nunmai_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at NUNMAI_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m nunmai_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "nunmai_cli.main"]
    if profile_arg:
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, nunmai_home, profile_arg) for generated launchers.
    ``home`` targets another profile's NUNMAI_HOME (per-profile cold-start, #110959)."""
    from nunmai_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    nunmai_home = str(home if home is not None else _nunmai_home())
    return (
        _preserve_nunmai_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        nunmai_home,
        _profile_arg(nunmai_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_nunmai_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_nunmai_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, nunmai_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "NUNMAI_HOME={nunmai_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_nunmai_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, nunmai_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('NUNMAI_HOME')}) = {q(nunmai_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_nunmai_home_path(venv_dir))}",
...
def _atomic_write(path: Path, content: str, tmp: Path) -> None:
    ...
    ``Nunmai_Gateway.tmp`` pops up in Notepad after every sign-in (#114093).
    """
    try:
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from nunmai_cli.gateway import PROJECT_ROOT

    python_path, working_dir, nunmai_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"NUNMAI_HOME": nunmai_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_nunmai_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_nunmai_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (NUNMAI_HOME,
    ...
    try:
        nunmai_home = str(_nunmai_home().resolve())
    except Exception:
        nunmai_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "NUNMAI_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if nunmai_home:
        env_overlay["NUNMAI_HOME"] = nunmai_home
...
def _stdin_is_interactive(*, isatty: bool, console_mode_ok: bool | None) -> bool:
    ...
    every character device — the NUL device included (`nunmai gateway start < NUL`, stdin=DEVNULL) — so
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("NUNMAI_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from nunmai_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nunmai {profile_arg} gateway start" if profile_arg else "nunmai gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Nunmai gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: nunmai gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run in the foreground later with: nunmai gateway run")
        return

    task_name = get_task_name()
    script_path = _write_task_script()
    # A pre-fix install that failed its Startup-folder swap left `Nunmai_Gateway.tmp` there, and the
    ...
    try:
        ...
        pass
    if force:
        # Pre-suffix strays (task ``Nunmai_Gateway``, Startup ``Nunmai_Gateway.vbs``) are unreachable by
        # the current names, so a plain reconcile never heals them (#116157).
        from nunmai_cli.gateway_windows_legacy import remove_legacy_launchers
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nunmai gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _nunmai_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``nunmai gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path(home)
        path.parent.mkdir(parents=True, exist_ok=True)
        from nunmai_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``nunmai gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _nunmai_home())
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``nunmai update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `nunmai gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_nunmai_home()}\\logs\\gateway.log\n    type {_nunmai_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_nunmai_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Nunmai gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: nunmai gateway status")
                    return
...
def _print_scheduled_task_drift(task_name: str) -> None:
    ...
    repair runs from ``start()`` / ``nunmai update`` via ``reconcile_scheduled_task``)."""
    drift = scheduled_task_drift(task_name)
    if drift:
        print(f"⚠ Scheduled Task registration predates the current template ({'; '.join(drift)})")
        print("  Repair: nunmai gateway start  (or: nunmai gateway install)")
...
def reconcile_scheduled_task(task_name: str) -> bool:
    ...
    ``schtasks`` refused (typically Access Denied — the elevating ``nunmai gateway install`` is the fallback)."""
    drift = scheduled_task_drift(task_name)
    if not drift:
        return False
    ...
    if not ok:
        print("  Repair manually: nunmai gateway install")
    return ok
...
def status(deep: bool = False) -> None:
    ...
    from nunmai_cli.gateway_windows_legacy import warn_legacy_launchers
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        # answer — the NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN override or a real TTY prompt — never on a
        # non-TTY default (#113977). Declining still starts the gateway; the command is ``start``.
        start_on_login = _install_choice_from_env("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN")
        if start_on_login is None:
            from nunmai_cli.setup import is_interactive_stdin, is_noninteractive, prompt_yes_no
        ...
        if start_on_login:
            ...
            return
        print("ℹ Login auto-start not installed; add it later with: nunmai gateway install")
...
def restart() -> None:
    ...
    from nunmai_cli.gateway import _wait_for_api_server_port_free  # avoid circular init
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `nunmai gateway status`."
        )
