...
def _preserve_nunmai_home_path(path: str | Path) -> str:
    """Render Nunmai-owned paths under the configured NUNMAI_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\nunmai`` as a symlink/junction to
    ...
    path lives under NUNMAI_HOME.
    """
    candidate = Path(path)
    try:
        from nunmai_cli.config import get_nunmai_home

        home = Path(get_nunmai_home())
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nunmai_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("NUNMAI_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("NUNMAI_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["NUNMAI_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NUNMAI_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NUNMAI_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NUNMAI_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Nunmai_Gateway``
    Named profile X: ``Nunmai_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during nunmai_cli boot.
    from nunmai_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nunmai\\gateway-service\\<task_name>.cmd``
    (or ``<NUNMAI_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Nunmai installs stay self-contained).
    """
    _assert_windows()
    from nunmai_cli.config import get_nunmai_home

    script_dir = Path(get_nunmai_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``NUNMAI_HOME`` whenever it
    ...
    source checkout only if ``NUNMAI_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NUNMAI_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m nunmai_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``nunmai gateway run``.
    ...
    lines.append(f'set "NUNMAI_HOME={nunmai_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NUNMAI_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports nunmai_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_nunmai_home_path(venv_dir)}"')
    pythonpath_entries = [
    ...
    prog_args = [python_exe_path, "-m", "nunmai_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_nunmai_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_nunmai_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('NUNMAI_HOME')}) = {_quote_vbs_string(nunmai_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('NUNMAI_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_nunmai_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_nunmai_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nunmai_home = str(Path(get_nunmai_home()))
    profile_arg = _profile_arg(nunmai_home)

    content = _build_gateway_cmd_script(python_path, working_dir, nunmai_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, nunmai_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Nunmai builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    project_root = _preserve_nunmai_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nunmai_home = str(Path(get_nunmai_home()))
    profile_arg = _profile_arg(nunmai_home)
    ...
    env_overlay = {
        ...
        "NUNMAI_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_nunmai_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_nunmai_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (NUNMAI_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m nunmai_cli.main
    ...
    try:
        nunmai_home = str(Path(get_nunmai_home()).resolve())
    except Exception:
        nunmai_home = ""
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m nunmai_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_nunmai_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("NUNMAI_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("NUNMAI_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `nunmai -p <profile> gateway install` must be safe.
    ...
    from nunmai_cli.gateway import find_gateway_pids, _profile_arg
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nunmai gateway start")
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nunmai_cli.config import get_nunmai_home
        print(f"    type {Path(get_nunmai_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_nunmai_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from nunmai_cli.config import get_nunmai_home

    nunmai_home = Path(get_nunmai_home())
    print()
    print("Next steps:")
    print("  nunmai gateway status                      # Check status")
    print(f"  type {nunmai_home}\\logs\\gateway.log       # View logs")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: nunmai gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nunmai gateway start")
            return
