...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_path, working_dir, nunmai_home, profile_arg = _launcher_settings()
...
def _start_attestation_path() -> Path:
    return _nunmai_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_nunmai_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
