"""Helpers shared by the Bitwarden and 1Password ``nunmai secrets`` CLIs.

Import-light on purpose: ``nunmai_cli.secrets_cli`` must stay free of the Bitwarden backend
...
def require_enabled(console: Console, cfg: dict, product: str, command: str) -> bool:
    ...
    console.print(f"[yellow]{product} integration is disabled.  Run "
                  f"`nunmai secrets {command} setup` first.[/yellow]")
    return False
