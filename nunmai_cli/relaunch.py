"""Unified self-relaunch for Nunmai CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``nunmai sessions browse`` / post-setup relaunch keep the user's mode."""
...
def resolve_nunmai_bin() -> Optional[str]:
    """Nunmai entry point: ``sys.argv[0]`` if a real executable, else ``which nunmai``, else ``None``
    (caller falls back to ``python -m nunmai_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m nunmai_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from nunmai_cli.linux_desktop_entry import _needs_interpreter
...
def build_relaunch_argv(
    ...
    argv = [bin_path] if bin_path else [sys.executable, "-m", "nunmai_cli.main"]
...
def relaunch(
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``nunmai`` not on
