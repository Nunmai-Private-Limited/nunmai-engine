...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``nunmai_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``nunmai_state._backup_db_file``.
    """
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="nunmai-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Nunmai process using this profile and retry. "
                "This includes the interactive `nunmai` CLI session this "
                ...
                "Nunmai process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `nunmai` session open, "
