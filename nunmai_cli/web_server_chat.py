...
from nunmai_cli.pty_session import PtySessionRegistry

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("nunmai_cli.web_server")


# /api/pty spawns ``nunmai --tui`` behind a pseudo-terminal and forwards bytes +
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from nunmai_cli.web_server import _is_accepted_host, app
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    """
    from nunmai_cli.web_server import _SESSION_TOKEN, app
...
def _resolve_chat_argv(
    ...
    """Resolve the argv + cwd + env for the chat PTY (what ``nunmai --tui`` runs).

    Tests monkeypatch this with a tiny fake command.  Env contract: resume goes
    through ``NUNMAI_TUI_RESUME`` (``ui-tui`` does not parse argv), resolved to
    the newest descendant; ``NUNMAI_TUI_GATEWAY_URL`` attaches to this process's
    ...
    ``profile`` scopes the ENTIRE chat by pointing ``NUNMAI_HOME`` at the profile
    dir, the same propagation ``nunmai -p <name>`` performs. ``workspace_cwd``
    (an already-validated host directory, ``chat_workspaces.resolve_chat_cwd``)
    is the workspace the user picked for a FRESH chat: it becomes ``NUNMAI_CWD``
    (where a self-spawned gateway starts) and ``NUNMAI_TUI_CWD`` (what the TUI
    ...
    """
    from nunmai_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from nunmai_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from nunmai_cli.main import PROJECT_ROOT
    from nunmai_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides NUNMAI_HOME before config is bridged into the env.
    ...
    if workspace_cwd:
        env["NUNMAI_CWD"] = workspace_cwd
        env["NUNMAI_TUI_CWD"] = workspace_cwd
    ...
    # which inherits the profile NUNMAI_HOME set above.
...
def _resolve_client_ws_host() -> Optional[str]:
    """Host the in-container WS client should dial: ``NUNMAI_DASHBOARD_WS_HOST``
...
async def _resolve_chat_argv_async(
    ...
    from nunmai_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from nunmai_cli.web_server import _get_pty_active_session_files
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix="nunmai-console")
