...
def _nunmai_home_for_pid(pid: int) -> str | None:
    """Best-effort ``NUNMAI_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("NUNMAI_HOME"):
            return home
    try:
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("nunmai" not in cmd and "nunmai_cli" not in cmd):
        return False
...
def _nunmai_home_dir() -> Path:
    ...
    return Path(override).expanduser() if override else Path.home() / ".nunmai"
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    import json
    root = base_dir if base_dir is not None else _nunmai_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
