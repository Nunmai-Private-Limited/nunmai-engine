...
``_detect_concurrent_nunmai_instances``) are lifted verbatim. References to
helpers that STAY in ``nunmai_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``nunmai_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``nunmai_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``nunmai_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``nunmai dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``nunmai update`` replaces files on disk, the running
    ...
    ``nunmai-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Nunmai Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``nunmai serve`` as
    ...
    environment variable ``NUNMAI_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`nunmai serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "nunmai.*dashboard"`) keeps us consistent
            # with `nunmai_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `nunmai --profile p serve --host <ip>` contains
    # neither "nunmai serve" nor "nunmai_cli.main serve". Every serve/
...
def _nunmai_home_for_pid(pid: int) -> str | None:
    """Best-effort ``NUNMAI_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("NUNMAI_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"NUNMAI_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Nunmai Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``nunmai update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    ...
    if nunmai_home:
        try:
            home_path = Path(nunmai_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(nunmai_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(nunmai_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``nunmai update``.

    Each candidate is ``(pid, argv, nunmai_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_nunmai_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``NUNMAI_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``NUNMAI_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``NUNMAI_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``nunmai update`` respawn detaches with ``start_new_session=True``, so
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``nunmai dashboard`` / ``nunmai serve`` processes.

    Called at the end of ``nunmai update`` (default ``reason``) and also
    from ``nunmai dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``nunmai update`` path), a detected
    ``nunmai-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``nunmai-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``nunmai update``'s systemd
    fleet-restart loop, which restarts ``nunmai-serve*`` units before this
    ...
    # When the Nunmai Desktop Electron app spawns this dashboard as a
    # backend child, it sets NUNMAI_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("NUNMAI_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `nunmai update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot NUNMAI_HOME before the kill so per-profile caps
        ...
        if already_restarted_units:
            # Already handled directly by the caller (e.g. nunmai update's
    ...
    #    remote backend (nunmai serve) under Restart=on-failure never comes
...
def _detect_concurrent_nunmai_instances(
    ...
    The Nunmai Desktop Electron app spawns ``nunmai.EXE`` as a backend child,
    so during ``nunmai update`` the user-invoked process and the desktop's
    ...
    shims (``nunmai.exe`` / ``nunmai-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``nunmai update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``nunmai.exe``) is a separate process from the Python
    ...
    for shim in _m()._nunmai_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated nunmai.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``nunmai update``
    ...
    #      nunmai.exe sitting *under* a non-Nunmai parent (e.g. a Nunmai
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        nunmai serve --host 127.0.0.1 --port 0
        nunmai serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "nunmai" not in cmd and "nunmai_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (NUNMAI_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("NUNMAI_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``nunmai serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``nunmai serve`` on a Mac
...
def _nunmai_home_dir() -> Path:
    """Resolved Nunmai home (NUNMAI_HOME override or ~/.nunmai)."""
    override = os.environ.get("NUNMAI_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "nunmaiPath", "nunmaiHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated NUNMAI_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{nunmai_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _nunmai_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local nunmai serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``nunmai serve`` backends with no parent.
    ...
    running under NUNMAI_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never NUNMAI_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports NUNMAI_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # NUNMAI_BACKEND_READY, so a sibling spawned seconds ago is not yet
