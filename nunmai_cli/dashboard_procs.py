"""Dashboard process-hygiene helpers — extracted from ``nunmai_cli/main.py``.

Helpers defined in ``nunmai_cli.main_dashboard`` / ``nunmai_cli.main_install_repair`` are imported at
...
from nunmai_cli._startup_fast import is_desktop_ssh_backend_argv
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("nunmai", "nunmai_cli.main", "nunmai_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "nunmai.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``nunmai update`` (every API call 401s). *exclude_pids* (Desktop's NUNMAI_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Nunmai
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``nunmai serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``NUNMAI_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`nunmai --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from nunmai_cli.process_identity import ledger_entries
...
def _nunmai_home_for_pid(pid: int) -> str | None:
    """The Nunmai home *pid* runs on, tri-state: ``None`` ONLY when its environment is unreadable
    ...
    exec-time env + argv (``nunmai -p X serve`` rewrites ``NUNMAI_HOME`` in ``os.environ`` AFTER
    startup, which ``/proc/<pid>/environ`` never reflects): a profile-shaped ``NUNMAI_HOME``
    without a flag is the home; otherwise the root is ``NUNMAI_HOME`` (its grandparent when
    ...
    if env is None:
        return None
    from nunmai_cli.main_dashboard import _dashboard_cmdline_for_pid
    from nunmai_cli.profiles import get_active_profile, normalize_profile_name, profile_root_for_env_home
    argv = _dashboard_cmdline_for_pid(pid) or []
    env_home = env.get("NUNMAI_HOME", "").strip()
    profile = _profile_flag_value(argv)
    if profile is None and env_home and (
        Path(env_home).parent.name == "profiles" or env.get("NUNMAI_UPDATE_POST_SWAP") == "1"
    ):
        return env_home
    if sys.platform == "win32":
        ...
        default_home = base / "nunmai"
    else:
        default_home = Path(env.get("HOME") or _pid_passwd_home(pid) or Path.home()) / ".nunmai"
    root = profile_root_for_env_home(env_home, default_home)
    fixed_identity = any(env.get(k) for k in ("NUNMAI_SUPERVISED_CHILD", "NUNMAI_S6_SUPERVISED_CHILD",
                                               "NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR")) or is_desktop_ssh_backend_argv(argv)
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``nunmai update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _pids_owned_by_nunmai_home(pids: list[int], home: str) -> list[int]:
    """Return only *pids* whose resolved Nunmai home (``_nunmai_home_for_pid``) is ``home``.

    Dashboard argv is discovery-only: it is not an ownership proof because
    several Nunmai installs and profiles can run the same command on one
    ...
    return [
        pid for pid in pids
        if (pid_home := _nunmai_home_for_pid(pid))
        and _normalized_home_for_compare(pid_home) == target
    ]


def _profile_key_for_respawn(argv: list[str], nunmai_home: str | None = None) -> str:
    """Stable owner key: ``NUNMAI_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if nunmai_home:
        parts = _resolved_home(nunmai_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(nunmai_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, nunmai_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``NUNMAI_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``NUNMAI_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``nunmai update`` respawn detaches
    ...
    for _pid, argv, nunmai_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and nunmai_home and _normalized_home_for_compare(nunmai_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, nunmai_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``NUNMAI_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("NUNMAI_DESKTOP_CHILD_PID", "").split(","):
        with contextlib.suppress(ValueError):
            out.add(int(part))
    return out


#: Executables that only *carry* a nunmai command line. A process headed by one of these
#: never serves traffic itself; when its argv matches the dashboard patterns it is a
#: wrapper around the command (``bash -c 'nunmai dashboard --stop'``), not a backend.
...
def _is_caller_wrapper_shell(pid: int, ancestors: set[int]) -> bool:
    ...
    ``bash -c 'nunmai dashboard --stop'`` wrapper) matches on its own argv. Ancestor alone
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_nunmai(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not nunmai-owned or process identity changed"))
            else:
...
# nunmai_cli/web_server.py::_lifespan: stop_hosted_room_service(timeout=5.0) + the startup-thread
...
# and keep the deleted state.db-wal inode open — the next nunmai start refuses with a FATAL
...
def _is_detached_session_leader(pid: int, tty: str) -> bool:
    ...
    ``nunmai --tui`` child is a session leader too (``pty.fork``) but owns the pts whose master the
...
def _posix_descendants(roots: list[int]) -> dict[int, tuple[int, int | None]]:
    ...
    subtrees. So is the calling process with its subtree and its ancestor chain: ``nunmai dashboard
    --stop`` / ``nunmai update`` run from a shell escape inside the hosted Chat TUI are same-session
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``nunmai dashboard`` / ``nunmai serve`` processes (update end, ``--stop``).
    ...
    ``NUNMAI_HOME`` are candidates; unknown ownership fails closed. This is
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``nunmai update``'s systemd fleet-restart loop, which restarts ``nunmai-serve*`` units before this
    ...
    """
    from nunmai_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. nunmai-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "nunmai-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + NUNMAI_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot NUNMAI_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _nunmai_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (nunmai serve) under Restart=on-failure never comes
...
def _detect_concurrent_nunmai_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``nunmai.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from nunmai_cli.main_install_repair import _nunmai_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _nunmai_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``nunmai serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    """
    from nunmai_cli.update_cmd_windows import _nunmai_holder_subcommand
    # Canonical token matcher, never argv substrings: ``kanban --preserve-cache`` contains "serve" and
    # ``vim notes about nunmai serve`` contains both markers — this predicate decides a kill.
    if _nunmai_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``nunmai serve`` it spawns. Such a backend
...
def _nunmai_home_dir() -> Path:
    """The process's Nunmai home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "nunmaiPath", "nunmaiHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated NUNMAI_HOME can't reject a legitimate backend.
...
def _remote_lock_roots(base_dir: Path | None) -> list[Path]:
    ...
    The Desktop writes SSH locks beneath the ROOT home (``~/.nunmai/desktop-ssh``), but a profile
    backend (``nunmai --profile X serve``) runs with ``NUNMAI_HOME=<root>/profiles/X`` — scanning only
    ...
    roots: list[Path] = []
    for home in (_nunmai_home_dir(), get_default_nunmai_root()):
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{nunmai_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local nunmai serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``nunmai serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / NUNMAI_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after NUNMAI_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
