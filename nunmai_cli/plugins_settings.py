...
writes edits back through :func:`nunmai_cli.plugins_state.save_plugin_setting` — the same writer
...
from nunmai_cli.plugins_state import _plugin_relative_segments, _plugin_settings_entry, save_plugin_setting
...
def plugin_settings_fields(plugin_id: str, plugin_dir: Optional[Path]) -> List[Dict[str, Any]]:
    ...
    if not schema:
        return []
    from nunmai_cli.config import get_env_value, load_config_readonly
