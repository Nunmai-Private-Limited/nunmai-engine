"""Gateway fleet restart + post-update verification for ``nunmai update``.

Split out of ``nunmai_cli/update_cmd.py``; every name is re-imported there so
``nunmai_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from nunmai_cli.update_cmd_common import _best_effort
from nunmai_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("nunmai_cli.update_cmd")

# Under NUNMAI_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "nunmai-gateway*", "nunmai-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from nunmai_cli.update_cmd import get_nunmai_home
...
def _fleet_restart_pending_marker_path() -> Path:
    """LEGACY per-``NUNMAI_HOME`` breadcrumb. Read-compat only — nothing writes it any more.

    One host runs one multiplexing gateway, so the pull→restart obligation is host-scoped
    (``nunmai_cli/update_host_obligation.py``). An obligation armed by the old per-profile code
...
def _write_legacy_fleet_restart_pending_marker(
    *, expected_sha: str = "", runtimes: list[dict] | None = None
    ...
    """Arm the LEGACY per-``NUNMAI_HOME`` marker. True when written. Never raises.

    Fallback only: ``$NUNMAI_HOME`` is writable by construction (the updater already writes its
...
def _write_fleet_restart_pending_marker(*, expected_sha: str = "", runtimes: list[dict] | None = None) -> None:
    ...
    An unwritable host state dir (``NUNMAI_GATEWAY_LOCK_DIR`` on a read-only mount, a container
    ...
    if runtimes == []:
        ...
        # a no-gateway host would then fail every later ``nunmai update`` (#115311).
        return
    from nunmai_cli.update_cmd import _m
    from nunmai_cli.update_host_obligation import host_obligation_path, write_host_obligation
    ...
    print(
        ...
        "restart gateways with `nunmai gateway restart` if this update is interrupted.",
        file=sys.stderr,
    )
...
def _clear_fleet_restart_pending_marker() -> None:
    ...
    from nunmai_cli.update_cmd import _m
    from nunmai_cli.update_host_obligation import clear_host_obligation
...
def _fleet_restart_obligation_armed() -> bool:
    ...
    from nunmai_cli.update_host_obligation import host_obligation_present
...
def _obligation_fields() -> dict[str, str] | None:
    ...
    """
    from nunmai_cli.update_host_obligation import host_obligation_present, obligation_fields
...
def _current_checkout_sha() -> str | None:
    ...
    from nunmai_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``nunmai update`` retriggers
...
def _receipt_reports_stale_runtime(receipt: dict, expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from nunmai_cli.update_cmd import _current_checkout_sha
    ...
    from nunmai_cli.update_receipt import row_is_external
...
def _live_fleet_covers_receipt(expected_sha: str | None, receipt: dict, owed: set[tuple[str, str]] | None, *, accept_states: tuple = ("current",)) -> bool:
    ...
    if not expected_sha:
        return False
    from nunmai_cli.update_receipt import collect_fleet_versions, row_is_external
...
def _marker_only_restart_obsolete() -> bool:
    ...
    """
    from nunmai_cli.update_cmd_fleet_checkout import checkout_contains
    from nunmai_cli.update_cmd_fleet_gatewayless import host_owes_no_gateway_restart, runtime_outside_gateway_evidence
...
def _pending_fleet_restart_needed(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    from nunmai_cli.update_serve_obligations import retain_receipt_manual_serves
...
def _update_owes_fleet_restart(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    # A fleet an operator has since restarted onto a moved checkout (``nunmai gateway restart`` —
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `nunmai update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `nunmai update` or `nunmai gateway restart`.", file=stream)


def _warn_pending_fleet_restart_on_startup() -> None:
    """Cheap CLI-startup hint. Never restarts; never raises."""
    from nunmai_cli.update_receipt import read_latest_receipt
    from nunmai_cli.update_serve_obligations import retain_receipt_manual_serves, warn_pending_manual_serves
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Restart every nunmai-gateway/serve unit ONCE PER LIVE HOST PROCESS.

    One host runs one multiplexing gateway, so leftover per-profile units
    (``nunmai-gateway-<profile>.service``) all point at the SAME live ``MainPID``; restarting
    ...
    """
    from nunmai_cli.update_host_obligation import collapse_units_to_host_processes
    ...
    for unit_key, owner_key in covered.items():
        print(
            ...
            "together with: nunmai gateway migrate"
        )
...
def _live_fleet_current_rows() -> list[dict] | None:
    ...
    if not checkout_sha:
        return None
    try:
        from nunmai_cli.update_receipt import collect_fleet_versions
        fleet = collect_fleet_versions()
    except Exception as exc:
...
def _restart_identity_sha() -> str:
    ...
    installs it exists for: each profile's ``nunmai update`` would re-kill the one shared
...
def _run_pending_fleet_restart() -> bool:
    ...
    ``nunmai update`` must attach to the first one's restart instead of killing the shared
    gateway again (the obligation record carries the proof).

    See #95294.
    """
    from nunmai_cli.update_cmd import _m
    from nunmai_cli.update_host_obligation import host_restart_already_completed, mark_host_restart_completed
    ...
    # Warn if legacy Nunmai gateway unit files are still installed. When both nunmai.service (from a
    # pre-rename install) and the current nunmai-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `nunmai update` surfaces the issue until the
    # user migrates.
    try:
    ...
    # A gateway this very update cold-started (or a manual `nunmai gateway restart` seconds
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all nunmai-gateway* units (default + profiles) plus
        ...
        # --- Launchd services (macOS) --- Restart EVERY ai.nunmai.gateway* LaunchAgent, not only the
...
def _defer_fleet_restart_after_update(*, update_complete: bool, resume_incomplete: bool = False) -> None:
    ...
    ``nunmai update --no-gateway-restart`` (cron running inside the gateway's
    ...
    next normal update (or ``nunmai gateway restart``) catches up.
    ...
    print("  Restart them separately: `nunmai gateway restart` or a daily-restart cron.")
    print("  (fleet restart deferred — marker kept for catch-up)")
    with suppress(Exception):
        from nunmai_cli.update_receipt import record_skip
...
def _apply_pending_fleet_restart_catchup(*, defer: bool = False) -> None:
    ...
    """
    from nunmai_cli.update_cmd import _run_pending_fleet_restart
    ...
    if defer:
        ...
        print("  Restart separately: `nunmai gateway restart` or next non-cron update.")
        return
    print()
    _warn_pending_fleet_restart()
    ...
    # clears, even after a successful manual `nunmai gateway restart` (#117051).
    fleet = _live_fleet_current_rows()
    if fleet is not None:
        from nunmai_cli.update_receipt import settle_latest_receipt_fleet
        ...
        if settled:
            ...
            return
    print("  ⚠ Fleet restart ran, but gateways are still off the checkout code. Recover with: nunmai gateway restart")
    sys.exit(1)
...
def _systemd_unit_owned_by_update(scope_cmd: list, svc_name: str) -> bool:
    ...
    ``nunmai-gateway*`` is an account-wide namespace: a second install's ``nunmai update`` used
    to drain and restart the account's real ``nunmai-gateway.service`` because the unit was
    ...
    """
    from nunmai_cli.update_fleet_scope import describe_skipped_runtime, systemd_unit_nunmai_home, home_in_update_scope
    home = systemd_unit_nunmai_home(scope_cmd, svc_name)
...
def _scoped_manual_gateway_pids(pids, *, keep=(), quiet: bool = False) -> list[int]:
    ...
    from nunmai_cli.update_fleet_scope import describe_skipped_runtime, partition_gateway_pids_by_scope
    keep = set(keep)
...
def _is_nunmai_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("nunmai-serve")``
    would accept ``nunmai-server.service``."""
    return (
...
def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each nunmai-gateway*/nunmai-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_nunmai_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``nunmai-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``nunmai-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``nunmai-gatewayd`` is never signalled.
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.nunmai.gateway*`` LaunchAgent
    ...
    # Units labelled before the profile-name suffix scheme (ai.nunmai.gateway-<hash>) are invisible
    ...
    from nunmai_cli.update_fleet_scope import describe_skipped_runtime, launchd_label_foreign_home
    for label in derived_labels + legacy_labels:
        ...
        # installed. A plist pinning a foreign NUNMAI_HOME is another install's job (#93349).
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``nunmai_cli.gateway`` no longer importing
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither NUNMAI_SPAWN
# nor NUNMAI_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a nunmai-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``nunmai_cli.gateway`` in the
    ...
    child's ``nunmai-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``nunmai_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """NUNMAI_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from nunmai_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to nunmai-gateway* qualifies (idempotent
...
def _repair_unit_without_fatal_exit_park(svc_name: str, scope: str) -> None:
    ...
    if system and not _needs_sudo(scope) and svc_name == get_service_name():
        # The refresh adopts the unit's NUNMAI_HOME into os.environ (sudo strips it); the rest of the
        # update keeps running for the invoking profile.
        launch_home = os.environ.get("NUNMAI_HOME")
        try:
            ...
            if launch_home is None:
                ...
                os.environ["NUNMAI_HOME"] = launch_home
        return
    print(
        ...
        f"    Repair: {'sudo ' if system else ''}nunmai gateway install{' --system' if system else ''}"
    )
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. nunmai-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    # budget, restart-policy mismatch). Mirrors `nunmai gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(
    ...
    """Restart every active nunmai-gateway*/nunmai-serve* systemd unit (user + system).
    ...
    """
    from nunmai_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            if unmapped_count > 1:
                print("    (or: nunmai -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from nunmai_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from nunmai_cli.update_abort_recovery import _owed_stale_serve_rows
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from nunmai_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/nunmai-serve`` vs ``system/nunmai-serve`` are different
    ...
    try:
        ...
        from nunmai_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        # macOS: EVERY ai.nunmai.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy nunmai.service fights nunmai-gateway.service over the bot token; warn on
    every update until migrated."""
    from nunmai_cli.gateway import (has_legacy_nunmai_units, _find_legacy_nunmai_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_nunmai_units()):
        return
    print()
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `nunmai serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `nunmai serve` (no unit) is left running its
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from nunmai_cli.update_receipt import print_fleet_version_matrix
        ...
        if print_fleet_version_matrix(_fleet_snapshot):
            ...
            from nunmai_cli.update_cmd_stale_survivors import signal_stale_fleet_survivors
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from nunmai_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            from nunmai_cli.update_serve_obligations import defer_manual_serve
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from nunmai_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``nunmai_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
