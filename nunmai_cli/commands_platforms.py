...
# Logger name parity with the origin module (tests capture "nunmai_cli.commands").
logger = logging.getLogger("nunmai_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``nunmai skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/nunmai <command>`` on Slack only: the registry
...
_SLACK_VIA_NUNMAI_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_NUNMAI_ONLY are skipped. ``/nunmai`` is always first for anything dropped."""
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/nunmai`` handler, plugin commands included."""
