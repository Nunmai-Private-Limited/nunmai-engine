...
setup.py names are resolved through the module object so test patches on ``nunmai_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Nunmai has one ($NUNMAI_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `nunmai setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `nunmai egress setup` then `nunmai egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `nunmai egress setup`.")
...
def _setup_backend_modal(config: dict) -> None:
    ...
    if managed_modal_available:
        ...
        use_managed_modal = _setup.prompt_choice(
            ...
            ["Use my Nunmai subscription", "Use my own Modal account"], default_idx) == 0
    if use_managed_modal:
        config["terminal"]["modal_mode"] = "managed"
        _setup.print_info("Modal execution will use the managed Nunmai gateway and bill to your subscription.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'nunmai-engine[vercel]'")
    _ensure_sdk("vercel", "pip install 'nunmai-engine[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Nunmai runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.nunmai/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
