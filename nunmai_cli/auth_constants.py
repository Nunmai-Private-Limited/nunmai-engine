...
Pure leaf: imports nothing from ``nunmai_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because nunmai_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("nunmai_cli.auth.httpx.Client")
