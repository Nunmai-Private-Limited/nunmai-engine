"""CLI subcommand: ``nunmai send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"nunmai send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"nunmai send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"nunmai send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `nunmai send`) would otherwise be
    ...
    if not platforms:
        ...
        print("channel discovery can populate ~/.nunmai/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_nunmai_env() -> None:
    """Populate ``os.environ`` from ``~/.nunmai/.env`` AND bridge top-level
    ...
    startup that ``nunmai send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.nunmai/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.nunmai/config.yaml`` into
       ...
       via ``nunmai config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``nunmai send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.nunmai/.env and ~/.nunmai/config.yaml into os.environ so the
    ...
    # Import lazily so `nunmai send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.nunmai/.env + ~/.nunmai/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  nunmai send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
