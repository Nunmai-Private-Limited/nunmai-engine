"""CLI subcommand: ``nunmai send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"nunmai send: {file_path} is not a text file. --file reads the "
                ...
                f'  nunmai send --to telegram "MEDIA:{file_path}"\n'
                f'  nunmai send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  nunmai send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"nunmai send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"nunmai send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"nunmai send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"nunmai send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"nunmai send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `nunmai gateway setup`, or run the gateway once so")
        from nunmai_constants import get_default_nunmai_root, get_nunmai_home, nunmai_home_key
        home, root = get_nunmai_home(), get_default_nunmai_root()
        ...
        if nunmai_home_key(root) != nunmai_home_key(home) and (root / "channel_directory.json").exists():
...
def _load_nunmai_env() -> None:
    """Populate the credential environment from ``<NUNMAI_HOME>/.env`` AND bridge top-level ``config.yaml``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_nunmai_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `nunmai send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "nunmai send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  nunmai send --to telegram \"hello\"\n"
            "  nunmai send --to discord:#ops --file report.md\n"
            "  nunmai send --list      # list available targets",
            _USAGE_EXIT)
    mentions = list(getattr(args, "mentions", None) or [])
    if mentions and target.split(":", 1)[0].strip().lower() != "whatsapp":
        _fail("nunmai send: --mention is only supported for WhatsApp targets.", _USAGE_EXIT)
    invalid_mentions = _invalid_whatsapp_mentions(mentions)
    if invalid_mentions:
        _fail(
            "nunmai send: invalid --mention value(s): "
    ...
    if message is None or not message.strip():
        _fail(
            "nunmai send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `nunmai send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `nunmai send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Nunmai "
            "is already configured for. Reuses the gateway's platform "
            f"credentials ({nunmai_home / '.env'} + "
            f"{nunmai_home / 'config.yaml'}) — no LLM, "
        ...
        epilog=(
            ...
            "  nunmai send --to whatsapp:GROUP@g.us --mention 15551234567 \"@15551234567 hello\"\n"
            "  nunmai send --to telegram \"MEDIA:./chart.png\"   # send a media attachment\n"
