...
Usage:
    nunmai pairing list              # Show all pending + approved users
    nunmai pairing approve <platform> <request-id|code>  # Approve a pairing request
    nunmai pairing revoke <platform> <user_id> # Revoke user access
    nunmai pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle nunmai pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: nunmai pairing {list|approve|revoke|clear-pending}")
        print("Run 'nunmai pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: nunmai pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.nunmai/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'nunmai pairing list' to see pending requests.\n")
