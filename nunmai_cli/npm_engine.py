...
Scope is deliberately narrow: Nunmai only upgrades an npm inside its **own** managed Node tree
(``$NUNMAI_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$NUNMAI_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$NUNMAI_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
    the "upgrade" would install a second npm elsewhere while the managed one stayed stale.
    """
    if not quiet:
        print(f"→ Upgrading Nunmai-managed npm to satisfy {npm_range}…", flush=True)
...
def _probe_version(npm: str) -> str | None:
    try:
        result = subprocess.run([npm, "--version"], timeout=30, env=with_nunmai_node_path(), **_RUN_KW)
    except (OSError, subprocess.SubprocessError):
        return None
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$NUNMAI_HOME/node`` and return a satisfying npm.
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool = False) -> str | None:
    ...
    if prefix is not None:
        # Nunmai owns this npm — upgrade in place. Only an npm-range failure is fixable this way.
