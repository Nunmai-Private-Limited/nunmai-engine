"""Implementation of the ``nunmai bundles`` CLI subcommand.

Mirrors the structure of ``nunmai_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `nunmai bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  nunmai bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``nunmai bundles`` argparse tree.

    Called from ``nunmai_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `nunmai bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``nunmai bundles <subcommand>`` to the right handler."""
