...
Split out of ``nunmai_cli/config.py``; every name is re-imported there, so
``nunmai_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``nunmai_cli.config``
...
logger = logging.getLogger("nunmai_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but Nunmai'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by Nunmai, not hand-curated.
...
def get_custom_provider_session_affinity_header(
    ...
    Opt-in per provider (default off): Nunmai never ships a session identifier to an endpoint
...
def get_custom_provider_context_length(
    ...
    """
    from nunmai_cli.config import get_compatible_custom_providers, load_config_readonly
