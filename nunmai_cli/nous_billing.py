"""Nunmai Portal Remote Spending HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``nunmai_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Nunmai already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``NUNMAI_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# nunmai_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``NUNMAI_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("NUNMAI_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nunmai Portal — run `nunmai portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    when there is no usable Nunmai session at all.
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Nunmai Portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Nunmai Portal: timed out", error="network_error"
        ) from exc
