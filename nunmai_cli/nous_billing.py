"""Nunmai Portal Remote Spending HTTP client.
...
# Keyed by nunmai_home_key() so a multiplex profile never bills through a sibling's token.
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into Nunmai Portal — run `nunmai portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _request(
    ...
    try:
        ...
        raise BillingError(f"Could not reach Nunmai Portal: {exc.reason}", error="network_error") from exc
        ...
        raise BillingError("Could not reach Nunmai Portal: timed out", error="network_error") from exc
