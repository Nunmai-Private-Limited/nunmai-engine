"""
Unified tool configuration for Nunmai Engine.

`nunmai tools` and `nunmai setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.nunmai/config.yaml under
...
from nunmai_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``nunmai tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`nunmai_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from nunmai_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _NUNMAI_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `nunmai tools` → Video Generation, which walks
...
# `nunmai tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `nunmai tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from nunmai_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `nunmai tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``nunmai tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from nunmai_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from nunmai_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``nunmai tools`` checklist actually offers
    for ``platform``.
    ...
    so ``nunmai tools`` never claims to add or remove a toolset the user was
...
from nunmai_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Nunmai routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set NUNMAI_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "NUNMAI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "NUNMAI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "NUNMAI_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "NUNMAI_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "NUNMAI_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `nunmai model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("NUNMAI_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``NUNMAI_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Nunmai telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Nunmai' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $NUNMAI_HOME/bin is never on PATH, so a bare which()
    # misses the uv Nunmai installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``nunmai update`` and
      by ``nunmai computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``nunmai update`` sets
    ...
    a further ~600s wait on Windows). ``nunmai computer-use install
    ...
    ``nunmai update`` already prints a contextual line before its update
    ...
    ``nunmai update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `nunmai update` calls this
    ...
    override = os.environ.get("NUNMAI_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    NUNMAI_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Nunmai' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Nunmai' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `nunmai update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Nunmai cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("NUNMAI_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "nunmai computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `nunmai computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `nunmai update` (require_confirmed_update=True) keeps the
    ...
    # `nunmai computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: nunmai computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: nunmai computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``nunmai update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `nunmai update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``nunmai update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `nunmai update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `nunmai update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.nunmai/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Nunmai process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Nunmai-managed ``$NUNMAI_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Nunmai installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Nunmai-managed node -> npx) rather than a bare
        # shutil.which — Nunmai-managed-Node-only setups resolve agent-browser
        ...
        # Nunmai-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Nunmai-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.nunmai/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.nunmai/config.yaml")
        ...
        # Run the full `nunmai auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.nunmai/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from nunmai_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: nunmai auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `nunmai auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: nunmai auth spotify")
        except Exception as exc:
        ...
        try:
            from nunmai_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: nunmai plugins enable observability/langfuse")
        _print_info("    Restart Nunmai for tracing to take effect.")
        _print_info("    Verify: nunmai plugins list")
        ...
        try:
            ...
            _print_info("    Run later: nunmai auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `nunmai auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: nunmai auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: nunmai auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``nunmai tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``nunmai tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: nunmai tools post-setup <key>")
        return 2
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from nunmai_cli.plugins import (
            get_plugin_manager,
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``nunmai-discord``, which contains those
...
#:
#: Saving ``nunmai tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[nunmai-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``nunmai tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"nunmai-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (nunmai-acp, nunmai-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``nunmai-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[nunmai-cli, a2a]`` must survive filtering just
    ...
    # "nunmai-cli" (which include all _NUNMAI_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[nunmai-cli, spotify]`` after enabling Spotify via ``nunmai
        ...
        # (e.g. "nunmai-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``nunmai tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `nunmai tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"nunmai-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("nunmai-")}
    ...
    # shipped as a bundled plugin but user must opt in via `nunmai tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `nunmai tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by nunmai tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `nunmai config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `nunmai` instead of
    # `nunmai-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `nunmai update`/`nunmai doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `nunmai tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (nunmai-cli, nunmai-telegram, etc.)
    ...
    # Opening `nunmai tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `nunmai tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``nunmai tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``nunmai tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from nunmai_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``nunmai tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from nunmai_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (nunmai_cli/web_server.py
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``nunmai model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``nunmai model`` aux-task
    ...
    """
    try:
        from nunmai_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `nunmai model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `nunmai model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nunmai tools` and `nunmai setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("◆ Nunmai Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://nunmai.in/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_nunmai_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'nunmai' or gateway restart.", Colors.DIM))
    print()
