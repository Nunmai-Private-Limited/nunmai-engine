...
from nunmai_cli.cli_output import print_info as _print_info
from nunmai_cli.colors import Colors, color
from nunmai_cli.config import cfg_get, load_config, save_config, get_env_value
...
from nunmai_cli.toolset_scope import (
...
# Re-exports: keep ``nunmai_cli.tools_config.X`` callers and test patch targets resolving.
from nunmai_cli.tools_config_cua import (  # noqa: F401
...
from nunmai_cli.tools_config_post_setup import (  # noqa: F401
...
from nunmai_cli.tools_config_providers import (  # noqa: F401
...
from nunmai_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _NUNMAI_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `nunmai tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``nunmai tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``nunmai-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"nunmai-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` / Nunmai keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_LANGFUSE_PUBLIC = ("NUNMAI_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("NUNMAI_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            _row("Nunmai Subscription", "subscription", "Managed OpenAI TTS billed to your subscription", tts_provider="openai",
    ...
    "stt": {
        ...
        "providers": [
            ...
            _row("Nunmai Subscription", "subscription", "Managed OpenAI transcription billed to your subscription",
    ...
    "web": {
        ...
        # non-provider firecrawl setup-flow rows live here: managed via Nunmai subscription, and self-hosted.
        "providers": [
            {"name": "Nunmai Subscription", "badge": "subscription", "tag": "Managed Firecrawl billed to your subscription",
    ...
    "image_gen": {
        ...
        # _plugin_image_gen_providers(). Only the managed "Nunmai Subscription" row lives here: ONE row for the
        ...
        "providers": [
            _row("Nunmai Subscription", "subscription",
                 "Managed image generation (FAL, Krea 2, Nunmai Portal models) billed to your subscription", **_NOUS,
                 managed_nous_feature="image_gen", override_env_vars=["FAL_KEY"], imagegen_backend="nous"),
        ],
    },
    "video_gen": {
        "name": "Video Generation", "icon": "🎬",
        # Mirrors image_gen: managed FAL video billed via the Nunmai Portal. Plugin-backed rows (FAL BYOK, xAI, …)
        ...
        "providers": [
            _row("Nunmai Subscription", "subscription", "Managed FAL video generation billed to your subscription", **_NOUS,
    ...
    "x_search": {
        ...
        "setup_note": (
            "Nunmai routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on the paid Nunmai row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Nunmai, text-only (no screenshots)",
            ...
            _row("Nunmai Subscription (Browser Use cloud)", "subscription", "Managed Browser Use billed to your subscription",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; NUNMAI_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("NUNMAI_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from nunmai_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``nunmai tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[nunmai-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (nunmai-acp, nunmai-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``nunmai-cli`` would re-enable disabled toolsets). A mixed list (``[nunmai-cli, spotify]``) still expands the
...
def _coerce_platform_toolsets_value(value, platform: str):
    ...
    The parser is shared with ``nunmai doctor`` and ``nunmai plugins`` (``toolset_validation``
    ...
    if platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "Run `nunmai tools` to reconfigure.", platform, value)
    return value
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``nunmai-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[nunmai-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[nunmai-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `nunmai config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``nunmai tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("nunmai-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``nunmai`` for ``nunmai-cli`` → no
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (nunmai-cli, ...) resolve to ALL tools and would silently override the user's
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    state = "off" if not enabled else ("collecting + sending to Nunmai" if send else "collecting locally")
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from nunmai_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``nunmai tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nunmai tools` / `nunmai setup tools`. ``first_install`` skips the menu (checklist + key
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('nunmai_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('nunmai_cli.nous_account', 'format_nous_portal_entitlement_message'),
