...
# Entry delimiter of the Nunmai memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `nunmai config edit` (or move it aside), then re-run the import."
...
def _translate_mcp_server(name: str, srv: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Map one Claude/Codex MCP server entry to Nunmai shape; returns (server, stripped secret paths)."""
...
class AgentImporter:
    ...
    def import_mcp_servers(self, servers: Dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                ...
                continue
            nunmai_srv, stripped = _translate_mcp_server(name, srv)
    ...
    def import_skills(self, source_root: Path) -> None:
        ...
        if not skill_dirs:
            ...
            return
        from nunmai_cli.agent_import_sync import skill_tree_digest
...
def import_agent_command(args) -> None:
    ...
    from nunmai_cli.setup import (Colors, color, print_header, print_info, print_success,
                                  print_error, prompt_yes_no)

    if getattr(args, "sync", False):
        from nunmai_cli.agent_import_sync import sync_imported_agents
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"Specify a custom path: nunmai import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), nunmai_home.resolve(),
    ...
    from nunmai_cli.agent_import_sync import update_sync_manifest
    try:
        update_sync_manifest(agent, source_dir.resolve(), nunmai_home.resolve(), overwrite, report)
        print_info("Source registered for sync — re-run 'nunmai import-agent --sync' "
