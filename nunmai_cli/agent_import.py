"""nunmai import-agent — import Claude Code / Codex CLI setups into Nunmai.

Usage:
    nunmai import-agent                       # auto-detect ~/.claude or ~/.codex
    nunmai import-agent claude-code           # import from ~/.claude
    nunmai import-agent codex                 # import from ~/.codex
    nunmai import-agent claude-code --dry-run # preview only, no changes
    nunmai import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``nunmai claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_nunmai.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in NUNMAI_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → NUNMAI_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in NUNMAI_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in NUNMAI_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → NUNMAI_HOME/skills/codex-imports/<name>/
...
deliberately via ``nunmai setup`` or config.yaml.
"""
...
# Same entry delimiter as the Nunmai memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`nunmai config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_nunmai.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Nunmai command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Nunmai glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Nunmai equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        nunmai_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → NUNMAI_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``nunmai import-agent`` (invoked from nunmai_cli.main)."""
    from nunmai_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: nunmai import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: nunmai import-agent claude-code   or   nunmai import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ◆ Nunmai — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: nunmai import-agent "
    ...
    print_info(f"Target:      {nunmai_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'nunmai setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: nunmai import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'nunmai setup' "
               "to configure providers, or add them to ~/.nunmai/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from nunmai_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'nunmai setup' or ~/.nunmai/.env.")
        print()
