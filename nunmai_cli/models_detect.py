...
Split out of ``nunmai_cli.models``. The detection ladder there consults static catalogs, then the
...
* the CURRENT provider's live catalog outranks every static guess (Codex early-access ids, Nunmai
...
def current_provider_catalog_match(model_name: str, current_provider: str) -> Optional[str]:
    ...
    Goes through :func:`nunmai_cli.models.cached_provider_model_ids` (1h TTL, stale-while-
    ...
    from nunmai_cli.models import cached_provider_model_ids, normalize_provider
...
def current_provider_owns_vendor(model_name: str, current_provider: str) -> bool:
    ...
    from nunmai_cli.model_normalize import detect_vendor
    from nunmai_cli.models import _AGGREGATOR_PROVIDERS, _PROVIDER_MODELS, normalize_provider
