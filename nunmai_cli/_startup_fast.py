...
This module is imported by ``nunmai_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no nunmai_cli.config,
...
``nunmai_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(nunmai_root: str) -> bool:
    ...
    active_profile = os.path.join(nunmai_root, "active_profile")
    try:
...
def container_mode_may_be_active() -> bool:
    ...
    if nunmai_home:
        ...
        return (
            ...
            and active_profile_may_override_home(nunmai_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".nunmai")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect NUNMAI_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    ``nunmai version`` subcommand. Every lazy block degrades gracefully —
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    container. Termux keeps the NUNMAI_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
