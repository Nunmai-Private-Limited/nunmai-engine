...
Imported by ``nunmai_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
nunmai_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
__all__ = [
    "project_root_str", "normalize_nunmai_home_env", "ensure_project_root_on_path", "is_termux_env",
...
def normalize_nunmai_home_env() -> None:
    """Expand ``~``/``$VAR`` in ``NUNMAI_HOME`` once, at process entry, and write it back.

    fish does not expand ``~`` inside ``VAR=~/...`` and every shell passes a quoted value
    through verbatim, so a literal tilde reaches the process. ``Path("~/.nunmai")`` is
    *relative*: the many raw ``os.environ["NUNMAI_HOME"]`` readers (this fast path, the
    active_profile probe, profile re-home, the dotenv loader) would each resolve it against
    cwd and scaffold a full home under ``<cwd>/~/.nunmai``. One expansion here gives every
    reader the same absolute spelling; ``nunmai_constants`` expands as well for non-CLI
    entry points. A relative value that is not tilde/variable-shaped is left alone.
    """
    raw = os.environ.get("NUNMAI_HOME", "")
    if not raw.strip():
        return
    expanded = os.path.expanduser(os.path.expandvars(raw.strip()))
    if expanded != raw:
        os.environ["NUNMAI_HOME"] = expanded
...
def is_desktop_ssh_backend_argv(argv: list[str]) -> bool:
    ...
    ``NUNMAI_DASHBOARD_SESSION_TOKEN`` env var the local pool spawn uses. Every reader of
...
def active_profile_may_override_home(nunmai_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect NUNMAI_HOME?"""
    active = (_read_text(os.path.join(nunmai_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".nunmai")


def _resolved_home() -> str:
    return os.environ.get("NUNMAI_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("NUNMAI_DEV") == "1" or is_container_startup_environment():
        return False
    nunmai_home = os.environ.get("NUNMAI_HOME", "").strip()
    if nunmai_home:
        if os.path.exists(os.path.join(nunmai_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(nunmai_home)))
        return parent_name != "profiles" and active_profile_may_override_home(nunmai_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``nunmai --version`` output (also used by /version).
    ...
    try:
        ...
        from nunmai_cli import __release_date__, __version__

        print(f"Nunmai Engine v{__version__} ({__release_date__})")
    ...
    try:
        from nunmai_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``nunmai --version`` before the heavy import wall.
    ...
    NUNMAI_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("NUNMAI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
