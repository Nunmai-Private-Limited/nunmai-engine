"""``nunmai profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``nunmai_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _profile_status(args):
    ...
    from nunmai_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → nunmai -p {p.name}")
    print()
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from nunmai_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from nunmai_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.nunmai)" if name == "default" else f"Switched to: {name}")
...
def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from nunmai_cli.profile_channels import (
...
def _profile_create(args):
    ...
    if cloned:
        if clone_all:
            ...
            from nunmai_cli.profile_memory_config import cloned_memory_provider
            memory_provider = cloned_memory_provider(profile_dir)
            if memory_provider:
                print(f"Cloned memory provider config ({memory_provider}) too.")
        if sync_imports:
            print(f"Import sources carried over — `nunmai -p {name} import-agent --sync` "
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  nunmai profile alias {name} --name <custom>")
    ...
    if served is not None and normalize_profile_name(name) in {normalize_profile_name(p) for p in served}:
        ...
        print("  nunmai gateway restart    Serve this profile from the running multiplexed gateway")
    else:
...
def _profile_delete(args):
    from nunmai_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_nunmai_home),
...
def _profile_show(args):
    ...
    if alias_name:
        print(f"Alias:   {alias_name} → nunmai -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_migrate_identity(args):
    ...
    from nunmai_cli.profile_identity import migrate_profile_identity
    try:
    ...
    if not migrated:
        _die(f"Error: session identity was not migrated. Restart or stop the gateway, then run:\n"
             f"    nunmai profile migrate-identity {args.old_name} {args.new_name}", err=True)
...
def _profile_purge_identity(args):
    ...
    if not purged:
        _die(f"Error: session identity was not purged. Restart or stop the gateway, then run:\n"
             f"    nunmai profile purge-identity {args.profile_name}", err=True)
    print(f"✓ Session/routing identity purged: {args.profile_name}")


def _profile_export(args):
    from nunmai_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from nunmai_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
...
def _profile_update(args):
    from nunmai_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    ...
    try:
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  nunmai -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("nunmai_requires", "Requires:     Nunmai "),
    ("source", "Source:       "),
...
# Order mirrors the original if/elif chain; None = bare ``nunmai profile``.
PROFILE_ACTIONS = {
