"""Which ``nunmai config`` keys live in ``.env`` instead of ``config.yaml``, and their lifecycle.

Platform setting keys such as ``FEISHU_HOME_CHANNEL`` had two writers: the platform setup flows and
``/sethome`` persist them to ``.env`` through ``save_env_value``, while ``nunmai config set`` only
...
``platform_gate_env``) resolves against — whether or not Nunmai enumerates it anywhere. Roughly 290
of the ~700 documented variables (``TELEGRAM_GROUP_ALLOWED_USERS``, ``NUNMAI_TIMEZONE``, ...) are
...
lifecycle in ``nunmai_cli.credential_lifecycle``.
"""
...
def is_registered_env_name(name: str) -> bool:
    """True when Nunmai itself enumerates ``name``: ``OPTIONAL_ENV_VARS`` / ``_EXTRA_ENV_KEYS``, or a
    self-configuring platform suffix so plugin adapters nobody listed (``IRC_HOME_CHANNEL``) count."""
    from nunmai_cli.config import _EXTRA_ENV_KEYS, OPTIONAL_ENV_VARS
    from nunmai_cli.setup_hidden_env import is_setup_hidden_env
...
def is_env_setting_key(key: str) -> bool:
    """True for a bare (undotted) key ``nunmai config`` stores in ``.env``: any ``UPPER_SNAKE`` name,
...
def _drop_config_yaml_copies(key: str) -> bool:
    ...
    from nunmai_cli.config import _write_user_config, get_config_path, require_readable_config_before_write
...
def remove_env_setting(key: str) -> bool:
    ...
    from nunmai_cli.config import remove_env_value
...
def read_env_setting(key: str) -> Optional[str]:
    ...
    from nunmai_cli.config import get_env_value, read_raw_config_readonly

    value = get_env_value(key.upper())
    if value is None:
        value = read_raw_config_readonly().get(key)
        if value is not None:
            print(f"  (note: {key} is a stale top-level config.yaml copy; `nunmai config set {key} <value>` "
                  f"moves it to .env, `nunmai config unset {key}` removes it)", file=sys.stderr)
    return value
