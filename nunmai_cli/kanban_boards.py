"""``nunmai kanban boards …`` — board directories, the ``current`` pointer and ``board.json``.
...
from nunmai_cli.kanban_output import _err, _fmt_counts, _json_out


def _dispatch_boards(args: argparse.Namespace) -> int:
    """``nunmai kanban boards <action>`` — filesystem-only, so it works before ``kanban init``."""
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print(f"Exported board {res['board']!r} → {res['archive']}\n"
          ...
          "Import it with `nunmai kanban boards import <archive>`.")
    return 0
