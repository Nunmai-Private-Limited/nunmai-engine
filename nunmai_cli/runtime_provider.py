...
Custom-provider lookup lives in :mod:`nunmai_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`nunmai_cli.runtime_provider_backends` — both re-exported here so
``nunmai_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from nunmai_cli import auth as auth_mod
...
from nunmai_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from nunmai_cli import config as _config_mod
from nunmai_cli import models as _models  # attribute access keeps ``nunmai_cli.models.<name>`` patches effective
from nunmai_constants import OPENROUTER_BASE_URL
from nunmai_cli.providers import determine_api_mode, get_provider, is_actual_route, is_official_openai_host, nous_api_mode
...
# lazily, so its first import can happen while a test has ``nunmai_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in nunmai_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in nunmai_cli/providers.py so the runtime resolver stays in
...
_NO_ANTHROPIC_CREDENTIALS_MSG = ("No Anthropic credentials found. Run 'nunmai auth add anthropic' to sign in, "
...
def is_foreign_provider_endpoint(provider: Optional[str], base_url: Optional[str]) -> bool:
    ...
    switch that kept the old URL (openai-codex + the Nunmai Portal URL sent the Codex slug to the Portal).
    ...
    if not token:
        ...
        if model and resolve_anthropic_token():
            raise AuthError(f"Anthropic credentials are rate-limited for {model}; "
                            "other Claude models remain available (see `nunmai auth list`).")
...
def _nous_min_key_ttl() -> int:
    return max(60, env_int("NUNMAI_NOUS_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nous_creds() -> Dict[str, Any]:
    return resolve_nous_runtime_credentials(timeout_seconds=float(get_secret_str("NUNMAI_NOUS_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``NUNMAI_INFERENCE_PROVIDER``, else
    ...
    return get_secret_str("NUNMAI_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from nunmai_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from nunmai_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider in _POOL_ENTRY_SIMPLE_MODES:
        ...
        if provider == "openai-codex":
            ...
            # NUNMAI_CODEX_BASE_URL override must apply consistently to every
            # credential source, including pooled OAuth credentials.
            override_url = get_secret_str("NUNMAI_CODEX_BASE_URL", "").strip().rstrip("/")
            if override_url:
...
def _openrouter_should_use_pool(requested_provider, model_cfg, explicit_api_key, explicit_base_url) -> bool:
    ...
    # OpenRouter host — `nunmai setup` persists https://openrouter.ai/api/v1 for plain installs,
...
def _refresh_nous_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    """Nunmai pool entries carry the agent_key (an invoke JWT) which the pool does not refresh on
    selection (avoids network calls in `nunmai auth list`); refresh here before falling back to
...
def _exchange_copilot_pool_entry(entry: Any, pool_api_key: str) -> str:
    ...
    from nunmai_cli.copilot_auth import get_copilot_api_token, validate_copilot_token
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "nous": _OAuthRuntimeSpec(_resolve_nous_creds, nous_api_mode, "portal", "expires_at",
                              "Auto-detected Nunmai provider but credentials failed"),
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "nunmai-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "nunmai-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.nunmai/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `nunmai setup` to install Vertex support.")
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('nunmai_cli.providers', 'custom_provider_aliases'),
...
def resolve_runtime_with_fallback(config: Optional[Dict[str, Any]], *, requested: Optional[str] = None,
    ...
    """
    from nunmai_cli.auth import AuthError, primary_failure_wording
    try:
        ...
        from nunmai_cli.fallback_config import effective_runtime_provider, get_fallback_chain, resolve_entry_api_key
