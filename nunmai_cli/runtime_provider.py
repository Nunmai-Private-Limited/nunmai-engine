...
from nunmai_cli.providers import custom_provider_aliases, custom_provider_slug
from nunmai_constants import OPENROUTER_BASE_URL
from nunmai_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`nunmai_cli.config.load_config`.

    Deliberately NOT a module-level ``from nunmai_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``nunmai_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``nunmai_cli.config.load_config`` OR
    ``nunmai_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from nunmai_cli.auth import resolve_provider as _resolve_provider
...
_VALID_API_MODES = {
    ...
    # instead of Nunmai' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from nunmai_cli.config import _canonical_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("NUNMAI_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/nunmai_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``NUNMAI_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("NUNMAI_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'nunmai model' or "
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("NUNMAI_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("NUNMAI_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from nunmai_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("NUNMAI_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "nunmai-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from nunmai_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Nunmai canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Nunmai-compatible importers).
