...
container's boot (``nunmai_cli/container_boot.py``) registers every NAMED slot down and lets the
root slot inherit their autostart intent; ``nunmai gateway migrate --multiplex`` (and the hook
``nunmai update`` runs) must be able to do the same thing to a slot that is UP, from inside the
...
def fold_named_slot_intent(default_prior_state: Optional[str],
    ...
    Without the fold an image only ever driven as ``nunmai -p coder gateway start`` came up with
...
def named_slot_name(profile: str) -> str:
    from nunmai_cli.service_manager import S6_SERVICE_PREFIX
...
def running_named_slots(profiles: Sequence[str], manager=None) -> list[str]:
    ...
    A registered-down slot (what boot leaves behind) is a start target for ``nunmai -p X gateway
...
def _wait_down(service_dir: Path, timeout_ms: int = 15000) -> None:
    from nunmai_cli.service_manager import _s6_run
    try:
