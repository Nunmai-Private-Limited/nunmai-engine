"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (nunmai gui/desktop).

Split out of ``nunmai_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
from nunmai_cli.desktop_console import desktop_console_output, desktop_launch_notice
from nunmai_platform.host import facts
from nunmai_cli.main_tui_launch import _npm_lifecycle_env
from nunmai_cli.main_web_build import (
    _hash_source_tree, _nixos_build_env, _stamp_is_current, _write_build_stamp)

# Log-record parity with the origin module.
logger = logging.getLogger("nunmai_cli.main")
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $NUNMAI_HOME."""
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / d / "Nunmai.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("nunmai", "Nunmai")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Nunmai.exe to the launcher. Prefer
        ...
        # wrong-architecture Nunmai.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Nunmai.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``nunmai update`` used to report
...
def _swap_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        if moved_aside:
            ...
            # never exited at all (a manual `nunmai update`/`nunmai desktop` run does not
...
# (Desktop → nunmai-setup --update → `nunmai update` → `nunmai desktop --build-only` → relaunch) rebuilds
# Nunmai.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Nunmai.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer Nunmai.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Nunmai is damaged" and loses every grant. Clear quarantine xattrs, then sign
...
def _desktop_macos_setup_tcc_identity(identity: str = "Nunmai Local Signing") -> bool:
    ...
    from nunmai_cli.main import PROJECT_ROOT
    if sys.platform != "darwin":
...
def _running_macos_app_bundles() -> set[Path]:
    """``.app`` bundles of every live Nunmai Desktop process. A running bundle is never swapped
    ...
    for proc in psutil.process_iter(["exe"]):
        ...
        if exe.endswith("/Contents/MacOS/Nunmai"):
...
def _install_rebuilt_desktop_app(desktop_dir: Path) -> tuple[list[Path], list[str]]:
    """Copy the rebuilt macOS bundle over every stale installed ``Nunmai.app`` (#52339).

    ``nunmai desktop --build-only`` (what ``nunmai update`` runs) packages into
    ...
    if rebuilt_exe is None:
        return [], []
    from nunmai_cli.gui_uninstall import packaged_gui_app_paths  # noqa: PLC0415
    # .../Nunmai.app/Contents/MacOS/Nunmai -> .../Nunmai.app
...
def _install_rebuilt_macos_bundles(
    ...
    for app in candidates:
        ...
        if app.resolve() in running:
            problems.append(
                f"{app} is running and was not refreshed; quit Nunmai Desktop and run "
                "`nunmai update` again (or update from inside the app)")
            continue
        tmp = app.parent / f"{app.name}.nunmai-update-new"
        old = app.parent / f"{app.name}.nunmai-update-old"
        shutil.rmtree(tmp, ignore_errors=True)
        shutil.rmtree(old, ignore_errors=True)
        try:
...
def _register_linux_desktop_entry(defer: bool = False):
    ...
    ``nunmai uninstall --gui`` removes it.
    ...
    from nunmai_cli.main import PROJECT_ROOT
    try:
        from nunmai_cli.linux_desktop_entry import DeferredDesktopEntryInstall, install_desktop_entry, is_supported
        if not is_supported():
            return None
        if defer:
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from nunmai_cli.main import PROJECT_ROOT
    from nunmai_cli.main_web_build import _run_npm_install_deterministic
    from nunmai_cli.update_cmd_deps import (
        DESKTOP_NPM_SCOPE, _clear_npm_lockfile_hash, _desktop_deps_changed, _record_npm_lockfile_hash)
    from nunmai_constants import get_default_nunmai_root, with_nunmai_node_path
    nunmai_root = get_default_nunmai_root()
    if not _desktop_deps_changed(nunmai_root) and (_electron_dir(PROJECT_ROOT) / "package.json").is_file():
        print("→ Desktop workspace dependencies unchanged, skipping install")
        return
    print("→ Installing desktop workspace dependencies...")
    _clear_npm_lockfile_hash(nunmai_root, DESKTOP_NPM_SCOPE)
    ...
    if install_result.returncode == 0:
        _record_npm_lockfile_hash(nunmai_root, DESKTOP_NPM_SCOPE)
        return
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Nunmai.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Nunmai.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Nunmai.exe locked on Windows, so the
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    for attr, key in (
        ("fake_boot", "NUNMAI_DESKTOP_BOOT_FAKE"), ("ignore_existing", "NUNMAI_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "nunmai_root", None):
        env["NUNMAI_DESKTOP_NUNMAI_ROOT"] = str(Path(args.nunmai_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["NUNMAI_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()
...
def cmd_gui(args: argparse.Namespace):
    ...
    from nunmai_cli.main import PROJECT_ROOT
    from nunmai_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    from nunmai_cli.linux_desktop_entry import launched_from_shell
    ...
    if not source_mode:
        desktop_launch_notice(f"→ Launching packaged Nunmai Desktop: {' '.join(launch_command)}")
    pass_fds: tuple[int, ...] = ()
