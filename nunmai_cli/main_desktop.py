...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Nunmai.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 nunmai-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
