"""Git plumbing for ``nunmai update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = "git remote add upstream https://github.com/Nunmai-Private-Limited/nunmai-engine.git"
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from nunmai_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old rescue refs (``refs/nunmai-update-backups/<kind>-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        ...
        for kind in ("orphan", "diverged"):
            prefix = f"refs/nunmai-update-backups/{kind}-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && nunmai update\n"
...
OFFICIAL_REPO_URLS = {
    ...
    "git@github.com:Nunmai-Private-Limited/nunmai-engine",
}
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from nunmai_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Nunmai repository.\n"
        "  This means you may miss updates from Nunmai-Private-Limited/nunmai-engine.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/Nunmai-Private-Limited/nunmai-engine.git")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from nunmai_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
_FETCH_FAILURE_RULES = (
    ...
    (lambda s: "Permission denied (publickey)" in s or "Host key verification failed" in s,
     ...
     " `origin` to HTTPS: `git remote set-url origin https://github.com/Nunmai-Private-Limited/nunmai-engine.git`."),
)
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped NUNMAI_HOME), then profile home as a fallback for custom layouts.

    The Nunmai-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped NUNMAI_HOME (``<root>/profiles/<name>``), so a profile-scoped ``nunmai update`` must look
    there (monerostar review, #87876).
    """
    from nunmai_cli.update_cmd import get_default_nunmai_root, get_nunmai_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_nunmai_root(), Path(get_nunmai_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from nunmai_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``nunmai update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``nunmai update``, which ships with the checkout
