...
A data-driven skin system that lets users (and Nunmai itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.nunmai/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.nunmai/skins/ therefore
...
.. code-block:: python
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.nunmai/skins/
...
Drop a YAML file in ``~/.nunmai/skins/<name>.yaml`` following the schema above.
