...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from nunmai_cli.main import _resolve_last_session
    target = (
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from nunmai_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    if not (ws_root / "node_modules" / "@nunmai" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/nunmai-ink/src")
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``NUNMAI_TUI_FORCE_BUILD=1`` forces a rebuild."""
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``NUNMAI_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    ...
    extras += [Path(nunmai_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``nunmai update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``nunmai update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _npm_lifecycle_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # the rebuild fails). ``$NUNMAI_HOME`` lives outside the git tree and the
    # update hand-off already carries ``NUNMAI_HOME`` down to every npm child.
    # An explicit ``NPM_CONFIG_USERCONFIG`` wins (#106373).
    from nunmai_constants import get_nunmai_home
    npmrc = get_nunmai_home() / "npmrc"
    if npmrc.is_file():
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``NUNMAI_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$NUNMAI_HOME/node`` tree a bare which() misses."""
    if bin == "node":
    ...
    if not path:
        print(
            ...
            "https://nodejs.org (run `nunmai doctor` for the install hint for your OS), then "
            "retry `nunmai --tui`. To keep working now, run `nunmai --cli`."
        )
        sys.exit(1)
    return path
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # nunmai_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   nunmai_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        ...
        # packages/nunmai-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "nunmai-ink", "TUI dev prebuild failed.")
...
def _launch_tui(
    ...
    from nunmai_cli.shared_session_attach import configure_tui_attachment
    try:
    ...
    for key, value in (
        ("NUNMAI_MODEL", model), ("NUNMAI_INFERENCE_MODEL", model),
        ("NUNMAI_TUI_PROVIDER", provider), ("NUNMAI_INFERENCE_PROVIDER", provider),
        ("NUNMAI_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("NUNMAI_TUI_SKILLS", skills_value),
        ("NUNMAI_TUI_QUERY", query), ("NUNMAI_TUI_IMAGE", image),
        ("NUNMAI_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("NUNMAI_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("NUNMAI_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("NUNMAI_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("NUNMAI_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # NUNMAI_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `nunmai --tui` try to resume a non-existent session; only forward the id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `nunmai update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``NUNMAI_KANBAN_BOARD`` so in-process tools and shelled-out
    ``nunmai kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``nunmai kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``nunmai kanban boards switch`` from another session can flip the file mid-turn, so the same
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.nunmai/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``NUNMAI_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``nunmai chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
