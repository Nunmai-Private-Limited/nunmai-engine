...
This module hosts the Nunmai billing/subscription methods lifted out of
``cli.py``'s ``NunmaiCLI`` class. ``NunmaiCLI`` inherits
...
Import discipline mirrors ``nunmai_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        """Print the Nunmai dollar balance block (two-bar view) when a Nunmai account
        ...
        model is unavailable. Agent-independent (a portal fetch gated on "a Nunmai
        ...
        wall-clock-bounded; honors NUNMAI_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Nunmai account is logged
        in (the balance block printed), since both commands are Nunmai-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Nunmai plan + browser hand-off.
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nunmai Portal.')}")
                print("  Run `nunmai portal` to log in, then /subscription.")
            return
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``nunmai_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `nunmai portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for Nunmai (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nunmai Portal.')}")
                print("  Run `nunmai portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Nunmai Engine page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Nunmai Engine page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Nunmai Research to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Nunmai Engine page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `nunmai portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        from nunmai_cli.nous_billing import BillingError, post_charge
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Nunmai Research to charge {card.masked} "
