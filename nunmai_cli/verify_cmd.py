"""``nunmai verify`` — detect a project's run recipe and smoke-test it.
...
``.nunmai/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``nunmai verify`` satisfies
