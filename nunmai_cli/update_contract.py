...
``nunmai update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/nunmai/image-provenance.json``,
   written by the image build — see :mod:`nunmai_cli.image_provenance`):
