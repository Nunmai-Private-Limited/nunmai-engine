"""Dependency-light venv recovery that runs BEFORE nunmai_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``nunmai_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``nunmai_cli.main`` to become importable again (force-reinstall of
...
# ``nunmai update`` renames the live ``nunmai*.exe`` shims aside (``nunmai.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``nunmai`` on PATH, and the command that would repair it IS ``nunmai update``. The
...
def _should_skip_external_secret_sources() -> bool:
    """True inside any ``nunmai update`` process (and its import probes).

    Every dotenv load in the process — ``nunmai_cli.main``, ``run_agent``, ``cli`` — consults
    ...
    Profile flags are stripped before ``nunmai_cli.main`` loads dotenv, so ``argv[1]`` is
...
# ``nunmai update`` writes this into the git dir right before git moves the checkout and removes it
...
# files already new; that mixed tree fails at import in every entry point, ``nunmai update`` included.
INTERRUPTED_PULL_MARKER = "nunmai-update-pull"
...
_RESTORE_CLAIM = "nunmai-update-pull.claim"
...
def _held_open(path: Path) -> bool:
    ...
    macOS/BSD have no portable check at all. The claim only orders Nunmai launches, so on those paths a
...
def restore_interrupted_pull(project_root: Path | None = None) -> bool:
    """Put back the files a killed ``nunmai update`` had half-moved to the new commit.
    ...
    for), so the install is whole again and ``nunmai update`` redoes the update from the start. Local
    ...
    Limits, by design: a torn ``nunmai_cli/__init__.py``, ``nunmai_bootstrap.py``, ``agent/__init__.py``
    or ``agent/jiter_preload.py`` (imported before the ``nunmai-engine`` hook) fails before this runs. A file git also changes that the user deleted, emptied or cut to a prefix of git's version
    ...
    try:
        ...
        with _restore_claim(marker.parent) as claimed:
            if not claimed:
                print("⚠ Another Nunmai launch is still repairing the checkout after an interrupted "
                      "`nunmai update`; if this one fails, launch again in a moment.", file=sys.stderr)
                return False
        ...
        print(f"⚠ Could not check for an interrupted `nunmai update`: {exc}", file=sys.stderr)
    return False
...
def _restore_holding_claim(root: Path, marker: Path) -> bool:
    ...
    if any((git_dir / name).exists() for name in _GIT_OPERATION_IN_PROGRESS):
        ...
        if (not _merge_advice_shown and merge_head.is_file()
            ...
            print(f"⚠ A killed `nunmai update` left its merge unfinished. Run `git -C {root} merge --abort`, "
    ...
    if not _release_dead_index_lock(git_dir):
        print("⚠ A running git holds the index after an interrupted `nunmai update`; the next launch "
    ...
    if restore or added:
        print("⚠ A previous `nunmai update` was killed while git was writing the new code — "
    ...
    print("  ✓ Checkout restored; `nunmai update` updates it again.", file=sys.stderr)
    if stash:
...
def relaunch_after_restore() -> None:
    ...
    Everything imported so far (this package, ``nunmai_bootstrap``, ``nunmai_cli.main`` itself) may
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which Nunmai
    vendors (``~/.nunmai/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".nunmai", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``nunmai acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``nunmai_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``nunmai_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls. Only acts when a recovery marker from a
    prior ``nunmai update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `nunmai update`: CLI and Desktop retries keep
...
# recovery: a persistently failing install (network down, a .pyd another Nunmai process maps) must
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``nunmai update`` (or
    ...
    venv extension mapped; this process, running before ``nunmai_cli.main``'s third-party imports, maps
    nothing yet, so the installer can replace ``.pyd`` files without hitting the lock.
    """
    try:
        from nunmai_cli import _install_repair as ir
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `nunmai update` was interrupted mid-install — "
            ...
            if new_attempts < _CORE_INSTALL_MAX_ATTEMPTS:
                print("  The next launch will retry; nunmai will keep working from "
