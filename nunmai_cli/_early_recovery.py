...
The ``nunmai`` console entry point is ``nunmai_cli.main:main``.  Importing
``nunmai_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``nunmai_cli.env_loader``, ``yaml`` via ``nunmai_cli.config``, ...).  In
...
a corrupted venv.  ``nunmai_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``nunmai_cli.main`` to become
...
#
# ``nunmai update`` renames the live ``nunmai*.exe`` shims aside
# (``nunmai.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``nunmai`` on PATH, and the command that would repair it IS ``nunmai
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY nunmai
    invocation, and ``nunmai acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Nunmai already vendors
    (``~/.nunmai/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".nunmai" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``nunmai acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``nunmai_cli.main``
...
def recover_if_needed(
    ...
    recovery marker from a prior ``nunmai update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `nunmai update`: CLI and Desktop retries preserve
...
# every `nunmai` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``nunmai update`` (or the self-lock preflight, #83569) left the
    ...
    ``nunmai_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``nunmai acp`` included).
    ...
    try:
        ...
        try:
            ...
            print(
                "  The next launch will retry; nunmai will keep working from "
