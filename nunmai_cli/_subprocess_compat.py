...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_NUNMAI_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Nunmai's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Nunmai shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_nunmai(text: str) -> bool:
    r"""True when *text* names Nunmai at a path-segment / token boundary.

    A bare ``"nunmai" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\snunmaia\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("nunmai", ".nunmai"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_nunmai(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Nunmai code."""
    try:
        import psutil
        ...
        return _text_names_nunmai(f"{command} {executable}")
    except Exception:
        return False


def pid_is_nunmai(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Nunmai process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_nunmai(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_nunmai`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``nunmai update`` on slow-WMI
    ...
    try:
        ...
        from nunmai_cli.local_runtime.processes import spawn_server
