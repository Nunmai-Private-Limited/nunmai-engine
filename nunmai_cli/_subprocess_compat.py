...
Nunmai is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `nunmai
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Nunmai' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Nunmai shells out to git from many non-interactive contexts — MCP catalog
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``nunmai update`` on slow-WMI machines
