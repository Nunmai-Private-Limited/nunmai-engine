"""Streamlined setup flows: the Nunmai Portal one-shot (`nunmai portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``nunmai_cli.setup`` take effect."""
...
logger = logging.getLogger("nunmai_cli.setup")
...
def _blank_slate_done(config: dict, nunmai_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from nunmai_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         nunmai skills opt-in --sync",
          "  Add MCP servers:     nunmai mcp add", *extra, "  Tune agent settings: nunmai setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from nunmai_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``nunmai model``. False when cancelled or failed (the
    message is already printed)."""
    from nunmai_cli.setup import _info
    try:
        from nunmai_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nunmai Portal setup (``nunmai setup --portal`` / ``nunmai portal``)."""
    from nunmai_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ Nunmai Setup — Nunmai Portal (one-shot)             │")
    _info(None, "  One subscription, 300+ models, plus the Tool Gateway:",
          "    web search, image generation, TTS, browser automation",
          "    — all routed through your Nunmai Portal sub.", None,
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        lead, *rest = provider_setup_failure_lines(exc, retry_command="nunmai portal")
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")

    if not _run_nous_flow(config, context="`nunmai portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `nunmai portal`."),
    ...
    _info("  Run `nunmai portal info` to inspect routing.", "  Run `nunmai` to start chatting.")


def _run_first_time_quick_setup(config: dict, nunmai_home, is_existing: bool):
    """Streamlined first-time setup via Nunmai Portal: OAuth, model, terminal & messaging;
    ...
    # Step 1: Nunmai Portal — OAuth login + model selection (provider set to "nous" by the save).
    print_header("Nunmai Portal", gap=True)
    ...
    _run_nous_flow(config, context="quick setup", cancel_exc=(KeyboardInterrupt, EOFError),
                   cancel_lines=(None, "Nunmai Portal setup cancelled."), print_error=_on_error)
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'nunmai setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `nunmai import`).
        from nunmai_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    nunmai setup")
    if gateway_choice != 0:
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Nunmai.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Nunmai update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``nunmai-engine`` skill
    ...
    try:
        ...
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "nunmai-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("nunmai-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``nunmai setup agent``."""
...
def _run_blank_slate_setup(config: dict, nunmai_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `nunmai-engine` skill is always kept so the agent",
          "can help you drive and configure Nunmai itself.", None)
    ...
    if path != 0:
        ...
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `nunmai update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, nunmai_home, "  Enable tools:        nunmai tools", "  Enable plugins:      nunmai plugins",
...
def _blank_slate_walkthrough(config: dict, nunmai_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `nunmai-engine`",
              "skill). A .no-bundled-skills marker keeps future",
              "`nunmai update` runs from re-injecting them. Opt back in any",
              "time with `nunmai skills opt-in --sync`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `nunmai plugins list` / `nunmai plugins install`.",
         "No plugins enabled. Add later with `nunmai plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `nunmai mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `nunmai mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, nunmai_home, "  Enable more tools:   nunmai tools")
...
def _run_quick_setup(config: dict, nunmai_home):
    ...
    from nunmai_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'nunmai setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Nunmai to messaging apps to chat from anywhere.",
              "You can configure these later with 'nunmai setup gateway'.")
