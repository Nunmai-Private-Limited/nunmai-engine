...
Split out of ``nunmai_cli/main.py``. Names that still live in main are imported lazily at call time.
"""

import contextlib

from typing import Optional
from nunmai_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nunmai/.env unless the active provider is 'custom' — a
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`nunmai_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Nunmai only falls back to a lightweight backend (OpenRouter,",
             "  Nunmai Portal) if the main model is unavailable.  Override a",
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Nunmai URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_main_reasoning_effort(model: str, provider: str) -> None:
    """The effort step every ``nunmai model`` flow shares: after a main-model pick, offer the
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "", existing_source: str = "") -> tuple:
    """API-key entry for ``nunmai setup`` / ``nunmai model``: first-time entry, or [K]eep / [R]eplace /
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `nunmai setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from nunmai_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from nunmai_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            print(f"    Nunmai will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from nunmai_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``nunmai model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from nunmai_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from nunmai_cli.models_catalog_static import group_providers, provider_group_for_slug
