...
class CLIChatTurnMixin:
    ...
    def _chat_run_agent(self, turn, message):
        ...
        try:
            ...
            from nunmai_cli.cli_chat_error_copy import chat_error_response
            turn.result = {
    ...
    def _chat_monitor_agent_thread(self, turn, agent_thread):
        ...
        from cli import _cprint, _nunmai_home, logger
    ...
    def _chat_print_response_panel(self, turn, response):
        ...
        if response and not (turn.result and turn.result.get("response_previewed", False)):
            ...
            # Billing CTA pins the single action (Nunmai → /topup, others → billing page) so it
            ...
            if turn.result and turn.result.get("failure_reason") == "billing":
                ...
                try:
                    ChatConsole().print(Panel(
                        "\n".join(_cta_lines), title="[#0B6B4F bold]⚡ Out of credits[/]",
                        title_align="left", border_style="#0B6B4F", box=rich_box.HORIZONTALS,
