...
def _cmd_list(args) -> int:
    ...
    if args.installed:
        ...
        if not pets:
            print("No pets installed. Try: nunmai pets install boba")
            return 0
...
def _cmd_install(args) -> int:
    ...
    if args.select or not _has_active_pet():
        ...
        print(f"  Make it active with: nunmai pets select {slug}")
    return 0
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        print("  → no pets installed. Run: nunmai pets install boba")
        ok = False
    elif active is None:
        print("  → active pet unresolved. Run: nunmai pets select <slug>")
        ok = False
    elif not enabled:
        print("  → pet display is disabled. Run: nunmai pets select " + active.slug)
