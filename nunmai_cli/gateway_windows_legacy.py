...
``Nunmai_Gateway``, the Startup entry ``Nunmai_Gateway.vbs``/``.cmd`` and the launcher pair
``gateway-service\\Nunmai_Gateway.{vbs,cmd}``. Every operation in ``gateway_windows`` is keyed on
``get_task_name()`` → ``Nunmai_Gateway_<suffix>``, so those strays are never queried, rewritten,
reported or removed: they keep launching a second gateway at logon, never pick up launcher
fixes, and ``nunmai gateway status`` prints ✓ while they do (#116157). This module enumerates
...
def _w():
    import nunmai_cli.gateway_windows as gateway_windows  # facade binding is the seam for tests
...
def _targets_this_home(text: str) -> bool:
    ...
    dir. The bare name is also the live identity of the default ``~/.nunmai`` profile, so a secondary
    profile that classified every ``Nunmai_Gateway`` object as its own stray would delete the default
    profile's gateway autostart on ``uninstall`` / ``install --force``."""
    w = _w()
    marker = w._normalize_windows_path(str(w._nunmai_home() / "gateway-service"))
...
def legacy_launcher_artifacts() -> list[tuple[str, str, Path | str]]:
    ...
    service_dir = w._nunmai_home() / "gateway-service"
    for path, label in (
...
def warn_legacy_launchers() -> bool:
    """``nunmai gateway status``: name every stray so nobody has to do file-mtime archaeology."""
    ...
    if artifacts:
        ...
        print("  Remove them with: nunmai gateway uninstall   (then: nunmai gateway install)")
