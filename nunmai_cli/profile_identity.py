...
def migrate_profile_identity(old_name: str, new_name: str) -> bool:
    ...
    ``nunmai profile migrate-identity <old> <new>`` for when that attempt failed. The rename
    ...
    """
    from nunmai_cli.profiles import _canon_valid, _live_default_multiplexer, _unknown_profile_error, get_profile_dir
...
def purge_profile_identity(profile: str) -> bool:
    ...
    """
    from nunmai_cli.profiles import _canon_valid, _live_default_multiplexer, profile_exists
...
def _purge_profile_identity(canon: str, live_mux: bool) -> bool:
    ...
    if live_mux:
        ...
        print(
            ...
            f"    nunmai profile purge-identity {canon}",
            file=sys.stderr)
        return False
...
def _migrate_checkpoint_identity(old_canon: str, new_canon: str) -> bool:
    ...
    print(
        ...
        f"    nunmai profile migrate-identity {old_canon} {new_canon}",
        file=sys.stderr,
    )
    return False
