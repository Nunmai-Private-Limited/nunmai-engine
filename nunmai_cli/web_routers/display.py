...
from nunmai_cli.web_server_chat import _ws_request_is_allowed
...
def _consume_display_ticket(ws: WebSocket) -> Optional[dict]:
    from nunmai_cli.dashboard_auth.ws_tickets import TicketInvalid, consume_ticket
    ...
    try:
        ...
        return None
    if info.get("provider") != "bot-desktop" or not info.get("nunmai_home"):
        return None
    return info
...
async def _bridge(ws: WebSocket, info: dict) -> None:
    ...
    sock = Path(info["nunmai_home"]) / "bot-desktop" / "rfb.sock"
    profile_home = str(info["nunmai_home"])
    profile_key = nunmai_home_key(profile_home)
