...
from nunmai_cli.config import redact_key
from nunmai_cli.web_deps import late
from nunmai_cli.web_server_files import _path_is_under
from nunmai_cli.web_server_gateway import _restart_gateway_after
from nunmai_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from nunmai_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("nunmai_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "nunmai_cli.web_server_memory")
...
save_config = late("save_config", "nunmai_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str,
                  profile: Optional[str] = None) -> dict:
    """Spawn a ``nunmai -p <profile> <argv>`` action; spawn failure -> 500.
...
# --- Webhooks: same JSON store as the CLI (nunmai_cli.webhook); the adapter
...
# --- Gateway lifecycle: spawn the real `nunmai gateway <verb>` so behaviour
...
async def start_gateway(profile: Optional[str] = None):
    from nunmai_cli.web_server_gateway import multiplexed_profile_refusal
    # The spawned `nunmai -p X gateway start` would refuse with exit 78 into an action log nobody reads;
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``nunmai auth add openai-codex``
            ...
            # (mirrors `nunmai auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from nunmai_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int, profile: Optional[str] = None):
    ...
    the same RemovalStep registry as ``nunmai auth remove``: each source cleans
...
async def run_backup(body: BackupRequest, profile: Optional[str] = None):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir(profile) / f"nunmai-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def create_hook(body: HookCreate, profile: Optional[str] = None):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from nunmai_cli.plugins import VALID_HOOKS as valid_hooks
