...
from nunmai_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
...
def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``nunmai <argv>`` action; spawn failure -> 500."""
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"nunmai-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
