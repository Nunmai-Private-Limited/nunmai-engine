...
from nunmai_cli.web_routers._common import config_scoped_to_thread
from nunmai_cli.web_server_dashboard import (
...
async def get_dashboard_themes(profile: Optional[str] = None):
    ...
    from `~/.nunmai/dashboard-themes/*.yaml` ship their normalised `definition`."""
    def _run():
        config = load_config()
...
async def get_plugins_catalog(request: Request):
    ...
    def _run():
        from nunmai_cli.plugins_cmd import _discover_all_plugins, _get_disabled_set, _get_enabled_set
        from nunmai_cli.plugins_cmd_catalog import installed_catalog_state
        from nunmai_cli.web_server_dashboard import _plugin_runtime_status
...
async def post_agent_plugin_activate(request: Request):
    """``nunmai plugins install`` / ``enable`` in another process asks this backend to load the plugin
    for ``home`` and hand its MCP servers and skills to that profile's open chats
    (``nunmai_cli.plugins_activation.load_and_go_live``). ``home`` must be a profile this host serves."""
    _require_token(request)
    try:
        body = await request.json()
    except Exception:
    ...
    from nunmai_constants import get_nunmai_home, nunmai_home_key, profile_name_for_home
    home = Path(str((body or {}).get("home") or "")).expanduser()
    profile = profile_name_for_home(home) if str(home) not in ("", ".") else None
    if profile is None:
        raise HTTPException(status_code=400, detail="Not a Nunmai profile home.")
    from nunmai_cli.plugins_activation import load_and_go_live

    def _run():
        with _config_profile_scope(None if profile == "default" else profile):
            if nunmai_home_key(get_nunmai_home()) != nunmai_home_key(home):
...
async def put_plugin_providers(request: Request, body: _PluginProvidersPutBody,
    ...
    _require_token(request)
    from nunmai_cli.plugins_cmd import _save_context_engine, _save_memory_provider
