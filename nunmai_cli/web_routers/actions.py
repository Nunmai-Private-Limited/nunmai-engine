"""Gateway restart/drain, Nunmai update and background-action status dashboard routes.

Extracted from ``nunmai_cli.web_server``; helpers/state that tests monkeypatch on
...
from nunmai_cli.config import format_docker_update_message, recommended_update_command_for_method
from nunmai_cli.web_deps import LateState, late
from nunmai_cli.web_server_gateway import _ACTION_LOG_FILES
from nunmai_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "nunmai_cli.config")
...
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "nunmai_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "nunmai_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "nunmai_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "nunmai_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import nunmai_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== nunmai-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Nunmai updates are managed outside this dashboard in containerized environments."
...
async def gateway_migrate_plan():
    ...
    from nunmai_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``nunmai gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from nunmai_cli.web_server_gateway import _spawn_nunmai_action
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_nunmai_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    ...
    return {
        "ok": False, "pid": None, "name": "nunmai-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/nunmai/update")
async def update_nunmai():
    ...
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "nunmai-update", "already_running": True}
    ...
    with http_failure("Failed to spawn nunmai update", 500, "Failed to start update"):
        proc = _spawn_nunmai_action(["update"], "nunmai-update", env_overrides={"NUNMAI_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "nunmai-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Nunmai is managed by Termux APT; run `pkg upgrade nunmai-engine`.",
}


@router.get("/api/nunmai/update/check")
async def check_nunmai_update(force: bool = False, profile: Optional[str] = None):
    ...
    try:
        from nunmai_cli.banner import check_for_updates, upstream_commits_behind
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``nunmai update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``nunmai update`` run since #91283, including
...
@status_router.get("/api/nunmai/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `nunmai update` run recorded).")
