...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "nunmai_cli.web_server_oauth")
...
_nous_plain_poller = late("_nous_plain_poller", "nunmai_cli.web_server_oauth")
_nous_promotion_poller = late("_nous_promotion_poller", "nunmai_cli.web_server_oauth")
_minimax_poller = late("_minimax_poller", "nunmai_cli.web_server_oauth")
_xai_device_poller = late("_xai_device_poller", "nunmai_cli.web_server_oauth")
...
def _codex_client(httpx) -> Any:
    ...
    """
    from nunmai_cli.auth_codex import _cap_codex_response_body
...
def _codex_post(httpx, url: str, **kwargs: Any) -> Any:
    ...
    """
    from nunmai_cli.auth_codex import _is_transient_transport_error
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from nunmai_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from nunmai_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
_PROVIDER_STATUS: Dict[str, tuple[str, Callable[[dict], dict]]] = {
    "nous": ("get_nous_auth_status_local", lambda r: {**_status_card(
        r, "nous_portal", r.get("portal_base_url") or "Nunmai Portal",
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    """Start a Nunmai sign-in. Over a free-tier identity (``nous.guest`` on) the whole sign-in is the
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from nunmai_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["nous"]
    portal_base_url = (
        os.getenv("NUNMAI_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Nunmai still
    # OWNS its credential (the PKCE file ~/.nunmai/.anthropic_oauth.json and its
    # credential-pool entry, written by `nunmai auth add anthropic`), so it is
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``nunmai model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"nunmai auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Nunmai-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_nunmai_oauth_file
...
async def poll_oauth_session(provider_id: str, session_id: str, profile: Optional[str] = None):
    ...
    return {
        ...
        # Nunmai over a free-tier identity: why a transfer ended, who signed in, and the default model
