...
Extracted from ``nunmai_cli.web_server``; app state and helpers are late-bound through
:mod:`nunmai_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from nunmai_cli.web_deps import LateState, late
from nunmai_cli.web_server_gateway import _display_system_platform
...
from nunmai_cli.config import get_config_path, get_env_path
from nunmai_constants import get_process_nunmai_home, profile_name_for_home
from nunmai_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from nunmai_cli.web_routers._common import config_scoped_to_thread, destructive_profile, scoped_to_thread
from pathlib import Path
from typing import Any, Dict, Optional

_log = logging.getLogger("nunmai_cli.web_server")
router = APIRouter()
# Mounted separately by web_server so /api/logs keeps its original route-table position.
logs_router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "nunmai_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "nunmai_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "nunmai_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "nunmai_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "nunmai_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "nunmai_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "nunmai_cli.web_server_lifecycle")
_spawn_nunmai_action = late("_spawn_nunmai_action", "nunmai_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "nunmai_cli.config")
get_nunmai_home = late("get_nunmai_home", "nunmai_cli.config")
_profile_cli_args = late("_profile_cli_args", "nunmai_cli.web_server_profiles")
...
load_config = late("load_config", "nunmai_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "nunmai_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Nunmai process uses it."""
...
async def get_ssh_ownership(request: Request):
    from nunmai_cli.web_server import _SSH_OWNER_NONCE
...
async def get_host_identity(request: Request):
    """Prove to an attaching `nunmai serve`/`dashboard` WHO owns this port.
    ...
    headless ``serve``, so a `nunmai dashboard` user is never routed to a backend with no UI.
    """
    _require_token(request)
    # ``role`` is the host ROLE this process published (gateway/host_rendezvous.ROLE_SERVE, or
    # ROLE_DESKTOP_SERVE for a Desktop-owned child), not the launch mode: `nunmai serve` and
    # `nunmai dashboard` are one host role that differ in SPA.
...
async def get_health_idle(request: Request):
    ...
    from nunmai_cli.web_server_idle_proof import idle_proof
    _require_token(request)
...
# Profile segment mirrors nunmai_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level NUNMAI_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``nunmai --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_nunmai_home())
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            # as ``nunmai gateway status`` so the sidebar strip and the CLI agree.
...
def _nous_session_validity() -> str:
    """Nunmai bootstrap-session validity for the NAS health sweep: a hosted agent whose Nunmai
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from nunmai_cli.web_server import DASHBOARD_HEALTH
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from nunmai_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of nunmai_cli.gateway blocks
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_nunmai_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Nunmai Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "nunmai_home": str(get_nunmai_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond nunmai home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "nunmai_version": __version__, "cpu_count": os.cpu_count()}
...
async def set_curator_paused(body: CuratorPause, profile: Optional[str] = None):
    from agent import curator
    # ``_state_file()`` is ``get_nunmai_home()/skills/.curator_state`` resolved at call
...
def _spawn_action(argv: list, name: str, prefix: str, profile: Optional[str] = None) -> dict:
    """Spawn a background ``nunmai -p <profile> <argv>`` action; a spawn failure is
    ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_nunmai_action(_profile_cli_args(profile) + argv, name)
    except Exception as exc:
...
# Portal — Nunmai Portal auth + Tool Gateway routing status (read-only).
