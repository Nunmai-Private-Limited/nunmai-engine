...
from nunmai_cli.web_routers._common import scoped_to_thread
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``nunmai <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_nunmai_action(argv, name)
    except Exception as exc:
