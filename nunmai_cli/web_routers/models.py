...
from nunmai_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
from nunmai_cli.web_routers._common import _CONFIG_MUTATION_LOCK, config_write_scope, http_failure
...
def get_recommended_default_model(provider: str = "", profile: Optional[str] = None):
    ...
    ``nunmai model``'s curation so GUI onboarding lands on a sensible default.
    Nunmai honors the user's free/paid tier. Any other provider gets the preferred
    ...
    Nunmai; ``model`` may be empty (caller degrades gracefully)."""
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.nunmai/config.yaml`` — applies to **new** sessions only; a running chat
