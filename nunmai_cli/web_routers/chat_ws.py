...
from nunmai_cli.pty_session import RegistryFull
from nunmai_cli.web_deps import LateState, late
from nunmai_cli.web_routers.chat_ws_errors import chat_start_failure_message
from nunmai_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "nunmai_cli.web_server_chat")
_profile_scope = late("_profile_scope", "nunmai_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "nunmai_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "nunmai_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "nunmai_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "nunmai_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "nunmai_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from nunmai_cli.web_server_chat import _LOOPBACK_HOSTS
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Nunmai Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from nunmai_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    ...
    if not resume:
        from nunmai_cli.web_routers.chat_workspaces import resolve_chat_cwd
        try:
...
async def gateway_ws(ws: WebSocket) -> None:
    if not await _close_unless_sidecar_allowed(ws):
        return
    from nunmai_cli.mcp_startup import start_deferred_mcp_discovery_now
...
# (NUNMAI_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
