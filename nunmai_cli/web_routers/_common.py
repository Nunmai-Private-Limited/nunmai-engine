...
late-binding seam in :mod:`nunmai_cli.web_deps` (web_server owns helpers/state;
...
log = logging.getLogger("nunmai_cli.web_server")
...
def destructive_profile(profile: Optional[str], route: str) -> Optional[str]:
    ...
    and `curl` against a plain ``nunmai serve`` is unchanged.
...
def spawn_profile_action(
    ...
    """Spawn a background ``nunmai -p <profile> <argv>`` action; a spawn
...
# instead of a 500. The file is never quarantined or renamed from here — that is `nunmai doctor`'s job.
...
CORRUPT_STORE_DETAIL = {
    ...
    "message": "state.db corrupt — run `nunmai doctor` (then `nunmai doctor --fix` or `nunmai sessions repair`).",
}
...
def corrupt_store_as_status(db_path):
    ...
    from nunmai_state_errors import is_malformed_db_error
    ...
    try:
        ...
        if last is None or now - last >= _CORRUPT_STORE_WARN_INTERVAL_S:
            ...
            log.warning("state.db at %s is corrupt (%s); dashboard reads return a status payload until it is "
                        "repaired — run `nunmai doctor`", db_path, exc)
        else:
