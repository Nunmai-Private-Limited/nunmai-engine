...
from nunmai_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, require_readable_config_before_write, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, get_compatible_custom_providers, _ENV_REF_RE, _deep_merge
from nunmai_cli.config_providers import _canonical_api_mode, _custom_provider_entry_to_provider_config
from nunmai_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "nunmai_cli.web_server_messaging")
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``nunmai model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `nunmai model` picker uses.
...
def _write_custom_endpoint(cfg: Dict[str, Any], body: CustomEndpointUpdate) -> Tuple[str, Dict[str, Any]]:
    ...
    # ``nunmai model`` does (``api_mode``; the runtime also reads the v12
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level NUNMAI_HOME (mirrors ``/api/config``).
    """
