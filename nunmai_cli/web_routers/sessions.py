...
from nunmai_cli.web_server_sessions import _maybe_auto_archive_for_profile, _session_latest_descendant
from nunmai_cli.web_models import (
    BulkDeleteSessions, SessionImport, SessionOwnerBackfill, SessionPrune, SessionRename)
from nunmai_cli.web_routers._common import CORRUPT_STORE_DETAIL, log as _log, destructive_profile, http_failure
from nunmai_state import is_malformed_db_error
from nunmai_state_errors import is_transient_sqlite_error
...
_cron_default_profile = late("_cron_default_profile", "nunmai_cli.web_server_cron")
...
_session_db_path_for_profile = late("_session_db_path_for_profile", "nunmai_cli.web_server_sessions")
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics (mirrors `nunmai sessions stats`)."""
    def _stats(db):
        out = {
...
def _history_profile_home(profile):
    ...
    # An omitted profile reads this process's DB (including custom NUNMAI_HOME),
...
async def get_session_timeline(
    ...
    """
    from nunmai_state_timeline import get_session_timeline as read_timeline
...
async def get_session_messages_around(
    ...
    from nunmai_state_timeline import get_session_messages_around as read_around
