...
from nunmai_cli.config import OPTIONAL_ENV_VARS, get_env_path
...
from nunmai_cli.web_server_messaging import (
...
_profile_scope = late("_profile_scope", "nunmai_cli.web_server_profiles")
_resolve_profile_dir = late("_resolve_profile_dir", "nunmai_cli.web_server_profiles")
_restart_gateway_after_whatsapp_onboarding = late("_restart_gateway_after_whatsapp_onboarding", "nunmai_cli.web_server_messaging")
_telegram_onboarding_request_sync = late("_telegram_onboarding_request_sync", "nunmai_cli.web_server_messaging")
_whatsapp_session_path = late("_whatsapp_session_path", "nunmai_cli.web_server_messaging")
_write_platform_enabled = late("_write_platform_enabled", "nunmai_cli.web_server_messaging")
load_env = late("load_env", "nunmai_cli.config")
load_config = late("load_config", "nunmai_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
remove_env_value = late("remove_env_value", "nunmai_cli.config")
save_env_value = late("save_env_value", "nunmai_cli.config")
_gateway_subcommand = late("_gateway_subcommand", "nunmai_cli.web_server_gateway")
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    for key, description, prompt, extra in (
        ...
        ("WEIXIN_ACCOUNT_ID", "iLink Bot account ID obtained through QR login in nunmai gateway setup", "iLink Bot account ID", {}),
        ("WEIXIN_TOKEN", "iLink Bot token obtained through QR login in nunmai gateway setup", "iLink Bot token", {"password": True}),
...
def _messaging_platform_payload(
    ...
    # and do NOT follow the NUNMAI_HOME contextvar override, so without it messaging
    ...
    if not enabled:
        ...
        # Same verdict /api/status gives: ``nunmai gateway stop`` keeps the last failure on disk,
...
def _platform_payloads(scoped_dir: Optional[Path], entries) -> list[dict[str, Any]]:
    ...
    NUNMAI_HOME contextvar; the gateway status readers do not, hence the explicit path)."""
    ...
    # ``nunmai --profile X serve``); the default home resolves to a name the multiplexer never serves.
    own_home = scoped_dir if scoped_dir is not None else get_process_nunmai_home()
    if (
        runtime is None
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env=with_nunmai_node_path(), creationflags=windows_hide_flags(),
        )
...
async def apply_telegram_onboarding(pairing_id: str, body: TelegramOnboardingApply, profile: Optional[str] = None):
    ...
    # saved token waiting on a manual restart click reads as "Nunmai is broken" from the
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    def _run():
        ...
        # credentials/state, not the root install's. load_env() honors the NUNMAI_HOME contextvar override;
        ...
        with _profile_scope(profile) as scoped_dir:
            return {
                ...
                "gateway_start_command": " ".join(["nunmai", *_gateway_subcommand(profile, "start")]),
...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (unrecognized NUNMAI_HOME) is outside
    ...
    from nunmai_cli.gateway_multiplex_mode import default_gateway_multiplexes
...
def _notify_multiplexer_hot_serve(profile: Optional[str]) -> bool:
    ...
    ``nunmai --profile X serve`` without the query (#109088), so X must resolve here too."""
    from nunmai_cli.gateway import _current_profile_name, named_profile_served_by_running_multiplexer
