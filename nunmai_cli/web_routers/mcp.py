...
web_server - reached via the late-binding seam in :mod:`nunmai_cli.web_deps`
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the NUNMAI_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_nunmai_home().expanduser().resolve(strict=False))

    def _read():
        with _profile_scope(profile):
            return _get_mcp_servers(), str(get_nunmai_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.nunmai_home == flow_home
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `nunmai mcp catalog` / `nunmai mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds NUNMAI_HOME-derived paths in the child.
