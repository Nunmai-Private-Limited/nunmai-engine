...
Shared helpers are reached via the late-binding seam in :mod:`nunmai_cli.web_deps`
...
import copy
import functools
from nunmai_cli.web_read_coalescing import coalesced_read
import inspect
import json
import logging
import re
...
from nunmai_cli.config import get_process_nunmai_home
from nunmai_cli.profiles import ProfileIdentitySettlementPending
from nunmai_cli.web_server_config import (
    _apply_main_model_assignment, _normalize_main_model_assignment, _validated_main_model_selection,
)
from nunmai_cli.web_server_gateway import _strip_session_list_rows
from nunmai_cli.web_routers._common import _CONFIG_MUTATION_LOCK
from nunmai_cli.web_server_profiles import (
...
from nunmai_state_health import STORAGE_CORRUPT, note_storage_error, storage_state
...
from nunmai_cli.web_models import (
    ...
    SessionPrScanBody)
from nunmai_cli.web_server_profiles import _config_profile_scope, _nunmai_home_scope
...
_cron_profile_home = late("_cron_profile_home", "nunmai_cli.web_server_cron")
...
def _scope_profile_name(path: Path) -> Optional[str]:
    ...
    ``"default"`` for the default root (its basename -- ``.nunmai`` or a custom root -- is not a
    ...
    if resolved == get_process_nunmai_home().resolve():
        return None
    if resolved == get_default_nunmai_root().resolve():
        return "default"
    return path.name


def _write_profile_model(profile_dir: Path, provider: str, model: str, validate_in: Optional[Path] = None) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (NUNMAI_HOME-scoped)
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    with _nunmai_home_scope(profile_dir), _CONFIG_MUTATION_LOCK:  # RMW span
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map nunmai_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
def _corrupt_profile_stores(targets) -> Dict[str, str]:
    ...
    as structurally corrupt (``nunmai_state_health``). Lets Desktop tell an empty or partial list
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _set_model() -> bool:
        ...
        try:
            _write_profile_model(path, provider, model, validate_in=get_process_nunmai_home())
    ...
    def _spawn_install(ident: str):
        return _spawn_nunmai_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``nunmai profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``nunmai profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the NUNMAI_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``nunmai profile
    ...
    def _run():
        from nunmai_cli import profile_describer
...
# ── Export / Import ── wraps nunmai_cli.profiles.export_profile / import_profile. Paths are
