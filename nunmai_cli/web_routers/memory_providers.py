...
from nunmai_cli.web_server_memory import (
...
from nunmai_cli.web_models import MemoryProviderConfigUpdate, MemoryProviderSetupRequest
from nunmai_cli.web_routers._common import _CONFIG_MUTATION_LOCK, scoped_to_thread
...
_dependency_importable = late("_dependency_importable", "nunmai_cli.web_server_memory")
...
def _honcho_resolvers(name: str):
    ...
    installed (bundled or ``$NUNMAI_HOME/plugins/``)."""
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # read-only and installs must go to NUNMAI_LAZY_INSTALL_TARGET, which
