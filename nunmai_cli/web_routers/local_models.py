...
from nunmai_cli import config as config_mod, web_deps
from nunmai_cli.web_routers._common import _CONFIG_MUTATION_LOCK, _config_profile_scope
from nunmai_cli.local_runtime import (
...
def _assign_default(job: Dict[str, Any], model_id: str) -> None:
    ...
    web_deps.late("_apply_model_assignment_sync", "nunmai_cli.web_server_config")("main", "llamacpp", model_id, "", "", "")
...
async def local_models_runtime_install(body: RuntimeInstallBody, profile: Optional[str] = None):
    ...
    def _run():
        ...
        # launch presets under get_nunmai_home() — scope so those land in the named profile.
...
def _terminate_state_pid() -> None:
    ...
    from nunmai_cli.local_runtime.recovery import stop_recorded_orphan

    if not stop_recorded_orphan():
        raise HTTPException(status_code=409, detail=(
            "Another Nunmai process owns this server, or its ownership could not be verified"))
...
async def local_models_sideload(body: SideloadBody):
    ...
    impossible) and bounce the router. The original stays put; delete-from-Nunmai removes only our link."""
    src = Path(body.path)
