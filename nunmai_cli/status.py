...
from nunmai_cli.status_auth import (  # renderers wired into _SECTIONS below
...
def _estop_status_line():
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `nunmai resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'nunmai-engine[vercel]')")
        ...
        # (fail-soft — never break `nunmai status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from nunmai_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "nunmai gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from nunmai_cli.gateway_multiplex_served import served_profile_ingress_urls
...
def _render_sessions(ctx):
    ...
    elif not (sessions_file := get_nunmai_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'nunmai doctor' for detailed diagnostics", "  Run 'nunmai setup' to configure"),
            Colors.DIM)
    print()


# Print order of `nunmai status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    ...
    # Shared by section renderers: config, --deep, and the Nunmai login facts Auth Providers derives
    # for the later Nunmai Tool Gateway section.
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('nunmai_cli.nous_account', 'get_nous_portal_account_info'),
