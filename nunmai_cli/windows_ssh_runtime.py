...
def _root() -> Path:
    ...
    # NUNMAI_HOME) cannot move the reader away from the helper's token.
...
def process_state(pid: int, creation_time_ns: int, nunmai_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    # argv[0] is the nunmai exe or (normally) the base Python, whose path varies by venv/uv
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    process that both owns the port and is the one we lock. nunmai_cli's parent is prepended
