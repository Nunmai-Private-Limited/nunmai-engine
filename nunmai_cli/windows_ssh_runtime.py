...
def _root() -> Path:
    ...
    # helper's token, including custom NUNMAI_HOME layouts.
    return get_default_nunmai_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, nunmai_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(nunmai_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the nunmai exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs nunmai_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "nunmai_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "nunmai_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, nunmai_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, nunmai_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves nunmai_cli's own location and prepends its parent, because the
    ...
    # the nunmai_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the nunmai_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["nunmaiPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Nunmai path must be absolute")
    nunmai_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(nunmai_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Nunmai Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('nunmai_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_nunmai(nunmai_path: str) -> dict[str, Any]:
    path = os.path.abspath(nunmai_path)
    if not os.path.isabs(nunmai_path) or not os.path.isfile(path):
        raise ValueError("Nunmai path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "nunmaiHome": str(get_default_nunmai_root()), "python": sys.executable}
