...
Usage:
    ...
    nunmai chat / gateway / setup / status / cron / doctor / update / ...
    nunmai --version           # Show version and update status
    nunmai <cmd> --help        # Per-command help
"""

# nunmai_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``nunmai update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``nunmai update``.
try:
    import nunmai_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass

# A `nunmai update` killed while git was writing the new tree leaves a mix of old and new files that
...
# dir, so if IT can't import nothing in nunmai_cli can.
...
# Inline path math so ``python nunmai_cli/main.py`` (script mode: sys.path[0]
# is nunmai_cli/, not the repo root) can import nunmai_cli._startup_fast.
...
# A literal ``~``/``$VAR`` in NUNMAI_HOME (fish, or any quoted value) must become absolute
# before the first reader — otherwise it resolves against cwd and scaffolds <cwd>/~/.nunmai.
_startup_fast.normalize_nunmai_home_env()

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``nunmai update`` may have left a recovery marker with a core package wiped;
# the nunmai_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``nunmai_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from nunmai_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke nunmai directly makes
    nunmai itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[nunmai] WARNING: this process is PID 1 with no init above it "
...
def _set_process_title() -> None:
    """Cosmetic: show 'nunmai' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``nunmai.exe``). Never fatal.
    """
    try:
...
# nunmai_cli.config is importable. Cached per config path so early callers
...
# sets NUNMAI_HOME, and a cache keyed on nothing pinned every later caller to
...
def _early_interface_config_path() -> str:
    ...
    return os.path.join(os.path.expanduser("~"), ".nunmai", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``NUNMAI_TUI=1``, then a
...
# `resetTerminalModes()` is the later cousin. ``NUNMAI_TUI_NO_EARLY_DISABLE``
...
# ``nunmai --version`` is answered before config/logging imports.
...
from nunmai_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
...
from nunmai_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
...
from nunmai_cli.subcommands.usage import build_usage_parser
...
from nunmai_cli.subcommands.secrets import build_secrets_parser
from nunmai_cli.subcommands.codex_runtime import build_codex_runtime_parser
from nunmai_cli.subcommands.egress import build_egress_parser
from nunmai_cli.subcommands.migrate import build_migrate_parser
from nunmai_cli.subcommands.checkpoints import build_checkpoints_parser
from nunmai_cli.subcommands.bundles import build_bundles_parser
from nunmai_cli.subcommands.curator import build_curator_parser
from nunmai_cli.subcommands.pets import build_pets_parser
from nunmai_cli.subcommands.journey import build_journey_parser
from nunmai_cli.subcommands.computer_use import build_computer_use_parser
from nunmai_cli.subcommands.sessions import build_sessions_parser
from nunmai_cli.subcommands.completion import build_completion_parser
...
# Profile override — MUST happen before any nunmai module import: many modules
# cache NUNMAI_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# NUNMAI_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.nunmai/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors nunmai_cli.profiles._PROFILE_ID_RE
...
def _looks_like_nunmai_invocation() -> bool:
    """False when ``sys.argv`` belongs to a test runner rather than a ``nunmai`` run.

    pytest's own ``-p no:xdist`` reaches ``_scan_profile_flag`` through ``sys.argv`` at import
    time; it must stay a silent skip, while a real ``nunmai -p 'Work Bot'`` must fail loudly.
    """
    return "pytest" not in (sys.argv[0] or "")


def _exit_invalid_profile_name(value: str) -> None:
    from nunmai_cli.profiles import _invalid_profile_name_error

    print(f"Error: {_invalid_profile_name_error(value)}", file=sys.stderr)
    print("Run `nunmai profile list` to see your profiles.", file=sys.stderr)
    sys.exit(2)
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`nunmai chat -p
    ...
    ``nunmai`` run — after a subcommand, ``-p`` may belong to that subcommand or a
    plugin (`nunmai kanban ... -p 8080`), and option-looking values (``no:xdist``,
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_nunmai_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    if name == "default":
        return None
    from nunmai_constants import named_profile_is_live, sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin NUNMAI_HOME to the profile dir; a bare invocation
    means "the root NUNMAI_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``nunmai profile use``) would silently redirect the default gateway into
    ...
    NUNMAI_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    NUNMAI_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), NUNMAI_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("NUNMAI_SUPERVISED_CHILD") or os.environ.get("NUNMAI_S6_SUPERVISED_CHILD"):
        return True
...
def _s6_supervised_gateway_run(argv: list) -> bool:
    ...
    if words[:2] != ["gateway", "run"] or "--no-supervise" in argv:
        return False
    if os.environ.get("NUNMAI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return False
    from nunmai_cli.service_manager import _s6_running
    return _s6_running()
...
def _apply_profile_override() -> None:
    ...
    # NUNMAI_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the nunmai root (systemd hardcodes NUNMAI_HOME=/root/.nunmai)
    # we must still read active_profile — the user may have run
    # `nunmai profile use` and the gateway should honour it (#22502).
    nunmai_home_env = os.environ.get("NUNMAI_HOME", "")
    if profile_name is None and nunmai_home_env and Path(nunmai_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and nunmai_home_env and os.environ.get("NUNMAI_UPDATE_POST_SWAP") == "1":
        return
...
# ``-p``/active_profile re-homed the process after nunmai_bootstrap ran: re-point the temp vars
# at THIS home's scratch dir (a user-set TMPDIR is still left alone).
try:
    from nunmai_constants import export_scratch_tmp_env as _export_scratch_tmp_env

    _export_scratch_tmp_env()
except Exception:
    pass  # an unwritable home leaves the system temp dir in place; never block startup

# Windows launcher self-heal — the ``nunmai`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``nunmai update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m nunmai_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no nunmai module may import before
...
# ``<checkout>\bin``, where ``nunmai update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``nunmai`` stopped
...
load_nunmai_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets → NUNMAI_REDACT_SECRETS BEFORE nunmai_logging
...
try:
    ...
    # serves nunmai_logging, nunmai_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from nunmai_cli.config_effective import load_user_config_effective as _load_effective_early
...
from nunmai_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from nunmai_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from nunmai_cli.process_identity import is_desktop_owned_backend as _is_desktop_owned_backend
from nunmai_cli.main_dashboard import (
...
from nunmai_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from nunmai_cli.main_provider_setup import (
...
from nunmai_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from nunmai_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from nunmai_cli.main_desktop import (
    cmd_gui,
)
from nunmai_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
...
from nunmai_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from nunmai_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from nunmai_cli.main_tui_launch import (
    _launch_tui,
...
def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.nunmai/.env assigns a non-empty value to any provider key."""
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    """Check if at least one inference provider is usable. Never creates one: the Nunmai free tier
    counts only once its identity exists, and the boot bootstrap (``nunmai_cli.free_tier_bootstrap``)
    ...
    """
    from nunmai_cli.config import DEFAULT_CONFIG, get_env_path, get_nunmai_home, load_config
    from nunmai_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    _has_nunmai_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_nunmai_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Nunmai is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_nunmai_config and not strict_profile_scope:
        try:
    ...
    # Nothing explicit anywhere: an existing Nunmai free-tier identity counts while the tier is on.
    try:
        from nunmai_cli.anon_auth import guest_enabled, has_guest
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``nunmai_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    """
    # A finite `nunmai -z`/`chat -q` run is CLI history too: `nunmai -z … --resume latest` chains on it.
    if source == "cli":
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from nunmai_state_ids import new_session_id as mint_session_id
        from nunmai_state_registry import acquire
            ...
            if _crumb_id:
                ...
                if last_id:
                    ...
                    print(
                        ...
                        "`nunmai`, or list sessions with `nunmai sessions list`.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from nunmai_cli.foreign_sessions import import_foreign_session, pick_foreign_session
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.nunmai/skills: there the banner
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `nunmai setup` (TTY) or exit 1 with guidance."""
    print()
    print(
...
def cmd_chat(args):
    ...
    from nunmai_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides NUNMAI_TUI/display.interface too.
    ...
    from nunmai_cli.free_tier_bootstrap import run_bootstrap
    ...
    if getattr(args, "source", None):
        ...
        os.environ["NUNMAI_SESSION_SOURCE_EXPLICIT"] = "1"
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older nunmai_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: NunmaiCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `nunmai` / `nunmai chat`.
        from nunmai_constants import emit_partial_update_hint
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``nunmai <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "nunmai_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "nunmai_cli.auth", "login_command", doc='Authenticate Nunmai CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "nunmai_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "nunmai_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "nunmai_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "nunmai_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "nunmai_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "nunmai_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "nunmai_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "nunmai_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "nunmai_cli.doctor", "run_doctor", forward_return=True, doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "nunmai_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "nunmai_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "nunmai_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "nunmai_cli.backup", "run_import", doc='Restore a Nunmai backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "nunmai_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nunmai Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "nunmai_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "nunmai_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "nunmai_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "nunmai_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "nunmai_cli.mcp_config", "mcp_command", forward_return=True)
cmd_claw = _forward_command("cmd_claw", "nunmai_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "nunmai_cli.agent_import", "import_agent_command")
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from nunmai_cli.auth import AuthError, format_auth_error, resolve_provider
    from nunmai_cli.config import get_compatible_custom_providers, get_env_value
    from nunmai_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'nunmai model' for "
    ...
    if not active:
        try:
            ...
            if exc.code == "no_provider_configured":
                ...
                # "run `nunmai model`" from inside `nunmai model` is circular.
                print("No provider is set up yet — pick one below. (Nunmai Portal works without an API key.)")
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("NUNMAI_INFERENCE_PROVIDER") or "auto"
    ...
    from nunmai_cli.models import _PROVIDER_LABELS
    ...
    # nunmai_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.nunmai/.env that poisons
...
# ``nunmai_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on nunmai_cli.main; importing update_cmd eagerly would cost every ``nunmai``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    ...
    "nunmai_cli.dashboard_procs": (
        "_detect_concurrent_nunmai_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `nunmai --version` pre-import fast path.
...
def _update_preflight_handled(args) -> bool:
    ...
    from nunmai_cli.config import is_managed, managed_error
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Nunmai runtime across profiles, its
    ...
    if getattr(args, "list_venv_holders", False):
        ...
        from nunmai_cli.update_cmd_windows import VENV_HOLDERS_EXIT, list_venv_holders
    ...
    if getattr(args, "check", False):
        ...
        from nunmai_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Nunmai Engine: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    # A child spawned off nunmai.exe: the parent still holds the shim (and the venv python)
    ...
    from nunmai_cli.update_handoff import wait_for_shim_parent_exit
    ...
    from nunmai_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from nunmai_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``nunmai -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from nunmai_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        own_home = str(get_nunmai_home())
        if not _find_stale_dashboard_pids(scope_home=own_home):
            print("No nunmai dashboard processes running for this profile.")
            sys.exit(0)
        ...
        from nunmai_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `nunmai serve` is headless/non-interactive: fail closed on a corrupt
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    NUNMAI_SERVE_HEADLESS=1). A shell inheriting those then running
    `nunmai dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    re-sets NUNMAI_SERVE_HEADLESS itself.
    ...
    NUNMAI_DASHBOARD_SESSION_TOKEN, which the terminal pane never receives and
    ...
    if (
        ...
        and _is_electron_packaged_web_dist(os.environ.get("NUNMAI_WEB_DIST", ""))
    ):
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    try:
        ...
        from nunmai_cli.main_dep_hints import missing_optional_deps_message
    ...
    if headless_backend and desktop:
        return True
    try:
        from nunmai_cli.mcp_startup import (
...
def cmd_completion(args, parser=None):
    ...
    from nunmai_cli import completion
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``nunmai`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``nunmai photon`` failing
    ...
    not import it, so the CLI registration never happens and ``nunmai photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("NUNMAI_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, NUNMAI_TUI env, display.interface config). Bare
    # `nunmai`/`nunmai chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: NUNMAI_YOLO_MODE must be set before any discovery
    ...
    try:
        from nunmai_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from nunmai_cli.mcp_startup import get_mcp_server_filter
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("NUNMAI_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `nunmai --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``nunmai-engine``) — matching is exact. ``NUNMAI_AGENT`` is the
    Nunmai-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Nunmai running inside another agent's terminal).
    """
...
def _cmd_sessions_lazy(args, **kwargs):
    """``nunmai sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from nunmai_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``nunmai`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``nunmai --help`` order; keep it stable. Groups
    live in ``nunmai_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``nunmai proxy`` (gateway group) is the INBOUND one.
    ...
    # "desktop" is canonical (Nunmai-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``nunmai -c model`` — 'model' is the session name)
...
def main():
    ...
    # Sweep stale ``nunmai.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_nunmai_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``nunmai update``, and
    ...
    # substring match is deliberately loose: over-matching (``nunmai skills
    # install update``) only defers recovery one launch; under-matching
    # (``nunmai -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from nunmai_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('nunmai_cli.cli_output', 'line_input'),
}
