...
# nunmai_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    try:
        ...
        candidate = Path(pwd.getpwnam(sudo_user).pw_dir) / ".nunmai" / "profiles" / name
...
try:
    ...
    # nunmai_logging and later raw reads: 3-4 config.yaml parses become one.
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``nunmai_state``
...
cmd_doctor = _forward_command("cmd_doctor", "nunmai_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
...
cmd_mcp = _forward_command("cmd_mcp", "nunmai_cli.mcp_config", "mcp_command")
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "nunmai_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_nunmai_modules",
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    (NUNMAI_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    NUNMAI_SERVE_HEADLESS itself.
    """
    if os.environ.get("NUNMAI_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("NUNMAI_WEB_DIST", "")):
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("NUNMAI_DESKTOP") == "1"
