...
Pre-reexec updaters (Nunmai ≤ v2026.9.14) purged only package prefixes
(``nunmai_cli``, ``gateway``, ``tools``, ``tui_gateway``, ``agent``) and left
root modules like ``utils`` cached in the updater process. The post-pull
gateway-restart phase then imports new ``nunmai_cli.gateway`` /
``nunmai_cli.config`` into that process; those need symbols the stale
``utils`` lacks (``file_signature``), and ``nunmai update`` exits 1 with
``gateway auto-restart failed: cannot import name 'file_signature' from
'utils'``.

Post-swap hand-off (``nunmai_cli.update_handoff``) makes this class dead for
...
freshly imported ``nunmai_cli`` code drops the incomplete cache before
importing ``utils``.
"""
