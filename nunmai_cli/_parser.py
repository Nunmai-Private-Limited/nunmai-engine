"""Top-level argparse construction for the nunmai CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``nunmai_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``NUNMAI_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/nunmai_cli/
...
def _cfg_path() -> str:
    """``~/.nunmai/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_nunmai_home()`` would emit the
    "[NUNMAI_HOME fallback] ... wrong profile" warning on every ``nunmai`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from nunmai_constants import display_nunmai_home, get_process_nunmai_home

    return f"{display_nunmai_home(get_process_nunmai_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``nunmai --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``nunmai_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    nunmai                        Start interactive chat
    nunmai chat -q "Hello"        Single query mode
    nunmai --tui                  Launch the modern TUI (or set display.interface: tui)
    nunmai --cli                  Force the classic REPL (overrides display.interface: tui)
    nunmai -c                     Resume the most recent session
    nunmai -c "my project"        Resume a session by name (latest in lineage)
    nunmai --resume <session_id>  Resume a specific session by ID
    nunmai --resume latest        Resume the most recent session (same as -c)
    nunmai --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    nunmai setup                  Run setup wizard
    nunmai logout                 Clear stored authentication
    nunmai auth add <provider>    Add a pooled credential
    nunmai auth list              List pooled credentials
    nunmai auth remove <p> <t>    Remove pooled credential by index, id, or label
    nunmai auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    nunmai auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    nunmai auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    nunmai model                  Select default model
    nunmai fallback [list]        Show fallback provider chain
    nunmai fallback add           Add a fallback provider (same picker as `nunmai model`)
    nunmai fallback remove        Remove a fallback provider from the chain
    nunmai config                 View configuration
    nunmai config edit            Edit config in $EDITOR
    nunmai config set model gpt-4 Set a config value
    nunmai gateway                Run messaging gateway
    nunmai gateway install        Install gateway background service
    nunmai gateway start          Start the installed gateway service
    nunmai gateway stop           Stop the gateway service
    nunmai gateway status         Show gateway status
    nunmai -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. nunmai -p coder gateway stop
    nunmai -s nunmai-engine-dev,github-auth
    nunmai -w                     Start in isolated git worktree
    nunmai sessions list          List past sessions
    nunmai sessions browse        Interactive session picker
    nunmai sessions rename ID T   Rename/title a session
    nunmai logs                   View agent.log (last 50 lines)
    nunmai logs -f                Follow agent.log in real time
    nunmai logs errors            View errors.log
    nunmai logs --since 1h        Lines from the last hour
    nunmai debug share             Upload debug report for support
    nunmai console                Open the safe Nunmai command console
    nunmai update                 Update to latest version
    nunmai dashboard              Start web UI dashboard (port 9119)
    nunmai dashboard --stop       Stop running dashboard processes
    nunmai dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via NUNMAI_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `nunmai setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to NUNMAI_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``nunmai -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/nunmai_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Nunmai Engine")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also NUNMAI_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Nunmai itself.")
...
class NunmaiArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "nunmai" at the top level and "nunmai gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = NunmaiArgumentParser(
        prog="nunmai", description="Nunmai Engine - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``nunmai [...] <command>`` instead of the brace list of
    # every subcommand name; ``nunmai --help`` still lists each command with its help row.
