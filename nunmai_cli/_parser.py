...
# argparse runs (it sets ``NUNMAI_HOME`` and strips itself from ``sys.argv``),
...
# tests/nunmai_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``nunmai --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``nunmai_cli.relaunch`` should carry over when
...
Examples:
    ...
    nunmai auth reset <provider>  Clear exhaustion status for a provider
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="nunmai",
        description="Nunmai Engine - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `nunmai chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    # top-level parser: when the user writes `nunmai -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/nunmai_cli/
