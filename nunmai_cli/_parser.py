...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.nunmai/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.nunmai/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
