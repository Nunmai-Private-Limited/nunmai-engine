...
``nunmai_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from nunmai_cli.urllib_security import open_credentialed_url
from nunmai_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from nunmai_cli.models_reasoning_caps import (
...
_NUNMAI_USER_AGENT = f"nunmai-cli/{_NUNMAI_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from nunmai_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from nunmai_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_nunmai_home() / "cache" / "openrouter_curated_catalog.json"
...
# ---------------------------------------------------------------------------
# Nunmai Portal free-model helpers — the Portal models endpoint is the source of truth for what is
...
def partition_nous_models_by_tier(
    ...
    """Split Nunmai models into (selectable, unavailable): free-tier users may only select free models
...
def check_nous_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    """True only when the Nunmai Portal user is KNOWN to be free-tier (unknown/error → False so this
...
# ---------------------------------------------------------------------------
# Nunmai Portal recommended models — curated paid/free suggestions plus dedicated compaction (aux)
...
def _nous_recommended_disk_path() -> "Path":
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from nunmai_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from nunmai_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    """The model a Nunmai account lands on without choosing one, honouring the account's tier.
    ...
    sign-in completion in ``nunmai_cli.anon_auth`` so both land on the same model.
    """
    from nunmai_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (nunmai-engine is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (Nunmai Portal,
...
# Reasoning-capability cache slots, one set per catalog (OpenRouter, Nunmai Portal). The logic
...
def fetch_openrouter_models(
    ...
    from nunmai_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from nunmai_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — nunmai-engine requires it and surfacing
...
def get_curated_nous_model_ids() -> list[str]:
    """Curated Nunmai Portal model ids: the remote catalog manifest, else the in-repo
    ``_PROVIDER_MODELS["nous"]`` snapshot. Always a list."""
    try:
        from nunmai_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from nunmai_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from nunmai_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from nunmai_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``nunmai model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from nunmai_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from nunmai_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``nunmai_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Nunmai should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from nunmai_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from nunmai_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from nunmai_cli.codex_models import get_codex_model_ids
    ...
    try:
        from nunmai_cli.auth import _codex_access_token_is_expiring, resolve_codex_runtime_credentials
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # Portal models still surface without a Nunmai release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``nunmai model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Nunmai actually ships. (#46850)
    ...
    from nunmai_cli.providers import is_official_openai_host
...
def _azure_foundry_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    """
    try:
        from nunmai_cli.azure_detect import _probe_openai_models
...
def _profile_live_catalog(normalized: str) -> Optional[list[str]]:
    ...
    if profile.auth_type == "external_process":
        ...
        # `nunmai model` offer one list: live ids plus any pinned id the probe omitted.
...
# every authed provider's /v1/models). One JSON file at $NUNMAI_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    from nunmai_constants import get_nunmai_home_override, nunmai_home_key
    inflight_key = cache_key if get_nunmai_home_override() is None else (nunmai_home_key(), cache_key)
...
def _provider_models_cache_path() -> Path:
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    if provider == "azure-foundry":
        try:
            from nunmai_cli.runtime_provider import _config_base_url_for_provider
    ...
    if provider == "openai-codex":
        from nunmai_cli.codex_models import codex_catalog_credential_identity
        ...
        try:
            ...
            for rel in ("auth.json", "credentials.json"):
                _mtime_part(rel, get_nunmai_home() / rel)
        except Exception:
            pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``nunmai model --refresh``."""
    try:
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _NUNMAI_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"nunmai-engine/{_NUNMAI_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=nunmai"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _NUNMAI_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from nunmai_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('nunmai_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('nunmai_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('nunmai_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('nunmai_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('nunmai_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('nunmai_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('nunmai_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('nunmai_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('nunmai_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('nunmai_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('nunmai_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('nunmai_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('nunmai_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('nunmai_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('nunmai_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('nunmai_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('nunmai_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('nunmai_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('nunmai_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('nunmai_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('nunmai_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('nunmai_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('nunmai_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('nunmai_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('nunmai_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('nunmai_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('nunmai_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('nunmai_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('nunmai_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('nunmai_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('nunmai_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
