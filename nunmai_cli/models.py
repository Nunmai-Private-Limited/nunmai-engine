...
Add, remove, or reorder entries here — both `nunmai setup` and
`nunmai` provider-selection will pick up the change automatically.
"""
...
from nunmai_cli.urllib_security import open_credentialed_url, url_origin
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nunmai model`
    flow without maintaining a separate static list.
    """
    from nunmai_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $NUNMAI_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nunmai-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NUNMAI_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Nunmai attribution and don't
    ...
    # endpoint expects (see nunmai_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Nunmai that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Nunmai that doesn't ship
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$NUNMAI_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  nunmai model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nunmai model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Nunmai provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nunmai model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nunmai model``, the
...
# In-repo fallback for the model Nunmai silently lands on when the user never
...
# ``nunmai model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in nunmai_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nunmai auth add openai-codex`` without
    ``nunmai model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nunmai model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nunmai-engine is tool-calling-first — every provider path assumes the model
...
# copy, `nunmai -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _NUNMAI_USER_AGENT}
    try:
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nunmai-engine
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Nunmai lands on when none is selected.
            desc = "default"
        else:
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return nunmai-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; nunmai's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nunmai model``, ``/model``, etc.).
    """
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _NUNMAI_USER_AGENT}
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Nunmai' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nunmai_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nunmai auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nunmai/.env``.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Nunmai release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Nunmai release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Nunmai release.
...
#
# Cache strategy:
#   - One JSON file at $NUNMAI_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NUNMAI_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `nunmai config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NUNMAI_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Nunmai' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Nunmai' native Anthropic adapter. The live catalog may
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nunmai_constants import get_nunmai_home
    return get_nunmai_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `nunmai moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Nunmai will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Nunmai will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Nunmai
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Nunmai cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Nunmai actually ships.  (#46850)
            #
