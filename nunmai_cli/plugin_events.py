...
caller runs in. Under ``nunmai serve`` (the Desktop backend, where plugin
...
child rides the host pipe to ``nunmai serve`` and fans out there. A process
with no Desktop client at all (``nunmai gateway run``, ``nunmai chat``, cron)
...
def broadcast_plugin_event(plugin_id: str, event: str, payload: Optional[dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        # A plugin-only process (``nunmai plugins validate`` importing the backend, a trimmed
