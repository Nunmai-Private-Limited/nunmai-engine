...
def _launchd_domain() -> str:
    ...
    See #40831, #23387.
    """
    # The cache lives on the facade: tests and callers reset ``nunmai_cli.gateway._resolved_launchd_domain``.
    gw = _gw()
...
def _launchd_reload_log_path() -> Path:
    ...
    return _gw().get_nunmai_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return _gw().get_nunmai_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    ...
    return [_gw().get_python_path(), "-m", "nunmai_cli.main", *_gw()._profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    return [_gw().get_python_path(), "-m", "nunmai_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    log_dir = _gw().get_nunmai_home() / "logs"
...
def _launchd_degrade_or_raise(exc: subprocess.CalledProcessError, what: str) -> None:
    ...
    if _gw()._launchctl_label_supervising_process(label):
        ...
        print("  Apply the definition with: nunmai gateway stop && nunmai gateway install --force")
        raise exc
...
def launchd_install(force: bool = False, *, start_now: bool = True):
        ...
        if load:
            ...
            if _gw().refresh_launchd_plist_if_needed():
                ...
                print(
                    ...
                    "Run 'nunmai gateway install --force' or check "
                    f"{display_nunmai_home()}/logs/launchd-reload.log for details."
                )
            return
    ...
    if not load:
        ...
        # what `nunmai gateway start` would kickstart instead of loading this plist.
        subprocess.run(
        ...
        print()
        print("Next steps:")
        print("  nunmai gateway start              # Start it now")
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = _gw().get_nunmai_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
