...
def _browser_available() -> bool:
    ...
    try:
        ...
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (NUNMAI_HOME / "node" / "bin",
                          NUNMAI_HOME / "node",
                          NUNMAI_HOME / "node_modules" / ".bin"):
    ...
    # nunmai_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``nunmai mcp test``.
    """
