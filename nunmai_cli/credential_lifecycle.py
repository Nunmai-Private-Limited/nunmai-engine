...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `nunmai auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``nunmai auth add``.
    """
    ...
    # that reads this var.
    try:
        from nunmai_cli.auth import unsuppress_credential_source
