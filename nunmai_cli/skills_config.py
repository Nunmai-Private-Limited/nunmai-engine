"""
Skills configuration for Nunmai Engine.
`nunmai skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.nunmai/config.yaml under:
...
from nunmai_cli.config import cfg_get, load_config, save_config
from nunmai_cli.colors import Colors, color
from nunmai_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``nunmai-engine``) are silently dropped from the
