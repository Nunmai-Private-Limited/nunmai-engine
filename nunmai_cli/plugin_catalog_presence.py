...
A portable plugin declares its app in ``plugin.json`` (``extensions.com.nousresearch.nunmai.servers``).
The catalog pins the commit, so the declaration is read once per pin from the repo at that commit and
judged by the same ``nunmai_platform`` resolver the installer and the Plugins-tab pill use. Nothing is
...
def _server_presence(decl: Any, liveness_raw: Any, title: str) -> Presence:
    ...
    from nunmai_platform.resolver.app import AppResolver
    from nunmai_platform.resolver.availability import availability
    from nunmai_platform.resolver.base import Effort
...
def presence(entry: Any) -> Presence:
    ...
    from nunmai_cli.agent_plugins import _server_declarations

    manifest = _pinned_manifest(entry.repo, entry.sha, entry.subdir)
    if not manifest:
        return UNKNOWN
    raw_servers = (manifest.get("extensions") or {}).get("com.nousresearch.nunmai", {}).get("servers") or {}
...
def onboarding_entries() -> list[Dict[str, Any]]:
    ...
    from nunmai_cli.plugins_cmd_catalog import normalized_platforms
