...
supported components into records consumed by Nunmai' existing skill and MCP
...
def _translate_remote(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    The returned record targets Nunmai' existing URL-based MCP runtime.
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    Directory scanning belongs to :mod:`nunmai_cli.plugins` so startup gating
