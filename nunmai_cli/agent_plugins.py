...
from nunmai_platform.declaration import Declaration, parse_declaration

_NUNMAI_EXTENSION = "com.nousresearch.nunmai"
...
def _server_declarations(
    ...
    namespace = manifest.get("extensions", {}).get(_NUNMAI_EXTENSION, {})
    raw_servers = namespace.get("servers", {})
    if not isinstance(raw_servers, dict):
        raise AgentPluginError(f"extension '{_NUNMAI_EXTENSION}'.servers must be an object")
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    in :mod:`nunmai_cli.plugins` so startup gating and full plugin discovery cannot drift apart."""
    from nunmai_cli.plugins import has_enabled_agent_plugin_mcp as _probe
