...
from nunmai_cli.web_server_memory import _normalize_memory_provider_name
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "auth.adopt_external_logins": {
        "type": "boolean",
        "description": (
            "Borrow and refresh the Codex CLI / Claude Code logins when Nunmai has no usable login of its own. "
            "Off: Nunmai uses only its own logins (`nunmai auth add <provider>`)."
        ),
        "category": "security",
    },
...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from nunmai_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → Nunmai-provider: when the provider is not a known provider/alias but the
    ...
    from nunmai_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and not is_custom_provider_slug and "/" in model_in:
        ...
        if cur_provider and normalize_provider(cur_provider) in _AGGREGATOR_PROVIDERS:
            ...
            from nunmai_cli.models_detect import provider_has_credentials
...
def _apply_main_model_assignment(model_cfg: "Any", result: "ModelSwitchResult", api_key: str = "") -> dict:
    ...
    (``nunmai_cli.model_switch.apply_model_selection``). An explicit key for a custom endpoint is
    ...
    Returns a new dict."""
    from nunmai_cli.model_switch import apply_model_selection
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    Long-lived dashboard / Desktop-owned ``nunmai serve`` processes freeze ``sys.modules``
    at boot; after ``nunmai update`` replaces the checkout, a first-time lazy import can
...
def _dashboard_skew_restart_hint() -> str:
    ...
    dashboard and Desktop-owned ``nunmai serve``; naming a systemd unit would mislead
    ...
    if os.environ.get("NUNMAI_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Nunmai Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Nunmai process to load the new code "
        "(nunmai dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def _apply_nous_gateway_defaults(cfg: dict) -> list:
    """Mirror the CLI's post-model-selection behaviour when switching main to Nunmai: route
    *unconfigured* tools through the Nunmai Tool Gateway. Purely additive — tools with a direct
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    ``nunmai model`` custom flow) so the picker gets a proper ready row instead of a "needs
    setup" dead-end. Dedups by base_url; never blocks the already-persisted assignment."""
    try:
        from nunmai_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _apply_main_assignment_sync(cfg: dict, provider: str, model: str, base_url: str, api_key: str,
    ...
    from nunmai_cli.free_tier_bootstrap import reconcile_record
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from nunmai_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
