...
cycle) and tests patch ``nunmai_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from nunmai_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from nunmai_cli.auth import deactivate_provider
...
def _login_retry_context(args) -> tuple[str, str]:
    ...
    among its positional args. Nunmai keeps ``nunmai portal``; every other OAuth provider is retried
    with ``nunmai auth add <provider>`` and named by its own portal host (``nunmai login`` no longer
    exists). Falls back to ``nunmai model`` when no provider config is in play."""
    pconfig = next((a for a in args if hasattr(a, "id") and hasattr(a, "portal_base_url")), None)
    if pconfig is None:
        return "nunmai model", "the sign-in service"
    ...
    if provider_id == "nous":
        return "nunmai portal", host
    return (f"nunmai auth add {provider_id}" if provider_id else "nunmai model"), host
...
def _run_login(login_fn, *args, **kwargs) -> bool:
    ...
    provider config passed to the helper, so a MiniMax failure never says ``nunmai portal`` /
