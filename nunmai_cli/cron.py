"""Cron subcommand for nunmai CLI."""
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     If no gateway is running: nunmai gateway install\n"
                "                    sudo nunmai gateway install --system  # Linux servers\n"
                "     Check status:  nunmai cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'nunmai cron create ...' "
...
def _delivery_fix_hint(job: Dict[str, Any]) -> str:
    return (f"Check the target with `nunmai cron status` or change it with "
            f"`nunmai cron edit {job.get('id', '<id>')} --deliver <target>`.")
...
def _missed_fire_issue(job: Dict[str, Any], fire_err: Dict[str, Any]) -> str:
    return (f"missed scheduled fire at {fire_err.get('at', '?')}: {_short_reason(fire_err['detail'])}. "
            "The messaging gateway was unreachable. Run `nunmai gateway restart`, then "
            f"`nunmai cron run {job.get('id', '<id>')}` to run it now.")
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `nunmai cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: nunmai cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: nunmai cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec nunmai nunmai cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list, restart_command: str = "nunmai gateway restart") -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `nunmai cron status`.\n"
        ...
        # `nunmai update` moved the checkout under a running gateway: its ticker yields every
...
def cron_status():
    ...
    from nunmai_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    ...
    if provider != "builtin":
        ...
        if host is not None:
            ...
            # `nunmai gateway restart` exits 78 for a served NAMED profile
            # (_guard_named_profile_under_multiplexer): the one host process is the default's.
            _print_ticker_health([host.pid], restart_command="nunmai --profile default gateway restart")
            ...
            if served_by_multiplexer:
                ...
                _print_ticker_health([], restart_command="nunmai --profile default gateway restart")
            else:
            ...
            print("\n  Start the ONE host gateway (it multiplexes every profile, this one included):\n"
                  "    nunmai --profile default gateway install   # user service\n"
                  "    sudo nunmai --profile default gateway install --system  # Linux servers: boot-time service\n"
                  "    nunmai --profile default gateway run       # Or run in foreground")
            if active not in ("default", "custom"):
                print("\n  It serves this profile automatically. If a per-profile service or gateway\n"
                      ...
                      "      nunmai --profile default gateway migrate --multiplex --dry-run\n"
                      "      nunmai --profile default gateway migrate --multiplex\n"
                      "  Check: nunmai cron status from this profile should show its ticker heartbeat.\n")
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Review the findings above, then run `nunmai cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits NUNMAI_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (NUNMAI_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _nunmai_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _nunmai_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``nunmai cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: nunmai cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: nunmai cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
