...
# commands at execution time when ``_NUNMAI_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: nunmai gateway install", Colors.DIM))
    print(color("                    sudo nunmai gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  nunmai cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'nunmai cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_incidents(args) -> int:
    ...
    ``nunmai cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``nunmai cron incidents ack <id>`` closes one so its failure ping stays
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: nunmai cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
...
def cron_status():
    ...
    if pids or gateway_alive_via_lock:
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec nunmai "
                        "nunmai cron ...`). Fix ownership to match the "
        ...
        print("    nunmai gateway install    # Install as a user service")
        print("    sudo nunmai gateway install --system  # Linux servers: boot-time system service")
        print("    nunmai gateway            # Or run in foreground")
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (NUNMAI_SESSION_KEY);
...
def cron_resume(args) -> int:
    ...
    run_at = _nunmai_now().isoformat() if args.run_now else args.run_at
    try:
...
def cron_notepad(args) -> int:
    """Handle ``nunmai cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: nunmai cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: nunmai cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: nunmai cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: nunmai cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
