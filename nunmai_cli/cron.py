...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: nunmai gateway install\n"
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              ...
              "  If heartbeat never appears, restart: nunmai gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: nunmai gateway restart")
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside NUNMAI_HOME/scripts: {script!r}"
    if not path.exists():
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `nunmai cron doctor` again.", Colors.DIM))
    return 1
