...
interactive CLI. Mixin bound onto ``NunmaiCLI`` via the MRO; cli.py-internal symbols are imported
...
from nunmai_cli.commands_completion import SlashCommandAutoSuggest, SlashCommandCompleter
...
from nunmai_cli.cli_footer_split import FooterSplit
...
class CLITuiMixin:
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from nunmai_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from nunmai_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import NunmaiCLI, _panel_box_width, _wrap_panel_text
        ...
        scroll_offset, visible = NunmaiCLI._compute_model_picker_viewport(
    ...
    def _get_model_picker_display_fragments(self):
        ...
        if state.get("stage", "provider") == "provider":
            ...
            from nunmai_cli.cli_model_switch_mixin import _picker_reasoning_rows
        ...
        if code := self._sudo_state.get("vault_code"):
            return self._render_sudo_style_panel(
                ...
                [f'{code["site"]} is asking for a one-time code (text message, email or authenticator app).',
                 'Type the code and press Enter; Nunmai enters it into the page for you.',
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        nunmai_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_handle_ctrl_z(self, event):
        ...
        agent_name = get_active_skin().get_branding("agent_name", "Nunmai Engine")
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _nunmai_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_nunmai_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
        ...
        try:
            ...
            if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                ...
                mark_seen(_nunmai_home / "config.yaml", BUSY_INPUT_FLAG)
    ...
    def _tui_enter_overlay(self, event) -> bool:
        ...
        if self._model_picker_state:
            try:
                ...
                from nunmai_cli.model_switch import resolve_persist_behavior
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.nunmai/pastes and return the placeholder for the buffer."""
        from cli import _nunmai_home, datetime, logger
        self._tui_paste_counter += 1
        paste_dir = _nunmai_home / "pastes"
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from nunmai_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_init_run_state(self):
        ...
        from nunmai_cli.config import get_config_path as _get_config_path
    ...
    def _tui_build_key_bindings(self):
        ...
        from nunmai_cli.cli_subagent_monitor import modal_prompt_active, open_monitor, toggle_dock
        for key in ('c-t', 'f6'):
    ...
    def _tui_build_layout(self, kb):
        ...
        cli_ref = self
        from nunmai_cli.cli_subagent_monitor import install_dock
        install_dock(self)
    ...
    def _tui_set_base_style(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#888888 italic',
            'status-bar': 'bg:#052819 #C0C0C0',
            'status-bar-strong': 'bg:#052819 #6BE7C8 bold',
            'status-bar-dim': 'bg:#052819 #8B8682',
            'status-bar-good': 'bg:#052819 #8FBC8F bold',
            'status-bar-warn': 'bg:#052819 #6BE7C8 bold',
            'status-bar-bad': 'bg:#052819 #FF8C00 bold',
            'status-bar-critical': 'bg:#052819 #FF6B6B bold',
            'status-bar-yolo': 'bg:#052819 #FF4444 bold',
            'status-bar-session-title': 'bg:#6BE7C8 #052819 bold',
            'input-rule': '#0B6B4F',
            'image-badge': '#87CEEB bold',
            'completion-menu': 'bg:#052819 #EFE9D7',
            'completion-menu.completion': 'bg:#052819 #EFE9D7',
            'completion-menu.completion.current': 'bg:#1F4A3D #6BE7C8',
            'completion-menu.meta.completion': 'bg:#052819 #888888',
            'completion-menu.meta.completion.current': 'bg:#1F4A3D #3FCFA9',
            'clarify-border': '#0B6B4F',
            'clarify-title': '#6BE7C8 bold',
            'clarify-question': '#EFE9D7 bold',
            'clarify-choice': '#AAAAAA',
            'clarify-selected': '#6BE7C8 bold',
            'clarify-active-other': '#6BE7C8 italic',
            'clarify-answer': '#98FB98',
            'clarify-countdown': '#0B6B4F',
            'sudo-prompt': '#FF6B6B bold',
            'sudo-border': '#0B6B4F',
            'sudo-title': '#FF6B6B bold',
            'sudo-text': '#EFE9D7',
            'approval-border': '#0B6B4F',
            'approval-title': '#FF8C00 bold',
            'approval-desc': '#EFE9D7 bold',
            'approval-cmd': '#AAAAAA italic',
            'approval-choice': '#AAAAAA',
            'approval-selected': '#6BE7C8 bold',
            'voice-prompt': '#87CEEB',
            'voice-recording': '#FF4444 bold',
            'voice-processing': '#FFA500 italic',
            'voice-status': 'bg:#052819 #87CEEB',
            'voice-status-recording': 'bg:#052819 #FF4444 bold'}
