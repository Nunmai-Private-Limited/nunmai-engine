...
Split out of ``nunmai_cli/model_switch.py``; every moved name is re-imported there so
``nunmai_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from nunmai_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("nunmai_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from nunmai_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Nunmai itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from nunmai_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
    from nunmai_cli.models_local import (
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from nunmai_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(nunmai_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from nunmai_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from nunmai_cli.models import _AGGREGATOR_PROVIDERS
    from nunmai_cli.providers import ALIASES
    for nunmai_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(nunmai_id)
        if alias_target and alias_target != nunmai_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(nunmai_id)
            if prof is not None and prof.name != nunmai_id:
                continue
        except Exception:
            pass
        if nunmai_id.lower() in seen:
            continue
        if nunmai_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(nunmai_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(nunmai_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield nunmai_id, mdev_id, pconfig, env_vars


def _auth_store_has_provider(*keys: str) -> bool:
    """True when ``auth.json`` has a ``providers`` entry under any of *keys*."""
    try:
        from nunmai_cli.auth import _load_auth_store
...
def _raw_pool_usable(nunmai_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from nunmai_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(nunmai_id):
            return _credential_pool_is_usable(nunmai_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, nunmai_slug: str, overlay, read_env) -> bool:
...
def _azure_entra_configured(read_env=os.environ.get) -> bool:
    ...
    from nunmai_cli.models import _get_model_config_dict
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True,
    ...
    """``cached_provider_model_ids`` (the SAME disk-cached list ``nunmai model`` builds), falling
    ...
    from nunmai_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    """Nunmai serves a huge live catalog; the picker shows ONLY the curated agentic list, augmented
    ...
    try:
        from nunmai_cli.models_pricing import get_pricing_for_provider
        from nunmai_cli.models import (
...
def _free_tier_nous_row(row: dict) -> dict | None:
    """The one free-tier rule for a Nunmai picker row, shared by every row builder.

    ``row`` carries at least ``name`` and ``models``. A guest identity carrying inference turns
    it into "Nunmai · free tier" with the single model ``nous/welcome`` (the welcome host serves
    nothing else). A guest that ``nous.guest: false`` has switched off yields ``None``: no Nunmai
    row at all, since there is nothing selectable. A real account (or no Nunmai state) passes the
    ...
    from nunmai_cli import anon_auth
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from nunmai_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Nunmai's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from nunmai_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from nunmai_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from nunmai_cli.auth import PROVIDER_REGISTRY
    from nunmai_cli.providers import NUNMAI_OVERLAYS
    from nunmai_cli.models import CANONICAL_PROVIDERS
    ...
    for nunmai_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(nunmai_id):
            _emit(nunmai_id, nunmai_id)

    mdev_to_nunmai = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in NUNMAI_OVERLAYS.items():
        nunmai_slug = mdev_to_nunmai.get(pid, pid)
        if _skip(seen, excluded_set, pid, nunmai_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, nunmai_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, nunmai_slug) or _pool_usable(nunmai_slug)):
            _emit(nunmai_slug, pid, nunmai_slug)
    ...
    # Nunmai excluded: its picker branch builds from the curated list and never reads the
...
class _PickerBuild:
    ...
    models.dev, 2 Nunmai-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``nunmai_cli.auth/models`` stay lazy so tests can patch those modules."""
    ...
    def add_builtin_row(
        ...
        if slug == "nous":
            # Free-tier identity: one row "Nunmai · free tier" / nous/welcome, or no row when
...
def _lap_lmstudio_row(b: _PickerBuild, user_providers: dict) -> None:
    ...
    if not (is_current or isinstance(configured, dict)):
        return
    from nunmai_cli.model_switch import _declared_model_ids
    ...
    b.add_builtin_row("lmstudio", get_label("lmstudio"), is_current, model_ids, "nunmai")
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from nunmai_cli.model_switch import _declared_model_ids, _scoped_key_env
    from agent.models_dev import get_provider_info
    for nunmai_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        ...
        if not (_any_env(env_vars, _scoped_key_env) or _raw_pool_usable(nunmai_id)):
            continue
        model_ids = _live_or_curated_ids(nunmai_id, b.curated, non_blocking=b.non_blocking_catalogs)
        ...
        configured = user_providers.get(nunmai_id) if isinstance(user_providers, dict) else None
        configured_models = _declared_model_ids(configured.get("models")) if isinstance(configured, dict) else []
        ...
        b.add_builtin_row(
            nunmai_id, display_name, b.current_provider in (nunmai_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, nunmai_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(nunmai_slug, b.current_provider)
    else:
        from nunmai_cli.model_switch import _scoped_key_env
        has_creds = _overlay_has_env_creds(pid, nunmai_slug, overlay, _scoped_key_env)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(nunmai_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, nunmai_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(nunmai_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(nunmai_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
            logger.debug("Credential pool check failed for %s: %s", nunmai_slug, exc)
    if not has_creds and nunmai_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_nunmai_oauth_credentials
            nunmai_creds = read_nunmai_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (nunmai_creds and nunmai_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    """Section 2: Nunmai-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from nunmai_cli.model_switch import _declared_model_ids
    from nunmai_cli.providers import NUNMAI_OVERLAYS

    # NUNMAI_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Nunmai IDs ("copilot").
    mdev_to_nunmai = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in NUNMAI_OVERLAYS.items():
        nunmai_slug = mdev_to_nunmai.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, nunmai_slug):
            continue
        if not _overlay_has_creds(b, pid, nunmai_slug, overlay):
            continue
        if nunmai_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = _live_or_curated_ids(nunmai_slug, b.curated, merge_models_dev=False,
                                             non_blocking=b.non_blocking_catalogs)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(nunmai_slug, b.curated, nunmai_slug, pid,
                                                 non_blocking=b.non_blocking_catalogs)
            ...
            tier_row = _free_tier_nous_row({"name": get_label(nunmai_slug), "models": []})
            ...
            model_ids = _live_or_curated_ids(nunmai_slug, b.curated, nunmai_slug, pid,
                                             non_blocking=b.non_blocking_catalogs)
        ...
        configured = user_providers.get(nunmai_slug) or user_providers.get(pid) if isinstance(user_providers, dict) else None
        if isinstance(configured, dict):
            model_ids = list(dict.fromkeys([*_declared_model_ids(configured.get("models")), *model_ids]))
        b.add_builtin_row(
            nunmai_slug, get_label(nunmai_slug), b.current_provider in (nunmai_slug, pid), model_ids, "nunmai")
...
def _lap_canonical_rows(b: _PickerBuild) -> None:
    ...
    for cp in CANONICAL_PROVIDERS:
        ...
        if not has_creds and cp_config is not None and cp_config.auth_type == "external_process":
            ...
            # evidence for listing (same gate as the copilot-acp overlay row and nunmai auth status).
            try:
                from nunmai_cli.auth import get_external_process_provider_status
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from nunmai_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from nunmai_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from nunmai_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from nunmai_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str,
    ...
    """Curated model lists keyed by nunmai provider id, plus the dynamic ones (nous manifest,
    ...
    from nunmai_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
    ...
    if "ollama-cloud" not in curated:
        from nunmai_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from nunmai_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    ("built-in", "nunmai", "canonical", "user-config", "model-config").
    ``force_fresh_nous_tier`` bypasses the short Nunmai tier cache (account-sensitive flows only);
    ...
    from nunmai_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (nunmai_id / mdev_id / canonical slug).
    b = _PickerBuild(
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from nunmai_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from nunmai_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`nunmai_cli.models.fetch_openrouter_models` (curated
    ...
    from nunmai_cli.model_switch import list_authenticated_providers
    from nunmai_cli.models import fetch_openrouter_models
