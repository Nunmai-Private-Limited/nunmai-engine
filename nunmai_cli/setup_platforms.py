"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``nunmai setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on nunmai_cli.setup, so those are imported lazily per function."""
...
def _profile_name_from_nunmai_home(nunmai_home) -> str | None:
    ...
    return nunmai_home.name if nunmai_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from nunmai_cli.setup import get_nunmai_home
    try:
...
def _declines_reconfigure(env_var: str, label: str, question: str) -> bool:
    ...
    from nunmai_cli.setup import get_env_value, print_info, prompt_yes_no
...
def _save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from nunmai_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from nunmai_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from nunmai_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from nunmai_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from nunmai_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from nunmai_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from nunmai_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from nunmai_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where Nunmai delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from nunmai_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if _declines_reconfigure("BLUEBUBBLES_SERVER_URL", "BlueBubbles", "Reconfigure BlueBubbles?"):
        return
    _info("Connects Nunmai to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from nunmai_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    print()
    _info("   Full guide: https://nunmai.in/docs/user-guide/messaging/webhooks/", None)
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   https://nunmai.in/docs/user-guide/messaging/webhooks/#configuring-routes",
          None,
          # Printed twice upstream; kept verbatim for output parity.
          "   Open config in your editor:  nunmai config edit",
          "   Open config in your editor:  nunmai config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from nunmai_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     nunmai config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from nunmai_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from nunmai_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from nunmai_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Nunmai from anywhere.",
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `nunmai import` /
    # `nunmai setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from nunmai_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
