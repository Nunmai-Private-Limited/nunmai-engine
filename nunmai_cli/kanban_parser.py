"""Argparse tree for ``nunmai kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $NUNMAI_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `nunmai kanban boards export`. The board always lands as a "
...
# Top-level ``nunmai kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $NUNMAI_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $NUNMAI_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $NUNMAI_PROFILE or 'user')"),
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.nunmai/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Nunmai profiles. "
    ...
    # resolution is NUNMAI_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `nunmai kanban boards switch <slug>` or the "
                                    "NUNMAI_KANBAN_BOARD env var). Use `nunmai kanban boards "
