...
Extracted from ``nunmai_cli/gateway.py``. Bodies read facade helpers through ``_gw()`` (late
binding on ``nunmai_cli.gateway``) so the seams tests and callers patch on the facade keep
...
def _gw():
    from nunmai_cli import gateway  # late: the facade imports this module
    return gateway
...
def _ld_library_path_line(system: bool, target_home_dir: str | None = None) -> str:
    ...
    ~/.nunmai/.env is too late for CUDA libs — #14613); system units remap caller-home components.
...
def _nunmai_home_for_target_user(target_home_dir: str) -> str:
    ...
    current_nunmai = Path(current_nunmai_raw).expanduser() if current_nunmai_raw else _gw().get_nunmai_home()
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    nunmai_home = _gw().get_nunmai_home()
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (NUNMAI_HOME, else _gw().PROJECT_ROOT). cwd is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        nunmai_home = _gw()._nunmai_home_for_target_user(home_dir)
        ...
        try:
            ...
            profile_arg = _gw()._profile_arg(nunmai_home, default_root=target_root)
        except ValueError:
            profile_arg = _gw()._profile_arg(nunmai_home)
        ...
        working_dir = str(nunmai_home) if nunmai_home else _gw()._remap_path_for_user(working_dir, home_dir)
        ...
        _gw()._append_node_dir_for_service(_target_node_entries, Path(nunmai_home) if nunmai_home else None)
        ...
        nunmai_home = str(_gw().get_nunmai_home().resolve())
        profile_arg = _gw()._profile_arg(nunmai_home)
        user_home = Path.home()
    ...
    watchdog_seconds = _gw()._systemd_watchdog_seconds(nunmai_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    _gw()._sync_nunmai_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    temp_roots = {
        ...
        Path("/tmp"), Path("/var/tmp"), Path("/private/tmp"), Path("/private/var/tmp"),  # no-tmp: ok — detects a temp NUNMAI_HOME in service definitions
    }
...
def _retire_nunmai_replace_dropin(system: bool = False) -> bool:
    """Unlink the ``20-replace.conf`` drop-in an older Nunmai wrote to end a respawn storm; True if removed.
    ...
    start (#119467). Only the Nunmai-authored file (recognised by its own comment) is touched.
    """
    unit_path = _gw().get_systemd_unit_path(system=system)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # _gw().systemd_unit_is_current is the NUNMAI_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    current = _gw().systemd_unit_is_current(system=system)
    if _retire_nunmai_replace_dropin(system=system):
        _gw()._run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
        print(f"↻ Removed the stale Nunmai --replace drop-in from the gateway {_gw()._service_scope_label(system)} service")
        if current:
            return True
    elif current:
        return False
    ...
    _gw()._run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_gw()._service_scope_label(system)} service definition to match the current Nunmai install")
    return True
