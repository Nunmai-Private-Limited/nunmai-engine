...
The provider list shown by ``nunmai model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`nunmai_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``nunmai model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("nunmai_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("nunmai_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    NUNMAI_OVERLAYS = _safe_import("nunmai_cli.providers", "NUNMAI_OVERLAYS", {})
    try:
