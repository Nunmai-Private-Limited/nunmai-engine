...
Automates the Nunmai Portal ``/local-dashboards`` flow: resolve a fresh Nunmai access token, POST
``{portal}/api/oauth/self-hosted-client`` (the ``agent:`` prefix is applied server-side),
write ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
    ...
    *, access_token: str, portal_base_url: str, name: Optional[str], custom_redirect_uri: Optional[str],
    ...
    try:
        ...
        try:  # structured JSON errors: {error, error_description}
            ...
            detail = ""
        if exc.code == 401:
            message = ("Nunmai Portal rejected the access token (401). "
        ...
        raise RuntimeError(f"Could not reach Nunmai Portal at {portal_base_url}: {exc.reason}") from exc
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    print(f"\n  Wrote to {get_env_path()}:\n    NUNMAI_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
    ...
    if custom_redirect_uri:
        try:  # example host matches the one the user registered
            ...
            host = "your-host"
        print(
            "  To require Nunmai login on your registered host, run the dashboard\n"
            f"  bound publicly (it must be reachable at https://{host}) and log in\n"
            "  at its /login page.")
    else:
        print(
            "  To require Nunmai login (e.g. exposing on your LAN or a public host):\n"
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to NUNMAI_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    from nunmai_cli.config import is_managed, save_env_value
    # Managed installs get the client id stamped in by the orchestrator (save_env_value refuses).
    if is_managed():
        print("✗ `nunmai dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nunmai Portal.\n"
                  "  Run `nunmai setup` (or `nunmai auth add nous`) first, then retry.")
        else:
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("NUNMAI_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("NUNMAI_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        ...
        print(f"✗ Failed to write NUNMAI_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  NUNMAI_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("NUNMAI_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("NUNMAI_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("NUNMAI_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("NUNMAI_DASHBOARD_PUBLIC_URL", public_url))
