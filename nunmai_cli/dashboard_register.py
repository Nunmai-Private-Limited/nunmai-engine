"""``nunmai dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the Nunmai Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.nunmai/.env``
as ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nunmai Portal access token from the existing login
     (``~/.nunmai/auth.json``), refreshing it if needed. Fails fast with a
     "run `nunmai setup`" hint when the user isn't logged in.
  ...
  3. Writes ``NUNMAI_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``NUNMAI_DASHBOARD_PORTAL_URL`` into ``~/.nunmai/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``NUNMAI_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the Nunmai login — this is the
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``nunmai dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Nunmai Portal rejected the access token (401). "
        ...
        raise RuntimeError(
            f"Could not reach Nunmai Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from nunmai_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    NUNMAI_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    NUNMAI_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    NUNMAI_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nunmai login only *engages* on a non-loopback bind. A plain\n"
        "  `nunmai dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Nunmai login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Nunmai login (e.g. exposing on your LAN or a public host):")
        print("    nunmai dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from nunmai_cli.auth import AuthError, resolve_nous_access_token
    from nunmai_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets NUNMAI_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `nunmai dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh Nunmai access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_nous_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nunmai Portal.")
            print("  Run `nunmai setup` (or `nunmai auth add nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Nunmai Portal access token: {exc}")
        sys.exit(1)
    ...
    # NUNMAI_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (NUNMAI_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("NUNMAI_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("NUNMAI_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write NUNMAI_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  NUNMAI_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      NUNMAI_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("NUNMAI_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("NUNMAI_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking NUNMAI_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as NUNMAI_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("NUNMAI_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("NUNMAI_DASHBOARD_PUBLIC_URL", public_url)
