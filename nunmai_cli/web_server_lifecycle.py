...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "nunmai_cli.gateway",
        "nunmai_cli.auth",  # provider auth state → copilot_auth → subprocess
        "nunmai_cli.copilot_auth",
        "nunmai_cli.runtime_provider",
        "nunmai_cli.skin_engine",  # resolve_skin() config + skin engine init
        "nunmai_cli.inventory",  # provider catalogs + models.dev cache
...
def _eager_reconcile_own_session_db() -> None:
    ...
    :func:`nunmai_cli.web_server_sessions._open_session_db_at_path`. Never
    raises: an unfixable store still gets the per-poll read-probe heal.
    """
    try:
        from nunmai_cli.web_server_sessions import _open_session_db_for_profile
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``NUNMAI_DESKTOP_READY_FILE`` is set.
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``NUNMAI_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``nunmai.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    start_marker = os.environ.get("NUNMAI_PARENT_START_MARKER") or None
    nonce = os.environ.get("NUNMAI_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable NUNMAI_PARENT_START_MARKER=%r / nonce; "
