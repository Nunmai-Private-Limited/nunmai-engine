...
``nunmai dashboard`` (fastapi + uvicorn) and ``nunmai acp`` (agent-client-protocol) are installed by
the ``[all]`` / ``[acp]`` extras. A partial install, a pip-less uv venv or an interrupted update can
leave them out; the built-in repair is ``nunmai update``, which reinstalls the extras into the same
...
def missing_optional_deps_message(surface: str, what: str, extra: str) -> str:
    ...
    return (
        ...
        "Run `nunmai update` to reinstall dependencies. If that fails, run manually:\n"
