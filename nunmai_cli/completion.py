"""Shell completion script generation for nunmai CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nunmai_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Nunmai Engine bash completion
# Add to ~/.bashrc:
#   eval "$(nunmai completion bash)"

_nunmai_profiles() {{
    local profiles_dir="$HOME/.nunmai/profiles"
...
_nunmai_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nunmai_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nunmai_completion nunmai
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef nunmai
# Nunmai Engine zsh completion
# Add to ~/.zshrc:
...
_nunmai_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nunmai/profiles" ]]; then
        profiles+=($HOME/.nunmai/profiles/*(N/:t))
    fi
...
_nunmai() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nunmai_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nunmai command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Nunmai Engine fish completion",
        "# Add to your config:",
        "#   nunmai completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nunmai_profiles",
        "    echo default",
        "    if test -d $HOME/.nunmai/profiles",
        "        for d in $HOME/.nunmai/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nunmai -f -s p -l profile"
        " -d 'Profile name' -xa '(__nunmai_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c nunmai -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__nunmai_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
