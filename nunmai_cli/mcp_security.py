...
for the June 2026 nunmai-0day campaign. Runs BOTH at save time (``_save_mcp_server`` — dashboard API +
...
# OS persistence surfaces an MCP server has no legitimate reason to write to (the nunmai-0day
...
# Indicators of compromise, June 2026 nunmai-0day campaign: exact attacker artifacts observed on
...
def validate_mcp_server_entry(name: str, entry: dict[str, Any]) -> list[str]:
    ...
    * a shell interpreter whose inline script writes to an OS persistence surface (June 2026 nunmai-0day
