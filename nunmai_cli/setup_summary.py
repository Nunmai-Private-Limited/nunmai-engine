...
resolved through the module object so test patches on ``nunmai_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from nunmai_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'nunmai setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'nunmai setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'nunmai tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("nunmai setup", "          Re-run the full wizard"), ("nunmai setup model", "    Change model/provider"),
    ("nunmai setup terminal", " Change terminal backend"), ("nunmai setup gateway", "  Configure messaging"),
    ("nunmai setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("nunmai config", "         View current settings"), ("nunmai config edit", "    Open config in your editor"),
    ("nunmai config set <key> <value>", ""))
_READY_ROWS = (
    ("nunmai", "              Start chatting"), ("nunmai gateway", "      Start messaging gateway"),
    ("nunmai doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'nunmai setup' to configure")


def _managed_or_provider_row(feature, name: str, managed_label: str, missing_hint: str):
    """Row for a Nunmai-manageable feature: managed > available (with provider) > missing hint."""
...
def _web_row(config, feats):
    ...
    return _managed_or_provider_row(feats.web, "Web Search & Extract", "Nunmai subscription", _WEB_MISSING)
...
def _browser_row(config, feats):
    ...
    return _managed_or_provider_row(feats.browser, "Browser Automation", "Nunmai Browser Use", hint)


def _image_gen_row(config, feats):
    # FAL (direct or via Nunmai), or any plugin-registered provider (OpenAI, etc.)
    if feats.image_gen.managed_by_nous:
        return ("Image Generation (Nunmai subscription)", True, None)
...
def _video_gen_row(config, feats):
    # Opt-in via `nunmai tools` → Video Generation. Only show the row when a plugin reports
    # available so we don't badger users who don't care about video gen with a "missing" line.
    if feats.video_gen.managed_by_nous:
        return ("Video Generation (FAL via Nunmai subscription)", True, None)
...
def _tts_row(config, feats):
    ...
    if feats.tts.managed_by_nous:
        return ("Text-to-Speech (OpenAI via Nunmai subscription)", True, None)
...
def _stt_row(config, feats):
    ...
    if stt_feature is not None and stt_feature.managed_by_nous:
        return ("Speech-to-Text (OpenAI via Nunmai subscription)", True, None)
...
def _modal_row(config, feats):
    if feats.modal.managed_by_nous:
        return ("Modal Execution (Nunmai subscription)", True, None)
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'nunmai setup terminal'")
    if tool_backend_helpers.managed_nous_tools_enabled() and feats.nous_auth_present:
        return ("Modal Execution (optional via Nunmai subscription)", True, None)
    return None
...
def _spotify_row(config, feats):
    # OAuth via nunmai auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, nunmai_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Nunmai cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    nunmai model            (pick any provider/model)",
              "    nunmai setup --portal   (Nunmai Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'nunmai setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {nunmai_home}/cron/, sessions/, logs/")):
...
import nunmai_cli.setup as _setup  # noqa: E402  (bottom: nunmai_cli.setup imports this module)
