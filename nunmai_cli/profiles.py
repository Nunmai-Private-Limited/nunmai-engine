"""Profile management for multiple isolated Nunmai instances."""
...
from nunmai_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
_PROFILE_ID_RE = PROFILE_ID_RE  # legacy alias; nunmai_constants.PROFILE_ID_RE is canonical
...
# stay visible; containers persist HOME state here). See nunmai_constants.get_subprocess_home().
...
# (``~/.nunmai``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``nunmai_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "nunmai-engine", ".worktrees", "profiles", "bin", "node_modules",
}) | LOCAL_RUNTIME_ROOT_DIRS

# Per-profile history excluded from --clone-all for ANY source: SQLite session store
# (+wal/shm, can reach many GB), session dirs, `nunmai backup` archives, quick-backup
...
# Marker written by `nunmai profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `nunmai update` all-profile sync, the
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this Nunmai profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_nunmai_home().resolve():
...
# Allow-list for ``export_profile("default")``: when NUNMAI_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Nunmai profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``nunmai_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"nunmai", "default", "test", "tmp", "root", "sudo"})

# Nunmai subcommands that cannot be used as profile names/aliases
_NUNMAI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the nunmai root (NOT the current NUNMAI_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_nunmai_home() / "profiles"


def _get_default_nunmai_home() -> Path:
    """Default (pre-profile) NUNMAI_HOME: ``~/.nunmai``, or NUNMAI_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from nunmai_constants import get_default_nunmai_root
    return get_default_nunmai_root()


def _get_active_profile_path() -> Path:
    return _get_default_nunmai_home() / "active_profile"
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Nunmai-generated wrapper (contains ``nunmai -p``)."""
    try:
        return "nunmai -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: nunmai profile create {canon}")
...
def _unknown_profile_error(canon: str) -> FileNotFoundError:
    ...
    return FileNotFoundError(f"No profile named '{canon}'. See your profiles with: nunmai profile list")


def _profile_exists_error(canon: str) -> FileExistsError:
    return FileExistsError(
        f"A profile named '{canon}' already exists. Switch to it with `nunmai profile use {canon}`, "
        "see all profiles with `nunmai profile list`, or choose a different name."
    )
...
def _invalid_profile_name_error(name: str) -> ValueError:
    ...
    return ValueError(
        ...
        f"Then run `nunmai profile create {suggestion}`."
    )
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.nunmai
    if not _PROFILE_ID_RE.match(name):
        raise _invalid_profile_name_error(name)
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved — it collides with either "
            f"the Nunmai installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NUNMAI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_nunmai_home()
    ...
    # profiles/nunmai may still exist and must keep resolving.
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    shell into a "real" profile on the next ``nunmai update`` (#95188, #94823, #99392)."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _NUNMAI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nunmai subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            nunmai_exe = shutil.which("nunmai") or "nunmai"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(nunmai_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `nunmai doctor --fix` surfaces
    # the detailed error in the target profile.
    with contextlib.suppress(Exception):
        from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override
        from nunmai_cli.config import check_config_version, migrate_config
...
# wrappers are a few hundred bytes with the ``nunmai -p X`` needle near the top; the wrapper
...
class ProfileInfo:
    ...
    display_name: str = ""
    # Bot Mode title (``profile.yaml`` ``ui_meta['nunmai-bots'].title``) — the name
    # the Bots roster shows. Presentation-only, like ``display_name``.
    bot_title: str = ""
    # Canonical ids this profile was previously known by (``nunmai profile rename``
    # appends here). Lets Bot Mode group chats re-link persisted member
    # descriptors to the renamed live profile (#110200).
    previous_names: List[str] = field(default_factory=list)
    # Backend-assigned role (``SETUP_ROLE`` or None). Only ``nunmai_cli.setup_profile`` writes it.
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `nunmai model` sets it later
        from nunmai_constants import get_nunmai_home
        from nunmai_cli.config import atomic_config_write, read_user_config_raw
        source = get_nunmai_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating NUNMAI_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from nunmai_cli.gateway import named_profile_served_by_running_multiplexer
...
def _count_skills(profile_dir: Path) -> int:
    ...
    synchronously when stale — detail surfaces (``nunmai profile info``, ``profiles.describe``)
...
def _cached_skill_count(profile_dir: Path) -> int:
    ...
    if due:
        threading.Thread(target=_count_skills, args=(profile_dir,),
                         name="nunmai-skill-count", daemon=True).start()
    cached = _SKILL_COUNT_CACHE.get(key)
    return cached[2] if cached is not None else 0


# profile.yaml — per-profile metadata (description, role, etc.)
# Deliberately tiny and separate from ``config.yaml`` (user-facing Nunmai config, ~5000
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    corrupt file on one profile must not break ``nunmai profile list``."""
    def _read() -> dict:
        data = _load_yaml_dict(profile_dir / "profile.yaml") or {}
        ui_meta = data.get("ui_meta")
        bot_title = ""
        if isinstance(ui_meta, dict):
            nunmai_bots = ui_meta.get("nunmai-bots")
            if isinstance(nunmai_bots, dict):
                bot_title = str(nunmai_bots.get("title") or "").strip()
        return {
...
def list_profiles(*, lazy_skill_count: bool = False) -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_nunmai_home()
...
def profile_is_standalone(home: Path) -> bool:
    ...
    if home == _get_default_nunmai_home():
...
def profiles_to_serve(multiplex: bool, *, include_standalone: bool = False,
    ...
    """``(profile_name, nunmai_home)`` pairs a gateway should serve — the single chokepoint
    ...
    out of the host multiplexer; a ``gateway.parked`` marker (``nunmai -p X gateway stop``) skips
    ...
    default = _get_default_nunmai_home()
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from nunmai_cli.auth import strip_cloned_single_use_oauth_grants
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    ``sync_imports`` also copies the source's ``import-sync.json`` (the ``nunmai import-agent``
    manifest) so the clone stays registered against the same external Claude Code / Codex trees
    and ``nunmai -p <clone> import-agent --sync`` keeps pulling from them. The link is to the
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    from nunmai_cli.profile_memory_config import active_memory_provider, clone_memory_provider_config
    clone_memory_provider_config(source_dir, profile_dir,
                                 active_memory_provider(_load_yaml_dict(source_dir / "config.yaml")))
    if sync_imports:
        from nunmai_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    makes two gateways fight over one bot (``nunmai_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``nunmai update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.nunmai).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from nunmai_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from nunmai_cli.profile_channels import strip_channel_settings
    ...
    # `nunmai -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `nunmai -p <name> auth add`)
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from nunmai_cli.default_soul import DEFAULT_SOUL_MD
        _seed_file_if_missing(profile_dir / "SOUL.md", DEFAULT_SOUL_MD)

    # Opt-out marker read by seed_profile_skills() and `nunmai update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`nunmai profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `nunmai profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from nunmai_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _purge_identity(canon: str) -> bool:
    ...
    from nunmai_cli.profile_identity import purge_profile_identity
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from nunmai_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches NUNMAI_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NUNMAI_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_nunmai_home() / ".env"
...
_NUNMAI_ARGV_MARKERS = ("nunmai_cli.main", "nunmai-gateway", "tui_gateway")
...
# `findOnPath('nunmai')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "nunmai".
...
# argv[1] is matched against exact names, not ``startswith("nunmai")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("nunmai-notes.py").
_NUNMAI_CONSOLE_SCRIPT_NAMES = frozenset({"nunmai", "nunmai-engine", "nunmai-acp"})


def _is_nunmai_argv(argv: list) -> bool:
    """True for a Nunmai process: entrypoint marker in argv, executable named ``nunmai*``,
    or a python interpreter directly exec'ing a known ``nunmai`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _NUNMAI_ARGV_MARKERS) or exe_name.startswith("nunmai"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _NUNMAI_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Nunmai *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`nunmai -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_nunmai_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by NUNMAI_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("NUNMAI_HOME", "")
...
def _stop_bot_desktop(profile_dir: Path) -> None:
    ...
    gateway shutdown does not reach it (its own session, its own pid file). Scoped through the nunmai-home
    ...
    if not runtime.is_supported_host():
        return
    token = set_nunmai_home_override(profile_dir)
    try:
        if runtime.stop():
...
class ProfileIdentitySettlementPending(RuntimeError):
    ...
    settled (``nunmai_cli.profile_identity.purge_profile_identity`` returned False).

    Subclasses ``RuntimeError`` so delete-failure handling that treats the error as fatal (the
    CLI's ``nunmai profile delete``) keeps working unchanged; surfaces that can report a partial
    ...
    def __init__(self, profile: str, path: Path):
        ...
        self.path = path
        self.retry_command = f"nunmai profile purge-identity {profile}"
        super().__init__(
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.nunmai).\nTo remove everything, use: nunmai uninstall")
    ...
    shutdown_mcp_servers(scope=nunmai_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from nunmai_state_registry import close_all_under as _close_session_dbs_under
    ...
    with contextlib.suppress(Exception):
        from nunmai_logging import release_profile_log_handlers
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``NUNMAI_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # The service name follows get_nunmai_home(): bind the override (the seam a multiplexed
    ...
    from nunmai_constants import reset_nunmai_home_override, set_nunmai_home_override
    old_home = os.environ.get("NUNMAI_HOME")
    home_token = set_nunmai_home_override(str(profile_dir))
    try:
        os.environ["NUNMAI_HOME"] = str(profile_dir)
        from nunmai_cli.gateway import get_service_name, get_launchd_plist_path, user_systemd_unit_dir
        ...
        os.environ.pop("NUNMAI_HOME", None)
        if old_home is not None:
            os.environ["NUNMAI_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's nunmai_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded NUNMAI_HOME "
...
def get_active_profile(root: Path | None = None) -> str:
    """Read the sticky active profile name (of *root*, default: this process's Nunmai root)."""
...
def get_active_profile_name() -> str:
    """Profile name inferred from NUNMAI_HOME: ``"default"`` when unset or ``~/.nunmai``, the
    name under ``~/.nunmai/profiles/<name>``, ``"custom"`` for any other path."""
    from nunmai_constants import get_nunmai_home
    resolved = get_nunmai_home().resolve()
    if resolved == _get_default_nunmai_home().resolve():
        return "default"
...
def current_profile_name(default: str | None = None) -> str | None:
    """Identity of the profile the current task runs FOR: the ``NUNMAI_HOME`` override when one is
    bound (a multiplexed cron tick, a routed gateway turn), else a launcher-pinned
    ``NUNMAI_PROFILE_NAME``/``NUNMAI_PROFILE`` (the kanban dispatcher pins it on its workers), else
    the name derived from the process's ``NUNMAI_HOME``; *default* when nothing names a profile.
    ...
    if get_nunmai_home_override() is None:
        for env_name in ("NUNMAI_PROFILE_NAME", "NUNMAI_PROFILE"):
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when NUNMAI_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_nunmai_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point NUNMAI_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/nunmai-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".nunmai-profile-exports", Path(tempfile.gettempdir()) / f"nunmai-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    NUNMAI_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Nunmai file the user may have lying alongside NUNMAI_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``nunmai
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.nunmai (dir name ".nunmai"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nunmai profile import <archive> --name <name>"
        )
    ...
    # target ~/.nunmai itself.
    canon = _canon_valid(inferred_name)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nunmai). "
            "Specify a different name: nunmai profile import <archive> --name <name>"
        )
    profile_dir = get_profile_dir(canon)
    if profile_dir.exists():
        raise _profile_exists_error(canon)
    _get_profiles_root().mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="nunmai_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"nunmai_{old_name}"
    legacy_old_host = f"nunmai.{old_name}"
    new_host = f"nunmai_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_nunmai_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``nunmai_<old>`` or legacy ``nunmai.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_nunmai_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from nunmai_constants import nunmai_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=nunmai_home_key(old_dir))
    ...
    from nunmai_cli.profile_identity import _migrate_profile_identity
...
def profile_root_for_env_home(env_home: str, default_root: Path) -> Path:
    """Nunmai root named by an exported ``NUNMAI_HOME``: the grandparent of a profile-shaped value
    (``<root>/profiles/<name>``, mirrors ``get_default_nunmai_root()``), the value itself otherwise,
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NUNMAI_HOME path string. Called early in the CLI entry
    point, before nunmai modules are imported, to set NUNMAI_HOME.

    When NUNMAI_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Nunmai-owned PYTHONPATH
    ...
    root = profile_root_for_env_home(os.environ.get("NUNMAI_HOME", ""), _get_default_nunmai_home())
    if canon == "default":
        return str(root)
