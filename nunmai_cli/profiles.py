...
Each profile is a fully independent NUNMAI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nunmai/profiles/<name>/`` by default.

The "default" profile is ``~/.nunmai`` itself — backward compatible,
zero migration needed.

Usage::

    nunmai profile create coder          # fresh profile + bundled skills
    nunmai profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nunmai profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nunmai -p coder chat                 # or via flag
    nunmai profile use coder             # set as sticky default
    nunmai profile delete coder          # remove profile + alias + service
"""
...
from nunmai_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See nunmai_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.nunmai``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   nunmai-engine  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `nunmai backup` archives
...
# Marker file written by `nunmai profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nunmai
...
# `nunmai skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Nunmai infrastructure directories that only the default profile
         (``~/.nunmai``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_nunmai_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.nunmai) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nunmai-engine",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when NUNMAI_HOME equals the
...
# into the export. The set below identifies the *known Nunmai profile
# artifacts* at the root of NUNMAI_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``nunmai_constants``.
...
_RESERVED_NAMES = frozenset({
    "nunmai", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nunmai root, NOT to the current NUNMAI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NUNMAI_HOME points outside
    ``~/.nunmai``, profiles live under ``NUNMAI_HOME/profiles/`` so
...
def _get_default_nunmai_home() -> Path:
    """Return the default (pre-profile) NUNMAI_HOME path.

    In standard deployments this is ``~/.nunmai``.
    In Docker/custom deployments where NUNMAI_HOME is outside ``~/.nunmai``
    (e.g. ``/opt/data``), returns NUNMAI_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``nunmai``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``nunmai`` profile inside ``~/.nunmai/``) or get refused
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Nunmai home
    (:func:`nunmai_constants.get_nunmai_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, nunmai subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Nunmai. If we
    ...
    try:
        ...
        # not be migrated. The next `nunmai doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``nunmai -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``nunmai profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``nunmai -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nunmai profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        # Creation must not fail over this; `nunmai model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``NUNMAI_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Nunmai config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, nunmai_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own NUNMAI_HOME. When
    ...
    The returned ``nunmai_home`` is the path to pass to
    ``set_nunmai_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_nunmai_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nunmai update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nunmai)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nunmai/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `nunmai -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `nunmai update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nunmai profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Nunmai (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nunmai profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nunmai -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NUNMAI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nunmai profile create
    ...
    only the essential skills (e.g. ``nunmai-engine``), reporting
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Nunmai *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Nunmai backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``NUNMAI_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `nunmai -p <canon> profile
    ...
    nunmai_markers = ("nunmai_cli.main", "nunmai-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('nunmai')` resolution) spawns the
    ...
    # interpreter, not "nunmai", so the checks below would otherwise miss it.
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Nunmai process: either an entrypoint marker in argv, a
            # resolved executable named `nunmai`, or a python interpreter
            # directly exec'ing a `nunmai`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_nunmai = (
                any(marker in joined for marker in nunmai_markers)
                or exe_name == "nunmai"
                or exe_name.startswith("nunmai")
            )
            if not is_nunmai and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: nunmai, nunmai-engine,
                # nunmai-acp) rather than a bare `startswith("nunmai")` --
                ...
                # "nunmai-notes.py" or "nunmai-unrelated-tool" as the shim,
                ...
                is_nunmai = script_stem in _NUNMAI_CONSOLE_SCRIPT_NAMES
            if not is_nunmai:
                continue
            ...
            # ...or by NUNMAI_HOME env pointing at this profile dir.
            if not bound:
                try:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nunmai).\n"
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_nunmai_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NUNMAI_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from nunmai_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NUNMAI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NUNMAI_HOME")
    try:
        os.environ["NUNMAI_HOME"] = str(profile_dir)
        from nunmai_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "NUNMAI_HOME" in os.environ:
            del os.environ["NUNMAI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nunmai/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nunmai profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NUNMAI_HOME.

    Returns ``"default"`` if NUNMAI_HOME is not set or points to ``~/.nunmai``.
    Returns the profile name if NUNMAI_HOME points into ``~/.nunmai/profiles/<name>``.
    Returns ``"custom"`` if NUNMAI_HOME is set to an unrecognized path.
    """
    from nunmai_constants import get_nunmai_home
    nunmai_home = get_nunmai_home()
    resolved = nunmai_home.resolve()

    default_resolved = _get_default_nunmai_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      NUNMAI_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Nunmai file the
      user may have lying alongside NUNMAI_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``nunmai sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``NUNMAI_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nunmai itself — its parent is ~/ and its
        # directory name is ".nunmai", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.nunmai itself — disallow
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_nunmai_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("nunmai_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NUNMAI_HOME path string.

    Called early in the CLI entry point, before any nunmai modules
    are imported, to set the NUNMAI_HOME environment variable.

    When NUNMAI_HOME is already set, the configured spelling IS the
    ...
    it to match Nunmai-owned PYTHONPATH entries written in the same
