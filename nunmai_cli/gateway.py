...
import asyncio
import contextlib
from nunmai_cli.cli_output import line_input  # noqa: F401 — resolved lazily by siblings through the facade
import json
import logging
import os
import shutil
...
from nunmai_cli import setup_platforms  # noqa: F401 — resolved lazily by siblings through the facade
...
from nunmai_cli.config import (  # noqa: F401 — resolved lazily by siblings through the facade
    get_env_value,
    get_nunmai_home,
    is_managed,
    managed_error,
...
from nunmai_cli.setup import (  # noqa: F401 — resolved lazily by siblings through the facade
    print_header,
    print_info,
    print_success,
    print_warning,
...
from nunmai_cli.colors import Colors, color  # noqa: F401 — resolved lazily by siblings through the facade
from nunmai_cli.gateway_service_unit import (  # noqa: F401 — resolved lazily by siblings through the facade
    _systemd_env_line,
    _installed_unit_ld_library_path,
    _ld_library_path_line,
    _nunmai_home_for_target_user,
...
def _scan_gateway_pids(
    ...
    from gateway.status import (
        ...
        profile_flag_value,
        nunmai_home_assignments,
        command_line_names_nunmai_home,
    )
    current_home = str(get_nunmai_home().resolve())
    # Forward slashes on both sides of the NUNMAI_HOME= match (mirrors gateway.status), and no
    # trailing separator: the assignments parser strips one, so the systemd ``Environment=``
    # spelling (``NUNMAI_HOME=/root/.nunmai/``) compares equal to the resolved home.
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            ...
            if profile_flag_value(command_lc) == current_profile_name_lc:
                return True
            return command_line_names_nunmai_home(command_lc, current_home_lc)
        ...
        # fallback stop could SIGTERM the named gateway). NUNMAI_HOME may come via env (invisible to
        # wmic/CIM), so only a non-matching explicit NUNMAI_HOME= disqualifies.
        if profile_flag_value(command_lc) is not None:
            return False
        return (not nunmai_home_assignments(command_lc)
                or command_line_names_nunmai_home(command_lc, current_home_lc))
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    """Profile gateways supervised by real, Nunmai-owned Windows services. Service-logon processes may
    hide their command lines, so identity = Nunmai's own PID file + a parent chain ending at a running
    SCM service PID whose name or binary path is Nunmai's (``gateway_windows.nunmai_owns_windows_service``).
    ...
    try:
        ...
        from nunmai_cli.gateway_windows import nunmai_owns_windows_service, nunmai_service_roots

        nunmai_roots = nunmai_service_roots()
        ...
        for service in psutil_module.win_service_iter():
            try:
                ...
                # PID nor its status may steer the pause. Only Nunmai-owned services reach the guards below.
                # The name alone settles Nunmai-named services; binpath (QueryServiceConfig) is asked only
                # for the rest, and a service that refuses even that to this user is one this user could
                # not `sc stop` either — never Nunmai's, never a reason to abort the enumeration.
                owned = nunmai_owns_windows_service(service_name, "", nunmai_roots)
                if not owned:
                    try:
                        service_binpath = str(_scm_service_field(service, "binpath") or "")
                    except (psutil_module.AccessDenied, OSError):
                        continue
                    owned = nunmai_owns_windows_service(service_name, service_binpath, nunmai_roots)
                if not owned:
                    continue
...
def _nunmai_home_pinned_by_unit(unit_path: Path) -> str | None:
    """``NUNMAI_HOME`` pinned by the unit file at *unit_path*, or None when absent/unreadable."""
    return _unit_environment_value(unit_path, "NUNMAI_HOME")
...
def _nunmai_home_from_systemd_unit_file(system: bool = False) -> str | None:
    ...
    return _nunmai_home_pinned_by_unit(get_systemd_unit_path(system=system))
        ...
        if active_state == "active" and new_pid and (previous_pid is None or new_pid != previous_pid):
            ...
            if gateway_state in ("running", "degraded"):
                ...
                if gateway_state == "degraded":
                    ...
                    print(f"⚠ {scope_label} gateway is DEGRADED — see `nunmai gateway status`")
                return True
...
def _print_multiplex_standalone_reason() -> None:
    ...
    from nunmai_cli.gateway_multiplex_mode import recorded_standalone_warning_lines
...
def _print_served_ingress_urls(profile: str | None = None) -> None:
    ...
    try:
        from nunmai_cli.gateway_multiplex_served import format_ingress_url_lines, served_profile_ingress_urls
...
def _print_unserved_shared_ingress(profile: str | None) -> None:
    ...
    try:
        from nunmai_cli.gateway_multiplex_served import served_profile_unserved_platforms
...
def _print_duplicate_credential_warnings() -> None:
    ...
    with contextlib.suppress(Exception):
        from nunmai_cli.gateway_migrate import duplicate_credential_findings
...
def _native_service_homes() -> set[Path]:
    ...
    from nunmai_constants import _get_platform_default_nunmai_home, sudo_invoker_default_home

    homes = {_get_platform_default_nunmai_home().resolve()}
...
def _bare_unit_pinned_home() -> Path | None:
    """Resolved ``NUNMAI_HOME`` pinned by an installed ``nunmai-gateway.service``, or None. The unit is the
    ...
    if not is_linux() or os.geteuid() != 0:  # windows-footgun: ok — behind is_linux()
        return None
    pinned = _nunmai_home_pinned_by_unit(_SYSTEM_UNIT_DIR / f"{_SERVICE_BASE}.service")
    if not pinned:
        return None
    try:
...
def _profile_suffix() -> str:
    """Service-name suffix for NUNMAI_HOME: "" for a home that owns the bare name, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path.

    Bare-name owners: this process's platform-native default (``~/.nunmai``), under sudo the invoking
    user's native default, and the home pinned by an installed ``nunmai-gateway.service``. Under sudo the
    naming basis moves MID-COMMAND — sudo strips NUNMAI_HOME and sets HOME=/root, then
    ``_sync_nunmai_home_from_systemd_unit()`` adopts the unit's own NUNMAI_HOME into ``os.environ`` — so a
    basis derived from the process alone names one unit before the adoption and another after it. The
    unit-pinned check must precede the profile branch: ``sudo nunmai gateway install --system`` resolves
    ...
    The bare name is deliberately NOT tied to ``get_default_nunmai_root()``: that helper treats any
    NUNMAI_HOME outside ``~/.nunmai`` (Docker ``/opt/data``, a temp dir) as "the root itself", which let a
    temp-home harness resolve to the default profile's ``nunmai-gateway`` unit and uninstall the
...
def _run_systemctl(args: list[str], *, system: bool = False, **kwargs) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        from nunmai_cli.gateway_command_errors import SystemctlUnavailableError
...
def refuses_container_user_scope_install(system: bool) -> bool:
    ...
    _print_info_lines(
        ...
        "",
        "  nunmai gateway run                                # run as the container's main process",
        ...
        "  sudo nunmai gateway install --system --run-as-user <user>",
    )
    return True
...
def legacy_launchd_labels_for_install(exclude=()) -> list[str]:
    ...
    A unit whose label predates the profile-name suffix scheme (``ai.nunmai.gateway-<8hex>`` from the
    ...
    pinned ``NUNMAI_HOME`` is this install's root or one of its ``profiles/<name>`` homes — ownership
    judged from the plist's content, never from label shape or directory membership — so the
    derivation's boundary holds: a sandboxed NUNMAI_HOME (tests, side-by-side installs) never
    ...
    try:
        home = Path(pwd.getpwuid(os.getuid()).pw_dir)  # windows-footgun: ok — POSIX launchd (macOS) helper, never invoked on Windows
        root = get_default_nunmai_root().resolve()
    except Exception:
        return []
    ...
    labels: set[str] = set()
    for plist_path in sorted(agents_dir.glob("ai.nunmai.gateway*.plist")):
        try:
            data = plistlib.loads(plist_path.read_bytes())
            label = data["Label"]
            pinned = Path(str(data["EnvironmentVariables"]["NUNMAI_HOME"])).expanduser().resolve()
        ...
        if not isinstance(label, str) or label in excluded or not label.startswith("ai.nunmai.gateway"):
            continue
...
def _systemd_graceful_restart_action(system: bool, pid: int) -> str | None:
    ...
    from nunmai_cli.update_cmd_drain_report import drain_progress_reporter
...
from nunmai_cli.gateway_launchd import (  # noqa: E402,F401 — facade re-exports; tests patch here
    get_launchd_label,
...
def _served_by_another_host_gateway(profile_name: str | None = None):
    ...
    you" would make `nunmai gateway restart` impossible for the profile that launched the host
    ...
    if gateway is None:
        return None
    try:
        from gateway.status import _get_process_nunmai_home, _same_nunmai_home
        if _same_nunmai_home(gateway.home, _get_process_nunmai_home()):
            return None
    except Exception:
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from nunmai_cli.gateway_multiplex_served import live_default_gateway_pid, recorded_served_profiles
        ...
        # default profile (`nunmai -p X` loads X's .env) and a config edit after start is not live yet.
        ...
        from nunmai_cli.gateway_multiplex_mode import explicit_multiplex_flag
...
def _served_profile_needs_no_service() -> bool:
    ...
    Shared by ``nunmai setup gateway`` / ``nunmai setup`` / ``nunmai import`` (``ensure_gateway_service``)
    and the ``nunmai gateway setup`` wizard. See #111958."""
    if not named_profile_served_by_running_multiplexer():
        ...
        # as `gateway install`, so `nunmai -p X setup` cannot grow a fleet member the verb refuses.
        return _named_profile_refused_under_multiplexer()
    from nunmai_cli.profiles import profile_is_standalone
    if profile_is_standalone(get_nunmai_home()):
...
def _named_profile_refused_under_multiplexer(force: bool = False) -> bool:
    ...
    ``--force`` is the one escape (a fleet split across UNIX users or a ``NUNMAI_HOME`` outside
    ...
    try:
        ...
        standalone = (profile_name_for_home(get_nunmai_home()) not in (None, "default")
                      and profile_is_standalone(get_nunmai_home()))
        ...
        new_standalone = (profile_name_for_home(get_nunmai_home()) not in (None, "default")
    ...
    if served:
        ...
        print()
        print(f"    nunmai -p {owner.profile_label if owner is not None else 'default'} gateway restart")
    else:
        ...
        print()
        print("    nunmai gateway migrate --multiplex")
    print()
    print("  A separate per-profile gateway (for a fleet split across UNIX users or a")
    print(f"  NUNMAI_HOME outside profiles/) needs --force:  nunmai -p {suffix} gateway install --force")
    print()
    from nunmai_constants import display_nunmai_home
    from nunmai_cli.gateway_multiplex_mode import STANDALONE_DEPRECATION_NOTICE
    print("  Temporary compatibility path while multiplexing gaps are closed: set")
    print(f"  gateway.standalone: true in {display_nunmai_home(get_nunmai_home())}/config.yaml,")
...
def _attach_to_host_gateway_or_guard(force: bool = False, replace: bool = False) -> None:
    ...
    try:
        ...
        decision = decide(get_nunmai_home(), replace=replace)
    except Exception:
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    print("  Check with `nunmai gateway status`.")
    print("  To restart it: `nunmai gateway restart`. To stop it: `nunmai gateway stop`.")
    print("  To replace it from here: `nunmai gateway run --replace`.")
    sys.exit(1)
...
from nunmai_cli.gateway_setup_wizard import (  # noqa: E402,F401 — facade re-exports; tests patch here
    _PLATFORMS,
    _all_platforms,
    _platform_status,
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        if isinstance(work, list) and work:
            from nunmai_cli.update_cmd_drain_report import describe_active_work_unit
...
def gateway_command(args):
    ...
    try:
        ...
        from nunmai_cli.gateway_command_errors import explain_service_failure
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    if not _dispatch_via_service_manager_if_s6("start"):
        return False
    # This process never reaches a GatewayRunner, so the watchdog armed by nunmai_cli.main's argv
    ...
    from nunmai_startup_watchdog import disarm_startup_watchdog
...
_NO_BACKEND_MESSAGES = {
    ...
    ("uninstall", "unsupported"): (1,
        ...
        "Stop a manually started gateway with: nunmai gateway stop"),
    ("start", "termux"): (1,
    ...
    ("start", "unsupported"): (1,
        ...
        "Run it directly with: nunmai gateway run"),
}
...
def _host_multiplexer_is_ours(owner) -> bool:
    try:
        from gateway.status import _get_process_nunmai_home, _same_nunmai_home
        return bool(_same_nunmai_home(owner.home, _get_process_nunmai_home()))
    except Exception:
        return False
...
def _print_unfolded_gateway_note(owner) -> None:
    ...
    print("  They were left running; fold them in with: nunmai gateway migrate --multiplex")


def _cmd_start(args):
    from nunmai_cli.gateway_profile_lifecycle import profile_lifecycle
...
def _cmd_stop(args):
    ...
    if not stop_all and not find_gateway_pids() and (
        ...
        print(f"    nunmai {owner_flag}gateway stop      # takes every served profile offline")
        print(f"    nunmai {owner_flag}gateway restart")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _restart_all(system: bool) -> None:
    ...
    if owner is not None and not _host_multiplexer_is_ours(owner):
        ...
        print()
        print(f"    nunmai -p {owner.profile_label} gateway restart --all")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _cmd_restart(args):
    ...
    from gateway.status import get_running_pid
    from nunmai_cli.gateway_supervised_restart import (
...
def _cmd_status(args):
    from nunmai_cli.gateway_profile_lifecycle import print_parked_status
    ...
    from nunmai_cli.profiles import get_active_profile_name, profile_is_standalone
...
def _print_standalone_by_config() -> None:
    ...
    from nunmai_cli.profiles import get_active_profile_name, profiles_to_serve
...
def _cmd_migrate(args):
    from nunmai_cli.gateway_migrate import cmd_migrate
    cmd_migrate(args)
