...
from nunmai_cli import setup_platforms
...
# display_nunmai_home is imported lazily: nunmai_constants may be a cached pre-update version.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``nunmai-gateway*`` /
    ``ai.nunmai.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.nunmai.gateway*`` LaunchAgent — the
    ...
    the ``nunmai-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "nunmai-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.nunmai.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``nunmai-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``nunmai-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.nunmai.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.nunmai.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `nunmai update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `nunmai update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``nunmai`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``nunmai gateway status`` from falsely counting the
    ``nunmai`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``nunmai gateway
    ...
    # Forward slashes on both sides of the NUNMAI_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # Default profile: accept unless argv advertises another profile. NUNMAI_HOME may come via
        # env (invisible to wmic/CIM), so only a non-matching explicit NUNMAI_HOME= disqualifies.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        return not ("nunmai_home=" in command_lc and f"nunmai_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``nunmai update`` hung exactly there on
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``nunmai update``)."""
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Nunmai's own PID file + a parent chain ending at a running SCM service
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``nunmai update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from nunmai_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (NUNMAI_HOME,
    ...
    if sys.platform == "win32":
        try:
            from nunmai_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_nunmai_home()) / "logs"
        ...
        # NUNMAI_HOME) the windowless base interpreter needs to import nunmai_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _nunmai_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``NUNMAI_HOME`` from the on-disk unit file — what refresh/compare already read, and reliable under ``sudo``."""
...
def _sync_nunmai_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``NUNMAI_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_nunmai_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    ...
    if not unit_home:
        ...
        unit_home = _parse_kv_pairs(env_line.split()).get("NUNMAI_HOME", "").strip()
    if unit_home and os.environ.get("NUNMAI_HOME", "").strip() != unit_home:
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}nunmai gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}nunmai gateway restart{scope_flag}")
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``nunmai gateway status``."""
    try:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from nunmai_cli.gateway_windows import get_task_name  # profile-aware task name
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.nunmai.gateway* fleet — not just the current profile's label — or a sibling
...
def _gw_windows():
    """Lazily import :mod:`nunmai_cli.gateway_windows` (Windows-only service backend)."""
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("NUNMAI_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``NUNMAI_GATEWAY_DETACHED=1``
...
def _profile_suffix() -> str:
    """Service-name suffix for NUNMAI_HOME: "" for the platform-native default home (``~/.nunmai``), the
    profile name for ``<root>/profiles/<name>``, else a short hash of the path.

    The bare name is reserved for the NATIVE default, not ``get_default_nunmai_root()``: that helper
    treats any NUNMAI_HOME outside ``~/.nunmai`` (Docker ``/opt/data``, a temp dir) as "the root itself",
    which let a temp-home harness resolve to the default profile's ``nunmai-gateway`` unit and uninstall
    the production gateway. Service names are host-wide identities; only the real default home owns the
    bare one."""
    import hashlib
    from nunmai_constants import _get_platform_default_nunmai_home, get_default_nunmai_root
    home = get_nunmai_home().resolve()
    if home == _get_platform_default_nunmai_home().resolve():
        return ""
    name = _profile_name_from_home(home, get_default_nunmai_root().resolve())
...
def _current_profile_name() -> str:
    ...
    from nunmai_constants import profile_name_for_home
    return profile_name_for_home(get_nunmai_home()) or _profile_suffix()


def _profile_arg(nunmai_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *nunmai_home*/*default_root* let a
...
def get_service_name() -> str:
    """Systemd service name: ``nunmai-gateway`` for default NUNMAI_HOME, ``nunmai-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
def _find_legacy_nunmai_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``nunmai.service``), which
    ...
    Detects unit files installed by older Nunmai versions that used a different service name (e.g. When both
    a legacy unit and the current ``nunmai-gateway.service`` are active, they fight over the same bot token
...
def remove_legacy_nunmai_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``nunmai setup``/``import``).
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.nunmai.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed NUNMAI_HOME never
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # Update-boundary: a gateway restarted mid-update can hold a stale nunmai_constants
            # without this symbol; see _reload_nunmai_constants() in nunmai_cli/managed_uv.py.
...
def _nunmai_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NUNMAI_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.nunmai[/profiles/x]`` → ``/home/alice/.nunmai[/profiles/x]``; custom paths kept as-is."""
    current_nunmai_raw = os.environ.get("NUNMAI_HOME", "").strip()
    current_nunmai = Path(current_nunmai_raw).expanduser() if current_nunmai_raw else get_nunmai_home()
    # Keep paths lexical: resolving a non-existent path can bake a different NUNMAI_HOME into the unit.
    current_default = Path.home() / ".nunmai"
    target_default = Path(target_home_dir) / ".nunmai"
    try:
        # Default ~/.nunmai or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_nunmai.relative_to(current_default))
    except ValueError:
        return str(current_nunmai)  # Completely custom path (not under ~/.nunmai) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    extras = (project_root / "node_modules" / ".bin", nunmai_home / "node" / "bin", nunmai_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (NUNMAI_HOME, else PROJECT_ROOT). cwd is
...
def _append_node_dir_for_service(path_entries: list[str], nunmai_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<nunmai_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from nunmai_constants import (nunmai_managed_node_tree_present, iter_nunmai_node_dirs)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Profile arg relative to the TARGET user's ~/.nunmai when nunmai_home lives under it.
        target_root = Path(home_dir) / ".nunmai"
        try:
            Path(nunmai_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(nunmai_home, default_root=target_root)
        except ValueError:
        ...
        _append_node_dir_for_service(_target_node_entries, Path(nunmai_home) if nunmai_home else None)
    ...
    watchdog_seconds = _systemd_watchdog_seconds(nunmai_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m nunmai_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="NUNMAI_HOME={nunmai_home}"
Environment="NUNMAI_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__NUNMAI_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # NUNMAI_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.nunmai, so refresh would rewrite a correct unit and status warn forever.
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir NUNMAI_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    ...
    candidates += re.findall(r"<key>NUNMAI_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    ...
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: NUNMAI_HOME resolves to a temporary directory ({temp_home}).")
    print(
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the NUNMAI_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp NUNMAI_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/nunmai_test"', "/nunmai_test/")):
        return False

    # Structural variant: refuse ANY temp-dir NUNMAI_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Nunmai install")
    return True
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("NUNMAI_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "NUNMAI_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin NUNMAI_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_nunmai_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}nunmai gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}nunmai gateway status{scope_flag}             # Check status")
...
def _systemd_unit_belongs_to_current_home(system: bool = False) -> bool:
    """False (with a warning) when the installed unit pins a NUNMAI_HOME other than this process's: the
    service name then resolved to ANOTHER install's gateway, and stop/disable/unlink would take it down."""
    _sync_nunmai_home_from_systemd_unit(system=system)  # sudo strips NUNMAI_HOME; adopt the unit's first
    unit_home = _nunmai_home_from_systemd_unit_file(system=system)
    if unit_home is None or Path(unit_home).expanduser().resolve() == get_nunmai_home().resolve():
        return True
    print_warning(
        f"Refusing to remove {get_systemd_unit_path(system=system)}: it runs NUNMAI_HOME={unit_home}, "
        f"but this process has NUNMAI_HOME={get_nunmai_home()}"
    )
    return False
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}nunmai gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # NUNMAI_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_restart(system: bool = False):
    ...
    # NUNMAI_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `nunmai
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}nunmai gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}nunmai gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}nunmai gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}nunmai gateway start{scope_flag}")
...
# at all and we degrade to a detached background process (the `nohup nunmai gateway run` workaround). See
# #23387.
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m nunmai_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "nunmai_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``nunmai update`` hands back to
    ...
    ``--external-supervisor`` so ``nunmai update`` sees the flag on the live grandchild argv and hands the
    ...
    return [get_python_path(), "-m", "nunmai_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup nunmai gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup nunmai gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
<dict>
    ...
    <dict>
        ...
        <key>NUNMAI_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Nunmai install")
    return True
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `nunmai gateway start` re-bootstraps.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    NUNMAI_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `nunmai update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Nunmai PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/nunmai" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 NUNMAI_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other NUNMAI_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's NUNMAI_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (nunmai_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("nunmai_home", "<unknown>"),
                )
        except Exception:
            pass
        return
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("NUNMAI_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Nunmai gateway as root inside the official Docker image.")
    print(
    ...
    print("  Set NUNMAI_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from nunmai_startup_watchdog import (
        ...
        try:
            from nunmai_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. NUNMAI_GATEWAY_EXIT_DIAG=0 opts out."""
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; NUNMAI_GATEWAY_MAX_STARTS / NUNMAI_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["NUNMAI_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["NUNMAI_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from nunmai_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (NUNMAI_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `nunmai gateway restart`.
    if supports_systemd_services():
        try:
...
_PLATFORMS = [
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Nunmai delivers cron results and notifications."},
...
def _prompt_unauthorized_access(*, is_email: bool) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: nunmai pairing approve <platform> <code>")
    elif is_email:
        print_success("  Unknown email senders will be ignored.")
    else:
        print_info("  Skipped — configure later with 'nunmai gateway setup'")
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Nunmai will open Tencent iLink QR login in this terminal.",
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `nunmai pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the nunmai_cli.setup cycle."""
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: nunmai gateway run",
        "  For persistence:   tmux new -s nunmai 'nunmai gateway run'",
        "  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'",
    ),
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: nunmai gateway run",
        "  Or start it manually in the background (best effort): nohup nunmai gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: nunmai gateway run",
    ),
}
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo nunmai gateway install --system")
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``NUNMAI_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    NUNMAI_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("NUNMAI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # NUNMAI_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("NUNMAI_S6_SUPERVISED_CHILD"):
        return False
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-nunmai
        # service's pattern in docker/s6-rc.d/main-nunmai/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `nunmai gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: nunmai gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  nunmai profile create <name>     # creates the s6 service slot",
        "  nunmai -p <name> gateway start   # bring it up via s6",
        "  nunmai status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: nunmai gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: nunmai gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: nunmai gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  nunmai profile delete <name>     # tears down the s6 service slot",
        "  nunmai -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: nunmai gateway run"),
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `nunmai gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    nunmai gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: nunmai gateway start",
        )
        sys.exit(1)
...
# ``nunmai gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  nunmai gateway install"),
    "other": (
        "To install as a service:", "  nunmai gateway install", "  sudo nunmai gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    ...
    "windows": ("  nunmai gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  nunmai gateway install  # Install as user service",
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: nunmai gateway status")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Nunmai gateway unit files (e.g. nunmai.service)."""
