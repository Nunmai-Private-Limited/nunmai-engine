"""``nunmai sessions repair-profiles`` — the CLI face of :mod:`nunmai_cli.sessions_repair_profiles`.
...
from nunmai_cli.sessions_repair_profiles import (
...
def cmd_repair_profiles(args) -> int:
    ...
    if apply:
        ...
        if live:
            ...
            print(f"A gateway is running for: {names}. It holds the routing index in memory and would "
                  "write it back over this repair. Stop it (`nunmai gateway stop`), then re-run --apply.",
                  file=sys.stderr)
            return 1
    ...
    try:
        ...
        if not getattr(args, "yes", False) and not as_json:
            from nunmai_cli.sessions_cmd import _confirm_prompt
