...
Split out of :mod:`nunmai_cli.plugins`. Names that tests patch on the origin
...
from nunmai_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from nunmai_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from nunmai_cli.plugins_manifest import (
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``nunmai_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from nunmai_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    # which is the key `nunmai plugins enable/disable` and the dashboard write (#27548); the manifest name
    ...
    if _origin._env_enabled("NUNMAI_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".nunmai" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set NUNMAI_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def resolve_manifest_winners(manifests: List[PluginManifest]) -> Dict[str, PluginManifest]:
    ...
    ``~/.nunmai/plugins/<name>`` is the documented way to override a bundled plugin, and is logged. A flat
    ...
    ``nunmai plugins enable kanban`` never activates unrelated code under the bundled name."""
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        ...
        return _placeholder(error, logging.WARNING, "Refusing to load removed Nunmai Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `nunmai`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `nunmai plugins enable {lookup_key}` to activate)", logging.DEBUG,
    ...
    if manifest.source != "bundled":
        ...
        from nunmai_cli.plugins_cmd_catalog import installed_plugin_removal
        removed = installed_plugin_removal(manifest.name, manifest.path)
        if removed is not None:
            error = f"removed from the Nunmai plugin catalog: {removed.reason or 'no reason recorded'}"
            return _placeholder(error, logging.WARNING, "Refusing to load plugin '%s' — %s; run `nunmai plugins remove`",
                                error)
