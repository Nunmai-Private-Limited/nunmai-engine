...
The Nunmai docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Nunmai Portal). This module fetches, validates, and caches that manifest,
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.nunmai/cache/model_catalog.json``.
...
def get_curated_nous_models() -> list[str] | None:
    """Return Nunmai Portal's curated list of model ids from the manifest.
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Nunmai silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`nunmai update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``nunmai update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.nunmai/cache/model_catalog.json`` via the same atomic writer the
