"""NUNMAI_HOME state checks for nunmai doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from nunmai_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(nunmai_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (nunmai_home if nunmai_home is not None else NUNMAI_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None, host_note: str = "") -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'nunmai sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'nunmai sessions optimize-storage' offline (with the host gateway stopped) to compact FTS storage"
...
def _memory_store_flags(nunmai_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(nunmai_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """NUNMAI_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from nunmai_cli.doctor import NUNMAI_HOME, _DHH
    nunmai_home = NUNMAI_HOME
    ensure_dir(f, should_fix, nunmai_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(nunmai_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, nunmai_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    _check_scratch_dir(nunmai_home, _DHH)
...
def unpruned_cache_hogs(nunmai_home: Path, min_bytes: int = _UNPRUNED_CACHE_WARN_BYTES) -> list[tuple[str, int]]:
    ...
    from nunmai_constants import scratch_dir_usage_bytes

    cache = nunmai_home / "cache"
...
def _check_scratch_dir(nunmai_home: Path, _DHH: str) -> None:
    ...
    scratch = get_scratch_dir(nunmai_home, prune=False)
    ...
    for name, nbytes in unpruned_cache_hogs(nunmai_home):
        check_warn(
    ...
    if tmpdir and tmpdir != os.environ.get(SCRATCH_DIR_MARKER_ENV, ""):
        check_info(f"TMPDIR={tmpdir} is set by you or the OS, so Nunmai leaves it alone")
...
def _write_health_reason(state_db_path: Path, *, should_fix: bool):
    ...
    from nunmai_state_repair import _db_opens_cleanly, _live_writer_holds_db
    ...
    if not should_fix and state_db_path.stat().st_size > _WRITE_PROBE_SNAPSHOT_MAX_BYTES:
        check_info("state.db write-health probe skipped: store is held by a live writer and larger than 1 GB "
                   "(run 'nunmai doctor --fix' to probe it)")
        return None
    import sqlite3
    import tempfile
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'nunmai doctor --fix' (or 'nunmai sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'nunmai doctor --fix' (or 'nunmai sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'nunmai {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'nunmai {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    from nunmai_state_repair import repair_state_db_schema
...
def _report_structural_damage(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, reason) -> bool:
    ...
    from nunmai_state_repair import state_db_has_structural_damage
...
def _classify_unreadable_state_db(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, exc: Exception) -> None:
    ...
    from nunmai_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from nunmai_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'nunmai sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            from nunmai_state_repair import _exclusive_repair_db_guard, _live_writer_holds_db
            ...
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "run 'nunmai doctor --fix' to checkpoint)")
            ...
            if not should_fix:
                return f.issues.append(
                    "Large WAL file — stop the profile's gateway, then run 'nunmai doctor --fix' to checkpoint")
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               f"({guard_error}; stop the profile's gateway and re-run 'nunmai doctor --fix')")
                    return f.issues.append(_SKIP)
...
def _retired_wal_holders(f: Finding, state_db_path: Path, _DHH: str) -> bool:
    ...
    from nunmai_constants import profile_cli_selector
    from nunmai_state_dbfile import iter_deleted_sqlite_sidecar_holders
    from nunmai_state_holders import describe_holder_pid
    ...
    f.issues.append(f"state.db retired WAL generation held by {rendered}{host_gateway_note()} — stop the host "
                    f"gateway, dashboard and cron writers among them ('nunmai {profile_cli_selector()}gateway "
                    "stop' stops the ONE host process serving every profile, quit the Desktop app), do not "
                    "delete the WAL yourself, then rerun 'nunmai doctor'")
    return True
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    ...
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: nunmai skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: nunmai memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: nunmai memory setup",
                        "No Honcho API key — run 'nunmai memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run nunmai memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: nunmai memory status")
    else:
        check_warn(f"{name} plugin not found", "run: nunmai memory setup")
...
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    ...
    name = _doctor_memory_config(NUNMAI_HOME).get("provider", "")
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"nunmai -p (\S+)", wrapper.read_text(encoding="utf-8"))
                if _m and not profile_exists(_m.group(1)):
                    check_warn(f"Orphan alias: {wrapper.name} → profile '{_m.group(1)}' no longer exists")
    # Same helper as the multiplex migration preflight, so doctor names the duplicates that make
    # `nunmai gateway migrate --multiplex` refuse (and made pre-multiplex standalone gateways race).
