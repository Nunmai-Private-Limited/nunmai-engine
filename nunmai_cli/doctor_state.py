...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'nunmai sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from nunmai_state_repair import _db_opens_cleanly, state_db_has_structural_damage
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run 'nunmai doctor --fix' to checkpoint")
            ...
            from nunmai_state_holders import live_writer_holds_db
            from nunmai_state_repair import _connect_repair_durable
