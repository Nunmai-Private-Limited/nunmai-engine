"""Credential sections of `nunmai status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``nunmai_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "nunmai model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "nunmai auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "nunmai auth add xai-oauth", _FILE_REFRESH_ROWS))
...
# Nunmai Tool Gateway per-feature state: first matching (predicate(feature, nous_auth), text(feature)).
_FEATURE_STATES = (
    (lambda f, _: f.managed_by_nous, lambda f: "active via Nunmai subscription"),
...
def _render_auth_providers(ctx):
    ...
    import nunmai_cli.auth as auth
    try:
        # Read-only display: the refresh-free snapshot, so `nunmai status` never performs an OAuth
    ...
    if nous_status.get("free_tier"):
        ...
        _status._row("Nunmai Portal", True, f"{FREE_TIER_LABEL} · {GUEST_MODEL}")
    ...
    _status._row("Nunmai Portal", logged_in,
         "logged in" if logged_in else "not logged in (Nunmai inference key configured)" if inference
         else "not logged in (run: nunmai portal)")
...
def _render_nous_gateway(ctx):
    if managed_nous_tools_enabled():
        ...
        _status._section("Nunmai Tool Gateway")
        print("  Nunmai Portal   ✓ managed tools available" if features.nous_auth_present
              else "  Nunmai Portal   ✗ not logged in")
        ...
        # Nunmai OAuth without entitlement, or an opaque inference key without Portal account
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: nunmai model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from nunmai_cli.models_local import probe_lmstudio_models
...
import nunmai_cli.status as _status  # noqa: E402  (bottom: nunmai_cli.status imports this module)
