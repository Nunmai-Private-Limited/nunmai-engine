"""What Nunmai does when an existing ``config.yaml`` cannot be read or parsed.
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "Nunmai is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "Nunmai is running on your last good settings until it is fixed, so recent changes are not applied.",
    "refuse-write": "Nothing was written, so the existing file is preserved."}
_PARSE_FAILURE_DEFAULTS_MSG = (
    "Nunmai is running on default settings until it is fixed, so none of your saved settings are applied.")
_PARSE_FAILURE_REPAIR_MSG = "Open it with `nunmai config edit`, fix {where}, then run `nunmai config check`."
_FIX_PERMS = "Fix the file permissions or move it aside first."
_FIX_YAML = (
    "Fix it with `nunmai config edit` and check with `nunmai config check`, or copy the newest good "
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what Nunmai is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    in ``nunmai_cli.config._load_config_impl``).
    """
    try:
        st = config_path.stat()
...
def _read_error_fix(exc: OSError) -> str:
    return _FIX_PERMS if isinstance(exc, PermissionError) else (
        "Try again; run `nunmai config check` if it keeps failing.")
...
def _backups_dir_display() -> str:
    ...
    return f"{display_nunmai_home()}/backups/config/"
