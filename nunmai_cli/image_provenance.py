"""Image-authored deployment provenance for immutable Nunmai runtimes.

The published image bakes ``/etc/nunmai/image-provenance.json`` outside both
``$NUNMAI_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/nunmai/image-provenance.json")
