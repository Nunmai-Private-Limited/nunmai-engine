...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("NUNMAI_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``nunmai gateway run --replace`` / ``gateway restart`` invoked from a shell
...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    ``nunmai kanban repair`` can pick its exit code. ``OperationalError``
...
def _open_configured(path: Path, under_lock) -> tuple[sqlite3.Connection, Any]:
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # DELETE with one ERROR log (see nunmai_state_wal._WAL_INCOMPAT_MARKERS).
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    :func:`kanban_db_path` (``NUNMAI_KANBAN_DB`` -> ``NUNMAI_KANBAN_BOARD`` ->
...
# harnesses seeding a board with a reduced schema have omitted. Nunmai's own
