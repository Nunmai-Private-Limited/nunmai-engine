...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["nunmai-acp"]) if n]
...
class SessionManager:
    ...
    def _get_db(self):
        ...
        minimal test env). ``NUNMAI_HOME`` is resolved here, not via the import-time
        ``DEFAULT_DB_PATH``, so test fixtures that change the env var later are honoured."""
        if self._db_instance is None:
            try:
                from nunmai_state import SessionDB
                self._db_instance = SessionDB(db_path=get_nunmai_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["nunmai-acp"], mcp_server_names=configured_mcp_servers),
