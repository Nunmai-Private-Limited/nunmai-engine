...
from nunmai_constants import get_nunmai_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when Nunmai runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (["nunmai-acp"] if toolsets is None else toolsets) if n]
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.nunmai/state.db``."""
    ...
    def _get_db(self):
        ...
        error in a minimal test env). ``NUNMAI_HOME`` is resolved here, not via the import-time
        ...
        if self._db_instance is None:
            try:
                ...
                self._db_instance = acquire(get_nunmai_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        from nunmai_cli.tools_config import _get_platform_tools, enabled_mcp_server_names
        ...
        if enabled_toolsets is None:
            ...
            # nunmai-acp; its MCP half (every enabled server, a listed-name allowlist, or none for ``no_mcp``)
        ...
        # picked up by NunmaiACPAgent._schedule_mcp_late_refresh.
        try:
        ...
        try:
            ...
            # The bare-AIAgent fallback dies with "No LLM provider configured. Run `nunmai setup`" on a
