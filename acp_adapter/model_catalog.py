"""ACP model picker: deduplicated ``provider:model`` rows from the Nunmai inventory + named endpoints."""
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        from nunmai_cli.config import (get_compatible_custom_providers, is_provider_enabled, load_config)
        from nunmai_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
        from nunmai_cli.model_switch_providers import _NativePickerModelList, _fetch_picker_live_models
        from nunmai_cli.model_switch_providers import _discover_flag
