...
# Maps ACP permission option ids to Nunmai approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    # Smart DENY override — the editor must not offer a scope Nunmai
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Nunmai semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_nunmai(outcome: object, *, allowed_option_ids: set[str]) -> str:
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_NUNMAI.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Nunmai-compatible approval callback that bridges to ACP.
