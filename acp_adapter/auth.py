"""ACP auth helpers — detect and advertise Nunmai authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "nunmai-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Nunmai runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Nunmai.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Nunmai credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Nunmai using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Nunmai provider", type="terminal", args=["--setup"],
        description=("Open Nunmai' interactive model/provider setup in a terminal. "
                     "Use this when Nunmai has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Nunmai can resolve any runtime provider credentials."""
