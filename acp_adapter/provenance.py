...
This is an additive Nunmai extension surfaced under ACP ``_meta.nunmai`` so
...
compression rotates the internal Nunmai head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_nunmai_session_id: The live internal Nunmai DB session id
            (``state.agent.session_id``).
        previous_nunmai_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"nunmai": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_nunmai_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_nunmai_session_id
    compression_depth = 0
    ...
    rotated = bool(
        ...
        and previous_nunmai_session_id != current_nunmai_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentNunmaiSessionId": current_nunmai_session_id,
        "rootNunmaiSessionId": root_id,
        "parentNunmaiSessionId": parent_id,
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"nunmai": {"sessionProvenance": prov}}
