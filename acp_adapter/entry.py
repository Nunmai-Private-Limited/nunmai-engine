"""CLI entry point for the nunmai-engine ACP adapter.

Loads ``~/.nunmai/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: nunmai acp / nunmai-acp
"""
...
try:
    ...
    # Partial ``nunmai update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Nunmai modules.
    nunmai_bootstrap.harden_import_path()

# `nunmai-acp` runs without nunmai_cli.main: repair a `nunmai update` killed mid-pull here, before
# importing anything else from the checkout (a no-op under `nunmai acp`, which already did).
...
def _load_env() -> None:
    """Load .env from NUNMAI_HOME (default ``~/.nunmai``)."""
    from nunmai_cli.env_loader import load_nunmai_dotenv

    nunmai_home = get_nunmai_home()
    loaded = load_nunmai_dotenv(nunmai_home=nunmai_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", nunmai_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="nunmai-acp", description="Run Nunmai Engine as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Nunmai version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Nunmai provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.nunmai/node/ "
...
def _print_version() -> None:
    ...
    print(nunmai_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import NunmaiACPAgent  # noqa: F401

    print("Nunmai ACP check OK")


def _run_setup() -> None:
    from nunmai_cli.main import main as nunmai_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "nunmai", "model"]
        nunmai_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting nunmai-engine ACP adapter")
    ...
    import acp
    from .server import NunmaiACPAgent

    # Windows: import the configured memory provider (and numpy) on the main thread before
    # the MCP-discovery and ACP stdin-reader threads start (nunmai_cli's ~150 ms
    ...
    if os.environ.get("NUNMAI_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
