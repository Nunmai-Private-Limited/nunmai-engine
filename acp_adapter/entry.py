...
Loads environment variables from ``~/.nunmai/.env``, configures logging
...
try:
    ...
    # shadowing Nunmai's own modules — ``nunmai acp`` can be started from any
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nunmai_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="nunmai-acp",
        description="Run Nunmai Engine as an ACP stdio server.",
    )
