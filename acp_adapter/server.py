...
from acp_adapter.commands import NUNMAI_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_nunmai_interactive_context, set_nunmai_interactive_context
...
class NunmaiACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Nunmai inventory (same substrate
        as ``nunmai model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``nunmai_cli.model_switch.switch_model`` seeded with the live
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_nunmai_session_id: str, previous_nunmai_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.nunmai.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_nunmai_session_id,
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_nunmai_session_id: Optional[str] = None, previous_nunmai_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_nunmai_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_nunmai_session_id or session_id, previous_nunmai_session_id
            ),
        )
        await self._send(
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``NUNMAI_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # NUNMAI_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Nunmai workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global NUNMAI_CRON_SESSION.
            ...
            stack.callback(reset_nunmai_interactive_context, set_nunmai_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "NUNMAI_SESSION_ID", os.environ.get("NUNMAI_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_nunmai_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_nunmai_id: Any,
        ...
        try:
            ...
            if conn and post_turn_nunmai_id and pre_turn_nunmai_id and post_turn_nunmai_id != pre_turn_nunmai_id:
                try:
                    await self._send_session_info_update(
                        session_id, current_nunmai_session_id=post_turn_nunmai_id,
    ...
    async def set_session_model(self, model_id: str, session_id: str, **kwargs: Any) -> SetSessionModelResponse | None:
        ...
        try:
            ...
            try:
                ...
                # internal error (-32603): the client attributes it to the request, not to Nunmai (#72439).
