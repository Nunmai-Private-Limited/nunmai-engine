"""ACP prompt content blocks -> Nunmai/OpenAI user-content payloads (text, images, resources)."""
...
def _resource_link_to_parts(block: ResourceContentBlock) -> list[dict[str, Any]]:
    ...
    if path is None:
        return _text_parts(
            **ident, body="[Resource link only; Nunmai cannot read non-file ACP resource URIs directly.]"
        )
